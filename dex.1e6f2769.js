(()=>{function Se(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ee(t=!1){let s=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}function se(t){let e=null,s=t.onClose||null,n=t.onOpen||null;function o(){if(e){e.classList.add("open"),window.addEventListener("resize",c),n&&setTimeout(n,10);return}let i=document.getElementById("windows-container");i||(i=document.createElement("div"),i.id="windows-container",i.className="windows-container",document.body.appendChild(i)),e=document.createElement("div"),e.id=t.id,e.className="window";let r=t.icon||"bx-window",g="",p="",f;t.tabs&&t.tabs.length>0?(g=`<div class="tab-bar">${t.tabs.map((x,w)=>{let y;return x.icon?y=`<i class="bx ${x.icon}"></i>`:x.title&&x.title.length>0?y=`<span class="tab-glyph">${x.title.charAt(0).toUpperCase()}</span>`:y='<i class="bx bx-question-mark"></i>',`
                ...
                `}).join("")}</div>`,f=`<div class="window-content">${t.tabs.map((x,w)=>`<div class="tab-content ${w===0?"active":""}" data-tab-content="${w}">${x.content}</div>`).join("")}</div>`):(t.title&&(p=`<div class="window-title">${t.title}</div>`),f=`<div class="window-content">${t.content}</div>`);let a=`
...
`,z=null,J=new Set,Ne=[];async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ve();let s=localStorage.getItem("service_map");if(!s){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(s).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let c=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let v=await fetch(c);if(!v.ok)throw new Error("Service is offline or unreachable.");let u=(await v.json()).events||[];if(Ne=u,z=Date.now(),T(3,z),u.length===0){e.innerHTML=$("empty","No events found.");return}t&&(e.innerHTML="");let h=f=>{let a=f.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let d=a.type,m=d==="engagement.decision"||d==="messaging.bot.sent_message"||d==="messaging.user.sent_message"||d==="moderation.explicit_content.deleted"||d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.cli.command",b="event-border-grey";m&&(d==="moderation.explicit_content.deleted"?b="event-border-red":d==="analysis.link.completed"||d==="analysis.visual.completed"?b="event-border-purple":d==="system.cli.command"?b="event-border-orange":b="event-border-blue");let x=m?"cursor-pointer":"",w=J.has(f.id),y=w?"expanded":"",C=w?"display: block;":"display: none;",S=new Date(f.timestamp*1e3),N=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Q=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),U=Ce(d,a),Z="";if(m){let _="";if(d==="engagement.decision")_=`
            ...
            `,m){let _=L.querySelector(".close-details-btn");_&&_.addEventListener("click",M=>{M.stopPropagation();let k=M.target.closest(".event-item");if(k){k.classList.remove("expanded"),J.delete(f.id);let E=k.querySelector(".event-details");E&&(E.style.display="none")}})}return L},i=Array.from(e.children),r=new Map(i.map(f=>[f.dataset.eventId,f])),g=new Set(u.map(f=>f.id));i.forEach(f=>{let a=f.dataset.eventId;(!a||!g.has(a))&&f.remove()});let p=null;u.forEach((f,a)=>{let d=r.get(f.id);(!d||t)&&(d&&d.remove(),d=h(f),!d)||(a===0?e.firstElementChild!==d&&e.prepend(d):p&&p.nextElementSibling!==d&&p.after(d),p=d)}),z=Date.now(),T(3,z)}catch(v){console.error("Error fetching events:",v),e.children.length===0&&(e.innerHTML=$("offline","Failed to load events.","The event service may be offline or unreachable."))}}function Ve(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ne.forEach(s=>J.add(s.id)),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{J.clear(),ne(!0)},e.dataset.listenerAttached="true")}var Be=()=>`
    <div class="notifications-actions">
...
`,G=null,q=new Set,le=[];async function P(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Ke(),t&&(e.innerHTML="<p>Updating...</p>");let s=localStorage.getItem("service_map");if(!s){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(s).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let c=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/events?ml=1000&format=json&event.type=system.notification.generated,system.analysis.audit`;try{let v=await fetch(c);if(!v.ok)throw new Error("Service is offline or unreachable.");let u=(await v.json()).events||[];G=Date.now(),T(0,G);let h=Date.now(),i=24*60*60*1e3,r=u.filter(m=>{let b=localStorage.getItem(`notification_read_ts_${m.id}`);if(!b)return!0;let x=parseInt(b);return h-x<i});if(le=r,t&&(e.innerHTML=""),r.length===0){e.innerHTML=$("empty","No notifications yet."),oe();return}let g=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let w=b.type==="system.analysis.audit",y=w?`Analysis Audit: ${b.tier?.toUpperCase()}`:b.title||"Untitled Notification",C=w?"Raw analyst input and output logs.":b.body||"No description provided.",S=w?"low":b.priority||"low",N=!w&&!!b.alert,Q=w?"audit":b.category||"system",U=b.related_event_ids||[],L=!!localStorage.getItem(`notification_read_ts_${m.id}`),_=new Date(m.timestamp*1e3),M=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=L?"event-border-grey":"event-border-blue";w&&(E=L?"event-border-grey":"event-border-purple"),!L&&N&&(E="event-border-red"),L&&(S==="high"||S==="critical")?E="event-border-red":L&&S==="medium"&&(E="event-border-orange");let j=L?"notification-read":"notification-unread",D=q.has(m.id),ze=D?"expanded":"",Je=D?"display: block;":"display: none;",me="";if(w)me=`
                    <div class="event-detail-row">
                ...
                `;else{let R="";U.length>0&&(R=`
                    ...
                    ${R}
                `}let B=document.createElement("div");B.className=`event-item notification-item ${E} ${j} ${ze} cursor-pointer`,B.dataset.notificationId=m.id,B.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),q.delete(m.id);let te=this.querySelector(".event-details");te&&(te.style.display="none")}else{this.classList.add("expanded"),q.add(m.id);let te=this.querySelector(".event-details");if(te&&(te.style.display="block"),!localStorage.getItem(`notification_read_ts_${m.id}`)){localStorage.setItem(`notification_read_ts_${m.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ue="event-border-grey";S==="high"||S==="critical"?ue="event-border-red":S==="medium"&&(ue="event-border-orange"),this.classList.add(ue),oe()}}},B.innerHTML=`
                <div class="event-time">
...
`,re=null,Y=new Set,Ue=[];async function V(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Xe();let s=localStorage.getItem("service_map");if(!s){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(s).services?.cs||[]).find(u=>u.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let o=n.domain==="0.0.0.0"?"127.0.0.1":n.domain,c=`http://${o}:${n.port}/events?ml=100&format=json&event.type=system.blueprint.generated`,v=`http://${o}:${n.port}/analyst/status`;try{let l=await fetch(v);if(l.ok){let u=await l.json(),h=document.getElementById("strategist-next-run");if(h&&u.strategist){let i=u.strategist.next_run,r=Math.floor(Date.now()/1e3),g=i-r;if(g<=0)h.textContent="Next T3: Ready (IDLE req)",h.style.color="#5eff5e";else{let p=Math.floor(g/60),f=g%60;h.textContent=`Next T3: ${p}m ${f}s`,h.style.color="#888"}}}}catch(l){console.error("Failed to fetch analyst status",l)}try{let l=await fetch(c);if(!l.ok)throw new Error("Service is offline or unreachable.");let h=(await l.json()).events||[];if(Ue=h,re=Date.now(),T(1,re),h.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),O(1,0);return}t&&(e.innerHTML="");let i=a=>{let d=a.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let m=d.title||"Untitled Blueprint",b=d.summary||d.body||"No summary provided.",x=d.content||"",w=d.category||"architecture",y=d.affected_services||[],C=d.implementation_path||[],S=new Date(a.timestamp*1e3),N=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Q=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),U=Y.has(a.id),Z=U?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${U?"expanded":""}`,L.dataset.blueprintId=a.id,L.onclick=function(E){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(a.id);let D=this.querySelector(".event-details");D&&(D.style.display="none")}else{this.classList.add("expanded"),Y.add(a.id);let D=this.querySelector(".event-details");D&&(D.style.display="block")}};let _=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",M="";C.length>0&&(M=`
            ...
            `;let k=L.querySelector(".close-details-btn");return k&&k.addEventListener("click",E=>{E.stopPropagation(),L.classList.remove("expanded");let j=L.querySelector(".event-details");j&&(j.style.display="none"),Y.delete(a.id)}),L},r=Array.from(e.children),g=new Map(r.map(a=>[a.dataset.blueprintId,a])),p=new Set(h.map(a=>a.id));r.forEach(a=>{let d=a.dataset.blueprintId;(!d||!p.has(d))&&a.remove()});let f=null;h.forEach((a,d)=>{let m=g.get(a.id);(!m||t)&&(m&&m.remove(),m=i(a),!m)||(d===0?e.firstElementChild!==m&&e.prepend(m):f&&f.nextElementSibling!==m&&f.after(m),f=m)}),O(1,h.length)}catch(l){console.error("Error fetching blueprints:",l),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Xe(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-reset-strategist");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ue.forEach(n=>Y.add(n.id)),V(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),V(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let n=localStorage.getItem("service_map");if(!n)return;let c=(JSON.parse(n).services?.cs||[]).find(u=>u.id==="dex-event-service");if(!c)return;let l=`http://${c.domain==="0.0.0.0"?"127.0.0.1":c.domain}:${c.port}/analyst/reset?tier=strategist`;s.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await fetch(l,{method:"POST"}),setTimeout(()=>{s.innerHTML="<i class='bx bx-check'></i> Reset Done",setTimeout(()=>{s.innerHTML="<i class='bx bx-refresh'></i> Reset Strategist"},2e3)},500),V()}catch(u){console.error("Failed to reset strategist",u),s.innerHTML="<i class='bx bx-error'></i> Failed"}},s.dataset.listenerAttached="true")}var je=()=>localStorage.getItem("service_map")?'<div id="services-widgets" class="system-monitor-widgets"><p>Loading services...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),Oe=()=>localStorage.getItem("service_map")?'<div id="models-widgets" class="system-monitor-widgets"><p>Loading models...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),qe=()=>localStorage.getItem("service_map")?'<div id="processes-widgets" class="system-monitor-widgets"><p>Loading processes...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),ce=null,de=null,pe=null;async function Pe(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(c=>c.id==="dex-event-service");if(!e)return null;let n=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/system_monitor`,o=await fetch(n);if(!o.ok)throw new Error(`HTTP error! status: ${o.status}`);return await o.json()}catch(t){return console.error("Error fetching system data:",t),null}}async function Qe(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(c=>c.id==="dex-event-service");if(!e)return null;let n=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/processes`,o=await fetch(n);if(!o.ok)throw new Error(`HTTP error! status: ${o.status}`);return await o.json()}catch(t){return console.error("Error fetching process data:",t),null}}async function ye(){let t=document.getElementById("services-widgets");if(!t)return;let e=await Pe();if(!e||!e.services){t.innerHTML=$("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}ce=Date.now(),T(5,ce);let s=e.services||[];Array.from(t.children).forEach(i=>{i.classList.contains("service-widget")||i.remove()});function n(i){return!i||i==="N/A"||i==="unknown"||i.trim()===""?"-":i}function o(i){if(!i||i==="N/A"||i==="unknown")return"-";let r=i.match(/^(\d+\.\d+\.\d+)/);return r?r[0]:i.split(".").slice(0,3).join(".")||"-"}function c(i){return!i||i.length<=28?i:i.substring(0,28)+"..."}function v(i){if(!i||i==="N/A"||i==="unknown")return"-";let r=i.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!r)return"-";let g=parseInt(r[1])||0,p=parseInt(r[2])||0,f=parseInt(r[3])||0,a=parseFloat(r[4])||0,d=g*86400+p*3600+f*60+a,m=Math.floor(d/86400);if(m>0)return`${m}d`;let b=Math.floor(d/3600);if(b>0)return`${b}h`;let x=Math.floor(d/60);return x>0?`${x}m`:`${Math.floor(d)}s`}function l(i){let r=i.status==="online",g=r?"service-widget-online":"service-widget-offline",p=r?"bx-check-circle":"bx-x-circle",f=r?"OK":"BAD",a=i.version?o(i.version.str):"-",d=i.cpu&&i.cpu!=="N/A"?i.cpu:"-",m=i.memory&&i.memory!=="N/A"?i.memory:"-",b=d!=="-"?"#00ff00":"#666",x=d!=="-"?"#fff":"#666",w=m!=="-"?"#00ff00":"#666",y=m!=="-"?"#fff":"#666",C=v(i.uptime),S="";return r?S=`
                ...
                </div>`:S='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${g}" data-service-id="${i.id}"><div class="service-widget-header"><i class="bx ${p}"></i><h3>${i.short_name||i.id}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${c(i.domain&&i.port?`${i.domain}:${i.port}`:"")}</span></div>${S}</div></div>`}let u=new Map(Array.from(t.querySelectorAll(".service-widget")).map(i=>[i.dataset.serviceId,i])),h=new Set(s.map(i=>i.id));for(let[i,r]of u)h.has(i)||r.remove();s.forEach(i=>{let r=l(i),g=u.get(i.id);g?g.outerHTML!==r&&(g.outerHTML=r):t.insertAdjacentHTML("beforeend",r)})}async function xe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Pe();if(!e){t.innerHTML=$("offline","Failed to load model status.");return}de=Date.now(),T(4,de);let s=e.models||[],n=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function o(l){let u=l.status==="Ready";return`
                ...
                </div>`}function c(l){let u=l.status==="Downloaded",h=u?"service-widget-online":"service-widget-offline",i=u?"OK":"MISSING",r=u&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",g=l.name;return l.type==="custom"&&g.endsWith(":latest")&&(g=g.replace(":latest","")),`<div class="service-widget ${h}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${u?"bx-check-circle":"bx-x-circle"}"></i><h3>${g}</h3><span class="service-widget-status">${i}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${r}</span></div></div></div>`}let v="";if(n&&(v+=o(n)),v+=s.map(c).join(""),!v){t.innerHTML=$("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function we(){let t=document.getElementById("processes-widgets");if(!t)return;let e=await Qe();if(e===null){t.innerHTML=$("offline","Failed to load process status.");return}if(pe=Date.now(),T(2,pe),e.length===0){t.innerHTML=$("empty","No active processes."),O(2,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function s(c){let v=Math.floor(Date.now()/1e3-c.start_time),l=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"";return`
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}">
                    <div class="service-widget-header">
                        <i class="bx bx-loader-alt bx-spin"></i>
                        <h3>Channel ${c.channel_id}</h3>
                        ${l}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
...
`,F=new Set,Re=[],X=null;async function H(t=!1){let e=document.getElementById("roadmap-list");if(!e)return;Ze();let s=localStorage.getItem("service_map");if(!s)return;let n=null;try{n=(JSON.parse(s).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{return}if(!n)return;let c=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/roadmap`;try{let v=await fetch(c);if(!v.ok)throw new Error("Offline");let l=await v.json();if(Re=l,l.length===0){e.innerHTML=$("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let u=p=>{let f=F.has(p.id),a=p.state==="draft",d=p.state==="published",m=p.state==="consumed",b="event-border-grey";d&&(b="event-border-blue"),m&&(b="event-border-purple");let w=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${b} cursor-pointer ${f?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=S=>{if(S.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",F.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",F.add(p.id))};let C=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;return y.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${I(p.content)}</div>

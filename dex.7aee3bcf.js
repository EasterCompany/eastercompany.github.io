...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Lt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),N(0,ce);let c=Date.now(),b=24*60*60*1e3,g=r.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let x=parseInt(f);return c-x<b});g.sort((d,f)=>{let x=_=>{let k=_.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},$=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=$(x(d)),L=$(x(f));return E!==L?L-E:f.timestamp-d.timestamp}),de=g;let v=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},w=[],p=new Set(g.map(d=>v(d))).size>1;if(g.length>0){let d=null;g.forEach(f=>{let x=v(f);p&&x!==d&&(w.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),w.push(f)})}if(t&&(e.innerHTML=""),g.length===0){e.innerHTML=S("empty","No alerts yet."),le();return}let n=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=f.title||"Untitled Alert",$=f.body||"No description provided.",E=f.priority||"low",L=!!f.alert,_=f.category||"system",k=f.related_event_ids||[],D=!!localStorage.getItem(`alert_read_ts_${d.id}`),O=new Date(d.timestamp*1e3),B=O.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=O.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=D?"event-border-grey":"event-border-blue";!D&&L&&(T="event-border-red"),D&&(E==="high"||E==="critical")?T="event-border-red":D&&E==="medium"&&(T="event-border-orange");let R=D?"alert-read":"alert-unread",H=q.has(d.id),be=H?"expanded":"",he=H?"display: block;":"display: none;",Te="",ke="";k.length>0&&(ke=`
...
`,G=new Set,je=[],J=null;async function W(t=!1){let e=document.getElementById("roadmap-list");if(e){$t();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(je=o,o.length===0){e.innerHTML=S("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=v=>{let w=G.has(v.id),u=v.state==="draft",p=v.state==="published",n=v.state==="consumed",i="event-border-grey";p&&(i="event-border-blue"),n&&(i="event-border-purple");let h=new Date(v.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${i} cursor-pointer ${w?"expanded":""}`,y.dataset.itemId=v.id,y.onclick=d=>{if(d.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(v.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",G.add(v.id))};let m=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${v.state.toUpperCase()}]</span>`;return y.innerHTML=`
      ...
      `,y.querySelector(".edit-btn")?.addEventListener("click",()=>Ct(v)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Tt(v)),y.querySelector(".delete-btn")?.addEventListener("click",()=>kt(v.id)),y.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(v.id)}),y},r=Array.from(e.children),c=new Map(r.map(v=>[v.dataset.itemId,v])),b=new Set(o.map(v=>v.id));r.forEach(v=>{let w=v.dataset.itemId;(!w||!b.has(w))&&v.remove()});let g=null;o.forEach((v,w)=>{let u=c.get(v.id);(!u||t)&&(u&&u.remove(),u=s(v),!u)||(w===0?e.firstElementChild!==u&&e.prepend(u):g&&g.nextElementSibling!==u&&g.after(u),g=u)})}catch{e.children.length===0&&(e.innerHTML=S("offline","Failed to load roadmap.","The event service may be offline."))}}}function $t(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{J=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",J=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=J?`/roadmap/${J}`:"/roadmap",b=J?"PATCH":"POST";try{await A(c,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",W(!0)}catch(g){console.error(g)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{je.forEach(r=>G.add(r.id)),W(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{G.clear(),W(!0)},s.dataset.listenerAttached="true")}function Ct(t){J=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Tt(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),W(!0)}catch(a){console.error(a)}}async function kt(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),G.delete(t),W(!0)}catch(e){console.error(e)}}var We=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ge=null,Y=new Set,Ke=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;It();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(Ke=r,Ge=Date.now(),N(2,Ge),r.length===0){e.innerHTML=S("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),U(1,0);return}t&&(e.innerHTML="");let c=u=>{let p=u.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let n=p.title||"Untitled Blueprint",i=p.summary||p.body||"No summary provided.",l=p.content||"",h=p.category||"architecture",y=p.affected_services||[],m=p.implementation_path||[],d=new Date(u.timestamp*1e3),f=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Y.has(u.id),E=$?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,L.dataset.blueprintId=u.id;let k={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";L.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),Y.add(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let M=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",D="";m.length>0&&(D=`
            ...
            `;let O=L.querySelector(".close-details-btn");return O&&O.addEventListener("click",B=>{B.stopPropagation(),L.classList.remove("expanded");let C=L.querySelector(".event-details");C&&(C.style.display="none"),Y.delete(u.id)}),L},b=Array.from(e.children),g=new Map(b.map(u=>[u.dataset.blueprintId,u])),v=new Set(r.map(u=>u.id));b.forEach(u=>{let p=u.dataset.blueprintId;(!p||!v.has(p))&&u.remove()});let w=null;r.forEach((u,p)=>{let n=g.get(u.id);(!n||t)&&(n&&n.remove(),n=c(u),!n)||(p===0?e.firstElementChild!==n&&e.prepend(n):w&&w.nextElementSibling!==n&&w.after(n),w=n)}),U(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=S("offline","Failed to load blueprints.","The event service may be offline."))}}function It(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ke.forEach(a=>Y.add(a.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),me(!0)},e.dataset.listenerAttached="true")}var Je=()=>`
...
`;async function Ee(){await Promise.all([W(),me()])}var Xe=()=>`
    <div class="alerts-actions">
...
`,Ye=null;async function pe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>pe(),e.dataset.listenerAttached="true");try{let a=await Ue("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ye=Date.now(),N(4,Ye),s.length===0){t.innerHTML=S("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((c,b)=>{let g=r[c.level]??10,v=r[b.level]??10;return g!==v?g-v:c.username.localeCompare(b.username)}),t.innerHTML=s.map(c=>{let b=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",g=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",v="#888";c.level==="Me"||c.level==="Master User"?v="#bb86fc":c.level==="Admin"?v="#cf6679":c.level==="Moderator"?v="#03dac6":c.level==="Contributor"&&(v="#ffa500");let w=c.level==="Me",u=w?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",p=w?"2px solid #bb86fc":`1px solid ${b}33`;return`
...
`);return`
        ...
        <div id="processes-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,rt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${tt()}
        </div>`;async function pt(){await Promise.all([ve(),Se(),Le()]),await ne()}var st=null,nt=null,it=null;async function ut(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function at(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Nt(){try{return await(await A("/processes")).json()}catch{return null}}async function Rt(){try{return await(await A("/analyst/status")).json()}catch{return null}}async function Se(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=S("offline","Failed to load hardware info.","The event service may be offline."));return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let h=0,y=0;n.STORAGE.forEach(x=>{h+=x.USED,y+=x.SIZE});let m=(h/(1024*1024*1024)).toFixed(1),d=(y/(1024*1024*1024)).toFixed(1),f=y>0?(h/y*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=i};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let i=await at();i?(o(i),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.children.length===0&&(e.innerHTML=S("offline","Failed to refresh.","The event service may be offline.")),a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let i=await at();o(i)}}if(!t)return;let s=await ut();if(!s||!s.services){t.children.length===0&&(t.innerHTML=S("offline","Failed to load system metrics.","The event service may be offline."));return}st=Date.now(),N(3,st);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function g(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function v(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,h=parseInt(i[2])||0,y=parseInt(i[3])||0,m=parseFloat(i[4])||0,d=l*86400+h*3600+y*60+m,f=Math.floor(d/86400);if(f>0)return`${f}d`;let x=Math.floor(d/3600);if(x>0)return`${x}h`;let $=Math.floor(d/60);return $>0?`${$}m`:`${Math.floor(d)}s`}function w(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",h=i?"bx-check-circle":"bx-x-circle",y=i?"OK":"BAD",m=n.version?b(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",f=n.memory&&n.memory!=="N/A"?n.memory:"-",x=d!=="-"?"#00ff00":"#666",$=d!=="-"?"#fff":"#666",E=f!=="-"?"#00ff00":"#666",L=f!=="-"?"#fff":"#666",_=v(n.uptime),k="";return i?k=`
                ...
                </div>`}function r(b){let g=b.status==="Downloaded",v=g?"service-widget-online":"service-widget-offline",w=g?"OK":"MISSING",u=g&&b.size>0?`${(b.size/1e9).toFixed(2)} GB`:"-",p=b.name;return b.type==="custom"&&p.endsWith(":latest")&&(p=p.replace(":latest","")),`<div class="service-widget ${v}" data-model-name="${b.name}"><div class="service-widget-header"><i class="bx ${g?"bx-check-circle":"bx-x-circle"}"></i><h3>${p}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${b.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${u}</span></div></div></div>`}let c="";if(o&&(c+=s(o)),c+=a.map(r).join(""),!c){t.innerHTML=S("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function ve(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),ve()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await Rt();if(c){let p=Math.floor(Date.now()/1e3),n=c.active_tier,i=(l,h,y)=>{if(n===y||y==="guardian"&&n==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let m=h-p;if(m<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let d=Math.floor(m/60),f=m%60;l.textContent=`${d}m ${f}s`,l.style.color="#fff"}};if(e&&c.guardian&&i(e,c.guardian.next_run,"guardian"),a&&c.architect&&i(a,c.architect.next_run,"architect"),o&&c.strategist&&i(o,c.strategist.next_run,"strategist"),s&&c.system_idle_time!==void 0){let l=c.system_idle_time,h=Math.floor(l/60),y=l%60;s.textContent=`${h}m ${y}s`,l>300?s.style.color="#5eff5e":l>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let b=await Nt(),g=document.getElementById("vitals-processes-val");if(g)if(b){let p=b.length;g.textContent=p>0?`${p} Active`:"Idle",g.style.color=p>0?"#bb86fc":"#fff"}else g.textContent="-",g.style.color="#888";if(b===null){t.children.length===0&&(t.innerHTML=S("offline","Failed to load process status.","The event service may be offline."));return}if(it=Date.now(),N(3,it),b.length===0){t.innerHTML=S("empty","No active processes."),U(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function v(p){let n=Math.floor(Date.now()/1e3-p.start_time),i=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",l=p.channel_id,h={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return h[l]?l=h[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Pt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
        <div class="cli-dashboard">
            ...
            <div class="cli-divider">
                ...
                <span>Interactive Demos (v${Ot})</span>
                <i class='bx bx-chevron-down'></i>
            </div>
    ...
    </div>`,e},Ce=!1;function Ut(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ce=!0,a}function gt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ce=!1}function vt(t,e,a="output"){if(!Ce)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=re(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function zt(t){let e=ft.find(s=>s.id===t);if(!e)return;let a=Ut(e);vt(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),vt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function bt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Pt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;zt(o)})})}async function Ft(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${we()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function ht(){Ft(),Pe(),Ae();let t=window.location.pathname;(t==="/dex"||t==="/dex/"||t.includes("/dex/index.html"))&&!t.includes("/contribute")&&bt();let e=Be();_e(e),Me();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let c=m=>{localStorage.setItem("dex_last_window",m)};function b(){return 1}function g(){for(;s.length>1;)s.shift().close(!0);let m=document.getElementById("windows-container"),d=document.querySelector("nav"),f=document.querySelector("footer"),x=window.location.pathname==="/"||window.location.pathname==="/index.html",$=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");m&&m.setAttribute("data-count",s.length);let E=s.length>0;s.forEach(M=>{let D=document.getElementById(M.id);D&&D.classList.remove("hide-close")}),ie(te());let L=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon");if(s.length>0){if(f?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),m&&(m.style.paddingTop="60px"),L&&(L.style.display="none"),k&&(k.style.display="none"),_){let M=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{c("alerts-window"),v(u)}),document.getElementById("switch-events").addEventListener("click",()=>{c("events-window"),v(p)}),document.getElementById("switch-monitor").addEventListener("click",()=>{c("monitor-window"),v(i)}),document.getElementById("switch-contacts").addEventListener("click",()=>{c("contacts-window"),v(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{c("workspace-window"),v(l)})):_.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),m&&(m.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),x||$?f?.classList.remove("hide"):f?.classList.add("hide"),L&&(L.style.display="flex"),k&&(k.style.display="block"),_&&(_.innerHTML="")}function v(m){if(m.isOpen()){m.close();return}for(;s.length>0;)s.pop().close(!0);s.push(m),m.open(),g()}function w(){[...s].forEach(d=>d.close()),s=[]}window.addEventListener("resize",g);let u=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:ze(),onOpen:()=>j(),onClose:()=>{let m=s.indexOf(u);m>-1&&s.splice(m,1),g()}}),p=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:et(),onOpen:()=>Q(),onClose:()=>{let m=s.indexOf(p);m>-1&&s.splice(m,1),g()}}),n=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Xe(),onOpen:()=>pe(),onClose:()=>{let m=s.indexOf(n);m>-1&&s.splice(m,1),g()}}),i=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:rt()},{icon:"bxs-component",title:"Processes",content:lt()},{icon:"bxs-brain",title:"Models",content:ct()},{icon:"bxs-hdd",title:"Hardware",content:dt()},{icon:"bxs-terminal",title:"Logs",content:mt()},{icon:"bxs-zap",title:"Analyst",content:ot()}],onOpen:()=>{Se(),ve(),Le(),ne()},onClose:()=>{let m=s.indexOf(i);m>-1&&s.splice(m,1),g()}}),l=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Je(),onOpen:()=>Ee(),onClose:()=>{let m=s.indexOf(l);m>-1&&s.splice(m,1),g()}}),h=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:fe(),onOpen:()=>{h.setContent(fe()),$e(h)},onClose:()=>{let m=s.indexOf(h);m>-1&&s.splice(m,1),g()}}),y=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dex/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let m=document.getElementById("dexter-dropdown");m&&(m.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");d&&m&&f&&(d.addEventListener("mouseenter",()=>{m.classList.add("active"),f.classList.add("active")}),d.addEventListener("mouseleave",()=>{m.classList.remove("active"),f.classList.remove("active")}),f.addEventListener("click",x=>{x.stopPropagation();let $=localStorage.getItem("dex_last_window")||"alerts-window",E=null;$==="alerts-window"?E=u:$==="events-window"?E=p:$==="monitor-window"?E=i:$==="contacts-window"?E=n:$==="workspace-window"&&(E=l),E&&v(E)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{c("alerts-window"),v(u)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{c("events-window"),v(p)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{c("monitor-window"),v(i)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{c("contacts-window"),v(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{c("workspace-window"),v(l)}),document.getElementById("settings-icon")?.addEventListener("click",()=>v(h)),document.getElementById("close-all-windows")?.addEventListener("click",()=>w()),setInterval(()=>{u.isOpen()&&j(),p.isOpen()&&Q(),n.isOpen()&&pe(),i.isOpen()&&pt(),l.isOpen()&&Ee()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{y.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",m=>{m.preventDefault();try{Ne(document.getElementById("email-input").value),window.location.reload()}catch(d){let f=document.getElementById("login-error");f&&(f.textContent=d.message,f.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ht):ht();})();

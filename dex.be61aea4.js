(()=>{function Ce(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function _e(t=!1){let a=`
    ...
    `,n=document.createElement("nav");n.innerHTML=a,document.body.prepend(n)}function Ae(){let t=`
...
`,re=null,z=new Set,ce=[];async function F(t=!1){let e=document.getElementById("notifications-list");if(!e)return;vt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await C(a);if(!n.ok)throw new Error("Service is offline or unreachable.");let r=(await n.json()).events||[];re=Date.now(),D(0,re);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let h=parseInt(v);return c-h<g});u.sort((d,v)=>{let h=N=>{let I=N.event;if(typeof I=="string")try{I=JSON.parse(I)}catch{return"low"}return(I.priority||"low").toLowerCase()},_=N=>N==="critical"?4:N==="high"?3:N==="medium"?2:1,$=_(h(d)),E=_(h(v));return $!==E?E-$:v.timestamp-d.timestamp}),ce=u;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},x=[],p=new Set(u.map(d=>f(d))).size>1;if(u.length>0){let d=null;u.forEach(v=>{let h=f(v);p&&h!==d&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),d=h),x.push(v)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=S("empty","No notifications yet."),oe();return}let i=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let h=v.title||"Untitled Notification",_=v.body||"No description provided.",$=v.priority||"low",E=!!v.alert,N=v.category||"system",I=v.related_event_ids||[],O=!!localStorage.getItem(`notification_read_ts_${d.id}`),R=new Date(d.timestamp*1e3),M=R.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=R.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=O?"event-border-grey":"event-border-blue";!O&&E&&(T="event-border-red"),O&&($==="high"||$==="critical")?T="event-border-red":O&&$==="medium"&&(T="event-border-orange");let H=O?"notification-read":"notification-unread",B=z.has(d.id),ue=B?"expanded":"",fe=B?"display: block;":"display: none;",Te="",Se="";I.length>0&&(Se=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${I.map(Q=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${Q.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),Te=`
                <div class="event-detail-row">
...
`,Fe=null,K=new Set,qe=[];async function de(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;wt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let n=await C(a);if(!n.ok)throw new Error("Service is offline or unreachable.");let r=(await n.json()).events||[];if(qe=r,Fe=Date.now(),D(2,Fe),r.length===0){e.innerHTML=S("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),P(1,0);return}t&&(e.innerHTML="");let c=m=>{let p=m.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let i=p.title||"Untitled Blueprint",s=p.summary||p.body||"No summary provided.",l=p.content||"",b=p.category||"architecture",y=p.affected_services||[],w=p.implementation_path||[],d=new Date(m.timestamp*1e3),v=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=K.has(m.id),$=_?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${_?"expanded":""}`,E.dataset.blueprintId=m.id;let I={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[b]||"bx-paint";E.onclick=function(M){if(this.classList.contains("expanded")){this.classList.remove("expanded"),K.delete(m.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),K.add(m.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let Z=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",O="";w.length>0&&(O=`
            ...
            `;let R=E.querySelector(".close-details-btn");return R&&R.addEventListener("click",M=>{M.stopPropagation(),E.classList.remove("expanded");let L=E.querySelector(".event-details");L&&(L.style.display="none"),K.delete(m.id)}),E},g=Array.from(e.children),u=new Map(g.map(m=>[m.dataset.blueprintId,m])),f=new Set(r.map(m=>m.id));g.forEach(m=>{let p=m.dataset.blueprintId;(!p||!f.has(p))&&m.remove()});let x=null;r.forEach((m,p)=>{let i=u.get(m.id);(!i||t)&&(i&&i.remove(),i=c(m),!i)||(p===0?e.firstElementChild!==i&&e.prepend(i):x&&x.nextElementSibling!==i&&x.after(i),x=i)}),P(2,r.length)}catch(n){console.error("Error fetching blueprints:",n),e.children.length===0&&(e.innerHTML=S("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function wt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{qe.forEach(a=>K.add(a.id)),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{K.clear(),de(!0)},e.dataset.listenerAttached="true")}var We=()=>`
...
`;async function he(){await Promise.all([q(),de()])}var Ve=()=>`
    <div class="notifications-actions">
...
`,pe=null,Y=new Set,ie=[],me="all",$t={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},Lt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Ke(t){for(let[e,a]of Object.entries($t))if(a.includes(t))return e;return"system"}function Tt(t){return Lt[t]||"bx-square-rounded"}async function X(t=!1){let e=document.getElementById("events-timeline");if(!e)return;St();let n=`/events?ml=${me==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let o=await C(n);if(!o.ok)throw new Error("Service is offline or unreachable.");let c=(await o.json()).events||[],g=c;if(me!=="all"&&(g=c.filter(i=>{let s=i.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!1}return Ke(s.type)===me})),ie=g.slice(0,50),pe=Date.now(),D(1,pe),ie.length===0){e.innerHTML=S("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=i=>{let s=i.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let l=s.type,b=Ke(l),y=Tt(l),w=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),d="event-border-grey";w&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?d="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?d="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?d="event-border-orange":l==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let v=w?"cursor-pointer":"",h=Y.has(i.id),_=h?"expanded":"",$=h?"display: block;":"display: none;",E=new Date(i.timestamp*1e3),N=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),Z=Je(l,s),O=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",R="";if(w){let L="";if(l==="engagement.decision")L=`
                    ...
                    `;else if(l==="messaging.user.sent_message"){let T="";s.attachments&&s.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(B=>{let ue=B.content_type&&B.content_type.startsWith("image/"),fe=(B.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${B.content_type} \u2022 ${fe}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),L=`
                        <div class="event-detail-row">
...
`);return`
    ...
    `;async function we(){await Promise.all([nt(),At(),It()]),await xe()}var Ze=null,Qe=null,et=null;async function it(){try{return await(await C("/system_monitor")).json()}catch{return null}}async function tt(){try{return await(await C("/system/hardware")).json()}catch{return null}}async function Ct(){try{return await(await C("/processes")).json()}catch{return null}}async function _t(){try{return await(await C("/analyst/status")).json()}catch{return null}}async function At(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),n=i=>{if(!i){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let s="",l=(i.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(s+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`:I='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${i.id}"><div class="service-widget-header"><i class="bx ${b}"></i><h3>${i.short_name||i.id}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${u(i.domain&&i.port?`${i.domain}:${i.port}`:"")}</span></div>${I}</div></div>`}let m=new Map(Array.from(t.querySelectorAll(".service-widget")).map(i=>[i.dataset.serviceId,i])),p=new Set(r.map(i=>i.id));for(let[i,s]of m)p.has(i)||s.remove();r.forEach(i=>{let s=x(i),l=m.get(i.id);l?l.outerHTML!==s&&(l.outerHTML=s):t.insertAdjacentHTML("beforeend",s)})}async function It(){let t=document.getElementById("models-widgets");if(!t)return;let e=await it();if(!e){t.innerHTML=S("offline","Failed to load model status.");return}Qe=Date.now(),D(3,Qe);let a=e.models||[],n=e.whisper;Array.from(t.children).forEach(g=>{g.classList.contains("service-widget")||g.remove()});function o(g){let u=g.status==="Ready";return`

...
`,le=new Set,_e=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ns();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();_e=s;let n=p=>{let x=le.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let E=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${o} cursor-pointer ${x?"expanded":""}`,h.dataset.itemId=p.id,h.onclick=f=>{if(f.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(p.id))};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;h.innerHTML=`
      ...
      `;let d=h.querySelector(".event-details");return d&&d.addEventListener("click",f=>{f.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>as(p)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>os(p)),h.querySelector(".delete-btn")?.addEventListener("click",()=>rs(p.id)),h.querySelector(".close-details-btn")?.addEventListener("click",f=>{f.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)}),h},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),u=new Set(s.map(p=>p.id));if(l.forEach(p=>{let x=p.dataset.itemId;(!x||!u.has(x))&&p.remove()}),!_e||_e.length===0){e.innerHTML=O("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;s.forEach((p,x)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(x===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=O("offline","Failed to load roadmap.","The event service may be offline."))}}}function ns(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input").value;if(!l.trim())return;let r=xe?`/roadmap/${xe}`:"/roadmap",u=xe?"PATCH":"POST";try{await B(r,{method:u,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(c){console.error(c)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{_e.forEach(l=>le.add(l.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function as(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function os(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function rs(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var bt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,gt=null,de=new Set,xt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ls();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let l=(await s.json()).events||[];if(xt=l,gt=Date.now(),X(2,gt),l.length===0){e.innerHTML=O("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),E=(v.content||"").trim(),h=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(D=>D.trim()),d=(v.implementation_path||[]).map(D=>D.trim()),f=v.source_event_ids||[],g=v.approved===!0,w=new Date(m.timestamp*1e3),A=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=de.has(m.id),k=S?"display: block;":"display: none;",y=document.createElement("div");y.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${g?"blueprint-approved":""}`,y.dataset.blueprintId=m.id,g&&(y.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",y.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let _=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";y.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let j=this.querySelector(".event-details");j&&(j.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let j=this.querySelector(".event-details");j&&(j.style.display="block")}};let N="";d.length>0&&(N=`
                ...
                `);let Z=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",F=z(),T=g?`
            ...
            `;let $=y.querySelector(".blueprint-approve-btn");$&&($.onclick=async D=>{D.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(U){console.error("Failed to approve blueprint:",U)}});let C=y.querySelector(".blueprint-delete-btn");C&&(C.onclick=async D=>{D.stopPropagation();let U=!g;C.innerHTML=U?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(j){console.error("Failed to delete blueprint:",j)}});let M=y.querySelector(".event-details");M&&M.addEventListener("click",D=>{D.stopPropagation()});let R=y.querySelector(".close-details-btn");return R&&R.addEventListener("click",D=>{D.stopPropagation(),y.classList.remove("expanded");let U=y.querySelector(".event-details");U&&(U.style.display="none"),de.delete(m.id)}),y},u=Array.from(e.children),c=new Map(u.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));u.forEach(m=>{let v=m.dataset.blueprintId;(!v||!p.has(v))&&m.remove()});let x=null;l.forEach((m,v)=>{let o=c.get(m.id);(!o||t)&&(o&&o.remove(),o=r(m),!o)||(v===0?e.firstElementChild!==o&&e.prepend(o):x&&x.nextElementSibling!==o&&x.after(o),x=o)}),we(2,l.length),dt()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=O("offline","Failed to load blueprints.","The event service may be offline."))}}async function yt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&n++}),ct(n)}catch{}}function ls(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xt.forEach(s=>de.add(s.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var ht=()=>`
...
`;async function Oe(){await Promise.all([ce(),pe()])}var wt=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let l=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,u=document.getElementById("new-chore-owner").value||"313071000877137920";if(l){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:u,natural_instruction:l,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(c){console.error(c),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let r=await(await B("/chores")).json();if(!r||r.length===0){t.innerHTML=O("empty","No active chores.","Create one to start monitoring.");return}let u=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),x=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=u,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${c.dataset.id}`,{method:"DELETE"}),ye())}})}catch(l){console.error("Failed to fetch chores",l)}}var cs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,ds=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);$t(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Le("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(ds=Date.now(),n.length===0){t.innerHTML=O("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,u)=>{let c=l[r.level]??10,p=l[u.level]??10;return c!==p?c-p:r.username.localeCompare(u.username)}),t.innerHTML=n.map(r=>{let u=r.color?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",c=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",p="#888";r.level==="Me"||r.level==="Master"?p="#bb86fc":r.level==="Admin"?p="#cf6679":r.level==="Moderator"?p="#03dac6":r.level==="Contributor"&&(p="#ffa500");let x=r.level==="Me",m=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",v=x?"2px solid #bb86fc":`1px solid ${u}33`;return`
...
`,Be=null,me=new Set,Ce=[],V="all",ms={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},us={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function fs(t){for(let[e,i]of Object.entries(ms))if(i.includes(t))return e;return"system"}function gs(t){return us[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;bs();let s=`/events?ml=${V==="all"?100:250}&format=json`;V!=="all"&&(s+=`&category=${V}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),Be=Date.now(),X(1,Be),Ce.length===0){e.innerHTML=O("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,E=fs(b),h=gs(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let f=a?"cursor-pointer":"",g=me.has(v.id),w=g?"expanded":"",A=g?"display: block;":"display: none;",L=new Date(v.timestamp*1e3),S=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),y=St(b,o),I=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",_="";if(a){let T="";if(b==="engagement.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                    ...
                    `}else if(b==="messaging.bot.sent_message"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C="";o.eval_count&&(C=`
                        ...
                        `);let M="";if(o.chat_history&&o.chat_history.length>0){let R=o.chat_history.length,D=o.chat_history.map((U,j)=>{let J=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&J==="ASSISTANT"&&(J="DEXTER");let G=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",W=j===R-1?"block":"none";return`
                    ...
                    `}else if(b==="system.analysis.audit"){let $=o.success?"#03dac6":"#ff4d4d",C=o.success?"SUCCESS":"FAILED",M=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,R="";o.error&&(R=`
                        ...
                        `);let D="";if(o.chat_history&&o.chat_history.length>0){let J=o.chat_history.length,G=o.chat_history.map((W,ue)=>{let Q=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&Q==="USER"&&(Q="SYSTEM"),!W.name&&Q==="ASSISTANT"&&(Q="AGENT");let K=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${H(W.content)}</div>
                                </div>
                            `}).join("");D=`
                            <div class="event-detail-block">
                    ...
                    `}else if(b==="messaging.user.sent_message"){let $="";o.attachments&&o.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${o.attachments.map(M=>{let R=M.content_type&&M.content_type.startsWith("image/"),D=(M.size/1024).toFixed(1)+" KB";return`
...
`),c=[...r.logs];if(c.length<25){let x=25-c.length;for(let m=0;m<x;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let p=c.map(x=>Me(x)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${kt()}
        </div>`;async function Nt(){await Promise.all([Ee(),Ge(),We(),ye()]),await $e()}var Tt=null,Mt=null;async function Ht(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function Lt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function ys(){try{return await(await B("/processes")).json()}catch{return null}}async function hs(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Ge(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),u=a=>{if(a){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let a=await Lt();a?(u(a),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let a=await Lt();u(a)}if(!t)return;let c=await Ht();if(!c||!c.services){t.children.length===0&&(t.innerHTML=O("offline","Failed to load system metrics.","The event service may be offline."));return}Tt=Date.now(),X(0,Tt);let p=c.services||[];Array.from(t.children).forEach(a=>{a.classList.contains("service-widget")||a.remove()});function x(a){return!a||a==="N/A"||a==="unknown"||a.trim()===""?"-":a}function m(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:a.split(".").slice(0,3).join(".")||"-"}function v(a){return!a||a.length<=28?a:a.substring(0,28)+"..."}function o(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,g=parseInt(d[2])||0,w=parseInt(d[3])||0,A=parseFloat(d[4])||0,L=f*86400+g*3600+w*60+A,S=Math.floor(L/86400);if(S>0)return`${S}d`;let k=Math.floor(L/3600);if(k>0)return`${k}h`;let y=Math.floor(L/60);return y>0?`${y}m`:`${Math.floor(L)}s`}function b(a){let d=a.status==="online",f=d?"service-widget-online":"service-widget-offline",g=d?"bx-check-circle":"bx-x-circle",w=d?"OK":"BAD",A=a.version?m(a.version.str):"-",L=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",S=a.memory&&a.memory!=="N/A"?a.memory:"-",k=L!=="-"?"#00ff00":"#666",y=L!=="-"?"#fff":"#666",I=S!=="-"?"#00ff00":"#666",_=S!=="-"?"#fff":"#666",N=o(a.uptime),q="";d?q=`
                <div class="service-widget-info">
                ...
                </div>`:q='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let Z=z()?"easter.company":v(a.domain&&a.port?`${a.domain}:${a.port}`:"");return`<div class="service-widget ${f}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${Z}</span></div>${q}</div></div>`}let E=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),h=new Set(p.map(a=>a.id));for(let[a,d]of E)h.has(a)||d.remove();p.forEach(a=>{let d=b(a),f=E.get(a.id);f?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function We(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ht();if(!e){t.children.length===0&&(t.innerHTML=O("offline","Failed to load model status.","The event service may be offline."));return}Mt=Date.now(),X(2,Mt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function n(c){let p=c.status==="Ready";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],ws=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    </div>`,e},Ze=!1;function Cs(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ze=!0,i}function Pt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ze=!1}function Rt(t,e,i="output"){if(!Ze)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Me(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function $s(t){let e=Ot.find(n=>n.id===t);if(!e)return;let i=Cs(e);Rt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(r=>setTimeout(r,400+Math.random()*600));let l="output";n.includes("[ERROR]")?l="error":n.includes("[SUCCESS]")||n.includes("\u2713")?l="success":n.includes("!")&&(l="warning"),Rt(i,n,l)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ut(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=ws();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;$s(s)})})}async function Es(){if(!z())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function jt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}et(),tt(),Es(),ot(),Qe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Ut();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=a=>{localStorage.setItem("dex_last_window",a)};function n(){for(;e.length>1;)e.shift().close(!0);let a=document.getElementById("windows-container"),d=document.querySelector("nav"),f=document.querySelector("footer"),g=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",e.length),e.forEach(I=>{let _=document.getElementById(I.id);_&&_.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon"),k=document.getElementById("close-all-windows"),y=window.innerWidth<880;if(!y){let I=document.getElementById("dexter-dropdown"),_=document.getElementById("dexter-menu-btn");I&&I.classList.remove("active"),_&&_.classList.remove("active"),a&&a.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(f?.classList.add("hide"),k&&(k.style.display=y?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),d?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),A&&(A.style.display=y?"flex":"none"),S&&(S.style.display=y?"block":"none"),!y&&L){let I=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(I)?(L.innerHTML=`

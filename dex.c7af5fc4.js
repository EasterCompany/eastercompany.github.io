...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Et(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),H(0,ce);let c=Date.now(),b=24*60*60*1e3,f=r.filter(d=>{let g=localStorage.getItem(`notification_read_ts_${d.id}`);if(!g)return!0;let h=parseInt(g);return c-h<b});f.sort((d,g)=>{let h=_=>{let k=_.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},L=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=L(h(d)),S=L(h(g));return E!==S?S-E:g.timestamp-d.timestamp}),de=f;let v=d=>{let g=d.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},w=[],p=new Set(f.map(d=>v(d))).size>1;if(f.length>0){let d=null;f.forEach(g=>{let h=v(g);p&&h!==d&&(w.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),d=h),w.push(g)})}if(t&&(e.innerHTML=""),f.length===0){e.innerHTML=C("empty","No notifications yet."),le();return}let n=d=>{let g=d.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let h=g.title||"Untitled Notification",L=g.body||"No description provided.",E=g.priority||"low",S=!!g.alert,_=g.category||"system",k=g.related_event_ids||[],D=!!localStorage.getItem(`notification_read_ts_${d.id}`),R=new Date(d.timestamp*1e3),B=R.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=R.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=D?"event-border-grey":"event-border-blue";!D&&S&&(T="event-border-red"),D&&(E==="high"||E==="critical")?T="event-border-red":D&&E==="medium"&&(T="event-border-orange");let O=D?"notification-read":"notification-unread",N=q.has(d.id),ge=N?"expanded":"",be=N?"display: block;":"display: none;",Te="",ke="";k.length>0&&(ke=`
...
`,je=null,Y=new Set,We=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Tt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(We=r,je=Date.now(),H(2,je),r.length===0){e.innerHTML=C("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),U(1,0);return}t&&(e.innerHTML="");let c=u=>{let p=u.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let n=p.title||"Untitled Blueprint",i=p.summary||p.body||"No summary provided.",l=p.content||"",x=p.category||"architecture",y=p.affected_services||[],m=p.implementation_path||[],d=new Date(u.timestamp*1e3),g=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=Y.has(u.id),E=L?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""}`,S.dataset.blueprintId=u.id;let k={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";S.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),Y.add(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let M=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",D="";m.length>0&&(D=`
            ...
            `;let R=S.querySelector(".close-details-btn");return R&&R.addEventListener("click",B=>{B.stopPropagation(),S.classList.remove("expanded");let $=S.querySelector(".event-details");$&&($.style.display="none"),Y.delete(u.id)}),S},b=Array.from(e.children),f=new Map(b.map(u=>[u.dataset.blueprintId,u])),v=new Set(r.map(u=>u.id));b.forEach(u=>{let p=u.dataset.blueprintId;(!p||!v.has(p))&&u.remove()});let w=null;r.forEach((u,p)=>{let n=f.get(u.id);(!n||t)&&(n&&n.remove(),n=c(u),!n)||(p===0?e.firstElementChild!==n&&e.prepend(n):w&&w.nextElementSibling!==n&&w.after(n),w=n)}),U(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=C("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Tt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{We.forEach(a=>Y.add(a.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),me(!0)},e.dataset.listenerAttached="true")}var Ve=()=>`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`}e.innerHTML=i};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let i=await nt();i?(o(i),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.children.length===0&&(e.innerHTML=C("offline","Failed to refresh.","The event service may be offline.")),a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let i=await nt();o(i)}}if(!t)return;let s=await mt();if(!s||!s.services){t.children.length===0&&(t.innerHTML=C("offline","Failed to load system metrics.","The event service may be offline."));return}et=Date.now(),H(3,et);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function f(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function v(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,x=parseInt(i[2])||0,y=parseInt(i[3])||0,m=parseFloat(i[4])||0,d=l*86400+x*3600+y*60+m,g=Math.floor(d/86400);if(g>0)return`${g}d`;let h=Math.floor(d/3600);if(h>0)return`${h}h`;let L=Math.floor(d/60);return L>0?`${L}m`:`${Math.floor(d)}s`}function w(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",x=i?"bx-check-circle":"bx-x-circle",y=i?"OK":"BAD",m=n.version?b(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",g=n.memory&&n.memory!=="N/A"?n.memory:"-",h=d!=="-"?"#00ff00":"#666",L=d!=="-"?"#fff":"#666",E=g!=="-"?"#00ff00":"#666",S=g!=="-"?"#fff":"#666",_=v(n.uptime),k="";return i?k=`

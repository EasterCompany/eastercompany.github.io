...
`,ue=null,G=new Set,ge=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let n=(await o.json()).events||[];ue=Date.now(),P(0,ue);let r=Date.now(),p=24*60*60*1e3,v=n.filter(i=>{let d=localStorage.getItem(`alert_read_ts_${i.id}`);if(!d)return!0;let x=parseInt(d);return r-x<p});v.sort((i,d)=>{let x=L=>{let M=L.event;if(typeof M=="string")try{M=JSON.parse(M)}catch{return"low"}return(M.priority||"low").toLowerCase()},S=L=>L==="critical"?4:L==="high"?3:L==="medium"?2:1,E=S(x(i)),$=S(x(d));return E!==$?$-E:d.timestamp-i.timestamp}),ge=v;let m=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},y=[],h=new Set(v.map(i=>m(i))).size>1;if(v.length>0){let i=null;v.forEach(d=>{let x=m(d);h&&x!==i&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),i=x),y.push(d)})}if(t&&(e.innerHTML=""),v.length===0){e.innerHTML=k("empty","No alerts yet."),me();return}let b=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let x=d.title||"Untitled Alert",S=d.body||"No description provided.",E=d.priority||"low",$=!!d.alert,L=d.category||"system",M=d.related_event_ids||[],A=d.audit_event_id,D=!!localStorage.getItem(`alert_read_ts_${i.id}`),N=new Date(i.timestamp*1e3),R=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=D?"event-border-grey":"event-border-blue";!D&&$&&(T="event-border-red"),D&&(E==="high"||E==="critical")?T="event-border-red":D&&E==="medium"&&(T="event-border-orange");let H=D?"alert-read":"alert-unread",B=G.has(i.id),ye=B?"expanded":"",he=B?"display: block;":"display: none;",Ae="",_e="";M.length>0&&(_e=`
...
`,V=new Set,Qe=[],Q=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(Qe=o,o.length===0){e.innerHTML=k("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let y=V.has(m.id),g=m.state==="draft",h=m.state==="published",b=m.state==="consumed",l="event-border-grey";h&&(l="event-border-blue"),b&&(l="event-border-purple");let c=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),f=document.createElement("div");f.className=`event-item notification-item ${l} cursor-pointer ${y?"expanded":""}`,f.dataset.itemId=m.id,f.onclick=d=>{if(d.target.closest("button"))return;f.classList.contains("expanded")?(f.classList.remove("expanded"),f.querySelector(".event-details").style.display="none",V.delete(m.id)):(f.classList.add("expanded"),f.querySelector(".event-details").style.display="block",V.add(m.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;f.innerHTML=`
        <div class="event-time">
...
`,et=null,ee=new Set,nt=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let n=(await o.json()).events||[];if(nt=n,et=Date.now(),P(2,et),n.length===0){e.innerHTML=k("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ne(2,0);return}t&&(e.innerHTML="");let r=g=>{let h=g.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let b=h.title||"Untitled Blueprint",l=h.summary||h.body||"No summary provided.",u=h.content||"",c=h.category||"architecture",f=h.affected_services||[],w=h.implementation_path||[],i=new Date(g.timestamp*1e3),d=i.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=i.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=ee.has(g.id),E=S?"display: block;":"display: none;",$=document.createElement("div");$.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""}`,$.dataset.blueprintId=g.id;let M={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";$.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ee.delete(g.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),ee.add(g.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let A=f.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${f.join(", ")}</span></div>`:"",O="";w.length>0&&(O=`
            ...
            `;let D=$.querySelector(".event-details");D&&D.addEventListener("click",R=>{R.stopPropagation()});let N=$.querySelector(".close-details-btn");return N&&N.addEventListener("click",R=>{R.stopPropagation(),$.classList.remove("expanded");let C=$.querySelector(".event-details");C&&(C.style.display="none"),ee.delete(g.id)}),$},p=Array.from(e.children),v=new Map(p.map(g=>[g.dataset.blueprintId,g])),m=new Set(n.map(g=>g.id));p.forEach(g=>{let h=g.dataset.blueprintId;(!h||!m.has(h))&&g.remove()});let y=null;n.forEach((g,h)=>{let b=v.get(g.id);(!b||t)&&(b&&b.remove(),b=r(g),!b)||(h===0?e.firstElementChild!==b&&e.prepend(b):y&&y.nextElementSibling!==b&&y.after(b),y=b)}),ne(2,n.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=k("offline","Failed to load blueprints.","The event service may be offline."))}}function Pt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(a=>ee.add(a.id)),fe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ee.clear(),fe(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`,Ot=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let a=await Ke("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ot=Date.now(),s.length===0){t.innerHTML=k("empty","No contacts found.","Check your Discord connection.");return}let n={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((r,p)=>{let v=n[r.level]??10,m=n[p.level]??10;return v!==m?v-m:r.username.localeCompare(p.username)}),t.innerHTML=s.map(r=>{let p=r.color?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",m="#888";r.level==="Me"||r.level==="Master"?m="#bb86fc":r.level==="Admin"?m="#cf6679":r.level==="Moderator"?m="#03dac6":r.level==="Contributor"&&(m="#ffa500");let y=r.level==="Me",g=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=y?"2px solid #bb86fc":`1px solid ${p}33`;return`
...
`,ve=null,J=new Set,oe=[],j="all",zt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},qt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function lt(t){for(let[e,a]of Object.entries(zt))if(a.includes(t))return e;return"system"}function Ft(t){return qt[t]||"bx-square-rounded"}async function Y(t=!1){let e=document.getElementById("events-timeline");if(!e)return;jt();let o=`/events?ml=${j==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let r=(await s.json()).events||[],p=r;if(j!=="all"&&(p=r.filter(b=>{let l=b.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return!1}return lt(l.type)===j})),oe=p.slice(0,50),ve=Date.now(),P(1,ve),oe.length===0){e.innerHTML=k("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let l=b.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return null}let u=l.type,c=lt(u),f=Ft(u),w=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),i="event-border-grey";w&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?i="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.analysis.audit"?i="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?i="event-border-orange":u==="system.test.completed"?i=l.test?.status==="OK"&&l.lint?.status==="OK"&&l.format?.status==="OK"?"event-border-blue":"event-border-red":i="event-border-blue");let d=w?"cursor-pointer":"",x=J.has(b.id),S=x?"expanded":"",E=x?"display: block;":"display: none;",$=new Date(b.timestamp*1e3),L=$.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=$.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=ot(u,l),O=l.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${l.user_level})</span>`:"",D="";if(w){let C="";if(u==="engagement.decision")C=`
                    ...
                    `;else if(u==="system.attention.expired"){let T=l.timestamp-l.last_active,H=Math.floor(T/60);C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="analysis.visual.completed"){let T="";l.base64_preview?T=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${l.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:l.url&&(T=`<div class="event-detail-block"><img src="${l.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="system.analysis.audit"){let T=l.success?"#03dac6":"#ff4d4d",H=l.success?"SUCCESS":"FAILED",B="";l.error&&(B=`
                            <div class="event-detail-block">
                        ...
                        <div class="event-detail-row">
                            <span class="detail-label">Status:</span>
                            <span class="detail-value" style="color: ${T}; font-weight: bold;">${H} (${l.attempts} attempts)</span>
                        </div>
                        <div class="event-detail-row">
                            <span class="detail-label">Duration:</span>
                    ...
                    `;else if(u==="messaging.user.sent_message"){let T="";l.attachments&&l.attachments.length>0&&(T=`
                            <div class="event-detail-block">
...
`);return`
            ...
            `}).join("");return t.innerHTML=n,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let p=unescape(r.dataset.logs);navigator.clipboard.writeText(p).then(()=>{let v=r.querySelector("i");v.classList.remove("bx-copy"),v.classList.add("bx-check"),setTimeout(()=>{v.classList.remove("bx-check"),v.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let p=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${p}?`))try{(await I(`/logs?service_id=${p}`,{method:"DELETE"})).ok&&le()}catch(v){console.error("Error clearing logs:",v)}})}),Gt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=k("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ft=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function wt(){await Promise.all([be(),Le(),Te()]),await le()}var dt=null,mt=null,pt=null;async function Et(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await I("/processes")).json()}catch{return null}}async function Vt(){try{return await(await I("/agent/status")).json()}catch{return null}}async function Le(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),a=document.querySelector("#hw-cpu .hw-content"),o=document.querySelector("#hw-ram .hw-content"),s=document.querySelector("#hw-gpu .hw-content"),n=document.querySelector("#hw-storage .hw-content"),r=c=>{if(c){if(o){let f=(c.MEMORY_BYTES/1073741824).toFixed(1);o.innerHTML=`
                ...
                </div>`}function n(p){let v=p.status==="Downloaded",m=v?"service-widget-online":"service-widget-offline",y=v?"OK":"MISSING",g=v&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",h=p.name;return p.type==="custom"&&h.endsWith(":latest")&&(h=h.replace(":latest","")),`<div class="service-widget ${m}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${h}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let r="";if(o&&(r+=s(o)),r+=a.map(n).join(""),!r){t.innerHTML=k("empty","No models found.");return}t.innerHTML!==r&&(t.innerHTML=r)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),a=document.getElementById("guardian-t2-val"),o=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),n=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{p.innerHTML="<i class='bx bx-error'></i> Failed"}},p.dataset.listenerAttached="true");let v=await Vt();if(v){let l=Math.floor(Date.now()/1e3),u=v.active_tier,c=d=>{d<0&&(d=0);let x=Math.floor(d/3600),S=Math.floor(d%3600/60),E=d%60;return x>0?`${x}h ${S}m`:S>0?`${S}m ${E}s`:`${E}s`},f=(d,x,S,E)=>{if(u===E)d.textContent="Working",d.style.color="#bb86fc";else if(E==="t1"&&u==="tests")d.textContent="Testing",d.style.color="#03dac6";else if(S){let L=S.next_run-l;if(L<=0)d.textContent="Ready",d.style.color="#5eff5e";else{let M=Math.floor(L/60),A=L%60;d.textContent=`${M}m ${A}s`,d.style.color="#fff"}}x&&S&&(x.innerHTML=`
        ...
        `)};e&&f(e,document.getElementById("guardian-t1-stats"),v.t1,"t1"),a&&f(a,document.getElementById("guardian-t2-stats"),v.t2,"t2");let w=document.getElementById("system-state-label"),i=document.getElementById("system-state-val");if(i&&v.system_state){let d=v.system_state,x=c(v.system_state_time||0);w&&(w.textContent=`State: ${d.toUpperCase()}`),i.textContent=x,d==="idle"?i.style.color=v.system_state_time>300?"#5eff5e":"#fff":i.style.color="#bb86fc"}s&&(s.textContent=c(v.total_idle_time||0)),n&&(n.textContent=c(v.total_active_time||0)),r&&(r.textContent=c(v.total_waste_time||0))}else[e,a,o,s,n,r].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let m=await Wt(),y=[],g=[];m&&(Array.isArray(m)?y=m:(y=m.active||[],g=m.history||[]));let h=document.getElementById("vitals-processes-val");if(h)if(m){let l=y.length;h.textContent=l>0?`${l} Active`:"Idle",h.style.color=l>0?"#bb86fc":"#fff"}else h.textContent="-",h.style.color="#888";if(m===null){t.children.length===0&&(t.innerHTML=k("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),P(1,pt),y.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=k("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,y,!1));let b=document.getElementById("processes-history-widgets");b&&(g.length===0?b.querySelector(".tab-placeholder")||(b.innerHTML=k("empty","No recent history.")):(b.querySelector(".tab-placeholder")&&(b.innerHTML=""),gt(b,g,!0))),ne(1,y.length)}function gt(t,e,a){function o(r){let p="";r.end_time?p=`${r.end_time-r.start_time}s`:p=`${Math.floor(Date.now()/1e3-r.start_time)}s`;let v=r.retries>0?`<span class="process-retry-badge">Retry ${r.retries}</span>`:"",m=r.channel_id,y={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};y[m]?m=y[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`);let g=a?"#888":"#fff",h=a?"border-left: 3px solid #666;":"";return`

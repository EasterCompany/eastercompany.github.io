...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ze()}
        </div>`;async function dt(){await Promise.all([ge(),Ee(),Se()]),await ne()}var et=null,tt=null,st=null;async function mt(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function nt(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Bt(){try{return await(await A("/processes")).json()}catch{return null}}async function Nt(){try{return await(await A("/analyst/status")).json()}catch{return null}}async function Ee(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>');return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`

...
`,Ae=null,re=new Set,Me=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Yt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await L(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];Ae=Date.now(),Z(0,Ae);let o=Date.now(),b=24*60*60*1e3,d=l.filter(f=>{let r=localStorage.getItem(`alert_read_ts_${f.id}`);if(!r)return!0;let x=parseInt(r);return o-x<b});d.sort((f,r)=>{let x=A=>{let h=A.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},H=A=>A==="critical"?4:A==="high"?3:A==="medium"?2:1,z=H(x(f)),N=H(x(r));return z!==N?N-z:r.timestamp-f.timestamp}),Me=d;let p=f=>{let r=f.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return"low"}return(r.priority||"low").toLowerCase()},y=[],g=new Set(d.map(f=>p(f))).size>1;if(d.length>0){let f=null;d.forEach(r=>{let x=p(r);g&&x!==f&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),f=x),y.push(r)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=R("empty","No alerts yet."),ke();return}let n=f=>{let r=f.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let x=(r.title||"Untitled Alert").trim(),H=(r.body||"No description provided.").trim(),z=r.summary||"",N=r.content||"",A=r.protocol||"",h=(r.priority||"low").toLowerCase(),O=!!r.alert,I=(r.category||"system").trim(),M=r.related_event_ids||[],B=r.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${f.id}`),S=new Date(f.timestamp*1e3),w=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=F?"event-border-grey":"event-border-blue";!F&&O&&(T="event-border-red"),F&&(h==="high"||h==="critical")?T="event-border-red":F&&h==="medium"&&(T="event-border-orange");let E=F?"alert-read":"alert-unread",U=re.has(f.id),q=U?"expanded":"",te=U?"display: block;":"display: none;",J="",G="";M.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${M.map(se=>`<a href="#" onclick="window.dexter.viewEvent('${se}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${se.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let K="";B&&(K=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(N)}</div>
            </div>
        `:Y=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(H)}</div>
            </div>
        `,J=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${I}</div>
                    </div>
                    ${ue}
                    ${K}
                    ${G}
...
`,pt=null,de=new Set,ft=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ss();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await L(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(ft=l,pt=Date.now(),Z(2,pt),l.length===0){e.innerHTML=R("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=m=>{let g=m.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let n=(g.title||"Untitled Blueprint").trim(),u=(g.summary||g.body||"No summary provided.").trim(),k=(g.content||"").trim(),$=(g.category||"architecture").trim(),c=(g.related_services||g.affected_services||[]).map(E=>E.trim()),v=(g.implementation_path||[]).map(E=>E.trim()),f=g.source_event_ids||[],r=g.approved===!0,x=new Date(m.timestamp*1e3),H=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),z=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),N=de.has(m.id),A=N?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${N?"expanded":""} ${r?"blueprint-approved":""}`,h.dataset.blueprintId=m.id,r&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let I=r?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[$]||"bx-paint";h.onclick=function(E){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let q=this.querySelector(".event-details");q&&(q.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let q=this.querySelector(".event-details");q&&(q.style.display="block")}};let M="";v.length>0&&(M=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(E=>`<li style="margin-bottom: 5px;">${D(E)}</li>`).join("")}</ul></div>
                    </div>
                `);let B="";f.length>0&&(B=`
                ...
                `);let j=c.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${c.join(", ")}</span></div>`:"<div></div>",F=r?`
            ...
            `;h.innerHTML=`
                ...
                <div class="event-icon" style="${r?"color: #03dac6;":""}"><i class='bx ${I}'></i></div>
                <div class="event-content">
            ...
            `;let S=h.querySelector(".blueprint-approve-btn");S&&(S.onclick=async E=>{E.stopPropagation(),S.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await L(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(U){console.error("Failed to approve blueprint:",U)}});let w=h.querySelector(".blueprint-delete-btn");w&&(w.onclick=async E=>{E.stopPropagation();let U=!r;w.innerHTML=U?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await L(`/events/${m.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let C=h.querySelector(".event-details");C&&C.addEventListener("click",E=>{E.stopPropagation()});let T=h.querySelector(".close-details-btn");return T&&T.addEventListener("click",E=>{E.stopPropagation(),h.classList.remove("expanded");let U=h.querySelector(".event-details");U&&(U.style.display="none"),de.delete(m.id)}),h},b=Array.from(e.children),d=new Map(b.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));b.forEach(m=>{let g=m.dataset.blueprintId;(!g||!p.has(g))&&m.remove()});let y=null;l.forEach((m,g)=>{let n=d.get(m.id);(!n||t)&&(n&&n.remove(),n=o(m),!n)||(g===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),he(2,l.length),at()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=R("offline","Failed to load blueprints.","The event service may be offline."))}}async function gt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await L(t);if(!e.ok)return;let i=(await e.json()).events||[],s=0;i.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&s++}),it(s)}catch{}}function ss(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),a=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ft.forEach(i=>de.add(i.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await L("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}var bt=()=>`
...
`,ns=null;async function Re(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Re(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=a=>{let i=a.target.closest(".user-profile-card");if(i)try{let s=JSON.parse(i.dataset.user);showUserProfile(s)}catch(s){console.error("Failed to parse user data",s)}},t.dataset.listenerAttached="true");try{let a=await rt("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(ns=Date.now(),s.length===0){t.innerHTML=R("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((o,b)=>{let d=l[o.level]??10,p=l[b.level]??10;return d!==p?d-p:o.username.localeCompare(b.username)}),t.innerHTML=s.map(o=>{let b=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",p="#888";o.level==="Me"||o.level==="Master"?p="#bb86fc":o.level==="Admin"?p="#cf6679":o.level==="Moderator"?p="#03dac6":o.level==="Contributor"&&(p="#ffa500");let y=o.level==="Me",m=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",g=y?"2px solid #bb86fc":`1px solid ${b}33`;return`
    ...
    <div class="system-section-header" style="margin-bottom: 20px;">
        ...
        <div id="event-filters" class="event-filters" style="margin-left: 20px; margin-bottom: 0;">
            <button class="notif-action-btn filter-btn ${W==="all"?"active":""}" data-filter="all">All</button>
            <button class="notif-action-btn filter-btn ${W==="messaging"?"active":""}" data-filter="messaging">Messaging</button>
            <button class="notif-action-btn filter-btn ${W==="system"?"active":""}" data-filter="system">System</button>
            <button class="notif-action-btn filter-btn ${W==="cognitive"?"active":""}" data-filter="cognitive">Cognitive</button>
            <button class="notif-action-btn filter-btn ${W==="moderation"?"active":""}" data-filter="moderation">Moderation</button>
        </div>
        <div class="alerts-actions" style="margin-left: auto; display: flex; gap: 10px; padding: 0;">
...
`,Ie=null,me=new Set,we=[],W="all",as={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},os={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function rs(t){for(let[e,a]of Object.entries(as))if(a.includes(t))return e;return"system"}function ls(t){return os[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;cs();let i=`/events?ml=${W==="all"?100:250}&format=json`;W!=="all"&&(i+=`&category=${W}`);try{let s=await L(i);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ie=Date.now(),Z(1,Ie),we.length===0){e.innerHTML=R("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,k=rs(u),$=ls(u),c=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),v="event-border-grey";c&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?v="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"?v="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?v="event-border-orange":u==="system.test.completed"?v=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let f=c?"cursor-pointer":"",r=me.has(g.id),x=r?"expanded":"",H=r?"display: block;":"display: none;",z=new Date(g.timestamp*1e3),N=z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=xt(u,n),O=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",I="";if(c){let S="";if(u==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let w=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`,C="";n.eval_count&&(C=`
                        ...
                        `);let T="";if(n.chat_history&&n.chat_history.length>0){let E=n.chat_history.length,U=n.chat_history.map((q,te)=>{let J=q.role.toUpperCase();J==="ASSISTANT"&&(J="DEXTER");let G=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",K=te===E-1?"block":"none";return`
                    ...
                    `}else if(u==="system.analysis.audit"){let w=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",T=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,E="";n.error&&(E=`
                            <div class="event-detail-block">
                                ${T("Error")}
                        ...
                        `}let q="";if(n.corrections&&n.corrections.length>0){let J=n.corrections.map((G,K)=>`
                    ...
                    `}else if(u==="messaging.user.sent_message"){let w="";n.attachments&&n.attachments.length>0&&(w=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(T=>{let E=T.content_type&&T.content_type.startsWith("image/"),U=(T.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${T.content_type} \u2022 ${U}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let M=document.createElement("div");M.className=`event-item ${v} ${f} ${x}`,M.dataset.eventId=g.id,M.onclick=function(S){if(!c)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},M.innerHTML=`
                <div class="event-time">
...
`,yt=ds;function ht(){return`
    ...
    `}var ps=null;async function Ce(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await L("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML=R("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=a.filter(o=>!i.includes(o.id));s.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),s.reverse();let l=s.map(o=>{let b=o.logs.join(`
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${o.id}" title="Clear Logs" style="${_()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let b=unescape(o.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let d=o.querySelector("i");d.classList.remove("bx-copy"),d.classList.add("bx-check"),setTimeout(()=>{d.classList.remove("bx-check"),d.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let b=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await L(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&Ce()}catch(d){console.error("Error clearing logs:",d)}})}),ps=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=R("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Et=()=>{let t=_()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ht()}
        </div>`;async function Mt(){await Promise.all([$e(),Ue(),qe()]),await Ce()}var wt=null,Ct=null;async function _t(){try{return await(await L("/system_monitor")).json()}catch{return null}}async function $t(){try{return await(await L("/system/hardware")).json()}catch{return null}}async function ms(){try{return await(await L("/processes")).json()}catch{return null}}async function us(){try{return await(await L("/agent/status")).json()}catch{return null}}async function Ue(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),a=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),b=c=>{if(c){if(a&&(a.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&c.STORAGE&&c.STORAGE.length>0?o.innerHTML=c.STORAGE.map(v=>{let f=(v.USED/1073741824).toFixed(1),r=(v.SIZE/(1024*1024*1024)).toFixed(1),x=v.SIZE>0?(v.USED/v.SIZE*100).toFixed(0):0,H=v.MOUNT_POINT||"Unmounted";return`
                ...
                </div>`:B='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let j=_()?"easter.company":g(c.domain&&c.port?`${c.domain}:${c.port}`:"");return`<div class="service-widget ${f}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${r}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${j}</span></div>${B}</div></div>`}let k=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),$=new Set(p.map(c=>c.id));for(let[c,v]of k)$.has(c)||v.remove();p.forEach(c=>{let v=u(c),f=k.get(c.id);f?f.outerHTML!==v&&(f.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}async function qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await _t();if(!e){t.children.length===0&&(t.innerHTML=R("offline","Failed to load model status.","The event service may be offline."));return}Ct=Date.now(),Z(2,Ct);let a=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function s(d){let p=d.status==="Ready";return`
                ...
                </div>`}function o(d){let p=d.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",g=p&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",n=d.name;return d.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let b="";if(i&&(b+=s(i)),e.xtts&&(b+=l(e.xtts)),b+=a.map(o).join(""),!b){t.innerHTML=R("empty","No models found.");return}t.innerHTML!==b&&(t.innerHTML=b)}async function $e(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),a=document.getElementById("guardian-architect-val"),i=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),b=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn");b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{b.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await L("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{b.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{b.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{b.innerHTML="<i class='bx bx-error'></i> Failed"}},b.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await L("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{d.innerHTML="<i class='bx bx-error'></i> Failed"}},d.dataset.listenerAttached="true");let p=await us();if(p){let $=Math.floor(Date.now()/1e3),c=p.active_tier,v=p.active_synthesis,f=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},r=h=>{h<0&&(h=0);let O=Math.floor(h/3600),I=Math.floor(h%3600/60),M=h%60;return O>0?`${O}h ${I}m`:I>0?`${I}m ${M}s`:`${M}s`},x=(h,O,I,M,B)=>{if(!h)return;let j=f[M]||M.toUpperCase(),F=h.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(F&&(F.textContent=j),B&&B.includes(M))h.textContent="Working",h.style.color="#bb86fc";else if(I){let w=I.next_run-$;if(w<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let C=Math.floor(w/60),T=w%60;h.textContent=`${C}m ${T}s`,h.style.color="#fff"}}O&&I&&(O.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${I.attempts||0}</span>
            <span style="color: ${I.failures>0?"#ffa500":"#666"}">Fails: ${I.failures||0}</span>
            <span style="color: ${I.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${I.absolute_failures||0}</span>
          </div>
        `)};e&&x(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",c),a&&x(a,document.getElementById("guardian-architect-stats"),p.architect,"architect",c);let H=document.getElementById("analyzer-synthesis-val"),z=document.getElementById("analyzer-synthesis-stats");H&&x(H,z,p.synthesis,"synthesis",v);let N=document.getElementById("system-state-label"),A=document.getElementById("system-state-val");if(A&&p.system_state){let h=p.system_state,O=r(p.system_state_time||0);N&&(N.textContent=`State: ${h.toUpperCase()}`),A.textContent=O,h==="idle"?A.style.color=p.system_state_time>300?"#5eff5e":"#fff":A.style.color="#bb86fc"}s&&(s.textContent=r(p.total_idle_time||0)),l&&(l.textContent=r(p.total_active_time||0)),o&&(o.textContent=r(p.total_waste_time||0))}else[e,a,i,s,l,o].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let y=await ms(),m=[],g=[],n=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],g=y.queue||[],n=y.history||[],n.sort(($,c)=>(c.end_time||0)-($.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=R("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Oe(t,m,!1));let u=document.getElementById("processes-queue-widgets");u&&(g.length===0?!u.querySelector(".tab-placeholder")&&!u.querySelector('div[style*="font-style: italic"]')&&(u.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(u.innerHTML="",Oe(u,g,!1)));let k=document.getElementById("processes-history-widgets");k&&(n.length===0?k.querySelector(".tab-placeholder")||(k.innerHTML=R("empty","No recent history.")):(k.querySelector(".tab-placeholder")&&(k.innerHTML=""),Oe(k,n,!0))),he(1,m.length)}function Oe(t,e,a){function i(b){let d="";b.end_time?d=`${b.end_time-b.start_time}s`:d=`${Math.floor(Date.now()/1e3-b.start_time)}s`;let p=b.retries>0?`<span class="process-retry-badge">Retry ${b.retries}</span>`:"",y=b.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[y]?y=m[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`

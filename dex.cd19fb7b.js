(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ze(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Xe=1e3;function ae(t){let e=null,a=t.onClose||null,i=t.onOpen||null;function s(){e&&(e.style.zIndex=++Xe)}function r(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",o),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Xe,e.addEventListener("mousedown",s);let g=t.icon||"bx-window",n="",u="",L;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((x,f)=>{let v=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,L=`<div class="window-content">${t.tabs.map((x,f)=>`<div class="tab-content ${f===0?"active":""}" data-tab-content="${f}">${x.content}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),L=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Ae=null,re=new Set,Me=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Yt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await M(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];Ae=Date.now(),Z(0,Ae);let o=Date.now(),b=24*60*60*1e3,c=r.filter(f=>{let v=localStorage.getItem(`alert_read_ts_${f.id}`);if(!v)return!0;let d=parseInt(v);return o-d<b});c.sort((f,v)=>{let d=_=>{let h=_.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},T=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,z=T(d(f)),H=T(d(v));return z!==H?H-z:v.timestamp-f.timestamp}),Me=c;let p=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},y=[],g=new Set(c.map(f=>p(f))).size>1;if(c.length>0){let f=null;c.forEach(v=>{let d=p(v);g&&d!==f&&(y.push({id:`divider-${d}`,type:"divider",label:d.toUpperCase()}),f=d),y.push(v)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),ke();return}let n=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let d=(v.title||"Untitled Alert").trim(),T=(v.body||"No description provided.").trim(),z=v.summary||"",H=v.content||"",_=v.protocol||"",h=(v.priority||"low").toLowerCase(),R=!!v.alert,B=(v.category||"system").trim(),A=v.related_event_ids||[],q=v.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${f.id}`),E=new Date(f.timestamp*1e3),w=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=j?"event-border-grey":"event-border-blue";!j&&R&&(k="event-border-red"),j&&(h==="high"||h==="critical")?k="event-border-red":j&&h==="medium"&&(k="event-border-orange");let S=j?"alert-read":"alert-unread",U=re.has(f.id),F=U?"expanded":"",te=U?"display: block;":"display: none;",J="",G="";A.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${A.map(se=>`<a href="#" onclick="window.dexter.viewEvent('${se}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${se.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let K="";q&&(K=`
...
`,le=new Set,_e=[],ve=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){Xt();try{let a=await M("/roadmap");if(!a.ok)throw new Error("Offline");let i=await a.json();_e=i;let s=p=>{let y=le.has(p.id),m=p.state==="published",g=p.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),g&&(n="event-border-purple");let L=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),$=document.createElement("div");$.className=`event-item notification-item ${n} cursor-pointer ${y?"expanded":""}`,$.dataset.itemId=p.id,$.onclick=f=>{if(f.target.closest("button"))return;$.classList.contains("expanded")?($.classList.remove("expanded"),$.querySelector(".event-details").style.display="none",le.delete(p.id)):($.classList.add("expanded"),$.querySelector(".event-details").style.display="block",le.add(p.id))};let l=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;$.innerHTML=`
        <div class="event-time">
...
`,pt=null,de=new Set,ft=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ss();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await M(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];if(ft=r,pt=Date.now(),Z(2,pt),r.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=m=>{let g=m.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let n=(g.title||"Untitled Blueprint").trim(),u=(g.summary||g.body||"No summary provided.").trim(),L=(g.content||"").trim(),$=(g.category||"architecture").trim(),l=(g.related_services||g.affected_services||[]).map(S=>S.trim()),x=(g.implementation_path||[]).map(S=>S.trim()),f=g.source_event_ids||[],v=g.approved===!0,d=new Date(m.timestamp*1e3),T=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),z=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),H=de.has(m.id),_=H?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${H?"expanded":""} ${v?"blueprint-approved":""}`,h.dataset.blueprintId=m.id,v&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let B=v?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[$]||"bx-paint";h.onclick=function(S){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="block")}};let A="";x.length>0&&(A=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${x.map(S=>`<li style="margin-bottom: 5px;">${D(S)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";f.length>0&&(q=`
                ...
                `);let O=l.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${l.join(", ")}</span></div>`:"<div></div>",j=v?`
            ...
            `;h.innerHTML=`
                ...
                <div class="event-icon" style="${v?"color: #03dac6;":""}"><i class='bx ${B}'></i></div>
                <div class="event-content">
...
`,Ie=null,me=new Set,we=[],W="all",as={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},os={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function rs(t){for(let[e,a]of Object.entries(as))if(a.includes(t))return e;return"system"}function ls(t){return os[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;cs();let i=`/events?ml=${W==="all"?100:250}&format=json`;W!=="all"&&(i+=`&category=${W}`);try{let s=await M(i);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ie=Date.now(),Z(1,Ie),we.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,L=rs(u),$=ls(u),l=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),x="event-border-grey";l&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?x="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"?x="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?x="event-border-orange":u==="system.test.completed"?x=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":x="event-border-blue");let f=l?"cursor-pointer":"",v=me.has(g.id),d=v?"expanded":"",T=v?"display: block;":"display: none;",z=new Date(g.timestamp*1e3),H=z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=xt(u,n),R=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",B="";if(l){let E="";if(u==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                    ...
                    `}else if(u==="system.attention.expired"){let w=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`,C=n.timestamp-n.last_active,k=Math.floor(C/60);E=`
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let w=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`,C="";n.eval_count&&(C=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let k="";if(n.chat_history&&n.chat_history.length>0){let S=n.chat_history.length,U=n.chat_history.map((F,te)=>{let J=F.role.toUpperCase();J==="ASSISTANT"&&(J="DEXTER");let G=F.role==="user"?"#03dac6":F.role==="system"?"#ffb74d":"#bb86fc",K=te===S-1?"block":"none";return`
                        ...
                        `}
                    `}else if(u==="analysis.user.message_signals"){let w=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`,C=n.signals||{};E=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(C.topics||[]).map(S=>`<span class="profile-badge">${S}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.visual.completed"){let w=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,C="";n.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(C=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="analysis.router.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let w=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",k=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,S="";n.error&&(S=`
                            <div class="event-detail-block">
                                ${k("Error")}
                        ...
                        ${F}
                        ${U}
                    `}else if(u==="system.test.completed"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(u==="messaging.user.sent_message"){let w="";n.attachments&&n.attachments.length>0&&(w=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(k=>{let S=k.content_type&&k.content_type.startsWith("image/"),U=(k.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${S?`<div class="attachment-thumb-container"><img src="${k.url}" alt="${k.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                ...
                `}let A=document.createElement("div");A.className=`event-item ${x} ${f} ${d}`,A.dataset.eventId=g.id,A.onclick=function(E){if(!l)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},A.innerHTML=`
                <div class="event-time">
...
`,yt=ds;function ht(){return`
    ...
    `}var ps=null;async function Ce(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await M("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=a.filter(o=>!i.includes(o.id));s.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),s.reverse();let r=s.map(o=>{let b=o.logs.join(`
`),c=[...o.logs];if(c.length<25){let y=25-c.length;for(let m=0;m<y;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let p=c.map(y=>Le(y)).join(`
`);return`
            ...
            ${ht()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&l.STORAGE&&l.STORAGE.length>0?o.innerHTML=l.STORAGE.map(x=>{let f=(x.USED/1073741824).toFixed(1),v=(x.SIZE/(1024*1024*1024)).toFixed(1),d=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,T=x.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let l=await $t();l?(b(l),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let l=await $t();b(l)}if(!t)return;let c=await _t();if(!c||!c.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}wt=Date.now(),Z(0,wt);let p=c.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:l.split(".").slice(0,3).join(".")||"-"}function g(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let f=parseInt(x[1])||0,v=parseInt(x[2])||0,d=parseInt(x[3])||0,T=parseFloat(x[4])||0,z=f*86400+v*3600+d*60+T,H=Math.floor(z/86400);if(H>0)return`${H}d`;let _=Math.floor(z/3600);if(_>0)return`${_}h`;let h=Math.floor(z/60);return h>0?`${h}m`:`${Math.floor(z)}s`}function u(l){let x=l.status==="online",f=x?"service-widget-online":"service-widget-offline",v=x?"bx-check-circle":"bx-x-circle",d=x?"OK":"BAD",T=l.version?m(l.version.str):"-",z=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",H=l.memory&&l.memory!=="N/A"?l.memory:"-",_=z!=="-"?"#00ff00":"#666",h=z!=="-"?"#fff":"#666",R=H!=="-"?"#00ff00":"#666",B=H!=="-"?"#fff":"#666",A=n(l.uptime),q="";x?q=`
                ...
                </div>`:q='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let O=I()?"easter.company":g(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${v}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${q}</div></div>`}let L=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),$=new Set(p.map(l=>l.id));for(let[l,x]of L)$.has(l)||x.remove();p.forEach(l=>{let x=u(l),f=L.get(l.id);f?f.outerHTML!==x&&(f.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await _t();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Ct=Date.now(),Z(2,Ct);let a=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let p=c.status==="Ready";return`
        ...
        `)};e&&d(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",l),a&&d(a,document.getElementById("guardian-architect-stats"),p.architect,"architect",l);let T=document.getElementById("analyzer-synthesis-val"),z=document.getElementById("analyzer-synthesis-stats");T&&d(T,z,p.synthesis,"synthesis",x);let H=document.getElementById("system-state-label"),_=document.getElementById("system-state-val");if(_&&p.system_state){let h=p.system_state,R=v(p.system_state_time||0);H&&(H.textContent=`State: ${h.toUpperCase()}`),_.textContent=R,h==="idle"?_.style.color=p.system_state_time>300?"#5eff5e":"#fff":_.style.color="#bb86fc"}s&&(s.textContent=v(p.total_idle_time||0)),r&&(r.textContent=v(p.total_active_time||0)),o&&(o.textContent=v(p.total_waste_time||0))}else[e,a,i,s,r,o].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});let y=await ms(),m=[],g=[],n=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],g=y.queue||[],n=y.history||[],n.sort(($,l)=>(l.end_time||0)-($.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Oe(t,m,!1));let u=document.getElementById("processes-queue-widgets");u&&(g.length===0?!u.querySelector(".tab-placeholder")&&!u.querySelector('div[style*="font-style: italic"]')&&(u.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(u.innerHTML="",Oe(u,g,!1)));let L=document.getElementById("processes-history-widgets");L&&(n.length===0?L.querySelector(".tab-placeholder")||(L.innerHTML=N("empty","No recent history.")):(L.querySelector(".tab-placeholder")&&(L.innerHTML=""),Oe(L,n,!0))),he(1,m.length)}function Oe(t,e,a){function i(b){let c="";b.end_time?c=`${b.end_time-b.start_time}s`:c=`${Math.floor(Date.now()/1e3-b.start_time)}s`;let p=b.retries>0?`<span class="process-retry-badge">Retry ${b.retries}</span>`:"",y=b.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[y]?y=m[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],gs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Fe=!0,a}function It(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Fe=!1}function Bt(t,e,a="output"){if(!Fe)return;let i=document.createElement("div");i.className=`terminal-line terminal-${a}`,a==="prompt"?i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:i.innerHTML=Le(e),t.appendChild(i),t.scrollTop=t.scrollHeight}async function vs(t){let e=Dt.find(s=>s.id===t);if(!e)return;let a=bs(e);Bt(a,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let s of i){await new Promise(o=>setTimeout(o,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),Bt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function zt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=gs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let i=e.querySelector("i");i.className="bx bx-check",i.style.color="#5eff5e",setTimeout(()=>{i.className="bx bx-copy",i.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let i=a.dataset.cmd;vs(i)})})}async function xs(){try{if(!(await M("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ht(){xs(),tt(),Ve();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&zt(),Ze(),Ye();let e=document.getElementById("nav-left-container");e&&e.addEventListener("click",()=>{if(s.length>0)y();else{let d=window.location.pathname;if(!(d==="/"||d==="/index.html")){let H=(d.endsWith("/")&&d.length>1?d.slice(0,-1):d).split("/");H.pop();let _=H.join("/")||"/";window.location.href=_}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",i=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!i&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let o=d=>{localStorage.setItem("dex_last_window",d)};function b(){return 1}function c(){for(;s.length>1;)s.shift().close(!0);let d=document.getElementById("windows-container"),T=document.querySelector("nav"),z=document.querySelector("footer"),H=window.location.pathname==="/"||window.location.pathname==="/index.html",_=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");d&&d.setAttribute("data-count",s.length),s.forEach(O=>{let j=document.getElementById(O.id);j&&j.classList.remove("hide-close")}),Ee(ye());let h=document.getElementById("dexter-menu-container"),R=document.getElementById("nav-window-switcher"),B=document.getElementById("settings-icon"),A=document.getElementById("close-all-windows"),q=window.innerWidth<768;if(Ke(s.length>0),s.length>0)if(z?.classList.add("hide"),A&&(A.style.display=q?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),T?.classList.add("window-open"),d&&(d.style.paddingTop="60px"),h&&(h.style.display="flex"),B&&(B.style.display="block"),!q&&R){let O=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(O)?(R.innerHTML=`
                      <div class="nav-switch-btn ${O==="alerts-window"?"active":""}" id="switch-alerts"><i class='bx bx-bell'></i> Alerts</div>
                      <div class="nav-switch-btn ${O==="events-window"?"active":""}" id="switch-events"><i class='bx bx-calendar-event'></i> Events</div>
                      <div class="nav-switch-btn ${O==="monitor-window"?"active":""}" id="switch-monitor"><i class='bx bx-pulse'></i> Monitor</div>
                      <div class="nav-switch-btn ${O==="contacts-window"?"active":""}" id="switch-contacts"><i class='bx bx-book-content'></i> Contacts</div>
                      <div class="nav-switch-btn ${O==="workspace-window"?"active":""}" id="switch-workspace"><i class='bx bx-brain'></i> Workspace</div>
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{o("alerts-window"),p(m)}),document.getElementById("switch-events").addEventListener("click",()=>{o("events-window"),p(g)}),document.getElementById("switch-monitor").addEventListener("click",()=>{o("monitor-window"),p(u)}),document.getElementById("switch-contacts").addEventListener("click",()=>{o("contacts-window"),p(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{o("workspace-window"),p(L)})):R.innerHTML=""}else R&&(R.innerHTML="");else T?.classList.remove("window-open"),A&&(A.style.display="none"),d&&(d.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),H||_?z?.classList.remove("hide"):z?.classList.add("hide"),h&&(h.style.display="flex"),B&&(B.style.display="block"),R&&(R.innerHTML="");be()}function p(d){if(d.isOpen()){d.close();return}for(;s.length>0;)s.pop().close(!0);s.push(d),d.open(),c()}function y(){[...s].forEach(T=>T.close()),s=[]}window.addEventListener("resize",c);let m=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:lt(),onOpen:()=>ne(),onClose:()=>{let d=s.indexOf(m);d>-1&&s.splice(d,1),c()}}),g=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Pe(),onOpen:()=>{g.setContent(Pe()),ie()},onClose:()=>{let d=s.indexOf(g);d>-1&&s.splice(d,1),c()}}),n=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:vt(),onOpen:()=>Re(),onClose:()=>{let d=s.indexOf(n);d>-1&&s.splice(d,1),c()}}),u=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Tt()},{icon:"bxs-component",title:"Processes",content:St()},{icon:"bxs-brain",title:"Models",content:kt()},{icon:"bxs-hdd",title:"Hardware",content:Lt()},{icon:"bxs-terminal",title:"Logs",content:At()},{icon:"bxs-zap",title:"Agents",content:Et()}].filter(d=>I()?d.title!=="Hardware"&&d.title!=="Logs":!0),onOpen:()=>{Ue(),$e(),qe(),Ce()},onClose:()=>{let d=s.indexOf(u);d>-1&&s.splice(d,1),c()}}),L=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:bt(),onOpen:()=>Ne(),onClose:()=>{let d=s.indexOf(L);d>-1&&s.splice(d,1),c()}}),$=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:Be(),onOpen:()=>{$.setContent(Be()),je($)},onClose:()=>{let d=s.indexOf($);d>-1&&s.splice(d,1),c()}});window.dexter={viewEvent:d=>{g.isOpen()||p(g),setTimeout(()=>{let T=document.querySelector(`.event-item[data-event-id="${d}"]`);T&&(T.scrollIntoView({behavior:"smooth",block:"center"}),T.classList.add("flash-highlight"),T.classList.contains("expanded")||T.click(),setTimeout(()=>T.classList.remove("flash-highlight"),2e3))},500)},viewAlert:d=>{m.isOpen()||p(m),setTimeout(()=>{let T=document.querySelector(`.event-item[data-alert-id="${d}"]`);T&&(T.scrollIntoView({behavior:"smooth",block:"center"}),T.classList.add("flash-highlight"),T.classList.contains("expanded")||T.click(),setTimeout(()=>T.classList.remove("flash-highlight"),2e3))},500)}};let l=document.getElementById("dexter-dropdown");l&&(l.innerHTML=`
          ...
          <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
      `);let x=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");x&&l&&f&&(f.addEventListener("click",d=>{d.stopPropagation();let T=l.classList.contains("active");l.classList.toggle("active"),f.classList.toggle("active")}),document.addEventListener("click",()=>{l.classList.remove("active"),f.classList.remove("active")}),l.addEventListener("click",d=>{d.stopPropagation()}));let v=()=>{l&&f&&(l.classList.remove("active"),f.classList.remove("active"))};document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{v(),o("alerts-window"),p(m)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{v(),o("events-window"),p(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{v(),o("monitor-window"),p(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{v(),o("contacts-window"),p(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{v(),o("workspace-window"),p(L)}),document.getElementById("settings-icon")?.addEventListener("click",()=>p($)),document.getElementById("close-all-windows")?.addEventListener("click",()=>y()),I()||setInterval(()=>{g.isOpen()&&ie(),m.isOpen()?ne():He()},1e3),setInterval(()=>{I()&&(m.isOpen()?ne():He()),L.isOpen()?Ne():gt(),I()&&g.isOpen()&&ie(),u.isOpen()&&Mt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ht):Ht();})();

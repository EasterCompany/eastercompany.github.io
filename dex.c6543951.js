(()=>{function He(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ne(t=!1){let e=window.location.pathname,o=e==="/"||e==="/index.html",i=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Oe=1e3;function F(t){let e=null,o=t.onClose||null,n=t.onOpen||null;function s(){e&&(e.style.zIndex=++Oe)}function i(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",l),n&&setTimeout(n,10);return}let g=document.getElementById("windows-container");g||(g=document.createElement("div"),g.id="windows-container",g.className="windows-container",document.body.appendChild(g)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Oe,e.addEventListener("mousedown",s);let w=t.icon||"bx-window",b="",r="",p;t.tabs&&t.tabs.length>0?(b=`<div class="tab-bar">${t.tabs.map((y,a)=>{let d=y.icon?`<i class="bx ${y.icon}"></i>`:`<span class="tab-glyph">${y.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,p=`<div class="window-content">${t.tabs.map((y,a)=>`<div class="tab-content ${a===0?"active":""}" data-tab-content="${a}">${y.content}</div>`).join("")}</div>`):(t.title&&(r=`<div class="window-title">${t.title}</div>`),p=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,V=new Set,Qe=[],Q=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let o=await I("/roadmap");if(!o.ok)throw new Error("Offline");let n=await o.json();if(Qe=n,n.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let h=V.has(m.id),g=m.state==="draft",w=m.state==="published",b=m.state==="consumed",r="event-border-grey";w&&(r="event-border-blue"),b&&(r="event-border-purple");let c=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),f=document.createElement("div");f.className=`event-item notification-item ${r} cursor-pointer ${h?"expanded":""}`,f.dataset.itemId=m.id,f.onclick=d=>{if(d.target.closest("button"))return;f.classList.contains("expanded")?(f.classList.remove("expanded"),f.querySelector(".event-details").style.display="none",V.delete(m.id)):(f.classList.add("expanded"),f.querySelector(".event-details").style.display="block",V.add(m.id))};let y=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;f.innerHTML=`
        <div class="event-time">
...
`,et=null,ee=new Set,nt=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let n=await I(o);if(!n.ok)throw new Error("Service is offline or unreachable.");let i=(await n.json()).events||[];if(nt=i,et=Date.now(),P(2,et),blueprints.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ne(1,0);return}t&&(e.innerHTML="");let l=g=>{let w=g.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return null}let b=w.title||"Untitled Blueprint",r=w.summary||w.body||"No summary provided.",p=w.content||"",c=w.category||"architecture",f=w.affected_services||[],y=w.implementation_path||[],a=new Date(g.timestamp*1e3),d=a.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=a.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ee.has(g.id),E=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,S.dataset.blueprintId=g.id;let M={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";S.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ee.delete(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="none")}else{this.classList.add("expanded"),ee.add(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="block")}};let A=f.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${f.join(", ")}</span></div>`:"",O="";y.length>0&&(O=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${y.map(R=>`<li>${_(R)}</li>`).join("")}
                        </ul>
                    </div>
                `),S.innerHTML=`
                <div class="event-time">
...
`,Ot=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let o=await Ke("/contacts");if(!o.ok)throw new Error("Service unreachable");let s=(await o.json()).members||[];if(Ot=Date.now(),s.length===0){t.innerHTML=T("empty","No contacts found.","Check your Discord connection.");return}let i={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((l,u)=>{let v=i[l.level]??10,m=i[u.level]??10;return v!==m?v-m:l.username.localeCompare(u.username)}),t.innerHTML=s.map(l=>{let u=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",m="#888";l.level==="Me"||l.level==="Master"?m="#bb86fc":l.level==="Admin"?m="#cf6679":l.level==="Moderator"?m="#03dac6":l.level==="Contributor"&&(m="#ffa500");let h=l.level==="Me",g=h?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",w=h?"2px solid #bb86fc":`1px solid ${u}33`;return`
                <div class="user-profile-section" style="background: ${g}; padding: 15px; border-radius: 8px; border: ${w}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${h?"#bb86fc":u}22, transparent 70%); pointer-events: none;"></div>
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${m}; font-weight: 600; text-transform: uppercase;">${h?"DEXTER (ME)":l.level}</span>
                        </div>
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}s&&(c.GPU&&c.GPU.length>0?s.innerHTML=c.GPU.map(f=>{let y=(f.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${f.LABEL}</span>
                            <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${y} <span style="font-size: 0.6em; color: #888; font-weight: normal;">GB VRAM</span></span>
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):s.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),i&&c.STORAGE&&c.STORAGE.length>0?i.innerHTML=c.STORAGE.map(f=>{let y=(f.USED/1073741824).toFixed(1),a=(f.SIZE/(1024*1024*1024)).toFixed(1),d=f.SIZE>0?(f.USED/f.SIZE*100).toFixed(0):0,x=f.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):i&&(i.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await ut();c?(l(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),o&&!o.hasChildNodes())){let c=await ut();l(c)}if(!t)return;let u=await Et();if(!u||!u.services){t.children.length===0&&(t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),P(0,dt);let v=u.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function m(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function h(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/^(\d+\.\d+\.\d+)/);return f?f[0]:c.split(".").slice(0,3).join(".")||"-"}function g(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function w(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!f)return"-";let y=parseInt(f[1])||0,a=parseInt(f[2])||0,d=parseInt(f[3])||0,x=parseFloat(f[4])||0,$=y*86400+a*3600+d*60+x,E=Math.floor($/86400);if(E>0)return`${E}d`;let S=Math.floor($/3600);if(S>0)return`${S}h`;let L=Math.floor($/60);return L>0?`${L}m`:`${Math.floor($)}s`}function b(c){let f=c.status==="online",y=f?"service-widget-online":"service-widget-offline",a=f?"bx-check-circle":"bx-x-circle",d=f?"OK":"BAD",x=c.version?h(c.version.str):"-",$=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",E=c.memory&&c.memory!=="N/A"?c.memory:"-",S=$!=="-"?"#00ff00":"#666",L=$!=="-"?"#fff":"#666",M=E!=="-"?"#00ff00":"#666",A=E!=="-"?"#fff":"#666",O=w(c.uptime),B="";return f?B=`
                ...
                </div>`:B='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${y}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${a}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${g(c.domain&&c.port?`${c.domain}:${c.port}`:"")}</span></div>${B}</div></div>`}let r=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),p=new Set(v.map(c=>c.id));for(let[c,f]of r)p.has(c)||f.remove();v.forEach(c=>{let f=b(c),y=r.get(c.id);y?y.outerHTML!==f&&(y.outerHTML=f):t.insertAdjacentHTML("beforeend",f)})}async function Le(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Et();if(!e){t.children.length===0&&(t.innerHTML=T("offline","Failed to load model status.","The event service may be offline."));return}mt=Date.now(),P(2,mt);let o=e.models||[],n=e.whisper;Array.from(t.children).forEach(u=>{u.classList.contains("service-widget")||u.remove()});function s(u){let v=u.status==="Ready";return`
                ...
                </div>`}function i(u){let v=u.status==="Downloaded",m=v?"service-widget-online":"service-widget-offline",h=v?"OK":"MISSING",g=v&&u.size>0?`${(u.size/1e9).toFixed(2)} GB`:"-",w=u.name;return u.type==="custom"&&w.endsWith(":latest")&&(w=w.replace(":latest","")),`<div class="service-widget ${m}" data-model-name="${u.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${w}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${u.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let l="";if(n&&(l+=s(n)),l+=o.map(i).join(""),!l){t.innerHTML=T("empty","No models found.");return}t.innerHTML!==l&&(t.innerHTML=l)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),o=document.getElementById("guardian-t2-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),i=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),u=document.getElementById("guardian-reset-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{u.innerHTML="<i class='bx bx-error'></i> Failed"}},u.dataset.listenerAttached="true");let v=await Vt();if(v){let r=Math.floor(Date.now()/1e3),p=v.active_tier,c=y=>{y<0&&(y=0);let a=Math.floor(y/3600),d=Math.floor(y%3600/60),x=y%60;return a>0?`${a}h ${d}m`:d>0?`${d}m ${x}s`:`${x}s`},f=(y,a,d,x)=>{if(p===x)y.textContent="Working",y.style.color="#bb86fc";else if(x==="t1"&&p==="tests")y.textContent="Testing",y.style.color="#03dac6";else if(d){let E=d.next_run-r;if(E<=0)y.textContent="Ready",y.style.color="#5eff5e";else{let S=Math.floor(E/60),L=E%60;y.textContent=`${S}m ${L}s`,y.style.color="#fff"}}a&&d&&(a.innerHTML=`
        ...
        `)};if(e&&f(e,document.getElementById("guardian-t1-stats"),v.t1,"t1"),o&&f(o,document.getElementById("guardian-t2-stats"),v.t2,"t2"),n&&v.system_idle_time!==void 0){let y=v.system_idle_time;n.textContent=c(y),y>300?n.style.color="#5eff5e":y>60?n.style.color="#ffa500":n.style.color="#fff"}s&&(s.textContent=c(v.total_idle_time||0)),i&&(i.textContent=c(v.total_active_time||0)),l&&(l.textContent=c(v.total_waste_time||0))}else[e,o,n,s,i,l].forEach(p=>{p&&(p.textContent="-",p.style.color="#666")});let m=await Wt(),h=[],g=[];m&&(Array.isArray(m)?h=m:(h=m.active||[],g=m.history||[]));let w=document.getElementById("vitals-processes-val");if(w)if(m){let r=h.length;w.textContent=r>0?`${r} Active`:"Idle",w.style.color=r>0?"#bb86fc":"#fff"}else w.textContent="-",w.style.color="#888";if(m===null){t.children.length===0&&(t.innerHTML=T("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),P(1,pt),h.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=T("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,h,!1));let b=document.getElementById("processes-history-widgets");b&&(g.length===0?b.querySelector(".tab-placeholder")||(b.innerHTML=T("empty","No recent history.")):(b.querySelector(".tab-placeholder")&&(b.innerHTML=""),gt(b,g,!0))),ne(1,h.length)}function gt(t,e,o){function n(l){let u="";l.end_time?u=`${l.end_time-l.start_time}s`:u=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let v=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",m=l.channel_id,h={"system-discord":"Discord Engine","system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};h[m]?m=h[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`);let g=o?"#888":"#fff",w=o?"border-left: 3px solid #666;":"";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,o}function St(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function $t(t,e,o="output"){if(!Ie)return;let n=document.createElement("div");n.className=`terminal-line terminal-${o}`,o==="prompt"?n.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:n.innerHTML=pe(e),t.appendChild(n),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let o=Yt(e);$t(o,`dex ${t}`,"prompt");let n=e.demo_output||["Executing command...","\u2713 Done."];for(let s of n){await new Promise(l=>setTimeout(l,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),$t(o,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code").textContent;navigator.clipboard.writeText(o).then(()=>{let n=e.querySelector("i");n.className="bx bx-check",n.style.color="#5eff5e",setTimeout(()=>{n.className="bx bx-copy",n.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-5px)",o.style.borderColor="rgba(255,255,255,0.15)",o.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)",o.style.borderColor="rgba(255,255,255,0.05)",o.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let n=o.dataset.cmd;Xt(n)})})}async function Zt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Lt(){Zt(),Ge(),He();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=Ue();Ne(e),Pe();let o=document.getElementById("nav-left-container");o&&o.addEventListener("click",()=>{if(i.length>0)g();else{let a=window.location.pathname;if(!(a==="/"||a==="/index.html")){let $=(a.endsWith("/")&&a.length>1?a.slice(0,-1):a).split("/");$.pop();let E=$.join("/")||"/";window.location.href=E}}});let n=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!n&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let u=a=>{localStorage.setItem("dex_last_window",a)};function v(){return 1}function m(){for(;i.length>1;)i.shift().close(!0);let a=document.getElementById("windows-container"),d=document.querySelector("nav"),x=document.querySelector("footer"),$=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",i.length),i.forEach(A=>{let O=document.getElementById(A.id);O&&O.classList.remove("hide-close")}),re(se());let S=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),M=document.getElementById("settings-icon");if(Re(i.length>0),i.length>0){if(x?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),S&&(S.style.display="none"),M&&(M.style.display="none"),L){let A=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{u("alerts-window"),h(w)}),document.getElementById("switch-events").addEventListener("click",()=>{u("events-window"),h(b)}),document.getElementById("switch-monitor").addEventListener("click",()=>{u("monitor-window"),h(p)}),document.getElementById("switch-contacts").addEventListener("click",()=>{u("contacts-window"),h(r)}),document.getElementById("switch-workspace").addEventListener("click",()=>{u("workspace-window"),h(c)})):L.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),$||E?x?.classList.remove("hide"):x?.classList.add("hide"),S&&(S.style.display="flex"),M&&(M.style.display="block"),L&&(L.innerHTML="");ie(Ve())}function h(a){if(a.isOpen()){a.close();return}for(;i.length>0;)i.pop().close(!0);i.push(a),a.open(),m()}function g(){[...i].forEach(d=>d.close()),i=[]}window.addEventListener("resize",m);let w=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Je(),onOpen:()=>W(),onClose:()=>{let a=i.indexOf(w);a>-1&&i.splice(a,1),m()}}),b=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:rt(),onOpen:()=>Y(),onClose:()=>{let a=i.indexOf(b);a>-1&&i.splice(a,1),m()}}),r=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:at(),onOpen:()=>Ce(),onClose:()=>{let a=i.indexOf(r);a>-1&&i.splice(a,1),m()}}),p=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:xt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Agents",content:ft()}],onOpen:()=>{Te(),be(),Le(),le()},onClose:()=>{let a=i.indexOf(p);a>-1&&i.splice(a,1),m()}}),c=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:it(),onOpen:()=>$e(),onClose:()=>{let a=i.indexOf(c);a>-1&&i.splice(a,1),m()}}),f=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:xe(),onOpen:()=>{f.setContent(xe()),ke(f)},onClose:()=>{let a=i.indexOf(f);a>-1&&i.splice(a,1),m()}}),y=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:a=>{b.isOpen()||h(b),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:a=>{w.isOpen()||h(w),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}},e){let a=document.getElementById("dexter-dropdown");a&&(a.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),x=document.getElementById("dexter-menu-btn");d&&a&&x&&(d.addEventListener("mouseenter",()=>{a.classList.add("active"),x.classList.add("active")}),d.addEventListener("mouseleave",()=>{a.classList.remove("active"),x.classList.remove("active")}),x.addEventListener("click",$=>{$.stopPropagation();let E=localStorage.getItem("dex_last_window")||"alerts-window",S=null;E==="alerts-window"?S=w:E==="events-window"?S=b:E==="monitor-window"?S=p:E==="contacts-window"?S=r:E==="workspace-window"&&(S=c),S&&h(S)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{u("alerts-window"),h(w)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{u("events-window"),h(b)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{u("monitor-window"),h(p)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{u("contacts-window"),h(r)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{u("workspace-window"),h(c)}),document.getElementById("settings-icon")?.addEventListener("click",()=>h(f)),document.getElementById("close-all-windows")?.addEventListener("click",()=>g()),setInterval(()=>{w.isOpen()?W():Ye(),b.isOpen()&&Y(),p.isOpen()&&wt(),c.isOpen()&&$e()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{y.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",a=>{a.preventDefault();try{qe(document.getElementById("email-input").value),window.location.reload()}catch(d){let x=document.getElementById("login-error");x&&(x.textContent=d.message,x.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Lt):Lt();})();

(()=>{function Re(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Oe(t=!1){let e=window.location.pathname,r=e==="/"||e==="/index.html",a=`
    ...
    `}
        ...
        `}
    `,g=document.createElement("nav");g.innerHTML=a,document.body.prepend(g)}function Pe(t){let e=window.location.pathname,r=e==="/"||e==="/index.html",o=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!o||!s||(t||!r?(o.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{o.style.transform="translateX(-3px)"},s.onmouseleave=()=>{o.style.transform="translateX(0)"}):(o.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function ze(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ue=1e3;function W(t){let e=null,r=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++Ue)}function a(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",g),o&&setTimeout(o,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ue,e.addEventListener("mousedown",s);let b=t.icon||"bx-window",n="",f="",C;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((x,i)=>{let u=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,C=`<div class="window-content">${t.tabs.map((x,i)=>`<div class="tab-content ${i===0?"active":""}" data-tab-content="${i}">${x.content}</div>`).join("")}</div>`):(t.title&&(f=`<div class="window-title">${t.title}</div>`),C=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,be=null,V=new Set,ve=[];async function J(t=!1){let e=document.getElementById("alerts-list");if(!e)return;_t(),t&&(e.innerHTML="<p>Updating...</p>");let r="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(r);if(!o.ok)throw new Error("Service is offline or unreachable.");let a=(await o.json()).events||[];be=Date.now(),U(0,be);let g=Date.now(),d=24*60*60*1e3,c=a.filter(i=>{let u=localStorage.getItem(`alert_read_ts_${i.id}`);if(!u)return!0;let v=parseInt(u);return g-v<d});c.sort((i,u)=>{let v=S=>{let _=S.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},L=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,w=L(v(i)),T=L(v(u));return w!==T?T-w:u.timestamp-i.timestamp}),ve=c;let m=i=>{let u=i.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},y=[],b=new Set(c.map(i=>m(i))).size>1;if(c.length>0){let i=null;c.forEach(u=>{let v=m(u);b&&v!==i&&(y.push({id:`divider-${v}`,type:"divider",label:v.toUpperCase()}),i=v),y.push(u)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=B("empty","No alerts yet."),ge();return}let n=i=>{let u=i.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let v=(u.title||"Untitled Alert").trim(),L=(u.body||"No description provided.").trim(),w=(u.priority||"low").toLowerCase(),T=!!u.alert,S=(u.category||"system").trim(),_=u.related_event_ids||[],D=u.audit_event_id,N=!!localStorage.getItem(`alert_read_ts_${i.id}`),z=new Date(i.timestamp*1e3),k=z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),E=z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=N?"event-border-grey":"event-border-blue";!N&&T&&($="event-border-red"),N&&(w==="high"||w==="critical")?$="event-border-red":N&&w==="medium"&&($="event-border-orange");let M=N?"alert-read":"alert-unread",I=V.has(i.id),O=I?"expanded":"",q=I?"display: block;":"display: none;",Me="",Be="";_.length>0&&(Be=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${_.map(G=>`<a href="#" onclick="window.dexter.viewEvent('${G}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px; font-family: monospace; width: fit-content;">${G.substring(0,8)}...</a>`).join(", ")}</span>
                    </div>`);let De="";D&&(De=`
              <div class="event-detail-row">
...
`,K=new Set,tt=[],te=null;async function Y(t=!1){let e=document.getElementById("roadmap-list");if(e){Mt();try{let r=await A("/roadmap");if(!r.ok)throw new Error("Offline");let o=await r.json();tt=o;let s=m=>{let y=K.has(m.id),p=m.state==="published",b=m.state==="consumed",n="event-border-grey";p&&(n="event-border-blue"),b&&(n="event-border-purple");let C=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${n} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=m.id,h.onclick=i=>{if(i.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",K.delete(m.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",K.add(m.id))};let l=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
      ...
      `;let x=h.querySelector(".event-details");return x&&x.addEventListener("click",i=>{i.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>Bt(m)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Dt(m)),h.querySelector(".delete-btn")?.addEventListener("click",()=>Ht(m.id)),h.querySelector(".close-details-btn")?.addEventListener("click",i=>{i.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",K.delete(m.id)}),h},a=Array.from(e.children),g=new Map(a.map(m=>[m.dataset.itemId,m])),d=new Set(o.map(m=>m.id));if(a.forEach(m=>{let y=m.dataset.itemId;(!y||!d.has(y))&&m.remove()}),o.length===0){e.innerHTML=B("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;o.forEach((m,y)=>{let p=g.get(m.id);(!p||t)&&(p&&p.remove(),p=s(m),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):c&&c.nextElementSibling!==p&&c.after(p),c=p)})}catch{e.children.length===0&&(e.innerHTML=B("offline","Failed to load roadmap.","The event service may be offline."))}}}function Mt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),r=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{te=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),r&&!r.dataset.listenerAttached&&(r.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",te=null},r.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let a=document.getElementById("roadmap-editor-input").value;if(!a.trim())return;let g=te?`/roadmap/${te}`:"/roadmap",d=te?"PATCH":"POST";try{await A(g,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:a})}),document.getElementById("roadmap-editor-container").style.display="none",Y(!0)}catch(c){console.error(c)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{tt.forEach(a=>K.add(a.id)),Y(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{K.clear(),Y(!0)},s.dataset.listenerAttached="true")}function Bt(t){te=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Dt(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),Y(!0)}catch(r){console.error(r)}}async function Ht(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),K.delete(t),Y(!0)}catch(e){console.error(e)}}var nt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,st=null,se=new Set,at=[];async function ne(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Nt();let r="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(r);if(!o.ok)throw new Error("Service is offline or unreachable.");let a=(await o.json()).events||[];if(at=a,st=Date.now(),U(2,st),a.length===0){e.innerHTML=B("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),oe(2,0);return}t&&(e.innerHTML="");let g=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),C=(b.content||"").trim(),h=(b.category||"architecture").trim(),l=(b.affected_services||[]).map(I=>I.trim()),x=(b.implementation_path||[]).map(I=>I.trim()),i=b.approved===!0,u=new Date(p.timestamp*1e3),v=u.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=u.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=se.has(p.id),T=w?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${w?"expanded":""} ${i?"blueprint-approved":""}`,S.dataset.blueprintId=p.id,i&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let D=i?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";S.onclick=function(I){if(this.classList.contains("expanded")){this.classList.remove("expanded"),se.delete(p.id);let q=this.querySelector(".event-details");q&&(q.style.display="none")}else{this.classList.add("expanded"),se.add(p.id);let q=this.querySelector(".event-details");q&&(q.style.display="block")}};let R="";x.length>0&&(R=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${x.map(I=>`<li style="margin-bottom: 5px;">${H(I)}</li>`).join("")}</ul></div>
                    </div>
                `);let N=l.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${l.join(", ")}</span></div>`:"<div></div>",z=i?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-icon" style="${i?"color: #03dac6;":""}"><i class='bx ${D}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${T}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px; color: #fff;">${H(f)}</div>
                        ...
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${H(C)}</div>
                        </div>
                        ${R}
                        ${z}
                    </div>
                </div>
            `;let k=S.querySelector(".blueprint-approve-btn");k&&(k.onclick=async I=>{I.stopPropagation(),k.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await A(`/events/${p.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ne(!0)}catch(O){console.error("Failed to approve blueprint:",O)}});let E=S.querySelector(".blueprint-delete-btn");E&&(E.onclick=async I=>{I.stopPropagation();let O=!i;E.innerHTML=O?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await A(`/events/${p.id}`,{method:"DELETE"})).ok&&ne(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let $=S.querySelector(".event-details");$&&$.addEventListener("click",I=>{I.stopPropagation()});let M=S.querySelector(".close-details-btn");return M&&M.addEventListener("click",I=>{I.stopPropagation(),S.classList.remove("expanded");let O=S.querySelector(".event-details");O&&(O.style.display="none"),se.delete(p.id)}),S},d=Array.from(e.children),c=new Map(d.map(p=>[p.dataset.blueprintId,p])),m=new Set(a.map(p=>p.id));d.forEach(p=>{let b=p.dataset.blueprintId;(!b||!m.has(b))&&p.remove()});let y=null;a.forEach((p,b)=>{let n=c.get(p.id);(!n||t)&&(n&&n.remove(),n=g(p),!n)||(b===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),oe(2,a.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=B("offline","Failed to load blueprints.","The event service may be offline."))}}function Nt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{at.forEach(r=>se.add(r.id)),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{se.clear(),ne(!0)},e.dataset.listenerAttached="true")}var ot=()=>`
...
`,Rt=null;async function Te(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Te(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true");try{let r=await Ye("/contacts");if(!r.ok)throw new Error("Service unreachable");let s=(await r.json()).members||[];if(Rt=Date.now(),s.length===0){t.innerHTML=B("empty","No contacts found.","Check your Discord connection.");return}let a={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((g,d)=>{let c=a[g.level]??10,m=a[d.level]??10;return c!==m?c-m:g.username.localeCompare(d.username)}),t.innerHTML=s.map(g=>{let d=g.color?"#"+g.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",c=g.status==="online"?"#5eff5e":g.status==="idle"?"#ffa500":g.status==="dnd"?"#ff4d4d":"#666",m="#888";g.level==="Me"||g.level==="Master"?m="#bb86fc":g.level==="Admin"?m="#cf6679":g.level==="Moderator"?m="#03dac6":g.level==="Contributor"&&(m="#ffa500");let y=g.level==="Me",p=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${d}33`;return`
...
`,xe=null,X=new Set,ce=[],P="all",Pt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},zt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Ut(t){for(let[e,r]of Object.entries(Pt))if(r.includes(t))return e;return"system"}function qt(t){return zt[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ft();let o=`/events?ml=${P==="all"?100:250}&format=json`;P!=="all"&&(o+=`&category=${P}`);try{let s=await A(o);if(!s.ok)throw new Error("Service unreachable");if(ce=((await s.json()).events||[]).filter(b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let f=n.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),xe=Date.now(),U(1,xe),ce.length===0){e.innerHTML=B("empty","No events found for this filter.");return}t&&(e.innerHTML="");let d=b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let f=n.type,C=Ut(f),h=qt(f),l=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),x="event-border-grey";l&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?x="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="system.analysis.audit"?x="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?x="event-border-orange":f==="system.test.completed"?x=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":x="event-border-blue");let i=l?"cursor-pointer":"",u=X.has(b.id),v=u?"expanded":"",L=u?"display: block;":"display: none;",w=new Date(b.timestamp*1e3),T=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=lt(f,n),D=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",R="";if(l){let k="";if(f==="engagement.decision"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                    ...
                    `}else if(f==="system.attention.expired"){let E=I=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${I}</h5>`,$=n.timestamp-n.last_active,M=Math.floor($/60);k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="moderation.explicit_content.deleted"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="analysis.link.completed"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.cli.command"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.analysis.audit"){let E=n.success?"#03dac6":"#ff4d4d",$=n.success?"SUCCESS":"FAILED",M=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,I="";n.error&&(I=`
                            <div class="event-detail-block">
                                ${M("Error")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05);">
                            ...
                            <div style="flex: 1; min-width: 150px;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${E}; font-weight: bold;">${$} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${n.attempts} att)</span></div>
                            </div>
                    ...
                    `}else if(f==="system.test.completed"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="error_occurred"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="system.cli.status"){let E=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(f==="messaging.user.sent_message"){let E="";n.attachments&&n.attachments.length>0&&(E=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(M=>{let I=M.content_type&&M.content_type.startsWith("image/"),O=(M.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${I?`<div class="attachment-thumb-container"><img src="${M.url}" alt="${M.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${M.url}" target="_blank" class="attachment-link">${M.filename}</a>
                                        <span class="attachment-meta">${M.content_type} \u2022 ${O}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let N=document.createElement("div");N.className=`event-item ${x} ${i} ${v}`,N.dataset.eventId=b.id,N.onclick=function(k){if(!l)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(b.id);let $=this.querySelector(".event-details");$&&($.style.display="none")}else{this.classList.add("expanded"),X.add(b.id);let $=this.querySelector(".event-details");$&&($.style.display="block")}},N.innerHTML=`
                <div class="event-time">
            ...
            `;let z=N.querySelector(".event-details");if(z&&z.addEventListener("click",k=>{k.stopPropagation()}),l){let k=N.querySelector(".close-details-btn");k&&k.addEventListener("click",E=>{E.stopPropagation();let $=E.target.closest(".event-item");if($){$.classList.remove("expanded"),X.delete(b.id);let M=$.querySelector(".event-details");M&&(M.style.display="none")}})}return N},c=Array.from(e.children),m=new Map(c.map(b=>[b.dataset.eventId,b])),y=new Set(ce.map(b=>b.id));c.forEach(b=>{let n=b.dataset.eventId;(!n||!y.has(n))&&b.remove()});let p=null;ce.forEach((b,n)=>{let f=m.get(b.id);(!f||t)&&(f&&f.remove(),f=d(b),!f)||(n===0?e.firstElementChild!==f&&e.prepend(f):p&&p.nextElementSibling!==f&&p.after(f),p=f)}),xe=Date.now(),U(1,xe)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=B("offline","Failed to load events.","The event service may be offline."))}}function Ft(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),r=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ce.forEach(s=>X.add(s.id)),Q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),Q(!0)},e.dataset.listenerAttached="true"),r&&!r.dataset.listenerAttached&&(r.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{r.querySelectorAll(".filter-btn").forEach(a=>a.classList.remove("active")),s.classList.add("active"),P=s.dataset.filter,Q(!0)}}),r.dataset.listenerAttached="true"),r&&r.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===P?s.classList.add("active"):s.classList.remove("active")});let o=document.getElementById("events-clear");o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{let s=P==="all"?"ALL":P.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let a="/events";if(P!=="all"?a+=`?category=${P}`:a+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await A(a,{method:"DELETE"})).ok)throw new Error("Failed to delete events");X.clear(),Q(!0)}catch(a){console.error("Failed to clear events:",a),alert("Failed to clear events. Check console.")}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}function ct(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var jt=null;async function de(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await A("/logs");if(!e.ok)throw new Error("Logs offline");let r=await e.json();if(!r||r.length===0)return t.innerHTML=B("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=r.filter(g=>!o.includes(g.id));s.forEach(g=>{g.logs&&Array.isArray(g.logs)?g.logs.reverse():g.logs=[]}),s.reverse();let a=s.map(g=>{let d=g.logs.join(`
`),c=[...g.logs];if(c.length<25){let y=25-c.length;for(let p=0;p<y;p++)c.push("")}else c.length>25&&(c=c.slice(-25));let m=c.map(y=>fe(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=a,document.querySelectorAll(".copy-logs-btn").forEach(g=>{g.addEventListener("click",()=>{let d=unescape(g.dataset.logs);navigator.clipboard.writeText(d).then(()=>{let c=g.querySelector("i");c.classList.remove("bx-copy"),c.classList.add("bx-check"),setTimeout(()=>{c.classList.remove("bx-check"),c.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(g=>{g.addEventListener("click",async()=>{let d=g.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${d}?`))try{(await A(`/logs?service_id=${d}`,{method:"DELETE"})).ok&&de()}catch(c){console.error("Error clearing logs:",c)}})}),jt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=B("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ut=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
                ...
                </div>`}if(o&&l.CPU&&l.CPU.length>0){let x=l.CPU[0];o.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}a&&(l.GPU&&l.GPU.length>0?a.innerHTML=l.GPU.map(x=>{let i=(x.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):a.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),g&&l.STORAGE&&l.STORAGE.length>0?g.innerHTML=l.STORAGE.map(x=>{let i=(x.USED/1073741824).toFixed(1),u=(x.SIZE/(1024*1024*1024)).toFixed(1),v=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,L=x.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):g&&(g.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let l=await pt();l?(d(l),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),o&&!o.hasChildNodes())){let l=await pt();d(l)}if(!t)return;let c=await ht();if(!c||!c.services){t.children.length===0&&(t.innerHTML=B("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),U(0,dt);let m=c.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function p(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let i=parseInt(x[1])||0,u=parseInt(x[2])||0,v=parseInt(x[3])||0,L=parseFloat(x[4])||0,w=i*86400+u*3600+v*60+L,T=Math.floor(w/86400);if(T>0)return`${T}d`;let S=Math.floor(w/3600);if(S>0)return`${S}h`;let _=Math.floor(w/60);return _>0?`${_}m`:`${Math.floor(w)}s`}function f(l){let x=l.status==="online",i=x?"service-widget-online":"service-widget-offline",u=x?"bx-check-circle":"bx-x-circle",v=x?"OK":"BAD",L=l.version?p(l.version.str):"-",w=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",T=l.memory&&l.memory!=="N/A"?l.memory:"-",S=w!=="-"?"#00ff00":"#666",_=w!=="-"?"#fff":"#666",D=T!=="-"?"#00ff00":"#666",R=T!=="-"?"#fff":"#666",N=n(l.uptime),z="";return x?z=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${_};">${w}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${D};"></i>
                        <span style="color: ${R};">${T}</span>
                    </div>
                </div>`:z='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${i}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${u}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${v}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${b(l.domain&&l.port?`${l.domain}:${l.port}`:"")}</span></div>${z}</div></div>`}let C=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),h=new Set(m.map(l=>l.id));for(let[l,x]of C)h.has(l)||x.remove();m.forEach(l=>{let x=f(l),i=C.get(l.id);i?i.outerHTML!==x&&(i.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function Ie(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ht();if(!e){t.children.length===0&&(t.innerHTML=B("offline","Failed to load model status.","The event service may be offline."));return}mt=Date.now(),U(2,mt);let r=e.models||[],o=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let m=c.status==="Ready";return`
                ...
                </div>`}function g(c){let m=c.status==="Downloaded",y=m?"service-widget-online":"service-widget-offline",p=m?"OK":"MISSING",b=m&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",n=c.name;return c.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let d="";if(o&&(d+=s(o)),e.xtts&&(d+=a(e.xtts)),d+=r.map(g).join(""),!d){t.innerHTML=B("empty","No models found.");return}t.innerHTML!==d&&(t.innerHTML=d)}async function ye(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),r=document.getElementById("guardian-t2-val"),o=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),a=document.getElementById("guardian-total-active"),g=document.getElementById("guardian-total-waste"),d=document.getElementById("guardian-reset-btn");d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),ye()}catch{d.innerHTML="<i class='bx bx-error'></i> Failed"}},d.dataset.listenerAttached="true");let c=await Wt();if(c){let C=Math.floor(Date.now()/1e3),h=c.active_tier,l=v=>{v<0&&(v=0);let L=Math.floor(v/3600),w=Math.floor(v%3600/60),T=v%60;return L>0?`${L}h ${w}m`:w>0?`${w}m ${T}s`:`${T}s`},x=(v,L,w,T)=>{if(h===T)v.textContent="Working",v.style.color="#bb86fc";else if(T==="t1"&&h==="tests")v.textContent="Testing",v.style.color="#03dac6";else if(w){let _=w.next_run-C;if(_<=0)v.textContent="Ready",v.style.color="#5eff5e";else{let D=Math.floor(_/60),R=_%60;v.textContent=`${D}m ${R}s`,v.style.color="#fff"}}L&&w&&(L.innerHTML=`
        ...
        `)};e&&x(e,document.getElementById("guardian-t1-stats"),c.t1,"t1"),r&&x(r,document.getElementById("guardian-t2-stats"),c.t2,"t2");let i=document.getElementById("system-state-label"),u=document.getElementById("system-state-val");if(u&&c.system_state){let v=c.system_state,L=l(c.system_state_time||0);i&&(i.textContent=`State: ${v.toUpperCase()}`),u.textContent=L,v==="idle"?u.style.color=c.system_state_time>300?"#5eff5e":"#fff":u.style.color="#bb86fc"}s&&(s.textContent=l(c.total_idle_time||0)),a&&(a.textContent=l(c.total_active_time||0)),g&&(g.textContent=l(c.total_waste_time||0))}else[e,r,o,s,a,g].forEach(h=>{h&&(h.textContent="-",h.style.color="#666")});let m=await Gt(),y=[],p=[],b=[];m&&(Array.isArray(m)?y=m:(y=m.active||[],p=m.queue||[],b=m.history||[],b.sort((C,h)=>(h.end_time||0)-(C.end_time||0)))),y.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=B("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Le(t,y,!1));let n=document.getElementById("processes-queue-widgets");n&&(p.length===0?!n.querySelector(".tab-placeholder")&&!n.querySelector('div[style*="font-style: italic"]')&&(n.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(n.innerHTML="",Le(n,p,!1)));let f=document.getElementById("processes-history-widgets");f&&(b.length===0?f.querySelector(".tab-placeholder")||(f.innerHTML=B("empty","No recent history.")):(f.querySelector(".tab-placeholder")&&(f.innerHTML=""),Le(f,b,!0))),oe(1,y.length)}function Le(t,e,r){function o(d){let c="";d.end_time?c=`${d.end_time-d.start_time}s`:c=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let m=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",y=d.channel_id,p={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};if(p[y]?y=p[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),r)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),_e=!0,r}function wt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),_e=!1}function Et(t,e,r="output"){if(!_e)return;let o=document.createElement("div");o.className=`terminal-line terminal-${r}`,r==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=fe(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Yt(t){let e=$t.find(s=>s.id===t);if(!e)return;let r=Kt(e);Et(r,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(g=>setTimeout(g,400+Math.random()*600));let a="output";s.includes("[ERROR]")?a="error":s.includes("[SUCCESS]")||s.includes("\u2713")?a="success":s.includes("!")&&(a="warning"),Et(r,s,a)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function St(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let r=e.querySelector("code").textContent;navigator.clipboard.writeText(r).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(r=>{r.addEventListener("mouseenter",()=>{r.style.transform="translateY(-5px)",r.style.borderColor="rgba(255,255,255,0.15)",r.style.backgroundColor="rgba(255,255,255,0.05)"}),r.addEventListener("mouseleave",()=>{r.style.transform="translateY(0)",r.style.borderColor="rgba(255,255,255,0.05)",r.style.backgroundColor="rgba(255,255,255,0.03)"}),r.addEventListener("click",()=>{let o=r.dataset.cmd;Yt(o)})})}async function Xt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${$e()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Tt(){Xt(),Ve(),Re();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&St();let e=qe();Oe(e),ze();let r=document.getElementById("nav-left-container");r&&r.addEventListener("click",()=>{if(a.length>0)p();else{let i=window.location.pathname;if(!(i==="/"||i==="/index.html")){let L=(i.endsWith("/")&&i.length>1?i.slice(0,-1):i).split("/");L.pop();let w=L.join("/")||"/";window.location.href=w}}});let o=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!o&&!s&&document.querySelector("footer")?.classList.add("hide");let a=[],g=document.getElementById("windows-container");g&&g.setAttribute("data-count","0");let d=i=>{localStorage.setItem("dex_last_window",i)};function c(){return 1}function m(){for(;a.length>1;)a.shift().close(!0);let i=document.getElementById("windows-container"),u=document.querySelector("nav"),v=document.querySelector("footer"),L=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");i&&i.setAttribute("data-count",a.length),a.forEach(D=>{let R=document.getElementById(D.id);R&&R.classList.remove("hide-close")}),me(ae());let T=document.getElementById("dexter-menu-container"),S=document.getElementById("nav-window-switcher"),_=document.getElementById("settings-icon");if(Pe(a.length>0),a.length>0){if(v?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),u?.classList.add("window-open"),i&&(i.style.paddingTop="60px"),T&&(T.style.display="none"),_&&(_.style.display="none"),S){let D=a[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(D)?(S.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{d("alerts-window"),y(b)}),document.getElementById("switch-events").addEventListener("click",()=>{d("events-window"),y(n)}),document.getElementById("switch-monitor").addEventListener("click",()=>{d("monitor-window"),y(C)}),document.getElementById("switch-contacts").addEventListener("click",()=>{d("contacts-window"),y(f)}),document.getElementById("switch-workspace").addEventListener("click",()=>{d("workspace-window"),y(h)})):S.innerHTML=""}}else u?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),i&&(i.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),L||w?v?.classList.remove("hide"):v?.classList.add("hide"),T&&(T.style.display="flex"),_&&(_.style.display="block"),S&&(S.innerHTML="");re(Ke())}function y(i){if(i.isOpen()){i.close();return}for(;a.length>0;)a.pop().close(!0);a.push(i),i.open(),m()}function p(){[...a].forEach(u=>u.close()),a=[]}window.addEventListener("resize",m);let b=W({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Xe(),onOpen:()=>J(),onClose:()=>{let i=a.indexOf(b);i>-1&&a.splice(i,1),m()}}),n=W({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Ce(),onOpen:()=>{n.setContent(Ce()),Q()},onClose:()=>{let i=a.indexOf(n);i>-1&&a.splice(i,1),m()}}),f=W({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:rt(),onOpen:()=>Te(),onClose:()=>{let i=a.indexOf(f);i>-1&&a.splice(i,1),m()}}),C=W({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:ft()},{icon:"bxs-component",title:"Processes",content:gt()},{icon:"bxs-brain",title:"Models",content:bt()},{icon:"bxs-hdd",title:"Hardware",content:vt()},{icon:"bxs-terminal",title:"Logs",content:xt()},{icon:"bxs-zap",title:"Agents",content:ut()}],onOpen:()=>{ke(),ye(),Ie(),de()},onClose:()=>{let i=a.indexOf(C);i>-1&&a.splice(i,1),m()}}),h=W({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ot(),onOpen:()=>Se(),onClose:()=>{let i=a.indexOf(h);i>-1&&a.splice(i,1),m()}}),l=W({id:"settings-window",title:"Settings",icon:"bx-cog",content:he(),onOpen:()=>{l.setContent(he()),Ae(l)},onClose:()=>{let i=a.indexOf(l);i>-1&&a.splice(i,1),m()}}),x=W({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:i=>{n.isOpen()||y(n),setTimeout(()=>{let u=document.querySelector(`.event-item[data-event-id="${i}"]`);u&&(u.scrollIntoView({behavior:"smooth",block:"center"}),u.classList.add("flash-highlight"),u.classList.contains("expanded")||u.click(),setTimeout(()=>u.classList.remove("flash-highlight"),2e3))},500)},viewAlert:i=>{b.isOpen()||y(b),setTimeout(()=>{let u=document.querySelector(`.event-item[data-alert-id="${i}"]`);u&&(u.scrollIntoView({behavior:"smooth",block:"center"}),u.classList.add("flash-highlight"),u.classList.contains("expanded")||u.click(),setTimeout(()=>u.classList.remove("flash-highlight"),2e3))},500)}},e){let i=document.getElementById("dexter-dropdown");i&&(i.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let u=document.getElementById("dexter-menu-container"),v=document.getElementById("dexter-menu-btn");u&&i&&v&&(u.addEventListener("mouseenter",()=>{i.classList.add("active"),v.classList.add("active")}),u.addEventListener("mouseleave",()=>{i.classList.remove("active"),v.classList.remove("active")}),v.addEventListener("click",L=>{L.stopPropagation();let w=localStorage.getItem("dex_last_window")||"alerts-window",T=null;w==="alerts-window"?T=b:w==="events-window"?T=n:w==="monitor-window"?T=C:w==="contacts-window"?T=f:w==="workspace-window"&&(T=h),T&&y(T)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{d("alerts-window"),y(b)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{d("events-window"),y(n)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{d("monitor-window"),y(C)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{d("contacts-window"),y(f)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{d("workspace-window"),y(h)}),document.getElementById("settings-icon")?.addEventListener("click",()=>y(l)),document.getElementById("close-all-windows")?.addEventListener("click",()=>p()),setInterval(()=>{b.isOpen()?J():Qe(),n.isOpen()&&Q(),C.isOpen()&&yt(),h.isOpen()&&Se()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{x.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",i=>{i.preventDefault();try{je(document.getElementById("email-input").value),window.location.reload()}catch(u){let v=document.getElementById("login-error");v&&(v.textContent=u.message,v.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Tt):Tt();})();

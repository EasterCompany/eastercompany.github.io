(()=>{function _e(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ie(t=!1){let a=`
        <div class="nav-left">
    ...
    `,i=document.createElement("nav");i.innerHTML=a,document.body.prepend(i)}function Ae(){let t=`
...
`,R=new Set,je=[],J=null;async function F(t=!1){let e=document.getElementById("roadmap-list");if(e){vt();try{let a=await L("/roadmap");if(!a.ok)throw new Error("Offline");let i=await a.json();if(je=i,i.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let o=f=>{let y=R.has(f.id),p=f.state==="draft",m=f.state==="published",n=f.state==="consumed",s="event-border-grey";m&&(s="event-border-blue"),n&&(s="event-border-purple");let b=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${s} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=f.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",R.delete(f.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",R.add(f.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;return h.innerHTML=`
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${S(f.content)}</div>
...
`,ce=null,X=new Set,te=[],de="all",Et={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},$t={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Ye(t){for(let[e,a]of Object.entries(Et))if(a.includes(t))return e;return"system"}function Lt(t){return $t[t]||"bx-square-rounded"}async function Z(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Tt();let i=`/events?ml=${de==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let o=await L(i);if(!o.ok)throw new Error("Service is offline or unreachable.");let c=(await o.json()).events||[],g=c;if(de!=="all"&&(g=c.filter(n=>{let s=n.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!1}return Ye(s.type)===de})),te=g.slice(0,50),ce=Date.now(),D(1,ce),te.length===0){e.innerHTML=T("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=n=>{let s=n.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let l=s.type,b=Ye(l),h=Lt(l),w=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),d="event-border-grey";w&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?d="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?d="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?d="event-border-orange":l==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let v=w?"cursor-pointer":"",x=X.has(n.id),C=x?"expanded":"",$=x?"display: block;":"display: none;",E=new Date(n.timestamp*1e3),N=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),W=Je(l,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",P="";if(w){let k="";if(l==="engagement.decision")k=`
                    ...
                    `;else if(l==="messaging.bot.sent_message")k=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="moderation.explicit_content.deleted")k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${S(s.raw_output)||"None"}</pre>
                        </div>
                    `;else if(l==="analysis.link.completed")k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row">
                            ...
                            <span class="detail-value">${S(s.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Description:</span>
                            <pre class="detail-pre">${S(s.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Content Summary:</span>
                            <pre class="detail-pre">${S(s.summary)||"None"}</pre>
                        </div>
                    `;else if(l==="analysis.visual.completed"){let I="";s.base64_preview?I=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(I=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(l==="system.cli.command")k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${S(s.output)||"No output recorded."}</pre>
                        </div>
                    `;else if(l==="system.analysis.audit")k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${S(s.raw_output)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Raw Input (Prompt):</span>
                            <pre class="detail-pre">${S(s.raw_input)}</pre>
                        </div>
                    `;else if(l==="system.test.completed")k=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="error_occurred")k=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${S(s.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Context:</span>
                            <pre class="detail-pre">${JSON.stringify(s.context||{},null,2)}</pre>
                        </div>
                    `;else if(l==="system.cli.status")k=`
                        <div class="event-detail-row">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${S(s.message)}</pre>
                        </div>
                    `;else if(l==="messaging.user.sent_message"){let I="";s.attachments&&s.attachments.length>0&&(I=`
                            <div class="event-detail-block">
                ...
                `}let A=document.createElement("div");if(A.className=`event-item ${d} ${v} ${C}`,A.dataset.eventId=n.id,A.onclick=function(k){if(!w)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(n.id);let O=this.querySelector(".event-details");O&&(O.style.display="none")}else{this.classList.add("expanded"),X.add(n.id);let O=this.querySelector(".event-details");O&&(O.style.display="block")}},A.innerHTML=`
                <div class="event-time">
...
`);return`
    ...
    `;async function we(){await Promise.all([at(),_t(),It()]),await xe()}var Qe=null,et=null,tt=null;async function it(){try{return await(await L("/system_monitor")).json()}catch{return null}}async function st(){try{return await(await L("/system/hardware")).json()}catch{return null}}async function St(){try{return await(await L("/processes")).json()}catch{return null}}async function Ct(){try{return await(await L("/analyst/status")).json()}catch{return null}}async function _t(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),i=n=>{if(!n){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let s="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(s+=`

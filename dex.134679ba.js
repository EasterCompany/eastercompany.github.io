...
`,ke=null,se=new Set,Te=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;jt(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await M(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];ke=Date.now(),W(0,ke);let l=Date.now(),m=24*60*60*1e3,p=i.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let y=parseInt(d);return l-y<m});p.sort((r,d)=>{let y=_=>{let C=_.event;if(typeof C=="string")try{C=JSON.parse(C)}catch{return"low"}return(C.priority||"low").toLowerCase()},S=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,T=S(y(r)),$=S(y(d));return T!==$?$-T:d.timestamp-r.timestamp}),Te=p;let g=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(p.map(r=>g(r))).size>1;if(p.length>0){let r=null;p.forEach(d=>{let y=g(d);b&&y!==r&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),r=y),x.push(d)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=B("empty","No alerts yet."),$e();return}let s=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let y=(d.title||"Untitled Alert").trim(),S=(d.body||"No description provided.").trim(),T=d.summary||"",$=d.content||"",_=d.protocol||"",C=(d.priority||"low").toLowerCase(),N=!!d.alert,P=(d.category||"system").trim(),H=d.related_event_ids||[],R=d.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${r.id}`),k=new Date(r.timestamp*1e3),w=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),E=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=q?"event-border-grey":"event-border-blue";!q&&N&&(A="event-border-red"),q&&(C==="high"||C==="critical")?A="event-border-red":q&&C==="medium"&&(A="event-border-orange");let L=q?"alert-read":"alert-unread",z=se.has(r.id),U=z?"expanded":"",he=z?"display: block;":"display: none;",F="",j="";H.length>0&&(j=`
        ...
        `,F=`
                ...
                ${V}
            `;let G=document.createElement("div");G.className=`event-item notification-item ${A} ${L} ${U} cursor-pointer priority-${C}`,G.dataset.alertId=r.id,G.onclick=function(Q){if(this.classList.contains("expanded")){this.classList.remove("expanded"),se.delete(r.id);let fe=this.querySelector(".event-details");fe&&(fe.style.display="none")}else{this.classList.add("expanded"),se.add(r.id);let fe=this.querySelector(".event-details");if(fe&&(fe.style.display="block"),!localStorage.getItem(`alert_read_ts_${r.id}`)){localStorage.setItem(`alert_read_ts_${r.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Me="event-border-grey";C==="high"||C==="critical"?Me="event-border-red":C==="medium"&&(Me="event-border-orange"),this.classList.add(Me),$e()}}};let qe=`${_?_.toUpperCase():"GUARDIAN"} ALERT: ${T||y}`,Nt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[P]||"bx-bell",zt=C==="high"||C==="critical"?"#ff4d4d":C==="medium"?"#ffa500":"#888";G.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${zt}; font-size: 0.8em; margin-left: 5px;">[${C.toUpperCase()}]</span>
                    </div>
...
`,pt=null,oe=new Set,gt=[];async function re(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Vt();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await M(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];if(gt=i,pt=Date.now(),W(2,pt),i.length===0){e.innerHTML=B("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ve(2,0);return}t&&(e.innerHTML="");let l=f=>{let b=f.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let s=(b.title||"Untitled Blueprint").trim(),u=(b.summary||b.body||"No summary provided.").trim(),I=(b.content||"").trim(),h=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(L=>L.trim()),v=(b.implementation_path||[]).map(L=>L.trim()),r=b.source_event_ids||[],d=b.approved===!0,y=new Date(f.timestamp*1e3),S=y.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=y.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=oe.has(f.id),_=$?"display: block;":"display: none;",C=document.createElement("div");C.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${d?"blueprint-approved":""}`,C.dataset.blueprintId=f.id,d&&(C.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",C.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let P=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";C.onclick=function(L){if(this.classList.contains("expanded")){this.classList.remove("expanded"),oe.delete(f.id);let U=this.querySelector(".event-details");U&&(U.style.display="none")}else{this.classList.add("expanded"),oe.add(f.id);let U=this.querySelector(".event-details");U&&(U.style.display="block")}};let H="";v.length>0&&(H=`
            ...
            `;let k=C.querySelector(".blueprint-approve-btn");k&&(k.onclick=async L=>{L.stopPropagation(),k.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await M(`/events/${f.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&re(!0)}catch(z){console.error("Failed to approve blueprint:",z)}});let w=C.querySelector(".blueprint-delete-btn");w&&(w.onclick=async L=>{L.stopPropagation();let z=!d;w.innerHTML=z?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await M(`/events/${f.id}`,{method:"DELETE"})).ok&&re(!0)}catch(U){console.error("Failed to delete blueprint:",U)}});let E=C.querySelector(".event-details");E&&E.addEventListener("click",L=>{L.stopPropagation()});let A=C.querySelector(".close-details-btn");return A&&A.addEventListener("click",L=>{L.stopPropagation(),C.classList.remove("expanded");let z=C.querySelector(".event-details");z&&(z.style.display="none"),oe.delete(f.id)}),C},m=Array.from(e.children),p=new Map(m.map(f=>[f.dataset.blueprintId,f])),g=new Set(i.map(f=>f.id));m.forEach(f=>{let b=f.dataset.blueprintId;(!b||!g.has(b))&&f.remove()});let x=null;i.forEach((f,b)=>{let s=p.get(f.id);(!s||t)&&(s&&s.remove(),s=l(f),!s)||(b===0?e.firstElementChild!==s&&e.prepend(s):x&&x.nextElementSibling!==s&&x.after(s),x=s)}),ve(2,i.length),it()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=B("offline","Failed to load blueprints.","The event service may be offline."))}}async function ft(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await M(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(i=>{let l=i.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),nt(n)}catch{}}function Vt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),o=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{gt.forEach(a=>oe.add(a.id)),re(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{oe.clear(),re(!0)},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await M("/events?type=system.blueprint.generated",{method:"DELETE"}),oe.clear(),re(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}var bt=()=>`
...
`,Le=null,le=new Set,xe=[],O="all",Xt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision"],moderation:["moderation.explicit_content.deleted"]},Zt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Qt(t){for(let[e,o]of Object.entries(Xt))if(o.includes(t))return e;return"system"}function es(t){return Zt[t]||"bx-square-rounded"}async function ce(t=!1){let e=document.getElementById("events-timeline");if(!e)return;ts();let a=`/events?ml=${O==="all"?100:250}&format=json`;O!=="all"&&(a+=`&category=${O}`);try{let n=await M(a);if(!n.ok)throw new Error("Service unreachable");if(xe=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Le=Date.now(),W(1,Le),xe.length===0){e.innerHTML=B("empty","No events found for this filter.");return}t&&(e.innerHTML="");let m=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,I=Qt(u),h=es(u),c=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),v="event-border-grey";c&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?v="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"?v="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?v="event-border-orange":u==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=le.has(b.id),y=d?"expanded":"",S=d?"display: block;":"display: none;",T=new Date(b.timestamp*1e3),$=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=xt(u,s),N=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",P="";if(c){let k="";if(u==="engagement.decision"){let w=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;k=`
                    ...
                    `}else if(u==="system.analysis.audit"){let w=s.success?"#03dac6":"#ff4d4d",E=s.success?"SUCCESS":"FAILED",A=F=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${F}</h5>`,L="";s.error&&(L=`
                        ...
                        `);let z="";if(s.chat_history&&s.chat_history.length>0){let F=s.chat_history.length,j=s.chat_history.map((Z,de)=>{let V=Z.role.toUpperCase();V==="USER"&&(V="SYSTEM"),V==="ASSISTANT"&&(V="AGENT");let G=Z.role==="user"?"#03dac6":Z.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${de}" style="display: ${de===0?"block":"none"};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${D(Z.content)}</div>
                                </div>
                            `}).join("");z=`
                            <div class="event-detail-block">
                        ...
                        `}let U="";if(s.corrections&&s.corrections.length>0){let F=s.corrections.map((j,Z)=>`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${yt()}
        </div>`;async function It(){await Promise.all([Ie(),Re(),Pe()]),await ye()}var ht=null,wt=null;async function At(){try{return await(await M("/system_monitor")).json()}catch{return null}}async function Ct(){try{return await(await M("/system/hardware")).json()}catch{return null}}async function ns(){try{return await(await M("/processes")).json()}catch{return null}}async function is(){try{return await(await M("/agent/status")).json()}catch{return null}}async function Re(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),l=document.querySelector("#hw-storage .hw-content"),m=c=>{if(c){if(o&&(o.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await Ct();c?(m(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await Ct();m(c)}if(!t)return;let p=await At();if(!p||!p.services){t.children.length===0&&(t.innerHTML=B("offline","Failed to load system metrics.","The event service may be offline."));return}ht=Date.now(),W(0,ht);let g=p.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function x(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function f(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function b(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function s(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let r=parseInt(v[1])||0,d=parseInt(v[2])||0,y=parseInt(v[3])||0,S=parseFloat(v[4])||0,T=r*86400+d*3600+y*60+S,$=Math.floor(T/86400);if($>0)return`${$}d`;let _=Math.floor(T/3600);if(_>0)return`${_}h`;let C=Math.floor(T/60);return C>0?`${C}m`:`${Math.floor(T)}s`}function u(c){let v=c.status==="online",r=v?"service-widget-online":"service-widget-offline",d=v?"bx-check-circle":"bx-x-circle",y=v?"OK":"BAD",S=c.version?f(c.version.str):"-",T=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",$=c.memory&&c.memory!=="N/A"?c.memory:"-",_=T!=="-"?"#00ff00":"#666",C=T!=="-"?"#fff":"#666",N=$!=="-"?"#00ff00":"#666",P=$!=="-"?"#fff":"#666",H=s(c.uptime),R="";return v?R=`

...
`,oe=null,q=new Set,le=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;gt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await $(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];oe=Date.now(),H(0,oe);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let x=parseInt(v);return c-x<g});u.sort((d,v)=>{let x=A=>{let _=A.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},S=A=>A==="critical"?4:A==="high"?3:A==="medium"?2:1,E=S(x(d)),L=S(x(v));return E!==L?L-E:v.timestamp-d.timestamp}),le=u;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},y=[],m=new Set(u.map(d=>f(d))).size>1;if(u.length>0){let d=null;u.forEach(v=>{let x=f(v);m&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(v)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=T("empty","No notifications yet."),ie();return}let n=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let x=v.title||"Untitled Notification",S=v.body||"No description provided.",E=v.priority||"low",L=!!v.alert,A=v.category||"system",_=v.related_event_ids||[],N=!!localStorage.getItem(`notification_read_ts_${d.id}`),P=new Date(d.timestamp*1e3),M=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=N?"event-border-grey":"event-border-blue";!N&&L&&(I="event-border-red"),N&&(E==="high"||E==="critical")?I="event-border-red":N&&E==="medium"&&(I="event-border-orange");let O=N?"notification-read":"notification-unread",B=q.has(d.id),pe=B?"expanded":"",me=B?"display: block;":"display: none;",ke="",Ce="";_.length>0&&(Ce=`
...
`);return`
    ...
    `;async function we(){await Promise.all([at(),_t(),At()]),await xe()}var Qe=null,et=null,tt=null;async function it(){try{return await(await $("/system_monitor")).json()}catch{return null}}async function st(){try{return await(await $("/system/hardware")).json()}catch{return null}}async function Ct(){try{return await(await $("/processes")).json()}catch{return null}}async function St(){try{return await(await $("/analyst/status")).json()}catch{return null}}async function _t(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),i=n=>{if(!n){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let s="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(s+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`}e.innerHTML=s};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let s=await st();s?(i(s),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML='<p style="color: #ff4d4d;">Failed to refresh.</p>',a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let s=await st();i(s)}}if(!t)return;let o=await it();if(!o||!o.services){t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}Qe=Date.now(),H(3,Qe);let r=o.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function g(n){if(!n||n==="N/A"||n==="unknown")return"-";let s=n.match(/^(\d+\.\d+\.\d+)/);return s?s[0]:n.split(".").slice(0,3).join(".")||"-"}function u(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function f(n){if(!n||n==="N/A"||n==="unknown")return"-";let s=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!s)return"-";let l=parseInt(s[1])||0,b=parseInt(s[2])||0,h=parseInt(s[3])||0,w=parseFloat(s[4])||0,d=l*86400+b*3600+h*60+w,v=Math.floor(d/86400);if(v>0)return`${v}d`;let x=Math.floor(d/3600);if(x>0)return`${x}h`;let S=Math.floor(d/60);return S>0?`${S}m`:`${Math.floor(d)}s`}function y(n){let s=n.status==="online",l=s?"service-widget-online":"service-widget-offline",b=s?"bx-check-circle":"bx-x-circle",h=s?"OK":"BAD",w=n.version?g(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",v=n.memory&&n.memory!=="N/A"?n.memory:"-",x=d!=="-"?"#00ff00":"#666",S=d!=="-"?"#fff":"#666",E=v!=="-"?"#00ff00":"#666",L=v!=="-"?"#fff":"#666",A=f(n.uptime),_="";return s?_=`

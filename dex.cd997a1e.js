(()=>{function Ae(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function _e(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",o=t?`
        ...
        ${o}
    `,m=document.createElement("nav");if(m.innerHTML=r,document.body.prepend(m),!a){let v=document.getElementById("nav-back-btn");v&&(v.addEventListener("click",()=>{let p=window.location.pathname,y=(p.endsWith("/")&&p.length>1?p.slice(0,-1):p).split("/");y.pop();let g=y.join("/")||"/";window.location.href=g}),v.addEventListener("mouseenter",()=>{v.style.transform="translateX(-3px)"}),v.addEventListener("mouseleave",()=>{v.style.transform="translateX(0)"}))}}function Me(){let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var De=1e3;function F(t){let e=null,a=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++De)}function r(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",m),o&&setTimeout(o,10);return}let g=document.getElementById("windows-container");g||(g=document.createElement("div"),g.id="windows-container",g.className="windows-container",document.body.appendChild(g)),e=document.createElement("div"),e.id=t.id,e.className="window",e.style.zIndex=++De,e.addEventListener("mousedown",s);let u=t.icon||"bx-window",n="",i="",l;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((x,c)=>{let f=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,l=`<div class="window-content">${t.tabs.map((x,c)=>`<div class="tab-content ${c===0?"active":""}" data-tab-content="${c}">${x.content}</div>`).join("")}</div>`):(t.title&&(i=`<div class="window-title">${t.title}</div>`),l=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${l}
        `,g.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",w=>{w.stopPropagation(),p()}),window.addEventListener("resize",m),t.tabs&&e.querySelectorAll(".tab").forEach(w=>{w.addEventListener("click",()=>{let d=w.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(x=>x.classList.remove("active")),w.classList.add("active"),e.querySelectorAll(".tab-content").forEach(x=>x.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${d}"]`).classList.add("active"),v(w,e)})}),setTimeout(()=>{e.classList.add("open"),o&&o()},10)}function m(){if(!e||!e.classList.contains("open"))return;let g=e.querySelector(".tab.active");g&&v(g,e)}function v(g,u){setTimeout(()=>{let n=u.querySelector(".tab-bar");if(!n)return;let i=Array.from(n.querySelectorAll(".tab")),l=i.indexOf(g),w=n.clientWidth,d=i[Math.max(0,l-2)],x=i[Math.min(i.length-1,l+2)],c=d.offsetLeft-n.offsetLeft-25,h=x.offsetLeft+x.offsetWidth-n.offsetLeft+25-c,C=h<=w?c-(w-h)/2:g.offsetLeft-n.offsetLeft-w/2+g.offsetWidth/2;n.scrollTo({left:C,behavior:"smooth"})},350)}function p(g=!1){e&&(window.removeEventListener("resize",m),g?(e.remove(),e=null):(e.classList.remove("open"),a&&a(),setTimeout(()=>{e?.remove(),e=null},400)))}function b(g){let u=e?.querySelector(".window-content");u&&(u.innerHTML=g)}function y(){return e&&e.classList.contains("open")}return{open:r,close:p,setContent:b,isOpen:y,focus:s,id:t.id}}function Be(){return!0}function Ne(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function He(t){localStorage.setItem("easter_user_email",t.trim())}var Oe="easter_theme",R={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function te(){return localStorage.getItem(Oe)||R.DARK}function Re(t){if(!Object.values(R).includes(t))throw new Error("Invalid theme");localStorage.setItem(Oe,t),ie(t)}function ie(t){let e=document.body;if(Object.values(R).forEach(o=>{e.classList.remove(`theme-${o}`)}),e.classList.add(`theme-${t}`),[R.LIGHT,R.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let o=document.createElement("div");o.className="background",document.body.prepend(o)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function Pe(){let t=te();ie(t)}function k(t,e,a=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=a?`<p class="placeholder-action">${a}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function I(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function B(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!a)return;if(!e){a.textContent="Last updated: never";return}let o=Date.now(),s=Math.floor((o-e)/1e3),r;s<60?r=`${s}s ago`:s<3600?r=`${Math.floor(s/60)}m ago`:r=`${Math.floor(s/3600)}h ago`,a.textContent=`Last updated: ${r}`}function P(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!a)return;let o=a.querySelector(".notification-badge");o&&(e>0?(o.textContent=e>9?"9+":e,o.style.display="flex"):o.style.display="none")}function le(){let t=document.getElementById("notifications-list");if(!t)return;let e=t.querySelectorAll(".notification-unread").length;P(0,e)}function ye(){return"https://event.easter.company"}function xt(){return"https://discord.easter.company"}var ht="http://127.0.0.1:8100",yt="http://127.0.0.1:8300",wt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function re(t){let e=I(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,r)=>{let m=wt[r];return m?`<span class="${m}">`:""});let a=(e.match(/<span/g)||[]).length,o=(e.match(/<\/span/g)||[]).length;return a>o&&(e+="</span>".repeat(a-o)),e}var V=null,K=null,ae=!1,oe=!1;async function A(t,e={}){if(V)try{let s=await fetch(V+t,e);if(s.ok)return s;V=null}catch{V=null}let a=ye(),o=ht;try{let s=await fetch(a+t,e);if(s.ok)return V=a,ae&&(console.log("\u2728 Production event service restored."),ae=!1),s;throw new Error("Primary failed")}catch{ae||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${o}`),ae=!0);try{let r=await fetch(o+t,e);if(r.ok)return V=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}async function Ue(t,e={}){if(K)try{let s=await fetch(K+t,e);if(s.ok)return s;K=null}catch{K=null}let a=xt(),o=yt;try{let s=await fetch(a+t,e);if(s.ok)return K=a,oe&&(console.log("\u2728 Production discord service restored."),oe=!1),s;throw new Error("Primary failed")}catch{oe||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${o}`),oe=!0);try{let r=await fetch(o+t,e);if(r.ok)return K=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}var ze=()=>`
...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Et(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),B(0,ce);let m=Date.now(),v=24*60*60*1e3,p=r.filter(c=>{let f=localStorage.getItem(`notification_read_ts_${c.id}`);if(!f)return!0;let h=parseInt(f);return m-h<v});p.sort((c,f)=>{let h=_=>{let L=_.event;if(typeof L=="string")try{L=JSON.parse(L)}catch{return"low"}return(L.priority||"low").toLowerCase()},C=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=C(h(c)),S=C(h(f));return E!==S?S-E:f.timestamp-c.timestamp}),de=p;let b=c=>{let f=c.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],u=new Set(p.map(c=>b(c))).size>1;if(p.length>0){let c=null;p.forEach(f=>{let h=b(f);u&&h!==c&&(y.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),c=h),y.push(f)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=k("empty","No notifications yet."),le();return}let n=c=>{let f=c.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let h=f.title||"Untitled Notification",C=f.body||"No description provided.",E=f.priority||"low",S=!!f.alert,_=f.category||"system",L=f.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${c.id}`),U=new Date(c.timestamp*1e3),M=U.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=U.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=H?"event-border-grey":"event-border-blue";!H&&S&&(T="event-border-red"),H&&(E==="high"||E==="critical")?T="event-border-red":H&&E==="medium"&&(T="event-border-orange");let N=H?"notification-read":"notification-unread",D=q.has(c.id),fe=D?"expanded":"",be=D?"display: block;":"display: none;",Te="",ke="";L.length>0&&(ke=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${L.map(Z=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${Z.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),Te=`
                <div class="event-detail-row">
                ...
                ${ke}
            `;let z=document.createElement("div");z.className=`event-item notification-item ${T} ${N} ${fe} cursor-pointer`,z.dataset.notificationId=c.id,z.onclick=function(Z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),q.delete(c.id);let ee=this.querySelector(".event-details");ee&&(ee.style.display="none")}else{this.classList.add("expanded"),q.add(c.id);let ee=this.querySelector(".event-details");if(ee&&(ee.style.display="block"),!localStorage.getItem(`notification_read_ts_${c.id}`)){localStorage.setItem(`notification_read_ts_${c.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let he="event-border-grey";E==="high"||E==="critical"?he="event-border-red":E==="medium"&&(he="event-border-orange"),this.classList.add(he),le()}}};let zt=h,bt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[_]||"bx-bell";z.innerHTML=`
            ...
            `;let Ie=z.querySelector(".close-details-btn");return Ie&&Ie.addEventListener("click",Z=>{Z.stopPropagation(),z.classList.remove("expanded");let xe=z.querySelector(".event-details");xe&&(xe.style.display="none"),q.delete(c.id)}),z},i=c=>{let f=document.createElement("div");f.className="notification-divider",f.dataset.notificationId=c.id;let h="#888888";return c.label==="CRITICAL"?h="#ff4d4d":c.label==="HIGH"?h="#ff8888":c.label==="MEDIUM"&&(h="#ffa500"),f.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,f.innerHTML=`<span style="white-space: nowrap;">${c.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,f},l=Array.from(e.children),w=new Map(l.map(c=>[c.dataset.notificationId,c])),d=new Set(y.map(c=>c.id));l.forEach(c=>{let f=c.dataset.notificationId;(!f||!d.has(f))&&c.remove()});let x=null;y.forEach((c,f)=>{let h=w.get(c.id);(!h||t)&&(h&&h.remove(),c.type==="divider"?h=i(c):h=n(c),!h)||(f===0?e.firstElementChild!==h&&e.prepend(h):x&&x.nextElementSibling!==h&&x.after(h),x=h)}),ce=Date.now(),B(0,ce),le()}catch(o){console.error("Error fetching notifications:",o),e.children.length===0&&(e.innerHTML=k("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function Et(){let t=document.getElementById("notif-read-all"),e=document.getElementById("notif-expand-all"),a=document.getElementById("notif-close-all"),o=document.getElementById("notif-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{de.forEach(s=>{localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.forEach(s=>{q.add(s.id),localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{q.clear(),j(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let s=Date.now()-1728e5;de.forEach(r=>{localStorage.setItem(`notification_read_ts_${r.id}`,s.toString())}),q.clear(),j(!0)},o.dataset.listenerAttached="true")}var Fe=()=>`
  <div class="notifications-actions">
...
`,W=new Set,qe=[],J=null;async function G(t=!1){let e=document.getElementById("roadmap-list");if(e){St();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(qe=o,o.length===0){e.innerHTML=k("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=b=>{let y=W.has(b.id),g=b.state==="draft",u=b.state==="published",n=b.state==="consumed",i="event-border-grey";u&&(i="event-border-blue"),n&&(i="event-border-purple");let w=new Date(b.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div");d.className=`event-item notification-item ${i} cursor-pointer ${y?"expanded":""}`,d.dataset.itemId=b.id,d.onclick=c=>{if(c.target.closest("button"))return;d.classList.contains("expanded")?(d.classList.remove("expanded"),d.querySelector(".event-details").style.display="none",W.delete(b.id)):(d.classList.add("expanded"),d.querySelector(".event-details").style.display="block",W.add(b.id))};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${b.state.toUpperCase()}]</span>`;return d.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${I(b.content)}</div>
          <div class="event-details" style="${y?"display: block;":"display: none;"} ">
            ...
            ${n?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(b.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `,d.querySelector(".edit-btn")?.addEventListener("click",()=>Lt(b)),d.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>$t(b)),d.querySelector(".delete-btn")?.addEventListener("click",()=>Ct(b.id)),d.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),d.classList.remove("expanded"),d.querySelector(".event-details").style.display="none",W.delete(b.id)}),d},r=Array.from(e.children),m=new Map(r.map(b=>[b.dataset.itemId,b])),v=new Set(o.map(b=>b.id));r.forEach(b=>{let y=b.dataset.itemId;(!y||!v.has(y))&&b.remove()});let p=null;o.forEach((b,y)=>{let g=m.get(b.id);(!g||t)&&(g&&g.remove(),g=s(b),!g)||(y===0?e.firstElementChild!==g&&e.prepend(g):p&&p.nextElementSibling!==g&&p.after(g),p=g)})}catch{e.innerHTML=k("error","Failed to load roadmap.")}}}function St(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{J=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",J=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let m=J?`/roadmap/${J}`:"/roadmap",v=J?"PATCH":"POST";try{await A(m,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",G(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{qe.forEach(r=>W.add(r.id)),G(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{W.clear(),G(!0)},s.dataset.listenerAttached="true")}function Lt(t){J=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function $t(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),G(!0)}catch(a){console.error(a)}}async function Ct(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),W.delete(t),G(!0)}catch(e){console.error(e)}}var We=()=>`
    <div class="notifications-actions">
...
`,je=null,Y=new Set,Ge=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Tt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(Ge=r,je=Date.now(),B(2,je),r.length===0){e.innerHTML=k("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),P(1,0);return}t&&(e.innerHTML="");let m=g=>{let u=g.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let n=u.title||"Untitled Blueprint",i=u.summary||u.body||"No summary provided.",l=u.content||"",w=u.category||"architecture",d=u.affected_services||[],x=u.implementation_path||[],c=new Date(g.timestamp*1e3),f=c.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=c.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=Y.has(g.id),E=C?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${C?"expanded":""}`,S.dataset.blueprintId=g.id;let L={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";S.onclick=function(M){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(g.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),Y.add(g.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let O=d.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${d.join(", ")}</div>`:"",H="";x.length>0&&(H=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${x.map(M=>`<li>${I(M)}</li>`).join("")}
                        </ul>
                    </div>
                `),S.innerHTML=`
                <div class="event-time">
            ...
            `;let U=S.querySelector(".close-details-btn");return U&&U.addEventListener("click",M=>{M.stopPropagation(),S.classList.remove("expanded");let $=S.querySelector(".event-details");$&&($.style.display="none"),Y.delete(g.id)}),S},v=Array.from(e.children),p=new Map(v.map(g=>[g.dataset.blueprintId,g])),b=new Set(r.map(g=>g.id));v.forEach(g=>{let u=g.dataset.blueprintId;(!u||!b.has(u))&&g.remove()});let y=null;r.forEach((g,u)=>{let n=p.get(g.id);(!n||t)&&(n&&n.remove(),n=m(g),!n)||(u===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),P(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=k("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Tt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ge.forEach(a=>Y.add(a.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),me(!0)},e.dataset.listenerAttached="true")}var Ve=()=>`
...
`;async function we(){await Promise.all([G(),me()])}var Je=()=>`
    <div class="notifications-actions">
...
`,Ke=null;async function pe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>pe(),e.dataset.listenerAttached="true");try{let a=await Ue("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ke=Date.now(),B(4,Ke),s.length===0){t.innerHTML=k("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((m,v)=>{let p=r[m.level]??10,b=r[v.level]??10;return p!==b?p-b:m.username.localeCompare(v.username)}),t.innerHTML=s.map(m=>{let v=m.color?"#"+m.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=m.status==="online"?"#5eff5e":m.status==="idle"?"#ffa500":m.status==="dnd"?"#ff4d4d":"#666",b="#888";m.level==="Me"||m.level==="Master User"?b="#bb86fc":m.level==="Admin"?b="#cf6679":m.level==="Moderator"?b="#03dac6":m.level==="Contributor"&&(b="#ffa500");let y=m.level==="Me",g=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",u=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-section" style="background: ${g}; padding: 15px; border-radius: 8px; border: ${u}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    ...
                    <div style="position: relative;">
                        <img src="${m.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${y?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
...
`,ue=null,X=new Set,se=[],ge="all",It={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},At={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Xe(t){for(let[e,a]of Object.entries(It))if(a.includes(t))return e;return"system"}function _t(t){return At[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Mt();let o=`/events?ml=${ge==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await A(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let m=(await s.json()).events||[],v=m;if(ge!=="all"&&(v=m.filter(n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!1}return Xe(i.type)===ge})),se=v.slice(0,50),ue=Date.now(),B(1,ue),se.length===0){e.innerHTML=k("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let l=i.type,w=Xe(l),d=_t(l),x=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),c="event-border-grey";x&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?c="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?c="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?c="event-border-orange":l==="system.test.completed"?c=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":c="event-border-blue");let f=x?"cursor-pointer":"",h=X.has(n.id),C=h?"expanded":"",E=h?"display: block;":"display: none;",S=new Date(n.timestamp*1e3),_=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),O=Ye(l,i),H=i.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${i.user_level})</span>`:"",U="";if(x){let $="";if(l==="engagement.decision")$=`
                    ...
                    `;else if(l==="messaging.bot.sent_message")$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="moderation.explicit_content.deleted")$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="analysis.link.completed")$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="analysis.visual.completed"){let T="";i.base64_preview?T=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${i.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:i.url&&(T=`<div class="event-detail-block"><img src="${i.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="system.analysis.audit")$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="system.test.completed")$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="messaging.user.sent_message"){let T="";i.attachments&&i.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${i.attachments.map(D=>{let fe=D.content_type&&D.content_type.startsWith("image/"),be=(D.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${fe?`<div class="attachment-thumb-container"><img src="${D.url}" alt="${D.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${D.url}" target="_blank" class="attachment-link">${D.filename}</a>
                                        <span class="attachment-meta">${D.content_type} \u2022 ${be}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),$=`
                        <div class="event-detail-row">
                ...
                `}let M=document.createElement("div");if(M.className=`event-item ${c} ${f} ${C}`,M.dataset.eventId=n.id,M.onclick=function($){if(!x)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(n.id);let N=this.querySelector(".event-details");N&&(N.style.display="none")}else{this.classList.add("expanded"),X.add(n.id);let N=this.querySelector(".event-details");N&&(N.style.display="block")}},M.innerHTML=`
                <div class="event-time">
            ...
            `,x){let $=M.querySelector(".close-details-btn");$&&$.addEventListener("click",T=>{T.stopPropagation();let N=T.target.closest(".event-item");if(N){N.classList.remove("expanded"),X.delete(n.id);let D=N.querySelector(".event-details");D&&(D.style.display="none")}})}return M},b=Array.from(e.children),y=new Map(b.map(n=>[n.dataset.eventId,n])),g=new Set(se.map(n=>n.id));b.forEach(n=>{let i=n.dataset.eventId;(!i||!g.has(i))&&n.remove()});let u=null;se.forEach((n,i)=>{let l=y.get(n.id);(!l||t)&&(l&&l.remove(),l=p(n),!l)||(i===0?e.firstElementChild!==l&&e.prepend(l):u&&u.nextElementSibling!==l&&u.after(l),u=l)}),ue=Date.now(),B(1,ue)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=k("offline","Failed to load events.","The event service may be offline or unreachable."))}}function Mt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),a=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{se.forEach(o=>X.add(o.id)),Q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),Q(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.querySelectorAll(".filter-btn").forEach(o=>{o.onclick=()=>{a.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active"),ge=o.dataset.filter,Q(!0)}}),a.dataset.listenerAttached="true")}function Ze(){return`
    ...
    `}var Dt=null;async function ne(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await A("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML='<p style="text-align: center; opacity: 0.5; padding: 20px;">No logs found.</p>',!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=a.filter(m=>!o.includes(m.id));s.forEach(m=>{m.logs&&Array.isArray(m.logs)?m.logs.reverse():m.logs=[]}),s.reverse();let r=s.map(m=>{let v=m.logs.join(`
`),p=[...m.logs];if(p.length<25){let y=25-p.length;for(let g=0;g<y;g++)p.push("")}else p.length>25&&(p=p.slice(-25));let b=p.map(y=>re(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=r,document.querySelectorAll(".copy-logs-btn").forEach(m=>{m.addEventListener("click",()=>{let v=unescape(m.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let p=m.querySelector("i");p.classList.remove("bx-copy"),p.classList.add("bx-check"),setTimeout(()=>{p.classList.remove("bx-check"),p.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(m=>{m.addEventListener("click",async()=>{let v=m.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await A(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&ne()}catch(p){console.error("Error clearing logs:",p)}})}),Dt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.innerHTML='<p style="text-align: center; color: #cf6679; padding: 20px;">Failed to load logs.</p>',!1}}var it=()=>`
        ...
        <div id="processes-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"><p style="grid-column: 1 / -1; text-align: center; padding: 20px; color: #666;">Loading processes...</p></div>`,ot=()=>`
        <div class="system-section-header">
            <i class='bx bxs-server' style="color: #03dac6;"></i>
            <h2>Services</h2>
        </div>
        <div id="services-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"><p style="grid-column: 1 / -1; text-align: center; padding: 20px; color: #666;">Loading services...</p></div>`,lt=()=>`
        <div class="system-section-header">
            <i class='bx bxs-brain' style="color: #03dac6;"></i>
            <h2>Cognitive Models</h2>
        </div>
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"><p style="grid-column: 1 / -1; text-align: center; padding: 20px; color: #666;">Loading models...</p></div>`,rt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
            ...
            </div>`,n.CPU&&n.CPU.length>0){let w=n.CPU[0];i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    <div class="service-widget-body">
                        <span style="font-size: 0.9em; color: #fff; display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${w.LABEL}">${w.LABEL}</span>
                        <span style="font-size: 0.8em; color: #888;">${w.COUNT} Cores / ${w.THREADS} Threads</span>
                    </div>
                </div>`}if(n.GPU&&n.GPU.length>0&&n.GPU.forEach((w,d)=>{let x=(w.VRAM/1073741824).toFixed(1);i+=`
                    <div class="service-widget" style="padding: 10px; min-height: 80px;">
                        <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let w=0,d=0;n.STORAGE.forEach(h=>{w+=h.USED,d+=h.SIZE});let x=(w/(1024*1024*1024)).toFixed(1),c=(d/(1024*1024*1024)).toFixed(1),f=d>0?(w/d*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    <div class="service-widget-body">
                        <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${x} / ${c} GB</span>
                ...
                </div>`}e.innerHTML=i};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let i=await nt();i?(o(i),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML='<p style="color: #ff4d4d;">Failed to refresh.</p>',a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let i=await nt();o(i)}}if(!t)return;let s=await mt();if(!s||!s.services){t.innerHTML=k("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}et=Date.now(),B(3,et);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function m(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function v(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function p(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,w=parseInt(i[2])||0,d=parseInt(i[3])||0,x=parseFloat(i[4])||0,c=l*86400+w*3600+d*60+x,f=Math.floor(c/86400);if(f>0)return`${f}d`;let h=Math.floor(c/3600);if(h>0)return`${h}h`;let C=Math.floor(c/60);return C>0?`${C}m`:`${Math.floor(c)}s`}function y(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",w=i?"bx-check-circle":"bx-x-circle",d=i?"OK":"BAD",x=n.version?v(n.version.str):"-",c=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",f=n.memory&&n.memory!=="N/A"?n.memory:"-",h=c!=="-"?"#00ff00":"#666",C=c!=="-"?"#fff":"#666",E=f!=="-"?"#00ff00":"#666",S=f!=="-"?"#fff":"#666",_=b(n.uptime),L="";return i?L=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${S};">${f}</span>
                    </div>
                </div>`:L='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${w}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${p(n.domain&&n.port?`${n.domain}:${n.port}`:"")}</span></div>${L}</div></div>`}let g=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),u=new Set(r.map(n=>n.id));for(let[n,i]of g)u.has(n)||i.remove();r.forEach(n=>{let i=y(n),l=g.get(n.id);l?l.outerHTML!==i&&(l.outerHTML=i):t.insertAdjacentHTML("beforeend",i)})}async function Se(){let t=document.getElementById("models-widgets");if(!t)return;let e=await mt();if(!e){t.innerHTML=k("offline","Failed to load model status.");return}tt=Date.now(),B(3,tt);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(v=>{v.classList.contains("service-widget")||v.remove()});function s(v){let p=v.status==="Ready";return`
                ...
                </div>`}function r(v){let p=v.status==="Downloaded",b=p?"service-widget-online":"service-widget-offline",y=p?"OK":"MISSING",g=p&&v.size>0?`${(v.size/1e9).toFixed(2)} GB`:"-",u=v.name;return v.type==="custom"&&u.endsWith(":latest")&&(u=u.replace(":latest","")),`<div class="service-widget ${b}" data-model-name="${v.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${u}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${v.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let m="";if(o&&(m+=s(o)),m+=a.map(r).join(""),!m){t.innerHTML=k("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function ve(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),ve()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let m=await Nt();if(m){let u=Math.floor(Date.now()/1e3),n=m.active_tier,i=(l,w,d)=>{if(n===d||d==="guardian"&&n==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let x=w-u;if(x<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let c=Math.floor(x/60),f=x%60;l.textContent=`${c}m ${f}s`,l.style.color="#fff"}};if(e&&m.guardian&&i(e,m.guardian.next_run,"guardian"),a&&m.architect&&i(a,m.architect.next_run,"architect"),o&&m.strategist&&i(o,m.strategist.next_run,"strategist"),s&&m.system_idle_time!==void 0){let l=m.system_idle_time,w=Math.floor(l/60),d=l%60;s.textContent=`${w}m ${d}s`,l>300?s.style.color="#5eff5e":l>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let v=await Bt(),p=document.getElementById("vitals-processes-val");if(p)if(v){let u=v.length;p.textContent=u>0?`${u} Active`:"Idle",p.style.color=u>0?"#bb86fc":"#fff"}else p.textContent="-",p.style.color="#888";if(v===null){t.innerHTML=k("offline","Failed to load process status.");return}if(st=Date.now(),B(3,st),v.length===0){t.innerHTML=k("empty","No active processes."),P(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function b(u){let n=Math.floor(Date.now()/1e3-u.start_time),i=u.retries>0?`<span class="process-retry-badge">Retry ${u.retries}</span>`:"",l=u.channel_id,w={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return w[l]?l=w[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`
                ...
                </div>`}let y=new Map(Array.from(t.querySelectorAll(".process-widget")).map(u=>[u.dataset.channelId,u])),g=new Set(v.map(u=>u.channel_id));for(let[u,n]of y)g.has(u)||n.remove();v.forEach(u=>{let n=b(u),i=y.get(u.channel_id);i?i.outerHTML!==n&&(i.outerHTML=n):t.insertAdjacentHTML("beforeend",n)}),P(3,v.length)}function Le(){let t=te(),e=Ne()||"master@easter.company",a={enabled:Notification.permission==="granted",supported:"Notification"in window},o=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ot=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[a,o]of Object.entries(t)){let s=gt.filter(r=>r.category===a);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ce=!0,a}function pt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ce=!1}function ut(t,e,a="output"){if(!Ce)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=re(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Pt(t){let e=gt.find(s=>s.id===t);if(!e)return;let a=Rt(e);ut(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(m=>setTimeout(m,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),ut(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ot();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;Pt(o)})})}async function Ut(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${ye()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function ft(){Ut(),Pe(),Ae();let t=window.location.pathname;(t==="/dex"||t==="/dex/"||t.includes("/dex/index.html"))&&!t.includes("/contribute")&&vt();let e=Be();_e(e),Me();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");function m(){return 1}function v(){for(;s.length>1;)s.shift().close(!0);let d=document.getElementById("windows-container"),x=document.querySelector("nav"),c=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");d&&d.setAttribute("data-count",s.length);let C=s.length>0;s.forEach(L=>{let O=document.getElementById(L.id);O&&O.classList.remove("hide-close")}),ie(te());let E=document.getElementById("dexter-menu-container"),S=document.getElementById("nav-window-switcher"),_=document.getElementById("settings-icon");if(s.length>0){if(c?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),x?.classList.add("window-open"),d&&(d.style.paddingTop="60px"),E&&(E.style.display="none"),_&&(_.style.display="none"),S){let L=s[0].id;["feed-window","events-window","monitor-window","contacts-window","workspace-window"].includes(L)?(S.innerHTML=`
                  ...
                  `,document.getElementById("switch-feed").addEventListener("click",()=>{saveWindowState("feed-window"),p(y)}),document.getElementById("switch-events").addEventListener("click",()=>{saveWindowState("events-window"),p(g)}),document.getElementById("switch-monitor").addEventListener("click",()=>{saveWindowState("monitor-window"),p(n)}),document.getElementById("switch-contacts").addEventListener("click",()=>{saveWindowState("contacts-window"),p(u)}),document.getElementById("switch-workspace").addEventListener("click",()=>{saveWindowState("workspace-window"),p(i)})):S.innerHTML=""}}else x?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),d&&(d.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),f||h?c?.classList.remove("hide"):c?.classList.add("hide"),E&&(E.style.display="flex"),_&&(_.style.display="block"),S&&(S.innerHTML="")}function p(d){if(d.isOpen()){d.close();return}for(;s.length>0;)s.pop().close(!0);s.push(d),d.open(),v()}function b(){[...s].forEach(x=>x.close()),s=[]}window.addEventListener("resize",v);let y=F({id:"feed-window",title:"Feed",icon:"bx-news",content:ze(),onOpen:()=>j(),onClose:()=>{let d=s.indexOf(y);d>-1&&s.splice(d,1),v()}}),g=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Qe(),onOpen:()=>Q(),onClose:()=>{let d=s.indexOf(g);d>-1&&s.splice(d,1),v()}}),u=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Je(),onOpen:()=>pe(),onClose:()=>{let d=s.indexOf(u);d>-1&&s.splice(d,1),v()}}),n=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bx-server",title:"Services",content:ot()},{icon:"bx-component",title:"Processes",content:at()},{icon:"bx-brain",title:"Models",content:lt()},{icon:"bx-hdd",title:"Hardware",content:rt()},{icon:"bx-terminal",title:"Logs",content:ct()},{icon:"bx-zap",title:"Analyst",content:it()}],onOpen:()=>{Ee(),ve(),Se(),ne()},onClose:()=>{let d=s.indexOf(n);d>-1&&s.splice(d,1),v()}}),i=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Ve(),onOpen:()=>we(),onClose:()=>{let d=s.indexOf(i);d>-1&&s.splice(d,1),v()}}),l=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:Le(),onOpen:()=>$e(l),onClose:()=>{let d=s.indexOf(l);d>-1&&s.splice(d,1),v()}}),w=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dex/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let d=document.getElementById("dexter-dropdown");d&&(d.innerHTML=`
            <div class="dropdown-item" id="feed-menu-item"><i class='bx bx-news'></i> Feed</div>
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let x=document.getElementById("dexter-menu-container"),c=document.getElementById("dexter-menu-btn");x&&d&&c&&(x.addEventListener("mouseenter",()=>{d.classList.add("active"),c.classList.add("active")}),x.addEventListener("mouseleave",()=>{d.classList.remove("active"),c.classList.remove("active")}),c.addEventListener("click",h=>{h.stopPropagation();let C=localStorage.getItem("dex_last_window")||"feed-window",E=null;C==="feed-window"?E=y:C==="events-window"?E=g:C==="monitor-window"?E=n:C==="contacts-window"?E=u:C==="workspace-window"&&(E=i),E&&p(E)}));let f=h=>{localStorage.setItem("dex_last_window",h)};document.getElementById("feed-menu-item")?.addEventListener("click",()=>{f("feed-window"),p(y)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{f("events-window"),p(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{f("monitor-window"),p(n)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{f("contacts-window"),p(u)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{f("workspace-window"),p(i)}),document.getElementById("settings-icon")?.addEventListener("click",()=>p(l)),document.getElementById("close-all-windows")?.addEventListener("click",()=>b()),setInterval(()=>{y.isOpen()&&j(),g.isOpen()&&Q(),u.isOpen()&&pe(),n.isOpen()&&dt(),i.isOpen()&&we()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{w.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",d=>{d.preventDefault();try{He(document.getElementById("email-input").value),window.location.reload()}catch(x){let c=document.getElementById("login-error");c&&(c.textContent=x.message,c.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ft):ft();})();

(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function We(t=!1){let e=window.location.pathname,i=e==="/"||e==="/index.html",o=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ye=1e3;function te(t){let e=null,i=t.onClose||null,a=t.onOpen||null;function n(){e&&(e.style.zIndex=++Ye)}function o(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),a&&setTimeout(a,10);return}let f=document.getElementById("windows-container");f||(f=document.createElement("div"),f.id="windows-container",f.className="windows-container",document.body.appendChild(f)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ye,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",s="",m="",E;t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((v,r)=>{let d=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,E=`<div class="window-content">${t.tabs.map((v,r)=>`<div class="tab-content ${r===0?"active":""}" data-tab-content="${r}">${v.content}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),E=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Le=null,ie=new Set,Me=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Vt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await B(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];Le=Date.now(),J(0,Le);let l=Date.now(),u=24*60*60*1e3,g=o.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let h=parseInt(d);return l-h<u});g.sort((r,d)=>{let h=I=>{let w=I.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return"low"}return(w.priority||"low").toLowerCase()},M=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,A=M(h(r)),L=M(h(d));return A!==L?L-A:d.timestamp-r.timestamp}),Me=g;let p=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(g.map(r=>p(r))).size>1;if(g.length>0){let r=null;g.forEach(d=>{let h=p(d);b&&h!==r&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),r=h),x.push(d)})}if(t&&(e.innerHTML=""),g.length===0){e.innerHTML=N("empty","No alerts yet."),ke();return}let s=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let h=(d.title||"Untitled Alert").trim(),M=(d.body||"No description provided.").trim(),A=d.summary||"",L=d.content||"",I=d.protocol||"",w=(d.priority||"low").toLowerCase(),H=!!d.alert,D=(d.category||"system").trim(),z=d.related_event_ids||[],O=d.audit_event_id,U=!!localStorage.getItem(`alert_read_ts_${r.id}`),S=new Date(r.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=U?"event-border-grey":"event-border-blue";!U&&H&&(T="event-border-red"),U&&(w==="high"||w==="critical")?T="event-border-red":U&&w==="medium"&&(T="event-border-orange");let k=U?"alert-read":"alert-unread",R=ie.has(r.id),P=R?"expanded":"",Q=R?"display: block;":"display: none;",j="",q="";z.length>0&&(q=`
...
`,ae=new Set,pt=[],ge=null;async function oe(t=!1){let e=document.getElementById("roadmap-list");if(e){Wt();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();pt=a;let n=p=>{let x=ae.has(p.id),f=p.state==="published",b=p.state==="consumed",s="event-border-grey";f&&(s="event-border-blue"),b&&(s="event-border-purple");let E=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${s} cursor-pointer ${x?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=r=>{if(r.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",ae.add(p.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;y.innerHTML=`
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${_(p.content)}</div>
...
`,mt=null,re=new Set,gt=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Xt();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await B(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(gt=o,mt=Date.now(),J(2,mt),o.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),xe(2,0);return}t&&(e.innerHTML="");let l=f=>{let b=f.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let s=(b.title||"Untitled Blueprint").trim(),m=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),y=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(k=>k.trim()),v=(b.implementation_path||[]).map(k=>k.trim()),r=b.source_event_ids||[],d=b.approved===!0,h=new Date(f.timestamp*1e3),M=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=re.has(f.id),I=L?"display: block;":"display: none;",w=document.createElement("div");w.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""} ${d?"blueprint-approved":""}`,w.dataset.blueprintId=f.id,d&&(w.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",w.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let D=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";w.onclick=function(k){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(f.id);let P=this.querySelector(".event-details");P&&(P.style.display="none")}else{this.classList.add("expanded"),re.add(f.id);let P=this.querySelector(".event-details");P&&(P.style.display="block")}};let z="";v.length>0&&(z=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(k=>`<li style="margin-bottom: 5px;">${_(k)}</li>`).join("")}</ul></div>
                    </div>
                `);let O="";r.length>0&&(O=`
...
`;function xt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"VS Code"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function yt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=Qt,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,p=`
        <div class="profile-card">
            ...
            <div class="profile-body">
                ...
                <div id="tab-raw" class="tab-content">${g()}</div>
            </div>
    ...
    `;t.innerHTML=p;let x=t.querySelector(".profile-card"),f=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),setTimeout(()=>t.remove(),300)}),b.forEach(E=>{E.addEventListener("click",()=>{b.forEach(y=>y.classList.remove("active")),s.forEach(y=>y.classList.remove("active")),E.classList.add("active"),t.querySelector(`#tab-${E.dataset.tab}`).classList.add("active")})}),f.addEventListener("click",()=>{x.classList.toggle("expanded");let E=x.classList.contains("expanded");f.innerHTML=E?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var ht=()=>`
...
`,es=null;async function Ne(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Ne(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let a=i.target.closest(".user-profile-card");if(a)try{let n=JSON.parse(a.dataset.user);yt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Te("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(es=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,u)=>{let g=o[l.level]??10,p=o[u.level]??10;return g!==p?g-p:l.username.localeCompare(u.username)}),t.innerHTML=n.map(l=>{let u=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",g=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",p="#888";l.level==="Me"||l.level==="Master"?p="#bb86fc":l.level==="Admin"?p="#cf6679":l.level==="Moderator"?p="#03dac6":l.level==="Contributor"&&(p="#ffa500");let x=l.level==="Me",f=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=x?"2px solid #bb86fc":`1px solid ${u}33`;return`
...
`,Ae=null,ce=new Set,ye=[],F="all",ss={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},is={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ns(t){for(let[e,i]of Object.entries(ss))if(i.includes(t))return e;return"system"}function as(t){return is[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;os();let a=`/events?ml=${F==="all"?100:250}&format=json`;F!=="all"&&(a+=`&category=${F}`);try{let n=await B(a);if(!n.ok)throw new Error("Service unreachable");if(ye=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ae=Date.now(),J(1,Ae),ye.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,E=ns(m),y=as(m),c=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),v="event-border-grey";c&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?v="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?v="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?v="event-border-orange":m==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=ce.has(b.id),h=d?"expanded":"",M=d?"display: block;":"display: none;",A=new Date(b.timestamp*1e3),L=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=wt(m,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",D="";if(c){let S="";if(m==="engagement.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                    ...
                    `}else if(m==="system.attention.expired"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$=s.timestamp-s.last_active,T=Math.floor($/60);S=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$="";s.eval_count&&($=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let T="";if(s.chat_history&&s.chat_history.length>0){let k=s.chat_history.length,R=s.chat_history.map((P,Q)=>{let j=P.role.toUpperCase();j==="ASSISTANT"&&(j="DEXTER");let q=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",W=Q===k-1?"block":"none";return`
                                <div class="history-slide" data-index="${Q}" style="display: ${W};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${_(P.content)}</div>
                                </div>
                            `}).join("");T=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(m==="analysis.user.message_signals"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$=s.signals||{};S=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.router.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let C=s.success?"#03dac6":"#ff4d4d",$=s.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,k="";s.error&&(k=`
                            <div class="event-detail-block">
                                ${T("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${_(s.error)}</pre>
                            </div>
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let j=s.chat_history.length,q=s.chat_history.map((W,pe)=>{let Z=W.role.toUpperCase();Z==="USER"&&(Z="SYSTEM"),Z==="ASSISTANT"&&(Z="AGENT");let G=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let P="";if(s.corrections&&s.corrections.length>0){let j=s.corrections.map((q,W)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${q.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${_(q.snippet)}</div>`:""}
                            </div>
                        `).join("");P=`
                            <div class="event-detail-block">
                        ...
                        `}let Q="";if(s.parsed_results&&s.parsed_results.length>0){let j=s.parsed_results.map(q=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${_(q.title)}</div>
                                <div style="color: #ddd;">${_(q.summary)}</div>
                            </div>
                        `).join("");Q=`
                            <div class="event-detail-block">
                                ${T("Parsed Results")}
                        ...
                        ${P}
                        ${R}
                    `}else if(m==="system.test.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(m==="messaging.user.sent_message"){let C="";s.attachments&&s.attachments.length>0&&(C=`
                            <div class="event-detail-block">
...
`),g=[...l.logs];if(g.length<25){let x=25-g.length;for(let f=0;f<x;f++)g.push("")}else g.length>25&&(g=g.slice(-25));let p=g.map(x=>Se(x)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=o,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let u=unescape(l.dataset.logs);navigator.clipboard.writeText(u).then(()=>{let g=l.querySelector("i");g.classList.remove("bx-copy"),g.classList.add("bx-check"),setTimeout(()=>{g.classList.remove("bx-check"),g.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let u=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${u}?`))try{(await B(`/logs?service_id=${u}`,{method:"DELETE"})).ok&&he()}catch(g){console.error("Error clearing logs:",g)}})}),ls=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Tt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${$t()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await St();c?(u(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await St();u(c)}if(!t)return;let g=await Dt();if(!g||!g.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}Et=Date.now(),J(0,Et);let p=g.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function x(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function f(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function b(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function s(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let r=parseInt(v[1])||0,d=parseInt(v[2])||0,h=parseInt(v[3])||0,M=parseFloat(v[4])||0,A=r*86400+d*3600+h*60+M,L=Math.floor(A/86400);if(L>0)return`${L}d`;let I=Math.floor(A/3600);if(I>0)return`${I}h`;let w=Math.floor(A/60);return w>0?`${w}m`:`${Math.floor(A)}s`}function m(c){let v=c.status==="online",r=v?"service-widget-online":"service-widget-offline",d=v?"bx-check-circle":"bx-x-circle",h=v?"OK":"BAD",M=c.version?f(c.version.str):"-",A=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",L=c.memory&&c.memory!=="N/A"?c.memory:"-",I=A!=="-"?"#00ff00":"#666",w=A!=="-"?"#fff":"#666",H=L!=="-"?"#00ff00":"#666",D=L!=="-"?"#fff":"#666",z=s(c.uptime),O="";return v?O=`
                ...
                </div>`}function o(g){let p=g.status==="Ready";return`
                ...
                </div>`}function l(g){let p=g.status==="Downloaded",x=p?"service-widget-online":"service-widget-offline",f=p?"OK":"MISSING",b=p&&g.size>0?`${(g.size/1e9).toFixed(2)} GB`:"-",s=g.name;return g.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${x}" data-model-name="${g.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${g.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let u="";if(a&&(u+=n(a)),e.xtts&&(u+=o(e.xtts)),u+=i.map(l).join(""),!u){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==u&&(t.innerHTML=u)}async function we(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),u=document.getElementById("guardian-reset-btn"),g=document.getElementById("analyzer-reset-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),we()}catch{u.innerHTML="<i class='bx bx-error'></i> Failed"}},u.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),we()}catch{g.innerHTML="<i class='bx bx-error'></i> Failed"}},g.dataset.listenerAttached="true");let p=await ds();if(p){let y=Math.floor(Date.now()/1e3),c=p.active_tier,v=p.active_synthesis,r=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},d=w=>{w<0&&(w=0);let H=Math.floor(w/3600),D=Math.floor(w%3600/60),z=w%60;return H>0?`${H}h ${D}m`:D>0?`${D}m ${z}s`:`${z}s`},h=(w,H,D,z,O)=>{if(!w)return;let V=r[z]||z.toUpperCase(),U=w.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(U&&(U.textContent=V),O&&O.includes(z))w.textContent="Working",w.style.color="#bb86fc";else if(D){let C=D.next_run-y;if(C<=0)w.textContent="Ready",w.style.color="#5eff5e";else{let $=Math.floor(C/60),T=C%60;w.textContent=`${$}m ${T}s`,w.style.color="#fff"}}H&&D&&(H.innerHTML=`
        ...
        `)};e&&h(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",c),i&&h(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",c);let M=document.getElementById("analyzer-synthesis-val"),A=document.getElementById("analyzer-synthesis-stats");M&&h(M,A,p.synthesis,"synthesis",v);let L=document.getElementById("system-state-label"),I=document.getElementById("system-state-val");if(I&&p.system_state){let w=p.system_state,H=d(p.system_state_time||0);L&&(L.textContent=`State: ${w.toUpperCase()}`),I.textContent=H,w==="idle"?I.style.color=p.system_state_time>300?"#5eff5e":"#fff":I.style.color="#bb86fc"}n&&(n.textContent=d(p.total_idle_time||0)),o&&(o.textContent=d(p.total_active_time||0)),l&&(l.textContent=d(p.total_waste_time||0))}else[e,i,a,n,o,l].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let x=await cs(),f=[],b=[],s=[];x&&(Array.isArray(x)?f=x:(f=x.active||[],b=x.queue||[],s=x.history||[],s.sort((y,c)=>(c.end_time||0)-(y.end_time||0)))),f.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Pe(t,f,!1));let m=document.getElementById("processes-queue-widgets");m&&(b.length===0?!m.querySelector(".tab-placeholder")&&!m.querySelector('div[style*="font-style: italic"]')&&(m.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(m.innerHTML="",Pe(m,b,!1)));let E=document.getElementById("processes-history-widgets");E&&(s.length===0?E.querySelector(".tab-placeholder")||(E.innerHTML=N("empty","No recent history.")):(E.querySelector(".tab-placeholder")&&(E.innerHTML=""),Pe(E,s,!0))),xe(1,f.length)}function Pe(t,e,i){function a(u){let g="";u.end_time?g=`${u.end_time-u.start_time}s`:g=`${Math.floor(Date.now()/1e3-u.start_time)}s`;let p=u.retries>0?`<span class="process-retry-badge">Retry ${u.retries}</span>`:"",x=u.channel_id,f={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(f[x]?x=f[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),i)return`
        <div class="process-history-item" data-channel-id="${u.channel_id}-${u.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${u.channel_id}-${u.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${b}">${g}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let n=i?".process-history-item":".process-widget",o=new Map(Array.from(t.querySelectorAll(n)).map(u=>[u.dataset.channelId,u])),l=new Set(e.map(u=>`${u.channel_id}-${u.start_time}`));for(let[u,g]of o)l.has(u)||g.remove();e.forEach(u=>{let g=`${u.channel_id}-${u.start_time}`,p=a(u),x=o.get(g);if(x){x.outerHTML!==p&&(x.outerHTML=p);let f=t.querySelector(`[data-channel-id="${g}"]`);f&&t.appendChild(f)}else t.insertAdjacentHTML("beforeend",p)})}function Ie(){let t=ve(),e=Qe()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`

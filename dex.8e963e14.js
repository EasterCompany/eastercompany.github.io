...
`,q=new Set,ze=[],K=null;async function j(t=!1){let e=document.getElementById("roadmap-list");if(e){bt();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let n=await a.json();if(ze=n,n.length===0){e.innerHTML=$("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let o=g=>{let y=q.has(g.id),p=g.state==="draft",m=g.state==="published",i=g.state==="consumed",s="event-border-grey";m&&(s="event-border-blue"),i&&(s="event-border-purple");let b=new Date(g.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),x=document.createElement("div");x.className=`event-item notification-item ${s} cursor-pointer ${y?"expanded":""}`,x.dataset.itemId=g.id,x.onclick=d=>{if(d.target.closest("button"))return;x.classList.contains("expanded")?(x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",q.delete(g.id)):(x.classList.add("expanded"),x.querySelector(".event-details").style.display="block",q.add(g.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${g.state.toUpperCase()}]</span>`;return x.innerHTML=`
        <div class="event-time">
                ...
                <div class="user-profile-section" style="background: ${p}; padding: 15px; border-radius: 8px; border: ${m}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${g}; font-weight: 600; text-transform: uppercase;">${y?"DEXTER (ME)":c.level}</span>
                        </div>
...
`),u=[...c.logs];if(u.length<25){let y=25-u.length;for(let p=0;p<y;p++)u.push("")}else u.length>25&&(u=u.slice(-25));let g=u.map(y=>le(y)).join(`
`);return`

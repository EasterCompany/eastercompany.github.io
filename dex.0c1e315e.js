(()=>{function Ye(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Xe(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,c=document.createElement("nav");c.innerHTML=s,document.body.prepend(c)}function fe(t){let e=window.location.pathname,i=e==="/"||e==="/index.html",a=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!a||!s||(t||!i?(a.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{a.style.transform="translateX(-3px)"},s.onmouseleave=()=>{a.style.transform="translateX(0)"}):(a.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function Qe(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var et=1e3;function ne(t){let e=null,i=t.onClose||null,a=t.onOpen||null;function s(){e&&(e.style.zIndex=++et)}function c(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",o),a&&setTimeout(a,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++et,e.addEventListener("mousedown",s);let b=t.icon||"bx-window",n="",g="",E;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((r,d)=>{let f=r.icon?`<i class="bx ${r.icon}"></i>`:`<span class="tab-glyph">${r.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
            ...
            ${E}
        `,u.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",x=>{x.stopPropagation(),p()}),window.addEventListener("resize",o),t.tabs&&e.querySelectorAll(".tab").forEach(x=>{x.addEventListener("click",()=>{let l=x.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(r=>r.classList.remove("active")),x.classList.add("active"),e.querySelectorAll(".tab-content").forEach(r=>r.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${l}"]`).classList.add("active"),v(x,e)})}),setTimeout(()=>{e.classList.add("open"),a&&a()},10)}function o(){if(!e||!e.classList.contains("open"))return;let u=e.querySelector(".tab.active");u&&v(u,e)}function v(u,b){setTimeout(()=>{let n=b.querySelector(".tab-bar");if(!n)return;let g=Array.from(n.querySelectorAll(".tab")),E=g.indexOf(u),x=n.clientWidth,l=g[Math.max(0,E-2)],r=g[Math.min(g.length-1,E+2)],d=l.offsetLeft-n.offsetLeft-25,w=r.offsetLeft+r.offsetWidth-n.offsetLeft+25-d,k=w<=x?d-(x-w)/2:u.offsetLeft-n.offsetLeft-x/2+u.offsetWidth/2;n.scrollTo({left:k,behavior:"smooth"})},350)}function p(u=!1){e&&(window.removeEventListener("resize",o),u?(e.remove(),e=null):(e.classList.remove("open"),i&&i(),setTimeout(()=>{e?.remove(),e=null},400)))}function m(u){let b=e?.querySelector(".window-content");b&&(b.innerHTML=u)}function y(){return e&&e.classList.contains("open")}return{open:c,close:p,setContent:m,isOpen:y,focus:s,id:t.id}}var tt="easter_theme",ae={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Ne(){return localStorage.getItem(tt)||ae.DARK}function st(t){if(!Object.values(ae).includes(t))throw new Error("Invalid theme");localStorage.setItem(tt,t),it(t)}function it(t){let e=document.body;if(Object.values(ae).forEach(i=>{e.classList.remove(`theme-${i}`)}),e.classList.add(`theme-${t}`),!document.querySelector(".background")){let i=document.createElement("div");i.className="background",document.body.prepend(i)}e.classList.add("is-animated")}function nt(){let t=Ne();it(t)}function O(t,e,i=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",c=i?`<p class="placeholder-action">${i}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${c}</div>`}function N(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function K(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!i)return;if(!e){i.textContent="Last updated: never";return}let a=Date.now(),s=Math.floor((a-e)/1e3),c;s<60?c=`${s}s ago`:s<3600?c=`${Math.floor(s/60)}m ago`:c=`${Math.floor(s/3600)}h ago`,i.textContent=`Last updated: ${c}`}var jt=0;function he(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!i)return;let a=i.querySelector(".notification-badge");a&&(e>0?(a.textContent=e>9?"9+":e,a.style.display="flex"):a.style.display="none")}var Q=0,ee=0;function ot(t){Q=t,ve()}function rt(t){ee=t,ve()}function ve(){let t=Q+ee;jt=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let i=document.getElementById("alerts-menu-item");if(i){let o=i.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="auto",i.appendChild(o)),o.textContent=Q>9?"9+":Q,o.style.display=Q>0?"flex":"none"}let a=document.getElementById("workspace-menu-item");if(a){let o=a.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="auto",a.appendChild(o)),o.textContent=ee>9?"9+":ee,o.style.display=ee>0?"flex":"none"}let s=document.getElementById("switch-alerts");if(s){let o=s.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="8px",s.appendChild(o)),o.textContent=Q>9?"9+":Q,o.style.display=Q>0?"flex":"none"}let c=document.getElementById("switch-workspace");if(c){let o=c.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="8px",c.appendChild(o)),o.textContent=ee>9?"9+":ee,o.style.display=ee>0?"flex":"none"}}function ke(){let t=document.getElementById("alerts-list");if(!t)return;Q=t.querySelectorAll(".alert-unread:not(.priority-low)").length,ve()}function lt(){let t=document.getElementById("blueprints-list");if(!t)return;ee=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,ve()}function Ft(){return"https://event.easter.company"}function Jt(){return"https://discord.easter.company"}var Gt="http://127.0.0.1:8100",Wt="http://127.0.0.1:8300",Vt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Te(t){let e=N(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,c)=>{let o=Vt[c];return o?`<span class="${o}">`:""});let i=(e.match(/<span/g)||[]).length,a=(e.match(/<\/span/g)||[]).length;return i>a&&(e+="</span>".repeat(i-a)),e}function oe(t){if(!t)return"";let e=N(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(i,a)=>{let s=a.split("|").map(c=>c.trim());return s.every(c=>c.match(/^:?-+:?$/))?"":`<div class="md-table-row">${s.map(c=>`<span class="md-table-cell">${c}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var Zt="https://sterling-javelin-12539.upstash.io",Kt="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function D(){return window.location.hostname==="easter.company"}var P=null,He=!1,ct="dex_dashboard_snapshot";function Yt(){let t=localStorage.getItem(ct);if(t)try{P=JSON.parse(t)}catch{P=null}}async function at(){if(!(!D()||He)){He=!0;try{let t=await es("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);P=e,localStorage.setItem(ct,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{He=!1}}}function Xt(){if(!P||!P.agent_status)return;let t=P.agent_status;typeof t.system_state_time=="number"&&(t.system_state_time+=1)}async function Qt(){if(!D())return;Yt();let t=Math.floor(Date.now()/1e3),e=P?t-P.timestamp:1/0;(!P||e>120)&&await at(),setInterval(()=>{let i=new Date,a=Math.floor(Date.now()/1e3),s=P?a-P.timestamp:1/0;(i.getSeconds()===59||s>300)&&at(),Xt()},1e3)}D()&&Qt();async function es(t,...e){try{let a=await(await fetch(Zt,{method:"POST",headers:{Authorization:`Bearer ${Kt}`},body:JSON.stringify([t,...e])})).json();if(a.error)throw new Error(a.error);return a.result}catch(i){return console.error("Upstash Error:",i),null}}var ge=null,be=null,Ee=!1,Se=!1;async function z(t,e={}){if(D()){if(!P)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(P.monitor),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(P.processes),{status:200});if(t.startsWith("/events")){let s=new URLSearchParams(t.split("?")[1]||""),c=s.get("type")||s.get("event.type"),o=s.get("category"),v=s.get("order")||"desc",p=[];return c==="system.notification.generated"?p=[...P.alerts||[]]:c==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?p=[...P.blueprints||[]]:o==="messaging"?p=[...P.messaging_events||[]]:o==="system"?p=[...P.system_events||[]]:o==="cognitive"?p=[...P.cognitive_events||[]]:o==="moderation"?p=[...P.moderation_events||[]]:p=[...P.events||[]],v==="asc"?p.sort((m,y)=>m.timestamp-y.timestamp):p.sort((m,y)=>y.timestamp-m.timestamp),new Response(JSON.stringify({events:p,count:p.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(P.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let s=t.split("/")[2],c=P.profiles?P.profiles[s]:null;return c?new Response(JSON.stringify(c),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(ge)try{let s=await fetch(ge+t,e);if(s.ok)return s;ge=null}catch{ge=null}let i=Ft(),a=Gt;try{let s=await fetch(i+t,e);if(s.ok)return ge=i,Ee&&(console.log("\u2728 Production event service restored."),Ee=!1),s;throw new Error("Primary failed")}catch{Ee||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${a}`),Ee=!0);try{let c=await fetch(a+t,e);if(c.ok)return ge=a,c;throw new Error("Fallback failed")}catch(c){throw c}}}async function Le(t,e={}){if(D()){if(!P)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(P.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let s=t.split("/")[2],c=(P.contacts?.members||[]).find(o=>o.id===s);return c?new Response(JSON.stringify(c),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(be)try{let s=await fetch(be+t,e);if(s.ok)return s;be=null}catch{be=null}let i=Jt(),a=Wt;try{let s=await fetch(i+t,e);if(s.ok)return be=i,Se&&(console.log("\u2728 Production discord service restored."),Se=!1),s;throw new Error("Primary failed")}catch{Se||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${a}`),Se=!0);try{let c=await fetch(a+t,e);if(c.ok)return be=a,c;throw new Error("Fallback failed")}catch(c){throw c}}}var dt=()=>`
...
`,Me=null,re=new Set,Ae=[];async function se(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ts(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await z(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let c=(await a.json()).events||[];Me=Date.now(),K(0,Me);let o=Date.now(),v=24*60*60*1e3,p=c.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let w=parseInt(f);return o-w<v});p.sort((d,f)=>{let w=S=>{let h=S.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},k=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,_=k(w(d)),A=k(w(f));return _!==A?A-_:f.timestamp-d.timestamp}),Ae=p;let m=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],b=new Set(p.map(d=>m(d))).size>1;if(p.length>0){let d=null;p.forEach(f=>{let w=m(f);b&&w!==d&&(y.push({id:`divider-${w}`,type:"divider",label:w.toUpperCase()}),d=w),y.push(f)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=O("empty","No alerts yet."),ke();return}let n=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let w=(f.title||"Untitled Alert").trim(),k=(f.body||"No description provided.").trim(),_=f.summary||"",A=f.content||"",S=f.protocol||"",h=(f.priority||"low").toLowerCase(),H=!!f.alert,T=(f.category||"system").trim(),I=f.related_event_ids||[],q=f.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${d.id}`),L=new Date(d.timestamp*1e3),$=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=F?"event-border-grey":"event-border-blue";!F&&H&&(M="event-border-red"),F&&(h==="high"||h==="critical")?M="event-border-red":F&&h==="medium"&&(M="event-border-orange");let R=F?"alert-read":"alert-unread",B=re.has(d.id),U=B?"expanded":"",j=B?"display: block;":"display: none;",J="",G="";I.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${I.map(te=>`<a href="#" onclick="window.dexter.viewEvent('${te}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${te.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let Y="";q&&(Y=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(A)}</div>
            </div>
        `:X=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(k)}</div>
            </div>
        `,J=`
                ...
                ${X}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${R} ${U} cursor-pointer priority-${h}`,Z.dataset.alertId=d.id,Z.onclick=function(te){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(d.id);let ye=this.querySelector(".event-details");ye&&(ye.style.display="none")}else{this.classList.add("expanded"),re.add(d.id);let ye=this.querySelector(".event-details");if(ye&&(ye.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ze="event-border-grey";h==="high"||h==="critical"?ze="event-border-red":h==="medium"&&(ze="event-border-orange"),this.classList.add(ze),ke()}}};let Ve=`${S?S.toUpperCase():"GUARDIAN"} ALERT: ${_||w}`,Ut={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[T]||"bx-bell",qt=h==="high"||h==="critical"?"#ff4d4d":h==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${qt}; font-size: 0.8em; margin-left: 5px;">[${h.toUpperCase()}]</span>
                    </div>
            ...
            `;let Ze=Z.querySelector(".event-details");Ze&&Ze.addEventListener("click",te=>{te.stopPropagation()});let Ke=Z.querySelector(".close-details-btn");return Ke&&Ke.addEventListener("click",te=>{te.stopPropagation(),Z.classList.remove("expanded");let De=Z.querySelector(".event-details");De&&(De.style.display="none"),re.delete(d.id)}),Z},g=d=>{let f=document.createElement("div");f.className="notification-divider",f.dataset.alertId=d.id;let w="#888888";return d.label==="CRITICAL"?w="#ff4d4d":d.label==="HIGH"?w="#ff8888":d.label==="MEDIUM"&&(w="#ffa500"),f.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${w}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,f.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${w}44, transparent); flex-grow: 1;"></div>`,f},E=Array.from(e.children),x=new Map(E.map(d=>[d.dataset.alertId,d])),l=new Set(y.map(d=>d.id));E.forEach(d=>{let f=d.dataset.alertId;(!f||!l.has(f))&&d.remove()});let r=null;y.forEach((d,f)=>{let w=x.get(d.id);(!w||t)&&(w&&w.remove(),d.type==="divider"?w=g(d):w=n(d),!w)||(f===0?e.firstElementChild!==w&&e.prepend(w):r&&r.nextElementSibling!==w&&r.after(w),r=w)}),Me=Date.now(),K(0,Me),ke()}catch(a){console.error("Error fetching alerts:",a),e.children.length===0&&(e.innerHTML=O("offline","Failed to load alerts.","The event service may be offline."))}}function ts(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),a=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ae.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),se(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Ae.forEach(s=>{re.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),se(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{re.clear(),se(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await z("/events?type=system.notification.generated",{method:"DELETE"});let s=Date.now()-48*60*60*1e3;Ae.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,s.toString())}),re.clear(),se(!0)}catch(s){console.error("Failed to clear alerts:",s)}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}async function Pe(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await z(t);if(!e.ok)return;let a=(await e.json()).events||[],s=0;a.forEach(c=>{let o=c.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}if((o.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||s++}),ot(s)}catch{}}var pt=()=>`
...
`,le=new Set,Ie=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ss();try{let i=await z("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();Ie=a;let s=m=>{let y=le.has(m.id),u=m.state==="published",b=m.state==="consumed",n="event-border-grey";u&&(n="event-border-blue"),b&&(n="event-border-purple");let E=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),x=document.createElement("div");x.className=`event-item notification-item ${n} cursor-pointer ${y?"expanded":""}`,x.dataset.itemId=m.id,x.onclick=d=>{if(d.target.closest("button"))return;x.classList.contains("expanded")?(x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",le.delete(m.id)):(x.classList.add("expanded"),x.querySelector(".event-details").style.display="block",le.add(m.id))};let l=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;x.innerHTML=`
        <div class="event-time">
      ...
      `;let r=x.querySelector(".event-details");return r&&r.addEventListener("click",d=>{d.stopPropagation()}),x.querySelector(".edit-btn")?.addEventListener("click",()=>is(m)),x.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ns(m)),x.querySelector(".delete-btn")?.addEventListener("click",()=>as(m.id)),x.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",le.delete(m.id)}),x},c=Array.from(e.children),o=new Map(c.map(m=>[m.dataset.itemId,m])),v=new Set(a.map(m=>m.id));if(c.forEach(m=>{let y=m.dataset.itemId;(!y||!v.has(y))&&m.remove()}),!Ie||Ie.length===0){e.innerHTML=O("empty","Your roadmap is empty.",D()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let p=null;a.forEach((m,y)=>{let u=o.get(m.id);(!u||t)&&(u&&u.remove(),u=s(m),!u)||(y===0?e.firstElementChild!==u&&e.prepend(u):p&&p.nextElementSibling!==u&&p.after(u),p=u)})}catch{e.children.length===0&&(e.innerHTML=O("offline","Failed to load roadmap.","The event service may be offline."))}}}function ss(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let o=xe?`/roadmap/${xe}`:"/roadmap",v=xe?"PATCH":"POST";try{await z(o,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{Ie.forEach(c=>le.add(c.id)),ce(!0)},a.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{le.clear(),ce(!0)},s.dataset.listenerAttached="true")}function is(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function ns(t){let e=t.state==="published"?"draft":"published";try{await z(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function as(t){if(confirm("Delete this roadmap item?"))try{await z(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var ft=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,ut=null,de=new Set,bt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;os();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await z(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let c=(await a.json()).events||[];if(bt=c,ut=Date.now(),K(2,ut),c.length===0){e.innerHTML=O("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=u=>{let b=u.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),x=(b.category||"architecture").trim(),l=(b.related_services||b.affected_services||[]).map(B=>B.trim()),r=(b.implementation_path||[]).map(B=>B.trim()),d=b.source_event_ids||[],f=b.approved===!0,w=new Date(u.timestamp*1e3),k=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=de.has(u.id),S=A?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${A?"expanded":""} ${f?"blueprint-approved":""}`,h.dataset.blueprintId=u.id,f&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let T=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";h.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(u.id);let j=this.querySelector(".event-details");j&&(j.style.display="none")}else{this.classList.add("expanded"),de.add(u.id);let j=this.querySelector(".event-details");j&&(j.style.display="block")}};let I="";r.length>0&&(I=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${r.map(B=>`<li style="margin-bottom: 5px;">${N(B)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";d.length>0&&(q=`
                ...
                `);let V=l.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${l.join(", ")}</span></div>`:"<div></div>",F=D(),L=f?`
            ...
            `;let $=h.querySelector(".blueprint-approve-btn");$&&($.onclick=async B=>{B.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await z(`/events/${u.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(U){console.error("Failed to approve blueprint:",U)}});let C=h.querySelector(".blueprint-delete-btn");C&&(C.onclick=async B=>{B.stopPropagation();let U=!f;C.innerHTML=U?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await z(`/events/${u.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(j){console.error("Failed to delete blueprint:",j)}});let M=h.querySelector(".event-details");M&&M.addEventListener("click",B=>{B.stopPropagation()});let R=h.querySelector(".close-details-btn");return R&&R.addEventListener("click",B=>{B.stopPropagation(),h.classList.remove("expanded");let U=h.querySelector(".event-details");U&&(U.style.display="none"),de.delete(u.id)}),h},v=Array.from(e.children),p=new Map(v.map(u=>[u.dataset.blueprintId,u])),m=new Set(c.map(u=>u.id));v.forEach(u=>{let b=u.dataset.blueprintId;(!b||!m.has(b))&&u.remove()});let y=null;c.forEach((u,b)=>{let n=p.get(u.id);(!n||t)&&(n&&n.remove(),n=o(u),!n)||(b===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),he(2,c.length),lt()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=O("offline","Failed to load blueprints.","The event service may be offline."))}}async function vt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await z(t);if(!e.ok)return;let a=(await e.json()).events||[],s=0;a.forEach(c=>{let o=c.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&s++}),rt(s)}catch{}}function os(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{bt.forEach(a=>de.add(a.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await z("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var xt=()=>`
...
`;function yt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function ht(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=rs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),Le(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();Oe(e,t,a)}else{console.log("Profile not found or error, using mock data.");let a=yt(t.id,t.username);Oe(e,t,a)}}).catch(i=>{console.error("Profile fetch error:",i);let a=yt(t.id,t.username);Oe(e,t,a)})}function Oe(t,e,i){let a=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",s=()=>{let E=(i.badges||[]).map(k=>`<span class="profile-badge ${k==="Creator"?"master":""}">${k}</span>`).join(""),x=(i.attributes||i.facts||[]).map(k=>{let _=k.key||k.k,A=k.value||k.v;return`
            <div class="fact-chip">
                <span class="fact-key">${_}:</span>
                <span class="fact-val">${A}</span>
            </div>
        `}).join(""),l=i.cognitive_model||i,r=l.technical_level||l.techLevel||0,d=l.communication_style||l.commStyle||"Unknown",f=l.patience_level||l.patience||"Unknown",w=l.vibe||"Unknown";return`
        ...
        `},c=()=>{let E=i.dossier||{},x=E.identity||{},l=E.career||{},r=E.personal||{},d=E.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${x.fullName||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Age Range</div>
                        <div class="dossier-value">${x.ageRange||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Location</div>
                        <div class="dossier-value">${x.location||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Gender</div>
                        <div class="dossier-value">${x.gender||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Sexuality</div>
                        <div class="dossier-value" style="color: #ffb74d;">${x.sexuality||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Relationship Status</div>
                        <div class="dossier-value">${x.relationship||"Unknown"}</div>
                    </div>
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${l.jobTitle||"Unknown"}</div>
                        <div style="font-size: 0.8em; color: #888; margin-top: 2px;">@ ${l.company||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                         <div class="dossier-label">Key Skills</div>
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(l.skills||[]).map(f=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${f}</span>`).join("")}
                         </div>
                    </div>
                </div>
            ...
            ${Object.entries(E).map(([x,l])=>`
                <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                    <div style="width: 10px; height: 150px; background: rgba(255,255,255,0.05); border-radius: 5px; position: relative; overflow: hidden;">
                        <div style="position: absolute; bottom: 0; left: 0; width: 100%; height: ${l}%; background: ${l>50?"#03dac6":"#cf6679"}; transition: height 1s;"></div>
                    </div>
                    <div style="font-size: 0.8em; text-transform: uppercase; letter-spacing: 1px; color: #aaa;">${x.substring(0,4)}</div>
                    <div style="font-family: monospace;">${l}%</div>
                </div>
            `).join("")}
        </div>
        <div style="margin-top: 40px;">
            ...
            <div style="height: 100px; border-bottom: 1px solid #333; display: flex; align-items: flex-end; gap: 5px; padding-bottom: 5px;">
                ${Array.from({length:40}).map(()=>{let x=Math.floor(Math.random()*80)+10;return`<div style="flex: 1; background: ${Math.random()>.7?"#cf6679":Math.random()>.5?"#03dac6":"#444"}; height: ${x}%; border-radius: 2px;"></div>`}).join("")}
            </div>
    ...
    `;t.innerHTML=m;let y=t.querySelector(".profile-card"),u=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),n=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),setTimeout(()=>t.remove(),300)}),b.forEach(E=>{E.addEventListener("click",()=>{b.forEach(x=>x.classList.remove("active")),n.forEach(x=>x.classList.remove("active")),E.classList.add("active"),t.querySelector(`#tab-${E.dataset.tab}`).classList.add("active")})}),u.addEventListener("click",()=>{y.classList.toggle("expanded");let E=y.classList.contains("expanded");u.innerHTML=E?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var wt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,ls=null;async function Ue(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ue(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let a=i.target.closest(".user-profile-card");if(a)try{let s=JSON.parse(a.dataset.user);ht(s)}catch(s){console.error("Failed to parse user data",s)}},t.dataset.listenerAttached="true");try{let i=await Le("/contacts");if(!i.ok)throw new Error("Service unreachable");let s=(await i.json()).members||[];if(ls=Date.now(),s.length===0){t.innerHTML=O("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((o,v)=>{let p=c[o.level]??10,m=c[v.level]??10;return p!==m?p-m:o.username.localeCompare(v.username)}),t.innerHTML=s.map(o=>{let v=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",m="#888";o.level==="Me"||o.level==="Master"?m="#bb86fc":o.level==="Admin"?m="#cf6679":o.level==="Moderator"?m="#03dac6":o.level==="Contributor"&&(m="#ffa500");let y=o.level==="Me",u=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(o).replace(/"/g,"&quot;")}"
                     style="background: ${u}; padding: 15px; border-radius: 8px; border: ${b}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s;">
...
`,_e=null,me=new Set,we=[],W="all",ds={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ms(t){for(let[e,i]of Object.entries(ds))if(i.includes(t))return e;return"system"}function us(t){return ps[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;fs();let a=`/events?ml=${W==="all"?100:250}&format=json`;W!=="all"&&(a+=`&category=${W}`);try{let s=await z(a);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let g=n.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),K(1,_e),we.length===0){e.innerHTML=O("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let g=n.type,E=ms(g),x=us(g),l=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),r="event-border-grey";l&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?r="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?r="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?r="event-border-orange":g==="system.test.completed"?r=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":r="event-border-blue");let d=l?"cursor-pointer":"",f=me.has(b.id),w=f?"expanded":"",k=f?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),A=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=Ct(g,n),H=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",T="";if(l){let L="";if(g==="engagement.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                    ...
                    `}else if(g==="system.attention.expired"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C=n.timestamp-n.last_active,M=Math.floor(C/60);L=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C="";n.eval_count&&(C=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(n.chat_history&&n.chat_history.length>0){let R=n.chat_history.length,B=n.chat_history.map((U,j)=>{let J=U.role.toUpperCase();J==="ASSISTANT"&&(J="DEXTER");let G=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",Y=j===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${Y};">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C=n.signals||{};L=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.visual.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,C="";n.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(C=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${$("Input Context")}
                            <pre class="detail-pre">${N(n.raw_input)||"None"}</pre>
                        </div>
                    `}else if(g==="system.cli.command"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let $=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",M=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,R="";n.error&&(R=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${N(n.error)}</pre>
                            </div>
                        `);let B="";if(n.chat_history&&n.chat_history.length>0){let J=n.chat_history.length,G=n.chat_history.map((Y,ue)=>{let X=Y.role.toUpperCase();X==="USER"&&(X="SYSTEM"),X==="ASSISTANT"&&(X="AGENT");let Z=Y.role==="user"?"#03dac6":Y.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${Z}; letter-spacing: 1px; font-weight: bold;">${X}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ue+1} of ${J}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${N(Y.content)}</div>
                                </div>
                            `}).join("");B=`
                            <div class="event-detail-block">
                        ...
                        `}let U="";if(n.corrections&&n.corrections.length>0){let J=n.corrections.map((G,Y)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${G.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${N(G.snippet)}</div>`:""}
                            </div>
                        `).join("");U=`
                            <div class="event-detail-block">
                        ...
                        `}let j="";if(n.parsed_results&&n.parsed_results.length>0){let J=n.parsed_results.map(G=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${N(G.title)}</div>
                                <div style="color: #ddd;">${N(G.summary)}</div>
                            </div>
                        `).join("");j=`
                            <div class="event-detail-block">
                                ${M("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${C} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${n.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${U}
                        ${B}
                    `}else if(g==="system.test.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let $="";n.attachments&&n.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(M=>{let R=M.content_type&&M.content_type.startsWith("image/"),B=(M.size/1024).toFixed(1)+" KB";return`
                ...
                `}let I=document.createElement("div");I.className=`event-item ${r} ${d} ${w}`,I.dataset.eventId=b.id,I.onclick=function(L){if(!l)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},I.innerHTML=`
                <div class="event-time">
            ...
            `;let q=I.querySelector(".event-details");if(q&&q.addEventListener("click",L=>{L.stopPropagation()}),l){let L=I.querySelector(".close-details-btn");L&&L.addEventListener("click",$=>{$.stopPropagation();let C=$.target.closest(".event-item");if(C){C.classList.remove("expanded"),me.delete(b.id);let M=C.querySelector(".event-details");M&&(M.style.display="none")}})}let V=I.querySelector(".prev-btn"),F=I.querySelector(".next-btn");if(V&&F){let L=0,$=I.querySelectorAll(".history-slide"),C=$.length,M=()=>{$.forEach((R,B)=>{R.style.display=B===L?"block":"none"}),V.disabled=L===0,F.disabled=L===C-1,V.style.opacity=L===0?"0.5":"1",F.style.opacity=L===C-1?"0.5":"1"};V.addEventListener("click",R=>{R.stopPropagation(),L>0&&(L--,M())}),F.addEventListener("click",R=>{R.stopPropagation(),L<C-1&&(L++,M())}),M()}return I},p=Array.from(e.children),m=new Map(p.map(b=>[b.dataset.eventId,b])),y=new Set(we.map(b=>b.id));p.forEach(b=>{let n=b.dataset.eventId;(!n||!y.has(n))&&b.remove()});let u=null;we.forEach((b,n)=>{let g=m.get(b.id);(!g||t)&&(g&&g.remove(),g=v(b),!g)||(n===0?e.firstElementChild!==g&&e.prepend(g):u&&u.nextElementSibling!==g&&u.after(g),u=g)}),_e=Date.now(),K(1,_e)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=O("offline","Failed to load events.","The event service may be offline."))}}function fs(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{we.forEach(s=>me.add(s.id)),ie(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{me.clear(),ie(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active")),s.classList.add("active"),W=s.dataset.filter,ie(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===W?s.classList.add("active"):s.classList.remove("active")});let a=document.getElementById("events-clear");a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{let s=W==="all"?"ALL":W.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let c="/events";if(W!=="all"?c+=`?category=${W}`:c+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await z(c,{method:"DELETE"})).ok)throw new Error("Failed to delete events");me.clear(),ie(!0)}catch(c){console.error("Failed to clear events:",c),alert("Failed to clear events. Check console.")}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}var gs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,$t=gs;function Et(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var bs=null;async function Ce(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await z("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=O("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let a=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=i.filter(o=>!a.includes(o.id));s.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),s.reverse();let c=s.map(o=>{let v=o.logs.join(`
`),p=[...o.logs];if(p.length<25){let y=25-p.length;for(let u=0;u<y;u++)p.push("")}else p.length>25&&(p=p.slice(-25));let m=p.map(y=>Te(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let v=unescape(o.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let p=o.querySelector("i");p.classList.remove("bx-copy"),p.classList.add("bx-check"),setTimeout(()=>{p.classList.remove("bx-check"),p.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let v=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await z(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&Ce()}catch(p){console.error("Error clearing logs:",p)}})}),bs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=O("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Lt=()=>{let t=D()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Et()}
        </div>`;async function Dt(){await Promise.all([$e(),Fe(),Je()]),await Ce()}var St=null,kt=null;async function zt(){try{return await(await z("/system_monitor")).json()}catch{return null}}async function Tt(){try{return await(await z("/system/hardware")).json()}catch{return null}}async function vs(){try{return await(await z("/processes")).json()}catch{return null}}async function xs(){try{return await(await z("/agent/status")).json()}catch{return null}}async function Fe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),v=l=>{if(l){if(i&&(i.innerHTML=`
                ...
                </div>`),s){let r=(l.MEMORY_BYTES/1073741824).toFixed(1);s.innerHTML=`
                ...
                </div>`}if(a&&l.CPU&&l.CPU.length>0){let r=l.CPU[0];a.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}c&&(l.GPU&&l.GPU.length>0?c.innerHTML=l.GPU.map(r=>{let d=(r.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&l.STORAGE&&l.STORAGE.length>0?o.innerHTML=l.STORAGE.map(r=>{let d=(r.USED/1073741824).toFixed(1),f=(r.SIZE/(1024*1024*1024)).toFixed(1),w=r.SIZE>0?(r.USED/r.SIZE*100).toFixed(0):0,k=r.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let l=await Tt();l?(v(l),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let l=await Tt();v(l)}if(!t)return;let p=await zt();if(!p||!p.services){t.children.length===0&&(t.innerHTML=O("offline","Failed to load system metrics.","The event service may be offline."));return}St=Date.now(),K(0,St);let m=p.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function u(l){if(!l||l==="N/A"||l==="unknown")return"-";let r=l.match(/^(\d+\.\d+\.\d+)/);return r?r[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let r=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!r)return"-";let d=parseInt(r[1])||0,f=parseInt(r[2])||0,w=parseInt(r[3])||0,k=parseFloat(r[4])||0,_=d*86400+f*3600+w*60+k,A=Math.floor(_/86400);if(A>0)return`${A}d`;let S=Math.floor(_/3600);if(S>0)return`${S}h`;let h=Math.floor(_/60);return h>0?`${h}m`:`${Math.floor(_)}s`}function g(l){let r=l.status==="online",d=r?"service-widget-online":"service-widget-offline",f=r?"bx-check-circle":"bx-x-circle",w=r?"OK":"BAD",k=l.version?u(l.version.str):"-",_=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",A=l.memory&&l.memory!=="N/A"?l.memory:"-",S=_!=="-"?"#00ff00":"#666",h=_!=="-"?"#fff":"#666",H=A!=="-"?"#00ff00":"#666",T=A!=="-"?"#fff":"#666",I=n(l.uptime),q="";r?q=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${h};">${_}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${H};"></i>
                        <span style="color: ${T};">${A}</span>
                    </div>
                </div>`:q='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let V=D()?"easter.company":b(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${d}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${f}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${V}</span></div>${q}</div></div>`}let E=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),x=new Set(m.map(l=>l.id));for(let[l,r]of E)x.has(l)||r.remove();m.forEach(l=>{let r=g(l),d=E.get(l.id);d?d.outerHTML!==r&&(d.outerHTML=r):t.insertAdjacentHTML("beforeend",r)})}async function Je(){let t=document.getElementById("models-widgets");if(!t)return;let e=await zt();if(!e){t.children.length===0&&(t.innerHTML=O("offline","Failed to load model status.","The event service may be offline."));return}kt=Date.now(),K(2,kt);let i=e.models||[],a=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function s(p){let m=p.status==="Ready";return`
                ...
                </div>`}function c(p){let m=p.status==="Ready";return`
                ...
                </div>`}function o(p){let m=p.status==="Downloaded",y=m?"service-widget-online":"service-widget-offline",u=m?"OK":"MISSING",b=m&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",n=p.name;return p.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(a&&(v+=s(a)),e.xtts&&(v+=c(e.xtts)),v+=i.map(o).join(""),!v){t.innerHTML=O("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function $e(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),p=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),$e()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),$e()}catch{p.innerHTML="<i class='bx bx-error'></i>"}},p.dataset.listenerAttached="true");let m=await xs();if(m){let x=Math.floor(Date.now()/1e3),l=m.active_tier,r=m.active_synthesis,d=m.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},f=h=>{h<0&&(h=0);let H=Math.floor(h/3600),T=Math.floor(h%3600/60),I=h%60;return H>0?`${H}h ${T}m`:T>0?`${T}m ${I}s`:`${I}s`},w=(h,H,T,I,q)=>{if(!h)return;let V=d[I]||I.toUpperCase(),F=h.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(F&&(F.textContent=V),q&&q.includes(I))h.textContent="Working",h.style.color="#bb86fc";else if(T){let $=T.next_run-x;if($<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let C=Math.floor($/60),M=$%60;h.textContent=`${C}m ${M}s`,h.style.color="#fff"}}H&&T&&(H.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${T.attempts||0}</span>
            <span style="color: ${T.failures>0?"#ffa500":"#666"}">Fails: ${T.failures||0}</span>
            <span style="color: ${T.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${T.absolute_failures||0}</span>
          </div>
        `)};e&&w(e,document.getElementById("guardian-sentry-stats"),m.sentry,"sentry",l),i&&w(i,document.getElementById("guardian-architect-stats"),m.architect,"architect",l);let k=document.getElementById("analyzer-synthesis-val"),_=document.getElementById("analyzer-synthesis-stats");k&&w(k,_,m.synthesis,"synthesis",r);let A=document.getElementById("system-state-label"),S=document.getElementById("system-state-val");if(S&&m.system_state){let h=m.system_state,H=f(m.system_state_time||0);A&&(A.textContent=`State: ${h.toUpperCase()}`),S.textContent=H,h==="idle"?S.style.color=m.system_state_time>300?"#5eff5e":"#fff":S.style.color="#bb86fc"}s&&(s.textContent=f(m.total_idle_time||0)),c&&(c.textContent=f(m.total_active_time||0)),o&&(o.textContent=f(m.total_waste_time||0))}else[e,i,a,s,c,o].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});let y=await vs(),u=[],b=[],n=[];y&&(Array.isArray(y)?u=y:(u=y.active||[],b=y.queue||[],n=y.history||[],n.sort((x,l)=>(l.end_time||0)-(x.end_time||0)))),u.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=O("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),je(t,u,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",je(g,b,!1)));let E=document.getElementById("processes-history-widgets");E&&(n.length===0?E.querySelector(".tab-placeholder")||(E.innerHTML=O("empty","No recent history.")):(E.querySelector(".tab-placeholder")&&(E.innerHTML=""),je(E,n,!0))),he(1,u.length)}function je(t,e,i){function a(v){let p="";v.end_time?p=`${v.end_time-v.start_time}s`:p=`${Math.floor(Date.now()/1e3-v.start_time)}s`;let m=v.retries>0?`<span class="process-retry-badge">Retry ${v.retries}</span>`:"",y=v.channel_id,u={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(u[y]?y=u[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),i)return`
        <div class="process-history-item" data-channel-id="${v.channel_id}-${v.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
                <i class='bx bx-check-circle' style="color: #03dac6; font-size: 1.1em;"></i>
                <span style="color: #fff; font-weight: 600;">${y}</span>
                <span style="color: #666; font-size: 0.9em; background: rgba(0,0,0,0.2); padding: 1px 6px; border-radius: 4px;">${v.state}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${v.channel_id}-${v.start_time}" style="">
                    <div class="service-widget-header">
                        <i class="bx bx-cog" style="color: ${b}"></i>
                        <h3 style="color: ${b}">${y}</h3>
                        ${m}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">State:</span>
                            <span class="info-value" style="color: ${b}">${v.state}</span>
                        </div>
                        <div class="service-widget-info">
                         ...
                         <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${b}">${v.pid}</span>
                        </div>
                    </div>
                </div>`}let s=i?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(s)).map(v=>[v.dataset.channelId,v])),o=new Set(e.map(v=>`${v.channel_id}-${v.start_time}`));for(let[v,p]of c)o.has(v)||p.remove();e.forEach(v=>{let p=`${v.channel_id}-${v.start_time}`,m=a(v),y=c.get(p);if(y){y.outerHTML!==m&&(y.outerHTML=m);let u=t.querySelector(`[data-channel-id="${p}"]`);u&&t.appendChild(u)}else t.insertAdjacentHTML("beforeend",m)})}function Be(){let t=Ne(),e={enabled:Notification.permission==="granted",supported:"Notification"in window};return`
            <div class="theme-selector">
                ${Object.values(ae).map(i=>`
                    <div class="theme-card ${t===i?"active":""}" data-theme="${i}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${i===ae.DARK?"Simple, clean, dark.":i===ae.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],hs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,a]of Object.entries(t)){let s=Pt.filter(c=>c.category===i);s.length!==0&&(e+=`
                    ...
                    ${s.map(c=>`
                        <div class="cli-command-card ${c.dummy?"dummy":""}" data-cmd="${c.id}" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 25px; transition: all 0.3s ease; cursor: pointer; position: relative; overflow: hidden;">
                            ...
                            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px; position: relative; z-index: 1;">
                                <i class='bx ${c.icon}' style="font-size: 2em; color: ${a.color};"></i>
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Nt(),document.getElementById("terminal-action-btn").onclick=()=>Nt());let i=document.getElementById("cli-terminal-body"),a=document.getElementById("cli-docs-pane");i.innerHTML="";let s=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return a.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),We=!0,i}function Nt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),We=!1}function Ht(t,e,i="output"){if(!We)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=Te(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Cs(t){let e=Pt.find(s=>s.id===t);if(!e)return;let i=ws(e);Ht(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";s.includes("[ERROR]")?c="error":s.includes("[SUCCESS]")||s.includes("\u2713")?c="success":s.includes("!")&&(c="warning"),Ht(i,s,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Rt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=hs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;Cs(a)})})}async function $s(){if(!D())try{if(!(await z("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ot(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}$s(),nt(),Ye();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Rt(),Xe(),Qe(),(()=>{let r=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),f=document.getElementById("close-all-windows"),w=document.getElementById("nav-left-container"),k=document.getElementById("dexter-dropdown"),_=document.getElementById("windows-container");r&&k&&(r.onclick=S=>{S.stopPropagation();let h=window.innerWidth<880;k.classList.toggle("active"),r.classList.toggle("active");let H=k.classList.contains("active");if(h){let T=document.querySelector("nav");H?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),T?.classList.add("window-open"),_?.classList.add("menu-open"),fe(!0)):(_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),T?.classList.remove("window-open"),fe(!1)))}}),d&&(d.onclick=S=>{S.stopPropagation(),m(x)}),f&&(f.onclick=S=>{S.stopPropagation(),y()});let A=(S,h,H)=>{let T=document.getElementById(S);T&&(T.onclick=I=>{I.stopPropagation(),l(),H&&o(H),m(h)})};A("alerts-menu-item",u,"alerts-window"),A("events-menu-item",b,"events-window"),A("monitor-menu-item",g,"monitor-window"),A("contacts-menu-item",n,"contacts-window"),A("workspace-menu-item",E,"workspace-window"),w&&(w.onclick=()=>{if(k&&k.classList.contains("active")){k.classList.remove("active"),r?.classList.remove("active"),_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(s.length>0)y();else{let S=window.location.pathname;if(!(S==="/"||S==="/index.html")){let T=(S.endsWith("/")&&S.length>1?S.slice(0,-1):S).split("/");T.pop();let I=T.join("/")||"/";window.location.href=I}}}),document.addEventListener("click",()=>{let S=window.innerWidth<880;k&&k.classList.contains("active")&&(k.classList.remove("active"),r?.classList.remove("active"),S&&(_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1))))}),k&&(k.onclick=S=>S.stopPropagation())})();let i=window.location.pathname==="/"||window.location.pathname==="/index.html",a=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!i&&!a&&document.querySelector("footer")?.classList.add("hide");let s=[],c=document.getElementById("windows-container");c&&c.setAttribute("data-count","0");let o=r=>{localStorage.setItem("dex_last_window",r)};function v(){return 1}function p(){for(;s.length>1;)s.shift().close(!0);let r=document.getElementById("windows-container"),d=document.querySelector("nav"),f=document.querySelector("footer"),w=window.location.pathname==="/"||window.location.pathname==="/index.html",k=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",s.length),s.forEach(T=>{let I=document.getElementById(T.id);I&&I.classList.remove("hide-close")});let _=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon"),h=document.getElementById("close-all-windows"),H=window.innerWidth<880;if(H||(dropdown&&dropdown.classList.remove("active"),menuBtn&&menuBtn.classList.remove("active"),r&&r.classList.remove("menu-open")),fe(s.length>0),s.length>0)if(f?.classList.add("hide"),h&&(h.style.display=H?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),d?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),_&&(_.style.display=H?"flex":"none"),S&&(S.style.display=H?"block":"none"),!H&&A){let T=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(T)?(A.innerHTML=`
                      <div class="nav-switch-btn ${T==="alerts-window"?"active":""}" id="switch-alerts"><i class='bx bx-bell'></i> Alerts</div>
                      <div class="nav-switch-btn ${T==="events-window"?"active":""}" id="switch-events"><i class='bx bx-calendar-event'></i> Events</div>
                      <div class="nav-switch-btn ${T==="monitor-window"?"active":""}" id="switch-monitor"><i class='bx bx-pulse'></i> Monitor</div>
                      <div class="nav-switch-btn ${T==="contacts-window"?"active":""}" id="switch-contacts"><i class='bx bx-book-content'></i> Contacts</div>
                      <div class="nav-switch-btn ${T==="workspace-window"?"active":""}" id="switch-workspace"><i class='bx bx-brain'></i> Workspace</div>
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{o("alerts-window"),m(u)}),document.getElementById("switch-events").addEventListener("click",()=>{o("events-window"),m(b)}),document.getElementById("switch-monitor").addEventListener("click",()=>{o("monitor-window"),m(g)}),document.getElementById("switch-contacts").addEventListener("click",()=>{o("contacts-window"),m(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{o("workspace-window"),m(E)})):A.innerHTML=""}else A&&(A.innerHTML="");else d?.classList.remove("window-open"),h&&(h.style.display="none"),r&&(r.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),w||k?f?.classList.remove("hide"):f?.classList.add("hide"),_&&(_.style.display="flex"),S&&(S.style.display="block"),A&&(A.innerHTML="");ve()}function m(r){if(r.isOpen()){r.close();return}for(;s.length>0;)s.pop().close(!0);s.push(r),r.open(),p()}function y(){[...s].forEach(d=>d.close()),s=[]}window.addEventListener("resize",p);let u=ne({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:dt(),onOpen:()=>se(),onClose:()=>{let r=s.indexOf(u);r>-1&&s.splice(r,1),p()}}),b=ne({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{b.setContent(qe()),ie()},onClose:()=>{let r=s.indexOf(b);r>-1&&s.splice(r,1),p()}}),n=ne({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:wt(),onOpen:()=>Ue(),onClose:()=>{let r=s.indexOf(n);r>-1&&s.splice(r,1),p()}}),g=ne({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:At()},{icon:"bxs-component",title:"Processes",content:Mt()},{icon:"bxs-brain",title:"Models",content:It()},{icon:"bxs-hdd",title:"Hardware",content:_t()},{icon:"bxs-terminal",title:"Logs",content:Bt()},{icon:"bxs-zap",title:"Agents",content:Lt()}].filter(r=>D()?r.title!=="Hardware"&&r.title!=="Logs":!0),onOpen:()=>{Fe(),$e(),Je(),Ce()},onClose:()=>{let r=s.indexOf(g);r>-1&&s.splice(r,1),p()}}),E=ne({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:xt(),onOpen:()=>Re(),onClose:()=>{let r=s.indexOf(E);r>-1&&s.splice(r,1),p()}}),x=ne({id:"settings-window",title:"Settings",icon:"bx-cog",content:Be(),onOpen:()=>{x.setContent(Be()),Ge(x)},onClose:()=>{let r=s.indexOf(x);r>-1&&s.splice(r,1),p()}});window.dexter={viewEvent:r=>{b.isOpen()||m(b),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${r}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:r=>{u.isOpen()||m(u),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${r}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let l=()=>{let r=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");r&&d&&(r.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{l(),o("alerts-window"),m(u)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{l(),o("events-window"),m(b)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{l(),o("monitor-window"),m(g)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{l(),o("contacts-window"),m(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{l(),o("workspace-window"),m(E)}),D()||setInterval(()=>{b.isOpen()&&ie(),u.isOpen()?se():Pe()},1e3),setInterval(()=>{D()&&(u.isOpen()?se():Pe()),E.isOpen()?Re():vt(),D()&&b.isOpen()&&ie(),g.isOpen()&&Dt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ot):Ot();})();

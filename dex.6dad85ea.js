(()=>{function _e(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ae(t=!1){let a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Me=1e3;function G(t){let e=null,a=t.onClose||null,o=t.onOpen||null;function i(){e&&(e.style.zIndex=++Me)}function c(){if(e){e.classList.add("open"),i(),window.addEventListener("resize",d),o&&setTimeout(o,10);return}let l=document.getElementById("windows-container");l||(l=document.createElement("div"),l.id="windows-container",l.className="windows-container",document.body.appendChild(l)),e=document.createElement("div"),e.id=t.id,e.className="window",e.style.zIndex=++Me,e.addEventListener("mousedown",i);let m=t.icon||"bx-window",s="",n="",r;t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((w,p)=>{let v=w.icon?`<i class="bx ${w.icon}"></i>`:`<span class="tab-glyph">${w.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
...
`,Re=null,Y=new Set,We=[];async function re(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;xt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await E(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let c=(await o.json()).events||[];if(We=c,Re=Date.now(),D(2,Re),c.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),P(1,0);return}t&&(e.innerHTML="");let d=l=>{let m=l.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let s=m.title||"Untitled Blueprint",n=m.summary||m.body||"No summary provided.",r=m.content||"",h=m.category||"architecture",x=m.affected_services||[],w=m.implementation_path||[],p=new Date(l.timestamp*1e3),v=p.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=p.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Y.has(l.id),C=S?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""}`,L.dataset.blueprintId=l.id,L.onclick=function(N){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(l.id);let I=this.querySelector(".event-details");I&&(I.style.display="none")}else{this.classList.add("expanded"),Y.add(l.id);let I=this.querySelector(".event-details");I&&(I.style.display="block")}};let H=x.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${x.join(", ")}</div>`:"",_="";w.length>0&&(_=`
...
`),u=[...d.logs];if(u.length<25){let b=25-u.length;for(let l=0;l<b;l++)u.push("")}else u.length>25&&(u=u.slice(-25));let f=u.map(b=>ae(b)).join(`
`);return`
    ...
    `;async function we(){await Promise.all([at(),_t(),At()]),await xe()}var Qe=null,et=null,tt=null;async function it(){try{return await(await E("/system_monitor")).json()}catch{return null}}async function st(){try{return await(await E("/system/hardware")).json()}catch{return null}}async function Ct(){try{return await(await E("/processes")).json()}catch{return null}}async function St(){try{return await(await E("/analyst/status")).json()}catch{return null}}async function _t(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=s=>{if(!s){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let n="",r=(s.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(n+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`:_='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${r}" data-service-id="${s.id}"><div class="service-widget-header"><i class="bx ${h}"></i><h3>${s.short_name||s.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${u(s.domain&&s.port?`${s.domain}:${s.port}`:"")}</span></div>${_}</div></div>`}let l=new Map(Array.from(t.querySelectorAll(".service-widget")).map(s=>[s.dataset.serviceId,s])),m=new Set(c.map(s=>s.id));for(let[s,n]of l)m.has(s)||n.remove();c.forEach(s=>{let n=b(s),r=l.get(s.id);r?r.outerHTML!==n&&(r.outerHTML=n):t.insertAdjacentHTML("beforeend",n)})}async function At(){let t=document.getElementById("models-widgets");if(!t)return;let e=await it();if(!e){t.innerHTML=$("offline","Failed to load model status.");return}et=Date.now(),D(3,et);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(g=>{g.classList.contains("service-widget")||g.remove()});function i(g){let u=g.status==="Ready";return`
...
`).forEach(b=>{if(b.trim()==="")return;let l="output";b.includes("[ERROR]")?l="error":b.includes("[SUCCESS]")||b.includes("\u2713")?l="success":b.includes("!")&&(l="warning"),Le(a,b,l)})}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed"}catch(o){Le(a,`Connection Error: ${o.message}`,"error"),document.getElementById("terminal-status-badge").className="terminal-status status-error",document.getElementById("terminal-status-badge").textContent="Failed"}finally{document.getElementById("terminal-action-btn").style.display="block"}}function rt(){let t=document.getElementById("cli-interface-container");t&&(t.innerHTML=It(),t.querySelectorAll(".cli-command-card").forEach(e=>{e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-5px)",e.style.borderColor="rgba(255,255,255,0.15)",e.style.backgroundColor="rgba(255,255,255,0.05)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)",e.style.borderColor="rgba(255,255,255,0.05)",e.style.backgroundColor="rgba(255,255,255,0.03)"}),e.addEventListener("click",()=>{let a=e.dataset.cmd;if(a==="chat"){alert("Connection to cognitive core pending deployment...");return}Bt(a)})}))}async function Dt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${be()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function ct(){Dt(),ze(),_e(),window.location.pathname.includes("/dex")&&rt();let t=Be();Ae(t),Ie();let e=[];function a(){let l=window.innerWidth;return l>=2500?4:l>=1800?3:l>=1200?2:1}function o(){let l=a();for(;e.length>l;)e.shift().close(!0);let m=document.getElementById("windows-container");m&&m.setAttribute("data-count",e.length)}function i(l){if(l.isOpen()){l.close();return}let m=a();e.length>=m&&e.shift().close(!0),e.push(l),l.open(),o()}function c(){[...e].forEach(m=>m.close()),e=[]}window.addEventListener("resize",o);let d=G({id:"feed-window",icon:"bx-news",tabs:[{icon:"bx-bell",title:"Notifications",content:Ue()},{icon:"bx-calendar-event",title:"Events",content:Xe()}],onOpen:()=>{F(),Z()},onClose:()=>{let l=e.indexOf(d);l>-1&&e.splice(l,1),o()}}),g=G({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bx-server",title:"System",content:nt()},{icon:"bx-book-content",title:"Contacts",content:Ke()}],onOpen:()=>{we(),he()},onClose:()=>{let l=e.indexOf(g);l>-1&&e.splice(l,1),o()}}),u=G({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Ge(),onOpen:()=>ye(),onClose:()=>{let l=e.indexOf(u);l>-1&&e.splice(l,1),o()}}),f=G({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ee(),onOpen:()=>$e(f),onClose:()=>{let l=e.indexOf(f);l>-1&&e.splice(l,1),o()}}),b=G({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="https://github.com/eastercompany" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});t?(document.getElementById("feed-icon")?.addEventListener("click",()=>i(d)),document.getElementById("monitor-icon")?.addEventListener("click",()=>i(g)),document.getElementById("workspace-icon")?.addEventListener("click",()=>i(u)),document.getElementById("settings-icon")?.addEventListener("click",()=>i(f)),document.getElementById("close-all-windows")?.addEventListener("click",()=>c()),setInterval(()=>{d.isOpen()&&(F(),Z()),g.isOpen()&&we(),u.isOpen()&&ye()},5e3)):document.getElementById("login-btn")?.addEventListener("click",()=>{b.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",l=>{l.preventDefault();try{He(document.getElementById("email-input").value),window.location.reload()}catch(m){let s=document.getElementById("login-error");s&&(s.textContent=m.message,s.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ct):ct();})();

(()=>{function Xe(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Qe(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var tt=1e3;function ae(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++tt)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",o),s&&setTimeout(s,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++tt,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",a="",g="",$;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((r,d)=>{let f=r.icon?`<i class="bx ${r.icon}"></i>`:`<span class="tab-glyph">${r.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,$=`<div class="window-content">${t.tabs.map((r,d)=>`<div class="tab-content ${d===0?"active":""}" data-tab-content="${d}">${r.content}</div>`).join("")}</div>`):(t.title&&(g=`<div class="window-title">${t.title}</div>`),$=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Ae=null,re=new Set,Ie=[];async function ie(t=!1){let e=document.getElementById("alerts-list");if(!e)return;rs(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ae=Date.now(),X(0,Ae);let o=Date.now(),v=24*60*60*1e3,l=c.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let x=parseInt(f);return o-x<v});l.sort((d,f)=>{let x=L=>{let S=L.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},A=L=>L==="critical"?4:L==="high"?3:L==="medium"?2:1,_=A(x(d)),T=A(x(f));return _!==T?T-_:f.timestamp-d.timestamp}),Ie=l;let m=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},h=[],b=new Set(l.map(d=>m(d))).size>1;if(l.length>0){let d=null;l.forEach(f=>{let x=m(f);b&&x!==d&&(h.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),h.push(f)})}if(t&&(e.innerHTML=""),l.length===0){e.innerHTML=H("empty","No alerts yet."),Te();return}let a=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),A=(f.body||"No description provided.").trim(),_=f.summary||"",T=f.content||"",L=f.protocol||"",S=(f.priority||"low").toLowerCase(),I=!!f.alert,U=(f.category||"system").trim(),E=f.related_event_ids||[],j=f.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),k=new Date(d.timestamp*1e3),w=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=q?"event-border-grey":"event-border-blue";!q&&I&&(M="event-border-red"),q&&(S==="high"||S==="critical")?M="event-border-red":q&&S==="medium"&&(M="event-border-orange");let P=q?"alert-read":"alert-unread",D=re.has(d.id),F=D?"expanded":"",J=D?"display: block;":"display: none;",W="",G="";E.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${E.map(se=>`<a href="#" onclick="window.dexter.viewEvent('${se}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${se.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let V="";j&&(V=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(T)}</div>
            </div>
        `:Q=`
            <div class="event-detail-block" style="text-align: left;">
...
`,le=new Set,Be=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ls();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();Be=s;let n=m=>{let h=le.has(m.id),p=m.state==="published",b=m.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),b&&(a="event-border-purple");let $=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${a} cursor-pointer ${h?"expanded":""}`,y.dataset.itemId=m.id,y.onclick=d=>{if(d.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",le.delete(m.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",le.add(m.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;y.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${O(m.content)}</div>
...
`,gt=null,de=new Set,xt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ms();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(xt=c,gt=Date.now(),X(2,gt),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),y=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(D=>D.trim()),r=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(p.timestamp*1e3),A=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=de.has(p.id),L=T?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${T?"expanded":""} ${f?"blueprint-approved":""}`,S.dataset.blueprintId=p.id,f&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";S.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let E="";r.length>0&&(E=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${r.map(D=>`<li style="margin-bottom: 5px;">${O(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let j="";d.length>0&&(j=`
                ...
                `);let N=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",q=z(),k=f?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${L}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${O(g)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${O($)}</div>
                        </div>
                        ${E}
                        ${j}
                        ${k}
            ...
            `;let w=S.querySelector(".blueprint-approve-btn");w&&(w.onclick=async D=>{D.stopPropagation(),w.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${p.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(F){console.error("Failed to approve blueprint:",F)}});let C=S.querySelector(".blueprint-delete-btn");C&&(C.onclick=async D=>{D.stopPropagation();let F=!f;C.innerHTML=F?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${p.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(J){console.error("Failed to delete blueprint:",J)}});let M=S.querySelector(".event-details");M&&M.addEventListener("click",D=>{D.stopPropagation()});let P=S.querySelector(".close-details-btn");return P&&P.addEventListener("click",D=>{D.stopPropagation(),S.classList.remove("expanded");let F=S.querySelector(".event-details");F&&(F.style.display="none"),de.delete(p.id)}),S},v=Array.from(e.children),l=new Map(v.map(p=>[p.dataset.blueprintId,p])),m=new Set(c.map(p=>p.id));v.forEach(p=>{let b=p.dataset.blueprintId;(!b||!m.has(b))&&p.remove()});let h=null;c.forEach((p,b)=>{let a=l.get(p.id);(!a||t)&&(a&&a.remove(),a=o(p),!a)||(b===0?e.firstElementChild!==a&&e.prepend(a):h&&h.nextElementSibling!==a&&h.after(a),h=a)}),we(2,c.length),ct()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=H("offline","Failed to load blueprints.","The event service may be offline."))}}async function yt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let o=c.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&n++}),lt(n)}catch{}}function ms(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xt.forEach(s=>de.add(s.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var ht=()=>`
...
`;function Ct(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function $t(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=us,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Me(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();Ue(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=Ct(t.id,t.username);Ue(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=Ct(t.id,t.username);Ue(e,t,s)})}function Ue(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(i.badges||[]).map(A=>`<span class="profile-badge ${A==="Creator"?"master":""}">${A}</span>`).join(""),y=(i.attributes||i.facts||[]).map(A=>{let _=A.key||A.k,T=A.value||A.v;return`
                ...
                <div class="profile-section">
                    ...
                    <div class="cog-metric">
                        ...
                        <div class="cog-bar-bg">
                            <div class="cog-bar-fill" style="width: ${Math.min(r*10,100)}%;"></div>
                        </div>
                    </div>
        ...
        `},c=()=>{let $=i.dossier||{},y=$.identity||{},u=$.career||{},r=$.personal||{},d=$.social||[];return`
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.hobbies||[]).map(f=>`<div class="dossier-list-item">${f}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.habits||[]).map(f=>`<div class="dossier-list-item">${f}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.vices||[]).map(f=>`<div class="dossier-list-item" style="color: #cf6679;">${f}</div>`).join("")}
                        </div>
                    </div>
                </div>
    ...
    `;t.innerHTML=m;let h=t.querySelector(".profile-card"),p=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),a=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),b.forEach($=>{$.addEventListener("click",()=>{b.forEach(y=>y.classList.remove("active")),a.forEach(y=>y.classList.remove("active")),$.classList.add("active"),t.querySelector(`#tab-${$.dataset.tab}`).classList.add("active")})}),p.addEventListener("click",()=>{h.classList.toggle("expanded");let $=h.classList.contains("expanded");p.innerHTML=$?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var St=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,fs=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);$t(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(fs=Date.now(),n.length===0){t.innerHTML=H("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((o,v)=>{let l=c[o.level]??10,m=c[v.level]??10;return l!==m?l-m:o.username.localeCompare(v.username)}),t.innerHTML=n.map(o=>{let v=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",l=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",m="#888";o.level==="Me"||o.level==="Master"?m="#bb86fc":o.level==="Admin"?m="#cf6679":o.level==="Moderator"?m="#03dac6":o.level==="Contributor"&&(m="#ffa500");let h=o.level==="Me",p=h?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=h?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-card" 
                    ...
                    <div style="position: relative;">
                        <img src="${o.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${h?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${m}; font-weight: 600; text-transform: uppercase;">${h?"DEXTER (ME)":o.level}</span>
                        </div>
...
`,_e=null,me=new Set,Ce=[],K="all",bs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},vs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function xs(t){for(let[e,i]of Object.entries(bs))if(i.includes(t))return e;return"system"}function ys(t){return vs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;hs();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,$=xs(g),y=ys(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),r="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?r="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?r="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?r="event-border-orange":g==="system.test.completed"?r=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":r="event-border-blue");let d=u?"cursor-pointer":"",f=me.has(b.id),x=f?"expanded":"",A=f?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),T=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Et(g,a),I=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",U="";if(u){let k="";if(g==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                    ...
                    `}else if(g==="system.attention.expired"){let w=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C=a.timestamp-a.last_active,M=Math.floor(C/60);k=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let w=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C="";a.eval_count&&(C=`
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let P=a.chat_history.length,D=a.chat_history.map((F,J)=>{let W=F.name?F.name.toUpperCase():F.role.toUpperCase();!F.name&&W==="ASSISTANT"&&(W="DEXTER");let G=F.role==="user"?"#03dac6":F.role==="system"?"#ffb74d":"#bb86fc",V=J===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${V};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${O(F.content)}</div>
                                </div>
                            `}).join("");M=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let w=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C=a.signals||{};k=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${w("Raw Model Output")}
                            <pre class="detail-pre">${O(a.raw_output)||"None"}</pre>
                        </div>
                    `}else if(g==="moderation.explicit_content.deleted"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${O(a.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${w("Description")}
                            <pre class="detail-pre">${O(a.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${w("Content Summary")}
                            <pre class="detail-pre">${O(a.summary)||"None"}</pre>
                        </div>
                    `}else if(g==="analysis.visual.completed"){let w=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,C="";a.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(C=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${w("Input Context")}
                            <pre class="detail-pre">${O(a.raw_input)||"None"}</pre>
                        </div>
                    `}else if(g==="system.cli.command"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${w("Output")}
                            <pre class="detail-pre">${O(a.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(g==="system.analysis.audit"){let w=a.success?"#03dac6":"#ff4d4d",C=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,P="";a.error&&(P=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${O(a.error)}</pre>
                            </div>
                        ...
                        `}let F="";if(a.corrections&&a.corrections.length>0){let W=a.corrections.map((G,V)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${G.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${O(G.snippet)}</div>`:""}
                            </div>
                        `).join("");F=`
                            <div class="event-detail-block">
                        ...
                        `}let J="";if(a.parsed_results&&a.parsed_results.length>0){let W=a.parsed_results.map(G=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${O(G.title)}</div>
                                <div style="color: #ddd;">${O(G.summary)}</div>
                            </div>
                        `).join("");J=`
                            <div class="event-detail-block">
                                ${M("Parsed Results")}
                        ...
                        ${F}
                        ${D}
                    `}else if(g==="system.test.completed"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                        ...
                        <div class="event-detail-block">
                            ${w("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${O(a.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${w("Context")}
                            <pre class="detail-pre">${JSON.stringify(a.context||{},null,2)}</pre>
                        </div>
                    `}else if(g==="system.cli.status"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ${w("Message")}
                            <pre class="detail-pre">${O(a.message)}</pre>
                        </div>
                ...
                `}let E=document.createElement("div");E.className=`event-item ${r} ${d} ${x}`,E.dataset.eventId=b.id,E.onclick=function(k){if(!u)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},E.innerHTML=`
                <div class="event-time">
...
`,kt=ws;function Tt(){return`
    ...
    `}var Cs=null;async function $e(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=H("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let s=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],n=i.filter(o=>!s.includes(o.id));n.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),n.reverse();let c=n.map(o=>{let v=o.logs.join(`
`),l=[...o.logs];if(l.length<25){let h=25-l.length;for(let p=0;p<h;p++)l.push("")}else l.length>25&&(l=l.slice(-25));let m=l.map(h=>Le(h)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let v=unescape(o.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let l=o.querySelector("i");l.classList.remove("bx-copy"),l.classList.add("bx-check"),setTimeout(()=>{l.classList.remove("bx-check"),l.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let v=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await B(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&$e()}catch(l){console.error("Error clearing logs:",l)}})}),Cs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=H("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var It=()=>{let t=z()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Tt()}
                ...
                </div>`),n){let d=(r.MEMORY_BYTES/1073741824).toFixed(1);n.innerHTML=`
                ...
                </div>`}if(s&&r.CPU&&r.CPU.length>0){let d=r.CPU[0];s.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}c&&(r.GPU&&r.GPU.length>0?c.innerHTML=r.GPU.map(d=>{let f=(d.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&r.STORAGE&&r.STORAGE.length>0?o.innerHTML=r.STORAGE.map(d=>{let f=(d.USED/1073741824).toFixed(1),x=(d.SIZE/(1024*1024*1024)).toFixed(1),A=d.SIZE>0?(d.USED/d.SIZE*100).toFixed(0):0,_=d.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            ...
                            <span style="font-size: 0.75em; color: #666; font-family: monospace; background: rgba(0,0,0,0.2); padding: 2px 6px; border-radius: 4px;">${_}</span>
                        </div>
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let r=await At();r?(v(r),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let r=await At();v(r)}if(!t)return;let l=await Pt();if(!l||!l.services){t.children.length===0&&(t.innerHTML=H("offline","Failed to load system metrics.","The event service may be offline."));return}Lt=Date.now(),X(0,Lt);let m=l.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function h(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function p(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:r.split(".").slice(0,3).join(".")||"-"}function b(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function a(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,x=parseInt(d[2])||0,A=parseInt(d[3])||0,_=parseFloat(d[4])||0,T=f*86400+x*3600+A*60+_,L=Math.floor(T/86400);if(L>0)return`${L}d`;let S=Math.floor(T/3600);if(S>0)return`${S}h`;let I=Math.floor(T/60);return I>0?`${I}m`:`${Math.floor(T)}s`}function g(r){let d=r.status==="online",f=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",A=d?"OK":"BAD",_=r.version?p(r.version.str):"-",T=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",L=r.memory&&r.memory!=="N/A"?r.memory:"-",S=T!=="-"?"#00ff00":"#666",I=T!=="-"?"#fff":"#666",U=L!=="-"?"#00ff00":"#666",E=L!=="-"?"#fff":"#666",j=a(r.uptime),N="";d?N=`
                <div class="service-widget-info">
                    <span class="info-label">Version:</span>
                    <span class="info-value metric-version-monospace">${_}</span>
                </div>
                ...
                </div>`:N='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let q=z()?"easter.company":b(r.domain&&r.port?`${r.domain}:${r.port}`:"");return`<div class="service-widget ${f}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${A}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${q}</span></div>${N}</div></div>`}let $=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),y=new Set(m.map(r=>r.id));for(let[r,d]of $)y.has(r)||d.remove();let u=new Set;m.forEach(r=>{if(u.has(r.id))return;u.add(r.id);let d=g(r),f=$.get(r.id);f&&f.parentNode?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function We(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Pt();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}Mt=Date.now(),X(2,Mt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let m=l.status==="Ready";return`
                ...
                </div>`}function o(l){let m=l.status==="Downloaded",h=m?"service-widget-online":"service-widget-offline",p=m?"OK":"MISSING",b=m&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",a=l.name;return l.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${h}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(s&&(v+=n(s)),e.xtts&&(v+=c(e.xtts)),v+=i.map(o).join(""),!v){t.innerHTML=H("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function Se(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),l=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=async()=>{l.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{l.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{l.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{l.innerHTML="<i class='bx bx-error'></i>"}},l.dataset.listenerAttached="true");let m=await Ss();if(m&&m.agents){let y=m.agents.guardian||{protocols:{}},u=m.agents.analyzer||{protocols:{}},r=m.system||{},d=Math.floor(Date.now()/1e3),f={sentry:"Sentry",architect:"Architect",synthesis:"Synthesis",alert_review:"Alert Review"},x=E=>{E<0&&(E=0);let j=Math.floor(E/3600),N=Math.floor(E%3600/60),q=E%60;return j>0?`${j}h ${N}m`:N>0?`${N}m ${q}s`:`${q}s`},A=(E,j,N,q)=>{if(!E)return;let k=f[q]||q.toUpperCase(),w=E.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(w&&(w.textContent=k),!N){E.textContent="Inactive",E.style.color="#666";return}let C=N.status;if(C==="Working")E.textContent="Working",E.style.color="#bb86fc";else if(C==="Ready")E.textContent="Ready",E.style.color="#5eff5e";else{let M=N.cooldown;if(z()&&(M=N.next_run-d),M<=0)E.textContent="Ready",E.style.color="#5eff5e";else{let P=Math.floor(M/60),D=M%60;E.textContent=`${P}m ${D}s`,E.style.color="#fff"}}j&&N.stats&&(j.innerHTML=`
          ...
          `)};e&&A(e,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry"),i&&A(i,document.getElementById("guardian-architect-stats"),y.protocols.architect,"architect");let _=document.getElementById("analyzer-synthesis-val"),T=document.getElementById("analyzer-synthesis-stats");_&&A(_,T,u.protocols.synthesis,"synthesis");let L=document.getElementById("imaginator-alert_review-val"),S=document.getElementById("imaginator-alert_review-stats");L&&A(L,S,y.protocols.alert_review,"alert_review");let I=document.getElementById("system-state-label"),U=document.getElementById("system-state-val");if(U&&r.state){let E=r.state,j=x(r.state_time||0);I&&(I.textContent=`State: ${E.toUpperCase()}`),U.textContent=j,E==="idle"?U.style.color=r.state_time>300?"#5eff5e":"#fff":U.style.color="#bb86fc"}n&&(n.textContent=x(r.metrics?.total_idle_time||0)),c&&(c.textContent=x(r.metrics?.total_active_time||0)),o&&(o.textContent=x(r.metrics?.total_waste_time||0))}else[e,i,s,n,c,o].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let h=await $s(),p=[],b=[],a=[];h&&(Array.isArray(h)?p=h:(p=h.active||[],b=h.queue||[],a=h.history||[],a.sort((y,u)=>(u.end_time||0)-(y.end_time||0)))),p.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=H("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Fe(t,p,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Fe(g,b,!1)));let $=document.getElementById("processes-history-widgets");$&&(a.length===0?$.querySelector(".tab-placeholder")||($.innerHTML=H("empty","No recent history.")):($.querySelector(".tab-placeholder")&&($.innerHTML=""),Fe($,a,!0))),we(1,p.length)}function Fe(t,e,i){function s(l){let m="";l.end_time?m=`${l.end_time-l.start_time}s`:m=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let h=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[p]?p=b[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
    ...
    `;async function Ut(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Rt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Rt(t)}async function Rt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=H("empty","No web history found.");return}let s=`
        ...
        `;i.forEach((v,l)=>{let m=new Date(v.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),h=v.screenshot?`<img src="data:image/png;base64,${v.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ve=!0,i}function jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ve=!1}function qt(t,e,i="output"){if(!Ve)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Ts(t){let e=Ft.find(n=>n.id===t);if(!e)return;let i=ks(e);qt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),qt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Jt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Es();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Ts(s)})})}async function Ls(){if(!z())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Wt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}Qe(),et(),Ls(),at(),Xe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Jt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),r=document.querySelector("nav"),d=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(I=>{let U=document.getElementById(I.id);U&&U.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),T=document.getElementById("settings-icon"),L=document.getElementById("close-all-windows"),S=window.innerWidth<880;if(!S){let I=document.getElementById("dexter-dropdown"),U=document.getElementById("dexter-menu-btn");I&&I.classList.remove("active"),U&&U.classList.remove("active"),u&&u.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),L&&(L.style.display=S?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",r?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),A&&(A.style.display=S?"flex":"none"),T&&(T.style.display=S?"block":"none"),!S&&_){let I=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(I)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(l)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(h)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(m)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):_.innerHTML=""}else _&&(_.innerHTML="");else r?.classList.remove("window-open"),L&&(L.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?d?.classList.remove("hide"):d?.classList.add("hide"),A&&(A.style.display="flex"),T&&(T.style.display="block"),_&&(_.innerHTML="");ve()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function o(){[...e].forEach(r=>r.close()),e=[]}window.addEventListener("resize",n);let v=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:mt(),onOpen:()=>ie(),onClose:()=>{let u=e.indexOf(v);u>-1&&e.splice(u,1),n()}}),l=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{l.setContent(qe()),ne()},onClose:()=>{let u=e.indexOf(l);u>-1&&e.splice(u,1),n()}}),m=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:St(),onOpen:()=>je(),onClose:()=>{let u=e.indexOf(m);u>-1&&e.splice(u,1),n()}}),h=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:_t()},{icon:"bxs-component",title:"Processes",content:Bt()},{icon:"bxs-brain",title:"Models",content:Dt()},{icon:"bx-globe",title:"Web",content:Ot()},{icon:"bxs-hdd",title:"Hardware",content:zt()},{icon:"bxs-terminal",title:"Logs",content:Nt()},{icon:"bxs-zap",title:"Agents",content:It()},{icon:"bx-task",title:"Chores",content:wt()}].filter(u=>z()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Je(),Se(),We(),Ut(),$e(),ye()},onClose:()=>{let u=e.indexOf(h);u>-1&&e.splice(u,1),n()}}),p=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ht(),onOpen:()=>Oe(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{b.setContent(De()),Ge(b)},onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{l.isOpen()||c(l),setTimeout(()=>{let r=document.querySelector(`.event-item[data-event-id="${u}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{v.isOpen()||c(v),setTimeout(()=>{let r=document.querySelector(`.event-item[data-alert-id="${u}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),r=document.getElementById("dexter-menu-btn");u&&r&&(u.classList.remove("active"),r.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),r=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),A=document.getElementById("windows-container");u&&x&&(u.onclick=T=>{T.stopPropagation();let L=window.innerWidth<880;if(L)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let I=u.getBoundingClientRect(),E=document.querySelector("nav").getBoundingClientRect(),j=220,N=10,k=I.left+I.width/2-j/2,w=E.right-10;k+j>w&&(k=w-j),k<10&&(k=10),x.style.top=E.bottom+N+"px",x.style.left=k+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),u.classList.toggle("active");let S=x.classList.contains("active");if(L){let I=document.querySelector("nav");S?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),I?.classList.add("window-open"),A?.classList.add("menu-open"),fe(!0)):(A?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),I?.classList.remove("window-open"),fe(!1)))}}),r&&(r.onclick=T=>{T.stopPropagation(),c(b)}),d&&(d.onclick=T=>{T.stopPropagation(),o()});let _=(T,L,S)=>{let I=document.getElementById(T);I&&(I.onclick=U=>{U.stopPropagation(),a(),S&&s(S),c(L)})};_("alerts-menu-item",v,"alerts-window"),_("events-menu-item",l,"events-window"),_("monitor-menu-item",h,"monitor-window"),_("contacts-menu-item",m,"contacts-window"),_("workspace-menu-item",p,"workspace-window"),f&&(f.onclick=T=>{if(T.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)o();else{let L=window.location.pathname;if(!(L==="/"||L==="/index.html")){let U=(L.endsWith("/")&&L.length>1?L.slice(0,-1):L).split("/");U.pop();let E=U.join("/")||"/";window.location.href=E}}}),document.addEventListener("click",()=>{let T=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),T&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),x&&(x.onclick=T=>T.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",y=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!y&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{l.isOpen()&&ne(),v.isOpen()?ie():Re()},1e3),setInterval(()=>{z()&&(v.isOpen()?ie():Re()),p.isOpen()?Oe():yt(),z()&&l.isOpen()&&ne(),h.isOpen()&&Ht()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Wt):Wt();})();

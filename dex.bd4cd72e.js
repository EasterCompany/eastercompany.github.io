...
`,yt=ds;function ht(){return`
    ...
    `}var ps=null;async function $e(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await I("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML=R("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=a.filter(o=>!i.includes(o.id));s.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),s.reverse();let r=s.map(o=>{let b=o.logs.join(`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ht()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let l=await $t();l?(b(l),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let l=await $t();b(l)}if(!t)return;let c=await It();if(!c||!c.services){t.children.length===0&&(t.innerHTML=R("offline","Failed to load system metrics.","The event service may be offline."));return}wt=Date.now(),Z(0,wt);let d=c.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:l.split(".").slice(0,3).join(".")||"-"}function g(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let f=parseInt(x[1])||0,v=parseInt(x[2])||0,p=parseInt(x[3])||0,E=parseFloat(x[4])||0,A=f*86400+v*3600+p*60+E,H=Math.floor(A/86400);if(H>0)return`${H}d`;let B=Math.floor(A/3600);if(B>0)return`${B}h`;let h=Math.floor(A/60);return h>0?`${h}m`:`${Math.floor(A)}s`}function u(l){let x=l.status==="online",f=x?"service-widget-online":"service-widget-offline",v=x?"bx-check-circle":"bx-x-circle",p=x?"OK":"BAD",E=l.version?m(l.version.str):"-",A=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",H=l.memory&&l.memory!=="N/A"?l.memory:"-",B=A!=="-"?"#00ff00":"#666",h=A!=="-"?"#fff":"#666",N=H!=="-"?"#00ff00":"#666",D=H!=="-"?"#fff":"#666",M=n(l.uptime),q="";x?q=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${D};">${H}</span>
                    </div>
                </div>`:q='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let O=_()?"easter.company":g(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${v}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${q}</div></div>`}let L=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),$=new Set(d.map(l=>l.id));for(let[l,x]of L)$.has(l)||x.remove();d.forEach(l=>{let x=u(l),f=L.get(l.id);f?f.outerHTML!==x&&(f.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function je(){let t=document.getElementById("models-widgets");if(!t)return;let e=await It();if(!e){t.children.length===0&&(t.innerHTML=R("offline","Failed to load model status.","The event service may be offline."));return}Ct=Date.now(),Z(2,Ct);let a=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let d=c.status==="Ready";return`
        ...
        `)};e&&p(e,document.getElementById("guardian-sentry-stats"),d.sentry,"sentry",l),a&&p(a,document.getElementById("guardian-architect-stats"),d.architect,"architect",l);let E=document.getElementById("analyzer-synthesis-val"),A=document.getElementById("analyzer-synthesis-stats");E&&p(E,A,d.synthesis,"synthesis",x);let H=document.getElementById("system-state-label"),B=document.getElementById("system-state-val");if(B&&d.system_state){let h=d.system_state,N=v(d.system_state_time||0);H&&(H.textContent=`State: ${h.toUpperCase()}`),B.textContent=N,h==="idle"?B.style.color=d.system_state_time>300?"#5eff5e":"#fff":B.style.color="#bb86fc"}s&&(s.textContent=v(d.total_idle_time||0)),r&&(r.textContent=v(d.total_active_time||0)),o&&(o.textContent=v(d.total_waste_time||0))}else[e,a,i,s,r,o].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});let y=await ms(),m=[],g=[],n=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],g=y.queue||[],n=y.history||[],n.sort(($,l)=>(l.end_time||0)-($.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=R("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Ue(t,m,!1));let u=document.getElementById("processes-queue-widgets");u&&(g.length===0?!u.querySelector(".tab-placeholder")&&!u.querySelector('div[style*="font-style: italic"]')&&(u.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(u.innerHTML="",Ue(u,g,!1)));let L=document.getElementById("processes-history-widgets");L&&(n.length===0?L.querySelector(".tab-placeholder")||(L.innerHTML=R("empty","No recent history.")):(L.querySelector(".tab-placeholder")&&(L.innerHTML=""),Ue(L,n,!0))),we(1,m.length)}function Ue(t,e,a){function i(b){let c="";b.end_time?c=`${b.end_time-b.start_time}s`:c=`${Math.floor(Date.now()/1e3-b.start_time)}s`;let d=b.retries>0?`<span class="process-retry-badge">Retry ${b.retries}</span>`:"",y=b.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[y]?y=m[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],gs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Je=!0,a}function _t(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Je=!1}function Bt(t,e,a="output"){if(!Je)return;let i=document.createElement("div");i.className=`terminal-line terminal-${a}`,a==="prompt"?i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:i.innerHTML=Ae(e),t.appendChild(i),t.scrollTop=t.scrollHeight}async function vs(t){let e=Dt.find(s=>s.id===t);if(!e)return;let a=bs(e);Bt(a,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let s of i){await new Promise(o=>setTimeout(o,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),Bt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function zt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=gs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let i=e.querySelector("i");i.className="bx bx-check",i.style.color="#5eff5e",setTimeout(()=>{i.className="bx bx-copy",i.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let i=a.dataset.cmd;vs(i)})})}async function xs(){try{if(!(await I("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ht(){xs(),tt(),Ze();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&zt(),Ke(),Ye();let e=document.getElementById("nav-left-container");e&&e.addEventListener("click",()=>{let p=document.getElementById("dexter-dropdown"),E=document.getElementById("dexter-menu-btn");if(p&&p.classList.contains("active")){p.classList.remove("active"),E.classList.remove("active"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(s.length>0)y();else{let A=window.location.pathname;if(!(A==="/"||A==="/index.html")){let h=(A.endsWith("/")&&A.length>1?A.slice(0,-1):A).split("/");h.pop();let N=h.join("/")||"/";window.location.href=N}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",i=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!i&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let o=p=>{localStorage.setItem("dex_last_window",p)};function b(){return 1}function c(){for(;s.length>1;)s.shift().close(!0);let p=document.getElementById("windows-container"),E=document.querySelector("nav"),A=document.querySelector("footer"),H=window.location.pathname==="/"||window.location.pathname==="/index.html",B=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");p&&p.setAttribute("data-count",s.length),s.forEach(O=>{let j=document.getElementById(O.id);j&&j.classList.remove("hide-close")}),Se(he());let h=document.getElementById("dexter-menu-container"),N=document.getElementById("nav-window-switcher"),D=document.getElementById("settings-icon"),M=document.getElementById("close-all-windows"),q=window.innerWidth<880;if(fe(s.length>0),s.length>0)if(A?.classList.add("hide"),M&&(M.style.display=q?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),E?.classList.add("window-open"),p&&(p.style.paddingTop="60px"),h&&(h.style.display="flex"),D&&(D.style.display="block"),!q&&N){let O=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(O)?(N.innerHTML=`
          ...
          <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
      `);let x=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");x&&l&&f&&(f.addEventListener("click",p=>{p.stopPropagation();let E=window.innerWidth<880,A=l.classList.contains("active");l.classList.toggle("active"),f.classList.toggle("active");let H=l.classList.contains("active");E&&(H?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),navbar?.classList.add("window-open"),fe(!0)):s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),navbar?.classList.remove("window-open"),fe(!1)))}),document.addEventListener("click",()=>{let p=window.innerWidth<880,E=l.classList.contains("active");l.classList.remove("active"),f.classList.remove("active"),p&&E&&s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),navbar?.classList.remove("window-open"),fe(!1))}),l.addEventListener("click",p=>{p.stopPropagation()}));let v=()=>{l&&f&&(l.classList.remove("active"),f.classList.remove("active"))};document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{v(),o("alerts-window"),d(m)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{v(),o("events-window"),d(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{v(),o("monitor-window"),d(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{v(),o("contacts-window"),d(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{v(),o("workspace-window"),d(L)}),document.getElementById("settings-icon")?.addEventListener("click",()=>d($)),document.getElementById("close-all-windows")?.addEventListener("click",()=>y()),_()||setInterval(()=>{g.isOpen()&&ie(),m.isOpen()?ne():Ne()},1e3),setInterval(()=>{_()&&(m.isOpen()?ne():Ne()),L.isOpen()?Re():gt(),_()&&g.isOpen()&&ie(),u.isOpen()&&Mt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ht):Ht();})();

...
`,Ae=null,re=new Set,Ie=[];async function ie(t=!1){let e=document.getElementById("alerts-list");if(!e)return;rs(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ae=Date.now(),X(0,Ae);let o=Date.now(),v=24*60*60*1e3,r=c.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let x=parseInt(f);return o-x<v});r.sort((d,f)=>{let x=T=>{let S=T.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},I=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,_=I(x(d)),k=I(x(f));return _!==k?k-_:f.timestamp-d.timestamp}),Ie=r;let p=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],b=new Set(r.map(d=>p(d))).size>1;if(r.length>0){let d=null;r.forEach(f=>{let x=p(f);b&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(f)})}if(t&&(e.innerHTML=""),r.length===0){e.innerHTML=H("empty","No alerts yet."),Te();return}let a=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),I=(f.body||"No description provided.").trim(),_=f.summary||"",k=f.content||"",T=f.protocol||"",S=(f.priority||"low").toLowerCase(),A=!!f.alert,L=(f.category||"system").trim(),D=f.related_event_ids||[],P=f.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${d.id}`),E=new Date(d.timestamp*1e3),w=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=F?"event-border-grey":"event-border-blue";!F&&A&&(M="event-border-red"),F&&(S==="high"||S==="critical")?M="event-border-red":F&&S==="medium"&&(M="event-border-orange");let R=F?"alert-read":"alert-unread",z=re.has(d.id),q=z?"expanded":"",J=z?"display: block;":"display: none;",W="",G="";D.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${D.map(se=>`<a href="#" onclick="window.dexter.viewEvent('${se}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${se.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let V="";P&&(V=`
...
`,le=new Set,Be=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ls();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();Be=s;let n=p=>{let y=le.has(p.id),m=p.state==="published",b=p.state==="consumed",a="event-border-grey";m&&(a="event-border-blue"),b&&(a="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=p.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(p.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${U(p.content)}</div>
...
`,gt=null,de=new Set,xt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ms();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(xt=c,gt=Date.now(),X(2,gt),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(z=>z.trim()),l=(b.implementation_path||[]).map(z=>z.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(m.timestamp*1e3),I=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=de.has(m.id),T=k?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,S.dataset.blueprintId=m.id,f&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let L=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";S.onclick=function(z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let D="";l.length>0&&(D=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${l.map(z=>`<li style="margin-bottom: 5px;">${U(z)}</li>`).join("")}</ul></div>
                    </div>
                `);let P="";d.length>0&&(P=`
                ...
                `);let j=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",F=N(),E=f?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-icon" style="${f?"color: #03dac6;":""}"><i class='bx ${L}'></i></div>
                <div class="event-content">
...
`;async function Oe(){await Promise.all([ce(),pe()])}var wt=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,o=document.getElementById("new-chore-url").value,v=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:v,natural_instruction:c,entry_url:o,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(r){console.error(r),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let o=await(await B("/chores")).json();if(!o||o.length===0){t.innerHTML=H("empty","No active chores.","Create one to start monitoring.");return}let v=o.map(r=>{let p=r.last_run===0?"Never":new Date(r.last_run*1e3).toLocaleString(),y=r.memory?r.memory.length:0,m=r.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=v,t.querySelectorAll(".delete-chore-btn").forEach(r=>{r.onclick=async p=>{p.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${r.dataset.id}`,{method:"DELETE"}),ye())}})}catch(c){console.error("Failed to fetch chores",c)}}var us=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Ct(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function $t(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=us,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `;t.innerHTML=p;let y=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),a=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),b.forEach($=>{$.addEventListener("click",()=>{b.forEach(h=>h.classList.remove("active")),a.forEach(h=>h.classList.remove("active")),$.classList.add("active"),t.querySelector(`#tab-${$.dataset.tab}`).classList.add("active")})}),m.addEventListener("click",()=>{y.classList.toggle("expanded");let $=y.classList.contains("expanded");m.innerHTML=$?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var St=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,_e=null,me=new Set,Ce=[],K="all",bs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},vs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function xs(t){for(let[e,i]of Object.entries(bs))if(i.includes(t))return e;return"system"}function ys(t){return vs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;hs();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,$=xs(g),h=ys(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),l="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?l="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?l="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?l="event-border-orange":g==="system.test.completed"?l=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let d=u?"cursor-pointer":"",f=me.has(b.id),x=f?"expanded":"",I=f?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),k=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Et(g,a),A=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",L="";if(u){let E="";if(g==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;E=`
                    ...
                    `}else if(g==="system.attention.expired"){let w=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C=a.timestamp-a.last_active,M=Math.floor(C/60);E=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let w=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C="";a.eval_count&&(C=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let R=a.chat_history.length,z=a.chat_history.map((q,J)=>{let W=q.name?q.name.toUpperCase():q.role.toUpperCase();!q.name&&W==="ASSISTANT"&&(W="DEXTER");let G=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",V=J===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${V};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${J+1} of ${R}</span>
                                    </div>
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let w=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,C=a.signals||{};E=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(g==="analysis.visual.completed"){let w=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,C="";a.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(C=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.analysis.audit"){let w=a.success?"#03dac6":"#ff4d4d",C=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,R="";a.error&&(R=`
                            <div class="event-detail-block">
                                ${M("Error")}
...
`),r=[...o.logs];if(r.length<25){let y=25-r.length;for(let m=0;m<y;m++)r.push("")}else r.length>25&&(r=r.slice(-25));let p=r.map(y=>Le(y)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Tt()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${I>90?"#cf6679":"#03dac6"}; width: ${I}%; height: 100%; box-shadow: 0 0 10px ${I>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let l=await At();l?(v(l),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let l=await At();v(l)}if(!t)return;let r=await Pt();if(!r||!r.services){t.children.length===0&&(t.innerHTML=H("offline","Failed to load system metrics.","The event service may be offline."));return}Lt=Date.now(),X(0,Lt);let p=r.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function a(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,x=parseInt(d[2])||0,I=parseInt(d[3])||0,_=parseFloat(d[4])||0,k=f*86400+x*3600+I*60+_,T=Math.floor(k/86400);if(T>0)return`${T}d`;let S=Math.floor(k/3600);if(S>0)return`${S}h`;let A=Math.floor(k/60);return A>0?`${A}m`:`${Math.floor(k)}s`}function g(l){let d=l.status==="online",f=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",I=d?"OK":"BAD",_=l.version?m(l.version.str):"-",k=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",T=l.memory&&l.memory!=="N/A"?l.memory:"-",S=k!=="-"?"#00ff00":"#666",A=k!=="-"?"#fff":"#666",L=T!=="-"?"#00ff00":"#666",D=T!=="-"?"#fff":"#666",P=a(l.uptime),j="";d?j=`
                <div class="service-widget-info">
                ...
                </div>`:j='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let F=N()?"easter.company":b(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${I}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${F}</span></div>${j}</div></div>`}let $=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),h=new Set(p.map(l=>l.id));for(let[l,d]of $)h.has(l)||d.remove();let u=new Set;p.forEach(l=>{if(u.has(l.id))return;u.add(l.id);let d=g(l),f=$.get(l.id);f&&f.parentNode?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function We(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Pt();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}Mt=Date.now(),X(2,Mt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function n(r){let p=r.status==="Ready";return`
                ...
                </div>`}function c(r){let p=r.status==="Ready";return`
                ...
                </div>`}function o(r){let p=r.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",b=p&&r.size>0?`${(r.size/1e9).toFixed(2)} GB`:"-",a=r.name;return r.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${r.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${r.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(s&&(v+=n(s)),e.xtts&&(v+=c(e.xtts)),v+=i.map(o).join(""),!v){t.innerHTML=H("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function Se(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),r=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{r.innerHTML="<i class='bx bx-error'></i>"}},r.dataset.listenerAttached="true");let p=await Ss();if(p){let h=Math.floor(Date.now()/1e3),u=p.active_tier,l=p.active_synthesis,d=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},f=L=>{L<0&&(L=0);let D=Math.floor(L/3600),P=Math.floor(L%3600/60),j=L%60;return D>0?`${D}h ${P}m`:P>0?`${P}m ${j}s`:`${j}s`},x=(L,D,P,j,F)=>{if(!L)return;let E=d[j]||j.toUpperCase(),w=L.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(w&&(w.textContent=E),F&&F.includes(j))L.textContent="Working",L.style.color="#bb86fc";else if(P){let M=P.next_run-h;if(M<=0)L.textContent="Ready",L.style.color="#5eff5e";else{let R=Math.floor(M/60),z=M%60;L.textContent=`${R}m ${z}s`,L.style.color="#fff"}}D&&P&&(D.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${P.attempts||0}</span>
            <span style="color: ${P.failures>0?"#ffa500":"#666"}">Fails: ${P.failures||0}</span>
            <span style="color: ${P.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${P.absolute_failures||0}</span>
          </div>
        `)};e&&x(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",u),i&&x(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",u);let I=document.getElementById("analyzer-synthesis-val"),_=document.getElementById("analyzer-synthesis-stats");I&&x(I,_,p.synthesis,"synthesis",l);let k=document.getElementById("imaginator-alert_review-val"),T=document.getElementById("imaginator-alert_review-stats");k&&x(k,T,p.alert_review,"alert_review",u);let S=document.getElementById("system-state-label"),A=document.getElementById("system-state-val");if(A&&p.system_state){let L=p.system_state,D=f(p.system_state_time||0);S&&(S.textContent=`State: ${L.toUpperCase()}`),A.textContent=D,L==="idle"?A.style.color=p.system_state_time>300?"#5eff5e":"#fff":A.style.color="#bb86fc"}n&&(n.textContent=f(p.total_idle_time||0)),c&&(c.textContent=f(p.total_active_time||0)),o&&(o.textContent=f(p.total_waste_time||0))}else[e,i,s,n,c,o].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let y=await $s(),m=[],b=[],a=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],b=y.queue||[],a=y.history||[],a.sort((h,u)=>(u.end_time||0)-(h.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=H("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Fe(t,m,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Fe(g,b,!1)));let $=document.getElementById("processes-history-widgets");$&&(a.length===0?$.querySelector(".tab-placeholder")||($.innerHTML=H("empty","No recent history.")):($.querySelector(".tab-placeholder")&&($.innerHTML=""),Fe($,a,!0))),we(1,m.length)}function Fe(t,e,i){function s(r){let p="";r.end_time?p=`${r.end_time-r.start_time}s`:p=`${Math.floor(Date.now()/1e3-r.start_time)}s`;let y=r.retries>0?`<span class="process-retry-badge">Retry ${r.retries}</span>`:"",m=r.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[m]?m=b[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),i)return`
                ...
                <div class="service-widget process-widget" data-channel-id="${r.channel_id}-${r.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${p}</span>
                        </div>
                         <div class="service-widget-info">
    ...
    `;async function Ut(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Rt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Rt(t)}async function Rt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=H("empty","No web history found.");return}let s=`
        ...
        `;i.forEach((v,r)=>{let p=new Date(v.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=v.screenshot?`<img src="data:image/png;base64,${v.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ve=!0,i}function jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ve=!1}function qt(t,e,i="output"){if(!Ve)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Ts(t){let e=Ft.find(n=>n.id===t);if(!e)return;let i=ks(e);qt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),qt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Jt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Es();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Ts(s)})})}async function Ls(){if(!N())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Wt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}Qe(),et(),Ls(),at(),Xe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Jt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),l=document.querySelector("nav"),d=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(A=>{let L=document.getElementById(A.id);L&&L.classList.remove("hide-close")});let I=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),S=window.innerWidth<880;if(!S){let A=document.getElementById("dexter-dropdown"),L=document.getElementById("dexter-menu-btn");A&&A.classList.remove("active"),L&&L.classList.remove("active"),u&&u.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=S?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",l?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),I&&(I.style.display=S?"flex":"none"),k&&(k.style.display=S?"block":"none"),!S&&_){let A=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(r)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(p)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(m)})):_.innerHTML=""}else _&&(_.innerHTML="");else l?.classList.remove("window-open"),T&&(T.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?d?.classList.remove("hide"):d?.classList.add("hide"),I&&(I.style.display="flex"),k&&(k.style.display="block"),_&&(_.innerHTML="");ve()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function o(){[...e].forEach(l=>l.close()),e=[]}window.addEventListener("resize",n);let v=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:mt(),onOpen:()=>ie(),onClose:()=>{let u=e.indexOf(v);u>-1&&e.splice(u,1),n()}}),r=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{r.setContent(qe()),ne()},onClose:()=>{let u=e.indexOf(r);u>-1&&e.splice(u,1),n()}}),p=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:St(),onOpen:()=>je(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),y=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:_t()},{icon:"bxs-component",title:"Processes",content:Bt()},{icon:"bxs-brain",title:"Models",content:Dt()},{icon:"bx-globe",title:"Web",content:Ot()},{icon:"bxs-hdd",title:"Hardware",content:zt()},{icon:"bxs-terminal",title:"Logs",content:Nt()},{icon:"bxs-zap",title:"Agents",content:It()},{icon:"bx-task",title:"Chores",content:wt()}].filter(u=>N()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Je(),Se(),We(),Ut(),$e(),ye()},onClose:()=>{let u=e.indexOf(y);u>-1&&e.splice(u,1),n()}}),m=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ht(),onOpen:()=>Oe(),onClose:()=>{let u=e.indexOf(m);u>-1&&e.splice(u,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{b.setContent(De()),Ge(b)},onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{r.isOpen()||c(r),setTimeout(()=>{let l=document.querySelector(`.event-item[data-event-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{v.isOpen()||c(v),setTimeout(()=>{let l=document.querySelector(`.event-item[data-alert-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),l=document.getElementById("dexter-menu-btn");u&&l&&(u.classList.remove("active"),l.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),l=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),I=document.getElementById("windows-container");u&&x&&(u.onclick=k=>{k.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let A=u.getBoundingClientRect(),D=document.querySelector("nav").getBoundingClientRect(),P=220,j=10,E=A.left+A.width/2-P/2,w=D.right-10;E+P>w&&(E=w-P),E<10&&(E=10),x.style.top=D.bottom+j+"px",x.style.left=E+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),u.classList.toggle("active");let S=x.classList.contains("active");if(T){let A=document.querySelector("nav");S?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),A?.classList.add("window-open"),I?.classList.add("menu-open"),fe(!0)):(I?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),A?.classList.remove("window-open"),fe(!1)))}}),l&&(l.onclick=k=>{k.stopPropagation(),c(b)}),d&&(d.onclick=k=>{k.stopPropagation(),o()});let _=(k,T,S)=>{let A=document.getElementById(k);A&&(A.onclick=L=>{L.stopPropagation(),a(),S&&s(S),c(T)})};_("alerts-menu-item",v,"alerts-window"),_("events-menu-item",r,"events-window"),_("monitor-menu-item",y,"monitor-window"),_("contacts-menu-item",p,"contacts-window"),_("workspace-menu-item",m,"workspace-window"),f&&(f.onclick=k=>{if(k.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)o();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let L=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");L.pop();let D=L.join("/")||"/";window.location.href=D}}}),document.addEventListener("click",()=>{let k=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),k&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),x&&(x.onclick=k=>k.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!h&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{r.isOpen()&&ne(),v.isOpen()?ie():Re()},1e3),setInterval(()=>{N()&&(v.isOpen()?ie():Re()),m.isOpen()?Oe():yt(),N()&&r.isOpen()&&ne(),y.isOpen()&&Ht()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Wt):Wt();})();

(()=>{function Ne(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function He(t=!1){let e=window.location.pathname,i=e==="/"||e==="/index.html",o=`
    ...
    `}
        ...
        `}
    `,l=document.createElement("nav");l.innerHTML=o,document.body.prepend(l)}function Re(t){let e=window.location.pathname,i=e==="/"||e==="/index.html",a=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!a||!s||(t||!i?(a.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{a.style.transform="translateX(-3px)"},s.onmouseleave=()=>{a.style.transform="translateX(0)"}):(a.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function Pe(){if(document.querySelector("footer"))return;let t=`
...
`,pe=null,F=new Set,ue=[];async function G(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await I(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];pe=Date.now(),O(0,pe);let l=Date.now(),f=24*60*60*1e3,v=o.filter(n=>{let m=localStorage.getItem(`alert_read_ts_${n.id}`);if(!m)return!0;let y=parseInt(m);return l-y<f});v.sort((n,m)=>{let y=k=>{let A=k.event;if(typeof A=="string")try{A=JSON.parse(A)}catch{return"low"}return(A.priority||"low").toLowerCase()},S=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,E=S(y(n)),$=S(y(m));return E!==$?$-E:m.timestamp-n.timestamp}),ue=v;let d=n=>{let m=n.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},x=[],h=new Set(v.map(n=>d(n))).size>1;if(v.length>0){let n=null;v.forEach(m=>{let y=d(m);h&&y!==n&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),n=y),x.push(m)})}if(t&&(e.innerHTML=""),v.length===0){e.innerHTML=C("empty","No alerts yet."),de();return}let b=n=>{let m=n.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let y=m.title||"Untitled Alert",S=m.body||"No description provided.",E=m.priority||"low",$=!!m.alert,k=m.category||"system",A=m.related_event_ids||[],D=m.audit_event_id,_=!!localStorage.getItem(`alert_read_ts_${n.id}`),N=new Date(n.timestamp*1e3),P=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=_?"event-border-grey":"event-border-blue";!_&&$&&(T="event-border-red"),_&&(E==="high"||E==="critical")?T="event-border-red":_&&E==="medium"&&(T="event-border-orange");let H=_?"alert-read":"alert-unread",R=F.has(n.id),xe=R?"expanded":"",he=R?"display: block;":"display: none;",Ae="",_e="";A.length>0&&(_e=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${A.map(q=>`<a href="#" onclick="window.dexter.viewEvent('${q}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px; font-family: monospace;">${q.substring(0,8)}...</a>`).join(", ")}</span>
                    </div>`);let Me="";D&&(Me=`
              <div class="event-detail-row">
...
`+S)}</div>
            ...
            `;let U=document.createElement("div");U.className=`event-item notification-item ${T} ${H} ${xe} cursor-pointer priority-${E}`,U.dataset.alertId=n.id,U.onclick=function(q){if(this.classList.contains("expanded")){this.classList.remove("expanded"),F.delete(n.id);let ee=this.querySelector(".event-details");ee&&(ee.style.display="none")}else{this.classList.add("expanded"),F.add(n.id);let ee=this.querySelector(".event-details");if(ee&&(ee.style.display="block"),!localStorage.getItem(`alert_read_ts_${n.id}`)){localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ee="event-border-grey";E==="high"||E==="critical"?Ee="event-border-red":E==="medium"&&(Ee="event-border-orange"),this.classList.add(Ee),de()}}};let Qt=y,kt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[k]||"bx-bell";U.innerHTML=`
...
`,W=new Set,Qe=[],Y=null;async function V(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let i=await I("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();if(Qe=a,a.length===0){e.innerHTML=C("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=d=>{let x=W.has(d.id),u=d.state==="draft",h=d.state==="published",b=d.state==="consumed",r="event-border-grey";h&&(r="event-border-blue"),b&&(r="event-border-purple");let c=new Date(d.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),g=document.createElement("div");g.className=`event-item notification-item ${r} cursor-pointer ${x?"expanded":""}`,g.dataset.itemId=d.id,g.onclick=m=>{if(m.target.closest("button"))return;g.classList.contains("expanded")?(g.classList.remove("expanded"),g.querySelector(".event-details").style.display="none",W.delete(d.id)):(g.classList.add("expanded"),g.querySelector(".event-details").style.display="block",W.add(d.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${d.state.toUpperCase()}]</span>`;g.innerHTML=`
        <div class="event-time">
      ...
      `;let n=g.querySelector(".event-details");return n&&n.addEventListener("click",m=>{m.stopPropagation()}),g.querySelector(".edit-btn")?.addEventListener("click",()=>Nt(d)),g.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ht(d)),g.querySelector(".delete-btn")?.addEventListener("click",()=>Rt(d.id)),g.querySelector(".close-details-btn")?.addEventListener("click",m=>{m.stopPropagation(),g.classList.remove("expanded"),g.querySelector(".event-details").style.display="none",W.delete(d.id)}),g},o=Array.from(e.children),l=new Map(o.map(d=>[d.dataset.itemId,d])),f=new Set(a.map(d=>d.id));o.forEach(d=>{let x=d.dataset.itemId;(!x||!f.has(x))&&d.remove()});let v=null;a.forEach((d,x)=>{let u=l.get(d.id);(!u||t)&&(u&&u.remove(),u=s(d),!u)||(x===0?e.firstElementChild!==u&&e.prepend(u):v&&v.nextElementSibling!==u&&v.after(u),v=u)})}catch{e.children.length===0&&(e.innerHTML=C("offline","Failed to load roadmap.","The event service may be offline."))}}}function Dt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Y=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",Y=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-input").value;if(!o.trim())return;let l=Y?`/roadmap/${Y}`:"/roadmap",f=Y?"PATCH":"POST";try{await I(l,{method:f,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})}),document.getElementById("roadmap-editor-container").style.display="none",V(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{Qe.forEach(o=>W.add(o.id)),V(!0)},a.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{W.clear(),V(!0)},s.dataset.listenerAttached="true")}function Nt(t){Y=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Ht(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),V(!0)}catch(i){console.error(i)}}async function Rt(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),W.delete(t),V(!0)}catch(e){console.error(e)}}var tt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,et=null,X=new Set,nt=[];async function ge(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await I(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(nt=o,et=Date.now(),O(2,et),o.length===0){e.innerHTML=C("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),se(1,0);return}t&&(e.innerHTML="");let l=u=>{let h=u.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let b=h.title||"Untitled Blueprint",r=h.summary||h.body||"No summary provided.",p=h.content||"",c=h.category||"architecture",g=h.affected_services||[],w=h.implementation_path||[],n=new Date(u.timestamp*1e3),m=n.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=n.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=X.has(u.id),E=S?"display: block;":"display: none;",$=document.createElement("div");$.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""}`,$.dataset.blueprintId=u.id;let A={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";$.onclick=function(P){if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),X.add(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let D=g.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${g.join(", ")}</span></div>`:"",B="";w.length>0&&(B=`
            ...
            `;let _=$.querySelector(".event-details");_&&_.addEventListener("click",P=>{P.stopPropagation()});let N=$.querySelector(".close-details-btn");return N&&N.addEventListener("click",P=>{P.stopPropagation(),$.classList.remove("expanded");let L=$.querySelector(".event-details");L&&(L.style.display="none"),X.delete(u.id)}),$},f=Array.from(e.children),v=new Map(f.map(u=>[u.dataset.blueprintId,u])),d=new Set(o.map(u=>u.id));f.forEach(u=>{let h=u.dataset.blueprintId;(!h||!d.has(h))&&u.remove()});let x=null;o.forEach((u,h)=>{let b=v.get(u.id);(!b||t)&&(b&&b.remove(),b=l(u),!b)||(h===0?e.firstElementChild!==b&&e.prepend(b):x&&x.nextElementSibling!==b&&x.after(b),x=b)}),se(2,o.length)}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=C("offline","Failed to load blueprints.","The event service may be offline."))}}function Pt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(i=>X.add(i.id)),ge(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),ge(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`,fe=null,Z=new Set,ae=[],ve="all",zt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},qt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function lt(t){for(let[e,i]of Object.entries(zt))if(i.includes(t))return e;return"system"}function jt(t){return qt[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ft();let a=`/events?ml=${ve==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(a);if(!s.ok)throw new Error("Service is offline or unreachable.");let l=(await s.json()).events||[],f=l;if(ve!=="all"&&(f=l.filter(b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!1}return lt(r.type)===ve})),ae=f.slice(0,50),fe=Date.now(),O(1,fe),ae.length===0){e.innerHTML=C("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let p=r.type,c=lt(p),g=jt(p),w=p==="engagement.decision"||p==="messaging.bot.sent_message"||p==="messaging.user.sent_message"||p==="moderation.explicit_content.deleted"||p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.cli.command"||p==="system.analysis.audit"||p==="system.test.completed"||p==="error_occurred"||p==="system.cli.status"||p==="system.attention.expired"||p.startsWith("system.roadmap")||p.startsWith("system.process"),n="event-border-grey";w&&(p==="moderation.explicit_content.deleted"||p==="error_occurred"?n="event-border-red":p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.analysis.audit"?n="event-border-purple":p==="system.attention.expired"||p==="system.cli.command"||p==="system.cli.status"?n="event-border-orange":p==="system.test.completed"?n=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":n="event-border-blue");let m=w?"cursor-pointer":"",y=Z.has(b.id),S=y?"expanded":"",E=y?"display: block;":"display: none;",$=new Date(b.timestamp*1e3),k=$.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=$.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),D=ot(p,r),B=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",_="";if(w){let L="";if(p==="engagement.decision")L=`
                    ...
                    `;else if(p==="system.attention.expired"){let T=r.timestamp-r.last_active,H=Math.floor(T/60);L=`
                        <div class="event-detail-row">
                ...
                `}let N=document.createElement("div");N.className=`event-item ${n} ${m} ${S}`,N.dataset.eventId=b.id,N.onclick=function(L){if(!w)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),Z.delete(b.id);let H=this.querySelector(".event-details");H&&(H.style.display="none")}else{this.classList.add("expanded"),Z.add(b.id);let H=this.querySelector(".event-details");H&&(H.style.display="block")}},N.innerHTML=`
                <div class="event-time">
            ...
            `;let P=N.querySelector(".event-details");if(P&&P.addEventListener("click",L=>{L.stopPropagation()}),w){let L=N.querySelector(".close-details-btn");L&&L.addEventListener("click",T=>{T.stopPropagation();let H=T.target.closest(".event-item");if(H){H.classList.remove("expanded"),Z.delete(b.id);let R=H.querySelector(".event-details");R&&(R.style.display="none")}})}return N},d=Array.from(e.children),x=new Map(d.map(b=>[b.dataset.eventId,b])),u=new Set(ae.map(b=>b.id));d.forEach(b=>{let r=b.dataset.eventId;(!r||!u.has(r))&&b.remove()});let h=null;ae.forEach((b,r)=>{let p=x.get(b.id);(!p||t)&&(p&&p.remove(),p=v(b),!p)||(r===0?e.firstElementChild!==p&&e.prepend(p):h&&h.nextElementSibling!==p&&h.after(p),h=p)}),fe=Date.now(),O(1,fe)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=C("offline","Failed to load events.","The event service may be offline."))}}function Ft(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ae.forEach(a=>Z.add(a.id)),Q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Z.clear(),Q(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(a=>{a.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),a.classList.add("active"),ve=a.dataset.filter,Q(!0)}}),i.dataset.listenerAttached="true")}function ct(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Gt=null;async function oe(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await I("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=C("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let a=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=i.filter(l=>!a.includes(l.id));s.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),s.reverse();let o=s.map(l=>{let f=l.logs.join(`
`),v=[...l.logs];if(v.length<25){let x=25-v.length;for(let u=0;u<x;u++)v.push("")}else v.length>25&&(v=v.slice(-25));let d=v.map(x=>me(x)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function wt(){await Promise.all([be(),Ce(),Te()]),await oe()}var dt=null,mt=null,pt=null;async function Et(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await I("/processes")).json()}catch{return null}}async function Vt(){try{return await(await I("/analyst/status")).json()}catch{return null}}async function Ce(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-cpu .hw-content"),a=document.querySelector("#hw-ram .hw-content"),s=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),l=c=>{if(c){if(a){let g=(c.MEMORY_BYTES/1073741824).toFixed(1);a.innerHTML=`
                ...
                </div>`}if(i&&c.CPU&&c.CPU.length>0){let g=c.CPU[0];i.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    <span style="font-size: 1.1em; color: #fff; font-weight: 600; letter-spacing: 0.5px;">${g.LABEL}</span>
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await ut();c?(l(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let c=await ut();l(c)}if(!t)return;let f=await Et();if(!f||!f.services){t.children.length===0&&(t.innerHTML=C("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),O(0,dt);let v=f.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function d(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function x(c){if(!c||c==="N/A"||c==="unknown")return"-";let g=c.match(/^(\d+\.\d+\.\d+)/);return g?g[0]:c.split(".").slice(0,3).join(".")||"-"}function u(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function h(c){if(!c||c==="N/A"||c==="unknown")return"-";let g=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!g)return"-";let w=parseInt(g[1])||0,n=parseInt(g[2])||0,m=parseInt(g[3])||0,y=parseFloat(g[4])||0,S=w*86400+n*3600+m*60+y,E=Math.floor(S/86400);if(E>0)return`${E}d`;let $=Math.floor(S/3600);if($>0)return`${$}h`;let k=Math.floor(S/60);return k>0?`${k}m`:`${Math.floor(S)}s`}function b(c){let g=c.status==="online",w=g?"service-widget-online":"service-widget-offline",n=g?"bx-check-circle":"bx-x-circle",m=g?"OK":"BAD",y=c.version?x(c.version.str):"-",S=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",E=c.memory&&c.memory!=="N/A"?c.memory:"-",$=S!=="-"?"#00ff00":"#666",k=S!=="-"?"#fff":"#666",A=E!=="-"?"#00ff00":"#666",D=E!=="-"?"#fff":"#666",B=h(c.uptime),_="";return g?_=`
                ...
                </div>`}function o(f){let v=f.status==="Downloaded",d=v?"service-widget-online":"service-widget-offline",x=v?"OK":"MISSING",u=v&&f.size>0?`${(f.size/1e9).toFixed(2)} GB`:"-",h=f.name;return f.type==="custom"&&h.endsWith(":latest")&&(h=h.replace(":latest","")),`<div class="service-widget ${d}" data-model-name="${f.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${h}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${f.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${u}</span></div></div></div>`}let l="";if(a&&(l+=s(a)),l+=i.map(o).join(""),!l){t.innerHTML=C("empty","No models found.");return}t.innerHTML!==l&&(t.innerHTML=l)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),i=document.getElementById("analyst-t2-val"),a=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),o=document.getElementById("analyst-total-idle"),l=document.getElementById("analyst-total-active"),f=document.getElementById("analyst-total-waste"),v=document.getElementById("analyst-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{v.innerHTML="<i class='bx bx-error'></i> Failed"}},v.dataset.listenerAttached="true");let d=await Vt();if(d){let p=Math.floor(Date.now()/1e3),c=d.active_tier,g=n=>{n<0&&(n=0);let m=Math.floor(n/3600),y=Math.floor(n%3600/60),S=n%60;return m>0?`${m}h ${y}m`:y>0?`${y}m ${S}s`:`${S}s`},w=(n,m,y)=>{if(c===y||y==="guardian"&&c==="tests"){n.textContent="Working",n.style.color="#bb86fc";return}let S=m-p;if(S<=0)n.textContent="Ready",n.style.color="#5eff5e";else{let E=Math.floor(S/60),$=S%60;n.textContent=`${E}m ${$}s`,n.style.color="#fff"}};if(e&&d.guardian&&w(e,d.guardian.next_run,"guardian"),i&&d.architect&&w(i,d.architect.next_run,"architect"),a&&d.strategist&&w(a,d.strategist.next_run,"strategist"),s&&d.system_idle_time!==void 0){let n=d.system_idle_time;s.textContent=g(n),n>300?s.style.color="#5eff5e":n>60?s.style.color="#ffa500":s.style.color="#fff"}o&&(o.textContent=g(d.total_idle_time||0)),l&&(l.textContent=g(d.total_active_time||0)),f&&(f.textContent=g(d.total_waste_time||0))}else[e,i,a,s,o,l,f].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let x=await Wt(),u=[],h=[];x&&(Array.isArray(x)?u=x:(u=x.active||[],h=x.history||[]));let b=document.getElementById("vitals-processes-val");if(b)if(x){let p=u.length;b.textContent=p>0?`${p} Active`:"Idle",b.style.color=p>0?"#bb86fc":"#fff"}else b.textContent="-",b.style.color="#888";if(x===null){t.children.length===0&&(t.innerHTML=C("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),O(1,pt),u.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=C("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,u,!1));let r=document.getElementById("processes-history-widgets");r&&(h.length===0?r.querySelector(".tab-placeholder")||(r.innerHTML=C("empty","No recent history.")):(r.querySelector(".tab-placeholder")&&(r.innerHTML=""),gt(r,h,!0))),se(1,u.length)}function gt(t,e,i){function a(l){let f="";l.end_time?f=`${l.end_time-l.start_time}s`:f=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let v=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",d=l.channel_id,x={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};x[d]?d=x[d]:/^\d+$/.test(d)&&(d=`Channel ${d}`);let u=i?"#888":"#fff",h=i?"border-left: 3px solid #666;":"";return`
                ...
                </div>`}let s=new Map(Array.from(t.querySelectorAll(".process-widget")).map(l=>[l.dataset.channelId,l])),o=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,f]of s)o.has(l)||f.remove();e.forEach(l=>{let f=`${l.channel_id}-${l.start_time}`,v=a(l),d=s.get(f);d?d.outerHTML!==v&&(d.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}function ye(){let t=te(),e=ze()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,a]of Object.entries(t)){let s=Lt.filter(o=>o.category===i);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,i}function St(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function $t(t,e,i="output"){if(!Ie)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=me(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Lt.find(s=>s.id===t);if(!e)return;let i=Yt(e);$t(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(l=>setTimeout(l,400+Math.random()*600));let o="output";s.includes("[ERROR]")?o="error":s.includes("[SUCCESS]")||s.includes("\u2713")?o="success":s.includes("!")&&(o="warning"),$t(i,s,o)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ct(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;Xt(a)})})}async function Zt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function Tt(){Zt(),Ge(),Ne();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Ct();let e=Ue();He(e),Pe();let i=document.getElementById("nav-left-container");i&&i.addEventListener("click",()=>{if(o.length>0)u();else{let n=window.location.pathname;if(!(n==="/"||n==="/index.html")){let S=(n.endsWith("/")&&n.length>1?n.slice(0,-1):n).split("/");S.pop();let E=S.join("/")||"/";window.location.href=E}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!s&&document.querySelector("footer")?.classList.add("hide");let o=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let f=n=>{localStorage.setItem("dex_last_window",n)};function v(){return 1}function d(){for(;o.length>1;)o.shift().close(!0);let n=document.getElementById("windows-container"),m=document.querySelector("nav"),y=document.querySelector("footer"),S=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");n&&n.setAttribute("data-count",o.length);let $=o.length>0;o.forEach(B=>{let _=document.getElementById(B.id);_&&_.classList.remove("hide-close")}),le(te());let k=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),D=document.getElementById("settings-icon");if(Re(o.length>0),o.length>0){if(y?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),m?.classList.add("window-open"),n&&(n.style.paddingTop="60px"),k&&(k.style.display="none"),D&&(D.style.display="none"),A){let B=o[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(B)?(A.innerHTML=`

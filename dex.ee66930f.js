(()=>{function Re(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Pe(t=!1){let e=window.location.pathname,o=e==="/"||e==="/index.html",i=`
...
`,be=null,V=new Set,ve=[];async function K(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Mt(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await k(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];be=Date.now(),U(0,be);let u=Date.now(),d=24*60*60*1e3,p=i.filter(n=>{let c=localStorage.getItem(`alert_read_ts_${n.id}`);if(!c)return!0;let g=parseInt(c);return u-g<d});p.sort((n,c)=>{let g=S=>{let I=S.event;if(typeof I=="string")try{I=JSON.parse(I)}catch{return"low"}return(I.priority||"low").toLowerCase()},C=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,w=C(g(n)),$=C(g(c));return w!==$?$-w:c.timestamp-n.timestamp}),ve=p;let m=n=>{let c=n.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return"low"}return(c.priority||"low").toLowerCase()},y=[],h=new Set(p.map(n=>m(n))).size>1;if(p.length>0){let n=null;p.forEach(c=>{let g=m(c);h&&g!==n&&(y.push({id:`divider-${g}`,type:"divider",label:g.toUpperCase()}),n=g),y.push(c)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=A("empty","No alerts yet."),ge();return}let v=n=>{let c=n.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return null}let g=(c.title||"Untitled Alert").trim(),C=(c.body||"No description provided.").trim(),w=(c.priority||"low").toLowerCase(),$=!!c.alert,S=(c.category||"system").trim(),I=c.related_event_ids||[],_=c.audit_event_id,N=!!localStorage.getItem(`alert_read_ts_${n.id}`),H=new Date(n.timestamp*1e3),j=H.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=H.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=N?"event-border-grey":"event-border-blue";!N&&$&&(M="event-border-red"),N&&(w==="high"||w==="critical")?M="event-border-red":N&&w==="medium"&&(M="event-border-orange");let D=N?"alert-read":"alert-unread",T=V.has(n.id),O=T?"expanded":"",z=T?"display: block;":"display: none;",Me="",Be="";I.length>0&&(Be=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${I.map(G=>`<a href="#" onclick="window.dexter.viewEvent('${G}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px; font-family: monospace; width: fit-content;">${G.substring(0,8)}...</a>`).join(", ")}</span>
                    </div>`);let De="";_&&(De=`
              <div class="event-detail-row">
...
`,st=null,se=new Set,at=[];async function ne(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Rt();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await k(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];if(at=i,st=Date.now(),U(2,st),i.length===0){e.innerHTML=A("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),oe(2,0);return}t&&(e.innerHTML="");let u=b=>{let h=b.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let v=(h.title||"Untitled Blueprint").trim(),l=(h.summary||h.body||"No summary provided.").trim(),f=(h.content||"").trim(),E=(h.category||"architecture").trim(),r=(h.affected_services||[]).map(T=>T.trim()),x=(h.implementation_path||[]).map(T=>T.trim()),n=h.approved===!0,c=new Date(b.timestamp*1e3),g=c.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=c.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=se.has(b.id),$=w?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${w?"expanded":""} ${n?"blueprint-approved":""}`,S.dataset.blueprintId=b.id,n&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let _=n?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[E]||"bx-paint";S.onclick=function(T){if(this.classList.contains("expanded")){this.classList.remove("expanded"),se.delete(b.id);let z=this.querySelector(".event-details");z&&(z.style.display="none")}else{this.classList.add("expanded"),se.add(b.id);let z=this.querySelector(".event-details");z&&(z.style.display="block")}};let P="";x.length>0&&(P=`
            ...
            `;let j=S.querySelector(".blueprint-approve-btn");j&&(j.onclick=async T=>{T.stopPropagation(),j.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await k(`/events/${b.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ne(!0)}catch(O){console.error("Failed to approve blueprint:",O)}});let L=S.querySelector(".blueprint-delete-btn");L&&(L.onclick=async T=>{T.stopPropagation();let O=!n;L.innerHTML=O?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await k(`/events/${b.id}`,{method:"DELETE"})).ok&&ne(!0)}catch(z){console.error("Failed to delete blueprint:",z)}});let M=S.querySelector(".event-details");M&&M.addEventListener("click",T=>{T.stopPropagation()});let D=S.querySelector(".close-details-btn");return D&&D.addEventListener("click",T=>{T.stopPropagation(),S.classList.remove("expanded");let O=S.querySelector(".event-details");O&&(O.style.display="none"),se.delete(b.id)}),S},d=Array.from(e.children),p=new Map(d.map(b=>[b.dataset.blueprintId,b])),m=new Set(i.map(b=>b.id));d.forEach(b=>{let h=b.dataset.blueprintId;(!h||!m.has(h))&&b.remove()});let y=null;i.forEach((b,h)=>{let v=p.get(b.id);(!v||t)&&(v&&v.remove(),v=u(b),!v)||(h===0?e.firstElementChild!==v&&e.prepend(v):y&&y.nextElementSibling!==v&&y.after(v),y=v)}),oe(2,i.length)}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=A("offline","Failed to load blueprints.","The event service may be offline."))}}function Rt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{at.forEach(o=>se.add(o.id)),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{se.clear(),ne(!0)},e.dataset.listenerAttached="true")}var ot=()=>`
...
`,xe=null,X=new Set,ce=[],R="all",zt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered","system.notification.logged"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.logged","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},Ut={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","system.blueprint.logged":"bx-paint","system.notification.logged":"bx-bell","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ct(t){for(let[e,o]of Object.entries(zt))if(o.includes(t))return e;return"system"}function qt(t){return Ut[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ft();let a=`/events?ml=${R==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await k(a);if(!s.ok)throw new Error("Service is offline or unreachable.");let u=(await s.json()).events||[],d=u;if(R!=="all"&&(d=u.filter(v=>{let l=v.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return!1}return ct(l.type)===R})),ce=d.slice(0,50),xe=Date.now(),U(1,xe),ce.length===0){e.innerHTML=A("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=v=>{let l=v.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return null}let f=l.type,E=ct(f),r=qt(f),x=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),n="event-border-grey";x&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?n="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="system.analysis.audit"?n="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?n="event-border-orange":f==="system.test.completed"?n=l.test?.status==="OK"&&l.lint?.status==="OK"&&l.format?.status==="OK"?"event-border-blue":"event-border-red":n="event-border-blue");let c=x?"cursor-pointer":"",g=X.has(v.id),C=g?"expanded":"",w=g?"display: block;":"display: none;",$=new Date(v.timestamp*1e3),S=$.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=$.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=rt(f,l),P=l.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${l.user_level})</span>`:"",N="";if(x){let L="";if(f==="engagement.decision")L=`
                    ...
                    `;else if(f==="system.analysis.audit"){let M=l.success?"#03dac6":"#ff4d4d",D=l.success?"SUCCESS":"FAILED",T="";l.error&&(T=`
                            <div class="event-detail-block">
                    ...
                    `;else if(f==="messaging.user.sent_message"){let M="";l.attachments&&l.attachments.length>0&&(M=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${l.attachments.map(T=>{let O=T.content_type&&T.content_type.startsWith("image/"),z=(T.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${O?`<div class="attachment-thumb-container"><img src="${T.url}" alt="${T.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${dt()}
        </div>`;async function ht(){await Promise.all([ye(),ke(),Ie()]),await de()}var pt=null,mt=null;async function wt(){try{return await(await k("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await k("/system/hardware")).json()}catch{return null}}async function Gt(){try{return await(await k("/processes")).json()}catch{return null}}async function Wt(){try{return await(await k("/agent/status")).json()}catch{return null}}async function ke(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),u=document.querySelector("#hw-storage .hw-content"),d=r=>{if(r){if(o&&(o.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):i.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),u&&r.STORAGE&&r.STORAGE.length>0?u.innerHTML=r.STORAGE.map(x=>{let n=(x.USED/1073741824).toFixed(1),c=(x.SIZE/(1024*1024*1024)).toFixed(1),g=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,C=x.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):u&&(u.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let r=await ut();r?(d(r),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let r=await ut();d(r)}if(!t)return;let p=await wt();if(!p||!p.services){t.children.length===0&&(t.innerHTML=A("offline","Failed to load system metrics.","The event service may be offline."));return}pt=Date.now(),U(0,pt);let m=p.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function y(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function b(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:r.split(".").slice(0,3).join(".")||"-"}function h(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function v(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let n=parseInt(x[1])||0,c=parseInt(x[2])||0,g=parseInt(x[3])||0,C=parseFloat(x[4])||0,w=n*86400+c*3600+g*60+C,$=Math.floor(w/86400);if($>0)return`${$}d`;let S=Math.floor(w/3600);if(S>0)return`${S}h`;let I=Math.floor(w/60);return I>0?`${I}m`:`${Math.floor(w)}s`}function l(r){let x=r.status==="online",n=x?"service-widget-online":"service-widget-offline",c=x?"bx-check-circle":"bx-x-circle",g=x?"OK":"BAD",C=r.version?b(r.version.str):"-",w=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",$=r.memory&&r.memory!=="N/A"?r.memory:"-",S=w!=="-"?"#00ff00":"#666",I=w!=="-"?"#fff":"#666",_=$!=="-"?"#00ff00":"#666",P=$!=="-"?"#fff":"#666",N=v(r.uptime),H="";return x?H=`
        ...
        `)};e&&x(e,document.getElementById("guardian-t1-stats"),p.t1,"t1"),o&&x(o,document.getElementById("guardian-t2-stats"),p.t2,"t2");let n=document.getElementById("system-state-label"),c=document.getElementById("system-state-val");if(c&&p.system_state){let g=p.system_state,C=r(p.system_state_time||0);n&&(n.textContent=`State: ${g.toUpperCase()}`),c.textContent=C,g==="idle"?c.style.color=p.system_state_time>300?"#5eff5e":"#fff":c.style.color="#bb86fc"}s&&(s.textContent=r(p.total_idle_time||0)),i&&(i.textContent=r(p.total_active_time||0)),u&&(u.textContent=r(p.total_waste_time||0))}else[e,o,a,s,i,u].forEach(E=>{E&&(E.textContent="-",E.style.color="#666")});let m=await Gt(),y=[],b=[],h=[];m&&(Array.isArray(m)?y=m:(y=m.active||[],b=m.queue||[],h=m.history||[],h.sort((f,E)=>(E.end_time||0)-(f.end_time||0)))),y.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=A("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Le(t,y,!1));let v=document.getElementById("processes-queue-widgets");v&&(b.length===0?!v.querySelector(".tab-placeholder")&&!v.querySelector('div[style*="font-style: italic"]')&&(v.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(v.innerHTML="",Le(v,b,!1)));let l=document.getElementById("processes-history-widgets");l&&(h.length===0?l.querySelector(".tab-placeholder")||(l.innerHTML=A("empty","No recent history.")):(l.querySelector(".tab-placeholder")&&(l.innerHTML=""),Le(l,h,!0))),oe(1,y.length)}function Le(t,e,o){function a(d){let p="";d.end_time?p=`${d.end_time-d.start_time}s`:p=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let m=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",y=d.channel_id,b={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};if(b[y]?y=b[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),o)return`
                ...
                ${Object.values(F).map(s=>`
                    <div class="theme-card ${t===s?"active":""}" data-theme="${s}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${s===F.DARK?"Simple, clean, dark.":s===F.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Kt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),_e=!0,o}function Et(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),_e=!1}function $t(t,e,o="output"){if(!_e)return;let a=document.createElement("div");a.className=`terminal-line terminal-${o}`,o==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=fe(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Yt(t){let e=St.find(s=>s.id===t);if(!e)return;let o=Jt(e);$t(o,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(u=>setTimeout(u,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),$t(o,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Kt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code").textContent;navigator.clipboard.writeText(o).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-5px)",o.style.borderColor="rgba(255,255,255,0.15)",o.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)",o.style.borderColor="rgba(255,255,255,0.05)",o.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=o.dataset.cmd;Yt(a)})})}async function Xt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${$e()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Ct(){Xt(),Ve(),Re();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=qe();Pe(e),ze();let o=document.getElementById("nav-left-container");o&&o.addEventListener("click",()=>{if(i.length>0)b();else{let n=window.location.pathname;if(!(n==="/"||n==="/index.html")){let C=(n.endsWith("/")&&n.length>1?n.slice(0,-1):n).split("/");C.pop();let w=C.join("/")||"/";window.location.href=w}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],u=document.getElementById("windows-container");u&&u.setAttribute("data-count","0");let d=n=>{localStorage.setItem("dex_last_window",n)};function p(){return 1}function m(){for(;i.length>1;)i.shift().close(!0);let n=document.getElementById("windows-container"),c=document.querySelector("nav"),g=document.querySelector("footer"),C=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");n&&n.setAttribute("data-count",i.length),i.forEach(_=>{let P=document.getElementById(_.id);P&&P.classList.remove("hide-close")}),pe(ae());let $=document.getElementById("dexter-menu-container"),S=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon");if(Oe(i.length>0),i.length>0){if(g?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),c?.classList.add("window-open"),n&&(n.style.paddingTop="60px"),$&&($.style.display="none"),I&&(I.style.display="none"),S){let _=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(S.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{d("alerts-window"),y(h)}),document.getElementById("switch-events").addEventListener("click",()=>{d("events-window"),y(v)}),document.getElementById("switch-monitor").addEventListener("click",()=>{d("monitor-window"),y(f)}),document.getElementById("switch-contacts").addEventListener("click",()=>{d("contacts-window"),y(l)}),document.getElementById("switch-workspace").addEventListener("click",()=>{d("workspace-window"),y(E)})):S.innerHTML=""}}else c?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),n&&(n.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),C||w?g?.classList.remove("hide"):g?.classList.add("hide"),$&&($.style.display="flex"),I&&(I.style.display="block"),S&&(S.innerHTML="");le(Je())}function y(n){if(n.isOpen()){n.close();return}for(;i.length>0;)i.pop().close(!0);i.push(n),n.open(),m()}function b(){[...i].forEach(c=>c.close()),i=[]}window.addEventListener("resize",m);let h=W({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Xe(),onOpen:()=>K(),onClose:()=>{let n=i.indexOf(h);n>-1&&i.splice(n,1),m()}}),v=W({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Ce(),onOpen:()=>{v.setContent(Ce()),Q()},onClose:()=>{let n=i.indexOf(v);n>-1&&i.splice(n,1),m()}}),l=W({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:lt(),onOpen:()=>Te(),onClose:()=>{let n=i.indexOf(l);n>-1&&i.splice(n,1),m()}}),f=W({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:ft()},{icon:"bxs-brain",title:"Models",content:vt()},{icon:"bxs-hdd",title:"Hardware",content:xt()},{icon:"bxs-terminal",title:"Logs",content:yt()},{icon:"bxs-zap",title:"Agents",content:gt()}],onOpen:()=>{ke(),ye(),Ie(),de()},onClose:()=>{let n=i.indexOf(f);n>-1&&i.splice(n,1),m()}}),E=W({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ot(),onOpen:()=>Se(),onClose:()=>{let n=i.indexOf(E);n>-1&&i.splice(n,1),m()}}),r=W({id:"settings-window",title:"Settings",icon:"bx-cog",content:he(),onOpen:()=>{r.setContent(he()),Ae(r)},onClose:()=>{let n=i.indexOf(r);n>-1&&i.splice(n,1),m()}}),x=W({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:n=>{v.isOpen()||y(v),setTimeout(()=>{let c=document.querySelector(`.event-item[data-event-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)},viewAlert:n=>{h.isOpen()||y(h),setTimeout(()=>{let c=document.querySelector(`.event-item[data-alert-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)}},e){let n=document.getElementById("dexter-dropdown");n&&(n.innerHTML=`

(()=>{function Ae(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function _e(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",o=t?`
        ...
        ${o}
    `,m=document.createElement("nav");if(m.innerHTML=r,document.body.prepend(m),!a){let v=document.getElementById("nav-back-btn");v&&(v.addEventListener("click",()=>{let p=window.location.pathname,w=(p.endsWith("/")&&p.length>1?p.slice(0,-1):p).split("/");w.pop();let g=w.join("/")||"/";window.location.href=g}),v.addEventListener("mouseenter",()=>{v.style.transform="translateX(-3px)"}),v.addEventListener("mouseleave",()=>{v.style.transform="translateX(0)"}))}}function Me(){let t=`
...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Et(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),B(0,ce);let m=Date.now(),v=24*60*60*1e3,p=r.filter(c=>{let b=localStorage.getItem(`notification_read_ts_${c.id}`);if(!b)return!0;let h=parseInt(b);return m-h<v});p.sort((c,b)=>{let h=A=>{let S=A.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},_=A=>A==="critical"?4:A==="high"?3:A==="medium"?2:1,L=_(h(c)),E=_(h(b));return L!==E?E-L:b.timestamp-c.timestamp}),de=p;let f=c=>{let b=c.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return"low"}return(b.priority||"low").toLowerCase()},w=[],u=new Set(p.map(c=>f(c))).size>1;if(p.length>0){let c=null;p.forEach(b=>{let h=f(b);u&&h!==c&&(w.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),c=h),w.push(b)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=T("empty","No notifications yet."),le();return}let n=c=>{let b=c.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let h=b.title||"Untitled Notification",_=b.body||"No description provided.",L=b.priority||"low",E=!!b.alert,A=b.category||"system",S=b.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${c.id}`),U=new Date(c.timestamp*1e3),M=U.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=U.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=H?"event-border-grey":"event-border-blue";!H&&E&&(C="event-border-red"),H&&(L==="high"||L==="critical")?C="event-border-red":H&&L==="medium"&&(C="event-border-orange");let N=H?"notification-read":"notification-unread",D=q.has(c.id),fe=D?"expanded":"",be=D?"display: block;":"display: none;",Te="",ke="";S.length>0&&(ke=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${S.map(Z=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${Z.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),Te=`
                <div class="event-detail-row">
            ...
            `;let z=document.createElement("div");z.className=`event-item notification-item ${C} ${N} ${fe} cursor-pointer`,z.dataset.notificationId=c.id,z.onclick=function(Z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),q.delete(c.id);let ee=this.querySelector(".event-details");ee&&(ee.style.display="none")}else{this.classList.add("expanded"),q.add(c.id);let ee=this.querySelector(".event-details");if(ee&&(ee.style.display="block"),!localStorage.getItem(`notification_read_ts_${c.id}`)){localStorage.setItem(`notification_read_ts_${c.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let he="event-border-grey";L==="high"||L==="critical"?he="event-border-red":L==="medium"&&(he="event-border-orange"),this.classList.add(he),le()}}};let zt=h,bt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[A]||"bx-bell";z.innerHTML=`
                ...
                <div class="event-content">
                    <div class="event-service">${A.toUpperCase()} ${E?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>
            ...
            `;let Ie=z.querySelector(".close-details-btn");return Ie&&Ie.addEventListener("click",Z=>{Z.stopPropagation(),z.classList.remove("expanded");let xe=z.querySelector(".event-details");xe&&(xe.style.display="none"),q.delete(c.id)}),z},i=c=>{let b=document.createElement("div");b.className="notification-divider",b.dataset.notificationId=c.id;let h="#888888";return c.label==="CRITICAL"?h="#ff4d4d":c.label==="HIGH"?h="#ff8888":c.label==="MEDIUM"&&(h="#ffa500"),b.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,b.innerHTML=`<span style="white-space: nowrap;">${c.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,b},l=Array.from(e.children),y=new Map(l.map(c=>[c.dataset.notificationId,c])),d=new Set(w.map(c=>c.id));l.forEach(c=>{let b=c.dataset.notificationId;(!b||!d.has(b))&&c.remove()});let x=null;w.forEach((c,b)=>{let h=y.get(c.id);(!h||t)&&(h&&h.remove(),c.type==="divider"?h=i(c):h=n(c),!h)||(b===0?e.firstElementChild!==h&&e.prepend(h):x&&x.nextElementSibling!==h&&x.after(h),x=h)}),ce=Date.now(),B(0,ce),le()}catch(o){console.error("Error fetching notifications:",o),e.children.length===0&&(e.innerHTML=T("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function Et(){let t=document.getElementById("notif-read-all"),e=document.getElementById("notif-expand-all"),a=document.getElementById("notif-close-all"),o=document.getElementById("notif-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{de.forEach(s=>{localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.forEach(s=>{q.add(s.id),localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{q.clear(),j(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let s=Date.now()-1728e5;de.forEach(r=>{localStorage.setItem(`notification_read_ts_${r.id}`,s.toString())}),q.clear(),j(!0)},o.dataset.listenerAttached="true")}var Fe=()=>`
  <div class="notifications-actions">
...
`,G=new Set,qe=[],J=null;async function W(t=!1){let e=document.getElementById("roadmap-list");if(e){St();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(qe=o,o.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=f=>{let w=G.has(f.id),g=f.state==="draft",u=f.state==="published",n=f.state==="consumed",i="event-border-grey";u&&(i="event-border-blue"),n&&(i="event-border-purple");let y=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div");d.className=`event-item notification-item ${i} cursor-pointer ${w?"expanded":""}`,d.dataset.itemId=f.id,d.onclick=c=>{if(c.target.closest("button"))return;d.classList.contains("expanded")?(d.classList.remove("expanded"),d.querySelector(".event-details").style.display="none",G.delete(f.id)):(d.classList.add("expanded"),d.querySelector(".event-details").style.display="block",G.add(f.id))};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;return d.innerHTML=`
        <div class="event-time">
      ...
      `,d.querySelector(".edit-btn")?.addEventListener("click",()=>Lt(f)),d.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>$t(f)),d.querySelector(".delete-btn")?.addEventListener("click",()=>Ct(f.id)),d.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),d.classList.remove("expanded"),d.querySelector(".event-details").style.display="none",G.delete(f.id)}),d},r=Array.from(e.children),m=new Map(r.map(f=>[f.dataset.itemId,f])),v=new Set(o.map(f=>f.id));r.forEach(f=>{let w=f.dataset.itemId;(!w||!v.has(w))&&f.remove()});let p=null;o.forEach((f,w)=>{let g=m.get(f.id);(!g||t)&&(g&&g.remove(),g=s(f),!g)||(w===0?e.firstElementChild!==g&&e.prepend(g):p&&p.nextElementSibling!==g&&p.after(g),p=g)})}catch{e.innerHTML=T("error","Failed to load roadmap.")}}}function St(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{J=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",J=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let m=J?`/roadmap/${J}`:"/roadmap",v=J?"PATCH":"POST";try{await I(m,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",W(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{qe.forEach(r=>G.add(r.id)),W(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{G.clear(),W(!0)},s.dataset.listenerAttached="true")}function Lt(t){J=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function $t(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),W(!0)}catch(a){console.error(a)}}async function Ct(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),G.delete(t),W(!0)}catch(e){console.error(e)}}var Ge=()=>`
    <div class="notifications-actions">
...
`,je=null,Y=new Set,We=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Tt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(We=r,je=Date.now(),B(2,je),r.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),P(1,0);return}t&&(e.innerHTML="");let m=g=>{let u=g.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let n=u.title||"Untitled Blueprint",i=u.summary||u.body||"No summary provided.",l=u.content||"",y=u.category||"architecture",d=u.affected_services||[],x=u.implementation_path||[],c=new Date(g.timestamp*1e3),b=c.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=c.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=Y.has(g.id),L=_?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${_?"expanded":""}`,E.dataset.blueprintId=g.id;let S={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";E.onclick=function(M){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),Y.add(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}};let O=d.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${d.join(", ")}</div>`:"",H="";x.length>0&&(H=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${x.map(M=>`<li>${k(M)}</li>`).join("")}
                        </ul>
                    </div>
                `),E.innerHTML=`
                <div class="event-time">
            ...
            `;let U=E.querySelector(".close-details-btn");return U&&U.addEventListener("click",M=>{M.stopPropagation(),E.classList.remove("expanded");let $=E.querySelector(".event-details");$&&($.style.display="none"),Y.delete(g.id)}),E},v=Array.from(e.children),p=new Map(v.map(g=>[g.dataset.blueprintId,g])),f=new Set(r.map(g=>g.id));v.forEach(g=>{let u=g.dataset.blueprintId;(!u||!f.has(u))&&g.remove()});let w=null;r.forEach((g,u)=>{let n=p.get(g.id);(!n||t)&&(n&&n.remove(),n=m(g),!n)||(u===0?e.firstElementChild!==n&&e.prepend(n):w&&w.nextElementSibling!==n&&w.after(n),w=n)}),P(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=T("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Tt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{We.forEach(a=>Y.add(a.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),me(!0)},e.dataset.listenerAttached="true")}var Ve=()=>`
...
`,Ke=null;async function pe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>pe(),e.dataset.listenerAttached="true");try{let a=await Ue("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ke=Date.now(),B(4,Ke),s.length===0){t.innerHTML=T("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((m,v)=>{let p=r[m.level]??10,f=r[v.level]??10;return p!==f?p-f:m.username.localeCompare(v.username)}),t.innerHTML=s.map(m=>{let v=m.color?"#"+m.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=m.status==="online"?"#5eff5e":m.status==="idle"?"#ffa500":m.status==="dnd"?"#ff4d4d":"#666",f="#888";m.level==="Me"||m.level==="Master User"?f="#bb86fc":m.level==="Admin"?f="#cf6679":m.level==="Moderator"?f="#03dac6":m.level==="Contributor"&&(f="#ffa500");let w=m.level==="Me",g=w?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",u=w?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-section" style="background: ${g}; padding: 15px; border-radius: 8px; border: ${u}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${w?"#bb86fc":v}22, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${m.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${w?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${f}; font-weight: 600; text-transform: uppercase;">${w?"DEXTER (ME)":m.level}</span>
                        </div>
...
`,ue=null,X=new Set,se=[],ge="all",It={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},At={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Xe(t){for(let[e,a]of Object.entries(It))if(a.includes(t))return e;return"system"}function _t(t){return At[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Mt();let o=`/events?ml=${ge==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let m=(await s.json()).events||[],v=m;if(ge!=="all"&&(v=m.filter(n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!1}return Xe(i.type)===ge})),se=v.slice(0,50),ue=Date.now(),B(1,ue),se.length===0){e.innerHTML=T("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let l=i.type,y=Xe(l),d=_t(l),x=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),c="event-border-grey";x&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?c="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?c="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?c="event-border-orange":l==="system.test.completed"?c=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":c="event-border-blue");let b=x?"cursor-pointer":"",h=X.has(n.id),_=h?"expanded":"",L=h?"display: block;":"display: none;",E=new Date(n.timestamp*1e3),A=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),O=Ye(l,i),H=i.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${i.user_level})</span>`:"",U="";if(x){let $="";if(l==="engagement.decision")$=`
                    ...
                    `;else if(l==="analysis.visual.completed"){let C="";i.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${i.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:i.url&&(C=`<div class="event-detail-block"><img src="${i.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(l==="messaging.user.sent_message"){let C="";i.attachments&&i.attachments.length>0&&(C=`
                            <div class="event-detail-block">
            ...
            `,x){let $=M.querySelector(".close-details-btn");$&&$.addEventListener("click",C=>{C.stopPropagation();let N=C.target.closest(".event-item");if(N){N.classList.remove("expanded"),X.delete(n.id);let D=N.querySelector(".event-details");D&&(D.style.display="none")}})}return M},f=Array.from(e.children),w=new Map(f.map(n=>[n.dataset.eventId,n])),g=new Set(se.map(n=>n.id));f.forEach(n=>{let i=n.dataset.eventId;(!i||!g.has(i))&&n.remove()});let u=null;se.forEach((n,i)=>{let l=w.get(n.id);(!l||t)&&(l&&l.remove(),l=p(n),!l)||(i===0?e.firstElementChild!==l&&e.prepend(l):u&&u.nextElementSibling!==l&&u.after(l),u=l)}),ue=Date.now(),B(1,ue)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=T("offline","Failed to load events.","The event service may be offline or unreachable."))}}function Mt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),a=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{se.forEach(o=>X.add(o.id)),Q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),Q(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.querySelectorAll(".filter-btn").forEach(o=>{o.onclick=()=>{a.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active"),ge=o.dataset.filter,Q(!0)}}),a.dataset.listenerAttached="true")}function Ze(){return`
...
`),p=[...m.logs];if(p.length<25){let w=25-p.length;for(let g=0;g<w;g++)p.push("")}else p.length>25&&(p=p.slice(-25));let f=p.map(w=>re(w)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ze()}
        </div>`;async function dt(){await Promise.all([ve(),Ee(),Se()]),await ne()}var et=null,tt=null,st=null;async function mt(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function nt(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Bt(){try{return await(await I("/processes")).json()}catch{return null}}async function Nt(){try{return await(await I("/analyst/status")).json()}catch{return null}}async function Ee(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`}if(n.GPU&&n.GPU.length>0&&n.GPU.forEach((y,d)=>{let x=(y.VRAM/1073741824).toFixed(1);i+=`
                    <div class="service-widget" style="padding: 10px; min-height: 80px;">
                        <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let y=0,d=0;n.STORAGE.forEach(h=>{y+=h.USED,d+=h.SIZE});let x=(y/(1024*1024*1024)).toFixed(1),c=(d/(1024*1024*1024)).toFixed(1),b=d>0?(y/d*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${_};">${c}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${L};"></i>
                        <span style="color: ${E};">${b}</span>
                    </div>
                </div>`:S='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${y}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${p(n.domain&&n.port?`${n.domain}:${n.port}`:"")}</span></div>${S}</div></div>`}let g=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),u=new Set(r.map(n=>n.id));for(let[n,i]of g)u.has(n)||i.remove();r.forEach(n=>{let i=w(n),l=g.get(n.id);l?l.outerHTML!==i&&(l.outerHTML=i):t.insertAdjacentHTML("beforeend",i)})}async function Se(){let t=document.getElementById("models-widgets");if(!t)return;let e=await mt();if(!e){t.innerHTML=T("offline","Failed to load model status.");return}tt=Date.now(),B(3,tt);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(v=>{v.classList.contains("service-widget")||v.remove()});function s(v){let p=v.status==="Ready";return`
                ...
                </div>`}function r(v){let p=v.status==="Downloaded",f=p?"service-widget-online":"service-widget-offline",w=p?"OK":"MISSING",g=p&&v.size>0?`${(v.size/1e9).toFixed(2)} GB`:"-",u=v.name;return v.type==="custom"&&u.endsWith(":latest")&&(u=u.replace(":latest","")),`<div class="service-widget ${f}" data-model-name="${v.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${u}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${v.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let m="";if(o&&(m+=s(o)),m+=a.map(r).join(""),!m){t.innerHTML=T("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function ve(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),ve()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let m=await Nt();if(m){let u=Math.floor(Date.now()/1e3),n=m.active_tier,i=(l,y,d)=>{if(n===d||d==="guardian"&&n==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let x=y-u;if(x<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let c=Math.floor(x/60),b=x%60;l.textContent=`${c}m ${b}s`,l.style.color="#fff"}};if(e&&m.guardian&&i(e,m.guardian.next_run,"guardian"),a&&m.architect&&i(a,m.architect.next_run,"architect"),o&&m.strategist&&i(o,m.strategist.next_run,"strategist"),s&&m.system_idle_time!==void 0){let l=m.system_idle_time,y=Math.floor(l/60),d=l%60;s.textContent=`${y}m ${d}s`,l>300?s.style.color="#5eff5e":l>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let v=await Bt(),p=document.getElementById("vitals-processes-val");if(p)if(v){let u=v.length;p.textContent=u>0?`${u} Active`:"Idle",p.style.color=u>0?"#bb86fc":"#fff"}else p.textContent="-",p.style.color="#888";if(v===null){t.innerHTML=T("offline","Failed to load process status.");return}if(st=Date.now(),B(3,st),v.length===0){t.innerHTML=T("empty","No active processes."),P(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function f(u){let n=Math.floor(Date.now()/1e3-u.start_time),i=u.retries>0?`<span class="process-retry-badge">Retry ${u.retries}</span>`:"",l=u.channel_id,y={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return y[l]?l=y[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ot=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ce=!0,a}function pt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ce=!1}function ut(t,e,a="output"){if(!Ce)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=re(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Pt(t){let e=gt.find(s=>s.id===t);if(!e)return;let a=Rt(e);ut(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(m=>setTimeout(m,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),ut(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ot();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;Pt(o)})})}async function Ut(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${ye()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function ft(){Ut(),Pe(),Ae();let t=window.location.pathname;(t==="/dex"||t==="/dex/"||t.includes("/dex/index.html"))&&!t.includes("/contribute")&&vt();let e=Be();_e(e),Me();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");function m(){return 1}function v(){for(;s.length>1;)s.shift().close(!0);let d=document.getElementById("windows-container"),x=document.querySelector("nav"),c=document.querySelector("footer"),b=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");d&&d.setAttribute("data-count",s.length);let _=s.length>0;s.forEach(S=>{let O=document.getElementById(S.id);O&&O.classList.remove("hide-close")}),ie(te());let L=document.getElementById("dexter-menu-container"),E=document.getElementById("nav-window-switcher"),A=document.getElementById("settings-icon");if(s.length>0){if(c?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),x?.classList.add("window-open"),d&&(d.style.paddingTop="60px"),L&&(L.style.display="none"),A&&(A.style.display="none"),E){let S=s[0].id;["feed-window","events-window","monitor-window","contacts-window","workspace-window"].includes(S)?(E.innerHTML=`
                  ...
                  `,document.getElementById("switch-feed").addEventListener("click",()=>p(w)),document.getElementById("switch-events").addEventListener("click",()=>p(g)),document.getElementById("switch-monitor").addEventListener("click",()=>p(n)),document.getElementById("switch-contacts").addEventListener("click",()=>p(u)),document.getElementById("switch-workspace").addEventListener("click",()=>p(i))):E.innerHTML=""}}else x?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),d&&(d.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),b||h?c?.classList.remove("hide"):c?.classList.add("hide"),L&&(L.style.display="flex"),A&&(A.style.display="block"),E&&(E.innerHTML="")}function p(d){if(d.isOpen()){d.close();return}for(;s.length>0;)s.pop().close(!0);s.push(d),d.open(),v()}function f(){[...s].forEach(x=>x.close()),s=[]}window.addEventListener("resize",v);let w=F({id:"feed-window",title:"Feed",icon:"bx-news",content:ze(),onOpen:()=>j(),onClose:()=>{let d=s.indexOf(w);d>-1&&s.splice(d,1),v()}}),g=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Qe(),onOpen:()=>Q(),onClose:()=>{let d=s.indexOf(g);d>-1&&s.splice(d,1),v()}}),u=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Je(),onOpen:()=>pe(),onClose:()=>{let d=s.indexOf(u);d>-1&&s.splice(d,1),v()}}),n=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bx-server",title:"Services",content:ot()},{icon:"bx-component",title:"Processes",content:at()},{icon:"bx-brain",title:"Models",content:lt()},{icon:"bx-hdd",title:"Hardware",content:rt()},{icon:"bx-terminal",title:"Logs",content:ct()},{icon:"bx-zap",title:"Analyst",content:it()}],onOpen:()=>{Ee(),ve(),Se(),ne()},onClose:()=>{let d=s.indexOf(n);d>-1&&s.splice(d,1),v()}}),i=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Ve(),onOpen:()=>we(),onClose:()=>{let d=s.indexOf(i);d>-1&&s.splice(d,1),v()}}),l=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:Le(),onOpen:()=>$e(l),onClose:()=>{let d=s.indexOf(l);d>-1&&s.splice(d,1),v()}}),y=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dex/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let d=document.getElementById("dexter-dropdown");d&&(d.innerHTML=`
            <div class="dropdown-item" id="feed-menu-item"><i class='bx bx-news'></i> Feed</div>
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let x=document.getElementById("dexter-menu-btn");x&&d&&(x.addEventListener("click",c=>{c.stopPropagation(),d.classList.toggle("active"),x.classList.toggle("active")}),document.addEventListener("click",c=>{!d.contains(c.target)&&c.target!==x&&(d.classList.remove("active"),x.classList.remove("active"))})),document.getElementById("feed-menu-item")?.addEventListener("click",()=>{d?.classList.remove("active"),p(w)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{d?.classList.remove("active"),p(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{d?.classList.remove("active"),p(n)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{d?.classList.remove("active"),p(u)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{d?.classList.remove("active"),p(i)}),document.getElementById("settings-icon")?.addEventListener("click",()=>p(l)),document.getElementById("close-all-windows")?.addEventListener("click",()=>f()),setInterval(()=>{w.isOpen()&&j(),g.isOpen()&&Q(),u.isOpen()&&pe(),n.isOpen()&&dt(),i.isOpen()&&we()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{y.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",d=>{d.preventDefault();try{He(document.getElementById("email-input").value),window.location.reload()}catch(x){let c=document.getElementById("login-error");c&&(c.textContent=x.message,c.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ft):ft();})();

(()=>{function Le(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Te(t=!1){let e=loggedIn?`
        <div class="nav-right">
    ...
    `,l=document.createElement("nav");l.innerHTML=i,document.body.prepend(l)}function ke(){let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}function ue(t){let e=null,i=t.onClose||null,l=t.onOpen||null;function a(){if(e){e.classList.add("open"),window.addEventListener("resize",r),l&&setTimeout(l,10);return}let o=document.getElementById("windows-container");o||(o=document.createElement("div"),o.id="windows-container",o.className="windows-container",document.body.appendChild(o)),e=document.createElement("div"),e.id=t.id,e.className="window";let f=t.icon||"bx-window",g=t.title?`<div class="window-title">${t.title}</div>`:"",s=`
            <div class="window-header">
                <i class="bx ${f}"></i>
                ${g}
                <i class="bx bx-x window-close"></i>
            </div>
        `;e.innerHTML=s+`<div class="window-content">${t.content||""}</div>`,o.appendChild(e);let p=e.querySelector(".window-close");p&&p.addEventListener("click",v=>{v.stopPropagation(),c()}),window.addEventListener("resize",r),setTimeout(()=>{e.classList.add("open"),l&&l()},10)}function r(){}function c(o=!1){e&&(window.removeEventListener("resize",r),o?(e.classList.add("switching"),e.classList.remove("open"),setTimeout(()=>{e&&e.parentNode&&e.parentNode.removeChild(e),e=null},200)):(e.classList.remove("open"),i&&i(),setTimeout(()=>{e&&e.parentNode&&e.parentNode.removeChild(e),e=null},400)))}function b(o){if(e){let f=e.querySelector(".window-content");f&&(f.innerHTML=o)}}function m(){return e&&e.classList.contains("open")}return{open:a,close:c,setContent:b,isOpen:m,id:t.id}}function Ce(){return!0}function Se(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function _e(t){localStorage.setItem("easter_user_email",t.trim())}var Ae="easter_theme",_={AUTO:"auto",DEFAULT:"default",ANIMATED:"animated"};function fe(){return localStorage.getItem(Ae)||_.AUTO}function st(){let t=window.innerWidth,e=window.innerHeight,i=window.innerHeight===window.screen.height&&window.innerWidth===window.screen.width;return t>1920&&e>1080?_.ANIMATED:(t===1920&&e===1080&&i||t<=1920||e<=1080,_.DEFAULT)}function Ie(t){if(!Object.values(_).includes(t))throw new Error("Invalid theme");localStorage.setItem(Ae,t),ge(t)}function ge(t,e=!1){let i=document.body,l=t===_.AUTO?st():t;if(e||(i.classList.add("theme-transitioning"),setTimeout(()=>{i.classList.remove("theme-transitioning")},300)),Object.values(_).forEach(a=>{i.classList.remove(`theme-${a}`)}),i.classList.add(`theme-${t}`),l===_.ANIMATED){if(!document.querySelector(".background")){let a=document.createElement("div");a.className="background",document.body.prepend(a)}}else{let a=document.querySelector(".background");a&&(e?a.remove():(a.style.animation="fadeOutBackground 0.4s ease forwards",setTimeout(()=>{a.remove()},400)))}}function Me(){let t=fe();if(ge(t,!0),t===_.AUTO){let e;window.addEventListener("resize",()=>{clearTimeout(e),e=setTimeout(()=>{ge(_.AUTO)},300)})}}function L(t,e,i=null){let a={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=i?`<p class="placeholder-action">${i}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${a} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function T(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function it(t,e){let i=document.getElementById(t);i&&(i.style.display=e?"block":"none")}function ne(){let t=document.getElementById("notifications-list");if(!t)return;let e=t.querySelectorAll(".notification-unread").length;it("notif-dot",e>0)}function ve(){return"https://event.easter.company"}var nt="http://127.0.0.1:8100";function at(){return"https://discord.easter.company"}var ot="http://127.0.0.1:8300",lt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function ae(t){let e=T(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(a,r)=>{let c=lt[r];return c?`<span class="${c}">`:""});let i=(e.match(/<span/g)||[]).length,l=(e.match(/<\/span/g)||[]).length;return i>l&&(e+="</span>".repeat(i-l)),e}var q=null,W=null,se=!1,ie=!1;async function $(t,e={}){if(q)try{let a=await fetch(q+t,e);if(a.ok)return a;q=null}catch{q=null}let i=ve(),l=nt;try{let a=await fetch(i+t,e);if(a.ok)return q=i,se&&(console.log("\u2728 Production event service restored."),se=!1),a;throw new Error("Primary failed")}catch{se||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${l}`),se=!0);try{let r=await fetch(l+t,e);if(r.ok)return q=l,r;throw new Error("Fallback failed")}catch(r){throw r}}}async function Be(t,e={}){if(W)try{let a=await fetch(W+t,e);if(a.ok)return a;W=null}catch{W=null}let i=at(),l=ot;try{let a=await fetch(i+t,e);if(a.ok)return W=i,ie&&(console.log("\u2728 Production discord service restored."),ie=!1),a;throw new Error("Primary failed")}catch{ie||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${l}`),ie=!0);try{let r=await fetch(l+t,e);if(r.ok)return W=l,r;throw new Error("Fallback failed")}catch(r){throw r}}}var Ne=()=>`
...
`,De=null,U=new Set,oe=[];async function G(t=!1){let e=document.getElementById("notifications-list");if(!e)return;rt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let l=await $(i);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];De=Date.now();let c=Date.now(),b=24*60*60*1e3,m=r.filter(u=>{let y=localStorage.getItem(`notification_read_ts_${u.id}`);if(!y)return!0;let h=parseInt(y);return c-h<b});m.sort((u,y)=>{let h=S=>{let D=S.event;if(typeof D=="string")try{D=JSON.parse(D)}catch{return"low"}return(D.priority||"low").toLowerCase()},B=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,k=B(h(u)),C=B(h(y));return k!==C?C-k:y.timestamp-u.timestamp}),oe=m;let o=u=>{let y=u.event;if(typeof y=="string")try{y=JSON.parse(y)}catch{return"low"}return(y.priority||"low").toLowerCase()},f=[],s=new Set(m.map(u=>o(u))).size>1;if(m.length>0){let u=null;m.forEach(y=>{let h=o(y);s&&h!==u&&(f.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),u=h),f.push(y)})}if(t&&(e.innerHTML=""),m.length===0){e.innerHTML=L("empty","No notifications yet."),ne();return}let p=u=>{let y=u.event;if(typeof y=="string")try{y=JSON.parse(y)}catch{return null}let h=y.title||"Untitled Notification",B=y.body||"No description provided.",k=y.priority||"low",C=!!y.alert,S=y.category||"system",D=y.related_event_ids||[],A=!!localStorage.getItem(`notification_read_ts_${u.id}`),z=new Date(u.timestamp*1e3),H=z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),O=z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=A?"event-border-grey":"event-border-blue";!A&&C&&(E="event-border-red"),A&&(k==="high"||k==="critical")?E="event-border-red":A&&k==="medium"&&(E="event-border-orange");let N=A?"notification-read":"notification-unread",I=U.has(u.id),M=I?"expanded":"",de=I?"display: block;":"display: none;",te="",Ee="";D.length>0&&(Ee=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${D.map(X=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${X.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),te=`
                <div class="event-detail-row">
                ...
                <div class="event-detail-block" style="text-align: left;">
                    ...
                    <p class="detail-pre" style="white-space: pre-wrap; margin-top: 5px; text-align: left;">${T(B)}</p>
                </div>
                ${Ee}
            `;let P=document.createElement("div");P.className=`event-item notification-item ${E} ${N} ${M} cursor-pointer`,P.dataset.notificationId=u.id,P.onclick=function(X){if(this.classList.contains("expanded")){this.classList.remove("expanded"),U.delete(u.id);let Z=this.querySelector(".event-details");Z&&(Z.style.display="none")}else{this.classList.add("expanded"),U.add(u.id);let Z=this.querySelector(".event-details");if(Z&&(Z.style.display="block"),!localStorage.getItem(`notification_read_ts_${u.id}`)){localStorage.setItem(`notification_read_ts_${u.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let me="event-border-grey";k==="high"||k==="critical"?me="event-border-red":k==="medium"&&(me="event-border-orange"),this.classList.add(me),ne()}}},P.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${H}</span>
                    <span class="event-date">${O}</span>
                </div>
                <div class="event-content">
                    <div class="event-service">${S.toUpperCase()} ${C?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>
                    <div class="event-message">${h}</div>
                    <div class="event-details" style="${de}">
            ...
            `;let $e=P.querySelector(".close-details-btn");return $e&&$e.addEventListener("click",X=>{X.stopPropagation(),P.classList.remove("expanded");let pe=P.querySelector(".event-details");pe&&(pe.style.display="none"),U.delete(u.id)}),P},v=u=>{let y=document.createElement("div");y.className="notification-divider",y.dataset.notificationId=u.id;let h="#888888";return u.label==="CRITICAL"?h="#ff4d4d":u.label==="HIGH"?h="#ff8888":u.label==="MEDIUM"&&(h="#ffa500"),y.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,y.innerHTML=`<span style="white-space: nowrap;">${u.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,y},n=Array.from(e.children),d=new Map(n.map(u=>[u.dataset.notificationId,u])),x=new Set(f.map(u=>u.id));n.forEach(u=>{let y=u.dataset.notificationId;(!y||!x.has(y))&&u.remove()});let w=null;f.forEach((u,y)=>{let h=d.get(u.id);(!h||t)&&(h&&h.remove(),u.type==="divider"?h=v(u):h=p(u),!h)||(y===0?e.firstElementChild!==h&&e.prepend(h):w&&w.nextElementSibling!==h&&w.after(h),w=h)}),De=Date.now(),ne()}catch(l){console.error("Error fetching notifications:",l),e.children.length===0&&(e.innerHTML=L("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function rt(){let t=document.getElementById("notif-read-all"),e=document.getElementById("notif-expand-all"),i=document.getElementById("notif-close-all"),l=document.getElementById("notif-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{oe.forEach(a=>{localStorage.getItem(`notification_read_ts_${a.id}`)||localStorage.setItem(`notification_read_ts_${a.id}`,Date.now().toString())}),G(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{oe.forEach(a=>{U.add(a.id),localStorage.getItem(`notification_read_ts_${a.id}`)||localStorage.setItem(`notification_read_ts_${a.id}`,Date.now().toString())}),G(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{U.clear(),G(!0)},i.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=()=>{let a=Date.now()-1728e5;oe.forEach(r=>{localStorage.setItem(`notification_read_ts_${r.id}`,a.toString())}),U.clear(),G(!0)},l.dataset.listenerAttached="true")}var He=()=>`
  <div class="notifications-actions">
...
`,j=new Set,ze=[],V=null;async function F(t=!1){let e=document.getElementById("roadmap-list");if(e){ct();try{let i=await $("/roadmap");if(!i.ok)throw new Error("Offline");let l=await i.json();if(ze=l,l.length===0){e.innerHTML=L("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let a=o=>{let f=j.has(o.id),g=o.state==="draft",s=o.state==="published",p=o.state==="consumed",v="event-border-grey";s&&(v="event-border-blue"),p&&(v="event-border-purple");let d=new Date(o.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),x=document.createElement("div");x.className=`event-item notification-item ${v} cursor-pointer ${f?"expanded":""}`,x.dataset.itemId=o.id,x.onclick=u=>{if(u.target.closest("button"))return;x.classList.contains("expanded")?(x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",j.delete(o.id)):(x.classList.add("expanded"),x.querySelector(".event-details").style.display="block",j.add(o.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${o.state.toUpperCase()}]</span>`;return x.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${T(o.content)}</div>
          <div class="event-details" style="${f?"display: block;":"display: none;"} ">
            ...
            ${p?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(o.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `,x.querySelector(".edit-btn")?.addEventListener("click",()=>dt(o)),x.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>pt(o)),x.querySelector(".delete-btn")?.addEventListener("click",()=>mt(o.id)),x.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",j.delete(o.id)}),x},r=Array.from(e.children),c=new Map(r.map(o=>[o.dataset.itemId,o])),b=new Set(l.map(o=>o.id));r.forEach(o=>{let f=o.dataset.itemId;(!f||!b.has(f))&&o.remove()});let m=null;l.forEach((o,f)=>{let g=c.get(o.id);(!g||t)&&(g&&g.remove(),g=a(o),!g)||(f===0?e.firstElementChild!==g&&e.prepend(g):m&&m.nextElementSibling!==g&&m.after(g),m=g)})}catch{e.innerHTML=L("error","Failed to load roadmap.")}}}function ct(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),l=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{V=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",V=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=V?`/roadmap/${V}`:"/roadmap",b=V?"PATCH":"POST";try{await $(c,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",F(!0)}catch(m){console.error(m)}},e.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=()=>{ze.forEach(r=>j.add(r.id)),F(!0)},l.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{j.clear(),F(!0)},a.dataset.listenerAttached="true")}function dt(t){V=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function pt(t){let e=t.state==="published"?"draft":"published";try{await $(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),F(!0)}catch(i){console.error(i)}}async function mt(t){if(confirm("Delete this roadmap item?"))try{await $(`/roadmap/${t}`,{method:"DELETE"}),j.delete(t),F(!0)}catch(e){console.error(e)}}var Oe=()=>`
    <div class="notifications-actions">
...
`,ut=null,J=new Set,Pe=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;gt();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let l=await $(i);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];if(Pe=r,ut=Date.now(),r.length===0){e.innerHTML=L("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle.");return}t&&(e.innerHTML="");let c=g=>{let s=g.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let p=s.title||"Untitled Blueprint",v=s.summary||s.body||"No summary provided.",n=s.content||"",d=s.category||"architecture",x=s.affected_services||[],w=s.implementation_path||[],u=new Date(g.timestamp*1e3),y=u.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=u.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),B=J.has(g.id),k=B?"display: block;":"display: none;",C=document.createElement("div");C.className=`event-item notification-item event-border-purple cursor-pointer ${B?"expanded":""}`,C.dataset.blueprintId=g.id,C.onclick=function(A){if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(g.id);let H=this.querySelector(".event-details");H&&(H.style.display="none")}else{this.classList.add("expanded"),J.add(g.id);let H=this.querySelector(".event-details");H&&(H.style.display="block")}};let S=x.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${x.join(", ")}</div>`:"",D="";w.length>0&&(D=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${w.map(A=>`<li>${T(A)}</li>`).join("")}
                        </ul>
                    </div>
                `),C.innerHTML=`
                <div class="event-time">
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${k}">
                        ...
                        <div class="event-detail-block" style="text-align: left; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px; margin: 10px 0;">
                            <p class="detail-pre" style="white-space: pre-wrap; font-family: 'JetBrains Mono', monospace; font-size: 0.85em; text-align: left; color: #ccc;">${T(n)}</p>
                        </div>
                        ${D}
                    </div>
                </div>
            `;let R=C.querySelector(".close-details-btn");return R&&R.addEventListener("click",A=>{A.stopPropagation(),C.classList.remove("expanded");let z=C.querySelector(".event-details");z&&(z.style.display="none"),J.delete(g.id)}),C},b=Array.from(e.children),m=new Map(b.map(g=>[g.dataset.blueprintId,g])),o=new Set(r.map(g=>g.id));b.forEach(g=>{let s=g.dataset.blueprintId;(!s||!o.has(s))&&g.remove()});let f=null;r.forEach((g,s)=>{let p=m.get(g.id);(!p||t)&&(p&&p.remove(),p=c(g),!p)||(s===0?e.firstElementChild!==p&&e.prepend(p):f&&f.nextElementSibling!==p&&f.after(p),f=p)})}catch(l){console.error("Error fetching blueprints:",l),e.children.length===0&&(e.innerHTML=L("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function gt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Pe.forEach(i=>J.add(i.id)),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{J.clear(),le(!0)},e.dataset.listenerAttached="true")}var Ue=()=>`
...
`;async function je(){await Promise.all([F(),le()])}var Fe=()=>`
    <div class="notifications-actions">
...
`,ft=null;async function K(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>K(),e.dataset.listenerAttached="true");try{let i=await Be("/contacts");if(!i.ok)throw new Error("Service unreachable");let a=(await i.json()).members||[];if(ft=Date.now(),a.length===0){t.innerHTML=L("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};a.sort((c,b)=>{let m=r[c.level]??10,o=r[b.level]??10;return m!==o?m-o:c.username.localeCompare(b.username)}),t.innerHTML=a.map(c=>{let b=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",m=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",o="#888";c.level==="Me"||c.level==="Master User"?o="#bb86fc":c.level==="Admin"?o="#cf6679":c.level==="Moderator"?o="#03dac6":c.level==="Contributor"&&(o="#ffa500");let f=c.level==="Me",g=f?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",s=f?"2px solid #bb86fc":`1px solid ${b}33`;return`
...
`,re=null,Y=new Set,Q=[],ce="all",yt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},bt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function qe(t){for(let[e,i]of Object.entries(yt))if(i.includes(t))return e;return"system"}function ht(t){return bt[t]||"bx-square-rounded"}async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;xt();let l=`/events?ml=${ce==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let a=await $(l);if(!a.ok)throw new Error("Service is offline or unreachable.");let c=(await a.json()).events||[];c.some(v=>{let n=v.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!1}return n.type==="messaging.user.joined_server"})&&K();let m=c;if(ce!=="all"&&(m=c.filter(v=>{let n=v.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!1}return qe(n.type)===ce})),Q=m.slice(0,50),re=Date.now(),updateTabTimestamp(1,re),Q.length===0){e.innerHTML=L("empty","No events found for this filter.");return}t&&(e.innerHTML="");let o=v=>{let n=v.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let d=n.type,x=qe(d),w=ht(d),u=d==="engagement.decision"||d==="messaging.bot.sent_message"||d==="messaging.user.sent_message"||d==="moderation.explicit_content.deleted"||d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.cli.command"||d==="system.analysis.audit"||d==="system.test.completed"||d==="error_occurred"||d==="system.cli.status"||d.startsWith("system.roadmap")||d.startsWith("system.process"),y="event-border-grey";u&&(d==="moderation.explicit_content.deleted"||d==="error_occurred"?y="event-border-red":d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.analysis.audit"?y="event-border-purple":d==="system.cli.command"||d==="system.cli.status"?y="event-border-orange":d==="system.test.completed"?y=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":y="event-border-blue");let h=u?"cursor-pointer":"",B=Y.has(v.id),k=B?"expanded":"",C=B?"display: block;":"display: none;",S=new Date(v.timestamp*1e3),D=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),R=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=Re(d,n),z=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",H="";if(u){let E="";if(d==="engagement.decision")E=`
                    ...
                    `;else if(d==="messaging.bot.sent_message")E=`
                        <div class="event-detail-row">
                    ...
                    `;else if(d==="moderation.explicit_content.deleted")E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${T(n.raw_output)||"None"}</pre>
                        </div>
                    `;else if(d==="analysis.link.completed")E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row">
                            ...
                            <span class="detail-value">${T(n.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Description:</span>
                            <pre class="detail-pre">${T(n.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Content Summary:</span>
                            <pre class="detail-pre">${T(n.summary)||"None"}</pre>
                        </div>
                    `;else if(d==="analysis.visual.completed"){let N="";n.base64_preview?N=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(N=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(d==="system.cli.command")E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${T(n.output)||"No output recorded."}</pre>
                        </div>
                    `;else if(d==="system.analysis.audit")E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${T(n.raw_output)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Raw Input (Prompt):</span>
                            <pre class="detail-pre">${T(n.raw_input)}</pre>
                        </div>
                    `;else if(d==="system.test.completed")E=`
                        <div class="event-detail-row">
                    ...
                    `;else if(d==="error_occurred")E=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${T(n.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Context:</span>
                            <pre class="detail-pre">${JSON.stringify(n.context||{},null,2)}</pre>
                        </div>
                    `;else if(d==="system.cli.status")E=`
                        <div class="event-detail-row">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${T(n.message)}</pre>
                        </div>
                    `;else if(d==="messaging.user.sent_message"){let N="";n.attachments&&n.attachments.length>0&&(N=`
                            <div class="event-detail-block">
                                <span class="detail-label">Attachments:</span>
                                <div class="attachments-grid">${n.attachments.map(M=>{let de=M.content_type&&M.content_type.startsWith("image/"),te=(M.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${de?`<div class="attachment-thumb-container"><img src="${M.url}" alt="${M.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${M.url}" target="_blank" class="attachment-link">${M.filename}</a>
                                        <span class="attachment-meta">${M.content_type} \u2022 ${te}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),E=`
                        <div class="event-detail-row">
                ...
                `}let O=document.createElement("div");if(O.className=`event-item ${y} ${h} ${k}`,O.dataset.eventId=v.id,O.onclick=function(E){if(!u)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(v.id);let I=this.querySelector(".event-details");I&&(I.style.display="none")}else{this.classList.add("expanded"),Y.add(v.id);let I=this.querySelector(".event-details");I&&(I.style.display="block")}},O.innerHTML=`
                <div class="event-time">
            ...
            `,u){let E=O.querySelector(".close-details-btn");E&&E.addEventListener("click",N=>{N.stopPropagation();let I=N.target.closest(".event-item");if(I){I.classList.remove("expanded"),Y.delete(v.id);let M=I.querySelector(".event-details");M&&(M.style.display="none")}})}return O},f=Array.from(e.children),g=new Map(f.map(v=>[v.dataset.eventId,v])),s=new Set(Q.map(v=>v.id));f.forEach(v=>{let n=v.dataset.eventId;(!n||!s.has(n))&&v.remove()});let p=null;Q.forEach((v,n)=>{let d=g.get(v.id);(!d||t)&&(d&&d.remove(),d=o(v),!d)||(n===0?e.firstElementChild!==d&&e.prepend(d):p&&p.nextElementSibling!==d&&p.after(d),p=d)}),re=Date.now(),updateTabTimestamp(1,re)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=L("offline","Failed to load events.","The event service may be offline or unreachable."))}}function xt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Q.forEach(l=>Y.add(l.id)),ee(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),ee(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(l=>{l.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(a=>a.classList.remove("active")),l.classList.add("active"),ce=l.dataset.filter,ee(!0)}}),i.dataset.listenerAttached="true")}function Ge(){return`
        <div id="logs-container" class="logs-container">
            <p>Loading logs...</p>
        </div>
    `}var wt=null;async function ye(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await $("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML='<p style="text-align: center; opacity: 0.5; padding: 20px;">No logs found.</p>',!1;let l=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],a=i.filter(c=>!l.includes(c.id));a.forEach(c=>{c.logs&&Array.isArray(c.logs)?c.logs.reverse():c.logs=[]}),a.reverse();let r=a.map(c=>{let b=c.logs.join(`
`),m=[...c.logs];if(m.length<25){let f=25-m.length;for(let g=0;g<f;g++)m.push("")}else m.length>25&&(m=m.slice(-25));let o=m.map(f=>ae(f)).join(`
`);return`
                <div class="log-report">
                    ...
                    <pre class="log-content">${o}</pre>
                </div>
            `}).join("");return t.innerHTML=r,document.querySelectorAll(".copy-logs-btn").forEach(c=>{c.addEventListener("click",()=>{let b=unescape(c.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let m=c.querySelector("i");m.classList.remove("bx-copy"),m.classList.add("bx-check"),setTimeout(()=>{m.classList.remove("bx-check"),m.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(c=>{c.addEventListener("click",async()=>{let b=c.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await $(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&ye()}catch(m){console.error("Error clearing logs:",m)}})}),wt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.innerHTML='<p style="text-align: center; color: #cf6679; padding: 20px;">Failed to load logs.</p>',!1}}var Je=()=>`
        <div class="system-section-header" style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
    ...
    `;async function Ke(){await Promise.all([Xe(),Ct(),St()]),await ye()}var Et=null,$t=null,Lt=null;async function Ye(){try{return await(await $("/system_monitor")).json()}catch{return null}}async function Ve(){try{return await(await $("/system/hardware")).json()}catch{return null}}async function Tt(){try{return await(await $("/processes")).json()}catch{return null}}async function kt(){try{return await(await $("/analyst/status")).json()}catch{return null}}async function Ct(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),i=document.getElementById("hardware-refresh-btn"),l=s=>{if(!s){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let p="",v=(s.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(p+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    ...
                    </div>`}),s.STORAGE&&s.STORAGE.length>0){let n=0,d=0;s.STORAGE.forEach(y=>{n+=y.USED,d+=y.SIZE});let x=(n/(1024*1024*1024)).toFixed(1),w=(d/(1024*1024*1024)).toFixed(1),u=d>0?(n/d*100).toFixed(0):0;p+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    <div class="service-widget-body">
                        <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${x} / ${w} GB</span>
                ...
                </div>`}e.innerHTML=p};if(e&&i){i.dataset.listenerAttached||(i.onclick=async()=>{i.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let p=await Ve();p?(l(p),i.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(i.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},i.dataset.listenerAttached="true");let s=e.querySelector("p");if(s&&s.textContent==="Loading hardware info..."){let p=await Ve();l(p)}}if(!t)return;let a=await Ye();if(!a||!a.services){t.innerHTML=L("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}Et=Date.now();let r=a.services||[];Array.from(t.children).forEach(s=>{s.classList.contains("service-widget")||s.remove()});function c(s){if(!s||s==="N/A"||s==="unknown")return"-";let p=s.match(/^(\d+\.\d+\.\d+)/);return p?p[0]:s.split(".").slice(0,3).join(".")||"-"}function b(s){return!s||s.length<=28?s:s.substring(0,28)+"..."}function m(s){if(!s||s==="N/A"||s==="unknown")return"-";let p=s.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!p)return"-";let v=parseInt(p[1])||0,n=parseInt(p[2])||0,d=parseInt(p[3])||0,x=parseFloat(p[4])||0,w=v*86400+n*3600+d*60+x,u=Math.floor(w/86400);if(u>0)return`${u}d`;let y=Math.floor(w/3600);if(y>0)return`${y}h`;let h=Math.floor(w/60);return h>0?`${h}m`:`${Math.floor(w)}s`}function o(s){let p=s.status==="online",v=p?"service-widget-online":"service-widget-offline",n=p?"bx-check-circle":"bx-x-circle",d=p?"OK":"BAD",x=s.version?c(s.version.str):"-",w=s.cpu&&s.cpu!=="N/A"?s.cpu:"-",u=s.memory&&s.memory!=="N/A"?s.memory:"-",y=w!=="-"?"#00ff00":"#666",h=w!=="-"?"#fff":"#666",B=u!=="-"?"#00ff00":"#666",k=u!=="-"?"#fff":"#666",C=m(s.uptime),S="";return p?S=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${k};">${u}</span>
                    </div>
                </div>`:S='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${v}" data-service-id="${s.id}"><div class="service-widget-header"><i class="bx ${n}"></i><h3>${s.short_name||s.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${b(s.domain&&s.port?`${s.domain}:${s.port}`:"")}</span></div>${S}</div></div>`}let f=new Map(Array.from(t.querySelectorAll(".service-widget")).map(s=>[s.dataset.serviceId,s])),g=new Set(r.map(s=>s.id));for(let[s,p]of f)g.has(s)||p.remove();r.forEach(s=>{let p=o(s),v=f.get(s.id);v?v.outerHTML!==p&&(v.outerHTML=p):t.insertAdjacentHTML("beforeend",p)})}async function St(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ye();if(!e){t.innerHTML=L("offline","Failed to load model status.");return}$t=Date.now();let i=e.models||[],l=e.whisper;Array.from(t.children).forEach(b=>{b.classList.contains("service-widget")||b.remove()});function a(b){let m=b.status==="Ready";return`<div class="service-widget ${m?"service-widget-online":"service-widget-offline"}" data-whisper-widget><div class="service-widget-header"><i class="bx bxs-microphone-alt"></i><h3>Whisper</h3><span class="service-widget-status">${m?"READY":"NOT FOUND"}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Status:</span><span class="info-value">${b.status}</span></div><div class="service-widget-info"><span class="info-label">Model:</span><span class="info-value">large-v3-turbo</span></div></div></div>`}function r(b){let m=b.status==="Downloaded",o=m?"service-widget-online":"service-widget-offline",f=m?"OK":"MISSING",g=m&&b.size>0?`${(b.size/1e9).toFixed(2)} GB`:"-",s=b.name;return b.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${o}" data-model-name="${b.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${b.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let c="";if(l&&(c+=a(l)),c+=i.map(r).join(""),!c){t.innerHTML=L("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function Xe(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),i=document.getElementById("analyst-t2-val"),l=document.getElementById("analyst-t3-val"),a=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await $("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),Xe()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await kt();if(c){let s=Math.floor(Date.now()/1e3),p=c.active_tier,v=(n,d,x)=>{if(p===x||x==="guardian"&&p==="tests"){n.textContent="Working",n.style.color="#bb86fc";return}let w=d-s;if(w<=0)n.textContent="Ready",n.style.color="#5eff5e";else{let u=Math.floor(w/60),y=w%60;n.textContent=`${u}m ${y}s`,n.style.color="#fff"}};if(e&&c.guardian&&v(e,c.guardian.next_run,"guardian"),i&&c.architect&&v(i,c.architect.next_run,"architect"),l&&c.strategist&&v(l,c.strategist.next_run,"strategist"),a&&c.system_idle_time!==void 0){let n=c.system_idle_time,d=Math.floor(n/60),x=n%60;a.textContent=`${d}m ${x}s`,n>300?a.style.color="#5eff5e":n>60?a.style.color="#ffa500":a.style.color="#fff"}}else[e,i,l,a].forEach(s=>{s&&(s.textContent="Offline",s.style.color="#ff4d4d")});let b=await Tt(),m=document.getElementById("vitals-processes-val");if(m)if(b){let s=b.length;m.textContent=s>0?`${s} Active`:"Idle",m.style.color=s>0?"#bb86fc":"#fff"}else m.textContent="-",m.style.color="#888";if(b===null){t.innerHTML=L("offline","Failed to load process status.");return}if(Lt=Date.now(),b.length===0){t.innerHTML=L("empty","No active processes.");return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function o(s){let p=Math.floor(Date.now()/1e3-s.start_time),v=s.retries>0?`<span class="process-retry-badge">Retry ${s.retries}</span>`:"",n=s.channel_id,d={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return d[n]?n=d[n]:/^\d+$/.test(n)&&(n=`Channel ${n}`),`<div class="service-widget process-widget" data-channel-id="${s.channel_id}"><div class="service-widget-header"><i class="bx bx-cog"></i><h3>${n}</h3>${v}</div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">State:</span><span class="info-value">${s.state}</span></div><div class="service-widget-info"><span class="info-label">Duration:</span><span class="info-value">${p}s</span></div><div class="service-widget-info"><span class="info-label">PID:</span><span class="info-value">${s.pid}</span></div></div></div>`}let f=new Map(Array.from(t.querySelectorAll(".process-widget")).map(s=>[s.dataset.channelId,s])),g=new Set(b.map(s=>s.channel_id));for(let[s,p]of f)g.has(s)||p.remove();b.forEach(s=>{let p=o(s),v=f.get(s.channel_id);v?v.outerHTML!==p&&(v.outerHTML=p):t.insertAdjacentHTML("beforeend",p)})}function be(){let t=fe(),e=Se()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},l=localStorage.getItem("easter_analytics_enabled")!=="false";return`
                ...
                ${Object.values(_).map(a=>`
                    <div class="theme-card ${t===a?"active":""}" data-theme="${a}">
                        ...
                        <div class="theme-info">
                            <h3>${a}</h3>
                            <p>${a===_.AUTO?"Automatic theme selection.":a===_.DEFAULT?"Simple, black, default.":"Colourful, not bright."}</p>
    ...
    `;for(let[i,l]of Object.entries(t)){let a=Qe.filter(r=>r.category===i);a.length!==0&&(e+=`
            <div class="cli-category-section" style="margin-bottom: 60px;">
                <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 30px; padding: 0 20px;">
                    <i class='bx ${l.icon}' style="color: ${l.color}; font-size: 1.8em;"></i>
                    <h2 style="font-size: 1.4em; text-transform: uppercase; letter-spacing: 3px; color: #eee; margin: 0;">${l.title}</h2>
                </div>
                    ...
                    ${a.map(r=>`
                        <div class="cli-command-card ${r.dummy?"dummy":""}" data-cmd="${r.id}" style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; padding: 30px; transition: all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1); cursor: pointer; position: relative; overflow: hidden; display: flex; flex-direction: column;">
                            ...
                            <div style="display: flex; align-items: center; gap: 18px; margin-bottom: 20px; position: relative; z-index: 1;">
                                <i class='bx ${r.icon}' style="font-size: 2.2em; color: ${l.color};"></i>
                            ...
                            <div class="usage-block" style="background: rgba(0,0,0,0.4); padding: 12px 18px; border-radius: 10px; font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${l.color}; position: relative; z-index: 1; border: 1px solid rgba(255,255,255,0.03);">
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Ze(),document.getElementById("terminal-action-btn").onclick=()=>Ze());let i=document.getElementById("cli-terminal-body");return i.innerHTML="",document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),we=!0,i}function Ze(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),we=!1}function xe(t,e,i="output"){if(!we)return;let l=document.createElement("div");l.className=`terminal-line terminal-${i}`,i==="prompt"?l.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:l.innerHTML=ae(e),t.appendChild(l),t.scrollTop=t.scrollHeight}async function It(t){let e=Qe.find(l=>l.id===t);if(!e)return;let i=At(e);xe(i,`dex ${t}`,"prompt");try{let a=(await $("/cli/execute",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({command:t,args:[]})})).body.getReader(),r=new TextDecoder;for(;;){let{value:c,done:b}=await a.read();if(b)break;r.decode(c,{stream:!0}).split(`
`).forEach(f=>{if(f.trim()==="")return;let g="output";f.includes("[ERROR]")?g="error":f.includes("[SUCCESS]")||f.includes("\u2713")?g="success":f.includes("!")&&(g="warning"),xe(i,f,g)})}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed"}catch(l){xe(i,`Connection Error: ${l.message}`,"error"),document.getElementById("terminal-status-badge").className="terminal-status status-error",document.getElementById("terminal-status-badge").textContent="Failed"}finally{document.getElementById("terminal-action-btn").style.display="block"}}function et(){let t=document.getElementById("cli-interface-container");t&&(t.innerHTML=_t(),t.querySelectorAll(".cli-command-card").forEach(e=>{e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-8px)",e.style.borderColor="rgba(255,255,255,0.15)",e.style.backgroundColor="rgba(255,255,255,0.04)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)",e.style.borderColor="rgba(255,255,255,0.05)",e.style.backgroundColor="rgba(255,255,255,0.02)"}),e.addEventListener("click",()=>{let i=e.dataset.cmd;if(i==="chat"){alert("Connection to cognitive core pending deployment...");return}It(i)})}))}async function Mt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${ve()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function tt(){Mt(),Me(),Le(),window.location.pathname.includes("/dex")&&et();let t=Ce();Te(t),ke();let e=document.querySelector("footer"),i=null;function l(){i=null,e?.classList.remove("hide"),document.querySelectorAll(".nav-right i").forEach(o=>o.classList.remove("active","inactive"))}function a(o,f=null,g=null){let s=i&&i.id===o.id;i&&i.close(!s),s?i=null:setTimeout(()=>{g===0?o.setContent(Ne()):g===1?o.setContent(We()):g===2?o.setContent(Ue()):g===3?o.setContent(Je()):g===4?o.setContent(Fe()):g===5&&o.setContent(be()),o.open(),i=o,o.activeTabIndex=g,document.querySelectorAll(".nav-right i").forEach(p=>{let v=p===f;p.classList.toggle("active",v),p.classList.toggle("inactive",!v&&f)}),e?.classList.add("hide"),r(),g===5&&he(o)},i?220:0)}async function r(o=!1){if(!m.isOpen())return;switch(m.activeTabIndex){case 0:await G(o);break;case 1:await ee(o);break;case 2:await je(o);break;case 3:await Ke(o);break;case 4:await K(o);break}}async function c(){let o=setInterval(()=>{if(!m.isOpen())return clearInterval(o);r()},3e3),f=setInterval(()=>{if(!m.isOpen())return clearInterval(f);K()},6e5)}let b=ue({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="https://github.com/eastercompany" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in",onClose:l}),m=ue({id:"main-window",icon:"bx-layer",onClose:l,onOpen:()=>{c()}});t?(document.getElementById("notif-icon")?.addEventListener("click",o=>a(m,o.currentTarget,0)),document.getElementById("events-icon")?.addEventListener("click",o=>a(m,o.currentTarget,1)),document.getElementById("ideas-icon")?.addEventListener("click",o=>a(m,o.currentTarget,2)),document.getElementById("system-icon")?.addEventListener("click",o=>a(m,o.currentTarget,3)),document.getElementById("contacts-icon")?.addEventListener("click",o=>a(m,o.currentTarget,4)),document.getElementById("settings-icon")?.addEventListener("click",o=>a(m,o.currentTarget,5))):document.getElementById("login-btn")?.addEventListener("click",()=>{a(b),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",o=>{o.preventDefault();try{_e(document.getElementById("email-input").value),window.location.reload()}catch(f){let g=document.getElementById("login-error");g&&(g.textContent=f.message,g.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",tt):tt();})();
//# sourceMappingURL=dex.733dd2a8.js.map

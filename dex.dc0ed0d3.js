...
`,ut=null,de=new Set,bt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;os();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await B(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let l=(await a.json()).events||[];if(bt=l,ut=Date.now(),Z(2,ut),l.length===0){e.innerHTML=R("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),S=(b.content||"").trim(),y=(b.category||"architecture").trim(),r=(b.related_services||b.affected_services||[]).map(T=>T.trim()),x=(b.implementation_path||[]).map(T=>T.trim()),u=b.source_event_ids||[],g=b.approved===!0,d=new Date(m.timestamp*1e3),E=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=de.has(m.id),I=A?"display: block;":"display: none;",w=document.createElement("div");w.className=`event-item notification-item event-border-purple cursor-pointer ${A?"expanded":""} ${g?"blueprint-approved":""}`,w.dataset.blueprintId=m.id,g&&(w.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",w.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let N=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";w.onclick=function(T){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="block")}};let _="";x.length>0&&(_=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${x.map(T=>`<li style="margin-bottom: 5px;">${z(T)}</li>`).join("")}</ul></div>
                    </div>
                `);let O="";u.length>0&&(O=`
                    ...
                    `}else if(f==="system.analysis.audit"){let C=n.success?"#03dac6":"#ff4d4d",$=n.success?"SUCCESS":"FAILED",L=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,T="";n.error&&(T=`
                        ...
                        `);let q="";if(n.chat_history&&n.chat_history.length>0){let J=n.chat_history.length,G=n.chat_history.map((K,ue)=>{let Y=K.role.toUpperCase();Y==="USER"&&(Y="SYSTEM"),Y==="ASSISTANT"&&(Y="AGENT");let V=K.role==="user"?"#03dac6":K.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${z(K.content)}</div>
                                </div>
                            `}).join("");q=`
                            <div class="event-detail-block">
                        ...
                        `}let F="";if(n.corrections&&n.corrections.length>0){let J=n.corrections.map((G,K)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${G.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${z(G.snippet)}</div>`:""}
                            </div>
                        `).join("");F=`
                            <div class="event-detail-block">
                        ...
                        `}let ee="";if(n.parsed_results&&n.parsed_results.length>0){let J=n.parsed_results.map(G=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${z(G.title)}</div>
                                <div style="color: #ddd;">${z(G.summary)}</div>
                            </div>
                        `).join("");ee=`
                            <div class="event-detail-block">
                                ${L("Parsed Results")}

...
`,he=null,X=new Set,we=[];async function Q(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Nt(),t&&(e.innerHTML="<p>Updating...</p>");let r="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await _(r);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];he=Date.now(),j(0,he);let l=Date.now(),p=24*60*60*1e3,d=i.filter(o=>{let m=localStorage.getItem(`alert_read_ts_${o.id}`);if(!m)return!0;let y=parseInt(m);return l-y<p});d.sort((o,m)=>{let y=T=>{let k=T.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},C=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,w=C(y(o)),$=C(y(m));return w!==$?$-w:m.timestamp-o.timestamp}),we=d;let g=o=>{let m=o.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},x=[],b=new Set(d.map(o=>g(o))).size>1;if(d.length>0){let o=null;d.forEach(m=>{let y=g(m);b&&y!==o&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),o=y),x.push(m)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=B("empty","No alerts yet."),xe();return}let n=o=>{let m=o.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let y=(m.title||"Untitled Alert").trim(),C=(m.body||"No description provided.").trim(),w=(m.priority||"low").toLowerCase(),$=!!m.alert,T=(m.category||"system").trim(),k=m.related_event_ids||[],N=m.audit_event_id,D=!!localStorage.getItem(`alert_read_ts_${o.id}`),P=new Date(o.timestamp*1e3),I=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=D?"event-border-grey":"event-border-blue";!D&&$&&(E="event-border-red"),D&&(w==="high"||w==="critical")?E="event-border-red":D&&w==="medium"&&(E="event-border-orange");let A=D?"alert-read":"alert-unread",q=X.has(o.id),F=q?"expanded":"",H=q?"display: block;":"display: none;",R="",z="";k.length>0&&(z=`
            <div style="flex: 1; min-width: 150px;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
                    ${k.map(K=>`<a href="#" onclick="window.dexter.viewEvent('${K}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${K.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let Ce="";N&&(Ce=`
...
`,Z=new Set,it=[],oe=null;async function ee(t=!1){let e=document.getElementById("roadmap-list");if(e){zt();try{let r=await _("/roadmap");if(!r.ok)throw new Error("Offline");let a=await r.json();it=a;let s=g=>{let x=Z.has(g.id),f=g.state==="published",b=g.state==="consumed",n="event-border-grey";f&&(n="event-border-blue"),b&&(n="event-border-purple");let L=new Date(g.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${n} cursor-pointer ${x?"expanded":""}`,h.dataset.itemId=g.id,h.onclick=o=>{if(o.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",Z.delete(g.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",Z.add(g.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${g.state.toUpperCase()}]</span>`;h.innerHTML=`
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${M(g.content)}</div>
...
`,at=null,re=new Set,lt=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ut();let r="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await _(r);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];if(lt=i,at=Date.now(),j(2,at),i.length===0){e.innerHTML=B("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),pe(2,0);return}t&&(e.innerHTML="");let l=f=>{let b=f.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),u=(b.summary||b.body||"No summary provided.").trim(),L=(b.content||"").trim(),h=(b.category||"architecture").trim(),c=(b.affected_services||[]).map(H=>H.trim()),v=(b.implementation_path||[]).map(H=>H.trim()),o=b.source_event_ids||[],m=b.approved===!0,y=new Date(f.timestamp*1e3),C=y.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=y.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=re.has(f.id),T=$?"display: block;":"display: none;",k=document.createElement("div");k.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${m?"blueprint-approved":""}`,k.dataset.blueprintId=f.id,m&&(k.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",k.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=m?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";k.onclick=function(H){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(f.id);let z=this.querySelector(".event-details");z&&(z.style.display="none")}else{this.classList.add("expanded"),re.add(f.id);let z=this.querySelector(".event-details");z&&(z.style.display="block")}};let D="";v.length>0&&(D=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(H=>`<li style="margin-bottom: 5px;">${M(H)}</li>`).join("")}</ul></div>
                    </div>
                `);let P="";o.length>0&&(P=`
            ...
            `;k.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${T}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${M(u)}</div>
            ...
            `;let E=k.querySelector(".blueprint-approve-btn");E&&(E.onclick=async H=>{H.stopPropagation(),E.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${f.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&le(!0)}catch(R){console.error("Failed to approve blueprint:",R)}});let A=k.querySelector(".blueprint-delete-btn");A&&(A.onclick=async H=>{H.stopPropagation();let R=!m;A.innerHTML=R?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${f.id}`,{method:"DELETE"})).ok&&le(!0)}catch(z){console.error("Failed to delete blueprint:",z)}});let q=k.querySelector(".event-details");q&&q.addEventListener("click",H=>{H.stopPropagation()});let F=k.querySelector(".close-details-btn");return F&&F.addEventListener("click",H=>{H.stopPropagation(),k.classList.remove("expanded");let R=k.querySelector(".event-details");R&&(R.style.display="none"),re.delete(f.id)}),k},p=Array.from(e.children),d=new Map(p.map(f=>[f.dataset.blueprintId,f])),g=new Set(i.map(f=>f.id));p.forEach(f=>{let b=f.dataset.blueprintId;(!b||!g.has(b))&&f.remove()});let x=null;i.forEach((f,b)=>{let n=d.get(f.id);(!n||t)&&(n&&n.remove(),n=l(f),!n)||(b===0?e.firstElementChild!==n&&e.prepend(n):x&&x.nextElementSibling!==n&&x.after(n),x=n)}),pe(2,i.length),Qe()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=B("offline","Failed to load blueprints.","The event service may be offline."))}}async function ct(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let a=(await e.json()).events||[],s=0;a.forEach(i=>{let l=i.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&s++}),Xe(s)}catch{}}function Ut(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{lt.forEach(r=>re.add(r.id)),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{re.clear(),le(!0)},e.dataset.listenerAttached="true")}var dt=()=>`
...
`;async function Ie(){await Promise.all([ee(),le()])}var pt=()=>`
...
`,Ee=null,te=new Set,ue=[],O="all",Ft={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision"],moderation:["moderation.explicit_content.deleted"]},Gt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Wt(t){for(let[e,r]of Object.entries(Ft))if(r.includes(t))return e;return"system"}function Vt(t){return Gt[t]||"bx-square-rounded"}async function se(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Jt();let a=`/events?ml=${O==="all"?100:250}&format=json`;O!=="all"&&(a+=`&category=${O}`);try{let s=await _(a);if(!s.ok)throw new Error("Service unreachable");if(ue=((await s.json()).events||[]).filter(b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ee=Date.now(),j(1,Ee),ue.length===0){e.innerHTML=B("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,L=Wt(u),h=Vt(u),c=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),v="event-border-grey";c&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?v="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"?v="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?v="event-border-orange":u==="system.test.completed"?v=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let o=c?"cursor-pointer":"",m=te.has(b.id),y=m?"expanded":"",C=m?"display: block;":"display: none;",w=new Date(b.timestamp*1e3),$=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=mt(u,n),N=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",U="";if(c){let I="";if(u==="engagement.decision"){let S=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;I=`
                    ...
                    `}else if(u==="system.attention.expired"){let S=q=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${q}</h5>`,E=n.timestamp-n.last_active,A=Math.floor(E/60);I=`
                    ...
                    `}else if(u==="analysis.visual.completed"){let S=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,E="";n.base64_preview?E=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(E=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="analysis.router.decision"){let S=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;I=`
                        ...
                        <div class="event-detail-block">
                            ${S("Input Context")}
                            <pre class="detail-pre">${M(n.raw_input)||"None"}</pre>
                        </div>
                    ...
                    `}else if(u==="system.analysis.audit"){let S=n.success?"#03dac6":"#ff4d4d",E=n.success?"SUCCESS":"FAILED",A=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,q="";n.error&&(q=`
                            <div class="event-detail-block">
                                ${A("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${M(n.error)}</pre>
                            </div>
                        `);let F="";if(n.corrections&&n.corrections.length>0){let R=n.corrections.map((z,Ce)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${z.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${M(z.snippet)}</div>`:""}
                            </div>
                        `).join("");F=`
                            <div class="event-detail-block">
                                ${A(`Corrections (${n.corrections.length})`)}
                                ${R}
                            </div>
                        `}let H="";if(n.parsed_results&&n.parsed_results.length>0){let R=n.parsed_results.map(z=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${M(z.title)}</div>
                                <div style="color: #ddd;">${M(z.summary)}</div>
                            </div>
                        `).join("");H=`
                            <div class="event-detail-block">
                                ${A("Parsed Results")}
                        ...
                        <div class="event-detail-block">
                            ${A("Input Context")}
                            <pre class="detail-pre" style="max-height: 200px; overflow-y: auto; color: #fff;">${M(n.input_context)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${A("Raw Output")}
                            <pre class="detail-pre" style="max-height: 300px; overflow-y: auto; color: #fff;">${M(n.raw_output||"(empty)")}</pre>
                        </div>
                    ...
                    `}else if(u==="error_occurred"){let S=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;I=`
                        ...
                        <div class="event-detail-block">
                            ${S("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${M(n.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${S("Context")}
                    ...
                    `}else if(u==="messaging.user.sent_message"){let S="";n.attachments&&n.attachments.length>0&&(S=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(A=>{let q=A.content_type&&A.content_type.startsWith("image/"),F=(A.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${q?`<div class="attachment-thumb-container"><img src="${A.url}" alt="${A.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${A.url}" target="_blank" class="attachment-link">${A.filename}</a>
                                        <span class="attachment-meta">${A.content_type} \u2022 ${F}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ut()}
        </div>`;async function $t(){await Promise.all([$e(),Be(),De()]),await ge()}var gt=null,ft=null;async function St(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function bt(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Yt(){try{return await(await _("/processes")).json()}catch{return null}}async function Xt(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Be(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),r=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),l=document.querySelector("#hw-storage .hw-content"),p=c=>{if(c){if(r&&(r.innerHTML=`
                ...
                ${Object.values(W).map(s=>`
                    <div class="theme-card ${t===s?"active":""}" data-theme="${s}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${s===W.DARK?"Simple, clean, dark.":s===W.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Zt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[r,a]of Object.entries(t)){let s=Tt.filter(i=>i.category===r);s.length!==0&&(e+=`

(()=>{function Te(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ie(t=!1){let i=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}function Z(t){let e=null,i=t.onClose||null,o=t.onOpen||null;function l(){if(e){e.classList.add("open"),window.addEventListener("resize",v),o&&setTimeout(o,10);return}let a=document.getElementById("windows-container");a||(a=document.createElement("div"),a.id="windows-container",a.className="windows-container",document.body.appendChild(a)),e=document.createElement("div"),e.id=t.id,e.className="window";let n=t.icon||"bx-window",m="",r="",c;t.tabs&&t.tabs.length>0?(m=`<div class="tab-bar">${t.tabs.map((L,S)=>{let h;return L.icon?h=`<i class="bx ${L.icon}"></i>`:L.title&&L.title.length>0?h=`<span class="tab-glyph">${L.title.charAt(0).toUpperCase()}</span>`:h='<i class="bx bx-question-mark"></i>',`
                ...
                `}).join("")}</div>`,c=`<div class="window-content">${t.tabs.map((L,S)=>`<div class="tab-content ${S===0?"active":""}" data-tab-content="${S}">${L.content}</div>`).join("")}</div>`):(t.title&&(r=`<div class="window-title">${t.title}</div>`),c=`<div class="window-content">${t.content}</div>`);let s=`
    ...
    `}var he=null;async function ye(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");let e=localStorage.getItem("service_map");if(!e)return t.classList.add("placeholder-active"),t.innerHTML=R("config","No service map configured.","Please upload your service-map.json in Settings to enable log monitoring."),!1;let i;try{i=JSON.parse(e)}catch(u){return console.error("Error parsing service_map from localStorage:",u),t.classList.add("placeholder-active"),t.innerHTML=R("error","Invalid service map data.","Please re-upload a valid service-map.json file in Settings."),!1}let o=null;if(i&&typeof i.services=="object"){let u=["cs","be","th"];for(let d of u)if(Array.isArray(i.services[d])){let f=i.services[d].find(x=>x.short_name==="event"||x.id==="event"||x.id==="dex-event-service");if(f){o=f;break}}}if(!o)return t.classList.add("placeholder-active"),t.innerHTML=R("error","Event service not found in service map.","Please ensure dex-event-service is configured in your service-map.json."),!1;let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/logs`;try{let u=await fetch(v);if(!u.ok)return t.classList.add("placeholder-active"),t.innerHTML=R("offline","Event service is offline.","Please ensure the event service is running."),!1;let d=await u.json();if(!d||d.length===0)return t.classList.add("placeholder-active"),t.innerHTML=R("empty","No logs found.","Service logs will appear here when available."),!1;let f=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],x=d.filter(n=>!f.includes(n.id));x.forEach(n=>{n.logs&&Array.isArray(n.logs)?n.logs.reverse():n.logs=[]}),x.reverse();let a=x.map(n=>{let m=n.logs.join(`
`);return`
...
`,W=null,J=new Set,Ue=[];async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ze();let i=localStorage.getItem("service_map");if(!i){e.innerHTML=E("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{e.innerHTML=E("error","Invalid service map data.");return}if(!o){e.innerHTML=E("error","Event service not found in service map.");return}let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let u=await fetch(v);if(!u.ok)throw new Error("Service is offline or unreachable.");let f=(await u.json()).events||[];if(Ue=f,W=Date.now(),_(3,W),f.length===0){e.innerHTML=E("empty","No events found.");return}t&&(e.innerHTML="");let x=c=>{let s=c.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let p=s.type,y=p==="engagement.decision"||p==="messaging.bot.sent_message"||p==="messaging.user.sent_message"||p==="moderation.explicit_content.deleted"||p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.cli.command"||p==="system.analysis.audit",$="event-border-grey";y&&(p==="moderation.explicit_content.deleted"?$="event-border-red":p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.analysis.audit"?$="event-border-purple":p==="system.cli.command"?$="event-border-orange":$="event-border-blue");let L=y?"cursor-pointer":"",S=J.has(c.id),h=S?"expanded":"",g=S?"display: block;":"display: none;",b=new Date(c.timestamp*1e3),w=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),H=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=Be(p,s),M="";if(y){let T="";if(p==="engagement.decision")T=`
                ...
                `}let N=document.createElement("div");if(N.className=`event-item ${$} ${L} ${h}`,N.dataset.eventId=c.id,N.onclick=function(T){if(!y)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(c.id);let I=this.querySelector(".event-details");I&&(I.style.display="none")}else{this.classList.add("expanded"),J.add(c.id);let I=this.querySelector(".event-details");I&&(I.style.display="block")}},N.innerHTML=`
...
`,le=null,V=new Set,Pe=[];async function te(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;tt();let i=localStorage.getItem("service_map");if(!i){e.innerHTML=E("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{e.innerHTML=E("error","Invalid service map data.");return}if(!o){e.innerHTML=E("error","Event service not found in service map.");return}let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=1000&format=json&event.type=system.blueprint.generated`;try{let u=await fetch(v);if(!u.ok)throw new Error("Service is offline or unreachable.");let f=(await u.json()).events||[];if(Pe=f,le=Date.now(),_(1,le),f.length===0){e.innerHTML=E("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),P(1,0);return}t&&(e.innerHTML="");let x=c=>{let s=c.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let p=s.title||"Untitled Blueprint",y=s.summary||s.body||"No summary provided.",$=s.content||"",L=s.category||"architecture",S=s.affected_services||[],h=s.implementation_path||[],g=new Date(c.timestamp*1e3),b=g.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=g.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),H=V.has(c.id),A=H?"display: block;":"display: none;",M=document.createElement("div");M.className=`event-item notification-item event-border-purple cursor-pointer ${H?"expanded":""}`,M.dataset.blueprintId=c.id,M.onclick=function(I){if(this.classList.contains("expanded")){this.classList.remove("expanded"),V.delete(c.id);let U=this.querySelector(".event-details");U&&(U.style.display="none")}else{this.classList.add("expanded"),V.add(c.id);let U=this.querySelector(".event-details");U&&(U.style.display="block")}};let N=S.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${S.join(", ")}</div>`:"",T="";h.length>0&&(T=`
    ...
    `:E("config","No service map configured.","Upload service-map.json in Settings."),re=null,ce=null,de=null;async function Re(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/system_monitor`,l=await fetch(o);if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);return await l.json()}catch(t){return console.error("Error fetching system data:",t),null}}async function st(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/processes`,l=await fetch(o);if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);return await l.json()}catch(t){return console.error("Error fetching process data:",t),null}}async function nt(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/analyst/status`,l=await fetch(o);return l.ok?await l.json():null}catch{return null}}async function xe(){let t=document.getElementById("services-widgets");if(!t)return;let e=await Re();if(!e||!e.services){t.innerHTML=E("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}re=Date.now(),_(5,re);let i=e.services||[];Array.from(t.children).forEach(a=>{a.classList.contains("service-widget")||a.remove()});function o(a){return!a||a==="N/A"||a==="unknown"||a.trim()===""?"-":a}function l(a){if(!a||a==="N/A"||a==="unknown")return"-";let n=a.match(/^(\d+\.\d+\.\d+)/);return n?n[0]:a.split(".").slice(0,3).join(".")||"-"}function v(a){return!a||a.length<=28?a:a.substring(0,28)+"..."}function u(a){if(!a||a==="N/A"||a==="unknown")return"-";let n=a.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!n)return"-";let m=parseInt(n[1])||0,r=parseInt(n[2])||0,c=parseInt(n[3])||0,s=parseFloat(n[4])||0,p=m*86400+r*3600+c*60+s,y=Math.floor(p/86400);if(y>0)return`${y}d`;let $=Math.floor(p/3600);if($>0)return`${$}h`;let L=Math.floor(p/60);return L>0?`${L}m`:`${Math.floor(p)}s`}function d(a){let n=a.status==="online",m=n?"service-widget-online":"service-widget-offline",r=n?"bx-check-circle":"bx-x-circle",c=n?"OK":"BAD",s=a.version?l(a.version.str):"-",p=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",y=a.memory&&a.memory!=="N/A"?a.memory:"-",$=p!=="-"?"#00ff00":"#666",L=p!=="-"?"#fff":"#666",S=y!=="-"?"#00ff00":"#666",h=y!=="-"?"#fff":"#666",g=u(a.uptime),b="";return n?b=`
                ...
                </div>`:b='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${m}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${r}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${c}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${v(a.domain&&a.port?`${a.domain}:${a.port}`:"")}</span></div>${b}</div></div>`}let f=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),x=new Set(i.map(a=>a.id));for(let[a,n]of f)x.has(a)||n.remove();i.forEach(a=>{let n=d(a),m=f.get(a.id);m?m.outerHTML!==n&&(m.outerHTML=n):t.insertAdjacentHTML("beforeend",n)})}async function we(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Re();if(!e){t.innerHTML=E("offline","Failed to load model status.");return}ce=Date.now(),_(4,ce);let i=e.models||[],o=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function l(d){let f=d.status==="Ready";return`
...
`,z=new Set,Je=[],K=null;async function O(t=!1){let e=document.getElementById("roadmap-list");if(!e)return;it();let i=localStorage.getItem("service_map");if(!i)return;let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{return}if(!o)return;let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/roadmap`;try{let u=await fetch(v);if(!u.ok)throw new Error("Offline");let d=await u.json();if(Je=d,d.length===0){e.innerHTML=E("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let f=r=>{let c=z.has(r.id),s=r.state==="draft",p=r.state==="published",y=r.state==="consumed",$="event-border-grey";p&&($="event-border-blue"),y&&($="event-border-purple");let S=new Date(r.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${$} cursor-pointer ${c?"expanded":""}`,h.dataset.itemId=r.id,h.onclick=b=>{if(b.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",z.delete(r.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",z.add(r.id))};let g=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${r.state.toUpperCase()}]</span>`;return h.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${C(r.content)}</div>

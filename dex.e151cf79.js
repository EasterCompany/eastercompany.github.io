(()=>{function Ce(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function _e(t=!1){let n=`
        <div class="nav-left">
    ...
    `,o=document.createElement("nav");o.innerHTML=n,document.body.prepend(o)}function Ae(){let t=`
...
`,G=null,O=new Set,ae=[];async function q(t=!1){let e=document.getElementById("notifications-list");if(!e)return;ot(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await $(n);if(!o.ok)throw new Error("Service is offline or unreachable.");let d=(await o.json()).events||[];G=Date.now(),C(0,G);let g=Date.now(),v=24*60*60*1e3,p=d.filter(c=>{let u=localStorage.getItem(`notification_read_ts_${c.id}`);if(!u)return!0;let y=parseInt(u);return g-y<v});p.sort((c,u)=>{let y=D=>{let _=D.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},A=D=>D==="critical"?4:D==="high"?3:D==="medium"?2:1,k=A(y(c)),L=A(y(u));return k!==L?L-k:u.timestamp-c.timestamp}),ae=p;let b=c=>{let u=c.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},m=[],r=new Set(p.map(c=>b(c))).size>1;if(p.length>0){let c=null;p.forEach(u=>{let y=b(u);r&&y!==c&&(m.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),c=y),m.push(u)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=S("empty","No notifications yet."),ie();return}let s=c=>{let u=c.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let y=u.title||"Untitled Notification",A=u.body||"No description provided.",k=u.priority||"low",L=!!u.alert,D=u.category||"system",_=u.related_event_ids||[],M=!!localStorage.getItem(`notification_read_ts_${c.id}`),N=new Date(c.timestamp*1e3),E=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),z=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=M?"event-border-grey":"event-border-blue";!M&&L&&(I="event-border-red"),M&&(k==="high"||k==="critical")?I="event-border-red":M&&k==="medium"&&(I="event-border-orange");let H=M?"notification-read":"notification-unread",te=O.has(c.id),pe=te?"expanded":"",st=te?"display: block;":"display: none;",Te="",ke="";_.length>0&&(ke=`
                ...
                ${ke}
            `;let P=document.createElement("div");P.className=`event-item notification-item ${I} ${H} ${pe} cursor-pointer`,P.dataset.notificationId=c.id,P.onclick=function(Z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),O.delete(c.id);let Q=this.querySelector(".event-details");Q&&(Q.style.display="none")}else{this.classList.add("expanded"),O.add(c.id);let Q=this.querySelector(".event-details");if(Q&&(Q.style.display="block"),!localStorage.getItem(`notification_read_ts_${c.id}`)){localStorage.setItem(`notification_read_ts_${c.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ue="event-border-grey";k==="high"||k==="critical"?ue="event-border-red":k==="medium"&&(ue="event-border-orange"),this.classList.add(ue),ie()}}},P.innerHTML=`
...
`,Oe=null,K=new Set,je=[];async function oe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;pt();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await $(n);if(!o.ok)throw new Error("Service is offline or unreachable.");let d=(await o.json()).events||[];if(je=d,Oe=Date.now(),C(2,Oe),d.length===0){e.innerHTML=S("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),U(1,0);return}t&&(e.innerHTML="");let g=f=>{let r=f.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let s=r.title||"Untitled Blueprint",i=r.summary||r.body||"No summary provided.",l=r.content||"",x=r.category||"architecture",h=r.affected_services||[],w=r.implementation_path||[],c=new Date(f.timestamp*1e3),u=c.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=c.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=K.has(f.id),k=A?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${A?"expanded":""}`,L.dataset.blueprintId=f.id,L.onclick=function(M){if(this.classList.contains("expanded")){this.classList.remove("expanded"),K.delete(f.id);let E=this.querySelector(".event-details");E&&(E.style.display="none")}else{this.classList.add("expanded"),K.add(f.id);let E=this.querySelector(".event-details");E&&(E.style.display="block")}};let D=h.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${h.join(", ")}</div>`:"",_="";w.length>0&&(_=`
...
`,J=null,Y=new Set,ee=[],le="all",ut={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},ft={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function We(t){for(let[e,n]of Object.entries(ut))if(n.includes(t))return e;return"system"}function gt(t){return ft[t]||"bx-square-rounded"}async function X(t=!1){let e=document.getElementById("events-timeline");if(!e)return;vt();let o=`/events?ml=${le==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let a=await $(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let g=(await a.json()).events||[],v=g;if(le!=="all"&&(v=g.filter(s=>{let i=s.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!1}return We(i.type)===le})),ee=v.slice(0,50),J=Date.now(),C(1,J),ee.length===0){e.innerHTML=S("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=s=>{let i=s.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let l=i.type,x=We(l),h=gt(l),w=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),c="event-border-grey";w&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?c="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?c="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?c="event-border-orange":l==="system.test.completed"?c=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":c="event-border-blue");let u=w?"cursor-pointer":"",y=Y.has(s.id),A=y?"expanded":"",k=y?"display: block;":"display: none;",L=new Date(s.timestamp*1e3),D=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),F=Fe(l,i),M="";if(w){let E="";if(l==="engagement.decision")E=`
                    ...
                    `;else if(l==="messaging.user.sent_message"){let z="";i.attachments&&i.attachments.length>0&&(z=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${i.attachments.map(H=>{let te=H.content_type&&H.content_type.startsWith("image/"),pe=(H.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${H.content_type} \u2022 ${pe}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),E=`
                        <div class="event-detail-row">
            ...
            `,w){let E=N.querySelector(".close-details-btn");E&&E.addEventListener("click",z=>{z.stopPropagation();let I=z.target.closest(".event-item");if(I){I.classList.remove("expanded"),Y.delete(s.id);let H=I.querySelector(".event-details");H&&(H.style.display="none")}})}return N},b=Array.from(e.children),m=new Map(b.map(s=>[s.dataset.eventId,s])),f=new Set(ee.map(s=>s.id));b.forEach(s=>{let i=s.dataset.eventId;(!i||!f.has(i))&&s.remove()});let r=null;ee.forEach((s,i)=>{let l=m.get(s.id);(!l||t)&&(l&&l.remove(),l=p(s),!l)||(i===0?e.firstElementChild!==l&&e.prepend(l):r&&r.nextElementSibling!==l&&r.after(l),r=l)}),J=Date.now(),C(1,J)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=S("offline","Failed to load events.","The event service may be offline or unreachable."))}}function vt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),n=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ee.forEach(o=>Y.add(o.id)),X(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),X(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.querySelectorAll(".filter-btn").forEach(o=>{o.onclick=()=>{n.querySelectorAll(".filter-btn").forEach(a=>a.classList.remove("active")),o.classList.add("active"),le=o.dataset.filter,X(!0)}}),n.dataset.listenerAttached="true")}function Ve(){return`
        <div id="logs-container" class="logs-container">
            <p>Loading logs...</p>
        </div>
    `}var bt=null;async function he(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await $("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML='<p style="text-align: center; opacity: 0.5; padding: 20px;">No logs found.</p>',!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],a=n.filter(g=>!o.includes(g.id));a.forEach(g=>{g.logs&&Array.isArray(g.logs)?g.logs.reverse():g.logs=[]}),a.reverse();let d=a.map(g=>{let v=g.logs.join(`
...
`);return`
    ...
    `;for(let[n,o]of Object.entries(t)){let a=Qe.filter(d=>d.category===n);a.length!==0&&(e+=`
...
`).forEach(m=>{if(m.trim()==="")return;let f="output";m.includes("[ERROR]")?f="error":m.includes("[SUCCESS]")||m.includes("\u2713")?f="success":m.includes("!")&&(f="warning"),$e(n,m,f)})}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed"}catch(o){$e(n,`Connection Error: ${o.message}`,"error"),document.getElementById("terminal-status-badge").className="terminal-status status-error",document.getElementById("terminal-status-badge").textContent="Failed"}finally{document.getElementById("terminal-action-btn").style.display="block"}}function et(){let t=document.getElementById("cli-interface-container");t&&(t.innerHTML=Et(),t.querySelectorAll(".cli-command-card").forEach(e=>{e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-5px)",e.style.borderColor="rgba(255,255,255,0.15)",e.style.backgroundColor="rgba(255,255,255,0.05)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)",e.style.borderColor="rgba(255,255,255,0.05)",e.style.backgroundColor="rgba(255,255,255,0.03)"}),e.addEventListener("click",()=>{let n=e.dataset.cmd;if(n==="chat"){alert("Connection to cognitive core pending deployment...");return}Lt(n)})}))}async function Tt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${be()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function tt(){Tt(),He(),Ce(),window.location.pathname.includes("/dex")&&et();let t=Ie();_e(t),Ae();let e=document.querySelector("footer"),n=null;function o(){n=null,e?.classList.remove("hide"),document.querySelectorAll(".nav-right i").forEach(p=>p.classList.remove("active","inactive"))}function a(p,b=null,m=null){let f=n&&n.id===p.id;if(f&&m!==null){let r=document.querySelector(`#${p.id} .tab[data-tab-index="${m}"]`);r&&r.click(),document.querySelectorAll(".nav-right i").forEach(s=>{let i=s===b;s.classList.toggle("active",i),s.classList.toggle("inactive",!i&&b)});return}n&&n.close(!f),f?n=null:setTimeout(()=>{if(p.open(),n=p,m!==null){let r=document.querySelector(`#${p.id} .tab[data-tab-index="${m}"]`);r&&r.click()}document.querySelectorAll(".nav-right i").forEach(r=>{let s=r===b;r.classList.toggle("active",s),r.classList.toggle("inactive",!s&&b)}),e?.classList.add("hide")},n?220:0)}async function d(){await Promise.all([q(),X(),ye(),xe()]);let p=setInterval(()=>{if(!v.isOpen())return clearInterval(p);C(0,G),C(1,J),C(3,de),C(3,re),C(3,ce)},1e3),b=setInterval(()=>{if(!v.isOpen())return clearInterval(b);q(),X(),ye(),xe()},3e3)}let g=fe({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="https://github.com/eastercompany" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in",onClose:o}),v=fe({id:"main-window",tabs:[{icon:"bx-bell",title:"Notifications",content:ze(),"data-tab-index":0},{icon:"bx-calendar-event",title:"Events",content:Ge(),"data-tab-index":1},{icon:"bx-bulb",title:"Ideas",content:Re(),"data-tab-index":2},{icon:"bx-server",title:"System",content:Je(),"data-tab-index":3},{icon:"bx-cog",title:"Settings",content:we(),"data-tab-index":4}],icon:"bx-layer",onClose:o,onOpen:()=>{d(),setTimeout(()=>Ee(v),50)}});t?(document.getElementById("notif-icon")?.addEventListener("click",p=>a(v,p.currentTarget,0)),document.getElementById("events-icon")?.addEventListener("click",p=>a(v,p.currentTarget,1)),document.getElementById("ideas-icon")?.addEventListener("click",p=>a(v,p.currentTarget,2)),document.getElementById("system-icon")?.addEventListener("click",p=>a(v,p.currentTarget,3)),document.getElementById("settings-icon")?.addEventListener("click",p=>a(v,p.currentTarget,4))):document.getElementById("login-btn")?.addEventListener("click",()=>{a(g),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",p=>{p.preventDefault();try{Be(document.getElementById("email-input").value),window.location.reload()}catch(b){let m=document.getElementById("login-error");m&&(m.textContent=b.message,m.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",tt):tt();})();
//# sourceMappingURL=dex.e151cf79.js.map

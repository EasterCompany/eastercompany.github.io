(()=>{function Ze(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ke(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
...
`,Me=null,re=new Set,Ie=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Yt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await I(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];Me=Date.now(),Z(0,Me);let o=Date.now(),b=24*60*60*1e3,c=r.filter(f=>{let v=localStorage.getItem(`alert_read_ts_${f.id}`);if(!v)return!0;let p=parseInt(v);return o-p<b});c.sort((f,v)=>{let p=B=>{let h=B.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},E=B=>B==="critical"?4:B==="high"?3:B==="medium"?2:1,L=E(p(f)),R=E(p(v));return L!==R?R-L:v.timestamp-f.timestamp}),Ie=c;let d=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},y=[],g=new Set(c.map(f=>d(f))).size>1;if(c.length>0){let f=null;c.forEach(v=>{let p=d(v);g&&p!==f&&(y.push({id:`divider-${p}`,type:"divider",label:p.toUpperCase()}),f=p),y.push(v)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),Le();return}let n=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let p=(v.title||"Untitled Alert").trim(),E=(v.body||"No description provided.").trim(),L=v.summary||"",R=v.content||"",B=v.protocol||"",h=(v.priority||"low").toLowerCase(),H=!!v.alert,D=(v.category||"system").trim(),M=v.related_event_ids||[],U=v.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${f.id}`),S=new Date(f.timestamp*1e3),w=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=j?"event-border-grey":"event-border-blue";!j&&H&&(k="event-border-red"),j&&(h==="high"||h==="critical")?k="event-border-red":j&&h==="medium"&&(k="event-border-orange");let T=j?"alert-read":"alert-unread",q=re.has(f.id),F=q?"expanded":"",te=q?"display: block;":"display: none;",J="",G="";M.length>0&&(G=`
            ...
            <div style="flex: 1; min-width: 100px; text-align: center;">
                ...
                <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #bb86fc; font-weight: bold;">${B}</div>
            </div>`);let Y="";R?Y=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(R)}</div>
            </div>
        `:Y=`
            <div class="event-detail-block" style="text-align: left;">
...
`,le=new Set,_e=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){Xt();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let i=await a.json();_e=i;let s=d=>{let y=le.has(d.id),m=d.state==="published",g=d.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),g&&(n="event-border-purple");let A=new Date(d.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),$=document.createElement("div");$.className=`event-item notification-item ${n} cursor-pointer ${y?"expanded":""}`,$.dataset.itemId=d.id,$.onclick=f=>{if(f.target.closest("button"))return;$.classList.contains("expanded")?($.classList.remove("expanded"),$.querySelector(".event-details").style.display="none",le.delete(d.id)):($.classList.add("expanded"),$.querySelector(".event-details").style.display="block",le.add(d.id))};let l=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${d.state.toUpperCase()}]</span>`;$.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-details" style="${y?"display: block;":"display: none;"} ">
            ...
            `}
            ${g?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(d.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `;let x=$.querySelector(".event-details");return x&&x.addEventListener("click",f=>{f.stopPropagation()}),$.querySelector(".edit-btn")?.addEventListener("click",()=>Qt(d)),$.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>es(d)),$.querySelector(".delete-btn")?.addEventListener("click",()=>ts(d.id)),$.querySelector(".close-details-btn")?.addEventListener("click",f=>{f.stopPropagation(),$.classList.remove("expanded"),$.querySelector(".event-details").style.display="none",le.delete(d.id)}),$},r=Array.from(e.children),o=new Map(r.map(d=>[d.dataset.itemId,d])),b=new Set(i.map(d=>d.id));if(r.forEach(d=>{let y=d.dataset.itemId;(!y||!b.has(y))&&d.remove()}),!_e||_e.length===0){e.innerHTML=N("empty","Your roadmap is empty.",_()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((d,y)=>{let m=o.get(d.id);(!m||t)&&(m&&m.remove(),m=s(d),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function Xt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let o=xe?`/roadmap/${xe}`:"/roadmap",b=xe?"PATCH":"POST";try{await I(o,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(c){console.error(c)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{_e.forEach(r=>le.add(r.id)),ce(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{le.clear(),ce(!0)},s.dataset.listenerAttached="true")}function Qt(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function es(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(a){console.error(a)}}async function ts(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,pt=null,de=new Set,ft=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ss();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await I(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];if(ft=r,pt=Date.now(),Z(2,pt),r.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=m=>{let g=m.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let n=(g.title||"Untitled Blueprint").trim(),u=(g.summary||g.body||"No summary provided.").trim(),A=(g.content||"").trim(),$=(g.category||"architecture").trim(),l=(g.related_services||g.affected_services||[]).map(T=>T.trim()),x=(g.implementation_path||[]).map(T=>T.trim()),f=g.source_event_ids||[],v=g.approved===!0,p=new Date(m.timestamp*1e3),E=p.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=p.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),R=de.has(m.id),B=R?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${R?"expanded":""} ${v?"blueprint-approved":""}`,h.dataset.blueprintId=m.id,v&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let D=v?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[$]||"bx-paint";h.onclick=function(T){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let F=this.querySelector(".event-details");F&&(F.style.display="block")}};let M="";x.length>0&&(M=`
            ...
            `;h.innerHTML=`
                ...
                <div class="event-icon" style="${v?"color: #03dac6;":""}"><i class='bx ${D}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${B}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${z(u)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${z(A)}</div>
                        </div>
                        ${M}
                        ${U}
                        ${j}
...
`;async function Re(){await Promise.all([ce(),pe()])}var vt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
                ...
                <div class="user-profile-card" 
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${d}; font-weight: 600; text-transform: uppercase;">${y?"DEXTER (ME)":o.level}</span>
                        </div>
...
`,Be=null,me=new Set,Ce=[],W="all",as={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},os={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function rs(t){for(let[e,a]of Object.entries(as))if(a.includes(t))return e;return"system"}function ls(t){return os[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;cs();let i=`/events?ml=${W==="all"?100:250}&format=json`;W!=="all"&&(i+=`&category=${W}`);try{let s=await I(i);if(!s.ok)throw new Error("Service unreachable");if(Ce=((await s.json()).events||[]).filter(g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Be=Date.now(),Z(1,Be),Ce.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,A=rs(u),$=ls(u),l=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),x="event-border-grey";l&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?x="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"?x="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?x="event-border-orange":u==="system.test.completed"?x=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":x="event-border-blue");let f=l?"cursor-pointer":"",v=me.has(g.id),p=v?"expanded":"",E=v?"display: block;":"display: none;",L=new Date(g.timestamp*1e3),R=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),B=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=xt(u,n),H=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",D="";if(l){let S="";if(u==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let w=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,C="";n.eval_count&&(C=`
                        ...
                        `);let k="";if(n.chat_history&&n.chat_history.length>0){let T=n.chat_history.length,q=n.chat_history.map((F,te)=>{let J=F.role.toUpperCase();J==="ASSISTANT"&&(J="DEXTER");let G=F.role==="user"?"#03dac6":F.role==="system"?"#ffb74d":"#bb86fc",K=te===T-1?"block":"none";return`
                    ...
                    `}else if(u==="messaging.user.sent_message"){let w="";n.attachments&&n.attachments.length>0&&(w=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(k=>{let T=k.content_type&&k.content_type.startsWith("image/"),q=(k.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${k.content_type} \u2022 ${q}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
...
`),c=[...o.logs];if(c.length<25){let y=25-c.length;for(let m=0;m<y;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let d=c.map(y=>Ae(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=r,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let b=unescape(o.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let c=o.querySelector("i");c.classList.remove("bx-copy"),c.classList.add("bx-check"),setTimeout(()=>{c.classList.remove("bx-check"),c.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let b=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await I(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&$e()}catch(c){console.error("Error clearing logs:",c)}})}),ps=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Et=()=>{let t=_()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ht()}
        </div>`;async function Mt(){await Promise.all([Ee(),qe(),je()]),await $e()}var wt=null,Ct=null;async function It(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function $t(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function ms(){try{return await(await I("/processes")).json()}catch{return null}}async function us(){try{return await(await I("/agent/status")).json()}catch{return null}}async function qe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),a=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),r=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),b=l=>{if(l){if(a&&(a.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&l.STORAGE&&l.STORAGE.length>0?o.innerHTML=l.STORAGE.map(x=>{let f=(x.USED/1073741824).toFixed(1),v=(x.SIZE/(1024*1024*1024)).toFixed(1),p=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,E=x.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${p>90?"#cf6679":"#03dac6"}; width: ${p}%; height: 100%; box-shadow: 0 0 10px ${p>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let l=await $t();l?(b(l),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let l=await $t();b(l)}if(!t)return;let c=await It();if(!c||!c.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}wt=Date.now(),Z(0,wt);let d=c.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:l.split(".").slice(0,3).join(".")||"-"}function g(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let f=parseInt(x[1])||0,v=parseInt(x[2])||0,p=parseInt(x[3])||0,E=parseFloat(x[4])||0,L=f*86400+v*3600+p*60+E,R=Math.floor(L/86400);if(R>0)return`${R}d`;let B=Math.floor(L/3600);if(B>0)return`${B}h`;let h=Math.floor(L/60);return h>0?`${h}m`:`${Math.floor(L)}s`}function u(l){let x=l.status==="online",f=x?"service-widget-online":"service-widget-offline",v=x?"bx-check-circle":"bx-x-circle",p=x?"OK":"BAD",E=l.version?m(l.version.str):"-",L=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",R=l.memory&&l.memory!=="N/A"?l.memory:"-",B=L!=="-"?"#00ff00":"#666",h=L!=="-"?"#fff":"#666",H=R!=="-"?"#00ff00":"#666",D=R!=="-"?"#fff":"#666",M=n(l.uptime),U="";x?U=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${D};">${R}</span>
                    </div>
                </div>`:U='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let O=_()?"easter.company":g(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${v}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${U}</div></div>`}let A=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),$=new Set(d.map(l=>l.id));for(let[l,x]of A)$.has(l)||x.remove();d.forEach(l=>{let x=u(l),f=A.get(l.id);f?f.outerHTML!==x&&(f.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function je(){let t=document.getElementById("models-widgets");if(!t)return;let e=await It();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Ct=Date.now(),Z(2,Ct);let a=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let d=c.status==="Ready";return`
                ...
                </div>`}function o(c){let d=c.status==="Downloaded",y=d?"service-widget-online":"service-widget-offline",m=d?"OK":"MISSING",g=d&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",n=c.name;return c.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${d?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let b="";if(i&&(b+=s(i)),e.xtts&&(b+=r(e.xtts)),b+=a.map(o).join(""),!b){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==b&&(t.innerHTML=b)}async function Ee(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),a=document.getElementById("guardian-architect-val"),i=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),r=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),b=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn");b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{b.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{b.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{b.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),Ee()}catch{b.innerHTML="<i class='bx bx-error'></i> Failed"}},b.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),Ee()}catch{c.innerHTML="<i class='bx bx-error'></i> Failed"}},c.dataset.listenerAttached="true");let d=await us();if(d){let $=Math.floor(Date.now()/1e3),l=d.active_tier,x=d.active_synthesis,f=d.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},v=h=>{h<0&&(h=0);let H=Math.floor(h/3600),D=Math.floor(h%3600/60),M=h%60;return H>0?`${H}h ${D}m`:D>0?`${D}m ${M}s`:`${M}s`},p=(h,H,D,M,U)=>{if(!h)return;let O=f[M]||M.toUpperCase(),j=h.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(j&&(j.textContent=O),U&&U.includes(M))h.textContent="Working",h.style.color="#bb86fc";else if(D){let w=D.next_run-$;if(w<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let C=Math.floor(w/60),k=w%60;h.textContent=`${C}m ${k}s`,h.style.color="#fff"}}H&&D&&(H.innerHTML=`
        ...
        `)};e&&p(e,document.getElementById("guardian-sentry-stats"),d.sentry,"sentry",l),a&&p(a,document.getElementById("guardian-architect-stats"),d.architect,"architect",l);let E=document.getElementById("analyzer-synthesis-val"),L=document.getElementById("analyzer-synthesis-stats");E&&p(E,L,d.synthesis,"synthesis",x);let R=document.getElementById("system-state-label"),B=document.getElementById("system-state-val");if(B&&d.system_state){let h=d.system_state,H=v(d.system_state_time||0);R&&(R.textContent=`State: ${h.toUpperCase()}`),B.textContent=H,h==="idle"?B.style.color=d.system_state_time>300?"#5eff5e":"#fff":B.style.color="#bb86fc"}s&&(s.textContent=v(d.total_idle_time||0)),r&&(r.textContent=v(d.total_active_time||0)),o&&(o.textContent=v(d.total_waste_time||0))}else[e,a,i,s,r,o].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});let y=await ms(),m=[],g=[],n=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],g=y.queue||[],n=y.history||[],n.sort(($,l)=>(l.end_time||0)-($.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Ue(t,m,!1));let u=document.getElementById("processes-queue-widgets");u&&(g.length===0?!u.querySelector(".tab-placeholder")&&!u.querySelector('div[style*="font-style: italic"]')&&(u.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(u.innerHTML="",Ue(u,g,!1)));let A=document.getElementById("processes-history-widgets");A&&(n.length===0?A.querySelector(".tab-placeholder")||(A.innerHTML=N("empty","No recent history.")):(A.querySelector(".tab-placeholder")&&(A.innerHTML=""),Ue(A,n,!0))),we(1,m.length)}function Ue(t,e,a){function i(b){let c="";b.end_time?c=`${b.end_time-b.start_time}s`:c=`${Math.floor(Date.now()/1e3-b.start_time)}s`;let d=b.retries>0?`<span class="process-retry-badge">Retry ${b.retries}</span>`:"",y=b.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[y]?y=m[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],gs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Je=!0,a}function _t(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Je=!1}function Bt(t,e,a="output"){if(!Je)return;let i=document.createElement("div");i.className=`terminal-line terminal-${a}`,a==="prompt"?i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:i.innerHTML=Ae(e),t.appendChild(i),t.scrollTop=t.scrollHeight}async function vs(t){let e=Dt.find(s=>s.id===t);if(!e)return;let a=bs(e);Bt(a,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let s of i){await new Promise(o=>setTimeout(o,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),Bt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function zt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=gs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let i=e.querySelector("i");i.className="bx bx-check",i.style.color="#5eff5e",setTimeout(()=>{i.className="bx bx-copy",i.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let i=a.dataset.cmd;vs(i)})})}async function xs(){try{if(!(await I("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ht(){xs(),tt(),Ze();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&zt(),Ke(),Ye();let e=document.getElementById("nav-left-container");e&&e.addEventListener("click",()=>{let p=document.getElementById("dexter-dropdown"),E=document.getElementById("dexter-menu-btn");if(p&&p.classList.contains("active")){p.classList.remove("active"),E.classList.remove("active"),r?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(s.length>0)y();else{let L=window.location.pathname;if(!(L==="/"||L==="/index.html")){let h=(L.endsWith("/")&&L.length>1?L.slice(0,-1):L).split("/");h.pop();let H=h.join("/")||"/";window.location.href=H}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",i=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!i&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let o=p=>{localStorage.setItem("dex_last_window",p)};function b(){return 1}function c(){for(;s.length>1;)s.shift().close(!0);let p=document.getElementById("windows-container"),E=document.querySelector("nav"),L=document.querySelector("footer"),R=window.location.pathname==="/"||window.location.pathname==="/index.html",B=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");p&&p.setAttribute("data-count",s.length),s.forEach(O=>{let j=document.getElementById(O.id);j&&j.classList.remove("hide-close")}),Se(he());let h=document.getElementById("dexter-menu-container"),H=document.getElementById("nav-window-switcher"),D=document.getElementById("settings-icon"),M=document.getElementById("close-all-windows"),U=window.innerWidth<880;if(fe(s.length>0),s.length>0)if(L?.classList.add("hide"),M&&(M.style.display=U?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),E?.classList.add("window-open"),p&&(p.style.paddingTop="60px"),h&&(h.style.display=U?"flex":"none"),D&&(D.style.display=U?"block":"none"),!U&&H){let O=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(O)?(H.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{o("alerts-window"),d(m)}),document.getElementById("switch-events").addEventListener("click",()=>{o("events-window"),d(g)}),document.getElementById("switch-monitor").addEventListener("click",()=>{o("monitor-window"),d(u)}),document.getElementById("switch-contacts").addEventListener("click",()=>{o("contacts-window"),d(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{o("workspace-window"),d(A)})):H.innerHTML=""}else H&&(H.innerHTML="");else E?.classList.remove("window-open"),M&&(M.style.display="none"),p&&(p.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),R||B?L?.classList.remove("hide"):L?.classList.add("hide"),h&&(h.style.display="flex"),D&&(D.style.display="block"),H&&(H.innerHTML="");ve()}function d(p){if(p.isOpen()){p.close();return}for(;s.length>0;)s.pop().close(!0);s.push(p),p.open(),c()}function y(){[...s].forEach(E=>E.close()),s=[]}window.addEventListener("resize",c);let m=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:lt(),onOpen:()=>ne(),onClose:()=>{let p=s.indexOf(m);p>-1&&s.splice(p,1),c()}}),g=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Oe(),onOpen:()=>{g.setContent(Oe()),ie()},onClose:()=>{let p=s.indexOf(g);p>-1&&s.splice(p,1),c()}}),n=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:vt(),onOpen:()=>Pe(),onClose:()=>{let p=s.indexOf(n);p>-1&&s.splice(p,1),c()}}),u=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Tt()},{icon:"bxs-component",title:"Processes",content:St()},{icon:"bxs-brain",title:"Models",content:kt()},{icon:"bxs-hdd",title:"Hardware",content:Lt()},{icon:"bxs-terminal",title:"Logs",content:At()},{icon:"bxs-zap",title:"Agents",content:Et()}].filter(p=>_()?p.title!=="Hardware"&&p.title!=="Logs":!0),onOpen:()=>{qe(),Ee(),je(),$e()},onClose:()=>{let p=s.indexOf(u);p>-1&&s.splice(p,1),c()}}),A=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:bt(),onOpen:()=>Re(),onClose:()=>{let p=s.indexOf(A);p>-1&&s.splice(p,1),c()}}),$=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{$.setContent(De()),Fe($)},onClose:()=>{let p=s.indexOf($);p>-1&&s.splice(p,1),c()}});window.dexter={viewEvent:p=>{g.isOpen()||d(g),setTimeout(()=>{let E=document.querySelector(`.event-item[data-event-id="${p}"]`);E&&(E.scrollIntoView({behavior:"smooth",block:"center"}),E.classList.add("flash-highlight"),E.classList.contains("expanded")||E.click(),setTimeout(()=>E.classList.remove("flash-highlight"),2e3))},500)},viewAlert:p=>{m.isOpen()||d(m),setTimeout(()=>{let E=document.querySelector(`.event-item[data-alert-id="${p}"]`);E&&(E.scrollIntoView({behavior:"smooth",block:"center"}),E.classList.add("flash-highlight"),E.classList.contains("expanded")||E.click(),setTimeout(()=>E.classList.remove("flash-highlight"),2e3))},500)}};let l=document.getElementById("dexter-dropdown");l&&(l.innerHTML=`
          ...
          <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
      `);let x=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");x&&l&&f&&(f.addEventListener("click",p=>{p.stopPropagation();let E=window.innerWidth<880;l.classList.toggle("active"),f.classList.toggle("active");let L=l.classList.contains("active");E&&(L?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),navbar?.classList.add("window-open"),r?.classList.add("menu-open"),fe(!0)):(r?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),navbar?.classList.remove("window-open"),fe(!1))))}),document.addEventListener("click",()=>{let p=window.innerWidth<880,E=l.classList.contains("active");l.classList.remove("active"),f.classList.remove("active"),p&&E&&(r?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),navbar?.classList.remove("window-open"),fe(!1)))}),l.addEventListener("click",p=>{p.stopPropagation()}));let v=()=>{l&&f&&(l.classList.remove("active"),f.classList.remove("active"))};document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{v(),o("alerts-window"),d(m)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{v(),o("events-window"),d(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{v(),o("monitor-window"),d(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{v(),o("contacts-window"),d(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{v(),o("workspace-window"),d(A)}),document.getElementById("settings-icon")?.addEventListener("click",()=>d($)),document.getElementById("close-all-windows")?.addEventListener("click",()=>y()),_()||setInterval(()=>{g.isOpen()&&ie(),m.isOpen()?ne():Ne()},1e3),setInterval(()=>{_()&&(m.isOpen()?ne():Ne()),A.isOpen()?Re():gt(),_()&&g.isOpen()&&ie(),u.isOpen()&&Mt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ht):Ht();})();

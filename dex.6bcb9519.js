(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function We(t=!1){let e=window.location.pathname,i=e==="/"||e==="/index.html",o=`
    ...
    `}
        ...
        `}
    `,l=document.createElement("nav");l.innerHTML=o,document.body.prepend(l)}function Ze(t){let e=window.location.pathname,i=e==="/"||e==="/index.html",a=document.getElementById("nav-chevron"),n=document.querySelector(".nav-left");!a||!n||(t||!i?(a.style.display="block",n.style.cursor="pointer",n.classList.add("recording"),n.onmouseenter=()=>{a.style.transform="translateX(-3px)"},n.onmouseleave=()=>{a.style.transform="translateX(0)"}):(a.style.display="none",n.style.cursor="default",n.classList.remove("recording"),n.onmouseenter=null,n.onmouseleave=null))}function Ke(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ye=1e3;function te(t){let e=null,i=t.onClose||null,a=t.onOpen||null;function n(){e&&(e.style.zIndex=++Ye)}function o(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),a&&setTimeout(a,10);return}let f=document.getElementById("windows-container");f||(f=document.createElement("div"),f.id="windows-container",f.className="windows-container",document.body.appendChild(f)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ye,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",s="",m="",C;t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((v,r)=>{let d=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,C=`<div class="window-content">${t.tabs.map((v,r)=>`<div class="tab-content ${r===0?"active":""}" data-tab-content="${r}">${v.content}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),C=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${C}
        `,f.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",y=>{y.stopPropagation(),p()}),window.addEventListener("resize",l),t.tabs&&e.querySelectorAll(".tab").forEach(y=>{y.addEventListener("click",()=>{let c=y.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(v=>v.classList.remove("active")),y.classList.add("active"),e.querySelectorAll(".tab-content").forEach(v=>v.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${c}"]`).classList.add("active"),u(y,e)})}),setTimeout(()=>{e.classList.add("open"),a&&a()},10)}function l(){if(!e||!e.classList.contains("open"))return;let f=e.querySelector(".tab.active");f&&u(f,e)}function u(f,b){setTimeout(()=>{let s=b.querySelector(".tab-bar");if(!s)return;let m=Array.from(s.querySelectorAll(".tab")),C=m.indexOf(f),y=s.clientWidth,c=m[Math.max(0,C-2)],v=m[Math.min(m.length-1,C+2)],r=c.offsetLeft-s.offsetLeft-25,h=v.offsetLeft+v.offsetWidth-s.offsetLeft+25-r,E=h<=y?r-(y-h)/2:f.offsetLeft-s.offsetLeft-y/2+f.offsetWidth/2;s.scrollTo({left:E,behavior:"smooth"})},350)}function p(f=!1){e&&(window.removeEventListener("resize",l),f?(e.remove(),e=null):(e.classList.remove("open"),i&&i(),setTimeout(()=>{e?.remove(),e=null},400)))}function g(f){let b=e?.querySelector(".window-content");b&&(b.innerHTML=f)}function x(){return e&&e.classList.contains("open")}return{open:o,close:p,setContent:g,isOpen:x,focus:n,id:t.id}}function Xe(){return!0}function Qe(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function et(t){localStorage.setItem("easter_user_email",t.trim())}var tt="easter_theme",K={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function ve(){return localStorage.getItem(tt)||K.DARK}function st(t){if(!Object.values(K).includes(t))throw new Error("Invalid theme");localStorage.setItem(tt,t),we(t)}function we(t){let e=document.body;if(Object.values(K).forEach(a=>{e.classList.remove(`theme-${a}`)}),e.classList.add(`theme-${t}`),[K.LIGHT,K.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let a=document.createElement("div");a.className="background",document.body.prepend(a)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function it(){let t=ve();we(t)}function D(t,e,i=null){let n={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",o=i?`<p class="placeholder-action">${i}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${n} placeholder-icon'></i><p class="placeholder-message">${e}</p>${o}</div>`}function _(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function J(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!i)return;if(!e){i.textContent="Last updated: never";return}let a=Date.now(),n=Math.floor((a-e)/1e3),o;n<60?o=`${n}s ago`:n<3600?o=`${Math.floor(n/60)}m ago`:o=`${Math.floor(n/3600)}h ago`,i.textContent=`Last updated: ${o}`}var jt=0;function xe(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!i)return;let a=i.querySelector(".notification-badge");a&&(e>0?(a.textContent=e>9?"9+":e,a.style.display="flex"):a.style.display="none")}var Y=0,X=0;function nt(t){Y=t,fe()}function at(t){X=t,fe()}function fe(){let t=Y+X;jt=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let i=document.getElementById("alerts-menu-item");if(i){let l=i.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="auto",i.appendChild(l)),l.textContent=Y>9?"9+":Y,l.style.display=Y>0?"flex":"none"}let a=document.getElementById("workspace-menu-item");if(a){let l=a.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="auto",a.appendChild(l)),l.textContent=X>9?"9+":X,l.style.display=X>0?"flex":"none"}let n=document.getElementById("switch-alerts");if(n){let l=n.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="8px",n.appendChild(l)),l.textContent=Y>9?"9+":Y,l.style.display=Y>0?"flex":"none"}let o=document.getElementById("switch-workspace");if(o){let l=o.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="8px",o.appendChild(l)),l.textContent=X>9?"9+":X,l.style.display=X>0?"flex":"none"}}function Ee(){let t=document.getElementById("alerts-list");if(!t)return;Y=t.querySelectorAll(".alert-unread:not(.priority-low)").length,fe()}function ot(){let t=document.getElementById("blueprints-list");if(!t)return;X=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,fe()}function De(){return"https://event.easter.company"}function qt(){return"https://discord.easter.company"}var Ft="http://127.0.0.1:8100",Gt="http://127.0.0.1:8300",Jt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function ke(t){let e=_(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(n,o)=>{let l=Jt[o];return l?`<span class="${l}">`:""});let i=(e.match(/<span/g)||[]).length,a=(e.match(/<\/span/g)||[]).length;return i>a&&(e+="</span>".repeat(i-a)),e}function se(t){if(!t)return"";let e=_(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(i,a)=>{let n=a.split("|").map(o=>o.trim());return n.every(o=>o.match(/^:?-+:?$/))?"":`<div class="md-table-row">${n.map(o=>`<span class="md-table-cell">${o}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var me=null,ue=null,Ce=!1,$e=!1;async function B(t,e={}){if(me)try{let n=await fetch(me+t,e);if(n.ok)return n;me=null}catch{me=null}let i=De(),a=Ft;try{let n=await fetch(i+t,e);if(n.ok)return me=i,Ce&&(console.log("\u2728 Production event service restored."),Ce=!1),n;throw new Error("Primary failed")}catch{Ce||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${a}`),Ce=!0);try{let o=await fetch(a+t,e);if(o.ok)return me=a,o;throw new Error("Fallback failed")}catch(o){throw o}}}async function Se(t,e={}){if(ue)try{let n=await fetch(ue+t,e);if(n.ok)return n;ue=null}catch{ue=null}let i=qt(),a=Gt;try{let n=await fetch(i+t,e);if(n.ok)return ue=i,$e&&(console.log("\u2728 Production discord service restored."),$e=!1),n;throw new Error("Primary failed")}catch{$e||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${a}`),$e=!0);try{let o=await fetch(a+t,e);if(o.ok)return ue=a,o;throw new Error("Fallback failed")}catch(o){throw o}}}var rt=()=>`
...
`,Te=null,ie=new Set,Le=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Vt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await B(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];Te=Date.now(),J(0,Te);let l=Date.now(),u=24*60*60*1e3,p=o.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let h=parseInt(d);return l-h<u});p.sort((r,d)=>{let h=I=>{let S=I.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},E=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,M=E(h(r)),k=E(h(d));return M!==k?k-M:d.timestamp-r.timestamp}),Le=p;let g=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(p.map(r=>g(r))).size>1;if(p.length>0){let r=null;p.forEach(d=>{let h=g(d);b&&h!==r&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),r=h),x.push(d)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=D("empty","No alerts yet."),Ee();return}let s=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let h=(d.title||"Untitled Alert").trim(),E=(d.body||"No description provided.").trim(),M=d.summary||"",k=d.content||"",I=d.protocol||"",S=(d.priority||"low").toLowerCase(),H=!!d.alert,U=(d.category||"system").trim(),z=d.related_event_ids||[],P=d.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${r.id}`),L=new Date(r.timestamp*1e3),$=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=F?"event-border-grey":"event-border-blue";!F&&H&&(A="event-border-red"),F&&(S==="high"||S==="critical")?A="event-border-red":F&&S==="medium"&&(A="event-border-orange");let T=F?"alert-read":"alert-unread",N=ie.has(r.id),R=N?"expanded":"",Q=N?"display: block;":"display: none;",O="",j="";z.length>0&&(j=`
        ...
        `:Z=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${se(E)}</div>
            </div>
        `,O=`
                ...
                ${Z}
            `;let G=document.createElement("div");G.className=`event-item notification-item ${A} ${T} ${R} cursor-pointer priority-${S}`,G.dataset.alertId=r.id,G.onclick=function(ee){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ie.delete(r.id);let be=this.querySelector(".event-details");be&&(be.style.display="none")}else{this.classList.add("expanded"),ie.add(r.id);let be=this.querySelector(".event-details");if(be&&(be.style.display="block"),!localStorage.getItem(`alert_read_ts_${r.id}`)){localStorage.setItem(`alert_read_ts_${r.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Be="event-border-grey";S==="high"||S==="critical"?Be="event-border-red":S==="medium"&&(Be="event-border-orange"),this.classList.add(Be),Ee()}}};let Fe=`${I?I.toUpperCase():"GUARDIAN"} ALERT: ${M||h}`,Ot={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[U]||"bx-bell",Ut=S==="high"||S==="critical"?"#ff4d4d":S==="medium"?"#ffa500":"#888";G.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Ut}; font-size: 0.8em; margin-left: 5px;">[${S.toUpperCase()}]</span>
                    </div>
            ...
            `;let Ge=G.querySelector(".event-details");Ge&&Ge.addEventListener("click",ee=>{ee.stopPropagation()});let Je=G.querySelector(".close-details-btn");return Je&&Je.addEventListener("click",ee=>{ee.stopPropagation(),G.classList.remove("expanded");let _e=G.querySelector(".event-details");_e&&(_e.style.display="none"),ie.delete(r.id)}),G},m=r=>{let d=document.createElement("div");d.className="notification-divider",d.dataset.alertId=r.id;let h="#888888";return r.label==="CRITICAL"?h="#ff4d4d":r.label==="HIGH"?h="#ff8888":r.label==="MEDIUM"&&(h="#ffa500"),d.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,d.innerHTML=`<span style="white-space: nowrap;">${r.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,d},C=Array.from(e.children),y=new Map(C.map(r=>[r.dataset.alertId,r])),c=new Set(x.map(r=>r.id));C.forEach(r=>{let d=r.dataset.alertId;(!d||!c.has(d))&&r.remove()});let v=null;x.forEach((r,d)=>{let h=y.get(r.id);(!h||t)&&(h&&h.remove(),r.type==="divider"?h=m(r):h=s(r),!h)||(d===0?e.firstElementChild!==h&&e.prepend(h):v&&v.nextElementSibling!==h&&v.after(h),v=h)}),Te=Date.now(),J(0,Te),Ee()}catch(a){console.error("Error fetching alerts:",a),e.children.length===0&&(e.innerHTML=D("offline","Failed to load alerts.","The event service may be offline."))}}function Vt(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),a=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Le.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Le.forEach(n=>{ie.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{ie.clear(),ne(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Le.forEach(o=>{localStorage.setItem(`alert_read_ts_${o.id}`,n.toString())}),ie.clear(),ne(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}async function lt(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(o=>{let l=o.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}if((l.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${o.id}`)||n++}),nt(n)}catch{}}var ct=()=>`
...
`,ae=new Set,pt=[],ge=null;async function oe(t=!1){let e=document.getElementById("roadmap-list");if(e){Wt();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();pt=a;let n=g=>{let x=ae.has(g.id),f=g.state==="published",b=g.state==="consumed",s="event-border-grey";f&&(s="event-border-blue"),b&&(s="event-border-purple");let C=new Date(g.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${s} cursor-pointer ${x?"expanded":""}`,y.dataset.itemId=g.id,y.onclick=r=>{if(r.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(g.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",ae.add(g.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${g.state.toUpperCase()}]</span>`;y.innerHTML=`
        <div class="event-time">
      ...
      `;let v=y.querySelector(".event-details");return v&&v.addEventListener("click",r=>{r.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>Zt(g)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Kt(g)),y.querySelector(".delete-btn")?.addEventListener("click",()=>Yt(g.id)),y.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(g.id)}),y},o=Array.from(e.children),l=new Map(o.map(g=>[g.dataset.itemId,g])),u=new Set(a.map(g=>g.id));if(o.forEach(g=>{let x=g.dataset.itemId;(!x||!u.has(x))&&g.remove()}),a.length===0){e.innerHTML=D("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let p=null;a.forEach((g,x)=>{let f=l.get(g.id);(!f||t)&&(f&&f.remove(),f=n(g),!f)||(x===0?e.firstElementChild!==f&&e.prepend(f):p&&p.nextElementSibling!==f&&p.after(f),p=f)})}catch{e.children.length===0&&(e.innerHTML=D("offline","Failed to load roadmap.","The event service may be offline."))}}}function Wt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ge=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ge=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-input").value;if(!o.trim())return;let l=ge?`/roadmap/${ge}`:"/roadmap",u=ge?"PATCH":"POST";try{await B(l,{method:u,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})}),document.getElementById("roadmap-editor-container").style.display="none",oe(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{pt.forEach(o=>ae.add(o.id)),oe(!0)},a.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ae.clear(),oe(!0)},n.dataset.listenerAttached="true")}function Zt(t){ge=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Kt(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),oe(!0)}catch(i){console.error(i)}}async function Yt(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),ae.delete(t),oe(!0)}catch(e){console.error(e)}}var ut=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,mt=null,re=new Set,gt=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Xt();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await B(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(gt=o,mt=Date.now(),J(2,mt),o.length===0){e.innerHTML=D("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),xe(2,0);return}t&&(e.innerHTML="");let l=f=>{let b=f.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let s=(b.title||"Untitled Blueprint").trim(),m=(b.summary||b.body||"No summary provided.").trim(),C=(b.content||"").trim(),y=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(T=>T.trim()),v=(b.implementation_path||[]).map(T=>T.trim()),r=b.source_event_ids||[],d=b.approved===!0,h=new Date(f.timestamp*1e3),E=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=re.has(f.id),I=k?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${d?"blueprint-approved":""}`,S.dataset.blueprintId=f.id,d&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";S.onclick=function(T){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(f.id);let R=this.querySelector(".event-details");R&&(R.style.display="none")}else{this.classList.add("expanded"),re.add(f.id);let R=this.querySelector(".event-details");R&&(R.style.display="block")}};let z="";v.length>0&&(z=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(T=>`<li style="margin-bottom: 5px;">${_(T)}</li>`).join("")}</ul></div>
                    </div>
                `);let P="";r.length>0&&(P=`
                ...
                `);let V=c.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${c.join(", ")}</span></div>`:"<div></div>",F=d?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-icon" style="${d?"color: #03dac6;":""}"><i class='bx ${U}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${I}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${_(m)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${_(C)}</div>
                        </div>
                        ${z}
                        ${P}
                        ${F}
            ...
            `;let L=S.querySelector(".blueprint-approve-btn");L&&(L.onclick=async T=>{T.stopPropagation(),L.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${f.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&le(!0)}catch(N){console.error("Failed to approve blueprint:",N)}});let $=S.querySelector(".blueprint-delete-btn");$&&($.onclick=async T=>{T.stopPropagation();let N=!d;$.innerHTML=N?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${f.id}`,{method:"DELETE"})).ok&&le(!0)}catch(R){console.error("Failed to delete blueprint:",R)}});let w=S.querySelector(".event-details");w&&w.addEventListener("click",T=>{T.stopPropagation()});let A=S.querySelector(".close-details-btn");return A&&A.addEventListener("click",T=>{T.stopPropagation(),S.classList.remove("expanded");let N=S.querySelector(".event-details");N&&(N.style.display="none"),re.delete(f.id)}),S},u=Array.from(e.children),p=new Map(u.map(f=>[f.dataset.blueprintId,f])),g=new Set(o.map(f=>f.id));u.forEach(f=>{let b=f.dataset.blueprintId;(!b||!g.has(b))&&f.remove()});let x=null;o.forEach((f,b)=>{let s=p.get(f.id);(!s||t)&&(s&&s.remove(),s=l(f),!s)||(b===0?e.firstElementChild!==s&&e.prepend(s):x&&x.nextElementSibling!==s&&x.after(s),x=s)}),xe(2,o.length),ot()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=D("offline","Failed to load blueprints.","The event service may be offline."))}}async function bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(o=>{let l=o.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),at(n)}catch{}}function Xt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{gt.forEach(a=>re.add(a.id)),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{re.clear(),le(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),re.clear(),le(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var vt=()=>`
...
`;async function ze(){await Promise.all([oe(),le()])}var Qt=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function xt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"VS Code"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function yt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=Qt,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),Se(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();He(e,t,a)}else{console.log("Profile not found or error, using mock data.");let a=xt(t.id,t.username);He(e,t,a)}}).catch(i=>{console.error("Profile fetch error:",i);let a=xt(t.id,t.username);He(e,t,a)})}function He(t,e,i){let a=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let C=(i.badges||[]).map(E=>`<span class="profile-badge ${E==="Creator"?"master":""}">${E}</span>`).join(""),y=(i.attributes||i.facts||[]).map(E=>{let M=E.key||E.k,k=E.value||E.v;return`
            <div class="fact-chip">
        ...
        `},o=()=>{let C=i.dossier||{},y=C.identity||{},c=C.career||{},v=C.personal||{},r=C.social||[];return`
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${_(e.username)}</h2>
                    <div class="profile-badges">${(i.badges||[]).map(C=>`<span class="profile-badge ${C==="Creator"?"master":""}">${C}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=g;let x=t.querySelector(".profile-card"),f=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),setTimeout(()=>t.remove(),300)}),b.forEach(C=>{C.addEventListener("click",()=>{b.forEach(y=>y.classList.remove("active")),s.forEach(y=>y.classList.remove("active")),C.classList.add("active"),t.querySelector(`#tab-${C.dataset.tab}`).classList.add("active")})}),f.addEventListener("click",()=>{x.classList.toggle("expanded");let C=x.classList.contains("expanded");f.innerHTML=C?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var ht=()=>`
...
`,es=null;async function Ne(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Ne(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let a=i.target.closest(".user-profile-card");if(a)try{let n=JSON.parse(a.dataset.user);yt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Se("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(es=Date.now(),n.length===0){t.innerHTML=D("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,u)=>{let p=o[l.level]??10,g=o[u.level]??10;return p!==g?p-g:l.username.localeCompare(u.username)}),t.innerHTML=n.map(l=>{let u=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",g="#888";l.level==="Me"||l.level==="Master"?g="#bb86fc":l.level==="Admin"?g="#cf6679":l.level==="Moderator"?g="#03dac6":l.level==="Contributor"&&(g="#ffa500");let x=l.level==="Me",f=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=x?"2px solid #bb86fc":`1px solid ${u}33`;return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(l).replace(/"/g,"&quot;")}"
                     style="background: ${f}; padding: 15px; border-radius: 8px; border: ${b}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s;">
...
`,Me=null,ce=new Set,ye=[],q="all",ss={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},is={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ns(t){for(let[e,i]of Object.entries(ss))if(i.includes(t))return e;return"system"}function as(t){return is[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;os();let a=`/events?ml=${q==="all"?100:250}&format=json`;q!=="all"&&(a+=`&category=${q}`);try{let n=await B(a);if(!n.ok)throw new Error("Service unreachable");if(ye=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Me=Date.now(),J(1,Me),ye.length===0){e.innerHTML=D("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,C=ns(m),y=as(m),c=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),v="event-border-grey";c&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?v="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?v="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?v="event-border-orange":m==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=ce.has(b.id),h=d?"expanded":"",E=d?"display: block;":"display: none;",M=new Date(b.timestamp*1e3),k=M.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=M.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=wt(m,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",U="";if(c){let L="";if(m==="engagement.decision"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,w=s.timestamp-s.last_active,A=Math.floor(w/60);L=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,w="";s.eval_count&&(w=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let A="";if(s.chat_history&&s.chat_history.length>0){let T=s.chat_history.length,N=s.chat_history.map((R,Q)=>{let O=R.role.toUpperCase();O==="ASSISTANT"&&(O="DEXTER");let j=R.role==="user"?"#03dac6":R.role==="system"?"#ffb74d":"#bb86fc",W=Q===T-1?"block":"none";return`
                                <div class="history-slide" data-index="${Q}" style="display: ${W};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${j}; letter-spacing: 1px; font-weight: bold;">${O}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${Q+1} of ${T}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${_(R.content)}</div>
                                </div>
                            `}).join("");A=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(m==="analysis.user.message_signals"){let $=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,w=s.signals||{};L=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(w.topics||[]).map(T=>`<span class="profile-badge">${T}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${$("Raw Model Output")}
                            <pre class="detail-pre">${_(s.raw_output)||"None"}</pre>
                        </div>
                    `}else if(m==="moderation.explicit_content.deleted"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${_(s.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${$("Description")}
                            <pre class="detail-pre">${_(s.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${$("Content Summary")}
                            <pre class="detail-pre">${_(s.summary)||"None"}</pre>
                        </div>
                    `}else if(m==="analysis.visual.completed"){let $=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,w="";s.base64_preview?w=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(w=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${$("Input Context")}
                            <pre class="detail-pre">${_(s.raw_input)||"None"}</pre>
                        </div>
                    `}else if(m==="system.cli.command"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${$("Output")}
                            <pre class="detail-pre">${_(s.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",w=s.success?"SUCCESS":"FAILED",A=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,T="";s.error&&(T=`
                            <div class="event-detail-block">
                                ${A("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${_(s.error)}</pre>
                            </div>
                        `);let N="";if(s.chat_history&&s.chat_history.length>0){let O=s.chat_history.length,j=s.chat_history.map((W,pe)=>{let Z=W.role.toUpperCase();Z==="USER"&&(Z="SYSTEM"),Z==="ASSISTANT"&&(Z="AGENT");let G=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${pe}" style="display: ${pe===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${pe+1} of ${O}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${_(W.content)}</div>
                                </div>
                            `}).join("");N=`
                            <div class="event-detail-block">
                                ...
                                <div class="history-carousel" style="position: relative; background: rgba(255,255,255,0.03); border-radius: 4px; padding: 15px;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                                        ...
                                        <button class="carousel-btn next-btn" style="background: rgba(255,255,255,0.1); border: none; color: #fff; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.8em; transition: background 0.2s;" ${O<=1?"disabled":""}>Next <i class='bx bx-chevron-right'></i></button>
                                    </div>
                                </div>
                            </div>
                        `}let R="";if(s.corrections&&s.corrections.length>0){let O=s.corrections.map((j,W)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${j.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${_(j.snippet)}</div>`:""}
                            </div>
                        `).join("");R=`
                            <div class="event-detail-block">
                        ...
                        `}let Q="";if(s.parsed_results&&s.parsed_results.length>0){let O=s.parsed_results.map(j=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${_(j.title)}</div>
                                <div style="color: #ddd;">${_(j.summary)}</div>
                            </div>
                        `).join("");Q=`
                            <div class="event-detail-block">
                                ${A("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${w} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${R}
                        ${N}
                    `}else if(m==="system.test.completed"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                        ...
                        <div class="event-detail-block">
                            ${$("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${_(s.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${$("Context")}
                            <pre class="detail-pre">${JSON.stringify(s.context||{},null,2)}</pre>
                        </div>
                    `}else if(m==="system.cli.status"){let $=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ${$("Message")}
                            <pre class="detail-pre">${_(s.message)}</pre>
                        </div>
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(A=>{let T=A.content_type&&A.content_type.startsWith("image/"),N=(A.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${T?`<div class="attachment-thumb-container"><img src="${A.url}" alt="${A.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${A.url}" target="_blank" class="attachment-link">${A.filename}</a>
                                        <span class="attachment-meta">${A.content_type} \u2022 ${N}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let z=document.createElement("div");z.className=`event-item ${v} ${r} ${h}`,z.dataset.eventId=b.id,z.onclick=function(L){if(!c)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),ce.delete(b.id);let w=this.querySelector(".event-details");w&&(w.style.display="none")}else{this.classList.add("expanded"),ce.add(b.id);let w=this.querySelector(".event-details");w&&(w.style.display="block")}},z.innerHTML=`
                <div class="event-time">
            ...
            `;let P=z.querySelector(".event-details");if(P&&P.addEventListener("click",L=>{L.stopPropagation()}),c){let L=z.querySelector(".close-details-btn");L&&L.addEventListener("click",$=>{$.stopPropagation();let w=$.target.closest(".event-item");if(w){w.classList.remove("expanded"),ce.delete(b.id);let A=w.querySelector(".event-details");A&&(A.style.display="none")}})}let V=z.querySelector(".prev-btn"),F=z.querySelector(".next-btn");if(V&&F){let L=0,$=z.querySelectorAll(".history-slide"),w=$.length,A=()=>{$.forEach((T,N)=>{T.style.display=N===L?"block":"none"}),V.disabled=L===0,F.disabled=L===w-1,V.style.opacity=L===0?"0.5":"1",F.style.opacity=L===w-1?"0.5":"1"};V.addEventListener("click",T=>{T.stopPropagation(),L>0&&(L--,A())}),F.addEventListener("click",T=>{T.stopPropagation(),L<w-1&&(L++,A())}),A()}return z},p=Array.from(e.children),g=new Map(p.map(b=>[b.dataset.eventId,b])),x=new Set(ye.map(b=>b.id));p.forEach(b=>{let s=b.dataset.eventId;(!s||!x.has(s))&&b.remove()});let f=null;ye.forEach((b,s)=>{let m=g.get(b.id);(!m||t)&&(m&&m.remove(),m=u(b),!m)||(s===0?e.firstElementChild!==m&&e.prepend(m):f&&f.nextElementSibling!==m&&f.after(m),f=m)}),Me=Date.now(),J(1,Me)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=D("offline","Failed to load events.","The event service may be offline."))}}function os(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ye.forEach(n=>ce.add(n.id)),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ce.clear(),de(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(o=>o.classList.remove("active")),n.classList.add("active"),q=n.dataset.filter,de(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===q?n.classList.add("active"):n.classList.remove("active")});let a=document.getElementById("events-clear");a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{let n=q==="all"?"ALL":q.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let o="/events";if(q!=="all"?o+=`?category=${q}`:o+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await B(o,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ce.clear(),de(!0)}catch(o){console.error("Failed to clear events:",o),alert("Failed to clear events. Check console.")}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}var rs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Ct=rs;function $t(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var ls=null;async function he(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=D("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let a=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],n=i.filter(l=>!a.includes(l.id));n.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),n.reverse();let o=n.map(l=>{let u=l.logs.join(`
`),p=[...l.logs];if(p.length<25){let x=25-p.length;for(let f=0;f<x;f++)p.push("")}else p.length>25&&(p=p.slice(-25));let g=p.map(x=>ke(x)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=o,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let u=unescape(l.dataset.logs);navigator.clipboard.writeText(u).then(()=>{let p=l.querySelector("i");p.classList.remove("bx-copy"),p.classList.add("bx-check"),setTimeout(()=>{p.classList.remove("bx-check"),p.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let u=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${u}?`))try{(await B(`/logs?service_id=${u}`,{method:"DELETE"})).ok&&he()}catch(p){console.error("Error clearing logs:",p)}})}),ls=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=D("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Tt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${$t()}
        </div>`;async function Bt(){await Promise.all([Ae(),Oe(),Ue()]),await he()}var Et=null,kt=null;async function Dt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function St(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function cs(){try{return await(await B("/processes")).json()}catch{return null}}async function ds(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Oe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),o=document.querySelector("#hw-gpu .hw-content"),l=document.querySelector("#hw-storage .hw-content"),u=c=>{if(c){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),l&&c.STORAGE&&c.STORAGE.length>0?l.innerHTML=c.STORAGE.map(v=>{let r=(v.USED/1073741824).toFixed(1),d=(v.SIZE/(1024*1024*1024)).toFixed(1),h=v.SIZE>0?(v.USED/v.SIZE*100).toFixed(0):0,E=v.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await St();c?(u(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await St();u(c)}if(!t)return;let p=await Dt();if(!p||!p.services){t.children.length===0&&(t.innerHTML=D("offline","Failed to load system metrics.","The event service may be offline."));return}Et=Date.now(),J(0,Et);let g=p.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function x(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function f(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function b(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function s(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let r=parseInt(v[1])||0,d=parseInt(v[2])||0,h=parseInt(v[3])||0,E=parseFloat(v[4])||0,M=r*86400+d*3600+h*60+E,k=Math.floor(M/86400);if(k>0)return`${k}d`;let I=Math.floor(M/3600);if(I>0)return`${I}h`;let S=Math.floor(M/60);return S>0?`${S}m`:`${Math.floor(M)}s`}function m(c){let v=c.status==="online",r=v?"service-widget-online":"service-widget-offline",d=v?"bx-check-circle":"bx-x-circle",h=v?"OK":"BAD",E=c.version?f(c.version.str):"-",M=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",k=c.memory&&c.memory!=="N/A"?c.memory:"-",I=M!=="-"?"#00ff00":"#666",S=M!=="-"?"#fff":"#666",H=k!=="-"?"#00ff00":"#666",U=k!=="-"?"#fff":"#666",z=s(c.uptime),P="";return v?P=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${U};">${k}</span>
                    </div>
                </div>`:P='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${r}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${d}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${b(c.domain&&c.port?`${c.domain}:${c.port}`:"")}</span></div>${P}</div></div>`}let C=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),y=new Set(g.map(c=>c.id));for(let[c,v]of C)y.has(c)||v.remove();g.forEach(c=>{let v=m(c),r=C.get(c.id);r?r.outerHTML!==v&&(r.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}async function Ue(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Dt();if(!e){t.children.length===0&&(t.innerHTML=D("offline","Failed to load model status.","The event service may be offline."));return}kt=Date.now(),J(2,kt);let i=e.models||[],a=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function n(p){let g=p.status==="Ready";return`
                ...
                </div>`}function o(p){let g=p.status==="Ready";return`
                ...
                </div>`}function l(p){let g=p.status==="Downloaded",x=g?"service-widget-online":"service-widget-offline",f=g?"OK":"MISSING",b=g&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",s=p.name;return p.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${x}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${g?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let u="";if(a&&(u+=n(a)),e.xtts&&(u+=o(e.xtts)),u+=i.map(l).join(""),!u){t.innerHTML=D("empty","No models found.");return}t.innerHTML!==u&&(t.innerHTML=u)}async function Ae(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),u=document.getElementById("guardian-reset-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),Ae()}catch{u.innerHTML="<i class='bx bx-error'></i> Failed"}},u.dataset.listenerAttached="true");let p=await ds();if(p){let C=Math.floor(Date.now()/1e3),y=p.active_tier,c=p.protocol_aliases||{sentry:"Sentry",architect:"Architect"},v=E=>{E<0&&(E=0);let M=Math.floor(E/3600),k=Math.floor(E%3600/60),I=E%60;return M>0?`${M}h ${k}m`:k>0?`${k}m ${I}s`:`${I}s`},r=(E,M,k,I)=>{if(!E)return;let S=c[I]||I.toUpperCase(),H=E.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(H&&(H.textContent=S),y&&y.includes(S))E.textContent="Working",E.style.color="#bb86fc";else if(k){let z=k.next_run-C;if(z<=0)E.textContent="Ready",E.style.color="#5eff5e";else{let P=Math.floor(z/60),V=z%60;E.textContent=`${P}m ${V}s`,E.style.color="#fff"}}M&&k&&(M.innerHTML=`
        ...
        `)};e&&r(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry"),i&&r(i,document.getElementById("guardian-architect-stats"),p.architect,"architect");let d=document.getElementById("system-state-label"),h=document.getElementById("system-state-val");if(h&&p.system_state){let E=p.system_state,M=v(p.system_state_time||0);d&&(d.textContent=`State: ${E.toUpperCase()}`),h.textContent=M,E==="idle"?h.style.color=p.system_state_time>300?"#5eff5e":"#fff":h.style.color="#bb86fc"}n&&(n.textContent=v(p.total_idle_time||0)),o&&(o.textContent=v(p.total_active_time||0)),l&&(l.textContent=v(p.total_waste_time||0))}else[e,i,a,n,o,l].forEach(y=>{y&&(y.textContent="-",y.style.color="#666")});let g=await cs(),x=[],f=[],b=[];g&&(Array.isArray(g)?x=g:(x=g.active||[],f=g.queue||[],b=g.history||[],b.sort((C,y)=>(y.end_time||0)-(C.end_time||0)))),x.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=D("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Pe(t,x,!1));let s=document.getElementById("processes-queue-widgets");s&&(f.length===0?!s.querySelector(".tab-placeholder")&&!s.querySelector('div[style*="font-style: italic"]')&&(s.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(s.innerHTML="",Pe(s,f,!1)));let m=document.getElementById("processes-history-widgets");m&&(b.length===0?m.querySelector(".tab-placeholder")||(m.innerHTML=D("empty","No recent history.")):(m.querySelector(".tab-placeholder")&&(m.innerHTML=""),Pe(m,b,!0))),xe(1,x.length)}function Pe(t,e,i){function a(u){let p="";u.end_time?p=`${u.end_time-u.start_time}s`:p=`${Math.floor(Date.now()/1e3-u.start_time)}s`;let g=u.retries>0?`<span class="process-retry-badge">Retry ${u.retries}</span>`:"",x=u.channel_id,f={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation"};if(f[x]?x=f[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),i)return`
        <div class="process-history-item" data-channel-id="${u.channel_id}-${u.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let n=i?".process-history-item":".process-widget",o=new Map(Array.from(t.querySelectorAll(n)).map(u=>[u.dataset.channelId,u])),l=new Set(e.map(u=>`${u.channel_id}-${u.start_time}`));for(let[u,p]of o)l.has(u)||p.remove();e.forEach(u=>{let p=`${u.channel_id}-${u.start_time}`,g=a(u),x=o.get(p);if(x){x.outerHTML!==g&&(x.outerHTML=g);let f=t.querySelector(`[data-channel-id="${p}"]`);f&&t.appendChild(f)}else t.insertAdjacentHTML("beforeend",g)})}function Ie(){let t=ve(),e=Qe()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
        <div class="cli-dashboard">
            ...
            <div class="cli-divider">
                ...
                <span>Interactive Demos (v${ps})</span>
                <i class='bx bx-chevron-down'></i>
            </div>
    `;for(let[i,a]of Object.entries(t)){let n=Nt.filter(o=>o.category===i);n.length!==0&&(e+=`
    ...
    </div>`,e},qe=!1;function us(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),qe=!0,i}function zt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),qe=!1}function Ht(t,e,i="output"){if(!qe)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=ke(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function fs(t){let e=Nt.find(n=>n.id===t);if(!e)return;let i=us(e);Ht(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let n of a){await new Promise(l=>setTimeout(l,400+Math.random()*600));let o="output";n.includes("[ERROR]")?o="error":n.includes("[SUCCESS]")||n.includes("\u2713")?o="success":n.includes("!")&&(o="warning"),Ht(i,n,o)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Rt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;fs(a)})})}async function gs(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${De()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Pt(){gs(),it(),Ve();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Rt();let e=Xe();We(e),Ke();let i=document.getElementById("nav-left-container");i&&i.addEventListener("click",()=>{if(o.length>0)f();else{let r=window.location.pathname;if(!(r==="/"||r==="/index.html")){let E=(r.endsWith("/")&&r.length>1?r.slice(0,-1):r).split("/");E.pop();let M=E.join("/")||"/";window.location.href=M}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",n=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!n&&document.querySelector("footer")?.classList.add("hide");let o=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let u=r=>{localStorage.setItem("dex_last_window",r)};function p(){return 1}function g(){for(;o.length>1;)o.shift().close(!0);let r=document.getElementById("windows-container"),d=document.querySelector("nav"),h=document.querySelector("footer"),E=window.location.pathname==="/"||window.location.pathname==="/index.html",M=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",o.length),o.forEach(H=>{let U=document.getElementById(H.id);U&&U.classList.remove("hide-close")}),we(ve());let k=document.getElementById("dexter-menu-container"),I=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon");if(Ze(o.length>0),o.length>0){if(h?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),k&&(k.style.display="none"),S&&(S.style.display="none"),I){let H=o[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(H)?(I.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),h=document.getElementById("dexter-menu-btn");d&&r&&h&&(d.addEventListener("mouseenter",()=>{r.classList.add("active"),h.classList.add("active")}),d.addEventListener("mouseleave",()=>{r.classList.remove("active"),h.classList.remove("active")}),h.addEventListener("click",E=>{E.stopPropagation();let M=localStorage.getItem("dex_last_window")||"alerts-window",k=null;M==="alerts-window"?k=b:M==="events-window"?k=s:M==="monitor-window"?k=C:M==="contacts-window"?k=m:M==="workspace-window"&&(k=y),k&&x(k)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{u("alerts-window"),x(b)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{u("events-window"),x(s)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{u("monitor-window"),x(C)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{u("contacts-window"),x(m)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{u("workspace-window"),x(y)}),document.getElementById("settings-icon")?.addEventListener("click",()=>x(c)),document.getElementById("close-all-windows")?.addEventListener("click",()=>f()),setInterval(()=>{b.isOpen()?ne():lt(),y.isOpen()?ze():bt(),s.isOpen()&&de(),C.isOpen()&&Bt()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{v.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",r=>{r.preventDefault();try{et(document.getElementById("email-input").value),window.location.reload()}catch(d){let h=document.getElementById("login-error");h&&(h.textContent=d.message,h.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Pt):Pt();})();

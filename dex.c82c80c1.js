(()=>{function ke(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ie(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",l=t?`
        <div class="nav-right">
...
`,re=null,z=new Set,ce=[];async function F(t=!1){let e=document.getElementById("notifications-list");if(!e)return;ft(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];re=Date.now(),B(0,re);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let b=localStorage.getItem(`notification_read_ts_${d.id}`);if(!b)return!0;let y=parseInt(b);return c-y<g});u.sort((d,b)=>{let y=C=>{let _=C.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},A=C=>C==="critical"?4:C==="high"?3:C==="medium"?2:1,L=A(y(d)),E=A(y(b));return L!==E?E-L:b.timestamp-d.timestamp}),ce=u;let v=d=>{let b=d.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return"low"}return(b.priority||"low").toLowerCase()},x=[],m=new Set(u.map(d=>v(d))).size>1;if(u.length>0){let d=null;u.forEach(b=>{let y=v(b);m&&y!==d&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),d=y),x.push(b)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=T("empty","No notifications yet."),oe();return}let i=d=>{let b=d.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let y=b.title||"Untitled Notification",A=b.body||"No description provided.",L=b.priority||"low",E=!!b.alert,C=b.category||"system",_=b.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${d.id}`),P=new Date(d.timestamp*1e3),D=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=H?"event-border-grey":"event-border-blue";!H&&E&&($="event-border-red"),H&&(L==="high"||L==="critical")?$="event-border-red":H&&L==="medium"&&($="event-border-orange");let N=H?"notification-read":"notification-unread",M=z.has(d.id),ue=M?"expanded":"",ge=M?"display: block;":"display: none;",$e="",Te="";_.length>0&&(Te=`
                ...
                <div class="event-detail-block" style="text-align: left;">
                    ...
                    <p class="detail-pre" style="white-space: pre-wrap; margin-top: 5px; text-align: left;">${k(A)}</p>
                </div>
                ${Te}
...
`,q=new Set,ze=[],K=null;async function j(t=!1){let e=document.getElementById("roadmap-list");if(e){vt();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let l=await a.json();if(ze=l,l.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=v=>{let x=q.has(v.id),p=v.state==="draft",m=v.state==="published",i=v.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),i&&(n="event-border-purple");let f=new Date(v.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${n} cursor-pointer ${x?"expanded":""}`,h.dataset.itemId=v.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",q.delete(v.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",q.add(v.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${v.state.toUpperCase()}]</span>`;return h.innerHTML=`
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${k(v.content)}</div>
...
`);return`
    ...
    `;async function we(){await Promise.all([nt(),It(),At()]),await xe()}var Qe=null,Ze=null,et=null;async function it(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function tt(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Ct(){try{return await(await I("/processes")).json()}catch{return null}}async function kt(){try{return await(await I("/analyst/status")).json()}catch{return null}}async function It(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),l=i=>{if(!i){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let n="",o=(i.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(n+=`

(()=>{function Xe(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Qe(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var tt=1e3;function ae(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++tt)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),s&&setTimeout(s,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++tt,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",a="",g="",E;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((r,d)=>{let u=r.icon?`<i class="bx ${r.icon}"></i>`:`<span class="tab-glyph">${r.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,E=`<div class="window-content">${t.tabs.map((r,d)=>`<div class="tab-content ${d===0?"active":""}" data-tab-content="${d}">${r.content}</div>`).join("")}</div>`):(t.title&&(g=`<div class="window-title">${t.title}</div>`),E=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Ae=null,re=new Set,Ie=[];async function ie(t=!1){let e=document.getElementById("alerts-list");if(!e)return;os(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await I(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ae=Date.now(),X(0,Ae);let l=Date.now(),v=24*60*60*1e3,o=c.filter(d=>{let u=localStorage.getItem(`alert_read_ts_${d.id}`);if(!u)return!0;let w=parseInt(u);return l-w<v});o.sort((d,u)=>{let w=k=>{let x=k.event;if(typeof x=="string")try{x=JSON.parse(x)}catch{return"low"}return(x.priority||"low").toLowerCase()},A=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,_=A(w(d)),S=A(w(u));return _!==S?S-_:u.timestamp-d.timestamp}),Ie=o;let p=d=>{let u=d.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},y=[],b=new Set(o.map(d=>p(d))).size>1;if(o.length>0){let d=null;o.forEach(u=>{let w=p(u);b&&w!==d&&(y.push({id:`divider-${w}`,type:"divider",label:w.toUpperCase()}),d=w),y.push(u)})}if(t&&(e.innerHTML=""),o.length===0){e.innerHTML=N("empty","No alerts yet."),Te();return}let a=d=>{let u=d.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let w=(u.title||"Untitled Alert").trim(),A=(u.body||"No description provided.").trim(),_=u.summary||"",S=u.content||"",k=u.protocol||"",x=(u.priority||"low").toLowerCase(),L=!!u.alert,B=(u.category||"system").trim(),H=u.related_event_ids||[],J=u.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${d.id}`),T=new Date(d.timestamp*1e3),$=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=j?"event-border-grey":"event-border-blue";!j&&L&&(M="event-border-red"),j&&(x==="high"||x==="critical")?M="event-border-red":j&&x==="medium"&&(M="event-border-orange");let O=j?"alert-read":"alert-unread",D=re.has(d.id),U=D?"expanded":"",q=D?"display: block;":"display: none;",F="",W="";H.length>0&&(W=`
        ...
        `,F=`
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${O} ${U} cursor-pointer priority-${x}`,Z.dataset.alertId=d.id,Z.onclick=function(se){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(d.id);let he=this.querySelector(".event-details");he&&(he.style.display="none")}else{this.classList.add("expanded"),re.add(d.id);let he=this.querySelector(".event-details");if(he&&(he.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ne="event-border-grey";x==="high"||x==="critical"?Ne="event-border-red":x==="medium"&&(Ne="event-border-orange"),this.classList.add(Ne),Te()}}};let Ke=`${k?k.toUpperCase():"GUARDIAN"} ALERT: ${_||w}`,Gt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[B]||"bx-bell",Wt=x==="high"||x==="critical"?"#ff4d4d":x==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Wt}; font-size: 0.8em; margin-left: 5px;">[${x.toUpperCase()}]</span>
                    </div>
...
`,ft=null,de=new Set,vt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ps();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await I(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(vt=c,ft=Date.now(),X(2,ft),c.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let l=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),h=(b.category||"architecture").trim(),f=(b.related_services||b.affected_services||[]).map(D=>D.trim()),r=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],u=b.approved===!0,w=new Date(m.timestamp*1e3),A=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=de.has(m.id),k=S?"display: block;":"display: none;",x=document.createElement("div");x.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${u?"blueprint-approved":""}`,x.dataset.blueprintId=m.id,u&&(x.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",x.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let B=u?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";x.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let q=this.querySelector(".event-details");q&&(q.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let q=this.querySelector(".event-details");q&&(q.style.display="block")}};let H="";r.length>0&&(H=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${r.map(D=>`<li style="margin-bottom: 5px;">${P(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let J="";d.length>0&&(J=`
                ...
                `);let G=f.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${f.join(", ")}</span></div>`:"<div></div>",j=z(),T=u?`
            ...
            `;x.innerHTML=`
                ...
                <div class="event-icon" style="${u?"color: #03dac6;":""}"><i class='bx ${B}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${k}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${P(g)}</div>
            ...
            `;let $=x.querySelector(".blueprint-approve-btn");$&&($.onclick=async D=>{D.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await I(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(U){console.error("Failed to approve blueprint:",U)}});let C=x.querySelector(".blueprint-delete-btn");C&&(C.onclick=async D=>{D.stopPropagation();let U=!u;C.innerHTML=U?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await I(`/events/${m.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let M=x.querySelector(".event-details");M&&M.addEventListener("click",D=>{D.stopPropagation()});let O=x.querySelector(".close-details-btn");return O&&O.addEventListener("click",D=>{D.stopPropagation(),x.classList.remove("expanded");let U=x.querySelector(".event-details");U&&(U.style.display="none"),de.delete(m.id)}),x},v=Array.from(e.children),o=new Map(v.map(m=>[m.dataset.blueprintId,m])),p=new Set(c.map(m=>m.id));v.forEach(m=>{let b=m.dataset.blueprintId;(!b||!p.has(b))&&m.remove()});let y=null;c.forEach((m,b)=>{let a=o.get(m.id);(!a||t)&&(a&&a.remove(),a=l(m),!a)||(b===0?e.firstElementChild!==a&&e.prepend(a):y&&y.nextElementSibling!==a&&y.after(a),y=a)}),we(2,c.length),ct()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function xt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await I(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let l=c.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),lt(n)}catch{}}function ps(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{vt.forEach(s=>de.add(s.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await I("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var yt=()=>`
...
`;function wt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Ct(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=ms,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),Me(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();Ue(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=wt(t.id,t.username);Ue(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=wt(t.id,t.username);Ue(e,t,s)})}function Ue(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let E=(i.badges||[]).map(A=>`<span class="profile-badge ${A==="Creator"?"master":""}">${A}</span>`).join(""),h=(i.attributes||i.facts||[]).map(A=>{let _=A.key||A.k,S=A.value||A.v;return`
        ...
        `},c=()=>{let E=i.dossier||{},h=E.identity||{},f=E.career||{},r=E.personal||{},d=E.social||[];return`
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.hobbies||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.habits||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(r.vices||[]).map(u=>`<div class="dossier-list-item" style="color: #cf6679;">${u}</div>`).join("")}
                        </div>
                    </div>
                </div>
...
`,us=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);Ct(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(us=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,v)=>{let o=c[l.level]??10,p=c[v.level]??10;return o!==p?o-p:l.username.localeCompare(v.username)}),t.innerHTML=n.map(l=>{let v=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",o=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",p="#888";l.level==="Me"||l.level==="Master"?p="#bb86fc":l.level==="Admin"?p="#cf6679":l.level==="Moderator"?p="#03dac6":l.level==="Contributor"&&(p="#ffa500");let y=l.level==="Me",m=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
...
`,_e=null,me=new Set,Ce=[],K="all",gs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},bs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function vs(t){for(let[e,i]of Object.entries(gs))if(i.includes(t))return e;return"system"}function xs(t){return bs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;ys();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await I(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,E=vs(g),h=xs(g),f=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),r="event-border-grey";f&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?r="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?r="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?r="event-border-orange":g==="system.test.completed"?r=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":r="event-border-blue");let d=f?"cursor-pointer":"",u=me.has(b.id),w=u?"expanded":"",A=u?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),S=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),x=Et(g,a),L=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",B="";if(f){let T="";if(g==="engagement.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                    ...
                    `}else if(g==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,C=a.timestamp-a.last_active,M=Math.floor(C/60);T=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,C="";a.eval_count&&(C=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let O=a.chat_history.length,D=a.chat_history.map((U,q)=>{let F=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&F==="ASSISTANT"&&(F="DEXTER");let W=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",V=q===O-1?"block":"none";return`
                                <div class="history-slide" data-index="${q}" style="display: ${V};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${W}; letter-spacing: 1px; font-weight: bold;">${F}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${q+1} of ${O}</span>
                                    </div>
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,C=a.signals||{};T=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.visual.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,C="";a.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(C=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let $=a.success?"#03dac6":"#ff4d4d",C=a.success?"SUCCESS":"FAILED",M=F=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${F}</h5>`,O="";a.error&&(O=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(a.error)}</pre>
                            </div>
                        `);let D="";if(a.chat_history&&a.chat_history.length>0){let F=a.chat_history.length,W=a.chat_history.map((V,ue)=>{let Q=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Q==="USER"&&(Q="SYSTEM"),!V.name&&Q==="ASSISTANT"&&(Q="AGENT");let Z=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ue+1} of ${F}</span>
                                    </div>
                        ...
                        `}let U="";if(a.corrections&&a.corrections.length>0){let F=a.corrections.map((W,V)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${W.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${P(W.snippet)}</div>`:""}
                            </div>
                        `).join("");U=`
                            <div class="event-detail-block">
                        ...
                        `}let q="";if(a.parsed_results&&a.parsed_results.length>0){let F=a.parsed_results.map(W=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${P(W.title)}</div>
                                <div style="color: #ddd;">${P(W.summary)}</div>
                            </div>
                        `).join("");q=`
                            <div class="event-detail-block">
                                ${M("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${C} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${a.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${U}
                        ${D}
                    `}else if(g==="system.test.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let $="";a.attachments&&a.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${a.attachments.map(M=>{let O=M.content_type&&M.content_type.startsWith("image/"),D=(M.size/1024).toFixed(1)+" KB";return`
...
`,St=hs;function kt(){return`
    ...
    `}var ws=null;async function $e(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await I("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let s=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],n=i.filter(l=>!s.includes(l.id));n.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),n.reverse();let c=n.map(l=>{let v=l.logs.join(`
`),o=[...l.logs];if(o.length<25){let y=25-o.length;for(let m=0;m<y;m++)o.push("")}else o.length>25&&(o=o.slice(-25));let p=o.map(y=>Le(y)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${kt()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(r.GPU&&r.GPU.length>0?c.innerHTML=r.GPU.map(d=>{let u=(d.VRAM/1073741824).toFixed(1);return`
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let r=await Mt();r?(v(r),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let r=await Mt();v(r)}if(!t)return;let o=await Ht();if(!o||!o.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}Tt=Date.now(),X(0,Tt);let p=o.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function y(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function m(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:r.split(".").slice(0,3).join(".")||"-"}function b(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function a(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let u=parseInt(d[1])||0,w=parseInt(d[2])||0,A=parseInt(d[3])||0,_=parseFloat(d[4])||0,S=u*86400+w*3600+A*60+_,k=Math.floor(S/86400);if(k>0)return`${k}d`;let x=Math.floor(S/3600);if(x>0)return`${x}h`;let L=Math.floor(S/60);return L>0?`${L}m`:`${Math.floor(S)}s`}function g(r){let d=r.status==="online",u=d?"service-widget-online":"service-widget-offline",w=d?"bx-check-circle":"bx-x-circle",A=d?"OK":"BAD",_=r.version?m(r.version.str):"-",S=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",k=r.memory&&r.memory!=="N/A"?r.memory:"-",x=S!=="-"?"#00ff00":"#666",L=S!=="-"?"#fff":"#666",B=k!=="-"?"#00ff00":"#666",H=k!=="-"?"#fff":"#666",J=a(r.uptime),G="";d?G=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${L};">${S}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${B};"></i>
                        <span style="color: ${H};">${k}</span>
                    </div>
                </div>`:G='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let j=z()?"easter.company":b(r.domain&&r.port?`${r.domain}:${r.port}`:"");return`<div class="service-widget ${u}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${w}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${A}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${j}</span></div>${G}</div></div>`}let E=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),h=new Set(p.map(r=>r.id));for(let[r,d]of E)h.has(r)||d.remove();let f=new Set;p.forEach(r=>{if(f.has(r.id))return;f.add(r.id);let d=g(r),u=E.get(r.id);u&&u.parentNode?u.outerHTML!==d&&(u.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function Ge(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ht();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Lt=Date.now(),X(2,Lt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function n(o){let p=o.status==="Ready";return`
                ...
                </div>`}function l(o){let p=o.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",b=p&&o.size>0?`${(o.size/1e9).toFixed(2)} GB`:"-",a=o.name;return o.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${o.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${o.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(s&&(v+=n(s)),e.xtts&&(v+=c(e.xtts)),v+=i.map(l).join(""),!v){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function Ee(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),o=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Ee()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{o.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{o.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{o.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Ee()}catch{o.innerHTML="<i class='bx bx-error'></i>"}},o.dataset.listenerAttached="true");let p=await $s();if(p){let h=Math.floor(Date.now()/1e3),f=p.active_tier,r=p.active_synthesis,d=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},u=x=>{x<0&&(x=0);let L=Math.floor(x/3600),B=Math.floor(x%3600/60),H=x%60;return L>0?`${L}h ${B}m`:B>0?`${B}m ${H}s`:`${H}s`},w=(x,L,B,H,J)=>{if(!x)return;let G=d[H]||H.toUpperCase(),j=x.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(j&&(j.textContent=G),J&&J.includes(H))x.textContent="Working",x.style.color="#bb86fc";else if(B){let $=B.next_run-h;if($<=0)x.textContent="Ready",x.style.color="#5eff5e";else{let C=Math.floor($/60),M=$%60;x.textContent=`${C}m ${M}s`,x.style.color="#fff"}}L&&B&&(L.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${B.attempts||0}</span>
            <span style="color: ${B.failures>0?"#ffa500":"#666"}">Fails: ${B.failures||0}</span>
            <span style="color: ${B.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${B.absolute_failures||0}</span>
          </div>
        `)};e&&w(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",f),i&&w(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",f);let A=document.getElementById("analyzer-synthesis-val"),_=document.getElementById("analyzer-synthesis-stats");A&&w(A,_,p.synthesis,"synthesis",r);let S=document.getElementById("system-state-label"),k=document.getElementById("system-state-val");if(k&&p.system_state){let x=p.system_state,L=u(p.system_state_time||0);S&&(S.textContent=`State: ${x.toUpperCase()}`),k.textContent=L,x==="idle"?k.style.color=p.system_state_time>300?"#5eff5e":"#fff":k.style.color="#bb86fc"}n&&(n.textContent=u(p.total_idle_time||0)),c&&(c.textContent=u(p.total_active_time||0)),l&&(l.textContent=u(p.total_waste_time||0))}else[e,i,s,n,c,l].forEach(f=>{f&&(f.textContent="-",f.style.color="#666")});let y=await Cs(),m=[],b=[],a=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],b=y.queue||[],a=y.history||[],a.sort((h,f)=>(f.end_time||0)-(h.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Fe(t,m,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Fe(g,b,!1)));let E=document.getElementById("processes-history-widgets");E&&(a.length===0?E.querySelector(".tab-placeholder")||(E.innerHTML=N("empty","No recent history.")):(E.querySelector(".tab-placeholder")&&(E.innerHTML=""),Fe(E,a,!0))),we(1,m.length)}function Fe(t,e,i){function s(o){let p="";o.end_time?p=`${o.end_time-o.start_time}s`:p=`${Math.floor(Date.now()/1e3-o.start_time)}s`;let y=o.retries>0?`<span class="process-retry-badge">Retry ${o.retries}</span>`:"",m=o.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[m]?m=b[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),i)return`
    ...
    `;async function Ot(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Pt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Pt(t)}async function Pt(t){try{let e=await I("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=N("empty","No web history found.");return}let s=`
        ...
        `;i.forEach((v,o)=>{let p=new Date(v.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=v.screenshot?`<img src="data:image/png;base64,${v.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ve=!0,i}function Ut(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ve=!1}function jt(t,e,i="output"){if(!Ve)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function ks(t){let e=qt.find(n=>n.id===t);if(!e)return;let i=Ss(e);jt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(l=>setTimeout(l,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),jt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ft(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Es();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;ks(s)})})}async function Ts(){if(!z())try{if(!(await I("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Jt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}Qe(),et(),Ts(),at(),Xe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Ft();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=f=>{localStorage.setItem("dex_last_window",f)};function n(){for(;e.length>1;)e.shift().close(!0);let f=document.getElementById("windows-container"),r=document.querySelector("nav"),d=document.querySelector("footer"),u=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");f&&f.setAttribute("data-count",e.length),e.forEach(L=>{let B=document.getElementById(L.id);B&&B.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon"),k=document.getElementById("close-all-windows"),x=window.innerWidth<880;if(!x){let L=document.getElementById("dexter-dropdown"),B=document.getElementById("dexter-menu-btn");L&&L.classList.remove("active"),B&&B.classList.remove("active"),f&&f.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),k&&(k.style.display=x?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),r?.classList.add("window-open"),f&&(f.style.paddingTop="60px"),A&&(A.style.display=x?"flex":"none"),S&&(S.style.display=x?"block":"none"),!x&&_){let L=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(L)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(o)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(p)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(m)})):_.innerHTML=""}else _&&(_.innerHTML="");else r?.classList.remove("window-open"),k&&(k.style.display="none"),f&&(f.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),u||w?d?.classList.remove("hide"):d?.classList.add("hide"),A&&(A.style.display="flex"),S&&(S.style.display="block"),_&&(_.innerHTML="");ve()}function c(f){if(f.isOpen()){f.close();return}for(;e.length>0;)e.pop().close(!0);e.push(f),f.open(),n()}function l(){[...e].forEach(r=>r.close()),e=[]}window.addEventListener("resize",n);let v=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:pt(),onOpen:()=>ie(),onClose:()=>{let f=e.indexOf(v);f>-1&&e.splice(f,1),n()}}),o=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{o.setContent(qe()),ne()},onClose:()=>{let f=e.indexOf(o);f>-1&&e.splice(f,1),n()}}),p=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:$t(),onOpen:()=>je(),onClose:()=>{let f=e.indexOf(p);f>-1&&e.splice(f,1),n()}}),y=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Bt()},{icon:"bxs-component",title:"Processes",content:It()},{icon:"bxs-brain",title:"Models",content:_t()},{icon:"bx-globe",title:"Web",content:Rt()},{icon:"bxs-hdd",title:"Hardware",content:Dt()},{icon:"bxs-terminal",title:"Logs",content:zt()},{icon:"bxs-zap",title:"Agents",content:At()},{icon:"bx-task",title:"Chores",content:ht()}].filter(f=>z()?f.title!=="Hardware"&&f.title!=="Logs":!0),onOpen:()=>{Je(),Ee(),Ge(),Ot(),$e(),ye()},onClose:()=>{let f=e.indexOf(y);f>-1&&e.splice(f,1),n()}}),m=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:yt(),onOpen:()=>Oe(),onClose:()=>{let f=e.indexOf(m);f>-1&&e.splice(f,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{b.setContent(De()),We(b)},onClose:()=>{let f=e.indexOf(b);f>-1&&e.splice(f,1),n()}});window.dexter={viewEvent:f=>{o.isOpen()||c(o),setTimeout(()=>{let r=document.querySelector(`.event-item[data-event-id="${f}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)},viewAlert:f=>{v.isOpen()||c(v),setTimeout(()=>{let r=document.querySelector(`.event-item[data-alert-id="${f}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let f=document.getElementById("dexter-dropdown"),r=document.getElementById("dexter-menu-btn");f&&r&&(f.classList.remove("active"),r.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let f=document.getElementById("dexter-menu-btn"),r=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),u=document.getElementById("nav-left-container"),w=document.getElementById("dexter-dropdown"),A=document.getElementById("windows-container");f&&w&&(f.onclick=S=>{S.stopPropagation();let k=window.innerWidth<880;w.classList.toggle("active"),f.classList.toggle("active");let x=w.classList.contains("active");if(k){let L=document.querySelector("nav");x?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),L?.classList.add("window-open"),A?.classList.add("menu-open"),fe(!0)):(A?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),L?.classList.remove("window-open"),fe(!1)))}}),r&&(r.onclick=S=>{S.stopPropagation(),c(b)}),d&&(d.onclick=S=>{S.stopPropagation(),l()});let _=(S,k,x)=>{let L=document.getElementById(S);L&&(L.onclick=B=>{B.stopPropagation(),a(),x&&s(x),c(k)})};_("alerts-menu-item",v,"alerts-window"),_("events-menu-item",o,"events-window"),_("monitor-menu-item",y,"monitor-window"),_("contacts-menu-item",p,"contacts-window"),_("workspace-menu-item",m,"workspace-window"),u&&(u.onclick=S=>{if(S.stopPropagation(),w&&w.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)l();else{let k=window.location.pathname;if(!(k==="/"||k==="/index.html")){let B=(k.endsWith("/")&&k.length>1?k.slice(0,-1):k).split("/");B.pop();let H=B.join("/")||"/";window.location.href=H}}}),document.addEventListener("click",()=>{let S=window.innerWidth<880;w&&w.classList.contains("active")&&(a(),S&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),w&&(w.onclick=S=>S.stopPropagation())})();let E=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!E&&!h&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{o.isOpen()&&ne(),v.isOpen()?ie():Re()},1e3),setInterval(()=>{z()&&(v.isOpen()?ie():Re()),m.isOpen()?Oe():xt(),z()&&o.isOpen()&&ne(),y.isOpen()&&Nt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jt):Jt();})();

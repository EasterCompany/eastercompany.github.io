(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function We(t=!1){let e=window.location.pathname,i=e==="/"||e==="/index.html",o=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ye=1e3;function te(t){let e=null,i=t.onClose||null,a=t.onOpen||null;function n(){e&&(e.style.zIndex=++Ye)}function o(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),a&&setTimeout(a,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ye,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",s="",g="",E;t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((v,r)=>{let d=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,E=`<div class="window-content">${t.tabs.map((v,r)=>`<div class="tab-content ${r===0?"active":""}" data-tab-content="${r}">${v.content}</div>`).join("")}</div>`):(t.title&&(g=`<div class="window-title">${t.title}</div>`),E=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Me=null,ie=new Set,Ae=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Yt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await _(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];Me=Date.now(),J(0,Me);let l=Date.now(),u=24*60*60*1e3,f=o.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let h=parseInt(d);return l-h<u});f.sort((r,d)=>{let h=I=>{let w=I.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return"low"}return(w.priority||"low").toLowerCase()},M=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,A=M(h(r)),L=M(h(d));return A!==L?L-A:d.timestamp-r.timestamp}),Ae=f;let p=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(f.map(r=>p(r))).size>1;if(f.length>0){let r=null;f.forEach(d=>{let h=p(d);b&&h!==r&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),r=h),x.push(d)})}if(t&&(e.innerHTML=""),f.length===0){e.innerHTML=N("empty","No alerts yet."),Se();return}let s=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let h=(d.title||"Untitled Alert").trim(),M=(d.body||"No description provided.").trim(),A=d.summary||"",L=d.content||"",I=d.protocol||"",w=(d.priority||"low").toLowerCase(),H=!!d.alert,D=(d.category||"system").trim(),z=d.related_event_ids||[],O=d.audit_event_id,U=!!localStorage.getItem(`alert_read_ts_${r.id}`),S=new Date(r.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=U?"event-border-grey":"event-border-blue";!U&&H&&(T="event-border-red"),U&&(w==="high"||w==="critical")?T="event-border-red":U&&w==="medium"&&(T="event-border-orange");let k=U?"alert-read":"alert-unread",R=ie.has(r.id),P=R?"expanded":"",Q=R?"display: block;":"display: none;",j="",q="";z.length>0&&(q=`
...
`,ae=new Set,pt=[],ge=null;async function oe(t=!1){let e=document.getElementById("roadmap-list");if(e){Xt();try{let i=await _("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();pt=a;let n=p=>{let x=ae.has(p.id),m=p.state==="published",b=p.state==="consumed",s="event-border-grey";m&&(s="event-border-blue"),b&&(s="event-border-purple");let E=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${s} cursor-pointer ${x?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=r=>{if(r.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",ae.add(p.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;y.innerHTML=`
      ...
      `;let v=y.querySelector(".event-details");return v&&v.addEventListener("click",r=>{r.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>Qt(p)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>es(p)),y.querySelector(".delete-btn")?.addEventListener("click",()=>ts(p.id)),y.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(p.id)}),y},o=Array.from(e.children),l=new Map(o.map(p=>[p.dataset.itemId,p])),u=new Set(a.map(p=>p.id));if(o.forEach(p=>{let x=p.dataset.itemId;(!x||!u.has(x))&&p.remove()}),a.length===0){e.innerHTML=N("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let f=null;a.forEach((p,x)=>{let m=l.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(x===0?e.firstElementChild!==m&&e.prepend(m):f&&f.nextElementSibling!==m&&f.after(m),f=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function Xt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ge=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ge=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-input").value;if(!o.trim())return;let l=ge?`/roadmap/${ge}`:"/roadmap",u=ge?"PATCH":"POST";try{await _(l,{method:u,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})}),document.getElementById("roadmap-editor-container").style.display="none",oe(!0)}catch(f){console.error(f)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{pt.forEach(o=>ae.add(o.id)),oe(!0)},a.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ae.clear(),oe(!0)},n.dataset.listenerAttached="true")}function Qt(t){ge=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function es(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),oe(!0)}catch(i){console.error(i)}}async function ts(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),ae.delete(t),oe(!0)}catch(e){console.error(e)}}var ut=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,mt=null,re=new Set,gt=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ss();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await _(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(gt=o,mt=Date.now(),J(2,mt),o.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),xe(2,0);return}t&&(e.innerHTML="");let l=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let s=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),y=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(k=>k.trim()),v=(b.implementation_path||[]).map(k=>k.trim()),r=b.source_event_ids||[],d=b.approved===!0,h=new Date(m.timestamp*1e3),M=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=re.has(m.id),I=L?"display: block;":"display: none;",w=document.createElement("div");w.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""} ${d?"blueprint-approved":""}`,w.dataset.blueprintId=m.id,d&&(w.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",w.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let D=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";w.onclick=function(k){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(m.id);let P=this.querySelector(".event-details");P&&(P.style.display="none")}else{this.classList.add("expanded"),re.add(m.id);let P=this.querySelector(".event-details");P&&(P.style.display="block")}};let z="";v.length>0&&(z=`
            ...
            `;w.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${I}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${B(g)}</div>
            ...
            `;let S=w.querySelector(".blueprint-approve-btn");S&&(S.onclick=async k=>{k.stopPropagation(),S.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&le(!0)}catch(R){console.error("Failed to approve blueprint:",R)}});let C=w.querySelector(".blueprint-delete-btn");C&&(C.onclick=async k=>{k.stopPropagation();let R=!d;C.innerHTML=R?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${m.id}`,{method:"DELETE"})).ok&&le(!0)}catch(P){console.error("Failed to delete blueprint:",P)}});let $=w.querySelector(".event-details");$&&$.addEventListener("click",k=>{k.stopPropagation()});let T=w.querySelector(".close-details-btn");return T&&T.addEventListener("click",k=>{k.stopPropagation(),w.classList.remove("expanded");let R=w.querySelector(".event-details");R&&(R.style.display="none"),re.delete(m.id)}),w},u=Array.from(e.children),f=new Map(u.map(m=>[m.dataset.blueprintId,m])),p=new Set(o.map(m=>m.id));u.forEach(m=>{let b=m.dataset.blueprintId;(!b||!p.has(b))&&m.remove()});let x=null;o.forEach((m,b)=>{let s=f.get(m.id);(!s||t)&&(s&&s.remove(),s=l(m),!s)||(b===0?e.firstElementChild!==s&&e.prepend(s):x&&x.nextElementSibling!==s&&x.after(s),x=s)}),xe(2,o.length),ot()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(o=>{let l=o.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),at(n)}catch{}}function ss(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{gt.forEach(a=>re.add(a.id)),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{re.clear(),le(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),re.clear(),le(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var vt=()=>`
...
`,ns=null;async function Ne(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Ne(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let a=i.target.closest(".user-profile-card");if(a)try{let n=JSON.parse(a.dataset.user);yt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Le("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(ns=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,u)=>{let f=o[l.level]??10,p=o[u.level]??10;return f!==p?f-p:l.username.localeCompare(u.username)}),t.innerHTML=n.map(l=>{let u=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",f=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",p="#888";l.level==="Me"||l.level==="Master"?p="#bb86fc":l.level==="Admin"?p="#cf6679":l.level==="Moderator"?p="#03dac6":l.level==="Contributor"&&(p="#ffa500");let x=l.level==="Me",m=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=x?"2px solid #bb86fc":`1px solid ${u}33`;return`
...
`,Ie=null,ce=new Set,ye=[],F="all",os={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ls(t){for(let[e,i]of Object.entries(os))if(i.includes(t))return e;return"system"}function cs(t){return rs[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;ds();let a=`/events?ml=${F==="all"?100:250}&format=json`;F!=="all"&&(a+=`&category=${F}`);try{let n=await _(a);if(!n.ok)throw new Error("Service unreachable");if(ye=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let g=s.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),Ie=Date.now(),J(1,Ie),ye.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let g=s.type,E=ls(g),y=cs(g),c=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),v="event-border-grey";c&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?v="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?v="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?v="event-border-orange":g==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=ce.has(b.id),h=d?"expanded":"",M=d?"display: block;":"display: none;",A=new Date(b.timestamp*1e3),L=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=wt(g,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",D="";if(c){let S="";if(g==="engagement.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                    ...
                    `}else if(g==="system.attention.expired"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$=s.timestamp-s.last_active,T=Math.floor($/60);S=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$="";s.eval_count&&($=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$=s.signals||{};S=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.router.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let C=s.success?"#03dac6":"#ff4d4d",$=s.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,k="";s.error&&(k=`
                            <div class="event-detail-block">
                                ${T("Error")}
                        ...
                        ${P}
                        ${R}
                    `}else if(g==="system.test.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let C="";s.attachments&&s.attachments.length>0&&(C=`
                            <div class="event-detail-block">
...
`),f=[...l.logs];if(f.length<25){let x=25-f.length;for(let m=0;m<x;m++)f.push("")}else f.length>25&&(f=f.slice(-25));let p=f.map(x=>Te(x)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=o,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let u=unescape(l.dataset.logs);navigator.clipboard.writeText(u).then(()=>{let f=l.querySelector("i");f.classList.remove("bx-copy"),f.classList.add("bx-check"),setTimeout(()=>{f.classList.remove("bx-check"),f.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let u=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${u}?`))try{(await _(`/logs?service_id=${u}`,{method:"DELETE"})).ok&&he()}catch(f){console.error("Error clearing logs:",f)}})}),ms=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Tt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${$t()}
        </div>`;async function Bt(){await Promise.all([we(),Oe(),Ue()]),await he()}var Et=null,kt=null;async function Dt(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function St(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function us(){try{return await(await _("/processes")).json()}catch{return null}}async function fs(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Oe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),o=document.querySelector("#hw-gpu .hw-content"),l=document.querySelector("#hw-storage .hw-content"),u=c=>{if(c){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await St();c?(u(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await St();u(c)}if(!t)return;let f=await Dt();if(!f||!f.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}Et=Date.now(),J(0,Et);let p=f.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function x(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function m(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function b(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function s(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let r=parseInt(v[1])||0,d=parseInt(v[2])||0,h=parseInt(v[3])||0,M=parseFloat(v[4])||0,A=r*86400+d*3600+h*60+M,L=Math.floor(A/86400);if(L>0)return`${L}d`;let I=Math.floor(A/3600);if(I>0)return`${I}h`;let w=Math.floor(A/60);return w>0?`${w}m`:`${Math.floor(A)}s`}function g(c){let v=c.status==="online",r=v?"service-widget-online":"service-widget-offline",d=v?"bx-check-circle":"bx-x-circle",h=v?"OK":"BAD",M=c.version?m(c.version.str):"-",A=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",L=c.memory&&c.memory!=="N/A"?c.memory:"-",I=A!=="-"?"#00ff00":"#666",w=A!=="-"?"#fff":"#666",H=L!=="-"?"#00ff00":"#666",D=L!=="-"?"#fff":"#666",z=s(c.uptime),O="";return v?O=`
                ...
                </div>`}function o(f){let p=f.status==="Ready";return`
                ...
                </div>`}function l(f){let p=f.status==="Downloaded",x=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",b=p&&f.size>0?`${(f.size/1e9).toFixed(2)} GB`:"-",s=f.name;return f.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${x}" data-model-name="${f.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${f.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let u="";if(a&&(u+=n(a)),e.xtts&&(u+=o(e.xtts)),u+=i.map(l).join(""),!u){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==u&&(t.innerHTML=u)}async function we(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),u=document.getElementById("guardian-reset-btn"),f=document.getElementById("analyzer-reset-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),we()}catch{u.innerHTML="<i class='bx bx-error'></i> Failed"}},u.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),we()}catch{f.innerHTML="<i class='bx bx-error'></i> Failed"}},f.dataset.listenerAttached="true");let p=await fs();if(p){let y=Math.floor(Date.now()/1e3),c=p.active_tier,v=p.active_synthesis,r=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},d=w=>{w<0&&(w=0);let H=Math.floor(w/3600),D=Math.floor(w%3600/60),z=w%60;return H>0?`${H}h ${D}m`:D>0?`${D}m ${z}s`:`${z}s`},h=(w,H,D,z,O)=>{if(!w)return;let V=r[z]||z.toUpperCase(),U=w.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(U&&(U.textContent=V),O&&O.includes(z))w.textContent="Working",w.style.color="#bb86fc";else if(D){let C=D.next_run-y;if(C<=0)w.textContent="Ready",w.style.color="#5eff5e";else{let $=Math.floor(C/60),T=C%60;w.textContent=`${$}m ${T}s`,w.style.color="#fff"}}H&&D&&(H.innerHTML=`
        ...
        `)};e&&h(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",c),i&&h(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",c);let M=document.getElementById("analyzer-synthesis-val"),A=document.getElementById("analyzer-synthesis-stats");M&&h(M,A,p.synthesis,"synthesis",v);let L=document.getElementById("system-state-label"),I=document.getElementById("system-state-val");if(I&&p.system_state){let w=p.system_state,H=d(p.system_state_time||0);L&&(L.textContent=`State: ${w.toUpperCase()}`),I.textContent=H,w==="idle"?I.style.color=p.system_state_time>300?"#5eff5e":"#fff":I.style.color="#bb86fc"}n&&(n.textContent=d(p.total_idle_time||0)),o&&(o.textContent=d(p.total_active_time||0)),l&&(l.textContent=d(p.total_waste_time||0))}else[e,i,a,n,o,l].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let x=await us(),m=[],b=[],s=[];x&&(Array.isArray(x)?m=x:(m=x.active||[],b=x.queue||[],s=x.history||[],s.sort((y,c)=>(c.end_time||0)-(y.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Pe(t,m,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Pe(g,b,!1)));let E=document.getElementById("processes-history-widgets");E&&(s.length===0?E.querySelector(".tab-placeholder")||(E.innerHTML=N("empty","No recent history.")):(E.querySelector(".tab-placeholder")&&(E.innerHTML=""),Pe(E,s,!0))),xe(1,m.length)}function Pe(t,e,i){function a(u){let f="";u.end_time?f=`${u.end_time-u.start_time}s`:f=`${Math.floor(Date.now()/1e3-u.start_time)}s`;let p=u.retries>0?`<span class="process-retry-badge">Retry ${u.retries}</span>`:"",x=u.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[x]?x=m[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),i)return`
        <div class="process-history-item" data-channel-id="${u.channel_id}-${u.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${u.channel_id}-${u.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${b}">${f}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let n=i?".process-history-item":".process-widget",o=new Map(Array.from(t.querySelectorAll(n)).map(u=>[u.dataset.channelId,u])),l=new Set(e.map(u=>`${u.channel_id}-${u.start_time}`));for(let[u,f]of o)l.has(u)||f.remove();e.forEach(u=>{let f=`${u.channel_id}-${u.start_time}`,p=a(u),x=o.get(f);if(x){x.outerHTML!==p&&(x.outerHTML=p);let m=t.querySelector(`[data-channel-id="${f}"]`);m&&t.appendChild(m)}else t.insertAdjacentHTML("beforeend",p)})}function _e(){let t=ve(),e=Qe()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`

...
`,Me=null,re=new Set,Ae=[];async function se(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ts(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await D(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let l=(await a.json()).events||[];Me=Date.now(),K(0,Me);let o=Date.now(),v=24*60*60*1e3,c=l.filter(u=>{let g=localStorage.getItem(`alert_read_ts_${u.id}`);if(!g)return!0;let d=parseInt(g);return o-d<v});c.sort((u,g)=>{let d=A=>{let w=A.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return"low"}return(w.priority||"low").toLowerCase()},E=A=>A==="critical"?4:A==="high"?3:A==="medium"?2:1,L=E(d(u)),M=E(d(g));return L!==M?M-L:g.timestamp-u.timestamp}),Ae=c;let p=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},h=[],b=new Set(c.map(u=>p(u))).size>1;if(c.length>0){let u=null;c.forEach(g=>{let d=p(g);b&&d!==u&&(h.push({id:`divider-${d}`,type:"divider",label:d.toUpperCase()}),u=d),h.push(g)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=O("empty","No alerts yet."),ke();return}let n=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let d=(g.title||"Untitled Alert").trim(),E=(g.body||"No description provided.").trim(),L=g.summary||"",M=g.content||"",A=g.protocol||"",w=(g.priority||"low").toLowerCase(),R=!!g.alert,N=(g.category||"system").trim(),I=g.related_event_ids||[],U=g.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${u.id}`),k=new Date(u.timestamp*1e3),$=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=j?"event-border-grey":"event-border-blue";!j&&R&&(T="event-border-red"),j&&(w==="high"||w==="critical")?T="event-border-red":j&&w==="medium"&&(T="event-border-orange");let P=j?"alert-read":"alert-unread",_=re.has(u.id),F=_?"expanded":"",J=_?"display: block;":"display: none;",G="",W="";I.length>0&&(W=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(M)}</div>
            </div>
        `:X=`
            <div class="event-detail-block" style="text-align: left;">
            ...
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${T} ${P} ${F} cursor-pointer priority-${w}`,Z.dataset.alertId=u.id,Z.onclick=function(te){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(u.id);let ye=this.querySelector(".event-details");ye&&(ye.style.display="none")}else{this.classList.add("expanded"),re.add(u.id);let ye=this.querySelector(".event-details");if(ye&&(ye.style.display="block"),!localStorage.getItem(`alert_read_ts_${u.id}`)){localStorage.setItem(`alert_read_ts_${u.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ze="event-border-grey";w==="high"||w==="critical"?ze="event-border-red":w==="medium"&&(ze="event-border-orange"),this.classList.add(ze),ke()}}};let Ve=`${A?A.toUpperCase():"GUARDIAN"} ALERT: ${L||d}`,Ut={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[N]||"bx-bell",qt=w==="high"||w==="critical"?"#ff4d4d":w==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${qt}; font-size: 0.8em; margin-left: 5px;">[${w.toUpperCase()}]</span>
                    </div>
...
`,le=new Set,Ie=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ss();try{let i=await D("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();Ie=a;let s=p=>{let h=le.has(p.id),m=p.state==="published",b=p.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),b&&(n="event-border-purple");let S=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${n} cursor-pointer ${h?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=u=>{if(u.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",le.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",le.add(p.id))};let r=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;y.innerHTML=`
      ...
      `;let x=y.querySelector(".event-details");return x&&x.addEventListener("click",u=>{u.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>is(p)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ns(p)),y.querySelector(".delete-btn")?.addEventListener("click",()=>as(p.id)),y.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",le.delete(p.id)}),y},l=Array.from(e.children),o=new Map(l.map(p=>[p.dataset.itemId,p])),v=new Set(a.map(p=>p.id));if(l.forEach(p=>{let h=p.dataset.itemId;(!h||!v.has(h))&&p.remove()}),!Ie||Ie.length===0){e.innerHTML=O("empty","Your roadmap is empty.",B()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;a.forEach((p,h)=>{let m=o.get(p.id);(!m||t)&&(m&&m.remove(),m=s(p),!m)||(h===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=O("offline","Failed to load roadmap.","The event service may be offline."))}}}function ss(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input").value;if(!l.trim())return;let o=xe?`/roadmap/${xe}`:"/roadmap",v=xe?"PATCH":"POST";try{await D(o,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(c){console.error(c)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{Ie.forEach(l=>le.add(l.id)),ce(!0)},a.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{le.clear(),ce(!0)},s.dataset.listenerAttached="true")}function is(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function ns(t){let e=t.state==="published"?"draft":"published";try{await D(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function as(t){if(confirm("Delete this roadmap item?"))try{await D(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var ft=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,ut=null,de=new Set,bt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;os();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await D(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let l=(await a.json()).events||[];if(bt=l,ut=Date.now(),K(2,ut),l.length===0){e.innerHTML=O("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),S=(b.content||"").trim(),y=(b.category||"architecture").trim(),r=(b.related_services||b.affected_services||[]).map(_=>_.trim()),x=(b.implementation_path||[]).map(_=>_.trim()),u=b.source_event_ids||[],g=b.approved===!0,d=new Date(m.timestamp*1e3),E=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=de.has(m.id),A=M?"display: block;":"display: none;",w=document.createElement("div");w.className=`event-item notification-item event-border-purple cursor-pointer ${M?"expanded":""} ${g?"blueprint-approved":""}`,w.dataset.blueprintId=m.id,g&&(w.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",w.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let N=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";w.onclick=function(_){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let I="";x.length>0&&(I=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${x.map(_=>`<li style="margin-bottom: 5px;">${z(_)}</li>`).join("")}</ul></div>
                    </div>
                `);let U="";u.length>0&&(U=`
                ...
                `);let q=r.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${r.join(", ")}</span></div>`:"<div></div>",j=B(),k=g?`
            ...
            `;w.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${A}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${z(f)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${z(S)}</div>
                        </div>
                        ${I}
                        ${U}
                        ${k}
            ...
            `;let $=w.querySelector(".blueprint-approve-btn");$&&($.onclick=async _=>{_.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await D(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(F){console.error("Failed to approve blueprint:",F)}});let C=w.querySelector(".blueprint-delete-btn");C&&(C.onclick=async _=>{_.stopPropagation();let F=!g;C.innerHTML=F?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await D(`/events/${m.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(J){console.error("Failed to delete blueprint:",J)}});let T=w.querySelector(".event-details");T&&T.addEventListener("click",_=>{_.stopPropagation()});let P=w.querySelector(".close-details-btn");return P&&P.addEventListener("click",_=>{_.stopPropagation(),w.classList.remove("expanded");let F=w.querySelector(".event-details");F&&(F.style.display="none"),de.delete(m.id)}),w},v=Array.from(e.children),c=new Map(v.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));v.forEach(m=>{let b=m.dataset.blueprintId;(!b||!p.has(b))&&m.remove()});let h=null;l.forEach((m,b)=>{let n=c.get(m.id);(!n||t)&&(n&&n.remove(),n=o(m),!n)||(b===0?e.firstElementChild!==n&&e.prepend(n):h&&h.nextElementSibling!==n&&h.after(n),h=n)}),he(2,l.length),lt()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=O("offline","Failed to load blueprints.","The event service may be offline."))}}async function vt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await D(t);if(!e.ok)return;let a=(await e.json()).events||[],s=0;a.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&s++}),rt(s)}catch{}}function os(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{bt.forEach(a=>de.add(a.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await D("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var xt=()=>`
...
`,_e=null,me=new Set,we=[],V="all",ds={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ms(t){for(let[e,i]of Object.entries(ds))if(i.includes(t))return e;return"system"}function us(t){return ps[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;fs();let a=`/events?ml=${V==="all"?100:250}&format=json`;V!=="all"&&(a+=`&category=${V}`);try{let s=await D(a);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let f=n.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),_e=Date.now(),K(1,_e),we.length===0){e.innerHTML=O("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let f=n.type,S=ms(f),y=us(f),r=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="analysis.user.message_signals"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),x="event-border-grey";r&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?x="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.analysis.audit"||f==="analysis.user.message_signals"?x="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?x="event-border-orange":f==="system.test.completed"?x=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":x="event-border-blue");let u=r?"cursor-pointer":"",g=me.has(b.id),d=g?"expanded":"",E=g?"display: block;":"display: none;",L=new Date(b.timestamp*1e3),M=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Ct(f,n),R=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",N="";if(r){let k="";if(f==="engagement.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C="";n.eval_count&&(C=`
                        ...
                        `);let T="";if(n.chat_history&&n.chat_history.length>0){let P=n.chat_history.length,_=n.chat_history.map((F,J)=>{let G=F.role.toUpperCase();G==="ASSISTANT"&&(G="DEXTER");let W=F.role==="user"?"#03dac6":F.role==="system"?"#ffb74d":"#bb86fc",Y=J===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${Y};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${z(F.content)}</div>
                                </div>
                            `}).join("");T=`
                            <div class="event-detail-block">
                    ...
                    `}else if(f==="analysis.user.message_signals"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C=n.signals||{};k=`
                        ...
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${$("Raw Model Output")}
                            <pre class="detail-pre">${z(n.raw_output)||"None"}</pre>
                        </div>
                    ...
                    `}else if(f==="analysis.link.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${z(n.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${$("Description")}
                            <pre class="detail-pre">${z(n.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${$("Content Summary")}
                            <pre class="detail-pre">${z(n.summary)||"None"}</pre>
                        </div>
                    ...
                    `}else if(f==="analysis.router.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        ...
                        <div class="event-detail-block">
                            ${$("Input Context")}
                            <pre class="detail-pre">${z(n.raw_input)||"None"}</pre>
                        </div>
                    `}else if(f==="system.cli.command"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        ...
                        <div class="event-detail-block">
                            ${$("Output")}
                            <pre class="detail-pre">${z(n.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(f==="system.analysis.audit"){let $=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",T=G=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${G}</h5>`,P="";n.error&&(P=`
                            <div class="event-detail-block">
                                ${T("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${z(n.error)}</pre>
                            </div>
                        `);let _="";if(n.chat_history&&n.chat_history.length>0){let G=n.chat_history.length,W=n.chat_history.map((Y,ue)=>{let X=Y.role.toUpperCase();X==="USER"&&(X="SYSTEM"),X==="ASSISTANT"&&(X="AGENT");let Z=Y.role==="user"?"#03dac6":Y.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${z(Y.content)}</div>
                                </div>
                            `}).join("");_=`
                            <div class="event-detail-block">
                        ...
                        `}let F="";if(n.corrections&&n.corrections.length>0){let G=n.corrections.map((W,Y)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${W.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${z(W.snippet)}</div>`:""}
                            </div>
                        `).join("");F=`
                            <div class="event-detail-block">
                        ...
                        `}let J="";if(n.parsed_results&&n.parsed_results.length>0){let G=n.parsed_results.map(W=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${z(W.title)}</div>
                                <div style="color: #ddd;">${z(W.summary)}</div>
                            </div>
                        `).join("");J=`
                            <div class="event-detail-block">
                                ${T("Parsed Results")}
                    ...
                    `}else if(f==="error_occurred"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        ...
                        <div class="event-detail-block">
                            ${$("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${z(n.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${$("Context")}
                    ...
                    `}else if(f==="system.cli.status"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        ...
                        <div class="event-detail-block">
                            ${$("Message")}
                            <pre class="detail-pre">${z(n.message)}</pre>
                        </div>
                    `}else if(f==="messaging.user.sent_message"){let $="";n.attachments&&n.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(T=>{let P=T.content_type&&T.content_type.startsWith("image/"),_=(T.size/1024).toFixed(1)+" KB";return`
            ...
            `;let U=I.querySelector(".event-details");if(U&&U.addEventListener("click",k=>{k.stopPropagation()}),r){let k=I.querySelector(".close-details-btn");k&&k.addEventListener("click",$=>{$.stopPropagation();let C=$.target.closest(".event-item");if(C){C.classList.remove("expanded"),me.delete(b.id);let T=C.querySelector(".event-details");T&&(T.style.display="none")}})}let q=I.querySelector(".prev-btn"),j=I.querySelector(".next-btn");if(q&&j){let k=0,$=I.querySelectorAll(".history-slide"),C=$.length,T=()=>{$.forEach((P,_)=>{P.style.display=_===k?"block":"none"}),q.disabled=k===0,j.disabled=k===C-1,q.style.opacity=k===0?"0.5":"1",j.style.opacity=k===C-1?"0.5":"1"};q.addEventListener("click",P=>{P.stopPropagation(),k>0&&(k--,T())}),j.addEventListener("click",P=>{P.stopPropagation(),k<C-1&&(k++,T())}),T()}return I},c=Array.from(e.children),p=new Map(c.map(b=>[b.dataset.eventId,b])),h=new Set(we.map(b=>b.id));c.forEach(b=>{let n=b.dataset.eventId;(!n||!h.has(n))&&b.remove()});let m=null;we.forEach((b,n)=>{let f=p.get(b.id);(!f||t)&&(f&&f.remove(),f=v(b),!f)||(n===0?e.firstElementChild!==f&&e.prepend(f):m&&m.nextElementSibling!==f&&m.after(f),m=f)}),_e=Date.now(),K(1,_e)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=O("offline","Failed to load events.","The event service may be offline."))}}function fs(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{we.forEach(s=>me.add(s.id)),ie(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{me.clear(),ie(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),s.classList.add("active"),V=s.dataset.filter,ie(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===V?s.classList.add("active"):s.classList.remove("active")});let a=document.getElementById("events-clear");a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{let s=V==="all"?"ALL":V.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(V!=="all"?l+=`?category=${V}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await D(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");me.clear(),ie(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}var gs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${o.id}" title="Clear Logs" style="${B()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let v=unescape(o.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let c=o.querySelector("i");c.classList.remove("bx-copy"),c.classList.add("bx-check"),setTimeout(()=>{c.classList.remove("bx-check"),c.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let v=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await D(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&Ce()}catch(c){console.error("Error clearing logs:",c)}})}),bs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=O("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Lt=()=>{let t=B()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Et()}
        </div>`;async function Dt(){await Promise.all([$e(),Fe(),Je()]),await Ce()}var St=null,kt=null;async function zt(){try{return await(await D("/system_monitor")).json()}catch{return null}}async function Tt(){try{return await(await D("/system/hardware")).json()}catch{return null}}async function vs(){try{return await(await D("/processes")).json()}catch{return null}}async function xs(){try{return await(await D("/agent/status")).json()}catch{return null}}async function Fe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),v=r=>{if(r){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let r=await Tt();r?(v(r),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let r=await Tt();v(r)}if(!t)return;let c=await zt();if(!c||!c.services){t.children.length===0&&(t.innerHTML=O("offline","Failed to load system metrics.","The event service may be offline."));return}St=Date.now(),K(0,St);let p=c.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function h(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function m(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:r.split(".").slice(0,3).join(".")||"-"}function b(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function n(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let u=parseInt(x[1])||0,g=parseInt(x[2])||0,d=parseInt(x[3])||0,E=parseFloat(x[4])||0,L=u*86400+g*3600+d*60+E,M=Math.floor(L/86400);if(M>0)return`${M}d`;let A=Math.floor(L/3600);if(A>0)return`${A}h`;let w=Math.floor(L/60);return w>0?`${w}m`:`${Math.floor(L)}s`}function f(r){let x=r.status==="online",u=x?"service-widget-online":"service-widget-offline",g=x?"bx-check-circle":"bx-x-circle",d=x?"OK":"BAD",E=r.version?m(r.version.str):"-",L=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",M=r.memory&&r.memory!=="N/A"?r.memory:"-",A=L!=="-"?"#00ff00":"#666",w=L!=="-"?"#fff":"#666",R=M!=="-"?"#00ff00":"#666",N=M!=="-"?"#fff":"#666",I=n(r.uptime),U="";x?U=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${R};"></i>
                        <span style="color: ${N};">${M}</span>
                    </div>
                </div>`:U='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let q=B()?"easter.company":b(r.domain&&r.port?`${r.domain}:${r.port}`:"");return`<div class="service-widget ${u}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${q}</span></div>${U}</div></div>`}let S=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),y=new Set(p.map(r=>r.id));for(let[r,x]of S)y.has(r)||x.remove();p.forEach(r=>{let x=f(r),u=S.get(r.id);u?u.outerHTML!==x&&(u.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function Je(){let t=document.getElementById("models-widgets");if(!t)return;let e=await zt();if(!e){t.children.length===0&&(t.innerHTML=O("offline","Failed to load model status.","The event service may be offline."));return}kt=Date.now(),K(2,kt);let i=e.models||[],a=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let p=c.status==="Ready";return`
                ...
                </div>`}function o(c){let p=c.status==="Downloaded",h=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",b=p&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",n=c.name;return c.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${h}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(a&&(v+=s(a)),e.xtts&&(v+=l(e.xtts)),v+=i.map(o).join(""),!v){t.innerHTML=O("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function $e(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),$e()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),$e()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true");let p=await xs();if(p){let y=Math.floor(Date.now()/1e3),r=p.active_tier,x=p.active_synthesis,u=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},g=w=>{w<0&&(w=0);let R=Math.floor(w/3600),N=Math.floor(w%3600/60),I=w%60;return R>0?`${R}h ${N}m`:N>0?`${N}m ${I}s`:`${I}s`},d=(w,R,N,I,U)=>{if(!w)return;let q=u[I]||I.toUpperCase(),j=w.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(j&&(j.textContent=q),U&&U.includes(I))w.textContent="Working",w.style.color="#bb86fc";else if(N){let $=N.next_run-y;if($<=0)w.textContent="Ready",w.style.color="#5eff5e";else{let C=Math.floor($/60),T=$%60;w.textContent=`${C}m ${T}s`,w.style.color="#fff"}}R&&N&&(R.innerHTML=`
        ...
        `)};e&&d(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",r),i&&d(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",r);let E=document.getElementById("analyzer-synthesis-val"),L=document.getElementById("analyzer-synthesis-stats");E&&d(E,L,p.synthesis,"synthesis",x);let M=document.getElementById("system-state-label"),A=document.getElementById("system-state-val");if(A&&p.system_state){let w=p.system_state,R=g(p.system_state_time||0);M&&(M.textContent=`State: ${w.toUpperCase()}`),A.textContent=R,w==="idle"?A.style.color=p.system_state_time>300?"#5eff5e":"#fff":A.style.color="#bb86fc"}s&&(s.textContent=g(p.total_idle_time||0)),l&&(l.textContent=g(p.total_active_time||0)),o&&(o.textContent=g(p.total_waste_time||0))}else[e,i,a,s,l,o].forEach(r=>{r&&(r.textContent="-",r.style.color="#666")});let h=await vs(),m=[],b=[],n=[];h&&(Array.isArray(h)?m=h:(m=h.active||[],b=h.queue||[],n=h.history||[],n.sort((y,r)=>(r.end_time||0)-(y.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=O("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),je(t,m,!1));let f=document.getElementById("processes-queue-widgets");f&&(b.length===0?!f.querySelector(".tab-placeholder")&&!f.querySelector('div[style*="font-style: italic"]')&&(f.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(f.innerHTML="",je(f,b,!1)));let S=document.getElementById("processes-history-widgets");S&&(n.length===0?S.querySelector(".tab-placeholder")||(S.innerHTML=O("empty","No recent history.")):(S.querySelector(".tab-placeholder")&&(S.innerHTML=""),je(S,n,!0))),he(1,m.length)}function je(t,e,i){function a(v){let c="";v.end_time?c=`${v.end_time-v.start_time}s`:c=`${Math.floor(Date.now()/1e3-v.start_time)}s`;let p=v.retries>0?`<span class="process-retry-badge">Retry ${v.retries}</span>`:"",h=v.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[h]?h=m[h]:/^\d+$/.test(h)&&(h=`Channel ${h}`),i)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],hs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,a]of Object.entries(t)){let s=Pt.filter(l=>l.category===i);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),We=!0,i}function Nt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),We=!1}function Ht(t,e,i="output"){if(!We)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=Te(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Cs(t){let e=Pt.find(s=>s.id===t);if(!e)return;let i=ws(e);Ht(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(o=>setTimeout(o,400+Math.random()*600));let l="output";s.includes("[ERROR]")?l="error":s.includes("[SUCCESS]")||s.includes("\u2713")?l="success":s.includes("!")&&(l="warning"),Ht(i,s,l)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Rt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=hs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;Cs(a)})})}async function $s(){if(!B())try{if(!(await D("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ot(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}$s(),nt(),Ye();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Rt(),Xe(),Qe();let e=document.getElementById("nav-left-container");e&&e.addEventListener("click",()=>{let d=document.getElementById("dexter-dropdown"),E=document.getElementById("dexter-menu-btn");if(d&&d.classList.contains("active")){d.classList.remove("active"),E.classList.remove("active"),l?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(s.length>0)h();else{let L=window.location.pathname;if(!(L==="/"||L==="/index.html")){let w=(L.endsWith("/")&&L.length>1?L.slice(0,-1):L).split("/");w.pop();let R=w.join("/")||"/";window.location.href=R}}});let i=window.location.pathname==="/"||window.location.pathname==="/index.html",a=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!i&&!a&&document.querySelector("footer")?.classList.add("hide");let s=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let o=d=>{localStorage.setItem("dex_last_window",d)};function v(){return 1}function c(){for(;s.length>1;)s.shift().close(!0);let d=document.getElementById("windows-container"),E=document.querySelector("nav"),L=document.querySelector("footer"),M=window.location.pathname==="/"||window.location.pathname==="/index.html",A=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");d&&d.setAttribute("data-count",s.length),s.forEach(q=>{let j=document.getElementById(q.id);j&&j.classList.remove("hide-close")});let w=document.getElementById("dexter-menu-container"),R=document.getElementById("nav-window-switcher"),N=document.getElementById("settings-icon"),I=document.getElementById("close-all-windows"),U=window.innerWidth<880;if(U||(r&&r.classList.remove("active"),u&&u.classList.remove("active"),d&&d.classList.remove("menu-open")),fe(s.length>0),s.length>0)if(L?.classList.add("hide"),I&&(I.style.display=U?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),E?.classList.add("window-open"),d&&(d.style.paddingTop="60px"),w&&(w.style.display=U?"flex":"none"),N&&(N.style.display=U?"block":"none"),!U&&R){let q=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(q)?(R.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{o("alerts-window"),p(m)}),document.getElementById("switch-events").addEventListener("click",()=>{o("events-window"),p(b)}),document.getElementById("switch-monitor").addEventListener("click",()=>{o("monitor-window"),p(f)}),document.getElementById("switch-contacts").addEventListener("click",()=>{o("contacts-window"),p(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{o("workspace-window"),p(S)})):R.innerHTML=""}else R&&(R.innerHTML="");else E?.classList.remove("window-open"),I&&(I.style.display="none"),d&&(d.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),M||A?L?.classList.remove("hide"):L?.classList.add("hide"),w&&(w.style.display="flex"),N&&(N.style.display="block"),R&&(R.innerHTML="");ve()}function p(d){if(d.isOpen()){d.close();return}for(;s.length>0;)s.pop().close(!0);s.push(d),d.open(),c()}function h(){[...s].forEach(E=>E.close()),s=[]}window.addEventListener("resize",c);let m=ne({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:dt(),onOpen:()=>se(),onClose:()=>{let d=s.indexOf(m);d>-1&&s.splice(d,1),c()}}),b=ne({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{b.setContent(qe()),ie()},onClose:()=>{let d=s.indexOf(b);d>-1&&s.splice(d,1),c()}}),n=ne({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:wt(),onOpen:()=>Ue(),onClose:()=>{let d=s.indexOf(n);d>-1&&s.splice(d,1),c()}}),f=ne({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:At()},{icon:"bxs-component",title:"Processes",content:Mt()},{icon:"bxs-brain",title:"Models",content:It()},{icon:"bxs-hdd",title:"Hardware",content:_t()},{icon:"bxs-terminal",title:"Logs",content:Bt()},{icon:"bxs-zap",title:"Agents",content:Lt()}].filter(d=>B()?d.title!=="Hardware"&&d.title!=="Logs":!0),onOpen:()=>{Fe(),$e(),Je(),Ce()},onClose:()=>{let d=s.indexOf(f);d>-1&&s.splice(d,1),c()}}),S=ne({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:xt(),onOpen:()=>Re(),onClose:()=>{let d=s.indexOf(S);d>-1&&s.splice(d,1),c()}}),y=ne({id:"settings-window",title:"Settings",icon:"bx-cog",content:Be(),onOpen:()=>{y.setContent(Be()),Ge(y)},onClose:()=>{let d=s.indexOf(y);d>-1&&s.splice(d,1),c()}});window.dexter={viewEvent:d=>{b.isOpen()||p(b),setTimeout(()=>{let E=document.querySelector(`.event-item[data-event-id="${d}"]`);E&&(E.scrollIntoView({behavior:"smooth",block:"center"}),E.classList.add("flash-highlight"),E.classList.contains("expanded")||E.click(),setTimeout(()=>E.classList.remove("flash-highlight"),2e3))},500)},viewAlert:d=>{m.isOpen()||p(m),setTimeout(()=>{let E=document.querySelector(`.event-item[data-alert-id="${d}"]`);E&&(E.scrollIntoView({behavior:"smooth",block:"center"}),E.classList.add("flash-highlight"),E.classList.contains("expanded")||E.click(),setTimeout(()=>E.classList.remove("flash-highlight"),2e3))},500)}};let r=document.getElementById("dexter-dropdown");r&&(r.innerHTML=`

(()=>{function Re(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Pe(t=!1){let e=window.location.pathname,l=e==="/"||e==="/index.html",i=`
        <div class="nav-left" id="nav-left-wrapper" style="gap: 0;">
            ${`
        <div id="nav-left-container" style="display: flex; align-items: center; transition: transform 0.2s ease; gap: 5px; cursor: ${l?"default":"pointer"};">
            <i class='bx bx-chevron-left' id="nav-chevron" style="font-size: 28px; color: #fff; display: ${l?"none":"block"};"></i>
    ...
    `}
        ...
        `}
    `,g=document.createElement("nav");g.innerHTML=i,document.body.prepend(g)}function Oe(t){let e=window.location.pathname,l=e==="/"||e==="/index.html",o=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!o||!s||(t||!l?(o.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{o.style.transform="translateX(-3px)"},s.onmouseleave=()=>{o.style.transform="translateX(0)"}):(o.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function ze(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ue=1e3;function W(t){let e=null,l=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++Ue)}function i(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",g),o&&setTimeout(o,10);return}let f=document.getElementById("windows-container");f||(f=document.createElement("div"),f.id="windows-container",f.className="windows-container",document.body.appendChild(f)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ue,e.addEventListener("mousedown",s);let v=t.icon||"bx-window",n="",u="",T;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((x,a)=>{let c=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,T=`<div class="window-content">${t.tabs.map((x,a)=>`<div class="tab-content ${a===0?"active":""}" data-tab-content="${a}">${x.content}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),T=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${T}
        `,f.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",w=>{w.stopPropagation(),d()}),window.addEventListener("resize",g),t.tabs&&e.querySelectorAll(".tab").forEach(w=>{w.addEventListener("click",()=>{let r=w.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(x=>x.classList.remove("active")),w.classList.add("active"),e.querySelectorAll(".tab-content").forEach(x=>x.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${r}"]`).classList.add("active"),p(w,e)})}),setTimeout(()=>{e.classList.add("open"),o&&o()},10)}function g(){if(!e||!e.classList.contains("open"))return;let f=e.querySelector(".tab.active");f&&p(f,e)}function p(f,v){setTimeout(()=>{let n=v.querySelector(".tab-bar");if(!n)return;let u=Array.from(n.querySelectorAll(".tab")),T=u.indexOf(f),w=n.clientWidth,r=u[Math.max(0,T-2)],x=u[Math.min(u.length-1,T+2)],a=r.offsetLeft-n.offsetLeft-25,b=x.offsetLeft+x.offsetWidth-n.offsetLeft+25-a,S=b<=w?a-(w-b)/2:f.offsetLeft-n.offsetLeft-w/2+f.offsetWidth/2;n.scrollTo({left:S,behavior:"smooth"})},350)}function d(f=!1){e&&(window.removeEventListener("resize",g),f?(e.remove(),e=null):(e.classList.remove("open"),l&&l(),setTimeout(()=>{e?.remove(),e=null},400)))}function m(f){let v=e?.querySelector(".window-content");v&&(v.innerHTML=f)}function y(){return e&&e.classList.contains("open")}return{open:i,close:d,setContent:m,isOpen:y,focus:s,id:t.id}}function qe(){return!0}function Fe(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function je(t){localStorage.setItem("easter_user_email",t.trim())}var Ge="easter_theme",j={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function ae(){return localStorage.getItem(Ge)||j.DARK}function We(t){if(!Object.values(j).includes(t))throw new Error("Invalid theme");localStorage.setItem(Ge,t),pe(t)}function pe(t){let e=document.body;if(Object.values(j).forEach(o=>{e.classList.remove(`theme-${o}`)}),e.classList.add(`theme-${t}`),[j.LIGHT,j.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let o=document.createElement("div");o.className="background",document.body.prepend(o)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function Ve(){let t=ae();pe(t)}function I(t,e,l=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",i=l?`<p class="placeholder-action">${l}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${i}</div>`}function M(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function U(t,e){let l=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!l)return;if(!e){l.textContent="Last updated: never";return}let o=Date.now(),s=Math.floor((o-e)/1e3),i;s<60?i=`${s}s ago`:s<3600?i=`${Math.floor(s/60)}m ago`:i=`${Math.floor(s/3600)}h ago`,l.textContent=`Last updated: ${i}`}var Ke=0;function Je(){return Ke}function oe(t,e){let l=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!l)return;let o=l.querySelector(".notification-badge");o&&(e>0?(o.textContent=e>9?"9+":e,o.style.display="flex"):o.style.display="none")}function le(t){Ke=t;let e=document.getElementById("dexter-nav-badge");e&&(t>0?e.style.display="flex":e.style.display="none");let l=document.getElementById("alerts-menu-item");if(l){let s=l.querySelector(".notification-badge");s||(s=document.createElement("span"),s.className="notification-badge",s.style.marginLeft="auto",l.appendChild(s)),t>0?(s.textContent=t>9?"9+":t,s.style.display="flex"):s.style.display="none"}let o=document.getElementById("switch-alerts");if(o){let s=o.querySelector(".notification-badge");s||(s=document.createElement("span"),s.className="notification-badge",s.style.marginLeft="8px",o.appendChild(s)),t>0?(s.textContent=t>9?"9+":t,s.style.display="flex"):s.style.display="none"}}function ge(){let t=document.getElementById("alerts-list");if(!t)return;let e=t.querySelectorAll(".alert-unread:not(.priority-low)").length;le(e)}function $e(){return"https://event.easter.company"}function Lt(){return"https://discord.easter.company"}var kt="http://127.0.0.1:8100",It="http://127.0.0.1:8300",At={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function fe(t){let e=M(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,i)=>{let g=At[i];return g?`<span class="${g}">`:""});let l=(e.match(/<span/g)||[]).length,o=(e.match(/<\/span/g)||[]).length;return l>o&&(e+="</span>".repeat(l-o)),e}function re(t){if(!t)return"";let e=M(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(l,o)=>{let s=o.split("|").map(i=>i.trim());return s.every(i=>i.match(/^:?-+:?$/))?"":`<div class="md-table-row">${s.map(i=>`<span class="md-table-cell">${i}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var Z=null,ee=null,me=!1,ue=!1;async function L(t,e={}){if(Z)try{let s=await fetch(Z+t,e);if(s.ok)return s;Z=null}catch{Z=null}let l=$e(),o=kt;try{let s=await fetch(l+t,e);if(s.ok)return Z=l,me&&(console.log("\u2728 Production event service restored."),me=!1),s;throw new Error("Primary failed")}catch{me||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${o}`),me=!0);try{let i=await fetch(o+t,e);if(i.ok)return Z=o,i;throw new Error("Fallback failed")}catch(i){throw i}}}async function Ye(t,e={}){if(ee)try{let s=await fetch(ee+t,e);if(s.ok)return s;ee=null}catch{ee=null}let l=Lt(),o=It;try{let s=await fetch(l+t,e);if(s.ok)return ee=l,ue&&(console.log("\u2728 Production discord service restored."),ue=!1),s;throw new Error("Primary failed")}catch{ue||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${o}`),ue=!0);try{let i=await fetch(o+t,e);if(i.ok)return ee=o,i;throw new Error("Fallback failed")}catch(i){throw i}}}var Xe=()=>`
...
`,be=null,V=new Set,ve=[];async function K(t=!1){let e=document.getElementById("alerts-list");if(!e)return;_t(),t&&(e.innerHTML="<p>Updating...</p>");let l="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await L(l);if(!o.ok)throw new Error("Service is offline or unreachable.");let i=(await o.json()).events||[];be=Date.now(),U(0,be);let g=Date.now(),p=24*60*60*1e3,d=i.filter(a=>{let c=localStorage.getItem(`alert_read_ts_${a.id}`);if(!c)return!0;let b=parseInt(c);return g-b<p});d.sort((a,c)=>{let b=E=>{let k=E.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},S=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,h=S(b(a)),$=S(b(c));return h!==$?$-h:c.timestamp-a.timestamp}),ve=d;let m=a=>{let c=a.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return"low"}return(c.priority||"low").toLowerCase()},y=[],v=new Set(d.map(a=>m(a))).size>1;if(d.length>0){let a=null;d.forEach(c=>{let b=m(c);v&&b!==a&&(y.push({id:`divider-${b}`,type:"divider",label:b.toUpperCase()}),a=b),y.push(c)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=I("empty","No alerts yet."),ge();return}let n=a=>{let c=a.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return null}let b=(c.title||"Untitled Alert").trim(),S=(c.body||"No description provided.").trim(),h=(c.priority||"low").toLowerCase(),$=!!c.alert,E=(c.category||"system").trim(),k=c.related_event_ids||[],A=c.audit_event_id,H=!!localStorage.getItem(`alert_read_ts_${a.id}`),z=new Date(a.timestamp*1e3),C=z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),B=z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=H?"event-border-grey":"event-border-blue";!H&&$&&(_="event-border-red"),H&&(h==="high"||h==="critical")?_="event-border-red":H&&h==="medium"&&(_="event-border-orange");let N=H?"alert-read":"alert-unread",D=V.has(a.id),O=D?"expanded":"",q=D?"display: block;":"display: none;",Me="",Be="";k.length>0&&(Be=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${k.map(G=>`<a href="#" onclick="window.dexter.viewEvent('${G}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px; font-family: monospace; width: fit-content;">${G.substring(0,8)}...</a>`).join(", ")}</span>
                    </div>`);let De="";A&&(De=`
              <div class="event-detail-row">
                ...
                <div class="event-detail-block" style="text-align: left;">
                    <div class="detail-pre" style="color: #fff;">${re(S)}</div>
                </div>
            `;let F=document.createElement("div");F.className=`event-item notification-item ${_} ${N} ${O} cursor-pointer priority-${h}`,F.dataset.alertId=a.id,F.onclick=function(G){if(this.classList.contains("expanded")){this.classList.remove("expanded"),V.delete(a.id);let ie=this.querySelector(".event-details");ie&&(ie.style.display="none")}else{this.classList.add("expanded"),V.add(a.id);let ie=this.querySelector(".event-details");if(ie&&(ie.style.display="block"),!localStorage.getItem(`alert_read_ts_${a.id}`)){localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ee="event-border-grey";h==="high"||h==="critical"?Ee="event-border-red":h==="medium"&&(Ee="event-border-orange"),this.classList.add(Ee),ge()}}};let Qt=b,Ct={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[E]||"bx-bell";F.innerHTML=`
                ...
                <div class="event-content">
                    <div class="event-service">${E.toUpperCase()} ${$?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>
                    <div class="event-message">${b}</div>
                    <div class="event-details" style="${q}">
                        ${Me}
                    </div>
                </div>
            `;let He=F.querySelector(".event-details");He&&He.addEventListener("click",G=>{G.stopPropagation()});let Ne=F.querySelector(".close-details-btn");return Ne&&Ne.addEventListener("click",G=>{G.stopPropagation(),F.classList.remove("expanded");let we=F.querySelector(".event-details");we&&(we.style.display="none"),V.delete(a.id)}),F},u=a=>{let c=document.createElement("div");c.className="notification-divider",c.dataset.alertId=a.id;let b="#888888";return a.label==="CRITICAL"?b="#ff4d4d":a.label==="HIGH"?b="#ff8888":a.label==="MEDIUM"&&(b="#ffa500"),c.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${b}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,c.innerHTML=`<span style="white-space: nowrap;">${a.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${b}44, transparent); flex-grow: 1;"></div>`,c},T=Array.from(e.children),w=new Map(T.map(a=>[a.dataset.alertId,a])),r=new Set(y.map(a=>a.id));T.forEach(a=>{let c=a.dataset.alertId;(!c||!r.has(c))&&a.remove()});let x=null;y.forEach((a,c)=>{let b=w.get(a.id);(!b||t)&&(b&&b.remove(),a.type==="divider"?b=u(a):b=n(a),!b)||(c===0?e.firstElementChild!==b&&e.prepend(b):x&&x.nextElementSibling!==b&&x.after(b),x=b)}),be=Date.now(),U(0,be),ge()}catch(o){console.error("Error fetching alerts:",o),e.children.length===0&&(e.innerHTML=I("offline","Failed to load alerts.","The event service may be offline."))}}function _t(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),l=document.getElementById("alerts-close-all"),o=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ve.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),K(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ve.forEach(s=>{V.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),K(!0)},e.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=()=>{V.clear(),K(!0)},l.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await L("/events?type=system.notification.generated",{method:"DELETE"});let s=Date.now()-48*60*60*1e3;ve.forEach(i=>{localStorage.setItem(`alert_read_ts_${i.id}`,s.toString())}),V.clear(),K(!0)}catch(s){console.error("Failed to clear alerts:",s)}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}async function Qe(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await L(t);if(!e.ok)return;let o=(await e.json()).events||[],s=0;o.forEach(i=>{let g=i.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{g={}}if((g.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${i.id}`)||s++}),le(s)}catch{}}var Ze=()=>`
...
`,J=new Set,tt=[],te=null;async function Y(t=!1){let e=document.getElementById("roadmap-list");if(e){Mt();try{let l=await L("/roadmap");if(!l.ok)throw new Error("Offline");let o=await l.json();if(tt=o,o.length===0){e.innerHTML=I("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let y=J.has(m.id),f=m.state==="draft",v=m.state==="published",n=m.state==="consumed",u="event-border-grey";v&&(u="event-border-blue"),n&&(u="event-border-purple");let w=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),r=document.createElement("div");r.className=`event-item notification-item ${u} cursor-pointer ${y?"expanded":""}`,r.dataset.itemId=m.id,r.onclick=c=>{if(c.target.closest("button"))return;r.classList.contains("expanded")?(r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",J.delete(m.id)):(r.classList.add("expanded"),r.querySelector(".event-details").style.display="block",J.add(m.id))};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;r.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-details" style="${y?"display: block;":"display: none;"} ">
            ...
            ${n?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(m.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `;let a=r.querySelector(".event-details");return a&&a.addEventListener("click",c=>{c.stopPropagation()}),r.querySelector(".edit-btn")?.addEventListener("click",()=>Bt(m)),r.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Dt(m)),r.querySelector(".delete-btn")?.addEventListener("click",()=>Ht(m.id)),r.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",J.delete(m.id)}),r},i=Array.from(e.children),g=new Map(i.map(m=>[m.dataset.itemId,m])),p=new Set(o.map(m=>m.id));i.forEach(m=>{let y=m.dataset.itemId;(!y||!p.has(y))&&m.remove()});let d=null;o.forEach((m,y)=>{let f=g.get(m.id);(!f||t)&&(f&&f.remove(),f=s(m),!f)||(y===0?e.firstElementChild!==f&&e.prepend(f):d&&d.nextElementSibling!==f&&d.after(f),d=f)})}catch{e.children.length===0&&(e.innerHTML=I("offline","Failed to load roadmap.","The event service may be offline."))}}}function Mt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),l=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{te=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",te=null},l.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let i=document.getElementById("roadmap-editor-input").value;if(!i.trim())return;let g=te?`/roadmap/${te}`:"/roadmap",p=te?"PATCH":"POST";try{await L(g,{method:p,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:i})}),document.getElementById("roadmap-editor-container").style.display="none",Y(!0)}catch(d){console.error(d)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{tt.forEach(i=>J.add(i.id)),Y(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{J.clear(),Y(!0)},s.dataset.listenerAttached="true")}function Bt(t){te=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Dt(t){let e=t.state==="published"?"draft":"published";try{await L(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),Y(!0)}catch(l){console.error(l)}}async function Ht(t){if(confirm("Delete this roadmap item?"))try{await L(`/roadmap/${t}`,{method:"DELETE"}),J.delete(t),Y(!0)}catch(e){console.error(e)}}var nt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,st=null,se=new Set,at=[];async function ne(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Nt();let l="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await L(l);if(!o.ok)throw new Error("Service is offline or unreachable.");let i=(await o.json()).events||[];if(at=i,st=Date.now(),U(2,st),i.length===0){e.innerHTML=I("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),oe(2,0);return}t&&(e.innerHTML="");let g=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let n=(v.title||"Untitled Blueprint").trim(),u=(v.summary||v.body||"No summary provided.").trim(),T=(v.content||"").trim(),w=(v.category||"architecture").trim(),r=(v.affected_services||[]).map(D=>D.trim()),x=(v.implementation_path||[]).map(D=>D.trim()),a=v.approved===!0,c=new Date(f.timestamp*1e3),b=c.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=c.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=se.has(f.id),$=h?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${h?"expanded":""} ${a?"blueprint-approved":""}`,E.dataset.blueprintId=f.id,a&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let A=a?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),se.delete(f.id);let q=this.querySelector(".event-details");q&&(q.style.display="none")}else{this.classList.add("expanded"),se.add(f.id);let q=this.querySelector(".event-details");q&&(q.style.display="block")}};let R="";x.length>0&&(R=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${x.map(D=>`<li style="margin-bottom: 5px;">${M(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let H=r.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${r.join(", ")}</span></div>`:"<div></div>",z=a?`
            ...
            `;E.innerHTML=`
                ${a?'<div class="blueprint-sparkle"></div>':""}
                <div class="event-time">
                    <span class="event-time-main">${b}</span>
                    <span class="event-date">${S}</span>
                </div>
                <div class="event-icon" style="${a?"color: #03dac6;":""}"><i class='bx ${A}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${$}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px; color: #fff;">${M(u)}</div>
                        ...
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${M(T)}</div>
                        </div>
                        ${R}
                        ${z}
                    </div>
                </div>
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async D=>{D.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await L(`/events/${f.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ne(!0)}catch(O){console.error("Failed to approve blueprint:",O)}});let B=E.querySelector(".blueprint-delete-btn");B&&(B.onclick=async D=>{D.stopPropagation();let O=!a;B.innerHTML=O?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await L(`/events/${f.id}`,{method:"DELETE"})).ok&&ne(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let _=E.querySelector(".event-details");_&&_.addEventListener("click",D=>{D.stopPropagation()});let N=E.querySelector(".close-details-btn");return N&&N.addEventListener("click",D=>{D.stopPropagation(),E.classList.remove("expanded");let O=E.querySelector(".event-details");O&&(O.style.display="none"),se.delete(f.id)}),E},p=Array.from(e.children),d=new Map(p.map(f=>[f.dataset.blueprintId,f])),m=new Set(i.map(f=>f.id));p.forEach(f=>{let v=f.dataset.blueprintId;(!v||!m.has(v))&&f.remove()});let y=null;i.forEach((f,v)=>{let n=d.get(f.id);(!n||t)&&(n&&n.remove(),n=g(f),!n)||(v===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),oe(2,i.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=I("offline","Failed to load blueprints.","The event service may be offline."))}}function Nt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{at.forEach(l=>se.add(l.id)),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{se.clear(),ne(!0)},e.dataset.listenerAttached="true")}var ot=()=>`
...
`;async function Se(){await Promise.all([Y(),ne()])}var lt=()=>`
...
`,Rt=null;async function Te(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Te(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true");try{let l=await Ye("/contacts");if(!l.ok)throw new Error("Service unreachable");let s=(await l.json()).members||[];if(Rt=Date.now(),s.length===0){t.innerHTML=I("empty","No contacts found.","Check your Discord connection.");return}let i={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((g,p)=>{let d=i[g.level]??10,m=i[p.level]??10;return d!==m?d-m:g.username.localeCompare(p.username)}),t.innerHTML=s.map(g=>{let p=g.color?"#"+g.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=g.status==="online"?"#5eff5e":g.status==="idle"?"#ffa500":g.status==="dnd"?"#ff4d4d":"#666",m="#888";g.level==="Me"||g.level==="Master"?m="#bb86fc":g.level==="Admin"?m="#cf6679":g.level==="Moderator"?m="#03dac6":g.level==="Contributor"&&(m="#ffa500");let y=g.level==="Me",f=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",v=y?"2px solid #bb86fc":`1px solid ${p}33`;return`
                <div class="user-profile-section" style="background: ${f}; padding: 15px; border-radius: 8px; border: ${v}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${y?"#bb86fc":p}22, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${g.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${y?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
...
`,xe=null,X=new Set,ce=[],P="all",Ot={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},zt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Ut(t){for(let[e,l]of Object.entries(Ot))if(l.includes(t))return e;return"system"}function qt(t){return zt[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ft();let o=`/events?ml=${P==="all"?100:250}&format=json`;P!=="all"&&(o+=`&category=${P}`);try{let s=await L(o);if(!s.ok)throw new Error("Service unreachable");if(ce=((await s.json()).events||[]).filter(v=>{let n=v.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),xe=Date.now(),U(1,xe),ce.length===0){e.innerHTML=I("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=v=>{let n=v.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,T=Ut(u),w=qt(u),r=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),x="event-border-grey";r&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?x="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.analysis.audit"?x="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?x="event-border-orange":u==="system.test.completed"?x=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":x="event-border-blue");let a=r?"cursor-pointer":"",c=X.has(v.id),b=c?"expanded":"",S=c?"display: block;":"display: none;",h=new Date(v.timestamp*1e3),$=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),E=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=rt(u,n),A=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",R="";if(r){let C="";if(u==="engagement.decision")C=`
                    ...
                    `;else if(u==="system.attention.expired"){let B=n.timestamp-n.last_active,_=Math.floor(B/60);C=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div class="detail-pre">${re(n.last_input||"None")}</div>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Thought Process (Last Output):</span>
                            <div class="detail-pre">${re(n.last_output||"None")}</div>
                        </div>
                    `}else if(u==="messaging.bot.sent_message")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="moderation.explicit_content.deleted")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="analysis.link.completed")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="analysis.visual.completed"){let B="";n.base64_preview?B=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(B=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="system.analysis.audit"){let B=n.success?"#03dac6":"#ff4d4d",_=n.success?"SUCCESS":"FAILED",N="";n.error&&(N=`
                            <div class="event-detail-block">
                                <span class="detail-label" style="color: #ff4d4d;">Error:</span>
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${M(n.error)}</pre>
                            </div>
                        `),C=`
                        <div class="event-detail-row">
                            <span class="detail-label">Agent:</span>
                            <span class="detail-value">${n.agent_name||"Guardian"}</span>
                        </div>
                        <div class="event-detail-row">
                            <span class="detail-label">Tier:</span>
                            <span class="detail-value" style="color: #bb86fc;">${n.tier}</span>
                        ...
                        <div class="event-detail-row">
                            <span class="detail-label">Status:</span>
                            <span class="detail-value" style="color: ${B}; font-weight: bold;">${_} (${n.attempts} attempts)</span>
                        </div>
                        <div class="event-detail-row">
                            <span class="detail-label">Duration:</span>
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre" style="max-height: 150px; overflow-y: auto;">${M(n.system_prompt)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Input Context:</span>
                            <pre class="detail-pre" style="max-height: 200px; overflow-y: auto;">${M(n.input_context)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Raw Output:</span>
                            <pre class="detail-pre">${M(n.raw_output||"(empty)")}</pre>
                        </div>
                    `}else if(u==="system.test.completed")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="error_occurred")C=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${M(n.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            <span class="detail-label">Context:</span>
                            <pre class="detail-pre">${JSON.stringify(n.context||{},null,2)}</pre>
                        </div>
                    `;else if(u==="system.cli.status")C=`
                        <div class="event-detail-row">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ...
                            <pre class="detail-pre">${M(n.message)}</pre>
                        </div>
                    `;else if(u==="messaging.user.sent_message"){let B="";n.attachments&&n.attachments.length>0&&(B=`
                            <div class="event-detail-block">
                                <span class="detail-label">Attachments:</span>
                                <div class="attachments-grid">${n.attachments.map(N=>{let D=N.content_type&&N.content_type.startsWith("image/"),O=(N.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${D?`<div class="attachment-thumb-container"><img src="${N.url}" alt="${N.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${N.url}" target="_blank" class="attachment-link">${N.filename}</a>
                                        <span class="attachment-meta">${N.content_type} \u2022 ${O}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),C=`
                        <div class="event-detail-row">
                ...
                `}let H=document.createElement("div");H.className=`event-item ${x} ${a} ${b}`,H.dataset.eventId=v.id,H.onclick=function(C){if(!r)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(v.id);let _=this.querySelector(".event-details");_&&(_.style.display="none")}else{this.classList.add("expanded"),X.add(v.id);let _=this.querySelector(".event-details");_&&(_.style.display="block")}},H.innerHTML=`
                <div class="event-time">
            ...
            `;let z=H.querySelector(".event-details");if(z&&z.addEventListener("click",C=>{C.stopPropagation()}),r){let C=H.querySelector(".close-details-btn");C&&C.addEventListener("click",B=>{B.stopPropagation();let _=B.target.closest(".event-item");if(_){_.classList.remove("expanded"),X.delete(v.id);let N=_.querySelector(".event-details");N&&(N.style.display="none")}})}return H},d=Array.from(e.children),m=new Map(d.map(v=>[v.dataset.eventId,v])),y=new Set(ce.map(v=>v.id));d.forEach(v=>{let n=v.dataset.eventId;(!n||!y.has(n))&&v.remove()});let f=null;ce.forEach((v,n)=>{let u=m.get(v.id);(!u||t)&&(u&&u.remove(),u=p(v),!u)||(n===0?e.firstElementChild!==u&&e.prepend(u):f&&f.nextElementSibling!==u&&f.after(u),f=u)}),xe=Date.now(),U(1,xe)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=I("offline","Failed to load events.","The event service may be offline."))}}function Ft(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),l=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ce.forEach(s=>X.add(s.id)),Q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),Q(!0)},e.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{l.querySelectorAll(".filter-btn").forEach(i=>i.classList.remove("active")),s.classList.add("active"),P=s.dataset.filter,Q(!0)}}),l.dataset.listenerAttached="true"),l&&l.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===P?s.classList.add("active"):s.classList.remove("active")});let o=document.getElementById("events-clear");o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{let s=P==="all"?"ALL":P.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let i="/events";if(P!=="all"?i+=`?category=${P}`:i+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await L(i,{method:"DELETE"})).ok)throw new Error("Failed to delete events");X.clear(),Q(!0)}catch(i){console.error("Failed to clear events:",i),alert("Failed to clear events. Check console.")}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}function ct(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var jt=null;async function de(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await L("/logs");if(!e.ok)throw new Error("Logs offline");let l=await e.json();if(!l||l.length===0)return t.innerHTML=I("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=l.filter(g=>!o.includes(g.id));s.forEach(g=>{g.logs&&Array.isArray(g.logs)?g.logs.reverse():g.logs=[]}),s.reverse();let i=s.map(g=>{let p=g.logs.join(`
`),d=[...g.logs];if(d.length<25){let y=25-d.length;for(let f=0;f<y;f++)d.push("")}else d.length>25&&(d=d.slice(-25));let m=d.map(y=>fe(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=i,document.querySelectorAll(".copy-logs-btn").forEach(g=>{g.addEventListener("click",()=>{let p=unescape(g.dataset.logs);navigator.clipboard.writeText(p).then(()=>{let d=g.querySelector("i");d.classList.remove("bx-copy"),d.classList.add("bx-check"),setTimeout(()=>{d.classList.remove("bx-check"),d.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(g=>{g.addEventListener("click",async()=>{let p=g.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${p}?`))try{(await L(`/logs?service_id=${p}`,{method:"DELETE"})).ok&&de()}catch(d){console.error("Error clearing logs:",d)}})}),jt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=I("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ut=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function yt(){await Promise.all([ye(),ke(),Ie()]),await de()}var dt=null,pt=null;async function ht(){try{return await(await L("/system_monitor")).json()}catch{return null}}async function mt(){try{return await(await L("/system/hardware")).json()}catch{return null}}async function Gt(){try{return await(await L("/processes")).json()}catch{return null}}async function Wt(){try{return await(await L("/agent/status")).json()}catch{return null}}async function ke(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),l=document.querySelector("#hw-os .hw-content"),o=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),g=document.querySelector("#hw-storage .hw-content"),p=r=>{if(r){if(l&&(l.innerHTML=`
                ...
                </div>`}if(o&&r.CPU&&r.CPU.length>0){let x=r.CPU[0];o.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}i&&(r.GPU&&r.GPU.length>0?i.innerHTML=r.GPU.map(x=>{let a=(x.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${x.LABEL}</span>
                            <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${a} <span style="font-size: 0.6em; color: #888; font-weight: normal;">GB VRAM</span></span>
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):i.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),g&&r.STORAGE&&r.STORAGE.length>0?g.innerHTML=r.STORAGE.map(x=>{let a=(x.USED/1073741824).toFixed(1),c=(x.SIZE/(1024*1024*1024)).toFixed(1),b=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,S=x.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${b>90?"#cf6679":"#03dac6"}; width: ${b}%; height: 100%; box-shadow: 0 0 10px ${b>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):g&&(g.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let r=await mt();r?(p(r),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),o&&!o.hasChildNodes())){let r=await mt();p(r)}if(!t)return;let d=await ht();if(!d||!d.services){t.children.length===0&&(t.innerHTML=I("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),U(0,dt);let m=d.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function y(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function f(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:r.split(".").slice(0,3).join(".")||"-"}function v(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function n(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let a=parseInt(x[1])||0,c=parseInt(x[2])||0,b=parseInt(x[3])||0,S=parseFloat(x[4])||0,h=a*86400+c*3600+b*60+S,$=Math.floor(h/86400);if($>0)return`${$}d`;let E=Math.floor(h/3600);if(E>0)return`${E}h`;let k=Math.floor(h/60);return k>0?`${k}m`:`${Math.floor(h)}s`}function u(r){let x=r.status==="online",a=x?"service-widget-online":"service-widget-offline",c=x?"bx-check-circle":"bx-x-circle",b=x?"OK":"BAD",S=r.version?f(r.version.str):"-",h=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",$=r.memory&&r.memory!=="N/A"?r.memory:"-",E=h!=="-"?"#00ff00":"#666",k=h!=="-"?"#fff":"#666",A=$!=="-"?"#00ff00":"#666",R=$!=="-"?"#fff":"#666",H=n(r.uptime),z="";return x?z=`
                ...
                </div>`:z='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${a}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${c}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${b}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${v(r.domain&&r.port?`${r.domain}:${r.port}`:"")}</span></div>${z}</div></div>`}let T=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),w=new Set(m.map(r=>r.id));for(let[r,x]of T)w.has(r)||x.remove();m.forEach(r=>{let x=u(r),a=T.get(r.id);a?a.outerHTML!==x&&(a.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function Ie(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ht();if(!e){t.children.length===0&&(t.innerHTML=I("offline","Failed to load model status.","The event service may be offline."));return}pt=Date.now(),U(2,pt);let l=e.models||[],o=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function s(d){let m=d.status==="Ready";return`
                ...
                </div>`}function g(d){let m=d.status==="Downloaded",y=m?"service-widget-online":"service-widget-offline",f=m?"OK":"MISSING",v=m&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",n=d.name;return d.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let p="";if(o&&(p+=s(o)),e.xtts&&(p+=i(e.xtts)),p+=l.map(g).join(""),!p){t.innerHTML=I("empty","No models found.");return}t.innerHTML!==p&&(t.innerHTML=p)}async function ye(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),l=document.getElementById("guardian-t2-val"),o=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),i=document.getElementById("guardian-total-active"),g=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await L("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),ye()}catch{p.innerHTML="<i class='bx bx-error'></i> Failed"}},p.dataset.listenerAttached="true");let d=await Wt();if(d){let T=Math.floor(Date.now()/1e3),w=d.active_tier,r=b=>{b<0&&(b=0);let S=Math.floor(b/3600),h=Math.floor(b%3600/60),$=b%60;return S>0?`${S}h ${h}m`:h>0?`${h}m ${$}s`:`${$}s`},x=(b,S,h,$)=>{if(w===$)b.textContent="Working",b.style.color="#bb86fc";else if($==="t1"&&w==="tests")b.textContent="Testing",b.style.color="#03dac6";else if(h){let k=h.next_run-T;if(k<=0)b.textContent="Ready",b.style.color="#5eff5e";else{let A=Math.floor(k/60),R=k%60;b.textContent=`${A}m ${R}s`,b.style.color="#fff"}}S&&h&&(S.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${h.attempts||0}</span>
            <span style="color: ${h.failures>0?"#ffa500":"#666"}">Fails: ${h.failures||0}</span>
            <span style="color: ${h.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${h.absolute_failures||0}</span>
          </div>
        `)};e&&x(e,document.getElementById("guardian-t1-stats"),d.t1,"t1"),l&&x(l,document.getElementById("guardian-t2-stats"),d.t2,"t2");let a=document.getElementById("system-state-label"),c=document.getElementById("system-state-val");if(c&&d.system_state){let b=d.system_state,S=r(d.system_state_time||0);a&&(a.textContent=`State: ${b.toUpperCase()}`),c.textContent=S,b==="idle"?c.style.color=d.system_state_time>300?"#5eff5e":"#fff":c.style.color="#bb86fc"}s&&(s.textContent=r(d.total_idle_time||0)),i&&(i.textContent=r(d.total_active_time||0)),g&&(g.textContent=r(d.total_waste_time||0))}else[e,l,o,s,i,g].forEach(w=>{w&&(w.textContent="-",w.style.color="#666")});let m=await Gt(),y=[],f=[],v=[];m&&(Array.isArray(m)?y=m:(y=m.active||[],f=m.queue||[],v=m.history||[],v.sort((T,w)=>(w.end_time||0)-(T.end_time||0)))),y.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=I("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Le(t,y,!1));let n=document.getElementById("processes-queue-widgets");n&&(f.length===0?!n.querySelector(".tab-placeholder")&&!n.querySelector('div[style*="font-style: italic"]')&&(n.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(n.innerHTML="",Le(n,f,!1)));let u=document.getElementById("processes-history-widgets");u&&(v.length===0?u.querySelector(".tab-placeholder")||(u.innerHTML=I("empty","No recent history.")):(u.querySelector(".tab-placeholder")&&(u.innerHTML=""),Le(u,v,!0))),oe(1,y.length)}function Le(t,e,l){function o(p){let d="";p.end_time?d=`${p.end_time-p.start_time}s`:d=`${Math.floor(Date.now()/1e3-p.start_time)}s`;let m=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",y=p.channel_id,f={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};if(f[y]?y=f[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),l)return`
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${p.channel_id}-${p.start_time}" style="">
                    <div class="service-widget-header">
                        ...
                        <h3 style="color: ${v}">${y}</h3>
                        ${m}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">State:</span>
                            <span class="info-value" style="color: ${v}">${p.state}</span>
                        </div>
                        <div class="service-widget-info">
                            <span class="info-label">Duration:</span>
                            <span class="info-value" style="color: ${v}">${d}</span>
                        </div>
                         <div class="service-widget-info">
                            <span class="info-label">PID:</span>
                            <span class="info-value" style="color: ${v}">${p.pid}</span>
                        </div>
                    </div>
                </div>`}let s=l?".process-history-item":".process-widget",i=new Map(Array.from(t.querySelectorAll(s)).map(p=>[p.dataset.channelId,p])),g=new Set(e.map(p=>`${p.channel_id}-${p.start_time}`));for(let[p,d]of i)g.has(p)||d.remove();e.forEach(p=>{let d=`${p.channel_id}-${p.start_time}`,m=o(p),y=i.get(d);if(y){y.outerHTML!==m&&(y.outerHTML=m);let f=t.querySelector(`[data-channel-id="${d}"]`);f&&t.appendChild(f)}else t.insertAdjacentHTML("beforeend",m)})}function he(){let t=ae(),e=Fe()||"master@easter.company",l={enabled:Notification.permission==="granted",supported:"Notification"in window},o=localStorage.getItem("easter_analytics_enabled")!=="false";return`
                ...
                <div class="settings-list">
                    <div class="settings-item">
                        <div class="settings-item-info">
                            ...
                            <span class="settings-item-description">${l.supported?"Receive desktop notifications":"Not supported in this browser"}</span>
                        </div>
                        <label class="toggle-switch">
                            <input type="checkbox" id="notifications-toggle" ${l.enabled?"checked":""} ${l.supported?"":"disabled"}>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Kt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[l,o]of Object.entries(t)){let s=$t.filter(i=>i.category===l);s.length!==0&&(e+=`
                    ...
                    ${s.map(i=>`
                        <div class="cli-command-card ${i.dummy?"dummy":""}" data-cmd="${i.id}" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 25px; transition: all 0.3s ease; cursor: pointer; position: relative; overflow: hidden;">
                            ...
                            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px; position: relative; z-index: 1;">
                                <i class='bx ${i.icon}' style="font-size: 2em; color: ${o.color};"></i>
    ...
    </div>`,e},_e=!1;function Jt(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>wt(),document.getElementById("terminal-action-btn").onclick=()=>wt());let l=document.getElementById("cli-terminal-body"),o=document.getElementById("cli-docs-pane");l.innerHTML="";let s=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return o.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),_e=!0,l}function wt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),_e=!1}function Et(t,e,l="output"){if(!_e)return;let o=document.createElement("div");o.className=`terminal-line terminal-${l}`,l==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=fe(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Yt(t){let e=$t.find(s=>s.id===t);if(!e)return;let l=Jt(e);Et(l,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(g=>setTimeout(g,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),Et(l,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function St(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Kt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let l=e.querySelector("code").textContent;navigator.clipboard.writeText(l).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(l=>{l.addEventListener("mouseenter",()=>{l.style.transform="translateY(-5px)",l.style.borderColor="rgba(255,255,255,0.15)",l.style.backgroundColor="rgba(255,255,255,0.05)"}),l.addEventListener("mouseleave",()=>{l.style.transform="translateY(0)",l.style.borderColor="rgba(255,255,255,0.05)",l.style.backgroundColor="rgba(255,255,255,0.03)"}),l.addEventListener("click",()=>{let o=l.dataset.cmd;Yt(o)})})}async function Xt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${$e()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Tt(){Xt(),Ve(),Re();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&St();let e=qe();Pe(e),ze();let l=document.getElementById("nav-left-container");l&&l.addEventListener("click",()=>{if(i.length>0)f();else{let a=window.location.pathname;if(!(a==="/"||a==="/index.html")){let S=(a.endsWith("/")&&a.length>1?a.slice(0,-1):a).split("/");S.pop();let h=S.join("/")||"/";window.location.href=h}}});let o=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!o&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],g=document.getElementById("windows-container");g&&g.setAttribute("data-count","0");let p=a=>{localStorage.setItem("dex_last_window",a)};function d(){return 1}function m(){for(;i.length>1;)i.shift().close(!0);let a=document.getElementById("windows-container"),c=document.querySelector("nav"),b=document.querySelector("footer"),S=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",i.length),i.forEach(A=>{let R=document.getElementById(A.id);R&&R.classList.remove("hide-close")}),pe(ae());let $=document.getElementById("dexter-menu-container"),E=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon");if(Oe(i.length>0),i.length>0){if(b?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),c?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),$&&($.style.display="none"),k&&(k.style.display="none"),E){let A=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(E.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{p("alerts-window"),y(v)}),document.getElementById("switch-events").addEventListener("click",()=>{p("events-window"),y(n)}),document.getElementById("switch-monitor").addEventListener("click",()=>{p("monitor-window"),y(T)}),document.getElementById("switch-contacts").addEventListener("click",()=>{p("contacts-window"),y(u)}),document.getElementById("switch-workspace").addEventListener("click",()=>{p("workspace-window"),y(w)})):E.innerHTML=""}}else c?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S||h?b?.classList.remove("hide"):b?.classList.add("hide"),$&&($.style.display="flex"),k&&(k.style.display="block"),E&&(E.innerHTML="");le(Je())}function y(a){if(a.isOpen()){a.close();return}for(;i.length>0;)i.pop().close(!0);i.push(a),a.open(),m()}function f(){[...i].forEach(c=>c.close()),i=[]}window.addEventListener("resize",m);let v=W({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Xe(),onOpen:()=>K(),onClose:()=>{let a=i.indexOf(v);a>-1&&i.splice(a,1),m()}}),n=W({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Ce(),onOpen:()=>{n.setContent(Ce()),Q()},onClose:()=>{let a=i.indexOf(n);a>-1&&i.splice(a,1),m()}}),u=W({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:lt(),onOpen:()=>Te(),onClose:()=>{let a=i.indexOf(u);a>-1&&i.splice(a,1),m()}}),T=W({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:ft()},{icon:"bxs-component",title:"Processes",content:gt()},{icon:"bxs-brain",title:"Models",content:bt()},{icon:"bxs-hdd",title:"Hardware",content:vt()},{icon:"bxs-terminal",title:"Logs",content:xt()},{icon:"bxs-zap",title:"Agents",content:ut()}],onOpen:()=>{ke(),ye(),Ie(),de()},onClose:()=>{let a=i.indexOf(T);a>-1&&i.splice(a,1),m()}}),w=W({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ot(),onOpen:()=>Se(),onClose:()=>{let a=i.indexOf(w);a>-1&&i.splice(a,1),m()}}),r=W({id:"settings-window",title:"Settings",icon:"bx-cog",content:he(),onOpen:()=>{r.setContent(he()),Ae(r)},onClose:()=>{let a=i.indexOf(r);a>-1&&i.splice(a,1),m()}}),x=W({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:a=>{n.isOpen()||y(n),setTimeout(()=>{let c=document.querySelector(`.event-item[data-event-id="${a}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)},viewAlert:a=>{v.isOpen()||y(v),setTimeout(()=>{let c=document.querySelector(`.event-item[data-alert-id="${a}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)}},e){let a=document.getElementById("dexter-dropdown");a&&(a.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let c=document.getElementById("dexter-menu-container"),b=document.getElementById("dexter-menu-btn");c&&a&&b&&(c.addEventListener("mouseenter",()=>{a.classList.add("active"),b.classList.add("active")}),c.addEventListener("mouseleave",()=>{a.classList.remove("active"),b.classList.remove("active")}),b.addEventListener("click",S=>{S.stopPropagation();let h=localStorage.getItem("dex_last_window")||"alerts-window",$=null;h==="alerts-window"?$=v:h==="events-window"?$=n:h==="monitor-window"?$=T:h==="contacts-window"?$=u:h==="workspace-window"&&($=w),$&&y($)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{p("alerts-window"),y(v)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{p("events-window"),y(n)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{p("monitor-window"),y(T)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{p("contacts-window"),y(u)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{p("workspace-window"),y(w)}),document.getElementById("settings-icon")?.addEventListener("click",()=>y(r)),document.getElementById("close-all-windows")?.addEventListener("click",()=>f()),setInterval(()=>{v.isOpen()?K():Qe(),n.isOpen()&&Q(),T.isOpen()&&yt(),w.isOpen()&&Se()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{x.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",a=>{a.preventDefault();try{je(document.getElementById("email-input").value),window.location.reload()}catch(c){let b=document.getElementById("login-error");b&&(b.textContent=c.message,b.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Tt):Tt();})();

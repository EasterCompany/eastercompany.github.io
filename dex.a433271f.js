...
`,V=new Set,Ze=[],Z=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let o=await A("/roadmap");if(!o.ok)throw new Error("Offline");let a=await o.json();if(Ze=a,a.length===0){e.innerHTML=k("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let b=V.has(m.id),f=m.state==="draft",h=m.state==="published",v=m.state==="consumed",l="event-border-grey";h&&(l="event-border-blue"),v&&(l="event-border-purple");let S=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),r=document.createElement("div");r.className=`event-item notification-item ${l} cursor-pointer ${b?"expanded":""}`,r.dataset.itemId=m.id,r.onclick=c=>{if(c.target.closest("button"))return;r.classList.contains("expanded")?(r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",V.delete(m.id)):(r.classList.add("expanded"),r.querySelector(".event-details").style.display="block",V.add(m.id))};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;r.innerHTML=`
        <div class="event-time">
      ...
      `;let n=r.querySelector(".event-details");return n&&n.addEventListener("click",c=>{c.stopPropagation()}),r.querySelector(".edit-btn")?.addEventListener("click",()=>Ht(m)),r.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Nt(m)),r.querySelector(".delete-btn")?.addEventListener("click",()=>Rt(m.id)),r.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",V.delete(m.id)}),r},i=Array.from(e.children),u=new Map(i.map(m=>[m.dataset.itemId,m])),d=new Set(a.map(m=>m.id));i.forEach(m=>{let b=m.dataset.itemId;(!b||!d.has(b))&&m.remove()});let p=null;a.forEach((m,b)=>{let f=u.get(m.id);(!f||t)&&(f&&f.remove(),f=s(m),!f)||(b===0?e.firstElementChild!==f&&e.prepend(f):p&&p.nextElementSibling!==f&&p.after(f),p=f)})}catch{e.children.length===0&&(e.innerHTML=k("offline","Failed to load roadmap.","The event service may be offline."))}}}function Dt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),o=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Z=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",Z=null},o.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let i=document.getElementById("roadmap-editor-input").value;if(!i.trim())return;let u=Z?`/roadmap/${Z}`:"/roadmap",d=Z?"PATCH":"POST";try{await A(u,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:i})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{Ze.forEach(i=>V.add(i.id)),K(!0)},a.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function Ht(t){Z=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Nt(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(o){console.error(o)}}async function Rt(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var tt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Pt=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let o=await Ke("/contacts");if(!o.ok)throw new Error("Service unreachable");let s=(await o.json()).members||[];if(Pt=Date.now(),s.length===0){t.innerHTML=k("empty","No contacts found.","Check your Discord connection.");return}let i={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((u,d)=>{let p=i[u.level]??10,m=i[d.level]??10;return p!==m?p-m:u.username.localeCompare(d.username)}),t.innerHTML=s.map(u=>{let d=u.color?"#"+u.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=u.status==="online"?"#5eff5e":u.status==="idle"?"#ffa500":u.status==="dnd"?"#ff4d4d":"#666",m="#888";u.level==="Me"||u.level==="Master"?m="#bb86fc":u.level==="Admin"?m="#cf6679":u.level==="Moderator"?m="#03dac6":u.level==="Contributor"&&(m="#ffa500");let b=u.level==="Me",f=b?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=b?"2px solid #bb86fc":`1px solid ${d}33`;return`
                <div class="user-profile-section" style="background: ${f}; padding: 15px; border-radius: 8px; border: ${h}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${b?"#bb86fc":d}22, transparent 70%); pointer-events: none;"></div>
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${m}; font-weight: 600; text-transform: uppercase;">${b?"DEXTER (ME)":u.level}</span>
                        </div>
...
`),p=[...u.logs];if(p.length<25){let b=25-p.length;for(let f=0;f<b;f++)p.push("")}else p.length>25&&(p=p.slice(-25));let m=p.map(b=>me(b)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function wt(){await Promise.all([be(),Te(),Le()]),await le()}var dt=null,pt=null,mt=null;async function Et(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await A("/processes")).json()}catch{return null}}async function Vt(){try{return await(await A("/agent/status")).json()}catch{return null}}async function Te(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),u=document.querySelector("#hw-storage .hw-content"),d=r=>{if(r){if(o&&(o.innerHTML=`
                ...
                </div>`}function u(p){let m=p.status==="Downloaded",b=m?"service-widget-online":"service-widget-offline",f=m?"OK":"MISSING",h=m&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",v=p.name;return p.type==="custom"&&v.endsWith(":latest")&&(v=v.replace(":latest","")),`<div class="service-widget ${b}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${v}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${h}</span></div></div></div>`}let d="";if(a&&(d+=s(a)),e.xtts&&(d+=i(e.xtts)),d+=o.map(u).join(""),!d){t.innerHTML=k("empty","No models found.");return}t.innerHTML!==d&&(t.innerHTML=d)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),o=document.getElementById("guardian-t2-val"),a=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),i=document.getElementById("guardian-total-active"),u=document.getElementById("guardian-total-waste"),d=document.getElementById("guardian-reset-btn");d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{d.innerHTML="<i class='bx bx-error'></i> Failed"}},d.dataset.listenerAttached="true");let p=await Vt();if(p){let l=Math.floor(Date.now()/1e3),g=p.active_tier,S=c=>{c<0&&(c=0);let y=Math.floor(c/3600),$=Math.floor(c%3600/60),w=c%60;return y>0?`${y}h ${$}m`:$>0?`${$}m ${w}s`:`${w}s`},r=(c,y,$,w)=>{if(g===w)c.textContent="Working",c.style.color="#bb86fc";else if(w==="t1"&&g==="tests")c.textContent="Testing",c.style.color="#03dac6";else if($){let T=$.next_run-l;if(T<=0)c.textContent="Ready",c.style.color="#5eff5e";else{let I=Math.floor(T/60),_=T%60;c.textContent=`${I}m ${_}s`,c.style.color="#fff"}}y&&$&&(y.innerHTML=`
        ...
        `)};e&&r(e,document.getElementById("guardian-t1-stats"),p.t1,"t1"),o&&r(o,document.getElementById("guardian-t2-stats"),p.t2,"t2");let x=document.getElementById("system-state-label"),n=document.getElementById("system-state-val");if(n&&p.system_state){let c=p.system_state,y=S(p.system_state_time||0);x&&(x.textContent=`State: ${c.toUpperCase()}`),n.textContent=y,c==="idle"?n.style.color=p.system_state_time>300?"#5eff5e":"#fff":n.style.color="#bb86fc"}s&&(s.textContent=S(p.total_idle_time||0)),i&&(i.textContent=S(p.total_active_time||0)),u&&(u.textContent=S(p.total_waste_time||0))}else[e,o,a,s,i,u].forEach(g=>{g&&(g.textContent="-",g.style.color="#666")});let m=await Wt(),b=[],f=[];m&&(Array.isArray(m)?b=m:(b=m.active||[],f=m.history||[],f.sort((l,g)=>(g.end_time||0)-(l.end_time||0))));let h=document.getElementById("vitals-processes-val");if(h)if(m){let l=b.length;h.textContent=l>0?`${l} Active`:"Idle",h.style.color=l>0?"#bb86fc":"#fff"}else h.textContent="-",h.style.color="#888";if(m===null){t.children.length===0&&(t.innerHTML=k("offline","Failed to load process status.","The event service may be offline."));return}mt=Date.now(),O(1,mt),b.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=k("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,b,!1));let v=document.getElementById("processes-history-widgets");v&&(f.length===0?v.querySelector(".tab-placeholder")||(v.innerHTML=k("empty","No recent history.")):(v.querySelector(".tab-placeholder")&&(v.innerHTML=""),gt(v,f,!0))),ne(1,b.length)}function gt(t,e,o){function a(d){let p="";d.end_time?p=`${d.end_time-d.start_time}s`:p=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let m=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",b=d.channel_id,f={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};if(f[b]?b=f[b]:/^\d+$/.test(b)&&(b=`Channel ${b}`),o)return`
        <div class="process-history-item" data-channel-id="${d.channel_id}-${d.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let s=o?".process-history-item":".process-widget",i=new Map(Array.from(t.querySelectorAll(s)).map(d=>[d.dataset.channelId,d])),u=new Set(e.map(d=>`${d.channel_id}-${d.start_time}`));for(let[d,p]of i)u.has(d)||p.remove();e.forEach(d=>{let p=`${d.channel_id}-${d.start_time}`,m=a(d),b=i.get(p);if(b){b.outerHTML!==m&&(b.outerHTML=m);let f=t.querySelector(`[data-channel-id="${p}"]`);f&&t.appendChild(f)}else t.insertAdjacentHTML("beforeend",m)})}function xe(){let t=se(),e=ze()||"master@easter.company",o={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,o}function $t(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function St(t,e,o="output"){if(!Ie)return;let a=document.createElement("div");a.className=`terminal-line terminal-${o}`,o==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=me(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let o=Yt(e);St(o,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(u=>setTimeout(u,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),St(o,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code").textContent;navigator.clipboard.writeText(o).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-5px)",o.style.borderColor="rgba(255,255,255,0.15)",o.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)",o.style.borderColor="rgba(255,255,255,0.05)",o.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=o.dataset.cmd;Xt(a)})})}async function Qt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${$e()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Lt(){Qt(),Ge(),He();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=Ue();Ne(e),Oe();let o=document.getElementById("nav-left-container");o&&o.addEventListener("click",()=>{if(i.length>0)f();else{let n=window.location.pathname;if(!(n==="/"||n==="/index.html")){let $=(n.endsWith("/")&&n.length>1?n.slice(0,-1):n).split("/");$.pop();let w=$.join("/")||"/";window.location.href=w}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],u=document.getElementById("windows-container");u&&u.setAttribute("data-count","0");let d=n=>{localStorage.setItem("dex_last_window",n)};function p(){return 1}function m(){for(;i.length>1;)i.shift().close(!0);let n=document.getElementById("windows-container"),c=document.querySelector("nav"),y=document.querySelector("footer"),$=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");n&&n.setAttribute("data-count",i.length),i.forEach(_=>{let P=document.getElementById(_.id);P&&P.classList.remove("hide-close")}),re(se());let E=document.getElementById("dexter-menu-container"),T=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon");if(Re(i.length>0),i.length>0){if(y?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),c?.classList.add("window-open"),n&&(n.style.paddingTop="60px"),E&&(E.style.display="none"),I&&(I.style.display="none"),T){let _=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(T.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{d("alerts-window"),b(h)}),document.getElementById("switch-events").addEventListener("click",()=>{d("events-window"),b(v)}),document.getElementById("switch-monitor").addEventListener("click",()=>{d("monitor-window"),b(g)}),document.getElementById("switch-contacts").addEventListener("click",()=>{d("contacts-window"),b(l)}),document.getElementById("switch-workspace").addEventListener("click",()=>{d("workspace-window"),b(S)})):T.innerHTML=""}}else c?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),n&&(n.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),$||w?y?.classList.remove("hide"):y?.classList.add("hide"),E&&(E.style.display="flex"),I&&(I.style.display="block"),T&&(T.innerHTML="");ie(Ve())}function b(n){if(n.isOpen()){n.close();return}for(;i.length>0;)i.pop().close(!0);i.push(n),n.open(),m()}function f(){[...i].forEach(c=>c.close()),i=[]}window.addEventListener("resize",m);let h=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Je(),onOpen:()=>W(),onClose:()=>{let n=i.indexOf(h);n>-1&&i.splice(n,1),m()}}),v=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:rt(),onOpen:()=>Y(),onClose:()=>{let n=i.indexOf(v);n>-1&&i.splice(n,1),m()}}),l=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:at(),onOpen:()=>Ce(),onClose:()=>{let n=i.indexOf(l);n>-1&&i.splice(n,1),m()}}),g=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:xt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Agents",content:ft()}],onOpen:()=>{Te(),be(),Le(),le()},onClose:()=>{let n=i.indexOf(g);n>-1&&i.splice(n,1),m()}}),S=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:it(),onOpen:()=>Se(),onClose:()=>{let n=i.indexOf(S);n>-1&&i.splice(n,1),m()}}),r=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:xe(),onOpen:()=>{r.setContent(xe()),ke(r)},onClose:()=>{let n=i.indexOf(r);n>-1&&i.splice(n,1),m()}}),x=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:n=>{v.isOpen()||b(v),setTimeout(()=>{let c=document.querySelector(`.event-item[data-event-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)},viewAlert:n=>{h.isOpen()||b(h),setTimeout(()=>{let c=document.querySelector(`.event-item[data-alert-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)}},e){let n=document.getElementById("dexter-dropdown");n&&(n.innerHTML=`

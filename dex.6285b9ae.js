...
`,re=null,z=new Set,ce=[];async function F(t=!1){let e=document.getElementById("notifications-list");if(!e)return;ft(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];re=Date.now(),M(0,re);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let y=parseInt(v);return c-y<g});u.sort((d,v)=>{let y=B=>{let A=B.event;if(typeof A=="string")try{A=JSON.parse(A)}catch{return"low"}return(A.priority||"low").toLowerCase()},L=B=>B==="critical"?4:B==="high"?3:B==="medium"?2:1,E=L(y(d)),C=L(y(v));return E!==C?C-E:v.timestamp-d.timestamp}),ce=u;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},x=[],m=new Set(u.map(d=>f(d))).size>1;if(u.length>0){let d=null;u.forEach(v=>{let y=f(v);m&&y!==d&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),d=y),x.push(v)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=T("empty","No notifications yet."),oe();return}let i=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let y=v.title||"Untitled Notification",L=v.body||"No description provided.",E=v.priority||"low",C=!!v.alert,B=v.category||"system",A=v.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${d.id}`),P=new Date(d.timestamp*1e3),_=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=H?"event-border-grey":"event-border-blue";!H&&C&&($="event-border-red"),H&&(E==="high"||E==="critical")?$="event-border-red":H&&E==="medium"&&($="event-border-orange");let N=H?"notification-read":"notification-unread",D=z.has(d.id),ue=D?"expanded":"",ge=D?"display: block;":"display: none;",$e="",Te="";A.length>0&&(Te=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${A.map(Z=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${Z.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),$e=`
                <div class="event-detail-row">
            ...
            `;let U=document.createElement("div");U.className=`event-item notification-item ${$} ${N} ${ue} cursor-pointer`,U.dataset.notificationId=d.id,U.onclick=function(Z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),z.delete(d.id);let ee=this.querySelector(".event-details");ee&&(ee.style.display="none")}else{this.classList.add("expanded"),z.add(d.id);let ee=this.querySelector(".event-details");if(ee&&(ee.style.display="block"),!localStorage.getItem(`notification_read_ts_${d.id}`)){localStorage.setItem(`notification_read_ts_${d.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ve="event-border-grey";E==="high"||E==="critical"?ve="event-border-red":E==="medium"&&(ve="event-border-orange"),this.classList.add(ve),oe()}}};let Ht=y,dt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[B]||"bx-bell";U.innerHTML=`
                ...
                <div class="event-content">
                    <div class="event-service">${B.toUpperCase()} ${C?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>
...
`,q=new Set,ze=[],K=null;async function j(t=!1){let e=document.getElementById("roadmap-list");if(e){vt();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let l=await a.json();if(ze=l,l.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=f=>{let x=q.has(f.id),p=f.state==="draft",m=f.state==="published",i=f.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),i&&(n="event-border-purple");let b=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${n} cursor-pointer ${x?"expanded":""}`,h.dataset.itemId=f.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",q.delete(f.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",q.add(f.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;return h.innerHTML=`
        <div class="event-time">
...
`,Fe=null,J=new Set,je=[];async function de(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;xt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];if(je=r,Fe=Date.now(),M(2,Fe),r.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),O(1,0);return}t&&(e.innerHTML="");let c=p=>{let m=p.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let i=m.title||"Untitled Blueprint",n=m.summary||m.body||"No summary provided.",o=m.content||"",b=m.category||"architecture",h=m.affected_services||[],w=m.implementation_path||[],d=new Date(p.timestamp*1e3),v=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=J.has(p.id),E=L?"display: block;":"display: none;",C=document.createElement("div");C.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""}`,C.dataset.blueprintId=p.id;let A={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[b]||"bx-paint";C.onclick=function(_){if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(p.id);let $=this.querySelector(".event-details");$&&($.style.display="none")}else{this.classList.add("expanded"),J.add(p.id);let $=this.querySelector(".event-details");$&&($.style.display="block")}};let Q=h.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${h.join(", ")}</div>`:"",H="";w.length>0&&(H=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${w.map(_=>`<li>${k(_)}</li>`).join("")}
                        </ul>
                    </div>
                `),C.innerHTML=`
                <div class="event-time">
...
`,Ve=null;async function ye(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>ye(),e.dataset.listenerAttached="true");try{let a=await Oe("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ve=Date.now(),M(4,Ve),s.length===0){t.innerHTML=T("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((c,g)=>{let u=r[c.level]??10,f=r[g.level]??10;return u!==f?u-f:c.username.localeCompare(g.username)}),t.innerHTML=s.map(c=>{let g=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",u=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",f="#888";c.level==="Me"||c.level==="Master User"?f="#bb86fc":c.level==="Admin"?f="#cf6679":c.level==="Moderator"?f="#03dac6":c.level==="Contributor"&&(f="#ffa500");let x=c.level==="Me",p=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",m=x?"2px solid #bb86fc":`1px solid ${g}33`;return`
...
`,me=null,Y=new Set,se=[],pe="all",Et={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},St={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Je(t){for(let[e,a]of Object.entries(Et))if(a.includes(t))return e;return"system"}function Lt(t){return St[t]||"bx-square-rounded"}async function X(t=!1){let e=document.getElementById("events-timeline");if(!e)return;$t();let l=`/events?ml=${pe==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(l);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],g=c;if(pe!=="all"&&(g=c.filter(i=>{let n=i.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!1}return Je(n.type)===pe})),se=g.slice(0,50),me=Date.now(),M(1,me),se.length===0){e.innerHTML=T("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=i=>{let n=i.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let o=n.type,b=Je(o),h=Lt(o),w=o==="engagement.decision"||o==="messaging.bot.sent_message"||o==="messaging.user.sent_message"||o==="moderation.explicit_content.deleted"||o==="analysis.link.completed"||o==="analysis.visual.completed"||o==="system.cli.command"||o==="system.analysis.audit"||o==="system.test.completed"||o==="error_occurred"||o==="system.cli.status"||o.startsWith("system.roadmap")||o.startsWith("system.process"),d="event-border-grey";w&&(o==="moderation.explicit_content.deleted"||o==="error_occurred"?d="event-border-red":o==="analysis.link.completed"||o==="analysis.visual.completed"||o==="system.analysis.audit"?d="event-border-purple":o==="system.cli.command"||o==="system.cli.status"?d="event-border-orange":o==="system.test.completed"?d=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let v=w?"cursor-pointer":"",y=Y.has(i.id),L=y?"expanded":"",E=y?"display: block;":"display: none;",C=new Date(i.timestamp*1e3),B=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),Q=Ke(o,n),H=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",P="";if(w){let S="";if(o==="engagement.decision")S=`
                ...
                `}let _=document.createElement("div");if(_.className=`event-item ${d} ${v} ${L}`,_.dataset.eventId=i.id,_.onclick=function(S){if(!w)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(i.id);let N=this.querySelector(".event-details");N&&(N.style.display="none")}else{this.classList.add("expanded"),Y.add(i.id);let N=this.querySelector(".event-details");N&&(N.style.display="block")}},_.innerHTML=`
                <div class="event-time">
...
`),u=[...c.logs];if(u.length<25){let x=25-u.length;for(let p=0;p<x;p++)u.push("")}else u.length>25&&(u=u.slice(-25));let f=u.map(x=>le(x)).join(`
`);return`
    ...
    `;async function we(){await Promise.all([nt(),It(),At()]),await xe()}var Qe=null,Ze=null,et=null;async function it(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function tt(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Ct(){try{return await(await I("/processes")).json()}catch{return null}}async function kt(){try{return await(await I("/analyst/status")).json()}catch{return null}}async function It(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),l=i=>{if(!i){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let n="",o=(i.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(n+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
            ...
            </div>`,i.CPU&&i.CPU.length>0){let b=i.CPU[0];n+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}if(i.GPU&&i.GPU.length>0&&i.GPU.forEach((b,h)=>{let w=(b.VRAM/1073741824).toFixed(1);n+=`
                    <div class="service-widget" style="padding: 10px; min-height: 80px;">
                        <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    </div>`}),i.STORAGE&&i.STORAGE.length>0){let b=0,h=0;i.STORAGE.forEach(y=>{b+=y.USED,h+=y.SIZE});let w=(b/(1024*1024*1024)).toFixed(1),d=(h/(1024*1024*1024)).toFixed(1),v=h>0?(b/h*100).toFixed(0):0;n+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=n};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let n=await tt();n?(l(n),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML='<p style="color: #ff4d4d;">Failed to refresh.</p>',a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let i=e.querySelector("p");if(i&&i.textContent==="Loading hardware info..."){let n=await tt();l(n)}}if(!t)return;let s=await it();if(!s||!s.services){t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}Qe=Date.now(),M(3,Qe);let r=s.services||[];Array.from(t.children).forEach(i=>{i.classList.contains("service-widget")||i.remove()});function c(i){return!i||i==="N/A"||i==="unknown"||i.trim()===""?"-":i}function g(i){if(!i||i==="N/A"||i==="unknown")return"-";let n=i.match(/^(\d+\.\d+\.\d+)/);return n?n[0]:i.split(".").slice(0,3).join(".")||"-"}function u(i){return!i||i.length<=28?i:i.substring(0,28)+"..."}function f(i){if(!i||i==="N/A"||i==="unknown")return"-";let n=i.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!n)return"-";let o=parseInt(n[1])||0,b=parseInt(n[2])||0,h=parseInt(n[3])||0,w=parseFloat(n[4])||0,d=o*86400+b*3600+h*60+w,v=Math.floor(d/86400);if(v>0)return`${v}d`;let y=Math.floor(d/3600);if(y>0)return`${y}h`;let L=Math.floor(d/60);return L>0?`${L}m`:`${Math.floor(d)}s`}function x(i){let n=i.status==="online",o=n?"service-widget-online":"service-widget-offline",b=n?"bx-check-circle":"bx-x-circle",h=n?"OK":"BAD",w=i.version?g(i.version.str):"-",d=i.cpu&&i.cpu!=="N/A"?i.cpu:"-",v=i.memory&&i.memory!=="N/A"?i.memory:"-",y=d!=="-"?"#00ff00":"#666",L=d!=="-"?"#fff":"#666",E=v!=="-"?"#00ff00":"#666",C=v!=="-"?"#fff":"#666",B=f(i.uptime),A="";return n?A=`
                ...
                </div>`:A='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${o}" data-service-id="${i.id}"><div class="service-widget-header"><i class="bx ${b}"></i><h3>${i.short_name||i.id}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${u(i.domain&&i.port?`${i.domain}:${i.port}`:"")}</span></div>${A}</div></div>`}let p=new Map(Array.from(t.querySelectorAll(".service-widget")).map(i=>[i.dataset.serviceId,i])),m=new Set(r.map(i=>i.id));for(let[i,n]of p)m.has(i)||n.remove();r.forEach(i=>{let n=x(i),o=p.get(i.id);o?o.outerHTML!==n&&(o.outerHTML=n):t.insertAdjacentHTML("beforeend",n)})}async function At(){let t=document.getElementById("models-widgets");if(!t)return;let e=await it();if(!e){t.innerHTML=T("offline","Failed to load model status.");return}Ze=Date.now(),M(3,Ze);let a=e.models||[],l=e.whisper;Array.from(t.children).forEach(g=>{g.classList.contains("service-widget")||g.remove()});function s(g){let u=g.status==="Ready";return`
                ...
                </div>`}function r(g){let u=g.status==="Downloaded",f=u?"service-widget-online":"service-widget-offline",x=u?"OK":"MISSING",p=u&&g.size>0?`${(g.size/1e9).toFixed(2)} GB`:"-",m=g.name;return g.type==="custom"&&m.endsWith(":latest")&&(m=m.replace(":latest","")),`<div class="service-widget ${f}" data-model-name="${g.name}"><div class="service-widget-header"><i class="bx ${u?"bx-check-circle":"bx-x-circle"}"></i><h3>${m}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${g.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${p}</span></div></div></div>`}let c="";if(l&&(c+=s(l)),c+=a.map(r).join(""),!c){t.innerHTML=T("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function nt(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),l=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),nt()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await kt();if(c){let m=Math.floor(Date.now()/1e3),i=c.active_tier,n=(o,b,h)=>{if(i===h||h==="guardian"&&i==="tests"){o.textContent="Working",o.style.color="#bb86fc";return}let w=b-m;if(w<=0)o.textContent="Ready",o.style.color="#5eff5e";else{let d=Math.floor(w/60),v=w%60;o.textContent=`${d}m ${v}s`,o.style.color="#fff"}};if(e&&c.guardian&&n(e,c.guardian.next_run,"guardian"),a&&c.architect&&n(a,c.architect.next_run,"architect"),l&&c.strategist&&n(l,c.strategist.next_run,"strategist"),s&&c.system_idle_time!==void 0){let o=c.system_idle_time,b=Math.floor(o/60),h=o%60;s.textContent=`${b}m ${h}s`,o>300?s.style.color="#5eff5e":o>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,l,s].forEach(i=>{i&&(i.textContent="Offline",i.style.color="#ff4d4d")});let g=await Ct(),u=document.getElementById("vitals-processes-val");if(u)if(g){let m=g.length;u.textContent=m>0?`${m} Active`:"Idle",u.style.color=m>0?"#bb86fc":"#fff"}else u.textContent="-",u.style.color="#888";if(g===null){t.innerHTML=T("offline","Failed to load process status.");return}if(et=Date.now(),M(3,et),g.length===0){t.innerHTML=T("empty","No active processes."),O(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function f(m){let i=Math.floor(Date.now()/1e3-m.start_time),n=m.retries>0?`<span class="process-retry-badge">Retry ${m.retries}</span>`:"",o=m.channel_id,b={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return b[o]?o=b[o]:/^\d+$/.test(o)&&(o=`Channel ${o}`),`

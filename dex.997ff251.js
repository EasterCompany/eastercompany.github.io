...
`,R=null,J=new Set,Be=[];async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ve();let a=localStorage.getItem("service_map");if(!a){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(a).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!o){e.innerHTML=$("error","Event service not found in service map.");return}let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let u=await fetch(v);if(!u.ok)throw new Error("Service is offline or unreachable.");let g=(await u.json()).events||[];if(Be=g,R=Date.now(),E(3,R),g.length===0){e.innerHTML=$("empty","No events found.");return}t&&(e.innerHTML="");let x=c=>{let s=c.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let p=s.type,f=p==="engagement.decision"||p==="messaging.bot.sent_message"||p==="messaging.user.sent_message"||p==="moderation.explicit_content.deleted"||p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.cli.command"||p==="system.analysis.audit",h="event-border-grey";f&&(p==="moderation.explicit_content.deleted"?h="event-border-red":p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.analysis.audit"?h="event-border-purple":p==="system.cli.command"?h="event-border-orange":h="event-border-blue");let y=f?"cursor-pointer":"",w=J.has(c.id),b=w?"expanded":"",T=w?"display: block;":"display: none;",S=new Date(c.timestamp*1e3),N=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),W=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),B=Ne(p,s),A="";if(f){let L="";if(p==="engagement.decision")L=`
                    ...
                    `;else if(p==="analysis.visual.completed"){let _="";s.base64_preview?_=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(_=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),L=`
                        <div class="event-detail-row">
                    ...
                    `;else if(p==="messaging.user.sent_message"){let _="";s.attachments&&s.attachments.length>0&&(_=`
                            <div class="event-detail-block">
                                <span class="detail-label">Attachments:</span>
                                <div class="attachments-grid">${s.attachments.map(M=>{let H=M.content_type&&M.content_type.startsWith("image/"),pe=(M.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${M.content_type} \u2022 ${pe}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),L=`
                        <div class="event-detail-row">
                ...
                `}let D=document.createElement("div");if(D.className=`event-item ${h} ${y} ${b}`,D.dataset.eventId=c.id,D.onclick=function(L){if(!f)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(c.id);let I=this.querySelector(".event-details");I&&(I.style.display="none")}else{this.classList.add("expanded"),J.add(c.id);let I=this.querySelector(".event-details");I&&(I.style.display="block")}},D.innerHTML=`
            ...
            `,f){let L=D.querySelector(".close-details-btn");L&&L.addEventListener("click",_=>{_.stopPropagation();let I=_.target.closest(".event-item");if(I){I.classList.remove("expanded"),J.delete(c.id);let M=I.querySelector(".event-details");M&&(M.style.display="none")}})}return D},i=Array.from(e.children),n=new Map(i.map(c=>[c.dataset.eventId,c])),m=new Set(g.map(c=>c.id));i.forEach(c=>{let s=c.dataset.eventId;(!s||!m.has(s))&&c.remove()});let r=null;g.forEach((c,s)=>{let p=n.get(c.id);(!p||t)&&(p&&p.remove(),p=x(c),!p)||(s===0?e.firstElementChild!==p&&e.prepend(p):r&&r.nextElementSibling!==p&&r.after(p),r=p)}),R=Date.now(),E(3,R)}catch(u){console.error("Error fetching events:",u),e.children.length===0&&(e.innerHTML=$("offline","Failed to load events.","The event service may be offline or unreachable."))}}function Ve(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Be.forEach(a=>J.add(a.id)),ee(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{J.clear(),ee(!0)},e.dataset.listenerAttached="true")}var He=()=>`
    <div class="notifications-actions">
...
`,G=null,P=new Set,ie=[];async function q(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Ye(),t&&(e.innerHTML="<p>Updating...</p>");let a=localStorage.getItem("service_map");if(!a){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(a).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!o){e.innerHTML=$("error","Event service not found in service map.");return}let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=1000&format=json&event.type=system.notification.generated`;try{let u=await fetch(v);if(!u.ok)throw new Error("Service is offline or unreachable.");let g=(await u.json()).events||[];G=Date.now(),E(0,G);let x=Date.now(),i=24*60*60*1e3,n=g.filter(f=>{let h=localStorage.getItem(`notification_read_ts_${f.id}`);if(!h)return!0;let y=parseInt(h);return x-y<i});if(n.sort((f,h)=>{let y=S=>{let N=S.event;if(typeof N=="string")try{N=JSON.parse(N)}catch{return"low"}return(N.priority||"low").toLowerCase()},w=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,b=w(y(f)),T=w(y(h));return b!==T?T-b:h.timestamp-f.timestamp}),ie=n,t&&(e.innerHTML=""),n.length===0){e.innerHTML=$("empty","No notifications yet."),ae();return}let m=f=>{let h=f.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let y=h.title||"Untitled Notification",w=h.body||"No description provided.",b=h.priority||"low",T=!!h.alert,S=h.category||"system",N=h.related_event_ids||[],B=!!localStorage.getItem(`notification_read_ts_${f.id}`),A=new Date(f.timestamp*1e3),D=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=B?"event-border-grey":"event-border-blue";!B&&T&&(_="event-border-red"),B&&(b==="high"||b==="critical")?_="event-border-red":B&&b==="medium"&&(_="event-border-orange");let I=B?"notification-read":"notification-unread",M=P.has(f.id),H=M?"expanded":"",pe=M?"display: block;":"display: none;",we="",$e="";N.length>0&&($e=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${N.map(X=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${X.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),we=`
                <div class="event-detail-row">
                ...
                ${$e}
            `;let j=document.createElement("div");j.className=`event-item notification-item ${_} ${I} ${H} cursor-pointer`,j.dataset.notificationId=f.id,j.onclick=function(X){if(this.classList.contains("expanded")){this.classList.remove("expanded"),P.delete(f.id);let Q=this.querySelector(".event-details");Q&&(Q.style.display="none")}else{this.classList.add("expanded"),P.add(f.id);let Q=this.querySelector(".event-details");if(Q&&(Q.style.display="block"),!localStorage.getItem(`notification_read_ts_${f.id}`)){localStorage.setItem(`notification_read_ts_${f.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ue="event-border-grey";b==="high"||b==="critical"?ue="event-border-red":b==="medium"&&(ue="event-border-orange"),this.classList.add(ue),ae()}}},j.innerHTML=`
                <div class="event-time">
            ...
            `;let Se=j.querySelector(".close-details-btn");return Se&&Se.addEventListener("click",X=>{X.stopPropagation(),j.classList.remove("expanded");let me=j.querySelector(".event-details");me&&(me.style.display="none"),P.delete(f.id)}),j},r=Array.from(e.children),c=new Map(r.map(f=>[f.dataset.notificationId,f])),s=new Set(n.map(f=>f.id));r.forEach(f=>{let h=f.dataset.notificationId;(!h||!s.has(h))&&f.remove()});let p=null;n.forEach((f,h)=>{let y=c.get(f.id);(!y||t)&&(y&&y.remove(),y=m(f),!y)||(h===0?e.firstElementChild!==y&&e.prepend(y):p&&p.nextElementSibling!==y&&p.after(y),p=y)}),G=Date.now(),E(0,G),ae()}catch(u){console.error("Error fetching notifications:",u),e.children.length===0&&(e.innerHTML=$("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function Ye(){let t=document.getElementById("notif-read-all"),e=document.getElementById("notif-expand-all"),a=document.getElementById("notif-close-all"),o=document.getElementById("notif-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ie.forEach(l=>{localStorage.getItem(`notification_read_ts_${l.id}`)||localStorage.setItem(`notification_read_ts_${l.id}`,Date.now().toString())}),q(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ie.forEach(l=>{P.add(l.id),localStorage.getItem(`notification_read_ts_${l.id}`)||localStorage.setItem(`notification_read_ts_${l.id}`,Date.now().toString())}),q(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{P.clear(),q(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let l=Date.now()-1728e5;ie.forEach(v=>{localStorage.setItem(`notification_read_ts_${v.id}`,l.toString())}),P.clear(),q(!0)},o.dataset.listenerAttached="true")}var je=()=>`
    <div class="notifications-actions">
...
`,oe=null,V=new Set,Ue=[];async function te(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ke();let a=localStorage.getItem("service_map");if(!a){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(a).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!o){e.innerHTML=$("error","Event service not found in service map.");return}let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=1000&format=json&event.type=system.blueprint.generated`;try{let u=await fetch(v);if(!u.ok)throw new Error("Service is offline or unreachable.");let g=(await u.json()).events||[];if(Ue=g,oe=Date.now(),E(1,oe),g.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),O(1,0);return}t&&(e.innerHTML="");let x=c=>{let s=c.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let p=s.title||"Untitled Blueprint",f=s.summary||s.body||"No summary provided.",h=s.content||"",y=s.category||"architecture",w=s.affected_services||[],b=s.implementation_path||[],T=new Date(c.timestamp*1e3),S=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),N=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),W=V.has(c.id),B=W?"display: block;":"display: none;",A=document.createElement("div");A.className=`event-item notification-item event-border-purple cursor-pointer ${W?"expanded":""}`,A.dataset.blueprintId=c.id,A.onclick=function(I){if(this.classList.contains("expanded")){this.classList.remove("expanded"),V.delete(c.id);let H=this.querySelector(".event-details");H&&(H.style.display="none")}else{this.classList.add("expanded"),V.add(c.id);let H=this.querySelector(".event-details");H&&(H.style.display="block")}};let D=w.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${w.join(", ")}</div>`:"",L="";b.length>0&&(L=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${b.map(I=>`<li>${k(I)}</li>`).join("")}
                        </ul>
                    </div>
                `),A.innerHTML=`
                <div class="event-time">
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${B}">
                        ...
                        <div class="event-detail-block" style="text-align: left; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px; margin: 10px 0;">
                            <p class="detail-pre" style="white-space: pre-wrap; font-family: 'JetBrains Mono', monospace; font-size: 0.85em; text-align: left; color: #ccc;">${k(h)}</p>
                        </div>
                        ${L}
                    </div>
                </div>
            `;let _=A.querySelector(".close-details-btn");return _&&_.addEventListener("click",I=>{I.stopPropagation(),A.classList.remove("expanded");let M=A.querySelector(".event-details");M&&(M.style.display="none"),V.delete(c.id)}),A},i=Array.from(e.children),n=new Map(i.map(c=>[c.dataset.blueprintId,c])),m=new Set(g.map(c=>c.id));i.forEach(c=>{let s=c.dataset.blueprintId;(!s||!m.has(s))&&c.remove()});let r=null;g.forEach((c,s)=>{let p=n.get(c.id);(!p||t)&&(p&&p.remove(),p=x(c),!p)||(s===0?e.firstElementChild!==p&&e.prepend(p):r&&r.nextElementSibling!==p&&r.after(p),r=p)}),O(1,g.length)}catch(u){console.error("Error fetching blueprints:",u),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Ke(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ue.forEach(a=>V.add(a.id)),te(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{V.clear(),te(!0)},e.dataset.listenerAttached="true")}var Oe=()=>localStorage.getItem("service_map")?'<div id="services-widgets" class="system-monitor-widgets"><p>Loading services...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),Pe=()=>localStorage.getItem("service_map")?'<div id="models-widgets" class="system-monitor-widgets"><p>Loading models...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),qe=()=>localStorage.getItem("service_map")?`
    ...
    `:$("config","No service map configured.","Upload service-map.json in Settings."),le=null,re=null,ce=null;async function Fe(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/system_monitor`,l=await fetch(o);if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);return await l.json()}catch(t){return console.error("Error fetching system data:",t),null}}async function Xe(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/processes`,l=await fetch(o);if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);return await l.json()}catch(t){return console.error("Error fetching process data:",t),null}}async function Qe(){if(!localStorage.getItem("service_map"))return null;try{let e=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!e)return null;let o=`http://${e.domain==="0.0.0.0"?"127.0.0.1":e.domain}:${e.port}/analyst/status`,l=await fetch(o);return l.ok?await l.json():null}catch{return null}}async function ye(){let t=document.getElementById("services-widgets");if(!t)return;let e=await Fe();if(!e||!e.services){t.innerHTML=$("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}le=Date.now(),E(5,le);let a=e.services||[];Array.from(t.children).forEach(i=>{i.classList.contains("service-widget")||i.remove()});function o(i){return!i||i==="N/A"||i==="unknown"||i.trim()===""?"-":i}function l(i){if(!i||i==="N/A"||i==="unknown")return"-";let n=i.match(/^(\d+\.\d+\.\d+)/);return n?n[0]:i.split(".").slice(0,3).join(".")||"-"}function v(i){return!i||i.length<=28?i:i.substring(0,28)+"..."}function u(i){if(!i||i==="N/A"||i==="unknown")return"-";let n=i.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!n)return"-";let m=parseInt(n[1])||0,r=parseInt(n[2])||0,c=parseInt(n[3])||0,s=parseFloat(n[4])||0,p=m*86400+r*3600+c*60+s,f=Math.floor(p/86400);if(f>0)return`${f}d`;let h=Math.floor(p/3600);if(h>0)return`${h}h`;let y=Math.floor(p/60);return y>0?`${y}m`:`${Math.floor(p)}s`}function d(i){let n=i.status==="online",m=n?"service-widget-online":"service-widget-offline",r=n?"bx-check-circle":"bx-x-circle",c=n?"OK":"BAD",s=i.version?l(i.version.str):"-",p=i.cpu&&i.cpu!=="N/A"?i.cpu:"-",f=i.memory&&i.memory!=="N/A"?i.memory:"-",h=p!=="-"?"#00ff00":"#666",y=p!=="-"?"#fff":"#666",w=f!=="-"?"#00ff00":"#666",b=f!=="-"?"#fff":"#666",T=u(i.uptime),S="";return n?S=`
                ...
                </div>`:S='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${m}" data-service-id="${i.id}"><div class="service-widget-header"><i class="bx ${r}"></i><h3>${i.short_name||i.id}</h3><span class="service-widget-status">${c}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${v(i.domain&&i.port?`${i.domain}:${i.port}`:"")}</span></div>${S}</div></div>`}let g=new Map(Array.from(t.querySelectorAll(".service-widget")).map(i=>[i.dataset.serviceId,i])),x=new Set(a.map(i=>i.id));for(let[i,n]of g)x.has(i)||n.remove();a.forEach(i=>{let n=d(i),m=g.get(i.id);m?m.outerHTML!==n&&(m.outerHTML=n):t.insertAdjacentHTML("beforeend",n)})}async function xe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Fe();if(!e){t.innerHTML=$("offline","Failed to load model status.");return}re=Date.now(),E(4,re);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function l(d){let g=d.status==="Ready";return`
...
`,F=new Set,ze=[],K=null;async function U(t=!1){let e=document.getElementById("roadmap-list");if(!e)return;Ze();let a=localStorage.getItem("service_map");if(!a)return;let o=null;try{o=(JSON.parse(a).services?.cs||[]).find(d=>d.id==="dex-event-service")}catch{return}if(!o)return;let v=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/roadmap`;try{let u=await fetch(v);if(!u.ok)throw new Error("Offline");let d=await u.json();if(ze=d,d.length===0){e.innerHTML=$("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let g=r=>{let c=F.has(r.id),s=r.state==="draft",p=r.state==="published",f=r.state==="consumed",h="event-border-grey";p&&(h="event-border-blue"),f&&(h="event-border-purple");let w=new Date(r.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),b=document.createElement("div");b.className=`event-item notification-item ${h} cursor-pointer ${c?"expanded":""}`,b.dataset.itemId=r.id,b.onclick=S=>{if(S.target.closest("button"))return;b.classList.contains("expanded")?(b.classList.remove("expanded"),b.querySelector(".event-details").style.display="none",F.delete(r.id)):(b.classList.add("expanded"),b.querySelector(".event-details").style.display="block",F.add(r.id))};let T=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${r.state.toUpperCase()}]</span>`;return b.innerHTML=`
      ...
      `,b.querySelector(".edit-btn")?.addEventListener("click",()=>et(r)),b.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>tt(r)),b.querySelector(".delete-btn")?.addEventListener("click",()=>st(r.id)),b.querySelector(".close-details-btn")?.addEventListener("click",S=>{S.stopPropagation(),b.classList.remove("expanded"),b.querySelector(".event-details").style.display="none",F.delete(r.id)}),b},x=Array.from(e.children),i=new Map(x.map(r=>[r.dataset.itemId,r])),n=new Set(d.map(r=>r.id));x.forEach(r=>{let c=r.dataset.itemId;(!c||!n.has(c))&&r.remove()});let m=null;d.forEach((r,c)=>{let s=i.get(r.id);(!s||t)&&(s&&s.remove(),s=g(r),!s)||(c===0?e.firstElementChild!==s&&e.prepend(s):m&&m.nextElementSibling!==s&&m.after(s),m=s)})}catch{e.innerHTML=$("error","Failed to load roadmap.")}}function Ze(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),l=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{K=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",K=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let v=document.getElementById("roadmap-editor-input").value;if(!v.trim())return;let d=JSON.parse(localStorage.getItem("service_map")).services.cs.find(n=>n.id==="dex-event-service"),g=d.domain==="0.0.0.0"?"127.0.0.1":d.domain,x=K?`http://${g}:${d.port}/roadmap/${K}`:`http://${g}:${d.port}/roadmap`,i=K?"PATCH":"POST";try{await fetch(x,{method:i,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:v})}),document.getElementById("roadmap-editor-container").style.display="none",U(!0)}catch(n){console.error(n)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{ze.forEach(v=>F.add(v.id)),U(!0)},o.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=()=>{F.clear(),U(!0)},l.dataset.listenerAttached="true")}function et(t){K=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function tt(t){let e=t.state==="published"?"draft":"published",o=JSON.parse(localStorage.getItem("service_map")).services.cs.find(v=>v.id==="dex-event-service"),l=o.domain==="0.0.0.0"?"127.0.0.1":o.domain;try{await fetch(`http://${l}:${o.port}/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),U(!0)}catch(v){console.error(v)}}async function st(t){if(!confirm("Delete this roadmap item?"))return;let a=JSON.parse(localStorage.getItem("service_map")).services.cs.find(l=>l.id==="dex-event-service"),o=a.domain==="0.0.0.0"?"127.0.0.1":a.domain;try{await fetch(`http://${o}:${a.port}/roadmap/${t}`,{method:"DELETE"}),F.delete(t),U(!0)}catch(l){console.error(l)}}function Re(){Ae(),Ee();let t=_e();Le(t),Te();let e=document.querySelector("footer"),a=null;function o(){a=null,e?.classList.remove("hide"),document.querySelectorAll(".nav-right i").forEach(n=>n.classList.remove("active","inactive"))}function l(n,m=null){let r=a&&a.id===n.id;a&&a.close(!r),r?a=null:setTimeout(()=>{n.open(),a=n,document.querySelectorAll(".nav-right i").forEach(c=>{let s=c===m;c.classList.toggle("active",s),c.classList.toggle("inactive",!s&&m)}),e?.classList.add("hide")},a?220:0)}async function v(){await Promise.all([ye(),xe(),ee(),de(),q(),te(),he()]);let n=setInterval(()=>{if(!i.isOpen())return clearInterval(n);E(4,be),E(3,R),E(5,re),E(6,le),E(2,ce),E(1,oe),E(0,G)},1e3),m=setInterval(()=>{if(!i.isOpen())return clearInterval(m);ee(),de(),q(),te(),he()},3e3),r=setInterval(()=>{if(!i.isOpen())return clearInterval(r);ye(),xe()},6e4)}async function u(){await U();let n=setInterval(()=>{if(!g.isOpen())return clearInterval(n);U()},5e3)}let d=Z({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="https://github.com/eastercompany" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in",onClose:o}),g=Z({id:"user-window",tabs:[{icon:"bx-user",title:"Profile",content:`<p style="padding: 20px;">Logged in as: ${ne()||"Unknown"}<br><br><span style="font-family: monospace; font-size: 0.8em; opacity: 0.6;">UUID: 313071000877137920</span></p>`,"data-tab-index":0},{icon:"bx-map-alt",title:"Roadmap",content:We(),"data-tab-index":1}],icon:"bx-user",onClose:o,onOpen:()=>setTimeout(u,100)}),x=Z({id:"settings-window",title:"Settings",content:Y(),icon:"bx-cog",onClose:o,onOpen:()=>{x.setContent(Y()),setTimeout(()=>se(x),50)}}),i=Z({id:"message-window",tabs:[{icon:"bx-bell",title:"Notifications",content:He(),"data-tab-index":0},{icon:"bx-paint",title:"Blueprints",content:je(),"data-tab-index":1},{icon:"bx-cog",title:"Processes",content:qe(),"data-tab-index":2},{icon:"bx-calendar-event",title:"Events",content:De(),"data-tab-index":3},{icon:"bx-history",title:"Logs",content:Ce(),"data-tab-index":4},{icon:"bx-brain",title:"Models",content:Pe(),"data-tab-index":5},{icon:"bx-line-chart",title:"Services",content:Oe(),"data-tab-index":6}],icon:"bxs-message-dots",onClose:o,onOpen:()=>setTimeout(v,100)});t?(document.getElementById("user-icon")?.addEventListener("click",n=>l(g,n.currentTarget)),document.getElementById("message-icon")?.addEventListener("click",n=>l(i,n.currentTarget)),document.getElementById("settings-icon")?.addEventListener("click",n=>l(x,n.currentTarget))):document.getElementById("login-btn")?.addEventListener("click",()=>{l(d),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",n=>{n.preventDefault();try{Ie(document.getElementById("email-input").value),window.location.reload()}catch(m){let r=document.getElementById("login-error");r&&(r.textContent=m.message,r.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Re):Re();})();
//# sourceMappingURL=dex.997ff251.js.map

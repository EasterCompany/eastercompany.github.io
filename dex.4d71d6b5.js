...
`,V=new Set,Ze=[],X=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Ht();try{let i=await I("/roadmap");if(!i.ok)throw new Error("Offline");let o=await i.json();if(Ze=o,o.length===0){e.innerHTML=$("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=p=>{let b=V.has(p.id),u=p.state==="draft",h=p.state==="published",n=p.state==="consumed",l="event-border-grey";h&&(l="event-border-blue"),n&&(l="event-border-purple");let x=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${l} cursor-pointer ${b?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=m=>{if(m.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",V.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",V.add(p.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;y.innerHTML=`
        <div class="event-time">
      ...
      `;let a=y.querySelector(".event-details");return a&&a.addEventListener("click",m=>{m.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>Nt(p)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Rt(p)),y.querySelector(".delete-btn")?.addEventListener("click",()=>Pt(p.id)),y.querySelector(".close-details-btn")?.addEventListener("click",m=>{m.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",V.delete(p.id)}),y},r=Array.from(e.children),c=new Map(r.map(p=>[p.dataset.itemId,p])),g=new Set(o.map(p=>p.id));r.forEach(p=>{let b=p.dataset.itemId;(!b||!g.has(b))&&p.remove()});let v=null;o.forEach((p,b)=>{let u=c.get(p.id);(!u||t)&&(u&&u.remove(),u=s(p),!u)||(b===0?e.firstElementChild!==u&&e.prepend(u):v&&v.nextElementSibling!==u&&v.after(u),v=u)})}catch{e.children.length===0&&(e.innerHTML=$("offline","Failed to load roadmap.","The event service may be offline."))}}}function Ht(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{X=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",X=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=X?`/roadmap/${X}`:"/roadmap",g=X?"PATCH":"POST";try{await I(c,{method:g,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{Ze.forEach(r=>V.add(r.id)),K(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function Nt(t){X=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Rt(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(i){console.error(i)}}async function Pt(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var tt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,et=null,Q=new Set,nt=[];async function ue(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ot();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await I(i);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(nt=r,et=Date.now(),N(2,et),r.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),j(1,0);return}t&&(e.innerHTML="");let c=u=>{let h=u.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let n=h.title||"Untitled Blueprint",l=h.summary||h.body||"No summary provided.",d=h.content||"",x=h.category||"architecture",y=h.affected_services||[],w=h.implementation_path||[],a=new Date(u.timestamp*1e3),m=a.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),f=a.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=Q.has(u.id),E=C?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${C?"expanded":""}`,S.dataset.blueprintId=u.id;let T={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";S.onclick=function(P){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Q.delete(u.id);let k=this.querySelector(".event-details");k&&(k.style.display="none")}else{this.classList.add("expanded"),Q.add(u.id);let k=this.querySelector(".event-details");k&&(k.style.display="block")}};let R=y.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${y.join(", ")}</span></div>`:"",M="";w.length>0&&(M=`
            ...
            `;let B=S.querySelector(".event-details");B&&B.addEventListener("click",P=>{P.stopPropagation()});let D=S.querySelector(".close-details-btn");return D&&D.addEventListener("click",P=>{P.stopPropagation(),S.classList.remove("expanded");let L=S.querySelector(".event-details");L&&(L.style.display="none"),Q.delete(u.id)}),S},g=Array.from(e.children),v=new Map(g.map(u=>[u.dataset.blueprintId,u])),p=new Set(r.map(u=>u.id));g.forEach(u=>{let h=u.dataset.blueprintId;(!h||!p.has(h))&&u.remove()});let b=null;r.forEach((u,h)=>{let n=v.get(u.id);(!n||t)&&(n&&n.remove(),n=c(u),!n)||(h===0?e.firstElementChild!==n&&e.prepend(n):b&&b.nextElementSibling!==n&&b.after(n),b=n)}),j(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline."))}}function Ot(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(i=>Q.add(i.id)),ue(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Q.clear(),ue(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`;async function $e(){await Promise.all([K(),ue()])}var ot=()=>`
    <div class="alerts-actions">
...
`,at=null;async function ge(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>ge(),e.dataset.listenerAttached="true");try{let i=await Ke("/contacts");if(!i.ok)throw new Error("Service unreachable");let s=(await i.json()).members||[];if(at=Date.now(),N(4,at),s.length===0){t.innerHTML=$("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((c,g)=>{let v=r[c.level]??10,p=r[g.level]??10;return v!==p?v-p:c.username.localeCompare(g.username)}),t.innerHTML=s.map(c=>{let g=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",p="#888";c.level==="Me"||c.level==="Master User"?p="#bb86fc":c.level==="Admin"?p="#cf6679":c.level==="Moderator"?p="#03dac6":c.level==="Contributor"&&(p="#ffa500");let b=c.level==="Me",u=b?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=b?"2px solid #bb86fc":`1px solid ${g}33`;return`
...
`,ve=null,Z=new Set,ie=[],fe="all",qt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},zt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function rt(t){for(let[e,i]of Object.entries(qt))if(i.includes(t))return e;return"system"}function Ft(t){return zt[t]||"bx-square-rounded"}async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;jt();let o=`/events?ml=${fe==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],g=c;if(fe!=="all"&&(g=c.filter(n=>{let l=n.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return!1}return rt(l.type)===fe})),ie=g.slice(0,50),ve=Date.now(),N(1,ve),ie.length===0){e.innerHTML=$("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=n=>{let l=n.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return null}let d=l.type,x=rt(d),y=Ft(d),w=d==="engagement.decision"||d==="messaging.bot.sent_message"||d==="messaging.user.sent_message"||d==="moderation.explicit_content.deleted"||d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.cli.command"||d==="system.analysis.audit"||d==="system.test.completed"||d==="error_occurred"||d==="system.cli.status"||d.startsWith("system.roadmap")||d.startsWith("system.process"),a="event-border-grey";w&&(d==="moderation.explicit_content.deleted"||d==="error_occurred"?a="event-border-red":d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.analysis.audit"?a="event-border-purple":d==="system.cli.command"||d==="system.cli.status"?a="event-border-orange":d==="system.test.completed"?a=l.test?.status==="OK"&&l.lint?.status==="OK"&&l.format?.status==="OK"?"event-border-blue":"event-border-red":a="event-border-blue");let m=w?"cursor-pointer":"",f=Z.has(n.id),C=f?"expanded":"",E=f?"display: block;":"display: none;",S=new Date(n.timestamp*1e3),_=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),R=lt(d,l),M=l.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${l.user_level})</span>`:"",B="";if(w){let L="";if(d==="engagement.decision")L=`
                ...
                `}let D=document.createElement("div");D.className=`event-item ${a} ${m} ${C}`,D.dataset.eventId=n.id,D.onclick=function(L){if(!w)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),Z.delete(n.id);let O=this.querySelector(".event-details");O&&(O.style.display="none")}else{this.classList.add("expanded"),Z.add(n.id);let O=this.querySelector(".event-details");O&&(O.style.display="block")}},D.innerHTML=`
                <div class="event-time">
...
`);return`
        ...
        <div id="processes-history-widgets" class="system-monitor-widgets" style="margin-bottom: 30px; opacity: 0.7;"></div>`,ht=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${dt()}
        </div>`;async function Et(){await Promise.all([be(),Le(),Ce()]),await ae()}var mt=null,pt=null,ut=null;async function St(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function gt(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await I("/processes")).json()}catch{return null}}async function Vt(){try{return await(await I("/analyst/status")).json()}catch{return null}}async function Le(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),i=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=$("offline","Failed to load hardware info.","The event service may be offline."));return}let l="",d=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(l+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,o]of Object.entries(t)){let s=Ct.filter(r=>r.category===i);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),ke=!0,i}function $t(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),ke=!1}function Lt(t,e,i="output"){if(!ke)return;let o=document.createElement("div");o.className=`terminal-line terminal-${i}`,i==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=de(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let i=Yt(e);Lt(i,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),Lt(i,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let o=i.dataset.cmd;Xt(o)})})}async function Qt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function kt(){Qt(),je(),De();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=Oe();He(e),Re();let i=document.getElementById("nav-left-container");i&&i.addEventListener("click",()=>{if(r.length>0)u();else{let a=window.location.pathname;if(!(a==="/"||a==="/index.html")){let C=(a.endsWith("/")&&a.length>1?a.slice(0,-1):a).split("/");C.pop();let E=C.join("/")||"/";window.location.href=E}}});let o=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!o&&!s&&document.querySelector("footer")?.classList.add("hide");let r=[],c=document.getElementById("windows-container");c&&c.setAttribute("data-count","0");let g=a=>{localStorage.setItem("dex_last_window",a)};function v(){return 1}function p(){for(;r.length>1;)r.shift().close(!0);let a=document.getElementById("windows-container"),m=document.querySelector("nav"),f=document.querySelector("footer"),C=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",r.length);let S=r.length>0;r.forEach(M=>{let B=document.getElementById(M.id);B&&B.classList.remove("hide-close")}),oe(se());let _=document.getElementById("dexter-menu-container"),T=document.getElementById("nav-window-switcher"),R=document.getElementById("settings-icon");if(Ne(r.length>0),r.length>0){if(f?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),m?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),_&&(_.style.display="none"),R&&(R.style.display="none"),T){let M=r[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(T.innerHTML=`

...
`,ne=null,R=new Set,Ce=[];async function Z(s=!1){let t=document.getElementById("events-timeline");if(!t)return;ze();let i=localStorage.getItem("service_map");if(!i){t.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{t.innerHTML=$("error","Invalid service map data.");return}if(!o){t.innerHTML=$("error","Event service not found in service map.");return}let m=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let v=await fetch(m);if(!v.ok)throw new Error("Service is offline or unreachable.");let u=(await v.json()).events||[];if(Ce=u,u.length===0){t.innerHTML=$("empty","No events found.");return}s&&(t.innerHTML="");let h=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let c=a.type,p=c==="engagement.decision"||c==="messaging.bot.sent_message"||c==="messaging.user.sent_message"||c==="moderation.explicit_content.deleted"||c==="analysis.link.completed"||c==="analysis.visual.completed"||c==="system.cli.command",g="event-border-grey";p&&(c==="moderation.explicit_content.deleted"?g="event-border-red":c==="analysis.link.completed"||c==="analysis.visual.completed"?g="event-border-purple":c==="system.cli.command"?g="event-border-orange":g="event-border-blue");let w=p?"cursor-pointer":"",x=R.has(b.id),E=x?"expanded":"",N=x?"display: block;":"display: none;",S=new Date(b.timestamp*1e3),B=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Y=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),U=Ie(c,a),K="";if(p){let _="";if(c==="engagement.decision")_=`
                        ...
                        `;else if(c==="messaging.bot.sent_message")_=`
                            <div class="event-detail-row">
                        ...
                        `;else if(c==="moderation.explicit_content.deleted")_=`
                            <div class="event-detail-row">
                        ...
                        `;else if(c==="analysis.link.completed")_=`
                            <div class="event-detail-row">
                        ...
                        `;else if(c==="analysis.visual.completed"){let k="";a.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(k=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),_=`
                            <div class="event-detail-row">
                        ...
                        `}else if(c==="system.cli.command")_=`
                            <div class="event-detail-row">
                    ...
                    `}let T=document.createElement("div");if(T.className=`event-item ${g} ${w} ${E}`,T.dataset.eventId=b.id,T.onclick=function(_){if(!p)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),R.delete(b.id);let I=this.querySelector(".event-details");I&&(I.style.display="none")}else{this.classList.add("expanded"),R.add(b.id);let I=this.querySelector(".event-details");I&&(I.style.display="block")}},T.innerHTML=`
...
`,ae=null,O=new Set,ie=[];async function P(s=!1){let t=document.getElementById("notifications-list");if(!t)return;Je(),s&&(t.innerHTML="<p>Updating...</p>");let i=localStorage.getItem("service_map");if(!i){t.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{t.innerHTML=$("error","Invalid service map data.");return}if(!o){t.innerHTML=$("error","Event service not found in service map.");return}let m=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=1000&format=json&event.type=system.notification.generated,system.analysis.audit`;try{let v=await fetch(m);if(!v.ok)throw new Error("Service is offline or unreachable.");let u=(await v.json()).events||[],h=Date.now(),n=24*60*60*1e3,d=u.filter(p=>{let g=localStorage.getItem(`notification_read_ts_${p.id}`);if(!g)return!0;let w=parseInt(g);return h-w<n});if(ie=d,s&&(t.innerHTML=""),d.length===0){t.innerHTML=$("empty","No notifications yet."),se();return}let f=p=>{let g=p.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let x=g.type==="system.analysis.audit",E=x?`Analysis Audit: ${g.tier?.toUpperCase()}`:g.title||"Untitled Notification",N=x?"Raw analyst input and output logs.":g.body||"No description provided.",S=x?"low":g.priority||"low",B=!x&&!!g.alert,Y=x?"audit":g.category||"system",U=g.related_event_ids||[],T=!!localStorage.getItem(`notification_read_ts_${p.id}`),_=new Date(p.timestamp*1e3),k=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=T?"event-border-grey":"event-border-blue";x&&(L=T?"event-border-grey":"event-border-purple"),!T&&B&&(L="event-border-red"),T&&(S==="high"||S==="critical")?L="event-border-red":T&&S==="medium"&&(L="event-border-orange");let j=T?"notification-read":"notification-unread",D=O.has(p.id),Pe=D?"expanded":"",Fe=D?"display: block;":"display: none;",de="";if(x)de=`
                    <div class="event-detail-row">
                ...
                `;else{let F="";U.length>0&&(F=`
                        <div class="event-detail-row">
                            ...
                            <span class="detail-value">${U.map(V=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${V.substring(0,8)}...</span>`).join(", ")}</span>
                        </div>`),de=`
                    <div class="event-detail-row">
            ...
            `;let we=H.querySelector(".close-details-btn");return we&&we.addEventListener("click",F=>{F.stopPropagation(),H.classList.remove("expanded");let V=H.querySelector(".event-details");V&&(V.style.display="none"),O.delete(p.id)}),H},y=Array.from(t.children),b=new Map(y.map(p=>[p.dataset.notificationId,p])),a=new Set(d.map(p=>p.id));y.forEach(p=>{let g=p.dataset.notificationId;(!g||!a.has(g))&&p.remove()});let c=null;d.forEach((p,g)=>{let w=b.get(p.id);(!w||s)&&(w&&w.remove(),w=f(p),!w)||(g===0?t.firstElementChild!==w&&t.prepend(w):c&&c.nextElementSibling!==w&&c.after(w),c=w)}),ae=Date.now(),M(0,ae),se()}catch(v){console.error("Error fetching notifications:",v),t.children.length===0&&(t.innerHTML=$("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function Je(){let s=document.getElementById("notif-read-all"),t=document.getElementById("notif-expand-all"),i=document.getElementById("notif-close-all"),o=document.getElementById("notif-clear");s&&!s.dataset.listenerAttached&&(s.onclick=()=>{ie.forEach(r=>{localStorage.getItem(`notification_read_ts_${r.id}`)||localStorage.setItem(`notification_read_ts_${r.id}`,Date.now().toString())}),P(!0)},s.dataset.listenerAttached="true"),t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ie.forEach(r=>{O.add(r.id),localStorage.getItem(`notification_read_ts_${r.id}`)||localStorage.setItem(`notification_read_ts_${r.id}`,Date.now().toString())}),P(!0)},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{O.clear(),P(!0)},i.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let r=Date.now()-1728e5;ie.forEach(m=>{localStorage.setItem(`notification_read_ts_${m.id}`,r.toString())}),O.clear(),P(!0)},o.dataset.listenerAttached="true")}var De=()=>`
    <div class="notifications-actions">
...
`,oe=null,z=new Set,He=[];async function J(s=!1){let t=document.getElementById("blueprints-list");if(!t)return;Ge();let i=localStorage.getItem("service_map");if(!i){t.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(i).services?.cs||[]).find(u=>u.id==="dex-event-service")}catch{t.innerHTML=$("error","Invalid service map data.");return}if(!o){t.innerHTML=$("error","Event service not found in service map.");return}let r=o.domain==="0.0.0.0"?"127.0.0.1":o.domain,m=`http://${r}:${o.port}/events?ml=100&format=json&event.type=system.blueprint.generated`,v=`http://${r}:${o.port}/analyst/status`;try{let l=await fetch(v);if(l.ok){let u=await l.json(),h=document.getElementById("strategist-next-run");if(h&&u.strategist){let n=u.strategist.next_run,d=Math.floor(Date.now()/1e3),f=n-d;if(f<=0)h.textContent="Next T3: Ready (IDLE req)",h.style.color="#5eff5e";else{let y=Math.floor(f/60),b=f%60;h.textContent=`Next T3: ${y}m ${b}s`,h.style.color="#888"}}}}catch(l){console.error("Failed to fetch analyst status",l)}try{let l=await fetch(m);if(!l.ok)throw new Error("Service is offline or unreachable.");let h=(await l.json()).events||[];if(He=h,h.length===0){t.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),q(1,0);return}s&&(t.innerHTML="");let n=a=>{let c=a.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return null}let p=c.title||"Untitled Blueprint",g=c.summary||c.body||"No summary provided.",w=c.content||"",x=c.category||"architecture",E=c.affected_services||[],N=c.implementation_path||[],S=new Date(a.timestamp*1e3),B=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Y=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),U=z.has(a.id),K=U?"display: block;":"display: none;",T=document.createElement("div");T.className=`event-item notification-item event-border-purple cursor-pointer ${U?"expanded":""}`,T.dataset.blueprintId=a.id,T.onclick=function(L){if(this.classList.contains("expanded")){this.classList.remove("expanded"),z.delete(a.id);let D=this.querySelector(".event-details");D&&(D.style.display="none")}else{this.classList.add("expanded"),z.add(a.id);let D=this.querySelector(".event-details");D&&(D.style.display="block")}};let _=E.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${E.join(", ")}</div>`:"",k="";N.length>0&&(k=`

...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Et()}
        </div>`;async function Dt(){await Promise.all([$e(),Fe(),Je()]),await Ce()}var St=null,kt=null;async function zt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function Tt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function vs(){try{return await(await B("/processes")).json()}catch{return null}}async function xs(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Fe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),v=r=>{if(r){if(i&&(i.innerHTML=`
        ...
        `)};e&&d(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",r),i&&d(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",r);let E=document.getElementById("analyzer-synthesis-val"),M=document.getElementById("analyzer-synthesis-stats");E&&d(E,M,p.synthesis,"synthesis",x);let A=document.getElementById("system-state-label"),I=document.getElementById("system-state-val");if(I&&p.system_state){let w=p.system_state,P=g(p.system_state_time||0);A&&(A.textContent=`State: ${w.toUpperCase()}`),I.textContent=P,w==="idle"?I.style.color=p.system_state_time>300?"#5eff5e":"#fff":I.style.color="#bb86fc"}s&&(s.textContent=g(p.total_idle_time||0)),l&&(l.textContent=g(p.total_active_time||0)),o&&(o.textContent=g(p.total_waste_time||0))}else[e,i,a,s,l,o].forEach(r=>{r&&(r.textContent="-",r.style.color="#666")});let h=await vs(),m=[],b=[],n=[];h&&(Array.isArray(h)?m=h:(m=h.active||[],b=h.queue||[],n=h.history||[],n.sort((y,r)=>(r.end_time||0)-(y.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=R("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),je(t,m,!1));let f=document.getElementById("processes-queue-widgets");f&&(b.length===0?!f.querySelector(".tab-placeholder")&&!f.querySelector('div[style*="font-style: italic"]')&&(f.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(f.innerHTML="",je(f,b,!1)));let S=document.getElementById("processes-history-widgets");S&&(n.length===0?S.querySelector(".tab-placeholder")||(S.innerHTML=R("empty","No recent history.")):(S.querySelector(".tab-placeholder")&&(S.innerHTML=""),je(S,n,!0))),he(1,m.length)}function je(t,e,i){function a(v){let c="";v.end_time?c=`${v.end_time-v.start_time}s`:c=`${Math.floor(Date.now()/1e3-v.start_time)}s`;let p=v.retries>0?`<span class="process-retry-badge">Retry ${v.retries}</span>`:"",h=v.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[h]?h=m[h]:/^\d+$/.test(h)&&(h=`Channel ${h}`),i)return`

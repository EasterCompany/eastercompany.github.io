        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ht()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let l=await $t();l?(b(l),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let l=await $t();b(l)}if(!t)return;let c=await It();if(!c||!c.services){t.children.length===0&&(t.innerHTML=R("offline","Failed to load system metrics.","The event service may be offline."));return}wt=Date.now(),Z(0,wt);let p=c.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:l.split(".").slice(0,3).join(".")||"-"}function g(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let x=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let f=parseInt(x[1])||0,v=parseInt(x[2])||0,d=parseInt(x[3])||0,E=parseFloat(x[4])||0,L=f*86400+v*3600+d*60+E,z=Math.floor(L/86400);if(z>0)return`${z}d`;let B=Math.floor(L/3600);if(B>0)return`${B}h`;let h=Math.floor(L/60);return h>0?`${h}m`:`${Math.floor(L)}s`}function u(l){let x=l.status==="online",f=x?"service-widget-online":"service-widget-offline",v=x?"bx-check-circle":"bx-x-circle",d=x?"OK":"BAD",E=l.version?m(l.version.str):"-",L=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",z=l.memory&&l.memory!=="N/A"?l.memory:"-",B=L!=="-"?"#00ff00":"#666",h=L!=="-"?"#fff":"#666",N=z!=="-"?"#00ff00":"#666",D=z!=="-"?"#fff":"#666",M=n(l.uptime),q="";x?q=`

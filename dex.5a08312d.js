(()=>{function Ee(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Le(s=!1){let n=`
    ...
    `,t=document.createElement("footer");t.innerHTML=s,document.body.appendChild(t)}function ne(s){let t=null,n=s.onClose||null,o=s.onOpen||null;function r(){if(t){t.classList.add("open"),window.addEventListener("resize",d),o&&setTimeout(o,10);return}let i=document.getElementById("windows-container");i||(i=document.createElement("div"),i.id="windows-container",i.className="windows-container",document.body.appendChild(i)),t=document.createElement("div"),t.id=s.id,t.className="window";let c=s.icon||"bx-window",b="",m="",g;s.tabs&&s.tabs.length>0?(b=`<div class="tab-bar">${s.tabs.map((w,$)=>{let x;return w.icon?x=`<i class="bx ${w.icon}"></i>`:w.title&&w.title.length>0?x=`<span class="tab-glyph">${w.title.charAt(0).toUpperCase()}</span>`:x='<i class="bx bx-question-mark"></i>',`
                ...
                `}).join("")}</div>`,g=`<div class="window-content">${s.tabs.map((w,$)=>`<div class="tab-content ${$===0?"active":""}" data-tab-content="${$}">${w.content}</div>`).join("")}</div>`):(s.title&&(m=`<div class="window-title">${s.title}</div>`),g=`<div class="window-content">${s.content}</div>`);let a=`
...
`,J=null,G=new Set,Be=[];async function ie(s=!1){let t=document.getElementById("events-timeline");if(!t)return;Ke();let n=localStorage.getItem("service_map");if(!n){t.innerHTML=S("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(n).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{t.innerHTML=S("error","Invalid service map data.");return}if(!o){t.innerHTML=S("error","Event service not found in service map.");return}let d=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let f=await fetch(d);if(!f.ok)throw new Error("Service is offline or unreachable.");let v=(await f.json()).events||[];if(Be=v,J=Date.now(),_(3,J),v.length===0){t.innerHTML=S("empty","No events found.");return}s&&(t.innerHTML="");let y=g=>{let a=g.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let p=a.type,u=p==="engagement.decision"||p==="messaging.bot.sent_message"||p==="messaging.user.sent_message"||p==="moderation.explicit_content.deleted"||p==="analysis.link.completed"||p==="analysis.visual.completed"||p==="system.cli.command",h="event-border-grey";u&&(p==="moderation.explicit_content.deleted"?h="event-border-red":p==="analysis.link.completed"||p==="analysis.visual.completed"?h="event-border-purple":p==="system.cli.command"?h="event-border-orange":h="event-border-blue");let w=u?"cursor-pointer":"",$=G.has(g.id),x=$?"expanded":"",D=$?"display: block;":"display: none;",E=new Date(g.timestamp*1e3),B=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Z=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),j=De(p,a),ee="";if(u){let I="";if(p==="engagement.decision")I=`
                        ...
                        `;else if(p==="messaging.bot.sent_message")I=`
                            <div class="event-detail-row">
                        ...
                        `;else if(p==="moderation.explicit_content.deleted")I=`
                            <div class="event-detail-row">
                        ...
                        `;else if(p==="analysis.link.completed")I=`
                            <div class="event-detail-row">
                        ...
                        `;else if(p==="analysis.visual.completed"){let k="";a.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(k=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),I=`
                            <div class="event-detail-row">
                        ...
                        `}else if(p==="system.cli.command")I=`
                            <div class="event-detail-row">
                        ...
                        `;else if(p==="messaging.user.sent_message"){let k="";a.attachments&&a.attachments.length>0&&(k=`
                                <div class="event-detail-block">
                    ...
                    `}let T=document.createElement("div");if(T.className=`event-item ${h} ${w} ${x}`,T.dataset.eventId=g.id,T.onclick=function(I){if(!u)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),G.delete(g.id);let A=this.querySelector(".event-details");A&&(A.style.display="none")}else{this.classList.add("expanded"),G.add(g.id);let A=this.querySelector(".event-details");A&&(A.style.display="block")}},T.innerHTML=`
                    <div class="event-time">
                        <span class="event-time-main">${B}</span>
                        <span class="event-date">${Z}</span>
                    </div>
                    <div class="event-content">
                        <div class="event-service">${g.service}</div>
                        <div class="event-message">${j}</div>
                        ${ee}
                    </div>
                `,u){let I=T.querySelector(".close-details-btn");I&&I.addEventListener("click",k=>{k.stopPropagation();let A=k.target.closest(".event-item");if(A){A.classList.remove("expanded"),G.delete(g.id);let L=A.querySelector(".event-details");L&&(L.style.display="none")}})}return T},i=Array.from(t.children),c=new Map(i.map(g=>[g.dataset.eventId,g])),b=new Set(v.map(g=>g.id));i.forEach(g=>{let a=g.dataset.eventId;(!a||!b.has(a))&&g.remove()});let m=null;v.forEach((g,a)=>{let p=c.get(g.id);(!p||s)&&(p&&p.remove(),p=y(g),!p)||(a===0?t.firstElementChild!==p&&t.prepend(p):m&&m.nextElementSibling!==p&&m.after(p),m=p)}),J=Date.now(),_(3,J)}catch(f){console.error("Error fetching events:",f),t.children.length===0&&(t.innerHTML=S("offline","Failed to load events.","The event service may be offline or unreachable."))}}function Ke(){let s=document.getElementById("events-expand-all"),t=document.getElementById("events-close-all");s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Be.forEach(n=>G.add(n.id)),ie(!0)},s.dataset.listenerAttached="true"),t&&!t.dataset.listenerAttached&&(t.onclick=()=>{G.clear(),ie(!0)},t.dataset.listenerAttached="true")}var He=()=>`
    <div class="notifications-actions">
...
`,Y=null,P=new Set,re=[];async function F(s=!1){let t=document.getElementById("notifications-list");if(!t)return;Xe(),s&&(t.innerHTML="<p>Updating...</p>");let n=localStorage.getItem("service_map");if(!n){t.innerHTML=S("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(n).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{t.innerHTML=S("error","Invalid service map data.");return}if(!o){t.innerHTML=S("error","Event service not found in service map.");return}let d=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/events?ml=1000&format=json&event.type=system.notification.generated,system.analysis.audit`;try{let f=await fetch(d);if(!f.ok)throw new Error("Service is offline or unreachable.");let v=(await f.json()).events||[];Y=Date.now(),_(0,Y);let y=Date.now(),i=24*60*60*1e3,c=v.filter(u=>{let h=localStorage.getItem(`notification_read_ts_${u.id}`);if(!h)return!0;let w=parseInt(h);return y-w<i});if(re=c,s&&(t.innerHTML=""),c.length===0){t.innerHTML=S("empty","No notifications yet."),le();return}let b=u=>{let h=u.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let $=h.type==="system.analysis.audit",x=$?`Analysis Audit: ${h.tier?.toUpperCase()}`:h.title||"Untitled Notification",D=$?"Raw analyst input and output logs.":h.body||"No description provided.",E=$?"low":h.priority||"low",B=!$&&!!h.alert,Z=$?"audit":h.category||"system",j=h.related_event_ids||[],T=!!localStorage.getItem(`notification_read_ts_${u.id}`),I=new Date(u.timestamp*1e3),k=I.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=I.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=T?"event-border-grey":"event-border-blue";$&&(L=T?"event-border-grey":"event-border-purple"),!T&&B&&(L="event-border-red"),T&&(E==="high"||E==="critical")?L="event-border-red":T&&E==="medium"&&(L="event-border-orange");let O=T?"notification-read":"notification-unread",N=P.has(u.id),Je=N?"expanded":"",Ge=N?"display: block;":"display: none;",ue="";if($)ue=`
                    <div class="event-detail-row">
                        <span class="detail-label">Tier:</span>
                        <span class="detail-value">${h.tier}</span>
                    </div>
                    <div class="event-detail-row">
                        <span class="detail-label">Model:</span>
                        <span class="detail-value">${h.model}</span>
                    </div>
                    <div class="event-detail-block">
                        <span class="detail-label">Raw Output:</span>
                        <pre class="detail-pre">${M(h.raw_output)}</pre>
                    </div>
                    <div class="event-detail-block">
                        <span class="detail-label">Raw Input (Prompt):</span>
                        <pre class="detail-pre">${M(h.raw_input)}</pre>
                    </div>
...
`,ce=null,V=new Set,je=[];async function K(s=!1){let t=document.getElementById("blueprints-list");if(!t)return;Qe();let n=localStorage.getItem("service_map");if(!n){t.innerHTML=S("config","No service map configured.","Upload service-map.json in Settings.");return}let o=null;try{o=(JSON.parse(n).services?.cs||[]).find(v=>v.id==="dex-event-service")}catch{t.innerHTML=S("error","Invalid service map data.");return}if(!o){t.innerHTML=S("error","Event service not found in service map.");return}let r=o.domain==="0.0.0.0"?"127.0.0.1":o.domain,d=`http://${r}:${o.port}/events?ml=100&format=json&event.type=system.blueprint.generated`,f=`http://${r}:${o.port}/analyst/status`;try{let l=await fetch(f);if(l.ok){let v=await l.json(),y=document.getElementById("strategist-next-run");if(y&&v.strategist){let i=v.strategist.next_run,c=Math.floor(Date.now()/1e3),b=i-c;if(b<=0)y.textContent="Next T3: Ready (IDLE req)",y.style.color="#5eff5e";else{let m=Math.floor(b/60),g=b%60;y.textContent=`Next T3: ${m}m ${g}s`,y.style.color="#888"}}}}catch(l){console.error("Failed to fetch analyst status",l)}try{let l=await fetch(d);if(!l.ok)throw new Error("Service is offline or unreachable.");let y=(await l.json()).events||[];if(je=y,ce=Date.now(),_(1,ce),y.length===0){t.innerHTML=S("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),q(1,0);return}s&&(t.innerHTML="");let i=a=>{let p=a.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let u=p.title||"Untitled Blueprint",h=p.summary||p.body||"No summary provided.",w=p.content||"",$=p.category||"architecture",x=p.affected_services||[],D=p.implementation_path||[],E=new Date(a.timestamp*1e3),B=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),Z=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),j=V.has(a.id),ee=j?"display: block;":"display: none;",T=document.createElement("div");T.className=`event-item notification-item event-border-purple cursor-pointer ${j?"expanded":""}`,T.dataset.blueprintId=a.id,T.onclick=function(L){if(this.classList.contains("expanded")){this.classList.remove("expanded"),V.delete(a.id);let N=this.querySelector(".event-details");N&&(N.style.display="none")}else{this.classList.add("expanded"),V.add(a.id);let N=this.querySelector(".event-details");N&&(N.style.display="block")}};let I=x.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${x.join(", ")}</div>`:"",k="";D.length>0&&(k=`
            ...
            `;let A=T.querySelector(".close-details-btn");return A&&A.addEventListener("click",L=>{L.stopPropagation(),T.classList.remove("expanded");let O=T.querySelector(".event-details");O&&(O.style.display="none"),V.delete(a.id)}),T},c=Array.from(t.children),b=new Map(c.map(a=>[a.dataset.blueprintId,a])),m=new Set(y.map(a=>a.id));c.forEach(a=>{let p=a.dataset.blueprintId;(!p||!m.has(p))&&a.remove()});let g=null;y.forEach((a,p)=>{let u=b.get(a.id);(!u||s)&&(u&&u.remove(),u=i(a),!u)||(p===0?t.firstElementChild!==u&&t.prepend(u):g&&g.nextElementSibling!==u&&g.after(u),g=u)}),q(1,y.length)}catch(l){console.error("Error fetching blueprints:",l),t.children.length===0&&(t.innerHTML=S("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function Qe(){let s=document.getElementById("blueprints-expand-all"),t=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-reset-strategist");s&&!s.dataset.listenerAttached&&(s.onclick=()=>{je.forEach(o=>V.add(e.id)),K(!0)},s.dataset.listenerAttached="true"),t&&!t.dataset.listenerAttached&&(t.onclick=()=>{V.clear(),K(!0)},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let o=localStorage.getItem("service_map");if(!o)return;let d=(JSON.parse(o).services?.cs||[]).find(v=>v.id==="dex-event-service");if(!d)return;let l=`http://${d.domain==="0.0.0.0"?"127.0.0.1":d.domain}:${d.port}/analyst/reset?tier=strategist`;n.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await fetch(l,{method:"POST"}),setTimeout(()=>{n.innerHTML="<i class='bx bx-check'></i> Reset Done",setTimeout(()=>{n.innerHTML="<i class='bx bx-refresh'></i> Reset Strategist"},2e3)},500),K()}catch(v){console.error("Failed to reset strategist",v),n.innerHTML="<i class='bx bx-error'></i> Failed"}},n.dataset.listenerAttached="true")}var Oe=()=>localStorage.getItem("service_map")?'<div id="services-widgets" class="system-monitor-widgets"><p>Loading services...</p></div>':S("config","No service map configured.","Upload service-map.json in Settings."),qe=()=>localStorage.getItem("service_map")?'<div id="models-widgets" class="system-monitor-widgets"><p>Loading models...</p></div>':S("config","No service map configured.","Upload service-map.json in Settings."),Pe=()=>localStorage.getItem("service_map")?'<div id="processes-widgets" class="system-monitor-widgets"><p>Loading processes...</p></div>':S("config","No service map configured.","Upload service-map.json in Settings."),de=null,pe=null,me=null;async function Fe(){if(!localStorage.getItem("service_map"))return null;try{let t=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(d=>d.id==="dex-event-service");if(!t)return null;let o=`http://${t.domain==="0.0.0.0"?"127.0.0.1":t.domain}:${t.port}/system_monitor`,r=await fetch(o);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);return await r.json()}catch(s){return console.error("Error fetching system data:",s),null}}async function Ze(){if(!localStorage.getItem("service_map"))return null;try{let t=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(d=>d.id==="dex-event-service");if(!t)return null;let o=`http://${t.domain==="0.0.0.0"?"127.0.0.1":t.domain}:${t.port}/processes`,r=await fetch(o);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);return await r.json()}catch(s){return console.error("Error fetching process data:",s),null}}async function xe(){let s=document.getElementById("services-widgets");if(!s)return;let t=await Fe();if(!t||!t.services){s.innerHTML=S("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}de=Date.now(),_(5,de);let n=t.services||[];Array.from(s.children).forEach(i=>{i.classList.contains("service-widget")||i.remove()});function o(i){return!i||i==="N/A"||i==="unknown"||i.trim()===""?"-":i}function r(i){if(!i||i==="N/A"||i==="unknown")return"-";let c=i.match(/^(\d+\.\d+\.\d+)/);return c?c[0]:i.split(".").slice(0,3).join(".")||"-"}function d(i){return!i||i.length<=28?i:i.substring(0,28)+"..."}function f(i){if(!i||i==="N/A"||i==="unknown")return"-";let c=i.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!c)return"-";let b=parseInt(c[1])||0,m=parseInt(c[2])||0,g=parseInt(c[3])||0,a=parseFloat(c[4])||0,p=b*86400+m*3600+g*60+a,u=Math.floor(p/86400);if(u>0)return`${u}d`;let h=Math.floor(p/3600);if(h>0)return`${h}h`;let w=Math.floor(p/60);return w>0?`${w}m`:`${Math.floor(p)}s`}function l(i){let c=i.status==="online",b=c?"service-widget-online":"service-widget-offline",m=c?"bx-check-circle":"bx-x-circle",g=c?"OK":"BAD",a=i.version?r(i.version.str):"-",p=i.cpu&&i.cpu!=="N/A"?i.cpu:"-",u=i.memory&&i.memory!=="N/A"?i.memory:"-",h=p!=="-"?"#00ff00":"#666",w=p!=="-"?"#fff":"#666",$=u!=="-"?"#00ff00":"#666",x=u!=="-"?"#fff":"#666",D=f(i.uptime),E="";return c?E=`
                ...
                </div>`:E='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${b}" data-service-id="${i.id}"><div class="service-widget-header"><i class="bx ${m}"></i><h3>${i.short_name||i.id}</h3><span class="service-widget-status">${g}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${d(i.domain&&i.port?`${i.domain}:${i.port}`:"")}</span></div>${E}</div></div>`}let v=new Map(Array.from(s.querySelectorAll(".service-widget")).map(i=>[i.dataset.serviceId,i])),y=new Set(n.map(i=>i.id));for(let[i,c]of v)y.has(i)||c.remove();n.forEach(i=>{let c=l(i),b=v.get(i.id);b?b.outerHTML!==c&&(b.outerHTML=c):s.insertAdjacentHTML("beforeend",c)})}async function we(){let s=document.getElementById("models-widgets");if(!s)return;let t=await Fe();if(!t){s.innerHTML=S("offline","Failed to load model status.");return}pe=Date.now(),_(4,pe);let n=t.models||[],o=t.whisper;Array.from(s.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function r(l){let v=l.status==="Ready";return`
                ...
                </div>`}function d(l){let v=l.status==="Downloaded",y=v?"service-widget-online":"service-widget-offline",i=v?"OK":"MISSING",c=v&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",b=l.name;return l.type==="custom"&&b.endsWith(":latest")&&(b=b.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${b}</h3><span class="service-widget-status">${i}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${c}</span></div></div></div>`}let f="";if(o&&(f+=r(o)),f+=n.map(d).join(""),!f){s.innerHTML=S("empty","No models found.");return}s.innerHTML!==f&&(s.innerHTML=f)}async function $e(){let s=document.getElementById("processes-widgets");if(!s)return;let t=await Ze();if(t===null){s.innerHTML=S("offline","Failed to load process status.");return}if(me=Date.now(),_(2,me),t.length===0){s.innerHTML=S("empty","No active processes."),q(2,0);return}(s.querySelector(".tab-placeholder")||s.querySelector("p"))&&(s.innerHTML="");function n(d){let f=Math.floor(Date.now()/1e3-d.start_time),l=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"";return`
                ...
                </div>`}let o=new Map(Array.from(s.querySelectorAll(".process-widget")).map(d=>[d.dataset.channelId,d])),r=new Set(t.map(d=>d.channel_id));for(let[d,f]of o)r.has(d)||f.remove();t.forEach(d=>{let f=n(d),l=o.get(d.channel_id);l?l.outerHTML!==f&&(l.outerHTML=f):s.insertAdjacentHTML("beforeend",f)}),q(2,t.length)}function X(){let s=be(),t=oe()||"user@easter.company",n={enabled:Notification.permission==="granted",supported:"Notification"in window},o=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
`,R=new Set,We=[],Q=null;async function U(s=!1){let t=document.getElementById("roadmap-list");if(!t)return;et();let n=localStorage.getItem("service_map");if(!n)return;let o=null;try{o=(JSON.parse(n).services?.cs||[]).find(l=>l.id==="dex-event-service")}catch{return}if(!o)return;let d=`http://${o.domain==="0.0.0.0"?"127.0.0.1":o.domain}:${o.port}/roadmap`;try{let f=await fetch(d);if(!f.ok)throw new Error("Offline");let l=await f.json();if(We=l,l.length===0){t.innerHTML=S("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}s&&(t.innerHTML="");let v=m=>{let g=R.has(m.id),a=m.state==="draft",p=m.state==="published",u=m.state==="consumed",h="event-border-grey";p&&(h="event-border-blue"),u&&(h="event-border-purple");let $=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),x=document.createElement("div");x.className=`event-item notification-item ${h} cursor-pointer ${g?"expanded":""}`,x.dataset.itemId=m.id,x.onclick=E=>{if(E.target.closest("button"))return;x.classList.contains("expanded")?(x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",R.delete(m.id)):(x.classList.add("expanded"),x.querySelector(".event-details").style.display="block",R.add(m.id))};let D=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;return x.innerHTML=`
                    ...
                    `,x.querySelector(".edit-btn")?.addEventListener("click",()=>tt(m)),x.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>st(m)),x.querySelector(".delete-btn")?.addEventListener("click",()=>nt(m.id)),x.querySelector(".close-details-btn")?.addEventListener("click",E=>{E.stopPropagation(),x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",R.delete(m.id)}),x},y=Array.from(t.children),i=new Map(y.map(m=>[m.dataset.itemId,m])),c=new Set(l.map(m=>m.id));y.forEach(m=>{let g=m.dataset.itemId;(!g||!c.has(g))&&m.remove()});let b=null;l.forEach((m,g)=>{let a=i.get(m.id);(!a||s)&&(a&&a.remove(),a=v(m),!a)||(g===0?t.firstElementChild!==a&&t.prepend(a):b&&b.nextElementSibling!==a&&b.after(a),b=a)})}catch{t.innerHTML=S("error","Failed to load roadmap.")}}function et(){let s=document.getElementById("roadmap-new"),t=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),r=document.getElementById("roadmap-close-all");s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Q=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",Q=null},n.dataset.listenerAttached="true"),t&&!t.dataset.listenerAttached&&(t.onclick=async()=>{let d=document.getElementById("roadmap-editor-input").value;if(!d.trim())return;let l=JSON.parse(localStorage.getItem("service_map")).services.cs.find(c=>c.id==="dex-event-service"),v=l.domain==="0.0.0.0"?"127.0.0.1":l.domain,y=Q?`http://${v}:${l.port}/roadmap/${Q}`:`http://${v}:${l.port}/roadmap`,i=Q?"PATCH":"POST";try{await fetch(y,{method:i,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:d})}),document.getElementById("roadmap-editor-container").style.display="none",U(!0)}catch(c){console.error(c)}},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{We.forEach(d=>R.add(d.id)),U(!0)},o.dataset.listenerAttached="true"),r&&!r.dataset.listenerAttached&&(r.onclick=()=>{R.clear(),U(!0)},r.dataset.listenerAttached="true")}function tt(s){Q=s.id,document.getElementById("roadmap-editor-input").value=s.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function st(s){let t=s.state==="published"?"draft":"published",o=JSON.parse(localStorage.getItem("service_map")).services.cs.find(d=>d.id==="dex-event-service"),r=o.domain==="0.0.0.0"?"127.0.0.1":o.domain;try{await fetch(`http://${r}:${o.port}/roadmap/${s.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:t})}),U(!0)}catch(d){console.error(d)}}async function nt(s){if(!confirm("Delete this roadmap item?"))return;let n=JSON.parse(localStorage.getItem("service_map")).services.cs.find(r=>r.id==="dex-event-service"),o=n.domain==="0.0.0.0"?"127.0.0.1":n.domain;try{await fetch(`http://${o}:${n.port}/roadmap/${s}`,{method:"DELETE"}),R.delete(s),U(!0)}catch(r){console.error(r)}}function ze(){Ae(),Ee();let s=_e();Le(s),Te();let t=document.querySelector("footer"),n=null;function o(){n=null,t?.classList.remove("hide"),document.querySelectorAll(".nav-right i").forEach(c=>c.classList.remove("active","inactive"))}function r(c,b=null){let m=n&&n.id===c.id;n&&n.close(!m),m?n=null:setTimeout(()=>{c.open(),n=c,document.querySelectorAll(".nav-right i").forEach(g=>{let a=g===b;g.classList.toggle("active",a),g.classList.toggle("inactive",!a&&b)}),t?.classList.add("hide")},n?220:0)}async function d(){await Promise.all([xe(),we(),ie(),$e(),F(),K(),ye()]);let c=setInterval(()=>{if(!i.isOpen())return clearInterval(c);_(4,he),_(3,J),_(5,pe),_(6,de),_(2,me),_(1,ce),_(0,Y)},1e3),b=setInterval(()=>{if(!i.isOpen())return clearInterval(b);ie(),$e(),F(),K(),ye()},3e3),m=setInterval(()=>{if(!i.isOpen())return clearInterval(m);xe(),we()},6e4)}async function f(){await U();let c=setInterval(()=>{if(!v.isOpen())return clearInterval(c);U()},5e3)}let l=ne({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="https://github.com/eastercompany" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in",onClose:o}),v=ne({id:"user-window",tabs:[{icon:"bx-user",title:"Profile",content:`<p style="padding: 20px;">Logged in as: ${oe()||"Unknown"}<br><br><span style="font-family: monospace; font-size: 0.8em; opacity: 0.6;">UUID: 313071000877137920</span></p>`,"data-tab-index":0},{icon:"bx-map-alt",title:"Roadmap",content:Re(),"data-tab-index":1}],icon:"bx-user",onClose:o,onOpen:()=>setTimeout(f,100)}),y=ne({id:"settings-window",title:"Settings",content:X(),icon:"bx-cog",onClose:o,onOpen:()=>{y.setContent(X()),setTimeout(()=>ae(y),50)}}),i=ne({id:"message-window",tabs:[{icon:"bx-bell",title:"Notifications",content:He(),"data-tab-index":0},{icon:"bx-paint",title:"Blueprints",content:Ue(),"data-tab-index":1},{icon:"bx-cog",title:"Processes",content:Pe(),"data-tab-index":2},{icon:"bx-calendar-event",title:"Events",content:Ne(),"data-tab-index":3},{icon:"bx-history",title:"Logs",content:Ce(),"data-tab-index":4},{icon:"bx-brain",title:"Models",content:qe(),"data-tab-index":5},{icon:"bx-line-chart",title:"Services",content:Oe(),"data-tab-index":6}],icon:"bxs-message-dots",onClose:o,onOpen:()=>setTimeout(d,100)});s?(document.getElementById("user-icon")?.addEventListener("click",c=>r(v,c.currentTarget)),document.getElementById("message-icon")?.addEventListener("click",c=>r(i,c.currentTarget)),document.getElementById("settings-icon")?.addEventListener("click",c=>r(y,c.currentTarget))):document.getElementById("login-btn")?.addEventListener("click",()=>{r(l),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",c=>{c.preventDefault();try{Ie(document.getElementById("email-input").value),window.location.reload()}catch(b){let m=document.getElementById("login-error");m&&(m.textContent=b.message,m.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ze):ze();})();
//# sourceMappingURL=dex.5a08312d.js.map

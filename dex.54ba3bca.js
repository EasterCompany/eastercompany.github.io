...
`,Ae=null,re=new Set,Ie=[];async function ie(t=!1){let e=document.getElementById("alerts-list");if(!e)return;os(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ae=Date.now(),X(0,Ae);let o=Date.now(),v=24*60*60*1e3,r=c.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let x=parseInt(f);return o-x<v});r.sort((d,f)=>{let x=T=>{let E=T.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},I=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,_=I(x(d)),S=I(x(f));return _!==S?S-_:f.timestamp-d.timestamp}),Ie=r;let p=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],b=new Set(r.map(d=>p(d))).size>1;if(r.length>0){let d=null;r.forEach(f=>{let x=p(f);b&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(f)})}if(t&&(e.innerHTML=""),r.length===0){e.innerHTML=H("empty","No alerts yet."),Te();return}let a=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),I=(f.body||"No description provided.").trim(),_=f.summary||"",S=f.content||"",T=f.protocol||"",E=(f.priority||"low").toLowerCase(),A=!!f.alert,L=(f.category||"system").trim(),D=f.related_event_ids||[],U=f.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${d.id}`),k=new Date(d.timestamp*1e3),C=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=j?"event-border-grey":"event-border-blue";!j&&A&&(M="event-border-red"),j&&(E==="high"||E==="critical")?M="event-border-red":j&&E==="medium"&&(M="event-border-orange");let P=j?"alert-read":"alert-unread",z=re.has(d.id),q=z?"expanded":"",J=z?"display: block;":"display: none;",W="",G="";D.length>0&&(G=`
            ...
            <div style="flex: 1; min-width: 120px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    <a href="#" onclick="window.dexter.viewEvent('${U}'); return false;" style="color: #bb86fc; text-decoration: none; border-bottom: 1px dashed rgba(187, 134, 252, 0.3); white-space: nowrap;">${U.substring(0,8)}...</a>
                </div>
            </div>`);let ue="";T&&(ue=`
        ...
        `,W=`
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${P} ${q} cursor-pointer priority-${E}`,Z.dataset.alertId=d.id,Z.onclick=function(se){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(d.id);let he=this.querySelector(".event-details");he&&(he.style.display="none")}else{this.classList.add("expanded"),re.add(d.id);let he=this.querySelector(".event-details");if(he&&(he.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ne="event-border-grey";E==="high"||E==="critical"?Ne="event-border-red":E==="medium"&&(Ne="event-border-orange"),this.classList.add(Ne),Te()}}};let Ke=`${T?T.toUpperCase():"GUARDIAN"} ALERT: ${_||x}`,Wt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[L]||"bx-bell",Gt=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";Z.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${C}</span>
                    <span class="event-date">${w}</span>
                </div>
                <div class="event-icon"><i class='bx ${Wt}'></i></div>
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Gt}; font-size: 0.8em; margin-left: 5px;">[${E.toUpperCase()}]</span>
                    </div>
...
`,le=new Set,Be=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){rs();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();Be=s;let n=p=>{let y=le.has(p.id),m=p.state==="published",b=p.state==="consumed",a="event-border-grey";m&&(a="event-border-blue"),b&&(a="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=p.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(p.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;h.innerHTML=`
      ...
      `;let l=h.querySelector(".event-details");return l&&l.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>ls(p)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>cs(p)),h.querySelector(".delete-btn")?.addEventListener("click",()=>ds(p.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)}),h},c=Array.from(e.children),o=new Map(c.map(p=>[p.dataset.itemId,p])),v=new Set(s.map(p=>p.id));if(c.forEach(p=>{let y=p.dataset.itemId;(!y||!v.has(y))&&p.remove()}),!Be||Be.length===0){e.innerHTML=H("empty","Your roadmap is empty.",N()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let r=null;s.forEach((p,y)=>{let m=o.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):r&&r.nextElementSibling!==m&&r.after(m),r=m)})}catch{e.children.length===0&&(e.innerHTML=H("offline","Failed to load roadmap.","The event service may be offline."))}}}function rs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let o=xe?`/roadmap/${xe}`:"/roadmap",v=xe?"PATCH":"POST";try{await B(o,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(r){console.error(r)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Be.forEach(c=>le.add(c.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function ls(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function cs(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function ds(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var gt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,ft=null,de=new Set,vt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ps();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(vt=c,ft=Date.now(),X(2,ft),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(z=>z.trim()),l=(b.implementation_path||[]).map(z=>z.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(m.timestamp*1e3),I=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=de.has(m.id),T=S?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${f?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,f&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let L=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";E.onclick=function(z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let D="";l.length>0&&(D=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${l.map(z=>`<li style="margin-bottom: 5px;">${O(z)}</li>`).join("")}</ul></div>
                    </div>
                `);let U="";d.length>0&&(U=`
                ...
                `);let F=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",j=N(),k=f?`
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async z=>{z.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(q){console.error("Failed to approve blueprint:",q)}});let w=E.querySelector(".blueprint-delete-btn");w&&(w.onclick=async z=>{z.stopPropagation();let q=!f;w.innerHTML=q?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(J){console.error("Failed to delete blueprint:",J)}});let M=E.querySelector(".event-details");M&&M.addEventListener("click",z=>{z.stopPropagation()});let P=E.querySelector(".close-details-btn");return P&&P.addEventListener("click",z=>{z.stopPropagation(),E.classList.remove("expanded");let q=E.querySelector(".event-details");q&&(q.style.display="none"),de.delete(m.id)}),E},v=Array.from(e.children),r=new Map(v.map(m=>[m.dataset.blueprintId,m])),p=new Set(c.map(m=>m.id));v.forEach(m=>{let b=m.dataset.blueprintId;(!b||!p.has(b))&&m.remove()});let y=null;c.forEach((m,b)=>{let a=r.get(m.id);(!a||t)&&(a&&a.remove(),a=o(m),!a)||(b===0?e.firstElementChild!==a&&e.prepend(a):y&&y.nextElementSibling!==a&&y.after(a),y=a)}),we(2,c.length),ct()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=H("offline","Failed to load blueprints.","The event service may be offline."))}}async function xt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let o=c.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&n++}),lt(n)}catch{}}function ps(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{vt.forEach(s=>de.add(s.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var yt=()=>`
...
`;async function Oe(){await Promise.all([ce(),pe()])}var ht=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,o=document.getElementById("new-chore-url").value,v=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:v,natural_instruction:c,entry_url:o,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(r){console.error(r),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let o=await(await B("/chores")).json();if(!o||o.length===0){t.innerHTML=H("empty","No active chores.","Create one to start monitoring.");return}let v=o.map(r=>{let p=r.last_run===0?"Never":new Date(r.last_run*1e3).toLocaleString(),y=r.memory?r.memory.length:0,m=r.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=v,t.querySelectorAll(".delete-chore-btn").forEach(r=>{r.onclick=async p=>{p.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${r.dataset.id}`,{method:"DELETE"}),ye())}})}catch(c){console.error("Failed to fetch chores",c)}}var ms=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,us=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);Ct(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(us=Date.now(),n.length===0){t.innerHTML=H("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((o,v)=>{let r=c[o.level]??10,p=c[v.level]??10;return r!==p?r-p:o.username.localeCompare(v.username)}),t.innerHTML=n.map(o=>{let v=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",r=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",p="#888";o.level==="Me"||o.level==="Master"?p="#bb86fc":o.level==="Admin"?p="#cf6679":o.level==="Moderator"?p="#03dac6":o.level==="Contributor"&&(p="#ffa500");let y=o.level==="Me",m=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
...
`,_e=null,me=new Set,Ce=[],K="all",gs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},bs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function vs(t){for(let[e,i]of Object.entries(gs))if(i.includes(t))return e;return"system"}function xs(t){return bs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;ys();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,$=vs(g),h=xs(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),l="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?l="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?l="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?l="event-border-orange":g==="system.test.completed"?l=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let d=u?"cursor-pointer":"",f=me.has(b.id),x=f?"expanded":"",I=f?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),S=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Et(g,a),A=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",L="";if(u){let k="";if(g==="engagement.decision"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                    ...
                    `}else if(g==="system.attention.expired"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w=a.timestamp-a.last_active,M=Math.floor(w/60);k=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w="";a.eval_count&&(w=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let P=a.chat_history.length,z=a.chat_history.map((q,J)=>{let W=q.name?q.name.toUpperCase():q.role.toUpperCase();!q.name&&W==="ASSISTANT"&&(W="DEXTER");let G=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",V=J===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${V};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${O(q.content)}</div>
                                </div>
                            `}).join("");M=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w=a.signals||{};k=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(w.topics||[]).map(P=>`<span class="profile-badge">${P}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.visual.completed"){let C=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,w="";a.base64_preview?w=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(w=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let C=a.success?"#03dac6":"#ff4d4d",w=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,P="";a.error&&(P=`
                            <div class="event-detail-block">
                                ${M("Error")}
                        ...
                        ${q}
                        ${z}
                    `}else if(g==="system.test.completed"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let C="";a.attachments&&a.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${a.attachments.map(M=>{let P=M.content_type&&M.content_type.startsWith("image/"),z=(M.size/1024).toFixed(1)+" KB";return`
...
`);return`
        ...
        <div id="services-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,_t=()=>`
        <div class="system-section-header">
            <i class='bx bxs-brain' style="color: #03dac6;"></i>
            <h2>Cognitive Models</h2>
        </div>
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Dt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${kt()}
        </div>`;async function Nt(){await Promise.all([Ee(),Je(),We(),ye()]),await $e()}var Tt=null,Lt=null;async function Ht(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function Mt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function Cs(){try{return await(await B("/processes")).json()}catch{return null}}async function $s(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Je(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),v=l=>{if(l){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let l=await Mt();l?(v(l),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let l=await Mt();v(l)}if(!t)return;let r=await Ht();if(!r||!r.services){t.children.length===0&&(t.innerHTML=H("offline","Failed to load system metrics.","The event service may be offline."));return}Tt=Date.now(),X(0,Tt);let p=r.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function m(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function a(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,x=parseInt(d[2])||0,I=parseInt(d[3])||0,_=parseFloat(d[4])||0,S=f*86400+x*3600+I*60+_,T=Math.floor(S/86400);if(T>0)return`${T}d`;let E=Math.floor(S/3600);if(E>0)return`${E}h`;let A=Math.floor(S/60);return A>0?`${A}m`:`${Math.floor(S)}s`}function g(l){let d=l.status==="online",f=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",I=d?"OK":"BAD",_=l.version?m(l.version.str):"-",S=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",T=l.memory&&l.memory!=="N/A"?l.memory:"-",E=S!=="-"?"#00ff00":"#666",A=S!=="-"?"#fff":"#666",L=T!=="-"?"#00ff00":"#666",D=T!=="-"?"#fff":"#666",U=a(l.uptime),F="";d?F=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${A};">${S}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${L};"></i>
                        <span style="color: ${D};">${T}</span>
                    </div>
                </div>`:F='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let j=N()?"easter.company":b(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${I}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${j}</span></div>${F}</div></div>`}let $=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),h=new Set(p.map(l=>l.id));for(let[l,d]of $)h.has(l)||d.remove();let u=new Set;p.forEach(l=>{if(u.has(l.id))return;u.add(l.id);let d=g(l),f=$.get(l.id);f&&f.parentNode?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function We(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ht();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}Lt=Date.now(),X(2,Lt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function n(r){let p=r.status==="Ready";return`
    ...
    `;async function Ot(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Pt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Pt(t)}async function Pt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=H("empty","No web history found.");return}let s=`
            <div style="display: flex; flex-direction: column; height: 100%;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,s]of Object.entries(t)){let n=qt.filter(c=>c.category===i);n.length!==0&&(e+=`
    ...
    </div>`,e},Ve=!1;function Ss(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Ut(),document.getElementById("terminal-action-btn").onclick=()=>Ut());let i=document.getElementById("cli-terminal-body"),s=document.getElementById("cli-docs-pane");i.innerHTML="";let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return s.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ve=!0,i}function Ut(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ve=!1}function jt(t,e,i="output"){if(!Ve)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function ks(t){let e=qt.find(n=>n.id===t);if(!e)return;let i=Ss(e);jt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),jt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ft(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Es();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;ks(s)})})}async function Ts(){if(!N())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Jt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}Qe(),et(),Ts(),at(),Xe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Ft();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),l=document.querySelector("nav"),d=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(A=>{let L=document.getElementById(A.id);L&&L.classList.remove("hide-close")});let I=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let A=document.getElementById("dexter-dropdown"),L=document.getElementById("dexter-menu-btn");A&&A.classList.remove("active"),L&&L.classList.remove("active"),u&&u.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",l?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),I&&(I.style.display=E?"flex":"none"),S&&(S.style.display=E?"block":"none"),!E&&_){let A=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(r)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(p)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(m)})):_.innerHTML=""}else _&&(_.innerHTML="");else l?.classList.remove("window-open"),T&&(T.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?d?.classList.remove("hide"):d?.classList.add("hide"),I&&(I.style.display="flex"),S&&(S.style.display="block"),_&&(_.innerHTML="");ve()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function o(){[...e].forEach(l=>l.close()),e=[]}window.addEventListener("resize",n);let v=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:pt(),onOpen:()=>ie(),onClose:()=>{let u=e.indexOf(v);u>-1&&e.splice(u,1),n()}}),r=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{r.setContent(qe()),ne()},onClose:()=>{let u=e.indexOf(r);u>-1&&e.splice(u,1),n()}}),p=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:$t(),onOpen:()=>je(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),y=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Bt()},{icon:"bxs-component",title:"Processes",content:It()},{icon:"bxs-brain",title:"Models",content:_t()},{icon:"bx-globe",title:"Web",content:Rt()},{icon:"bxs-hdd",title:"Hardware",content:Dt()},{icon:"bxs-terminal",title:"Logs",content:zt()},{icon:"bxs-zap",title:"Agents",content:At()},{icon:"bx-task",title:"Chores",content:ht()}].filter(u=>N()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Je(),Ee(),We(),Ot(),$e(),ye()},onClose:()=>{let u=e.indexOf(y);u>-1&&e.splice(u,1),n()}}),m=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:yt(),onOpen:()=>Oe(),onClose:()=>{let u=e.indexOf(m);u>-1&&e.splice(u,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{b.setContent(De()),Ge(b)},onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{r.isOpen()||c(r),setTimeout(()=>{let l=document.querySelector(`.event-item[data-event-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{v.isOpen()||c(v),setTimeout(()=>{let l=document.querySelector(`.event-item[data-alert-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),l=document.getElementById("dexter-menu-btn");u&&l&&(u.classList.remove("active"),l.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),l=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),I=document.getElementById("windows-container");u&&x&&(u.onclick=S=>{S.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let A=u.getBoundingClientRect(),L=document.querySelector("nav").getBoundingClientRect(),D=220,U=15,j=A.left+A.width/2-D/2,k=L.right-10;j+D>k&&(j=k-D),j<10&&(j=10),x.style.top=A.bottom+U+"px",x.style.left=j+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),u.classList.toggle("active");let E=x.classList.contains("active");if(T){let A=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),A?.classList.add("window-open"),I?.classList.add("menu-open"),fe(!0)):(I?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),A?.classList.remove("window-open"),fe(!1)))}}),l&&(l.onclick=S=>{S.stopPropagation(),c(b)}),d&&(d.onclick=S=>{S.stopPropagation(),o()});let _=(S,T,E)=>{let A=document.getElementById(S);A&&(A.onclick=L=>{L.stopPropagation(),a(),E&&s(E),c(T)})};_("alerts-menu-item",v,"alerts-window"),_("events-menu-item",r,"events-window"),_("monitor-menu-item",y,"monitor-window"),_("contacts-menu-item",p,"contacts-window"),_("workspace-menu-item",m,"workspace-window"),f&&(f.onclick=S=>{if(S.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)o();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let L=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");L.pop();let D=L.join("/")||"/";window.location.href=D}}}),document.addEventListener("click",()=>{let S=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),S&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),x&&(x.onclick=S=>S.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!h&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{r.isOpen()&&ne(),v.isOpen()?ie():Re()},1e3),setInterval(()=>{N()&&(v.isOpen()?ie():Re()),m.isOpen()?Oe():xt(),N()&&r.isOpen()&&ne(),y.isOpen()&&Nt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jt):Jt();})();

(()=>{function Ye(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Xe(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var et=1e3;function ne(t){let e=null,i=t.onClose||null,a=t.onOpen||null;function s(){e&&(e.style.zIndex=++et)}function c(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",o),a&&setTimeout(a,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++et,e.addEventListener("mousedown",s);let b=t.icon||"bx-window",n="",f="",E;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((g,r)=>{let d=g.icon?`<i class="bx ${g.icon}"></i>`:`<span class="tab-glyph">${g.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,E=`<div class="window-content">${t.tabs.map((g,r)=>`<div class="tab-content ${r===0?"active":""}" data-tab-content="${r}">${g.content}</div>`).join("")}</div>`):(t.title&&(f=`<div class="window-title">${t.title}</div>`),E=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Me=null,re=new Set,Ae=[];async function se(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ts(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await z(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let c=(await a.json()).events||[];Me=Date.now(),K(0,Me);let o=Date.now(),v=24*60*60*1e3,p=c.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let w=parseInt(d);return o-w<v});p.sort((r,d)=>{let w=S=>{let h=S.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},I=S=>S==="critical"?4:S==="high"?3:S==="medium"?2:1,M=I(w(r)),_=I(w(d));return M!==_?_-M:d.timestamp-r.timestamp}),Ae=p;let m=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},y=[],b=new Set(p.map(r=>m(r))).size>1;if(p.length>0){let r=null;p.forEach(d=>{let w=m(d);b&&w!==r&&(y.push({id:`divider-${w}`,type:"divider",label:w.toUpperCase()}),r=w),y.push(d)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=R("empty","No alerts yet."),ke();return}let n=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let w=(d.title||"Untitled Alert").trim(),I=(d.body||"No description provided.").trim(),M=d.summary||"",_=d.content||"",S=d.protocol||"",h=(d.priority||"low").toLowerCase(),O=!!d.alert,A=(d.category||"system").trim(),L=d.related_event_ids||[],U=d.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${r.id}`),k=new Date(r.timestamp*1e3),$=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=F?"event-border-grey":"event-border-blue";!F&&O&&(T="event-border-red"),F&&(h==="high"||h==="critical")?T="event-border-red":F&&h==="medium"&&(T="event-border-orange");let P=F?"alert-read":"alert-unread",B=re.has(r.id),q=B?"expanded":"",j=B?"display: block;":"display: none;",J="",G="";L.length>0&&(G=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${L.map(te=>`<a href="#" onclick="window.dexter.viewEvent('${te}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${te.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let Y="";U&&(Y=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(_)}</div>
            </div>
        `:X=`
            <div class="event-detail-block" style="text-align: left;">
        ...
        `,J=`
                ...
                ${X}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${T} ${P} ${q} cursor-pointer priority-${h}`,Z.dataset.alertId=r.id,Z.onclick=function(te){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(r.id);let ye=this.querySelector(".event-details");ye&&(ye.style.display="none")}else{this.classList.add("expanded"),re.add(r.id);let ye=this.querySelector(".event-details");if(ye&&(ye.style.display="block"),!localStorage.getItem(`alert_read_ts_${r.id}`)){localStorage.setItem(`alert_read_ts_${r.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ze="event-border-grey";h==="high"||h==="critical"?ze="event-border-red":h==="medium"&&(ze="event-border-orange"),this.classList.add(ze),ke()}}};let Ve=`${S?S.toUpperCase():"GUARDIAN"} ALERT: ${M||w}`,Ut={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[A]||"bx-bell",qt=h==="high"||h==="critical"?"#ff4d4d":h==="medium"?"#ffa500":"#888";Z.innerHTML=`
...
`,ut=null,de=new Set,bt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;os();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await z(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let c=(await a.json()).events||[];if(bt=c,ut=Date.now(),K(2,ut),c.length===0){e.innerHTML=R("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=u=>{let b=u.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let n=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),x=(b.category||"architecture").trim(),l=(b.related_services||b.affected_services||[]).map(B=>B.trim()),g=(b.implementation_path||[]).map(B=>B.trim()),r=b.source_event_ids||[],d=b.approved===!0,w=new Date(u.timestamp*1e3),I=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=de.has(u.id),S=_?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${_?"expanded":""} ${d?"blueprint-approved":""}`,h.dataset.blueprintId=u.id,d&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let A=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";h.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(u.id);let j=this.querySelector(".event-details");j&&(j.style.display="none")}else{this.classList.add("expanded"),de.add(u.id);let j=this.querySelector(".event-details");j&&(j.style.display="block")}};let L="";g.length>0&&(L=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${g.map(B=>`<li style="margin-bottom: 5px;">${N(B)}</li>`).join("")}</ul></div>
                    </div>
                `);let U="";r.length>0&&(U=`
                ...
                `);let V=l.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${l.join(", ")}</span></div>`:"<div></div>",F=D(),k=d?`
            ...
            `;h.innerHTML=`
                ...
                <div class="event-icon" style="${d?"color: #03dac6;":""}"><i class='bx ${A}'></i></div>
                <div class="event-content">
            ...
            `;let $=h.querySelector(".blueprint-approve-btn");$&&($.onclick=async B=>{B.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await z(`/events/${u.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&pe(!0)}catch(q){console.error("Failed to approve blueprint:",q)}});let C=h.querySelector(".blueprint-delete-btn");C&&(C.onclick=async B=>{B.stopPropagation();let q=!d;C.innerHTML=q?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await z(`/events/${u.id}`,{method:"DELETE"})).ok&&pe(!0)}catch(j){console.error("Failed to delete blueprint:",j)}});let T=h.querySelector(".event-details");T&&T.addEventListener("click",B=>{B.stopPropagation()});let P=h.querySelector(".close-details-btn");return P&&P.addEventListener("click",B=>{B.stopPropagation(),h.classList.remove("expanded");let q=h.querySelector(".event-details");q&&(q.style.display="none"),de.delete(u.id)}),h},v=Array.from(e.children),p=new Map(v.map(u=>[u.dataset.blueprintId,u])),m=new Set(c.map(u=>u.id));v.forEach(u=>{let b=u.dataset.blueprintId;(!b||!m.has(b))&&u.remove()});let y=null;c.forEach((u,b)=>{let n=p.get(u.id);(!n||t)&&(n&&n.remove(),n=o(u),!n)||(b===0?e.firstElementChild!==n&&e.prepend(n):y&&y.nextElementSibling!==n&&y.after(n),y=n)}),he(2,c.length),lt()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=R("offline","Failed to load blueprints.","The event service may be offline."))}}async function vt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await z(t);if(!e.ok)return;let a=(await e.json()).events||[],s=0;a.forEach(c=>{let o=c.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&s++}),rt(s)}catch{}}function os(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{bt.forEach(a=>de.add(a.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.clear(),pe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await z("/events?type=system.blueprint.generated",{method:"DELETE"}),de.clear(),pe(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var xt=()=>`
...
`;function yt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function ht(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=rs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),Le(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();Oe(e,t,a)}else{console.log("Profile not found or error, using mock data.");let a=yt(t.id,t.username);Oe(e,t,a)}}).catch(i=>{console.error("Profile fetch error:",i);let a=yt(t.id,t.username);Oe(e,t,a)})}function Oe(t,e,i){let a=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",s=()=>{let E=(i.badges||[]).map(I=>`<span class="profile-badge ${I==="Creator"?"master":""}">${I}</span>`).join(""),x=(i.attributes||i.facts||[]).map(I=>{let M=I.key||I.k,_=I.value||I.v;return`
            <div class="fact-chip">
                <span class="fact-key">${M}:</span>
                <span class="fact-val">${_}</span>
            </div>
        `}).join(""),l=i.cognitive_model||i,g=l.technical_level||l.techLevel||0,r=l.communication_style||l.commStyle||"Unknown",d=l.patience_level||l.patience||"Unknown",w=l.vibe||"Unknown";return`
        ...
        `},c=()=>{let E=i.dossier||{},x=E.identity||{},l=E.career||{},g=E.personal||{},r=E.social||[];return`
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(l.skills||[]).map(d=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${d}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(g.hobbies||[]).map(d=>`<div class="dossier-list-item">${d}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(g.habits||[]).map(d=>`<div class="dossier-list-item">${d}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(g.vices||[]).map(d=>`<div class="dossier-list-item" style="color: #cf6679;">${d}</div>`).join("")}
                        </div>
                    </div>
                </div>
...
`,_e=null,me=new Set,we=[],W="all",ds={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ms(t){for(let[e,i]of Object.entries(ds))if(i.includes(t))return e;return"system"}function us(t){return ps[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;fs();let a=`/events?ml=${W==="all"?100:250}&format=json`;W!=="all"&&(a+=`&category=${W}`);try{let s=await z(a);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let f=n.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),_e=Date.now(),K(1,_e),we.length===0){e.innerHTML=R("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let n=b.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let f=n.type,E=ms(f),x=us(f),l=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="analysis.user.message_signals"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),g="event-border-grey";l&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?g="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.analysis.audit"||f==="analysis.user.message_signals"?g="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?g="event-border-orange":f==="system.test.completed"?g=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":g="event-border-blue");let r=l?"cursor-pointer":"",d=me.has(b.id),w=d?"expanded":"",I=d?"display: block;":"display: none;",M=new Date(b.timestamp*1e3),_=M.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=M.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=Ct(f,n),O=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",A="";if(l){let k="";if(f==="engagement.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                    ...
                    `}else if(f==="system.attention.expired"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C=n.timestamp-n.last_active,T=Math.floor(C/60);k=`
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C="";n.eval_count&&(C=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let T="";if(n.chat_history&&n.chat_history.length>0){let P=n.chat_history.length,B=n.chat_history.map((q,j)=>{let J=q.role.toUpperCase();J==="ASSISTANT"&&(J="DEXTER");let G=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",Y=j===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${Y};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${N(q.content)}</div>
                                </div>
                            `}).join("");T=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(f==="analysis.user.message_signals"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,C=n.signals||{};k=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(f==="moderation.explicit_content.deleted"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="analysis.link.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="analysis.visual.completed"){let $=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,C="";n.base64_preview?C=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(C=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="analysis.router.decision"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.cli.command"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.analysis.audit"){let $=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",T=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,P="";n.error&&(P=`
                            <div class="event-detail-block">
                                ${T("Error")}
                        ...
                        `}let q="";if(n.corrections&&n.corrections.length>0){let J=n.corrections.map((G,Y)=>`
                        ...
                        ${q}
                        ${B}
                    `}else if(f==="system.test.completed"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="error_occurred"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="system.cli.status"){let $=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(f==="messaging.user.sent_message"){let $="";n.attachments&&n.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${n.attachments.map(T=>{let P=T.content_type&&T.content_type.startsWith("image/"),B=(T.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${P?`<div class="attachment-thumb-container"><img src="${T.url}" alt="${T.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${T.url}" target="_blank" class="attachment-link">${T.filename}</a>
                                        <span class="attachment-meta">${T.content_type} \u2022 ${B}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),k=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let L=document.createElement("div");L.className=`event-item ${g} ${r} ${w}`,L.dataset.eventId=b.id,L.onclick=function(k){if(!l)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(b.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},L.innerHTML=`
                <div class="event-time">
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Et()}
                ...
                </div>`),s){let g=(l.MEMORY_BYTES/1073741824).toFixed(1);s.innerHTML=`
                ...
                </div>`}if(a&&l.CPU&&l.CPU.length>0){let g=l.CPU[0];a.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}c&&(l.GPU&&l.GPU.length>0?c.innerHTML=l.GPU.map(g=>{let r=(g.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&l.STORAGE&&l.STORAGE.length>0?o.innerHTML=l.STORAGE.map(g=>{let r=(g.USED/1073741824).toFixed(1),d=(g.SIZE/(1024*1024*1024)).toFixed(1),w=g.SIZE>0?(g.USED/g.SIZE*100).toFixed(0):0,I=g.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let l=await Tt();l?(v(l),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let l=await Tt();v(l)}if(!t)return;let p=await zt();if(!p||!p.services){t.children.length===0&&(t.innerHTML=R("offline","Failed to load system metrics.","The event service may be offline."));return}St=Date.now(),K(0,St);let m=p.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function u(l){if(!l||l==="N/A"||l==="unknown")return"-";let g=l.match(/^(\d+\.\d+\.\d+)/);return g?g[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function n(l){if(!l||l==="N/A"||l==="unknown")return"-";let g=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!g)return"-";let r=parseInt(g[1])||0,d=parseInt(g[2])||0,w=parseInt(g[3])||0,I=parseFloat(g[4])||0,M=r*86400+d*3600+w*60+I,_=Math.floor(M/86400);if(_>0)return`${_}d`;let S=Math.floor(M/3600);if(S>0)return`${S}h`;let h=Math.floor(M/60);return h>0?`${h}m`:`${Math.floor(M)}s`}function f(l){let g=l.status==="online",r=g?"service-widget-online":"service-widget-offline",d=g?"bx-check-circle":"bx-x-circle",w=g?"OK":"BAD",I=l.version?u(l.version.str):"-",M=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",_=l.memory&&l.memory!=="N/A"?l.memory:"-",S=M!=="-"?"#00ff00":"#666",h=M!=="-"?"#fff":"#666",O=_!=="-"?"#00ff00":"#666",A=_!=="-"?"#fff":"#666",L=n(l.uptime),U="";g?U=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${A};">${_}</span>
                    </div>
                </div>`:U='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let V=D()?"easter.company":b(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${r}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${d}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${V}</span></div>${U}</div></div>`}let E=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),x=new Set(m.map(l=>l.id));for(let[l,g]of E)x.has(l)||g.remove();m.forEach(l=>{let g=f(l),r=E.get(l.id);r?r.outerHTML!==g&&(r.outerHTML=g):t.insertAdjacentHTML("beforeend",g)})}async function Je(){let t=document.getElementById("models-widgets");if(!t)return;let e=await zt();if(!e){t.children.length===0&&(t.innerHTML=R("offline","Failed to load model status.","The event service may be offline."));return}kt=Date.now(),K(2,kt);let i=e.models||[],a=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function s(p){let m=p.status==="Ready";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],hs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),We=!0,i}function Nt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),We=!1}function Ht(t,e,i="output"){if(!We)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=Te(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Cs(t){let e=Pt.find(s=>s.id===t);if(!e)return;let i=ws(e);Ht(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";s.includes("[ERROR]")?c="error":s.includes("[SUCCESS]")||s.includes("\u2713")?c="success":s.includes("!")&&(c="warning"),Ht(i,s,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Rt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=hs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;Cs(a)})})}async function $s(){if(!D())try{if(!(await z("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ot(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}$s(),nt(),Ye();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Rt(),Xe(),Qe(),(()=>{let r=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),w=document.getElementById("close-all-windows"),I=document.getElementById("nav-left-container"),M=document.getElementById("dexter-dropdown"),_=document.getElementById("windows-container");r&&M&&(r.onclick=S=>{S.stopPropagation();let h=window.innerWidth<880;M.classList.toggle("active"),r.classList.toggle("active");let O=M.classList.contains("active");if(h){let A=document.querySelector("nav");O?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),A?.classList.add("window-open"),_?.classList.add("menu-open"),fe(!0)):(_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),A?.classList.remove("window-open"),fe(!1)))}}),d&&(d.onclick=()=>m(x)),w&&(w.onclick=()=>y()),I&&(I.onclick=()=>{if(M&&M.classList.contains("active")){M.classList.remove("active"),r?.classList.remove("active"),_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(s.length>0)y();else{let S=window.location.pathname;if(!(S==="/"||S==="/index.html")){let A=(S.endsWith("/")&&S.length>1?S.slice(0,-1):S).split("/");A.pop();let L=A.join("/")||"/";window.location.href=L}}}),document.addEventListener("click",()=>{let S=window.innerWidth<880;M&&M.classList.contains("active")&&(M.classList.remove("active"),r?.classList.remove("active"),S&&(_?.classList.remove("menu-open"),s.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1))))}),M&&(M.onclick=S=>S.stopPropagation())})();let i=window.location.pathname==="/"||window.location.pathname==="/index.html",a=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!i&&!a&&document.querySelector("footer")?.classList.add("hide");let s=[],c=document.getElementById("windows-container");c&&c.setAttribute("data-count","0");let o=r=>{localStorage.setItem("dex_last_window",r)};function v(){return 1}function p(){for(;s.length>1;)s.shift().close(!0);let r=document.getElementById("windows-container"),d=document.querySelector("nav"),w=document.querySelector("footer"),I=window.location.pathname==="/"||window.location.pathname==="/index.html",M=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",s.length),s.forEach(L=>{let U=document.getElementById(L.id);U&&U.classList.remove("hide-close")});let _=document.getElementById("dexter-menu-container"),S=document.getElementById("nav-window-switcher"),h=document.getElementById("settings-icon"),O=document.getElementById("close-all-windows"),A=window.innerWidth<880;if(A||(l&&l.classList.remove("active"),menuBtn&&menuBtn.classList.remove("active"),r&&r.classList.remove("menu-open")),fe(s.length>0),s.length>0)if(w?.classList.add("hide"),O&&(O.style.display=A?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),d?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),_&&(_.style.display=A?"flex":"none"),h&&(h.style.display=A?"block":"none"),!A&&S){let L=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(L)?(S.innerHTML=`
          ...
          <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
      `);let g=()=>{let r=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");r&&d&&(r.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{g(),o("alerts-window"),m(u)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{g(),o("events-window"),m(b)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{g(),o("monitor-window"),m(f)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{g(),o("contacts-window"),m(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{g(),o("workspace-window"),m(E)}),D()||setInterval(()=>{b.isOpen()&&ie(),u.isOpen()?se():Pe()},1e3),setInterval(()=>{D()&&(u.isOpen()?se():Pe()),E.isOpen()?Re():vt(),D()&&b.isOpen()&&ie(),f.isOpen()&&Dt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ot):Ot();})();

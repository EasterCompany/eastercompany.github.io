(()=>{function He(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ne(t=!1){let e=window.location.pathname,o=e==="/"||e==="/index.html",i=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Oe=1e3;function F(t){let e=null,o=t.onClose||null,n=t.onOpen||null;function s(){e&&(e.style.zIndex=++Oe)}function i(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",l),n&&setTimeout(n,10);return}let g=document.getElementById("windows-container");g||(g=document.createElement("div"),g.id="windows-container",g.className="windows-container",document.body.appendChild(g)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Oe,e.addEventListener("mousedown",s);let w=t.icon||"bx-window",b="",r="",u;t.tabs&&t.tabs.length>0?(b=`<div class="tab-bar">${t.tabs.map((h,a)=>{let d=h.icon?`<i class="bx ${h.icon}"></i>`:`<span class="tab-glyph">${h.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,u=`<div class="window-content">${t.tabs.map((h,a)=>`<div class="tab-content ${a===0?"active":""}" data-tab-content="${a}">${h.content}</div>`).join("")}</div>`):(t.title&&(r=`<div class="window-title">${t.title}</div>`),u=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,ue=null,G=new Set,ge=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await I(o);if(!n.ok)throw new Error("Service is offline or unreachable.");let i=(await n.json()).events||[];ue=Date.now(),P(0,ue);let l=Date.now(),p=24*60*60*1e3,v=i.filter(a=>{let d=localStorage.getItem(`alert_read_ts_${a.id}`);if(!d)return!0;let y=parseInt(d);return l-y<p});v.sort((a,d)=>{let y=L=>{let M=L.event;if(typeof M=="string")try{M=JSON.parse(M)}catch{return"low"}return(M.priority||"low").toLowerCase()},$=L=>L==="critical"?4:L==="high"?3:L==="medium"?2:1,E=$(y(a)),S=$(y(d));return E!==S?S-E:d.timestamp-a.timestamp}),ge=v;let m=a=>{let d=a.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],w=new Set(v.map(a=>m(a))).size>1;if(v.length>0){let a=null;v.forEach(d=>{let y=m(d);w&&y!==a&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),a=y),x.push(d)})}if(t&&(e.innerHTML=""),v.length===0){e.innerHTML=T("empty","No alerts yet."),me();return}let b=a=>{let d=a.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let y=d.title||"Untitled Alert",$=d.body||"No description provided.",E=d.priority||"low",S=!!d.alert,L=d.category||"system",M=d.related_event_ids||[],A=d.audit_event_id,B=!!localStorage.getItem(`alert_read_ts_${a.id}`),N=new Date(a.timestamp*1e3),R=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=B?"event-border-grey":"event-border-blue";!B&&S&&(k="event-border-red"),B&&(E==="high"||E==="critical")?k="event-border-red":B&&E==="medium"&&(k="event-border-orange");let D=B?"alert-read":"alert-unread",H=G.has(a.id),ye=H?"expanded":"",he=H?"display: block;":"display: none;",Ae="",_e="";M.length>0&&(_e=`
...
`,V=new Set,Qe=[],Q=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let o=await I("/roadmap");if(!o.ok)throw new Error("Offline");let n=await o.json();if(Qe=n,n.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let x=V.has(m.id),g=m.state==="draft",w=m.state==="published",b=m.state==="consumed",r="event-border-grey";w&&(r="event-border-blue"),b&&(r="event-border-purple");let c=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),f=document.createElement("div");f.className=`event-item notification-item ${r} cursor-pointer ${x?"expanded":""}`,f.dataset.itemId=m.id,f.onclick=d=>{if(d.target.closest("button"))return;f.classList.contains("expanded")?(f.classList.remove("expanded"),f.querySelector(".event-details").style.display="none",V.delete(m.id)):(f.classList.add("expanded"),f.querySelector(".event-details").style.display="block",V.add(m.id))};let h=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;f.innerHTML=`
        <div class="event-time">
...
`,et=null,ee=new Set,nt=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let n=await I(o);if(!n.ok)throw new Error("Service is offline or unreachable.");let i=(await n.json()).events||[];if(nt=i,et=Date.now(),P(2,et),i.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ne(2,0);return}t&&(e.innerHTML="");let l=g=>{let w=g.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return null}let b=w.title||"Untitled Blueprint",r=w.summary||w.body||"No summary provided.",u=w.content||"",c=w.category||"architecture",f=w.affected_services||[],h=w.implementation_path||[],a=new Date(g.timestamp*1e3),d=a.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=a.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ee.has(g.id),E=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,S.dataset.blueprintId=g.id;let M={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";S.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ee.delete(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="none")}else{this.classList.add("expanded"),ee.add(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="block")}};let A=f.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${f.join(", ")}</span></div>`:"",O="";h.length>0&&(O=`
            ...
            `;let B=S.querySelector(".event-details");B&&B.addEventListener("click",R=>{R.stopPropagation()});let N=S.querySelector(".close-details-btn");return N&&N.addEventListener("click",R=>{R.stopPropagation(),S.classList.remove("expanded");let C=S.querySelector(".event-details");C&&(C.style.display="none"),ee.delete(g.id)}),S},p=Array.from(e.children),v=new Map(p.map(g=>[g.dataset.blueprintId,g])),m=new Set(i.map(g=>g.id));p.forEach(g=>{let w=g.dataset.blueprintId;(!w||!m.has(w))&&g.remove()});let x=null;i.forEach((g,w)=>{let b=v.get(g.id);(!b||t)&&(b&&b.remove(),b=l(g),!b)||(w===0?e.firstElementChild!==b&&e.prepend(b):x&&x.nextElementSibling!==b&&x.after(b),x=b)}),ne(2,i.length)}catch(n){console.error("Error fetching blueprints:",n),e.children.length===0&&(e.innerHTML=T("offline","Failed to load blueprints.","The event service may be offline."))}}function Pt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(o=>ee.add(o.id)),fe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ee.clear(),fe(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`,Ot=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let o=await Ke("/contacts");if(!o.ok)throw new Error("Service unreachable");let s=(await o.json()).members||[];if(Ot=Date.now(),s.length===0){t.innerHTML=T("empty","No contacts found.","Check your Discord connection.");return}let i={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((l,p)=>{let v=i[l.level]??10,m=i[p.level]??10;return v!==m?v-m:l.username.localeCompare(p.username)}),t.innerHTML=s.map(l=>{let p=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",m="#888";l.level==="Me"||l.level==="Master"?m="#bb86fc":l.level==="Admin"?m="#cf6679":l.level==="Moderator"?m="#03dac6":l.level==="Contributor"&&(m="#ffa500");let x=l.level==="Me",g=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",w=x?"2px solid #bb86fc":`1px solid ${p}33`;return`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function wt(){await Promise.all([be(),Te(),Le()]),await le()}var dt=null,mt=null,pt=null;async function Et(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await I("/processes")).json()}catch{return null}}async function Vt(){try{return await(await I("/guardian/status")).json()}catch{return null}}async function Te(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),s=document.querySelector("#hw-gpu .hw-content"),i=document.querySelector("#hw-storage .hw-content"),l=c=>{if(c){if(n){let f=(c.MEMORY_BYTES/1073741824).toFixed(1);n.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):i&&(i.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await ut();c?(l(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),o&&!o.hasChildNodes())){let c=await ut();l(c)}if(!t)return;let p=await Et();if(!p||!p.services){t.children.length===0&&(t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),P(0,dt);let v=p.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function m(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function x(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/^(\d+\.\d+\.\d+)/);return f?f[0]:c.split(".").slice(0,3).join(".")||"-"}function g(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function w(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!f)return"-";let h=parseInt(f[1])||0,a=parseInt(f[2])||0,d=parseInt(f[3])||0,y=parseFloat(f[4])||0,$=h*86400+a*3600+d*60+y,E=Math.floor($/86400);if(E>0)return`${E}d`;let S=Math.floor($/3600);if(S>0)return`${S}h`;let L=Math.floor($/60);return L>0?`${L}m`:`${Math.floor($)}s`}function b(c){let f=c.status==="online",h=f?"service-widget-online":"service-widget-offline",a=f?"bx-check-circle":"bx-x-circle",d=f?"OK":"BAD",y=c.version?x(c.version.str):"-",$=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",E=c.memory&&c.memory!=="N/A"?c.memory:"-",S=$!=="-"?"#00ff00":"#666",L=$!=="-"?"#fff":"#666",M=E!=="-"?"#00ff00":"#666",A=E!=="-"?"#fff":"#666",O=w(c.uptime),B="";return f?B=`
                ...
                </div>`}function i(p){let v=p.status==="Downloaded",m=v?"service-widget-online":"service-widget-offline",x=v?"OK":"MISSING",g=v&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",w=p.name;return p.type==="custom"&&w.endsWith(":latest")&&(w=w.replace(":latest","")),`<div class="service-widget ${m}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${w}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let l="";if(n&&(l+=s(n)),l+=o.map(i).join(""),!l){t.innerHTML=T("empty","No models found.");return}t.innerHTML!==l&&(t.innerHTML=l)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),o=document.getElementById("guardian-t2-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),i=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{p.innerHTML="<i class='bx bx-error'></i> Failed"}},p.dataset.listenerAttached="true");let v=await Vt();if(v){let r=Math.floor(Date.now()/1e3),u=v.active_tier,c=h=>{h<0&&(h=0);let a=Math.floor(h/3600),d=Math.floor(h%3600/60),y=h%60;return a>0?`${a}h ${d}m`:d>0?`${d}m ${y}s`:`${y}s`},f=(h,a,d,y)=>{if(u===y)h.textContent="Working",h.style.color="#bb86fc";else if(y==="t1"&&u==="tests")h.textContent="Testing",h.style.color="#03dac6";else if(d){let E=d.next_run-r;if(E<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let S=Math.floor(E/60),L=E%60;h.textContent=`${S}m ${L}s`,h.style.color="#fff"}}a&&d&&(a.innerHTML=`
        ...
        `)};if(e&&f(e,document.getElementById("guardian-t1-stats"),v.t1,"t1"),o&&f(o,document.getElementById("guardian-t2-stats"),v.t2,"t2"),n&&v.system_idle_time!==void 0){let h=v.system_idle_time;n.textContent=c(h),h>300?n.style.color="#5eff5e":h>60?n.style.color="#ffa500":n.style.color="#fff"}s&&(s.textContent=c(v.total_idle_time||0)),i&&(i.textContent=c(v.total_active_time||0)),l&&(l.textContent=c(v.total_waste_time||0))}else[e,o,n,s,i,l].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let m=await Wt(),x=[],g=[];m&&(Array.isArray(m)?x=m:(x=m.active||[],g=m.history||[]));let w=document.getElementById("vitals-processes-val");if(w)if(m){let r=x.length;w.textContent=r>0?`${r} Active`:"Idle",w.style.color=r>0?"#bb86fc":"#fff"}else w.textContent="-",w.style.color="#888";if(m===null){t.children.length===0&&(t.innerHTML=T("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),P(1,pt),x.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=T("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,x,!1));let b=document.getElementById("processes-history-widgets");b&&(g.length===0?b.querySelector(".tab-placeholder")||(b.innerHTML=T("empty","No recent history.")):(b.querySelector(".tab-placeholder")&&(b.innerHTML=""),gt(b,g,!0))),ne(1,x.length)}function gt(t,e,o){function n(l){let p="";l.end_time?p=`${l.end_time-l.start_time}s`:p=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let v=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",m=l.channel_id,x={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};x[m]?m=x[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`);let g=o?"#888":"#fff",w=o?"border-left: 3px solid #666;":"";return`
                ...
                </div>`}let s=new Map(Array.from(t.querySelectorAll(".process-widget")).map(l=>[l.dataset.channelId,l])),i=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,p]of s)i.has(l)||p.remove();e.forEach(l=>{let p=`${l.channel_id}-${l.start_time}`,v=n(l),m=s.get(p);if(m){if(m.outerHTML!==v){m.outerHTML=v;let x=t.querySelector(`[data-channel-id="${p}"]`);x&&s.set(p,x)}}else{t.insertAdjacentHTML("beforeend",v);let x=t.querySelector(`[data-channel-id="${p}"]`);x&&s.set(p,x)}})}function xe(){let t=se(),e=ze()||"master@easter.company",o={enabled:Notification.permission==="granted",supported:"Notification"in window},n=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,o}function St(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function $t(t,e,o="output"){if(!Ie)return;let n=document.createElement("div");n.className=`terminal-line terminal-${o}`,o==="prompt"?n.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:n.innerHTML=pe(e),t.appendChild(n),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let o=Yt(e);$t(o,`dex ${t}`,"prompt");let n=e.demo_output||["Executing command...","\u2713 Done."];for(let s of n){await new Promise(l=>setTimeout(l,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),$t(o,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code").textContent;navigator.clipboard.writeText(o).then(()=>{let n=e.querySelector("i");n.className="bx bx-check",n.style.color="#5eff5e",setTimeout(()=>{n.className="bx bx-copy",n.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-5px)",o.style.borderColor="rgba(255,255,255,0.15)",o.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)",o.style.borderColor="rgba(255,255,255,0.05)",o.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let n=o.dataset.cmd;Xt(n)})})}async function Zt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Lt(){Zt(),Ge(),He();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=Ue();Ne(e),Pe();let o=document.getElementById("nav-left-container");o&&o.addEventListener("click",()=>{if(i.length>0)g();else{let a=window.location.pathname;if(!(a==="/"||a==="/index.html")){let $=(a.endsWith("/")&&a.length>1?a.slice(0,-1):a).split("/");$.pop();let E=$.join("/")||"/";window.location.href=E}}});let n=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!n&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let p=a=>{localStorage.setItem("dex_last_window",a)};function v(){return 1}function m(){for(;i.length>1;)i.shift().close(!0);let a=document.getElementById("windows-container"),d=document.querySelector("nav"),y=document.querySelector("footer"),$=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",i.length),i.forEach(A=>{let O=document.getElementById(A.id);O&&O.classList.remove("hide-close")}),re(se());let S=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),M=document.getElementById("settings-icon");if(Re(i.length>0),i.length>0){if(y?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),S&&(S.style.display="none"),M&&(M.style.display="none"),L){let A=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{p("alerts-window"),x(w)}),document.getElementById("switch-events").addEventListener("click",()=>{p("events-window"),x(b)}),document.getElementById("switch-monitor").addEventListener("click",()=>{p("monitor-window"),x(u)}),document.getElementById("switch-contacts").addEventListener("click",()=>{p("contacts-window"),x(r)}),document.getElementById("switch-workspace").addEventListener("click",()=>{p("workspace-window"),x(c)})):L.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),$||E?y?.classList.remove("hide"):y?.classList.add("hide"),S&&(S.style.display="flex"),M&&(M.style.display="block"),L&&(L.innerHTML="");ie(Ve())}function x(a){if(a.isOpen()){a.close();return}for(;i.length>0;)i.pop().close(!0);i.push(a),a.open(),m()}function g(){[...i].forEach(d=>d.close()),i=[]}window.addEventListener("resize",m);let w=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Je(),onOpen:()=>W(),onClose:()=>{let a=i.indexOf(w);a>-1&&i.splice(a,1),m()}}),b=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:rt(),onOpen:()=>Y(),onClose:()=>{let a=i.indexOf(b);a>-1&&i.splice(a,1),m()}}),r=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:at(),onOpen:()=>Ce(),onClose:()=>{let a=i.indexOf(r);a>-1&&i.splice(a,1),m()}}),u=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:xt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Agents",content:ft()}],onOpen:()=>{Te(),be(),Le(),le()},onClose:()=>{let a=i.indexOf(u);a>-1&&i.splice(a,1),m()}}),c=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:it(),onOpen:()=>$e(),onClose:()=>{let a=i.indexOf(c);a>-1&&i.splice(a,1),m()}}),f=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:xe(),onOpen:()=>{f.setContent(xe()),ke(f)},onClose:()=>{let a=i.indexOf(f);a>-1&&i.splice(a,1),m()}}),h=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:a=>{b.isOpen()||x(b),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:a=>{w.isOpen()||x(w),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}},e){let a=document.getElementById("dexter-dropdown");a&&(a.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),y=document.getElementById("dexter-menu-btn");d&&a&&y&&(d.addEventListener("mouseenter",()=>{a.classList.add("active"),y.classList.add("active")}),d.addEventListener("mouseleave",()=>{a.classList.remove("active"),y.classList.remove("active")}),y.addEventListener("click",$=>{$.stopPropagation();let E=localStorage.getItem("dex_last_window")||"alerts-window",S=null;E==="alerts-window"?S=w:E==="events-window"?S=b:E==="monitor-window"?S=u:E==="contacts-window"?S=r:E==="workspace-window"&&(S=c),S&&x(S)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{p("alerts-window"),x(w)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{p("events-window"),x(b)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{p("monitor-window"),x(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{p("contacts-window"),x(r)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{p("workspace-window"),x(c)}),document.getElementById("settings-icon")?.addEventListener("click",()=>x(f)),document.getElementById("close-all-windows")?.addEventListener("click",()=>g()),setInterval(()=>{w.isOpen()?W():Ye(),b.isOpen()&&Y(),u.isOpen()&&wt(),c.isOpen()&&$e()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{h.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",a=>{a.preventDefault();try{qe(document.getElementById("email-input").value),window.location.reload()}catch(d){let y=document.getElementById("login-error");y&&(y.textContent=d.message,y.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Lt):Lt();})();

(()=>{function Ne(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function He(t=!1){let e=window.location.pathname,n=e==="/"||e==="/index.html",o=`
    ...
    `}
        ...
        `}
    `,l=document.createElement("nav");l.innerHTML=o,document.body.prepend(l)}function Re(t){let e=window.location.pathname,n=e==="/"||e==="/index.html",a=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!a||!s||(t||!n?(a.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{a.style.transform="translateX(-3px)"},s.onmouseleave=()=>{a.style.transform="translateX(0)"}):(a.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function Pe(){if(document.querySelector("footer"))return;let t=`
...
`,pe=null,F=new Set,ue=[];async function G(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await I(n);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];pe=Date.now(),O(0,pe);let l=Date.now(),f=24*60*60*1e3,b=o.filter(i=>{let d=localStorage.getItem(`alert_read_ts_${i.id}`);if(!d)return!0;let x=parseInt(d);return l-x<f});b.sort((i,d)=>{let x=k=>{let A=k.event;if(typeof A=="string")try{A=JSON.parse(A)}catch{return"low"}return(A.priority||"low").toLowerCase()},$=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,E=$(x(i)),S=$(x(d));return E!==S?S-E:d.timestamp-i.timestamp}),ue=b;let p=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},y=[],h=new Set(b.map(i=>p(i))).size>1;if(b.length>0){let i=null;b.forEach(d=>{let x=p(d);h&&x!==i&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),i=x),y.push(d)})}if(t&&(e.innerHTML=""),b.length===0){e.innerHTML=C("empty","No alerts yet."),de();return}let v=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let x=d.title||"Untitled Alert",$=d.body||"No description provided.",E=d.priority||"low",S=!!d.alert,k=d.category||"system",A=d.related_event_ids||[],D=d.audit_event_id,_=!!localStorage.getItem(`alert_read_ts_${i.id}`),N=new Date(i.timestamp*1e3),P=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=_?"event-border-grey":"event-border-blue";!_&&S&&(T="event-border-red"),_&&(E==="high"||E==="critical")?T="event-border-red":_&&E==="medium"&&(T="event-border-orange");let H=_?"alert-read":"alert-unread",R=F.has(i.id),he=R?"expanded":"",xe=R?"display: block;":"display: none;",Ae="",_e="";A.length>0&&(_e=`
...
`,et=null,X=new Set,nt=[];async function ge(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await I(n);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(nt=o,et=Date.now(),O(2,et),o.length===0){e.innerHTML=C("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),se(1,0);return}t&&(e.innerHTML="");let l=u=>{let h=u.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let v=h.title||"Untitled Blueprint",r=h.summary||h.body||"No summary provided.",m=h.content||"",c=h.category||"architecture",g=h.affected_services||[],w=h.implementation_path||[],i=new Date(u.timestamp*1e3),d=i.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=i.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=X.has(u.id),E=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,S.dataset.blueprintId=u.id;let A={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";S.onclick=function(P){if(this.classList.contains("expanded")){this.classList.remove("expanded"),X.delete(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),X.add(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let D=g.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${g.join(", ")}</span></div>`:"",B="";w.length>0&&(B=`
            ...
            `;let _=S.querySelector(".event-details");_&&_.addEventListener("click",P=>{P.stopPropagation()});let N=S.querySelector(".close-details-btn");return N&&N.addEventListener("click",P=>{P.stopPropagation(),S.classList.remove("expanded");let L=S.querySelector(".event-details");L&&(L.style.display="none"),X.delete(u.id)}),S},f=Array.from(e.children),b=new Map(f.map(u=>[u.dataset.blueprintId,u])),p=new Set(o.map(u=>u.id));f.forEach(u=>{let h=u.dataset.blueprintId;(!h||!p.has(h))&&u.remove()});let y=null;o.forEach((u,h)=>{let v=b.get(u.id);(!v||t)&&(v&&v.remove(),v=l(u),!v)||(h===0?e.firstElementChild!==v&&e.prepend(v):y&&y.nextElementSibling!==v&&y.after(v),y=v)}),se(2,o.length)}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=C("offline","Failed to load blueprints.","The event service may be offline."))}}function Pt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(n=>X.add(n.id)),ge(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{X.clear(),ge(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`,Ot=null;async function Le(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Le(),e.dataset.listenerAttached="true");try{let n=await Ke("/contacts");if(!n.ok)throw new Error("Service unreachable");let s=(await n.json()).members||[];if(Ot=Date.now(),s.length===0){t.innerHTML=C("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((l,f)=>{let b=o[l.level]??10,p=o[f.level]??10;return b!==p?b-p:l.username.localeCompare(f.username)}),t.innerHTML=s.map(l=>{let f=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",b=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",p="#888";l.level==="Me"||l.level==="Master"?p="#bb86fc":l.level==="Admin"?p="#cf6679":l.level==="Moderator"?p="#03dac6":l.level==="Contributor"&&(p="#ffa500");let y=l.level==="Me",u=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=y?"2px solid #bb86fc":`1px solid ${f}33`;return`
                <div class="user-profile-section" style="background: ${u}; padding: 15px; border-radius: 8px; border: ${h}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${y?"#bb86fc":f}22, transparent 70%); pointer-events: none;"></div>
...
`,fe=null,Z=new Set,ae=[],ve="all",qt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},zt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function lt(t){for(let[e,n]of Object.entries(qt))if(n.includes(t))return e;return"system"}function jt(t){return zt[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ft();let a=`/events?ml=${ve==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(a);if(!s.ok)throw new Error("Service is offline or unreachable.");let l=(await s.json()).events||[],f=l;if(ve!=="all"&&(f=l.filter(v=>{let r=v.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!1}return lt(r.type)===ve})),ae=f.slice(0,50),fe=Date.now(),O(1,fe),ae.length===0){e.innerHTML=C("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=v=>{let r=v.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let m=r.type,c=lt(m),g=jt(m),w=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),i="event-border-grey";w&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?i="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="system.analysis.audit"?i="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?i="event-border-orange":m==="system.test.completed"?i=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":i="event-border-blue");let d=w?"cursor-pointer":"",x=Z.has(v.id),$=x?"expanded":"",E=x?"display: block;":"display: none;",S=new Date(v.timestamp*1e3),k=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),D=ot(m,r),B=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",_="";if(w){let L="";if(m==="engagement.decision")L=`
                    ...
                    `;else if(m==="messaging.user.sent_message"){let T="";r.attachments&&r.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${r.attachments.map(R=>{let he=R.content_type&&R.content_type.startsWith("image/"),xe=(R.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${he?`<div class="attachment-thumb-container"><img src="${R.url}" alt="${R.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${R.url}" target="_blank" class="attachment-link">${R.filename}</a>
                                        <span class="attachment-meta">${R.content_type} \u2022 ${xe}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),L=`
                        <div class="event-detail-row">
                ...
                `}let N=document.createElement("div");N.className=`event-item ${i} ${d} ${$}`,N.dataset.eventId=v.id,N.onclick=function(L){if(!w)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),Z.delete(v.id);let H=this.querySelector(".event-details");H&&(H.style.display="none")}else{this.classList.add("expanded"),Z.add(v.id);let H=this.querySelector(".event-details");H&&(H.style.display="block")}},N.innerHTML=`
                <div class="event-time">
...
`);return`
            ...
            `}).join("");return t.innerHTML=o,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let f=unescape(l.dataset.logs);navigator.clipboard.writeText(f).then(()=>{let b=l.querySelector("i");b.classList.remove("bx-copy"),b.classList.add("bx-check"),setTimeout(()=>{b.classList.remove("bx-check"),b.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let f=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${f}?`))try{(await I(`/logs?service_id=${f}`,{method:"DELETE"})).ok&&oe()}catch(b){console.error("Error clearing logs:",b)}})}),Gt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=C("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ft=()=>`
        <div class="system-section-header">
            ...
            ${ct()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):s.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&c.STORAGE&&c.STORAGE.length>0?o.innerHTML=c.STORAGE.map(g=>{let w=(g.USED/1073741824).toFixed(1),i=(g.SIZE/(1024*1024*1024)).toFixed(1),d=g.SIZE>0?(g.USED/g.SIZE*100).toFixed(0):0,x=g.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await ut();c?(l(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),n&&!n.hasChildNodes())){let c=await ut();l(c)}if(!t)return;let f=await Et();if(!f||!f.services){t.children.length===0&&(t.innerHTML=C("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),O(0,dt);let b=f.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function p(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function y(c){if(!c||c==="N/A"||c==="unknown")return"-";let g=c.match(/^(\d+\.\d+\.\d+)/);return g?g[0]:c.split(".").slice(0,3).join(".")||"-"}function u(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function h(c){if(!c||c==="N/A"||c==="unknown")return"-";let g=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!g)return"-";let w=parseInt(g[1])||0,i=parseInt(g[2])||0,d=parseInt(g[3])||0,x=parseFloat(g[4])||0,$=w*86400+i*3600+d*60+x,E=Math.floor($/86400);if(E>0)return`${E}d`;let S=Math.floor($/3600);if(S>0)return`${S}h`;let k=Math.floor($/60);return k>0?`${k}m`:`${Math.floor($)}s`}function v(c){let g=c.status==="online",w=g?"service-widget-online":"service-widget-offline",i=g?"bx-check-circle":"bx-x-circle",d=g?"OK":"BAD",x=c.version?y(c.version.str):"-",$=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",E=c.memory&&c.memory!=="N/A"?c.memory:"-",S=$!=="-"?"#00ff00":"#666",k=$!=="-"?"#fff":"#666",A=E!=="-"?"#00ff00":"#666",D=E!=="-"?"#fff":"#666",B=h(c.uptime),_="";return g?_=`
                ...
                </div>`}function o(f){let b=f.status==="Downloaded",p=b?"service-widget-online":"service-widget-offline",y=b?"OK":"MISSING",u=b&&f.size>0?`${(f.size/1e9).toFixed(2)} GB`:"-",h=f.name;return f.type==="custom"&&h.endsWith(":latest")&&(h=h.replace(":latest","")),`<div class="service-widget ${p}" data-model-name="${f.name}"><div class="service-widget-header"><i class="bx ${b?"bx-check-circle":"bx-x-circle"}"></i><h3>${h}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${f.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${u}</span></div></div></div>`}let l="";if(a&&(l+=s(a)),l+=n.map(o).join(""),!l){t.innerHTML=C("empty","No models found.");return}t.innerHTML!==l&&(t.innerHTML=l)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),n=document.getElementById("analyst-t2-val"),a=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),o=document.getElementById("analyst-reset-btn");o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{o.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{o.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{o.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),be()}catch{o.innerHTML="<i class='bx bx-error'></i> Failed"}},o.dataset.listenerAttached="true");let l=await Vt();if(l){let h=Math.floor(Date.now()/1e3),v=l.active_tier,r=(m,c,g)=>{if(v===g||g==="guardian"&&v==="tests"){m.textContent="Working",m.style.color="#bb86fc";return}let w=c-h;if(w<=0)m.textContent="Ready",m.style.color="#5eff5e";else{let i=Math.floor(w/60),d=w%60;m.textContent=`${i}m ${d}s`,m.style.color="#fff"}};if(e&&l.guardian&&r(e,l.guardian.next_run,"guardian"),n&&l.architect&&r(n,l.architect.next_run,"architect"),a&&l.strategist&&r(a,l.strategist.next_run,"strategist"),s&&l.system_idle_time!==void 0){let m=l.system_idle_time,c=Math.floor(m/60),g=m%60;s.textContent=`${c}m ${g}s`,m>300?s.style.color="#5eff5e":m>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,n,a,s].forEach(v=>{v&&(v.textContent="Offline",v.style.color="#ff4d4d")});let f=await Wt(),b=[],p=[];f&&(Array.isArray(f)?b=f:(b=f.active||[],p=f.history||[]));let y=document.getElementById("vitals-processes-val");if(y)if(f){let h=b.length;y.textContent=h>0?`${h} Active`:"Idle",y.style.color=h>0?"#bb86fc":"#fff"}else y.textContent="-",y.style.color="#888";if(f===null){t.children.length===0&&(t.innerHTML=C("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),O(1,pt),b.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=C("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,b,!1));let u=document.getElementById("processes-history-widgets");u&&(p.length===0?u.querySelector(".tab-placeholder")||(u.innerHTML=C("empty","No recent history.")):(u.querySelector(".tab-placeholder")&&(u.innerHTML=""),gt(u,p,!0))),se(1,b.length)}function gt(t,e,n){function a(l){let f="";l.end_time?f=`${l.end_time-l.start_time}s`:f=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let b=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,y={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};y[p]?p=y[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`);let u=n?"#888":"#fff",h=n?"border-left: 3px solid #666;":"";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,n}function St(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function $t(t,e,n="output"){if(!Ie)return;let a=document.createElement("div");a.className=`terminal-line terminal-${n}`,n==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=me(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Lt.find(s=>s.id===t);if(!e)return;let n=Yt(e);$t(n,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(l=>setTimeout(l,400+Math.random()*600));let o="output";s.includes("[ERROR]")?o="error":s.includes("[SUCCESS]")||s.includes("\u2713")?o="success":s.includes("!")&&(o="warning"),$t(n,s,o)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ct(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code").textContent;navigator.clipboard.writeText(n).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(n=>{n.addEventListener("mouseenter",()=>{n.style.transform="translateY(-5px)",n.style.borderColor="rgba(255,255,255,0.15)",n.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{n.style.transform="translateY(0)",n.style.borderColor="rgba(255,255,255,0.05)",n.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=n.dataset.cmd;Xt(a)})})}async function Zt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function Tt(){Zt(),Ge(),Ne();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Ct();let e=Ue();He(e),Pe();let n=document.getElementById("nav-left-container");n&&n.addEventListener("click",()=>{if(o.length>0)u();else{let i=window.location.pathname;if(!(i==="/"||i==="/index.html")){let $=(i.endsWith("/")&&i.length>1?i.slice(0,-1):i).split("/");$.pop();let E=$.join("/")||"/";window.location.href=E}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!s&&document.querySelector("footer")?.classList.add("hide");let o=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let f=i=>{localStorage.setItem("dex_last_window",i)};function b(){return 1}function p(){for(;o.length>1;)o.shift().close(!0);let i=document.getElementById("windows-container"),d=document.querySelector("nav"),x=document.querySelector("footer"),$=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");i&&i.setAttribute("data-count",o.length);let S=o.length>0;o.forEach(B=>{let _=document.getElementById(B.id);_&&_.classList.remove("hide-close")}),le(te());let k=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),D=document.getElementById("settings-icon");if(Re(o.length>0),o.length>0){if(x?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),i&&(i.style.paddingTop="60px"),k&&(k.style.display="none"),D&&(D.style.display="none"),A){let B=o[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(B)?(A.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),x=document.getElementById("dexter-menu-btn");d&&i&&x&&(d.addEventListener("mouseenter",()=>{i.classList.add("active"),x.classList.add("active")}),d.addEventListener("mouseleave",()=>{i.classList.remove("active"),x.classList.remove("active")}),x.addEventListener("click",$=>{$.stopPropagation();let E=localStorage.getItem("dex_last_window")||"alerts-window",S=null;E==="alerts-window"?S=h:E==="events-window"?S=v:E==="monitor-window"?S=m:E==="contacts-window"?S=r:E==="workspace-window"&&(S=c),S&&y(S)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{f("alerts-window"),y(h)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{f("events-window"),y(v)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{f("monitor-window"),y(m)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{f("contacts-window"),y(r)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{f("workspace-window"),y(c)}),document.getElementById("settings-icon")?.addEventListener("click",()=>y(g)),document.getElementById("close-all-windows")?.addEventListener("click",()=>u()),setInterval(()=>{h.isOpen()?G():Ye(),v.isOpen()&&Q(),m.isOpen()&&wt(),c.isOpen()&&$e()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{w.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",i=>{i.preventDefault();try{ze(document.getElementById("email-input").value),window.location.reload()}catch(d){let x=document.getElementById("login-error");x&&(x.textContent=d.message,x.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Tt):Tt();})();

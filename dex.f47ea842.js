...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${tt()}
        </div>`;async function pt(){await Promise.all([ve(),Ee(),Se()]),await ne()}var st=null,nt=null,it=null;async function ut(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function at(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Ht(){try{return await(await A("/processes")).json()}catch{return null}}async function Rt(){try{return await(await A("/analyst/status")).json()}catch{return null}}async function Ee(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=S("offline","Failed to load hardware info.","The event service may be offline."));return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`

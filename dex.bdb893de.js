...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Et(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),H(0,ce);let c=Date.now(),b=24*60*60*1e3,g=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let x=parseInt(v);return c-x<b});g.sort((d,v)=>{let x=_=>{let k=_.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},L=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=L(x(d)),S=L(x(v));return E!==S?S-E:v.timestamp-d.timestamp}),de=g;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},w=[],p=new Set(g.map(d=>f(d))).size>1;if(g.length>0){let d=null;g.forEach(v=>{let x=f(v);p&&x!==d&&(w.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),w.push(v)})}if(t&&(e.innerHTML=""),g.length===0){e.innerHTML=T("empty","No notifications yet."),le();return}let n=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let x=v.title||"Untitled Notification",L=v.body||"No description provided.",E=v.priority||"low",S=!!v.alert,_=v.category||"system",k=v.related_event_ids||[],D=!!localStorage.getItem(`notification_read_ts_${d.id}`),R=new Date(d.timestamp*1e3),B=R.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=R.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=D?"event-border-grey":"event-border-blue";!D&&S&&(C="event-border-red"),D&&(E==="high"||E==="critical")?C="event-border-red":D&&E==="medium"&&(C="event-border-orange");let O=D?"notification-read":"notification-unread",N=q.has(d.id),ve=N?"expanded":"",be=N?"display: block;":"display: none;",Te="",ke="";k.length>0&&(ke=`
...
`,G=new Set,qe=[],J=null;async function W(t=!1){let e=document.getElementById("roadmap-list");if(e){St();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(qe=o,o.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=f=>{let w=G.has(f.id),u=f.state==="draft",p=f.state==="published",n=f.state==="consumed",i="event-border-grey";p&&(i="event-border-blue"),n&&(i="event-border-purple");let h=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${i} cursor-pointer ${w?"expanded":""}`,y.dataset.itemId=f.id,y.onclick=d=>{if(d.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(f.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",G.add(f.id))};let m=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;return y.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${I(f.content)}</div>
      ...
      `,y.querySelector(".edit-btn")?.addEventListener("click",()=>Lt(f)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>$t(f)),y.querySelector(".delete-btn")?.addEventListener("click",()=>Ct(f.id)),y.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(f.id)}),y},r=Array.from(e.children),c=new Map(r.map(f=>[f.dataset.itemId,f])),b=new Set(o.map(f=>f.id));r.forEach(f=>{let w=f.dataset.itemId;(!w||!b.has(w))&&f.remove()});let g=null;o.forEach((f,w)=>{let u=c.get(f.id);(!u||t)&&(u&&u.remove(),u=s(f),!u)||(w===0?e.firstElementChild!==u&&e.prepend(u):g&&g.nextElementSibling!==u&&g.after(u),g=u)})}catch{e.children.length===0&&(e.innerHTML=T("error","Failed to load roadmap."))}}}function St(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{J=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",J=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=J?`/roadmap/${J}`:"/roadmap",b=J?"PATCH":"POST";try{await A(c,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",W(!0)}catch(g){console.error(g)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{qe.forEach(r=>G.add(r.id)),W(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{G.clear(),W(!0)},s.dataset.listenerAttached="true")}function Lt(t){J=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function $t(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),W(!0)}catch(a){console.error(a)}}async function Ct(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),G.delete(t),W(!0)}catch(e){console.error(e)}}var Ge=()=>`
    <div class="notifications-actions">
...
`,je=null,Y=new Set,We=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Tt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(We=r,je=Date.now(),H(2,je),r.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),U(1,0);return}t&&(e.innerHTML="");let c=u=>{let p=u.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let n=p.title||"Untitled Blueprint",i=p.summary||p.body||"No summary provided.",l=p.content||"",h=p.category||"architecture",y=p.affected_services||[],m=p.implementation_path||[],d=new Date(u.timestamp*1e3),v=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=Y.has(u.id),E=L?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""}`,S.dataset.blueprintId=u.id;let k={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";S.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(u.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),Y.add(u.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}};let M=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",D="";m.length>0&&(D=`
...
`,ue=null,X=new Set,se=[],ge="all",It={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},At={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Xe(t){for(let[e,a]of Object.entries(It))if(a.includes(t))return e;return"system"}function _t(t){return At[t]||"bx-square-rounded"}async function Q(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Mt();let o=`/events?ml=${ge==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await A(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],b=c;if(ge!=="all"&&(b=c.filter(n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!1}return Xe(i.type)===ge})),se=b.slice(0,50),ue=Date.now(),H(1,ue),se.length===0){e.innerHTML=T("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let l=i.type,h=Xe(l),y=_t(l),m=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),d="event-border-grey";m&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?d="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?d="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?d="event-border-orange":l==="system.test.completed"?d=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let v=m?"cursor-pointer":"",x=X.has(n.id),L=x?"expanded":"",E=x?"display: block;":"display: none;",S=new Date(n.timestamp*1e3),_=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=Ye(l,i),D=i.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${i.user_level})</span>`:"",R="";if(m){let $="";if(l==="engagement.decision")$=`
                    ...
                    `;else if(l==="messaging.user.sent_message"){let C="";i.attachments&&i.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${i.attachments.map(N=>{let ve=N.content_type&&N.content_type.startsWith("image/"),be=(N.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${ve?`<div class="attachment-thumb-container"><img src="${N.url}" alt="${N.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
...
`),g=[...c.logs];if(g.length<25){let w=25-g.length;for(let u=0;u<w;u++)g.push("")}else g.length>25&&(g=g.slice(-25));let f=g.map(w=>re(w)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ze()}
        </div>`;async function dt(){await Promise.all([fe(),Ee(),Se()]),await ne()}var et=null,tt=null,st=null;async function mt(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function nt(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Bt(){try{return await(await A("/processes")).json()}catch{return null}}async function Nt(){try{return await(await A("/analyst/status")).json()}catch{return null}}async function Ee(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`}if(n.GPU&&n.GPU.length>0&&n.GPU.forEach((h,y)=>{let m=(h.VRAM/1073741824).toFixed(1);i+=`
                    <div class="service-widget" style="padding: 10px; min-height: 80px;">
                        <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let h=0,y=0;n.STORAGE.forEach(x=>{h+=x.USED,y+=x.SIZE});let m=(h/(1024*1024*1024)).toFixed(1),d=(y/(1024*1024*1024)).toFixed(1),v=y>0?(h/y*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=i};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let i=await nt();i?(o(i),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML='<p style="color: #ff4d4d;">Failed to refresh.</p>',a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let i=await nt();o(i)}}if(!t)return;let s=await mt();if(!s||!s.services){t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}et=Date.now(),H(3,et);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function g(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function f(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,h=parseInt(i[2])||0,y=parseInt(i[3])||0,m=parseFloat(i[4])||0,d=l*86400+h*3600+y*60+m,v=Math.floor(d/86400);if(v>0)return`${v}d`;let x=Math.floor(d/3600);if(x>0)return`${x}h`;let L=Math.floor(d/60);return L>0?`${L}m`:`${Math.floor(d)}s`}function w(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",h=i?"bx-check-circle":"bx-x-circle",y=i?"OK":"BAD",m=n.version?b(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",v=n.memory&&n.memory!=="N/A"?n.memory:"-",x=d!=="-"?"#00ff00":"#666",L=d!=="-"?"#fff":"#666",E=v!=="-"?"#00ff00":"#666",S=v!=="-"?"#fff":"#666",_=f(n.uptime),k="";return i?k=`
                ...
                </div>`:k='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${h}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${g(n.domain&&n.port?`${n.domain}:${n.port}`:"")}</span></div>${k}</div></div>`}let u=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),p=new Set(r.map(n=>n.id));for(let[n,i]of u)p.has(n)||i.remove();r.forEach(n=>{let i=w(n),l=u.get(n.id);l?l.outerHTML!==i&&(l.outerHTML=i):t.insertAdjacentHTML("beforeend",i)})}async function Se(){let t=document.getElementById("models-widgets");if(!t)return;let e=await mt();if(!e){t.innerHTML=T("offline","Failed to load model status.");return}tt=Date.now(),H(3,tt);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(b=>{b.classList.contains("service-widget")||b.remove()});function s(b){let g=b.status==="Ready";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ot=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ce=!0,a}function pt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ce=!1}function ut(t,e,a="output"){if(!Ce)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=re(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Pt(t){let e=gt.find(s=>s.id===t);if(!e)return;let a=Rt(e);ut(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),ut(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function ft(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ot();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;Pt(o)})})}async function Ut(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${ye()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function vt(){Ut(),Pe(),Ae();let t=window.location.pathname;(t==="/dex"||t==="/dex/"||t.includes("/dex/index.html"))&&!t.includes("/contribute")&&ft();let e=Be();_e(e),Me();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let c=m=>{localStorage.setItem("dex_last_window",m)};function b(){return 1}function g(){for(;s.length>1;)s.shift().close(!0);let m=document.getElementById("windows-container"),d=document.querySelector("nav"),v=document.querySelector("footer"),x=window.location.pathname==="/"||window.location.pathname==="/index.html",L=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");m&&m.setAttribute("data-count",s.length);let E=s.length>0;s.forEach(M=>{let D=document.getElementById(M.id);D&&D.classList.remove("hide-close")}),ie(te());let S=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon");if(s.length>0){if(v?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),m&&(m.style.paddingTop="60px"),S&&(S.style.display="none"),k&&(k.style.display="none"),_){let M=s[0].id;["feed-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-feed").addEventListener("click",()=>{c("feed-window"),f(u)}),document.getElementById("switch-events").addEventListener("click",()=>{c("events-window"),f(p)}),document.getElementById("switch-monitor").addEventListener("click",()=>{c("monitor-window"),f(i)}),document.getElementById("switch-contacts").addEventListener("click",()=>{c("contacts-window"),f(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{c("workspace-window"),f(l)})):_.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),m&&(m.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),x||L?v?.classList.remove("hide"):v?.classList.add("hide"),S&&(S.style.display="flex"),k&&(k.style.display="block"),_&&(_.innerHTML="")}function f(m){if(m.isOpen()){m.close();return}for(;s.length>0;)s.pop().close(!0);s.push(m),m.open(),g()}function w(){[...s].forEach(d=>d.close()),s=[]}window.addEventListener("resize",g);let u=F({id:"feed-window",title:"Feed",icon:"bx-news",content:ze(),onOpen:()=>j(),onClose:()=>{let m=s.indexOf(u);m>-1&&s.splice(m,1),g()}}),p=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Qe(),onOpen:()=>Q(),onClose:()=>{let m=s.indexOf(p);m>-1&&s.splice(m,1),g()}}),n=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Je(),onOpen:()=>pe(),onClose:()=>{let m=s.indexOf(n);m>-1&&s.splice(m,1),g()}}),i=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bx-server",title:"Services",content:ot()},{icon:"bx-component",title:"Processes",content:at()},{icon:"bx-brain",title:"Models",content:lt()},{icon:"bx-hdd",title:"Hardware",content:rt()},{icon:"bx-terminal",title:"Logs",content:ct()},{icon:"bx-zap",title:"Analyst",content:it()}],onOpen:()=>{Ee(),fe(),Se(),ne()},onClose:()=>{let m=s.indexOf(i);m>-1&&s.splice(m,1),g()}}),l=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Ve(),onOpen:()=>we(),onClose:()=>{let m=s.indexOf(l);m>-1&&s.splice(m,1),g()}}),h=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:Le(),onOpen:()=>$e(h),onClose:()=>{let m=s.indexOf(h);m>-1&&s.splice(m,1),g()}}),y=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dex/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let m=document.getElementById("dexter-dropdown");m&&(m.innerHTML=`
            <div class="dropdown-item" id="feed-menu-item"><i class='bx bx-news'></i> Feed</div>

(()=>{function He(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ne(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",n=`
    ...
    `}
        ...
        `}
    `,l=document.createElement("nav");l.innerHTML=n,document.body.prepend(l)}function Re(t){let e=window.location.pathname,a=e==="/"||e==="/index.html",o=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!o||!s||(t||!a?(o.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{o.style.transform="translateX(-3px)"},s.onmouseleave=()=>{o.style.transform="translateX(0)"}):(o.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function Pe(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Oe=1e3;function F(t){let e=null,a=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++Oe)}function n(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",l),o&&setTimeout(o,10);return}let g=document.getElementById("windows-container");g||(g=document.createElement("div"),g.id="windows-container",g.className="windows-container",document.body.appendChild(g)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Oe,e.addEventListener("mousedown",s);let h=t.icon||"bx-window",b="",r="",u;t.tabs&&t.tabs.length>0?(b=`<div class="tab-bar">${t.tabs.map((w,i)=>{let d=w.icon?`<i class="bx ${w.icon}"></i>`:`<span class="tab-glyph">${w.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,u=`<div class="window-content">${t.tabs.map((w,i)=>`<div class="tab-content ${i===0?"active":""}" data-tab-content="${i}">${w.content}</div>`).join("")}</div>`):(t.title&&(r=`<div class="window-title">${t.title}</div>`),u=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,ue=null,G=new Set,ge=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let n=(await o.json()).events||[];ue=Date.now(),P(0,ue);let l=Date.now(),p=24*60*60*1e3,v=n.filter(i=>{let d=localStorage.getItem(`alert_read_ts_${i.id}`);if(!d)return!0;let x=parseInt(d);return l-x<p});v.sort((i,d)=>{let x=L=>{let _=L.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},S=L=>L==="critical"?4:L==="high"?3:L==="medium"?2:1,E=S(x(i)),$=S(x(d));return E!==$?$-E:d.timestamp-i.timestamp}),ge=v;let m=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},y=[],h=new Set(v.map(i=>m(i))).size>1;if(v.length>0){let i=null;v.forEach(d=>{let x=m(d);h&&x!==i&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),i=x),y.push(d)})}if(t&&(e.innerHTML=""),v.length===0){e.innerHTML=T("empty","No alerts yet."),me();return}let b=i=>{let d=i.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let x=d.title||"Untitled Alert",S=d.body||"No description provided.",E=d.priority||"low",$=!!d.alert,L=d.category||"system",_=d.related_event_ids||[],A=d.audit_event_id,B=!!localStorage.getItem(`alert_read_ts_${i.id}`),N=new Date(i.timestamp*1e3),R=N.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=N.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=B?"event-border-grey":"event-border-blue";!B&&$&&(k="event-border-red"),B&&(E==="high"||E==="critical")?k="event-border-red":B&&E==="medium"&&(k="event-border-orange");let D=B?"alert-read":"alert-unread",H=G.has(i.id),ye=H?"expanded":"",he=H?"display: block;":"display: none;",Ae="",_e="";_.length>0&&(_e=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${_.map(q=>`<a href="#" onclick="window.dexter.viewEvent('${q}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px; font-family: monospace; width: fit-content;">${q.substring(0,8)}...</a>`).join(", ")}</span>
                    </div>`);let Me="";A&&(Me=`
              <div class="event-detail-row">
...
`+S)}</div>
            ...
            `;let U=document.createElement("div");U.className=`event-item notification-item ${k} ${D} ${ye} cursor-pointer priority-${E}`,U.dataset.alertId=i.id,U.onclick=function(q){if(this.classList.contains("expanded")){this.classList.remove("expanded"),G.delete(i.id);let te=this.querySelector(".event-details");te&&(te.style.display="none")}else{this.classList.add("expanded"),G.add(i.id);let te=this.querySelector(".event-details");if(te&&(te.style.display="block"),!localStorage.getItem(`alert_read_ts_${i.id}`)){localStorage.setItem(`alert_read_ts_${i.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ee="event-border-grey";E==="high"||E==="critical"?Ee="event-border-red":E==="medium"&&(Ee="event-border-orange"),this.classList.add(Ee),me()}}};let Qt=x,kt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[L]||"bx-bell";U.innerHTML=`
                ...
                <div class="event-content">
                    <div class="event-service">${L.toUpperCase()} ${$?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>
                    <div class="event-message">${x}</div>
                    <div class="event-details" style="${he}">
                        ${Ae}
                    </div>
                </div>
            `;let Be=U.querySelector(".event-details");Be&&Be.addEventListener("click",q=>{q.stopPropagation()});let De=U.querySelector(".close-details-btn");return De&&De.addEventListener("click",q=>{q.stopPropagation(),U.classList.remove("expanded");let we=U.querySelector(".event-details");we&&(we.style.display="none"),G.delete(i.id)}),U},r=i=>{let d=document.createElement("div");d.className="notification-divider",d.dataset.alertId=i.id;let x="#888888";return i.label==="CRITICAL"?x="#ff4d4d":i.label==="HIGH"?x="#ff8888":i.label==="MEDIUM"&&(x="#ffa500"),d.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,d.innerHTML=`<span style="white-space: nowrap;">${i.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,d},u=Array.from(e.children),c=new Map(u.map(i=>[i.dataset.alertId,i])),f=new Set(y.map(i=>i.id));u.forEach(i=>{let d=i.dataset.alertId;(!d||!f.has(d))&&i.remove()});let w=null;y.forEach((i,d)=>{let x=c.get(i.id);(!x||t)&&(x&&x.remove(),i.type==="divider"?x=r(i):x=b(i),!x)||(d===0?e.firstElementChild!==x&&e.prepend(x):w&&w.nextElementSibling!==x&&w.after(x),w=x)}),ue=Date.now(),P(0,ue),me()}catch(o){console.error("Error fetching alerts:",o),e.children.length===0&&(e.innerHTML=T("offline","Failed to load alerts.","The event service may be offline."))}}function Bt(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),a=document.getElementById("alerts-close-all"),o=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ge.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ge.forEach(s=>{G.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{G.clear(),W(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await I("/events?type=system.notification.generated",{method:"DELETE"});let s=Date.now()-48*60*60*1e3;ge.forEach(n=>{localStorage.setItem(`alert_read_ts_${n.id}`,s.toString())}),G.clear(),W(!0)}catch(s){console.error("Failed to clear alerts:",s)}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}async function Ye(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await I(t);if(!e.ok)return;let o=(await e.json()).events||[],s=0;o.forEach(n=>{let l=n.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}if((l.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${n.id}`)||s++}),ie(s)}catch{}}var Xe=()=>`
...
`,V=new Set,Qe=[],Q=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let a=await I("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(Qe=o,o.length===0){e.innerHTML=T("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let y=V.has(m.id),g=m.state==="draft",h=m.state==="published",b=m.state==="consumed",r="event-border-grey";h&&(r="event-border-blue"),b&&(r="event-border-purple");let c=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),f=document.createElement("div");f.className=`event-item notification-item ${r} cursor-pointer ${y?"expanded":""}`,f.dataset.itemId=m.id,f.onclick=d=>{if(d.target.closest("button"))return;f.classList.contains("expanded")?(f.classList.remove("expanded"),f.querySelector(".event-details").style.display="none",V.delete(m.id)):(f.classList.add("expanded"),f.querySelector(".event-details").style.display="block",V.add(m.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;f.innerHTML=`
      ...
      `;let i=f.querySelector(".event-details");return i&&i.addEventListener("click",d=>{d.stopPropagation()}),f.querySelector(".edit-btn")?.addEventListener("click",()=>Ht(m)),f.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Nt(m)),f.querySelector(".delete-btn")?.addEventListener("click",()=>Rt(m.id)),f.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),f.classList.remove("expanded"),f.querySelector(".event-details").style.display="none",V.delete(m.id)}),f},n=Array.from(e.children),l=new Map(n.map(m=>[m.dataset.itemId,m])),p=new Set(o.map(m=>m.id));n.forEach(m=>{let y=m.dataset.itemId;(!y||!p.has(y))&&m.remove()});let v=null;o.forEach((m,y)=>{let g=l.get(m.id);(!g||t)&&(g&&g.remove(),g=s(m),!g)||(y===0?e.firstElementChild!==g&&e.prepend(g):v&&v.nextElementSibling!==g&&v.after(g),v=g)})}catch{e.children.length===0&&(e.innerHTML=T("offline","Failed to load roadmap.","The event service may be offline."))}}}function Dt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Q=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",Q=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let n=document.getElementById("roadmap-editor-input").value;if(!n.trim())return;let l=Q?`/roadmap/${Q}`:"/roadmap",p=Q?"PATCH":"POST";try{await I(l,{method:p,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:n})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{Qe.forEach(n=>V.add(n.id)),K(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function Ht(t){Q=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Nt(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(a){console.error(a)}}async function Rt(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var tt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,et=null,ee=new Set,nt=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Pt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await I(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let n=(await o.json()).events||[];if(nt=n,et=Date.now(),P(2,et),n.length===0){e.innerHTML=T("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ne(2,0);return}t&&(e.innerHTML="");let l=g=>{let h=g.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let b=h.title||"Untitled Blueprint",r=h.summary||h.body||"No summary provided.",u=h.content||"",c=h.category||"architecture",f=h.affected_services||[],w=h.implementation_path||[],i=new Date(g.timestamp*1e3),d=i.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=i.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=ee.has(g.id),E=S?"display: block;":"display: none;",$=document.createElement("div");$.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""}`,$.dataset.blueprintId=g.id;let _={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[c]||"bx-paint";$.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ee.delete(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="none")}else{this.classList.add("expanded"),ee.add(g.id);let k=this.querySelector(".event-details");k&&(k.style.display="block")}};let A=f.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${f.join(", ")}</span></div>`:"",O="";w.length>0&&(O=`
            ...
            `;let B=$.querySelector(".event-details");B&&B.addEventListener("click",R=>{R.stopPropagation()});let N=$.querySelector(".close-details-btn");return N&&N.addEventListener("click",R=>{R.stopPropagation(),$.classList.remove("expanded");let C=$.querySelector(".event-details");C&&(C.style.display="none"),ee.delete(g.id)}),$},p=Array.from(e.children),v=new Map(p.map(g=>[g.dataset.blueprintId,g])),m=new Set(n.map(g=>g.id));p.forEach(g=>{let h=g.dataset.blueprintId;(!h||!m.has(h))&&g.remove()});let y=null;n.forEach((g,h)=>{let b=v.get(g.id);(!b||t)&&(b&&b.remove(),b=l(g),!b)||(h===0?e.firstElementChild!==b&&e.prepend(b):y&&y.nextElementSibling!==b&&y.after(b),y=b)}),ne(2,n.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=T("offline","Failed to load blueprints.","The event service may be offline."))}}function Pt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{nt.forEach(a=>ee.add(a.id)),fe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ee.clear(),fe(!0)},e.dataset.listenerAttached="true")}var it=()=>`
...
`,Ot=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let a=await Ke("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Ot=Date.now(),s.length===0){t.innerHTML=T("empty","No contacts found.","Check your Discord connection.");return}let n={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((l,p)=>{let v=n[l.level]??10,m=n[p.level]??10;return v!==m?v-m:l.username.localeCompare(p.username)}),t.innerHTML=s.map(l=>{let p=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",m="#888";l.level==="Me"||l.level==="Master"?m="#bb86fc":l.level==="Admin"?m="#cf6679":l.level==="Moderator"?m="#03dac6":l.level==="Contributor"&&(m="#ffa500");let y=l.level==="Me",g=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=y?"2px solid #bb86fc":`1px solid ${p}33`;return`
...
`,ve=null,J=new Set,oe=[],j="all",zt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},qt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function lt(t){for(let[e,a]of Object.entries(zt))if(a.includes(t))return e;return"system"}function Ft(t){return qt[t]||"bx-square-rounded"}async function Y(t=!1){let e=document.getElementById("events-timeline");if(!e)return;jt();let o=`/events?ml=${j==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let l=(await s.json()).events||[],p=l;if(j!=="all"&&(p=l.filter(b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!1}return lt(r.type)===j})),oe=p.slice(0,50),ve=Date.now(),P(1,ve),oe.length===0){e.innerHTML=T("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let u=r.type,c=lt(u),f=Ft(u),w=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),i="event-border-grey";w&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?i="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="system.analysis.audit"?i="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?i="event-border-orange":u==="system.test.completed"?i=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":i="event-border-blue");let d=w?"cursor-pointer":"",x=J.has(b.id),S=x?"expanded":"",E=x?"display: block;":"display: none;",$=new Date(b.timestamp*1e3),L=$.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=$.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=ot(u,r),O=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",B="";if(w){let C="";if(u==="engagement.decision")C=`
                    ...
                    `;else if(u==="system.attention.expired"){let k=r.timestamp-r.last_active,D=Math.floor(k/60);C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="analysis.visual.completed"){let k="";r.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${r.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:r.url&&(k=`<div class="event-detail-block"><img src="${r.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(u==="system.analysis.audit")C=`
                        <div class="event-detail-row">
                            <span class="detail-label">Tier:</span>
                    ...
                    `;else if(u==="messaging.user.sent_message"){let k="";r.attachments&&r.attachments.length>0&&(k=`
                            <div class="event-detail-block">
...
`);return`
            ...
            `}).join("");return t.innerHTML=n,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let p=unescape(l.dataset.logs);navigator.clipboard.writeText(p).then(()=>{let v=l.querySelector("i");v.classList.remove("bx-copy"),v.classList.add("bx-check"),setTimeout(()=>{v.classList.remove("bx-check"),v.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let p=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${p}?`))try{(await I(`/logs?service_id=${p}`,{method:"DELETE"})).ok&&le()}catch(v){console.error("Error clearing logs:",v)}})}),Gt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=T("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ft=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
                ...
                </div>`}if(a&&c.CPU&&c.CPU.length>0){let f=c.CPU[0];a.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}s&&(c.GPU&&c.GPU.length>0?s.innerHTML=c.GPU.map(f=>{let w=(f.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):s.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),n&&c.STORAGE&&c.STORAGE.length>0?n.innerHTML=c.STORAGE.map(f=>{let w=(f.USED/1073741824).toFixed(1),i=(f.SIZE/(1024*1024*1024)).toFixed(1),d=f.SIZE>0?(f.USED/f.SIZE*100).toFixed(0):0,x=f.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):n&&(n.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await ut();c?(l(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await ut();l(c)}if(!t)return;let p=await Et();if(!p||!p.services){t.children.length===0&&(t.innerHTML=T("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),P(0,dt);let v=p.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function m(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function y(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/^(\d+\.\d+\.\d+)/);return f?f[0]:c.split(".").slice(0,3).join(".")||"-"}function g(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function h(c){if(!c||c==="N/A"||c==="unknown")return"-";let f=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!f)return"-";let w=parseInt(f[1])||0,i=parseInt(f[2])||0,d=parseInt(f[3])||0,x=parseFloat(f[4])||0,S=w*86400+i*3600+d*60+x,E=Math.floor(S/86400);if(E>0)return`${E}d`;let $=Math.floor(S/3600);if($>0)return`${$}h`;let L=Math.floor(S/60);return L>0?`${L}m`:`${Math.floor(S)}s`}function b(c){let f=c.status==="online",w=f?"service-widget-online":"service-widget-offline",i=f?"bx-check-circle":"bx-x-circle",d=f?"OK":"BAD",x=c.version?y(c.version.str):"-",S=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",E=c.memory&&c.memory!=="N/A"?c.memory:"-",$=S!=="-"?"#00ff00":"#666",L=S!=="-"?"#fff":"#666",_=E!=="-"?"#00ff00":"#666",A=E!=="-"?"#fff":"#666",O=h(c.uptime),B="";return f?B=`
                ...
                </div>`:B='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${w}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${i}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${d}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${g(c.domain&&c.port?`${c.domain}:${c.port}`:"")}</span></div>${B}</div></div>`}let r=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),u=new Set(v.map(c=>c.id));for(let[c,f]of r)u.has(c)||f.remove();v.forEach(c=>{let f=b(c),w=r.get(c.id);w?w.outerHTML!==f&&(w.outerHTML=f):t.insertAdjacentHTML("beforeend",f)})}async function Te(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Et();if(!e){t.children.length===0&&(t.innerHTML=T("offline","Failed to load model status.","The event service may be offline."));return}mt=Date.now(),P(2,mt);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function s(p){let v=p.status==="Ready";return`
                ...
                </div>`}function n(p){let v=p.status==="Downloaded",m=v?"service-widget-online":"service-widget-offline",y=v?"OK":"MISSING",g=v&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",h=p.name;return p.type==="custom"&&h.endsWith(":latest")&&(h=h.replace(":latest","")),`<div class="service-widget ${m}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${h}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let l="";if(o&&(l+=s(o)),l+=a.map(n).join(""),!l){t.innerHTML=T("empty","No models found.");return}t.innerHTML!==l&&(t.innerHTML=l)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),a=document.getElementById("guardian-t2-val"),o=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),n=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await I("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{p.innerHTML="<i class='bx bx-error'></i> Failed"}},p.dataset.listenerAttached="true");let v=await Vt();if(v){let r=Math.floor(Date.now()/1e3),u=v.active_tier,c=d=>{d<0&&(d=0);let x=Math.floor(d/3600),S=Math.floor(d%3600/60),E=d%60;return x>0?`${x}h ${S}m`:S>0?`${S}m ${E}s`:`${E}s`},f=(d,x,S,E)=>{if(u===E)d.textContent="Working",d.style.color="#bb86fc";else if(E==="t1"&&u==="tests")d.textContent="Testing",d.style.color="#03dac6";else if(S){let L=S.next_run-r;if(L<=0)d.textContent="Ready",d.style.color="#5eff5e";else{let _=Math.floor(L/60),A=L%60;d.textContent=`${_}m ${A}s`,d.style.color="#fff"}}x&&S&&(x.innerHTML=`
        ...
        `)};e&&f(e,document.getElementById("guardian-t1-stats"),v.t1,"t1"),a&&f(a,document.getElementById("guardian-t2-stats"),v.t2,"t2");let w=document.getElementById("system-state-label"),i=document.getElementById("system-state-val");if(i&&v.system_state){let d=v.system_state,x=c(v.system_state_time||0);w&&(w.textContent=`State: ${d.toUpperCase()}`),i.textContent=x,d==="idle"?i.style.color=v.system_state_time>300?"#5eff5e":"#fff":i.style.color="#bb86fc"}s&&(s.textContent=c(v.total_idle_time||0)),n&&(n.textContent=c(v.total_active_time||0)),l&&(l.textContent=c(v.total_waste_time||0))}else[e,a,o,s,n,l].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let m=await Wt(),y=[],g=[];m&&(Array.isArray(m)?y=m:(y=m.active||[],g=m.history||[]));let h=document.getElementById("vitals-processes-val");if(h)if(m){let r=y.length;h.textContent=r>0?`${r} Active`:"Idle",h.style.color=r>0?"#bb86fc":"#fff"}else h.textContent="-",h.style.color="#888";if(m===null){t.children.length===0&&(t.innerHTML=T("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),P(1,pt),y.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=T("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,y,!1));let b=document.getElementById("processes-history-widgets");b&&(g.length===0?b.querySelector(".tab-placeholder")||(b.innerHTML=T("empty","No recent history.")):(b.querySelector(".tab-placeholder")&&(b.innerHTML=""),gt(b,g,!0))),ne(1,y.length)}function gt(t,e,a){function o(l){let p="";l.end_time?p=`${l.end_time-l.start_time}s`:p=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let v=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",m=l.channel_id,y={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};y[m]?m=y[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`);let g=a?"#888":"#fff",h=a?"border-left: 3px solid #666;":"";return`
                <div class="service-widget process-widget" data-channel-id="${l.channel_id}-${l.start_time}" style="${h}">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${g}">${p}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let s=new Map(Array.from(t.querySelectorAll(".process-widget")).map(l=>[l.dataset.channelId,l])),n=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,p]of s)n.has(l)||p.remove();e.forEach(l=>{let p=`${l.channel_id}-${l.start_time}`,v=o(l),m=s.get(p);if(m){if(m.outerHTML!==v){m.outerHTML=v;let y=t.querySelector(`[data-channel-id="${p}"]`);y&&s.set(p,y)}}else{t.insertAdjacentHTML("beforeend",v);let y=t.querySelector(`[data-channel-id="${p}"]`);y&&s.set(p,y)}})}function xe(){let t=se(),e=ze()||"master@easter.company",a={enabled:Notification.permission==="granted",supported:"Notification"in window},o=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[a,o]of Object.entries(t)){let s=Ct.filter(n=>n.category===a);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,a}function St(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function $t(t,e,a="output"){if(!Ie)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=pe(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let a=Yt(e);$t(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(l=>setTimeout(l,400+Math.random()*600));let n="output";s.includes("[ERROR]")?n="error":s.includes("[SUCCESS]")||s.includes("\u2713")?n="success":s.includes("!")&&(n="warning"),$t(a,s,n)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Lt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;Xt(o)})})}async function Zt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Tt(){Zt(),Ge(),He();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Lt();let e=Ue();Ne(e),Pe();let a=document.getElementById("nav-left-container");a&&a.addEventListener("click",()=>{if(n.length>0)g();else{let i=window.location.pathname;if(!(i==="/"||i==="/index.html")){let S=(i.endsWith("/")&&i.length>1?i.slice(0,-1):i).split("/");S.pop();let E=S.join("/")||"/";window.location.href=E}}});let o=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!o&&!s&&document.querySelector("footer")?.classList.add("hide");let n=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let p=i=>{localStorage.setItem("dex_last_window",i)};function v(){return 1}function m(){for(;n.length>1;)n.shift().close(!0);let i=document.getElementById("windows-container"),d=document.querySelector("nav"),x=document.querySelector("footer"),S=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");i&&i.setAttribute("data-count",n.length),n.forEach(A=>{let O=document.getElementById(A.id);O&&O.classList.remove("hide-close")}),re(se());let $=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),_=document.getElementById("settings-icon");if(Re(n.length>0),n.length>0){if(x?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),i&&(i.style.paddingTop="60px"),$&&($.style.display="none"),_&&(_.style.display="none"),L){let A=n[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{p("alerts-window"),y(h)}),document.getElementById("switch-events").addEventListener("click",()=>{p("events-window"),y(b)}),document.getElementById("switch-monitor").addEventListener("click",()=>{p("monitor-window"),y(u)}),document.getElementById("switch-contacts").addEventListener("click",()=>{p("contacts-window"),y(r)}),document.getElementById("switch-workspace").addEventListener("click",()=>{p("workspace-window"),y(c)})):L.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),i&&(i.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S||E?x?.classList.remove("hide"):x?.classList.add("hide"),$&&($.style.display="flex"),_&&(_.style.display="block"),L&&(L.innerHTML="");ie(Ve())}function y(i){if(i.isOpen()){i.close();return}for(;n.length>0;)n.pop().close(!0);n.push(i),i.open(),m()}function g(){[...n].forEach(d=>d.close()),n=[]}window.addEventListener("resize",m);let h=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Je(),onOpen:()=>W(),onClose:()=>{let i=n.indexOf(h);i>-1&&n.splice(i,1),m()}}),b=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:rt(),onOpen:()=>Y(),onClose:()=>{let i=n.indexOf(b);i>-1&&n.splice(i,1),m()}}),r=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:at(),onOpen:()=>Ce(),onClose:()=>{let i=n.indexOf(r);i>-1&&n.splice(i,1),m()}}),u=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:xt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Agents",content:ft()}],onOpen:()=>{Le(),be(),Te(),le()},onClose:()=>{let i=n.indexOf(u);i>-1&&n.splice(i,1),m()}}),c=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:it(),onOpen:()=>$e(),onClose:()=>{let i=n.indexOf(c);i>-1&&n.splice(i,1),m()}}),f=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:xe(),onOpen:()=>{f.setContent(xe()),ke(f)},onClose:()=>{let i=n.indexOf(f);i>-1&&n.splice(i,1),m()}}),w=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:i=>{b.isOpen()||y(b),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${i}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:i=>{h.isOpen()||y(h),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${i}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}},e){let i=document.getElementById("dexter-dropdown");i&&(i.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),x=document.getElementById("dexter-menu-btn");d&&i&&x&&(d.addEventListener("mouseenter",()=>{i.classList.add("active"),x.classList.add("active")}),d.addEventListener("mouseleave",()=>{i.classList.remove("active"),x.classList.remove("active")}),x.addEventListener("click",S=>{S.stopPropagation();let E=localStorage.getItem("dex_last_window")||"alerts-window",$=null;E==="alerts-window"?$=h:E==="events-window"?$=b:E==="monitor-window"?$=u:E==="contacts-window"?$=r:E==="workspace-window"&&($=c),$&&y($)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{p("alerts-window"),y(h)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{p("events-window"),y(b)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{p("monitor-window"),y(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{p("contacts-window"),y(r)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{p("workspace-window"),y(c)}),document.getElementById("settings-icon")?.addEventListener("click",()=>y(f)),document.getElementById("close-all-windows")?.addEventListener("click",()=>g()),setInterval(()=>{h.isOpen()?W():Ye(),b.isOpen()&&Y(),u.isOpen()&&wt(),c.isOpen()&&$e()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{w.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",i=>{i.preventDefault();try{qe(document.getElementById("email-input").value),window.location.reload()}catch(d){let x=document.getElementById("login-error");x&&(x.textContent=d.message,x.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Tt):Tt();})();

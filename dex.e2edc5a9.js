(()=>{function ge(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function be(e=!1){let n=`
        <div class="nav-left">
...
`);return`
            ...
            `}).join("");return e.innerHTML=s,document.querySelectorAll(".copy-logs-btn").forEach(d=>{d.addEventListener("click",()=>{let u=unescape(d.dataset.logs);navigator.clipboard.writeText(u).then(()=>{let b=d.querySelector("i");b.classList.remove("bx-copy"),b.classList.add("bx-check"),setTimeout(()=>{b.classList.remove("bx-check"),b.classList.add("bx-copy")},2e3)})})}),ae=Date.now(),!0}catch(f){return console.error("Error fetching logs:",f),e.classList.add("placeholder-active"),e.innerHTML=O("offline","Failed to load logs.","The event service may be offline, unreachable, or blocked by a browser extension (e.g., ad blocker)."),!1}}function $(e,t,n=null){let r={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[e]||"bx-info-circle",a=n?`<p class="placeholder-action">${n}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${r} placeholder-icon'></i><p class="placeholder-message">${t}</p>${a}</div>`}function C(e){return e&&e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function E(e,t){let n=document.querySelector(`.tab[data-tab-index="${e}"] .tab-subtitle`);if(!n)return;if(!t){n.textContent="Last updated: never";return}let r=(Date.now()-t)/1e3,a;if(r<30)a=`${Math.floor(r)}s ago`;else{n.textContent="Last updated: never";return}n.textContent=`Last updated: ${a}`}function j(e,t){let n=document.querySelector(`.tab[data-tab-index="${e}"]`);if(!n)return;let m=n.querySelector(".notification-badge");m&&(t>0?(m.textContent=t>9?"9+":t,m.style.display="flex"):m.style.display="none")}function G(){let e=document.getElementById("notifications-list");if(!e)return;let t=e.querySelectorAll(".notification-unread").length;j(0,t)}var He={message_received:"{user} posted in {channel}: {message}",action_performed:"{actor} {action} {target}",log_entry:"[{level}] {message}",error_occurred:"ERROR: {error}",status_change:"{entity} changed status to {new_status}",metric_recorded:"{metric_name}: {value}{unit}","messaging.user.joined_voice":"{user_name} joined voice channel {channel_name}","messaging.user.left_voice":"{user_name} left voice channel {channel_name}","messaging.user.sent_message":"{user_name} in {channel_name}: {content}","messaging.bot.sent_message":"Dexter sent in {channel_name}: {content}","messaging.bot.joined_voice":"Dexter joined voice channel {channel_name}","messaging.bot.voice_response":"Dexter said: {content}","messaging.bot.status_update":"Dexter status changed to {status}: {details}","messaging.user.speaking.started":"{user_name} started speaking","messaging.user.speaking.stopped":"{user_name} stopped speaking","messaging.user.transcribed":"{user_name} said: {transcription}","messaging.user.joined_server":"{user_name} joined {server_name}","messaging.webhook.message":"{user_name} (Webhook): {content}","webhook.processed":"Webhook processed: {status}",voice_speaking_started:"User {user_id} started speaking in voice channel {channel_id}",voice_speaking_stopped:"User {user_id} stopped speaking in voice channel {channel_id}",voice_transcribed:"{user_name} said: {transcription}","engagement.decision":"Engagement Decision: {decision} ({reason})",bot_response:"Bot Responded: {response}","moderation.explicit_content.deleted":"Explicit content deleted in {channel_name} from {user_name}: {reason}","analysis.link.completed":"Analyzed link: {url}","analysis.visual.completed":"Analyzed image: {filename}","system.cli.command":"CLI Command: {command} {args} ({status})","system.cli.status":"CLI Status: {message}","system.notification.generated":"Notification ({priority}): {title}"};function Le(e,t){let n=He[e];if(e==="voice_transcribed"&&!t.user_name&&t.user_id&&(n="User {user_id} said in voice channel {channel_id}: {transcription}"),e==="system.notification.generated"&&(n=`Notification (${t.priority?t.priority.charAt(0).toUpperCase()+t.priority.slice(1):"Low"}): ${t.title}`),!n)return e;let m=n.replace(/\{(\w+)\}/g,(r,a)=>t[a]!==void 0&&t[a]!==null?t[a]:r);return(e==="messaging.user.transcribed"||e==="voice_transcribed")&&t.detected_language&&t.detected_language!=="en"&&t.english_translation&&(m+=` (Translation: ${t.english_translation})`),m}var Ee=()=>'<div id="events-timeline" class="events-timeline"><p>Loading events...</p></div>',V=null;async function re(){let e=document.getElementById("events-timeline");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(f=>f.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let r=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let a=new Set(Array.from(e.querySelectorAll(".event-item.expanded")).map(o=>o.dataset.eventId).filter(o=>o)),f=await fetch(r);if(!f.ok)throw new Error("Service is offline or unreachable.");let v=(await f.json()).events||[];if(v.length===0){e.innerHTML=$("empty","No events found.");return}let y=o=>{let i=o.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let c=i.type,p=c==="engagement.decision"||c==="messaging.bot.sent_message"||c==="messaging.user.sent_message"||c==="moderation.explicit_content.deleted"||c==="analysis.link.completed"||c==="analysis.visual.completed"||c==="system.cli.command",g="event-border-grey";p&&(c==="moderation.explicit_content.deleted"?g="event-border-red":c==="analysis.link.completed"||c==="analysis.visual.completed"?g="event-border-purple":c==="system.cli.command"?g="event-border-orange":g="event-border-blue");let h=p?"cursor-pointer":"",x=a.has(o.id),w=x?"expanded":"",k=x?"display: block;":"display: none;",I=new Date(o.timestamp*1e3),U=I.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),W=I.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),D=Le(c,i),_="";if(p){let L="";if(c==="engagement.decision")L=`
                        ...
                        `;else if(c==="analysis.visual.completed"){let S="";i.base64_preview?S=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${i.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:i.url&&(S=`<div class="event-detail-block"><img src="${i.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),L=`
                            <div class="event-detail-row">
                        ...
                        `;else if(c==="messaging.user.sent_message"){let S="";i.attachments&&i.attachments.length>0&&(S=`
                                <div class="event-detail-block">
                                    <span class="detail-label">Attachments:</span>
                                    <div class="attachments-grid">${i.attachments.map(T=>{let B=T.content_type&&T.content_type.startsWith("image/"),ee=(T.size/1024).toFixed(1)+" KB";return`
                                    <div class="attachment-item">
                                        ...
                                        <div class="attachment-info">
                                            ...
                                            <span class="attachment-meta">${T.content_type} \u2022 ${ee}</span>
                                        </div>
                                    </div>`}).join("")}</div>
                                </div>`),L=`
                            <div class="event-detail-row">
                    ...
                    `}let A=document.createElement("div");if(A.className=`event-item ${g} ${h} ${w}`,A.dataset.eventId=o.id,A.onclick=function(L){if(!p)return;this.classList.toggle("expanded");let S=this.querySelector(".event-details");S&&(S.style.display=S.style.display==="none"?"block":"none")},A.innerHTML=`
                ...
                `,p){let L=A.querySelector(".close-details-btn");L&&L.addEventListener("click",S=>{S.stopPropagation();let N=S.target.closest(".event-item");if(N){N.classList.remove("expanded");let T=N.querySelector(".event-details");T&&(T.style.display="none")}})}return A},s=Array.from(e.children),d=new Map(s.map(o=>[o.dataset.eventId,o])),u=new Set(v.map(o=>o.id));s.forEach(o=>{(!o.dataset.eventId||!u.has(o.dataset.eventId))&&o.remove()});let b=null;v.forEach((o,i)=>{let c=d.get(o.id);!c&&(c=y(o),!c)||(i===0?e.firstElementChild!==c&&e.prepend(c):b&&b.nextElementSibling!==c&&b.after(c),b=c)}),V=Date.now(),E(2,V)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=$("offline","Failed to load events.","The event service may be offline or unreachable."))}}var _e=()=>'<div id="notifications-list" class="notifications-list events-timeline" style="display: flex; flex-direction: column; gap: 15px;"><p>Loading notifications...</p></div>',Y=null;async function le(){let e=document.getElementById("notifications-list");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(f=>f.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let r=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/events?ml=1000&format=json&event.type=system.notification.generated`;try{let a=await fetch(r);if(!a.ok)throw new Error("Service is offline or unreachable.");let l=(await a.json()).events||[],v=Date.now(),y=24*60*60*1e3,s=l.filter(p=>{let g=localStorage.getItem(`notification_read_ts_${p.id}`);if(!g)return!0;let h=parseInt(g);return v-h<y});if(s.length===0){e.innerHTML=$("empty","No notifications yet."),G();return}let d=p=>{let g=p.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let h=g.title||"Untitled Notification",x=g.body||"No description provided.",w=g.priority||"low",k=!!g.alert,I=g.category||"system",U=g.related_event_ids||[],D=!!localStorage.getItem(`notification_read_ts_${p.id}`),_=new Date(p.timestamp*1e3),A=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=D?"event-border-grey":"event-border-blue";!D&&k&&(S="event-border-red"),D&&(w==="high"||w==="critical")?S="event-border-red":D&&w==="medium"&&(S="event-border-orange");let N=D?"notification-read":"notification-unread",T=u.has(p.id),B=T?"expanded":"",ee=T?"display: block;":"display: none;",ue="";U.length>0&&(ue=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${U.map(F=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${F.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`);let H=document.createElement("div");H.className=`event-item notification-item ${S} ${N} ${B} cursor-pointer`,H.dataset.notificationId=p.id,H.onclick=function(F){this.classList.toggle("expanded");let q=this.querySelector(".event-details");if(q){let fe=q.style.display==="none";if(q.style.display=fe?"block":"none",fe){if(u.add(p.id),!localStorage.getItem(`notification_read_ts_${p.id}`)){localStorage.setItem(`notification_read_ts_${p.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue"),this.classList.remove("event-border-red");let te="event-border-grey";w==="high"||w==="critical"?te="event-border-red":w==="medium"&&(te="event-border-orange"),this.classList.add(te),G()}}else u.delete(p.id)}},H.innerHTML=`
                <div class="event-time">
            ...
            `;let ve=H.querySelector(".close-details-btn");return ve&&ve.addEventListener("click",F=>{F.stopPropagation(),H.classList.remove("expanded");let q=H.querySelector(".event-details");q&&(q.style.display="none"),u.delete(p.id)}),H},u=new Set(Array.from(e.querySelectorAll(".event-item.expanded")).map(p=>p.dataset.notificationId).filter(p=>p)),b=Array.from(e.children),o=new Map(b.map(p=>[p.dataset.notificationId,p])),i=new Set(s.map(p=>p.id));b.forEach(p=>{(!p.dataset.notificationId||!i.has(p.dataset.notificationId))&&p.remove()});let c=null;s.forEach((p,g)=>{let h=o.get(p.id);!h&&(h=d(p),!h)||(g===0?e.firstElementChild!==h&&e.prepend(h):c&&c.nextElementSibling!==h&&c.after(h),c=h)}),Y=Date.now(),E(0,Y),G()}catch(a){console.error("Error fetching notifications:",a),e.children.length===0&&(e.innerHTML=$("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}var Me=()=>'<div id="blueprints-list" class="blueprints-list events-timeline" style="display: flex; flex-direction: column; gap: 15px;"><p>Loading blueprints...</p></div>',K=null;async function ce(){let e=document.getElementById("blueprints-list");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=$("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(f=>f.id==="dex-event-service")}catch{e.innerHTML=$("error","Invalid service map data.");return}if(!n){e.innerHTML=$("error","Event service not found in service map.");return}let r=`http://${n.domain==="0.0.0.0"?"127.0.0.1":n.domain}:${n.port}/events?ml=100&format=json&event.type=system.blueprint.generated`;try{let a=await fetch(r);if(!a.ok)throw new Error("Service is offline or unreachable.");let l=(await a.json()).events||[];if(l.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),j(1,0);return}let v=o=>{let i=o.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let c=i.title||"Untitled Blueprint",p=i.summary||i.body||"No summary provided.",g=i.content||"",h=i.category||"architecture",x=i.affected_services||[],w=i.implementation_path||[],k=new Date(o.timestamp*1e3),I=k.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),U=k.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),W=y.has(o.id),D=W?"display: block;":"display: none;",_=document.createElement("div");_.className=`event-item notification-item event-border-purple cursor-pointer ${W?"expanded":""}`,_.dataset.blueprintId=o.id,_.onclick=function(N){this.classList.toggle("expanded");let T=this.querySelector(".event-details");if(T){let B=T.style.display==="none";T.style.display=B?"block":"none",B?y.add(o.id):y.delete(o.id)}};let A=x.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${x.join(", ")}</div>`:"",L="";w.length>0&&(L=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${w.map(N=>`<li>${C(N)}</li>`).join("")}
                        </ul>
                    </div>
                `),_.innerHTML=`
                <div class="event-time">
            ...
            `;let S=_.querySelector(".close-details-btn");return S&&S.addEventListener("click",N=>{N.stopPropagation(),_.classList.remove("expanded");let T=_.querySelector(".event-details");T&&(T.style.display="none"),y.delete(o.id)}),_},y=new Set(Array.from(e.querySelectorAll(".event-item.expanded")).map(o=>o.dataset.blueprintId).filter(o=>o)),s=Array.from(e.children),d=new Map(s.map(o=>[o.dataset.blueprintId,o])),u=new Set(l.map(o=>o.id));s.forEach(o=>{o.dataset.blueprintId&&!u.has(o.dataset.blueprintId)&&o.remove()});let b=null;l.forEach((o,i)=>{let c=d.get(o.id);!c&&(c=v(o),!c)||(i===0?e.firstElementChild!==c&&e.prepend(c):b&&b.nextElementSibling!==c&&b.after(c),b=c)}),K=Date.now(),E(1,K),j(1,l.length)}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}var ke=()=>localStorage.getItem("service_map")?'<div id="services-widgets" class="system-monitor-widgets"><p>Loading services...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),Ie=()=>localStorage.getItem("service_map")?'<div id="models-widgets" class="system-monitor-widgets"><p>Loading models...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),Ce=()=>localStorage.getItem("service_map")?'<div id="processes-widgets" class="system-monitor-widgets"><p>Loading processes...</p></div>':$("config","No service map configured.","Upload service-map.json in Settings."),X=null,Q=null,Z=null;async function Ne(){if(!localStorage.getItem("service_map"))return null;try{let t=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(a=>a.id==="dex-event-service");if(!t)return null;let m=`http://${t.domain==="0.0.0.0"?"127.0.0.1":t.domain}:${t.port}/system_monitor`,r=await fetch(m);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);return await r.json()}catch(e){return console.error("Error fetching system data:",e),null}}async function je(){if(!localStorage.getItem("service_map"))return null;try{let t=(JSON.parse(localStorage.getItem("service_map")).services?.cs||[]).find(a=>a.id==="dex-event-service");if(!t)return null;let m=`http://${t.domain==="0.0.0.0"?"127.0.0.1":t.domain}:${t.port}/processes`,r=await fetch(m);if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);return await r.json()}catch(e){return console.error("Error fetching process data:",e),null}}async function de(){let e=document.getElementById("services-widgets");if(!e)return;let t=await Ne();if(!t||!t.services){e.innerHTML=$("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}X=Date.now(),E(5,X);let n=t.services||[];Array.from(e.children).forEach(s=>{s.classList.contains("service-widget")||s.remove()});function m(s){return!s||s==="N/A"||s==="unknown"||s.trim()===""?"-":s}function r(s){if(!s||s==="N/A"||s==="unknown")return"-";let d=s.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:s.split(".").slice(0,3).join(".")||"-"}function a(s){return!s||s.length<=28?s:s.substring(0,28)+"..."}function f(s){if(!s||s==="N/A"||s==="unknown")return"-";let d=s.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let u=parseInt(d[1])||0,b=parseInt(d[2])||0,o=parseInt(d[3])||0,i=parseFloat(d[4])||0,c=u*86400+b*3600+o*60+i,p=Math.floor(c/86400);if(p>0)return`${p}d`;let g=Math.floor(c/3600);if(g>0)return`${g}h`;let h=Math.floor(c/60);return h>0?`${h}m`:`${Math.floor(c)}s`}function l(s){let d=s.status==="online",u=d?"service-widget-online":"service-widget-offline",b=d?"bx-check-circle":"bx-x-circle",o=d?"OK":"BAD",i=s.version?r(s.version.str):"-",c=s.cpu&&s.cpu!=="N/A"?s.cpu:"-",p=s.memory&&s.memory!=="N/A"?s.memory:"-",g=c!=="-"?"#00ff00":"#666",h=c!=="-"?"#fff":"#666",x=p!=="-"?"#00ff00":"#666",w=p!=="-"?"#fff":"#666",k=f(s.uptime),I="";return d?I=`
                ...
                </div>`:I='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${u}" data-service-id="${s.id}"><div class="service-widget-header"><i class="bx ${b}"></i><h3>${s.short_name||s.id}</h3><span class="service-widget-status">${o}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${a(s.domain&&s.port?`${s.domain}:${s.port}`:"")}</span></div>${I}</div></div>`}let v=new Map(Array.from(e.querySelectorAll(".service-widget")).map(s=>[s.dataset.serviceId,s])),y=new Set(n.map(s=>s.id));for(let[s,d]of v)y.has(s)||d.remove();n.forEach(s=>{let d=l(s),u=v.get(s.id);u?u.outerHTML!==d&&(u.outerHTML=d):e.insertAdjacentHTML("beforeend",d)})}async function pe(){let e=document.getElementById("models-widgets");if(!e)return;let t=await Ne();if(!t){e.innerHTML=$("offline","Failed to load model status.");return}Q=Date.now(),E(4,Q);let n=t.models||[],m=t.whisper;Array.from(e.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function r(l){let v=l.status==="Ready";return`

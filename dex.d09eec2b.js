...
`,le=new Set,ft=[],ve=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){is();try{let i=await _("/roadmap");if(!i.ok)throw new Error("Offline");let a=await i.json();ft=a;let n=p=>{let x=le.has(p.id),g=p.state==="published",b=p.state==="consumed",s="event-border-grey";g&&(s="event-border-blue"),b&&(s="event-border-purple");let E=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${s} cursor-pointer ${x?"expanded":""}`,y.dataset.itemId=p.id,y.onclick=r=>{if(r.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",le.delete(p.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",le.add(p.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;y.innerHTML=`
        <div class="event-time">
      ...
      `;let v=y.querySelector(".event-details");return v&&v.addEventListener("click",r=>{r.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>ns(p)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>as(p)),y.querySelector(".delete-btn")?.addEventListener("click",()=>os(p.id)),y.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",le.delete(p.id)}),y},o=Array.from(e.children),l=new Map(o.map(p=>[p.dataset.itemId,p])),m=new Set(a.map(p=>p.id));if(o.forEach(p=>{let x=p.dataset.itemId;(!x||!m.has(x))&&p.remove()}),a.length===0){e.innerHTML=N("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let u=null;a.forEach((p,x)=>{let g=l.get(p.id);(!g||t)&&(g&&g.remove(),g=n(p),!g)||(x===0?e.firstElementChild!==g&&e.prepend(g):u&&u.nextElementSibling!==g&&u.after(g),u=g)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function is(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ve=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ve=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-input").value;if(!o.trim())return;let l=ve?`/roadmap/${ve}`:"/roadmap",m=ve?"PATCH":"POST";try{await _(l,{method:m,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(u){console.error(u)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{ft.forEach(o=>le.add(o.id)),ce(!0)},a.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function ns(t){ve=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function as(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function os(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var bt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,gt=null,de=new Set,xt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;rs();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await _(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(xt=o,gt=Date.now(),G(2,gt),o.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let l=g=>{let b=g.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let s=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),E=(b.content||"").trim(),y=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(k=>k.trim()),v=(b.implementation_path||[]).map(k=>k.trim()),r=b.source_event_ids||[],d=b.approved===!0,h=new Date(g.timestamp*1e3),M=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=de.has(g.id),I=L?"display: block;":"display: none;",w=document.createElement("div");w.className=`event-item notification-item event-border-purple cursor-pointer ${L?"expanded":""} ${d?"blueprint-approved":""}`,w.dataset.blueprintId=g.id,d&&(w.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",w.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let D=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";w.onclick=function(k){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(g.id);let P=this.querySelector(".event-details");P&&(P.style.display="none")}else{this.classList.add("expanded"),de.add(g.id);let P=this.querySelector(".event-details");P&&(P.style.display="block")}};let z="";v.length>0&&(z=`
                ...
                `);let W=c.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${c.join(", ")}</span></div>`:"<div></div>",U=d?`
...
`,_e=null,me=new Set,we=[],F="all",ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ms={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function us(t){for(let[e,i]of Object.entries(ps))if(i.includes(t))return e;return"system"}function fs(t){return ms[t]||"bx-square-rounded"}async function ae(t=!1){let e=document.getElementById("events-timeline");if(!e)return;gs();let a=`/events?ml=${F==="all"?100:250}&format=json`;F!=="all"&&(a+=`&category=${F}`);try{let n=await _(a);if(!n.ok)throw new Error("Service unreachable");if(we=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let f=s.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),_e=Date.now(),G(1,_e),we.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let m=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let f=s.type,E=us(f),y=fs(f),c=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="analysis.user.message_signals"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),v="event-border-grey";c&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?v="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.analysis.audit"||f==="analysis.user.message_signals"?v="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?v="event-border-orange":f==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=me.has(b.id),h=d?"expanded":"",M=d?"display: block;":"display: none;",A=new Date(b.timestamp*1e3),L=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Et(f,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",D="";if(c){let S="";if(f==="engagement.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$="";s.eval_count&&($=`
                        ...
                        `);let T="";if(s.chat_history&&s.chat_history.length>0){let k=s.chat_history.length,R=s.chat_history.map((P,ee)=>{let j=P.role.toUpperCase();j==="ASSISTANT"&&(j="DEXTER");let q=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",V=ee===k-1?"block":"none";return`
                                <div class="history-slide" data-index="${ee}" style="display: ${V};">
                    ...
                    `}else if(f==="system.analysis.audit"){let C=s.success?"#03dac6":"#ff4d4d",$=s.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,k="";s.error&&(k=`
                        ...
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let j=s.chat_history.length,q=s.chat_history.map((V,ue)=>{let K=V.role.toUpperCase();K==="USER"&&(K="SYSTEM"),K==="ASSISTANT"&&(K="AGENT");let J=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ue}" style="display: ${ue===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${J}; letter-spacing: 1px; font-weight: bold;">${K}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ue+1} of ${j}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${B(V.content)}</div>
                                </div>
                            `}).join("");R=`
                            <div class="event-detail-block">
                        ...
                        `}let P="";if(s.corrections&&s.corrections.length>0){let j=s.corrections.map((q,V)=>`
...
`),u=[...l.logs];if(u.length<25){let x=25-u.length;for(let g=0;g<x;g++)u.push("")}else u.length>25&&(u=u.slice(-25));let p=u.map(x=>Le(x)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${St()}
                ...
                </div>`}function l(u){let p=u.status==="Downloaded",x=p?"service-widget-online":"service-widget-offline",g=p?"OK":"MISSING",b=p&&u.size>0?`${(u.size/1e9).toFixed(2)} GB`:"-",s=u.name;return u.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${x}" data-model-name="${u.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${g}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${u.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let m="";if(a&&(m+=n(a)),e.xtts&&(m+=o(e.xtts)),m+=i.map(l).join(""),!m){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function $e(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),m=document.getElementById("guardian-reset-btn"),u=document.getElementById("analyzer-reset-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{m.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{m.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{m.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{m.innerHTML="<i class='bx bx-error'></i> Failed"}},m.dataset.listenerAttached="true"),u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{u.innerHTML="<i class='bx bx-error'></i> Failed"}},u.dataset.listenerAttached="true");let p=await ys();if(p){let y=Math.floor(Date.now()/1e3),c=p.active_tier,v=p.active_synthesis,r=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},d=w=>{w<0&&(w=0);let H=Math.floor(w/3600),D=Math.floor(w%3600/60),z=w%60;return H>0?`${H}h ${D}m`:D>0?`${D}m ${z}s`:`${z}s`},h=(w,H,D,z,O)=>{if(!w)return;let W=r[z]||z.toUpperCase(),U=w.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(U&&(U.textContent=W),O&&O.includes(z))w.textContent="Working",w.style.color="#bb86fc";else if(D){let C=D.next_run-y;if(C<=0)w.textContent="Ready",w.style.color="#5eff5e";else{let $=Math.floor(C/60),T=C%60;w.textContent=`${$}m ${T}s`,w.style.color="#fff"}}H&&D&&(H.innerHTML=`
        ...
        `)};e&&h(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",c),i&&h(i,document.getElementById("guardian-architect-stats"),p.architect,"architect",c);let M=document.getElementById("analyzer-synthesis-val"),A=document.getElementById("analyzer-synthesis-stats");M&&h(M,A,p.synthesis,"synthesis",v);let L=document.getElementById("system-state-label"),I=document.getElementById("system-state-val");if(I&&p.system_state){let w=p.system_state,H=d(p.system_state_time||0);L&&(L.textContent=`State: ${w.toUpperCase()}`),I.textContent=H,w==="idle"?I.style.color=p.system_state_time>300?"#5eff5e":"#fff":I.style.color="#bb86fc"}n&&(n.textContent=d(p.total_idle_time||0)),o&&(o.textContent=d(p.total_active_time||0)),l&&(l.textContent=d(p.total_waste_time||0))}else[e,i,a,n,o,l].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let x=await xs(),g=[],b=[],s=[];x&&(Array.isArray(x)?g=x:(g=x.active||[],b=x.queue||[],s=x.history||[],s.sort((y,c)=>(c.end_time||0)-(y.end_time||0)))),g.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Ue(t,g,!1));let f=document.getElementById("processes-queue-widgets");f&&(b.length===0?!f.querySelector(".tab-placeholder")&&!f.querySelector('div[style*="font-style: italic"]')&&(f.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(f.innerHTML="",Ue(f,b,!1)));let E=document.getElementById("processes-history-widgets");E&&(s.length===0?E.querySelector(".tab-placeholder")||(E.innerHTML=N("empty","No recent history.")):(E.querySelector(".tab-placeholder")&&(E.innerHTML=""),Ue(E,s,!0))),he(1,g.length)}function Ue(t,e,i){function a(m){let u="";m.end_time?u=`${m.end_time-m.start_time}s`:u=`${Math.floor(Date.now()/1e3-m.start_time)}s`;let p=m.retries>0?`<span class="process-retry-badge">Retry ${m.retries}</span>`:"",x=m.channel_id,g={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(g[x]?x=g[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),i)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],ws=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Je=!0,i}function Rt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Je=!1}function Pt(t,e,i="output"){if(!Je)return;let a=document.createElement("div");a.className=`terminal-line terminal-${i}`,i==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=Le(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function $s(t){let e=Ot.find(n=>n.id===t);if(!e)return;let i=Cs(e);Pt(i,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let n of a){await new Promise(l=>setTimeout(l,400+Math.random()*600));let o="output";n.includes("[ERROR]")?o="error":n.includes("[SUCCESS]")||n.includes("\u2713")?o="success":n.includes("!")&&(o="warning"),Pt(i,n,o)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Ut(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=ws();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let a=i.dataset.cmd;$s(a)})})}async function Es(){try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function jt(){Es(),at(),Ze();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Ut();let e=et();Ke(e),Xe();let i=document.getElementById("nav-left-container");i&&i.addEventListener("click",()=>{if(o.length>0)g();else{let r=window.location.pathname;if(!(r==="/"||r==="/index.html")){let M=(r.endsWith("/")&&r.length>1?r.slice(0,-1):r).split("/");M.pop();let A=M.join("/")||"/";window.location.href=A}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",n=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!n&&document.querySelector("footer")?.classList.add("hide");let o=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let m=r=>{localStorage.setItem("dex_last_window",r)};function u(){return 1}function p(){for(;o.length>1;)o.shift().close(!0);let r=document.getElementById("windows-container"),d=document.querySelector("nav"),h=document.querySelector("footer"),M=window.location.pathname==="/"||window.location.pathname==="/index.html",A=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",o.length),o.forEach(H=>{let D=document.getElementById(H.id);D&&D.classList.remove("hide-close")}),Ee(ye());let L=document.getElementById("dexter-menu-container"),I=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon");if(Ye(o.length>0),o.length>0){if(h?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),L&&(L.style.display="none"),w&&(w.style.display="none"),I){let H=o[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(H)?(I.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{m("alerts-window"),x(b)}),document.getElementById("switch-events").addEventListener("click",()=>{m("events-window"),x(s)}),document.getElementById("switch-monitor").addEventListener("click",()=>{m("monitor-window"),x(E)}),document.getElementById("switch-contacts").addEventListener("click",()=>{m("contacts-window"),x(f)}),document.getElementById("switch-workspace").addEventListener("click",()=>{m("workspace-window"),x(y)})):I.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),r&&(r.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),M||A?h?.classList.remove("hide"):h?.classList.add("hide"),L&&(L.style.display="flex"),w&&(w.style.display="block"),I&&(I.innerHTML="");be()}function x(r){if(r.isOpen()){r.close();return}for(;o.length>0;)o.pop().close(!0);o.push(r),r.open(),p()}function g(){[...o].forEach(d=>d.close()),o=[]}window.addEventListener("resize",p);let b=se({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:pt(),onOpen:()=>ne(),onClose:()=>{let r=o.indexOf(b);r>-1&&o.splice(r,1),p()}}),s=se({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Oe(),onOpen:()=>{s.setContent(Oe()),ae()},onClose:()=>{let r=o.indexOf(s);r>-1&&o.splice(r,1),p()}}),f=se({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:$t(),onOpen:()=>Pe(),onClose:()=>{let r=o.indexOf(f);r>-1&&o.splice(r,1),p()}}),E=se({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:_t()},{icon:"bxs-component",title:"Processes",content:It()},{icon:"bxs-brain",title:"Models",content:Bt()},{icon:"bxs-hdd",title:"Hardware",content:Dt()},{icon:"bxs-terminal",title:"Logs",content:zt()},{icon:"bxs-zap",title:"Agents",content:At()}],onOpen:()=>{je(),$e(),qe(),Ce()},onClose:()=>{let r=o.indexOf(E);r>-1&&o.splice(r,1),p()}}),y=se({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ht(),onOpen:()=>Ne(),onClose:()=>{let r=o.indexOf(y);r>-1&&o.splice(r,1),p()}}),c=se({id:"settings-window",title:"Settings",icon:"bx-cog",content:Be(),onOpen:()=>{c.setContent(Be()),Fe(c)},onClose:()=>{let r=o.indexOf(c);r>-1&&o.splice(r,1),p()}}),v=se({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:r=>{s.isOpen()||x(s),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${r}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:r=>{b.isOpen()||x(b),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${r}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}},e){let r=document.getElementById("dexter-dropdown");r&&(r.innerHTML=`

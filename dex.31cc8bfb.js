(()=>{function He(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ne(t=!1){let e=window.location.pathname,o=e==="/"||e==="/index.html",i=`
    ...
    `}
        ...
        `}
    `,u=document.createElement("nav");u.innerHTML=i,document.body.prepend(u)}function Re(t){let e=window.location.pathname,o=e==="/"||e==="/index.html",a=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!a||!s||(t||!o?(a.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{a.style.transform="translateX(-3px)"},s.onmouseleave=()=>{a.style.transform="translateX(0)"}):(a.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function Oe(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Pe=1e3;function F(t){let e=null,o=t.onClose||null,a=t.onOpen||null;function s(){e&&(e.style.zIndex=++Pe)}function i(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",u),a&&setTimeout(a,10);return}let f=document.getElementById("windows-container");f||(f=document.createElement("div"),f.id="windows-container",f.className="windows-container",document.body.appendChild(f)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Pe,e.addEventListener("mousedown",s);let h=t.icon||"bx-window",v="",l="",g;t.tabs&&t.tabs.length>0?(v=`<div class="tab-bar">${t.tabs.map((x,n)=>{let c=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,g=`<div class="window-content">${t.tabs.map((x,n)=>`<div class="tab-content ${n===0?"active":""}" data-tab-content="${n}">${x.content}</div>`).join("")}</div>`):(t.title&&(l=`<div class="window-title">${t.title}</div>`),g=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,ue=null,G=new Set,ge=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Bt(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await A(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];ue=Date.now(),O(0,ue);let u=Date.now(),m=24*60*60*1e3,d=i.filter(n=>{let c=localStorage.getItem(`alert_read_ts_${n.id}`);if(!c)return!0;let y=parseInt(c);return u-y<m});d.sort((n,c)=>{let y=T=>{let I=T.event;if(typeof I=="string")try{I=JSON.parse(I)}catch{return"low"}return(I.priority||"low").toLowerCase()},$=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,w=$(y(n)),E=$(y(c));return w!==E?E-w:c.timestamp-n.timestamp}),ge=d;let p=n=>{let c=n.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return"low"}return(c.priority||"low").toLowerCase()},b=[],h=new Set(d.map(n=>p(n))).size>1;if(d.length>0){let n=null;d.forEach(c=>{let y=p(c);h&&y!==n&&(b.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),n=y),b.push(c)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=k("empty","No alerts yet."),me();return}let v=n=>{let c=n.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return null}let y=c.title||"Untitled Alert",$=c.body||"No description provided.",w=c.priority||"low",E=!!c.alert,T=c.category||"system",I=c.related_event_ids||[],_=c.audit_event_id,N=!!localStorage.getItem(`alert_read_ts_${n.id}`),B=new Date(n.timestamp*1e3),R=B.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=B.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=N?"event-border-grey":"event-border-blue";!N&&E&&(L="event-border-red"),N&&(w==="high"||w==="critical")?L="event-border-red":N&&w==="medium"&&(L="event-border-orange");let H=N?"alert-read":"alert-unread",D=G.has(n.id),ye=D?"expanded":"",he=D?"display: block;":"display: none;",Ae="",_e="";I.length>0&&(_e=`
...
`,V=new Set,Ze=[],Z=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Dt();try{let o=await A("/roadmap");if(!o.ok)throw new Error("Offline");let a=await o.json();if(Ze=a,a.length===0){e.innerHTML=k("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=p=>{let b=V.has(p.id),f=p.state==="draft",h=p.state==="published",v=p.state==="consumed",l="event-border-grey";h&&(l="event-border-blue"),v&&(l="event-border-purple");let S=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),r=document.createElement("div");r.className=`event-item notification-item ${l} cursor-pointer ${b?"expanded":""}`,r.dataset.itemId=p.id,r.onclick=c=>{if(c.target.closest("button"))return;r.classList.contains("expanded")?(r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",V.delete(p.id)):(r.classList.add("expanded"),r.querySelector(".event-details").style.display="block",V.add(p.id))};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;r.innerHTML=`
        <div class="event-time">
      ...
      `;let n=r.querySelector(".event-details");return n&&n.addEventListener("click",c=>{c.stopPropagation()}),r.querySelector(".edit-btn")?.addEventListener("click",()=>Ht(p)),r.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Nt(p)),r.querySelector(".delete-btn")?.addEventListener("click",()=>Rt(p.id)),r.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),r.classList.remove("expanded"),r.querySelector(".event-details").style.display="none",V.delete(p.id)}),r},i=Array.from(e.children),u=new Map(i.map(p=>[p.dataset.itemId,p])),m=new Set(a.map(p=>p.id));i.forEach(p=>{let b=p.dataset.itemId;(!b||!m.has(b))&&p.remove()});let d=null;a.forEach((p,b)=>{let f=u.get(p.id);(!f||t)&&(f&&f.remove(),f=s(p),!f)||(b===0?e.firstElementChild!==f&&e.prepend(f):d&&d.nextElementSibling!==f&&d.after(f),d=f)})}catch{e.children.length===0&&(e.innerHTML=k("offline","Failed to load roadmap.","The event service may be offline."))}}}function Dt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),o=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Z=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",Z=null},o.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let i=document.getElementById("roadmap-editor-input").value;if(!i.trim())return;let u=Z?`/roadmap/${Z}`:"/roadmap",m=Z?"PATCH":"POST";try{await A(u,{method:m,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:i})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(d){console.error(d)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{Ze.forEach(i=>V.add(i.id)),K(!0)},a.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function Ht(t){Z=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Nt(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(o){console.error(o)}}async function Rt(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var tt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,et=null,ee=new Set,nt=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ot();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await A(o);if(!a.ok)throw new Error("Service is offline or unreachable.");let i=(await a.json()).events||[];if(nt=i,et=Date.now(),O(2,et),i.length===0){e.innerHTML=k("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ne(2,0);return}t&&(e.innerHTML="");let u=f=>{let h=f.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let v=h.title||"Untitled Blueprint",l=h.summary||h.body||"No summary provided.",g=h.content||"",S=h.category||"architecture",r=h.affected_services||[],x=h.implementation_path||[],n=new Date(f.timestamp*1e3),c=n.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=n.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ee.has(f.id),w=$?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,E.dataset.blueprintId=f.id;let I={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[S]||"bx-paint";E.onclick=function(R){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ee.delete(f.id);let L=this.querySelector(".event-details");L&&(L.style.display="none")}else{this.classList.add("expanded"),ee.add(f.id);let L=this.querySelector(".event-details");L&&(L.style.display="block")}};let _=r.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${r.join(", ")}</span></div>`:"",P="";x.length>0&&(P=`
                    <div class="blueprint-path">
                        <h5>Implementation Path</h5>
                        <ul>
                            ${x.map(R=>`<li>${M(R)}</li>`).join("")}
                        </ul>
                    </div>
                `),E.innerHTML=`
                <div class="event-time">
...
`,Pt=null;async function Ce(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>Ce(),e.dataset.listenerAttached="true");try{let o=await Ke("/contacts");if(!o.ok)throw new Error("Service unreachable");let s=(await o.json()).members||[];if(Pt=Date.now(),s.length===0){t.innerHTML=k("empty","No contacts found.","Check your Discord connection.");return}let i={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((u,m)=>{let d=i[u.level]??10,p=i[m.level]??10;return d!==p?d-p:u.username.localeCompare(m.username)}),t.innerHTML=s.map(u=>{let m=u.color?"#"+u.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=u.status==="online"?"#5eff5e":u.status==="idle"?"#ffa500":u.status==="dnd"?"#ff4d4d":"#666",p="#888";u.level==="Me"||u.level==="Master"?p="#bb86fc":u.level==="Admin"?p="#cf6679":u.level==="Moderator"?p="#03dac6":u.level==="Contributor"&&(p="#ffa500");let b=u.level==="Me",f=b?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",h=b?"2px solid #bb86fc":`1px solid ${m}33`;return`
                <div class="user-profile-section" style="background: ${f}; padding: 15px; border-radius: 8px; border: ${h}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${b?"#bb86fc":m}22, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${u.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${b?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${p}; font-weight: 600; text-transform: uppercase;">${b?"DEXTER (ME)":u.level}</span>
                        </div>
...
`,ve=null,J=new Set,oe=[],j="all",zt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},qt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function lt(t){for(let[e,o]of Object.entries(zt))if(o.includes(t))return e;return"system"}function Ft(t){return qt[t]||"bx-square-rounded"}async function Y(t=!1){let e=document.getElementById("events-timeline");if(!e)return;jt();let a=`/events?ml=${j==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await A(a);if(!s.ok)throw new Error("Service is offline or unreachable.");let u=(await s.json()).events||[],m=u;if(j!=="all"&&(m=u.filter(v=>{let l=v.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return!1}return lt(l.type)===j})),oe=m.slice(0,50),ve=Date.now(),O(1,ve),oe.length===0){e.innerHTML=k("empty","No events found for this filter.");return}t&&(e.innerHTML="");let d=v=>{let l=v.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return null}let g=l.type,S=lt(g),r=Ft(g),x=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),n="event-border-grey";x&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?n="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="system.analysis.audit"?n="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?n="event-border-orange":g==="system.test.completed"?n=l.test?.status==="OK"&&l.lint?.status==="OK"&&l.format?.status==="OK"?"event-border-blue":"event-border-red":n="event-border-blue");let c=x?"cursor-pointer":"",y=J.has(v.id),$=y?"expanded":"",w=y?"display: block;":"display: none;",E=new Date(v.timestamp*1e3),T=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),_=ot(g,l),P=l.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${l.user_level})</span>`:"",N="";if(x){let C="";if(g==="engagement.decision")C=`
                    ...
                    `;else if(g==="system.attention.expired"){let L=l.timestamp-l.last_active,H=Math.floor(L/60);C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="messaging.bot.sent_message")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(g==="moderation.explicit_content.deleted")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(g==="analysis.link.completed")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(g==="analysis.visual.completed"){let L="";l.base64_preview?L=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${l.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:l.url&&(L=`<div class="event-detail-block"><img src="${l.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(g==="system.analysis.audit"){let L=l.success?"#03dac6":"#ff4d4d",H=l.success?"SUCCESS":"FAILED",D="";l.error&&(D=`
                            <div class="event-detail-block">
                    ...
                    `}else if(g==="system.test.completed")C=`
                        <div class="event-detail-row">
                    ...
                    `;else if(g==="messaging.user.sent_message"){let L="";l.attachments&&l.attachments.length>0&&(L=`
                            <div class="event-detail-block">
                ...
                `}let B=document.createElement("div");B.className=`event-item ${n} ${c} ${$}`,B.dataset.eventId=v.id,B.onclick=function(C){if(!x)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(v.id);let H=this.querySelector(".event-details");H&&(H.style.display="none")}else{this.classList.add("expanded"),J.add(v.id);let H=this.querySelector(".event-details");H&&(H.style.display="block")}},B.innerHTML=`
                <div class="event-time">
...
`),d=[...u.logs];if(d.length<25){let b=25-d.length;for(let f=0;f<b;f++)d.push("")}else d.length>25&&(d=d.slice(-25));let p=d.map(b=>pe(b)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=i,document.querySelectorAll(".copy-logs-btn").forEach(u=>{u.addEventListener("click",()=>{let m=unescape(u.dataset.logs);navigator.clipboard.writeText(m).then(()=>{let d=u.querySelector("i");d.classList.remove("bx-copy"),d.classList.add("bx-check"),setTimeout(()=>{d.classList.remove("bx-check"),d.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(u=>{u.addEventListener("click",async()=>{let m=u.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${m}?`))try{(await A(`/logs?service_id=${m}`,{method:"DELETE"})).ok&&le()}catch(d){console.error("Error clearing logs:",d)}})}),Gt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=k("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var ft=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ct()}
        </div>`;async function wt(){await Promise.all([be(),Te(),Le()]),await le()}var dt=null,mt=null,pt=null;async function Et(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function ut(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Wt(){try{return await(await A("/processes")).json()}catch{return null}}async function Vt(){try{return await(await A("/agent/status")).json()}catch{return null}}async function Te(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),i=document.querySelector("#hw-gpu .hw-content"),u=document.querySelector("#hw-storage .hw-content"),m=r=>{if(r){if(o&&(o.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):i.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),u&&r.STORAGE&&r.STORAGE.length>0?u.innerHTML=r.STORAGE.map(x=>{let n=(x.USED/1073741824).toFixed(1),c=(x.SIZE/(1024*1024*1024)).toFixed(1),y=x.SIZE>0?(x.USED/x.SIZE*100).toFixed(0):0,$=x.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):u&&(u.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let r=await ut();r?(m(r),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let r=await ut();m(r)}if(!t)return;let d=await Et();if(!d||!d.services){t.children.length===0&&(t.innerHTML=k("offline","Failed to load system metrics.","The event service may be offline."));return}dt=Date.now(),O(0,dt);let p=d.services||[];Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function b(r){return!r||r==="N/A"||r==="unknown"||r.trim()===""?"-":r}function f(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/^(\d+\.\d+\.\d+)/);return x?x[0]:r.split(".").slice(0,3).join(".")||"-"}function h(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function v(r){if(!r||r==="N/A"||r==="unknown")return"-";let x=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!x)return"-";let n=parseInt(x[1])||0,c=parseInt(x[2])||0,y=parseInt(x[3])||0,$=parseFloat(x[4])||0,w=n*86400+c*3600+y*60+$,E=Math.floor(w/86400);if(E>0)return`${E}d`;let T=Math.floor(w/3600);if(T>0)return`${T}h`;let I=Math.floor(w/60);return I>0?`${I}m`:`${Math.floor(w)}s`}function l(r){let x=r.status==="online",n=x?"service-widget-online":"service-widget-offline",c=x?"bx-check-circle":"bx-x-circle",y=x?"OK":"BAD",$=r.version?f(r.version.str):"-",w=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",E=r.memory&&r.memory!=="N/A"?r.memory:"-",T=w!=="-"?"#00ff00":"#666",I=w!=="-"?"#fff":"#666",_=E!=="-"?"#00ff00":"#666",P=E!=="-"?"#fff":"#666",N=v(r.uptime),B="";return x?B=`
                ...
                </div>`:B='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${n}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${c}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${y}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${h(r.domain&&r.port?`${r.domain}:${r.port}`:"")}</span></div>${B}</div></div>`}let g=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),S=new Set(p.map(r=>r.id));for(let[r,x]of g)S.has(r)||x.remove();p.forEach(r=>{let x=l(r),n=g.get(r.id);n?n.outerHTML!==x&&(n.outerHTML=x):t.insertAdjacentHTML("beforeend",x)})}async function Le(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Et();if(!e){t.children.length===0&&(t.innerHTML=k("offline","Failed to load model status.","The event service may be offline."));return}mt=Date.now(),O(2,mt);let o=e.models||[],a=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function s(d){let p=d.status==="Ready";return`
                ...
                </div>`}function i(d){let p=d.status==="Ready";return`
                ...
                </div>`}function u(d){let p=d.status==="Downloaded",b=p?"service-widget-online":"service-widget-offline",f=p?"OK":"MISSING",h=p&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",v=d.name;return d.type==="custom"&&v.endsWith(":latest")&&(v=v.replace(":latest","")),`<div class="service-widget ${b}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${v}</h3><span class="service-widget-status">${f}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${h}</span></div></div></div>`}let m="";if(a&&(m+=s(a)),e.xtts&&(m+=i(e.xtts)),m+=o.map(u).join(""),!m){t.innerHTML=k("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-t1-val"),o=document.getElementById("guardian-t2-val"),a=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),i=document.getElementById("guardian-total-active"),u=document.getElementById("guardian-total-waste"),m=document.getElementById("guardian-reset-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{m.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/guardian/reset?tier=all",{method:"POST"}),setTimeout(()=>{m.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{m.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),be()}catch{m.innerHTML="<i class='bx bx-error'></i> Failed"}},m.dataset.listenerAttached="true");let d=await Vt();if(d){let l=Math.floor(Date.now()/1e3),g=d.active_tier,S=c=>{c<0&&(c=0);let y=Math.floor(c/3600),$=Math.floor(c%3600/60),w=c%60;return y>0?`${y}h ${$}m`:$>0?`${$}m ${w}s`:`${w}s`},r=(c,y,$,w)=>{if(g===w)c.textContent="Working",c.style.color="#bb86fc";else if(w==="t1"&&g==="tests")c.textContent="Testing",c.style.color="#03dac6";else if($){let T=$.next_run-l;if(T<=0)c.textContent="Ready",c.style.color="#5eff5e";else{let I=Math.floor(T/60),_=T%60;c.textContent=`${I}m ${_}s`,c.style.color="#fff"}}y&&$&&(y.innerHTML=`
        ...
        `)};e&&r(e,document.getElementById("guardian-t1-stats"),d.t1,"t1"),o&&r(o,document.getElementById("guardian-t2-stats"),d.t2,"t2");let x=document.getElementById("system-state-label"),n=document.getElementById("system-state-val");if(n&&d.system_state){let c=d.system_state,y=S(d.system_state_time||0);x&&(x.textContent=`State: ${c.toUpperCase()}`),n.textContent=y,c==="idle"?n.style.color=d.system_state_time>300?"#5eff5e":"#fff":n.style.color="#bb86fc"}s&&(s.textContent=S(d.total_idle_time||0)),i&&(i.textContent=S(d.total_active_time||0)),u&&(u.textContent=S(d.total_waste_time||0))}else[e,o,a,s,i,u].forEach(g=>{g&&(g.textContent="-",g.style.color="#666")});let p=await Wt(),b=[],f=[];p&&(Array.isArray(p)?b=p:(b=p.active||[],f=p.history||[]));let h=document.getElementById("vitals-processes-val");if(h)if(p){let l=b.length;h.textContent=l>0?`${l} Active`:"Idle",h.style.color=l>0?"#bb86fc":"#fff"}else h.textContent="-",h.style.color="#888";if(p===null){t.children.length===0&&(t.innerHTML=k("offline","Failed to load process status.","The event service may be offline."));return}pt=Date.now(),O(1,pt),b.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=k("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),gt(t,b,!1));let v=document.getElementById("processes-history-widgets");v&&(f.length===0?v.querySelector(".tab-placeholder")||(v.innerHTML=k("empty","No recent history.")):(v.querySelector(".tab-placeholder")&&(v.innerHTML=""),gt(v,f,!0))),ne(1,b.length)}function gt(t,e,o){function a(m){let d="";m.end_time?d=`${m.end_time-m.start_time}s`:d=`${Math.floor(Date.now()/1e3-m.start_time)}s`;let p=m.retries>0?`<span class="process-retry-badge">Retry ${m.retries}</span>`:"",b=m.channel_id,f={"system-guardian":"Guardian Worker","system-cli-op":"CLI Operation"};if(f[b]?b=f[b]:/^\d+$/.test(b)&&(b=`Channel ${b}`),o)return`
        <div class="process-history-item" data-channel-id="${m.channel_id}-${m.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
                ...
                <span style="color: #fff; font-weight: 600;">${b}</span>
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${m.channel_id}-${m.start_time}" style="">
                    <div class="service-widget-header">
                        ...
                        <h3 style="color: ${h}">${b}</h3>
                        ${p}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">State:</span>
                            <span class="info-value" style="color: ${h}">${m.state}</span>
                        </div>
                        <div class="service-widget-info">
                         ...
                         <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${h}">${m.pid}</span>
                        </div>
                    </div>
                </div>`}let s=o?".process-history-item":".process-widget",i=new Map(Array.from(t.querySelectorAll(s)).map(m=>[m.dataset.channelId,m])),u=new Set(e.map(m=>`${m.channel_id}-${m.start_time}`));for(let[m,d]of i)u.has(m)||d.remove();e.forEach(m=>{let d=`${m.channel_id}-${m.start_time}`,p=a(m),b=i.get(d);if(b){if(b.outerHTML!==p){b.outerHTML=p;let f=t.querySelector(`[data-channel-id="${d}"]`);f&&i.set(d,f)}}else{t.insertAdjacentHTML("beforeend",p);let f=t.querySelector(`[data-channel-id="${d}"]`);f&&i.set(d,f)}})}function xe(){let t=se(),e=ze()||"master@easter.company",o={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Jt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[o,a]of Object.entries(t)){let s=Ct.filter(i=>i.category===o);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ie=!0,o}function $t(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ie=!1}function St(t,e,o="output"){if(!Ie)return;let a=document.createElement("div");a.className=`terminal-line terminal-${o}`,o==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=pe(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function Xt(t){let e=Ct.find(s=>s.id===t);if(!e)return;let o=Yt(e);St(o,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let s of a){await new Promise(u=>setTimeout(u,400+Math.random()*600));let i="output";s.includes("[ERROR]")?i="error":s.includes("[SUCCESS]")||s.includes("\u2713")?i="success":s.includes("!")&&(i="warning"),St(o,s,i)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Tt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Jt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code").textContent;navigator.clipboard.writeText(o).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(o=>{o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-5px)",o.style.borderColor="rgba(255,255,255,0.15)",o.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)",o.style.borderColor="rgba(255,255,255,0.05)",o.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=o.dataset.cmd;Xt(a)})})}async function Qt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${$e()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Lt(){Qt(),Ge(),He();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Tt();let e=Ue();Ne(e),Oe();let o=document.getElementById("nav-left-container");o&&o.addEventListener("click",()=>{if(i.length>0)f();else{let n=window.location.pathname;if(!(n==="/"||n==="/index.html")){let $=(n.endsWith("/")&&n.length>1?n.slice(0,-1):n).split("/");$.pop();let w=$.join("/")||"/";window.location.href=w}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!s&&document.querySelector("footer")?.classList.add("hide");let i=[],u=document.getElementById("windows-container");u&&u.setAttribute("data-count","0");let m=n=>{localStorage.setItem("dex_last_window",n)};function d(){return 1}function p(){for(;i.length>1;)i.shift().close(!0);let n=document.getElementById("windows-container"),c=document.querySelector("nav"),y=document.querySelector("footer"),$=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");n&&n.setAttribute("data-count",i.length),i.forEach(_=>{let P=document.getElementById(_.id);P&&P.classList.remove("hide-close")}),re(se());let E=document.getElementById("dexter-menu-container"),T=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon");if(Re(i.length>0),i.length>0){if(y?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),c?.classList.add("window-open"),n&&(n.style.paddingTop="60px"),E&&(E.style.display="none"),I&&(I.style.display="none"),T){let _=i[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(T.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{m("alerts-window"),b(h)}),document.getElementById("switch-events").addEventListener("click",()=>{m("events-window"),b(v)}),document.getElementById("switch-monitor").addEventListener("click",()=>{m("monitor-window"),b(g)}),document.getElementById("switch-contacts").addEventListener("click",()=>{m("contacts-window"),b(l)}),document.getElementById("switch-workspace").addEventListener("click",()=>{m("workspace-window"),b(S)})):T.innerHTML=""}}else c?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),n&&(n.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),$||w?y?.classList.remove("hide"):y?.classList.add("hide"),E&&(E.style.display="flex"),I&&(I.style.display="block"),T&&(T.innerHTML="");ie(Ve())}function b(n){if(n.isOpen()){n.close();return}for(;i.length>0;)i.pop().close(!0);i.push(n),n.open(),p()}function f(){[...i].forEach(c=>c.close()),i=[]}window.addEventListener("resize",p);let h=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Je(),onOpen:()=>W(),onClose:()=>{let n=i.indexOf(h);n>-1&&i.splice(n,1),p()}}),v=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:rt(),onOpen:()=>Y(),onClose:()=>{let n=i.indexOf(v);n>-1&&i.splice(n,1),p()}}),l=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:at(),onOpen:()=>Ce(),onClose:()=>{let n=i.indexOf(l);n>-1&&i.splice(n,1),p()}}),g=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:bt()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:xt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Agents",content:ft()}],onOpen:()=>{Te(),be(),Le(),le()},onClose:()=>{let n=i.indexOf(g);n>-1&&i.splice(n,1),p()}}),S=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:it(),onOpen:()=>Se(),onClose:()=>{let n=i.indexOf(S);n>-1&&i.splice(n,1),p()}}),r=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:xe(),onOpen:()=>{r.setContent(xe()),ke(r)},onClose:()=>{let n=i.indexOf(r);n>-1&&i.splice(n,1),p()}}),x=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(window.dexter={viewEvent:n=>{v.isOpen()||b(v),setTimeout(()=>{let c=document.querySelector(`.event-item[data-event-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)},viewAlert:n=>{h.isOpen()||b(h),setTimeout(()=>{let c=document.querySelector(`.event-item[data-alert-id="${n}"]`);c&&(c.scrollIntoView({behavior:"smooth",block:"center"}),c.classList.add("flash-highlight"),c.classList.contains("expanded")||c.click(),setTimeout(()=>c.classList.remove("flash-highlight"),2e3))},500)}},e){let n=document.getElementById("dexter-dropdown");n&&(n.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let c=document.getElementById("dexter-menu-container"),y=document.getElementById("dexter-menu-btn");c&&n&&y&&(c.addEventListener("mouseenter",()=>{n.classList.add("active"),y.classList.add("active")}),c.addEventListener("mouseleave",()=>{n.classList.remove("active"),y.classList.remove("active")}),y.addEventListener("click",$=>{$.stopPropagation();let w=localStorage.getItem("dex_last_window")||"alerts-window",E=null;w==="alerts-window"?E=h:w==="events-window"?E=v:w==="monitor-window"?E=g:w==="contacts-window"?E=l:w==="workspace-window"&&(E=S),E&&b(E)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{m("alerts-window"),b(h)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{m("events-window"),b(v)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{m("monitor-window"),b(g)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{m("contacts-window"),b(l)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{m("workspace-window"),b(S)}),document.getElementById("settings-icon")?.addEventListener("click",()=>b(r)),document.getElementById("close-all-windows")?.addEventListener("click",()=>f()),setInterval(()=>{h.isOpen()?W():Ye(),v.isOpen()&&Y(),g.isOpen()&&wt(),S.isOpen()&&Se()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{x.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",n=>{n.preventDefault();try{qe(document.getElementById("email-input").value),window.location.reload()}catch(c){let y=document.getElementById("login-error");y&&(y.textContent=c.message,y.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Lt):Lt();})();

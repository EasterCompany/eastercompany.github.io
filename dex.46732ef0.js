        ...
        `,W=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${T}</div>
                    </div>
                    ${ue}
                    ${V}
                    ${G}
...
`,ft=null,de=new Set,vt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ps();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(vt=c,ft=Date.now(),X(2,ft),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(z=>z.trim()),l=(b.implementation_path||[]).map(z=>z.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(m.timestamp*1e3),I=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),_=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=de.has(m.id),k=S?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${f?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,f&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let T=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";E.onclick=function(z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let D="";l.length>0&&(D=`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-icon" style="${f?"color: #03dac6;":""}"><i class='bx ${T}'></i></div>
                <div class="event-content">
...
`,_e=null,me=new Set,Ce=[],K="all",gs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},bs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function vs(t){for(let[e,i]of Object.entries(gs))if(i.includes(t))return e;return"system"}function xs(t){return bs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;ys();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,$=vs(g),h=xs(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),l="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?l="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?l="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?l="event-border-orange":g==="system.test.completed"?l=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let d=u?"cursor-pointer":"",f=me.has(b.id),x=f?"expanded":"",I=f?"display: block;":"display: none;",_=new Date(b.timestamp*1e3),S=_.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=_.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Et(g,a),A=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",T="";if(u){let L="";if(g==="engagement.decision"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;L=`
                    ...
                    `}else if(g==="system.attention.expired"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w=a.timestamp-a.last_active,M=Math.floor(w/60);L=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w="";a.eval_count&&(w=`
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let P=a.chat_history.length,z=a.chat_history.map((j,J)=>{let W=j.name?j.name.toUpperCase():j.role.toUpperCase();!j.name&&W==="ASSISTANT"&&(W="DEXTER");let G=j.role==="user"?"#03dac6":j.role==="system"?"#ffb74d":"#bb86fc",V=J===P-1?"block":"none";return`
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let C=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,w=a.signals||{};L=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">

(()=>{function _e(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Me(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",o=t?`
        <div class="nav-right">
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Be=1e3;function F(t){let e=null,a=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++Be)}function r(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",c),o&&setTimeout(o,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Be,e.addEventListener("mousedown",s);let p=t.icon||"bx-window",n="",i="",l;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((m,d)=>{let v=m.icon?`<i class="bx ${m.icon}"></i>`:`<span class="tab-glyph">${m.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
            ...
            ${l}
        `,u.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",y=>{y.stopPropagation(),g()}),window.addEventListener("resize",c),t.tabs&&e.querySelectorAll(".tab").forEach(y=>{y.addEventListener("click",()=>{let x=y.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(m=>m.classList.remove("active")),y.classList.add("active"),e.querySelectorAll(".tab-content").forEach(m=>m.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${x}"]`).classList.add("active"),b(y,e)})}),setTimeout(()=>{e.classList.add("open"),o&&o()},10)}function c(){if(!e||!e.classList.contains("open"))return;let u=e.querySelector(".tab.active");u&&b(u,e)}function b(u,p){setTimeout(()=>{let n=p.querySelector(".tab-bar");if(!n)return;let i=Array.from(n.querySelectorAll(".tab")),l=i.indexOf(u),y=n.clientWidth,x=i[Math.max(0,l-2)],m=i[Math.min(i.length-1,l+2)],d=x.offsetLeft-n.offsetLeft-25,h=m.offsetLeft+m.offsetWidth-n.offsetLeft+25-d,C=h<=y?d-(y-h)/2:u.offsetLeft-n.offsetLeft-y/2+u.offsetWidth/2;n.scrollTo({left:C,behavior:"smooth"})},350)}function g(u=!1){e&&(window.removeEventListener("resize",c),u?(e.remove(),e=null):(e.classList.remove("open"),a&&a(),setTimeout(()=>{e?.remove(),e=null},400)))}function f(u){let p=e?.querySelector(".window-content");p&&(p.innerHTML=u)}function w(){return e&&e.classList.contains("open")}return{open:r,close:g,setContent:f,isOpen:w,focus:s,id:t.id}}function He(){return!0}function Ne(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function Re(t){localStorage.setItem("easter_user_email",t.trim())}var Pe="easter_theme",z={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function se(){return localStorage.getItem(Pe)||z.DARK}function Oe(t){if(!Object.values(z).includes(t))throw new Error("Invalid theme");localStorage.setItem(Pe,t),oe(t)}function oe(t){let e=document.body;if(Object.values(z).forEach(o=>{e.classList.remove(`theme-${o}`)}),e.classList.add(`theme-${t}`),[z.LIGHT,z.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let o=document.createElement("div");o.className="background",document.body.prepend(o)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function Ue(){let t=se();oe(t)}function $(t,e,a=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=a?`<p class="placeholder-action">${a}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function A(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function B(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!a)return;if(!e){a.textContent="Last updated: never";return}let o=Date.now(),s=Math.floor((o-e)/1e3),r;s<60?r=`${s}s ago`:s<3600?r=`${Math.floor(s/60)}m ago`:r=`${Math.floor(s/3600)}h ago`,a.textContent=`Last updated: ${r}`}function q(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!a)return;let o=a.querySelector(".notification-badge");o&&(e>0?(o.textContent=e>9?"9+":e,o.style.display="flex"):o.style.display="none")}function ce(){let t=document.getElementById("alerts-list");if(!t)return;let e=t.querySelectorAll(".alert-unread").length;q(0,e)}function Ee(){return"https://event.easter.company"}function Et(){return"https://discord.easter.company"}var St="http://127.0.0.1:8100",$t="http://127.0.0.1:8300",Lt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function de(t){let e=A(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,r)=>{let c=Lt[r];return c?`<span class="${c}">`:""});let a=(e.match(/<span/g)||[]).length,o=(e.match(/<\/span/g)||[]).length;return a>o&&(e+="</span>".repeat(a-o)),e}function ze(t){if(!t)return"";let e=A(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var J=null,Y=null,le=!1,re=!1;async function k(t,e={}){if(J)try{let s=await fetch(J+t,e);if(s.ok)return s;J=null}catch{J=null}let a=Ee(),o=St;try{let s=await fetch(a+t,e);if(s.ok)return J=a,le&&(console.log("\u2728 Production event service restored."),le=!1),s;throw new Error("Primary failed")}catch{le||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${o}`),le=!0);try{let r=await fetch(o+t,e);if(r.ok)return J=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}async function qe(t,e={}){if(Y)try{let s=await fetch(Y+t,e);if(s.ok)return s;Y=null}catch{Y=null}let a=Et(),o=$t;try{let s=await fetch(a+t,e);if(s.ok)return Y=a,re&&(console.log("\u2728 Production discord service restored."),re=!1),s;throw new Error("Primary failed")}catch{re||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${o}`),re=!0);try{let r=await fetch(o+t,e);if(r.ok)return Y=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}var Fe=()=>`
...
`,me=null,G=new Set,pe=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Ct(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await k(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];me=Date.now(),B(0,me);let c=Date.now(),b=24*60*60*1e3,g=r.filter(d=>{let v=localStorage.getItem(`alert_read_ts_${d.id}`);if(!v)return!0;let h=parseInt(v);return c-h<b});g.sort((d,v)=>{let h=I=>{let T=I.event;if(typeof T=="string")try{T=JSON.parse(T)}catch{return"low"}return(T.priority||"low").toLowerCase()},C=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,S=C(h(d)),L=C(h(v));return S!==L?L-S:v.timestamp-d.timestamp}),pe=g;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},w=[],p=new Set(g.map(d=>f(d))).size>1;if(g.length>0){let d=null;g.forEach(v=>{let h=f(v);p&&h!==d&&(w.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),d=h),w.push(v)})}if(t&&(e.innerHTML=""),g.length===0){e.innerHTML=$("empty","No alerts yet."),ce();return}let n=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let h=v.title||"Untitled Alert",C=v.body||"No description provided.",S=v.priority||"low",L=!!v.alert,I=v.category||"system",T=v.related_event_ids||[],D=!!localStorage.getItem(`alert_read_ts_${d.id}`),O=new Date(d.timestamp*1e3),R=O.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),P=O.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=D?"event-border-grey":"event-border-blue";!D&&L&&(E="event-border-red"),D&&(S==="high"||S==="critical")?E="event-border-red":D&&S==="medium"&&(E="event-border-orange");let _=D?"alert-read":"alert-unread",H=G.has(d.id),N=H?"expanded":"",ye=H?"display: block;":"display: none;",ae="",ke="";T.length>0&&(ke=`
...
`+C)}</div>
            ...
            `;let U=document.createElement("div");U.className=`event-item notification-item ${E} ${_} ${N} cursor-pointer`,U.dataset.alertId=d.id,U.onclick=function(j){if(this.classList.contains("expanded")){this.classList.remove("expanded"),G.delete(d.id);let te=this.querySelector(".event-details");te&&(te.style.display="none")}else{this.classList.add("expanded"),G.add(d.id);let te=this.querySelector(".event-details");if(te&&(te.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let we="event-border-grey";S==="high"||S==="critical"?we="event-border-red":S==="medium"&&(we="event-border-orange"),this.classList.add(we),ce()}}};let Gt=h,wt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[I]||"bx-bell";U.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${ye}">
                        ${ae}
                    </div>
                </div>
            `;let Ie=U.querySelector(".event-details");Ie&&Ie.addEventListener("click",j=>{j.stopPropagation()});let Ae=U.querySelector(".close-details-btn");return Ae&&Ae.addEventListener("click",j=>{j.stopPropagation(),U.classList.remove("expanded");let xe=U.querySelector(".event-details");xe&&(xe.style.display="none"),G.delete(d.id)}),U},i=d=>{let v=document.createElement("div");v.className="notification-divider",v.dataset.alertId=d.id;let h="#888888";return d.label==="CRITICAL"?h="#ff4d4d":d.label==="HIGH"?h="#ff8888":d.label==="MEDIUM"&&(h="#ffa500"),v.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,v.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,v},l=Array.from(e.children),y=new Map(l.map(d=>[d.dataset.alertId,d])),x=new Set(w.map(d=>d.id));l.forEach(d=>{let v=d.dataset.alertId;(!v||!x.has(v))&&d.remove()});let m=null;w.forEach((d,v)=>{let h=y.get(d.id);(!h||t)&&(h&&h.remove(),d.type==="divider"?h=i(d):h=n(d),!h)||(v===0?e.firstElementChild!==h&&e.prepend(h):m&&m.nextElementSibling!==h&&m.after(h),m=h)}),me=Date.now(),B(0,me),ce()}catch(o){console.error("Error fetching alerts:",o),e.children.length===0&&(e.innerHTML=$("offline","Failed to load alerts.","The event service may be offline."))}}function Ct(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),a=document.getElementById("alerts-close-all"),o=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{pe.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{pe.forEach(s=>{G.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{G.clear(),W(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let s=Date.now()-1728e5;pe.forEach(r=>{localStorage.setItem(`alert_read_ts_${r.id}`,s.toString())}),G.clear(),W(!0)},o.dataset.listenerAttached="true")}var je=()=>`
...
`,V=new Set,We=[],X=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Tt();try{let a=await k("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(We=o,o.length===0){e.innerHTML=$("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=f=>{let w=V.has(f.id),u=f.state==="draft",p=f.state==="published",n=f.state==="consumed",i="event-border-grey";p&&(i="event-border-blue"),n&&(i="event-border-purple");let y=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),x=document.createElement("div");x.className=`event-item notification-item ${i} cursor-pointer ${w?"expanded":""}`,x.dataset.itemId=f.id,x.onclick=v=>{if(v.target.closest("button"))return;x.classList.contains("expanded")?(x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",V.delete(f.id)):(x.classList.add("expanded"),x.querySelector(".event-details").style.display="block",V.add(f.id))};let m=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;x.innerHTML=`
        <div class="event-time">
      ...
      `;let d=x.querySelector(".event-details");return d&&d.addEventListener("click",v=>{v.stopPropagation()}),x.querySelector(".edit-btn")?.addEventListener("click",()=>kt(f)),x.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>It(f)),x.querySelector(".delete-btn")?.addEventListener("click",()=>At(f.id)),x.querySelector(".close-details-btn")?.addEventListener("click",v=>{v.stopPropagation(),x.classList.remove("expanded"),x.querySelector(".event-details").style.display="none",V.delete(f.id)}),x},r=Array.from(e.children),c=new Map(r.map(f=>[f.dataset.itemId,f])),b=new Set(o.map(f=>f.id));r.forEach(f=>{let w=f.dataset.itemId;(!w||!b.has(w))&&f.remove()});let g=null;o.forEach((f,w)=>{let u=c.get(f.id);(!u||t)&&(u&&u.remove(),u=s(f),!u)||(w===0?e.firstElementChild!==u&&e.prepend(u):g&&g.nextElementSibling!==u&&g.after(u),g=u)})}catch{e.children.length===0&&(e.innerHTML=$("offline","Failed to load roadmap.","The event service may be offline."))}}}function Tt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{X=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",X=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=X?`/roadmap/${X}`:"/roadmap",b=X?"PATCH":"POST";try{await k(c,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(g){console.error(g)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{We.forEach(r=>V.add(r.id)),K(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function kt(t){X=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function It(t){let e=t.state==="published"?"draft":"published";try{await k(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(a){console.error(a)}}async function At(t){if(confirm("Delete this roadmap item?"))try{await k(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var Ke=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ve=null,Q=new Set,Ye=[];async function ue(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;_t();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await k(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(Ye=r,Ve=Date.now(),B(2,Ve),r.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),q(1,0);return}t&&(e.innerHTML="");let c=u=>{let p=u.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let n=p.title||"Untitled Blueprint",i=p.summary||p.body||"No summary provided.",l=p.content||"",y=p.category||"architecture",x=p.affected_services||[],m=p.implementation_path||[],d=new Date(u.timestamp*1e3),v=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=Q.has(u.id),S=C?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${C?"expanded":""}`,L.dataset.blueprintId=u.id;let T={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";L.onclick=function(P){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Q.delete(u.id);let _=this.querySelector(".event-details");_&&(_.style.display="none")}else{this.classList.add("expanded"),Q.add(u.id);let _=this.querySelector(".event-details");_&&(_.style.display="block")}};let M=x.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${x.join(", ")}</span></div>`:"",D="";m.length>0&&(D=`
            ...
            `;let O=L.querySelector(".event-details");O&&O.addEventListener("click",P=>{P.stopPropagation()});let R=L.querySelector(".close-details-btn");return R&&R.addEventListener("click",P=>{P.stopPropagation(),L.classList.remove("expanded");let E=L.querySelector(".event-details");E&&(E.style.display="none"),Q.delete(u.id)}),L},b=Array.from(e.children),g=new Map(b.map(u=>[u.dataset.blueprintId,u])),f=new Set(r.map(u=>u.id));b.forEach(u=>{let p=u.dataset.blueprintId;(!p||!f.has(p))&&u.remove()});let w=null;r.forEach((u,p)=>{let n=g.get(u.id);(!n||t)&&(n&&n.remove(),n=c(u),!n)||(p===0?e.firstElementChild!==n&&e.prepend(n):w&&w.nextElementSibling!==n&&w.after(n),w=n)}),q(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline."))}}function _t(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ye.forEach(a=>Q.add(a.id)),ue(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Q.clear(),ue(!0)},e.dataset.listenerAttached="true")}var Xe=()=>`
...
`;async function Se(){await Promise.all([K(),ue()])}var Ze=()=>`
    <div class="alerts-actions">
...
`,Qe=null;async function ve(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>ve(),e.dataset.listenerAttached="true");try{let a=await qe("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(Qe=Date.now(),B(4,Qe),s.length===0){t.innerHTML=$("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((c,b)=>{let g=r[c.level]??10,f=r[b.level]??10;return g!==f?g-f:c.username.localeCompare(b.username)}),t.innerHTML=s.map(c=>{let b=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",g=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",f="#888";c.level==="Me"||c.level==="Master User"?f="#bb86fc":c.level==="Admin"?f="#cf6679":c.level==="Moderator"?f="#03dac6":c.level==="Contributor"&&(f="#ffa500");let w=c.level==="Me",u=w?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",p=w?"2px solid #bb86fc":`1px solid ${b}33`;return`
...
`,ge=null,Z=new Set,ne=[],fe="all",Dt={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},Bt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function tt(t){for(let[e,a]of Object.entries(Dt))if(a.includes(t))return e;return"system"}function Ht(t){return Bt[t]||"bx-square-rounded"}async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Nt();let o=`/events?ml=${fe==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await k(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],b=c;if(fe!=="all"&&(b=c.filter(n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!1}return tt(i.type)===fe})),ne=b.slice(0,50),ge=Date.now(),B(1,ge),ne.length===0){e.innerHTML=$("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=n=>{let i=n.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let l=i.type,y=tt(l),x=Ht(l),m=l==="engagement.decision"||l==="messaging.bot.sent_message"||l==="messaging.user.sent_message"||l==="moderation.explicit_content.deleted"||l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.cli.command"||l==="system.analysis.audit"||l==="system.test.completed"||l==="error_occurred"||l==="system.cli.status"||l.startsWith("system.roadmap")||l.startsWith("system.process"),d="event-border-grey";m&&(l==="moderation.explicit_content.deleted"||l==="error_occurred"?d="event-border-red":l==="analysis.link.completed"||l==="analysis.visual.completed"||l==="system.analysis.audit"?d="event-border-purple":l==="system.cli.command"||l==="system.cli.status"?d="event-border-orange":l==="system.test.completed"?d=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let v=m?"cursor-pointer":"",h=Z.has(n.id),C=h?"expanded":"",S=h?"display: block;":"display: none;",L=new Date(n.timestamp*1e3),I=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=et(l,i),D=i.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${i.user_level})</span>`:"",O="";if(m){let E="";if(l==="engagement.decision")E=`
                    ...
                    `;else if(l==="messaging.user.sent_message"){let _="";i.attachments&&i.attachments.length>0&&(_=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${i.attachments.map(N=>{let ye=N.content_type&&N.content_type.startsWith("image/"),ae=(N.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${ye?`<div class="attachment-thumb-container"><img src="${N.url}" alt="${N.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${N.url}" target="_blank" class="attachment-link">${N.filename}</a>
                                        <span class="attachment-meta">${N.content_type} \u2022 ${ae}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),E=`
                        <div class="event-detail-row">
            ...
            `;let P=R.querySelector(".event-details");if(P&&P.addEventListener("click",E=>{E.stopPropagation()}),m){let E=R.querySelector(".close-details-btn");E&&E.addEventListener("click",_=>{_.stopPropagation();let H=_.target.closest(".event-item");if(H){H.classList.remove("expanded"),Z.delete(n.id);let N=H.querySelector(".event-details");N&&(N.style.display="none")}})}return R},f=Array.from(e.children),w=new Map(f.map(n=>[n.dataset.eventId,n])),u=new Set(ne.map(n=>n.id));f.forEach(n=>{let i=n.dataset.eventId;(!i||!u.has(i))&&n.remove()});let p=null;ne.forEach((n,i)=>{let l=w.get(n.id);(!l||t)&&(l&&l.remove(),l=g(n),!l)||(i===0?e.firstElementChild!==l&&e.prepend(l):p&&p.nextElementSibling!==l&&p.after(l),p=l)}),ge=Date.now(),B(1,ge)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=$("offline","Failed to load events.","The event service may be offline."))}}function Nt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),a=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ne.forEach(o=>Z.add(o.id)),ee(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Z.clear(),ee(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.querySelectorAll(".filter-btn").forEach(o=>{o.onclick=()=>{a.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active"),fe=o.dataset.filter,ee(!0)}}),a.dataset.listenerAttached="true")}function nt(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Rt=null;async function ie(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await k("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML=$("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=a.filter(c=>!o.includes(c.id));s.forEach(c=>{c.logs&&Array.isArray(c.logs)?c.logs.reverse():c.logs=[]}),s.reverse();let r=s.map(c=>{let b=c.logs.join(`
`),g=[...c.logs];if(g.length<25){let w=25-g.length;for(let u=0;u<w;u++)g.push("")}else g.length>25&&(g=g.slice(-25));let f=g.map(w=>de(w)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=r,document.querySelectorAll(".copy-logs-btn").forEach(c=>{c.addEventListener("click",()=>{let b=unescape(c.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let g=c.querySelector("i");g.classList.remove("bx-copy"),g.classList.add("bx-check"),setTimeout(()=>{g.classList.remove("bx-check"),g.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(c=>{c.addEventListener("click",async()=>{let b=c.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await k(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&ie()}catch(g){console.error("Error clearing logs:",g)}})}),Rt=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=$("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var rt=()=>`
        ...
        <div id="processes-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,dt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${nt()}
        </div>`;async function vt(){await Promise.all([be(),$e(),Le()]),await ie()}var it=null,at=null,ot=null;async function gt(){try{return await(await k("/system_monitor")).json()}catch{return null}}async function lt(){try{return await(await k("/system/hardware")).json()}catch{return null}}async function Pt(){try{return await(await k("/processes")).json()}catch{return null}}async function Ot(){try{return await(await k("/analyst/status")).json()}catch{return null}}async function $e(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=$("offline","Failed to load hardware info.","The event service may be offline."));return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let y=0,x=0;n.STORAGE.forEach(h=>{y+=h.USED,x+=h.SIZE});let m=(y/(1024*1024*1024)).toFixed(1),d=(x/(1024*1024*1024)).toFixed(1),v=x>0?(y/x*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=i};if(e&&a&&(a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let n=await lt();n?(o(n),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.children.length===0&&(e.innerHTML=$("offline","Failed to refresh.","The event service may be offline.")),a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true"),e.children.length===0)){let n=await lt();o(n)}if(!t)return;let s=await gt();if(!s||!s.services){t.children.length===0&&(t.innerHTML=$("offline","Failed to load system metrics.","The event service may be offline."));return}it=Date.now(),B(0,it);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function g(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function f(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,y=parseInt(i[2])||0,x=parseInt(i[3])||0,m=parseFloat(i[4])||0,d=l*86400+y*3600+x*60+m,v=Math.floor(d/86400);if(v>0)return`${v}d`;let h=Math.floor(d/3600);if(h>0)return`${h}h`;let C=Math.floor(d/60);return C>0?`${C}m`:`${Math.floor(d)}s`}function w(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",y=i?"bx-check-circle":"bx-x-circle",x=i?"OK":"BAD",m=n.version?b(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",v=n.memory&&n.memory!=="N/A"?n.memory:"-",h=d!=="-"?"#00ff00":"#666",C=d!=="-"?"#fff":"#666",S=v!=="-"?"#00ff00":"#666",L=v!=="-"?"#fff":"#666",I=f(n.uptime),T="";return i?T=`
                ...
                </div>`:T='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${y}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${g(n.domain&&n.port?`${n.domain}:${n.port}`:"")}</span></div>${T}</div></div>`}let u=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),p=new Set(r.map(n=>n.id));for(let[n,i]of u)p.has(n)||i.remove();r.forEach(n=>{let i=w(n),l=u.get(n.id);l?l.outerHTML!==i&&(l.outerHTML=i):t.insertAdjacentHTML("beforeend",i)})}async function Le(){let t=document.getElementById("models-widgets");if(!t)return;let e=await gt();if(!e){t.children.length===0&&(t.innerHTML=$("offline","Failed to load model status.","The event service may be offline."));return}at=Date.now(),B(2,at);let a=e.models||[],o=e.whisper;Array.from(t.children).forEach(b=>{b.classList.contains("service-widget")||b.remove()});function s(b){let g=b.status==="Ready";return`
                ...
                </div>`}function r(b){let g=b.status==="Downloaded",f=g?"service-widget-online":"service-widget-offline",w=g?"OK":"MISSING",u=g&&b.size>0?`${(b.size/1e9).toFixed(2)} GB`:"-",p=b.name;return b.type==="custom"&&p.endsWith(":latest")&&(p=p.replace(":latest","")),`<div class="service-widget ${f}" data-model-name="${b.name}"><div class="service-widget-header"><i class="bx ${g?"bx-check-circle":"bx-x-circle"}"></i><h3>${p}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${b.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${u}</span></div></div></div>`}let c="";if(o&&(c+=s(o)),c+=a.map(r).join(""),!c){t.innerHTML=$("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function be(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await k("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),be()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await Ot();if(c){let p=Math.floor(Date.now()/1e3),n=c.active_tier,i=(l,y,x)=>{if(n===x||x==="guardian"&&n==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let m=y-p;if(m<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let d=Math.floor(m/60),v=m%60;l.textContent=`${d}m ${v}s`,l.style.color="#fff"}};if(e&&c.guardian&&i(e,c.guardian.next_run,"guardian"),a&&c.architect&&i(a,c.architect.next_run,"architect"),o&&c.strategist&&i(o,c.strategist.next_run,"strategist"),s&&c.system_idle_time!==void 0){let l=c.system_idle_time,y=Math.floor(l/60),x=l%60;s.textContent=`${y}m ${x}s`,l>300?s.style.color="#5eff5e":l>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let b=await Pt(),g=document.getElementById("vitals-processes-val");if(g)if(b){let p=b.length;g.textContent=p>0?`${p} Active`:"Idle",g.style.color=p>0?"#bb86fc":"#fff"}else g.textContent="-",g.style.color="#888";if(b===null){t.children.length===0&&(t.innerHTML=$("offline","Failed to load process status.","The event service may be offline."));return}if(ot=Date.now(),B(1,ot),b.length===0){t.innerHTML=$("empty","No active processes."),q(1,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function f(p){let n=Math.floor(Date.now()/1e3-p.start_time),i=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",l=p.channel_id,y={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return y[l]?l=y[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],zt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
        <div class="cli-dashboard">
            ...
            <div class="cli-divider">
                ...
                <span>Interactive Demos (v${Ut})</span>
                <i class='bx bx-chevron-down'></i>
            </div>
    `;for(let[a,o]of Object.entries(t)){let s=ht.filter(r=>r.category===a);s.length!==0&&(e+=`
    ...
    </div>`,e},Te=!1;function qt(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>ft(),document.getElementById("terminal-action-btn").onclick=()=>ft());let a=document.getElementById("cli-terminal-body"),o=document.getElementById("cli-docs-pane");a.innerHTML="";let s=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return o.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Te=!0,a}function ft(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Te=!1}function bt(t,e,a="output"){if(!Te)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=de(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Ft(t){let e=ht.find(s=>s.id===t);if(!e)return;let a=qt(e);bt(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),bt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function yt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=zt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;Ft(o)})})}async function jt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Ee()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function xt(){jt(),Ue(),_e();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&yt();let e=He();Me(e),De();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let c=m=>{localStorage.setItem("dex_last_window",m)};function b(){return 1}function g(){for(;s.length>1;)s.shift().close(!0);let m=document.getElementById("windows-container"),d=document.querySelector("nav"),v=document.querySelector("footer"),h=window.location.pathname==="/"||window.location.pathname==="/index.html",C=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");m&&m.setAttribute("data-count",s.length);let S=s.length>0;s.forEach(M=>{let D=document.getElementById(M.id);D&&D.classList.remove("hide-close")}),oe(se());let L=document.getElementById("dexter-menu-container"),I=document.getElementById("nav-window-switcher"),T=document.getElementById("settings-icon");if(s.length>0){if(v?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),m&&(m.style.paddingTop="60px"),L&&(L.style.display="none"),T&&(T.style.display="none"),I){let M=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(I.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{c("alerts-window"),f(u)}),document.getElementById("switch-events").addEventListener("click",()=>{c("events-window"),f(p)}),document.getElementById("switch-monitor").addEventListener("click",()=>{c("monitor-window"),f(i)}),document.getElementById("switch-contacts").addEventListener("click",()=>{c("contacts-window"),f(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{c("workspace-window"),f(l)})):I.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),m&&(m.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),h||C?v?.classList.remove("hide"):v?.classList.add("hide"),L&&(L.style.display="flex"),T&&(T.style.display="block"),I&&(I.innerHTML="")}function f(m){if(m.isOpen()){m.close();return}for(;s.length>0;)s.pop().close(!0);s.push(m),m.open(),g()}function w(){[...s].forEach(d=>d.close()),s=[]}window.addEventListener("resize",g);let u=F({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Fe(),onOpen:()=>W(),onClose:()=>{let m=s.indexOf(u);m>-1&&s.splice(m,1),g()}}),p=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:st(),onOpen:()=>ee(),onClose:()=>{let m=s.indexOf(p);m>-1&&s.splice(m,1),g()}}),n=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ze(),onOpen:()=>ve(),onClose:()=>{let m=s.indexOf(n);m>-1&&s.splice(m,1),g()}}),i=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:dt()},{icon:"bxs-component",title:"Processes",content:ct()},{icon:"bxs-brain",title:"Models",content:mt()},{icon:"bxs-hdd",title:"Hardware",content:pt()},{icon:"bxs-terminal",title:"Logs",content:ut()},{icon:"bxs-zap",title:"Analyst",content:rt()}],onOpen:()=>{$e(),be(),Le(),ie()},onClose:()=>{let m=s.indexOf(i);m>-1&&s.splice(m,1),g()}}),l=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Xe(),onOpen:()=>Se(),onClose:()=>{let m=s.indexOf(l);m>-1&&s.splice(m,1),g()}}),y=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:he(),onOpen:()=>{y.setContent(he()),Ce(y)},onClose:()=>{let m=s.indexOf(y);m>-1&&s.splice(m,1),g()}}),x=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let m=document.getElementById("dexter-dropdown");m&&(m.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),v=document.getElementById("dexter-menu-btn");d&&m&&v&&(d.addEventListener("mouseenter",()=>{m.classList.add("active"),v.classList.add("active")}),d.addEventListener("mouseleave",()=>{m.classList.remove("active"),v.classList.remove("active")}),v.addEventListener("click",h=>{h.stopPropagation();let C=localStorage.getItem("dex_last_window")||"alerts-window",S=null;C==="alerts-window"?S=u:C==="events-window"?S=p:C==="monitor-window"?S=i:C==="contacts-window"?S=n:C==="workspace-window"&&(S=l),S&&f(S)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{c("alerts-window"),f(u)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{c("events-window"),f(p)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{c("monitor-window"),f(i)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{c("contacts-window"),f(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{c("workspace-window"),f(l)}),document.getElementById("settings-icon")?.addEventListener("click",()=>f(y)),document.getElementById("close-all-windows")?.addEventListener("click",()=>w()),setInterval(()=>{u.isOpen()&&W(),p.isOpen()&&ee(),n.isOpen()&&ve(),i.isOpen()&&vt(),l.isOpen()&&Se()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{x.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",m=>{m.preventDefault();try{Re(document.getElementById("email-input").value),window.location.reload()}catch(d){let v=document.getElementById("login-error");v&&(v.textContent=d.message,v.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",xt):xt();})();

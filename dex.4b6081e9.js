(()=>{function Be(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function De(t=!1){let i=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}function he(t){let e=null,i=t.onClose||null,o=t.onOpen||null;function a(){if(e){e.classList.add("open"),window.addEventListener("resize",r),o&&setTimeout(o,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window";let b=t.icon||"bx-window",c="",s="",n;t.tabs&&t.tabs.length>0?(c=`<div class="tab-bar">${t.tabs.map((p,g)=>{let y;return p.icon?y=`<i class="bx ${p.icon}"></i>`:p.title&&p.title.length>0?y=`<span class="tab-glyph">${p.title.charAt(0).toUpperCase()}</span>`:y='<i class="bx bx-question-mark"></i>',`
                ...
                `}).join("")}</div>`,n=`<div class="window-content">${t.tabs.map((p,g)=>`<div class="tab-content ${g===0?"active":""}" data-tab-content="${g}">${p.content}</div>`).join("")}</div>`):(t.title&&(s=`<div class="window-title">${t.title}</div>`),n=`<div class="window-content">${t.content}</div>`);let l=`
...
`,K=null,q=new Set,le=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;ut(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await E(i);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];K=Date.now(),L(0,K);let m=Date.now(),v=24*60*60*1e3,d=r.filter(p=>{let g=localStorage.getItem(`notification_read_ts_${p.id}`);if(!g)return!0;let y=parseInt(g);return m-y<v});d.sort((p,g)=>{let y=N=>{let _=N.event;if(typeof _=="string")try{_=JSON.parse(_)}catch{return"low"}return(_.priority||"low").toLowerCase()},A=N=>N==="critical"?4:N==="high"?3:N==="medium"?2:1,k=A(y(p)),T=A(y(g));return k!==T?T-k:g.timestamp-p.timestamp}),le=d;let f=p=>{let g=p.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},u=[],c=new Set(d.map(p=>f(p))).size>1;if(d.length>0){let p=null;d.forEach(g=>{let y=f(g);c&&y!==p&&(u.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),p=y),u.push(g)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=$("empty","No notifications yet."),ae();return}let s=p=>{let g=p.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let y=g.title||"Untitled Notification",A=g.body||"No description provided.",k=g.priority||"low",T=!!g.alert,N=g.category||"system",_=g.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${p.id}`),O=new Date(p.timestamp*1e3),M=O.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=O.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=H?"event-border-grey":"event-border-blue";!H&&T&&(I="event-border-red"),H&&(k==="high"||k==="critical")?I="event-border-red":H&&k==="medium"&&(I="event-border-orange");let P=H?"notification-read":"notification-unread",B=q.has(p.id),ge=B?"expanded":"",ve=B?"display: block;":"display: none;",Ae="",Ie="";_.length>0&&(Ie=`
...
`,ce=null;async function de(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>de(),e.dataset.listenerAttached="true");try{let i=await je("/contacts");if(!i.ok)throw new Error("Service unreachable");let a=(await i.json()).members||[];if(ce=Date.now(),L(4,ce),a.length===0){t.innerHTML=$("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};a.sort((m,v)=>{let d=r[m.level]??10,f=r[v.level]??10;return d!==f?d-f:m.username.localeCompare(v.username)}),t.innerHTML=a.map(m=>{let v=m.color?"#"+m.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=m.status==="online"?"#5eff5e":m.status==="idle"?"#ffa500":m.status==="dnd"?"#ff4d4d":"#666",f="#888";m.level==="Me"||m.level==="Master User"?f="#bb86fc":m.level==="Admin"?f="#cf6679":m.level==="Moderator"?f="#03dac6":m.level==="Contributor"&&(f="#ffa500");let u=m.level==="Me",b=u?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",c=u?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-section" style="background: ${b}; padding: 15px; border-radius: 8px; border: ${c}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${u?"#bb86fc":v}22, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${m.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${u?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${f}; font-weight: 600; text-transform: uppercase;">${u?"DEXTER (ME)":m.level}</span>
                        </div>
...
`),d=[...m.logs];if(d.length<25){let u=25-d.length;for(let b=0;b<u;b++)d.push("")}else d.length>25&&(d=d.slice(-25));let f=d.map(u=>oe(u)).join(`
`);return`
    ...
    `;async function Te(){await Promise.all([it(),St(),kt()]),await Le()}var me=null,ue=null,fe=null;async function nt(){try{return await(await E("/system_monitor")).json()}catch{return null}}async function tt(){try{return await(await E("/system/hardware")).json()}catch{return null}}async function Tt(){try{return await(await E("/processes")).json()}catch{return null}}async function Ct(){try{return await(await E("/analyst/status")).json()}catch{return null}}async function St(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),i=document.getElementById("hardware-refresh-btn"),o=s=>{if(!s){e.innerHTML='<p style="color: #ff4d4d;">Failed to load hardware info.</p>';return}let n="",l=(s.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(n+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    ...
                    </div>`}),s.STORAGE&&s.STORAGE.length>0){let x=0,h=0;s.STORAGE.forEach(y=>{x+=y.USED,h+=y.SIZE});let w=(x/(1024*1024*1024)).toFixed(1),p=(h/(1024*1024*1024)).toFixed(1),g=h>0?(x/h*100).toFixed(0):0;n+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=n};if(e&&i){i.dataset.listenerAttached||(i.onclick=async()=>{i.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let n=await tt();n?(o(n),i.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML='<p style="color: #ff4d4d;">Failed to refresh.</p>',i.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},i.dataset.listenerAttached="true");let s=e.querySelector("p");if(s&&s.textContent==="Loading hardware info..."){let n=await tt();o(n)}}if(!t)return;let a=await nt();if(!a||!a.services){t.innerHTML=$("offline","Failed to load system metrics.","The event service may be offline or unreachable.");return}me=Date.now(),L(3,me);let r=a.services||[];Array.from(t.children).forEach(s=>{s.classList.contains("service-widget")||s.remove()});function m(s){return!s||s==="N/A"||s==="unknown"||s.trim()===""?"-":s}function v(s){if(!s||s==="N/A"||s==="unknown")return"-";let n=s.match(/^(\d+\.\d+\.\d+)/);return n?n[0]:s.split(".").slice(0,3).join(".")||"-"}function d(s){return!s||s.length<=28?s:s.substring(0,28)+"..."}function f(s){if(!s||s==="N/A"||s==="unknown")return"-";let n=s.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!n)return"-";let l=parseInt(n[1])||0,x=parseInt(n[2])||0,h=parseInt(n[3])||0,w=parseFloat(n[4])||0,p=l*86400+x*3600+h*60+w,g=Math.floor(p/86400);if(g>0)return`${g}d`;let y=Math.floor(p/3600);if(y>0)return`${y}h`;let A=Math.floor(p/60);return A>0?`${A}m`:`${Math.floor(p)}s`}function u(s){let n=s.status==="online",l=n?"service-widget-online":"service-widget-offline",x=n?"bx-check-circle":"bx-x-circle",h=n?"OK":"BAD",w=s.version?v(s.version.str):"-",p=s.cpu&&s.cpu!=="N/A"?s.cpu:"-",g=s.memory&&s.memory!=="N/A"?s.memory:"-",y=p!=="-"?"#00ff00":"#666",A=p!=="-"?"#fff":"#666",k=g!=="-"?"#00ff00":"#666",T=g!=="-"?"#fff":"#666",N=f(s.uptime),_="";return n?_=`
                ...
                </div>`:_='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${l}" data-service-id="${s.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${s.short_name||s.id}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${d(s.domain&&s.port?`${s.domain}:${s.port}`:"")}</span></div>${_}</div></div>`}let b=new Map(Array.from(t.querySelectorAll(".service-widget")).map(s=>[s.dataset.serviceId,s])),c=new Set(r.map(s=>s.id));for(let[s,n]of b)c.has(s)||n.remove();r.forEach(s=>{let n=u(s),l=b.get(s.id);l?l.outerHTML!==n&&(l.outerHTML=n):t.insertAdjacentHTML("beforeend",n)})}async function kt(){let t=document.getElementById("models-widgets");if(!t)return;let e=await nt();if(!e){t.innerHTML=$("offline","Failed to load model status.");return}ue=Date.now(),L(3,ue);let i=e.models||[],o=e.whisper;Array.from(t.children).forEach(v=>{v.classList.contains("service-widget")||v.remove()});function a(v){let d=v.status==="Ready";return`
                ...
                </div>`}function r(v){let d=v.status==="Downloaded",f=d?"service-widget-online":"service-widget-offline",u=d?"OK":"MISSING",b=d&&v.size>0?`${(v.size/1e9).toFixed(2)} GB`:"-",c=v.name;return v.type==="custom"&&c.endsWith(":latest")&&(c=c.replace(":latest","")),`<div class="service-widget ${f}" data-model-name="${v.name}"><div class="service-widget-header"><i class="bx ${d?"bx-check-circle":"bx-x-circle"}"></i><h3>${c}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${v.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let m="";if(o&&(m+=a(o)),m+=i.map(r).join(""),!m){t.innerHTML=$("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function it(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),i=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),a=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await E("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),it()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let m=await Ct();if(m){let c=Math.floor(Date.now()/1e3),s=m.active_tier,n=(l,x,h)=>{if(s===h||h==="guardian"&&s==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let w=x-c;if(w<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let p=Math.floor(w/60),g=w%60;l.textContent=`${p}m ${g}s`,l.style.color="#fff"}};if(e&&m.guardian&&n(e,m.guardian.next_run,"guardian"),i&&m.architect&&n(i,m.architect.next_run,"architect"),o&&m.strategist&&n(o,m.strategist.next_run,"strategist"),a&&m.system_idle_time!==void 0){let l=m.system_idle_time,x=Math.floor(l/60),h=l%60;a.textContent=`${x}m ${h}s`,l>300?a.style.color="#5eff5e":l>60?a.style.color="#ffa500":a.style.color="#fff"}}else[e,i,o,a].forEach(s=>{s&&(s.textContent="Offline",s.style.color="#ff4d4d")});let v=await Tt(),d=document.getElementById("vitals-processes-val");if(d)if(v){let c=v.length;d.textContent=c>0?`${c} Active`:"Idle",d.style.color=c>0?"#bb86fc":"#fff"}else d.textContent="-",d.style.color="#888";if(v===null){t.innerHTML=$("offline","Failed to load process status.");return}if(fe=Date.now(),L(3,fe),v.length===0){t.innerHTML=$("empty","No active processes."),z(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function f(c){let s=Math.floor(Date.now()/1e3-c.start_time),n=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",l=c.channel_id,x={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return x[l]?l=x[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`

...
`,Ae=null,re=new Set,Ie=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ss(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await _(i);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];Ae=Date.now(),W(0,Ae);let l=Date.now(),u=24*60*60*1e3,p=o.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let h=parseInt(d);return l-h<u});p.sort((r,d)=>{let h=I=>{let w=I.event;if(typeof w=="string")try{w=JSON.parse(w)}catch{return"low"}return(w.priority||"low").toLowerCase()},M=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,A=M(h(r)),L=M(h(d));return A!==L?L-A:d.timestamp-r.timestamp}),Ie=p;let m=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(p.map(r=>m(r))).size>1;if(p.length>0){let r=null;p.forEach(d=>{let h=m(d);b&&h!==r&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),r=h),x.push(d)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=N("empty","No alerts yet."),Te();return}let s=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let h=(d.title||"Untitled Alert").trim(),M=(d.body||"No description provided.").trim(),A=d.summary||"",L=d.content||"",I=d.protocol||"",w=(d.priority||"low").toLowerCase(),H=!!d.alert,D=(d.category||"system").trim(),z=d.related_event_ids||[],O=d.audit_event_id,U=!!localStorage.getItem(`alert_read_ts_${r.id}`),S=new Date(r.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=U?"event-border-grey":"event-border-blue";!U&&H&&(T="event-border-red"),U&&(w==="high"||w==="critical")?T="event-border-red":U&&w==="medium"&&(T="event-border-orange");let k=U?"alert-read":"alert-unread",R=re.has(r.id),P=R?"expanded":"",ee=R?"display: block;":"display: none;",j="",q="";z.length>0&&(q=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${z.map(te=>`<a href="#" onclick="window.dexter.viewEvent('${te}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${te.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let Z="";O&&(Z=`
...
`,_e=null,me=new Set,we=[],J="all",ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ms={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function us(t){for(let[e,i]of Object.entries(ps))if(i.includes(t))return e;return"system"}function fs(t){return ms[t]||"bx-square-rounded"}async function ae(t=!1){let e=document.getElementById("events-timeline");if(!e)return;gs();let a=`/events?ml=${J==="all"?100:250}&format=json`;J!=="all"&&(a+=`&category=${J}`);try{let n=await _(a);if(!n.ok)throw new Error("Service unreachable");if(we=((await n.json()).events||[]).filter(b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let f=s.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),_e=Date.now(),W(1,_e),we.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=b=>{let s=b.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let f=s.type,E=us(f),y=fs(f),c=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="analysis.user.message_signals"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),v="event-border-grey";c&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?v="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.analysis.audit"||f==="analysis.user.message_signals"?v="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?v="event-border-orange":f==="system.test.completed"?v=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=me.has(b.id),h=d?"expanded":"",M=d?"display: block;":"display: none;",A=new Date(b.timestamp*1e3),L=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Et(f,s),H=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",D="";if(c){let S="";if(f==="engagement.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;S=`
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let C=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`,$="";s.eval_count&&($=`
                        ...
                        `);let T="";if(s.chat_history&&s.chat_history.length>0){let k=s.chat_history.length,R=s.chat_history.map((P,ee)=>{let j=P.role.toUpperCase();j==="ASSISTANT"&&(j="DEXTER");let q=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",Z=ee===k-1?"block":"none";return`
                                <div class="history-slide" data-index="${ee}" style="display: ${Z};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${q}; letter-spacing: 1px; font-weight: bold;">${j}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ee+1} of ${k}</span>
                                    </div>
                    ...
                    `}else if(f==="system.analysis.audit"){let C=s.success?"#03dac6":"#ff4d4d",$=s.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,k="";s.error&&(k=`
                        ...
                        `}let ee="";if(s.parsed_results&&s.parsed_results.length>0){let j=s.parsed_results.map(q=>`

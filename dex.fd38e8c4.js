...
`,ce=null,q=new Set,de=[];async function j(t=!1){let e=document.getElementById("notifications-list");if(!e)return;Lt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];ce=Date.now(),H(0,ce);let c=Date.now(),b=24*60*60*1e3,f=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let h=parseInt(v);return c-h<b});f.sort((d,v)=>{let h=_=>{let k=_.event;if(typeof k=="string")try{k=JSON.parse(k)}catch{return"low"}return(k.priority||"low").toLowerCase()},$=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=$(h(d)),L=$(h(v));return E!==L?L-E:v.timestamp-d.timestamp}),de=f;let g=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},w=[],p=new Set(f.map(d=>g(d))).size>1;if(f.length>0){let d=null;f.forEach(v=>{let h=g(v);p&&h!==d&&(w.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),d=h),w.push(v)})}if(t&&(e.innerHTML=""),f.length===0){e.innerHTML=S("empty","No notifications yet."),le();return}let n=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let h=v.title||"Untitled Notification",$=v.body||"No description provided.",E=v.priority||"low",L=!!v.alert,_=v.category||"system",k=v.related_event_ids||[],D=!!localStorage.getItem(`notification_read_ts_${d.id}`),O=new Date(d.timestamp*1e3),B=O.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=O.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=D?"event-border-grey":"event-border-blue";!D&&L&&(T="event-border-red"),D&&(E==="high"||E==="critical")?T="event-border-red":D&&E==="medium"&&(T="event-border-orange");let R=D?"notification-read":"notification-unread",N=q.has(d.id),ve=N?"expanded":"",be=N?"display: block;":"display: none;",Te="",ke="";k.length>0&&(ke=`
            ...
            `;let Ie=z.querySelector(".close-details-btn");return Ie&&Ie.addEventListener("click",Z=>{Z.stopPropagation(),z.classList.remove("expanded");let he=z.querySelector(".event-details");he&&(he.style.display="none"),q.delete(d.id)}),z},i=d=>{let v=document.createElement("div");v.className="notification-divider",v.dataset.notificationId=d.id;let h="#888888";return d.label==="CRITICAL"?h="#ff4d4d":d.label==="HIGH"?h="#ff8888":d.label==="MEDIUM"&&(h="#ffa500"),v.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,v.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,v},l=Array.from(e.children),x=new Map(l.map(d=>[d.dataset.notificationId,d])),y=new Set(w.map(d=>d.id));l.forEach(d=>{let v=d.dataset.notificationId;(!v||!y.has(v))&&d.remove()});let m=null;w.forEach((d,v)=>{let h=x.get(d.id);(!h||t)&&(h&&h.remove(),d.type==="divider"?h=i(d):h=n(d),!h)||(v===0?e.firstElementChild!==h&&e.prepend(h):m&&m.nextElementSibling!==h&&m.after(h),m=h)}),ce=Date.now(),H(0,ce),le()}catch(o){console.error("Error fetching notifications:",o),e.children.length===0&&(e.innerHTML=S("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}function Lt(){let t=document.getElementById("notif-read-all"),e=document.getElementById("notif-expand-all"),a=document.getElementById("notif-close-all"),o=document.getElementById("notif-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{de.forEach(s=>{localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{de.forEach(s=>{q.add(s.id),localStorage.getItem(`notification_read_ts_${s.id}`)||localStorage.setItem(`notification_read_ts_${s.id}`,Date.now().toString())}),j(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{q.clear(),j(!0)},a.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let s=Date.now()-1728e5;de.forEach(r=>{localStorage.setItem(`notification_read_ts_${r.id}`,s.toString())}),q.clear(),j(!0)},o.dataset.listenerAttached="true")}var Fe=()=>`
...
`,G=new Set,je=[],J=null;async function W(t=!1){let e=document.getElementById("roadmap-list");if(e){$t();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let o=await a.json();if(je=o,o.length===0){e.innerHTML=S("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=g=>{let w=G.has(g.id),u=g.state==="draft",p=g.state==="published",n=g.state==="consumed",i="event-border-grey";p&&(i="event-border-blue"),n&&(i="event-border-purple");let x=new Date(g.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${i} cursor-pointer ${w?"expanded":""}`,y.dataset.itemId=g.id,y.onclick=d=>{if(d.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(g.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",G.add(g.id))};let m=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${g.state.toUpperCase()}]</span>`;return y.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${I(g.content)}</div>
      ...
      `,y.querySelector(".edit-btn")?.addEventListener("click",()=>Ct(g)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Tt(g)),y.querySelector(".delete-btn")?.addEventListener("click",()=>kt(g.id)),y.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",G.delete(g.id)}),y},r=Array.from(e.children),c=new Map(r.map(g=>[g.dataset.itemId,g])),b=new Set(o.map(g=>g.id));r.forEach(g=>{let w=g.dataset.itemId;(!w||!b.has(w))&&g.remove()});let f=null;o.forEach((g,w)=>{let u=c.get(g.id);(!u||t)&&(u&&u.remove(),u=s(g),!u)||(w===0?e.firstElementChild!==u&&e.prepend(u):f&&f.nextElementSibling!==u&&f.after(u),f=u)})}catch{e.children.length===0&&(e.innerHTML=S("error","Failed to load roadmap."))}}}function $t(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{J=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",J=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=J?`/roadmap/${J}`:"/roadmap",b=J?"PATCH":"POST";try{await A(c,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",W(!0)}catch(f){console.error(f)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{je.forEach(r=>G.add(r.id)),W(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{G.clear(),W(!0)},s.dataset.listenerAttached="true")}function Ct(t){J=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Tt(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),W(!0)}catch(a){console.error(a)}}async function kt(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),G.delete(t),W(!0)}catch(e){console.error(e)}}var We=()=>`
    <div class="notifications-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none;">
...
`,Ge=null,Y=new Set,Ke=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;It();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await A(a);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(Ke=r,Ge=Date.now(),H(2,Ge),r.length===0){e.innerHTML=S("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),U(1,0);return}t&&(e.innerHTML="");let c=u=>{let p=u.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let n=p.title||"Untitled Blueprint",i=p.summary||p.body||"No summary provided.",l=p.content||"",x=p.category||"architecture",y=p.affected_services||[],m=p.implementation_path||[],d=new Date(u.timestamp*1e3),v=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Y.has(u.id),E=$?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""}`,L.dataset.blueprintId=u.id;let k={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";L.onclick=function(B){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Y.delete(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),Y.add(u.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let M=y.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${y.join(", ")}</div>`:"",D="";m.length>0&&(D=`
...
`;async function we(){await Promise.all([W(),me()])}var Xe=()=>`
    <div class="notifications-actions">
...
`),f=[...c.logs];if(f.length<25){let w=25-f.length;for(let u=0;u<w;u++)f.push("")}else f.length>25&&(f=f.slice(-25));let g=f.map(w=>re(w)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${tt()}
        </div>`;async function pt(){await Promise.all([ge(),Ee(),Se()]),await ne()}var st=null,nt=null,it=null;async function ut(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function at(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function Ht(){try{return await(await A("/processes")).json()}catch{return null}}async function Rt(){try{return await(await A("/analyst/status")).json()}catch{return null}}async function Ee(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),a=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=S("offline","Failed to load hardware info.","The event service may be offline."));return}let i="",l=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(i+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let x=0,y=0;n.STORAGE.forEach(h=>{x+=h.USED,y+=h.SIZE});let m=(x/(1024*1024*1024)).toFixed(1),d=(y/(1024*1024*1024)).toFixed(1),v=y>0?(x/y*100).toFixed(0):0;i+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=i};if(e&&a){a.dataset.listenerAttached||(a.onclick=async()=>{a.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let i=await at();i?(o(i),a.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.children.length===0&&(e.innerHTML=S("offline","Failed to refresh.","The event service may be offline.")),a.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{a.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},a.dataset.listenerAttached="true");let n=e.querySelector("p");if(n&&n.textContent==="Loading hardware info..."){let i=await at();o(i)}}if(!t)return;let s=await ut();if(!s||!s.services){t.children.length===0&&(t.innerHTML=S("offline","Failed to load system metrics.","The event service may be offline."));return}st=Date.now(),H(3,st);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function b(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/^(\d+\.\d+\.\d+)/);return i?i[0]:n.split(".").slice(0,3).join(".")||"-"}function f(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function g(n){if(!n||n==="N/A"||n==="unknown")return"-";let i=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!i)return"-";let l=parseInt(i[1])||0,x=parseInt(i[2])||0,y=parseInt(i[3])||0,m=parseFloat(i[4])||0,d=l*86400+x*3600+y*60+m,v=Math.floor(d/86400);if(v>0)return`${v}d`;let h=Math.floor(d/3600);if(h>0)return`${h}h`;let $=Math.floor(d/60);return $>0?`${$}m`:`${Math.floor(d)}s`}function w(n){let i=n.status==="online",l=i?"service-widget-online":"service-widget-offline",x=i?"bx-check-circle":"bx-x-circle",y=i?"OK":"BAD",m=n.version?b(n.version.str):"-",d=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",v=n.memory&&n.memory!=="N/A"?n.memory:"-",h=d!=="-"?"#00ff00":"#666",$=d!=="-"?"#fff":"#666",E=v!=="-"?"#00ff00":"#666",L=v!=="-"?"#fff":"#666",_=g(n.uptime),k="";return i?k=`
                ...
                </div>`}function r(b){let f=b.status==="Downloaded",g=f?"service-widget-online":"service-widget-offline",w=f?"OK":"MISSING",u=f&&b.size>0?`${(b.size/1e9).toFixed(2)} GB`:"-",p=b.name;return b.type==="custom"&&p.endsWith(":latest")&&(p=p.replace(":latest","")),`<div class="service-widget ${g}" data-model-name="${b.name}"><div class="service-widget-header"><i class="bx ${f?"bx-check-circle":"bx-x-circle"}"></i><h3>${p}</h3><span class="service-widget-status">${w}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${b.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${u}</span></div></div></div>`}let c="";if(o&&(c+=s(o)),c+=a.map(r).join(""),!c){t.innerHTML=S("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function ge(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),a=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),ge()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await Rt();if(c){let p=Math.floor(Date.now()/1e3),n=c.active_tier,i=(l,x,y)=>{if(n===y||y==="guardian"&&n==="tests"){l.textContent="Working",l.style.color="#bb86fc";return}let m=x-p;if(m<=0)l.textContent="Ready",l.style.color="#5eff5e";else{let d=Math.floor(m/60),v=m%60;l.textContent=`${d}m ${v}s`,l.style.color="#fff"}};if(e&&c.guardian&&i(e,c.guardian.next_run,"guardian"),a&&c.architect&&i(a,c.architect.next_run,"architect"),o&&c.strategist&&i(o,c.strategist.next_run,"strategist"),s&&c.system_idle_time!==void 0){let l=c.system_idle_time,x=Math.floor(l/60),y=l%60;s.textContent=`${x}m ${y}s`,l>300?s.style.color="#5eff5e":l>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,a,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let b=await Ht(),f=document.getElementById("vitals-processes-val");if(f)if(b){let p=b.length;f.textContent=p>0?`${p} Active`:"Idle",f.style.color=p>0?"#bb86fc":"#fff"}else f.textContent="-",f.style.color="#888";if(b===null){t.children.length===0&&(t.innerHTML=S("offline","Failed to load process status.","The event service may be offline."));return}if(it=Date.now(),H(3,it),b.length===0){t.innerHTML=S("empty","No active processes."),U(3,0);return}(t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML="");function g(p){let n=Math.floor(Date.now()/1e3-p.start_time),i=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",l=p.channel_id,x={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};return x[l]?l=x[l]:/^\d+$/.test(l)&&(l=`Channel ${l}`),`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Pt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[a,o]of Object.entries(t)){let s=vt.filter(r=>r.category===a);s.length!==0&&(e+=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Ce=!0,a}function ft(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Ce=!1}function gt(t,e,a="output"){if(!Ce)return;let o=document.createElement("div");o.className=`terminal-line terminal-${a}`,a==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=re(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function zt(t){let e=vt.find(s=>s.id===t);if(!e)return;let a=Ut(e);gt(a,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),gt(a,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function bt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Pt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let o=a.dataset.cmd;zt(o)})})}async function Ft(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${ye()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function ht(){Ft(),Pe(),Ae();let t=window.location.pathname;(t==="/dex"||t==="/dex/"||t.includes("/dex/index.html"))&&!t.includes("/contribute")&&bt();let e=Be();_e(e),Me();let a=window.location.pathname==="/"||window.location.pathname==="/index.html",o=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!o&&document.querySelector("footer")?.classList.add("hide");let s=[],r=document.getElementById("windows-container");r&&r.setAttribute("data-count","0");let c=m=>{localStorage.setItem("dex_last_window",m)};function b(){return 1}function f(){for(;s.length>1;)s.shift().close(!0);let m=document.getElementById("windows-container"),d=document.querySelector("nav"),v=document.querySelector("footer"),h=window.location.pathname==="/"||window.location.pathname==="/index.html",$=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");m&&m.setAttribute("data-count",s.length);let E=s.length>0;s.forEach(M=>{let D=document.getElementById(M.id);D&&D.classList.remove("hide-close")}),ie(te());let L=document.getElementById("dexter-menu-container"),_=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon");if(s.length>0){if(v?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),m&&(m.style.paddingTop="60px"),L&&(L.style.display="none"),k&&(k.style.display="none"),_){let M=s[0].id;["feed-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(_.innerHTML=`
                  ...
                  `,document.getElementById("switch-feed").addEventListener("click",()=>{c("feed-window"),g(u)}),document.getElementById("switch-events").addEventListener("click",()=>{c("events-window"),g(p)}),document.getElementById("switch-monitor").addEventListener("click",()=>{c("monitor-window"),g(i)}),document.getElementById("switch-contacts").addEventListener("click",()=>{c("contacts-window"),g(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{c("workspace-window"),g(l)})):_.innerHTML=""}}else d?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),m&&(m.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),h||$?v?.classList.remove("hide"):v?.classList.add("hide"),L&&(L.style.display="flex"),k&&(k.style.display="block"),_&&(_.innerHTML="")}function g(m){if(m.isOpen()){m.close();return}for(;s.length>0;)s.pop().close(!0);s.push(m),m.open(),f()}function w(){[...s].forEach(d=>d.close()),s=[]}window.addEventListener("resize",f);let u=F({id:"feed-window",title:"Feed",icon:"bx-news",content:ze(),onOpen:()=>j(),onClose:()=>{let m=s.indexOf(u);m>-1&&s.splice(m,1),f()}}),p=F({id:"events-window",title:"Events",icon:"bx-calendar-event",content:et(),onOpen:()=>Q(),onClose:()=>{let m=s.indexOf(p);m>-1&&s.splice(m,1),f()}}),n=F({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Xe(),onOpen:()=>pe(),onClose:()=>{let m=s.indexOf(n);m>-1&&s.splice(m,1),f()}}),i=F({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:rt()},{icon:"bxs-component",title:"Processes",content:lt()},{icon:"bxs-brain",title:"Models",content:ct()},{icon:"bxs-hdd",title:"Hardware",content:dt()},{icon:"bxs-terminal",title:"Logs",content:mt()},{icon:"bxs-zap",title:"Analyst",content:ot()}],onOpen:()=>{Ee(),ge(),Se(),ne()},onClose:()=>{let m=s.indexOf(i);m>-1&&s.splice(m,1),f()}}),l=F({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Je(),onOpen:()=>we(),onClose:()=>{let m=s.indexOf(l);m>-1&&s.splice(m,1),f()}}),x=F({id:"settings-window",title:"Settings",icon:"bx-cog",content:Le(),onOpen:()=>$e(x),onClose:()=>{let m=s.indexOf(x);m>-1&&s.splice(m,1),f()}}),y=F({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dex/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let m=document.getElementById("dexter-dropdown");m&&(m.innerHTML=`
            <div class="dropdown-item" id="feed-menu-item"><i class='bx bx-news'></i> Feed</div>
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),v=document.getElementById("dexter-menu-btn");d&&m&&v&&(d.addEventListener("mouseenter",()=>{m.classList.add("active"),v.classList.add("active")}),d.addEventListener("mouseleave",()=>{m.classList.remove("active"),v.classList.remove("active")}),v.addEventListener("click",h=>{h.stopPropagation();let $=localStorage.getItem("dex_last_window")||"feed-window",E=null;$==="feed-window"?E=u:$==="events-window"?E=p:$==="monitor-window"?E=i:$==="contacts-window"?E=n:$==="workspace-window"&&(E=l),E&&g(E)})),document.getElementById("feed-menu-item")?.addEventListener("click",()=>{c("feed-window"),g(u)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{c("events-window"),g(p)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{c("monitor-window"),g(i)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{c("contacts-window"),g(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{c("workspace-window"),g(l)}),document.getElementById("settings-icon")?.addEventListener("click",()=>g(x)),document.getElementById("close-all-windows")?.addEventListener("click",()=>w()),setInterval(()=>{u.isOpen()&&j(),p.isOpen()&&Q(),n.isOpen()&&pe(),i.isOpen()&&pt(),l.isOpen()&&we()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{y.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",m=>{m.preventDefault();try{He(document.getElementById("email-input").value),window.location.reload()}catch(d){let v=document.getElementById("login-error");v&&(v.textContent=d.message,v.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ht):ht();})();

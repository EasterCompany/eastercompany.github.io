...
The Time-to-First-Token (TTFT) is the most critical metric for user satisfaction in LLM chat. As conversation history scales from 10 to 100 messages, the "Prompt Processing" phase of inference grows exponentially. To combat this, systems use summarization. However, if the system summarizes _during_ the user's request, the latency spike is unacceptable.

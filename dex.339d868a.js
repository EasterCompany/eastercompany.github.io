(()=>{function ge(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function fe(e=!1){let n=`
    ...
    `,t=document.createElement("footer");t.innerHTML=e,document.body.appendChild(t)}function F(e){let t=null,n=e.onClose||null,u=e.onOpen||null;function o(){if(t){t.classList.add("open"),u&&setTimeout(u,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),t=document.createElement("div"),t.id=e.id,t.className="window";let b=e.icon||"bx-window",s="",m="",g;e.tabs&&e.tabs.length>0?(s=`<div class="tab-bar">${e.tabs.map((l,f)=>{let h;return l.icon?h=`<i class="bx ${l.icon}"></i>`:l.title&&l.title.length>0?h=`<span class="tab-glyph">${l.title.charAt(0).toUpperCase()}</span>`:h='<i class="bx bx-question-mark"></i>',`
...
`);return`
            ...
            `}).join("");return e.innerHTML=s,document.querySelectorAll(".copy-logs-btn").forEach(m=>{m.addEventListener("click",()=>{let g=unescape(m.dataset.logs);navigator.clipboard.writeText(g).then(()=>{let y=m.querySelector("i");y.classList.remove("bx-copy"),y.classList.add("bx-check"),setTimeout(()=>{y.classList.remove("bx-check"),y.classList.add("bx-copy")},2e3)})})}),ae=Date.now(),!0}catch(v){return console.error("Error fetching logs:",v),e.classList.add("placeholder-active"),e.innerHTML=O("offline","Failed to load logs.","The event service may be offline, unreachable, or blocked by a browser extension (e.g., ad blocker)."),!1}}function w(e,t,n=null){let o={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[e]||"bx-info-circle",i=n?`<p class="placeholder-action">${n}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${o} placeholder-icon'></i><p class="placeholder-message">${t}</p>${i}</div>`}function A(e){return e&&e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function $(e,t){let n=document.querySelector(`.tab[data-tab-index="${e}"] .tab-subtitle`);if(!n)return;if(!t){n.textContent="Last updated: never";return}let o=(Date.now()-t)/1e3,i;if(o<30)i=`${Math.floor(o)}s ago`;else{n.textContent="Last updated: never";return}n.textContent=`Last updated: ${i}`}function B(e,t){let n=document.querySelector(`.tab[data-tab-index="${e}"]`);if(!n)return;let u=n.querySelector(".notification-badge");u&&(t>0?(u.textContent=t>9?"9+":t,u.style.display="flex"):u.style.display="none")}function G(){let e=document.getElementById("notifications-list");if(!e)return;let t=e.querySelectorAll(".notification-unread").length;B(0,t)}var De={message_received:"{user} posted in {channel}: {message}",action_performed:"{actor} {action} {target}",log_entry:"[{level}] {message}",error_occurred:"ERROR: {error}",status_change:"{entity} changed status to {new_status}",metric_recorded:"{metric_name}: {value}{unit}","messaging.user.joined_voice":"{user_name} joined voice channel {channel_name}","messaging.user.left_voice":"{user_name} left voice channel {channel_name}","messaging.user.sent_message":"{user_name} in {channel_name}: {content}","messaging.bot.sent_message":"Dexter sent in {channel_name}: {content}","messaging.bot.joined_voice":"Dexter joined voice channel {channel_name}","messaging.bot.voice_response":"Dexter said: {content}","messaging.bot.status_update":"Dexter status changed to {status}: {details}","messaging.user.speaking.started":"{user_name} started speaking","messaging.user.speaking.stopped":"{user_name} stopped speaking","messaging.user.transcribed":"{user_name} said: {transcription}","messaging.user.joined_server":"{user_name} joined {server_name}","messaging.webhook.message":"{user_name} (Webhook): {content}","webhook.processed":"Webhook processed: {status}",voice_speaking_started:"User {user_id} started speaking in voice channel {channel_id}",voice_speaking_stopped:"User {user_id} stopped speaking in voice channel {channel_id}",voice_transcribed:"{user_name} said: {transcription}","engagement.decision":"Engagement Decision: {decision} ({reason})",bot_response:"Bot Responded: {response}","moderation.explicit_content.deleted":"Explicit content deleted in {channel_name} from {user_name}: {reason}","analysis.link.completed":"Analyzed link: {url}","analysis.visual.completed":"Analyzed image: {filename}","system.cli.command":"CLI Command: {command} {args} ({status})","system.cli.status":"CLI Status: {message}","system.notification.generated":"Notification ({priority}): {title}"};function Te(e,t){let n=De[e];if(e==="voice_transcribed"&&!t.user_name&&t.user_id&&(n="User {user_id} said in voice channel {channel_id}: {transcription}"),e==="system.notification.generated"&&(n=`Notification (${t.priority?t.priority.charAt(0).toUpperCase()+t.priority.slice(1):"Low"}): ${t.title}`),!n)return e;let u=n.replace(/\{(\w+)\}/g,(o,i)=>t[i]!==void 0&&t[i]!==null?t[i]:o);return(e==="messaging.user.transcribed"||e==="voice_transcribed")&&t.detected_language&&t.detected_language!=="en"&&t.english_translation&&(u+=` (Translation: ${t.english_translation})`),u}var Le=()=>'<div id="events-timeline" class="events-timeline"><p>Loading events...</p></div>',V=null;async function re(){let e=document.getElementById("events-timeline");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=w("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(v=>v.id==="dex-event-service")}catch{e.innerHTML=w("error","Invalid service map data.");return}if(!n){e.innerHTML=w("error","Event service not found in service map.");return}let o=`http://${n.domain==="0.0.0.0"?"localhost":n.domain}:${n.port}/events?ml=50&format=json&exclude_types=system.notification.generated`;try{let i=new Set(Array.from(e.querySelectorAll(".event-item.expanded")).map(c=>c.dataset.eventId).filter(c=>c)),v=await fetch(o);if(!v.ok)throw new Error("Service is offline or unreachable.");let p=(await v.json()).events||[];if(p.length===0){e.innerHTML=w("empty","No events found.");return}let b=c=>{let a=c.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let r=a.type,l=r==="engagement.decision"||r==="messaging.bot.sent_message"||r==="messaging.user.sent_message"||r==="moderation.explicit_content.deleted"||r==="analysis.link.completed"||r==="analysis.visual.completed"||r==="system.cli.command",f="event-border-grey";l&&(r==="moderation.explicit_content.deleted"?f="event-border-red":r==="analysis.link.completed"||r==="analysis.visual.completed"?f="event-border-purple":r==="system.cli.command"?f="event-border-orange":f="event-border-blue");let h=l?"cursor-pointer":"",T=i.has(c.id),x=T?"expanded":"",k=T?"display: block;":"display: none;",E=new Date(c.timestamp*1e3),I=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),D=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),H=Te(r,a),C="";if(l){let S="";if(r==="engagement.decision")S=`
                        ...
                        `;else if(r==="messaging.bot.sent_message")S=`
                            <div class="event-detail-row">
                        ...
                        `;else if(r==="moderation.explicit_content.deleted")S=`
                            <div class="event-detail-row">
                        ...
                        `;else if(r==="analysis.link.completed")S=`
                            <div class="event-detail-row">
                        ...
                        `;else if(r==="analysis.visual.completed"){let L="";a.base64_preview?L=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(L=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),S=`
                            <div class="event-detail-row">
                        ...
                        `}else if(r==="system.cli.command")S=`
                            <div class="event-detail-row">
                        ...
                        `;else if(r==="messaging.user.sent_message"){let L="";a.attachments&&a.attachments.length>0&&(L=`
                                <div class="event-detail-block">
                    ...
                    `}let N=document.createElement("div");if(N.className=`event-item ${f} ${h} ${x}`,N.dataset.eventId=c.id,N.onclick=function(S){if(!l)return;this.classList.toggle("expanded");let L=this.querySelector(".event-details");L&&(L.style.display=L.style.display==="none"?"block":"none")},N.innerHTML=`
                ...
                `,l){let S=N.querySelector(".close-details-btn");S&&S.addEventListener("click",L=>{L.stopPropagation();let U=L.target.closest(".event-item");if(U){U.classList.remove("expanded");let M=U.querySelector(".event-details");M&&(M.style.display="none")}})}return N},s=Array.from(e.children),m=new Map(s.map(c=>[c.dataset.eventId,c])),g=new Set(p.map(c=>c.id));s.forEach(c=>{(!c.dataset.eventId||!g.has(c.dataset.eventId))&&c.remove()});let y=null;p.forEach((c,a)=>{let r=m.get(c.id);!r&&(r=b(c),!r)||(a===0?e.firstElementChild!==r&&e.prepend(r):y&&y.nextElementSibling!==r&&y.after(r),y=r)}),V=Date.now(),$(2,V)}catch(i){console.error("Error fetching events:",i),e.children.length===0&&(e.innerHTML=w("offline","Failed to load events.","The event service may be offline or unreachable."))}}var Ee=()=>'<div id="notifications-list" class="notifications-list events-timeline" style="display: flex; flex-direction: column; gap: 15px;"><p>Loading notifications...</p></div>',Y=null;async function le(){let e=document.getElementById("notifications-list");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=w("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(v=>v.id==="dex-event-service")}catch{e.innerHTML=w("error","Invalid service map data.");return}if(!n){e.innerHTML=w("error","Event service not found in service map.");return}let o=`http://${n.domain==="0.0.0.0"?"localhost":n.domain}:${n.port}/events?ml=1000&format=json&event.type=system.notification.generated`;try{let i=await fetch(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let d=(await i.json()).events||[],p=Date.now(),b=24*60*60*1e3,s=d.filter(l=>{let f=localStorage.getItem(`notification_read_ts_${l.id}`);if(!f)return!0;let h=parseInt(f);return p-h<b});if(s.length===0){e.innerHTML=w("empty","No notifications yet."),G();return}let m=l=>{let f=l.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let h=f.title||"Untitled Notification",T=f.body||"No description provided.",x=f.priority||"low",k=f.category||"system",E=f.related_event_ids||[],D=!!localStorage.getItem(`notification_read_ts_${l.id}`),H=new Date(l.timestamp*1e3),C=H.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),N=H.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=D?"event-border-grey":"event-border-blue";D&&(x==="high"||x==="critical")?S="event-border-red":D&&x==="medium"&&(S="event-border-orange");let L=D?"notification-read":"notification-unread",U=g.has(l.id),M=U?"expanded":"",ee=U?"display: block;":"display: none;",z="";E.length>0&&(z=`
                    <div class="event-detail-row">
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${ee}">
                        ...
                        <div class="event-detail-block" style="text-align: left;">
                            ...
                            <p class="detail-pre" style="white-space: pre-wrap; margin-top: 5px; text-align: left;">${A(T)}</p>
                        </div>
                        ${z}
                    </div>
                </div>
            `;let ue=j.querySelector(".close-details-btn");return ue&&ue.addEventListener("click",W=>{W.stopPropagation(),j.classList.remove("expanded");let q=j.querySelector(".event-details");q&&(q.style.display="none"),g.delete(l.id)}),j},g=new Set(Array.from(e.querySelectorAll(".event-item.expanded")).map(l=>l.dataset.notificationId).filter(l=>l)),y=Array.from(e.children),c=new Map(y.map(l=>[l.dataset.notificationId,l])),a=new Set(s.map(l=>l.id));y.forEach(l=>{(!l.dataset.notificationId||!a.has(l.dataset.notificationId))&&l.remove()});let r=null;s.forEach((l,f)=>{let h=c.get(l.id);!h&&(h=m(l),!h)||(f===0?e.firstElementChild!==h&&e.prepend(h):r&&r.nextElementSibling!==h&&r.after(h),r=h)}),Y=Date.now(),$(0,Y),G()}catch(i){console.error("Error fetching notifications:",i),e.children.length===0&&(e.innerHTML=w("offline","Failed to load notifications.","The event service may be offline or unreachable."))}}var _e=()=>'<div id="blueprints-list" class="blueprints-list events-timeline" style="display: flex; flex-direction: column; gap: 15px;"><p>Loading blueprints...</p></div>',K=null;async function ce(){let e=document.getElementById("blueprints-list");if(!e)return;let t=localStorage.getItem("service_map");if(!t){e.innerHTML=w("config","No service map configured.","Upload service-map.json in Settings.");return}let n=null;try{n=(JSON.parse(t).services?.cs||[]).find(v=>v.id==="dex-event-service")}catch{e.innerHTML=w("error","Invalid service map data.");return}if(!n){e.innerHTML=w("error","Event service not found in service map.");return}let o=`http://${n.domain==="0.0.0.0"?"localhost":n.domain}:${n.port}/events?ml=100&format=json&event.type=system.blueprint.generated`;try{let i=await fetch(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let d=(await i.json()).events||[];if(d.length===0){e.innerHTML=w("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),B(1,0);return}let p=c=>{let a=c.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let r=a.title||"Untitled Blueprint",l=a.body||"No details provided.",f=a.category||"architecture",h=new Date(c.timestamp*1e3),T=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=b.has(c.id),E=k?"display: block;":"display: none;",I=document.createElement("div");I.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""}`,I.dataset.blueprintId=c.id,I.onclick=function(H){this.classList.toggle("expanded");let C=this.querySelector(".event-details");if(C){let N=C.style.display==="none";C.style.display=N?"block":"none",N?b.add(c.id):b.delete(c.id)}},I.innerHTML=`
                <div class="event-time">

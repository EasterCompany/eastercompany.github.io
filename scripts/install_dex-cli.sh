...
# Initialize the Dexter environment by running the tool itself
echo "→ Initializing Dexter environment..."
if ! "$DEX_EXECUTABLE_PATH" system scan; then
    echo "❌ Error: Failed to initialize the Dexter environment."
    exit 1
fi

echo ""
echo "✓ dex-cli installation complete!"
echo ""
echo "To get started, please restart your terminal or run 'source $SHELL_RC'."
echo "Then you can run: dex help"
echo ""

...
`,le=new Set,_e=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){is();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();_e=s;let n=p=>{let x=le.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let E=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${o} cursor-pointer ${x?"expanded":""}`,h.dataset.itemId=p.id,h.onclick=f=>{if(f.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(p.id))};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;h.innerHTML=`
      ...
      `;let d=h.querySelector(".event-details");return d&&d.addEventListener("click",f=>{f.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>ns(p)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>as(p)),h.querySelector(".delete-btn")?.addEventListener("click",()=>os(p.id)),h.querySelector(".close-details-btn")?.addEventListener("click",f=>{f.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(p.id)}),h},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),u=new Set(s.map(p=>p.id));if(l.forEach(p=>{let x=p.dataset.itemId;(!x||!u.has(x))&&p.remove()}),!_e||_e.length===0){e.innerHTML=O("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;s.forEach((p,x)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(x===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=O("offline","Failed to load roadmap.","The event service may be offline."))}}}function is(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input").value;if(!l.trim())return;let r=xe?`/roadmap/${xe}`:"/roadmap",u=xe?"PATCH":"POST";try{await B(r,{method:u,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(c){console.error(c)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{_e.forEach(l=>le.add(l.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function ns(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function as(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function os(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var gt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`;async function Oe(){await Promise.all([ce(),pe()])}var ht=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let l=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,u=document.getElementById("new-chore-owner").value||"313071000877137920";if(l){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:u,natural_instruction:l,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(c){console.error(c),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let r=await(await B("/chores")).json();if(!r||r.length===0){t.innerHTML=O("empty","No active chores.","Create one to start monitoring.");return}let u=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),x=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=u,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${c.dataset.id}`,{method:"DELETE"}),ye())}})}catch(l){console.error("Failed to fetch chores",l)}}var ls=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,cs=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);Ct(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(cs=Date.now(),n.length===0){t.innerHTML=O("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,u)=>{let c=l[r.level]??10,p=l[u.level]??10;return c!==p?c-p:r.username.localeCompare(u.username)}),t.innerHTML=n.map(r=>{let u=r.color?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",c=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",p="#888";r.level==="Me"||r.level==="Master"?p="#bb86fc":r.level==="Admin"?p="#cf6679":r.level==="Moderator"?p="#03dac6":r.level==="Contributor"&&(p="#ffa500");let x=r.level==="Me",m=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",v=x?"2px solid #bb86fc":`1px solid ${u}33`;return`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${St()}
        </div>`;async function zt(){await Promise.all([Ee(),Je(),Ge(),ye()]),await $e()}var kt=null,Tt=null;async function Nt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function Lt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function vs(){try{return await(await B("/processes")).json()}catch{return null}}async function xs(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Je(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),u=a=>{if(a){if(i&&(i.innerHTML=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],ys=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,s]of Object.entries(t)){let n=Rt.filter(l=>l.category===i);n.length!==0&&(e+=`
    ...
    </div>`,e},Ve=!1;function hs(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Ht(),document.getElementById("terminal-action-btn").onclick=()=>Ht());let i=document.getElementById("cli-terminal-body"),s=document.getElementById("cli-docs-pane");i.innerHTML="";let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return s.innerHTML=`

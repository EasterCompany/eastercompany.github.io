(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ze(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Xe=1e3;function ae(t){let e=null,a=t.onClose||null,i=t.onOpen||null;function s(){e&&(e.style.zIndex=++Xe)}function l(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",o),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Xe,e.addEventListener("mousedown",s);let g=t.icon||"bx-window",n="",u="",k;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((v,f)=>{let r=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,k=`<div class="window-content">${t.tabs.map((v,f)=>`<div class="tab-content ${f===0?"active":""}" data-tab-content="${f}">${v.content}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),k=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
    ...
    <div class="system-section-header" style="margin-bottom: 20px;">
        ...
        <div class="alerts-actions" style="margin-left: auto; display: flex; gap: 10px; padding: 0;">
            ...
            <button id="alerts-clear" class="notif-action-btn danger" style="${I()?"display: none;":""}"><i class='bx bx-trash'></i> Clear</button>
        </div>
    </div>
...
`,Ae=null,re=new Set,Me=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Yt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await A(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];Ae=Date.now(),Z(0,Ae);let o=Date.now(),b=24*60*60*1e3,d=l.filter(f=>{let r=localStorage.getItem(`alert_read_ts_${f.id}`);if(!r)return!0;let x=parseInt(r);return o-x<b});d.sort((f,r)=>{let x=M=>{let h=M.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},z=M=>M==="critical"?4:M==="high"?3:M==="medium"?2:1,D=z(x(f)),H=z(x(r));return D!==H?H-D:r.timestamp-f.timestamp}),Me=d;let p=f=>{let r=f.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return"low"}return(r.priority||"low").toLowerCase()},y=[],g=new Set(d.map(f=>p(f))).size>1;if(d.length>0){let f=null;d.forEach(r=>{let x=p(r);g&&x!==f&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),f=x),y.push(r)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=N("empty","No alerts yet."),ke();return}let n=f=>{let r=f.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let x=(r.title||"Untitled Alert").trim(),z=(r.body||"No description provided.").trim(),D=r.summary||"",H=r.content||"",M=r.protocol||"",h=(r.priority||"low").toLowerCase(),P=!!r.alert,L=(r.category||"system").trim(),_=r.related_event_ids||[],q=r.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${f.id}`),S=new Date(f.timestamp*1e3),w=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=j?"event-border-grey":"event-border-blue";!j&&P&&(T="event-border-red"),j&&(h==="high"||h==="critical")?T="event-border-red":j&&h==="medium"&&(T="event-border-orange");let E=j?"alert-read":"alert-unread",O=re.has(f.id),U=O?"expanded":"",te=O?"display: block;":"display: none;",F="",J="";_.length>0&&(J=`
        ...
        `:Y=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${oe(z)}</div>
            </div>
        `,F=`
                ...
                ${Y}
            `;let V=document.createElement("div");V.className=`event-item notification-item ${T} ${E} ${U} cursor-pointer priority-${h}`,V.dataset.alertId=f.id,V.onclick=function(se){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(f.id);let xe=this.querySelector(".event-details");xe&&(xe.style.display="none")}else{this.classList.add("expanded"),re.add(f.id);let xe=this.querySelector(".event-details");if(xe&&(xe.style.display="block"),!localStorage.getItem(`alert_read_ts_${f.id}`)){localStorage.setItem(`alert_read_ts_${f.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ze="event-border-grey";h==="high"||h==="critical"?ze="event-border-red":h==="medium"&&(ze="event-border-orange"),this.classList.add(ze),ke()}}};let Je=`${M?M.toUpperCase():"GUARDIAN"} ALERT: ${D||x}`,Nt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[L]||"bx-bell",Rt=h==="high"||h==="critical"?"#ff4d4d":h==="medium"?"#ffa500":"#888";V.innerHTML=`
...
`,le=new Set,_e=[],ve=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){Xt();try{let a=await A("/roadmap");if(!a.ok)throw new Error("Offline");let i=await a.json();_e=i;let s=p=>{let y=le.has(p.id),m=p.state==="published",g=p.state==="consumed",n="event-border-grey";m&&(n="event-border-blue"),g&&(n="event-border-purple");let k=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),$=document.createElement("div");$.className=`event-item notification-item ${n} cursor-pointer ${y?"expanded":""}`,$.dataset.itemId=p.id,$.onclick=f=>{if(f.target.closest("button"))return;$.classList.contains("expanded")?($.classList.remove("expanded"),$.querySelector(".event-details").style.display="none",le.delete(p.id)):($.classList.add("expanded"),$.querySelector(".event-details").style.display="block",le.add(p.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;$.innerHTML=`
        <div class="event-time">
      ...
      `;let v=$.querySelector(".event-details");return v&&v.addEventListener("click",f=>{f.stopPropagation()}),$.querySelector(".edit-btn")?.addEventListener("click",()=>Qt(p)),$.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>es(p)),$.querySelector(".delete-btn")?.addEventListener("click",()=>ts(p.id)),$.querySelector(".close-details-btn")?.addEventListener("click",f=>{f.stopPropagation(),$.classList.remove("expanded"),$.querySelector(".event-details").style.display="none",le.delete(p.id)}),$},l=Array.from(e.children),o=new Map(l.map(p=>[p.dataset.itemId,p])),b=new Set(i.map(p=>p.id));if(l.forEach(p=>{let y=p.dataset.itemId;(!y||!b.has(y))&&p.remove()}),!_e||_e.length===0){e.innerHTML=N("empty","Your roadmap is empty.",I()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let d=null;i.forEach((p,y)=>{let m=o.get(p.id);(!m||t)&&(m&&m.remove(),m=s(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):d&&d.nextElementSibling!==m&&d.after(m),d=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function Xt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ve=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ve=null},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input").value;if(!l.trim())return;let o=ve?`/roadmap/${ve}`:"/roadmap",b=ve?"PATCH":"POST";try{await A(o,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(d){console.error(d)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{_e.forEach(l=>le.add(l.id)),ce(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{le.clear(),ce(!0)},s.dataset.listenerAttached="true")}function Qt(t){ve=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function es(t){let e=t.state==="published"?"draft":"published";try{await A(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(a){console.error(a)}}async function ts(t){if(confirm("Delete this roadmap item?"))try{await A(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
        <button id="blueprints-expand-all" class="notif-action-btn"><i class='bx bx-expand'></i> Expand All</button>
        <button id="blueprints-close-all" class="notif-action-btn"><i class='bx bx-collapse'></i> Close All</button>
        <button id="blueprints-clear" class="notif-action-btn danger" style="${I()?"display: none;":""}"><i class='bx bx-trash'></i> Clear</button>
    </div>
`,ut=()=>`
...
`,pt=null,de=new Set,ft=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ss();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await A(a);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(ft=l,pt=Date.now(),Z(2,pt),l.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),he(2,0);return}t&&(e.innerHTML="");let o=m=>{let g=m.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let n=(g.title||"Untitled Blueprint").trim(),u=(g.summary||g.body||"No summary provided.").trim(),k=(g.content||"").trim(),$=(g.category||"architecture").trim(),c=(g.related_services||g.affected_services||[]).map(E=>E.trim()),v=(g.implementation_path||[]).map(E=>E.trim()),f=g.source_event_ids||[],r=g.approved===!0,x=new Date(m.timestamp*1e3),z=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),D=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),H=de.has(m.id),M=H?"display: block;":"display: none;",h=document.createElement("div");h.className=`event-item notification-item event-border-purple cursor-pointer ${H?"expanded":""} ${r?"blueprint-approved":""}`,h.dataset.blueprintId=m.id,r&&(h.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",h.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let L=r?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[$]||"bx-paint";h.onclick=function(E){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(m.id);let U=this.querySelector(".event-details");U&&(U.style.display="none")}else{this.classList.add("expanded"),de.add(m.id);let U=this.querySelector(".event-details");U&&(U.style.display="block")}};let _="";v.length>0&&(_=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(E=>`<li style="margin-bottom: 5px;">${B(E)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";f.length>0&&(q=`
            ...
            `;h.innerHTML=`
                ...
                <div class="event-icon" style="${r?"color: #03dac6;":""}"><i class='bx ${L}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${M}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${B(u)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${B(k)}</div>
                        </div>
                        ${_}
                        ${q}
                        ${j}
...
`;async function Ne(){await Promise.all([ce(),pe()])}var vt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
        ...
        <h2>Contacts</h2>
        <button id="contacts-refresh" class="notif-action-btn" style="margin-left: auto; ${I()?"display: none;":""}"><i class='bx bx-refresh'></i> Refresh</button>
    </div>
...
`,ns=null;async function Re(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Re(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=a=>{let i=a.target.closest(".user-profile-card");if(i)try{let s=JSON.parse(i.dataset.user);showUserProfile(s)}catch(s){console.error("Failed to parse user data",s)}},t.dataset.listenerAttached="true");try{let a=await rt("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(ns=Date.now(),s.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((o,b)=>{let d=l[o.level]??10,p=l[b.level]??10;return d!==p?d-p:o.username.localeCompare(b.username)}),t.innerHTML=s.map(o=>{let b=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",p="#888";o.level==="Me"||o.level==="Master"?p="#bb86fc":o.level==="Admin"?p="#cf6679":o.level==="Moderator"?p="#03dac6":o.level==="Contributor"&&(p="#ffa500");let y=o.level==="Me",m=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",g=y?"2px solid #bb86fc":`1px solid ${b}33`;return`
    ...
    <div class="system-section-header" style="margin-bottom: 20px;">
        ...
        <div class="alerts-actions" style="margin-left: auto; display: flex; gap: 10px; padding: 0;">
            ...
            <button id="events-clear" class="notif-action-btn danger" style="${I()?"display: none;":""}"><i class='bx bx-trash'></i> Clear</button>
        </div>
    </div>
...
`,Ie=null,me=new Set,we=[],G="all",as={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},os={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function rs(t){for(let[e,a]of Object.entries(as))if(a.includes(t))return e;return"system"}function ls(t){return os[t]||"bx-square-rounded"}async function ie(t=!1){let e=document.getElementById("events-timeline");if(!e)return;cs();let i=`/events?ml=${G==="all"?100:250}&format=json`;G!=="all"&&(i+=`&category=${G}`);try{let s=await A(i);if(!s.ok)throw new Error("Service unreachable");if(we=((await s.json()).events||[]).filter(g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!0}let u=n.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ie=Date.now(),Z(1,Ie),we.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=g=>{let n=g.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let u=n.type,k=rs(u),$=ls(u),c=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),v="event-border-grey";c&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?v="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"?v="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?v="event-border-orange":u==="system.test.completed"?v=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let f=c?"cursor-pointer":"",r=me.has(g.id),x=r?"expanded":"",z=r?"display: block;":"display: none;",D=new Date(g.timestamp*1e3),H=D.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=D.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),h=xt(u,n),P=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",L="";if(c){let S="";if(u==="engagement.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                    ...
                    `}else if(u==="analysis.user.message_signals"){let w=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`,C=n.signals||{};S=`
                        ...
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${w("Raw Model Output")}
                            <pre class="detail-pre">${B(n.raw_output)||"None"}</pre>
                        </div>
                    ...
                    `}else if(u==="analysis.link.completed"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${B(n.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${w("Description")}
                            <pre class="detail-pre">${B(n.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${w("Content Summary")}
                            <pre class="detail-pre">${B(n.summary)||"None"}</pre>
                        </div>
                    ...
                    `}else if(u==="analysis.router.decision"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                        ...
                        <div class="event-detail-block">
                            ${w("Input Context")}
                            <pre class="detail-pre">${B(n.raw_input)||"None"}</pre>
                        </div>
                    `}else if(u==="system.cli.command"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                        ...
                        <div class="event-detail-block">
                            ${w("Output")}
                            <pre class="detail-pre">${B(n.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(u==="system.analysis.audit"){let w=n.success?"#03dac6":"#ff4d4d",C=n.success?"SUCCESS":"FAILED",T=F=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${F}</h5>`,E="";n.error&&(E=`
                            <div class="event-detail-block">
                                ${T("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${B(n.error)}</pre>
                            </div>
                    ...
                    `}else if(u==="error_occurred"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                        ...
                        <div class="event-detail-block">
                            ${w("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${B(n.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${w("Context")}
                    ...
                    `}else if(u==="system.cli.status"){let w=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`;S=`
                        ...
                        <div class="event-detail-block">
                            ${w("Message")}
                            <pre class="detail-pre">${B(n.message)}</pre>
                        </div>
                ...
                `}let _=document.createElement("div");_.className=`event-item ${v} ${f} ${x}`,_.dataset.eventId=g.id,_.onclick=function(S){if(!c)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),me.delete(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="none")}else{this.classList.add("expanded"),me.add(g.id);let C=this.querySelector(".event-details");C&&(C.style.display="block")}},_.innerHTML=`
                <div class="event-time">
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${o.id}" title="Clear Logs" style="${I()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let b=unescape(o.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let d=o.querySelector("i");d.classList.remove("bx-copy"),d.classList.add("bx-check"),setTimeout(()=>{d.classList.remove("bx-check"),d.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let b=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await A(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&Ce()}catch(d){console.error("Error clearing logs:",d)}})}),ps=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Et=()=>{let t=I()?"display: none;":"";return`
        <div class="system-section-header">
            <i class='bx bxs-pie-chart-alt-2' style="color: #03dac6;"></i>
            <h2>Summary ${I()?'<span style="color: #666; font-size: 0.6em; margin-left: 10px; font-weight: normal; font-style: italic;">* Public data is approximated</span>':""}</h2>
        </div>
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${ht()}
        </div>`;async function Mt(){await Promise.all([$e(),Ue(),qe()]),await Ce()}var wt=null,Ct=null;async function _t(){try{return await(await A("/system_monitor")).json()}catch{return null}}async function $t(){try{return await(await A("/system/hardware")).json()}catch{return null}}async function ms(){try{return await(await A("/processes")).json()}catch{return null}}async function us(){try{return await(await A("/agent/status")).json()}catch{return null}}async function Ue(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),a=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),b=c=>{if(c){if(a&&(a.innerHTML=`
                ...
                </div>`}if(i&&c.CPU&&c.CPU.length>0){let v=c.CPU[0];i.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}l&&(c.GPU&&c.GPU.length>0?l.innerHTML=c.GPU.map(v=>{let f=(v.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),o&&c.STORAGE&&c.STORAGE.length>0?o.innerHTML=c.STORAGE.map(v=>{let f=(v.USED/1073741824).toFixed(1),r=(v.SIZE/(1024*1024*1024)).toFixed(1),x=v.SIZE>0?(v.USED/v.SIZE*100).toFixed(0):0,z=v.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            ...
                            <span style="font-size: 0.75em; color: #666; font-family: monospace; background: rgba(0,0,0,0.2); padding: 2px 6px; border-radius: 4px;">${z}</span>
                        </div>
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await $t();c?(b(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let c=await $t();b(c)}if(!t)return;let d=await _t();if(!d||!d.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));return}wt=Date.now(),Z(0,wt);let p=d.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function y(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function m(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function g(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function n(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let f=parseInt(v[1])||0,r=parseInt(v[2])||0,x=parseInt(v[3])||0,z=parseFloat(v[4])||0,D=f*86400+r*3600+x*60+z,H=Math.floor(D/86400);if(H>0)return`${H}d`;let M=Math.floor(D/3600);if(M>0)return`${M}h`;let h=Math.floor(D/60);return h>0?`${h}m`:`${Math.floor(D)}s`}function u(c){let v=c.status==="online",f=v?"service-widget-online":"service-widget-offline",r=v?"bx-check-circle":"bx-x-circle",x=v?"OK":"BAD",z=c.version?m(c.version.str):"-",D=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",H=c.memory&&c.memory!=="N/A"?c.memory:"-",M=D!=="-"?"#00ff00":"#666",h=D!=="-"?"#fff":"#666",P=H!=="-"?"#00ff00":"#666",L=H!=="-"?"#fff":"#666",_=n(c.uptime),q="";v?q=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${h};">${D}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${P};"></i>
                        <span style="color: ${L};">${H}</span>
                    </div>
                </div>`:q='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let W=I()?"easter.company":g(c.domain&&c.port?`${c.domain}:${c.port}`:"");return`<div class="service-widget ${f}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${r}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${W}</span></div>${q}</div></div>`}let k=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),$=new Set(p.map(c=>c.id));for(let[c,v]of k)$.has(c)||v.remove();p.forEach(c=>{let v=u(c),f=k.get(c.id);f?f.outerHTML!==v&&(f.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}async function qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await _t();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Ct=Date.now(),Z(2,Ct);let a=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function s(d){let p=d.status==="Ready";return`
                ...
                </div>`}function o(d){let p=d.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",g=p&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",n=d.name;return d.type==="custom"&&n.endsWith(":latest")&&(n=n.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${n}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let b="";if(i&&(b+=s(i)),e.xtts&&(b+=l(e.xtts)),b+=a.map(o).join(""),!b){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==b&&(t.innerHTML=b)}async function $e(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),a=document.getElementById("guardian-architect-val"),i=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),b=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn");b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{b.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{b.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{b.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{b.innerHTML="<i class='bx bx-error'></i> Failed"}},b.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await A("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),$e()}catch{d.innerHTML="<i class='bx bx-error'></i> Failed"}},d.dataset.listenerAttached="true");let p=await us();if(p){let $=Math.floor(Date.now()/1e3),c=p.active_tier,v=p.active_synthesis,f=p.protocol_aliases||{sentry:"Sentry",architect:"Architect",synthesis:"Synthesis"},r=h=>{h<0&&(h=0);let P=Math.floor(h/3600),L=Math.floor(h%3600/60),_=h%60;return P>0?`${P}h ${L}m`:L>0?`${L}m ${_}s`:`${_}s`},x=(h,P,L,_,q)=>{if(!h)return;let W=f[_]||_.toUpperCase(),j=h.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(j&&(j.textContent=W),q&&q.includes(_))h.textContent="Working",h.style.color="#bb86fc";else if(L){let w=L.next_run-$;if(w<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let C=Math.floor(w/60),T=w%60;h.textContent=`${C}m ${T}s`,h.style.color="#fff"}}P&&L&&(P.innerHTML=`
          <div style="display: flex; flex-direction: column; gap: 2px;">
            <span>Runs: ${L.attempts||0}</span>
            <span style="color: ${L.failures>0?"#ffa500":"#666"}">Fails: ${L.failures||0}</span>
            <span style="color: ${L.absolute_failures>0?"#ff4d4d":"#666"}">Aborted: ${L.absolute_failures||0}</span>
          </div>
        `)};e&&x(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry",c),a&&x(a,document.getElementById("guardian-architect-stats"),p.architect,"architect",c);let z=document.getElementById("analyzer-synthesis-val"),D=document.getElementById("analyzer-synthesis-stats");z&&x(z,D,p.synthesis,"synthesis",v);let H=document.getElementById("system-state-label"),M=document.getElementById("system-state-val");if(M&&p.system_state){let h=p.system_state,P=r(p.system_state_time||0);H&&(H.textContent=`State: ${h.toUpperCase()}`),M.textContent=P,h==="idle"?M.style.color=p.system_state_time>300?"#5eff5e":"#fff":M.style.color="#bb86fc"}s&&(s.textContent=r(p.total_idle_time||0)),l&&(l.textContent=r(p.total_active_time||0)),o&&(o.textContent=r(p.total_waste_time||0))}else[e,a,i,s,l,o].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});let y=await ms(),m=[],g=[],n=[];y&&(Array.isArray(y)?m=y:(m=y.active||[],g=y.queue||[],n=y.history||[],n.sort(($,c)=>(c.end_time||0)-($.end_time||0)))),m.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=N("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Oe(t,m,!1));let u=document.getElementById("processes-queue-widgets");u&&(g.length===0?!u.querySelector(".tab-placeholder")&&!u.querySelector('div[style*="font-style: italic"]')&&(u.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(u.innerHTML="",Oe(u,g,!1)));let k=document.getElementById("processes-history-widgets");k&&(n.length===0?k.querySelector(".tab-placeholder")||(k.innerHTML=N("empty","No recent history.")):(k.querySelector(".tab-placeholder")&&(k.innerHTML=""),Oe(k,n,!0))),he(1,m.length)}function Oe(t,e,a){function i(b){let d="";b.end_time?d=`${b.end_time-b.start_time}s`:d=`${Math.floor(Date.now()/1e3-b.start_time)}s`;let p=b.retries>0?`<span class="process-retry-badge">Retry ${b.retries}</span>`:"",y=b.channel_id,m={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(m[y]?y=m[y]:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`
                ...
                <div class="service-widget process-widget" data-channel-id="${b.channel_id}-${b.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${g}">${d}</span>
                        </div>
                         <div class="service-widget-info">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],gs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),Fe=!0,a}function It(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),Fe=!1}function Bt(t,e,a="output"){if(!Fe)return;let i=document.createElement("div");i.className=`terminal-line terminal-${a}`,a==="prompt"?i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:i.innerHTML=Le(e),t.appendChild(i),t.scrollTop=t.scrollHeight}async function vs(t){let e=Dt.find(s=>s.id===t);if(!e)return;let a=bs(e);Bt(a,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let s of i){await new Promise(o=>setTimeout(o,400+Math.random()*600));let l="output";s.includes("[ERROR]")?l="error":s.includes("[SUCCESS]")||s.includes("\u2713")?l="success":s.includes("!")&&(l="warning"),Bt(a,s,l)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function zt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=gs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code").textContent;navigator.clipboard.writeText(a).then(()=>{let i=e.querySelector("i");i.className="bx bx-check",i.style.color="#5eff5e",setTimeout(()=>{i.className="bx bx-copy",i.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(a=>{a.addEventListener("mouseenter",()=>{a.style.transform="translateY(-5px)",a.style.borderColor="rgba(255,255,255,0.15)",a.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{a.style.transform="translateY(0)",a.style.borderColor="rgba(255,255,255,0.05)",a.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let i=a.dataset.cmd;vs(i)})})}async function xs(){try{if(!(await A("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Ht(){xs(),tt(),Ve();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&zt(),Ze(),Ye();let e=document.getElementById("nav-left-container");e&&e.addEventListener("click",()=>{if(s.length>0)y();else{let r=window.location.pathname;if(!(r==="/"||r==="/index.html")){let D=(r.endsWith("/")&&r.length>1?r.slice(0,-1):r).split("/");D.pop();let H=D.join("/")||"/";window.location.href=H}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",i=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!i&&document.querySelector("footer")?.classList.add("hide");let s=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let o=r=>{localStorage.setItem("dex_last_window",r)};function b(){return 1}function d(){for(;s.length>1;)s.shift().close(!0);let r=document.getElementById("windows-container"),x=document.querySelector("nav"),z=document.querySelector("footer"),D=window.location.pathname==="/"||window.location.pathname==="/index.html",H=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",s.length),s.forEach(L=>{let _=document.getElementById(L.id);_&&_.classList.remove("hide-close")}),Ee(ye());let M=document.getElementById("dexter-menu-container"),h=document.getElementById("nav-window-switcher"),P=document.getElementById("settings-icon");if(Ke(s.length>0),s.length>0){if(z?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),x?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),M&&(M.style.display="none"),P&&(P.style.display="none"),h){let L=s[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(L)?(h.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{o("alerts-window"),p(m)}),document.getElementById("switch-events").addEventListener("click",()=>{o("events-window"),p(g)}),document.getElementById("switch-monitor").addEventListener("click",()=>{o("monitor-window"),p(u)}),document.getElementById("switch-contacts").addEventListener("click",()=>{o("contacts-window"),p(n)}),document.getElementById("switch-workspace").addEventListener("click",()=>{o("workspace-window"),p(k)})):h.innerHTML=""}}else x?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),r&&(r.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),D||H?z?.classList.remove("hide"):z?.classList.add("hide"),M&&(M.style.display="flex"),P&&(P.style.display="block"),h&&(h.innerHTML="");be()}function p(r){if(r.isOpen()){r.close();return}for(;s.length>0;)s.pop().close(!0);s.push(r),r.open(),d()}function y(){[...s].forEach(x=>x.close()),s=[]}window.addEventListener("resize",d);let m=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:lt(),onOpen:()=>ne(),onClose:()=>{let r=s.indexOf(m);r>-1&&s.splice(r,1),d()}}),g=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Pe(),onOpen:()=>{g.setContent(Pe()),ie()},onClose:()=>{let r=s.indexOf(g);r>-1&&s.splice(r,1),d()}}),n=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:vt(),onOpen:()=>Re(),onClose:()=>{let r=s.indexOf(n);r>-1&&s.splice(r,1),d()}}),u=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Tt()},{icon:"bxs-component",title:"Processes",content:St()},{icon:"bxs-brain",title:"Models",content:kt()},{icon:"bxs-hdd",title:"Hardware",content:Lt()},{icon:"bxs-terminal",title:"Logs",content:At()},{icon:"bxs-zap",title:"Agents",content:Et()}].filter(r=>I()?r.title!=="Hardware"&&r.title!=="Logs":!0),onOpen:()=>{Ue(),$e(),qe(),Ce()},onClose:()=>{let r=s.indexOf(u);r>-1&&s.splice(r,1),d()}}),k=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:bt(),onOpen:()=>Ne(),onClose:()=>{let r=s.indexOf(k);r>-1&&s.splice(r,1),d()}}),$=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:Be(),onOpen:()=>{$.setContent(Be()),je($)},onClose:()=>{let r=s.indexOf($);r>-1&&s.splice(r,1),d()}});window.dexter={viewEvent:r=>{g.isOpen()||p(g),setTimeout(()=>{let x=document.querySelector(`.event-item[data-event-id="${r}"]`);x&&(x.scrollIntoView({behavior:"smooth",block:"center"}),x.classList.add("flash-highlight"),x.classList.contains("expanded")||x.click(),setTimeout(()=>x.classList.remove("flash-highlight"),2e3))},500)},viewAlert:r=>{m.isOpen()||p(m),setTimeout(()=>{let x=document.querySelector(`.event-item[data-alert-id="${r}"]`);x&&(x.scrollIntoView({behavior:"smooth",block:"center"}),x.classList.add("flash-highlight"),x.classList.contains("expanded")||x.click(),setTimeout(()=>x.classList.remove("flash-highlight"),2e3))},500)}};let c=document.getElementById("dexter-dropdown");c&&(c.innerHTML=`
          ...
          <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
      `);let v=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");v&&c&&f&&(f.addEventListener("click",r=>{r.stopPropagation();let x=c.classList.contains("active");c.classList.toggle("active"),f.classList.toggle("active")}),document.addEventListener("click",()=>{c.classList.remove("active"),f.classList.remove("active")}),c.addEventListener("click",r=>{r.stopPropagation()})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{o("alerts-window"),p(m)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{o("events-window"),p(g)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{o("monitor-window"),p(u)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{o("contacts-window"),p(n)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{o("workspace-window"),p(k)}),document.getElementById("settings-icon")?.addEventListener("click",()=>p($)),document.getElementById("close-all-windows")?.addEventListener("click",()=>y()),I()||setInterval(()=>{g.isOpen()&&ie(),m.isOpen()?ne():He()},1e3),setInterval(()=>{I()&&(m.isOpen()?ne():He()),k.isOpen()?Ne():gt(),I()&&g.isOpen()&&ie(),u.isOpen()&&Mt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ht):Ht();})();

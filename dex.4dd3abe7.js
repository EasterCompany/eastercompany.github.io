(()=>{function Ve(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function We(t=!1){let e=window.location.pathname,s=e==="/"||e==="/index.html",o=`
        <div class="nav-left" id="nav-left-wrapper" style="gap: 0;">
            ${`
        <div id="nav-left-container" style="display: flex; align-items: center; transition: transform 0.2s ease; gap: 5px; cursor: ${s?"default":"pointer"};">
            <i class='bx bx-chevron-left' id="nav-chevron" style="font-size: 28px; color: #fff; display: ${s?"none":"block"};"></i>
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ye=1e3;function te(t){let e=null,s=t.onClose||null,a=t.onOpen||null;function n(){e&&(e.style.zIndex=++Ye)}function o(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),a&&setTimeout(a,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ye,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",i="",f="",w;t.tabs&&t.tabs.length>0?(i=`<div class="tab-bar">${t.tabs.map((v,r)=>{let d=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,w=`<div class="window-content">${t.tabs.map((v,r)=>`<div class="tab-content ${r===0?"active":""}" data-tab-content="${r}">${v.content}</div>`).join("")}</div>`):(t.title&&(f=`<div class="window-title">${t.title}</div>`),w=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${w}
        `,u.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",y=>{y.stopPropagation(),p()}),window.addEventListener("resize",l),t.tabs&&e.querySelectorAll(".tab").forEach(y=>{y.addEventListener("click",()=>{let c=y.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(v=>v.classList.remove("active")),y.classList.add("active"),e.querySelectorAll(".tab-content").forEach(v=>v.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${c}"]`).classList.add("active"),m(y,e)})}),setTimeout(()=>{e.classList.add("open"),a&&a()},10)}function l(){if(!e||!e.classList.contains("open"))return;let u=e.querySelector(".tab.active");u&&m(u,e)}function m(u,b){setTimeout(()=>{let i=b.querySelector(".tab-bar");if(!i)return;let f=Array.from(i.querySelectorAll(".tab")),w=f.indexOf(u),y=i.clientWidth,c=f[Math.max(0,w-2)],v=f[Math.min(f.length-1,w+2)],r=c.offsetLeft-i.offsetLeft-25,h=v.offsetLeft+v.offsetWidth-i.offsetLeft+25-r,C=h<=y?r-(y-h)/2:u.offsetLeft-i.offsetLeft-y/2+u.offsetWidth/2;i.scrollTo({left:C,behavior:"smooth"})},350)}function p(u=!1){e&&(window.removeEventListener("resize",l),u?(e.remove(),e=null):(e.classList.remove("open"),s&&s(),setTimeout(()=>{e?.remove(),e=null},400)))}function g(u){let b=e?.querySelector(".window-content");b&&(b.innerHTML=u)}function x(){return e&&e.classList.contains("open")}return{open:o,close:p,setContent:g,isOpen:x,focus:n,id:t.id}}function Xe(){return!0}function Qe(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function et(t){localStorage.setItem("easter_user_email",t.trim())}var tt="easter_theme",Z={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function ve(){return localStorage.getItem(tt)||Z.DARK}function st(t){if(!Object.values(Z).includes(t))throw new Error("Invalid theme");localStorage.setItem(tt,t),we(t)}function we(t){let e=document.body;if(Object.values(Z).forEach(a=>{e.classList.remove(`theme-${a}`)}),e.classList.add(`theme-${t}`),[Z.LIGHT,Z.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let a=document.createElement("div");a.className="background",document.body.prepend(a)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function it(){let t=ve();we(t)}function D(t,e,s=null){let n={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",o=s?`<p class="placeholder-action">${s}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${n} placeholder-icon'></i><p class="placeholder-message">${e}</p>${o}</div>`}function B(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function J(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!s)return;if(!e){s.textContent="Last updated: never";return}let a=Date.now(),n=Math.floor((a-e)/1e3),o;n<60?o=`${n}s ago`:n<3600?o=`${Math.floor(n/60)}m ago`:o=`${Math.floor(n/3600)}h ago`,s.textContent=`Last updated: ${o}`}var jt=0;function xe(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!s)return;let a=s.querySelector(".notification-badge");a&&(e>0?(a.textContent=e>9?"9+":e,a.style.display="flex"):a.style.display="none")}var K=0,Y=0;function nt(t){K=t,fe()}function at(t){Y=t,fe()}function fe(){let t=K+Y;jt=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let s=document.getElementById("alerts-menu-item");if(s){let l=s.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="auto",s.appendChild(l)),l.textContent=K>9?"9+":K,l.style.display=K>0?"flex":"none"}let a=document.getElementById("workspace-menu-item");if(a){let l=a.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="auto",a.appendChild(l)),l.textContent=Y>9?"9+":Y,l.style.display=Y>0?"flex":"none"}let n=document.getElementById("switch-alerts");if(n){let l=n.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="8px",n.appendChild(l)),l.textContent=K>9?"9+":K,l.style.display=K>0?"flex":"none"}let o=document.getElementById("switch-workspace");if(o){let l=o.querySelector(".notification-badge");l||(l=document.createElement("span"),l.className="notification-badge",l.style.marginLeft="8px",o.appendChild(l)),l.textContent=Y>9?"9+":Y,l.style.display=Y>0?"flex":"none"}}function Ee(){let t=document.getElementById("alerts-list");if(!t)return;K=t.querySelectorAll(".alert-unread:not(.priority-low)").length,fe()}function ot(){let t=document.getElementById("blueprints-list");if(!t)return;Y=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,fe()}function De(){return"https://event.easter.company"}function qt(){return"https://discord.easter.company"}var Ft="http://127.0.0.1:8100",Gt="http://127.0.0.1:8300",Jt={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function ke(t){let e=B(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(n,o)=>{let l=Jt[o];return l?`<span class="${l}">`:""});let s=(e.match(/<span/g)||[]).length,a=(e.match(/<\/span/g)||[]).length;return s>a&&(e+="</span>".repeat(s-a)),e}function se(t){if(!t)return"";let e=B(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(s,a)=>{let n=a.split("|").map(o=>o.trim());return n.every(o=>o.match(/^:?-+:?$/))?"":`<div class="md-table-row">${n.map(o=>`<span class="md-table-cell">${o}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var me=null,ue=null,Ce=!1,$e=!1;async function _(t,e={}){if(me)try{let n=await fetch(me+t,e);if(n.ok)return n;me=null}catch{me=null}let s=De(),a=Ft;try{let n=await fetch(s+t,e);if(n.ok)return me=s,Ce&&(console.log("\u2728 Production event service restored."),Ce=!1),n;throw new Error("Primary failed")}catch{Ce||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${a}`),Ce=!0);try{let o=await fetch(a+t,e);if(o.ok)return me=a,o;throw new Error("Fallback failed")}catch(o){throw o}}}async function Se(t,e={}){if(ue)try{let n=await fetch(ue+t,e);if(n.ok)return n;ue=null}catch{ue=null}let s=qt(),a=Gt;try{let n=await fetch(s+t,e);if(n.ok)return ue=s,$e&&(console.log("\u2728 Production discord service restored."),$e=!1),n;throw new Error("Primary failed")}catch{$e||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${a}`),$e=!0);try{let o=await fetch(a+t,e);if(o.ok)return ue=a,o;throw new Error("Fallback failed")}catch(o){throw o}}}var rt=()=>`
...
`,Te=null,ie=new Set,Le=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Vt(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let a=await _(s);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];Te=Date.now(),J(0,Te);let l=Date.now(),m=24*60*60*1e3,p=o.filter(r=>{let d=localStorage.getItem(`alert_read_ts_${r.id}`);if(!d)return!0;let h=parseInt(d);return l-h<m});p.sort((r,d)=>{let h=I=>{let S=I.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},C=I=>I==="critical"?4:I==="high"?3:I==="medium"?2:1,L=C(h(r)),k=C(h(d));return L!==k?k-L:d.timestamp-r.timestamp}),Le=p;let g=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},x=[],b=new Set(p.map(r=>g(r))).size>1;if(p.length>0){let r=null;p.forEach(d=>{let h=g(d);b&&h!==r&&(x.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),r=h),x.push(d)})}if(t&&(e.innerHTML=""),p.length===0){e.innerHTML=D("empty","No alerts yet."),Ee();return}let i=r=>{let d=r.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let h=(d.title||"Untitled Alert").trim(),C=(d.body||"No description provided.").trim(),L=d.summary||"",k=d.content||"",I=d.protocol||"",S=(d.priority||"low").toLowerCase(),N=!!d.alert,O=(d.category||"system").trim(),H=d.related_event_ids||[],P=d.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${r.id}`),T=new Date(r.timestamp*1e3),$=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),E=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=F?"event-border-grey":"event-border-blue";!F&&N&&(M="event-border-red"),F&&(S==="high"||S==="critical")?M="event-border-red":F&&S==="medium"&&(M="event-border-orange");let A=F?"alert-read":"alert-unread",z=ie.has(r.id),R=z?"expanded":"",X=z?"display: block;":"display: none;",q="",U="";H.length>0&&(U=`
        ...
        `,q=`
                ...
                ${W}
            `;let G=document.createElement("div");G.className=`event-item notification-item ${M} ${A} ${R} cursor-pointer priority-${S}`,G.dataset.alertId=r.id,G.onclick=function(ee){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ie.delete(r.id);let be=this.querySelector(".event-details");be&&(be.style.display="none")}else{this.classList.add("expanded"),ie.add(r.id);let be=this.querySelector(".event-details");if(be&&(be.style.display="block"),!localStorage.getItem(`alert_read_ts_${r.id}`)){localStorage.setItem(`alert_read_ts_${r.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let _e="event-border-grey";S==="high"||S==="critical"?_e="event-border-red":S==="medium"&&(_e="event-border-orange"),this.classList.add(_e),Ee()}}};let Fe=`${I?I.toUpperCase():"GUARDIAN"} ALERT: ${L||h}`,Ot={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[O]||"bx-bell",Ut=S==="high"||S==="critical"?"#ff4d4d":S==="medium"?"#ffa500":"#888";G.innerHTML=`
            ...
            `;let Ge=G.querySelector(".event-details");Ge&&Ge.addEventListener("click",ee=>{ee.stopPropagation()});let Je=G.querySelector(".close-details-btn");return Je&&Je.addEventListener("click",ee=>{ee.stopPropagation(),G.classList.remove("expanded");let Be=G.querySelector(".event-details");Be&&(Be.style.display="none"),ie.delete(r.id)}),G},f=r=>{let d=document.createElement("div");d.className="notification-divider",d.dataset.alertId=r.id;let h="#888888";return r.label==="CRITICAL"?h="#ff4d4d":r.label==="HIGH"?h="#ff8888":r.label==="MEDIUM"&&(h="#ffa500"),d.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,d.innerHTML=`<span style="white-space: nowrap;">${r.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,d},w=Array.from(e.children),y=new Map(w.map(r=>[r.dataset.alertId,r])),c=new Set(x.map(r=>r.id));w.forEach(r=>{let d=r.dataset.alertId;(!d||!c.has(d))&&r.remove()});let v=null;x.forEach((r,d)=>{let h=y.get(r.id);(!h||t)&&(h&&h.remove(),r.type==="divider"?h=f(r):h=i(r),!h)||(d===0?e.firstElementChild!==h&&e.prepend(h):v&&v.nextElementSibling!==h&&v.after(h),v=h)}),Te=Date.now(),J(0,Te),Ee()}catch(a){console.error("Error fetching alerts:",a),e.children.length===0&&(e.innerHTML=D("offline","Failed to load alerts.","The event service may be offline."))}}function Vt(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),s=document.getElementById("alerts-close-all"),a=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Le.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Le.forEach(n=>{ie.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{ie.clear(),ne(!0)},s.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Le.forEach(o=>{localStorage.setItem(`alert_read_ts_${o.id}`,n.toString())}),ie.clear(),ne(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}async function lt(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await _(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(o=>{let l=o.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}if((l.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${o.id}`)||n++}),nt(n)}catch{}}var ct=()=>`
...
`,ae=new Set,pt=[],ge=null;async function oe(t=!1){let e=document.getElementById("roadmap-list");if(e){Wt();try{let s=await _("/roadmap");if(!s.ok)throw new Error("Offline");let a=await s.json();pt=a;let n=g=>{let x=ae.has(g.id),u=g.state==="published",b=g.state==="consumed",i="event-border-grey";u&&(i="event-border-blue"),b&&(i="event-border-purple");let w=new Date(g.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),y=document.createElement("div");y.className=`event-item notification-item ${i} cursor-pointer ${x?"expanded":""}`,y.dataset.itemId=g.id,y.onclick=r=>{if(r.target.closest("button"))return;y.classList.contains("expanded")?(y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(g.id)):(y.classList.add("expanded"),y.querySelector(".event-details").style.display="block",ae.add(g.id))};let c=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${g.state.toUpperCase()}]</span>`;y.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${B(g.content)}</div>
      ...
      `;let v=y.querySelector(".event-details");return v&&v.addEventListener("click",r=>{r.stopPropagation()}),y.querySelector(".edit-btn")?.addEventListener("click",()=>Zt(g)),y.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Kt(g)),y.querySelector(".delete-btn")?.addEventListener("click",()=>Yt(g.id)),y.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),y.classList.remove("expanded"),y.querySelector(".event-details").style.display="none",ae.delete(g.id)}),y},o=Array.from(e.children),l=new Map(o.map(g=>[g.dataset.itemId,g])),m=new Set(a.map(g=>g.id));if(o.forEach(g=>{let x=g.dataset.itemId;(!x||!m.has(x))&&g.remove()}),a.length===0){e.innerHTML=D("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let p=null;a.forEach((g,x)=>{let u=l.get(g.id);(!u||t)&&(u&&u.remove(),u=n(g),!u)||(x===0?e.firstElementChild!==u&&e.prepend(u):p&&p.nextElementSibling!==u&&p.after(u),p=u)})}catch{e.children.length===0&&(e.innerHTML=D("offline","Failed to load roadmap.","The event service may be offline."))}}}function Wt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),a=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ge=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ge=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-input").value;if(!o.trim())return;let l=ge?`/roadmap/${ge}`:"/roadmap",m=ge?"PATCH":"POST";try{await _(l,{method:m,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})}),document.getElementById("roadmap-editor-container").style.display="none",oe(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{pt.forEach(o=>ae.add(o.id)),oe(!0)},a.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ae.clear(),oe(!0)},n.dataset.listenerAttached="true")}function Zt(t){ge=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Kt(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),oe(!0)}catch(s){console.error(s)}}async function Yt(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),ae.delete(t),oe(!0)}catch(e){console.error(e)}}var ut=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,mt=null,re=new Set,gt=[];async function le(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Xt();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let a=await _(s);if(!a.ok)throw new Error("Service is offline or unreachable.");let o=(await a.json()).events||[];if(gt=o,mt=Date.now(),J(2,mt),o.length===0){e.innerHTML=D("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),xe(2,0);return}t&&(e.innerHTML="");let l=u=>{let b=u.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let i=(b.title||"Untitled Blueprint").trim(),f=(b.summary||b.body||"No summary provided.").trim(),w=(b.content||"").trim(),y=(b.category||"architecture").trim(),c=(b.related_services||b.affected_services||[]).map(A=>A.trim()),v=(b.implementation_path||[]).map(A=>A.trim()),r=b.source_event_ids||[],d=b.approved===!0,h=new Date(u.timestamp*1e3),C=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=re.has(u.id),I=k?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${d?"blueprint-approved":""}`,S.dataset.blueprintId=u.id,d&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let O=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[y]||"bx-paint";S.onclick=function(A){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(u.id);let R=this.querySelector(".event-details");R&&(R.style.display="none")}else{this.classList.add("expanded"),re.add(u.id);let R=this.querySelector(".event-details");R&&(R.style.display="block")}};let H="";v.length>0&&(H=`
                ...
                `);let P="";r.length>0&&(P=`
                    <div class="blueprint-source" style="margin-top: 15px;">
                        ...
                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                            ${r.map(A=>`
                                <a href="#" onclick="window.dexter.viewEvent('${A}'); return false;" style="color: #03dac6; text-decoration: none; font-size: 0.75em; font-family: 'JetBrains Mono', monospace; padding: 4px 8px; background: rgba(3, 218, 198, 0.05); border: 1px solid rgba(3, 218, 198, 0.1); border-radius: 4px;">
                                    <i class='bx bx-link-external'></i> ${A.substring(0,8)}...
                                </a>
                            `).join("")}
                        </div>
                    </div>
            ...
            `;S.innerHTML=`
                ...
                <div class="event-icon" style="${d?"color: #03dac6;":""}"><i class='bx ${O}'></i></div>
                <div class="event-content">
            ...
            `;let T=S.querySelector(".blueprint-approve-btn");T&&(T.onclick=async A=>{A.stopPropagation(),T.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${u.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&le(!0)}catch(z){console.error("Failed to approve blueprint:",z)}});let $=S.querySelector(".blueprint-delete-btn");$&&($.onclick=async A=>{A.stopPropagation();let z=!d;$.innerHTML=z?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${u.id}`,{method:"DELETE"})).ok&&le(!0)}catch(R){console.error("Failed to delete blueprint:",R)}});let E=S.querySelector(".event-details");E&&E.addEventListener("click",A=>{A.stopPropagation()});let M=S.querySelector(".close-details-btn");return M&&M.addEventListener("click",A=>{A.stopPropagation(),S.classList.remove("expanded");let z=S.querySelector(".event-details");z&&(z.style.display="none"),re.delete(u.id)}),S},m=Array.from(e.children),p=new Map(m.map(u=>[u.dataset.blueprintId,u])),g=new Set(o.map(u=>u.id));m.forEach(u=>{let b=u.dataset.blueprintId;(!b||!g.has(b))&&u.remove()});let x=null;o.forEach((u,b)=>{let i=p.get(u.id);(!i||t)&&(i&&i.remove(),i=l(u),!i)||(b===0?e.firstElementChild!==i&&e.prepend(i):x&&x.nextElementSibling!==i&&x.after(i),x=i)}),xe(2,o.length),ot()}catch(a){console.error("Error fetching blueprints:",a),e.children.length===0&&(e.innerHTML=D("offline","Failed to load blueprints.","The event service may be offline."))}}async function bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let a=(await e.json()).events||[],n=0;a.forEach(o=>{let l=o.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),at(n)}catch{}}function Xt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{gt.forEach(a=>re.add(a.id)),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{re.clear(),le(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),re.clear(),le(!0)}catch(a){console.error("Failed to clear blueprints:",a)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var vt=()=>`
...
`;async function He(){await Promise.all([oe(),le()])}var Qt=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function xt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"VS Code"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function yt(t){if(!document.getElementById("dex-profile-styles")){let s=document.createElement("style");s.id="dex-profile-styles",s.textContent=Qt,document.head.appendChild(s)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",s=>{s.target===e&&(e.classList.remove("active"),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),Se(`/profile/${t.id}`).then(async s=>{if(s.ok){let a=await s.json();ze(e,t,a)}else{console.log("Profile not found or error, using mock data.");let a=xt(t.id,t.username);ze(e,t,a)}}).catch(s=>{console.error("Profile fetch error:",s);let a=xt(t.id,t.username);ze(e,t,a)})}function ze(t,e,s){let a=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let w=(s.badges||[]).map(C=>`<span class="profile-badge ${C==="Creator"?"master":""}">${C}</span>`).join(""),y=(s.attributes||s.facts||[]).map(C=>{let L=C.key||C.k,k=C.value||C.v;return`
            <div class="fact-chip">
                <span class="fact-key">${L}:</span>
                <span class="fact-val">${k}</span>
            </div>
        `}).join(""),c=s.cognitive_model||s,v=c.technical_level||c.techLevel||0,r=c.communication_style||c.commStyle||"Unknown",d=c.patience_level||c.patience||"Unknown",h=c.vibe||"Unknown";return`
        ...
        `},o=()=>{let w=s.dossier||{},y=w.identity||{},c=w.career||{},v=w.personal||{},r=w.social||[];return`
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${B(e.username)}</h2>
                    <div class="profile-badges">${(s.badges||[]).map(w=>`<span class="profile-badge ${w==="Creator"?"master":""}">${w}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=g;let x=t.querySelector(".profile-card"),u=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),i=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),setTimeout(()=>t.remove(),300)}),b.forEach(w=>{w.addEventListener("click",()=>{b.forEach(y=>y.classList.remove("active")),i.forEach(y=>y.classList.remove("active")),w.classList.add("active"),t.querySelector(`#tab-${w.dataset.tab}`).classList.add("active")})}),u.addEventListener("click",()=>{x.classList.toggle("expanded");let w=x.classList.contains("expanded");u.innerHTML=w?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var ht=()=>`
...
`,es=null;async function Ne(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...",await Ne(),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let a=s.target.closest(".user-profile-card");if(a)try{let n=JSON.parse(a.dataset.user);yt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let s=await Se("/contacts");if(!s.ok)throw new Error("Service unreachable");let n=(await s.json()).members||[];if(es=Date.now(),n.length===0){t.innerHTML=D("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,m)=>{let p=o[l.level]??10,g=o[m.level]??10;return p!==g?p-g:l.username.localeCompare(m.username)}),t.innerHTML=n.map(l=>{let m=l.color?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",g="#888";l.level==="Me"||l.level==="Master"?g="#bb86fc":l.level==="Admin"?g="#cf6679":l.level==="Moderator"?g="#03dac6":l.level==="Contributor"&&(g="#ffa500");let x=l.level==="Me",u=x?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=x?"2px solid #bb86fc":`1px solid ${m}33`;return`
...
`,Me=null,ce=new Set,ye=[],j="all",ss={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision"],moderation:["moderation.explicit_content.deleted"]},is={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ns(t){for(let[e,s]of Object.entries(ss))if(s.includes(t))return e;return"system"}function as(t){return is[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;os();let a=`/events?ml=${j==="all"?100:250}&format=json`;j!=="all"&&(a+=`&category=${j}`);try{let n=await _(a);if(!n.ok)throw new Error("Service unreachable");if(ye=((await n.json()).events||[]).filter(b=>{let i=b.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return!0}let f=i.type;return!(f==="system.blueprint.generated"||f==="system.notification.generated")}),Me=Date.now(),J(1,Me),ye.length===0){e.innerHTML=D("empty","No events found for this filter.");return}t&&(e.innerHTML="");let m=b=>{let i=b.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{return null}let f=i.type,w=ns(f),y=as(f),c=f==="engagement.decision"||f==="messaging.bot.sent_message"||f==="messaging.user.sent_message"||f==="moderation.explicit_content.deleted"||f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.cli.command"||f==="system.analysis.audit"||f==="system.test.completed"||f==="error_occurred"||f==="system.cli.status"||f==="system.attention.expired"||f.startsWith("system.roadmap")||f.startsWith("system.process"),v="event-border-grey";c&&(f==="moderation.explicit_content.deleted"||f==="error_occurred"?v="event-border-red":f==="analysis.link.completed"||f==="analysis.visual.completed"||f==="analysis.router.decision"||f==="system.analysis.audit"?v="event-border-purple":f==="system.attention.expired"||f==="system.cli.command"||f==="system.cli.status"?v="event-border-orange":f==="system.test.completed"?v=i.test?.status==="OK"&&i.lint?.status==="OK"&&i.format?.status==="OK"?"event-border-blue":"event-border-red":v="event-border-blue");let r=c?"cursor-pointer":"",d=ce.has(b.id),h=d?"expanded":"",C=d?"display: block;":"display: none;",L=new Date(b.timestamp*1e3),k=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=wt(f,i),N=i.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${i.user_level})</span>`:"",O="";if(c){let T="";if(f==="engagement.decision"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                    ...
                    `}else if(f==="system.attention.expired"){let $=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,E=i.timestamp-i.last_active,M=Math.floor(E/60);T=`
                    ...
                    `}else if(f==="messaging.bot.sent_message"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,E="";if(i.chat_history&&i.chat_history.length>0){let M=i.chat_history.length,A=i.chat_history.map((z,R)=>{let X=z.role.toUpperCase();X==="ASSISTANT"&&(X="DEXTER");let q=z.role==="user"?"#03dac6":z.role==="system"?"#ffb74d":"#bb86fc",U=R===M-1?"block":"none";return`
                                <div class="history-slide" data-index="${R}" style="display: ${U};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${R+1} of ${M}</span>
                                    </div>
                        ...
                        `}
                    `}else if(f==="moderation.explicit_content.deleted"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="analysis.link.completed"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="analysis.visual.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,E="";i.base64_preview?E=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${i.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:i.url&&(E=`<div class="event-detail-block"><img src="${i.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="analysis.router.decision"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.cli.command"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="system.analysis.audit"){let $=i.success?"#03dac6":"#ff4d4d",E=i.success?"SUCCESS":"FAILED",M=q=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${q}</h5>`,A="";i.error&&(A=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${B(i.error)}</pre>
                            </div>
                        `);let z="";if(i.chat_history&&i.chat_history.length>0){let q=i.chat_history.length,U=i.chat_history.map((Q,pe)=>{let W=Q.role.toUpperCase();W==="USER"&&(W="SYSTEM"),W==="ASSISTANT"&&(W="AGENT");let G=Q.role==="user"?"#03dac6":Q.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let R="";if(i.corrections&&i.corrections.length>0){let q=i.corrections.map((U,Q)=>`
                        ...
                        `).join("");R=`
                            <div class="event-detail-block">
                                ${M(`Corrections (${i.corrections.length})`)}
                                ${q}
                            </div>
                        `}let X="";if(i.parsed_results&&i.parsed_results.length>0){let q=i.parsed_results.map(U=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${E} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${i.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${R}
                        ${z}
                    `}else if(f==="system.test.completed"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(f==="error_occurred"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(f==="system.cli.status"){let $=E=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${E}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(f==="messaging.user.sent_message"){let $="";i.attachments&&i.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${i.attachments.map(M=>{let A=M.content_type&&M.content_type.startsWith("image/"),z=(M.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${A?`<div class="attachment-thumb-container"><img src="${M.url}" alt="${M.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                ...
                `}let H=document.createElement("div");H.className=`event-item ${v} ${r} ${h}`,H.dataset.eventId=b.id,H.onclick=function(T){if(!c)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),ce.delete(b.id);let E=this.querySelector(".event-details");E&&(E.style.display="none")}else{this.classList.add("expanded"),ce.add(b.id);let E=this.querySelector(".event-details");E&&(E.style.display="block")}},H.innerHTML=`
                <div class="event-time">
            ...
            `;let P=H.querySelector(".event-details");if(P&&P.addEventListener("click",T=>{T.stopPropagation()}),c){let T=H.querySelector(".close-details-btn");T&&T.addEventListener("click",$=>{$.stopPropagation();let E=$.target.closest(".event-item");if(E){E.classList.remove("expanded"),ce.delete(b.id);let M=E.querySelector(".event-details");M&&(M.style.display="none")}})}let V=H.querySelector(".prev-btn"),F=H.querySelector(".next-btn");if(V&&F){let T=0,$=H.querySelectorAll(".history-slide"),E=$.length,M=()=>{$.forEach((A,z)=>{A.style.display=z===T?"block":"none"}),V.disabled=T===0,F.disabled=T===E-1,V.style.opacity=T===0?"0.5":"1",F.style.opacity=T===E-1?"0.5":"1"};V.addEventListener("click",A=>{A.stopPropagation(),T>0&&(T--,M())}),F.addEventListener("click",A=>{A.stopPropagation(),T<E-1&&(T++,M())}),M()}return H},p=Array.from(e.children),g=new Map(p.map(b=>[b.dataset.eventId,b])),x=new Set(ye.map(b=>b.id));p.forEach(b=>{let i=b.dataset.eventId;(!i||!x.has(i))&&b.remove()});let u=null;ye.forEach((b,i)=>{let f=g.get(b.id);(!f||t)&&(f&&f.remove(),f=m(b),!f)||(i===0?e.firstElementChild!==f&&e.prepend(f):u&&u.nextElementSibling!==f&&u.after(f),u=f)}),Me=Date.now(),J(1,Me)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=D("offline","Failed to load events.","The event service may be offline."))}}function os(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),s=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ye.forEach(n=>ce.add(n.id)),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ce.clear(),de(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{s.querySelectorAll(".filter-btn").forEach(o=>o.classList.remove("active")),n.classList.add("active"),j=n.dataset.filter,de(!0)}}),s.dataset.listenerAttached="true"),s&&s.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===j?n.classList.add("active"):n.classList.remove("active")});let a=document.getElementById("events-clear");a&&!a.dataset.listenerAttached&&(a.onclick=async()=>{let n=j==="all"?"ALL":j.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){a.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let o="/events";if(j!=="all"?o+=`?category=${j}`:o+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await _(o,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ce.clear(),de(!0)}catch(o){console.error("Failed to clear events:",o),alert("Failed to clear events. Check console.")}finally{a.innerHTML="<i class='bx bx-trash'></i> Clear"}}},a.dataset.listenerAttached="true")}var rs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Ct=rs;function $t(){return`
    ...
    `}var ls=null;async function he(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let s=await e.json();if(!s||s.length===0)return t.innerHTML=D("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let a=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],n=s.filter(l=>!a.includes(l.id));n.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),n.reverse();let o=n.map(l=>{let m=l.logs.join(`
`),p=[...l.logs];if(p.length<25){let x=25-p.length;for(let u=0;u<x;u++)p.push("")}else p.length>25&&(p=p.slice(-25));let g=p.map(x=>ke(x)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${$t()}
        </div>`;async function _t(){await Promise.all([Ae(),Oe(),Ue()]),await he()}var Et=null,kt=null;async function Dt(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function St(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function cs(){try{return await(await _("/processes")).json()}catch{return null}}async function ds(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Oe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),a=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),o=document.querySelector("#hw-gpu .hw-content"),l=document.querySelector("#hw-storage .hw-content"),m=c=>{if(c){if(s&&(s.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),l&&c.STORAGE&&c.STORAGE.length>0?l.innerHTML=c.STORAGE.map(v=>{let r=(v.USED/1073741824).toFixed(1),d=(v.SIZE/(1024*1024*1024)).toFixed(1),h=v.SIZE>0?(v.USED/v.SIZE*100).toFixed(0):0,C=v.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let c=await St();c?(m(c),e.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},e.dataset.listenerAttached="true"),a&&!a.hasChildNodes())){let c=await St();m(c)}if(!t)return;let p=await Dt();if(!p||!p.services){t.children.length===0&&(t.innerHTML=D("offline","Failed to load system metrics.","The event service may be offline."));return}Et=Date.now(),J(0,Et);let g=p.services||[];Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function x(c){return!c||c==="N/A"||c==="unknown"||c.trim()===""?"-":c}function u(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/^(\d+\.\d+\.\d+)/);return v?v[0]:c.split(".").slice(0,3).join(".")||"-"}function b(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function i(c){if(!c||c==="N/A"||c==="unknown")return"-";let v=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!v)return"-";let r=parseInt(v[1])||0,d=parseInt(v[2])||0,h=parseInt(v[3])||0,C=parseFloat(v[4])||0,L=r*86400+d*3600+h*60+C,k=Math.floor(L/86400);if(k>0)return`${k}d`;let I=Math.floor(L/3600);if(I>0)return`${I}h`;let S=Math.floor(L/60);return S>0?`${S}m`:`${Math.floor(L)}s`}function f(c){let v=c.status==="online",r=v?"service-widget-online":"service-widget-offline",d=v?"bx-check-circle":"bx-x-circle",h=v?"OK":"BAD",C=c.version?u(c.version.str):"-",L=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",k=c.memory&&c.memory!=="N/A"?c.memory:"-",I=L!=="-"?"#00ff00":"#666",S=L!=="-"?"#fff":"#666",N=k!=="-"?"#00ff00":"#666",O=k!=="-"?"#fff":"#666",H=i(c.uptime),P="";return v?P=`
                ...
                </div>`:P='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${r}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${d}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${b(c.domain&&c.port?`${c.domain}:${c.port}`:"")}</span></div>${P}</div></div>`}let w=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),y=new Set(g.map(c=>c.id));for(let[c,v]of w)y.has(c)||v.remove();g.forEach(c=>{let v=f(c),r=w.get(c.id);r?r.outerHTML!==v&&(r.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}async function Ue(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Dt();if(!e){t.children.length===0&&(t.innerHTML=D("offline","Failed to load model status.","The event service may be offline."));return}kt=Date.now(),J(2,kt);let s=e.models||[],a=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function n(p){let g=p.status==="Ready";return`
                ...
                </div>`}function l(p){let g=p.status==="Downloaded",x=g?"service-widget-online":"service-widget-offline",u=g?"OK":"MISSING",b=g&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",i=p.name;return p.type==="custom"&&i.endsWith(":latest")&&(i=i.replace(":latest","")),`<div class="service-widget ${x}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${g?"bx-check-circle":"bx-x-circle"}"></i><h3>${i}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let m="";if(a&&(m+=n(a)),e.xtts&&(m+=o(e.xtts)),m+=s.map(l).join(""),!m){t.innerHTML=D("empty","No models found.");return}t.innerHTML!==m&&(t.innerHTML=m)}async function Ae(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),s=document.getElementById("guardian-architect-val"),a=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),m=document.getElementById("guardian-reset-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{m.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{m.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{m.innerHTML="<i class='bx bx-refresh'></i> Reset"},2e3)},500),Ae()}catch{m.innerHTML="<i class='bx bx-error'></i> Failed"}},m.dataset.listenerAttached="true");let p=await ds();if(p){let w=Math.floor(Date.now()/1e3),y=p.active_tier,c=p.protocol_aliases||{sentry:"Sentry",architect:"Architect"},v=C=>{C<0&&(C=0);let L=Math.floor(C/3600),k=Math.floor(C%3600/60),I=C%60;return L>0?`${L}h ${k}m`:k>0?`${k}m ${I}s`:`${I}s`},r=(C,L,k,I)=>{if(!C)return;let S=c[I]||I.toUpperCase(),N=C.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(N&&(N.textContent=S),y&&y.includes(S))C.textContent="Working",C.style.color="#bb86fc";else if(k){let H=k.next_run-w;if(H<=0)C.textContent="Ready",C.style.color="#5eff5e";else{let P=Math.floor(H/60),V=H%60;C.textContent=`${P}m ${V}s`,C.style.color="#fff"}}L&&k&&(L.innerHTML=`
        ...
        `)};e&&r(e,document.getElementById("guardian-sentry-stats"),p.sentry,"sentry"),s&&r(s,document.getElementById("guardian-architect-stats"),p.architect,"architect");let d=document.getElementById("system-state-label"),h=document.getElementById("system-state-val");if(h&&p.system_state){let C=p.system_state,L=v(p.system_state_time||0);d&&(d.textContent=`State: ${C.toUpperCase()}`),h.textContent=L,C==="idle"?h.style.color=p.system_state_time>300?"#5eff5e":"#fff":h.style.color="#bb86fc"}n&&(n.textContent=v(p.total_idle_time||0)),o&&(o.textContent=v(p.total_active_time||0)),l&&(l.textContent=v(p.total_waste_time||0))}else[e,s,a,n,o,l].forEach(y=>{y&&(y.textContent="-",y.style.color="#666")});let g=await cs(),x=[],u=[],b=[];g&&(Array.isArray(g)?x=g:(x=g.active||[],u=g.queue||[],b=g.history||[],b.sort((w,y)=>(y.end_time||0)-(w.end_time||0)))),x.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=D("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Pe(t,x,!1));let i=document.getElementById("processes-queue-widgets");i&&(u.length===0?!i.querySelector(".tab-placeholder")&&!i.querySelector('div[style*="font-style: italic"]')&&(i.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(i.innerHTML="",Pe(i,u,!1)));let f=document.getElementById("processes-history-widgets");f&&(b.length===0?f.querySelector(".tab-placeholder")||(f.innerHTML=D("empty","No recent history.")):(f.querySelector(".tab-placeholder")&&(f.innerHTML=""),Pe(f,b,!0))),xe(1,x.length)}function Pe(t,e,s){function a(m){let p="";m.end_time?p=`${m.end_time-m.start_time}s`:p=`${Math.floor(Date.now()/1e3-m.start_time)}s`;let g=m.retries>0?`<span class="process-retry-badge">Retry ${m.retries}</span>`:"",x=m.channel_id,u={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation"};if(u[x]?x=u[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),s)return`
        <div class="process-history-item" data-channel-id="${m.channel_id}-${m.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let n=s?".process-history-item":".process-widget",o=new Map(Array.from(t.querySelectorAll(n)).map(m=>[m.dataset.channelId,m])),l=new Set(e.map(m=>`${m.channel_id}-${m.start_time}`));for(let[m,p]of o)l.has(m)||p.remove();e.forEach(m=>{let p=`${m.channel_id}-${m.start_time}`,g=a(m),x=o.get(p);if(x){x.outerHTML!==g&&(x.outerHTML=g);let u=t.querySelector(`[data-channel-id="${p}"]`);u&&t.appendChild(u)}else t.insertAdjacentHTML("beforeend",g)})}function Ie(){let t=ve(),e=Qe()||"master@easter.company",s={enabled:Notification.permission==="granted",supported:"Notification"in window},a=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,a]of Object.entries(t)){let n=Nt.filter(o=>o.category===s);n.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Ht(),document.getElementById("terminal-action-btn").onclick=()=>Ht());let s=document.getElementById("cli-terminal-body"),a=document.getElementById("cli-docs-pane");s.innerHTML="";let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return a.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),qe=!0,s}function Ht(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),qe=!1}function zt(t,e,s="output"){if(!qe)return;let a=document.createElement("div");a.className=`terminal-line terminal-${s}`,s==="prompt"?a.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:a.innerHTML=ke(e),t.appendChild(a),t.scrollTop=t.scrollHeight}async function fs(t){let e=Nt.find(n=>n.id===t);if(!e)return;let s=us(e);zt(s,`dex ${t}`,"prompt");let a=e.demo_output||["Executing command...","\u2713 Done."];for(let n of a){await new Promise(l=>setTimeout(l,400+Math.random()*600));let o="output";n.includes("[ERROR]")?o="error":n.includes("[SUCCESS]")||n.includes("\u2713")?o="success":n.includes("!")&&(o="warning"),zt(s,n,o)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Rt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let s=e.querySelector("code").textContent;navigator.clipboard.writeText(s).then(()=>{let a=e.querySelector("i");a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(s=>{s.addEventListener("mouseenter",()=>{s.style.transform="translateY(-5px)",s.style.borderColor="rgba(255,255,255,0.15)",s.style.backgroundColor="rgba(255,255,255,0.05)"}),s.addEventListener("mouseleave",()=>{s.style.transform="translateY(0)",s.style.borderColor="rgba(255,255,255,0.05)",s.style.backgroundColor="rgba(255,255,255,0.03)"}),s.addEventListener("click",()=>{let a=s.dataset.cmd;fs(a)})})}async function gs(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${De()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable.")}}function Pt(){gs(),it(),Ve();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Rt();let e=Xe();We(e),Ke();let s=document.getElementById("nav-left-container");s&&s.addEventListener("click",()=>{if(o.length>0)u();else{let r=window.location.pathname;if(!(r==="/"||r==="/index.html")){let C=(r.endsWith("/")&&r.length>1?r.slice(0,-1):r).split("/");C.pop();let L=C.join("/")||"/";window.location.href=L}}});let a=window.location.pathname==="/"||window.location.pathname==="/index.html",n=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!a&&!n&&document.querySelector("footer")?.classList.add("hide");let o=[],l=document.getElementById("windows-container");l&&l.setAttribute("data-count","0");let m=r=>{localStorage.setItem("dex_last_window",r)};function p(){return 1}function g(){for(;o.length>1;)o.shift().close(!0);let r=document.getElementById("windows-container"),d=document.querySelector("nav"),h=document.querySelector("footer"),C=window.location.pathname==="/"||window.location.pathname==="/index.html",L=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");r&&r.setAttribute("data-count",o.length),o.forEach(N=>{let O=document.getElementById(N.id);O&&O.classList.remove("hide-close")}),we(ve());let k=document.getElementById("dexter-menu-container"),I=document.getElementById("nav-window-switcher"),S=document.getElementById("settings-icon");if(Ze(o.length>0),o.length>0){if(h?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),d?.classList.add("window-open"),r&&(r.style.paddingTop="60px"),k&&(k.style.display="none"),S&&(S.style.display="none"),I){let N=o[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(N)?(I.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let d=document.getElementById("dexter-menu-container"),h=document.getElementById("dexter-menu-btn");d&&r&&h&&(d.addEventListener("mouseenter",()=>{r.classList.add("active"),h.classList.add("active")}),d.addEventListener("mouseleave",()=>{r.classList.remove("active"),h.classList.remove("active")}),h.addEventListener("click",C=>{C.stopPropagation();let L=localStorage.getItem("dex_last_window")||"alerts-window",k=null;L==="alerts-window"?k=b:L==="events-window"?k=i:L==="monitor-window"?k=w:L==="contacts-window"?k=f:L==="workspace-window"&&(k=y),k&&x(k)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{m("alerts-window"),x(b)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{m("events-window"),x(i)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{m("monitor-window"),x(w)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{m("contacts-window"),x(f)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{m("workspace-window"),x(y)}),document.getElementById("settings-icon")?.addEventListener("click",()=>x(c)),document.getElementById("close-all-windows")?.addEventListener("click",()=>u()),setInterval(()=>{b.isOpen()?ne():lt(),y.isOpen()?He():bt(),i.isOpen()&&de(),w.isOpen()&&_t()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{v.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",r=>{r.preventDefault();try{et(document.getElementById("email-input").value),window.location.reload()}catch(d){let h=document.getElementById("login-error");h&&(h.textContent=d.message,h.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Pt):Pt();})();

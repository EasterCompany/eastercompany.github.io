...
`,Ct=null,be=new Set,Tt=[];async function ve(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ts();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await B(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(Tt=l,Ct=Date.now(),ee(2,Ct),l.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ke(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),$=(v.content||"").trim(),w=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(_=>_.trim()),d=(v.implementation_path||[]).map(_=>_.trim()),u=v.source_event_ids||[],g=v.approved===!0,x=new Date(m.timestamp*1e3),M=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=be.has(m.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${g?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,g&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(_){let P=this;if(P.classList.contains("expanded")){P.classList.remove("expanded"),be.delete(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="none")}else{P.classList.add("expanded"),be.add(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="block")}};let O="";d.length>0&&(O=`
  ...
  `}async function Te(){let t=document.getElementById("progress-view-root");t&&(ae!==kt&&(t.innerHTML=Lt(),kt=ae),ae==="ACTIVE"&&Ls())}function Ls(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),s=document.getElementById("active-task-progress-text"),i=document.getElementById("terminal-output");if(t&&Z&&(t.innerText=Z.title),e&&Z&&(e.style.width=`${Z.progress}%`),s&&Z&&(s.innerText=`${Z.progress}% COMPLETE`),i){let n=i.children.length;if($e.length>n){for(let l=n;l<$e.length;l++){let r=$e[l],f=document.createElement("div");f.className="terminal-line",f.innerHTML=`
    ...
    `;async function Re(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),f=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=f?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),l&&(l.value=""),r&&(r.value=""),Re()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active chores.","Create one to start monitoring.");return}let f=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=f,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this chore?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Re())}})}catch(l){console.error("Failed to fetch chores",l)}}var It=()=>`

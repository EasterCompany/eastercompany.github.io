...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];At=c,St=Date.now(),ee(2,St);let r=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let C=(m.title||"Untitled Blueprint").trim(),I=(m.summary||m.body||"No summary provided.").trim(),o=(m.content||"").trim(),l=(m.category||"architecture").trim(),u=(m.related_services||m.affected_services||[]).map(N=>N.trim()),g=(m.implementation_path||[]).map(N=>N.trim()),E=m.source_event_ids||[],f=m.approved===!0,T=new Date(s.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=ye.has(s.id),S=k?"display: block;":"display: none;",M=document.createElement("div");M.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,M.dataset.blueprintId=s.id,f&&(M.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",M.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[l]||"bx-paint";M.onclick=function(N){let B=this;if(B.classList.contains("expanded")){B.classList.remove("expanded"),ye.delete(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="none")}else{B.classList.add("expanded"),ye.add(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="block")}};let q="";g.length>0&&(q=`
                ...
                `);let A=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",$=D(),L=f?`
...
`;async function Xe(){await Promise.all([ve(),xe(),Le(),Be()])}var Ds=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,Ue=null,we=new Set,He=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await _(i);if(!a.ok)throw new Error("Service unreachable");if(He=((await a.json()).events||[]).filter(y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ue=Date.now(),ee(1,Ue),He.length===0){e.innerHTML=U("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,C=Os(m),I=js(m),o=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),l="event-border-grey";o&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?l="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?l="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?l="event-border-orange":m==="system.test.completed"?l=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let u=o?"cursor-pointer":"",g=we.has(y.id),E=g?"expanded":"",f=g?"display: block;":"display: none;",T=new Date(y.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=Ft(m,s),S=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",M="";if(o){let A="";if(m==="engagement.decision"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L=s.timestamp-s.last_active,H=Math.floor(L/60);A=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="";s.eval_count&&(L=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `}
                    `}else if(m==="analysis.user.message_signals"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L=s.signals||{};A=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(L.topics||[]).map(P=>`<span class="profile-badge">${P}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.visual.completed"){let $=H=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${H}</h5>`,L="";s.base64_preview?L=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(L=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",H=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,P="";s.error&&(P=`
                            <div class="event-detail-block">
                                ${H("Error")}
                        ...
                        ${z}
                        ${V}
                    `}else if(m==="system.test.completed"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            </div>`}let l=o.status==="online",u=l?"service-widget-online":"service-widget-offline",g=l?"bx-check-circle":"bx-x-circle",E=l?"OK":"BAD",f=o.version?w(o.version.str):"-",T=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",h=o.memory&&o.memory!=="N/A"?o.memory:"-",v=T!=="-"?"#00ff00":"#666",k=T!=="-"?"#fff":"#666",S=h!=="-"?"#00ff00":"#666",M=h!=="-"?"#fff":"#666",R=y(o.uptime),W="";l?W=`
          ...
          `)};n&&M(n,document.getElementById("guardian-sentry-stats"),u.protocols.sentry,"sentry");let R=document.getElementById("analyzer-synthesis-val"),W=document.getElementById("analyzer-synthesis-stats");R&&M(R,W,g.protocols.synthesis,"synthesis");let q=document.getElementById("imaginator-alert_review-val"),G=document.getElementById("imaginator-alert_review-stats");q&&M(q,G,E.protocols.alert_review,"alert_review");let A=document.getElementById("fabricator-construction-val"),$=document.getElementById("fabricator-construction-stats");A&&M(A,$,f.protocols.construction,"construction");let L=document.getElementById("courier-researcher-val"),H=document.getElementById("courier-researcher-stats");L&&M(L,H,T.protocols.researcher,"researcher");let P=document.getElementById("system-state-label"),V=document.getElementById("system-state-val");if(V&&h.state){let z=h.state,N=S(h.state_time||0);P&&(P.textContent=`State: ${z.toUpperCase()}`),V.textContent=N,z==="idle"?V.style.color=h.state_time>300?"#5eff5e":"#fff":V.style.color="#bb86fc",p&&(z==="paused"?(p.innerHTML="<i class='bx bx-play'></i>",p.title="Resume System",p.style.color="#ff9800"):(p.innerHTML="<i class='bx bx-pause'></i>",p.title="Pause System",p.style.color=""))}a&&(a.textContent=S(h.metrics?.total_idle_time||0)),c&&(c.textContent=S(h.metrics?.total_active_time||0)),r&&(r.textContent=S(h.metrics?.total_waste_time||0))}else[n,i,a,c,r].forEach(g=>{g&&(g.textContent="-",g.style.color="#666")});if(t)return;let s=await Ws(),m=[],C=[],I=[];s&&(Array.isArray(s)?m=s:(m=s.active||[],C=s.queue||[],I=s.history||[],I.sort((u,g)=>(g.end_time||0)-(u.end_time||0)))),e&&(m.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=U("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),st(e,m,!1)));let o=document.getElementById("processes-queue-widgets");o&&(C.length===0?!o.querySelector(".tab-placeholder")&&!o.querySelector('div[style*="font-style: italic"]')&&(o.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(o.innerHTML="",st(o,C,!1)));let l=document.getElementById("processes-history-widgets");l&&(I.length===0?l.querySelector(".tab-placeholder")||(l.innerHTML=U("empty","No recent history.")):(l.querySelector(".tab-placeholder")&&(l.innerHTML=""),st(l,I,!0))),Ne(1,m.length)}function st(t,e,n){function i(d){let x="";d.end_time?x=`${d.end_time-d.start_time}s`:x=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let w=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",p=d.channel_id,y={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(y[p]?p=y[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),n)return`

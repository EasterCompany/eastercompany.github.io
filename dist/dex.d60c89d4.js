"use strict";(()=>{function ct(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.prepend(t)}}function dt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,o=document.createElement("nav");o.innerHTML=s,document.body.prepend(o);let i=document.createElement("div");i.id="dexter-dropdown",i.className="dexter-dropdown",i.innerHTML=`
    ...
    `,document.body.appendChild(i)}function ke(t){let e=window.location.pathname,a=e==="/"||e==="/index.html",n=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!n||!s||(t||!a?(n.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{n.style.transform="translateX(-3px)"},s.onmouseleave=()=>{n.style.transform="translateX(0)"}):(n.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function pt(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var mt=1e3;function le(t){let e=null,a=t.onClose||null,n=t.onOpen||null;function s(){e&&(e.style.zIndex=(++mt).toString())}function o(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",i),n&&setTimeout(n,10);return}let y=document.getElementById("windows-container");y||(y=document.createElement("div"),y.id="windows-container",y.className="windows-container",document.body.appendChild(y)),e=document.createElement("div"),e.id=t.id,e.className="window",t.className&&e.classList.add(t.className),t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++mt).toString(),e.addEventListener("mousedown",s);let h=t.icon||"bx-window",r="",u="",L,I='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(r=`<div class="tab-bar">${t.tabs.map((v,x)=>{let d=v.icon?`<i class="bx ${v.icon}"></i>`:`<span class="tab-glyph">${v.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,L=`<div class="window-content">${t.tabs.map((v,x)=>`<div class="tab-content ${x===0?"active":""}" data-tab-content="${x}">${v.content}${I}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),L=`<div class="window-content">${t.content||""}${I}</div>`),e.innerHTML=`
            ...
            ${L}
        `,t.appendToBody?document.body.appendChild(e):y.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",b=>{b.stopPropagation(),c()}),window.addEventListener("resize",i),t.tabs&&e.querySelectorAll(".tab").forEach(b=>{b.addEventListener("click",()=>{if(!e)return;let f=b.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(x=>x.classList.remove("active")),b.classList.add("active"),e.querySelectorAll(".tab-content").forEach(x=>x.classList.remove("active"));let v=e.querySelector(`.tab-content[data-tab-content="${f}"]`);v&&v.classList.add("active"),m(b,e)})}),setTimeout(()=>{e&&e.classList.add("open"),n&&n()},10)}function i(){if(!e||!e.classList.contains("open"))return;let y=e.querySelector(".tab.active");y&&m(y,e)}function m(y,h){setTimeout(()=>{let r=h.querySelector(".tab-bar");if(!r)return;let u=Array.from(r.querySelectorAll(".tab")),L=u.indexOf(y),I=r.clientWidth,b=u[Math.max(0,L-2)],f=u[Math.min(u.length-1,L+2)],v=r,x=b.offsetLeft-v.offsetLeft-25,l=f.offsetLeft+f.offsetWidth-v.offsetLeft+25-x,$=l<=I?x-(I-l)/2:y.offsetLeft-v.offsetLeft-I/2+y.offsetWidth/2;r.scrollTo({left:$,behavior:"smooth"})},350)}function c(y=!1){e&&(window.removeEventListener("resize",i),y?(e.remove(),e=null):(e.classList.remove("open"),a&&a(),setTimeout(()=>{e?.remove(),e=null},400)))}function g(y){t.content=y;let h=e?.querySelector(".window-content");h&&(h.innerHTML=y)}function k(){return!!(e&&e.classList.contains("open"))}return{open:o,close:c,setContent:g,isOpen:k,focus:s,id:t.id}}var ut="easter_theme",se={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Ke(){return localStorage.getItem(ut)||se.DARK}function ft(t){if(!Object.values(se).includes(t))throw new Error("Invalid theme");localStorage.setItem(ut,t),gt(t)}function gt(t){let e=document.body;Object.values(se).forEach(o=>{e.classList.remove(`theme-${o}`)}),e.classList.add(`theme-${t}`);let a=document.querySelector('meta[name="theme-color"]');a||(a=document.createElement("meta"),a.name="theme-color",document.getElementsByTagName("head")[0].appendChild(a));let n={[se.DARK]:"#050507",[se.LIGHT]:"#FFFFFF",[se.LEGACY]:"#000000"},s=n[t]||n[se.DARK];if(a.setAttribute("content",s),!document.querySelector(".background")){let o=document.createElement("div");o.className="background",document.body.prepend(o)}e.classList.add("is-animated")}function bt(){let t=Ke();gt(t)}function W(t,e,a=null,n="default"){let o={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",i=a?`<p class="placeholder-action">${a}</p>`:"";return`<div class="tab-placeholder" data-placeholder-id="${n}"><i class='bx ${o} placeholder-icon'></i><p class="placeholder-message">${e}</p>${i}</div>`}function N(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function pe(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!a)return;if(!e){a.textContent="Last updated: never";return}let n=Date.now(),s=Math.floor((n-e)/1e3),o;s<60?o=`${s}s ago`:s<3600?o=`${Math.floor(s/60)}m ago`:o=`${Math.floor(s/3600)}h ago`,a.textContent=`Last updated: ${o}`}var ns=0;function yt(t,e){let a=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!a)return;let n=a.querySelector(".notification-badge");n&&(e>0?(n.textContent=e>9?"9+":e.toString(),n.style.display="flex"):n.style.display="none")}var ce=0,de=0;function xt(t){ce=t,Ie()}function Ie(){let t=ce+de;ns=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let a=document.getElementById("alerts-menu-item");if(a){let i=a.querySelector(".notification-badge");i||(i=document.createElement("span"),i.className="notification-badge",i.style.marginLeft="auto",a.appendChild(i)),i.textContent=ce>9?"9+":ce.toString(),i.style.display=ce>0?"flex":"none"}let n=document.getElementById("workspace-menu-item");if(n){let i=n.querySelector(".notification-badge");i||(i=document.createElement("span"),i.className="notification-badge",i.style.marginLeft="auto",n.appendChild(i)),i.textContent=de>9?"9+":de.toString(),i.style.display=de>0?"flex":"none"}let s=document.getElementById("switch-alerts");if(s){let i=s.querySelector(".notification-badge");i||(i=document.createElement("span"),i.className="notification-badge",i.style.marginLeft="8px",s.appendChild(i)),i.textContent=ce>9?"9+":ce.toString(),i.style.display=ce>0?"flex":"none"}let o=document.getElementById("switch-workspace");if(o){let i=o.querySelector(".notification-badge");i||(i=document.createElement("span"),i.className="notification-badge",i.style.marginLeft="8px",o.appendChild(i)),i.textContent=de>9?"9+":de.toString(),i.style.display=de>0?"flex":"none"}}function Xe(){let t=document.getElementById("alerts-list");if(!t)return;ce=t.querySelectorAll(".alert-unread:not(.priority-low)").length,Ie()}async function ht(){try{de=(await(await H("/roadmap/stats")).json()).open_issues||0,Ie()}catch(t){console.error("Failed to update open issue count:",t)}}function is(){return"https://event.easter.company"}function as(){return"https://discord.easter.company"}var os="http://127.0.0.1:8100",rs="http://127.0.0.1:8300",ls={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Ue(t){let e=N(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,o)=>{let i=ls[o];return i?`<span class="${i}">`:""});let a=(e.match(/<span/g)||[]).length,n=(e.match(/<\/span/g)||[]).length;return a>n&&(e+="</span>".repeat(a-n)),e}function ne(t){if(!t)return"";let e=N(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^<strong>(.*)<\/strong>$/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(a,n)=>{let s=n.split("|").map(o=>o.trim());return s.every(o=>o.match(/^:?-+:?$/))?"":`<div class="md-table-row">${s.map(o=>`<span class="md-table-cell">${o}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}function oe(){let t=document.getElementById("windows-container"),e=t?parseInt(t.getAttribute("data-count")||"0"):0,a=document.querySelector(".profile-overlay.active"),n=document.querySelector("#cli-terminal-overlay.active");return e>0||!!a||!!n}var cs="https://sterling-javelin-12539.upstash.io",ds="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function z(){return window.location.hostname.includes("easter.company")}var R=null,be={lastDashboard:0,lastFrontend:0},Ye=!1,wt=0,Et="dex_dashboard_snapshot";function ps(){let t=localStorage.getItem(Et);if(t)try{let e=JSON.parse(t);R=e,be.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{R=null}}async function vt(){if(!(!z()||Ye)){Ye=!0,wt=Math.floor(Date.now()/1e3);try{let t=await fs("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);R=e,be.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),be.lastFrontend=Date.now(),localStorage.setItem(Et,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Ye=!1}}}function ms(){if(!R||!R.agent_status)return;let t=R.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function us(){if(!z())return;ps();let t=Math.floor(Date.now()/1e3),e=R?t-R.timestamp:1/0;(!R||e>120)&&await vt(),setInterval(()=>{let a=new Date,n=Math.floor(Date.now()/1e3),s=R?n-R.timestamp:1/0,o=n-wt,i=a.getSeconds()===59,m=s>300,c=o>60;(i&&c||m&&c)&&vt(),ms()},1e3)}z()&&us();async function fs(t,...e){try{let n=await(await fetch(cs,{method:"POST",headers:{Authorization:`Bearer ${ds}`},body:JSON.stringify([t,...e])})).json();if(n.error)throw new Error(n.error);return n.result}catch(a){return console.error("Upstash Error:",a),null}}var Se=null,Le=null,Pe=!1,je=!1;async function H(t,e={}){if(z()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(R.monitor),{status:200});if(t.startsWith("/system/options"))return new Response(JSON.stringify(R.options||{}),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(R.processes),{status:200});if(t.startsWith("/events")){let s=new URLSearchParams(t.split("?")[1]||""),o=s.get("type")||s.get("event.type"),i=s.get("category"),m=s.get("order")||"desc",c=[];return o==="system.notification.generated"?c=[...R.alerts||[]]:o==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?c=[...R.blueprints||[]]:i==="messaging"?c=[...R.messaging_events||[]]:i==="system"?c=[...R.system_events||[]]:i==="cognitive"?c=[...R.cognitive_events||[]]:i==="moderation"?c=[...R.moderation_events||[]]:c=[...R.events||[]],m==="asc"?c.sort((g,k)=>g.timestamp-k.timestamp):c.sort((g,k)=>k.timestamp-g.timestamp),new Response(JSON.stringify({events:c,count:c.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(R.agent_status||{}),{status:200});if(t.startsWith("/roadmap/stats"))return new Response(JSON.stringify({open_issues:R.open_issue_count||0}),{status:200});if(t.startsWith("/profile/")){let s=t.split("/")[2],o=R.profiles?R.profiles[s]:null;return o?new Response(JSON.stringify(o),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/view")?new Response(JSON.stringify(R.web_view||{}),{status:200}):t.startsWith("/web/history")?new Response(JSON.stringify(R.web_history||[]),{status:200}):t.startsWith("/roadmap")?new Response(JSON.stringify(R.github_issues||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify(R.chores||[]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(Se)try{let s=await fetch(Se+t,e);if(s.ok)return s;Se=null}catch{Se=null}let a=is(),n=os;window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1"&&!z()&&(n=`http://${window.location.hostname}:8100`);try{let s=await fetch(a+t,e);if(s.ok)return Se=a,Pe&&(console.log("\u2728 Production event service restored."),Pe=!1),s;throw new Error("Primary failed")}catch{Pe||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${n}`),Pe=!0);try{let o=await fetch(n+t,e);if(o.ok)return Se=n,o;throw new Error("Fallback failed")}catch(o){if(t==="/roadmap/stats")return new Response(JSON.stringify({open_issues:0}),{status:200});throw o}}}async function ve(t,e={}){if(z()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(R.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let s=t.split("/")[2],o=(R.contacts?.members||[]).find(i=>i.id===s);return o?new Response(JSON.stringify(o),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(Le)try{let s=await fetch(Le+t,e);if(s.ok)return s;Le=null}catch{Le=null}let a=as(),n=rs;window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1"&&!z()&&(n=`http://${window.location.hostname}:8300`);try{let s=await fetch(a+t,e);if(s.ok)return Le=a,je&&(console.log("\u2728 Production discord service restored."),je=!1),s;throw new Error("Primary failed")}catch{je||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${n}`),je=!0);try{let o=await fetch(n+t,e);if(o.ok)return Le=n,o;throw new Error("Fallback failed")}catch(o){throw o}}}var Tt=()=>`
...
`,qe=null,ye=new Set,Fe=[];async function me(t=!1){let e=document.getElementById("alerts-list");if(!e)return;gs(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await H(a);if(!n.ok)throw new Error("Service is offline or unreachable.");let o=(await n.json()).events||[];qe=Date.now(),pe(0,qe);let i=Date.now(),m=24*60*60*1e3,c=o.filter(f=>{let v=localStorage.getItem(`alert_read_ts_${f.id}`);if(!v)return!0;let x=parseInt(v);return i-x<m});c.sort((f,v)=>{let x=E=>{let p=E.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return"low"}return(p.priority||"low").toLowerCase()},d=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,l=d(x(f)),$=d(x(v));return l!==$?$-l:v.timestamp-f.timestamp}),Fe=c;let g=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},k=[];if(c.length===0)k.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let v=new Set(c.map(d=>g(d))).size>1,x=null;c.forEach(d=>{let l=g(d);v&&l!==x&&(k.push({id:`divider-${l}`,type:"divider",label:l.toUpperCase()}),x=l),k.push(d)})}t&&(e.innerHTML="");let y=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let x=(v.title||"Untitled Alert").trim(),d=(v.body||"No description provided.").trim(),l=v.summary||"",$=v.content||"",E=v.protocol||"",p=(v.priority||"low").toLowerCase(),w=!!v.alert,A=(v.category||"system").trim(),_=v.related_event_ids||[],B=v.audit_event_id,V=!!localStorage.getItem(`alert_read_ts_${f.id}`),Z=new Date(f.timestamp*1e3),M=Z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=Z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=V?"event-border-grey":"event-border-blue";!V&&w&&(S="event-border-red"),V&&(p==="high"||p==="critical")?S="event-border-red":V&&p==="medium"&&(S="event-border-orange");let C=V?"alert-read":"alert-unread",O=ye.has(f.id),D=O?"expanded":"",K=O?"display: block;":"display: none;",U="",j="";_.length>0&&(j=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ne($)}</div>
            </div>
        `:ee=`
            <div class="event-detail-block" style="text-align: left;">
        ...
        `,U=`
                ...
                ${ee}
            `;let Y=document.createElement("div");Y.className=`event-item notification-item ${S} ${C} ${D} cursor-pointer priority-${p}`,Y.dataset.alertId=f.id,Y.onclick=function(X){let J=this;if(J.classList.contains("expanded")){J.classList.remove("expanded"),ye.delete(f.id);let ge=J.querySelector(".event-details");ge&&(ge.style.display="none")}else{J.classList.add("expanded"),ye.add(f.id);let ge=J.querySelector(".event-details");if(ge&&(ge.style.display="block"),!localStorage.getItem(`alert_read_ts_${f.id}`)){localStorage.setItem(`alert_read_ts_${f.id}`,Date.now().toString()),J.classList.add("alert-read"),J.classList.remove("alert-unread"),J.classList.remove("event-border-blue","event-border-red","event-border-purple");let Me="event-border-grey";p==="high"||p==="critical"?Me="event-border-red":p==="medium"&&(Me="event-border-orange"),J.classList.add(Me),Xe()}}};let Ce=`${E?E.toUpperCase():"GUARDIAN"} ALERT: ${l||x}`,Ge={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[A]||"bx-bell",Ne=p==="high"||p==="critical"?"#ff4d4d":p==="medium"?"#ffa500":"#888";Y.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    ...
                    <div class="event-message">${ne(Ce)}</div>
                    <div class="event-details" style="${K}">
                        ${U}
                    </div>
                </div>
            `;let fe=Y.querySelector(".event-details");fe&&fe.addEventListener("click",X=>{X.stopPropagation()});let F=Y.querySelector(".close-details-btn");return F&&F.addEventListener("click",X=>{X.stopPropagation(),Y.classList.remove("expanded");let J=Y.querySelector(".event-details");J&&(J.style.display="none"),ye.delete(f.id)}),Y},h=f=>{let v=document.createElement("div");v.className="notification-divider",v.dataset.alertId=f.id;let x="#888888";return f.label==="CRITICAL"?x="#ff4d4d":f.label==="HIGH"?x="#ff8888":f.label==="MEDIUM"&&(x="#ffa500"),v.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,v.innerHTML=`<span style="white-space: nowrap;">${f.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,v},r=f=>{let v=document.createElement("div");v.innerHTML=W("empty",f.message,null,f.id);let x=v.firstElementChild;return x.dataset.alertId=f.id,x},u=Array.from(e.children),L=new Map(u.map(f=>[f.dataset.alertId,f])),I=new Set(k.map(f=>f.id));u.forEach(f=>{let v=f.dataset.alertId;(!v||!I.has(v))&&f.remove()});let b=null;k.forEach((f,v)=>{let x=L.get(f.id);if(!x||t){if(x&&x.remove(),f.type==="divider")x=h(f);else if(f.type==="placeholder")x=r(f);else{let d=y(f);if(!d)return;x=d}if(!x)return}v===0?e.firstElementChild!==x&&e.prepend(x):b&&b.nextElementSibling!==x&&b.after(x),b=x}),qe=Date.now(),pe(0,qe),Xe()}catch(n){console.error("Error fetching alerts:",n),e.children.length===0&&(e.innerHTML=W("offline","Failed to load alerts.","The event service may be offline."))}}function gs(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),a=document.getElementById("alerts-close-all"),n=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Fe.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Fe.forEach(s=>{ye.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),me(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{ye.clear(),me(!0)},a.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await H("/events?type=system.notification.generated",{method:"DELETE"});let s=Date.now()-48*60*60*1e3;Fe.forEach(o=>{localStorage.setItem(`alert_read_ts_${o.id}`,s.toString())}),ye.clear(),me(!0)}catch(s){console.error("Failed to clear alerts:",s)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}async function Ze(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await H(t);if(!e.ok)return;let n=(await e.json()).events||[],s=0;n.forEach(o=>{let i=o.event;if(typeof i=="string")try{i=JSON.parse(i)}catch{i={}}if((i.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${o.id}`)||s++}),xt(s)}catch{}}var kt=()=>`
...
`,Ae=new Set,Qe=[],$t=t=>{let e=Ae.has(t.number),n=new Date(t.createdAt).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),s=document.createElement("div");return s.className=`event-item notification-item event-border-blue cursor-pointer ${e?"expanded":""}`,s.dataset.issueNumber=t.number,s.id=`roadmap-issue-${t.number}`,s.onclick=o=>{if(o.target.closest("button")||o.target.closest("textarea"))return;if(s.classList.contains("expanded")){s.classList.remove("expanded");let m=s.querySelector(".event-details");m&&(m.style.display="none"),Ae.delete(t.number)}else{s.classList.add("expanded");let m=s.querySelector(".event-details");m&&(m.style.display="block"),Ae.add(t.number)}},s.innerHTML=`
          ...
          <div class="event-content">
            ...
            <div class="event-message" style="font-weight: bold; margin-bottom: 5px;">${N(t.title)}</div>
  ...
  `,s.querySelector(".comment-btn")?.addEventListener("click",async()=>{let o=s.querySelector(".comment-input"),i=o.value.trim();i&&(await vs(t.number,i),o.value="",xe(!0))}),s.querySelector(".close-btn")?.addEventListener("click",async()=>{confirm(`Close issue #${t.number}?`)&&(await ys(t.number),xe(!0))}),s};async function xe(t=!1){let e=document.getElementById("roadmap-list"),a=document.getElementById("roadmap-timeline-container");if(e){bs();try{let n=await H("/roadmap");if(!n.ok)throw new Error("Offline");let s=await n.json();if(s.sort((c,g)=>new Date(c.createdAt).getTime()-new Date(g.createdAt).getTime()),!t&&JSON.stringify(s)===JSON.stringify(Qe))return;Qe=s;let o=Array.from(e.children),i=new Map(o.filter(c=>c.dataset.issueNumber).map(c=>[c.dataset.issueNumber,c]));if(s.length===0){e.innerHTML=W("empty","No open issues found.","Dexter is currently in a perfect state.");return}let m={};if(s.forEach(c=>{let g="General",k=c.title.match(/^\[(.*?)\]/);k&&(g=k[1].toLowerCase()),m[g]||(m[g]=[]),m[g].push(c)}),e.innerHTML="",Object.entries(m).forEach(([c,g])=>{let k=`<div class="service-category-header" style="margin: 20px 0 10px 0; color: #888; font-size: 0.7em; text-transform: uppercase; letter-spacing: 2px;">${c}</div>`;e.insertAdjacentHTML("beforeend",k),g.forEach(y=>{let h=y.number.toString(),r=i.get(h);if(r&&!t){let u=r.querySelector(".event-message");u&&u.textContent!==y.title?e.appendChild($t(y)):e.appendChild(r)}else e.appendChild($t(y))})}),a){let c=s.slice(0,4),g=`
        <div class="roadmap-timeline" style="display: flex; gap: 5px; background: rgba(255,255,255,0.02); padding: 10px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); overflow-x: auto; align-items: center;">
          ...
          ${c.map((k,y)=>`
            <div class="timeline-dot-wrap" style="display: flex; align-items: center; gap: 5px;">
                <div class="timeline-dot" title="#${k.number}: ${k.title}" style="width: 12px; height: 12px; border-radius: 50%; background: #bb86fc; flex-shrink: 0; opacity: 0.8; cursor: pointer;" onclick="dexter.viewRoadmapIssue(${k.number})"></div>
                <span style="font-size: 0.7em; color: #aaa; white-space: nowrap;">#${k.number}</span>
                ${y<c.length-1?"<div style='width: 15px; height: 1px; background: rgba(255,255,255,0.1); flex-shrink: 0; margin: 0 5px;'></div>":""}
            </div>
          `).join("")}
            ${s.length>4?`<span style="font-size: 0.7em; color: #444; margin-left: 10px;">+${s.length-4} more</span>`:""}
        </div>
      `;a.innerHTML=g}}catch{if(e.querySelector(".placeholder-message.offline"))return;e.innerHTML=W("offline","Failed to load roadmap.","Could not sync with GitHub.")}}}window.dexter||(window.dexter={});window.dexter.viewRoadmapIssue=t=>{let e=document.getElementById(`roadmap-issue-${t}`);e&&(e.scrollIntoView({behavior:"smooth",block:"center"}),e.classList.contains("expanded")||e.click())};function bs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),a=document.getElementById("roadmap-cancel"),n=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{let o=document.getElementById("roadmap-editor-title"),i=document.getElementById("roadmap-editor-body");o&&(o.value=""),i&&(i.value="");let m=document.getElementById("roadmap-editor-container");m&&(m.style.display="block")},t.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="none")},a.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let o=document.getElementById("roadmap-editor-title"),i=document.getElementById("roadmap-editor-body"),m=o?o.value.trim():"",c=i?i.value.trim():"";if(!m||!c){alert("Title and Body are required.");return}try{await H("/roadmap",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:m,body:c})});let g=document.getElementById("roadmap-editor-container");g&&(g.style.display="none"),xe(!0)}catch(g){console.error(g)}},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{Qe.forEach(o=>Ae.add(o.number)),xe(!0)},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Ae.clear(),xe(!0)},s.dataset.listenerAttached="true")}async function vs(t,e){await H(`/roadmap/${t}/comment`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({body:e})})}async function ys(t){await H(`/roadmap/${t}`,{method:"DELETE"})}var Lt=()=>`
...
`,Ct=()=>`
    ...
    `,_e=[],Be={"313071000877137920":"Owen",dexter:"Dexter"},ie=[],he=null;async function He(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),a=document.getElementById("create-chore-form"),n=document.getElementById("save-chore-btn"),s=document.getElementById("cancel-chore-btn"),o=document.getElementById("new-chore-owner"),i=document.getElementById("selected-recipients"),m=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),g=document.getElementById("new-chore-url"),k=document.getElementById("new-chore-schedule"),y=document.getElementById("new-chore-time"),h=document.getElementById("new-chore-timezone"),r=document.getElementById("new-chore-time-group");if(h&&!h.dataset.initialValueAttached){let l=Intl.DateTimeFormat().resolvedOptions().timeZone,$=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l){$=!0;break}if(!$){let E=document.createElement("option");E.value=l,E.textContent=l.replace("_"," "),h.appendChild(E)}h.value=l,h.dataset.initialValueAttached="true"}k&&r&&!k.dataset.timeListenerAttached&&(k.addEventListener("change",()=>{r.style.display=k.value==="daily"?"block":"none"}),k.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),L=document.getElementById("form-icon");function I(){if(i){if(ie.length===0){i.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}i.innerHTML=ie.sort((l,$)=>{let E=l.startsWith("channel:"),p=$.startsWith("channel:");return E&&!p?-1:!E&&p?1:0}).map(l=>{let $=Be[l]||l,E=l.startsWith("channel:");return`
      ...
      `}).join(""),i.querySelectorAll(".remove-recipient").forEach(l=>{l.onclick=()=>{let $=l.dataset.id;ie=ie.filter(E=>E!==$),I()}})}}function b(l){l&&!ie.includes(l)&&(ie.push(l),I()),o&&(o.value="")}function f(l=null){if(a){if(he=l?l.id:null,l){if(u&&(u.textContent="Update Research Task"),L&&(L.className="bx bx-edit-alt",L.style.color="#03dac6"),n&&(n.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=l.natural_instruction),g&&(g.value=l.execution_plan?.entry_url||""),k&&(k.value=l.schedule,r&&(r.style.display=l.schedule==="daily"?"block":"none")),y&&l.run_at&&(y.value=l.run_at),h&&l.timezone){let $=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l.timezone){$=!0;break}if(!$){let E=document.createElement("option");E.value=l.timezone,E.textContent=l.timezone.replace("_"," "),h.appendChild(E)}h.value=l.timezone}ie=l.recipients||(l.owner_id?[l.owner_id]:[]),I()}else u&&(u.textContent="Initialize Research Task"),L&&(L.className="bx bx-plus-circle",L.style.color="#bb86fc"),n&&(n.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),g&&(g.value=""),k&&(k.value="every_24h"),ie=[],I();a.style.display="block",a.scrollIntoView({behavior:"smooth",block:"start"})}}let v=document.getElementById("contacts-group"),x=document.getElementById("channels-group");if(v&&!v.dataset.populated&&!z())try{ve("/contacts").then(async l=>{l.ok&&(((await l.json()).members||[]).forEach(p=>{if(Be[p.id]=p.nickname||p.username,p.id==="313071000877137920")return;let w=document.createElement("option");w.value=p.id;let A=p.nickname||p.username;w.textContent=`${A} (${p.username})`,v.appendChild(w)}),v.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(x&&!x.dataset.populated&&!z())try{ve("/channels").then(async l=>{l.ok&&((await l.json()).forEach(E=>{let p=`channel:${E.id}`;Be[p]=E.name;let w=document.createElement("option");w.value=p,w.textContent=`#${E.name} (${E.guild})`,x.appendChild(w)}),x.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}o&&!o.dataset.listenerAttached&&(o.onchange=()=>{b(o.value)},o.dataset.listenerAttached="true"),m&&!m.dataset.listenerAttached&&(m.onchange=()=>d(),m.dataset.listenerAttached="true");function d(){if(!t)return;let l=m?.value||"all",$=l==="all"?_e:_e.filter(p=>(p.recipients||(p.owner_id?[p.owner_id]:[])).includes(l));if($.length===0){t.innerHTML=W("empty",l==="all"?"No active tasks.":"No tasks found for this owner.",z()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let E=$.map(p=>{let w=p.last_run===0?"Never":new Date(p.last_run*1e3).toLocaleString(),A=p.memory?p.memory.length:0,_=p.status==="active"?"service-widget-online":"service-widget-offline",B=p.status==="active"?"ACTIVE":"PAUSED",V=(p.recipients||(p.owner_id?[p.owner_id]:[])).sort((M,T)=>{let S=M.startsWith("channel:"),C=T.startsWith("channel:");return S&&!C?-1:!S&&C?1:0}).map(M=>{let T=Be[M]||M.substring(0,8),S=M.startsWith("channel:");return`<span title="${T}" style="display: flex; align-items: center; gap: 3px; white-space: nowrap;"><i class='bx ${M==="dexter"?"bx-terminal":S?"bx-hash":"bx-user"}'></i>${T}</span>`}).join("<span style='color: #444; margin: 0 5px;'>|</span>"),Z=`${p.schedule}${p.run_at?" @ "+p.run_at+(p.timezone?" ("+p.timezone.split("/").pop()?.replace("_"," ")+")":""):""}`;return`
            ...
            `}).join("");t.innerHTML=E,t.querySelectorAll(".edit-chore-btn").forEach(p=>{p.onclick=w=>{w.stopPropagation();let A=p.dataset.id,_=_e.find(B=>B.id===A);_&&f(_)}}),t.querySelectorAll(".reset-chore-btn").forEach(p=>{p.onclick=async w=>{w.stopPropagation();let A=p.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(p.innerHTML="<i class='bx bx-loader-alt spin'></i>",await H(`/chores/${A}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),He())}}),t.querySelectorAll(".delete-chore-btn").forEach(p=>{p.onclick=async w=>{w.stopPropagation();let A=p.dataset.id;confirm("Delete this task?")&&(await H(`/chores/${A}`,{method:"DELETE"}),He())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{a&&(a.style.display==="none"||he!==null?f(null):a.style.display="none")},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{a&&(a.style.display="none")},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),$=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),p=l?.value,w=E?.value||"every_24h",A=w==="daily"?y?.value:"",_=w==="daily"?h?.value:"";if(!p)return;if(ie.length===0){alert("Please add at least one recipient.");return}let B=n.innerHTML;n.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let P=he?"PATCH":"POST",V=he?`/chores/${he}`:"/chores";await H(V,{method:P,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:ie,natural_instruction:p,entry_url:$?.value,schedule:w,run_at:A,timezone:_})}),a&&(a.style.display="none"),l&&(l.value=""),$&&($.value=""),he=null,ie=[],He()}catch(P){console.error(P),alert(he?"Failed to update research task":"Failed to create research task")}finally{n.innerHTML=B}},n.dataset.listenerAttached="true");try{let l=await H("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);if(_e=await l.json()||[],m){let E=m.value,p=new Set;_e.forEach(w=>{w.owner_id&&p.add(w.owner_id),w.recipients&&w.recipients.forEach(A=>p.add(A))}),m.innerHTML='<option value="all">All Recipients</option>',p.forEach(w=>{let A=document.createElement("option");A.value=w;let _=w.startsWith("channel:"),B=Be[w]||`Recipient: ${w.substring(0,8)}`;A.textContent=(_&&!B.startsWith("#")?"#":"")+B,m.appendChild(A)}),m.value=E,m.selectedIndex===-1&&(m.value="all")}d()}catch(l){console.error("Failed to fetch chores",l)}}var Mt=()=>`
...
`;async function et(){await Promise.all([xe(),He()])}var xs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function At(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=xs,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),oe()||(document.body.style.overflow=""),setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden";let a=n=>{e.innerHTML=`
    ...
    `;let s=e.querySelector(".close-profile-btn");s&&s.addEventListener("click",()=>{e.click()})};ve(`/profile/${t.id}`).then(async n=>{if(n.ok){let s=await n.json();hs(e,t,s)}else console.log("Profile not found (404) or error."),a("Profile Data Unavailable")}).catch(n=>{console.error("Profile fetch error:",n),a("Connection Failed")})}function hs(t,e,a){let n=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",s=()=>{let L=(a.attributes||a.facts||[]).map(d=>{let l=d.key||d.k,$=d.value||d.v;return`
            <div class="fact-chip">
                <span class="fact-key">${l}:</span>
                <span class="fact-val">${$}</span>
            </div>
        `}).join(""),I=a.cognitive_model||a,b=I.technical_level||I.techLevel||0,f=I.communication_style||I.commStyle||"Unknown",v=I.patience_level||I.patience||"Unknown",x=I.vibe||"Unknown";return`
        ...
        `},o=()=>{let L=a.dossier||{},I=L.identity||{},b=L.career||{},f=L.personal||{},v=L.social||[];return`
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(b.skills||[]).map(x=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${x}</span>`).join("")}
                         </div>
                    </div>
                </div>
...
`,ws=null;async function tt(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await tt(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=a=>{let s=a.target.closest(".user-profile-card");if(s&&s.dataset.user)try{let o=JSON.parse(s.dataset.user);At(o)}catch(o){console.error("Failed to parse user data",o)}},t.dataset.listenerAttached="true");try{let a=await ve("/contacts");if(!a.ok)throw new Error("Service unreachable");let s=(await a.json()).members||[];if(ws=Date.now(),s.length===0){t.innerHTML=W("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((i,m)=>{let c=o[i.level]??10,g=o[m.level]??10;return c!==g?c-g:i.username.localeCompare(m.username)}),t.innerHTML=s.map(i=>{let m=i.color&&i.color!==0,c=m?"#"+i.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",g=i.status==="online"?"#5eff5e":i.status==="idle"?"#ffa500":i.status==="dnd"?"#ff4d4d":"#666",k="#888";i.level==="Me"||i.level==="Master"?k="#bb86fc":i.level==="Admin"?k="#cf6679":i.level==="Moderator"?k="#03dac6":i.level==="Contributor"&&(k="#ffa500");let y=i.level==="Me",h=y?"rgba(187, 134, 252, 0.08)":m?`${c}11`:"rgba(255,255,255,0.03)",r=y?"2px solid #bb86fc":m?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(i).replace(/"/g,"&quot;")}"
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${y?"#bb86fc":m?c:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${i.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${y?"2px solid #bb86fc":m?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
...
`,Je=null,we=new Set,ze=[],Q="all",Ts={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals","system.cognitive.model_inference"],moderation:["moderation.explicit_content.deleted"]},$s={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart","system.cognitive.model_inference":"bx-brain"};function ks(t){for(let[e,a]of Object.entries(Ts))if(a.includes(t))return e;return"system"}function Ss(t){return $s[t]||"bx-square-rounded"}async function ue(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ls();let n=`/events?ml=${Q==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Q!=="all"&&(n+=`&category=${Q}`);try{let s=await H(n);if(!s.ok)throw new Error("Service unreachable");if(ze=((await s.json()).events||[]).filter(h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!0}let u=r.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Je=Date.now(),pe(1,Je),ze.length===0){e.innerHTML=W("empty","No events found for this filter.");return}t&&(e.innerHTML="");let m=h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let u=r.type,L=ks(u),I=Ss(u),b=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.cognitive.model_inference"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),f="event-border-grey";b&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?f="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?f="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?f="event-border-orange":u==="system.test.completed"?f=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":f="event-border-blue");let v=b?"cursor-pointer":"",x=we.has(h.id),d=x?"expanded":"",l=x?"display: block;":"display: none;",$=new Date(h.timestamp*1e3),E=$.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=$.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Bt(u,r),A=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",_="";if(b){let M="";if(u==="engagement.decision"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                    ...
                    `}else if(u==="system.cognitive.model_inference"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                    ...
                    `}else if(u==="system.attention.expired"){let T=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=r.timestamp-r.last_active,C=Math.floor(S/60);M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let T=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,S="";if(r.embed){let D=r.embed,K=D.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${N(D.title)}</div>`:"",U=D.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${ne(D.description)}</div>`:"",j=(D.fields||[]).map(q=>`
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${N(q.name)}</div>
                <div style="font-size: 0.85em; color: #eee; white-space: pre-wrap;">${ne(q.value)}</div>
              </div>
            `).join("");S=`
            ...
            `}let C="";r.eval_count&&(C=`
                        ...
                        `);let O="";if(r.chat_history&&r.chat_history.length>0){let D=r.chat_history.length,K=r.chat_history.map((U,j)=>{let q=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&q==="ASSISTANT"&&(q="DEXTER");let G=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",ee=j===D-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${ee};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${G}; letter-spacing: 1px; font-weight: bold;">${q}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${j+1} of ${D}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${N(U.content)}</div>
                                </div>
                            `}).join("");O=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let T=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,S="",C=[],O=r.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let D=O.match(/{.*?}/g);D&&(C=D.map(K=>{let U=K.substring(1,K.length-1),j=U.indexOf(" "),q=U.substring(0,j),G=U.substring(j+1);return G=G.replace(/\[\]$/,"").trim(),{role:q,content:G}}))}if(C.length>0&&r.response_raw&&C.push({role:"assistant",content:r.response_raw}),C.length>0){let D=C.length,K=C.map((U,j)=>{let q=U.role.toUpperCase();q==="ASSISTANT"&&(q="DEXTER");let G=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",ee=j===D-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${ee};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${G}; letter-spacing: 1px; font-weight: bold;">${q}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${j+1} of ${D}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let T=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=r.signals||{};M=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${T("Raw Model Output")}
                            <pre class="detail-pre">${N(r.raw_output)||"None"}</pre>
                        </div>
                    `}else if(u==="moderation.explicit_content.deleted"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${N(r.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${T("Description")}
                            <pre class="detail-pre">${N(r.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${T("Content Summary")}
                            <pre class="detail-pre">${N(r.summary)||"None"}</pre>
                        </div>
                    `}else if(u==="analysis.visual.completed"){let T=C=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${C}</h5>`,S="";r.base64_preview?S=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${r.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:r.url?S=`<div class="event-detail-block"><img src="${r.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:S=`
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(u==="analysis.router.decision"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${T("Input Context")}
                            <pre class="detail-pre">${N(r.raw_input)||"None"}</pre>
                        </div>
                    `}else if(u==="system.cli.command"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${T("Output")}
                            <pre class="detail-pre">${N(r.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(u==="system.analysis.audit"){let T=r.success?"#03dac6":"#ff4d4d",S=r.success?"SUCCESS":"FAILED",C=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,O="";r.error&&(O=`
                            <div class="event-detail-block">
                                ${C("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${N(r.error)}</pre>
                            </div>
                        `);let D="";if(r.chat_history&&r.chat_history.length>0){let j=r.chat_history.length,q=r.chat_history.map((G,ee)=>{let Y=G.name?G.name.toUpperCase():G.role.toUpperCase();!G.name&&Y==="USER"&&(Y="SYSTEM"),!G.name&&Y==="ASSISTANT"&&(Y="AGENT");let Ce=G.role==="user"?"#03dac6":G.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let K="";if(r.corrections&&r.corrections.length>0){let j=r.corrections.map((q,G)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${q.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${N(q.snippet)}</div>`:""}
                            </div>
                        `).join("");K=`
                            <div class="event-detail-block">
                        ...
                        `}let U="";if(r.parsed_results&&r.parsed_results.length>0){let j=r.parsed_results.map(q=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${N(q.title)}</div>
                                <div style="color: #ddd;">${N(q.summary)}</div>
                            </div>
                        `).join("");U=`
                            <div class="event-detail-block">
                                ${C("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${T}; font-weight: bold;">${S} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${r.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Duration</div>
                        ...
                        ${K}
                        ${D}
                    `}else if(u==="system.test.completed"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                        ...
                        <div class="event-detail-block">
                            ${T("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${N(r.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${T("Context")}
                            <pre class="detail-pre">${JSON.stringify(r.context||{},null,2)}</pre>
                        </div>
                    `}else if(u==="system.cli.status"){let T=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ${T("Message")}
                            <pre class="detail-pre">${N(r.message)}</pre>
                        </div>
                    `}else if(u==="messaging.user.sent_message"){let T="";r.attachments&&r.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${r.attachments.map(C=>{let O=C.content_type&&C.content_type.startsWith("image/"),D=(C.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${O?`<div class="attachment-thumb-container"><img src="${C.url}" alt="${C.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                ...
                `}let B=document.createElement("div");B.className=`event-item ${f} ${v} ${d}`,B.dataset.eventId=h.id,B.onclick=function(M){if(!b)return;let T=this;if(T.classList.contains("expanded")){T.classList.remove("expanded"),we.delete(h.id);let C=T.querySelector(".event-details");C&&(C.style.display="none")}else{T.classList.add("expanded"),we.add(h.id);let C=T.querySelector(".event-details");C&&(C.style.display="block")}},B.innerHTML=`
                <div class="event-time">
            ...
            `;let P=B.querySelector(".event-details");if(P&&P.addEventListener("click",M=>{M.stopPropagation()}),b){let M=B.querySelector(".close-details-btn");M&&M.addEventListener("click",T=>{T.stopPropagation();let S=T.target.closest(".event-item");if(S){S.classList.remove("expanded"),we.delete(h.id);let C=S.querySelector(".event-details");C&&(C.style.display="none")}})}let V=B.querySelector(".prev-btn"),Z=B.querySelector(".next-btn");if(V&&Z){let M=0,T=Array.from(B.querySelectorAll(".history-slide")),S=T.length,C=()=>{T.forEach((O,D)=>{O.style.display=D===M?"block":"none"}),V.disabled=M===0,Z.disabled=M===S-1,V.style.opacity=M===0?"0.5":"1",Z.style.opacity=M===S-1?"0.5":"1"};V.addEventListener("click",O=>{O.stopPropagation(),M>0&&(M--,C())}),Z.addEventListener("click",O=>{O.stopPropagation(),M<S-1&&(M++,C())}),C()}return B},c=Array.from(e.children),g=new Map(c.map(h=>[h.dataset.eventId,h])),k=new Set(ze.map(h=>h.id));c.forEach(h=>{let r=h.dataset.eventId;(!r||!k.has(r))&&h.remove()});let y=null;ze.forEach((h,r)=>{let u=g.get(h.id);(!u||t)&&(u&&u.remove(),u=m(h),!u)||(r===0?e.firstElementChild!==u&&e.prepend(u):y&&y.nextElementSibling!==u&&y.after(u),y=u)}),Je=Date.now(),pe(1,Je)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=W("offline","Failed to load events.","The event service may be offline."))}}function Ls(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),a=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ze.forEach(s=>we.add(s.id)),ue(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{we.clear(),ue(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{a.querySelectorAll(".filter-btn").forEach(o=>o.classList.remove("active")),s.classList.add("active"),Q=s.dataset.filter||"all",ue(!0)}}),a.dataset.listenerAttached="true"),a&&a.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===Q?s.classList.add("active"):s.classList.remove("active")});let n=document.getElementById("events-clear");n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let s=Q==="all"?"ALL":Q.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let o="/events";if(Q!=="all"?o+=`?category=${Q}`:o+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await H(o,{method:"DELETE"})).ok)throw new Error("Failed to delete events");we.clear(),ue(!0)}catch(o){console.error("Failed to clear events:",o),alert("Failed to clear events. Check console.")}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}function Ht(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Cs=null;async function De(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await H("/logs");if(!e.ok)throw new Error("Logs offline");let a=await e.json();if(!a||a.length===0)return t.innerHTML=W("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let n=["local-model-0","local-cache-0","upstash-redis-rw","upstash-redis-ro","easter-company","easter-company-production","dex-test-service","easter-company-root"],s=a.filter(i=>!n.includes(i.id));s.forEach(i=>{i.logs&&Array.isArray(i.logs)?i.logs.reverse():i.logs=[]}),s.reverse();let o=s.map(i=>{let m=i.logs.join(`
`),c=[...i.logs];if(c.length<25){let k=25-c.length;for(let y=0;y<k;y++)c.push("")}else c.length>25&&(c=c.slice(-25));let g=c.map(k=>Ue(k)).join(`
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${i.id}" title="Clear Logs" style="${z()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=o,document.querySelectorAll(".copy-logs-btn").forEach(i=>{i.addEventListener("click",()=>{let m=i,c=unescape(m.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let g=m.querySelector("i");g&&(g.classList.remove("bx-copy"),g.classList.add("bx-check"),setTimeout(()=>{g.classList.remove("bx-check"),g.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(i=>{i.addEventListener("click",async()=>{let c=i.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await H(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&De()}catch(g){console.error("Error clearing logs:",g)}})}),Cs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=W("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Ee="STANDBY",zt=null,ae=[],te=null,nt=null,Dt=()=>`
    <div id="progress-view-root" class="progress-container" style="flex: 1; overflow-y: auto;">
        ${Rt()}
    </div>
  `;function Rt(){switch(Ee){case"ACTIVE":return Is();case"COMPLETED":return As();case"STANDBY":default:return Ms()}}function Ms(){return`
    ...
    <div class="mission-summary-card">
        ...
        <div class="summary-stat-grid">
            <div class="summary-stat">
                <span class="summary-stat-value">${nt?.duration||"-"}</span>
        ...
        <div style="background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; margin-bottom: 30px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-family: 'JetBrains Mono'; font-size: 0.9em; color: #bb86fc;">${nt?.result||"Task Finalized"}</span>
                <i class='bx bx-chevron-right' style="color: #444;"></i>
            </div>
        </div>
    </div>
  `}async function Nt(){try{let e=await(await H("/processes")).json(),a=e.active||[],n=e.history||[],s=a.find(i=>i.channel_id==="system-fabricator");if(s){Ee="ACTIVE";let m=Math.floor(Date.now()/1e3)-s.start_time,c=10,g="REVIEW";m>30&&(c=30,g="INVESTIGATION"),m>120&&(c=60,g="CONSTRUCTION"),m>300&&(c=90,g="REPORTING"),te={title:s.state||"Fabricating system improvements...",progress:c,phase:g};let k=new Date(s.updated_at*1e3).toLocaleTimeString();(ae.length===0||ae[ae.length-1].msg!==s.state)&&(ae.push({time:k,msg:s.state}),ae.length>50&&ae.shift())}else{let i=n.find(m=>m.channel_id==="system-fabricator");if(i&&Date.now()/1e3-i.end_time<60){Ee="COMPLETED";let m=i.end_time-i.start_time;nt={duration:`${Math.floor(m/60)}m ${m%60}s`,steps:0,result:i.state,timestamp:i.end_time}}else Ee="STANDBY",ae=[]}let o=document.getElementById("progress-view-root");o&&(Ee!==zt&&(o.innerHTML=Rt(),zt=Ee),Ee==="ACTIVE"&&_s())}catch(t){console.error("Failed to update progress:",t)}}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),a=document.getElementById("active-task-progress-text"),n=document.getElementById("active-task-phase"),s=document.getElementById("terminal-output");if(t&&te&&(t.innerText=te.title),e&&te&&(e.style.width=`${te.progress}%`),a&&te&&(a.innerText=`${te.progress}% COMPLETE`),n&&te&&(n.innerText=`PHASE: ${te.phase}`),s){let o=s.children.length;if(ae.length>o){for(let i=o;i<ae.length;i++){let m=ae[i],c=document.createElement("div");c.className="terminal-line",c.innerHTML=`
            <span class="line-time">${m.time}</span>
            <span class="line-prefix">></span>
            <span class="line-msg">${N(m.msg)}</span>
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ht()}
        </div>`;async function Wt(){Bs(),await Promise.all([re(),Te(),ht()]),await De()}var We=null;function Bs(){We||(We=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(We),We=null;return}re(!0)},1e3))}var Ot=null;async function Hs(){try{return await(await H("/system_monitor")).json()}catch{return null}}async function Pt(){try{return await(await H("/system/hardware")).json()}catch{return null}}async function zs(){try{return await(await H("/processes")).json()}catch{return null}}async function Ds(){try{return await(await H("/agent/status")).json()}catch{return null}}async function Te(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),a=document.getElementById("global-restart-btn"),n=document.getElementById("global-stop-btn"),s=document.getElementById("global-start-btn"),o=(d,l)=>{d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{let $=d.innerHTML;d.innerHTML="<i class='bx bx-loader-alt spin'></i>",d.disabled=!0;try{await H(`/system/service/${l}`,{method:"POST",body:JSON.stringify({service:"all"})}),setTimeout(()=>Te(),2e3),setTimeout(()=>Te(),5e3),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML=$,d.disabled=!1},1e3)},1e3)}catch(E){console.error(E),d.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{d.innerHTML=$,d.disabled=!1},2e3)}},d.dataset.listenerAttached="true")};o(a,"restart"),o(n,"stop"),o(s,"start");let i=document.querySelector("#hw-os .hw-content"),m=document.querySelector("#hw-cpu .hw-content"),c=document.querySelector("#hw-ram .hw-content"),g=document.querySelector("#hw-gpu .hw-content"),k=document.querySelector("#hw-storage .hw-content"),y=d=>{if(d){if(i&&(i.innerHTML=`
                ...
                </div>`}if(m&&d.CPU&&d.CPU.length>0){let l=d.CPU[0];m.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):k&&(k.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let d=await Pt();d?(y(d),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),m&&!m.hasChildNodes())){let d=await Pt();y(d)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async d=>{let l=d.target.closest(".svc-btn");if(!l)return;let $=l,E=$.dataset.action,p=$.dataset.service;if(!E||!p)return;let w=$.innerHTML;$.innerHTML="<i class='bx bx-loader-alt spin'></i>",$.classList.add("loading"),$.disabled=!0;try{await H(`/system/service/${E}`,{method:"POST",body:JSON.stringify({service:p})}),setTimeout(()=>Te(),1e3),setTimeout(()=>Te(),3e3)}catch(A){console.error(A),alert(`Failed to ${E} ${p}`),$.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{$.innerHTML=w,$.classList.remove("loading"),$.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let h=await Hs();if(!h||!h.services){t.children.length===0&&(t.innerHTML=W("offline","Failed to load system metrics.","The event service may be offline."));return}Ot=z()&&be.lastDashboard||Date.now(),pe(3,Ot||0);let r=h.services||[],u={cli:[],fe:[],cs:[],be:[],th:[],co:[],md:[],prd:[],os:[]},L={cli:"CLI",fe:"Front-end",cs:"Core",be:"Backend",th:"Third Party",co:"Cognitive",md:"Models",prd:"Production",os:"Other"};r.forEach(d=>{u[d.type]?u[d.type].push(d):u.os.push(d)});let I="";Object.entries(u).forEach(([d,l])=>{l.length!==0&&(I+=`
            ...
            <div class="service-category-grid system-monitor-widgets" style="padding-bottom: 20px;">
                ${l.map($=>x($)).join("")}
            </div>`)}),t.innerHTML!==I&&(t.innerHTML=I);function b(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/^(\d+\.\d+\.\d+)/);return l?l[0]:d.split(".").slice(0,3).join(".")||"-"}function f(d){return!d||d.length<=22?d:d.substring(0,22)+"..."}function v(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!l)return"-";let $=parseInt(l[1])||0,E=parseInt(l[2])||0,p=parseInt(l[3])||0,w=parseFloat(l[4])||0,A=$*86400+E*3600+p*60+w,_=Math.floor(A/86400);if(_>0)return`${_}d`;let B=Math.floor(A/3600);if(B>0)return`${B}h`;let P=Math.floor(A/60);return P>0?`${P}m`:`${Math.floor(A)}s`}function x(d){if(d.id==="upstash-redis-ro"){let D=z()&&(be.lastFrontend||be.lastDashboard)||Date.now();return`
            ...
            </div>`}let l=d.status==="online",$=l?"service-widget-online":"service-widget-offline",E=l?"bx-check-circle":"bx-x-circle",p=l?"OK":"BAD",w=d.version?b(d.version.str):"-",A=d.cpu&&d.cpu!=="N/A"?d.cpu:"-",_=d.memory&&d.memory!=="N/A"?d.memory:"-",B=A!=="-"?"#00ff00":"#666",P=A!=="-"?"#fff":"#666",V=_!=="-"?"#00ff00":"#666",Z=_!=="-"?"#fff":"#666",M=v(d.uptime),T="",S=d.type!=="os"&&d.type!=="cli"&&d.type!=="prd"&&d.type!=="md",C="";if(S&&!z()&&(C=`
            <div class="service-controls">
            ...
            </div>`),l){let D=d.type==="md"?"Size:":"Version:",K=d.type==="md"?d.memory:w,U=d.type==="md"?"Type:":"RAM:",j=d.type==="md"?d.health_message||"Model":_;T=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item" title="${U}">
                ...
                </div>`}else T='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold; margin-top: auto;"><span>OFFLINE</span></div>';let O=z()||d.type==="md"?"easter.company":f(d.domain&&d.port?`${d.domain}:${d.port}`:"");return`<div class="service-widget ${$}" data-service-id="${d.id}"><div class="service-widget-header"><i class="bx ${E}"></i><h3>${d.short_name||d.id}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${T}</div>${C}</div>`}}async function re(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let a=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),i=document.getElementById("guardian-total-waste"),m=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),g=document.getElementById("fabricator-reset-btn"),k=document.getElementById("fabricator-progress-btn"),y=document.getElementById("courier-reset-btn"),h=document.getElementById("system-pause-toggle-btn");k&&!k.dataset.listenerAttached&&(k.onclick=()=>{let d=document.querySelector("nav");d&&(d.style.display="none");let l=le({id:"fabricator-progress-window",title:"Mission Control",icon:"bx-loader-circle",className:"fabricator-progress-modal",appendToBody:!0,content:`
                        ...
                        `,onOpen:()=>{let p=document.getElementById("fabricator-progress-window");p&&p.querySelector(".close-modal-btn")?.addEventListener("click",()=>l.close()),Nt()},onClose:()=>{d&&(d.style.display=""),window.removeEventListener("popstate",$),document.removeEventListener("visibilitychange",E)}}),$=()=>l.close(),E=()=>{document.hidden&&l.close()};window.addEventListener("popstate",$),document.addEventListener("visibilitychange",E),l.open()},k.dataset.listenerAttached="true"),h&&!h.dataset.listenerAttached&&(h.onclick=async()=>{let l=h.querySelector(".bx-play")?"/agent/resume":"/agent/pause";h.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await H(l,{method:"POST"}),await re()}catch{h.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>re(),2e3)}},h.dataset.listenerAttached="true"),m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await H("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{m.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{m.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{m.innerHTML="<i class='bx bx-error'></i>"}},m.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await H("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await H("/agent/reset?protocol=review",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await H("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{y.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{y.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true");let r=document.getElementById("architect-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i>",setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500)},r.dataset.listenerAttached="true");let u=await Ds();if(u&&u.agents){let d=u.agents.guardian||{protocols:{}},l=u.agents.analyzer||{protocols:{}},$=u.agents.architect||{protocols:{}},E=u.agents.fabricator||{protocols:{}},p=u.agents.courier||{protocols:{}},w=u.system||{},A=Math.floor(Date.now()/1e3),_={sentry:"Sentry",synthesis:"Synthesis",architect:"Architect",review:"Review",issue:"Issue",construct:"Construct",reporter:"Reporter",researcher:"Researcher",compressor:"Compressor"},B=F=>{F<0&&(F=0);let X=Math.floor(F/3600),J=Math.floor(F%3600/60),$e=F%60;return X>0?`${X}h ${J}m`:J>0?`${J}m ${$e}s`:`${$e}s`},P=(F,X,J,$e,ge)=>{if(!F)return;let Me=_[$e]||$e.toUpperCase(),rt=F.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(rt&&(rt.textContent=Me),ge==="paused"){F.textContent="PAUSED",F.style.color="#ff9800";return}if(!J){F.textContent="Inactive",F.style.color="#666";return}let lt=J.status;if(lt==="Working")F.textContent="Working",F.style.color="#bb86fc";else if(lt==="Ready")F.textContent="Ready",F.style.color="#5eff5e";else{let Oe=J.cooldown;if(z()&&(Oe=J.next_run-A),Oe<=0)F.textContent="Ready",F.style.color="#5eff5e";else{let ts=Math.floor(Oe/60),ss=Oe%60;F.textContent=`${ts}m ${ss}s`,F.style.color="#fff"}}X&&J.stats&&(X.innerHTML=`
          ...
          `)};a&&P(a,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry",w.state);let V=document.getElementById("analyzer-summary-val"),Z=document.getElementById("analyzer-summary-stats");V&&P(V,Z,l.protocols.summary,"summary",w.state);let M=document.getElementById("analyzer-synthesis-val"),T=document.getElementById("analyzer-synthesis-stats");M&&P(M,T,l.protocols.synthesis,"synthesis",w.state);let S=document.getElementById("architect-alert_review-val"),C=document.getElementById("architect-alert_review-stats");S&&P(S,C,$.protocols.architect,"architect",w.state);let O=document.getElementById("fabricator-review-val"),D=document.getElementById("fabricator-review-stats");O&&P(O,D,E.protocols.review,"review",w.state);let K=document.getElementById("fabricator-issue-val"),U=document.getElementById("fabricator-issue-stats");K&&P(K,U,E.protocols.issue,"issue",w.state);let j=document.getElementById("fabricator-construct-val"),q=document.getElementById("fabricator-construct-stats");j&&P(j,q,E.protocols.construct,"construct",w.state);let G=document.getElementById("fabricator-reporter-val"),ee=document.getElementById("fabricator-reporter-stats");G&&P(G,ee,E.protocols.reporter,"reporter",w.state);let Y=document.getElementById("courier-researcher-val"),Ce=document.getElementById("courier-researcher-stats");Y&&P(Y,Ce,p.protocols.researcher,"researcher",w.state);let Re=document.getElementById("courier-compressor-val"),Ge=document.getElementById("courier-compressor-stats");Re&&P(Re,Ge,p.protocols.compressor,"compressor",w.state);let Ne=document.getElementById("system-state-label"),fe=document.getElementById("system-state-val");if(fe&&w.state){let F=w.state,X=B(w.state_time||0);Ne&&(Ne.textContent=`State: ${F.toUpperCase()}`),fe.textContent=X,F==="idle"?fe.style.color=w.state_time>300?"#5eff5e":"#fff":fe.style.color="#bb86fc",h&&(F==="paused"?(h.innerHTML="<i class='bx bx-play'></i>",h.title="Resume System",h.style.color="#ff9800"):(h.innerHTML="<i class='bx bx-pause'></i>",h.title="Pause System",h.style.color=""))}s&&(s.textContent=B(w.metrics?.total_idle_time||0)),o&&(o.textContent=B(w.metrics?.total_active_time||0)),i&&(i.textContent=B(w.metrics?.total_waste_time||0))}else[a,n,s,o,i].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});if(t)return;let L=await zs(),I=[],b=[],f=[];L&&(Array.isArray(L)?I=L:(I=L.active||[],b=L.queue||[],f=L.history||[],f.sort((d,l)=>(l.end_time||0)-(d.end_time||0)))),e&&(I.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=W("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,I,!1)));let v=document.getElementById("processes-queue-widgets");v&&(b.length===0?!v.querySelector(".tab-placeholder")&&!v.querySelector('div[style*="font-style: italic"]')&&(v.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(v.innerHTML="",it(v,b,!1)));let x=document.getElementById("processes-history-widgets");x&&(f.length===0?x.querySelector(".tab-placeholder")||(x.innerHTML=W("empty","No recent history.")):(x.querySelector(".tab-placeholder")&&(x.innerHTML=""),it(x,f,!0))),yt(0,I.length)}function it(t,e,a){function n(c){let g="";c.end_time?g=`${c.end_time-c.start_time}s`:g=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let k=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",y=c.channel_id,h={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-architect":"Architect Agent","system-fabricator":"Fabricator Agent","system-test":"System Validation","voice-mode":"Voice Interaction","system-context-summary-test-channel-id":"Context Summary Test"};if(h[y]?y=h[y]:y.startsWith("system-context-summary-")?y=`Context Compression: ${y.replace("system-context-summary-","")}`:/^\d+$/.test(y)&&(y=`Channel ${y}`),a)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let s=a?".process-history-item":".process-widget",o=new Map(Array.from(t.querySelectorAll(s)).map(c=>[c.dataset.channelId,c])),i=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,g]of o)c&&!i.has(c)&&g.remove();let m=new Set;e.forEach(c=>{let g=`${c.channel_id}-${c.start_time}`;if(m.has(g))return;m.add(g);let k=n(c),y=o.get(g);if(y&&y.parentNode){y.outerHTML!==k&&(y.outerHTML=k);let h=t.querySelector(`[data-channel-id="${g}"]`);h&&t.appendChild(h)}else t.insertAdjacentHTML("beforeend",k)})}function Ve(){let t=Ke(),e="Notification"in window,a={enabled:e&&Notification.permission==="granted",supported:e};return`
            ...
            </div>`}function at(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(n=>{n.addEventListener("click",function(){let s=this.dataset.theme;s&&(ft(s),t.setContent(Ve()),at(t))})});let a=document.getElementById("notifications-toggle");a&&"Notification"in window?a.onclick=async n=>{let s=n.target;if(s.checked)try{await Notification.requestPermission()!=="granted"&&(s.checked=!1)}catch{s.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),s.checked=!0)}:a&&(a.disabled=!0),Rs()}async function Rs(){let t=document.getElementById("service-config-list");if(!t)return;let e=z();try{let n=await(await H("/system/options")).json(),s=n.services||n||{},o="",i=(m,c,g)=>`

        <div class="settings-item">

            <div class="settings-item-info">

                <span class="settings-item-label">${m}</span>
    ...
    `;async function Kt(){let t=document.getElementById("web-analysis-sidebar"),e=document.getElementById("web-sidebar-toggle"),a=document.getElementById("web-switch-btn"),n=document.getElementById("web-refresh-btn"),s=document.getElementById("web-view-title");e&&!e.dataset.listenerAttached&&(e.onclick=()=>{if(t){let i=t.style.display==="none";t.style.display=i?"flex":"none",e.style.color=i?"#03dac6":""}},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{if(t){let i=t.style.display!=="none";t.style.display=i?"none":"flex",s&&(s.textContent=i?"Web View":"Web Analysis")}},a.dataset.listenerAttached="true");let o=()=>{let i=window.innerWidth<880;t&&e&&a&&s&&(i?(e.style.display="none",t.style.display="none",s.textContent="Web View"):(e.style.display="flex",t.style.display="flex",s.textContent="Web View"))};window.hasWebResizeListener||(window.addEventListener("resize",o),window.hasWebResizeListener=!0),o(),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{n.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Vt(),n.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{n.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},n.dataset.listenerAttached="true"),await Vt()}async function Vt(){let t=document.getElementById("web-analysis-content"),e=document.getElementById("web-main-frame"),a=document.getElementById("web-frame-placeholder");if(!(!t||!e||!a))try{let n=await H("/web/view");if(!n.ok)throw new Error("Failed to fetch web view state");let s=await n.json();if(!s||!s.url){a.style.display="flex",e.style.display="none",t.innerHTML=`
                <div style="text-align: center; color: #444; padding-top: 50px;">
                    <i class='bx bx-analyse' style="font-size: 2rem; opacity: 0.3;"></i>
                    <p style="font-size: 0.7em; text-transform: uppercase; letter-spacing: 1px; margin-top: 10px;">No active analysis</p>
                </div>`;return}e.src!==s.url&&(a.style.display="none",e.style.display="block",e.src=s.url);let o="";if(s.metadata&&(o+=`
            ...
            `),s.scrape&&(o+=`
                <div class="analysis-section" style="margin-bottom: 25px;">
                    ...
                    <div style="background: rgba(0,0,0,0.3); border-radius: 8px; padding: 12px; border: 1px solid rgba(255,255,255,0.03); max-height: 200px; overflow-y: auto; font-family: 'JetBrains Mono', monospace; font-size: 0.7em; color: #aaa; white-space: pre-wrap;">${N(s.scrape.content)||"No content."}</div>
                </div>
            `),s.visual){let i=s.visual.screenshot||s.visual.screenshot_base64||s.visual.base64,m=i?`<img src="data:image/png;base64,${i}" style="width: 100%; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); margin-top: 10px; cursor: pointer;" onclick="window.open('data:image/png;base64,${i}')">`:"";o+=`
                <div class="analysis-section" style="margin-bottom: 25px;">
            ...
            `}o||(o=`<div style="text-align: center; color: #444; padding-top: 50px;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ns=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[a,n]of Object.entries(t)){let s=Zt.filter(o=>o.category===a);s.length!==0&&(e+=`
    ...
    </div>`,e},ot=!1;function Os(t){let e=document.getElementById("cli-terminal-overlay");if(!e){e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>Yt());let g=document.getElementById("terminal-action-btn");g&&(g.onclick=()=>Yt())}let a=document.getElementById("cli-terminal-body"),n=document.getElementById("cli-docs-pane");a&&(a.innerHTML="");let s=t.docs||{overview:t.description,details:[],extended_usage:t.usage};n&&(n.innerHTML=`
    ...
    `);let o=document.getElementById("terminal-cmd-name");o&&(o.textContent=`dex ${t.id}`);let i=document.getElementById("terminal-status-badge");i&&(i.className="terminal-status status-running",i.textContent="Running");let m=document.getElementById("terminal-action-btn");return m&&(m.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",ot=!0,a}function Yt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),oe()||(document.body.style.overflow=""),ot=!1}function Xt(t,e,a="output"){if(!ot||!t)return;let n=document.createElement("div");if(n.className=`terminal-line terminal-${a}`,a==="prompt")n.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let s=Ue(e);n.innerHTML=s||e}t.appendChild(n),t.scrollTop=t.scrollHeight}async function Ps(t){let e=Zt.find(i=>i.id===t);if(!e)return;let a=Os(e);a&&Xt(a,`dex ${t}`,"prompt");let n=e.demo_output||["Executing command...","\u2713 Done."];for(let i of n){await new Promise(c=>setTimeout(c,400+Math.random()*600));let m="output";i.includes("[ERROR]")?m="error":i.includes("[SUCCESS]")||i.includes("\u2713")?m="success":i.includes("!")&&(m="warning"),a&&Xt(a,i,m)}let s=document.getElementById("terminal-status-badge");s&&(s.className="terminal-status status-success",s.textContent="Completed (Demo)");let o=document.getElementById("terminal-action-btn");o&&(o.style.display="block")}function Qt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ns();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let a=e.querySelector("code"),n=a?a.textContent:"";n&&navigator.clipboard.writeText(n).then(()=>{let s=e.querySelector("i");s&&(s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(a=>{let n=a;a.addEventListener("mouseenter",()=>{n.style.transform="translateY(-5px)",n.style.borderColor="rgba(255,255,255,0.15)",n.style.backgroundColor="rgba(255,255,255,0.05)"}),a.addEventListener("mouseleave",()=>{n.style.transform="translateY(0)",n.style.borderColor="rgba(255,255,255,0.05)",n.style.backgroundColor="rgba(255,255,255,0.03)"}),a.addEventListener("click",()=>{let s=n.dataset.cmd;s&&Ps(s)})})}async function js(){if(!z())try{if(!(await H("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function es(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}dt(),pt(),js(),bt(),ct();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Qt();let e=[],a=document.getElementById("windows-container");a&&a.setAttribute("data-count","0");let n=b=>{localStorage.setItem("dex_last_window",b)};function s(){for(;e.length>1;)e.shift().close(!0);let b=document.getElementById("windows-container"),f=document.querySelector("nav"),v=document.querySelector("footer"),x=window.location.pathname==="/"||window.location.pathname==="/index.html",d=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");b&&b.setAttribute("data-count",e.length.toString()),e.forEach(_=>{let B=document.getElementById(_.id);B&&B.classList.remove("hide-close")});let l=document.getElementById("dexter-menu-container"),$=document.getElementById("nav-window-switcher"),E=document.getElementById("settings-icon"),p=document.getElementById("docs-icon"),w=document.getElementById("close-all-windows"),A=window.innerWidth<880;if(!A){let _=document.getElementById("dexter-dropdown"),B=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),B&&B.classList.remove("active"),b&&b.classList.remove("menu-open")}if(ke(e.length>0),e.length>0)if(v?.classList.add("hide"),w&&(w.style.display=A?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",f?.classList.add("window-open"),b&&(b.style.paddingTop="60px"),l&&(l.style.display=A?"flex":"none"),p&&(p.style.display=A?"block":"none"),E&&(E.style.display=A?"block":"none"),!A&&$){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?($.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{n("alerts-window"),o(m)}),document.getElementById("switch-events")?.addEventListener("click",()=>{n("events-window"),o(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{n("monitor-window"),o(k)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{n("contacts-window"),o(g)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{n("workspace-window"),o(y)})):$.innerHTML=""}else $&&($.innerHTML="");else f?.classList.remove("window-open"),w&&(w.style.display="none"),b&&(b.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),oe()||(document.body.style.overflow=""),x||d?v?.classList.remove("hide"):v?.classList.add("hide"),l&&(l.style.display="flex"),p&&(p.style.display="block"),E&&(E.style.display="block"),$&&($.innerHTML="");Ie()}function o(b){if(b.isOpen()){b.close();return}for(;e.length>0;)e.pop().close(!0);e.push(b),b.open(),s()}function i(){[...e].forEach(f=>f.close()),e=[]}window.addEventListener("resize",s);let m=le({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>me(),onClose:()=>{let b=e.indexOf(m);b>-1&&e.splice(b,1),s()}}),c=le({id:"events-window",title:"Events",icon:"bx-calendar-event",content:st(),onOpen:()=>{c.setContent(st()),ue()},onClose:()=>{let b=e.indexOf(c);b>-1&&e.splice(b,1),s()}}),g=le({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:_t(),onOpen:()=>tt(),onClose:()=>{let b=e.indexOf(g);b>-1&&e.splice(b,1),s()}}),k=le({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-component",title:"Processes",content:Ut()},{icon:"bxs-zap",title:"Agents",content:jt()},{icon:"bx-globe",title:"Web",content:Gt()},{icon:"bxs-server",title:"Services",content:qt()},{icon:"bxs-terminal",title:"Logs",content:Jt()},{icon:"bxs-hdd",title:"Hardware",content:Ft()}].filter(b=>z()?b.title!=="Hardware"&&b.title!=="Logs":!0),onOpen:()=>{re(),Te(),Kt(),De()},onClose:()=>{let b=e.indexOf(k);b>-1&&e.splice(b,1),s()}}),y=le({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-map-alt",title:"Roadmap",content:Mt()},{icon:"bx-task",title:"Tasks",content:It()}],onOpen:()=>et(),onClose:()=>{let b=e.indexOf(y);b>-1&&e.splice(b,1),s()}}),h=le({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ve(),onOpen:()=>{h.setContent(Ve()),at(h)},onClose:()=>{let b=e.indexOf(h);b>-1&&e.splice(b,1),s()}});window.dexter||(window.dexter={}),window.dexter.viewEvent=b=>{c.isOpen()||o(c),setTimeout(()=>{let f=document.querySelector(`.event-item[data-event-id="${b}"]`);f&&(f.scrollIntoView({behavior:"smooth",block:"center"}),f.classList.add("flash-highlight"),f.classList.contains("expanded")||f.click(),setTimeout(()=>f.classList.remove("flash-highlight"),2e3))},500)},window.dexter.viewAlert=b=>{m.isOpen()||o(m),setTimeout(()=>{let f=document.querySelector(`.event-item[data-alert-id="${b}"]`);f&&(f.scrollIntoView({behavior:"smooth",block:"center"}),f.classList.add("flash-highlight"),f.classList.contains("expanded")||f.click(),setTimeout(()=>f.classList.remove("flash-highlight"),2e3))},500)};let r=()=>{let b=document.getElementById("dexter-dropdown"),f=document.getElementById("dexter-menu-btn");b&&f&&(b.classList.remove("active"),f.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let b=document.getElementById("dexter-menu-btn"),f=document.getElementById("settings-icon"),v=document.getElementById("docs-icon"),x=document.getElementById("close-all-windows"),d=document.getElementById("nav-left-container"),l=document.getElementById("dexter-dropdown"),$=document.getElementById("windows-container");b&&l&&(b.onclick=p=>{p.stopPropagation();let w=window.innerWidth<880;if(w)l.style.top="",l.style.left="",l.style.right="",l.style.transform="";else{let _=b.getBoundingClientRect(),P=document.querySelector("nav").getBoundingClientRect(),V=220,Z=10,T=_.left+_.width/2-V/2,S=P.right-10;T+V>S&&(T=S-V),T<10&&(T=10),l.style.top=P.bottom+Z+"px",l.style.left=T+"px",l.style.right="auto",l.style.transform="none"}l.classList.toggle("active"),b.classList.toggle("active");let A=l.classList.contains("active");if(w){let _=document.querySelector("nav");A?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),$?.classList.add("menu-open"),ke(!0)):($?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),oe()||ke(!1)))}}),f&&(f.onclick=p=>{p.stopPropagation(),o(h)}),v&&(v.onclick=p=>{p.stopPropagation(),window.location.href="/docs"}),x&&(x.onclick=p=>{p.stopPropagation(),i()});let E=(p,w,A)=>{let _=document.getElementById(p);_&&(_.onclick=B=>{B.stopPropagation(),r(),A&&n(A),o(w)})};E("alerts-menu-item",m,"alerts-window"),E("events-menu-item",c,"events-window"),E("monitor-menu-item",k,"monitor-window"),E("contacts-menu-item",g,"contacts-window"),E("workspace-menu-item",y,"workspace-window"),d&&(d.onclick=p=>{if(p.stopPropagation(),l&&l.classList.contains("active")){r(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),oe()||ke(!1));return}if(e.length>0)i();else{let w=window.location.pathname;if(!(w==="/"||w==="/index.html")){let _=w.endsWith("/")&&w.length>1?w.slice(0,-1):w;if(_.includes("/docs/page/")){window.location.href="/";return}let B=_.split("/");B.pop();let P=B.join("/")||"/";window.location.href=P}}}),document.addEventListener("click",()=>{let p=window.innerWidth<880;l&&l.classList.contains("active")&&(r(),p&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),oe()||ke(!1)))}),l&&(l.onclick=p=>p.stopPropagation())})();let L=window.location.pathname==="/"||window.location.pathname==="/index.html",I=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!L&&!I&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{c.isOpen()&&ue(),m.isOpen()?me():Ze()},1e3),setInterval(()=>{z()&&(m.isOpen()?me():Ze()),y.isOpen()&&et(),z()&&c.isOpen()&&ue(),k.isOpen()&&Wt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",es):es();})();

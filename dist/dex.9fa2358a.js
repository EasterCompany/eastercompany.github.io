"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,c=document.createElement("nav");c.innerHTML=a,document.body.prepend(c);let r=document.createElement("div");r.id="dexter-dropdown",r.className="dexter-dropdown",r.innerHTML=`
...
`,Oe=null,fe=new Set,je=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];Oe=Date.now(),te(0,Oe);let r=Date.now(),v=24*60*60*1e3,d=c.filter(l=>{let g=localStorage.getItem(`alert_read_ts_${l.id}`);if(!g)return!0;let b=parseInt(g);return r-b<v});d.sort((l,g)=>{let b=h=>{let f=h.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},C=h=>h==="critical"?4:h==="high"?3:h==="medium"?2:1,u=C(b(l)),$=C(b(g));return u!==$?$-u:g.timestamp-l.timestamp}),je=d;let x=l=>{let g=l.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},E=[];if(d.length===0)E.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let g=new Set(d.map(C=>x(C))).size>1,b=null;d.forEach(C=>{let u=x(C);g&&u!==b&&(E.push({id:`divider-${u}`,type:"divider",label:u.toUpperCase()}),b=u),E.push(C)})}t&&(e.innerHTML="");let p=l=>{let g=l.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let b=(g.title||"Untitled Alert").trim(),C=(g.body||"No description provided.").trim(),u=g.summary||"",$=g.content||"",h=g.protocol||"",f=(g.priority||"low").toLowerCase(),k=!!g.alert,A=(g.category||"system").trim(),I=g.related_event_ids||[],R=g.audit_event_id,U=!!localStorage.getItem(`alert_read_ts_${l.id}`),G=new Date(l.timestamp*1e3),L=G.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=G.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=U?"event-border-grey":"event-border-blue";!U&&k&&(M="event-border-red"),U&&(f==="high"||f==="critical")?M="event-border-red":U&&f==="medium"&&(M="event-border-orange");let H=U?"alert-read":"alert-unread",q=fe.has(l.id),F=q?"expanded":"",K=q?"display: block;":"display: none;",S="",D="";I.length>0&&(D=`
        ...
        `,S=`
                ...
                ${Q}
            `;let V=document.createElement("div");V.className=`event-item notification-item ${M} ${H} ${F} cursor-pointer priority-${f}`,V.dataset.alertId=l.id,V.onclick=function(ce){let X=this;if(X.classList.contains("expanded")){X.classList.remove("expanded"),fe.delete(l.id);let Ae=X.querySelector(".event-details");Ae&&(Ae.style.display="none")}else{X.classList.add("expanded"),fe.add(l.id);let Ae=X.querySelector(".event-details");if(Ae&&(Ae.style.display="block"),!localStorage.getItem(`alert_read_ts_${l.id}`)){localStorage.setItem(`alert_read_ts_${l.id}`,Date.now().toString()),X.classList.add("alert-read"),X.classList.remove("alert-unread"),X.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ve="event-border-grey";f==="high"||f==="critical"?Ve="event-border-red":f==="medium"&&(Ve="event-border-orange"),X.classList.add(Ve),Ye()}}};let we=`${h?h.toUpperCase():"GUARDIAN"} ALERT: ${u||b}`,We={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[A]||"bx-bell",Ge=f==="high"||f==="critical"?"#ff4d4d":f==="medium"?"#ffa500":"#888";V.innerHTML=`
...
`,ge=new Set,St=[],Se=null;async function be(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await _("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();St=i;let a=p=>{let y=ge.has(p.id),s=p.state==="published",m=p.state==="consumed",w="event-border-grey";s&&(w="event-border-blue"),m&&(w="event-border-purple");let o=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),l=document.createElement("div");l.className=`event-item notification-item ${w} cursor-pointer ${y?"expanded":""}`,l.dataset.itemId=p.id,l.onclick=C=>{if(C.target.closest("button"))return;if(l.classList.contains("expanded")){l.classList.remove("expanded");let $=l.querySelector(".event-details");$&&($.style.display="none"),ge.delete(p.id)}else{l.classList.add("expanded");let $=l.querySelector(".event-details");$&&($.style.display="block"),ge.add(p.id)}};let g=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;l.innerHTML=`
        <div class="event-time">
      ...
      `;let b=l.querySelector(".event-details");return b&&b.addEventListener("click",C=>{C.stopPropagation()}),l.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(p)),l.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(p)),l.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(p.id)),l.querySelector(".close-details-btn")?.addEventListener("click",C=>{C.stopPropagation(),l.classList.remove("expanded");let u=l.querySelector(".event-details");u&&(u.style.display="none"),ge.delete(p.id)}),l},c=p=>{let y=document.createElement("div");y.innerHTML=j("empty",p.message,p.action,p.id);let s=y.firstElementChild;return s.dataset.itemId=p.id,s},r=[];!i||i.length===0?r.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(p=>r.push({...p,type:"item"}));let v=Array.from(e.children),d=new Map(v.map(p=>[p.dataset.itemId,p])),x=new Set(r.map(p=>p.id));v.forEach(p=>{let y=p.dataset.itemId;(!y||!x.has(y))&&p.remove()}),t&&(e.innerHTML="");let E=null;r.forEach((p,y)=>{let s=d.get(p.id);(!s||t)&&(s&&s.remove(),p.type==="placeholder"?s=c(p):s=a(p),!s)||(y===0?e.firstElementChild!==s&&e.prepend(s):E&&E.nextElementSibling!==s&&E.after(s),E=s)})}catch{e.children.length===0&&(e.innerHTML=j("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Se=null;let c=document.getElementById("roadmap-editor-input");c&&(c.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let c=document.getElementById("roadmap-editor-container");c&&(c.style.display="none"),Se=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input"),r=c?c.value:"";if(!r.trim())return;let v=Se?`/roadmap/${Se}`:"/roadmap",d=Se?"PATCH":"POST";try{await _(v,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let x=document.getElementById("roadmap-editor-container");x&&(x.style.display="none"),be(!0)}catch(x){console.error(x)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{St.forEach(c=>ge.add(c.id)),be(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{ge.clear(),be(!0)},a.dataset.listenerAttached="true")}function Ss(t){Se=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),be(!0)}catch(n){console.error(n)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),ge.delete(t),be(!0)}catch(e){console.error(e)}}var Lt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`;function Ut(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function qt(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",ue(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();et(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=Ut(t.id,t.username);et(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=Ut(t.id,t.username);et(e,t,i)})}function et(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let w=(n.attributes||n.facts||[]).map(C=>{let u=C.key||C.k,$=C.value||C.v;return`
        ...
        `}).join(""),B=n.cognitive_model||n,o=B.technical_level||B.techLevel||0,l=B.communication_style||B.commStyle||"Unknown",g=B.patience_level||B.patience||"Unknown",b=B.vibe||"Unknown";return`
                ...
                <div class="profile-section">
                    ...
                    <div class="cog-metric">
                        <div class="cog-label">
                            ...
                            <span style="color: #03dac6;">${l}</span>
                        </div>
                    </div>
        ...
        `},c=()=>{let w=n.dossier||{},B=w.identity||{},o=w.career||{},l=w.personal||{},g=w.social||[];return`
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.hobbies||[]).map(b=>`<div class="dossier-list-item">${b}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.habits||[]).map(b=>`<div class="dossier-list-item">${b}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.vices||[]).map(b=>`<div class="dossier-list-item" style="color: #cf6679;">${b}</div>`).join("")}
                        </div>
                    </div>
                </div>
...
`,zs=null;async function tt(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await tt(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let a=n.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let c=JSON.parse(a.dataset.user);qt(c)}catch(c){console.error("Failed to parse user data",c)}},t.dataset.listenerAttached="true");try{let n=await ue("/contacts");if(!n.ok)throw new Error("Service unreachable");let a=(await n.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=j("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((r,v)=>{let d=c[r.level]??10,x=c[v.level]??10;return d!==x?d-x:r.username.localeCompare(v.username)}),t.innerHTML=a.map(r=>{let v=r.color&&r.color!==0,d=v?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",x=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",E="#888";r.level==="Me"||r.level==="Master"?E="#bb86fc":r.level==="Admin"?E="#cf6679":r.level==="Moderator"?E="#03dac6":r.level==="Contributor"&&(E="#ffa500");let p=r.level==="Me",y=p?"rgba(187, 134, 252, 0.08)":v?`${d}11`:"rgba(255,255,255,0.03)",s=p?"2px solid #bb86fc":v?`1px solid ${d}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":v?`2px solid ${d}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${v?d:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.nickname||r.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${E}; font-weight: 600; text-transform: uppercase;">${p?"DEXTER (ME)":r.level}</span>
...
`,Wt=qs;function Gt(){return`
    ...
    `}var Fs=null;async function De(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=j("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(r=>!i.includes(r.id));a.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),a.reverse();let c=a.map(r=>{let v=r.logs.join(`
`),d=[...r.logs];if(d.length<25){let E=25-d.length;for(let p=0;p<E;p++)d.push("")}else d.length>25&&(d=d.slice(-25));let x=d.map(E=>Pe(E)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Gt()}
                ...
                </div>`),a){let l=(o.MEMORY_BYTES/1073741824).toFixed(1);a.innerHTML=`
                ...
                </div>`}if(i&&o.CPU&&o.CPU.length>0){let l=o.CPU[0];i.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}c&&(o.GPU&&o.GPU.length>0?c.innerHTML=o.GPU.map(l=>{let g=(l.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),r&&o.STORAGE&&o.STORAGE.length>0?r.innerHTML=o.STORAGE.map(l=>{let g=(l.USED/1073741824).toFixed(1),b=(l.SIZE/(1024*1024*1024)).toFixed(1),C=l.SIZE>0?(l.USED/l.SIZE*100).toFixed(0):0,u=l.MOUNT_POINT||"Unmounted";return`
            ...
            </div>`}let l=o.status==="online",g=l?"service-widget-online":"service-widget-offline",b=l?"bx-check-circle":"bx-x-circle",C=l?"OK":"BAD",u=o.version?E(o.version.str):"-",$=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",h=o.memory&&o.memory!=="N/A"?o.memory:"-",f=$!=="-"?"#00ff00":"#666",k=$!=="-"?"#fff":"#666",A=h!=="-"?"#00ff00":"#666",I=h!=="-"?"#fff":"#666",R=y(o.uptime),W="";l?W=`

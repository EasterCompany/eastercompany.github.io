...
`);return`
  ...
  `}async function Nt(){try{let e=await(await H("/processes")).json(),i=e.active||[],n=e.history||[],s=i.find(o=>o.channel_id==="system-fabricator");if(s){Ee="ACTIVE";let m=Math.floor(Date.now()/1e3)-s.start_time,c=10,g="REVIEW";m>30&&(c=30,g="INVESTIGATION"),m>120&&(c=60,g="CONSTRUCTION"),m>300&&(c=90,g="REPORTING"),te={title:s.state||"Fabricating system improvements...",progress:c,phase:g};let k=new Date(s.updated_at*1e3).toLocaleTimeString();(ae.length===0||ae[ae.length-1].msg!==s.state)&&(ae.push({time:k,msg:s.state}),ae.length>50&&ae.shift())}else{let o=n.find(m=>m.channel_id==="system-fabricator");if(o&&Date.now()/1e3-o.end_time<60){Ee="COMPLETED";let m=o.end_time-o.start_time;nt={duration:`${Math.floor(m/60)}m ${m%60}s`,steps:0,result:o.state,timestamp:o.end_time}}else Ee="STANDBY",ae=[]}let a=document.getElementById("progress-view-root");a&&(Ee!==zt&&(a.innerHTML=Rt(),zt=Ee),Ee==="ACTIVE"&&_s())}catch(t){console.error("Failed to update progress:",t)}}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),i=document.getElementById("active-task-progress-text"),n=document.getElementById("active-task-phase"),s=document.getElementById("terminal-output");if(t&&te&&(t.innerText=te.title),e&&te&&(e.style.width=`${te.progress}%`),i&&te&&(i.innerText=`${te.progress}% COMPLETE`),n&&te&&(n.innerText=`PHASE: ${te.phase}`),s){let a=s.children.length;if(ae.length>a){for(let o=a;o<ae.length;o++){let m=ae[o],c=document.createElement("div");c.className="terminal-line",c.innerHTML=`
    ...
    `;async function Kt(){let t=document.getElementById("web-analysis-sidebar"),e=document.getElementById("web-sidebar-toggle"),i=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=()=>{if(t){let n=t.style.display==="none";t.style.display=n?"flex":"none",e.style.color=n?"#03dac6":""}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{i.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Vt(),i.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},i.dataset.listenerAttached="true"),await Vt()}async function Vt(){let t=document.getElementById("web-analysis-content"),e=document.getElementById("web-main-frame"),i=document.getElementById("web-frame-placeholder");if(!(!t||!e||!i))try{let n=await H("/web/view");if(!n.ok)throw new Error("Failed to fetch web view state");let s=await n.json();if(!s||!s.url){i.style.display="flex",e.style.display="none",t.innerHTML=`
                <div style="text-align: center; color: #444; padding-top: 50px;">
                    <i class='bx bx-analyse' style="font-size: 2rem; opacity: 0.3;"></i>
                    <p style="font-size: 0.7em; text-transform: uppercase; letter-spacing: 1px; margin-top: 10px;">No active analysis</p>
                </div>`;return}e.src!==s.url&&(i.style.display="none",e.style.display="block",e.src=s.url);let a="";if(s.metadata&&(a+=`
            ...
            `),s.visual){let o=s.visual.screenshot||s.visual.screenshot_base64||s.visual.base64,m=o?`<img src="data:image/png;base64,${o}" style="width: 100%; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); margin-top: 10px; cursor: pointer;" onclick="window.open('data:image/png;base64,${o}')">`:"";a+=`
                <div class="analysis-section" style="margin-bottom: 25px;">

"use strict";(()=>{function et(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function tt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",s=`
    ...
    `,l=document.createElement("nav");l.innerHTML=s,document.body.prepend(l);let r=document.createElement("div");r.id="dexter-dropdown",r.className="dexter-dropdown",r.innerHTML=`
    ...
    `,document.body.appendChild(r)}function ve(t){let e=window.location.pathname,n=e==="/"||e==="/index.html",i=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!i||!s||(t||!n?(i.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{i.style.transform="translateX(-3px)"},s.onmouseleave=()=>{i.style.transform="translateX(0)"}):(i.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function st(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var nt=1e3;function ce(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function s(){e&&(e.style.zIndex=(++nt).toString())}function l(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",r),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++nt).toString(),e.addEventListener("mousedown",s);let v=t.icon||"bx-window",o="",b="",$;t.tabs&&t.tabs.length>0?(o=`<div class="tab-bar">${t.tabs.map((d,u)=>{let f=d.icon?`<i class="bx ${d.icon}"></i>`:`<span class="tab-glyph">${d.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
            ...
            ${$}
        `,m.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",w=>{w.stopPropagation(),c()}),window.addEventListener("resize",r),t.tabs&&e.querySelectorAll(".tab").forEach(w=>{w.addEventListener("click",()=>{if(!e)return;let a=w.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(u=>u.classList.remove("active")),w.classList.add("active"),e.querySelectorAll(".tab-content").forEach(u=>u.classList.remove("active"));let d=e.querySelector(`.tab-content[data-tab-content="${a}"]`);d&&d.classList.add("active"),g(w,e)})}),setTimeout(()=>{e&&e.classList.add("open"),i&&i()},10)}function r(){if(!e||!e.classList.contains("open"))return;let m=e.querySelector(".tab.active");m&&g(m,e)}function g(m,v){setTimeout(()=>{let o=v.querySelector(".tab-bar");if(!o)return;let b=Array.from(o.querySelectorAll(".tab")),$=b.indexOf(m),w=o.clientWidth,a=b[Math.max(0,$-2)],d=b[Math.min(b.length-1,$+2)],u=o,f=a.offsetLeft-u.offsetLeft-25,M=d.offsetLeft+d.offsetWidth-u.offsetLeft+25-f,A=M<=w?f-(w-M)/2:m.offsetLeft-u.offsetLeft-w/2+m.offsetWidth/2;o.scrollTo({left:A,behavior:"smooth"})},350)}function c(m=!1){e&&(window.removeEventListener("resize",r),m?(e.remove(),e=null):(e.classList.remove("open"),n&&n(),setTimeout(()=>{e?.remove(),e=null},400)))}function p(m){t.content=m;let v=e?.querySelector(".window-content");v&&(v.innerHTML=m)}function y(){return!!(e&&e.classList.contains("open"))}return{open:l,close:c,setContent:p,isOpen:y,focus:s,id:t.id}}var it="easter_theme",X={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Pe(){return localStorage.getItem(it)||X.DARK}function at(t){if(!Object.values(X).includes(t))throw new Error("Invalid theme");localStorage.setItem(it,t),ot(t)}function ot(t){let e=document.body;Object.values(X).forEach(l=>{e.classList.remove(`theme-${l}`)}),e.classList.add(`theme-${t}`);let n=document.querySelector('meta[name="theme-color"]');n||(n=document.createElement("meta"),n.name="theme-color",document.getElementsByTagName("head")[0].appendChild(n));let i={[X.DARK]:"#050507",[X.LIGHT]:"#FFFFFF",[X.LEGACY]:"#000000"},s=i[t]||i[X.DARK];if(n.setAttribute("content",s),!document.querySelector(".background")){let l=document.createElement("div");l.className="background",document.body.prepend(l)}e.classList.add("is-animated")}function rt(){let t=Pe();ot(t)}function z(t,e,n=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",l=n?`<p class="placeholder-action">${n}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${l}</div>`}function P(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function Q(t,e){let n=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!n)return;if(!e){n.textContent="Last updated: never";return}let i=Date.now(),s=Math.floor((i-e)/1e3),l;s<60?l=`${s}s ago`:s<3600?l=`${Math.floor(s/60)}m ago`:l=`${Math.floor(s/3600)}h ago`,n.textContent=`Last updated: ${l}`}var Xt=0;function $e(t,e){let n=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!n)return;let i=n.querySelector(".notification-badge");i&&(e>0?(i.textContent=e>9?"9+":e.toString(),i.style.display="flex"):i.style.display="none")}var te=0,se=0;function ct(t){te=t,he()}function dt(t){se=t,he()}function he(){let t=te+se;Xt=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let n=document.getElementById("alerts-menu-item");if(n){let r=n.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",n.appendChild(r)),r.textContent=te>9?"9+":te.toString(),r.style.display=te>0?"flex":"none"}let i=document.getElementById("workspace-menu-item");if(i){let r=i.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",i.appendChild(r)),r.textContent=se>9?"9+":se.toString(),r.style.display=se>0?"flex":"none"}let s=document.getElementById("switch-alerts");if(s){let r=s.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",s.appendChild(r)),r.textContent=te>9?"9+":te.toString(),r.style.display=te>0?"flex":"none"}let l=document.getElementById("switch-workspace");if(l){let r=l.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",l.appendChild(r)),r.textContent=se>9?"9+":se.toString(),r.style.display=se>0?"flex":"none"}}function Me(){let t=document.getElementById("alerts-list");if(!t)return;te=t.querySelectorAll(".alert-unread:not(.priority-low)").length,he()}function pt(){let t=document.getElementById("blueprints-list");if(!t)return;se=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,he()}function Qt(){return"https://event.easter.company"}function es(){return"https://discord.easter.company"}var ts="http://127.0.0.1:8100",ss="http://127.0.0.1:8300",ns={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Ae(t){let e=P(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,l)=>{let r=ns[l];return r?`<span class="${r}">`:""});let n=(e.match(/<span/g)||[]).length,i=(e.match(/<\/span/g)||[]).length;return n>i&&(e+="</span>".repeat(n-i)),e}function de(t){if(!t)return"";let e=P(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(n,i)=>{let s=i.split("|").map(l=>l.trim());return s.every(l=>l.match(/^:?-+:?$/))?"":`<div class="md-table-row">${s.map(l=>`<span class="md-table-cell">${l}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}var is="https://sterling-javelin-12539.upstash.io",as="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function H(){return window.location.hostname.includes("easter.company")}var R=null,ne={lastDashboard:0,lastFrontend:0},Oe=!1,mt=0,ut="dex_dashboard_snapshot";function os(){let t=localStorage.getItem(ut);if(t)try{let e=JSON.parse(t);R=e,ne.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{R=null}}async function lt(){if(!(!H()||Oe)){Oe=!0,mt=Math.floor(Date.now()/1e3);try{let t=await cs("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);R=e,ne.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),ne.lastFrontend=Date.now(),localStorage.setItem(ut,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Oe=!1}}}function rs(){if(!R||!R.agent_status)return;let t=R.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function ls(){if(!H())return;os();let t=Math.floor(Date.now()/1e3),e=R?t-R.timestamp:1/0;(!R||e>120)&&await lt(),setInterval(()=>{let n=new Date,i=Math.floor(Date.now()/1e3),s=R?i-R.timestamp:1/0,l=i-mt,r=n.getSeconds()===59,g=s>300,c=l>60;(r&&c||g&&c)&&lt(),rs()},1e3)}H()&&ls();async function cs(t,...e){try{let i=await(await fetch(is,{method:"POST",headers:{Authorization:`Bearer ${as}`},body:JSON.stringify([t,...e])})).json();if(i.error)throw new Error(i.error);return i.result}catch(n){return console.error("Upstash Error:",n),null}}var ye=null,xe=null,ke=!1,Le=!1;async function B(t,e={}){if(H()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(R.monitor),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(R.processes),{status:200});if(t.startsWith("/events")){let s=new URLSearchParams(t.split("?")[1]||""),l=s.get("type")||s.get("event.type"),r=s.get("category"),g=s.get("order")||"desc",c=[];return l==="system.notification.generated"?c=[...R.alerts||[]]:l==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?c=[...R.blueprints||[]]:r==="messaging"?c=[...R.messaging_events||[]]:r==="system"?c=[...R.system_events||[]]:r==="cognitive"?c=[...R.cognitive_events||[]]:r==="moderation"?c=[...R.moderation_events||[]]:c=[...R.events||[]],g==="asc"?c.sort((p,y)=>p.timestamp-y.timestamp):c.sort((p,y)=>y.timestamp-p.timestamp),new Response(JSON.stringify({events:c,count:c.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(R.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let s=t.split("/")[2],l=R.profiles?R.profiles[s]:null;return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(R.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify([]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(ye)try{let s=await fetch(ye+t,e);if(s.ok)return s;ye=null}catch{ye=null}let n=Qt(),i=ts;try{let s=await fetch(n+t,e);if(s.ok)return ye=n,ke&&(console.log("\u2728 Production event service restored."),ke=!1),s;throw new Error("Primary failed")}catch{ke||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${i}`),ke=!0);try{let l=await fetch(i+t,e);if(l.ok)return ye=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}async function Ie(t,e={}){if(H()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(R.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let s=t.split("/")[2],l=(R.contacts?.members||[]).find(r=>r.id===s);return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(xe)try{let s=await fetch(xe+t,e);if(s.ok)return s;xe=null}catch{xe=null}let n=es(),i=ss;try{let s=await fetch(n+t,e);if(s.ok)return xe=n,Le&&(console.log("\u2728 Production discord service restored."),Le=!1),s;throw new Error("Primary failed")}catch{Le||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${i}`),Le=!0);try{let l=await fetch(i+t,e);if(l.ok)return xe=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}var ft=()=>`
...
`,Be=null,pe=new Set,_e=[];async function oe(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ds(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await B(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];Be=Date.now(),Q(0,Be);let r=Date.now(),g=24*60*60*1e3,c=l.filter(u=>{let f=localStorage.getItem(`alert_read_ts_${u.id}`);if(!f)return!0;let x=parseInt(f);return r-x<g});c.sort((u,f)=>{let x=k=>{let E=k.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},M=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,A=M(x(u)),I=M(x(f));return A!==I?I-A:f.timestamp-u.timestamp}),_e=c;let p=u=>{let f=u.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],v=new Set(c.map(u=>p(u))).size>1;if(c.length>0){let u=null;c.forEach(f=>{let x=p(f);v&&x!==u&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),u=x),y.push(f)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=z("empty","No alerts yet."),Me();return}let o=u=>{let f=u.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),M=(f.body||"No description provided.").trim(),A=f.summary||"",I=f.content||"",k=f.protocol||"",E=(f.priority||"low").toLowerCase(),D=!!f.alert,U=(f.category||"system").trim(),O=f.related_event_ids||[],j=f.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${u.id}`),T=new Date(u.timestamp*1e3),C=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=J?"event-border-grey":"event-border-blue";!J&&D&&(S="event-border-red"),J&&(E==="high"||E==="critical")?S="event-border-red":J&&E==="medium"&&(S="event-border-orange");let L=J?"alert-read":"alert-unread",_=pe.has(u.id),N=_?"expanded":"",V=_?"display: block;":"display: none;",q="",F="";O.length>0&&(F=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${O.map(ae=>`<a href="#" onclick="window.dexter.viewEvent('${ae}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ae.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let W="";j&&(W=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${de(I)}</div>
            </div>
        `:ee=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${de(M)}</div>
            </div>
        `,q=`
                ...
                ${ee}
            `;let Y=document.createElement("div");Y.className=`event-item notification-item ${S} ${L} ${N} cursor-pointer priority-${E}`,Y.dataset.alertId=u.id,Y.onclick=function(ae){let Z=this;if(Z.classList.contains("expanded")){Z.classList.remove("expanded"),pe.delete(u.id);let Ee=Z.querySelector(".event-details");Ee&&(Ee.style.display="none")}else{Z.classList.add("expanded"),pe.add(u.id);let Ee=Z.querySelector(".event-details");if(Ee&&(Ee.style.display="block"),!localStorage.getItem(`alert_read_ts_${u.id}`)){localStorage.setItem(`alert_read_ts_${u.id}`,Date.now().toString()),Z.classList.add("alert-read"),Z.classList.remove("alert-unread"),Z.classList.remove("event-border-blue","event-border-red","event-border-purple");let Re="event-border-grey";E==="high"||E==="critical"?Re="event-border-red":E==="medium"&&(Re="event-border-orange"),Z.classList.add(Re),Me()}}};let Ye=`${k?k.toUpperCase():"GUARDIAN"} ALERT: ${A||x}`,Zt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[U]||"bx-bell",Yt=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";Y.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    ...
                    <div class="event-message">${de(Ye)}</div>
                    <div class="event-details" style="${V}">
                        ${q}
                    </div>
                </div>
            `;let Xe=Y.querySelector(".event-details");Xe&&Xe.addEventListener("click",ae=>{ae.stopPropagation()});let Qe=Y.querySelector(".close-details-btn");return Qe&&Qe.addEventListener("click",ae=>{ae.stopPropagation(),Y.classList.remove("expanded");let Z=Y.querySelector(".event-details");Z&&(Z.style.display="none"),pe.delete(u.id)}),Y},b=u=>{let f=document.createElement("div");f.className="notification-divider",f.dataset.alertId=u.id;let x="#888888";return u.label==="CRITICAL"?x="#ff4d4d":u.label==="HIGH"?x="#ff8888":u.label==="MEDIUM"&&(x="#ffa500"),f.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,f.innerHTML=`<span style="white-space: nowrap;">${u.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,f},$=Array.from(e.children),w=new Map($.map(u=>[u.dataset.alertId,u])),a=new Set(y.map(u=>u.id));$.forEach(u=>{let f=u.dataset.alertId;(!f||!a.has(f))&&u.remove()});let d=null;y.forEach((u,f)=>{let x=w.get(u.id);if(!x||t){if(x&&x.remove(),u.type==="divider")x=b(u);else{let M=o(u);if(!M)return;x=M}if(!x)return}f===0?e.firstElementChild!==x&&e.prepend(x):d&&d.nextElementSibling!==x&&d.after(x),d=x}),Be=Date.now(),Q(0,Be),Me()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=z("offline","Failed to load alerts.","The event service may be offline."))}}function ds(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),n=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{_e.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),oe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{_e.forEach(s=>{pe.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),oe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{pe.clear(),oe(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let s=Date.now()-48*60*60*1e3;_e.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,s.toString())}),pe.clear(),oe(!0)}catch(s){console.error("Failed to clear alerts:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function Ue(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],s=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||s++}),ct(s)}catch{}}var gt=()=>`
...
`,me=new Set,He=[],we=null;async function ue(t=!1){let e=document.getElementById("roadmap-list");if(e){ps();try{let n=await B("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();He=i;let s=p=>{let y=me.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${o} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=p.id,w.onclick=u=>{if(u.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),me.delete(p.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),me.add(p.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;w.innerHTML=`
        <div class="event-time">
      ...
      `;let d=w.querySelector(".event-details");return d&&d.addEventListener("click",u=>{u.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>ms(p)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>us(p)),w.querySelector(".delete-btn")?.addEventListener("click",()=>fs(p.id)),w.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),w.classList.remove("expanded");let f=w.querySelector(".event-details");f&&(f.style.display="none"),me.delete(p.id)}),w},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),g=new Set(i.map(p=>p.id));if(l.forEach(p=>{let y=p.dataset.itemId;(!y||!g.has(y))&&p.remove()}),!He||He.length===0){e.innerHTML=z("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((p,y)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=s(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=z("offline","Failed to load roadmap.","The event service may be offline."))}}}function ps(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{we=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),we=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let g=we?`/roadmap/${we}`:"/roadmap",c=we?"PATCH":"POST";try{await B(g,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let p=document.getElementById("roadmap-editor-container");p&&(p.style.display="none"),ue(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{He.forEach(l=>me.add(l.id)),ue(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{me.clear(),ue(!0)},s.dataset.listenerAttached="true")}function ms(t){we=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function us(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ue(!0)}catch(n){console.error(n)}}async function fs(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),me.delete(t),ue(!0)}catch(e){console.error(e)}}var yt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,vt=null,fe=new Set,ht=[];async function ge(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;gs();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await B(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(ht=l,vt=Date.now(),Q(2,vt),l.length===0){e.innerHTML=z("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),$e(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),$=(v.content||"").trim(),w=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(_=>_.trim()),d=(v.implementation_path||[]).map(_=>_.trim()),u=v.source_event_ids||[],f=v.approved===!0,x=new Date(m.timestamp*1e3),M=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=fe.has(m.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${f?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,f&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(_){let N=this;if(N.classList.contains("expanded")){N.classList.remove("expanded"),fe.delete(m.id);let q=N.querySelector(".event-details");q&&(q.style.display="none")}else{N.classList.add("expanded"),fe.add(m.id);let q=N.querySelector(".event-details");q&&(q.style.display="block")}};let O="";d.length>0&&(O=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${d.map(_=>`<li style="margin-bottom: 5px;">${P(_)}</li>`).join("")}</ul></div>
                    </div>
                `);let j="";u.length>0&&(j=`
                ...
                `);let G=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",J=H(),T=f?`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-icon" style="${f?"color: #03dac6;":""}"><i class='bx ${U}'></i></div>
                <div class="event-content">
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async _=>{_.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ge(!0)}catch(N){console.error("Failed to approve blueprint:",N)}});let h=E.querySelector(".blueprint-delete-btn");h&&(h.onclick=async _=>{_.stopPropagation();let N=!f;h.innerHTML=N?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&ge(!0)}catch(V){console.error("Failed to delete blueprint:",V)}});let S=E.querySelector(".event-details");S&&S.addEventListener("click",_=>{_.stopPropagation()});let L=E.querySelector(".close-details-btn");return L&&L.addEventListener("click",_=>{_.stopPropagation(),E.classList.remove("expanded");let N=E.querySelector(".event-details");N&&(N.style.display="none"),fe.delete(m.id)}),E},g=Array.from(e.children),c=new Map(g.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));g.forEach(m=>{let v=m.dataset.blueprintId;(!v||!p.has(v))&&m.remove()});let y=null;l.forEach((m,v)=>{let o=c.get(m.id);if(!o||t){o&&o.remove();let b=r(m);if(!b)return;o=b}v===0?e.firstElementChild!==o&&e.prepend(o):y&&y.nextElementSibling!==o&&y.after(o),y=o}),$e(2,l.length),pt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=z("offline","Failed to load blueprints.","The event service may be offline."))}}async function wt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],s=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&s++}),dt(s)}catch{}}function gs(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ht.forEach(i=>fe.add(i.id)),ge(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{fe.clear(),ge(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),fe.clear(),ge(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var Ct=()=>`
...
`;async function qe(){await Promise.all([ue(),ge()])}var $t=()=>`
    ...
    `;async function Ce(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),s=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display=n.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{n&&(n.style.display="none")},s.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),g=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=g?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),n&&(n.style.display="none"),l&&(l.value=""),r&&(r.value=""),Ce()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=z("empty","No active chores.","Create one to start monitoring.");return}let g=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=g,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this chore?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Ce())}})}catch(l){console.error("Failed to fetch chores",l)}}var bs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function St(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Tt(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=bs,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ie(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();je(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=St(t.id,t.username);je(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=St(t.id,t.username);je(e,t,i)})}function je(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",s=()=>{let $=(n.attributes||n.facts||[]).map(x=>{let M=x.key||x.k,A=x.value||x.v;return`
            <div class="fact-chip">
                <span class="fact-key">${M}:</span>
                <span class="fact-val">${A}</span>
            </div>
        `}).join(""),w=n.cognitive_model||n,a=w.technical_level||w.techLevel||0,d=w.communication_style||w.commStyle||"Unknown",u=w.patience_level||w.patience||"Unknown",f=w.vibe||"Unknown";return`
        ...
        `},l=()=>{let $=n.dossier||{},w=$.identity||{},a=$.career||{},d=$.personal||{},u=$.social||[];return`
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${P(e.username)}</h2>
                    <div class="profile-badges">${(n.badges||[]).map($=>`<span class="profile-badge ${$==="Creator"?"master":""}">${$}</span>`).join("")}</div>
                </div>
            </div>
...
`,vs=null;async function Fe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Fe(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let s=n.target.closest(".user-profile-card");if(s&&s.dataset.user)try{let l=JSON.parse(s.dataset.user);Tt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let n=await Ie("/contacts");if(!n.ok)throw new Error("Service unreachable");let s=(await n.json()).members||[];if(vs=Date.now(),s.length===0){t.innerHTML=z("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((r,g)=>{let c=l[r.level]??10,p=l[g.level]??10;return c!==p?c-p:r.username.localeCompare(g.username)}),t.innerHTML=s.map(r=>{let g=r.color&&r.color!==0,c=g?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let m=r.level==="Me",v=m?"rgba(187, 134, 252, 0.08)":g?`${c}11`:"rgba(255,255,255,0.03)",o=m?"2px solid #bb86fc":g?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${m?"2px solid #bb86fc":g?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${g?c:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.username}</h3>
...
`,De=null,be=new Set,Se=[],K="all",xs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},hs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ws(t){for(let[e,n]of Object.entries(xs))if(n.includes(t))return e;return"system"}function Cs(t){return hs[t]||"bx-square-rounded"}async function re(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Es();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let s=await B(i);if(!s.ok)throw new Error("Service unreachable");if(Se=((await s.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),De=Date.now(),Q(1,De),Se.length===0){e.innerHTML=z("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,$=ws(b),w=Cs(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let u=a?"cursor-pointer":"",f=be.has(v.id),x=f?"expanded":"",M=f?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Lt(b,o),D=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",U="";if(a){let T="";if(b==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                    ...
                    `}else if(b==="system.attention.expired"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=o.timestamp-o.last_active,S=Math.floor(h/60);T=`
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${S} minutes</span>
                        </div>
                        <div class="event-detail-block">
                            ${C("Context (Last Input)")}
                            <div class="detail-pre">${de(o.last_input||"None")}</div>
                        </div>
                        <div class="event-detail-block">
                            ${C("Thought Process (Last Output)")}
                            <div class="detail-pre">${de(o.last_output||"None")}</div>
                        </div>
                    `}else if(b==="messaging.bot.sent_message"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h="";o.eval_count&&(h=`
                        ...
                        `);let S="";if(o.chat_history&&o.chat_history.length>0){let L=o.chat_history.length,_=o.chat_history.map((N,V)=>{let q=N.name?N.name.toUpperCase():N.role.toUpperCase();!N.name&&q==="ASSISTANT"&&(q="DEXTER");let F=N.role==="user"?"#03dac6":N.role==="system"?"#ffb74d":"#bb86fc",W=V===L-1?"block":"none";return`
                                <div class="history-slide" data-index="${V}" style="display: ${W};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${F}; letter-spacing: 1px; font-weight: bold;">${q}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${V+1} of ${L}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(N.content)}</div>
                                </div>
                            `}).join("");S=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(b==="analysis.user.message_signals"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=o.signals||{};T=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(b==="moderation.explicit_content.deleted"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="analysis.link.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="analysis.visual.completed"){let C=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`,h="";o.base64_preview?h=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${o.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:o.url&&(h=`<div class="event-detail-block"><img src="${o.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(b==="analysis.router.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="system.cli.command"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="system.analysis.audit"){let C=o.success?"#03dac6":"#ff4d4d",h=o.success?"SUCCESS":"FAILED",S=q=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${q}</h5>`,L="";o.error&&(L=`
                            <div class="event-detail-block">
                                ${S("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(o.error)}</pre>
                            </div>
                        `);let _="";if(o.chat_history&&o.chat_history.length>0){let q=o.chat_history.length,F=o.chat_history.map((W,ie)=>{let ee=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&ee==="USER"&&(ee="SYSTEM"),!W.name&&ee==="ASSISTANT"&&(ee="AGENT");let Y=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ie}" style="display: ${ie===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${Y}; letter-spacing: 1px; font-weight: bold;">${ee}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ie+1} of ${q}</span>
                                    </div>
                        ...
                        `}let N="";if(o.corrections&&o.corrections.length>0){let q=o.corrections.map((F,W)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${F.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${P(F.snippet)}</div>`:""}
                            </div>
                        `).join("");N=`
                            <div class="event-detail-block">
                                ${S(`Corrections (${o.corrections.length})`)}
                                ${q}
                            </div>
                        `}let V="";if(o.parsed_results&&o.parsed_results.length>0){let q=o.parsed_results.map(F=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${P(F.title)}</div>
                                <div style="color: #ddd;">${P(F.summary)}</div>
                            </div>
                        `).join("");V=`
                            <div class="event-detail-block">
                                ${S("Parsed Results")}
                        ...
                        ${N}
                        ${_}
                    `}else if(b==="system.test.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="error_occurred"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(b==="system.cli.status"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(b==="messaging.user.sent_message"){let C="";o.attachments&&o.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${o.attachments.map(S=>{let L=S.content_type&&S.content_type.startsWith("image/"),_=(S.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${L?`<div class="attachment-thumb-container"><img src="${S.url}" alt="${S.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${S.url}" target="_blank" class="attachment-link">${S.filename}</a>
                                        <span class="attachment-meta">${S.content_type} \u2022 ${_}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),T=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${d} ${u} ${x}`,O.dataset.eventId=v.id,O.onclick=function(T){if(!a)return;let C=this;if(C.classList.contains("expanded")){C.classList.remove("expanded"),be.delete(v.id);let S=C.querySelector(".event-details");S&&(S.style.display="none")}else{C.classList.add("expanded"),be.add(v.id);let S=C.querySelector(".event-details");S&&(S.style.display="block")}},O.innerHTML=`
                <div class="event-time">
            ...
            `;let j=O.querySelector(".event-details");if(j&&j.addEventListener("click",T=>{T.stopPropagation()}),a){let T=O.querySelector(".close-details-btn");T&&T.addEventListener("click",C=>{C.stopPropagation();let h=C.target.closest(".event-item");if(h){h.classList.remove("expanded"),be.delete(v.id);let S=h.querySelector(".event-details");S&&(S.style.display="none")}})}let G=O.querySelector(".prev-btn"),J=O.querySelector(".next-btn");if(G&&J){let T=0,C=Array.from(O.querySelectorAll(".history-slide")),h=C.length,S=()=>{C.forEach((L,_)=>{L.style.display=_===T?"block":"none"}),G.disabled=T===0,J.disabled=T===h-1,G.style.opacity=T===0?"0.5":"1",J.style.opacity=T===h-1?"0.5":"1"};G.addEventListener("click",L=>{L.stopPropagation(),T>0&&(T--,S())}),J.addEventListener("click",L=>{L.stopPropagation(),T<h-1&&(T++,S())}),S()}return O},c=Array.from(e.children),p=new Map(c.map(v=>[v.dataset.eventId,v])),y=new Set(Se.map(v=>v.id));c.forEach(v=>{let o=v.dataset.eventId;(!o||!y.has(o))&&v.remove()});let m=null;Se.forEach((v,o)=>{let b=p.get(v.id);(!b||t)&&(b&&b.remove(),b=g(v),!b)||(o===0?e.firstElementChild!==b&&e.prepend(b):m&&m.nextElementSibling!==b&&m.after(b),m=b)}),De=Date.now(),Q(1,De)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=z("offline","Failed to load events.","The event service may be offline."))}}function Es(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),n=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Se.forEach(s=>be.add(s.id)),re(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{be.clear(),re(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.querySelectorAll(".filter-btn").forEach(s=>{s.onclick=()=>{n.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),s.classList.add("active"),K=s.dataset.filter||"all",re(!0)}}),n.dataset.listenerAttached="true"),n&&n.querySelectorAll(".filter-btn").forEach(s=>{s.dataset.filter===K?s.classList.add("active"):s.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let s=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${s} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(K!=="all"?l+=`?category=${K}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await B(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");be.clear(),re(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var $s=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Mt=$s;function At(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Ss=null;async function Te(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=z("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],s=n.filter(r=>!i.includes(r.id));s.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),s.reverse();let l=s.map(r=>{let g=r.logs.join(`
`),c=[...r.logs];if(c.length<25){let y=25-c.length;for(let m=0;m<y;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let p=c.map(y=>Ae(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let g=r,c=unescape(g.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let p=g.querySelector("i");p&&(p.classList.remove("bx-copy"),p.classList.add("bx-check"),setTimeout(()=>{p.classList.remove("bx-check"),p.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let c=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await B(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&Te()}catch(p){console.error("Error clearing logs:",p)}})}),Ss=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=z("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Ht=()=>{let t=H()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${At()}
        </div>`;async function Ot(){Ts(),await Promise.all([le(),Ge(),Ve(),Ce()]),await Te()}var ze=null;function Ts(){ze||(ze=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(ze),ze=null;return}le(!0)},1e3))}var It=null,Bt=null;async function Ut(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function _t(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function ks(){try{return await(await B("/processes")).json()}catch{return null}}async function Ls(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Ge(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),n=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),s=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),g=a=>{if(a){if(n&&(n.innerHTML=`
                ...
                </div>`),s){let d=(a.MEMORY_BYTES/1073741824).toFixed(1);s.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let a=await _t();a?(g(a),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let a=await _t();g(a)}if(!t)return;let c=await Ut();if(!c||!c.services){t.children.length===0&&(t.innerHTML=z("offline","Failed to load system metrics.","The event service may be offline."));let a=document.querySelector('[data-service-id="upstash-redis-ro"]');if(a){a.className="service-widget service-widget-offline",a.querySelector(".service-widget-status").textContent="ERROR";let d=a.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}It=H()&&ne.lastDashboard||Date.now(),Q(0,It||0);let p=c.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let a=document.getElementById("upstash-countdown");if(a){let f=59-new Date().getSeconds();f<=0&&(f+=60),a.textContent=`${f}s`}},1e3)),Array.from(t.children).forEach(a=>{a.classList.contains("service-widget")||a.remove()});function y(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:a.split(".").slice(0,3).join(".")||"-"}function m(a){return!a||a.length<=28?a:a.substring(0,28)+"..."}function v(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let u=parseInt(d[1])||0,f=parseInt(d[2])||0,x=parseInt(d[3])||0,M=parseFloat(d[4])||0,A=u*86400+f*3600+x*60+M,I=Math.floor(A/86400);if(I>0)return`${I}d`;let k=Math.floor(A/3600);if(k>0)return`${k}h`;let E=Math.floor(A/60);return E>0?`${E}m`:`${Math.floor(A)}s`}function o(a){if(a.id==="upstash-redis-ro"){let J=H()&&(ne.lastFrontend||ne.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=a.status==="online",u=d?"service-widget-online":"service-widget-offline",f=d?"bx-check-circle":"bx-x-circle",x=d?"OK":"BAD",M=a.version?y(a.version.str):"-",A=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",I=a.memory&&a.memory!=="N/A"?a.memory:"-",k=A!=="-"?"#00ff00":"#666",E=A!=="-"?"#fff":"#666",D=I!=="-"?"#00ff00":"#666",U=I!=="-"?"#fff":"#666",O=v(a.uptime),j="";d?j=`
                <div class="service-widget-info">
                ...
                </div>`:j='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let G=H()?"easter.company":m(a.domain&&a.port?`${a.domain}:${a.port}`:"");return`<div class="service-widget ${u}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${f}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${G}</span></div>${j}</div></div>`}let b=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),$=new Set(p.map(a=>a.id));for(let[a,d]of b)$.has(a)||d.remove();let w=new Set;p.forEach(a=>{if(w.has(a.id))return;w.add(a.id);let d=o(a),u=b.get(a.id);if(u&&u.parentNode)if(a.id==="upstash-redis-ro"){let f=H()&&(ne.lastFrontend||ne.lastDashboard)||Date.now(),x=new Date(f).toLocaleTimeString(),M=u.querySelector(".sync-time-label");M&&(M.textContent=`Last synced: ${x}`)}else u.outerHTML!==d&&(u.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Ve(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ut();if(!e){t.children.length===0&&(t.innerHTML=z("offline","Failed to load model status.","The event service may be offline."));return}Bt=Date.now(),Q(2,Bt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function s(c){let p=c.status==="Ready";return`
                ...
                </div>`}function r(c){let p=c.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",v=p&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",o=c.name;return c.type==="custom"&&o.endsWith(":latest")&&(o=o.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${o}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let g="";if(i&&(g+=s(i)),e.xtts&&(g+=l(e.xtts)),g+=n.map(r).join(""),!g){t.innerHTML=z("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function le(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),p=document.getElementById("fabricator-reset-btn"),y=document.getElementById("system-pause-toggle-btn");y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{let u=y.querySelector(".bx-play")?"/agent/resume":"/agent/pause";y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B(u,{method:"POST"}),setTimeout(()=>le(),500)}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),le()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),le()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),le()}catch{p.innerHTML="<i class='bx bx-error'></i>"}},p.dataset.listenerAttached="true");let m=await Ls();if(m&&m.agents){let d=m.agents.guardian||{protocols:{}},u=m.agents.analyzer||{protocols:{}},f=m.agents.imaginator||{protocols:{}},x=m.agents.fabricator||{protocols:{}},M=m.system||{},A=Math.floor(Date.now()/1e3),I={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction"},k=h=>{h<0&&(h=0);let S=Math.floor(h/3600),L=Math.floor(h%3600/60),_=h%60;return S>0?`${S}h ${L}m`:L>0?`${L}m ${_}s`:`${_}s`},E=(h,S,L,_)=>{if(!h)return;let N=I[_]||_.toUpperCase(),V=h.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(V&&(V.textContent=N),!L){h.textContent="Inactive",h.style.color="#666";return}let q=L.status;if(q==="Working")h.textContent="Working",h.style.color="#bb86fc";else if(q==="Ready")h.textContent="Ready",h.style.color="#5eff5e";else{let F=L.cooldown;if(H()&&(F=L.next_run-A),F<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let W=Math.floor(F/60),ie=F%60;h.textContent=`${W}m ${ie}s`,h.style.color="#fff"}}S&&L.stats&&(S.innerHTML=`
          ...
          `)};n&&E(n,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry");let D=document.getElementById("analyzer-synthesis-val"),U=document.getElementById("analyzer-synthesis-stats");D&&E(D,U,u.protocols.synthesis,"synthesis");let O=document.getElementById("imaginator-alert_review-val"),j=document.getElementById("imaginator-alert_review-stats");O&&E(O,j,f.protocols.alert_review,"alert_review");let G=document.getElementById("fabricator-construction-val"),J=document.getElementById("fabricator-construction-stats");G&&E(G,J,x.protocols.construction,"construction");let T=document.getElementById("system-state-label"),C=document.getElementById("system-state-val");if(C&&M.state){let h=M.state,S=k(M.state_time||0);T&&(T.textContent=`State: ${h.toUpperCase()}`),C.textContent=S,h==="idle"?C.style.color=M.state_time>300?"#5eff5e":"#fff":C.style.color="#bb86fc",y&&(y.querySelector(".bx-loader-alt")||y.querySelector(".bx-error")||(h==="paused"?(y.innerHTML="<i class='bx bx-play'></i>",y.title="Resume System",y.style.color="#ff9800"):(y.innerHTML="<i class='bx bx-pause'></i>",y.title="Pause System",y.style.color="")))}s&&(s.textContent=k(M.metrics?.total_idle_time||0)),l&&(l.textContent=k(M.metrics?.total_active_time||0)),r&&(r.textContent=k(M.metrics?.total_waste_time||0))}else[n,i,s,l,r].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});if(t)return;let v=await ks(),o=[],b=[],$=[];v&&(Array.isArray(v)?o=v:(o=v.active||[],b=v.queue||[],$=v.history||[],$.sort((d,u)=>(u.end_time||0)-(d.end_time||0)))),e&&(o.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=z("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),We(e,o,!1)));let w=document.getElementById("processes-queue-widgets");w&&(b.length===0?!w.querySelector(".tab-placeholder")&&!w.querySelector('div[style*="font-style: italic"]')&&(w.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(w.innerHTML="",We(w,b,!1)));let a=document.getElementById("processes-history-widgets");a&&($.length===0?a.querySelector(".tab-placeholder")||(a.innerHTML=z("empty","No recent history.")):(a.querySelector(".tab-placeholder")&&(a.innerHTML=""),We(a,$,!0))),$e(1,o.length)}function We(t,e,n){function i(c){let p="";c.end_time?p=`${c.end_time-c.start_time}s`:p=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let y=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(v[m]?m=v[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),n)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let s=n?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(s)).map(c=>[c.dataset.channelId,c])),r=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,p]of l)c&&!r.has(c)&&p.remove();let g=new Set;e.forEach(c=>{let p=`${c.channel_id}-${c.start_time}`;if(g.has(p))return;g.add(p);let y=i(c),m=l.get(p);if(m&&m.parentNode){m.outerHTML!==y&&(m.outerHTML=y);let v=t.querySelector(`[data-channel-id="${p}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",y)})}function Ne(){let t=Pe(),e="Notification"in window,n={enabled:e&&Notification.permission==="granted",supported:e};return`
                ...
                ${Object.values(X).map(i=>`
                    <div class="theme-card ${t===i?"active":""}" data-theme="${i}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${i===X.DARK?"Simple, clean, dark.":i===X.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
            ...
            </div>`}function Ke(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let s=this.dataset.theme;s&&(at(s),t.setContent(Ne()),Ke(t))})});let n=document.getElementById("notifications-toggle");n&&"Notification"in window?n.onclick=async i=>{let s=i.target;if(s.checked)try{await Notification.requestPermission()!=="granted"&&(s.checked=!1)}catch{s.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),s.checked=!0)}:n&&(n.disabled=!0)}var jt=()=>`
    ...
    `;async function Ft(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await qt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await qt(t)}async function qt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let n=await e.json();if(!n||n.length===0){t.innerHTML=z("empty","No web history found.");return}let i=`
        ...
        `;n.forEach((g,c)=>{let p=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[n,i]of Object.entries(t)){let s=Gt.filter(l=>l.category===n);s.length!==0&&(e+=`
    ...
    </div>`,e},Ze=!1;function As(t){let e=document.getElementById("cli-terminal-overlay");if(!e){e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>Jt());let p=document.getElementById("terminal-action-btn");p&&(p.onclick=()=>Jt())}let n=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");n&&(n.innerHTML="");let s=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let g=document.getElementById("terminal-action-btn");return g&&(g.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",Ze=!0,n}function Jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ze=!1}function Wt(t,e,n="output"){if(!Ze||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let s=Ae(e);i.innerHTML=s||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Is(t){let e=Gt.find(r=>r.id===t);if(!e)return;let n=As(e);n&&Wt(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let r of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let g="output";r.includes("[ERROR]")?g="error":r.includes("[SUCCESS]")||r.includes("\u2713")?g="success":r.includes("!")&&(g="warning"),n&&Wt(n,r,g)}let s=document.getElementById("terminal-status-badge");s&&(s.className="terminal-status status-success",s.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function Vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s&&(s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let s=i.dataset.cmd;s&&Is(s)})})}async function Bs(){if(!H())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Kt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}tt(),st(),Bs(),rt(),et();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Vt();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=a=>{localStorage.setItem("dex_last_window",a)};function s(){for(;e.length>1;)e.shift().close(!0);let a=document.getElementById("windows-container"),d=document.querySelector("nav"),u=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",e.length.toString()),e.forEach(D=>{let U=document.getElementById(D.id);U&&U.classList.remove("hide-close")});let M=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon"),k=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let D=document.getElementById("dexter-dropdown"),U=document.getElementById("dexter-menu-btn");D&&D.classList.remove("active"),U&&U.classList.remove("active"),a&&a.classList.remove("menu-open")}if(ve(e.length>0),e.length>0)if(u?.classList.add("hide"),k&&(k.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),M&&(M.style.display=E?"flex":"none"),I&&(I.style.display=E?"block":"none"),!E&&A){let D=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(D)?(A.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(g)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(y)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(p)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(m)})):A.innerHTML=""}else A&&(A.innerHTML="");else d?.classList.remove("window-open"),k&&(k.style.display="none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?u?.classList.remove("hide"):u?.classList.add("hide"),M&&(M.style.display="flex"),I&&(I.style.display="block"),A&&(A.innerHTML="");he()}function l(a){if(a.isOpen()){a.close();return}for(;e.length>0;)e.pop().close(!0);e.push(a),a.open(),s()}function r(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",s);let g=ce({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:ft(),onOpen:()=>oe(),onClose:()=>{let a=e.indexOf(g);a>-1&&e.splice(a,1),s()}}),c=ce({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Je(),onOpen:()=>{c.setContent(Je()),re()},onClose:()=>{let a=e.indexOf(c);a>-1&&e.splice(a,1),s()}}),p=ce({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:kt(),onOpen:()=>Fe(),onClose:()=>{let a=e.indexOf(p);a>-1&&e.splice(a,1),s()}}),y=ce({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:zt()},{icon:"bxs-component",title:"Processes",content:Dt()},{icon:"bxs-brain",title:"Models",content:Nt()},{icon:"bx-globe",title:"Web",content:jt()},{icon:"bxs-hdd",title:"Hardware",content:Rt()},{icon:"bxs-terminal",title:"Logs",content:Pt()},{icon:"bxs-zap",title:"Agents",content:Ht()},{icon:"bx-task",title:"Chores",content:$t()}].filter(a=>H()?a.title!=="Hardware"&&a.title!=="Logs":!0),onOpen:()=>{Ge(),le(),Ve(),Ft(),Te(),Ce()},onClose:()=>{let a=e.indexOf(y);a>-1&&e.splice(a,1),s()}}),m=ce({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-map-alt",title:"Roadmap",content:Ct()},{icon:"bx-paint",title:"Blueprints",content:Et()}],onOpen:()=>qe(),onClose:()=>{let a=e.indexOf(m);a>-1&&e.splice(a,1),s()}}),v=ce({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ne(),onOpen:()=>{v.setContent(Ne()),Ke(v)},onClose:()=>{let a=e.indexOf(v);a>-1&&e.splice(a,1),s()}});window.dexter={viewEvent:a=>{c.isOpen()||l(c),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:a=>{g.isOpen()||l(g),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let o=()=>{let a=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");a&&d&&(a.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let a=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),u=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),M=document.getElementById("windows-container");a&&x&&(a.onclick=I=>{I.stopPropagation();let k=window.innerWidth<880;if(k)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let D=a.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),j=220,G=10,T=D.left+D.width/2-j/2,C=O.right-10;T+j>C&&(T=C-j),T<10&&(T=10),x.style.top=O.bottom+G+"px",x.style.left=T+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),a.classList.toggle("active");let E=x.classList.contains("active");if(k){let D=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),D?.classList.add("window-open"),M?.classList.add("menu-open"),ve(!0)):(M?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),D?.classList.remove("window-open"),ve(!1)))}}),d&&(d.onclick=I=>{I.stopPropagation(),l(v)}),u&&(u.onclick=I=>{I.stopPropagation(),r()});let A=(I,k,E)=>{let D=document.getElementById(I);D&&(D.onclick=U=>{U.stopPropagation(),o(),E&&i(E),l(k)})};A("alerts-menu-item",g,"alerts-window"),A("events-menu-item",c,"events-window"),A("monitor-menu-item",y,"monitor-window"),A("contacts-menu-item",p,"contacts-window"),A("workspace-menu-item",m,"workspace-window"),f&&(f.onclick=I=>{if(I.stopPropagation(),x&&x.classList.contains("active")){o(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ve(!1));return}if(e.length>0)r();else{let k=window.location.pathname;if(!(k==="/"||k==="/index.html")){let U=(k.endsWith("/")&&k.length>1?k.slice(0,-1):k).split("/");U.pop();let O=U.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let I=window.innerWidth<880;x&&x.classList.contains("active")&&(o(),I&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ve(!1)))}),x&&(x.onclick=I=>I.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!w&&document.querySelector("footer")?.classList.add("hide"),H()||setInterval(()=>{c.isOpen()&&re(),g.isOpen()?oe():Ue()},1e3),setInterval(()=>{H()&&(g.isOpen()?oe():Ue()),m.isOpen()?qe():wt(),H()&&c.isOpen()&&re(),y.isOpen()&&Ot()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Kt):Kt();})();

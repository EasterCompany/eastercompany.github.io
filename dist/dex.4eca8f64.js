...
`,Pe=null,ge=new Set,Oe=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];Pe=Date.now(),ee(0,Pe);let r=Date.now(),b=24*60*60*1e3,d=c.filter(l=>{let u=localStorage.getItem(`alert_read_ts_${l.id}`);if(!u)return!0;let g=parseInt(u);return r-g<b});d.sort((l,u)=>{let g=h=>{let v=h.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},E=h=>h==="critical"?4:h==="high"?3:h==="medium"?2:1,f=E(g(l)),T=E(g(u));return f!==T?T-f:u.timestamp-l.timestamp}),Oe=d;let x=l=>{let u=l.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},w=[];if(d.length===0)w.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let u=new Set(d.map(E=>x(E))).size>1,g=null;d.forEach(E=>{let f=x(E);u&&f!==g&&(w.push({id:`divider-${f}`,type:"divider",label:f.toUpperCase()}),g=f),w.push(E)})}t&&(e.innerHTML="");let p=l=>{let u=l.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let g=(u.title||"Untitled Alert").trim(),E=(u.body||"No description provided.").trim(),f=u.summary||"",T=u.content||"",h=u.protocol||"",v=(u.priority||"low").toLowerCase(),k=!!u.alert,S=(u.category||"system").trim(),L=u.related_event_ids||[],R=u.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${l.id}`),G=new Date(l.timestamp*1e3),A=G.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=G.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=q?"event-border-grey":"event-border-blue";!q&&k&&(M="event-border-red"),q&&(v==="high"||v==="critical")?M="event-border-red":q&&v==="medium"&&(M="event-border-orange");let H=q?"alert-read":"alert-unread",P=ge.has(l.id),V=P?"expanded":"",z=P?"display: block;":"display: none;",N="",B="";L.length>0&&(B=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue(T)}</div>
            </div>
        `:se=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue(E)}</div>
            </div>
        `,N=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${S}</div>
                    </div>
                    ${J}
                    ${F}
                    ${B}
                ...
                ${se}
            `;let K=document.createElement("div");K.className=`event-item notification-item ${M} ${H} ${V} cursor-pointer priority-${v}`,K.dataset.alertId=l.id,K.onclick=function(ce){let X=this;if(X.classList.contains("expanded")){X.classList.remove("expanded"),ge.delete(l.id);let Le=X.querySelector(".event-details");Le&&(Le.style.display="none")}else{X.classList.add("expanded"),ge.add(l.id);let Le=X.querySelector(".event-details");if(Le&&(Le.style.display="block"),!localStorage.getItem(`alert_read_ts_${l.id}`)){localStorage.setItem(`alert_read_ts_${l.id}`,Date.now().toString()),X.classList.add("alert-read"),X.classList.remove("alert-unread"),X.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ge="event-border-grey";v==="high"||v==="critical"?Ge="event-border-red":v==="medium"&&(Ge="event-border-orange"),X.classList.add(Ge),Ze()}}};let le=`${h?h.toUpperCase():"GUARDIAN"} ALERT: ${f||g}`,We={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[S]||"bx-bell",ms=v==="high"||v==="critical"?"#ff4d4d":v==="medium"?"#ffa500":"#888";K.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    ...
                    <div class="event-message">${ue(le)}</div>
                    <div class="event-details" style="${z}">
                        ${N}
                    </div>
                </div>
            `;let rt=K.querySelector(".event-details");rt&&rt.addEventListener("click",ce=>{ce.stopPropagation()});let lt=K.querySelector(".close-details-btn");return lt&&lt.addEventListener("click",ce=>{ce.stopPropagation(),K.classList.remove("expanded");let X=K.querySelector(".event-details");X&&(X.style.display="none"),ge.delete(l.id)}),K},y=l=>{let u=document.createElement("div");u.className="notification-divider",u.dataset.alertId=l.id;let g="#888888";return l.label==="CRITICAL"?g="#ff4d4d":l.label==="HIGH"?g="#ff8888":l.label==="MEDIUM"&&(g="#ffa500"),u.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${g}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,u.innerHTML=`<span style="white-space: nowrap;">${l.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${g}44, transparent); flex-grow: 1;"></div>`,u},s=l=>{let u=document.createElement("div");u.innerHTML=U("empty",l.message,null,l.id);let g=u.firstElementChild;return g.dataset.alertId=l.id,g},m=Array.from(e.children),C=new Map(m.map(l=>[l.dataset.alertId,l])),I=new Set(w.map(l=>l.id));m.forEach(l=>{let u=l.dataset.alertId;(!u||!I.has(u))&&l.remove()});let o=null;w.forEach((l,u)=>{let g=C.get(l.id);if(!g||t){if(g&&g.remove(),l.type==="divider")g=y(l);else if(l.type==="placeholder")g=s(l);else{let E=p(l);if(!E)return;g=E}if(!g)return}u===0?e.firstElementChild!==g&&e.prepend(g):o&&o.nextElementSibling!==g&&o.after(g),o=g}),Pe=Date.now(),ee(0,Pe),Ze()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=U("offline","Failed to load alerts.","The event service may be offline."))}}function $s(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),n=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Oe.forEach(a=>{localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Oe.forEach(a=>{ge.add(a.id),localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ge.clear(),de(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.notification.generated",{method:"DELETE"});let a=Date.now()-48*60*60*1e3;Oe.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,a.toString())}),ge.clear(),de(!0)}catch(a){console.error("Failed to clear alerts:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function Ye(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||a++}),yt(a)}catch{}}var Tt=()=>`
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];At=c,St=Date.now(),ee(2,St);let r=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let C=(m.title||"Untitled Blueprint").trim(),I=(m.summary||m.body||"No summary provided.").trim(),o=(m.content||"").trim(),l=(m.category||"architecture").trim(),u=(m.related_services||m.affected_services||[]).map(N=>N.trim()),g=(m.implementation_path||[]).map(N=>N.trim()),E=m.source_event_ids||[],f=m.approved===!0,T=new Date(s.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=ye.has(s.id),S=k?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,L.dataset.blueprintId=s.id,f&&(L.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",L.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[l]||"bx-paint";L.onclick=function(N){let B=this;if(B.classList.contains("expanded")){B.classList.remove("expanded"),ye.delete(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="none")}else{B.classList.add("expanded"),ye.add(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="block")}};let q="";g.length>0&&(q=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${g.map(N=>`<li style="margin-bottom: 5px;">${O(N)}</li>`).join("")}</ul></div>
                    </div>
                `);let G="";E.length>0&&(G=`
                ...
                `);let A=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",$=D(),M=f?`
            ...
            `;L.innerHTML=`
                ...
                <div class="event-icon" style="${f?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${S}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${O(I)}</div>
            ...
            `;let H=L.querySelector(".blueprint-approve-btn");H&&(H.onclick=async N=>{N.stopPropagation(),H.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(B){console.error("Failed to approve blueprint:",B)}});let P=L.querySelector(".blueprint-delete-btn");P&&(P.onclick=async N=>{N.stopPropagation();let B=!f;P.innerHTML=B?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${s.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(F){console.error("Failed to delete blueprint:",F)}});let V=L.querySelector(".event-details");V&&V.addEventListener("click",N=>{N.stopPropagation()});let z=L.querySelector(".close-details-btn");return z&&z.addEventListener("click",N=>{N.stopPropagation(),L.classList.remove("expanded");let B=L.querySelector(".event-details");B&&(B.style.display="none"),ye.delete(s.id)}),L},b=s=>{let m=document.createElement("div");m.innerHTML=U("empty",s.message,s.action,s.id);let C=m.firstElementChild;return C.dataset.blueprintId=s.id,C},d=[];c.length===0?d.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):c.forEach(s=>d.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let x=Array.from(e.children),w=new Map(x.map(s=>[s.dataset.blueprintId,s])),p=new Set(d.map(s=>s.id));x.forEach(s=>{let m=s.dataset.blueprintId;(!m||!p.has(m))&&s.remove()});let y=null;d.forEach((s,m)=>{let C=w.get(s.id);(!C||t)&&(C&&C.remove(),s.type==="placeholder"?C=b(s):C=r(s),!C)||(m===0?e.firstElementChild!==C&&e.prepend(C):y&&y.nextElementSibling!==C&&y.after(C),y=C)}),Ne(2,c.length),ht()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=U("offline","Failed to load blueprints.","The event service may be offline."))}}async function It(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&a++}),xt(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(i=>ye.add(i.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var re="STANDBY",Bt=null,Se=[],Y=null,je=null,Ht=()=>`
    <div id="progress-view-root" class="progress-container">
        <!-- Initial render will happen here -->
        ${_t()}
    </div>
  `;function _t(){switch(re){case"ACTIVE":return Bs();case"COMPLETED":return Hs();case"STANDBY":default:return Is()}}function Is(){return`
  ...
  `}async function Me(){let t=document.getElementById("progress-view-root");t&&(re!==Bt&&(t.innerHTML=_t(),Bt=re),re==="ACTIVE"&&_s())}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),n=document.getElementById("active-task-progress-text"),i=document.getElementById("terminal-output");if(t&&Y&&(t.innerText=Y.title),e&&Y&&(e.style.width=`${Y.progress}%`),n&&Y&&(n.innerText=`${Y.progress}% COMPLETE`),i){let a=i.children.length;if(Se.length>a){for(let c=a;c<Se.length;c++){let r=Se[c],b=document.createElement("div");b.className="terminal-line",b.innerHTML=`
            <span class="line-time">${r.time}</span>
            <span class="line-prefix">></span>
            <span class="line-msg">${O(r.msg)}</span>
        `,i.appendChild(b)}i.scrollTop=i.scrollHeight}}}window.addEventListener("dex-mock-start",()=>{re="ACTIVE",Se=[{time:new Date().toLocaleTimeString(),msg:"Initializing cognitive engine..."},{time:new Date().toLocaleTimeString(),msg:"Connecting to event-service..."}],Y={title:"Refactoring System Event Bus",progress:10},Me();let t=0,e=setInterval(()=>{if(re!=="ACTIVE"){clearInterval(e);return}t++,Y.progress=Math.min(100,Y.progress+Math.floor(Math.random()*15));let n=["Analyzing internal/bus.go...","Checking circular dependencies.","Optimizing channel throughput.","Writing unit tests for refactor.","Deploying to staging environment.","Verifying system integrity."];t<n.length&&Se.push({time:new Date().toLocaleTimeString(),msg:n[t]}),Y.progress>=100&&(re="COMPLETED",je={duration:"1m 12s",steps:32,result:"Optimized Event Bus"},clearInterval(e)),Me()},1500)});window.addEventListener("dex-mock-stop",()=>{re="STANDBY",Me()});window.addEventListener("dex-mock-reset",()=>{re="STANDBY",Me()});var Dt=()=>`
  <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px; align-items: center;">
    ...
    <button id="create-chore-btn" class="notif-action-btn" style="${D()?"display: none;":""}" title="New Task"><i class='bx bx-plus'></i></button>
  </div>
`,zt=()=>`
        <!-- Create Task Form -->
    ...
    `,Ae=[],Ie={"313071000877137920":"Owen",dexter:"Dexter"},te=[],he=null;async function Be(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),c=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),b=document.getElementById("task-filter-owner"),d=document.getElementById("new-chore-instruction"),x=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),p=document.getElementById("new-chore-time"),y=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(y&&!y.dataset.initialValueAttached){let f=Intl.DateTimeFormat().resolvedOptions().timeZone,T=!1;for(let h=0;h<y.options.length;h++)if(y.options[h].value===f){T=!0;break}if(!T){let h=document.createElement("option");h.value=f,h.textContent=f.replace("_"," "),y.appendChild(h)}y.value=f,y.dataset.initialValueAttached="true"}w&&s&&!w.dataset.timeListenerAttached&&(w.addEventListener("change",()=>{s.style.display=w.value==="daily"?"block":"none"}),w.dataset.timeListenerAttached="true");let m=document.getElementById("form-title"),C=document.getElementById("form-icon");function I(){if(r){if(te.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=te.map(f=>{let T=Ie[f]||f,h=f.startsWith("channel:");return`
      ...
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(f=>{f.onclick=()=>{let T=f.dataset.id;te=te.filter(h=>h!==T),I()}})}}function o(f){f&&!te.includes(f)&&(te.push(f),I()),c&&(c.value="")}function l(f=null){if(n){if(he=f?f.id:null,f){if(m&&(m.textContent="Update Research Task"),C&&(C.className="bx bx-edit-alt",C.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),d&&(d.value=f.natural_instruction),x&&(x.value=f.execution_plan?.entry_url||""),w&&(w.value=f.schedule,s&&(s.style.display=f.schedule==="daily"?"block":"none")),p&&f.run_at&&(p.value=f.run_at),y&&f.timezone){let T=!1;for(let h=0;h<y.options.length;h++)if(y.options[h].value===f.timezone){T=!0;break}if(!T){let h=document.createElement("option");h.value=f.timezone,h.textContent=f.timezone.replace("_"," "),y.appendChild(h)}y.value=f.timezone}te=f.recipients||(f.owner_id?[f.owner_id]:[]),I()}else m&&(m.textContent="Initialize Research Task"),C&&(C.className="bx bx-plus-circle",C.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),d&&(d.value=""),x&&(x.value=""),w&&(w.value="every_24h"),te=[],I();n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"})}}let u=document.getElementById("contacts-group"),g=document.getElementById("channels-group");if(u&&!u.dataset.populated&&!D())try{fe("/contacts").then(async f=>{f.ok&&(((await f.json()).members||[]).forEach(v=>{if(Ie[v.id]=v.nickname||v.username,v.id==="313071000877137920")return;let k=document.createElement("option");k.value=v.id;let S=v.nickname||v.username;k.textContent=`${S} (${v.username})`,u.appendChild(k)}),u.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(g&&!g.dataset.populated&&!D())try{fe("/channels").then(async f=>{f.ok&&((await f.json()).forEach(h=>{let v=`channel:${h.id}`;Ie[v]=h.name;let k=document.createElement("option");k.value=v,k.textContent=`#${h.name} (${h.guild})`,g.appendChild(k)}),g.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}c&&!c.dataset.listenerAttached&&(c.onchange=()=>{o(c.value)},c.dataset.listenerAttached="true"),b&&!b.dataset.listenerAttached&&(b.onchange=()=>E(),b.dataset.listenerAttached="true");function E(){if(!t)return;let f=b?.value||"all",T=f==="all"?Ae:Ae.filter(v=>(v.recipients||(v.owner_id?[v.owner_id]:[])).includes(f));if(T.length===0){t.innerHTML=U("empty",f==="all"?"No active tasks.":"No tasks found for this owner.",D()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let h=T.map(v=>{let k=v.last_run===0?"Never":new Date(v.last_run*1e3).toLocaleString(),S=v.memory?v.memory.length:0,L=v.status==="active"?"#03dac6":"#666",W=(v.recipients||(v.owner_id?[v.owner_id]:[])).map(q=>{let G=Ie[q]||q.substring(0,8),A=q.startsWith("channel:");return`<span title="${G}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${q==="dexter"?"bx-terminal":A?"bx-hash":"bx-user"}'></i>${G}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${L}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            ...
                            <span style="font-size: 0.85em; color: #03dac6; font-weight: bold;">${S}</span>
                        </div>
            ...
            `}).join("");t.innerHTML=h,t.querySelectorAll(".edit-chore-btn").forEach(v=>{v.onclick=k=>{k.stopPropagation();let S=v.dataset.id,L=Ae.find(R=>R.id===S);L&&l(L)}}),t.querySelectorAll(".reset-chore-btn").forEach(v=>{v.onclick=async k=>{k.stopPropagation();let S=v.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(v.innerHTML="<i class='bx bx-loader-alt spin'></i>",await _(`/chores/${S}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),Be())}}),t.querySelectorAll(".delete-chore-btn").forEach(v=>{v.onclick=async k=>{k.stopPropagation();let S=v.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${S}`,{method:"DELETE"}),Be())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||he!==null?l(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let f=document.getElementById("new-chore-instruction"),T=document.getElementById("new-chore-url"),h=document.getElementById("new-chore-schedule"),v=f?.value,k=h?.value||"every_24h",S=k==="daily"?p?.value:"",L=k==="daily"?y?.value:"";if(!v)return;if(te.length===0){alert("Please add at least one recipient.");return}let R=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=he?"PATCH":"POST",q=he?`/chores/${he}`:"/chores";await _(q,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:te,natural_instruction:v,entry_url:T?.value,schedule:k,run_at:S,timezone:L})}),n&&(n.style.display="none"),f&&(f.value=""),T&&(T.value=""),he=null,te=[],Be()}catch(W){console.error(W),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=R}},i.dataset.listenerAttached="true");try{let f=await _("/chores");if(!f.ok)throw new Error(`HTTP error! status: ${f.status}`);if(Ae=await f.json()||[],b){let h=b.value,v=new Set;Ae.forEach(k=>{k.owner_id&&v.add(k.owner_id),k.recipients&&k.recipients.forEach(S=>v.add(S))}),b.innerHTML='<option value="all">All Recipients</option>',v.forEach(k=>{let S=document.createElement("option");S.value=k;let L=k.startsWith("channel:"),R=Ie[k]||`Recipient: ${k.substring(0,8)}`;S.textContent=(L&&!R.startsWith("#")?"#":"")+R,b.appendChild(S)}),b.value=h,b.selectedIndex===-1&&(b.value="all")}E()}catch(f){console.error("Failed to fetch chores",f)}}var Nt=()=>`
...
`,Ue=null,we=new Set,He=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await _(i);if(!a.ok)throw new Error("Service unreachable");if(He=((await a.json()).events||[]).filter(y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ue=Date.now(),ee(1,Ue),He.length===0){e.innerHTML=U("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,C=Os(m),I=js(m),o=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),l="event-border-grey";o&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?l="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?l="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?l="event-border-orange":m==="system.test.completed"?l=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let u=o?"cursor-pointer":"",g=we.has(y.id),E=g?"expanded":"",f=g?"display: block;":"display: none;",T=new Date(y.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=Ft(m,s),S=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",L="";if(o){let A="";if(m==="engagement.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,M=s.timestamp-s.last_active,H=Math.floor(M/60);A=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,M="";s.eval_count&&(M=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let H="";if(s.chat_history&&s.chat_history.length>0){let P=s.chat_history.length,V=s.chat_history.map((z,N)=>{let B=z.name?z.name.toUpperCase():z.role.toUpperCase();!z.name&&B==="ASSISTANT"&&(B="DEXTER");let F=z.role==="user"?"#03dac6":z.role==="system"?"#ffb74d":"#bb86fc",J=N===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${N}" style="display: ${J};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${F}; letter-spacing: 1px; font-weight: bold;">${B}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${N+1} of ${P}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${O(z.content)}</div>
                                </div>
                            `}).join("");H=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(m==="analysis.user.message_signals"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,M=s.signals||{};A=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(M.topics||[]).map(P=>`<span class="profile-badge">${P}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",M=s.success?"SUCCESS":"FAILED",H=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,P="";s.error&&(P=`
                            <div class="event-detail-block">
                                ${H("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${O(s.error)}</pre>
                            </div>
                        `);let V="";if(s.chat_history&&s.chat_history.length>0){let B=s.chat_history.length,F=s.chat_history.map((J,se)=>{let K=J.name?J.name.toUpperCase():J.role.toUpperCase();!J.name&&K==="USER"&&(K="SYSTEM"),!J.name&&K==="ASSISTANT"&&(K="AGENT");let le=J.role==="user"?"#03dac6":J.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${se}" style="display: ${se===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${le}; letter-spacing: 1px; font-weight: bold;">${K}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${se+1} of ${B}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${O(J.content)}</div>
                                </div>
                            `}).join("");V=`
                            <div class="event-detail-block">
                        ...
                        `}let z="";if(s.corrections&&s.corrections.length>0){let B=s.corrections.map((F,J)=>`
                        ...
                        `}let N="";if(s.parsed_results&&s.parsed_results.length>0){let B=s.parsed_results.map(F=>`
                    ...
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(H=>{let P=H.content_type&&H.content_type.startsWith("image/"),V=(H.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${P?`<div class="attachment-thumb-container"><img src="${H.url}" alt="${H.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${H.url}" target="_blank" class="attachment-link">${H.filename}</a>
                                        <span class="attachment-meta">${H.content_type} \u2022 ${V}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let R=document.createElement("div");R.className=`event-item ${l} ${u} ${E}`,R.dataset.eventId=y.id,R.onclick=function(A){if(!o)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),we.delete(y.id);let H=$.querySelector(".event-details");H&&(H.style.display="none")}else{$.classList.add("expanded"),we.add(y.id);let H=$.querySelector(".event-details");H&&(H.style.display="block")}},R.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${h}</span>
                    <span class="event-date">${v}</span>
                </div>
            ...
            `;let W=R.querySelector(".event-details");if(W&&W.addEventListener("click",A=>{A.stopPropagation()}),o){let A=R.querySelector(".close-details-btn");A&&A.addEventListener("click",$=>{$.stopPropagation();let M=$.target.closest(".event-item");if(M){M.classList.remove("expanded"),we.delete(y.id);let H=M.querySelector(".event-details");H&&(H.style.display="none")}})}let q=R.querySelector(".prev-btn"),G=R.querySelector(".next-btn");if(q&&G){let A=0,$=Array.from(R.querySelectorAll(".history-slide")),M=$.length,H=()=>{$.forEach((P,V)=>{P.style.display=V===A?"block":"none"}),q.disabled=A===0,G.disabled=A===M-1,q.style.opacity=A===0?"0.5":"1",G.style.opacity=A===M-1?"0.5":"1"};q.addEventListener("click",P=>{P.stopPropagation(),A>0&&(A--,H())}),G.addEventListener("click",P=>{P.stopPropagation(),A<M-1&&(A++,H())}),H()}return R},d=Array.from(e.children),x=new Map(d.map(y=>[y.dataset.eventId,y])),w=new Set(He.map(y=>y.id));d.forEach(y=>{let s=y.dataset.eventId;(!s||!w.has(s))&&y.remove()});let p=null;He.forEach((y,s)=>{let m=x.get(y.id);(!m||t)&&(m&&m.remove(),m=b(y),!m)||(s===0?e.firstElementChild!==m&&e.prepend(m):p&&p.nextElementSibling!==m&&p.after(m),p=m)}),Ue=Date.now(),ee(1,Ue)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=U("offline","Failed to load events.","The event service may be offline."))}}function Us(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),n=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{He.forEach(a=>we.add(a.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{we.clear(),pe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.querySelectorAll(".filter-btn").forEach(a=>{a.onclick=()=>{n.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active")),a.classList.add("active"),Z=a.dataset.filter||"all",pe(!0)}}),n.dataset.listenerAttached="true"),n&&n.querySelectorAll(".filter-btn").forEach(a=>{a.dataset.filter===Z?a.classList.add("active"):a.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let a=Z==="all"?"ALL":Z.toUpperCase();if(confirm(`Are you sure you want to delete ${a} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let c="/events";if(Z!=="all"?c+=`?category=${Z}`:c+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await _(c,{method:"DELETE"})).ok)throw new Error("Failed to delete events");we.clear(),pe(!0)}catch(c){console.error("Failed to clear events:",c),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var qs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Jt=qs;function Wt(){return`
    ...
    `}var Fs=null;async function _e(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=U("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(r=>!i.includes(r.id));a.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),a.reverse();let c=a.map(r=>{let b=r.logs.join(`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
        </div>`;async function ss(){Js(),await Promise.all([ne(),nt(),it()]),await _e()}var qe=null;function Js(){qe||(qe=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(qe),qe=null;return}ne(!0)},1e3))}var Gt=null,Vt=null;async function ns(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function Kt(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await _("/processes")).json()}catch{return null}}async function Gs(){try{return await(await _("/agent/status")).json()}catch{return null}}async function nt(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),n=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),a=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),b=o=>{if(o){if(n&&(n.innerHTML=`
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let l=o.status==="online",u=l?"service-widget-online":"service-widget-offline",g=l?"bx-check-circle":"bx-x-circle",E=l?"OK":"BAD",f=o.version?w(o.version.str):"-",T=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",h=o.memory&&o.memory!=="N/A"?o.memory:"-",v=T!=="-"?"#00ff00":"#666",k=T!=="-"?"#fff":"#666",S=h!=="-"?"#00ff00":"#666",L=h!=="-"?"#fff":"#666",R=y(o.uptime),W="";l?W=`
                ...
                </div>`:W='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let q=D()?"easter.company":p(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${u}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${E}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${q}</span></div>${W}</div></div>`}let m=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),C=new Set(x.map(o=>o.id));for(let[o,l]of m)C.has(o)||l.remove();let I=new Set;x.forEach(o=>{if(I.has(o.id))return;I.add(o.id);let l=s(o),u=m.get(o.id);if(u&&u.parentNode)if(o.id==="upstash-redis-ro"){let g=D()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),E=new Date(g).toLocaleTimeString(),f=u.querySelector(".sync-time-label");f&&(f.textContent=`Last synced: ${E}`)}else u.outerHTML!==l&&(u.outerHTML=l);else t.insertAdjacentHTML("beforeend",l)})}async function it(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=U("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),ee(2,Vt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function a(d){let x=d.status==="Ready";return`
          ...
          `)};n&&L(n,document.getElementById("guardian-sentry-stats"),u.protocols.sentry,"sentry");let R=document.getElementById("analyzer-synthesis-val"),W=document.getElementById("analyzer-synthesis-stats");R&&L(R,W,g.protocols.synthesis,"synthesis");let q=document.getElementById("imaginator-alert_review-val"),G=document.getElementById("imaginator-alert_review-stats");q&&L(q,G,E.protocols.alert_review,"alert_review");let A=document.getElementById("fabricator-construction-val"),$=document.getElementById("fabricator-construction-stats");A&&L(A,$,f.protocols.construction,"construction");let M=document.getElementById("courier-researcher-val"),H=document.getElementById("courier-researcher-stats");M&&L(M,H,T.protocols.researcher,"researcher");let P=document.getElementById("system-state-label"),V=document.getElementById("system-state-val");if(V&&h.state){let z=h.state,N=S(h.state_time||0);P&&(P.textContent=`State: ${z.toUpperCase()}`),V.textContent=N,z==="idle"?V.style.color=h.state_time>300?"#5eff5e":"#fff":V.style.color="#bb86fc",p&&(z==="paused"?(p.innerHTML="<i class='bx bx-play'></i>",p.title="Resume System",p.style.color="#ff9800"):(p.innerHTML="<i class='bx bx-pause'></i>",p.title="Pause System",p.style.color=""))}a&&(a.textContent=S(h.metrics?.total_idle_time||0)),c&&(c.textContent=S(h.metrics?.total_active_time||0)),r&&(r.textContent=S(h.metrics?.total_waste_time||0))}else[n,i,a,c,r].forEach(g=>{g&&(g.textContent="-",g.style.color="#666")});if(t)return;let s=await Ws(),m=[],C=[],I=[];s&&(Array.isArray(s)?m=s:(m=s.active||[],C=s.queue||[],I=s.history||[],I.sort((u,g)=>(g.end_time||0)-(u.end_time||0)))),e&&(m.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=U("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),st(e,m,!1)));let o=document.getElementById("processes-queue-widgets");o&&(C.length===0?!o.querySelector(".tab-placeholder")&&!o.querySelector('div[style*="font-style: italic"]')&&(o.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(o.innerHTML="",st(o,C,!1)));let l=document.getElementById("processes-history-widgets");l&&(I.length===0?l.querySelector(".tab-placeholder")||(l.innerHTML=U("empty","No recent history.")):(l.querySelector(".tab-placeholder")&&(l.innerHTML=""),st(l,I,!0))),Ne(1,m.length)}function st(t,e,n){function i(d){let x="";d.end_time?x=`${d.end_time-d.start_time}s`:x=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let w=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",p=d.channel_id,y={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(y[p]?p=y[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),n)return`
                ...
                ${Object.values(Q).map(i=>`
                    <div class="theme-card ${t===i?"active":""}" data-theme="${i}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${i===Q.DARK?"Simple, clean, dark.":i===Q.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Vs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `);let c=document.getElementById("terminal-cmd-name");c&&(c.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let b=document.getElementById("terminal-action-btn");return b&&(b.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",ot=!0,n}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",ot=!1}function ls(t,e,n="output"){if(!ot||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Re(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Zs(t){let e=cs.find(r=>r.id===t);if(!e)return;let n=Ks(e);n&&ls(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let r of i){await new Promise(d=>setTimeout(d,400+Math.random()*600));let b="output";r.includes("[ERROR]")?b="error":r.includes("[SUCCESS]")||r.includes("\u2713")?b="success":r.includes("!")&&(b="warning"),n&&ls(n,r,b)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let c=document.getElementById("terminal-action-btn");c&&(c.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Vs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Zs(a)})})}async function Ys(){if(!D())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}dt(),pt(),Ys(),bt(),ct();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=o=>{localStorage.setItem("dex_last_window",o)};function a(){for(;e.length>1;)e.shift().close(!0);let o=document.getElementById("windows-container"),l=document.querySelector("nav"),u=document.querySelector("footer"),g=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");o&&o.setAttribute("data-count",e.length.toString()),e.forEach(S=>{let L=document.getElementById(S.id);L&&L.classList.remove("hide-close")});let f=document.getElementById("dexter-menu-container"),T=document.getElementById("nav-window-switcher"),h=document.getElementById("settings-icon"),v=document.getElementById("close-all-windows"),k=window.innerWidth<880;if(!k){let S=document.getElementById("dexter-dropdown"),L=document.getElementById("dexter-menu-btn");S&&S.classList.remove("active"),L&&L.classList.remove("active"),o&&o.classList.remove("menu-open")}if(Ce(e.length>0),e.length>0)if(u?.classList.add("hide"),v&&(v.style.display=k?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",l?.classList.add("window-open"),o&&(o.style.paddingTop="60px"),f&&(f.style.display=k?"flex":"none"),h&&(h.style.display=k?"block":"none"),!k&&T){let S=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(S)?(T.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),c(b)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),c(d)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),c(w)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),c(x)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),c(p)})):T.innerHTML=""}else T&&(T.innerHTML="");else l?.classList.remove("window-open"),v&&(v.style.display="none"),o&&(o.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",g||E?u?.classList.remove("hide"):u?.classList.add("hide"),f&&(f.style.display="flex"),h&&(h.style.display="block"),T&&(T.innerHTML="");$e()}function c(o){if(o.isOpen()){o.close();return}for(;e.length>0;)e.pop().close(!0);e.push(o),o.open(),a()}function r(){[...e].forEach(l=>l.close()),e=[]}window.addEventListener("resize",a);let b=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Et(),onOpen:()=>de(),onClose:()=>{let o=e.indexOf(b);o>-1&&e.splice(o,1),a()}}),d=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:tt(),onOpen:()=>{d.setContent(tt()),pe()},onClose:()=>{let o=e.indexOf(d);o>-1&&e.splice(o,1),a()}}),x=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:qt(),onOpen:()=>et(),onClose:()=>{let o=e.indexOf(x);o>-1&&e.splice(o,1),a()}}),w=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(o=>D()?o.title!=="Hardware"&&o.title!=="Logs":!0),onOpen:()=>{nt(),ne(),it(),os(),_e()},onClose:()=>{let o=e.indexOf(w);o>-1&&e.splice(o,1),a()}}),p=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Nt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Pt()},{icon:"bx-task",title:"Tasks",content:Ot()}],onOpen:()=>Xe(),onClose:()=>{let o=e.indexOf(p);o>-1&&e.splice(o,1),a()}}),y=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:Fe(),onOpen:()=>{y.setContent(Fe()),at(y)},onClose:()=>{let o=e.indexOf(y);o>-1&&e.splice(o,1),a()}});window.dexter={viewEvent:o=>{d.isOpen()||c(d),setTimeout(()=>{let l=document.querySelector(`.event-item[data-event-id="${o}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)},viewAlert:o=>{b.isOpen()||c(b),setTimeout(()=>{let l=document.querySelector(`.event-item[data-alert-id="${o}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let o=document.getElementById("dexter-dropdown"),l=document.getElementById("dexter-menu-btn");o&&l&&(o.classList.remove("active"),l.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let o=document.getElementById("dexter-menu-btn"),l=document.getElementById("settings-icon"),u=document.getElementById("close-all-windows"),g=document.getElementById("nav-left-container"),E=document.getElementById("dexter-dropdown"),f=document.getElementById("windows-container");o&&E&&(o.onclick=h=>{h.stopPropagation();let v=window.innerWidth<880;if(v)E.style.top="",E.style.left="",E.style.right="",E.style.transform="";else{let S=o.getBoundingClientRect(),R=document.querySelector("nav").getBoundingClientRect(),W=220,q=10,A=S.left+S.width/2-W/2,$=R.right-10;A+W>$&&(A=$-W),A<10&&(A=10),E.style.top=R.bottom+q+"px",E.style.left=A+"px",E.style.right="auto",E.style.transform="none"}E.classList.toggle("active"),o.classList.toggle("active");let k=E.classList.contains("active");if(v){let S=document.querySelector("nav");k?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),S?.classList.add("window-open"),f?.classList.add("menu-open"),Ce(!0)):(f?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S?.classList.remove("window-open"),Ce(!1)))}}),l&&(l.onclick=h=>{h.stopPropagation(),c(y)}),u&&(u.onclick=h=>{h.stopPropagation(),r()});let T=(h,v,k)=>{let S=document.getElementById(h);S&&(S.onclick=L=>{L.stopPropagation(),s(),k&&i(k),c(v)})};T("alerts-menu-item",b,"alerts-window"),T("events-menu-item",d,"events-window"),T("monitor-menu-item",w,"monitor-window"),T("contacts-menu-item",x,"contacts-window"),T("workspace-menu-item",p,"workspace-window"),g&&(g.onclick=h=>{if(h.stopPropagation(),E&&E.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1));return}if(e.length>0)r();else{let v=window.location.pathname;if(!(v==="/"||v==="/index.html")){let L=(v.endsWith("/")&&v.length>1?v.slice(0,-1):v).split("/");L.pop();let R=L.join("/")||"/";window.location.href=R}}}),document.addEventListener("click",()=>{let h=window.innerWidth<880;E&&E.classList.contains("active")&&(s(),h&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1)))}),E&&(E.onclick=h=>h.stopPropagation())})();let C=window.location.pathname==="/"||window.location.pathname==="/index.html",I=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!C&&!I&&document.querySelector("footer")?.classList.add("hide"),D()||setInterval(()=>{d.isOpen()&&pe(),b.isOpen()?de():Ye()},1e3),setInterval(()=>{D()&&(b.isOpen()?de():Ye()),p.isOpen()?Xe():It(),D()&&d.isOpen()&&pe(),w.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

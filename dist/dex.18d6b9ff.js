...
`;async function Ge(){await Promise.all([ue(),ge(),$e()])}var Bt=()=>`
    ...
    `;async function Te(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),g=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=g?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),l&&(l.value=""),r&&(r.value=""),Te()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active chores.","Create one to start monitoring.");return}let g=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=g,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this chore?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Te())}})}catch(l){console.error("Failed to fetch chores",l)}}var ks=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,Pe=null,ye=new Set,Le=[],K="all",As={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Is={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Bs(t){for(let[e,s]of Object.entries(As))if(s.includes(t))return e;return"system"}function _s(t){return Is[t]||"bx-square-rounded"}async function re(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Hs();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await B(i);if(!n.ok)throw new Error("Service unreachable");if(Le=((await n.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),Pe=Date.now(),Q(1,Pe),Le.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,$=Bs(b),w=_s(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let u=a?"cursor-pointer":"",f=ye.has(v.id),x=f?"expanded":"",M=f?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=zt(b,o),D=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",j="";if(a){let S="";if(b==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                    ...
                    `}else if(b==="messaging.bot.sent_message"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h="";o.eval_count&&(h=`
                        ...
                        `);let T="";if(o.chat_history&&o.chat_history.length>0){let L=o.chat_history.length,_=o.chat_history.map((R,V)=>{let U=R.name?R.name.toUpperCase():R.role.toUpperCase();!R.name&&U==="ASSISTANT"&&(U="DEXTER");let F=R.role==="user"?"#03dac6":R.role==="system"?"#ffb74d":"#bb86fc",W=V===L-1?"block":"none";return`
                    ...
                    `}else if(b==="system.analysis.audit"){let C=o.success?"#03dac6":"#ff4d4d",h=o.success?"SUCCESS":"FAILED",T=U=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${U}</h5>`,L="";o.error&&(L=`
                            <div class="event-detail-block">
                                ${T("Error")}

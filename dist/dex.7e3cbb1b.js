                ...
                <div class="user-profile-card" 
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${k}; font-weight: 600; text-transform: uppercase;">${y?"DEXTER (ME)":o.level}</span>
...
`),c=[...o.logs];if(c.length<25){let k=25-c.length;for(let y=0;y<k;y++)c.push("")}else c.length>25&&(c=c.slice(-25));let g=c.map(k=>Ue(k)).join(`
`);return`
  ...
  `}async function Nt(){try{let e=await(await H("/processes")).json(),i=e.active||[],n=e.history||[],s=i.find(o=>o.channel_id==="system-fabricator");if(s){Ee="ACTIVE";let m=Math.floor(Date.now()/1e3)-s.start_time,c=10,g="REVIEW";m>30&&(c=30,g="INVESTIGATION"),m>120&&(c=60,g="CONSTRUCTION"),m>300&&(c=90,g="REPORTING"),te={title:s.state||"Fabricating system improvements...",progress:c,phase:g};let k=new Date(s.updated_at*1e3).toLocaleTimeString();(ae.length===0||ae[ae.length-1].msg!==s.state)&&(ae.push({time:k,msg:s.state}),ae.length>50&&ae.shift())}else{let o=n.find(m=>m.channel_id==="system-fabricator");if(o&&Date.now()/1e3-o.end_time<60){Ee="COMPLETED";let m=o.end_time-o.start_time;nt={duration:`${Math.floor(m/60)}m ${m%60}s`,steps:0,result:o.state,timestamp:o.end_time}}else Ee="STANDBY",ae=[]}let a=document.getElementById("progress-view-root");a&&(Ee!==zt&&(a.innerHTML=Rt(),zt=Ee),Ee==="ACTIVE"&&_s())}catch(t){console.error("Failed to update progress:",t)}}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),i=document.getElementById("active-task-progress-text"),n=document.getElementById("active-task-phase"),s=document.getElementById("terminal-output");if(t&&te&&(t.innerText=te.title),e&&te&&(e.style.width=`${te.progress}%`),i&&te&&(i.innerText=`${te.progress}% COMPLETE`),n&&te&&(n.innerText=`PHASE: ${te.phase}`),s){let a=s.children.length;if(ae.length>a){for(let o=a;o<ae.length;o++){let m=ae[o],c=document.createElement("div");c.className="terminal-line",c.innerHTML=`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-controls">
                ...
                </div>`}else T='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold; margin-top: auto;"><span>OFFLINE</span></div>';let O=z()||d.type==="md"?"easter.company":f(d.domain&&d.port?`${d.domain}:${d.port}`:"");return`<div class="service-widget ${$}" data-service-id="${d.id}"><div class="service-widget-header"><i class="bx ${E}"></i><h3>${d.short_name||d.id}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${T}</div>${C}</div>`}}async function re(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),a=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),m=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),g=document.getElementById("fabricator-reset-btn"),k=document.getElementById("fabricator-progress-btn"),y=document.getElementById("courier-reset-btn"),h=document.getElementById("system-pause-toggle-btn");k&&!k.dataset.listenerAttached&&(k.onclick=()=>{let d=document.querySelector("nav");d&&(d.style.display="none");let l=le({id:"fabricator-progress-window",title:"Mission Control",icon:"bx-loader-circle",className:"fabricator-progress-modal",appendToBody:!0,content:`

"use strict";(()=>{function at(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function ot(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,l=document.createElement("nav");l.innerHTML=n,document.body.prepend(l);let a=document.createElement("div");a.id="dexter-dropdown",a.className="dexter-dropdown",a.innerHTML=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var lt=1e3;function pe(t){let e=null,s=t.onClose||null,i=t.onOpen||null;function n(){e&&(e.style.zIndex=(++lt).toString())}function l(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",a),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++lt).toString(),e.addEventListener("mousedown",n);let b=t.icon||"bx-window",r="",v="",$;t.tabs&&t.tabs.length>0?(r=`<div class="tab-bar">${t.tabs.map((p,u)=>{let g=p.icon?`<i class="bx ${p.icon}"></i>`:`<span class="tab-glyph">${p.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,$=`<div class="window-content">${t.tabs.map((p,u)=>`<div class="tab-content ${u===0?"active":""}" data-tab-content="${u}">${p.content}</div>`).join("")}</div>`):(t.title&&(v=`<div class="window-title">${t.title}</div>`),$=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${$}
        `,m.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",w=>{w.stopPropagation(),c()}),window.addEventListener("resize",a),t.tabs&&e.querySelectorAll(".tab").forEach(w=>{w.addEventListener("click",()=>{if(!e)return;let o=w.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(u=>u.classList.remove("active")),w.classList.add("active"),e.querySelectorAll(".tab-content").forEach(u=>u.classList.remove("active"));let p=e.querySelector(`.tab-content[data-tab-content="${o}"]`);p&&p.classList.add("active"),f(w,e)})}),setTimeout(()=>{e&&e.classList.add("open"),i&&i()},10)}function a(){if(!e||!e.classList.contains("open"))return;let m=e.querySelector(".tab.active");m&&f(m,e)}function f(m,b){setTimeout(()=>{let r=b.querySelector(".tab-bar");if(!r)return;let v=Array.from(r.querySelectorAll(".tab")),$=v.indexOf(m),w=r.clientWidth,o=v[Math.max(0,$-2)],p=v[Math.min(v.length-1,$+2)],u=r,g=o.offsetLeft-u.offsetLeft-25,M=p.offsetLeft+p.offsetWidth-u.offsetLeft+25-g,A=M<=w?g-(w-M)/2:m.offsetLeft-u.offsetLeft-w/2+m.offsetWidth/2;r.scrollTo({left:A,behavior:"smooth"})},350)}function c(m=!1){e&&(window.removeEventListener("resize",a),m?(e.remove(),e=null):(e.classList.remove("open"),s&&s(),setTimeout(()=>{e?.remove(),e=null},400)))}function d(m){t.content=m;let b=e?.querySelector(".window-content");b&&(b.innerHTML=m)}function y(){return!!(e&&e.classList.contains("open"))}return{open:l,close:c,setContent:d,isOpen:y,focus:n,id:t.id}}var ct="easter_theme",Q={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Fe(){return localStorage.getItem(ct)||Q.DARK}function dt(t){if(!Object.values(Q).includes(t))throw new Error("Invalid theme");localStorage.setItem(ct,t),pt(t)}function pt(t){let e=document.body;Object.values(Q).forEach(l=>{e.classList.remove(`theme-${l}`)}),e.classList.add(`theme-${t}`);let s=document.querySelector('meta[name="theme-color"]');s||(s=document.createElement("meta"),s.name="theme-color",document.getElementsByTagName("head")[0].appendChild(s));let i={[Q.DARK]:"#050507",[Q.LIGHT]:"#FFFFFF",[Q.LEGACY]:"#000000"},n=i[t]||i[Q.DARK];if(s.setAttribute("content",n),!document.querySelector(".background")){let l=document.createElement("div");l.className="background",document.body.prepend(l)}e.classList.add("is-animated")}function mt(){let t=Fe();pt(t)}function N(t,e,s=null){let n={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",l=s?`<p class="placeholder-action">${s}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${n} placeholder-icon'></i><p class="placeholder-message">${e}</p>${l}</div>`}function z(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function ee(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!s)return;if(!e){s.textContent="Last updated: never";return}let i=Date.now(),n=Math.floor((i-e)/1e3),l;n<60?l=`${n}s ago`:n<3600?l=`${Math.floor(n/60)}m ago`:l=`${Math.floor(n/3600)}h ago`,s.textContent=`Last updated: ${l}`}var cs=0;function Me(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!s)return;let i=s.querySelector(".notification-badge");i&&(e>0?(i.textContent=e>9?"9+":e.toString(),i.style.display="flex"):i.style.display="none")}var se=0,ne=0;function ft(t){se=t,Ce()}function gt(t){ne=t,Ce()}function Ce(){let t=se+ne;cs=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let s=document.getElementById("alerts-menu-item");if(s){let a=s.querySelector(".notification-badge");a||(a=document.createElement("span"),a.className="notification-badge",a.style.marginLeft="auto",s.appendChild(a)),a.textContent=se>9?"9+":se.toString(),a.style.display=se>0?"flex":"none"}let i=document.getElementById("workspace-menu-item");if(i){let a=i.querySelector(".notification-badge");a||(a=document.createElement("span"),a.className="notification-badge",a.style.marginLeft="auto",i.appendChild(a)),a.textContent=ne>9?"9+":ne.toString(),a.style.display=ne>0?"flex":"none"}let n=document.getElementById("switch-alerts");if(n){let a=n.querySelector(".notification-badge");a||(a=document.createElement("span"),a.className="notification-badge",a.style.marginLeft="8px",n.appendChild(a)),a.textContent=se>9?"9+":se.toString(),a.style.display=se>0?"flex":"none"}let l=document.getElementById("switch-workspace");if(l){let a=l.querySelector(".notification-badge");a||(a=document.createElement("span"),a.className="notification-badge",a.style.marginLeft="8px",l.appendChild(a)),a.textContent=ne>9?"9+":ne.toString(),a.style.display=ne>0?"flex":"none"}}function _e(){let t=document.getElementById("alerts-list");if(!t)return;se=t.querySelectorAll(".alert-unread:not(.priority-low)").length,Ce()}function bt(){let t=document.getElementById("blueprints-list");if(!t)return;ne=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,Ce()}function ds(){return"https://event.easter.company"}function ps(){return"https://discord.easter.company"}var ms="http://127.0.0.1:8100",us="http://127.0.0.1:8300",fs={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function He(t){let e=z(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(n,l)=>{let a=fs[l];return a?`<span class="${a}">`:""});let s=(e.match(/<span/g)||[]).length,i=(e.match(/<\/span/g)||[]).length;return s>i&&(e+="</span>".repeat(s-i)),e}function me(t){if(!t)return"";let e=z(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(s,i)=>{let n=i.split("|").map(l=>l.trim());return n.every(l=>l.match(/^:?-+:?$/))?"":`<div class="md-table-row">${n.map(l=>`<span class="md-table-cell">${l}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}var gs="https://sterling-javelin-12539.upstash.io",bs="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function B(){return window.location.hostname.includes("easter.company")}var R=null,ie={lastDashboard:0,lastFrontend:0},Je=!1,vt=0,yt="dex_dashboard_snapshot";function vs(){let t=localStorage.getItem(yt);if(t)try{let e=JSON.parse(t);R=e,ie.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{R=null}}async function ut(){if(!(!B()||Je)){Je=!0,vt=Math.floor(Date.now()/1e3);try{let t=await hs("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);R=e,ie.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),ie.lastFrontend=Date.now(),localStorage.setItem(yt,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Je=!1}}}function ys(){if(!R||!R.agent_status)return;let t=R.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function xs(){if(!B())return;vs();let t=Math.floor(Date.now()/1e3),e=R?t-R.timestamp:1/0;(!R||e>120)&&await ut(),setInterval(()=>{let s=new Date,i=Math.floor(Date.now()/1e3),n=R?i-R.timestamp:1/0,l=i-vt,a=s.getSeconds()===59,f=n>300,c=l>60;(a&&c||f&&c)&&ut(),ys()},1e3)}B()&&xs();async function hs(t,...e){try{let i=await(await fetch(gs,{method:"POST",headers:{Authorization:`Bearer ${bs}`},body:JSON.stringify([t,...e])})).json();if(i.error)throw new Error(i.error);return i.result}catch(s){return console.error("Upstash Error:",s),null}}var he=null,we=null,Ie=!1,Be=!1;async function _(t,e={}){if(B()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(R.monitor),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(R.processes),{status:200});if(t.startsWith("/events")){let n=new URLSearchParams(t.split("?")[1]||""),l=n.get("type")||n.get("event.type"),a=n.get("category"),f=n.get("order")||"desc",c=[];return l==="system.notification.generated"?c=[...R.alerts||[]]:l==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?c=[...R.blueprints||[]]:a==="messaging"?c=[...R.messaging_events||[]]:a==="system"?c=[...R.system_events||[]]:a==="cognitive"?c=[...R.cognitive_events||[]]:a==="moderation"?c=[...R.moderation_events||[]]:c=[...R.events||[]],f==="asc"?c.sort((d,y)=>d.timestamp-y.timestamp):c.sort((d,y)=>y.timestamp-d.timestamp),new Response(JSON.stringify({events:c,count:c.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(R.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let n=t.split("/")[2],l=R.profiles?R.profiles[n]:null;return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(R.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify([]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(he)try{let n=await fetch(he+t,e);if(n.ok)return n;he=null}catch{he=null}let s=ds(),i=ms;try{let n=await fetch(s+t,e);if(n.ok)return he=s,Ie&&(console.log("\u2728 Production event service restored."),Ie=!1),n;throw new Error("Primary failed")}catch{Ie||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${i}`),Ie=!0);try{let l=await fetch(i+t,e);if(l.ok)return he=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}async function Ee(t,e={}){if(B()){if(!R)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(R.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let n=t.split("/")[2],l=(R.contacts?.members||[]).find(a=>a.id===n);return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(we)try{let n=await fetch(we+t,e);if(n.ok)return n;we=null}catch{we=null}let s=ps(),i=us;try{let n=await fetch(s+t,e);if(n.ok)return we=s,Be&&(console.log("\u2728 Production discord service restored."),Be=!1),n;throw new Error("Primary failed")}catch{Be||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${i}`),Be=!0);try{let l=await fetch(i+t,e);if(l.ok)return we=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}var xt=()=>`
...
`,De=null,ue=new Set,ze=[];async function ce(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ws(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];De=Date.now(),ee(0,De);let a=Date.now(),f=24*60*60*1e3,c=l.filter(u=>{let g=localStorage.getItem(`alert_read_ts_${u.id}`);if(!g)return!0;let x=parseInt(g);return a-x<f});c.sort((u,g)=>{let x=k=>{let E=k.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},M=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,A=M(x(u)),I=M(x(g));return A!==I?I-A:g.timestamp-u.timestamp}),ze=c;let d=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},y=[],b=new Set(c.map(u=>d(u))).size>1;if(c.length>0){let u=null;c.forEach(g=>{let x=d(g);b&&x!==u&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),u=x),y.push(g)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),_e();return}let r=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let x=(g.title||"Untitled Alert").trim(),M=(g.body||"No description provided.").trim(),A=g.summary||"",I=g.content||"",k=g.protocol||"",E=(g.priority||"low").toLowerCase(),D=!!g.alert,j=(g.category||"system").trim(),O=g.related_event_ids||[],q=g.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${u.id}`),S=new Date(u.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=J?"event-border-grey":"event-border-blue";!J&&D&&(T="event-border-red"),J&&(E==="high"||E==="critical")?T="event-border-red":J&&E==="medium"&&(T="event-border-orange");let L=J?"alert-read":"alert-unread",H=ue.has(u.id),P=H?"expanded":"",V=H?"display: block;":"display: none;",U="",F="";O.length>0&&(F=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${O.map(le=>`<a href="#" onclick="window.dexter.viewEvent('${le}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${le.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let W="";q&&(W=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${me(I)}</div>
            </div>
        `:te=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${me(M)}</div>
            </div>
        `,U=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${j}</div>
                    </div>
                    ${re}
                    ${W}
                    ${F}
                ...
                ${te}
            `;let X=document.createElement("div");X.className=`event-item notification-item ${T} ${L} ${P} cursor-pointer priority-${E}`,X.dataset.alertId=u.id,X.onclick=function(le){let Y=this;if(Y.classList.contains("expanded")){Y.classList.remove("expanded"),ue.delete(u.id);let ke=Y.querySelector(".event-details");ke&&(ke.style.display="none")}else{Y.classList.add("expanded"),ue.add(u.id);let ke=Y.querySelector(".event-details");if(ke&&(ke.style.display="block"),!localStorage.getItem(`alert_read_ts_${u.id}`)){localStorage.setItem(`alert_read_ts_${u.id}`,Date.now().toString()),Y.classList.add("alert-read"),Y.classList.remove("alert-unread"),Y.classList.remove("event-border-blue","event-border-red","event-border-purple");let qe="event-border-grey";E==="high"||E==="critical"?qe="event-border-red":E==="medium"&&(qe="event-border-orange"),Y.classList.add(qe),_e()}}};let st=`${k?k.toUpperCase():"GUARDIAN"} ALERT: ${A||x}`,rs={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[j]||"bx-bell",ls=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";X.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${C}</span>
                    <span class="event-date">${h}</span>
                </div>
                <div class="event-icon"><i class='bx ${rs}'></i></div>
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${ls}; font-size: 0.8em; margin-left: 5px;">[${E.toUpperCase()}]</span>
                    </div>
                    <div class="event-message">${me(st)}</div>
                    <div class="event-details" style="${V}">
                        ${U}
                    </div>
                </div>
            `;let nt=X.querySelector(".event-details");nt&&nt.addEventListener("click",le=>{le.stopPropagation()});let it=X.querySelector(".close-details-btn");return it&&it.addEventListener("click",le=>{le.stopPropagation(),X.classList.remove("expanded");let Y=X.querySelector(".event-details");Y&&(Y.style.display="none"),ue.delete(u.id)}),X},v=u=>{let g=document.createElement("div");g.className="notification-divider",g.dataset.alertId=u.id;let x="#888888";return u.label==="CRITICAL"?x="#ff4d4d":u.label==="HIGH"?x="#ff8888":u.label==="MEDIUM"&&(x="#ffa500"),g.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,g.innerHTML=`<span style="white-space: nowrap;">${u.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,g},$=Array.from(e.children),w=new Map($.map(u=>[u.dataset.alertId,u])),o=new Set(y.map(u=>u.id));$.forEach(u=>{let g=u.dataset.alertId;(!g||!o.has(g))&&u.remove()});let p=null;y.forEach((u,g)=>{let x=w.get(u.id);if(!x||t){if(x&&x.remove(),u.type==="divider")x=v(u);else{let M=r(u);if(!M)return;x=M}if(!x)return}g===0?e.firstElementChild!==x&&e.prepend(x):p&&p.nextElementSibling!==x&&p.after(x),p=x}),De=Date.now(),ee(0,De),_e()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load alerts.","The event service may be offline."))}}function ws(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),s=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ze.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ce(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ze.forEach(n=>{ue.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ce(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{ue.clear(),ce(!0)},s.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;ze.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,n.toString())}),ue.clear(),ce(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function We(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(l=>{let a=l.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{a={}}if((a.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||n++}),ft(n)}catch{}}var ht=()=>`
...
`,fe=new Set,Ne=[],$e=null;async function ge(t=!1){let e=document.getElementById("roadmap-list");if(e){Cs();try{let s=await _("/roadmap");if(!s.ok)throw new Error("Offline");let i=await s.json();Ne=i;let n=d=>{let y=fe.has(d.id),m=d.state==="published",b=d.state==="consumed",r="event-border-grey";m&&(r="event-border-blue"),b&&(r="event-border-purple");let $=new Date(d.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${r} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=d.id,w.onclick=u=>{if(u.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),fe.delete(d.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),fe.add(d.id)}};let o=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${d.state.toUpperCase()}]</span>`;w.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${z(d.content)}</div>
      ...
      `;let p=w.querySelector(".event-details");return p&&p.addEventListener("click",u=>{u.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>Es(d)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>$s(d)),w.querySelector(".delete-btn")?.addEventListener("click",()=>Ts(d.id)),w.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),w.classList.remove("expanded");let g=w.querySelector(".event-details");g&&(g.style.display="none"),fe.delete(d.id)}),w},l=Array.from(e.children),a=new Map(l.map(d=>[d.dataset.itemId,d])),f=new Set(i.map(d=>d.id));if(l.forEach(d=>{let y=d.dataset.itemId;(!y||!f.has(y))&&d.remove()}),!Ne||Ne.length===0){e.innerHTML=N("empty","Your roadmap is empty.",B()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((d,y)=>{let m=a.get(d.id);(!m||t)&&(m&&m.remove(),m=n(d),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function Cs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{$e=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let a=document.getElementById("roadmap-editor-container");a&&(a.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),$e=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),a=l?l.value:"";if(!a.trim())return;let f=$e?`/roadmap/${$e}`:"/roadmap",c=$e?"PATCH":"POST";try{await _(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:a})});let d=document.getElementById("roadmap-editor-container");d&&(d.style.display="none"),ge(!0)}catch(d){console.error(d)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{Ne.forEach(l=>fe.add(l.id)),ge(!0)},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{fe.clear(),ge(!0)},n.dataset.listenerAttached="true")}function Es(t){$e=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function $s(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ge(!0)}catch(s){console.error(s)}}async function Ts(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),ge(!0)}catch(e){console.error(e)}}var Et=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ct=null,be=new Set,Tt=[];async function ve(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ss();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(Tt=l,Ct=Date.now(),ee(2,Ct),l.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Me(2,0);return}t&&(e.innerHTML="");let a=m=>{let b=m.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let r=(b.title||"Untitled Blueprint").trim(),v=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),w=(b.category||"architecture").trim(),o=(b.related_services||b.affected_services||[]).map(H=>H.trim()),p=(b.implementation_path||[]).map(H=>H.trim()),u=b.source_event_ids||[],g=b.approved===!0,x=new Date(m.timestamp*1e3),M=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=be.has(m.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${g?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,g&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let j=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(H){let P=this;if(P.classList.contains("expanded")){P.classList.remove("expanded"),be.delete(m.id);let U=P.querySelector(".event-details");U&&(U.style.display="none")}else{P.classList.add("expanded"),be.add(m.id);let U=P.querySelector(".event-details");U&&(U.style.display="block")}};let O="";p.length>0&&(O=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${p.map(H=>`<li style="margin-bottom: 5px;">${z(H)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";u.length>0&&(q=`
                ...
                `);let G=o.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${o.join(", ")}</span></div>`:"<div></div>",J=B(),S=g?`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-icon" style="${g?"color: #03dac6;":""}"><i class='bx ${j}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${k}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${z(v)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${z($)}</div>
                        </div>
                        ${O}
                        ${q}
                        ${S}
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async H=>{H.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ve(!0)}catch(P){console.error("Failed to approve blueprint:",P)}});let h=E.querySelector(".blueprint-delete-btn");h&&(h.onclick=async H=>{H.stopPropagation();let P=!g;h.innerHTML=P?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${m.id}`,{method:"DELETE"})).ok&&ve(!0)}catch(V){console.error("Failed to delete blueprint:",V)}});let T=E.querySelector(".event-details");T&&T.addEventListener("click",H=>{H.stopPropagation()});let L=E.querySelector(".close-details-btn");return L&&L.addEventListener("click",H=>{H.stopPropagation(),E.classList.remove("expanded");let P=E.querySelector(".event-details");P&&(P.style.display="none"),be.delete(m.id)}),E},f=Array.from(e.children),c=new Map(f.map(m=>[m.dataset.blueprintId,m])),d=new Set(l.map(m=>m.id));f.forEach(m=>{let b=m.dataset.blueprintId;(!b||!d.has(b))&&m.remove()});let y=null;l.forEach((m,b)=>{let r=c.get(m.id);if(!r||t){r&&r.remove();let v=a(m);if(!v)return;r=v}b===0?e.firstElementChild!==r&&e.prepend(r):y&&y.nextElementSibling!==r&&y.after(r),y=r}),Me(2,l.length),bt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function St(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(l=>{let a=l.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{a={}}a.approved!==!0&&n++}),gt(n)}catch{}}function Ss(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Tt.forEach(i=>be.add(i.id)),ve(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{be.clear(),ve(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),be.clear(),ve(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var ae="STANDBY",kt=null,Te=[],Z=null,Pe=null,Mt=()=>`
    <div id="progress-view-root" class="progress-container">
        <!-- Initial render will happen here -->
        ${Lt()}
    </div>
  `;function Lt(){switch(ae){case"ACTIVE":return Ms();case"COMPLETED":return Ls();case"STANDBY":default:return ks()}}function ks(){return`
  ...
  `}function Ms(){let t=Te.map(e=>`
    <div class="terminal-line">
        <span class="line-time">${e.time}</span>
        <span class="line-prefix">></span>
        <span class="line-msg">${z(e.msg)}</span>
    </div>
  `).join("");return`
    <div class="active-task-card">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <div>
                ...
                <h2 id="active-task-title" style="margin: 5px 0 0 0; font-size: 1.2em;">${Z?.title||"Processing..."}</h2>
            </div>
        ...
        <div class="task-progress-bar">
            <div id="active-task-progress-fill" class="task-progress-fill" style="width: ${Z?.progress||0}%"></div>
        </div>
        <div style="margin-top: 8px; display: flex; justify-content: space-between; font-size: 0.75em; font-family: 'JetBrains Mono'; color: #666;">
            ...
            <span id="active-task-progress-text">${Z?.progress||0}% COMPLETE</span>
        </div>
    </div>
  ...
  `}async function Se(){let t=document.getElementById("progress-view-root");t&&(ae!==kt&&(t.innerHTML=Lt(),kt=ae),ae==="ACTIVE"&&As())}function As(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),s=document.getElementById("active-task-progress-text"),i=document.getElementById("terminal-output");if(t&&Z&&(t.innerText=Z.title),e&&Z&&(e.style.width=`${Z.progress}%`),s&&Z&&(s.innerText=`${Z.progress}% COMPLETE`),i){let n=i.children.length;if(Te.length>n){for(let l=n;l<Te.length;l++){let a=Te[l],f=document.createElement("div");f.className="terminal-line",f.innerHTML=`
            <span class="line-time">${a.time}</span>
            <span class="line-prefix">></span>
            <span class="line-msg">${z(a.msg)}</span>
        `,i.appendChild(f)}i.scrollTop=i.scrollHeight}}}window.addEventListener("dex-mock-start",()=>{ae="ACTIVE",Te=[{time:new Date().toLocaleTimeString(),msg:"Initializing cognitive engine..."},{time:new Date().toLocaleTimeString(),msg:"Connecting to event-service..."}],Z={title:"Refactoring System Event Bus",progress:10},Se();let t=0,e=setInterval(()=>{if(ae!=="ACTIVE"){clearInterval(e);return}t++,Z.progress=Math.min(100,Z.progress+Math.floor(Math.random()*15));let s=["Analyzing internal/bus.go...","Checking circular dependencies.","Optimizing channel throughput.","Writing unit tests for refactor.","Deploying to staging environment.","Verifying system integrity."];t<s.length&&Te.push({time:new Date().toLocaleTimeString(),msg:s[t]}),Z.progress>=100&&(ae="COMPLETED",Pe={duration:"1m 12s",steps:32,result:"Optimized Event Bus"},clearInterval(e)),Se()},1500)});window.addEventListener("dex-mock-stop",()=>{ae="STANDBY",Se()});window.addEventListener("dex-mock-reset",()=>{ae="STANDBY",Se()});var At=()=>`
  <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,It=()=>`
    ...
    `;async function Re(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner");if(l&&!l.dataset.populated&&!B())try{let a=await Ee("/contacts");a.ok&&(((await a.json()).members||[]).forEach(d=>{if(d.id==="313071000877137920")return;let y=document.createElement("option");y.value=d.id;let m=d.nickname||d.username;y.textContent=`${m} (${d.username})`,l.appendChild(y)}),l.dataset.populated="true")}catch{console.warn("Failed to fetch contacts for dropdown")}if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let a=document.getElementById("new-chore-instruction"),f=document.getElementById("new-chore-url"),c=document.getElementById("new-chore-owner"),d=a?.value,y=f?.value,m=c?.value||"313071000877137920";if(d){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:m,natural_instruction:d,entry_url:y,schedule:"every_6h"})}),s&&(s.style.display="none"),a&&(a.value=""),f&&(f.value=""),Re()}catch(b){console.error(b),alert("Failed to create research task")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let a=await _("/chores");if(!a.ok)throw new Error(`HTTP error! status: ${a.status}`);let f=await a.json();if(!Array.isArray(f)||f.length===0){t.innerHTML=N("empty","No active tasks.",B()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let c=f.map(d=>{let y=d.last_run===0?"Never":new Date(d.last_run*1e3).toLocaleString(),m=d.memory?d.memory.length:0,b=d.status==="active"?"#03dac6":"#666";return`
                <div class="service-widget" style="border-left: 3px solid ${b}; width: 100%;">
                    <div class="service-widget-header">
                        <i class='bx bx-search-alt' style="color: ${b}"></i>
                        <h3 style="font-size: 0.95em; white-space: normal; line-height: 1.4;">${d.natural_instruction}</h3>
                        <div style="margin-left: auto; display: flex; gap: 10px;">
                            <button class="icon-btn delete-chore-btn" data-id="${d.id}" style="background: none; border: none; color: #cf6679; cursor: pointer; padding: 5px;"><i class='bx bx-trash'></i></button>
                        </div>
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">Schedule:</span>
                            <span class="info-value">${d.schedule}</span>
                        </div>
                        <div class="service-widget-info">
                        ...
                        <div class="service-widget-info" style="grid-column: span 2; margin-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 5px;">
                            ...
                            <span class="info-value" style="font-size: 0.8em; opacity: 0.8; word-break: break-all;">${d.execution_plan.entry_url||"Auto-detected"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=c,t.querySelectorAll(".delete-chore-btn").forEach(d=>{d.onclick=async y=>{y.stopPropagation();let m=d.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${m}`,{method:"DELETE"}),Re())}})}catch(a){console.error("Failed to fetch chores",a)}}var Bt=()=>`
...
`;async function Ge(){await Promise.all([ge(),ve(),Se(),Re()])}var Is=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function zt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Nt(t){if(!document.getElementById("dex-profile-styles")){let s=document.createElement("style");s.id="dex-profile-styles",s.textContent=Is,document.head.appendChild(s)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",s=>{s.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ee(`/profile/${t.id}`).then(async s=>{if(s.ok){let i=await s.json();Ve(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=zt(t.id,t.username);Ve(e,t,i)}}).catch(s=>{console.error("Profile fetch error:",s);let i=zt(t.id,t.username);Ve(e,t,i)})}function Ve(t,e,s){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(s.attributes||s.facts||[]).map(x=>{let M=x.key||x.k,A=x.value||x.v;return`
            <div class="fact-chip">
                <span class="fact-key">${M}:</span>
                <span class="fact-val">${A}</span>
            </div>
        `}).join(""),w=s.cognitive_model||s,o=w.technical_level||w.techLevel||0,p=w.communication_style||w.commStyle||"Unknown",u=w.patience_level||w.patience||"Unknown",g=w.vibe||"Unknown";return`
        ...
        `},l=()=>{let $=s.dossier||{},w=$.identity||{},o=$.career||{},p=$.personal||{},u=$.social||[];return`
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.hobbies||[]).map(g=>`<div class="dossier-list-item">${g}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.habits||[]).map(g=>`<div class="dossier-list-item">${g}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.vices||[]).map(g=>`<div class="dossier-list-item" style="color: #cf6679;">${g}</div>`).join("")}
                        </div>
                    </div>
                </div>
            ...
            ${(s.topics||[]).map($=>`
                <div class="topic-bar">
                    <div class="topic-header">
                        ...
                        <span style="color: #bb86fc;">${$.val}%</span>
                    </div>
                    <div class="topic-track">
                        <div class="topic-fill" style="width: ${$.val}%"></div>
                    </div>
                </div>
            `).join("")}
        </div>
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${z(e.username)}</h2>
                    <div class="profile-badges">${(s.badges||[]).map($=>`<span class="profile-badge ${$==="Creator"?"master":""}">${$}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=d;let y=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),b=t.querySelectorAll(".profile-tab-btn"),r=t.querySelectorAll(".tab-content"),v=t.querySelector(".close-profile-btn");v&&v.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),b.forEach($=>{$.addEventListener("click",()=>{b.forEach(p=>p.classList.remove("active")),r.forEach(p=>p.classList.remove("active")),$.classList.add("active");let w=$.dataset.tab,o=t.querySelector(`#tab-${w}`);o&&o.classList.add("active")})}),m&&m.addEventListener("click",()=>{y.classList.toggle("expanded");let $=y.classList.contains("expanded");m.innerHTML=$?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Pt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,Bs=null;async function Ke(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ke(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let n=s.target.closest(".user-profile-card");if(n&&n.dataset.user)try{let l=JSON.parse(n.dataset.user);Nt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let s=await Ee("/contacts");if(!s.ok)throw new Error("Service unreachable");let n=(await s.json()).members||[];if(Bs=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((a,f)=>{let c=l[a.level]??10,d=l[f.level]??10;return c!==d?c-d:a.username.localeCompare(f.username)}),t.innerHTML=n.map(a=>{let f=a.color&&a.color!==0,c=f?"#"+a.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",d=a.status==="online"?"#5eff5e":a.status==="idle"?"#ffa500":a.status==="dnd"?"#ff4d4d":"#666",y="#888";a.level==="Me"||a.level==="Master"?y="#bb86fc":a.level==="Admin"?y="#cf6679":a.level==="Moderator"?y="#03dac6":a.level==="Contributor"&&(y="#ffa500");let m=a.level==="Me",b=m?"rgba(187, 134, 252, 0.08)":f?`${c}11`:"rgba(255,255,255,0.03)",r=m?"2px solid #bb86fc":f?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(a).replace(/"/g,"&quot;")}"
                     style="background: ${b}; padding: 15px; border-radius: 8px; border: ${r}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s, box-shadow 0.2s;">
                    ...
                    <div style="position: relative;">
                        <img src="${a.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${m?"2px solid #bb86fc":f?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${f?c:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${a.nickname||a.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${y}; font-weight: 600; text-transform: uppercase;">${m?"DEXTER (ME)":a.level}</span>
    ...
    <div class="system-section-header" style="margin-bottom: 20px;">
        ...
        <div id="event-filters" class="event-filters" style="margin-left: 20px; margin-bottom: 0;">
            <button class="notif-action-btn filter-btn ${K==="all"?"active":""}" data-filter="all" title="All Events"><i class='bx bx-list-ul'></i></button>
            <button class="notif-action-btn filter-btn ${K==="messaging"?"active":""}" data-filter="messaging" title="Messaging"><i class='bx bx-message-square-detail'></i></button>
            <button class="notif-action-btn filter-btn ${K==="system"?"active":""}" data-filter="system" title="System"><i class='bx bx-cog'></i></button>
            <button class="notif-action-btn filter-btn ${K==="cognitive"?"active":""}" data-filter="cognitive" title="Cognitive"><i class='bx bx-brain'></i></button>
            <button class="notif-action-btn filter-btn ${K==="moderation"?"active":""}" data-filter="moderation" title="Moderation"><i class='bx bx-shield-quarter'></i></button>
        </div>
        <div class="alerts-actions" style="margin-left: auto; display: flex; gap: 10px; padding: 0;">
...
`,Oe=null,ye=new Set,Le=[],K="all",Hs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ds={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function zs(t){for(let[e,s]of Object.entries(Hs))if(s.includes(t))return e;return"system"}function Ns(t){return Ds[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ps();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await _(i);if(!n.ok)throw new Error("Service unreachable");if(Le=((await n.json()).events||[]).filter(b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!0}let v=r.type;return!(v==="system.blueprint.generated"||v==="system.notification.generated")}),Oe=Date.now(),ee(1,Oe),Le.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=b=>{let r=b.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let v=r.type,$=zs(v),w=Ns(v),o=v==="engagement.decision"||v==="messaging.bot.sent_message"||v==="messaging.user.sent_message"||v==="moderation.explicit_content.deleted"||v==="analysis.link.completed"||v==="analysis.visual.completed"||v==="analysis.router.decision"||v==="analysis.user.message_signals"||v==="system.cli.command"||v==="system.analysis.audit"||v==="system.test.completed"||v==="error_occurred"||v==="system.cli.status"||v==="system.attention.expired"||v.startsWith("system.roadmap")||v.startsWith("system.process"),p="event-border-grey";o&&(v==="moderation.explicit_content.deleted"||v==="error_occurred"?p="event-border-red":v==="analysis.link.completed"||v==="analysis.visual.completed"||v==="analysis.router.decision"||v==="system.analysis.audit"||v==="analysis.user.message_signals"?p="event-border-purple":v==="system.attention.expired"||v==="system.cli.command"||v==="system.cli.status"?p="event-border-orange":v==="system.test.completed"?p=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":p="event-border-blue");let u=o?"cursor-pointer":"",g=ye.has(b.id),x=g?"expanded":"",M=g?"display: block;":"display: none;",A=new Date(b.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Rt(v,r),D=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",j="";if(o){let S="";if(v==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        ...
                        <div class="event-detail-block">
                            ${C("Input Decision")}
                            <pre class="detail-pre">${r.input_decision||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${C("Context History")}
                    ...
                    `}else if(v==="system.attention.expired"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=r.timestamp-r.last_active,T=Math.floor(h/60);S=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <div class="detail-pre">${me(r.last_input||"None")}</div>
                        </div>
                        <div class="event-detail-block">
                            ${C("Thought Process (Last Output)")}
                            <div class="detail-pre">${me(r.last_output||"None")}</div>
                        </div>
                    `}else if(v==="messaging.bot.sent_message"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h="";r.eval_count&&(h=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let T="";if(r.chat_history&&r.chat_history.length>0){let L=r.chat_history.length,H=r.chat_history.map((P,V)=>{let U=P.name?P.name.toUpperCase():P.role.toUpperCase();!P.name&&U==="ASSISTANT"&&(U="DEXTER");let F=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",W=V===L-1?"block":"none";return`
                                <div class="history-slide" data-index="${V}" style="display: ${W};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${F}; letter-spacing: 1px; font-weight: bold;">${U}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${V+1} of ${L}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${z(P.content)}</div>
                                </div>
                            `}).join("");T=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(v==="analysis.user.message_signals"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=r.signals||{};S=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Sentiment</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${h.sentiment>.3?"#03dac6":h.sentiment<-.3?"#cf6679":"#aaa"}; font-weight: bold;">${h.sentiment?.toFixed(2)||0}</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Intent</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; text-transform: uppercase;">${h.intent||"N/A"}</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Tech Depth</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${h.technical_depth||0}/10</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Mood</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #bb86fc;">${h.mood||"N/A"}</div>
                            </div>
                        </div>
                        <div class="event-detail-block">
                            ${C("Extracted Topics")}
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(h.topics||[]).map(L=>`<span class="profile-badge">${L}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${C("Raw Model Output")}
                            <pre class="detail-pre">${z(r.raw_output)||"None"}</pre>
                        </div>
                    `}else if(v==="moderation.explicit_content.deleted"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(v==="analysis.link.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${z(r.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${C("Description")}
                            <pre class="detail-pre">${z(r.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${C("Content Summary")}
                            <pre class="detail-pre">${z(r.summary)||"None"}</pre>
                        </div>
                    `}else if(v==="analysis.visual.completed"){let C=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,h="";r.base64_preview?h=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${r.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:r.url&&(h=`<div class="event-detail-block"><img src="${r.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(v==="analysis.router.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${C("Input Context")}
                            <pre class="detail-pre">${z(r.raw_input)||"None"}</pre>
                        </div>
                    `}else if(v==="system.cli.command"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${C("Output")}
                            <pre class="detail-pre">${z(r.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(v==="system.analysis.audit"){let C=r.success?"#03dac6":"#ff4d4d",h=r.success?"SUCCESS":"FAILED",T=U=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${U}</h5>`,L="";r.error&&(L=`
                            <div class="event-detail-block">
                                ${T("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${z(r.error)}</pre>
                            </div>
                        `);let H="";if(r.chat_history&&r.chat_history.length>0){let U=r.chat_history.length,F=r.chat_history.map((W,re)=>{let te=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&te==="USER"&&(te="SYSTEM"),!W.name&&te==="ASSISTANT"&&(te="AGENT");let X=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${re}" style="display: ${re===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${X}; letter-spacing: 1px; font-weight: bold;">${te}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${re+1} of ${U}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${z(W.content)}</div>
                                </div>
                            `}).join("");H=`
                            <div class="event-detail-block">
                        ...
                        `}let P="";if(r.corrections&&r.corrections.length>0){let U=r.corrections.map((F,W)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${F.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${z(F.snippet)}</div>`:""}
                            </div>
                        `).join("");P=`
                            <div class="event-detail-block">
                                ${T(`Corrections (${r.corrections.length})`)}
                                ${U}
                            </div>
                        `}let V="";if(r.parsed_results&&r.parsed_results.length>0){let U=r.parsed_results.map(F=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${z(F.title)}</div>
                                <div style="color: #ddd;">${z(F.summary)}</div>
                            </div>
                        `).join("");V=`
                            <div class="event-detail-block">
                                ${T("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${C}; font-weight: bold;">${h} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${r.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${P}
                        ${H}
                    `}else if(v==="system.test.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(v==="error_occurred"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                        ...
                        <div class="event-detail-block">
                            ${C("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${z(r.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${C("Context")}
                            <pre class="detail-pre">${JSON.stringify(r.context||{},null,2)}</pre>
                        </div>
                    `}else if(v==="system.cli.status"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ${C("Message")}
                            <pre class="detail-pre">${z(r.message)}</pre>
                        </div>
                    `}else if(v==="messaging.user.sent_message"){let C="";r.attachments&&r.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${r.attachments.map(T=>{let L=T.content_type&&T.content_type.startsWith("image/"),H=(T.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${L?`<div class="attachment-thumb-container"><img src="${T.url}" alt="${T.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${T.url}" target="_blank" class="attachment-link">${T.filename}</a>
                                        <span class="attachment-meta">${T.content_type} \u2022 ${H}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${p} ${u} ${x}`,O.dataset.eventId=b.id,O.onclick=function(S){if(!o)return;let C=this;if(C.classList.contains("expanded")){C.classList.remove("expanded"),ye.delete(b.id);let T=C.querySelector(".event-details");T&&(T.style.display="none")}else{C.classList.add("expanded"),ye.add(b.id);let T=C.querySelector(".event-details");T&&(T.style.display="block")}},O.innerHTML=`
                <div class="event-time">
            ...
            `;let q=O.querySelector(".event-details");if(q&&q.addEventListener("click",S=>{S.stopPropagation()}),o){let S=O.querySelector(".close-details-btn");S&&S.addEventListener("click",C=>{C.stopPropagation();let h=C.target.closest(".event-item");if(h){h.classList.remove("expanded"),ye.delete(b.id);let T=h.querySelector(".event-details");T&&(T.style.display="none")}})}let G=O.querySelector(".prev-btn"),J=O.querySelector(".next-btn");if(G&&J){let S=0,C=Array.from(O.querySelectorAll(".history-slide")),h=C.length,T=()=>{C.forEach((L,H)=>{L.style.display=H===S?"block":"none"}),G.disabled=S===0,J.disabled=S===h-1,G.style.opacity=S===0?"0.5":"1",J.style.opacity=S===h-1?"0.5":"1"};G.addEventListener("click",L=>{L.stopPropagation(),S>0&&(S--,T())}),J.addEventListener("click",L=>{L.stopPropagation(),S<h-1&&(S++,T())}),T()}return O},c=Array.from(e.children),d=new Map(c.map(b=>[b.dataset.eventId,b])),y=new Set(Le.map(b=>b.id));c.forEach(b=>{let r=b.dataset.eventId;(!r||!y.has(r))&&b.remove()});let m=null;Le.forEach((b,r)=>{let v=d.get(b.id);(!v||t)&&(v&&v.remove(),v=f(b),!v)||(r===0?e.firstElementChild!==v&&e.prepend(v):m&&m.nextElementSibling!==v&&m.after(v),m=v)}),Oe=Date.now(),ee(1,Oe)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=N("offline","Failed to load events.","The event service may be offline."))}}function Ps(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),s=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Le.forEach(n=>ye.add(n.id)),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),de(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{s.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),n.classList.add("active"),K=n.dataset.filter||"all",de(!0)}}),s.dataset.listenerAttached="true"),s&&s.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===K?n.classList.add("active"):n.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let n=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(K!=="all"?l+=`?category=${K}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await _(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ye.clear(),de(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var Rs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Ot=Rs;function jt(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Os=null;async function Ae(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let s=await e.json();if(!s||s.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],n=s.filter(a=>!i.includes(a.id));n.forEach(a=>{a.logs&&Array.isArray(a.logs)?a.logs.reverse():a.logs=[]}),n.reverse();let l=n.map(a=>{let f=a.logs.join(`
`),c=[...a.logs];if(c.length<25){let y=25-c.length;for(let m=0;m<y;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let d=c.map(y=>He(y)).join(`
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${a.id}" title="Clear Logs" style="${B()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(a=>{a.addEventListener("click",()=>{let f=a,c=unescape(f.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let d=f.querySelector("i");d&&(d.classList.remove("bx-copy"),d.classList.add("bx-check"),setTimeout(()=>{d.classList.remove("bx-check"),d.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(a=>{a.addEventListener("click",async()=>{let c=a.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await _(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&Ae()}catch(d){console.error("Error clearing logs:",d)}})}),Os=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Jt=()=>{let t=B()?"display: none;":"";return`
        ...
        <div id="processes-history-widgets" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 30px; opacity: 0.8;"></div>`,Gt=()=>`
        <div class="system-section-header">
        ...
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Kt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${jt()}
        </div>`;async function Yt(){js(),await Promise.all([oe(),Xe(),Qe()]),await Ae()}var je=null;function js(){je||(je=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(je),je=null;return}oe(!0)},1e3))}var Ut=null,qt=null;async function Xt(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function Ft(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Us(){try{return await(await _("/processes")).json()}catch{return null}}async function qs(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Xe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),a=document.querySelector("#hw-storage .hw-content"),f=o=>{if(o){if(s&&(s.innerHTML=`
                ...
                </div>`),n){let p=(o.MEMORY_BYTES/1073741824).toFixed(1);n.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <span style="font-size: 0.9em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">Total System Memory</span>
                    <span style="font-size: 1.8em; font-weight: bold; color: #fff;">${p} <span style="font-size: 0.5em; color: #888; font-weight: normal; margin-left: 2px;">GB</span></span>
                </div>`}if(i&&o.CPU&&o.CPU.length>0){let p=o.CPU[0];i.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    <span style="font-size: 1.1em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">${p.LABEL}</span>
                    <div style="display: flex; gap: 30px;">
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #03dac6; line-height: 1;">${p.COUNT}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Cores</span></div>
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #bb86fc; line-height: 1;">${p.THREADS}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Threads</span></div>
                    </div>
                </div>`}l&&(o.GPU&&o.GPU.length>0?l.innerHTML=o.GPU.map(p=>{let u=(p.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${p.LABEL}</span>
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),a&&o.STORAGE&&o.STORAGE.length>0?a.innerHTML=o.STORAGE.map(p=>{let u=(p.USED/1073741824).toFixed(1),g=(p.SIZE/(1024*1024*1024)).toFixed(1),x=p.SIZE>0?(p.USED/p.SIZE*100).toFixed(0):0,M=p.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                ...
                                <span style="font-weight: 600; color: #fff; font-size: 0.95em;">${p.DEVICE}</span>
                            </div>
                    ...
                    </div>`}).join(""):a&&(a.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await Ft();o?(f(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let o=await Ft();f(o)}if(!t)return;let c=await Xt();if(!c||!c.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let p=o.querySelector(".service-widget-body");p&&(p.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Ut=B()&&ie.lastDashboard||Date.now(),ee(0,Ut||0);let d=c.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let g=59-new Date().getSeconds();g<=0&&(g+=60),o.textContent=`${g}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function y(o){if(!o||o==="N/A"||o==="unknown")return"-";let p=o.match(/^(\d+\.\d+\.\d+)/);return p?p[0]:o.split(".").slice(0,3).join(".")||"-"}function m(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function b(o){if(!o||o==="N/A"||o==="unknown")return"-";let p=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!p)return"-";let u=parseInt(p[1])||0,g=parseInt(p[2])||0,x=parseInt(p[3])||0,M=parseFloat(p[4])||0,A=u*86400+g*3600+x*60+M,I=Math.floor(A/86400);if(I>0)return`${I}d`;let k=Math.floor(A/3600);if(k>0)return`${k}h`;let E=Math.floor(A/60);return E>0?`${E}m`:`${Math.floor(A)}s`}function r(o){if(o.id==="upstash-redis-ro"){let J=B()&&(ie.lastFrontend||ie.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                <div class="service-widget-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 0;">
                    ...
                    <div class="sync-time-label" style="font-size: 0.6em; color: #444; margin-top: 10px;">Last synced: ${new Date(J).toLocaleTimeString()}</div>
                </div>
            </div>`}let p=o.status==="online",u=p?"service-widget-online":"service-widget-offline",g=p?"bx-check-circle":"bx-x-circle",x=p?"OK":"BAD",M=o.version?y(o.version.str):"-",A=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",I=o.memory&&o.memory!=="N/A"?o.memory:"-",k=A!=="-"?"#00ff00":"#666",E=A!=="-"?"#fff":"#666",D=I!=="-"?"#00ff00":"#666",j=I!=="-"?"#fff":"#666",O=b(o.uptime),q="";p?q=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${E};">${A}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${D};"></i>
                        <span style="color: ${j};">${I}</span>
                    </div>
                </div>`:q='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let G=B()?"easter.company":m(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${u}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${G}</span></div>${q}</div></div>`}let v=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),$=new Set(d.map(o=>o.id));for(let[o,p]of v)$.has(o)||p.remove();let w=new Set;d.forEach(o=>{if(w.has(o.id))return;w.add(o.id);let p=r(o),u=v.get(o.id);if(u&&u.parentNode)if(o.id==="upstash-redis-ro"){let g=B()&&(ie.lastFrontend||ie.lastDashboard)||Date.now(),x=new Date(g).toLocaleTimeString(),M=u.querySelector(".sync-time-label");M&&(M.textContent=`Last synced: ${x}`)}else u.outerHTML!==p&&(u.outerHTML=p);else t.insertAdjacentHTML("beforeend",p)})}async function Qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Xt();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}qt=Date.now(),ee(2,qt);let s=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function n(c){let d=c.status==="Ready";return`
                ...
                </div>`}function l(c){let d=c.status==="Ready";return`
                ...
                </div>`}function a(c){let d=c.status==="Downloaded",y=d?"service-widget-online":"service-widget-offline",m=d?"OK":"MISSING",b=d&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",r=c.name;return c.type==="custom"&&r.endsWith(":latest")&&(r=r.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${d?"bx-check-circle":"bx-x-circle"}"></i><h3>${r}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let f="";if(i&&(f+=n(i)),e.xtts&&(f+=l(e.xtts)),f+=s.map(a).join(""),!f){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function oe(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let s=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),a=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),d=document.getElementById("fabricator-reset-btn"),y=document.getElementById("system-pause-toggle-btn");y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{let u=y.querySelector(".bx-play")?"/agent/resume":"/agent/pause";y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _(u,{method:"POST"}),await oe()}catch{y.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>oe(),2e3)}},y.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{d.innerHTML="<i class='bx bx-error'></i>"}},d.dataset.listenerAttached="true");let m=await qs();if(m&&m.agents){let p=m.agents.guardian||{protocols:{}},u=m.agents.analyzer||{protocols:{}},g=m.agents.imaginator||{protocols:{}},x=m.agents.fabricator||{protocols:{}},M=m.system||{},A=Math.floor(Date.now()/1e3),I={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction"},k=h=>{h<0&&(h=0);let T=Math.floor(h/3600),L=Math.floor(h%3600/60),H=h%60;return T>0?`${T}h ${L}m`:L>0?`${L}m ${H}s`:`${H}s`},E=(h,T,L,H)=>{if(!h)return;let P=I[H]||H.toUpperCase(),V=h.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(V&&(V.textContent=P),!L){h.textContent="Inactive",h.style.color="#666";return}let U=L.status;if(U==="Working")h.textContent="Working",h.style.color="#bb86fc";else if(U==="Ready")h.textContent="Ready",h.style.color="#5eff5e";else{let F=L.cooldown;if(B()&&(F=L.next_run-A),F<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let W=Math.floor(F/60),re=F%60;h.textContent=`${W}m ${re}s`,h.style.color="#fff"}}T&&L.stats&&(T.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
              <span>Runs: ${L.stats.runs||0}</span>
              <span style="color: ${L.stats.failures>0?"#ffa500":"#666"}">Fails: ${L.stats.failures||0}</span>
              <span style="color: ${L.stats.aborted>0?"#ff4d4d":"#666"}">Aborted: ${L.stats.aborted||0}</span>
            </div>
          `)};s&&E(s,document.getElementById("guardian-sentry-stats"),p.protocols.sentry,"sentry");let D=document.getElementById("analyzer-synthesis-val"),j=document.getElementById("analyzer-synthesis-stats");D&&E(D,j,u.protocols.synthesis,"synthesis");let O=document.getElementById("imaginator-alert_review-val"),q=document.getElementById("imaginator-alert_review-stats");O&&E(O,q,g.protocols.alert_review,"alert_review");let G=document.getElementById("fabricator-construction-val"),J=document.getElementById("fabricator-construction-stats");G&&E(G,J,x.protocols.construction,"construction");let S=document.getElementById("system-state-label"),C=document.getElementById("system-state-val");if(C&&M.state){let h=M.state,T=k(M.state_time||0);S&&(S.textContent=`State: ${h.toUpperCase()}`),C.textContent=T,h==="idle"?C.style.color=M.state_time>300?"#5eff5e":"#fff":C.style.color="#bb86fc",y&&(h==="paused"?(y.innerHTML="<i class='bx bx-play'></i>",y.title="Resume System",y.style.color="#ff9800"):(y.innerHTML="<i class='bx bx-pause'></i>",y.title="Pause System",y.style.color=""))}n&&(n.textContent=k(M.metrics?.total_idle_time||0)),l&&(l.textContent=k(M.metrics?.total_active_time||0)),a&&(a.textContent=k(M.metrics?.total_waste_time||0))}else[s,i,n,l,a].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});if(t)return;let b=await Us(),r=[],v=[],$=[];b&&(Array.isArray(b)?r=b:(r=b.active||[],v=b.queue||[],$=b.history||[],$.sort((p,u)=>(u.end_time||0)-(p.end_time||0)))),e&&(r.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=N("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),Ye(e,r,!1)));let w=document.getElementById("processes-queue-widgets");w&&(v.length===0?!w.querySelector(".tab-placeholder")&&!w.querySelector('div[style*="font-style: italic"]')&&(w.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(w.innerHTML="",Ye(w,v,!1)));let o=document.getElementById("processes-history-widgets");o&&($.length===0?o.querySelector(".tab-placeholder")||(o.innerHTML=N("empty","No recent history.")):(o.querySelector(".tab-placeholder")&&(o.innerHTML=""),Ye(o,$,!0))),Me(1,r.length)}function Ye(t,e,s){function i(c){let d="";c.end_time?d=`${c.end_time-c.start_time}s`:d=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let y=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[m]?m=b[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),s)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
        ...
        </div>`;let r="#fff";return`
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}-${c.start_time}" style="">
                    <div class="service-widget-header">
                        <i class="bx bx-cog" style="color: ${r}"></i>
                        <h3 style="color: ${r}">${m}</h3>
                        ${y}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">State:</span>
                            <span class="info-value" style="color: ${r}">${c.state}</span>
                        </div>
                        <div class="service-widget-info">
                            <span class="info-label">Duration:</span>
                            <span class="info-value" style="color: ${r}">${d}</span>
                        </div>
                         <div class="service-widget-info">
                            <span class="info-label">PID:</span>
                            <span class="info-value" style="color: ${r}">${c.pid}</span>
                        </div>
                    </div>
                </div>`}let n=s?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(n)).map(c=>[c.dataset.channelId,c])),a=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,d]of l)c&&!a.has(c)&&d.remove();let f=new Set;e.forEach(c=>{let d=`${c.channel_id}-${c.start_time}`;if(f.has(d))return;f.add(d);let y=i(c),m=l.get(d);if(m&&m.parentNode){m.outerHTML!==y&&(m.outerHTML=y);let b=t.querySelector(`[data-channel-id="${d}"]`);b&&t.appendChild(b)}else t.insertAdjacentHTML("beforeend",y)})}function Ue(){let t=Fe(),e="Notification"in window,s={enabled:e&&Notification.permission==="granted",supported:e};return`
            ...
            </div>`}function et(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let n=this.dataset.theme;n&&(dt(n),t.setContent(Ue()),et(t))})});let s=document.getElementById("notifications-toggle");s&&"Notification"in window?s.onclick=async i=>{let n=i.target;if(n.checked)try{await Notification.requestPermission()!=="granted"&&(n.checked=!1)}catch{n.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),n.checked=!0)}:s&&(s.disabled=!0)}var es=()=>`
    ...
    `;async function ts(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Qt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Qt(t)}async function Qt(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=N("empty","No web history found.");return}let i=`
            <div style="display: flex; flex-direction: column; height: 100%;">
        ...
        `;s.forEach((f,c)=>{let d=new Date(f.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=f.screenshot?`<img src="data:image/png;base64,${f.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Fs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,i]of Object.entries(t)){let n=is.filter(l=>l.category===s);n.length!==0&&(e+=`
    ...
    </div>`,e},tt=!1;function Js(t){let e=document.getElementById("cli-terminal-overlay");if(!e){e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>ss());let d=document.getElementById("terminal-action-btn");d&&(d.onclick=()=>ss())}let s=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");s&&(s.innerHTML="");let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-running",a.textContent="Running");let f=document.getElementById("terminal-action-btn");return f&&(f.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",tt=!0,s}function ss(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",tt=!1}function ns(t,e,s="output"){if(!tt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${s}`,s==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let n=He(e);i.innerHTML=n||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ws(t){let e=is.find(a=>a.id===t);if(!e)return;let s=Js(e);s&&ns(s,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let a of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let f="output";a.includes("[ERROR]")?f="error":a.includes("[SUCCESS]")||a.includes("\u2713")?f="success":a.includes("!")&&(f="warning"),s&&ns(s,a,f)}let n=document.getElementById("terminal-status-badge");n&&(n.className="terminal-status status-success",n.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function as(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Fs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let s=e.querySelector("code"),i=s?s.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let n=e.querySelector("i");n&&(n.className="bx bx-check",n.style.color="#5eff5e",setTimeout(()=>{n.className="bx bx-copy",n.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(s=>{let i=s;s.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),s.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),s.addEventListener("click",()=>{let n=i.dataset.cmd;n&&Ws(n)})})}async function Gs(){if(!B())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function os(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}ot(),rt(),Gs(),mt(),at();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&as();let e=[],s=document.getElementById("windows-container");s&&s.setAttribute("data-count","0");let i=o=>{localStorage.setItem("dex_last_window",o)};function n(){for(;e.length>1;)e.shift().close(!0);let o=document.getElementById("windows-container"),p=document.querySelector("nav"),u=document.querySelector("footer"),g=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");o&&o.setAttribute("data-count",e.length.toString()),e.forEach(D=>{let j=document.getElementById(D.id);j&&j.classList.remove("hide-close")});let M=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon"),k=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let D=document.getElementById("dexter-dropdown"),j=document.getElementById("dexter-menu-btn");D&&D.classList.remove("active"),j&&j.classList.remove("active"),o&&o.classList.remove("menu-open")}if(xe(e.length>0),e.length>0)if(u?.classList.add("hide"),k&&(k.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",p?.classList.add("window-open"),o&&(o.style.paddingTop="60px"),M&&(M.style.display=E?"flex":"none"),I&&(I.style.display=E?"block":"none"),!E&&A){let D=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(D)?(A.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(f)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(y)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(d)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(m)})):A.innerHTML=""}else A&&(A.innerHTML="");else p?.classList.remove("window-open"),k&&(k.style.display="none"),o&&(o.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",g||x?u?.classList.remove("hide"):u?.classList.add("hide"),M&&(M.style.display="flex"),I&&(I.style.display="block"),A&&(A.innerHTML="");Ce()}function l(o){if(o.isOpen()){o.close();return}for(;e.length>0;)e.pop().close(!0);e.push(o),o.open(),n()}function a(){[...e].forEach(p=>p.close()),e=[]}window.addEventListener("resize",n);let f=pe({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:xt(),onOpen:()=>ce(),onClose:()=>{let o=e.indexOf(f);o>-1&&e.splice(o,1),n()}}),c=pe({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Ze(),onOpen:()=>{c.setContent(Ze()),de()},onClose:()=>{let o=e.indexOf(c);o>-1&&e.splice(o,1),n()}}),d=pe({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Pt(),onOpen:()=>Ke(),onClose:()=>{let o=e.indexOf(d);o>-1&&e.splice(o,1),n()}}),y=pe({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Gt()},{icon:"bxs-component",title:"Processes",content:Wt()},{icon:"bxs-brain",title:"Models",content:Vt()},{icon:"bx-globe",title:"Web",content:es()},{icon:"bxs-hdd",title:"Hardware",content:Kt()},{icon:"bxs-terminal",title:"Logs",content:Zt()},{icon:"bxs-zap",title:"Agents",content:Jt()}].filter(o=>B()?o.title!=="Hardware"&&o.title!=="Logs":!0),onOpen:()=>{Xe(),oe(),Qe(),ts(),Ae()},onClose:()=>{let o=e.indexOf(y);o>-1&&e.splice(o,1),n()}}),m=pe({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Bt()},{icon:"bx-map-alt",title:"Roadmap",content:_t()},{icon:"bx-paint",title:"Blueprints",content:Ht()},{icon:"bx-task",title:"Tasks",content:Dt()}],onOpen:()=>Ge(),onClose:()=>{let o=e.indexOf(m);o>-1&&e.splice(o,1),n()}}),b=pe({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ue(),onOpen:()=>{b.setContent(Ue()),et(b)},onClose:()=>{let o=e.indexOf(b);o>-1&&e.splice(o,1),n()}});window.dexter={viewEvent:o=>{c.isOpen()||l(c),setTimeout(()=>{let p=document.querySelector(`.event-item[data-event-id="${o}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)},viewAlert:o=>{f.isOpen()||l(f),setTimeout(()=>{let p=document.querySelector(`.event-item[data-alert-id="${o}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)}};let r=()=>{let o=document.getElementById("dexter-dropdown"),p=document.getElementById("dexter-menu-btn");o&&p&&(o.classList.remove("active"),p.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let o=document.getElementById("dexter-menu-btn"),p=document.getElementById("settings-icon"),u=document.getElementById("close-all-windows"),g=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),M=document.getElementById("windows-container");o&&x&&(o.onclick=I=>{I.stopPropagation();let k=window.innerWidth<880;if(k)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let D=o.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),q=220,G=10,S=D.left+D.width/2-q/2,C=O.right-10;S+q>C&&(S=C-q),S<10&&(S=10),x.style.top=O.bottom+G+"px",x.style.left=S+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),o.classList.toggle("active");let E=x.classList.contains("active");if(k){let D=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),D?.classList.add("window-open"),M?.classList.add("menu-open"),xe(!0)):(M?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),D?.classList.remove("window-open"),xe(!1)))}}),p&&(p.onclick=I=>{I.stopPropagation(),l(b)}),u&&(u.onclick=I=>{I.stopPropagation(),a()});let A=(I,k,E)=>{let D=document.getElementById(I);D&&(D.onclick=j=>{j.stopPropagation(),r(),E&&i(E),l(k)})};A("alerts-menu-item",f,"alerts-window"),A("events-menu-item",c,"events-window"),A("monitor-menu-item",y,"monitor-window"),A("contacts-menu-item",d,"contacts-window"),A("workspace-menu-item",m,"workspace-window"),g&&(g.onclick=I=>{if(I.stopPropagation(),x&&x.classList.contains("active")){r(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),xe(!1));return}if(e.length>0)a();else{let k=window.location.pathname;if(!(k==="/"||k==="/index.html")){let j=(k.endsWith("/")&&k.length>1?k.slice(0,-1):k).split("/");j.pop();let O=j.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let I=window.innerWidth<880;x&&x.classList.contains("active")&&(r(),I&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),xe(!1)))}),x&&(x.onclick=I=>I.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!w&&document.querySelector("footer")?.classList.add("hide"),B()||setInterval(()=>{c.isOpen()&&de(),f.isOpen()?ce():We()},1e3),setInterval(()=>{B()&&(f.isOpen()?ce():We()),m.isOpen()?Ge():St(),B()&&c.isOpen()&&de(),y.isOpen()&&Yt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",os):os();})();

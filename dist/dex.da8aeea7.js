"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function me(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++ut).toString())}function l(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",o),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",a);let y=t.icon||"bx-window",s="",u="",T,H='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((h,v)=>{let r=h.icon?`<i class="bx ${h.icon}"></i>`:`<span class="tab-glyph">${h.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,T=`<div class="window-content">${t.tabs.map((h,v)=>`<div class="tab-content ${v===0?"active":""}" data-tab-content="${v}">${h.content}${H}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),T=`<div class="window-content">${t.content||""}${H}</div>`),e.innerHTML=`
...
`,je=null,ge=new Set,Ue=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];je=Date.now(),te(0,je);let o=Date.now(),f=24*60*60*1e3,c=l.filter(p=>{let h=localStorage.getItem(`alert_read_ts_${p.id}`);if(!h)return!0;let v=parseInt(h);return o-v<f});c.sort((p,h)=>{let v=w=>{let g=w.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},r=w=>w==="critical"?4:w==="high"?3:w==="medium"?2:1,d=r(v(p)),C=r(v(h));return d!==C?C-d:h.timestamp-p.timestamp}),Ue=c;let x=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},E=[];if(c.length===0)E.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let h=new Set(c.map(r=>x(r))).size>1,v=null;c.forEach(r=>{let d=x(r);h&&d!==v&&(E.push({id:`divider-${d}`,type:"divider",label:d.toUpperCase()}),v=d),E.push(r)})}t&&(e.innerHTML="");let m=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let v=(h.title||"Untitled Alert").trim(),r=(h.body||"No description provided.").trim(),d=h.summary||"",C=h.content||"",w=h.protocol||"",g=(h.priority||"low").toLowerCase(),S=!!h.alert,M=(h.category||"system").trim(),A=h.related_event_ids||[],R=h.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${p.id}`),V=new Date(p.timestamp*1e3),I=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=J?"event-border-grey":"event-border-blue";!J&&S&&(L="event-border-red"),J&&(g==="high"||g==="critical")?L="event-border-red":J&&g==="medium"&&(L="event-border-orange");let B=J?"alert-read":"alert-unread",O=ge.has(p.id),P=O?"expanded":"",G=O?"display: block;":"display: none;",k="",_="";A.length>0&&(_=`
...
`,fe=new Set,St=[],Me=null;async function be(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await D("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();St=i;let a=m=>{let y=fe.has(m.id),s=m.state==="published",u=m.state==="consumed",T="event-border-grey";s&&(T="event-border-blue"),u&&(T="event-border-purple");let b=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),p=document.createElement("div");p.className=`event-item notification-item ${T} cursor-pointer ${y?"expanded":""}`,p.dataset.itemId=m.id,p.onclick=r=>{if(r.target.closest("button"))return;if(p.classList.contains("expanded")){p.classList.remove("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="none"),fe.delete(m.id)}else{p.classList.add("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="block"),fe.add(m.id)}};let h=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;p.innerHTML=`
        <div class="event-time">
      ...
      `;let v=p.querySelector(".event-details");return v&&v.addEventListener("click",r=>{r.stopPropagation()}),p.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(m)),p.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(m)),p.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(m.id)),p.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),p.classList.remove("expanded");let d=p.querySelector(".event-details");d&&(d.style.display="none"),fe.delete(m.id)}),p},l=m=>{let y=document.createElement("div");y.innerHTML=F("empty",m.message,m.action,m.id);let s=y.firstElementChild;return s.dataset.itemId=m.id,s},o=[];!i||i.length===0?o.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:N()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(m=>o.push({...m,type:"item"}));let f=Array.from(e.children),c=new Map(f.map(m=>[m.dataset.itemId,m])),x=new Set(o.map(m=>m.id));f.forEach(m=>{let y=m.dataset.itemId;(!y||!x.has(y))&&m.remove()}),t&&(e.innerHTML="");let E=null;o.forEach((m,y)=>{let s=c.get(m.id);(!s||t)&&(s&&s.remove(),m.type==="placeholder"?s=l(m):s=a(m),!s)||(y===0?e.firstElementChild!==s&&e.prepend(s):E&&E.nextElementSibling!==s&&E.after(s),E=s)})}catch{e.children.length===0&&(e.innerHTML=F("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Me=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),o=l?l.value:"";if(!o.trim())return;let f=Me?`/roadmap/${Me}`:"/roadmap",c=Me?"PATCH":"POST";try{await D(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})});let x=document.getElementById("roadmap-editor-container");x&&(x.style.display="none"),be(!0)}catch(x){console.error(x)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{St.forEach(l=>fe.add(l.id)),be(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{fe.clear(),be(!0)},a.dataset.listenerAttached="true")}function Ss(t){Me=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await D(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),be(!0)}catch(n){console.error(n)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await D(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),be(!0)}catch(e){console.error(e)}}var Lt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Mt=null,ve=new Set,It=[];async function ye(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];It=l,Mt=Date.now(),te(2,Mt);let o=s=>{let u=s.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let T=(u.title||"Untitled Blueprint").trim(),H=(u.summary||u.body||"No summary provided.").trim(),b=(u.content||"").trim(),p=(u.category||"architecture").trim(),h=(u.related_services||u.affected_services||[]).map(k=>k.trim()),v=(u.implementation_path||[]).map(k=>k.trim()),r=u.source_event_ids||[],d=u.approved===!0,C=new Date(s.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=ve.has(s.id),M=S?"display: block;":"display: none;",A=document.createElement("div");A.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${d?"blueprint-approved":""}`,A.dataset.blueprintId=s.id,d&&(A.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",A.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=d?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[p]||"bx-paint";A.onclick=function(k){let _=this;if(_.classList.contains("expanded")){_.classList.remove("expanded"),ve.delete(s.id);let U=_.querySelector(".event-details");U&&(U.style.display="none")}else{_.classList.add("expanded"),ve.add(s.id);let U=_.querySelector(".event-details");U&&(U.style.display="block")}};let J="";v.length>0&&(J=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${v.map(k=>`<li style="margin-bottom: 5px;">${j(k)}</li>`).join("")}</ul></div>
                    </div>
                `);let V="";r.length>0&&(V=`
                ...
                `);let I=h.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${h.join(", ")}</span></div>`:"<div></div>",$=N(),L=d?`
            ...
            `;A.innerHTML=`
                ...
                <div class="event-icon" style="${d?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
            ...
            `;let B=A.querySelector(".blueprint-approve-btn");B&&(B.onclick=async k=>{k.stopPropagation(),B.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await D(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ye(!0)}catch(_){console.error("Failed to approve blueprint:",_)}});let O=A.querySelector(".blueprint-delete-btn");O&&(O.onclick=async k=>{k.stopPropagation();let _=!d;O.innerHTML=_?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await D(`/events/${s.id}`,{method:"DELETE"})).ok&&ye(!0)}catch(z){console.error("Failed to delete blueprint:",z)}});let P=A.querySelector(".event-details");P&&P.addEventListener("click",k=>{k.stopPropagation()});let G=A.querySelector(".close-details-btn");return G&&G.addEventListener("click",k=>{k.stopPropagation(),A.classList.remove("expanded");let _=A.querySelector(".event-details");_&&(_.style.display="none"),ve.delete(s.id)}),A},f=s=>{let u=document.createElement("div");u.innerHTML=F("empty",s.message,s.action,s.id);let T=u.firstElementChild;return T.dataset.blueprintId=s.id,T},c=[];l.length===0?c.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):l.forEach(s=>c.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let x=Array.from(e.children),E=new Map(x.map(s=>[s.dataset.blueprintId,s])),m=new Set(c.map(s=>s.id));x.forEach(s=>{let u=s.dataset.blueprintId;(!u||!m.has(u))&&s.remove()});let y=null;c.forEach((s,u)=>{let T=E.get(s.id);(!T||t)&&(T&&T.remove(),s.type==="placeholder"?T=f(s):T=o(s),!T)||(u===0?e.firstElementChild!==T&&e.prepend(T):y&&y.nextElementSibling!==T&&y.after(T),y=T)}),Pe(2,l.length),wt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=F("offline","Failed to load blueprints.","The event service may be offline."))}}async function Bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await D(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&a++}),ht(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{It.forEach(i=>ve.add(i.id)),ye(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ve.clear(),ye(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await D("/events?type=system.blueprint.generated",{method:"DELETE"}),ve.clear(),ye(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var le="STANDBY",Ht=null,Le=[],X=null,qe=null,_t=()=>`
...
`,Nt=()=>`
    ...
    `,Be=[],He={"313071000877137920":"Owen",dexter:"Dexter"},se=[],xe=null;async function _e(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),o=document.getElementById("selected-recipients"),f=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),x=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),m=document.getElementById("new-chore-time"),y=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(y&&!y.dataset.initialValueAttached){let d=Intl.DateTimeFormat().resolvedOptions().timeZone,C=!1;for(let w=0;w<y.options.length;w++)if(y.options[w].value===d){C=!0;break}if(!C){let w=document.createElement("option");w.value=d,w.textContent=d.replace("_"," "),y.appendChild(w)}y.value=d,y.dataset.initialValueAttached="true"}E&&s&&!E.dataset.timeListenerAttached&&(E.addEventListener("change",()=>{s.style.display=E.value==="daily"?"block":"none"}),E.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),T=document.getElementById("form-icon");function H(){if(o){if(se.length===0){o.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}o.innerHTML=se.sort((d,C)=>{let w=d.startsWith("channel:"),g=C.startsWith("channel:");return w&&!g?-1:!w&&g?1:0}).map(d=>{let C=He[d]||d,w=d.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${d==="dexter"?"#bb86fc":w?"#03dac6":"#fff"};">
          <i class='bx ${d==="dexter"?"bx-terminal":w?"bx-hash":"bx-user"}'></i>
          <span>${C}</span>
          <i class='bx bx-x remove-recipient' data-id="${d}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),o.querySelectorAll(".remove-recipient").forEach(d=>{d.onclick=()=>{let C=d.dataset.id;se=se.filter(w=>w!==C),H()}})}}function b(d){d&&!se.includes(d)&&(se.push(d),H()),l&&(l.value="")}function p(d=null){if(n){if(xe=d?d.id:null,d){if(u&&(u.textContent="Update Research Task"),T&&(T.className="bx bx-edit-alt",T.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=d.natural_instruction),x&&(x.value=d.execution_plan?.entry_url||""),E&&(E.value=d.schedule,s&&(s.style.display=d.schedule==="daily"?"block":"none")),m&&d.run_at&&(m.value=d.run_at),y&&d.timezone){let C=!1;for(let w=0;w<y.options.length;w++)if(y.options[w].value===d.timezone){C=!0;break}if(!C){let w=document.createElement("option");w.value=d.timezone,w.textContent=d.timezone.replace("_"," "),y.appendChild(w)}y.value=d.timezone}se=d.recipients||(d.owner_id?[d.owner_id]:[]),H()}else u&&(u.textContent="Initialize Research Task"),T&&(T.className="bx bx-plus-circle",T.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),x&&(x.value=""),E&&(E.value="every_24h"),se=[],H();n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"})}}let h=document.getElementById("contacts-group"),v=document.getElementById("channels-group");if(h&&!h.dataset.populated&&!N())try{ue("/contacts").then(async d=>{d.ok&&(((await d.json()).members||[]).forEach(g=>{if(He[g.id]=g.nickname||g.username,g.id==="313071000877137920")return;let S=document.createElement("option");S.value=g.id;let M=g.nickname||g.username;S.textContent=`${M} (${g.username})`,h.appendChild(S)}),h.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(v&&!v.dataset.populated&&!N())try{ue("/channels").then(async d=>{d.ok&&((await d.json()).forEach(w=>{let g=`channel:${w.id}`;He[g]=w.name;let S=document.createElement("option");S.value=g,S.textContent=`#${w.name} (${w.guild})`,v.appendChild(S)}),v.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{b(l.value)},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onchange=()=>r(),f.dataset.listenerAttached="true");function r(){if(!t)return;let d=f?.value||"all",C=d==="all"?Be:Be.filter(g=>(g.recipients||(g.owner_id?[g.owner_id]:[])).includes(d));if(C.length===0){t.innerHTML=F("empty",d==="all"?"No active tasks.":"No tasks found for this owner.",N()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let w=C.map(g=>{let S=g.last_run===0?"Never":new Date(g.last_run*1e3).toLocaleString(),M=g.memory?g.memory.length:0,A=g.status==="active"?"#03dac6":"#666",W=(g.recipients||(g.owner_id?[g.owner_id]:[])).sort((J,V)=>{let I=J.startsWith("channel:"),$=V.startsWith("channel:");return I&&!$?-1:!I&&$?1:0}).map(J=>{let V=He[J]||J.substring(0,8),I=J.startsWith("channel:");return`<span title="${V}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${J==="dexter"?"bx-terminal":I?"bx-hash":"bx-user"}'></i>${V}</span>`}).join("<span style='color: #444;'>, </span>");return`
            ...
            `}).join("");t.innerHTML=w,t.querySelectorAll(".edit-chore-btn").forEach(g=>{g.onclick=S=>{S.stopPropagation();let M=g.dataset.id,A=Be.find(R=>R.id===M);A&&p(A)}}),t.querySelectorAll(".reset-chore-btn").forEach(g=>{g.onclick=async S=>{S.stopPropagation();let M=g.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(g.innerHTML="<i class='bx bx-loader-alt spin'></i>",await D(`/chores/${M}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),_e())}}),t.querySelectorAll(".delete-chore-btn").forEach(g=>{g.onclick=async S=>{S.stopPropagation();let M=g.dataset.id;confirm("Delete this task?")&&(await D(`/chores/${M}`,{method:"DELETE"}),_e())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||xe!==null?p(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let d=document.getElementById("new-chore-instruction"),C=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),g=d?.value,S=w?.value||"every_24h",M=S==="daily"?m?.value:"",A=S==="daily"?y?.value:"";if(!g)return;if(se.length===0){alert("Please add at least one recipient.");return}let R=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=xe?"PATCH":"POST",J=xe?`/chores/${xe}`:"/chores";await D(J,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:g,entry_url:C?.value,schedule:S,run_at:M,timezone:A})}),n&&(n.style.display="none"),d&&(d.value=""),C&&(C.value=""),xe=null,se=[],_e()}catch(W){console.error(W),alert(xe?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=R}},i.dataset.listenerAttached="true");try{let d=await D("/chores");if(!d.ok)throw new Error(`HTTP error! status: ${d.status}`);if(Be=await d.json()||[],f){let w=f.value,g=new Set;Be.forEach(S=>{S.owner_id&&g.add(S.owner_id),S.recipients&&S.recipients.forEach(M=>g.add(M))}),f.innerHTML='<option value="all">All Recipients</option>',g.forEach(S=>{let M=document.createElement("option");M.value=S;let A=S.startsWith("channel:"),R=He[S]||`Recipient: ${S.substring(0,8)}`;M.textContent=(A&&!R.startsWith("#")?"#":"")+R,f.appendChild(M)}),f.value=w,f.selectedIndex===-1&&(f.value="all")}r()}catch(d){console.error("Failed to fetch chores",d)}}var Rt=()=>`
...
`;function Ut(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function qt(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",ue(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();tt(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=Ut(t.id,t.username);tt(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=Ut(t.id,t.username);tt(e,t,i)})}function tt(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let T=(n.attributes||n.facts||[]).map(r=>{let d=r.key||r.k,C=r.value||r.v;return`
                    ...
                    <div class="cog-metric">
                     ...
                     <div class="cog-metric">
                        <div class="cog-label">
                            ...
                            <span style="font-family: 'JetBrains Mono'; color: #bb86fc;">${v}</span>
                        </div>
                    </div>
                </div>
        ...
        `},l=()=>{let T=n.dossier||{},H=T.identity||{},b=T.career||{},p=T.personal||{},h=T.social||[];return`
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(b.skills||[]).map(v=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${v}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.hobbies||[]).map(v=>`<div class="dossier-list-item">${v}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.habits||[]).map(v=>`<div class="dossier-list-item">${v}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(p.vices||[]).map(v=>`<div class="dossier-list-item" style="color: #cf6679;">${v}</div>`).join("")}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="dossier-item" style="background: none; border: none; padding: 0;">
                        ${h.length>0?h.map(v=>`
                            <div class="friend-chip">
                                <div style="width: 35px; height: 35px; background: #333; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #888;">${v.name.substring(0,1)}</div>
                                <div style="flex: 1;">
                                    <div style="color: #fff; font-weight: 500;">${v.name}</div>
                                    <div style="font-size: 0.75em; color: #888;">${v.relation}</div>
                                </div>
                                <div style="font-size: 0.8em; color: #03dac6;">${v.trust}</div>
                            </div>
                        `).join(""):'<div style="color: #666; font-style: italic;">No associates mapped.</div>'}
                    </div>
                </div>
            </div>
        `},o=()=>{let T=n.traits||{};return`
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${j(e.username)}</h2>
                    <div class="profile-badges">${(n.badges||[]).map(T=>`<span class="profile-badge ${T==="Creator"?"master":""}">${T}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=x;let E=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),y=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content"),u=t.querySelector(".close-profile-btn");u&&u.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),y.forEach(T=>{T.addEventListener("click",()=>{y.forEach(p=>p.classList.remove("active")),s.forEach(p=>p.classList.remove("active")),T.classList.add("active");let H=T.dataset.tab,b=t.querySelector(`#tab-${H}`);b&&b.classList.add("active")})}),m&&m.addEventListener("click",()=>{E.classList.toggle("expanded");let T=E.classList.contains("expanded");m.innerHTML=T?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Ft=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function st(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await st(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let a=n.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let l=JSON.parse(a.dataset.user);qt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let n=await ue("/contacts");if(!n.ok)throw new Error("Service unreachable");let a=(await n.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=F("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((o,f)=>{let c=l[o.level]??10,x=l[f.level]??10;return c!==x?c-x:o.username.localeCompare(f.username)}),t.innerHTML=a.map(o=>{let f=o.color&&o.color!==0,c=f?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",x=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",E="#888";o.level==="Me"||o.level==="Master"?E="#bb86fc":o.level==="Admin"?E="#cf6679":o.level==="Moderator"?E="#03dac6":o.level==="Contributor"&&(E="#ffa500");let m=o.level==="Me",y=m?"rgba(187, 134, 252, 0.08)":f?`${c}11`:"rgba(255,255,255,0.03)",s=m?"2px solid #bb86fc":f?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div style="position: relative;">
                        <img src="${o.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${m?"2px solid #bb86fc":f?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${f?c:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${o.nickname||o.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${E}; font-weight: 600; text-transform: uppercase;">${m?"DEXTER (ME)":o.level}</span>
...
`,Fe=null,he=new Set,De=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(De=((await a.json()).events||[]).filter(y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Fe=Date.now(),te(1,Fe),De.length===0){e.innerHTML=F("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,T=Os(u),H=js(u),b=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),p="event-border-grey";b&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?p="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?p="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?p="event-border-orange":u==="system.test.completed"?p=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":p="event-border-blue");let h=b?"cursor-pointer":"",v=he.has(y.id),r=v?"expanded":"",d=v?"display: block;":"display: none;",C=new Date(y.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Jt(u,s),M=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",A="";if(b){let I="";if(u==="engagement.decision"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                    ...
                    `}else if(u==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.timestamp-s.last_active,B=Math.floor(L/60);I=`
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="";if(s.embed){let P=s.embed,G=P.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(P.title)}</div>`:"",k=P.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${re(P.description)}</div>`:"",_=(P.fields||[]).map(z=>`
              <div style="margin-top: 10px;">
            ...
            `}let B="";s.eval_count&&(B=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let P=s.chat_history.length,G=s.chat_history.map((k,_)=>{let z=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&z==="ASSISTANT"&&(z="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=_===P-1?"block":"none";return`
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let $=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="",B=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let P=O.match(/{.*?}/g);P&&(B=P.map(G=>{let k=G.substring(1,G.length-1),_=k.indexOf(" "),z=k.substring(0,_),U=k.substring(_+1);return U=U.replace(/\[\]$/,"").trim(),{role:z,content:U}}))}if(B.length>0&&s.response_raw&&B.push({role:"assistant",content:s.response_raw}),B.length>0){let P=B.length,G=B.map((k,_)=>{let z=k.role.toUpperCase();z==="ASSISTANT"&&(z="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=_===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${_}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${z}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${_+1} of ${P}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.signals||{};I=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.visual.completed"){let $=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,L="";s.base64_preview?L=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(L=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="analysis.router.decision"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",B=_=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${_}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${B("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let P="";if(s.chat_history&&s.chat_history.length>0){let _=s.chat_history.length,z=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ce=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let _=s.corrections.map((z,U)=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${L} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${G}
                        ${P}
                    `}else if(u==="system.test.completed"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let $=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(u==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(B=>{let O=B.content_type&&B.content_type.startsWith("image/"),P=(B.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${B.content_type} \u2022 ${P}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let R=document.createElement("div");R.className=`event-item ${p} ${h} ${r}`,R.dataset.eventId=y.id,R.onclick=function(I){if(!b)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),he.delete(y.id);let B=$.querySelector(".event-details");B&&(B.style.display="none")}else{$.classList.add("expanded"),he.add(y.id);let B=$.querySelector(".event-details");B&&(B.style.display="block")}},R.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${g}</span>
                </div>
...
`),c=[...o.logs];if(c.length<25){let E=25-c.length;for(let m=0;m<E;m++)c.push("")}else c.length>25&&(c=c.slice(-25));let x=c.map(E=>Oe(E)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let f=o,c=unescape(f.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let x=f.querySelector("i");x&&(x.classList.remove("bx-copy"),x.classList.add("bx-check"),setTimeout(()=>{x.classList.remove("bx-check"),x.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let c=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await D(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&ze()}catch(x){console.error("Error clearing logs:",x)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=F("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Yt=()=>{let t=N()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Gt()}
                ...
                </div>`),c){let d=(r.MEMORY_BYTES/1073741824).toFixed(1);c.innerHTML=`
                ...
                </div>`}if(f&&r.CPU&&r.CPU.length>0){let d=r.CPU[0];f.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}x&&(r.GPU&&r.GPU.length>0?x.innerHTML=r.GPU.map(d=>{let C=(d.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):x.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),E&&r.STORAGE&&r.STORAGE.length>0?E.innerHTML=r.STORAGE.map(d=>{let C=(d.USED/1073741824).toFixed(1),w=(d.SIZE/(1024*1024*1024)).toFixed(1),g=d.SIZE>0?(d.USED/d.SIZE*100).toFixed(0):0,S=d.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):E&&(E.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let r=await Zt();r?(m(r),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),f&&!f.hasChildNodes())){let r=await Zt();m(r)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async r=>{let d=r.target.closest(".svc-btn");if(!d)return;let C=d,w=C.dataset.action,g=C.dataset.service;if(!w||!g)return;let S=C.innerHTML;C.innerHTML="<i class='bx bx-loader-alt spin'></i>",C.classList.add("loading"),C.disabled=!0;try{await D(`/system/service/${w}`,{method:"POST",body:JSON.stringify({service:g})}),setTimeout(()=>we(),1e3),setTimeout(()=>we(),3e3)}catch(M){console.error(M),alert(`Failed to ${w} ${g}`),C.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{C.innerHTML=S,C.classList.remove("loading"),C.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let y=await is();if(!y||!y.services){t.children.length===0&&(t.innerHTML=F("offline","Failed to load system metrics.","The event service may be offline."));let r=document.querySelector('[data-service-id="upstash-redis-ro"]');if(r){r.className="service-widget service-widget-offline",r.querySelector(".service-widget-status").textContent="ERROR";let d=r.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Vt=N()&&oe.lastDashboard||Date.now(),te(0,Vt||0);let s=y.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let r=document.getElementById("upstash-countdown");if(r){let w=59-new Date().getSeconds();w<=0&&(w+=60),r.textContent=`${w}s`}},1e3)),Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function u(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:r.split(".").slice(0,3).join(".")||"-"}function T(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function H(r){if(!r||r==="N/A"||r==="unknown")return"-";let d=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let C=parseInt(d[1])||0,w=parseInt(d[2])||0,g=parseInt(d[3])||0,S=parseFloat(d[4])||0,M=C*86400+w*3600+g*60+S,A=Math.floor(M/86400);if(A>0)return`${A}d`;let R=Math.floor(M/3600);if(R>0)return`${R}h`;let W=Math.floor(M/60);return W>0?`${W}m`:`${Math.floor(M)}s`}function b(r){if(r.id==="upstash-redis-ro"){let P=N()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                <div class="service-widget-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 0;">
                    ...
                    <div class="sync-time-label" style="font-size: 0.6em; color: #444; margin-top: 10px;">Last synced: ${new Date(P).toLocaleTimeString()}</div>
                </div>
            </div>`}let d=r.status==="online",C=d?"service-widget-online":"service-widget-offline",w=d?"bx-check-circle":"bx-x-circle",g=d?"OK":"BAD",S=r.version?u(r.version.str):"-",M=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",A=r.memory&&r.memory!=="N/A"?r.memory:"-",R=M!=="-"?"#00ff00":"#666",W=M!=="-"?"#fff":"#666",J=A!=="-"?"#00ff00":"#666",V=A!=="-"?"#fff":"#666",I=H(r.uptime),$="",L=r.type!=="os"&&r.type!=="cli"&&r.type!=="prd",B="";L&&!N()&&(B=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                ...
                <button class="svc-btn svc-stop" title="Stop Service" data-action="stop" data-service="${r.id}" ${d?"":"disabled"}><i class='bx bx-stop'></i></button>
                <button class="svc-btn svc-start" title="Start Service" data-action="start" data-service="${r.id}" ${d?"disabled":""}><i class='bx bx-play'></i></button>
            </div>`),d?$=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        <i class="bx bxs-microchip" style="color: ${R};"></i>
                ...
                </div>${B}`:$=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${B}`;let O=N()?"easter.company":T(r.domain&&r.port?`${r.domain}:${r.port}`:"");return`<div class="service-widget ${C}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${w}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${g}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${$}</div></div>`}let p=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),h=new Set(s.map(r=>r.id));for(let[r,d]of p)h.has(r)||d.remove();let v=new Set;s.forEach(r=>{if(v.has(r.id))return;v.add(r.id);let d=b(r),C=p.get(r.id);if(C&&C.parentNode)if(r.id==="upstash-redis-ro"){let w=N()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),g=new Date(w).toLocaleTimeString(),S=C.querySelector(".sync-time-label");S&&(S.textContent=`Last synced: ${g}`)}else C.outerHTML!==d&&(C.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function at(){let t=document.getElementById("models-widgets");if(!t)return;let e=await is();if(!e){t.children.length===0&&(t.innerHTML=F("offline","Failed to load model status.","The event service may be offline."));return}Kt=Date.now(),te(2,Kt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function a(c){let x=c.status==="Ready";return`
                ...
                </div>`}function l(c){let x=c.status==="Ready";return`
                ...
                </div>`}function o(c){let x=c.status==="Downloaded",E=x?"service-widget-online":"service-widget-offline",m=x?"OK":"MISSING",y=x&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",s=c.name;return c.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${E}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${x?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${y}</span></div></div></div>`}let f="";if(i&&(f+=a(i)),e.xtts&&(f+=l(e.xtts)),f+=n.map(o).join(""),!f){t.innerHTML=F("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),x=document.getElementById("fabricator-reset-btn"),E=document.getElementById("courier-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let r=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D(r,{method:"POST"}),await ne()}catch{m.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},m.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),x&&!x.dataset.listenerAttached&&(x.onclick=async()=>{x.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{x.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{x.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{x.innerHTML="<i class='bx bx-error'></i>"}},x.dataset.listenerAttached="true"),E&&!E.dataset.listenerAttached&&(E.onclick=async()=>{E.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{E.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{E.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{E.innerHTML="<i class='bx bx-error'></i>"}},E.dataset.listenerAttached="true");let y=document.getElementById("imaginator-reset-btn");y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{y.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{y.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let v=s.agents.guardian||{protocols:{}},r=s.agents.analyzer||{protocols:{}},d=s.agents.imaginator||{protocols:{}},C=s.agents.fabricator||{protocols:{}},w=s.agents.courier||{protocols:{}},g=s.system||{},S=Math.floor(Date.now()/1e3),M={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},A=k=>{k<0&&(k=0);let _=Math.floor(k/3600),z=Math.floor(k%3600/60),U=k%60;return _>0?`${_}h ${z}m`:z>0?`${z}m ${U}s`:`${U}s`},R=(k,_,z,U)=>{if(!k)return;let Y=M[U]||U.toUpperCase(),K=k.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(K&&(K.textContent=Y),!z){k.textContent="Inactive",k.style.color="#666";return}let Ce=z.status;if(Ce==="Working")k.textContent="Working",k.style.color="#bb86fc";else if(Ce==="Ready")k.textContent="Ready",k.style.color="#5eff5e";else{let Ee=z.cooldown;if(N()&&(Ee=z.next_run-S),Ee<=0)k.textContent="Ready",k.style.color="#5eff5e";else{let Ge=Math.floor(Ee/60),Ve=Ee%60;k.textContent=`${Ge}m ${Ve}s`,k.style.color="#fff"}}_&&z.stats&&(_.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};n&&R(n,document.getElementById("guardian-sentry-stats"),v.protocols.sentry,"sentry");let W=document.getElementById("analyzer-synthesis-val"),J=document.getElementById("analyzer-synthesis-stats");W&&R(W,J,r.protocols.synthesis,"synthesis");let V=document.getElementById("imaginator-alert_review-val"),I=document.getElementById("imaginator-alert_review-stats");V&&R(V,I,d.protocols.alert_review,"alert_review");let $=document.getElementById("fabricator-construction-val"),L=document.getElementById("fabricator-construction-stats");$&&R($,L,C.protocols.construction,"construction");let B=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");B&&R(B,O,w.protocols.researcher,"researcher");let P=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&g.state){let k=g.state,_=A(g.state_time||0);P&&(P.textContent=`State: ${k.toUpperCase()}`),G.textContent=_,k==="idle"?G.style.color=g.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",m&&(k==="paused"?(m.innerHTML="<i class='bx bx-play'></i>",m.title="Resume System",m.style.color="#ff9800"):(m.innerHTML="<i class='bx bx-pause'></i>",m.title="Pause System",m.style.color=""))}a&&(a.textContent=A(g.metrics?.total_idle_time||0)),l&&(l.textContent=A(g.metrics?.total_active_time||0)),o&&(o.textContent=A(g.metrics?.total_waste_time||0))}else[n,i,a,l,o].forEach(r=>{r&&(r.textContent="-",r.style.color="#666")});if(t)return;let u=await Ws(),T=[],H=[],b=[];u&&(Array.isArray(u)?T=u:(T=u.active||[],H=u.queue||[],b=u.history||[],b.sort((v,r)=>(r.end_time||0)-(v.end_time||0)))),e&&(T.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=F("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,T,!1)));let p=document.getElementById("processes-queue-widgets");p&&(H.length===0?!p.querySelector(".tab-placeholder")&&!p.querySelector('div[style*="font-style: italic"]')&&(p.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(p.innerHTML="",it(p,H,!1)));let h=document.getElementById("processes-history-widgets");h&&(b.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=F("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),it(h,b,!0))),Pe(1,T.length)}function it(t,e,n){function i(c){let x="";c.end_time?x=`${c.end_time-c.start_time}s`:x=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let E=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,y={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(y[m]?m=y[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),n)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
                ...
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}-${c.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${s}">${c.state}</span>
                        </div>
                        <div class="service-widget-info">
            ...
            </div>`}function ot(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let a=this.dataset.theme;a&&(ft(a),t.setContent(We()),ot(t))})});let n=document.getElementById("notifications-toggle");n&&"Notification"in window?n.onclick=async i=>{let a=i.target;if(a.checked)try{await Notification.requestPermission()!=="granted"&&(a.checked=!1)}catch{a.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),a.checked=!0)}:n&&(n.disabled=!0),Vs()}async function Vs(){let t=document.getElementById("service-config-list");if(!t)return;let e=N();try{let i=await(await D("/system/options")).json();if(!i||Object.keys(i).length===0){t.innerHTML='<div style="padding: 20px; text-align: center; color: #666;">No configurable services found.</div>';return}let a="",l=(o,f,c)=>`
        <div class="settings-item">
            <div class="settings-item-info">
                <span class="settings-item-label">${o}</span>
                <span class="settings-item-description">Enable GPU acceleration (CUDA)</span>
            </div>
            <label class="toggle-switch">
                <input type="checkbox" class="service-device-toggle" data-service="${f}" ${c==="cuda"?"checked":""} ${e?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>
        </div>`;if(i.stt){let o=i.stt.device||"cpu";a+=l("STT Service","stt",o)}if(i.tts){let o=i.tts.device||"cpu";a+=l("TTS Service","tts",o)}if(e&&(a+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Configuration is read-only in public mode.</div>'),t.innerHTML=a,e)return;t.querySelectorAll(".service-device-toggle").forEach(o=>{o.addEventListener("change",async f=>{let c=f.target,x=c.dataset.service,E=c.checked?"cuda":"cpu";c.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:x,key:"device",value:E})})}catch(m){console.error("Failed to update config",m),c.checked=!c.checked,alert("Failed to update configuration.")}finally{c.disabled=!1}})})}catch{t.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load configuration.</div>'}}var os=()=>`
    ...
    `;async function rs(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await as(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await as(t)}async function as(t){try{let e=await D("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let n=await e.json();if(!n||n.length===0){t.innerHTML=F("empty","No web history found.");return}let i=`
        ...
        `;n.forEach((f,c)=>{let x=new Date(f.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),E=f.screenshot?`<img src="data:image/png;base64,${f.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ks=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[n,i]of Object.entries(t)){let a=ds.filter(l=>l.category===n);a.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>ls());let x=document.getElementById("terminal-action-btn");x&&(x.onclick=()=>ls())}let n=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");n&&(n.innerHTML="");let a=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let o=document.getElementById("terminal-status-badge");o&&(o.className="terminal-status status-running",o.textContent="Running");let f=document.getElementById("terminal-action-btn");return f&&(f.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",rt=!0,n}function ls(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",rt=!1}function cs(t,e,n="output"){if(!rt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Oe(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ys(t){let e=ds.find(o=>o.id===t);if(!e)return;let n=Zs(e);n&&cs(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let o of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let f="output";o.includes("[ERROR]")?f="error":o.includes("[SUCCESS]")||o.includes("\u2713")?f="success":o.includes("!")&&(f="warning"),n&&cs(n,o,f)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function ps(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ks();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Ys(a)})})}async function Xs(){if(!N())try{if(!(await D("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ms(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Xs(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ps();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=b=>{localStorage.setItem("dex_last_window",b)};function a(){for(;e.length>1;)e.shift().close(!0);let b=document.getElementById("windows-container"),p=document.querySelector("nav"),h=document.querySelector("footer"),v=window.location.pathname==="/"||window.location.pathname==="/index.html",r=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");b&&b.setAttribute("data-count",e.length.toString()),e.forEach(M=>{let A=document.getElementById(M.id);A&&A.classList.remove("hide-close")});let d=document.getElementById("dexter-menu-container"),C=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon"),g=document.getElementById("close-all-windows"),S=window.innerWidth<880;if(!S){let M=document.getElementById("dexter-dropdown"),A=document.getElementById("dexter-menu-btn");M&&M.classList.remove("active"),A&&A.classList.remove("active"),b&&b.classList.remove("menu-open")}if(Te(e.length>0),e.length>0)if(h?.classList.add("hide"),g&&(g.style.display=S?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",p?.classList.add("window-open"),b&&(b.style.paddingTop="60px"),d&&(d.style.display=S?"flex":"none"),w&&(w.style.display=S?"block":"none"),!S&&C){let M=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(M)?(C.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(f)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(E)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(x)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(m)})):C.innerHTML=""}else C&&(C.innerHTML="");else p?.classList.remove("window-open"),g&&(g.style.display="none"),b&&(b.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",v||r?h?.classList.remove("hide"):h?.classList.add("hide"),d&&(d.style.display="flex"),w&&(w.style.display="block"),C&&(C.innerHTML="");Se()}function l(b){if(b.isOpen()){b.close();return}for(;e.length>0;)e.pop().close(!0);e.push(b),b.open(),a()}function o(){[...e].forEach(p=>p.close()),e=[]}window.addEventListener("resize",a);let f=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>de(),onClose:()=>{let b=e.indexOf(f);b>-1&&e.splice(b,1),a()}}),c=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:nt(),onOpen:()=>{c.setContent(nt()),pe()},onClose:()=>{let b=e.indexOf(c);b>-1&&e.splice(b,1),a()}}),x=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ft(),onOpen:()=>st(),onClose:()=>{let b=e.indexOf(x);b>-1&&e.splice(b,1),a()}}),E=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Qt()},{icon:"bxs-component",title:"Processes",content:Xt()},{icon:"bxs-brain",title:"Models",content:es()},{icon:"bx-globe",title:"Web",content:os()},{icon:"bxs-hdd",title:"Hardware",content:ts()},{icon:"bxs-terminal",title:"Logs",content:ss()},{icon:"bxs-zap",title:"Agents",content:Yt()}].filter(b=>N()?b.title!=="Hardware"&&b.title!=="Logs":!0),onOpen:()=>{we(),ne(),at(),rs(),ze()},onClose:()=>{let b=e.indexOf(E);b>-1&&e.splice(b,1),a()}}),m=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Rt()},{icon:"bx-map-alt",title:"Roadmap",content:Pt()},{icon:"bx-paint",title:"Blueprints",content:Ot()},{icon:"bx-task",title:"Tasks",content:jt()}],onOpen:()=>et(),onClose:()=>{let b=e.indexOf(m);b>-1&&e.splice(b,1),a()}}),y=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:We(),onOpen:()=>{y.setContent(We()),ot(y)},onClose:()=>{let b=e.indexOf(y);b>-1&&e.splice(b,1),a()}});window.dexter={viewEvent:b=>{c.isOpen()||l(c),setTimeout(()=>{let p=document.querySelector(`.event-item[data-event-id="${b}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)},viewAlert:b=>{f.isOpen()||l(f),setTimeout(()=>{let p=document.querySelector(`.event-item[data-alert-id="${b}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let b=document.getElementById("dexter-dropdown"),p=document.getElementById("dexter-menu-btn");b&&p&&(b.classList.remove("active"),p.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let b=document.getElementById("dexter-menu-btn"),p=document.getElementById("settings-icon"),h=document.getElementById("close-all-windows"),v=document.getElementById("nav-left-container"),r=document.getElementById("dexter-dropdown"),d=document.getElementById("windows-container");b&&r&&(b.onclick=w=>{w.stopPropagation();let g=window.innerWidth<880;if(g)r.style.top="",r.style.left="",r.style.right="",r.style.transform="";else{let M=b.getBoundingClientRect(),R=document.querySelector("nav").getBoundingClientRect(),W=220,J=10,I=M.left+M.width/2-W/2,$=R.right-10;I+W>$&&(I=$-W),I<10&&(I=10),r.style.top=R.bottom+J+"px",r.style.left=I+"px",r.style.right="auto",r.style.transform="none"}r.classList.toggle("active"),b.classList.toggle("active");let S=r.classList.contains("active");if(g){let M=document.querySelector("nav");S?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),M?.classList.add("window-open"),d?.classList.add("menu-open"),Te(!0)):(d?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),M?.classList.remove("window-open"),Te(!1)))}}),p&&(p.onclick=w=>{w.stopPropagation(),l(y)}),h&&(h.onclick=w=>{w.stopPropagation(),o()});let C=(w,g,S)=>{let M=document.getElementById(w);M&&(M.onclick=A=>{A.stopPropagation(),s(),S&&i(S),l(g)})};C("alerts-menu-item",f,"alerts-window"),C("events-menu-item",c,"events-window"),C("monitor-menu-item",E,"monitor-window"),C("contacts-menu-item",x,"contacts-window"),C("workspace-menu-item",m,"workspace-window"),v&&(v.onclick=w=>{if(w.stopPropagation(),r&&r.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Te(!1));return}if(e.length>0)o();else{let g=window.location.pathname;if(!(g==="/"||g==="/index.html")){let A=(g.endsWith("/")&&g.length>1?g.slice(0,-1):g).split("/");A.pop();let R=A.join("/")||"/";window.location.href=R}}}),document.addEventListener("click",()=>{let w=window.innerWidth<880;r&&r.classList.contains("active")&&(s(),w&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Te(!1)))}),r&&(r.onclick=w=>w.stopPropagation())})();let T=window.location.pathname==="/"||window.location.pathname==="/index.html",H=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!T&&!H&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{c.isOpen()&&pe(),f.isOpen()?de():Qe()},1e3),setInterval(()=>{N()&&(f.isOpen()?de():Qe()),m.isOpen()?et():Bt(),N()&&c.isOpen()&&pe(),E.isOpen()&&ns()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ms):ms();})();

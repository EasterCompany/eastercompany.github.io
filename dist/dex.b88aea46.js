"use strict";(()=>{function ct(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function dt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var mt=1e3;function me(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++mt).toString())}function c(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",l),i&&setTimeout(i,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++mt).toString(),e.addEventListener("mousedown",a);let x=t.icon||"bx-window",s="",m="",C,L='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((f,b)=>{let u=f.icon?`<i class="bx ${f.icon}"></i>`:`<span class="tab-glyph">${f.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,C=`<div class="window-content">${t.tabs.map((f,b)=>`<div class="tab-content ${b===0?"active":""}" data-tab-content="${b}">${f.content}${L}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),C=`<div class="window-content">${t.content||""}${L}</div>`),e.innerHTML=`
...
`,Pe=null,ge=new Set,Oe=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];Pe=Date.now(),ee(0,Pe);let l=Date.now(),g=24*60*60*1e3,d=c.filter(r=>{let f=localStorage.getItem(`alert_read_ts_${r.id}`);if(!f)return!0;let b=parseInt(f);return l-b<g});d.sort((r,f)=>{let b=v=>{let h=v.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},u=v=>v==="critical"?4:v==="high"?3:v==="medium"?2:1,T=u(b(r)),E=u(b(f));return T!==E?E-T:f.timestamp-r.timestamp}),Oe=d;let y=r=>{let f=r.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},w=[];if(d.length===0)w.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let f=new Set(d.map(u=>y(u))).size>1,b=null;d.forEach(u=>{let T=y(u);f&&T!==b&&(w.push({id:`divider-${T}`,type:"divider",label:T.toUpperCase()}),b=T),w.push(u)})}t&&(e.innerHTML="");let p=r=>{let f=r.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let b=(f.title||"Untitled Alert").trim(),u=(f.body||"No description provided.").trim(),T=f.summary||"",E=f.content||"",v=f.protocol||"",h=(f.priority||"low").toLowerCase(),I=!!f.alert,A=(f.category||"system").trim(),S=f.related_event_ids||[],O=f.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${r.id}`),K=new Date(r.timestamp*1e3),M=K.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=K.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=F?"event-border-grey":"event-border-blue";!F&&I&&(k="event-border-red"),F&&(h==="high"||h==="critical")?k="event-border-red":F&&h==="medium"&&(k="event-border-orange");let H=F?"alert-read":"alert-unread",R=ge.has(r.id),G=R?"expanded":"",z=R?"display: block;":"display: none;",N="",B="";S.length>0&&(B=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${S.map(ce=>`<a href="#" onclick="window.dexter.viewEvent('${ce}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ce.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let q="";O&&(q=`
            ...
            <div style="flex: 1; min-width: 100px; text-align: center;">
                ...
                <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #bb86fc; font-weight: bold;">${v}</div>
            </div>`);let se="";E?se=`
        ...
        `:se=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue(u)}</div>
            </div>
        `,N=`
                ...
                ${se}
            `;let V=document.createElement("div");V.className=`event-item notification-item ${k} ${H} ${G} cursor-pointer priority-${h}`,V.dataset.alertId=r.id,V.onclick=function(ce){let X=this;if(X.classList.contains("expanded")){X.classList.remove("expanded"),ge.delete(r.id);let Le=X.querySelector(".event-details");Le&&(Le.style.display="none")}else{X.classList.add("expanded"),ge.add(r.id);let Le=X.querySelector(".event-details");if(Le&&(Le.style.display="block"),!localStorage.getItem(`alert_read_ts_${r.id}`)){localStorage.setItem(`alert_read_ts_${r.id}`,Date.now().toString()),X.classList.add("alert-read"),X.classList.remove("alert-unread"),X.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ge="event-border-grey";h==="high"||h==="critical"?Ge="event-border-red":h==="medium"&&(Ge="event-border-orange"),X.classList.add(Ge),Ze()}}};let le=`${v?v.toUpperCase():"GUARDIAN"} ALERT: ${T||b}`,We={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[A]||"bx-bell",ms=h==="high"||h==="critical"?"#ff4d4d":h==="medium"?"#ffa500":"#888";V.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${ms}; font-size: 0.8em; margin-left: 5px;">[${h.toUpperCase()}]</span>
                    </div>
...
`,be=new Set,kt=[],ke=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await _("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();kt=i;let a=p=>{let x=be.has(p.id),s=p.state==="published",m=p.state==="consumed",C="event-border-grey";s&&(C="event-border-blue"),m&&(C="event-border-purple");let o=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),r=document.createElement("div");r.className=`event-item notification-item ${C} cursor-pointer ${x?"expanded":""}`,r.dataset.itemId=p.id,r.onclick=u=>{if(u.target.closest("button"))return;if(r.classList.contains("expanded")){r.classList.remove("expanded");let E=r.querySelector(".event-details");E&&(E.style.display="none"),be.delete(p.id)}else{r.classList.add("expanded");let E=r.querySelector(".event-details");E&&(E.style.display="block"),be.add(p.id)}};let f=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;r.innerHTML=`
        <div class="event-time">
      ...
      `;let b=r.querySelector(".event-details");return b&&b.addEventListener("click",u=>{u.stopPropagation()}),r.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(p)),r.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(p)),r.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(p.id)),r.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),r.classList.remove("expanded");let T=r.querySelector(".event-details");T&&(T.style.display="none"),be.delete(p.id)}),r},c=p=>{let x=document.createElement("div");x.innerHTML=U("empty",p.message,p.action,p.id);let s=x.firstElementChild;return s.dataset.itemId=p.id,s},l=[];!i||i.length===0?l.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:D()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(p=>l.push({...p,type:"item"}));let g=Array.from(e.children),d=new Map(g.map(p=>[p.dataset.itemId,p])),y=new Set(l.map(p=>p.id));g.forEach(p=>{let x=p.dataset.itemId;(!x||!y.has(x))&&p.remove()}),t&&(e.innerHTML="");let w=null;l.forEach((p,x)=>{let s=d.get(p.id);(!s||t)&&(s&&s.remove(),p.type==="placeholder"?s=c(p):s=a(p),!s)||(x===0?e.firstElementChild!==s&&e.prepend(s):w&&w.nextElementSibling!==s&&w.after(s),w=s)})}catch{e.children.length===0&&(e.innerHTML=U("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ke=null;let c=document.getElementById("roadmap-editor-input");c&&(c.value="");let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let c=document.getElementById("roadmap-editor-container");c&&(c.style.display="none"),ke=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input"),l=c?c.value:"";if(!l.trim())return;let g=ke?`/roadmap/${ke}`:"/roadmap",d=ke?"PATCH":"POST";try{await _(g,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})});let y=document.getElementById("roadmap-editor-container");y&&(y.style.display="none"),ve(!0)}catch(y){console.error(y)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(c=>be.add(c.id)),ve(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{be.clear(),ve(!0)},a.dataset.listenerAttached="true")}function Ss(t){ke=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(n){console.error(n)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];At=c,St=Date.now(),ee(2,St);let l=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let C=(m.title||"Untitled Blueprint").trim(),L=(m.summary||m.body||"No summary provided.").trim(),o=(m.content||"").trim(),r=(m.category||"architecture").trim(),f=(m.related_services||m.affected_services||[]).map(N=>N.trim()),b=(m.implementation_path||[]).map(N=>N.trim()),u=m.source_event_ids||[],T=m.approved===!0,E=new Date(s.timestamp*1e3),v=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=ye.has(s.id),A=I?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${T?"blueprint-approved":""}`,S.dataset.blueprintId=s.id,T&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=T?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[r]||"bx-paint";S.onclick=function(N){let B=this;if(B.classList.contains("expanded")){B.classList.remove("expanded"),ye.delete(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="none")}else{B.classList.add("expanded"),ye.add(s.id);let J=B.querySelector(".event-details");J&&(J.style.display="block")}};let F="";b.length>0&&(F=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${b.map(N=>`<li style="margin-bottom: 5px;">${P(N)}</li>`).join("")}</ul></div>
                    </div>
                `);let K="";u.length>0&&(K=`
                ...
                `);let M=f.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${f.join(", ")}</span></div>`:"<div></div>",$=D(),k=T?`
            ...
            `;S.innerHTML=`
                ${T?'<div class="blueprint-sparkle"></div>':""}
                <div class="event-time">
                    <span class="event-time-main">${v}</span>
                    <span class="event-date">${h}</span>
                </div>
                <div class="event-icon" style="${T?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${A}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${P(L)}</div>
            ...
            `;let H=S.querySelector(".blueprint-approve-btn");H&&(H.onclick=async N=>{N.stopPropagation(),H.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(B){console.error("Failed to approve blueprint:",B)}});let R=S.querySelector(".blueprint-delete-btn");R&&(R.onclick=async N=>{N.stopPropagation();let B=!T;R.innerHTML=B?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${s.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let G=S.querySelector(".event-details");G&&G.addEventListener("click",N=>{N.stopPropagation()});let z=S.querySelector(".close-details-btn");return z&&z.addEventListener("click",N=>{N.stopPropagation(),S.classList.remove("expanded");let B=S.querySelector(".event-details");B&&(B.style.display="none"),ye.delete(s.id)}),S},g=s=>{let m=document.createElement("div");m.innerHTML=U("empty",s.message,s.action,s.id);let C=m.firstElementChild;return C.dataset.blueprintId=s.id,C},d=[];c.length===0?d.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):c.forEach(s=>d.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let y=Array.from(e.children),w=new Map(y.map(s=>[s.dataset.blueprintId,s])),p=new Set(d.map(s=>s.id));y.forEach(s=>{let m=s.dataset.blueprintId;(!m||!p.has(m))&&s.remove()});let x=null;d.forEach((s,m)=>{let C=w.get(s.id);(!C||t)&&(C&&C.remove(),s.type==="placeholder"?C=g(s):C=l(s),!C)||(m===0?e.firstElementChild!==C&&e.prepend(C):x&&x.nextElementSibling!==C&&x.after(C),x=C)}),Ne(2,c.length),ht()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=U("offline","Failed to load blueprints.","The event service may be offline."))}}async function It(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(c=>{let l=c.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&a++}),xt(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(i=>ye.add(i.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var re="STANDBY",Bt=null,Se=[],Y=null,je=null,Ht=()=>`
...
`,zt=()=>`
    ...
    `,Ae=[],Ie={"313071000877137920":"Owen",dexter:"Dexter"},te=[],he=null;async function Be(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),c=document.getElementById("new-chore-owner"),l=document.getElementById("selected-recipients"),g=document.getElementById("task-filter-owner"),d=document.getElementById("new-chore-instruction"),y=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),p=document.getElementById("new-chore-time"),x=document.getElementById("new-chore-time-group");w&&x&&!w.dataset.timeListenerAttached&&(w.addEventListener("change",()=>{x.style.display=w.value==="daily"?"block":"none"}),w.dataset.timeListenerAttached="true");let s=document.getElementById("form-title"),m=document.getElementById("form-icon");function C(){if(l){if(te.length===0){l.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}l.innerHTML=te.map(u=>{let T=Ie[u]||u,E=u.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${u==="dexter"?"#bb86fc":E?"#03dac6":"#fff"};">
          <i class='bx ${u==="dexter"?"bx-terminal":E?"bx-hash":"bx-user"}'></i>
          <span>${T}</span>
          <i class='bx bx-x remove-recipient' data-id="${u}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),l.querySelectorAll(".remove-recipient").forEach(u=>{u.onclick=()=>{let T=u.dataset.id;te=te.filter(E=>E!==T),C()}})}}function L(u){u&&!te.includes(u)&&(te.push(u),C()),c&&(c.value="")}function o(u=null){n&&(he=u?u.id:null,u?(s&&(s.textContent="Update Research Task"),m&&(m.className="bx bx-edit-alt",m.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),d&&(d.value=u.natural_instruction),y&&(y.value=u.execution_plan?.entry_url||""),w&&(w.value=u.schedule,x&&(x.style.display=u.schedule==="daily"?"block":"none")),p&&u.run_at&&(p.value=u.run_at),te=u.recipients||(u.owner_id?[u.owner_id]:[]),C()):(s&&(s.textContent="Initialize Research Task"),m&&(m.className="bx bx-plus-circle",m.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),d&&(d.value=""),y&&(y.value=""),w&&(w.value="every_24h"),te=[],C()),n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"}))}let r=document.getElementById("contacts-group"),f=document.getElementById("channels-group");if(r&&!r.dataset.populated&&!D())try{fe("/contacts").then(async u=>{u.ok&&(((await u.json()).members||[]).forEach(v=>{if(Ie[v.id]=v.nickname||v.username,v.id==="313071000877137920")return;let h=document.createElement("option");h.value=v.id;let I=v.nickname||v.username;h.textContent=`${I} (${v.username})`,r.appendChild(h)}),r.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(f&&!f.dataset.populated&&!D())try{fe("/channels").then(async u=>{u.ok&&((await u.json()).forEach(E=>{let v=`channel:${E.id}`;Ie[v]=E.name;let h=document.createElement("option");h.value=v,h.textContent=`#${E.name} (${E.guild})`,f.appendChild(h)}),f.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}c&&!c.dataset.listenerAttached&&(c.onchange=()=>{L(c.value)},c.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onchange=()=>b(),g.dataset.listenerAttached="true");function b(){if(!t)return;let u=g?.value||"all",T=u==="all"?Ae:Ae.filter(v=>(v.recipients||(v.owner_id?[v.owner_id]:[])).includes(u));if(T.length===0){t.innerHTML=U("empty",u==="all"?"No active tasks.":"No tasks found for this owner.",D()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let E=T.map(v=>{let h=v.last_run===0?"Never":new Date(v.last_run*1e3).toLocaleString(),I=v.memory?v.memory.length:0,A=v.status==="active"?"#03dac6":"#666",O=(v.recipients||(v.owner_id?[v.owner_id]:[])).map(W=>{let F=Ie[W]||W.substring(0,8),K=W.startsWith("channel:");return`<span title="${F}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${W==="dexter"?"bx-terminal":K?"bx-hash":"bx-user"}'></i>${F}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${A}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                        ...
                        <div style="display: flex; gap: 15px; flex: 1;">
                            ...
                            <div style="text-align: left;">
                                ...
                                <div style="margin-top: 8px; display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                                    ...
                                    <span style="font-size: 0.7em; color: #666; font-family: 'JetBrains Mono', monospace;"><i class='bx bx-time' style="margin-right: 4px;"></i>${v.schedule}${v.run_at?" @ "+v.run_at:""}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            ...
                            <span style="font-size: 0.85em; color: #03dac6; font-weight: bold;">${I}</span>
                        </div>
            ...
            `}).join("");t.innerHTML=E,t.querySelectorAll(".edit-chore-btn").forEach(v=>{v.onclick=h=>{h.stopPropagation();let I=v.dataset.id,A=Ae.find(S=>S.id===I);A&&o(A)}}),t.querySelectorAll(".reset-chore-btn").forEach(v=>{v.onclick=async h=>{h.stopPropagation();let I=v.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(v.innerHTML="<i class='bx bx-loader-alt spin'></i>",await _(`/chores/${I}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),Be())}}),t.querySelectorAll(".delete-chore-btn").forEach(v=>{v.onclick=async h=>{h.stopPropagation();let I=v.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${I}`,{method:"DELETE"}),Be())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||he!==null?o(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let u=document.getElementById("new-chore-instruction"),T=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),v=u?.value,h=E?.value||"every_24h",I=h==="daily"?p?.value:"";if(!v)return;if(te.length===0){alert("Please add at least one recipient.");return}let A=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let S=he?"PATCH":"POST",O=he?`/chores/${he}`:"/chores";await _(O,{method:S,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:te,natural_instruction:v,entry_url:T?.value,schedule:h,run_at:I})}),n&&(n.style.display="none"),u&&(u.value=""),T&&(T.value=""),he=null,te=[],Be()}catch(S){console.error(S),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=A}},i.dataset.listenerAttached="true");try{let u=await _("/chores");if(!u.ok)throw new Error(`HTTP error! status: ${u.status}`);if(Ae=await u.json()||[],g){let E=g.value,v=new Set;Ae.forEach(h=>{h.owner_id&&v.add(h.owner_id),h.recipients&&h.recipients.forEach(I=>v.add(I))}),g.innerHTML='<option value="all">All Recipients</option>',v.forEach(h=>{let I=document.createElement("option");I.value=h;let A=h.startsWith("channel:"),S=Ie[h]||`Recipient: ${h.substring(0,8)}`;I.textContent=(A&&!S.startsWith("#")?"#":"")+S,g.appendChild(I)}),g.value=E,g.selectedIndex===-1&&(g.value="all")}b()}catch(u){console.error("Failed to fetch chores",u)}}var Nt=()=>`
...
`;function jt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Ut(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",fe(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();Qe(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=jt(t.id,t.username);Qe(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=jt(t.id,t.username);Qe(e,t,i)})}function Qe(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let C=(n.attributes||n.facts||[]).map(u=>{let T=u.key||u.k,E=u.value||u.v;return`
        ...
        `},c=()=>{let C=n.dossier||{},L=C.identity||{},o=C.career||{},r=C.personal||{},f=C.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${L.fullName||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Age Range</div>
                        <div class="dossier-value">${L.ageRange||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Location</div>
                        <div class="dossier-value">${L.location||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Gender</div>
                        <div class="dossier-value">${L.gender||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Sexuality</div>
                        <div class="dossier-value" style="color: #ffb74d;">${L.sexuality||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Relationship Status</div>
                        <div class="dossier-value">${L.relationship||"Unknown"}</div>
                    </div>
...
`,zs=null;async function et(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await et(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let a=n.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let c=JSON.parse(a.dataset.user);Ut(c)}catch(c){console.error("Failed to parse user data",c)}},t.dataset.listenerAttached="true");try{let n=await fe("/contacts");if(!n.ok)throw new Error("Service unreachable");let a=(await n.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=U("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((l,g)=>{let d=c[l.level]??10,y=c[g.level]??10;return d!==y?d-y:l.username.localeCompare(g.username)}),t.innerHTML=a.map(l=>{let g=l.color&&l.color!==0,d=g?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",y=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",w="#888";l.level==="Me"||l.level==="Master"?w="#bb86fc":l.level==="Admin"?w="#cf6679":l.level==="Moderator"?w="#03dac6":l.level==="Contributor"&&(w="#ffa500");let p=l.level==="Me",x=p?"rgba(187, 134, 252, 0.08)":g?`${d}11`:"rgba(255,255,255,0.03)",s=p?"2px solid #bb86fc":g?`1px solid ${d}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${p?"#bb86fc":g?d:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${l.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":g?`2px solid ${d}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                        <div style="position: absolute; bottom: 0; right: 0; width: 12px; height: 12px; background: ${y}; border: 2px solid #1a1a1a; border-radius: 50%;"></div>
                    </div>
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${g?d:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${l.nickname||l.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${w}; font-weight: 600; text-transform: uppercase;">${p?"DEXTER (ME)":l.level}</span>
...
`,Ue=null,we=new Set,He=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await _(i);if(!a.ok)throw new Error("Service unreachable");if(He=((await a.json()).events||[]).filter(x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ue=Date.now(),ee(1,Ue),He.length===0){e.innerHTML=U("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,C=Os(m),L=js(m),o=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),r="event-border-grey";o&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?r="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?r="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?r="event-border-orange":m==="system.test.completed"?r=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":r="event-border-blue");let f=o?"cursor-pointer":"",b=we.has(x.id),u=b?"expanded":"",T=b?"display: block;":"display: none;",E=new Date(x.timestamp*1e3),v=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=Ft(m,s),A=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",S="";if(o){let M="";if(m==="engagement.decision"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k=s.timestamp-s.last_active,H=Math.floor(k/60);M=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k="";s.eval_count&&(k=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let H="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,G=s.chat_history.map((z,N)=>{let B=z.name?z.name.toUpperCase():z.role.toUpperCase();!z.name&&B==="ASSISTANT"&&(B="DEXTER");let q=z.role==="user"?"#03dac6":z.role==="system"?"#ffb74d":"#bb86fc",J=N===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${N}" style="display: ${J};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${q}; letter-spacing: 1px; font-weight: bold;">${B}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${N+1} of ${R}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(z.content)}</div>
                                </div>
                            `}).join("");H=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(m==="analysis.user.message_signals"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k=s.signals||{};M=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(k.topics||[]).map(R=>`<span class="profile-badge">${R}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(m==="analysis.visual.completed"){let $=H=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${H}</h5>`,k="";s.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(k=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",k=s.success?"SUCCESS":"FAILED",H=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,R="";s.error&&(R=`
                            <div class="event-detail-block">
                                ${H("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(s.error)}</pre>
                            </div>
                        `);let G="";if(s.chat_history&&s.chat_history.length>0){let B=s.chat_history.length,q=s.chat_history.map((J,se)=>{let V=J.name?J.name.toUpperCase():J.role.toUpperCase();!J.name&&V==="USER"&&(V="SYSTEM"),!J.name&&V==="ASSISTANT"&&(V="AGENT");let le=J.role==="user"?"#03dac6":J.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${se}" style="display: ${se===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${le}; letter-spacing: 1px; font-weight: bold;">${V}</span>
                        ...
                        `}let z="";if(s.corrections&&s.corrections.length>0){let B=s.corrections.map((q,J)=>`
                        ...
                        `}let N="";if(s.parsed_results&&s.parsed_results.length>0){let B=s.parsed_results.map(q=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${k} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(H=>{let R=H.content_type&&H.content_type.startsWith("image/"),G=(H.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${R?`<div class="attachment-thumb-container"><img src="${H.url}" alt="${H.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${H.url}" target="_blank" class="attachment-link">${H.filename}</a>
                                        <span class="attachment-meta">${H.content_type} \u2022 ${G}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${r} ${f} ${u}`,O.dataset.eventId=x.id,O.onclick=function(M){if(!o)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),we.delete(x.id);let H=$.querySelector(".event-details");H&&(H.style.display="none")}else{$.classList.add("expanded"),we.add(x.id);let H=$.querySelector(".event-details");H&&(H.style.display="block")}},O.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${v}</span>
                    <span class="event-date">${h}</span>
                </div>
                <div class="event-icon"><i class='bx ${L}'></i></div>
                <div class="event-content">
                    <div class="event-service">
...
`,Jt=qs;function Wt(){return`
    ...
    `}var Fs=null;async function _e(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=U("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(l=>!i.includes(l.id));a.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),a.reverse();let c=a.map(l=>{let g=l.logs.join(`
`),d=[...l.logs];if(d.length<25){let w=25-d.length;for(let p=0;p<w;p++)d.push("")}else d.length>25&&(d=d.slice(-25));let y=d.map(w=>Re(w)).join(`
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${l.id}" title="Clear Logs" style="${D()?"display: none;":""}">
                                <i class="bx bx-trash"></i>
                            </button>
                        </div>
                    </div>
                    <pre class="log-content">${y}</pre>
                </div>
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let g=l,d=unescape(g.dataset.logs||"");navigator.clipboard.writeText(d).then(()=>{let y=g.querySelector("i");y&&(y.classList.remove("bx-copy"),y.classList.add("bx-check"),setTimeout(()=>{y.classList.remove("bx-check"),y.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let d=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${d}?`))try{(await _(`/logs?service_id=${d}`,{method:"DELETE"})).ok&&_e()}catch(y){console.error("Error clearing logs:",y)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=U("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Zt=()=>{let t=D()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(o.GPU&&o.GPU.length>0?c.innerHTML=o.GPU.map(r=>{let f=(r.VRAM/1073741824).toFixed(1);return`
                    ...
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${Number(u)>90?"#cf6679":"#03dac6"}; width: ${u}%; height: 100%; box-shadow: 0 0 10px ${Number(u)>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await Kt();o?(g(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let o=await Kt();g(o)}if(!t)return;let d=await ns();if(!d||!d.services){t.children.length===0&&(t.innerHTML=U("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let r=o.querySelector(".service-widget-body");r&&(r.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Gt=D()&&oe.lastDashboard||Date.now(),ee(0,Gt||0);let y=d.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let b=59-new Date().getSeconds();b<=0&&(b+=60),o.textContent=`${b}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function w(o){if(!o||o==="N/A"||o==="unknown")return"-";let r=o.match(/^(\d+\.\d+\.\d+)/);return r?r[0]:o.split(".").slice(0,3).join(".")||"-"}function p(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function x(o){if(!o||o==="N/A"||o==="unknown")return"-";let r=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!r)return"-";let f=parseInt(r[1])||0,b=parseInt(r[2])||0,u=parseInt(r[3])||0,T=parseFloat(r[4])||0,E=f*86400+b*3600+u*60+T,v=Math.floor(E/86400);if(v>0)return`${v}d`;let h=Math.floor(E/3600);if(h>0)return`${h}h`;let I=Math.floor(E/60);return I>0?`${I}m`:`${Math.floor(E)}s`}function s(o){if(o.id==="upstash-redis-ro"){let K=D()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let r=o.status==="online",f=r?"service-widget-online":"service-widget-offline",b=r?"bx-check-circle":"bx-x-circle",u=r?"OK":"BAD",T=o.version?w(o.version.str):"-",E=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",v=o.memory&&o.memory!=="N/A"?o.memory:"-",h=E!=="-"?"#00ff00":"#666",I=E!=="-"?"#fff":"#666",A=v!=="-"?"#00ff00":"#666",S=v!=="-"?"#fff":"#666",O=x(o.uptime),W="";r?W=`
                ...
                </div>`:W='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let F=D()?"easter.company":p(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${f}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${b}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${F}</span></div>${W}</div></div>`}let m=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),C=new Set(y.map(o=>o.id));for(let[o,r]of m)C.has(o)||r.remove();let L=new Set;y.forEach(o=>{if(L.has(o.id))return;L.add(o.id);let r=s(o),f=m.get(o.id);if(f&&f.parentNode)if(o.id==="upstash-redis-ro"){let b=D()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),u=new Date(b).toLocaleTimeString(),T=f.querySelector(".sync-time-label");T&&(T.textContent=`Last synced: ${u}`)}else f.outerHTML!==r&&(f.outerHTML=r);else t.insertAdjacentHTML("beforeend",r)})}async function it(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=U("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),ee(2,Vt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function a(d){let y=d.status==="Ready";return`
                <div class="service-widget ${y?"service-widget-online":"service-widget-offline"}" data-whisper-widget>
                    <div class="service-widget-header">
                        <i class="bx bxs-microphone-alt"></i>
                        <h3>Whisper</h3>
                        <span class="service-widget-status">${y?"READY":"NOT FOUND"}</span>
                    </div>
                ...
                </div>`}function c(d){let y=d.status==="Ready";return`
                <div class="service-widget ${y?"service-widget-online":"service-widget-offline"}" data-xtts-widget>
                    <div class="service-widget-header">
                ...
                </div>`}function l(d){let y=d.status==="Downloaded",w=y?"service-widget-online":"service-widget-offline",p=y?"OK":"MISSING",x=y&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",s=d.name;return d.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${w}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${y?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${x}</span></div></div></div>`}let g="";if(i&&(g+=a(i)),e.xtts&&(g+=c(e.xtts)),g+=n.map(l).join(""),!g){t.innerHTML=U("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn"),y=document.getElementById("fabricator-reset-btn"),w=document.getElementById("courier-reset-btn"),p=document.getElementById("system-pause-toggle-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{let b=p.querySelector(".bx-play")?"/agent/resume":"/agent/pause";p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _(b,{method:"POST"}),await ne()}catch{p.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},p.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{d.innerHTML="<i class='bx bx-error'></i>"}},d.dataset.listenerAttached="true"),y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{y.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{y.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true"),w&&!w.dataset.listenerAttached&&(w.onclick=async()=>{w.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{w.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{w.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{w.innerHTML="<i class='bx bx-error'></i>"}},w.dataset.listenerAttached="true");let x=await Gs();if(x&&x.agents){let f=x.agents.guardian||{protocols:{}},b=x.agents.analyzer||{protocols:{}},u=x.agents.imaginator||{protocols:{}},T=x.agents.fabricator||{protocols:{}},E=x.agents.courier||{protocols:{}},v=x.system||{},h=Math.floor(Date.now()/1e3),I={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},A=z=>{z<0&&(z=0);let N=Math.floor(z/3600),B=Math.floor(z%3600/60),q=z%60;return N>0?`${N}h ${B}m`:B>0?`${B}m ${q}s`:`${q}s`},S=(z,N,B,q)=>{if(!z)return;let J=I[q]||q.toUpperCase(),se=z.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(se&&(se.textContent=J),!B){z.textContent="Inactive",z.style.color="#666";return}let V=B.status;if(V==="Working")z.textContent="Working",z.style.color="#bb86fc";else if(V==="Ready")z.textContent="Ready",z.style.color="#5eff5e";else{let le=B.cooldown;if(D()&&(le=B.next_run-h),le<=0)z.textContent="Ready",z.style.color="#5eff5e";else{let Je=Math.floor(le/60),We=le%60;z.textContent=`${Je}m ${We}s`,z.style.color="#fff"}}N&&B.stats&&(N.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};n&&S(n,document.getElementById("guardian-sentry-stats"),f.protocols.sentry,"sentry");let O=document.getElementById("analyzer-synthesis-val"),W=document.getElementById("analyzer-synthesis-stats");O&&S(O,W,b.protocols.synthesis,"synthesis");let F=document.getElementById("imaginator-alert_review-val"),K=document.getElementById("imaginator-alert_review-stats");F&&S(F,K,u.protocols.alert_review,"alert_review");let M=document.getElementById("fabricator-construction-val"),$=document.getElementById("fabricator-construction-stats");M&&S(M,$,T.protocols.construction,"construction");let k=document.getElementById("courier-researcher-val"),H=document.getElementById("courier-researcher-stats");k&&S(k,H,E.protocols.researcher,"researcher");let R=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&v.state){let z=v.state,N=A(v.state_time||0);R&&(R.textContent=`State: ${z.toUpperCase()}`),G.textContent=N,z==="idle"?G.style.color=v.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",p&&(z==="paused"?(p.innerHTML="<i class='bx bx-play'></i>",p.title="Resume System",p.style.color="#ff9800"):(p.innerHTML="<i class='bx bx-pause'></i>",p.title="Pause System",p.style.color=""))}a&&(a.textContent=A(v.metrics?.total_idle_time||0)),c&&(c.textContent=A(v.metrics?.total_active_time||0)),l&&(l.textContent=A(v.metrics?.total_waste_time||0))}else[n,i,a,c,l].forEach(b=>{b&&(b.textContent="-",b.style.color="#666")});if(t)return;let s=await Ws(),m=[],C=[],L=[];s&&(Array.isArray(s)?m=s:(m=s.active||[],C=s.queue||[],L=s.history||[],L.sort((f,b)=>(b.end_time||0)-(f.end_time||0)))),e&&(m.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=U("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),st(e,m,!1)));let o=document.getElementById("processes-queue-widgets");o&&(C.length===0?!o.querySelector(".tab-placeholder")&&!o.querySelector('div[style*="font-style: italic"]')&&(o.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(o.innerHTML="",st(o,C,!1)));let r=document.getElementById("processes-history-widgets");r&&(L.length===0?r.querySelector(".tab-placeholder")||(r.innerHTML=U("empty","No recent history.")):(r.querySelector(".tab-placeholder")&&(r.innerHTML=""),st(r,L,!0))),Ne(1,m.length)}function st(t,e,n){function i(d){let y="";d.end_time?y=`${d.end_time-d.start_time}s`:y=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let w=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",p=d.channel_id,x={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(x[p]?p=x[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),n)return`
        <div class="process-history-item" data-channel-id="${d.channel_id}-${d.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                <span>PID: ${d.pid}</span>
                <span style="color: #03dac6; min-width: 40px; text-align: right;">${y}</span>
            </div>
                ...
                <div class="service-widget process-widget" data-channel-id="${d.channel_id}-${d.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${s}">${y}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let a=n?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(a)).map(d=>[d.dataset.channelId,d])),l=new Set(e.map(d=>`${d.channel_id}-${d.start_time}`));for(let[d,y]of c)d&&!l.has(d)&&y.remove();let g=new Set;e.forEach(d=>{let y=`${d.channel_id}-${d.start_time}`;if(g.has(y))return;g.add(y);let w=i(d),p=c.get(y);if(p&&p.parentNode){p.outerHTML!==w&&(p.outerHTML=w);let x=t.querySelector(`[data-channel-id="${y}"]`);x&&t.appendChild(x)}else t.insertAdjacentHTML("beforeend",w)})}function Fe(){let t=Ve(),e="Notification"in window,n={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function os(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await is(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await is(t)}async function is(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let n=await e.json();if(!n||n.length===0){t.innerHTML=U("empty","No web history found.");return}let i=`
        ...
        `;n.forEach((g,d)=>{let y=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),w=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
                <div class="web-card" style="flex: 0 0 100%; height: 100%; display: flex; flex-direction: column; scroll-snap-align: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-sizing: border-box; max-width: 100%; overflow: hidden;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; flex-shrink: 0;">
                        ...
                        <span style="color: #888; font-size: 0.8em; font-family: monospace;">${y}</span>
                    </div>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Vs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[n,i]of Object.entries(t)){let a=cs.filter(c=>c.category===n);a.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let d=document.getElementById("close-terminal-btn");d&&(d.onclick=()=>rs());let y=document.getElementById("terminal-action-btn");y&&(y.onclick=()=>rs())}let n=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");n&&(n.innerHTML="");let a=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let c=document.getElementById("terminal-cmd-name");c&&(c.textContent=`dex ${t.id}`);let l=document.getElementById("terminal-status-badge");l&&(l.className="terminal-status status-running",l.textContent="Running");let g=document.getElementById("terminal-action-btn");return g&&(g.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",ot=!0,n}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",ot=!1}function ls(t,e,n="output"){if(!ot||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Re(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Zs(t){let e=cs.find(l=>l.id===t);if(!e)return;let n=Ks(e);n&&ls(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let l of i){await new Promise(d=>setTimeout(d,400+Math.random()*600));let g="output";l.includes("[ERROR]")?g="error":l.includes("[SUCCESS]")||l.includes("\u2713")?g="success":l.includes("!")&&(g="warning"),n&&ls(n,l,g)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let c=document.getElementById("terminal-action-btn");c&&(c.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Vs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Zs(a)})})}async function Ys(){if(!D())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}dt(),pt(),Ys(),bt(),ct();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=o=>{localStorage.setItem("dex_last_window",o)};function a(){for(;e.length>1;)e.shift().close(!0);let o=document.getElementById("windows-container"),r=document.querySelector("nav"),f=document.querySelector("footer"),b=window.location.pathname==="/"||window.location.pathname==="/index.html",u=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");o&&o.setAttribute("data-count",e.length.toString()),e.forEach(A=>{let S=document.getElementById(A.id);S&&S.classList.remove("hide-close")});let T=document.getElementById("dexter-menu-container"),E=document.getElementById("nav-window-switcher"),v=document.getElementById("settings-icon"),h=document.getElementById("close-all-windows"),I=window.innerWidth<880;if(!I){let A=document.getElementById("dexter-dropdown"),S=document.getElementById("dexter-menu-btn");A&&A.classList.remove("active"),S&&S.classList.remove("active"),o&&o.classList.remove("menu-open")}if(Ce(e.length>0),e.length>0)if(f?.classList.add("hide"),h&&(h.style.display=I?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",r?.classList.add("window-open"),o&&(o.style.paddingTop="60px"),T&&(T.style.display=I?"flex":"none"),v&&(v.style.display=I?"block":"none"),!I&&E){let A=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(A)?(E.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),c(g)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),c(d)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),c(w)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),c(y)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),c(p)})):E.innerHTML=""}else E&&(E.innerHTML="");else r?.classList.remove("window-open"),h&&(h.style.display="none"),o&&(o.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",b||u?f?.classList.remove("hide"):f?.classList.add("hide"),T&&(T.style.display="flex"),v&&(v.style.display="block"),E&&(E.innerHTML="");$e()}function c(o){if(o.isOpen()){o.close();return}for(;e.length>0;)e.pop().close(!0);e.push(o),o.open(),a()}function l(){[...e].forEach(r=>r.close()),e=[]}window.addEventListener("resize",a);let g=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Et(),onOpen:()=>de(),onClose:()=>{let o=e.indexOf(g);o>-1&&e.splice(o,1),a()}}),d=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:tt(),onOpen:()=>{d.setContent(tt()),pe()},onClose:()=>{let o=e.indexOf(d);o>-1&&e.splice(o,1),a()}}),y=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:qt(),onOpen:()=>et(),onClose:()=>{let o=e.indexOf(y);o>-1&&e.splice(o,1),a()}}),w=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(o=>D()?o.title!=="Hardware"&&o.title!=="Logs":!0),onOpen:()=>{nt(),ne(),it(),os(),_e()},onClose:()=>{let o=e.indexOf(w);o>-1&&e.splice(o,1),a()}}),p=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Nt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Pt()},{icon:"bx-task",title:"Tasks",content:Ot()}],onOpen:()=>Xe(),onClose:()=>{let o=e.indexOf(p);o>-1&&e.splice(o,1),a()}}),x=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:Fe(),onOpen:()=>{x.setContent(Fe()),at(x)},onClose:()=>{let o=e.indexOf(x);o>-1&&e.splice(o,1),a()}});window.dexter={viewEvent:o=>{d.isOpen()||c(d),setTimeout(()=>{let r=document.querySelector(`.event-item[data-event-id="${o}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)},viewAlert:o=>{g.isOpen()||c(g),setTimeout(()=>{let r=document.querySelector(`.event-item[data-alert-id="${o}"]`);r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.classList.add("flash-highlight"),r.classList.contains("expanded")||r.click(),setTimeout(()=>r.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let o=document.getElementById("dexter-dropdown"),r=document.getElementById("dexter-menu-btn");o&&r&&(o.classList.remove("active"),r.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let o=document.getElementById("dexter-menu-btn"),r=document.getElementById("settings-icon"),f=document.getElementById("close-all-windows"),b=document.getElementById("nav-left-container"),u=document.getElementById("dexter-dropdown"),T=document.getElementById("windows-container");o&&u&&(o.onclick=v=>{v.stopPropagation();let h=window.innerWidth<880;if(h)u.style.top="",u.style.left="",u.style.right="",u.style.transform="";else{let A=o.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),W=220,F=10,M=A.left+A.width/2-W/2,$=O.right-10;M+W>$&&(M=$-W),M<10&&(M=10),u.style.top=O.bottom+F+"px",u.style.left=M+"px",u.style.right="auto",u.style.transform="none"}u.classList.toggle("active"),o.classList.toggle("active");let I=u.classList.contains("active");if(h){let A=document.querySelector("nav");I?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),A?.classList.add("window-open"),T?.classList.add("menu-open"),Ce(!0)):(T?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),A?.classList.remove("window-open"),Ce(!1)))}}),r&&(r.onclick=v=>{v.stopPropagation(),c(x)}),f&&(f.onclick=v=>{v.stopPropagation(),l()});let E=(v,h,I)=>{let A=document.getElementById(v);A&&(A.onclick=S=>{S.stopPropagation(),s(),I&&i(I),c(h)})};E("alerts-menu-item",g,"alerts-window"),E("events-menu-item",d,"events-window"),E("monitor-menu-item",w,"monitor-window"),E("contacts-menu-item",y,"contacts-window"),E("workspace-menu-item",p,"workspace-window"),b&&(b.onclick=v=>{if(v.stopPropagation(),u&&u.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1));return}if(e.length>0)l();else{let h=window.location.pathname;if(!(h==="/"||h==="/index.html")){let S=(h.endsWith("/")&&h.length>1?h.slice(0,-1):h).split("/");S.pop();let O=S.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let v=window.innerWidth<880;u&&u.classList.contains("active")&&(s(),v&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1)))}),u&&(u.onclick=v=>v.stopPropagation())})();let C=window.location.pathname==="/"||window.location.pathname==="/index.html",L=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!C&&!L&&document.querySelector("footer")?.classList.add("hide"),D()||setInterval(()=>{d.isOpen()&&pe(),g.isOpen()?de():Ye()},1e3),setInterval(()=>{D()&&(g.isOpen()?de():Ye()),p.isOpen()?Xe():It(),D()&&d.isOpen()&&pe(),w.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

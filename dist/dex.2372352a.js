"use strict";(()=>{function ct(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function dt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var mt=1e3;function me(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++mt).toString())}function c(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",r),i&&setTimeout(i,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++mt).toString(),e.addEventListener("mousedown",a);let y=t.icon||"bx-window",s="",u="",w,S='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((d,f)=>{let x=d.icon?`<i class="bx ${d.icon}"></i>`:`<span class="tab-glyph">${d.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,w=`<div class="window-content">${t.tabs.map((d,f)=>`<div class="tab-content ${f===0?"active":""}" data-tab-content="${f}">${d.content}${S}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),w=`<div class="window-content">${t.content||""}${S}</div>`),e.innerHTML=`
...
`,Pe=null,ge=new Set,Oe=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await B(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];Pe=Date.now(),ee(0,Pe);let r=Date.now(),g=24*60*60*1e3,p=c.filter(l=>{let d=localStorage.getItem(`alert_read_ts_${l.id}`);if(!d)return!0;let f=parseInt(d);return r-f<g});p.sort((l,d)=>{let f=E=>{let $=E.event;if(typeof $=="string")try{$=JSON.parse($)}catch{return"low"}return($.priority||"low").toLowerCase()},x=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,b=x(f(l)),h=x(f(d));return b!==h?h-b:d.timestamp-l.timestamp}),Oe=p;let v=l=>{let d=l.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return"low"}return(d.priority||"low").toLowerCase()},C=[];if(p.length===0)C.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let d=new Set(p.map(x=>v(x))).size>1,f=null;p.forEach(x=>{let b=v(x);d&&b!==f&&(C.push({id:`divider-${b}`,type:"divider",label:b.toUpperCase()}),f=b),C.push(x)})}t&&(e.innerHTML="");let m=l=>{let d=l.event;if(typeof d=="string")try{d=JSON.parse(d)}catch{return null}let f=(d.title||"Untitled Alert").trim(),x=(d.body||"No description provided.").trim(),b=d.summary||"",h=d.content||"",E=d.protocol||"",$=(d.priority||"low").toLowerCase(),N=!!d.alert,_=(d.category||"system").trim(),L=d.related_event_ids||[],O=d.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${l.id}`),K=new Date(l.timestamp*1e3),M=K.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=K.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=J?"event-border-grey":"event-border-blue";!J&&N&&(k="event-border-red"),J&&($==="high"||$==="critical")?k="event-border-red":J&&$==="medium"&&(k="event-border-orange");let I=J?"alert-read":"alert-unread",R=ge.has(l.id),W=R?"expanded":"",D=R?"display: block;":"display: none;",z="",A="";L.length>0&&(A=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue(h)}</div>
            </div>
        `:se=`
            <div class="event-detail-block" style="text-align: left;">
        ...
        `,z=`
                ...
                ${se}
            `;let G=document.createElement("div");G.className=`event-item notification-item ${k} ${I} ${W} cursor-pointer priority-${$}`,G.dataset.alertId=l.id,G.onclick=function(ce){let X=this;if(X.classList.contains("expanded")){X.classList.remove("expanded"),ge.delete(l.id);let Le=X.querySelector(".event-details");Le&&(Le.style.display="none")}else{X.classList.add("expanded"),ge.add(l.id);let Le=X.querySelector(".event-details");if(Le&&(Le.style.display="block"),!localStorage.getItem(`alert_read_ts_${l.id}`)){localStorage.setItem(`alert_read_ts_${l.id}`,Date.now().toString()),X.classList.add("alert-read"),X.classList.remove("alert-unread"),X.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ge="event-border-grey";$==="high"||$==="critical"?Ge="event-border-red":$==="medium"&&(Ge="event-border-orange"),X.classList.add(Ge),Ze()}}};let le=`${E?E.toUpperCase():"GUARDIAN"} ALERT: ${b||f}`,We={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[_]||"bx-bell",ms=$==="high"||$==="critical"?"#ff4d4d":$==="medium"?"#ffa500":"#888";G.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${ms}; font-size: 0.8em; margin-left: 5px;">[${$.toUpperCase()}]</span>
                    </div>
            ...
            `;let rt=G.querySelector(".event-details");rt&&rt.addEventListener("click",ce=>{ce.stopPropagation()});let lt=G.querySelector(".close-details-btn");return lt&&lt.addEventListener("click",ce=>{ce.stopPropagation(),G.classList.remove("expanded");let X=G.querySelector(".event-details");X&&(X.style.display="none"),ge.delete(l.id)}),G},y=l=>{let d=document.createElement("div");d.className="notification-divider",d.dataset.alertId=l.id;let f="#888888";return l.label==="CRITICAL"?f="#ff4d4d":l.label==="HIGH"?f="#ff8888":l.label==="MEDIUM"&&(f="#ffa500"),d.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${f}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,d.innerHTML=`<span style="white-space: nowrap;">${l.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${f}44, transparent); flex-grow: 1;"></div>`,d},s=l=>{let d=document.createElement("div");d.innerHTML=U("empty",l.message,null,l.id);let f=d.firstElementChild;return f.dataset.alertId=l.id,f},u=Array.from(e.children),w=new Map(u.map(l=>[l.dataset.alertId,l])),S=new Set(C.map(l=>l.id));u.forEach(l=>{let d=l.dataset.alertId;(!d||!S.has(d))&&l.remove()});let o=null;C.forEach((l,d)=>{let f=w.get(l.id);if(!f||t){if(f&&f.remove(),l.type==="divider")f=y(l);else if(l.type==="placeholder")f=s(l);else{let x=m(l);if(!x)return;f=x}if(!f)return}d===0?e.firstElementChild!==f&&e.prepend(f):o&&o.nextElementSibling!==f&&o.after(f),o=f}),Pe=Date.now(),ee(0,Pe),Ze()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=U("offline","Failed to load alerts.","The event service may be offline."))}}function $s(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),n=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Oe.forEach(a=>{localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Oe.forEach(a=>{ge.add(a.id),localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ge.clear(),de(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let a=Date.now()-48*60*60*1e3;Oe.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,a.toString())}),ge.clear(),de(!0)}catch(a){console.error("Failed to clear alerts:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function Ye(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||a++}),yt(a)}catch{}}var Tt=()=>`
...
`,be=new Set,kt=[],ke=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await B("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();kt=i;let a=m=>{let y=be.has(m.id),s=m.state==="published",u=m.state==="consumed",w="event-border-grey";s&&(w="event-border-blue"),u&&(w="event-border-purple");let o=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),l=document.createElement("div");l.className=`event-item notification-item ${w} cursor-pointer ${y?"expanded":""}`,l.dataset.itemId=m.id,l.onclick=x=>{if(x.target.closest("button"))return;if(l.classList.contains("expanded")){l.classList.remove("expanded");let h=l.querySelector(".event-details");h&&(h.style.display="none"),be.delete(m.id)}else{l.classList.add("expanded");let h=l.querySelector(".event-details");h&&(h.style.display="block"),be.add(m.id)}};let d=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;l.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${P(m.content)}</div>
      ...
      `;let f=l.querySelector(".event-details");return f&&f.addEventListener("click",x=>{x.stopPropagation()}),l.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(m)),l.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(m)),l.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(m.id)),l.querySelector(".close-details-btn")?.addEventListener("click",x=>{x.stopPropagation(),l.classList.remove("expanded");let b=l.querySelector(".event-details");b&&(b.style.display="none"),be.delete(m.id)}),l},c=m=>{let y=document.createElement("div");y.innerHTML=U("empty",m.message,m.action,m.id);let s=y.firstElementChild;return s.dataset.itemId=m.id,s},r=[];!i||i.length===0?r.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(m=>r.push({...m,type:"item"}));let g=Array.from(e.children),p=new Map(g.map(m=>[m.dataset.itemId,m])),v=new Set(r.map(m=>m.id));g.forEach(m=>{let y=m.dataset.itemId;(!y||!v.has(y))&&m.remove()}),t&&(e.innerHTML="");let C=null;r.forEach((m,y)=>{let s=p.get(m.id);(!s||t)&&(s&&s.remove(),m.type==="placeholder"?s=c(m):s=a(m),!s)||(y===0?e.firstElementChild!==s&&e.prepend(s):C&&C.nextElementSibling!==s&&C.after(s),C=s)})}catch{e.children.length===0&&(e.innerHTML=U("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ke=null;let c=document.getElementById("roadmap-editor-input");c&&(c.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let c=document.getElementById("roadmap-editor-container");c&&(c.style.display="none"),ke=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input"),r=c?c.value:"";if(!r.trim())return;let g=ke?`/roadmap/${ke}`:"/roadmap",p=ke?"PATCH":"POST";try{await B(g,{method:p,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let v=document.getElementById("roadmap-editor-container");v&&(v.style.display="none"),ve(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(c=>be.add(c.id)),ve(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{be.clear(),ve(!0)},a.dataset.listenerAttached="true")}function Ss(t){ke=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(n){console.error(n)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await B(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];At=c,St=Date.now(),ee(2,St);let r=s=>{let u=s.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let w=(u.title||"Untitled Blueprint").trim(),S=(u.summary||u.body||"No summary provided.").trim(),o=(u.content||"").trim(),l=(u.category||"architecture").trim(),d=(u.related_services||u.affected_services||[]).map(z=>z.trim()),f=(u.implementation_path||[]).map(z=>z.trim()),x=u.source_event_ids||[],b=u.approved===!0,h=new Date(s.timestamp*1e3),E=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),N=ye.has(s.id),_=N?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${N?"expanded":""} ${b?"blueprint-approved":""}`,L.dataset.blueprintId=s.id,b&&(L.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",L.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let V=b?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[l]||"bx-paint";L.onclick=function(z){let A=this;if(A.classList.contains("expanded")){A.classList.remove("expanded"),ye.delete(s.id);let F=A.querySelector(".event-details");F&&(F.style.display="none")}else{A.classList.add("expanded"),ye.add(s.id);let F=A.querySelector(".event-details");F&&(F.style.display="block")}};let J="";f.length>0&&(J=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${f.map(z=>`<li style="margin-bottom: 5px;">${P(z)}</li>`).join("")}</ul></div>
                    </div>
                `);let K="";x.length>0&&(K=`
                ...
                `);let M=d.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${d.join(", ")}</span></div>`:"<div></div>",T=H(),k=b?`
            ...
            `;L.innerHTML=`
                ${b?'<div class="blueprint-sparkle"></div>':""}
                <div class="event-time">
                    <span class="event-time-main">${E}</span>
                    <span class="event-date">${$}</span>
                </div>
                <div class="event-icon" style="${b?"color: #03dac6;":""}"><i class='bx ${V}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${_}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${P(S)}</div>
            ...
            `;let I=L.querySelector(".blueprint-approve-btn");I&&(I.onclick=async z=>{z.stopPropagation(),I.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(A){console.error("Failed to approve blueprint:",A)}});let R=L.querySelector(".blueprint-delete-btn");R&&(R.onclick=async z=>{z.stopPropagation();let A=!b;R.innerHTML=A?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${s.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let W=L.querySelector(".event-details");W&&W.addEventListener("click",z=>{z.stopPropagation()});let D=L.querySelector(".close-details-btn");return D&&D.addEventListener("click",z=>{z.stopPropagation(),L.classList.remove("expanded");let A=L.querySelector(".event-details");A&&(A.style.display="none"),ye.delete(s.id)}),L},g=s=>{let u=document.createElement("div");u.innerHTML=U("empty",s.message,s.action,s.id);let w=u.firstElementChild;return w.dataset.blueprintId=s.id,w},p=[];c.length===0?p.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):c.forEach(s=>p.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let v=Array.from(e.children),C=new Map(v.map(s=>[s.dataset.blueprintId,s])),m=new Set(p.map(s=>s.id));v.forEach(s=>{let u=s.dataset.blueprintId;(!u||!m.has(u))&&s.remove()});let y=null;p.forEach((s,u)=>{let w=C.get(s.id);(!w||t)&&(w&&w.remove(),s.type==="placeholder"?w=g(s):w=r(s),!w)||(u===0?e.firstElementChild!==w&&e.prepend(w):y&&y.nextElementSibling!==w&&y.after(w),y=w)}),Ne(2,c.length),ht()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=U("offline","Failed to load blueprints.","The event service may be offline."))}}async function It(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&a++}),xt(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(i=>ye.add(i.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var re="STANDBY",Bt=null,Se=[],Y=null,je=null,Ht=()=>`
...
`,zt=()=>`
    ...
    `,Ae=[],Ie={"313071000877137920":"Owen",dexter:"Dexter"},te=[],he=null;async function Be(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),c=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),g=document.getElementById("task-filter-owner"),p=document.getElementById("new-chore-instruction"),v=document.getElementById("new-chore-url"),C=document.getElementById("new-chore-schedule"),m=document.getElementById("form-title"),y=document.getElementById("form-icon");function s(){if(r){if(te.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=te.map(d=>{let f=Ie[d]||d,x=d.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${d==="dexter"?"#bb86fc":x?"#03dac6":"#fff"};">
          <i class='bx ${d==="dexter"?"bx-terminal":x?"bx-hash":"bx-user"}'></i>
          <span>${f}</span>
          <i class='bx bx-x remove-recipient' data-id="${d}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(d=>{d.onclick=()=>{let f=d.dataset.id;te=te.filter(x=>x!==f),s()}})}}function u(d){d&&!te.includes(d)&&(te.push(d),s()),c&&(c.value="")}function w(d=null){n&&(he=d?d.id:null,d?(m&&(m.textContent="Update Research Task"),y&&(y.className="bx bx-edit-alt",y.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),p&&(p.value=d.natural_instruction),v&&(v.value=d.execution_plan?.entry_url||""),C&&(C.value=d.schedule),te=d.recipients||(d.owner_id?[d.owner_id]:[]),s()):(m&&(m.textContent="Initialize Research Task"),y&&(y.className="bx bx-plus-circle",y.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),p&&(p.value=""),v&&(v.value=""),C&&(C.value="every_24h"),te=[],s()),n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"}))}let S=document.getElementById("contacts-group"),o=document.getElementById("channels-group");if(S&&!S.dataset.populated&&!H())try{fe("/contacts").then(async d=>{d.ok&&(((await d.json()).members||[]).forEach(b=>{if(Ie[b.id]=b.nickname||b.username,b.id==="313071000877137920")return;let h=document.createElement("option");h.value=b.id;let E=b.nickname||b.username;h.textContent=`${E} (${b.username})`,S.appendChild(h)}),S.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(o&&!o.dataset.populated&&!H())try{fe("/channels").then(async d=>{d.ok&&((await d.json()).forEach(x=>{let b=`channel:${x.id}`;Ie[b]=x.name;let h=document.createElement("option");h.value=b,h.textContent=`#${x.name} (${x.guild})`,o.appendChild(h)}),o.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}c&&!c.dataset.listenerAttached&&(c.onchange=()=>{u(c.value)},c.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onchange=()=>l(),g.dataset.listenerAttached="true");function l(){if(!t)return;let d=g?.value||"all",f=d==="all"?Ae:Ae.filter(b=>(b.recipients||(b.owner_id?[b.owner_id]:[])).includes(d));if(f.length===0){t.innerHTML=U("empty",d==="all"?"No active tasks.":"No tasks found for this owner.",H()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let x=f.map(b=>{let h=b.last_run===0?"Never":new Date(b.last_run*1e3).toLocaleString(),E=b.memory?b.memory.length:0,$=b.status==="active"?"#03dac6":"#666",_=(b.recipients||(b.owner_id?[b.owner_id]:[])).map(L=>{let O=Ie[L]||L.substring(0,8),V=L.startsWith("channel:");return`<span title="${O}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${L==="dexter"?"bx-terminal":V?"bx-hash":"bx-user"}'></i>${O}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${$}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    <div class="service-widget-header" style="display: flex; align-items: flex-start; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 15px; margin-bottom: 15px;">
                        <div style="display: flex; gap: 15px; flex: 1;">
                            <i class='bx bx-search-alt' style="color: ${$}; font-size: 1.5em; margin-top: 2px;"></i>
                            <div style="text-align: left;">
                                <h3 style="font-size: 1.1em; white-space: normal; line-height: 1.4; font-weight: 600; margin: 0;">${b.natural_instruction}</h3>
                                <div style="margin-top: 8px; display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                                    ...
                                    <span style="font-size: 0.7em; color: #666; font-family: 'JetBrains Mono', monospace;"><i class='bx bx-time' style="margin-right: 4px;"></i>${b.schedule}</span>
                                </div>
                            </div>
                        </div>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <button class="icon-btn reset-chore-btn" data-id="${b.id}" title="Reset Progress" style="background: none; border: none; color: #ff9800; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-refresh' style="font-size: 1.2em;"></i></button>
                            <button class="icon-btn edit-chore-btn" data-id="${b.id}" style="background: none; border: none; color: #bb86fc; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-edit-alt' style="font-size: 1.2em;"></i></button>
                            <button class="icon-btn delete-chore-btn" data-id="${b.id}" style="background: none; border: none; color: #cf6679; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-trash' style="font-size: 1.2em;"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px; flex: 1; min-width: 200px;">
                            ...
                            <span style="font-size: 0.8em; color: #888; font-family: 'JetBrains Mono'; word-break: break-all;">${b.execution_plan.entry_url||"Autonomous Detection"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=x,t.querySelectorAll(".edit-chore-btn").forEach(b=>{b.onclick=h=>{h.stopPropagation();let E=b.dataset.id,$=Ae.find(N=>N.id===E);$&&w($)}}),t.querySelectorAll(".reset-chore-btn").forEach(b=>{b.onclick=async h=>{h.stopPropagation();let E=b.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(b.innerHTML="<i class='bx bx-loader-alt spin'></i>",await B(`/chores/${E}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),Be())}}),t.querySelectorAll(".delete-chore-btn").forEach(b=>{b.onclick=async h=>{h.stopPropagation();let E=b.dataset.id;confirm("Delete this task?")&&(await B(`/chores/${E}`,{method:"DELETE"}),Be())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||he!==null?w(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let d=document.getElementById("new-chore-instruction"),f=document.getElementById("new-chore-url"),x=document.getElementById("new-chore-schedule"),b=d?.value,h=x?.value||"every_24h";if(!b)return;if(te.length===0){alert("Please add at least one recipient.");return}let E=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let $=he?"PATCH":"POST",N=he?`/chores/${he}`:"/chores";await B(N,{method:$,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:te,natural_instruction:b,entry_url:f?.value,schedule:h})}),n&&(n.style.display="none"),d&&(d.value=""),f&&(f.value=""),he=null,te=[],Be()}catch($){console.error($),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=E}},i.dataset.listenerAttached="true");try{let d=await B("/chores");if(!d.ok)throw new Error(`HTTP error! status: ${d.status}`);if(Ae=await d.json()||[],g){let x=g.value,b=new Set;Ae.forEach(h=>{h.owner_id&&b.add(h.owner_id),h.recipients&&h.recipients.forEach(E=>b.add(E))}),g.innerHTML='<option value="all">All Recipients</option>',b.forEach(h=>{let E=document.createElement("option");E.value=h;let $=h.startsWith("channel:"),N=Ie[h]||`Recipient: ${h.substring(0,8)}`;E.textContent=($&&!N.startsWith("#")?"#":"")+N,g.appendChild(E)}),g.value=x,g.selectedIndex===-1&&(g.value="all")}l()}catch(d){console.error("Failed to fetch chores",d)}}var Nt=()=>`
...
`;function jt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Ut(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",fe(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();Qe(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=jt(t.id,t.username);Qe(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=jt(t.id,t.username);Qe(e,t,i)})}function Qe(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let w=(n.attributes||n.facts||[]).map(x=>{let b=x.key||x.k,h=x.value||x.v;return`
                    ...
                    <div class="cog-metric">
                     ...
                     <div class="cog-metric">
                        <div class="cog-label">
                            ...
                            <span style="font-family: 'JetBrains Mono'; color: #bb86fc;">${f}</span>
                        </div>
                    </div>
                </div>
        ...
        `},c=()=>{let w=n.dossier||{},S=w.identity||{},o=w.career||{},l=w.personal||{},d=w.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${S.fullName||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Age Range</div>
                        <div class="dossier-value">${S.ageRange||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Location</div>
                        <div class="dossier-value">${S.location||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Gender</div>
                        <div class="dossier-value">${S.gender||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Sexuality</div>
                        <div class="dossier-value" style="color: #ffb74d;">${S.sexuality||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Relationship Status</div>
                        <div class="dossier-value">${S.relationship||"Unknown"}</div>
                    </div>
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(o.skills||[]).map(f=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${f}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.hobbies||[]).map(f=>`<div class="dossier-list-item">${f}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.habits||[]).map(f=>`<div class="dossier-list-item">${f}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(l.vices||[]).map(f=>`<div class="dossier-list-item" style="color: #cf6679;">${f}</div>`).join("")}
                        </div>
                    </div>
                </div>
    ...
    `,v=`
        <div class="profile-card">
            ...
            <div class="profile-body">
                ...
                <div id="tab-raw" class="tab-content">${p()}</div>
            </div>
    ...
    `;t.innerHTML=v;let C=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),y=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content"),u=t.querySelector(".close-profile-btn");u&&u.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),y.forEach(w=>{w.addEventListener("click",()=>{y.forEach(l=>l.classList.remove("active")),s.forEach(l=>l.classList.remove("active")),w.classList.add("active");let S=w.dataset.tab,o=t.querySelector(`#tab-${S}`);o&&o.classList.add("active")})}),m&&m.addEventListener("click",()=>{C.classList.toggle("expanded");let w=C.classList.contains("expanded");m.innerHTML=w?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var qt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function et(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await et(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let a=n.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let c=JSON.parse(a.dataset.user);Ut(c)}catch(c){console.error("Failed to parse user data",c)}},t.dataset.listenerAttached="true");try{let n=await fe("/contacts");if(!n.ok)throw new Error("Service unreachable");let a=(await n.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=U("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((r,g)=>{let p=c[r.level]??10,v=c[g.level]??10;return p!==v?p-v:r.username.localeCompare(g.username)}),t.innerHTML=a.map(r=>{let g=r.color&&r.color!==0,p=g?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",C="#888";r.level==="Me"||r.level==="Master"?C="#bb86fc":r.level==="Admin"?C="#cf6679":r.level==="Moderator"?C="#03dac6":r.level==="Contributor"&&(C="#ffa500");let m=r.level==="Me",y=m?"rgba(187, 134, 252, 0.08)":g?`${p}11`:"rgba(255,255,255,0.03)",s=m?"2px solid #bb86fc":g?`1px solid ${p}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${m?"#bb86fc":g?p:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${m?"2px solid #bb86fc":g?`2px solid ${p}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${g?p:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.nickname||r.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${C}; font-weight: 600; text-transform: uppercase;">${m?"DEXTER (ME)":r.level}</span>
...
`,Ue=null,we=new Set,He=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await B(i);if(!a.ok)throw new Error("Service unreachable");if(He=((await a.json()).events||[]).filter(y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ue=Date.now(),ee(1,Ue),He.length===0){e.innerHTML=U("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,w=Os(u),S=js(u),o=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),l="event-border-grey";o&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?l="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"?l="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?l="event-border-orange":u==="system.test.completed"?l=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let d=o?"cursor-pointer":"",f=we.has(y.id),x=f?"expanded":"",b=f?"display: block;":"display: none;",h=new Date(y.timestamp*1e3),E=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),N=Ft(u,s),_=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",L="";if(o){let M="";if(u==="engagement.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                    ...
                    `}else if(u==="system.attention.expired"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k=s.timestamp-s.last_active,I=Math.floor(k/60);M=`
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k="";s.eval_count&&(k=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let I="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,W=s.chat_history.map((D,z)=>{let A=D.name?D.name.toUpperCase():D.role.toUpperCase();!D.name&&A==="ASSISTANT"&&(A="DEXTER");let q=D.role==="user"?"#03dac6":D.role==="system"?"#ffb74d":"#bb86fc",F=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${F};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${q}; letter-spacing: 1px; font-weight: bold;">${A}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${z+1} of ${R}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(D.content)}</div>
                                </div>
                            `}).join("");I=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(u==="analysis.user.message_signals"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,k=s.signals||{};M=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.router.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let T=s.success?"#03dac6":"#ff4d4d",k=s.success?"SUCCESS":"FAILED",I=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,R="";s.error&&(R=`
                            <div class="event-detail-block">
                                ${I("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(s.error)}</pre>
                            </div>
                        `);let W="";if(s.chat_history&&s.chat_history.length>0){let A=s.chat_history.length,q=s.chat_history.map((F,se)=>{let G=F.name?F.name.toUpperCase():F.role.toUpperCase();!F.name&&G==="USER"&&(G="SYSTEM"),!F.name&&G==="ASSISTANT"&&(G="AGENT");let le=F.role==="user"?"#03dac6":F.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${se}" style="display: ${se===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${le}; letter-spacing: 1px; font-weight: bold;">${G}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${se+1} of ${A}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(F.content)}</div>
                                </div>
                            `}).join("");W=`
                            <div class="event-detail-block">
                                ...
                                <div class="history-carousel" style="position: relative; background: rgba(255,255,255,0.03); border-radius: 4px; padding: 15px;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                                        ...
                                        <button class="carousel-btn next-btn" style="background: rgba(255,255,255,0.1); border: none; color: #fff; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.8em; transition: background 0.2s;" ${A<=1?"disabled":""}>Next <i class='bx bx-chevron-right'></i></button>
                                    </div>
                                </div>
                            </div>
                        `}let D="";if(s.corrections&&s.corrections.length>0){let A=s.corrections.map((q,F)=>`
                        ...
                        `}let z="";if(s.parsed_results&&s.parsed_results.length>0){let A=s.parsed_results.map(q=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${T}; font-weight: bold;">${k} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${D}
                        ${W}
                    `}else if(u==="system.test.completed"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(u==="messaging.user.sent_message"){let T="";s.attachments&&s.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(I=>{let R=I.content_type&&I.content_type.startsWith("image/"),W=(I.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${R?`<div class="attachment-thumb-container"><img src="${I.url}" alt="${I.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${I.url}" target="_blank" class="attachment-link">${I.filename}</a>
                                        <span class="attachment-meta">${I.content_type} \u2022 ${W}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),M=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${l} ${d} ${x}`,O.dataset.eventId=y.id,O.onclick=function(M){if(!o)return;let T=this;if(T.classList.contains("expanded")){T.classList.remove("expanded"),we.delete(y.id);let I=T.querySelector(".event-details");I&&(I.style.display="none")}else{T.classList.add("expanded"),we.add(y.id);let I=T.querySelector(".event-details");I&&(I.style.display="block")}},O.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${E}</span>
                    <span class="event-date">${$}</span>
                </div>
                <div class="event-icon"><i class='bx ${S}'></i></div>
                <div class="event-content">
                    <div class="event-service">
...
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let g=r,p=unescape(g.dataset.logs||"");navigator.clipboard.writeText(p).then(()=>{let v=g.querySelector("i");v&&(v.classList.remove("bx-copy"),v.classList.add("bx-check"),setTimeout(()=>{v.classList.remove("bx-check"),v.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let p=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${p}?`))try{(await B(`/logs?service_id=${p}`,{method:"DELETE"})).ok&&_e()}catch(v){console.error("Error clearing logs:",v)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=U("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Zt=()=>{let t=H()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(o.GPU&&o.GPU.length>0?c.innerHTML=o.GPU.map(l=>{let d=(l.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${l.LABEL}</span>
                            <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${d} <span style="font-size: 0.6em; color: #888; font-weight: normal;">GB VRAM</span></span>
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),r&&o.STORAGE&&o.STORAGE.length>0?r.innerHTML=o.STORAGE.map(l=>{let d=(l.USED/1073741824).toFixed(1),f=(l.SIZE/(1024*1024*1024)).toFixed(1),x=l.SIZE>0?(l.USED/l.SIZE*100).toFixed(0):0,b=l.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            ...
                            <span style="font-size: 0.75em; color: #666; font-family: monospace; background: rgba(0,0,0,0.2); padding: 2px 6px; border-radius: 4px;">${b}</span>
                        </div>
                        ...
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${Number(x)>90?"#cf6679":"#03dac6"}; width: ${x}%; height: 100%; box-shadow: 0 0 10px ${Number(x)>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await Kt();o?(g(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let o=await Kt();g(o)}if(!t)return;let p=await ns();if(!p||!p.services){t.children.length===0&&(t.innerHTML=U("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let l=o.querySelector(".service-widget-body");l&&(l.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Gt=H()&&oe.lastDashboard||Date.now(),ee(0,Gt||0);let v=p.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let f=59-new Date().getSeconds();f<=0&&(f+=60),o.textContent=`${f}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function C(o){if(!o||o==="N/A"||o==="unknown")return"-";let l=o.match(/^(\d+\.\d+\.\d+)/);return l?l[0]:o.split(".").slice(0,3).join(".")||"-"}function m(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function y(o){if(!o||o==="N/A"||o==="unknown")return"-";let l=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!l)return"-";let d=parseInt(l[1])||0,f=parseInt(l[2])||0,x=parseInt(l[3])||0,b=parseFloat(l[4])||0,h=d*86400+f*3600+x*60+b,E=Math.floor(h/86400);if(E>0)return`${E}d`;let $=Math.floor(h/3600);if($>0)return`${$}h`;let N=Math.floor(h/60);return N>0?`${N}m`:`${Math.floor(h)}s`}function s(o){if(o.id==="upstash-redis-ro"){let K=H()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let l=o.status==="online",d=l?"service-widget-online":"service-widget-offline",f=l?"bx-check-circle":"bx-x-circle",x=l?"OK":"BAD",b=o.version?C(o.version.str):"-",h=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",E=o.memory&&o.memory!=="N/A"?o.memory:"-",$=h!=="-"?"#00ff00":"#666",N=h!=="-"?"#fff":"#666",_=E!=="-"?"#00ff00":"#666",L=E!=="-"?"#fff":"#666",O=y(o.uptime),V="";l?V=`
                <div class="service-widget-info">
                    <span class="info-label">Version:</span>
                    <span class="info-value metric-version-monospace">${b}</span>
                </div>
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${N};">${h}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${_};"></i>
                        <span style="color: ${L};">${E}</span>
                    </div>
                </div>`:V='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let J=H()?"easter.company":m(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${d}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${f}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${J}</span></div>${V}</div></div>`}let u=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),w=new Set(v.map(o=>o.id));for(let[o,l]of u)w.has(o)||l.remove();let S=new Set;v.forEach(o=>{if(S.has(o.id))return;S.add(o.id);let l=s(o),d=u.get(o.id);if(d&&d.parentNode)if(o.id==="upstash-redis-ro"){let f=H()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),x=new Date(f).toLocaleTimeString(),b=d.querySelector(".sync-time-label");b&&(b.textContent=`Last synced: ${x}`)}else d.outerHTML!==l&&(d.outerHTML=l);else t.insertAdjacentHTML("beforeend",l)})}async function it(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=U("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),ee(2,Vt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function a(p){let v=p.status==="Ready";return`
                ...
                </div>`}function c(p){let v=p.status==="Ready";return`
                ...
                </div>`}function r(p){let v=p.status==="Downloaded",C=v?"service-widget-online":"service-widget-offline",m=v?"OK":"MISSING",y=v&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",s=p.name;return p.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${C}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${y}</span></div></div></div>`}let g="";if(i&&(g+=a(i)),e.xtts&&(g+=c(e.xtts)),g+=n.map(r).join(""),!g){t.innerHTML=U("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),p=document.getElementById("analyzer-reset-btn"),v=document.getElementById("fabricator-reset-btn"),C=document.getElementById("courier-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let f=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B(f,{method:"POST"}),await ne()}catch{m.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},m.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{p.innerHTML="<i class='bx bx-error'></i>"}},p.dataset.listenerAttached="true"),v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),C&&!C.dataset.listenerAttached&&(C.onclick=async()=>{C.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{C.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{C.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{C.innerHTML="<i class='bx bx-error'></i>"}},C.dataset.listenerAttached="true");let y=await Gs();if(y&&y.agents){let d=y.agents.guardian||{protocols:{}},f=y.agents.analyzer||{protocols:{}},x=y.agents.imaginator||{protocols:{}},b=y.agents.fabricator||{protocols:{}},h=y.agents.courier||{protocols:{}},E=y.system||{},$=Math.floor(Date.now()/1e3),N={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},_=D=>{D<0&&(D=0);let z=Math.floor(D/3600),A=Math.floor(D%3600/60),q=D%60;return z>0?`${z}h ${A}m`:A>0?`${A}m ${q}s`:`${q}s`},L=(D,z,A,q)=>{if(!D)return;let F=N[q]||q.toUpperCase(),se=D.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(se&&(se.textContent=F),!A){D.textContent="Inactive",D.style.color="#666";return}let G=A.status;if(G==="Working")D.textContent="Working",D.style.color="#bb86fc";else if(G==="Ready")D.textContent="Ready",D.style.color="#5eff5e";else{let le=A.cooldown;if(H()&&(le=A.next_run-$),le<=0)D.textContent="Ready",D.style.color="#5eff5e";else{let Je=Math.floor(le/60),We=le%60;D.textContent=`${Je}m ${We}s`,D.style.color="#fff"}}z&&A.stats&&(z.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
              <span>Runs: ${A.stats.runs||0}</span>
              <span style="color: ${A.stats.failures>0?"#ffa500":"#666"}">Fails: ${A.stats.failures||0}</span>
              <span style="color: ${A.stats.aborted>0?"#ff4d4d":"#666"}">Aborted: ${A.stats.aborted||0}</span>
            </div>
          `)};n&&L(n,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry");let O=document.getElementById("analyzer-synthesis-val"),V=document.getElementById("analyzer-synthesis-stats");O&&L(O,V,f.protocols.synthesis,"synthesis");let J=document.getElementById("imaginator-alert_review-val"),K=document.getElementById("imaginator-alert_review-stats");J&&L(J,K,x.protocols.alert_review,"alert_review");let M=document.getElementById("fabricator-construction-val"),T=document.getElementById("fabricator-construction-stats");M&&L(M,T,b.protocols.construction,"construction");let k=document.getElementById("courier-researcher-val"),I=document.getElementById("courier-researcher-stats");k&&L(k,I,h.protocols.researcher,"researcher");let R=document.getElementById("system-state-label"),W=document.getElementById("system-state-val");if(W&&E.state){let D=E.state,z=_(E.state_time||0);R&&(R.textContent=`State: ${D.toUpperCase()}`),W.textContent=z,D==="idle"?W.style.color=E.state_time>300?"#5eff5e":"#fff":W.style.color="#bb86fc",m&&(D==="paused"?(m.innerHTML="<i class='bx bx-play'></i>",m.title="Resume System",m.style.color="#ff9800"):(m.innerHTML="<i class='bx bx-pause'></i>",m.title="Pause System",m.style.color=""))}a&&(a.textContent=_(E.metrics?.total_idle_time||0)),c&&(c.textContent=_(E.metrics?.total_active_time||0)),r&&(r.textContent=_(E.metrics?.total_waste_time||0))}else[n,i,a,c,r].forEach(f=>{f&&(f.textContent="-",f.style.color="#666")});if(t)return;let s=await Ws(),u=[],w=[],S=[];s&&(Array.isArray(s)?u=s:(u=s.active||[],w=s.queue||[],S=s.history||[],S.sort((d,f)=>(f.end_time||0)-(d.end_time||0)))),e&&(u.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=U("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),st(e,u,!1)));let o=document.getElementById("processes-queue-widgets");o&&(w.length===0?!o.querySelector(".tab-placeholder")&&!o.querySelector('div[style*="font-style: italic"]')&&(o.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(o.innerHTML="",st(o,w,!1)));let l=document.getElementById("processes-history-widgets");l&&(S.length===0?l.querySelector(".tab-placeholder")||(l.innerHTML=U("empty","No recent history.")):(l.querySelector(".tab-placeholder")&&(l.innerHTML=""),st(l,S,!0))),Ne(1,u.length)}function st(t,e,n){function i(p){let v="";p.end_time?v=`${p.end_time-p.start_time}s`:v=`${Math.floor(Date.now()/1e3-p.start_time)}s`;let C=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",m=p.channel_id,y={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(y[m]?m=y[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),n)return`
                ...
                <div class="service-widget process-widget" data-channel-id="${p.channel_id}-${p.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${s}">${p.state}</span>
                        </div>
                        <div class="service-widget-info">
                            <span class="info-label">Duration:</span>
                            <span class="info-value" style="color: ${s}">${v}</span>
                        </div>
                         <div class="service-widget-info">
                            <span class="info-label">PID:</span>
                            <span class="info-value" style="color: ${s}">${p.pid}</span>
                        </div>
                    </div>
                </div>`}let a=n?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(a)).map(p=>[p.dataset.channelId,p])),r=new Set(e.map(p=>`${p.channel_id}-${p.start_time}`));for(let[p,v]of c)p&&!r.has(p)&&v.remove();let g=new Set;e.forEach(p=>{let v=`${p.channel_id}-${p.start_time}`;if(g.has(v))return;g.add(v);let C=i(p),m=c.get(v);if(m&&m.parentNode){m.outerHTML!==C&&(m.outerHTML=C);let y=t.querySelector(`[data-channel-id="${v}"]`);y&&t.appendChild(y)}else t.insertAdjacentHTML("beforeend",C)})}function Fe(){let t=Ve(),e="Notification"in window,n={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function os(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await is(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await is(t)}async function is(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let n=await e.json();if(!n||n.length===0){t.innerHTML=U("empty","No web history found.");return}let i=`
        ...
        `;n.forEach((g,p)=>{let v=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),C=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
                <div class="web-card" style="flex: 0 0 100%; height: 100%; display: flex; flex-direction: column; scroll-snap-align: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-sizing: border-box; max-width: 100%; overflow: hidden;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; flex-shrink: 0;">
                        <span style="color: #03dac6; font-weight: bold; font-family: monospace;">#${p+1}</span>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Vs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[n,i]of Object.entries(t)){let a=cs.filter(c=>c.category===n);a.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let p=document.getElementById("close-terminal-btn");p&&(p.onclick=()=>rs());let v=document.getElementById("terminal-action-btn");v&&(v.onclick=()=>rs())}let n=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");n&&(n.innerHTML="");let a=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let c=document.getElementById("terminal-cmd-name");c&&(c.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let g=document.getElementById("terminal-action-btn");return g&&(g.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",ot=!0,n}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",ot=!1}function ls(t,e,n="output"){if(!ot||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Re(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Zs(t){let e=cs.find(r=>r.id===t);if(!e)return;let n=Ks(e);n&&ls(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let r of i){await new Promise(p=>setTimeout(p,400+Math.random()*600));let g="output";r.includes("[ERROR]")?g="error":r.includes("[SUCCESS]")||r.includes("\u2713")?g="success":r.includes("!")&&(g="warning"),n&&ls(n,r,g)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let c=document.getElementById("terminal-action-btn");c&&(c.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Vs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Zs(a)})})}async function Ys(){if(!H())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}dt(),pt(),Ys(),bt(),ct();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=o=>{localStorage.setItem("dex_last_window",o)};function a(){for(;e.length>1;)e.shift().close(!0);let o=document.getElementById("windows-container"),l=document.querySelector("nav"),d=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");o&&o.setAttribute("data-count",e.length.toString()),e.forEach(_=>{let L=document.getElementById(_.id);L&&L.classList.remove("hide-close")});let b=document.getElementById("dexter-menu-container"),h=document.getElementById("nav-window-switcher"),E=document.getElementById("settings-icon"),$=document.getElementById("close-all-windows"),N=window.innerWidth<880;if(!N){let _=document.getElementById("dexter-dropdown"),L=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),L&&L.classList.remove("active"),o&&o.classList.remove("menu-open")}if(Ce(e.length>0),e.length>0)if(d?.classList.add("hide"),$&&($.style.display=N?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",l?.classList.add("window-open"),o&&(o.style.paddingTop="60px"),b&&(b.style.display=N?"flex":"none"),E&&(E.style.display=N?"block":"none"),!N&&h){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(h.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),c(g)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),c(p)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),c(C)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),c(v)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),c(m)})):h.innerHTML=""}else h&&(h.innerHTML="");else l?.classList.remove("window-open"),$&&($.style.display="none"),o&&(o.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?d?.classList.remove("hide"):d?.classList.add("hide"),b&&(b.style.display="flex"),E&&(E.style.display="block"),h&&(h.innerHTML="");$e()}function c(o){if(o.isOpen()){o.close();return}for(;e.length>0;)e.pop().close(!0);e.push(o),o.open(),a()}function r(){[...e].forEach(l=>l.close()),e=[]}window.addEventListener("resize",a);let g=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Et(),onOpen:()=>de(),onClose:()=>{let o=e.indexOf(g);o>-1&&e.splice(o,1),a()}}),p=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:tt(),onOpen:()=>{p.setContent(tt()),pe()},onClose:()=>{let o=e.indexOf(p);o>-1&&e.splice(o,1),a()}}),v=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:qt(),onOpen:()=>et(),onClose:()=>{let o=e.indexOf(v);o>-1&&e.splice(o,1),a()}}),C=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(o=>H()?o.title!=="Hardware"&&o.title!=="Logs":!0),onOpen:()=>{nt(),ne(),it(),os(),_e()},onClose:()=>{let o=e.indexOf(C);o>-1&&e.splice(o,1),a()}}),m=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Nt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Pt()},{icon:"bx-task",title:"Tasks",content:Ot()}],onOpen:()=>Xe(),onClose:()=>{let o=e.indexOf(m);o>-1&&e.splice(o,1),a()}}),y=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:Fe(),onOpen:()=>{y.setContent(Fe()),at(y)},onClose:()=>{let o=e.indexOf(y);o>-1&&e.splice(o,1),a()}});window.dexter={viewEvent:o=>{p.isOpen()||c(p),setTimeout(()=>{let l=document.querySelector(`.event-item[data-event-id="${o}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)},viewAlert:o=>{g.isOpen()||c(g),setTimeout(()=>{let l=document.querySelector(`.event-item[data-alert-id="${o}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let o=document.getElementById("dexter-dropdown"),l=document.getElementById("dexter-menu-btn");o&&l&&(o.classList.remove("active"),l.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let o=document.getElementById("dexter-menu-btn"),l=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),b=document.getElementById("windows-container");o&&x&&(o.onclick=E=>{E.stopPropagation();let $=window.innerWidth<880;if($)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let _=o.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),V=220,J=10,M=_.left+_.width/2-V/2,T=O.right-10;M+V>T&&(M=T-V),M<10&&(M=10),x.style.top=O.bottom+J+"px",x.style.left=M+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),o.classList.toggle("active");let N=x.classList.contains("active");if($){let _=document.querySelector("nav");N?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),b?.classList.add("menu-open"),Ce(!0)):(b?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),Ce(!1)))}}),l&&(l.onclick=E=>{E.stopPropagation(),c(y)}),d&&(d.onclick=E=>{E.stopPropagation(),r()});let h=(E,$,N)=>{let _=document.getElementById(E);_&&(_.onclick=L=>{L.stopPropagation(),s(),N&&i(N),c($)})};h("alerts-menu-item",g,"alerts-window"),h("events-menu-item",p,"events-window"),h("monitor-menu-item",C,"monitor-window"),h("contacts-menu-item",v,"contacts-window"),h("workspace-menu-item",m,"workspace-window"),f&&(f.onclick=E=>{if(E.stopPropagation(),x&&x.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1));return}if(e.length>0)r();else{let $=window.location.pathname;if(!($==="/"||$==="/index.html")){let L=($.endsWith("/")&&$.length>1?$.slice(0,-1):$).split("/");L.pop();let O=L.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let E=window.innerWidth<880;x&&x.classList.contains("active")&&(s(),E&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1)))}),x&&(x.onclick=E=>E.stopPropagation())})();let w=window.location.pathname==="/"||window.location.pathname==="/index.html",S=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!w&&!S&&document.querySelector("footer")?.classList.add("hide"),H()||setInterval(()=>{p.isOpen()&&pe(),g.isOpen()?de():Ye()},1e3),setInterval(()=>{H()&&(g.isOpen()?de():Ye()),m.isOpen()?Xe():It(),H()&&p.isOpen()&&pe(),C.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

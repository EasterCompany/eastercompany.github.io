...
`,Mt=()=>`
    ...
    `,_e=[],Be={"313071000877137920":"Owen",dexter:"Dexter"},ie=[],he=null;async function He(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),n=document.getElementById("save-chore-btn"),s=document.getElementById("cancel-chore-btn"),o=document.getElementById("new-chore-owner"),a=document.getElementById("selected-recipients"),p=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),g=document.getElementById("new-chore-url"),$=document.getElementById("new-chore-schedule"),x=document.getElementById("new-chore-time"),h=document.getElementById("new-chore-timezone"),r=document.getElementById("new-chore-time-group");if(h&&!h.dataset.initialValueAttached){let l=Intl.DateTimeFormat().resolvedOptions().timeZone,S=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l){S=!0;break}if(!S){let E=document.createElement("option");E.value=l,E.textContent=l.replace("_"," "),h.appendChild(E)}h.value=l,h.dataset.initialValueAttached="true"}$&&r&&!$.dataset.timeListenerAttached&&($.addEventListener("change",()=>{r.style.display=$.value==="daily"?"block":"none"}),$.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),L=document.getElementById("form-icon");function I(){if(a){if(ie.length===0){a.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}a.innerHTML=ie.sort((l,S)=>{let E=l.startsWith("channel:"),m=S.startsWith("channel:");return E&&!m?-1:!E&&m?1:0}).map(l=>{let S=Be[l]||l,E=l.startsWith("channel:");return`
      ...
      `}).join(""),a.querySelectorAll(".remove-recipient").forEach(l=>{l.onclick=()=>{let S=l.dataset.id;ie=ie.filter(E=>E!==S),I()}})}}function b(l){l&&!ie.includes(l)&&(ie.push(l),I()),o&&(o.value="")}function f(l=null){if(i){if(he=l?l.id:null,l){if(u&&(u.textContent="Update Research Task"),L&&(L.className="bx bx-edit-alt",L.style.color="#03dac6"),n&&(n.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=l.natural_instruction),g&&(g.value=l.execution_plan?.entry_url||""),$&&($.value=l.schedule,r&&(r.style.display=l.schedule==="daily"?"block":"none")),x&&l.run_at&&(x.value=l.run_at),h&&l.timezone){let S=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l.timezone){S=!0;break}if(!S){let E=document.createElement("option");E.value=l.timezone,E.textContent=l.timezone.replace("_"," "),h.appendChild(E)}h.value=l.timezone}ie=l.recipients||(l.owner_id?[l.owner_id]:[]),I()}else u&&(u.textContent="Initialize Research Task"),L&&(L.className="bx bx-plus-circle",L.style.color="#bb86fc"),n&&(n.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),g&&(g.value=""),$&&($.value="every_24h"),ie=[],I();i.style.display="block",i.scrollIntoView({behavior:"smooth",block:"start"})}}let v=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(v&&!v.dataset.populated&&!z())try{ve("/contacts").then(async l=>{l.ok&&(((await l.json()).members||[]).forEach(m=>{if(Be[m.id]=m.nickname||m.username,m.id==="313071000877137920")return;let w=document.createElement("option");w.value=m.id;let A=m.nickname||m.username;w.textContent=`${A} (${m.username})`,v.appendChild(w)}),v.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!z())try{ve("/channels").then(async l=>{l.ok&&((await l.json()).forEach(E=>{let m=`channel:${E.id}`;Be[m]=E.name;let w=document.createElement("option");w.value=m,w.textContent=`#${E.name} (${E.guild})`,y.appendChild(w)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}o&&!o.dataset.listenerAttached&&(o.onchange=()=>{b(o.value)},o.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onchange=()=>d(),p.dataset.listenerAttached="true");function d(){if(!t)return;let l=p?.value||"all",S=l==="all"?_e:_e.filter(m=>(m.recipients||(m.owner_id?[m.owner_id]:[])).includes(l));if(S.length===0){t.innerHTML=W("empty",l==="all"?"No active tasks.":"No tasks found for this owner.",z()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let E=S.map(m=>{let w=m.last_run===0?"Never":new Date(m.last_run*1e3).toLocaleString(),A=m.memory?m.memory.length:0,_=m.status==="active"?"service-widget-online":"service-widget-offline",B=m.status==="active"?"ACTIVE":"PAUSED",G=(m.recipients||(m.owner_id?[m.owner_id]:[])).sort((C,T)=>{let k=C.startsWith("channel:"),M=T.startsWith("channel:");return k&&!M?-1:!k&&M?1:0}).map(C=>{let T=Be[C]||C.substring(0,8),k=C.startsWith("channel:");return`<span title="${T}" style="display: flex; align-items: center; gap: 3px; white-space: nowrap;"><i class='bx ${C==="dexter"?"bx-terminal":k?"bx-hash":"bx-user"}'></i>${T}</span>`}).join("<span style='color: #444; margin: 0 5px;'>|</span>"),Z=`${m.schedule}${m.run_at?" @ "+m.run_at+(m.timezone?" ("+m.timezone.split("/").pop()?.replace("_"," ")+")":""):""}`;return`
                ...
                <div class="user-profile-card" 
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${$}; font-weight: 600; text-transform: uppercase;">${x?"DEXTER (ME)":a.level}</span>
...
`,Je=null,we=new Set,ze=[],Q="all",Ts={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals","system.cognitive.model_inference"],moderation:["moderation.explicit_content.deleted"]},$s={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart","system.cognitive.model_inference":"bx-brain"};function Ss(t){for(let[e,i]of Object.entries(Ts))if(i.includes(t))return e;return"system"}function ks(t){return $s[t]||"bx-square-rounded"}async function ue(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ls();let n=`/events?ml=${Q==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Q!=="all"&&(n+=`&category=${Q}`);try{let s=await H(n);if(!s.ok)throw new Error("Service unreachable");if(ze=((await s.json()).events||[]).filter(h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!0}let u=r.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Je=Date.now(),pe(1,Je),ze.length===0){e.innerHTML=W("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let u=r.type,L=Ss(u),I=ks(u),b=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.cognitive.model_inference"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),f="event-border-grey";b&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?f="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?f="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?f="event-border-orange":u==="system.test.completed"?f=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":f="event-border-blue");let v=b?"cursor-pointer":"",y=we.has(h.id),d=y?"expanded":"",l=y?"display: block;":"display: none;",S=new Date(h.timestamp*1e3),E=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),m=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Bt(u,r),A=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",_="";if(b){let C="";if(u==="engagement.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                    ...
                    `}else if(u==="system.cognitive.model_inference"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                    ...
                    `}else if(u==="system.attention.expired"){let T=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,k=r.timestamp-r.last_active,M=Math.floor(k/60);C=`
                        <div class="event-detail-row">
            ...
            `}let M="";r.eval_count&&(M=`
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let T=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,k="",M=[],N=r.raw_input||"";if(N.startsWith("[{")&&N.endsWith("}]")){let D=N.match(/{.*?}/g);D&&(M=D.map(K=>{let q=K.substring(1,K.length-1),j=q.indexOf(" "),U=q.substring(0,j),V=q.substring(j+1);return V=V.replace(/\[\]$/,"").trim(),{role:U,content:V}}))}if(M.length>0&&r.response_raw&&M.push({role:"assistant",content:r.response_raw}),M.length>0){let D=M.length,K=M.map((q,j)=>{let U=q.role.toUpperCase();U==="ASSISTANT"&&(U="DEXTER");let V=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",ee=j===D-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${ee};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${V}; letter-spacing: 1px; font-weight: bold;">${U}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${j+1} of ${D}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let T=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,k=r.signals||{};C=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.visual.completed"){let T=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,k="";r.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${r.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:r.url?k=`<div class="event-detail-block"><img src="${r.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:k=`
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(u==="analysis.router.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let T=r.success?"#03dac6":"#ff4d4d",k=r.success?"SUCCESS":"FAILED",M=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,N="";r.error&&(N=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(r.error)}</pre>
                            </div>
                        `);let D="";if(r.chat_history&&r.chat_history.length>0){let j=r.chat_history.length,U=r.chat_history.map((V,ee)=>{let Y=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Y==="USER"&&(Y="SYSTEM"),!V.name&&Y==="ASSISTANT"&&(Y="AGENT");let Me=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ee}" style="display: ${ee===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${Me}; letter-spacing: 1px; font-weight: bold;">${Y}</span>
                        ...
                        `).join("");K=`
                            <div class="event-detail-block">
                                ${M(`Corrections (${r.corrections.length})`)}
                                ${j}
                            </div>
                        ...
                        ${K}
                        ${D}
                    `}else if(u==="system.test.completed"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(u==="messaging.user.sent_message"){let T="";r.attachments&&r.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${r.attachments.map(M=>{let N=M.content_type&&M.content_type.startsWith("image/"),D=(M.size/1024).toFixed(1)+" KB";return`
                ...
                `}let B=document.createElement("div");B.className=`event-item ${f} ${v} ${d}`,B.dataset.eventId=h.id,B.onclick=function(C){if(!b)return;let T=this;if(T.classList.contains("expanded")){T.classList.remove("expanded"),we.delete(h.id);let M=T.querySelector(".event-details");M&&(M.style.display="none")}else{T.classList.add("expanded"),we.add(h.id);let M=T.querySelector(".event-details");M&&(M.style.display="block")}},B.innerHTML=`
                <div class="event-time">
...
`),c=[...a.logs];if(c.length<25){let $=25-c.length;for(let x=0;x<$;x++)c.push("")}else c.length>25&&(c=c.slice(-25));let g=c.map($=>qe($)).join(`
`);return`
  ...
  `}async function Nt(){try{let e=await(await H("/processes")).json(),i=e.active||[],n=e.history||[],s=i.find(a=>a.channel_id==="system-fabricator");if(s){Ee="ACTIVE";let p=Math.floor(Date.now()/1e3)-s.start_time,c=10,g="REVIEW";p>30&&(c=30,g="INVESTIGATION"),p>120&&(c=60,g="CONSTRUCTION"),p>300&&(c=90,g="REPORTING"),te={title:s.state||"Fabricating system improvements...",progress:c,phase:g};let $=new Date(s.updated_at*1e3).toLocaleTimeString();(ae.length===0||ae[ae.length-1].msg!==s.state)&&(ae.push({time:$,msg:s.state}),ae.length>50&&ae.shift())}else{let a=n.find(p=>p.channel_id==="system-fabricator");if(a&&Date.now()/1e3-a.end_time<60){Ee="COMPLETED";let p=a.end_time-a.start_time;nt={duration:`${Math.floor(p/60)}m ${p%60}s`,steps:0,result:a.state,timestamp:a.end_time}}else Ee="STANDBY",ae=[]}let o=document.getElementById("progress-view-root");o&&(Ee!==zt&&(o.innerHTML=Rt(),zt=Ee),Ee==="ACTIVE"&&_s())}catch(t){console.error("Failed to update progress:",t)}}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),i=document.getElementById("active-task-progress-text"),n=document.getElementById("active-task-phase"),s=document.getElementById("terminal-output");if(t&&te&&(t.innerText=te.title),e&&te&&(e.style.width=`${te.progress}%`),i&&te&&(i.innerText=`${te.progress}% COMPLETE`),n&&te&&(n.innerText=`PHASE: ${te.phase}`),s){let o=s.children.length;if(ae.length>o){for(let a=o;a<ae.length;a++){let p=ae[a],c=document.createElement("div");c.className="terminal-line",c.innerHTML=`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-category-grid system-monitor-widgets" style="padding-bottom: 20px;">
                ${l.map(S=>y(S)).join("")}
            </div>`)}),t.innerHTML!==I&&(t.innerHTML=I);function b(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/^(\d+\.\d+\.\d+)/);return l?l[0]:d.split(".").slice(0,3).join(".")||"-"}function f(d){return!d||d.length<=22?d:d.substring(0,22)+"..."}function v(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!l)return"-";let S=parseInt(l[1])||0,E=parseInt(l[2])||0,m=parseInt(l[3])||0,w=parseFloat(l[4])||0,A=S*86400+E*3600+m*60+w,_=Math.floor(A/86400);if(_>0)return`${_}d`;let B=Math.floor(A/3600);if(B>0)return`${B}h`;let O=Math.floor(A/60);return O>0?`${O}m`:`${Math.floor(A)}s`}function y(d){if(d.id==="upstash-redis-ro"){let D=z()&&(be.lastFrontend||be.lastDashboard)||Date.now();return`
            ...
            </div>`}let l=d.status==="online",S=l?"service-widget-online":"service-widget-offline",E=l?"bx-check-circle":"bx-x-circle",m=l?"OK":"BAD",w=d.version?b(d.version.str):"-",A=d.cpu&&d.cpu!=="N/A"?d.cpu:"-",_=d.memory&&d.memory!=="N/A"?d.memory:"-",B=A!=="-"?"#00ff00":"#666",O=A!=="-"?"#fff":"#666",G=_!=="-"?"#00ff00":"#666",Z=_!=="-"?"#fff":"#666",C=v(d.uptime),T="",k=d.type!=="os"&&d.type!=="cli"&&d.type!=="prd"&&d.type!=="md",M="";if(k&&!z()&&(M=`
            <div class="service-controls">
                ...
                </div>`}else T='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold; margin-top: auto;"><span>OFFLINE</span></div>';let N=z()||d.type==="md"?"easter.company":f(d.domain&&d.port?`${d.domain}:${d.port}`:"");return`<div class="service-widget ${S}" data-service-id="${d.id}"><div class="service-widget-header"><i class="bx ${E}"></i><h3>${d.short_name||d.id}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${N}</span></div>${T}</div>${M}</div>`}}async function re(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),a=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),g=document.getElementById("fabricator-reset-btn"),$=document.getElementById("fabricator-progress-btn"),x=document.getElementById("courier-reset-btn"),h=document.getElementById("system-pause-toggle-btn");$&&!$.dataset.listenerAttached&&($.onclick=()=>{let d=document.querySelector("nav");d&&(d.style.display="none");let l=le({id:"fabricator-progress-window",title:"Mission Control",icon:"bx-loader-circle",className:"fabricator-progress-modal",appendToBody:!0,content:`
          ...
          `)};i&&O(i,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry",w.state);let G=document.getElementById("analyzer-summary-val"),Z=document.getElementById("analyzer-summary-stats");G&&O(G,Z,l.protocols.summary,"summary",w.state);let C=document.getElementById("analyzer-synthesis-val"),T=document.getElementById("analyzer-synthesis-stats");C&&O(C,T,l.protocols.synthesis,"synthesis",w.state);let k=document.getElementById("imaginator-alert_review-val"),M=document.getElementById("imaginator-alert_review-stats");k&&O(k,M,S.protocols.architect,"architect",w.state);let N=document.getElementById("fabricator-review-val"),D=document.getElementById("fabricator-review-stats");N&&O(N,D,E.protocols.review,"review",w.state);let K=document.getElementById("fabricator-issue-val"),q=document.getElementById("fabricator-issue-stats");K&&O(K,q,E.protocols.issue,"issue",w.state);let j=document.getElementById("fabricator-construct-val"),U=document.getElementById("fabricator-construct-stats");j&&O(j,U,E.protocols.construct,"construct",w.state);let V=document.getElementById("fabricator-reporter-val"),ee=document.getElementById("fabricator-reporter-stats");V&&O(V,ee,E.protocols.reporter,"reporter",w.state);let Y=document.getElementById("courier-researcher-val"),Me=document.getElementById("courier-researcher-stats");Y&&O(Y,Me,m.protocols.researcher,"researcher",w.state);let Re=document.getElementById("courier-compressor-val"),Ve=document.getElementById("courier-compressor-stats");Re&&O(Re,Ve,m.protocols.compressor,"compressor",w.state);let Ne=document.getElementById("system-state-label"),fe=document.getElementById("system-state-val");if(fe&&w.state){let F=w.state,X=B(w.state_time||0);Ne&&(Ne.textContent=`State: ${F.toUpperCase()}`),fe.textContent=X,F==="idle"?fe.style.color=w.state_time>300?"#5eff5e":"#fff":fe.style.color="#bb86fc",h&&(F==="paused"?(h.innerHTML="<i class='bx bx-play'></i>",h.title="Resume System",h.style.color="#ff9800"):(h.innerHTML="<i class='bx bx-pause'></i>",h.title="Pause System",h.style.color=""))}s&&(s.textContent=B(w.metrics?.total_idle_time||0)),o&&(o.textContent=B(w.metrics?.total_active_time||0)),a&&(a.textContent=B(w.metrics?.total_waste_time||0))}else[i,n,s,o,a].forEach(l=>{l&&(l.textContent="-",l.style.color="#666")});if(t)return;let L=await zs(),I=[],b=[],f=[];L&&(Array.isArray(L)?I=L:(I=L.active||[],b=L.queue||[],f=L.history||[],f.sort((d,l)=>(l.end_time||0)-(d.end_time||0)))),e&&(I.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=W("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,I,!1)));let v=document.getElementById("processes-queue-widgets");v&&(b.length===0?!v.querySelector(".tab-placeholder")&&!v.querySelector('div[style*="font-style: italic"]')&&(v.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(v.innerHTML="",it(v,b,!1)));let y=document.getElementById("processes-history-widgets");y&&(f.length===0?y.querySelector(".tab-placeholder")||(y.innerHTML=W("empty","No recent history.")):(y.querySelector(".tab-placeholder")&&(y.innerHTML=""),it(y,f,!0))),yt(1,I.length)}function it(t,e,i){function n(c){let g="";c.end_time?g=`${c.end_time-c.start_time}s`:g=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let $=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",x=c.channel_id,h={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-architect":"Architect Agent","system-fabricator":"Fabricator Agent","system-test":"System Validation","voice-mode":"Voice Interaction","system-context-summary-test-channel-id":"Context Summary Test"};if(h[x]?x=h[x]:/^\d+$/.test(x)&&(x=`Channel ${x}`),i)return`

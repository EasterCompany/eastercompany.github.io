...
`,fe=new Set,kt=[],Me=null;async function be(t=!1){let e=document.getElementById("roadmap-list");if(e){Ss();try{let n=await _("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();kt=i;let a=m=>{let v=fe.has(m.id),s=m.state==="published",u=m.state==="consumed",C="event-border-grey";s&&(C="event-border-blue"),u&&(C="event-border-purple");let f=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),p=document.createElement("div");p.className=`event-item notification-item ${C} cursor-pointer ${v?"expanded":""}`,p.dataset.itemId=m.id,p.onclick=c=>{if(c.target.closest("button"))return;if(p.classList.contains("expanded")){p.classList.remove("expanded");let w=p.querySelector(".event-details");w&&(w.style.display="none"),fe.delete(m.id)}else{p.classList.add("expanded");let w=p.querySelector(".event-details");w&&(w.style.display="block"),fe.add(m.id)}};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;p.innerHTML=`
      ...
      `;let y=p.querySelector(".event-details");return y&&y.addEventListener("click",c=>{c.stopPropagation()}),p.querySelector(".edit-btn")?.addEventListener("click",()=>ks(m)),p.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(m)),p.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(m.id)),p.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),p.classList.remove("expanded");let r=p.querySelector(".event-details");r&&(r.style.display="none"),fe.delete(m.id)}),p},l=m=>{let v=document.createElement("div");v.innerHTML=J("empty",m.message,m.action,m.id);let s=v.firstElementChild;return s.dataset.itemId=m.id,s},o=[];!i||i.length===0?o.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:R()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(m=>o.push({...m,type:"item"}));let b=Array.from(e.children),d=new Map(b.map(m=>[m.dataset.itemId,m])),h=new Set(o.map(m=>m.id));b.forEach(m=>{let v=m.dataset.itemId;(!v||!h.has(v))&&m.remove()}),t&&(e.innerHTML="");let T=null;o.forEach((m,v)=>{let s=d.get(m.id);(!s||t)&&(s&&s.remove(),m.type==="placeholder"?s=l(m):s=a(m),!s)||(v===0?e.firstElementChild!==s&&e.prepend(s):T&&T.nextElementSibling!==s&&T.after(s),T=s)})}catch{e.children.length===0&&(e.innerHTML=J("offline","Failed to load roadmap.","The event service may be offline."))}}}function Ss(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Me=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),o=l?l.value:"";if(!o.trim())return;let b=Me?`/roadmap/${Me}`:"/roadmap",d=Me?"PATCH":"POST";try{await _(b,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})});let h=document.getElementById("roadmap-editor-container");h&&(h.style.display="none"),be(!0)}catch(h){console.error(h)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(l=>fe.add(l.id)),be(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{fe.clear(),be(!0)},a.dataset.listenerAttached="true")}function ks(t){Me=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),be(!0)}catch(n){console.error(n)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),be(!0)}catch(e){console.error(e)}}var Lt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">

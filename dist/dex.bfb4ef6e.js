"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,l=document.createElement("nav");l.innerHTML=a,document.body.prepend(l);let o=document.createElement("div");o.id="dexter-dropdown",o.className="dexter-dropdown",o.innerHTML=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function me(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++ut).toString())}function l(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",o),i&&setTimeout(i,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",a);let v=t.icon||"bx-window",s="",m="",E,H='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((h,y)=>{let r=h.icon?`<i class="bx ${h.icon}"></i>`:`<span class="tab-glyph">${h.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,E=`<div class="window-content">${t.tabs.map((h,y)=>`<div class="tab-content ${y===0?"active":""}" data-tab-content="${y}">${h.content}${H}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),E=`<div class="window-content">${t.content||""}${H}</div>`),e.innerHTML=`
            ...
            ${E}
        `,u.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",b=>{b.stopPropagation(),d()}),window.addEventListener("resize",o),t.tabs&&e.querySelectorAll(".tab").forEach(b=>{b.addEventListener("click",()=>{if(!e)return;let p=b.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(y=>y.classList.remove("active")),b.classList.add("active"),e.querySelectorAll(".tab-content").forEach(y=>y.classList.remove("active"));let h=e.querySelector(`.tab-content[data-tab-content="${p}"]`);h&&h.classList.add("active"),f(b,e)})}),setTimeout(()=>{e&&e.classList.add("open"),i&&i()},10)}function o(){if(!e||!e.classList.contains("open"))return;let u=e.querySelector(".tab.active");u&&f(u,e)}function f(u,v){setTimeout(()=>{let s=v.querySelector(".tab-bar");if(!s)return;let m=Array.from(s.querySelectorAll(".tab")),E=m.indexOf(u),H=s.clientWidth,b=m[Math.max(0,E-2)],p=m[Math.min(m.length-1,E+2)],h=s,y=b.offsetLeft-h.offsetLeft-25,c=p.offsetLeft+p.offsetWidth-h.offsetLeft+25-y,C=c<=H?y-(H-c)/2:u.offsetLeft-h.offsetLeft-H/2+u.offsetWidth/2;s.scrollTo({left:C,behavior:"smooth"})},350)}function d(u=!1){e&&(window.removeEventListener("resize",o),u?(e.remove(),e=null):(e.classList.remove("open"),n&&n(),setTimeout(()=>{e?.remove(),e=null},400)))}function x(u){t.content=u;let v=e?.querySelector(".window-content");v&&(v.innerHTML=u)}function T(){return!!(e&&e.classList.contains("open"))}return{open:l,close:d,setContent:x,isOpen:T,focus:a,id:t.id}}var gt="easter_theme",ee={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Ze(){return localStorage.getItem(gt)||ee.DARK}function ft(t){if(!Object.values(ee).includes(t))throw new Error("Invalid theme");localStorage.setItem(gt,t),bt(t)}function bt(t){let e=document.body;Object.values(ee).forEach(l=>{e.classList.remove(`theme-${l}`)}),e.classList.add(`theme-${t}`);let n=document.querySelector('meta[name="theme-color"]');n||(n=document.createElement("meta"),n.name="theme-color",document.getElementsByTagName("head")[0].appendChild(n));let i={[ee.DARK]:"#050507",[ee.LIGHT]:"#FFFFFF",[ee.LEGACY]:"#000000"},a=i[t]||i[ee.DARK];if(n.setAttribute("content",a),!document.querySelector(".background")){let l=document.createElement("div");l.className="background",document.body.prepend(l)}e.classList.add("is-animated")}function vt(){let t=Ze();bt(t)}function F(t,e,n=null,i="default"){let l={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",o=n?`<p class="placeholder-action">${n}</p>`:"";return`<div class="tab-placeholder" data-placeholder-id="${i}"><i class='bx ${l} placeholder-icon'></i><p class="placeholder-message">${e}</p>${o}</div>`}function j(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function te(t,e){let n=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!n)return;if(!e){n.textContent="Last updated: never";return}let i=Date.now(),a=Math.floor((i-e)/1e3),l;a<60?l=`${a}s ago`:a<3600?l=`${Math.floor(a/60)}m ago`:l=`${Math.floor(a/3600)}h ago`,n.textContent=`Last updated: ${l}`}var us=0;function Re(t,e){let n=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!n)return;let i=n.querySelector(".notification-badge");i&&(e>0?(i.textContent=e>9?"9+":e.toString(),i.style.display="flex"):i.style.display="none")}var ie=0,ae=0;function xt(t){ie=t,Se()}function ht(t){ae=t,Se()}function Se(){let t=ie+ae;us=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let n=document.getElementById("alerts-menu-item");if(n){let o=n.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="auto",n.appendChild(o)),o.textContent=ie>9?"9+":ie.toString(),o.style.display=ie>0?"flex":"none"}let i=document.getElementById("workspace-menu-item");if(i){let o=i.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="auto",i.appendChild(o)),o.textContent=ae>9?"9+":ae.toString(),o.style.display=ae>0?"flex":"none"}let a=document.getElementById("switch-alerts");if(a){let o=a.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="8px",a.appendChild(o)),o.textContent=ie>9?"9+":ie.toString(),o.style.display=ie>0?"flex":"none"}let l=document.getElementById("switch-workspace");if(l){let o=l.querySelector(".notification-badge");o||(o=document.createElement("span"),o.className="notification-badge",o.style.marginLeft="8px",l.appendChild(o)),o.textContent=ae>9?"9+":ae.toString(),o.style.display=ae>0?"flex":"none"}}function Xe(){let t=document.getElementById("alerts-list");if(!t)return;ie=t.querySelectorAll(".alert-unread:not(.priority-low)").length,Se()}function wt(){let t=document.getElementById("blueprints-list");if(!t)return;ae=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,Se()}function gs(){return"https://event.easter.company"}function fs(){return"https://discord.easter.company"}var bs="http://127.0.0.1:8100",vs="http://127.0.0.1:8300",ys={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Oe(t){let e=j(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(a,l)=>{let o=ys[l];return o?`<span class="${o}">`:""});let n=(e.match(/<span/g)||[]).length,i=(e.match(/<\/span/g)||[]).length;return n>i&&(e+="</span>".repeat(n-i)),e}function re(t){if(!t)return"";let e=j(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^<strong>(.*)<\/strong>$/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(n,i)=>{let a=i.split("|").map(l=>l.trim());return a.every(l=>l.match(/^:?-+:?$/))?"":`<div class="md-table-row">${a.map(l=>`<span class="md-table-cell">${l}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}var xs="https://sterling-javelin-12539.upstash.io",hs="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function N(){return window.location.hostname.includes("easter.company")}var q=null,oe={lastDashboard:0,lastFrontend:0},Ye=!1,Ct=0,Et="dex_dashboard_snapshot";function ws(){let t=localStorage.getItem(Et);if(t)try{let e=JSON.parse(t);q=e,oe.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{q=null}}async function yt(){if(!(!N()||Ye)){Ye=!0,Ct=Math.floor(Date.now()/1e3);try{let t=await Ts("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);q=e,oe.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),oe.lastFrontend=Date.now(),localStorage.setItem(Et,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Ye=!1}}}function Cs(){if(!q||!q.agent_status)return;let t=q.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function Es(){if(!N())return;ws();let t=Math.floor(Date.now()/1e3),e=q?t-q.timestamp:1/0;(!q||e>120)&&await yt(),setInterval(()=>{let n=new Date,i=Math.floor(Date.now()/1e3),a=q?i-q.timestamp:1/0,l=i-Ct,o=n.getSeconds()===59,f=a>300,d=l>60;(o&&d||f&&d)&&yt(),Cs()},1e3)}N()&&Es();async function Ts(t,...e){try{let i=await(await fetch(xs,{method:"POST",headers:{Authorization:`Bearer ${hs}`},body:JSON.stringify([t,...e])})).json();if(i.error)throw new Error(i.error);return i.result}catch(n){return console.error("Upstash Error:",n),null}}var $e=null,ke=null,Ne=!1,Pe=!1;async function _(t,e={}){if(N()){if(!q)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(q.monitor),{status:200});if(t.startsWith("/system/options"))return new Response(JSON.stringify(q.options||{}),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(q.processes),{status:200});if(t.startsWith("/events")){let a=new URLSearchParams(t.split("?")[1]||""),l=a.get("type")||a.get("event.type"),o=a.get("category"),f=a.get("order")||"desc",d=[];return l==="system.notification.generated"?d=[...q.alerts||[]]:l==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?d=[...q.blueprints||[]]:o==="messaging"?d=[...q.messaging_events||[]]:o==="system"?d=[...q.system_events||[]]:o==="cognitive"?d=[...q.cognitive_events||[]]:o==="moderation"?d=[...q.moderation_events||[]]:d=[...q.events||[]],f==="asc"?d.sort((x,T)=>x.timestamp-T.timestamp):d.sort((x,T)=>T.timestamp-x.timestamp),new Response(JSON.stringify({events:d,count:d.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(q.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let a=t.split("/")[2],l=q.profiles?q.profiles[a]:null;return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(q.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify(q.chores||[]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if($e)try{let a=await fetch($e+t,e);if(a.ok)return a;$e=null}catch{$e=null}let n=gs(),i=bs;try{let a=await fetch(n+t,e);if(a.ok)return $e=n,Ne&&(console.log("\u2728 Production event service restored."),Ne=!1),a;throw new Error("Primary failed")}catch{Ne||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${i}`),Ne=!0);try{let l=await fetch(i+t,e);if(l.ok)return $e=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}async function ue(t,e={}){if(N()){if(!q)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(q.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let a=t.split("/")[2],l=(q.contacts?.members||[]).find(o=>o.id===a);return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(ke)try{let a=await fetch(ke+t,e);if(a.ok)return a;ke=null}catch{ke=null}let n=fs(),i=vs;try{let a=await fetch(n+t,e);if(a.ok)return ke=n,Pe&&(console.log("\u2728 Production discord service restored."),Pe=!1),a;throw new Error("Primary failed")}catch{Pe||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${i}`),Pe=!0);try{let l=await fetch(i+t,e);if(l.ok)return ke=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}var Tt=()=>`
...
`,je=null,ge=new Set,Ue=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];je=Date.now(),te(0,je);let o=Date.now(),f=24*60*60*1e3,d=l.filter(p=>{let h=localStorage.getItem(`alert_read_ts_${p.id}`);if(!h)return!0;let y=parseInt(h);return o-y<f});d.sort((p,h)=>{let y=w=>{let g=w.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},r=w=>w==="critical"?4:w==="high"?3:w==="medium"?2:1,c=r(y(p)),C=r(y(h));return c!==C?C-c:h.timestamp-p.timestamp}),Ue=d;let x=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},T=[];if(d.length===0)T.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let h=new Set(d.map(r=>x(r))).size>1,y=null;d.forEach(r=>{let c=x(r);h&&c!==y&&(T.push({id:`divider-${c}`,type:"divider",label:c.toUpperCase()}),y=c),T.push(r)})}t&&(e.innerHTML="");let u=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let y=(h.title||"Untitled Alert").trim(),r=(h.body||"No description provided.").trim(),c=h.summary||"",C=h.content||"",w=h.protocol||"",g=(h.priority||"low").toLowerCase(),S=!!h.alert,L=(h.category||"system").trim(),A=h.related_event_ids||[],P=h.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${p.id}`),V=new Date(p.timestamp*1e3),I=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=J?"event-border-grey":"event-border-blue";!J&&S&&(M="event-border-red"),J&&(g==="high"||g==="critical")?M="event-border-red":J&&g==="medium"&&(M="event-border-orange");let B=J?"alert-read":"alert-unread",O=ge.has(p.id),R=O?"expanded":"",G=O?"display: block;":"display: none;",k="",D="";A.length>0&&(D=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${A.map(ce=>`<a href="#" onclick="window.dexter.viewEvent('${ce}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ce.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let z="";P&&(z=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${re(C)}</div>
            </div>
        `:Y=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${re(r)}</div>
            </div>
        `,k=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${L}</div>
                    </div>
                    ${U}
                    ${z}
                    ${D}
            ...
            `;let lt=K.querySelector(".event-details");lt&&lt.addEventListener("click",ce=>{ce.stopPropagation()});let ct=K.querySelector(".close-details-btn");return ct&&ct.addEventListener("click",ce=>{ce.stopPropagation(),K.classList.remove("expanded");let Q=K.querySelector(".event-details");Q&&(Q.style.display="none"),ge.delete(p.id)}),K},v=p=>{let h=document.createElement("div");h.className="notification-divider",h.dataset.alertId=p.id;let y="#888888";return p.label==="CRITICAL"?y="#ff4d4d":p.label==="HIGH"?y="#ff8888":p.label==="MEDIUM"&&(y="#ffa500"),h.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${y}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,h.innerHTML=`<span style="white-space: nowrap;">${p.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${y}44, transparent); flex-grow: 1;"></div>`,h},s=p=>{let h=document.createElement("div");h.innerHTML=F("empty",p.message,null,p.id);let y=h.firstElementChild;return y.dataset.alertId=p.id,y},m=Array.from(e.children),E=new Map(m.map(p=>[p.dataset.alertId,p])),H=new Set(T.map(p=>p.id));m.forEach(p=>{let h=p.dataset.alertId;(!h||!H.has(h))&&p.remove()});let b=null;T.forEach((p,h)=>{let y=E.get(p.id);if(!y||t){if(y&&y.remove(),p.type==="divider")y=v(p);else if(p.type==="placeholder")y=s(p);else{let r=u(p);if(!r)return;y=r}if(!y)return}h===0?e.firstElementChild!==y&&e.prepend(y):b&&b.nextElementSibling!==y&&b.after(y),b=y}),je=Date.now(),te(0,je),Xe()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=F("offline","Failed to load alerts.","The event service may be offline."))}}function $s(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),n=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ue.forEach(a=>{localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Ue.forEach(a=>{ge.add(a.id),localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),de(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ge.clear(),de(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.notification.generated",{method:"DELETE"});let a=Date.now()-48*60*60*1e3;Ue.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,a.toString())}),ge.clear(),de(!0)}catch(a){console.error("Failed to clear alerts:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function Qe(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}if((o.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||a++}),xt(a)}catch{}}var $t=()=>`
...
`,fe=new Set,St=[],Le=null;async function be(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await _("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();St=i;let a=u=>{let v=fe.has(u.id),s=u.state==="published",m=u.state==="consumed",E="event-border-grey";s&&(E="event-border-blue"),m&&(E="event-border-purple");let b=new Date(u.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),p=document.createElement("div");p.className=`event-item notification-item ${E} cursor-pointer ${v?"expanded":""}`,p.dataset.itemId=u.id,p.onclick=r=>{if(r.target.closest("button"))return;if(p.classList.contains("expanded")){p.classList.remove("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="none"),fe.delete(u.id)}else{p.classList.add("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="block"),fe.add(u.id)}};let h=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${u.state.toUpperCase()}]</span>`;p.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${j(u.content)}</div>
      ...
      `;let y=p.querySelector(".event-details");return y&&y.addEventListener("click",r=>{r.stopPropagation()}),p.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(u)),p.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ls(u)),p.querySelector(".delete-btn")?.addEventListener("click",()=>Ms(u.id)),p.querySelector(".close-details-btn")?.addEventListener("click",r=>{r.stopPropagation(),p.classList.remove("expanded");let c=p.querySelector(".event-details");c&&(c.style.display="none"),fe.delete(u.id)}),p},l=u=>{let v=document.createElement("div");v.innerHTML=F("empty",u.message,u.action,u.id);let s=v.firstElementChild;return s.dataset.itemId=u.id,s},o=[];!i||i.length===0?o.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:N()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(u=>o.push({...u,type:"item"}));let f=Array.from(e.children),d=new Map(f.map(u=>[u.dataset.itemId,u])),x=new Set(o.map(u=>u.id));f.forEach(u=>{let v=u.dataset.itemId;(!v||!x.has(v))&&u.remove()}),t&&(e.innerHTML="");let T=null;o.forEach((u,v)=>{let s=d.get(u.id);(!s||t)&&(s&&s.remove(),u.type==="placeholder"?s=l(u):s=a(u),!s)||(v===0?e.firstElementChild!==s&&e.prepend(s):T&&T.nextElementSibling!==s&&T.after(s),T=s)})}catch{e.children.length===0&&(e.innerHTML=F("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Le=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Le=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),o=l?l.value:"";if(!o.trim())return;let f=Le?`/roadmap/${Le}`:"/roadmap",d=Le?"PATCH":"POST";try{await _(f,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})});let x=document.getElementById("roadmap-editor-container");x&&(x.style.display="none"),be(!0)}catch(x){console.error(x)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{St.forEach(l=>fe.add(l.id)),be(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{fe.clear(),be(!0)},a.dataset.listenerAttached="true")}function Ss(t){Le=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ls(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),be(!0)}catch(n){console.error(n)}}async function Ms(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),be(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Lt=null,ve=new Set,It=[];async function ye(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];It=l,Lt=Date.now(),te(2,Lt);let o=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let E=(m.title||"Untitled Blueprint").trim(),H=(m.summary||m.body||"No summary provided.").trim(),b=(m.content||"").trim(),p=(m.category||"architecture").trim(),h=(m.related_services||m.affected_services||[]).map(k=>k.trim()),y=(m.implementation_path||[]).map(k=>k.trim()),r=m.source_event_ids||[],c=m.approved===!0,C=new Date(s.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=ve.has(s.id),L=S?"display: block;":"display: none;",A=document.createElement("div");A.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${c?"blueprint-approved":""}`,A.dataset.blueprintId=s.id,c&&(A.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",A.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=c?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[p]||"bx-paint";A.onclick=function(k){let D=this;if(D.classList.contains("expanded")){D.classList.remove("expanded"),ve.delete(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="none")}else{D.classList.add("expanded"),ve.add(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="block")}};let J="";y.length>0&&(J=`
                ...
                `);let I=h.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${h.join(", ")}</span></div>`:"<div></div>",$=N(),M=c?`
            ...
            `;A.innerHTML=`
                ${c?'<div class="blueprint-sparkle"></div>':""}
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${g}</span>
                </div>
                <div class="event-icon" style="${c?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${L}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${j(H)}</div>
            ...
            `;let B=A.querySelector(".blueprint-approve-btn");B&&(B.onclick=async k=>{k.stopPropagation(),B.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ye(!0)}catch(D){console.error("Failed to approve blueprint:",D)}});let O=A.querySelector(".blueprint-delete-btn");O&&(O.onclick=async k=>{k.stopPropagation();let D=!c;O.innerHTML=D?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${s.id}`,{method:"DELETE"})).ok&&ye(!0)}catch(z){console.error("Failed to delete blueprint:",z)}});let R=A.querySelector(".event-details");R&&R.addEventListener("click",k=>{k.stopPropagation()});let G=A.querySelector(".close-details-btn");return G&&G.addEventListener("click",k=>{k.stopPropagation(),A.classList.remove("expanded");let D=A.querySelector(".event-details");D&&(D.style.display="none"),ve.delete(s.id)}),A},f=s=>{let m=document.createElement("div");m.innerHTML=F("empty",s.message,s.action,s.id);let E=m.firstElementChild;return E.dataset.blueprintId=s.id,E},d=[];l.length===0?d.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):l.forEach(s=>d.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let x=Array.from(e.children),T=new Map(x.map(s=>[s.dataset.blueprintId,s])),u=new Set(d.map(s=>s.id));x.forEach(s=>{let m=s.dataset.blueprintId;(!m||!u.has(m))&&s.remove()});let v=null;d.forEach((s,m)=>{let E=T.get(s.id);(!E||t)&&(E&&E.remove(),s.type==="placeholder"?E=f(s):E=o(s),!E)||(m===0?e.firstElementChild!==E&&e.prepend(E):v&&v.nextElementSibling!==E&&v.after(E),v=E)}),Re(2,l.length),wt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=F("offline","Failed to load blueprints.","The event service may be offline."))}}async function Bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&a++}),ht(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{It.forEach(i=>ve.add(i.id)),ye(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ve.clear(),ye(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),ve.clear(),ye(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var le="STANDBY",Ht=null,Me=[],X=null,qe=null,_t=()=>`
  ...
  `}async function Ae(){let t=document.getElementById("progress-view-root");t&&(le!==Ht&&(t.innerHTML=Dt(),Ht=le),le==="ACTIVE"&&_s())}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),n=document.getElementById("active-task-progress-text"),i=document.getElementById("terminal-output");if(t&&X&&(t.innerText=X.title),e&&X&&(e.style.width=`${X.progress}%`),n&&X&&(n.innerText=`${X.progress}% COMPLETE`),i){let a=i.children.length;if(Me.length>a){for(let l=a;l<Me.length;l++){let o=Me[l],f=document.createElement("div");f.className="terminal-line",f.innerHTML=`
            <span class="line-time">${o.time}</span>
            <span class="line-prefix">></span>
            <span class="line-msg">${j(o.msg)}</span>
        `,i.appendChild(f)}i.scrollTop=i.scrollHeight}}}window.addEventListener("dex-mock-start",()=>{le="ACTIVE",Me=[{time:new Date().toLocaleTimeString(),msg:"Initializing cognitive engine..."},{time:new Date().toLocaleTimeString(),msg:"Connecting to event-service..."}],X={title:"Refactoring System Event Bus",progress:10},Ae();let t=0,e=setInterval(()=>{if(le!=="ACTIVE"){clearInterval(e);return}t++,X.progress=Math.min(100,X.progress+Math.floor(Math.random()*15));let n=["Analyzing internal/bus.go...","Checking circular dependencies.","Optimizing channel throughput.","Writing unit tests for refactor.","Deploying to staging environment.","Verifying system integrity."];t<n.length&&Me.push({time:new Date().toLocaleTimeString(),msg:n[t]}),X.progress>=100&&(le="COMPLETED",qe={duration:"1m 12s",steps:32,result:"Optimized Event Bus"},clearInterval(e)),Ae()},1500)});window.addEventListener("dex-mock-stop",()=>{le="STANDBY",Ae()});window.addEventListener("dex-mock-reset",()=>{le="STANDBY",Ae()});var zt=()=>`
...
`,Nt=()=>`
    ...
    `,Be=[],He={"313071000877137920":"Owen",dexter:"Dexter"},se=[],xe=null;async function _e(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),o=document.getElementById("selected-recipients"),f=document.getElementById("task-filter-owner"),d=document.getElementById("new-chore-instruction"),x=document.getElementById("new-chore-url"),T=document.getElementById("new-chore-schedule"),u=document.getElementById("new-chore-time"),v=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(v&&!v.dataset.initialValueAttached){let c=Intl.DateTimeFormat().resolvedOptions().timeZone,C=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===c){C=!0;break}if(!C){let w=document.createElement("option");w.value=c,w.textContent=c.replace("_"," "),v.appendChild(w)}v.value=c,v.dataset.initialValueAttached="true"}T&&s&&!T.dataset.timeListenerAttached&&(T.addEventListener("change",()=>{s.style.display=T.value==="daily"?"block":"none"}),T.dataset.timeListenerAttached="true");let m=document.getElementById("form-title"),E=document.getElementById("form-icon");function H(){if(o){if(se.length===0){o.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}o.innerHTML=se.sort((c,C)=>{let w=c.startsWith("channel:"),g=C.startsWith("channel:");return w&&!g?-1:!w&&g?1:0}).map(c=>{let C=He[c]||c,w=c.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${c==="dexter"?"#bb86fc":w?"#03dac6":"#fff"};">
          <i class='bx ${c==="dexter"?"bx-terminal":w?"bx-hash":"bx-user"}'></i>
          <span>${C}</span>
          <i class='bx bx-x remove-recipient' data-id="${c}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),o.querySelectorAll(".remove-recipient").forEach(c=>{c.onclick=()=>{let C=c.dataset.id;se=se.filter(w=>w!==C),H()}})}}function b(c){c&&!se.includes(c)&&(se.push(c),H()),l&&(l.value="")}function p(c=null){if(n){if(xe=c?c.id:null,c){if(m&&(m.textContent="Update Research Task"),E&&(E.className="bx bx-edit-alt",E.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),d&&(d.value=c.natural_instruction),x&&(x.value=c.execution_plan?.entry_url||""),T&&(T.value=c.schedule,s&&(s.style.display=c.schedule==="daily"?"block":"none")),u&&c.run_at&&(u.value=c.run_at),v&&c.timezone){let C=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===c.timezone){C=!0;break}if(!C){let w=document.createElement("option");w.value=c.timezone,w.textContent=c.timezone.replace("_"," "),v.appendChild(w)}v.value=c.timezone}se=c.recipients||(c.owner_id?[c.owner_id]:[]),H()}else m&&(m.textContent="Initialize Research Task"),E&&(E.className="bx bx-plus-circle",E.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),d&&(d.value=""),x&&(x.value=""),T&&(T.value="every_24h"),se=[],H();n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"})}}let h=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(h&&!h.dataset.populated&&!N())try{ue("/contacts").then(async c=>{c.ok&&(((await c.json()).members||[]).forEach(g=>{if(He[g.id]=g.nickname||g.username,g.id==="313071000877137920")return;let S=document.createElement("option");S.value=g.id;let L=g.nickname||g.username;S.textContent=`${L} (${g.username})`,h.appendChild(S)}),h.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!N())try{ue("/channels").then(async c=>{c.ok&&((await c.json()).forEach(w=>{let g=`channel:${w.id}`;He[g]=w.name;let S=document.createElement("option");S.value=g,S.textContent=`#${w.name} (${w.guild})`,y.appendChild(S)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{b(l.value)},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onchange=()=>r(),f.dataset.listenerAttached="true");function r(){if(!t)return;let c=f?.value||"all",C=c==="all"?Be:Be.filter(g=>(g.recipients||(g.owner_id?[g.owner_id]:[])).includes(c));if(C.length===0){t.innerHTML=F("empty",c==="all"?"No active tasks.":"No tasks found for this owner.",N()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let w=C.map(g=>{let S=g.last_run===0?"Never":new Date(g.last_run*1e3).toLocaleString(),L=g.memory?g.memory.length:0,A=g.status==="active"?"#03dac6":"#666",W=(g.recipients||(g.owner_id?[g.owner_id]:[])).sort((J,V)=>{let I=J.startsWith("channel:"),$=V.startsWith("channel:");return I&&!$?-1:!I&&$?1:0}).map(J=>{let V=He[J]||J.substring(0,8),I=J.startsWith("channel:");return`<span title="${V}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${J==="dexter"?"bx-terminal":I?"bx-hash":"bx-user"}'></i>${V}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${A}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    <div class="service-widget-header" style="display: flex; align-items: flex-start; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 15px; margin-bottom: 15px;">
                        <div style="display: flex; gap: 15px; flex: 1;">
                            <i class='bx bx-search-alt' style="color: ${A}; font-size: 1.5em; margin-top: 2px;"></i>
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            ...
                            <span style="font-size: 0.85em; color: #03dac6; font-weight: bold;">${L}</span>
                        </div>
            ...
            `}).join("");t.innerHTML=w,t.querySelectorAll(".edit-chore-btn").forEach(g=>{g.onclick=S=>{S.stopPropagation();let L=g.dataset.id,A=Be.find(P=>P.id===L);A&&p(A)}}),t.querySelectorAll(".reset-chore-btn").forEach(g=>{g.onclick=async S=>{S.stopPropagation();let L=g.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(g.innerHTML="<i class='bx bx-loader-alt spin'></i>",await _(`/chores/${L}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),_e())}}),t.querySelectorAll(".delete-chore-btn").forEach(g=>{g.onclick=async S=>{S.stopPropagation();let L=g.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${L}`,{method:"DELETE"}),_e())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||xe!==null?p(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let c=document.getElementById("new-chore-instruction"),C=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),g=c?.value,S=w?.value||"every_24h",L=S==="daily"?u?.value:"",A=S==="daily"?v?.value:"";if(!g)return;if(se.length===0){alert("Please add at least one recipient.");return}let P=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=xe?"PATCH":"POST",J=xe?`/chores/${xe}`:"/chores";await _(J,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:g,entry_url:C?.value,schedule:S,run_at:L,timezone:A})}),n&&(n.style.display="none"),c&&(c.value=""),C&&(C.value=""),xe=null,se=[],_e()}catch(W){console.error(W),alert(xe?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=P}},i.dataset.listenerAttached="true");try{let c=await _("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);if(Be=await c.json()||[],f){let w=f.value,g=new Set;Be.forEach(S=>{S.owner_id&&g.add(S.owner_id),S.recipients&&S.recipients.forEach(L=>g.add(L))}),f.innerHTML='<option value="all">All Recipients</option>',g.forEach(S=>{let L=document.createElement("option");L.value=S;let A=S.startsWith("channel:"),P=He[S]||`Recipient: ${S.substring(0,8)}`;L.textContent=(A&&!P.startsWith("#")?"#":"")+P,f.appendChild(L)}),f.value=w,f.selectedIndex===-1&&(f.value="all")}r()}catch(c){console.error("Failed to fetch chores",c)}}var Pt=()=>`
...
`;async function et(){await Promise.all([be(),ye(),Ae(),_e()])}var Ds=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Ut(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function qt(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",ue(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();tt(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=Ut(t.id,t.username);tt(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=Ut(t.id,t.username);tt(e,t,i)})}function tt(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let E=(n.attributes||n.facts||[]).map(r=>{let c=r.key||r.k,C=r.value||r.v;return`
        ...
        `},l=()=>{let E=n.dossier||{},H=E.identity||{},b=E.career||{},p=E.personal||{},h=E.social||[];return`
    ...
    `;t.innerHTML=x;let T=t.querySelector(".profile-card"),u=t.querySelector("#profile-expand-toggle"),v=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content"),m=t.querySelector(".close-profile-btn");m&&m.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),v.forEach(E=>{E.addEventListener("click",()=>{v.forEach(p=>p.classList.remove("active")),s.forEach(p=>p.classList.remove("active")),E.classList.add("active");let H=E.dataset.tab,b=t.querySelector(`#tab-${H}`);b&&b.classList.add("active")})}),u&&u.addEventListener("click",()=>{T.classList.toggle("expanded");let E=T.classList.contains("expanded");u.innerHTML=E?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Ft=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function st(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await st(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=n=>{let a=n.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let l=JSON.parse(a.dataset.user);qt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let n=await ue("/contacts");if(!n.ok)throw new Error("Service unreachable");let a=(await n.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=F("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((o,f)=>{let d=l[o.level]??10,x=l[f.level]??10;return d!==x?d-x:o.username.localeCompare(f.username)}),t.innerHTML=a.map(o=>{let f=o.color&&o.color!==0,d=f?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",x=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",T="#888";o.level==="Me"||o.level==="Master"?T="#bb86fc":o.level==="Admin"?T="#cf6679":o.level==="Moderator"?T="#03dac6":o.level==="Contributor"&&(T="#ffa500");let u=o.level==="Me",v=u?"rgba(187, 134, 252, 0.08)":f?`${d}11`:"rgba(255,255,255,0.03)",s=u?"2px solid #bb86fc":f?`1px solid ${d}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${u?"#bb86fc":f?d:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${o.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${u?"2px solid #bb86fc":f?`2px solid ${d}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${T}; font-weight: 600; text-transform: uppercase;">${u?"DEXTER (ME)":o.level}</span>
...
`,Fe=null,he=new Set,De=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Ps))if(n.includes(t))return e;return"system"}function js(t){return Rs[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await _(i);if(!a.ok)throw new Error("Service unreachable");if(De=((await a.json()).events||[]).filter(v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Fe=Date.now(),te(1,Fe),De.length===0){e.innerHTML=F("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,E=Os(m),H=js(m),b=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.bot.voice_response"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),p="event-border-grey";b&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?p="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"||m==="engagement.decision"?p="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?p="event-border-orange":m==="system.test.completed"?p=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":p="event-border-blue");let h=b?"cursor-pointer":"",y=he.has(v.id),r=y?"expanded":"",c=y?"display: block;":"display: none;",C=new Date(v.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Jt(m,s),L=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",A="";if(b){let I="";if(m==="engagement.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.timestamp-s.last_active,B=Math.floor(M/60);I=`
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${B} minutes</span>
                        </div>
                        <div class="event-detail-block">
                            ${$("Context (Last Input)")}
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,M="";if(s.embed){let R=s.embed,G=R.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(R.title)}</div>`:"",k=R.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${re(R.description)}</div>`:"",D=(R.fields||[]).map(z=>`
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${j(z.name)}</div>
            ...
            `}let B="";s.eval_count&&(B=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,G=s.chat_history.map((k,D)=>{let z=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&z==="ASSISTANT"&&(z="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=D===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${D}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${z}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${D+1} of ${R}</span>
                                    </div>
                        ...
                        `}
                    `}else if(m==="messaging.bot.voice_response"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,M="",B=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let R=O.match(/{.*?}/g);R&&(B=R.map(G=>{let k=G.substring(1,G.length-1),D=k.indexOf(" "),z=k.substring(0,D),U=k.substring(D+1);return U=U.replace(/\[\]$/,"").trim(),{role:z,content:U}}))}if(B.length>0&&s.response_raw&&B.push({role:"assistant",content:s.response_raw}),B.length>0){let R=B.length,G=B.map((k,D)=>{let z=k.role.toUpperCase();z==="ASSISTANT"&&(z="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=D===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${D}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${z}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${D+1} of ${R}</span>
                    ...
                    `}else if(m==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.signals||{};I=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.visual.completed"){let $=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,M="";s.base64_preview?M=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(M=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",M=s.success?"SUCCESS":"FAILED",B=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${B("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let D=s.chat_history.length,z=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ce=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${Y}" style="display: ${Y===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${Y+1} of ${D}</span>
                                    </div>
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let D=s.corrections.map((z,U)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${z.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${j(z.snippet)}</div>`:""}
                            </div>
                        `).join("");G=`
                            <div class="event-detail-block">
                                ${B(`Corrections (${s.corrections.length})`)}
                                ${D}
                            </div>
                        `}let k="";if(s.parsed_results&&s.parsed_results.length>0){let D=s.parsed_results.map(z=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${j(z.title)}</div>
                                <div style="color: #ddd;">${j(z.summary)}</div>
                            </div>
                        `).join("");k=`
                            <div class="event-detail-block">
                                ${B("Parsed Results")}
                        ...
                        ${G}
                        ${R}
                    `}else if(m==="system.test.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(B=>{let O=B.content_type&&B.content_type.startsWith("image/"),R=(B.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${O?`<div class="attachment-thumb-container"><img src="${B.url}" alt="${B.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${B.url}" target="_blank" class="attachment-link">${B.filename}</a>
                                        <span class="attachment-meta">${B.content_type} \u2022 ${R}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),I=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let P=document.createElement("div");P.className=`event-item ${p} ${h} ${r}`,P.dataset.eventId=v.id,P.onclick=function(I){if(!b)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),he.delete(v.id);let B=$.querySelector(".event-details");B&&(B.style.display="none")}else{$.classList.add("expanded"),he.add(v.id);let B=$.querySelector(".event-details");B&&(B.style.display="block")}},P.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${g}</span>
                </div>
            ...
            `;let W=P.querySelector(".event-details");if(W&&W.addEventListener("click",I=>{I.stopPropagation()}),b){let I=P.querySelector(".close-details-btn");I&&I.addEventListener("click",$=>{$.stopPropagation();let M=$.target.closest(".event-item");if(M){M.classList.remove("expanded"),he.delete(v.id);let B=M.querySelector(".event-details");B&&(B.style.display="none")}})}let J=P.querySelector(".prev-btn"),V=P.querySelector(".next-btn");if(J&&V){let I=0,$=Array.from(P.querySelectorAll(".history-slide")),M=$.length,B=()=>{$.forEach((O,R)=>{O.style.display=R===I?"block":"none"}),J.disabled=I===0,V.disabled=I===M-1,J.style.opacity=I===0?"0.5":"1",V.style.opacity=I===M-1?"0.5":"1"};J.addEventListener("click",O=>{O.stopPropagation(),I>0&&(I--,B())}),V.addEventListener("click",O=>{O.stopPropagation(),I<M-1&&(I++,B())}),B()}return P},d=Array.from(e.children),x=new Map(d.map(v=>[v.dataset.eventId,v])),T=new Set(De.map(v=>v.id));d.forEach(v=>{let s=v.dataset.eventId;(!s||!T.has(s))&&v.remove()});let u=null;De.forEach((v,s)=>{let m=x.get(v.id);(!m||t)&&(m&&m.remove(),m=f(v),!m)||(s===0?e.firstElementChild!==m&&e.prepend(m):u&&u.nextElementSibling!==m&&u.after(m),u=m)}),Fe=Date.now(),te(1,Fe)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=F("offline","Failed to load events.","The event service may be offline."))}}function Us(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),n=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{De.forEach(a=>he.add(a.id)),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{he.clear(),pe(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.querySelectorAll(".filter-btn").forEach(a=>{a.onclick=()=>{n.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),a.classList.add("active"),Z=a.dataset.filter||"all",pe(!0)}}),n.dataset.listenerAttached="true"),n&&n.querySelectorAll(".filter-btn").forEach(a=>{a.dataset.filter===Z?a.classList.add("active"):a.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let a=Z==="all"?"ALL":Z.toUpperCase();if(confirm(`Are you sure you want to delete ${a} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(Z!=="all"?l+=`?category=${Z}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await _(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");he.clear(),pe(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var qs=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Wt=qs;function Gt(){return`
    ...
    `}var Fs=null;async function ze(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=F("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(o=>!i.includes(o.id));a.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),a.reverse();let l=a.map(o=>{let f=o.logs.join(`
`),d=[...o.logs];if(d.length<25){let T=25-d.length;for(let u=0;u<T;u++)d.push("")}else d.length>25&&(d=d.slice(-25));let x=d.map(T=>Oe(T)).join(`
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${o.id}" title="Clear Logs" style="${N()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let f=o,d=unescape(f.dataset.logs||"");navigator.clipboard.writeText(d).then(()=>{let x=f.querySelector("i");x&&(x.classList.remove("bx-copy"),x.classList.add("bx-check"),setTimeout(()=>{x.classList.remove("bx-check"),x.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let d=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${d}?`))try{(await _(`/logs?service_id=${d}`,{method:"DELETE"})).ok&&ze()}catch(x){console.error("Error clearing logs:",x)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=F("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Yt=()=>{let t=N()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Gt()}
        </div>`;async function ns(){Js(),await Promise.all([ne(),we(),at()]),await ze()}var Je=null;function Js(){Je||(Je=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Je),Je=null;return}ne(!0)},1e3))}var Vt=null,Kt=null;async function is(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function Zt(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await _("/processes")).json()}catch{return null}}async function Gs(){try{return await(await _("/agent/status")).json()}catch{return null}}async function we(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),n=document.getElementById("global-restart-btn"),i=document.getElementById("global-stop-btn"),a=document.getElementById("global-start-btn"),l=(r,c)=>{r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{let C=r.innerHTML;r.innerHTML="<i class='bx bx-loader-alt spin'></i>",r.disabled=!0;try{await _(`/system/service/${c}`,{method:"POST",body:JSON.stringify({service:"all"})}),setTimeout(()=>we(),2e3),setTimeout(()=>we(),5e3),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{r.innerHTML=C,r.disabled=!1},1e3)},1e3)}catch(w){console.error(w),r.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{r.innerHTML=C,r.disabled=!1},2e3)}},r.dataset.listenerAttached="true")};l(n,"restart"),l(i,"stop"),l(a,"start");let o=document.querySelector("#hw-os .hw-content"),f=document.querySelector("#hw-cpu .hw-content"),d=document.querySelector("#hw-ram .hw-content"),x=document.querySelector("#hw-gpu .hw-content"),T=document.querySelector("#hw-storage .hw-content"),u=r=>{if(r){if(o&&(o.innerHTML=`
                ...
                </div>`),d){let c=(r.MEMORY_BYTES/1073741824).toFixed(1);d.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <span style="font-size: 0.9em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">Total System Memory</span>
                    <span style="font-size: 1.8em; font-weight: bold; color: #fff;">${c} <span style="font-size: 0.5em; color: #888; font-weight: normal; margin-left: 2px;">GB</span></span>
                </div>`}if(f&&r.CPU&&r.CPU.length>0){let c=r.CPU[0];f.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    <span style="font-size: 1.1em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">${c.LABEL}</span>
                    <div style="display: flex; gap: 30px;">
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #03dac6; line-height: 1;">${c.COUNT}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Cores</span></div>
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #bb86fc; line-height: 1;">${c.THREADS}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Threads</span></div>
                    </div>
                </div>`}x&&(r.GPU&&r.GPU.length>0?x.innerHTML=r.GPU.map(c=>{let C=(c.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${c.LABEL}</span>
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):x.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),T&&r.STORAGE&&r.STORAGE.length>0?T.innerHTML=r.STORAGE.map(c=>{let C=(c.USED/1073741824).toFixed(1),w=(c.SIZE/(1024*1024*1024)).toFixed(1),g=c.SIZE>0?(c.USED/c.SIZE*100).toFixed(0):0,S=c.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                ...
                                <span style="font-weight: 600; color: #fff; font-size: 0.95em;">${c.DEVICE}</span>
                            </div>
                    ...
                    </div>`}).join(""):T&&(T.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let r=await Zt();r?(u(r),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),f&&!f.hasChildNodes())){let r=await Zt();u(r)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async r=>{let c=r.target.closest(".svc-btn");if(!c)return;let C=c,w=C.dataset.action,g=C.dataset.service;if(!w||!g)return;let S=C.innerHTML;C.innerHTML="<i class='bx bx-loader-alt spin'></i>",C.classList.add("loading"),C.disabled=!0;try{await _(`/system/service/${w}`,{method:"POST",body:JSON.stringify({service:g})}),setTimeout(()=>we(),1e3),setTimeout(()=>we(),3e3)}catch(L){console.error(L),alert(`Failed to ${w} ${g}`),C.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{C.innerHTML=S,C.classList.remove("loading"),C.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let v=await is();if(!v||!v.services){t.children.length===0&&(t.innerHTML=F("offline","Failed to load system metrics.","The event service may be offline."));let r=document.querySelector('[data-service-id="upstash-redis-ro"]');if(r){r.className="service-widget service-widget-offline",r.querySelector(".service-widget-status").textContent="ERROR";let c=r.querySelector(".service-widget-body");c&&(c.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Vt=N()&&oe.lastDashboard||Date.now(),te(0,Vt||0);let s=v.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let r=document.getElementById("upstash-countdown");if(r){let w=59-new Date().getSeconds();w<=0&&(w+=60),r.textContent=`${w}s`}},1e3)),Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function m(r){if(!r||r==="N/A"||r==="unknown")return"-";let c=r.match(/^(\d+\.\d+\.\d+)/);return c?c[0]:r.split(".").slice(0,3).join(".")||"-"}function E(r){return!r||r.length<=28?r:r.substring(0,28)+"..."}function H(r){if(!r||r==="N/A"||r==="unknown")return"-";let c=r.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!c)return"-";let C=parseInt(c[1])||0,w=parseInt(c[2])||0,g=parseInt(c[3])||0,S=parseFloat(c[4])||0,L=C*86400+w*3600+g*60+S,A=Math.floor(L/86400);if(A>0)return`${A}d`;let P=Math.floor(L/3600);if(P>0)return`${P}h`;let W=Math.floor(L/60);return W>0?`${W}m`:`${Math.floor(L)}s`}function b(r){if(r.id==="upstash-redis-ro"){let R=N()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            ...
            </div>`}let c=r.status==="online",C=c?"service-widget-online":"service-widget-offline",w=c?"bx-check-circle":"bx-x-circle",g=c?"OK":"BAD",S=r.version?m(r.version.str):"-",L=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",A=r.memory&&r.memory!=="N/A"?r.memory:"-",P=L!=="-"?"#00ff00":"#666",W=L!=="-"?"#fff":"#666",J=A!=="-"?"#00ff00":"#666",V=A!=="-"?"#fff":"#666",I=H(r.uptime),$="",M=r.type!=="os"&&r.type!=="cli"&&r.type!=="prd",B="";M&&!N()&&(B=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                <button class="svc-btn svc-restart" title="Restart Service" data-action="restart" data-service="${r.id}"><i class='bx bx-refresh'></i></button>
                <button class="svc-btn svc-stop" title="Stop Service" data-action="stop" data-service="${r.id}" ${c?"":"disabled"}><i class='bx bx-stop'></i></button>
                <button class="svc-btn svc-start" title="Start Service" data-action="start" data-service="${r.id}" ${c?"disabled":""}><i class='bx bx-play'></i></button>
            </div>`),c?$=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${W};">${L}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${J};"></i>
                        <span style="color: ${V};">${A}</span>
                    </div>
                </div>${B}`:$=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${B}`;let O=N()?"easter.company":E(r.domain&&r.port?`${r.domain}:${r.port}`:"");return`<div class="service-widget ${C}" data-service-id="${r.id}"><div class="service-widget-header"><i class="bx ${w}"></i><h3>${r.short_name||r.id}</h3><span class="service-widget-status">${g}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${$}</div></div>`}let p=new Map(Array.from(t.querySelectorAll(".service-widget")).map(r=>[r.dataset.serviceId,r])),h=new Set(s.map(r=>r.id));for(let[r,c]of p)h.has(r)||c.remove();let y=new Set;s.forEach(r=>{if(y.has(r.id))return;y.add(r.id);let c=b(r),C=p.get(r.id);if(C&&C.parentNode)if(r.id==="upstash-redis-ro"){let w=N()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),g=new Date(w).toLocaleTimeString(),S=C.querySelector(".sync-time-label");S&&(S.textContent=`Last synced: ${g}`)}else C.outerHTML!==c&&(C.outerHTML=c);else t.insertAdjacentHTML("beforeend",c)})}async function at(){let t=document.getElementById("models-widgets");if(!t)return;let e=await is();if(!e){t.children.length===0&&(t.innerHTML=F("offline","Failed to load model status.","The event service may be offline."));return}Kt=Date.now(),te(2,Kt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function a(d){let x=d.status==="Ready";return`
                ...
                </div>`}function l(d){let x=d.status==="Ready";return`
                ...
                </div>`}function o(d){let x=d.status==="Downloaded",T=x?"service-widget-online":"service-widget-offline",u=x?"OK":"MISSING",v=x&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",s=d.name;return d.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${T}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${x?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let f="";if(i&&(f+=a(i)),e.xtts&&(f+=l(e.xtts)),f+=n.map(o).join(""),!f){t.innerHTML=F("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn"),x=document.getElementById("fabricator-reset-btn"),T=document.getElementById("courier-reset-btn"),u=document.getElementById("system-pause-toggle-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{let r=u.querySelector(".bx-play")?"/agent/resume":"/agent/pause";u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _(r,{method:"POST"}),await ne()}catch{u.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},u.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{d.innerHTML="<i class='bx bx-error'></i>"}},d.dataset.listenerAttached="true"),x&&!x.dataset.listenerAttached&&(x.onclick=async()=>{x.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{x.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{x.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{x.innerHTML="<i class='bx bx-error'></i>"}},x.dataset.listenerAttached="true"),T&&!T.dataset.listenerAttached&&(T.onclick=async()=>{T.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{T.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{T.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{T.innerHTML="<i class='bx bx-error'></i>"}},T.dataset.listenerAttached="true");let v=document.getElementById("imaginator-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let y=s.agents.guardian||{protocols:{}},r=s.agents.analyzer||{protocols:{}},c=s.agents.imaginator||{protocols:{}},C=s.agents.fabricator||{protocols:{}},w=s.agents.courier||{protocols:{}},g=s.system||{},S=Math.floor(Date.now()/1e3),L={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},A=k=>{k<0&&(k=0);let D=Math.floor(k/3600),z=Math.floor(k%3600/60),U=k%60;return D>0?`${D}h ${z}m`:z>0?`${z}m ${U}s`:`${U}s`},P=(k,D,z,U)=>{if(!k)return;let Y=L[U]||U.toUpperCase(),K=k.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(K&&(K.textContent=Y),!z){k.textContent="Inactive",k.style.color="#666";return}let Ce=z.status;if(Ce==="Working")k.textContent="Working",k.style.color="#bb86fc";else if(Ce==="Ready")k.textContent="Ready",k.style.color="#5eff5e";else{let Ee=z.cooldown;if(N()&&(Ee=z.next_run-S),Ee<=0)k.textContent="Ready",k.style.color="#5eff5e";else{let Ge=Math.floor(Ee/60),Ve=Ee%60;k.textContent=`${Ge}m ${Ve}s`,k.style.color="#fff"}}D&&z.stats&&(D.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};n&&P(n,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry");let W=document.getElementById("analyzer-synthesis-val"),J=document.getElementById("analyzer-synthesis-stats");W&&P(W,J,r.protocols.synthesis,"synthesis");let V=document.getElementById("imaginator-alert_review-val"),I=document.getElementById("imaginator-alert_review-stats");V&&P(V,I,c.protocols.alert_review,"alert_review");let $=document.getElementById("fabricator-construction-val"),M=document.getElementById("fabricator-construction-stats");$&&P($,M,C.protocols.construction,"construction");let B=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");B&&P(B,O,w.protocols.researcher,"researcher");let R=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&g.state){let k=g.state,D=A(g.state_time||0);R&&(R.textContent=`State: ${k.toUpperCase()}`),G.textContent=D,k==="idle"?G.style.color=g.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",u&&(k==="paused"?(u.innerHTML="<i class='bx bx-play'></i>",u.title="Resume System",u.style.color="#ff9800"):(u.innerHTML="<i class='bx bx-pause'></i>",u.title="Pause System",u.style.color=""))}a&&(a.textContent=A(g.metrics?.total_idle_time||0)),l&&(l.textContent=A(g.metrics?.total_active_time||0)),o&&(o.textContent=A(g.metrics?.total_waste_time||0))}else[n,i,a,l,o].forEach(r=>{r&&(r.textContent="-",r.style.color="#666")});if(t)return;let m=await Ws(),E=[],H=[],b=[];m&&(Array.isArray(m)?E=m:(E=m.active||[],H=m.queue||[],b=m.history||[],b.sort((y,r)=>(r.end_time||0)-(y.end_time||0)))),e&&(E.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=F("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,E,!1)));let p=document.getElementById("processes-queue-widgets");p&&(H.length===0?!p.querySelector(".tab-placeholder")&&!p.querySelector('div[style*="font-style: italic"]')&&(p.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(p.innerHTML="",it(p,H,!1)));let h=document.getElementById("processes-history-widgets");h&&(b.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=F("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),it(h,b,!0))),Re(1,E.length)}function it(t,e,n){function i(d){let x="";d.end_time?x=`${d.end_time-d.start_time}s`:x=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let T=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",u=d.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(v[u]?u=v[u]:/^\d+$/.test(u)&&(u=`Channel ${u}`),n)return`
        <div class="process-history-item" data-channel-id="${d.channel_id}-${d.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let a=n?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(a)).map(d=>[d.dataset.channelId,d])),o=new Set(e.map(d=>`${d.channel_id}-${d.start_time}`));for(let[d,x]of l)d&&!o.has(d)&&x.remove();let f=new Set;e.forEach(d=>{let x=`${d.channel_id}-${d.start_time}`;if(f.has(x))return;f.add(x);let T=i(d),u=l.get(x);if(u&&u.parentNode){u.outerHTML!==T&&(u.outerHTML=T);let v=t.querySelector(`[data-channel-id="${x}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",T)})}function We(){let t=Ze(),e="Notification"in window,n={enabled:e&&Notification.permission==="granted",supported:e};return`
            ...
            </div>`}function ot(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let a=this.dataset.theme;a&&(ft(a),t.setContent(We()),ot(t))})});let n=document.getElementById("notifications-toggle");n&&"Notification"in window?n.onclick=async i=>{let a=i.target;if(a.checked)try{await Notification.requestPermission()!=="granted"&&(a.checked=!1)}catch{a.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),a.checked=!0)}:n&&(n.disabled=!0),Vs()}async function Vs(){let t=document.getElementById("service-config-list"),e=document.getElementById("ollama-config-list");if(!t||!e)return;let n=N();try{let a=await(await _("/system/options")).json(),l=a.services||{},o=a.ollama||{},f="",d=(s,m,E)=>`
        <div class="settings-item">
            <div class="settings-item-info">
                <span class="settings-item-label">${s}</span>
                <span class="settings-item-description">Enable GPU acceleration (CUDA)</span>
            </div>
            <label class="toggle-switch">
                <input type="checkbox" class="service-device-toggle" data-service="${m}" ${E==="cuda"?"checked":""} ${n?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>
        </div>`;l.stt&&(f+=d("STT Service","stt",l.stt.device||"cpu")),l.tts&&(f+=d("TTS Service","tts",l.tts.device||"cpu")),f?n&&(f+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Service configuration is read-only in public mode.</div>'):f='<div style="padding: 20px; text-align: center; color: #666;">No configurable services found.</div>',t.innerHTML=f;let x="",T=o.force_utility_cpu!==!1;if(x+=`
        <div class="settings-item">
            <div class="settings-item-info">
            ...
            <label class="toggle-switch">
                <input type="checkbox" id="ollama-force-cpu-toggle" ${T?"checked":""} ${n?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>
        </div>`,n?x+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Optimization is read-only in public mode.</div>':x+=`<div style="font-size: 0.7em; color: #ffa500; font-style: italic; margin-top: 10px; text-align: center;"><i class='bx bx-info-circle'></i> Changing these settings requires a 'dex build' to re-apply.</div>`,e.innerHTML=x,n)return;t.querySelectorAll(".service-device-toggle").forEach(s=>{s.addEventListener("change",async m=>{let E=m.target,H=E.dataset.service,b=E.checked?"cuda":"cpu";E.disabled=!0;try{await _("/system/options",{method:"POST",body:JSON.stringify({service:H,key:"device",value:b})})}catch{E.checked=!E.checked,alert("Failed to update configuration.")}finally{E.disabled=!1}})});let v=document.getElementById("ollama-force-cpu-toggle");v&&v.addEventListener("change",async s=>{let m=s.target,E=m.checked?"true":"false";m.disabled=!0;try{await _("/system/options",{method:"POST",body:JSON.stringify({service:"ollama",key:"force_utility_cpu",value:E})})}catch{m.checked=!m.checked,alert("Failed to update optimization settings.")}finally{m.disabled=!1}})}catch{t.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load configuration.</div>',e.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load optimization settings.</div>'}}var os=()=>`
    ...
    `;async function rs(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await as(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await as(t)}async function as(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let n=await e.json();if(!n||n.length===0){t.innerHTML=F("empty","No web history found.");return}let i=`
        ...
        `;n.forEach((f,d)=>{let x=new Date(f.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),T=f.screenshot?`<img src="data:image/png;base64,${f.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ks=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[n,i]of Object.entries(t)){let a=ds.filter(l=>l.category===n);a.length!==0&&(e+=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let o=document.getElementById("terminal-status-badge");o&&(o.className="terminal-status status-running",o.textContent="Running");let f=document.getElementById("terminal-action-btn");return f&&(f.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",rt=!0,n}function ls(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",rt=!1}function cs(t,e,n="output"){if(!rt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${n}`,n==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Oe(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ys(t){let e=ds.find(o=>o.id===t);if(!e)return;let n=Zs(e);n&&cs(n,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let o of i){await new Promise(d=>setTimeout(d,400+Math.random()*600));let f="output";o.includes("[ERROR]")?f="error":o.includes("[SUCCESS]")||o.includes("\u2713")?f="success":o.includes("!")&&(f="warning"),n&&cs(n,o,f)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function ps(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ks();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let n=e.querySelector("code"),i=n?n.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(n=>{let i=n;n.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),n.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),n.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Ys(a)})})}async function Xs(){if(!N())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ms(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Xs(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ps();let e=[],n=document.getElementById("windows-container");n&&n.setAttribute("data-count","0");let i=b=>{localStorage.setItem("dex_last_window",b)};function a(){for(;e.length>1;)e.shift().close(!0);let b=document.getElementById("windows-container"),p=document.querySelector("nav"),h=document.querySelector("footer"),y=window.location.pathname==="/"||window.location.pathname==="/index.html",r=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");b&&b.setAttribute("data-count",e.length.toString()),e.forEach(L=>{let A=document.getElementById(L.id);A&&A.classList.remove("hide-close")});let c=document.getElementById("dexter-menu-container"),C=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon"),g=document.getElementById("close-all-windows"),S=window.innerWidth<880;if(!S){let L=document.getElementById("dexter-dropdown"),A=document.getElementById("dexter-menu-btn");L&&L.classList.remove("active"),A&&A.classList.remove("active"),b&&b.classList.remove("menu-open")}if(Te(e.length>0),e.length>0)if(h?.classList.add("hide"),g&&(g.style.display=S?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",p?.classList.add("window-open"),b&&(b.style.paddingTop="60px"),c&&(c.style.display=S?"flex":"none"),w&&(w.style.display=S?"block":"none"),!S&&C){let L=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(L)?(C.innerHTML=`
                      <div class="nav-switch-btn ${L==="alerts-window"?"active":""}" id="switch-alerts"><i class='bx bx-bell'></i> Alerts</div>
                      <div class="nav-switch-btn ${L==="events-window"?"active":""}" id="switch-events"><i class='bx bx-calendar-event'></i> Events</div>
                      <div class="nav-switch-btn ${L==="monitor-window"?"active":""}" id="switch-monitor"><i class='bx bx-pulse'></i> Monitor</div>
                      <div class="nav-switch-btn ${L==="contacts-window"?"active":""}" id="switch-contacts"><i class='bx bx-book-content'></i> Contacts</div>
                      <div class="nav-switch-btn ${L==="workspace-window"?"active":""}" id="switch-workspace"><i class='bx bx-brain'></i> Workspace</div>
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(f)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(d)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(T)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(x)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(u)})):C.innerHTML=""}else C&&(C.innerHTML="");else p?.classList.remove("window-open"),g&&(g.style.display="none"),b&&(b.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",y||r?h?.classList.remove("hide"):h?.classList.add("hide"),c&&(c.style.display="flex"),w&&(w.style.display="block"),C&&(C.innerHTML="");Se()}function l(b){if(b.isOpen()){b.close();return}for(;e.length>0;)e.pop().close(!0);e.push(b),b.open(),a()}function o(){[...e].forEach(p=>p.close()),e=[]}window.addEventListener("resize",a);let f=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>de(),onClose:()=>{let b=e.indexOf(f);b>-1&&e.splice(b,1),a()}}),d=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:nt(),onOpen:()=>{d.setContent(nt()),pe()},onClose:()=>{let b=e.indexOf(d);b>-1&&e.splice(b,1),a()}}),x=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ft(),onOpen:()=>st(),onClose:()=>{let b=e.indexOf(x);b>-1&&e.splice(b,1),a()}}),T=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Qt()},{icon:"bxs-component",title:"Processes",content:Xt()},{icon:"bxs-brain",title:"Models",content:es()},{icon:"bx-globe",title:"Web",content:os()},{icon:"bxs-hdd",title:"Hardware",content:ts()},{icon:"bxs-terminal",title:"Logs",content:ss()},{icon:"bxs-zap",title:"Agents",content:Yt()}].filter(b=>N()?b.title!=="Hardware"&&b.title!=="Logs":!0),onOpen:()=>{we(),ne(),at(),rs(),ze()},onClose:()=>{let b=e.indexOf(T);b>-1&&e.splice(b,1),a()}}),u=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Pt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Ot()},{icon:"bx-task",title:"Tasks",content:jt()}],onOpen:()=>et(),onClose:()=>{let b=e.indexOf(u);b>-1&&e.splice(b,1),a()}}),v=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:We(),onOpen:()=>{v.setContent(We()),ot(v)},onClose:()=>{let b=e.indexOf(v);b>-1&&e.splice(b,1),a()}});window.dexter={viewEvent:b=>{d.isOpen()||l(d),setTimeout(()=>{let p=document.querySelector(`.event-item[data-event-id="${b}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)},viewAlert:b=>{f.isOpen()||l(f),setTimeout(()=>{let p=document.querySelector(`.event-item[data-alert-id="${b}"]`);p&&(p.scrollIntoView({behavior:"smooth",block:"center"}),p.classList.add("flash-highlight"),p.classList.contains("expanded")||p.click(),setTimeout(()=>p.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let b=document.getElementById("dexter-dropdown"),p=document.getElementById("dexter-menu-btn");b&&p&&(b.classList.remove("active"),p.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let b=document.getElementById("dexter-menu-btn"),p=document.getElementById("settings-icon"),h=document.getElementById("close-all-windows"),y=document.getElementById("nav-left-container"),r=document.getElementById("dexter-dropdown"),c=document.getElementById("windows-container");b&&r&&(b.onclick=w=>{w.stopPropagation();let g=window.innerWidth<880;if(g)r.style.top="",r.style.left="",r.style.right="",r.style.transform="";else{let L=b.getBoundingClientRect(),P=document.querySelector("nav").getBoundingClientRect(),W=220,J=10,I=L.left+L.width/2-W/2,$=P.right-10;I+W>$&&(I=$-W),I<10&&(I=10),r.style.top=P.bottom+J+"px",r.style.left=I+"px",r.style.right="auto",r.style.transform="none"}r.classList.toggle("active"),b.classList.toggle("active");let S=r.classList.contains("active");if(g){let L=document.querySelector("nav");S?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),L?.classList.add("window-open"),c?.classList.add("menu-open"),Te(!0)):(c?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),L?.classList.remove("window-open"),Te(!1)))}}),p&&(p.onclick=w=>{w.stopPropagation(),l(v)}),h&&(h.onclick=w=>{w.stopPropagation(),o()});let C=(w,g,S)=>{let L=document.getElementById(w);L&&(L.onclick=A=>{A.stopPropagation(),s(),S&&i(S),l(g)})};C("alerts-menu-item",f,"alerts-window"),C("events-menu-item",d,"events-window"),C("monitor-menu-item",T,"monitor-window"),C("contacts-menu-item",x,"contacts-window"),C("workspace-menu-item",u,"workspace-window"),y&&(y.onclick=w=>{if(w.stopPropagation(),r&&r.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Te(!1));return}if(e.length>0)o();else{let g=window.location.pathname;if(!(g==="/"||g==="/index.html")){let A=(g.endsWith("/")&&g.length>1?g.slice(0,-1):g).split("/");A.pop();let P=A.join("/")||"/";window.location.href=P}}}),document.addEventListener("click",()=>{let w=window.innerWidth<880;r&&r.classList.contains("active")&&(s(),w&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Te(!1)))}),r&&(r.onclick=w=>w.stopPropagation())})();let E=window.location.pathname==="/"||window.location.pathname==="/index.html",H=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!E&&!H&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{d.isOpen()&&pe(),f.isOpen()?de():Qe()},1e3),setInterval(()=>{N()&&(f.isOpen()?de():Qe()),u.isOpen()?et():Bt(),N()&&d.isOpen()&&pe(),T.isOpen()&&ns()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ms):ms();})();

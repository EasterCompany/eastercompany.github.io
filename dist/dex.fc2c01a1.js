...
`,Oe=null,ge=new Set,je=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ks(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await _(s);if(!n.ok)throw new Error("Service is offline or unreachable.");let l=(await n.json()).events||[];Oe=Date.now(),te(0,Oe);let r=Date.now(),g=24*60*60*1e3,d=l.filter(c=>{let p=localStorage.getItem(`alert_read_ts_${c.id}`);if(!p)return!0;let b=parseInt(p);return r-b<g});d.sort((c,p)=>{let b=M=>{let T=M.event;if(typeof T=="string")try{T=JSON.parse(T)}catch{return"low"}return(T.priority||"low").toLowerCase()},x=M=>M==="critical"?4:M==="high"?3:M==="medium"?2:1,C=x(b(c)),$=x(b(p));return C!==$?$-C:p.timestamp-c.timestamp}),je=d;let u=c=>{let p=c.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return"low"}return(p.priority||"low").toLowerCase()},h=[],v=new Set(d.map(c=>u(c))).size>1;if(d.length>0){let c=null;d.forEach(p=>{let b=u(p);v&&b!==c&&(h.push({id:`divider-${b}`,type:"divider",label:b.toUpperCase()}),c=b),h.push(p)})}if(t&&(e.innerHTML=""),d.length===0){e.innerHTML=j("empty","No alerts yet."),Re();return}let o=c=>{let p=c.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return null}let b=(p.title||"Untitled Alert").trim(),x=(p.body||"No description provided.").trim(),C=p.summary||"",$=p.content||"",M=p.protocol||"",T=(p.priority||"low").toLowerCase(),H=!!p.alert,N=(p.category||"system").trim(),P=p.related_event_ids||[],q=p.audit_event_id,W=!!localStorage.getItem(`alert_read_ts_${c.id}`),A=new Date(c.timestamp*1e3),E=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=W?"event-border-grey":"event-border-blue";!W&&H&&(I="event-border-red"),W&&(T==="high"||T==="critical")?I="event-border-red":W&&T==="medium"&&(I="event-border-orange");let O=W?"alert-read":"alert-unread",z=ge.has(c.id),L=z?"expanded":"",F=z?"display: block;":"display: none;",D="",J="";P.length>0&&(J=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue($)}</div>
            </div>
        `:X=`
            <div class="event-detail-block" style="text-align: left;">
...
`,be=new Set,Ue=[],ke=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){Ss();try{let s=await _("/roadmap");if(!s.ok)throw new Error("Offline");let n=await s.json();Ue=n;let i=u=>{let h=be.has(u.id),m=u.state==="published",v=u.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let k=new Date(u.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${o} cursor-pointer ${h?"expanded":""}`,w.dataset.itemId=u.id,w.onclick=c=>{if(c.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let b=w.querySelector(".event-details");b&&(b.style.display="none"),be.delete(u.id)}else{w.classList.add("expanded");let b=w.querySelector(".event-details");b&&(b.style.display="block"),be.add(u.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${u.state.toUpperCase()}]</span>`;w.innerHTML=`
      ...
      `;let f=w.querySelector(".event-details");return f&&f.addEventListener("click",c=>{c.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>Ms(u)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ls(u)),w.querySelector(".delete-btn")?.addEventListener("click",()=>As(u.id)),w.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),w.classList.remove("expanded");let p=w.querySelector(".event-details");p&&(p.style.display="none"),be.delete(u.id)}),w},l=Array.from(e.children),r=new Map(l.map(u=>[u.dataset.itemId,u])),g=new Set(n.map(u=>u.id));if(l.forEach(u=>{let h=u.dataset.itemId;(!h||!g.has(h))&&u.remove()}),!Ue||Ue.length===0){e.innerHTML=j("empty","Your roadmap is empty.",B()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let d=null;n.forEach((u,h)=>{let m=r.get(u.id);(!m||t)&&(m&&m.remove(),m=i(u),!m)||(h===0?e.firstElementChild!==m&&e.prepend(m):d&&d.nextElementSibling!==m&&d.after(m),d=m)})}catch{e.children.length===0&&(e.innerHTML=j("offline","Failed to load roadmap.","The event service may be offline."))}}}function Ss(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),n=document.getElementById("roadmap-expand-all"),i=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ke=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),ke=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let g=ke?`/roadmap/${ke}`:"/roadmap",d=ke?"PATCH":"POST";try{await _(g,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let u=document.getElementById("roadmap-editor-container");u&&(u.style.display="none"),ve(!0)}catch(u){console.error(u)}},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{Ue.forEach(l=>be.add(l.id)),ve(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{be.clear(),ve(!0)},i.dataset.listenerAttached="true")}function Ms(t){ke=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function Ls(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(s){console.error(s)}}async function As(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Is();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let n=await _(s);if(!n.ok)throw new Error("Service is offline or unreachable.");let l=(await n.json()).events||[];if(At=l,St=Date.now(),te(2,St),l.length===0){e.innerHTML=j("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Ae(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),y=(v.summary||v.body||"No summary provided.").trim(),k=(v.content||"").trim(),w=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(z=>z.trim()),f=(v.implementation_path||[]).map(z=>z.trim()),c=v.source_event_ids||[],p=v.approved===!0,b=new Date(m.timestamp*1e3),x=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),C=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ye.has(m.id),M=$?"display: block;":"display: none;",T=document.createElement("div");T.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${p?"blueprint-approved":""}`,T.dataset.blueprintId=m.id,p&&(T.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",T.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let N=p?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";T.onclick=function(z){let L=this;if(L.classList.contains("expanded")){L.classList.remove("expanded"),ye.delete(m.id);let D=L.querySelector(".event-details");D&&(D.style.display="none")}else{L.classList.add("expanded"),ye.add(m.id);let D=L.querySelector(".event-details");D&&(D.style.display="block")}};let P="";f.length>0&&(P=`
                ...
                `);let G=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",W=B(),A=p?`
            ...
            `;let E=T.querySelector(".blueprint-approve-btn");E&&(E.onclick=async z=>{z.stopPropagation(),E.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(L){console.error("Failed to approve blueprint:",L)}});let S=T.querySelector(".blueprint-delete-btn");S&&(S.onclick=async z=>{z.stopPropagation();let L=!p;S.innerHTML=L?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${m.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(F){console.error("Failed to delete blueprint:",F)}});let I=T.querySelector(".event-details");I&&I.addEventListener("click",z=>{z.stopPropagation()});let O=T.querySelector(".close-details-btn");return O&&O.addEventListener("click",z=>{z.stopPropagation(),T.classList.remove("expanded");let L=T.querySelector(".event-details");L&&(L.style.display="none"),ye.delete(m.id)}),T},g=Array.from(e.children),d=new Map(g.map(m=>[m.dataset.blueprintId,m])),u=new Set(l.map(m=>m.id));g.forEach(m=>{let v=m.dataset.blueprintId;(!v||!u.has(v))&&m.remove()});let h=null;l.forEach((m,v)=>{let o=d.get(m.id);if(!o||t){o&&o.remove();let y=r(m);if(!y)return;o=y}v===0?e.firstElementChild!==o&&e.prepend(o):h&&h.nextElementSibling!==o&&h.after(o),h=o}),Ae(2,l.length),wt()}catch(n){console.error("Error fetching blueprints:",n),e.children.length===0&&(e.innerHTML=j("offline","Failed to load blueprints.","The event service may be offline."))}}async function It(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let n=(await e.json()).events||[],i=0;n.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&i++}),ht(i)}catch{}}function Is(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(n=>ye.add(n.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(n){console.error("Failed to clear blueprints:",n)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var le="STANDBY",Bt=null,Se=[],Y=null,qe=null,_t=()=>`
    <div id="progress-view-root" class="progress-container">
        <!-- Initial render will happen here -->
        ${Ht()}
    </div>
  `;function Ht(){switch(le){case"ACTIVE":return _s();case"COMPLETED":return Hs();case"STANDBY":default:return Bs()}}function Bs(){return`
...
`,zt=()=>`
    ...
    `,Ie=[],Be={"313071000877137920":"Owen",dexter:"Dexter"},se=[],he=null;async function Fe(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),n=document.getElementById("save-chore-btn"),i=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),g=document.getElementById("task-filter-owner"),d=document.getElementById("new-chore-instruction"),u=document.getElementById("new-chore-url"),h=document.getElementById("new-chore-schedule"),m=document.getElementById("form-title"),v=document.getElementById("form-icon");function o(){if(r){if(se.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=se.map(c=>{let p=Be[c]||c,b=c.startsWith("channel:");return`
      ...
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(c=>{c.onclick=()=>{let p=c.dataset.id;se=se.filter(b=>b!==p),o()}})}}function y(c){c&&!se.includes(c)&&(se.push(c),o()),l&&(l.value="")}function k(c=null){s&&(he=c?c.id:null,c?(m&&(m.textContent="Update Research Task"),v&&(v.className="bx bx-edit-alt",v.style.color="#03dac6"),n&&(n.innerHTML="<i class='bx bx-check'></i> Update Task"),d&&(d.value=c.natural_instruction),u&&(u.value=c.execution_plan?.entry_url||""),h&&(h.value=c.schedule),se=c.recipients||(c.owner_id?[c.owner_id]:[]),o()):(m&&(m.textContent="Initialize Research Task"),v&&(v.className="bx bx-plus-circle",v.style.color="#bb86fc"),n&&(n.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),d&&(d.value=""),u&&(u.value=""),h&&(h.value="every_24h"),se=["313071000877137920"],o()),s.style.display="block",s.scrollIntoView({behavior:"smooth",block:"start"}))}let w=document.getElementById("contacts-group"),a=document.getElementById("channels-group");if(w&&!w.dataset.populated&&!B())try{fe("/contacts").then(async c=>{c.ok&&(((await c.json()).members||[]).forEach(x=>{if(Be[x.id]=x.nickname||x.username,x.id==="313071000877137920")return;let C=document.createElement("option");C.value=x.id;let $=x.nickname||x.username;C.textContent=`${$} (${x.username})`,w.appendChild(C)}),w.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(a&&!a.dataset.populated&&!B())try{fe("/channels").then(async c=>{c.ok&&((await c.json()).forEach(b=>{let x=`channel:${b.id}`;Be[x]=`#${b.name}`;let C=document.createElement("option");C.value=x,C.textContent=`#${b.name} (${b.guild})`,a.appendChild(C)}),a.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{y(l.value)},l.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onchange=()=>f(),g.dataset.listenerAttached="true");function f(){if(!t)return;let c=g?.value||"all",p=c==="all"?Ie:Ie.filter(x=>(x.recipients||(x.owner_id?[x.owner_id]:[])).includes(c));if(p.length===0){t.innerHTML=j("empty",c==="all"?"No active tasks.":"No tasks found for this owner.",B()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let b=p.map(x=>{let C=x.last_run===0?"Never":new Date(x.last_run*1e3).toLocaleString(),$=x.memory?x.memory.length:0,M=x.status==="active"?"#03dac6":"#666",H=(x.recipients||(x.owner_id?[x.owner_id]:[])).map(N=>{let P=Be[N]||N.substring(0,8),q=N.startsWith("channel:");return`<span title="${P}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${N==="dexter"?"bx-terminal":q?"bx-hash":"bx-user"}'></i>${P}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${M}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px;">
                            ...
                            <span style="font-size: 0.85em; color: #03dac6; font-weight: bold;">${$}</span>
                        </div>
            ...
            `}).join("");t.innerHTML=b,t.querySelectorAll(".edit-chore-btn").forEach(x=>{x.onclick=C=>{C.stopPropagation();let $=x.dataset.id,M=Ie.find(T=>T.id===$);M&&k(M)}}),t.querySelectorAll(".delete-chore-btn").forEach(x=>{x.onclick=async C=>{C.stopPropagation();let $=x.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${$}`,{method:"DELETE"}),Fe())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display==="none"||he!==null?k(null):s.style.display="none")},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{s&&(s.style.display="none")},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let c=document.getElementById("new-chore-instruction"),p=document.getElementById("new-chore-url"),b=document.getElementById("new-chore-schedule"),x=c?.value,C=b?.value||"every_24h";if(!x)return;let $=n.innerHTML;n.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let M=he?"PATCH":"POST",T=he?`/chores/${he}`:"/chores";await _(T,{method:M,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:x,entry_url:p?.value,schedule:C})}),s&&(s.style.display="none"),c&&(c.value=""),p&&(p.value=""),he=null,se=[],Fe()}catch(M){console.error(M),alert(he?"Failed to update research task":"Failed to create research task")}finally{n.innerHTML=$}},n.dataset.listenerAttached="true");try{let c=await _("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);if(Ie=await c.json()||[],g){let b=g.value,x=new Set;Ie.forEach(C=>{C.owner_id&&x.add(C.owner_id),C.recipients&&C.recipients.forEach($=>x.add($))}),g.innerHTML='<option value="all">All Recipients</option>',x.forEach(C=>{let $=document.createElement("option");$.value=C,$.textContent=Be[C]||`Recipient: ${C.substring(0,8)}`,g.appendChild($)}),g.value=b,g.selectedIndex===-1&&(g.value="all")}f()}catch(c){console.error("Failed to fetch chores",c)}}var Nt=()=>`
...
`;async function Xe(){await Promise.all([ve(),xe(),Me(),Fe()])}var zs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,Je=null,we=new Set,_e=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Os={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function js(t){for(let[e,s]of Object.entries(Ps))if(s.includes(t))return e;return"system"}function Us(t){return Os[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;qs();let n=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(n+=`&category=${Z}`);try{let i=await _(n);if(!i.ok)throw new Error("Service unreachable");if(_e=((await i.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let y=o.type;return!(y==="system.blueprint.generated"||y==="system.notification.generated")}),Je=Date.now(),te(1,Je),_e.length===0){e.innerHTML=j("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let y=o.type,k=js(y),w=Us(y),a=y==="engagement.decision"||y==="messaging.bot.sent_message"||y==="messaging.user.sent_message"||y==="moderation.explicit_content.deleted"||y==="analysis.link.completed"||y==="analysis.visual.completed"||y==="analysis.router.decision"||y==="analysis.user.message_signals"||y==="system.cli.command"||y==="system.analysis.audit"||y==="system.test.completed"||y==="error_occurred"||y==="system.cli.status"||y==="system.attention.expired"||y.startsWith("system.roadmap")||y.startsWith("system.process"),f="event-border-grey";a&&(y==="moderation.explicit_content.deleted"||y==="error_occurred"?f="event-border-red":y==="analysis.link.completed"||y==="analysis.visual.completed"||y==="analysis.router.decision"||y==="system.analysis.audit"||y==="analysis.user.message_signals"?f="event-border-purple":y==="system.attention.expired"||y==="system.cli.command"||y==="system.cli.status"?f="event-border-orange":y==="system.test.completed"?f=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":f="event-border-blue");let c=a?"cursor-pointer":"",p=we.has(v.id),b=p?"expanded":"",x=p?"display: block;":"display: none;",C=new Date(v.timestamp*1e3),$=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),M=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=Ft(y,o),H=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",N="";if(a){let A="";if(y==="engagement.decision"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                    ...
                    `}else if(y==="system.attention.expired"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=o.timestamp-o.last_active,I=Math.floor(S/60);A=`
                    ...
                    `}else if(y==="messaging.bot.sent_message"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S="";o.eval_count&&(S=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `}
                    `}else if(y==="analysis.user.message_signals"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=o.signals||{};A=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(y==="moderation.explicit_content.deleted"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="analysis.link.completed"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="analysis.router.decision"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="system.cli.command"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="system.analysis.audit"){let E=o.success?"#03dac6":"#ff4d4d",S=o.success?"SUCCESS":"FAILED",I=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,O="";o.error&&(O=`
                            <div class="event-detail-block">
                                ${I("Error")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${E}; font-weight: bold;">${S} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${o.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${L}
                        ${z}
                    `}else if(y==="system.test.completed"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="error_occurred"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(y==="system.cli.status"){let E=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(y==="messaging.user.sent_message"){let E="";o.attachments&&o.attachments.length>0&&(E=`
                            <div class="event-detail-block">
                ...
                `}let P=document.createElement("div");P.className=`event-item ${f} ${c} ${b}`,P.dataset.eventId=v.id,P.onclick=function(A){if(!a)return;let E=this;if(E.classList.contains("expanded")){E.classList.remove("expanded"),we.delete(v.id);let I=E.querySelector(".event-details");I&&(I.style.display="none")}else{E.classList.add("expanded"),we.add(v.id);let I=E.querySelector(".event-details");I&&(I.style.display="block")}},P.innerHTML=`
                <div class="event-time">
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${r.id}" title="Clear Logs" style="${B()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let g=r,d=unescape(g.dataset.logs||"");navigator.clipboard.writeText(d).then(()=>{let u=g.querySelector("i");u&&(u.classList.remove("bx-copy"),u.classList.add("bx-check"),setTimeout(()=>{u.classList.remove("bx-check"),u.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let d=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${d}?`))try{(await _(`/logs?service_id=${d}`,{method:"DELETE"})).ok&&He()}catch(u){console.error("Error clearing logs:",u)}})}),Js=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=j("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Zt=()=>{let t=B()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
        </div>`;async function ss(){Ws(),await Promise.all([ne(),nt(),it()]),await He()}var We=null;function Ws(){We||(We=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(We),We=null;return}ne(!0)},1e3))}var Gt=null,Vt=null;async function ns(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function Kt(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Gs(){try{return await(await _("/processes")).json()}catch{return null}}async function Vs(){try{return await(await _("/agent/status")).json()}catch{return null}}async function nt(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),n=document.querySelector("#hw-cpu .hw-content"),i=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),g=a=>{if(a){if(s&&(s.innerHTML=`
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let f=a.status==="online",c=f?"service-widget-online":"service-widget-offline",p=f?"bx-check-circle":"bx-x-circle",b=f?"OK":"BAD",x=a.version?h(a.version.str):"-",C=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",$=a.memory&&a.memory!=="N/A"?a.memory:"-",M=C!=="-"?"#00ff00":"#666",T=C!=="-"?"#fff":"#666",H=$!=="-"?"#00ff00":"#666",N=$!=="-"?"#fff":"#666",P=v(a.uptime),q="";f?q=`
                ...
                </div>`}function r(d){let u=d.status==="Downloaded",h=u?"service-widget-online":"service-widget-offline",m=u?"OK":"MISSING",v=u&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",o=d.name;return d.type==="custom"&&o.endsWith(":latest")&&(o=o.replace(":latest","")),`<div class="service-widget ${h}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${u?"bx-check-circle":"bx-x-circle"}"></i><h3>${o}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let g="";if(n&&(g+=i(n)),e.xtts&&(g+=l(e.xtts)),g+=s.map(r).join(""),!g){t.innerHTML=j("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let s=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),i=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn"),u=document.getElementById("fabricator-reset-btn"),h=document.getElementById("courier-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let p=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _(p,{method:"POST"}),await ne()}catch{m.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},m.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{d.innerHTML="<i class='bx bx-error'></i>"}},d.dataset.listenerAttached="true"),u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{u.innerHTML="<i class='bx bx-error'></i>"}},u.dataset.listenerAttached="true"),h&&!h.dataset.listenerAttached&&(h.onclick=async()=>{h.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{h.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{h.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{h.innerHTML="<i class='bx bx-error'></i>"}},h.dataset.listenerAttached="true");let v=await Vs();if(v&&v.agents){let c=v.agents.guardian||{protocols:{}},p=v.agents.analyzer||{protocols:{}},b=v.agents.imaginator||{protocols:{}},x=v.agents.fabricator||{protocols:{}},C=v.agents.courier||{protocols:{}},$=v.system||{},M=Math.floor(Date.now()/1e3),T={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},H=L=>{L<0&&(L=0);let F=Math.floor(L/3600),D=Math.floor(L%3600/60),J=L%60;return F>0?`${F}h ${D}m`:D>0?`${D}m ${J}s`:`${J}s`},N=(L,F,D,J)=>{if(!L)return;let V=T[J]||J.toUpperCase(),ie=L.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(ie&&(ie.textContent=V),!D){L.textContent="Inactive",L.style.color="#666";return}let X=D.status;if(X==="Working")L.textContent="Working",L.style.color="#bb86fc";else if(X==="Ready")L.textContent="Ready",L.style.color="#5eff5e";else{let K=D.cooldown;if(B()&&(K=D.next_run-M),K<=0)L.textContent="Ready",L.style.color="#5eff5e";else{let De=Math.floor(K/60),rt=K%60;L.textContent=`${De}m ${rt}s`,L.style.color="#fff"}}F&&D.stats&&(F.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">

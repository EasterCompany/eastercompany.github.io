"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.prepend(t)}}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,l=document.createElement("nav");l.innerHTML=a,document.body.prepend(l);let r=document.createElement("div");r.id="dexter-dropdown",r.className="dexter-dropdown",r.innerHTML=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function ue(t){let e=null,o=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++ut).toString())}function l(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",r),i&&setTimeout(i,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",a);let v=t.icon||"bx-window",s="",u="",C,M='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((x,y)=>{let A=x.icon?`<i class="bx ${x.icon}"></i>`:`<span class="tab-glyph">${x.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,C=`<div class="window-content">${t.tabs.map((x,y)=>`<div class="tab-content ${y===0?"active":""}" data-tab-content="${y}">${x.content}${M}</div>`).join("")}</div>`):(t.title&&(u=`<div class="window-title">${t.title}</div>`),C=`<div class="window-content">${t.content||""}${M}</div>`),e.innerHTML=`
            ...
            ${C}
        `,p.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",g=>{g.stopPropagation(),c()}),window.addEventListener("resize",r),t.tabs&&e.querySelectorAll(".tab").forEach(g=>{g.addEventListener("click",()=>{if(!e)return;let d=g.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(y=>y.classList.remove("active")),g.classList.add("active"),e.querySelectorAll(".tab-content").forEach(y=>y.classList.remove("active"));let x=e.querySelector(`.tab-content[data-tab-content="${d}"]`);x&&x.classList.add("active"),f(g,e)})}),setTimeout(()=>{e&&e.classList.add("open"),i&&i()},10)}function r(){if(!e||!e.classList.contains("open"))return;let p=e.querySelector(".tab.active");p&&f(p,e)}function f(p,v){setTimeout(()=>{let s=v.querySelector(".tab-bar");if(!s)return;let u=Array.from(s.querySelectorAll(".tab")),C=u.indexOf(p),M=s.clientWidth,g=u[Math.max(0,C-2)],d=u[Math.min(u.length-1,C+2)],x=s,y=g.offsetLeft-x.offsetLeft-25,n=d.offsetLeft+d.offsetWidth-x.offsetLeft+25-y,b=n<=M?y-(M-n)/2:p.offsetLeft-x.offsetLeft-M/2+p.offsetWidth/2;s.scrollTo({left:b,behavior:"smooth"})},350)}function c(p=!1){e&&(window.removeEventListener("resize",r),p?(e.remove(),e=null):(e.classList.remove("open"),o&&o(),setTimeout(()=>{e?.remove(),e=null},400)))}function h(p){t.content=p;let v=e?.querySelector(".window-content");v&&(v.innerHTML=p)}function E(){return!!(e&&e.classList.contains("open"))}return{open:l,close:c,setContent:h,isOpen:E,focus:a,id:t.id}}var ft="easter_theme",ee={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Ye(){return localStorage.getItem(ft)||ee.DARK}function gt(t){if(!Object.values(ee).includes(t))throw new Error("Invalid theme");localStorage.setItem(ft,t),bt(t)}function bt(t){let e=document.body;Object.values(ee).forEach(l=>{e.classList.remove(`theme-${l}`)}),e.classList.add(`theme-${t}`);let o=document.querySelector('meta[name="theme-color"]');o||(o=document.createElement("meta"),o.name="theme-color",document.getElementsByTagName("head")[0].appendChild(o));let i={[ee.DARK]:"#050507",[ee.LIGHT]:"#FFFFFF",[ee.LEGACY]:"#000000"},a=i[t]||i[ee.DARK];if(o.setAttribute("content",a),!document.querySelector(".background")){let l=document.createElement("div");l.className="background",document.body.prepend(l)}e.classList.add("is-animated")}function vt(){let t=Ye();bt(t)}function J(t,e,o=null,i="default"){let l={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=o?`<p class="placeholder-action">${o}</p>`:"";return`<div class="tab-placeholder" data-placeholder-id="${i}"><i class='bx ${l} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function j(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function te(t,e){let o=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!o)return;if(!e){o.textContent="Last updated: never";return}let i=Date.now(),a=Math.floor((i-e)/1e3),l;a<60?l=`${a}s ago`:a<3600?l=`${Math.floor(a/60)}m ago`:l=`${Math.floor(a/3600)}h ago`,o.textContent=`Last updated: ${l}`}var ms=0;function Oe(t,e){let o=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!o)return;let i=o.querySelector(".notification-badge");i&&(e>0?(i.textContent=e>9?"9+":e.toString(),i.style.display="flex"):i.style.display="none")}var ae=0,oe=0;function xt(t){ae=t,Le()}function ht(t){oe=t,Le()}function Le(){let t=ae+oe;ms=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let o=document.getElementById("alerts-menu-item");if(o){let r=o.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",o.appendChild(r)),r.textContent=ae>9?"9+":ae.toString(),r.style.display=ae>0?"flex":"none"}let i=document.getElementById("workspace-menu-item");if(i){let r=i.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",i.appendChild(r)),r.textContent=oe>9?"9+":oe.toString(),r.style.display=oe>0?"flex":"none"}let a=document.getElementById("switch-alerts");if(a){let r=a.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",a.appendChild(r)),r.textContent=ae>9?"9+":ae.toString(),r.style.display=ae>0?"flex":"none"}let l=document.getElementById("switch-workspace");if(l){let r=l.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",l.appendChild(r)),r.textContent=oe>9?"9+":oe.toString(),r.style.display=oe>0?"flex":"none"}}function Qe(){let t=document.getElementById("alerts-list");if(!t)return;ae=t.querySelectorAll(".alert-unread:not(.priority-low)").length,Le()}function wt(){let t=document.getElementById("blueprints-list");if(!t)return;oe=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,Le()}function us(){return"https://event.easter.company"}function fs(){return"https://discord.easter.company"}var gs="http://127.0.0.1:8100",bs="http://127.0.0.1:8300",vs={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function je(t){let e=j(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(a,l)=>{let r=vs[l];return r?`<span class="${r}">`:""});let o=(e.match(/<span/g)||[]).length,i=(e.match(/<\/span/g)||[]).length;return o>i&&(e+="</span>".repeat(o-i)),e}function le(t){if(!t)return"";let e=j(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^<strong>(.*)<\/strong>$/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(o,i)=>{let a=i.split("|").map(l=>l.trim());return a.every(l=>l.match(/^:?-+:?$/))?"":`<div class="md-table-row">${a.map(l=>`<span class="md-table-cell">${l}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}function ie(){let t=document.getElementById("windows-container"),e=t?parseInt(t.getAttribute("data-count")||"0"):0,o=document.querySelector(".profile-overlay.active"),i=document.querySelector("#cli-terminal-overlay.active");return e>0||!!o||!!i}var ys="https://sterling-javelin-12539.upstash.io",xs="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function P(){return window.location.hostname.includes("easter.company")}var F=null,re={lastDashboard:0,lastFrontend:0},Xe=!1,Ct=0,Et="dex_dashboard_snapshot";function hs(){let t=localStorage.getItem(Et);if(t)try{let e=JSON.parse(t);F=e,re.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{F=null}}async function yt(){if(!(!P()||Xe)){Xe=!0,Ct=Math.floor(Date.now()/1e3);try{let t=await Es("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);F=e,re.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),re.lastFrontend=Date.now(),localStorage.setItem(Et,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Xe=!1}}}function ws(){if(!F||!F.agent_status)return;let t=F.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function Cs(){if(!P())return;hs();let t=Math.floor(Date.now()/1e3),e=F?t-F.timestamp:1/0;(!F||e>120)&&await yt(),setInterval(()=>{let o=new Date,i=Math.floor(Date.now()/1e3),a=F?i-F.timestamp:1/0,l=i-Ct,r=o.getSeconds()===59,f=a>300,c=l>60;(r&&c||f&&c)&&yt(),ws()},1e3)}P()&&Cs();async function Es(t,...e){try{let i=await(await fetch(ys,{method:"POST",headers:{Authorization:`Bearer ${xs}`},body:JSON.stringify([t,...e])})).json();if(i.error)throw new Error(i.error);return i.result}catch(o){return console.error("Upstash Error:",o),null}}var Se=null,ke=null,Pe=!1,Re=!1;async function D(t,e={}){if(P()){if(!F)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(F.monitor),{status:200});if(t.startsWith("/system/options"))return new Response(JSON.stringify(F.options||{}),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(F.processes),{status:200});if(t.startsWith("/events")){let a=new URLSearchParams(t.split("?")[1]||""),l=a.get("type")||a.get("event.type"),r=a.get("category"),f=a.get("order")||"desc",c=[];return l==="system.notification.generated"?c=[...F.alerts||[]]:l==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?c=[...F.blueprints||[]]:r==="messaging"?c=[...F.messaging_events||[]]:r==="system"?c=[...F.system_events||[]]:r==="cognitive"?c=[...F.cognitive_events||[]]:r==="moderation"?c=[...F.moderation_events||[]]:c=[...F.events||[]],f==="asc"?c.sort((h,E)=>h.timestamp-E.timestamp):c.sort((h,E)=>E.timestamp-h.timestamp),new Response(JSON.stringify({events:c,count:c.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(F.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let a=t.split("/")[2],l=F.profiles?F.profiles[a]:null;return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(F.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify(F.chores||[]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(Se)try{let a=await fetch(Se+t,e);if(a.ok)return a;Se=null}catch{Se=null}let o=us(),i=gs;try{let a=await fetch(o+t,e);if(a.ok)return Se=o,Pe&&(console.log("\u2728 Production event service restored."),Pe=!1),a;throw new Error("Primary failed")}catch{Pe||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${i}`),Pe=!0);try{let l=await fetch(i+t,e);if(l.ok)return Se=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}async function fe(t,e={}){if(P()){if(!F)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(F.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let a=t.split("/")[2],l=(F.contacts?.members||[]).find(r=>r.id===a);return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(ke)try{let a=await fetch(ke+t,e);if(a.ok)return a;ke=null}catch{ke=null}let o=fs(),i=bs;try{let a=await fetch(o+t,e);if(a.ok)return ke=o,Re&&(console.log("\u2728 Production discord service restored."),Re=!1),a;throw new Error("Primary failed")}catch{Re||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${i}`),Re=!0);try{let l=await fetch(i+t,e);if(l.ok)return ke=i,l;throw new Error("Fallback failed")}catch(l){throw l}}}var Tt=()=>`
...
`,qe=null,ge=new Set,Ue=[];async function pe(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Ts(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await D(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];qe=Date.now(),te(0,qe);let r=Date.now(),f=24*60*60*1e3,c=l.filter(d=>{let x=localStorage.getItem(`alert_read_ts_${d.id}`);if(!x)return!0;let y=parseInt(x);return r-y<f});c.sort((d,x)=>{let y=w=>{let m=w.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},A=w=>w==="critical"?4:w==="high"?3:w==="medium"?2:1,n=A(y(d)),b=A(y(x));return n!==b?b-n:x.timestamp-d.timestamp}),Ue=c;let h=d=>{let x=d.event;if(typeof x=="string")try{x=JSON.parse(x)}catch{return"low"}return(x.priority||"low").toLowerCase()},E=[];if(c.length===0)E.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let x=new Set(c.map(A=>h(A))).size>1,y=null;c.forEach(A=>{let n=h(A);x&&n!==y&&(E.push({id:`divider-${n}`,type:"divider",label:n.toUpperCase()}),y=n),E.push(A)})}t&&(e.innerHTML="");let p=d=>{let x=d.event;if(typeof x=="string")try{x=JSON.parse(x)}catch{return null}let y=(x.title||"Untitled Alert").trim(),A=(x.body||"No description provided.").trim(),n=x.summary||"",b=x.content||"",w=x.protocol||"",m=(x.priority||"low").toLowerCase(),$=!!x.alert,B=(x.category||"system").trim(),S=x.related_event_ids||[],H=x.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),V=new Date(d.timestamp*1e3),_=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=q?"event-border-grey":"event-border-blue";!q&&$&&(L="event-border-red"),q&&(m==="high"||m==="critical")?L="event-border-red":q&&m==="medium"&&(L="event-border-orange");let I=q?"alert-read":"alert-unread",O=ge.has(d.id),R=O?"expanded":"",G=O?"display: block;":"display: none;",k="",z="";S.length>0&&(z=`
            ...
            <div style="flex: 1; min-width: 120px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    <a href="#" onclick="window.dexter.viewEvent('${H}'); return false;" style="color: #bb86fc; text-decoration: none; border-bottom: 1px dashed rgba(187, 134, 252, 0.3); white-space: nowrap;">${H.substring(0,8)}...</a>
                </div>
            </div>`);let U="";w&&(U=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${le(b)}</div>
            </div>
        `:Y=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${le(A)}</div>
            </div>
        `,k=`
            ...
            `;let K=document.createElement("div");K.className=`event-item notification-item ${L} ${I} ${R} cursor-pointer priority-${m}`,K.dataset.alertId=d.id,K.onclick=function(de){let Q=this;if(Q.classList.contains("expanded")){Q.classList.remove("expanded"),ge.delete(d.id);let Be=Q.querySelector(".event-details");Be&&(Be.style.display="none")}else{Q.classList.add("expanded"),ge.add(d.id);let Be=Q.querySelector(".event-details");if(Be&&(Be.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),Q.classList.add("alert-read"),Q.classList.remove("alert-unread"),Q.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ze="event-border-grey";m==="high"||m==="critical"?Ze="event-border-red":m==="medium"&&(Ze="event-border-orange"),Q.classList.add(Ze),Qe()}}};let Ee=`${w?w.toUpperCase():"GUARDIAN"} ALERT: ${n||y}`,Ve={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[B]||"bx-bell",Ke=m==="high"||m==="critical"?"#ff4d4d":m==="medium"?"#ffa500":"#888";K.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Ke}; font-size: 0.8em; margin-left: 5px;">[${m.toUpperCase()}]</span>
                    </div>
            ...
            `;let lt=K.querySelector(".event-details");lt&&lt.addEventListener("click",de=>{de.stopPropagation()});let ct=K.querySelector(".close-details-btn");return ct&&ct.addEventListener("click",de=>{de.stopPropagation(),K.classList.remove("expanded");let Q=K.querySelector(".event-details");Q&&(Q.style.display="none"),ge.delete(d.id)}),K},v=d=>{let x=document.createElement("div");x.className="notification-divider",x.dataset.alertId=d.id;let y="#888888";return d.label==="CRITICAL"?y="#ff4d4d":d.label==="HIGH"?y="#ff8888":d.label==="MEDIUM"&&(y="#ffa500"),x.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${y}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,x.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${y}44, transparent); flex-grow: 1;"></div>`,x},s=d=>{let x=document.createElement("div");x.innerHTML=J("empty",d.message,null,d.id);let y=x.firstElementChild;return y.dataset.alertId=d.id,y},u=Array.from(e.children),C=new Map(u.map(d=>[d.dataset.alertId,d])),M=new Set(E.map(d=>d.id));u.forEach(d=>{let x=d.dataset.alertId;(!x||!M.has(x))&&d.remove()});let g=null;E.forEach((d,x)=>{let y=C.get(d.id);if(!y||t){if(y&&y.remove(),d.type==="divider")y=v(d);else if(d.type==="placeholder")y=s(d);else{let A=p(d);if(!A)return;y=A}if(!y)return}x===0?e.firstElementChild!==y&&e.prepend(y):g&&g.nextElementSibling!==y&&g.after(y),g=y}),qe=Date.now(),te(0,qe),Qe()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=J("offline","Failed to load alerts.","The event service may be offline."))}}function Ts(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),o=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ue.forEach(a=>{localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),pe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Ue.forEach(a=>{ge.add(a.id),localStorage.getItem(`alert_read_ts_${a.id}`)||localStorage.setItem(`alert_read_ts_${a.id}`,Date.now().toString())}),pe(!0)},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{ge.clear(),pe(!0)},o.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await D("/events?type=system.notification.generated",{method:"DELETE"});let a=Date.now()-48*60*60*1e3;Ue.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,a.toString())}),ge.clear(),pe(!0)}catch(a){console.error("Failed to clear alerts:",a)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function et(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await D(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||a++}),xt(a)}catch{}}var $t=()=>`
...
`,be=new Set,kt=[],Me=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){$s();try{let o=await D("/roadmap");if(!o.ok)throw new Error("Offline");let i=await o.json();kt=i;let a=p=>{let v=be.has(p.id),s=p.state==="published",u=p.state==="consumed",C="event-border-grey";s&&(C="event-border-blue"),u&&(C="event-border-purple");let g=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div");d.className=`event-item notification-item ${C} cursor-pointer ${v?"expanded":""}`,d.dataset.itemId=p.id,d.onclick=A=>{if(A.target.closest("button"))return;if(d.classList.contains("expanded")){d.classList.remove("expanded");let b=d.querySelector(".event-details");b&&(b.style.display="none"),be.delete(p.id)}else{d.classList.add("expanded");let b=d.querySelector(".event-details");b&&(b.style.display="block"),be.add(p.id)}};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;d.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${j(p.content)}</div>
          <div class="event-details" style="${v?"display: block;":"display: none;"} ">
            ...
            `}
            ${u?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(p.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `;let y=d.querySelector(".event-details");return y&&y.addEventListener("click",A=>{A.stopPropagation()}),d.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(p)),d.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ks(p)),d.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(p.id)),d.querySelector(".close-details-btn")?.addEventListener("click",A=>{A.stopPropagation(),d.classList.remove("expanded");let n=d.querySelector(".event-details");n&&(n.style.display="none"),be.delete(p.id)}),d},l=p=>{let v=document.createElement("div");v.innerHTML=J("empty",p.message,p.action,p.id);let s=v.firstElementChild;return s.dataset.itemId=p.id,s},r=[];!i||i.length===0?r.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:P()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(p=>r.push({...p,type:"item"}));let f=Array.from(e.children),c=new Map(f.map(p=>[p.dataset.itemId,p])),h=new Set(r.map(p=>p.id));f.forEach(p=>{let v=p.dataset.itemId;(!v||!h.has(v))&&p.remove()}),t&&(e.innerHTML="");let E=null;r.forEach((p,v)=>{let s=c.get(p.id);(!s||t)&&(s&&s.remove(),p.type==="placeholder"?s=l(p):s=a(p),!s)||(v===0?e.firstElementChild!==s&&e.prepend(s):E&&E.nextElementSibling!==s&&E.after(s),E=s)})}catch{e.children.length===0&&(e.innerHTML=J("offline","Failed to load roadmap.","The event service may be offline."))}}}function $s(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),o=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Me=null},o.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let f=Me?`/roadmap/${Me}`:"/roadmap",c=Me?"PATCH":"POST";try{await D(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let h=document.getElementById("roadmap-editor-container");h&&(h.style.display="none"),ve(!0)}catch(h){console.error(h)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(l=>be.add(l.id)),ve(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{be.clear(),ve(!0)},a.dataset.listenerAttached="true")}function Ss(t){Me=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block",o.scrollIntoView({behavior:"smooth"}))}async function ks(t){let e=t.state==="published"?"draft":"published";try{await D(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(o){console.error(o)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await D(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Lt=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ms();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];At=l,Lt=Date.now(),te(2,Lt);let r=s=>{let u=s.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let C=(u.title||"Untitled Blueprint").trim(),M=(u.summary||u.body||"No summary provided.").trim(),g=(u.content||"").trim(),d=(u.category||"architecture").trim(),x=(u.related_services||u.affected_services||[]).map(k=>k.trim()),y=(u.implementation_path||[]).map(k=>k.trim()),A=u.source_event_ids||[],n=u.approved===!0,b=new Date(s.timestamp*1e3),w=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),m=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ye.has(s.id),B=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${n?"blueprint-approved":""}`,S.dataset.blueprintId=s.id,n&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=n?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[d]||"bx-paint";S.onclick=function(k){let z=this;if(z.classList.contains("expanded")){z.classList.remove("expanded"),ye.delete(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="none")}else{z.classList.add("expanded"),ye.add(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="block")}};let q="";y.length>0&&(q=`
                ...
                `);let _=x.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${x.join(", ")}</span></div>`:"<div></div>",T=P(),L=n?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-icon" style="${n?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${B}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${j(M)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${j(g)}</div>
                        </div>
                        ${q}
                        ${V}
                        ${L}
            ...
            `;let I=S.querySelector(".blueprint-approve-btn");I&&(I.onclick=async k=>{k.stopPropagation(),I.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await D(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(z){console.error("Failed to approve blueprint:",z)}});let O=S.querySelector(".blueprint-delete-btn");O&&(O.onclick=async k=>{k.stopPropagation();let z=!n;O.innerHTML=z?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await D(`/events/${s.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(N){console.error("Failed to delete blueprint:",N)}});let R=S.querySelector(".event-details");R&&R.addEventListener("click",k=>{k.stopPropagation()});let G=S.querySelector(".close-details-btn");return G&&G.addEventListener("click",k=>{k.stopPropagation(),S.classList.remove("expanded");let z=S.querySelector(".event-details");z&&(z.style.display="none"),ye.delete(s.id)}),S},f=s=>{let u=document.createElement("div");u.innerHTML=J("empty",s.message,s.action,s.id);let C=u.firstElementChild;return C.dataset.blueprintId=s.id,C},c=[];l.length===0?c.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):l.forEach(s=>c.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let h=Array.from(e.children),E=new Map(h.map(s=>[s.dataset.blueprintId,s])),p=new Set(c.map(s=>s.id));h.forEach(s=>{let u=s.dataset.blueprintId;(!u||!p.has(u))&&s.remove()});let v=null;c.forEach((s,u)=>{let C=E.get(s.id);(!C||t)&&(C&&C.remove(),s.type==="placeholder"?C=f(s):C=r(s),!C)||(u===0?e.firstElementChild!==C&&e.prepend(C):v&&v.nextElementSibling!==C&&v.after(C),v=C)}),Oe(2,l.length),wt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=J("offline","Failed to load blueprints.","The event service may be offline."))}}async function Bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await D(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&a++}),ht(a)}catch{}}function Ms(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),o=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(i=>ye.add(i.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await D("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}var ce="STANDBY",Ht=null,Ie=[],X=null,Fe=null,_t=()=>`
...
`,Nt=()=>`
    ...
    `,He=[],_e={"313071000877137920":"Owen",dexter:"Dexter"},se=[],he=null;async function De(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),o=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),f=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),h=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),p=document.getElementById("new-chore-time"),v=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(v&&!v.dataset.initialValueAttached){let n=Intl.DateTimeFormat().resolvedOptions().timeZone,b=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===n){b=!0;break}if(!b){let w=document.createElement("option");w.value=n,w.textContent=n.replace("_"," "),v.appendChild(w)}v.value=n,v.dataset.initialValueAttached="true"}E&&s&&!E.dataset.timeListenerAttached&&(E.addEventListener("change",()=>{s.style.display=E.value==="daily"?"block":"none"}),E.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),C=document.getElementById("form-icon");function M(){if(r){if(se.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=se.sort((n,b)=>{let w=n.startsWith("channel:"),m=b.startsWith("channel:");return w&&!m?-1:!w&&m?1:0}).map(n=>{let b=_e[n]||n,w=n.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${n==="dexter"?"#bb86fc":w?"#03dac6":"#fff"};">
          <i class='bx ${n==="dexter"?"bx-terminal":w?"bx-hash":"bx-user"}'></i>
          <span>${b}</span>
          <i class='bx bx-x remove-recipient' data-id="${n}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(n=>{n.onclick=()=>{let b=n.dataset.id;se=se.filter(w=>w!==b),M()}})}}function g(n){n&&!se.includes(n)&&(se.push(n),M()),l&&(l.value="")}function d(n=null){if(o){if(he=n?n.id:null,n){if(u&&(u.textContent="Update Research Task"),C&&(C.className="bx bx-edit-alt",C.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=n.natural_instruction),h&&(h.value=n.execution_plan?.entry_url||""),E&&(E.value=n.schedule,s&&(s.style.display=n.schedule==="daily"?"block":"none")),p&&n.run_at&&(p.value=n.run_at),v&&n.timezone){let b=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===n.timezone){b=!0;break}if(!b){let w=document.createElement("option");w.value=n.timezone,w.textContent=n.timezone.replace("_"," "),v.appendChild(w)}v.value=n.timezone}se=n.recipients||(n.owner_id?[n.owner_id]:[]),M()}else u&&(u.textContent="Initialize Research Task"),C&&(C.className="bx bx-plus-circle",C.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),h&&(h.value=""),E&&(E.value="every_24h"),se=[],M();o.style.display="block",o.scrollIntoView({behavior:"smooth",block:"start"})}}let x=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(x&&!x.dataset.populated&&!P())try{fe("/contacts").then(async n=>{n.ok&&(((await n.json()).members||[]).forEach(m=>{if(_e[m.id]=m.nickname||m.username,m.id==="313071000877137920")return;let $=document.createElement("option");$.value=m.id;let B=m.nickname||m.username;$.textContent=`${B} (${m.username})`,x.appendChild($)}),x.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!P())try{fe("/channels").then(async n=>{n.ok&&((await n.json()).forEach(w=>{let m=`channel:${w.id}`;_e[m]=w.name;let $=document.createElement("option");$.value=m,$.textContent=`#${w.name} (${w.guild})`,y.appendChild($)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{g(l.value)},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onchange=()=>A(),f.dataset.listenerAttached="true");function A(){if(!t)return;let n=f?.value||"all",b=n==="all"?He:He.filter(m=>(m.recipients||(m.owner_id?[m.owner_id]:[])).includes(n));if(b.length===0){t.innerHTML=J("empty",n==="all"?"No active tasks.":"No tasks found for this owner.",P()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let w=b.map(m=>{let $=m.last_run===0?"Never":new Date(m.last_run*1e3).toLocaleString(),B=m.memory?m.memory.length:0,S=m.status==="active"?"#03dac6":"#666",W=(m.recipients||(m.owner_id?[m.owner_id]:[])).sort((q,V)=>{let _=q.startsWith("channel:"),T=V.startsWith("channel:");return _&&!T?-1:!_&&T?1:0}).map(q=>{let V=_e[q]||q.substring(0,8),_=q.startsWith("channel:");return`<span title="${V}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${q==="dexter"?"bx-terminal":_?"bx-hash":"bx-user"}'></i>${V}</span>`}).join("<span style='color: #444;'>, </span>");return`
            ...
            `}).join("");t.innerHTML=w,t.querySelectorAll(".edit-chore-btn").forEach(m=>{m.onclick=$=>{$.stopPropagation();let B=m.dataset.id,S=He.find(H=>H.id===B);S&&d(S)}}),t.querySelectorAll(".reset-chore-btn").forEach(m=>{m.onclick=async $=>{$.stopPropagation();let B=m.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(m.innerHTML="<i class='bx bx-loader-alt spin'></i>",await D(`/chores/${B}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),De())}}),t.querySelectorAll(".delete-chore-btn").forEach(m=>{m.onclick=async $=>{$.stopPropagation();let B=m.dataset.id;confirm("Delete this task?")&&(await D(`/chores/${B}`,{method:"DELETE"}),De())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{o&&(o.style.display==="none"||he!==null?d(null):o.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{o&&(o.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let n=document.getElementById("new-chore-instruction"),b=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),m=n?.value,$=w?.value||"every_24h",B=$==="daily"?p?.value:"",S=$==="daily"?v?.value:"";if(!m)return;if(se.length===0){alert("Please add at least one recipient.");return}let H=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=he?"PATCH":"POST",q=he?`/chores/${he}`:"/chores";await D(q,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:m,entry_url:b?.value,schedule:$,run_at:B,timezone:S})}),o&&(o.style.display="none"),n&&(n.value=""),b&&(b.value=""),he=null,se=[],De()}catch(W){console.error(W),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=H}},i.dataset.listenerAttached="true");try{let n=await D("/chores");if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);if(He=await n.json()||[],f){let w=f.value,m=new Set;He.forEach($=>{$.owner_id&&m.add($.owner_id),$.recipients&&$.recipients.forEach(B=>m.add(B))}),f.innerHTML='<option value="all">All Recipients</option>',m.forEach($=>{let B=document.createElement("option");B.value=$;let S=$.startsWith("channel:"),H=_e[$]||`Recipient: ${$.substring(0,8)}`;B.textContent=(S&&!H.startsWith("#")?"#":"")+H,f.appendChild(B)}),f.value=w,f.selectedIndex===-1&&(f.value="all")}A()}catch(n){console.error("Failed to fetch chores",n)}}var Pt=()=>`
...
`;async function tt(){await Promise.all([ve(),xe(),Ae(),De()])}var _s=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function qt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=_s,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `;let a=e.querySelector(".close-profile-btn");a&&a.addEventListener("click",()=>{e.click()})};fe(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();Ds(e,t,a)}else console.log("Profile not found (404) or error."),o("Profile Data Unavailable")}).catch(i=>{console.error("Profile fetch error:",i),o("Connection Failed")})}function Ds(t,e,o){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let C=(o.attributes||o.facts||[]).map(A=>{let n=A.key||A.k,b=A.value||A.v;return`
            <div class="fact-chip">
                <span class="fact-key">${n}:</span>
                <span class="fact-val">${b}</span>
            </div>
        `}).join(""),M=o.cognitive_model||o,g=M.technical_level||M.techLevel||0,d=M.communication_style||M.commStyle||"Unknown",x=M.patience_level||M.patience||"Unknown",y=M.vibe||"Unknown";return`
        ...
        `},l=()=>{let C=o.dossier||{},M=C.identity||{},g=C.career||{},d=C.personal||{},x=C.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${M.fullName||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Age Range</div>
                        <div class="dossier-value">${M.ageRange||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Location</div>
                        <div class="dossier-value">${M.location||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Gender</div>
                        <div class="dossier-value">${M.gender||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Sexuality</div>
                        <div class="dossier-value" style="color: #ffb74d;">${M.sexuality||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Relationship Status</div>
                        <div class="dossier-value">${M.relationship||"Unknown"}</div>
                    </div>
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(g.skills||[]).map(y=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${y}</span>`).join("")}
                         </div>
                    </div>
                </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin-top: 30px; text-align: center;">
            ${Object.entries(C).map(([M,g])=>`
                <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                    ...
                    <div style="font-size: 0.8em; text-transform: uppercase; letter-spacing: 1px; color: #aaa;">${M.substring(0,4)}</div>
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${j(e.username)}</h2>
                    <div class="profile-badges">${(o.badges||[]).map(C=>`<span class="profile-badge ${C==="Creator"?"master":""}">${C}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=h;let E=t.querySelector(".profile-card"),p=t.querySelector("#profile-expand-toggle"),v=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content"),u=t.querySelector(".close-profile-btn");u&&u.addEventListener("click",()=>{t.classList.remove("active"),ie()||(document.body.style.overflow=""),setTimeout(()=>t.remove(),300)}),v.forEach(C=>{C.addEventListener("click",()=>{v.forEach(d=>d.classList.remove("active")),s.forEach(d=>d.classList.remove("active")),C.classList.add("active");let M=C.dataset.tab,g=t.querySelector(`#tab-${M}`);g&&g.classList.add("active")})}),p&&p.addEventListener("click",()=>{E.classList.toggle("expanded");let C=E.classList.contains("expanded");p.innerHTML=C?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Ut=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function st(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await st(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=o=>{let a=o.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let l=JSON.parse(a.dataset.user);qt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let o=await fe("/contacts");if(!o.ok)throw new Error("Service unreachable");let a=(await o.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=J("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((r,f)=>{let c=l[r.level]??10,h=l[f.level]??10;return c!==h?c-h:r.username.localeCompare(f.username)}),t.innerHTML=a.map(r=>{let f=r.color&&r.color!==0,c=f?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",h=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",E="#888";r.level==="Me"||r.level==="Master"?E="#bb86fc":r.level==="Admin"?E="#cf6679":r.level==="Moderator"?E="#03dac6":r.level==="Contributor"&&(E="#ffa500");let p=r.level==="Me",v=p?"rgba(187, 134, 252, 0.08)":f?`${c}11`:"rgba(255,255,255,0.03)",s=p?"2px solid #bb86fc":f?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${p?"#bb86fc":f?c:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":f?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${f?c:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.nickname||r.username}</h3>
...
`,Je=null,we=new Set,ze=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,o]of Object.entries(Ps))if(o.includes(t))return e;return"system"}function js(t){return Rs[t]||"bx-square-rounded"}async function me(t=!1){let e=document.getElementById("events-timeline");if(!e)return;qs();let i=`/events?ml=${Z==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(ze=((await a.json()).events||[]).filter(v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Je=Date.now(),te(1,Je),ze.length===0){e.innerHTML=J("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,C=Os(u),M=js(u),g=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),d="event-border-grey";g&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?d="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?d="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?d="event-border-orange":u==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let x=g?"cursor-pointer":"",y=we.has(v.id),A=y?"expanded":"",n=y?"display: block;":"display: none;",b=new Date(v.timestamp*1e3),w=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),m=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Ft(u,s),B=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",S="";if(g){let _="";if(u==="engagement.decision"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                    ...
                    `}else if(u==="system.attention.expired"){let T=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.timestamp-s.last_active,I=Math.floor(L/60);_=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <div class="detail-pre">${le(s.last_input||"None")}</div>
                        </div>
                        <div class="event-detail-block">
                            ${T("Thought Process (Last Output)")}
                            <div class="detail-pre">${le(s.last_output||"None")}</div>
                        </div>
                    `}else if(u==="messaging.bot.sent_message"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,L="";if(s.embed){let R=s.embed,G=R.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(R.title)}</div>`:"",k=R.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${le(R.description)}</div>`:"",z=(R.fields||[]).map(N=>`
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${j(N.name)}</div>
                <div style="font-size: 0.85em; color: #eee; white-space: pre-wrap;">${le(N.value)}</div>
              </div>
            `).join("");L=`
            ...
            `}let I="";s.eval_count&&(I=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,G=s.chat_history.map((k,z)=>{let N=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&N==="ASSISTANT"&&(N="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,L="",I=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let R=O.match(/{.*?}/g);R&&(I=R.map(G=>{let k=G.substring(1,G.length-1),z=k.indexOf(" "),N=k.substring(0,z),U=k.substring(z+1);return U=U.replace(/\[\]$/,"").trim(),{role:N,content:U}}))}if(I.length>0&&s.response_raw&&I.push({role:"assistant",content:s.response_raw}),I.length>0){let R=I.length,G=I.map((k,z)=>{let N=k.role.toUpperCase();N==="ASSISTANT"&&(N="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${N}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${z+1} of ${R}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let T=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.signals||{};_=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.visual.completed"){let T=I=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${I}</h5>`,L="";s.base64_preview?L=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(L=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),_=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="analysis.router.decision"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let T=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",I=z=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${z}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${I("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let z=s.chat_history.length,N=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ee=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let z=s.corrections.map((N,U)=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${T}; font-weight: bold;">${L} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${G}
                        ${R}
                    `}else if(u==="system.test.completed"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                ...
                `}let H=document.createElement("div");H.className=`event-item ${d} ${x} ${A}`,H.dataset.eventId=v.id,H.onclick=function(_){if(!g)return;let T=this;if(T.classList.contains("expanded")){T.classList.remove("expanded"),we.delete(v.id);let I=T.querySelector(".event-details");I&&(I.style.display="none")}else{T.classList.add("expanded"),we.add(v.id);let I=T.querySelector(".event-details");I&&(I.style.display="block")}},H.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${m}</span>
                </div>
            ...
            `;let W=H.querySelector(".event-details");if(W&&W.addEventListener("click",_=>{_.stopPropagation()}),g){let _=H.querySelector(".close-details-btn");_&&_.addEventListener("click",T=>{T.stopPropagation();let L=T.target.closest(".event-item");if(L){L.classList.remove("expanded"),we.delete(v.id);let I=L.querySelector(".event-details");I&&(I.style.display="none")}})}let q=H.querySelector(".prev-btn"),V=H.querySelector(".next-btn");if(q&&V){let _=0,T=Array.from(H.querySelectorAll(".history-slide")),L=T.length,I=()=>{T.forEach((O,R)=>{O.style.display=R===_?"block":"none"}),q.disabled=_===0,V.disabled=_===L-1,q.style.opacity=_===0?"0.5":"1",V.style.opacity=_===L-1?"0.5":"1"};q.addEventListener("click",O=>{O.stopPropagation(),_>0&&(_--,I())}),V.addEventListener("click",O=>{O.stopPropagation(),_<L-1&&(_++,I())}),I()}return H},c=Array.from(e.children),h=new Map(c.map(v=>[v.dataset.eventId,v])),E=new Set(ze.map(v=>v.id));c.forEach(v=>{let s=v.dataset.eventId;(!s||!E.has(s))&&v.remove()});let p=null;ze.forEach((v,s)=>{let u=h.get(v.id);(!u||t)&&(u&&u.remove(),u=f(v),!u)||(s===0?e.firstElementChild!==u&&e.prepend(u):p&&p.nextElementSibling!==u&&p.after(u),p=u)}),Je=Date.now(),te(1,Je)}catch(a){console.error("Error fetching events:",a),e.children.length===0&&(e.innerHTML=J("offline","Failed to load events.","The event service may be offline."))}}function qs(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),o=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ze.forEach(a=>we.add(a.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{we.clear(),me(!0)},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.querySelectorAll(".filter-btn").forEach(a=>{a.onclick=()=>{o.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),a.classList.add("active"),Z=a.dataset.filter||"all",me(!0)}}),o.dataset.listenerAttached="true"),o&&o.querySelectorAll(".filter-btn").forEach(a=>{a.dataset.filter===Z?a.classList.add("active"):a.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let a=Z==="all"?"ALL":Z.toUpperCase();if(confirm(`Are you sure you want to delete ${a} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(Z!=="all"?l+=`?category=${Z}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await D(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");we.clear(),me(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var Us=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${r.id}" title="Clear Logs" style="${P()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let f=r,c=unescape(f.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let h=f.querySelector("i");h&&(h.classList.remove("bx-copy"),h.classList.add("bx-check"),setTimeout(()=>{h.classList.remove("bx-check"),h.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let c=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await D(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&Ne()}catch(h){console.error("Error clearing logs:",h)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=J("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Zt=()=>{let t=P()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
        </div>`;async function ss(){Js(),await Promise.all([ne(),Ce(),at()]),await Ne()}var We=null;function Js(){We||(We=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(We),We=null;return}ne(!0)},1e3))}var Gt=null,Vt=null;async function ns(){try{return await(await D("/system_monitor")).json()}catch{return null}}async function Kt(){try{return await(await D("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await D("/processes")).json()}catch{return null}}async function Gs(){try{return await(await D("/agent/status")).json()}catch{return null}}async function Ce(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.getElementById("global-restart-btn"),i=document.getElementById("global-stop-btn"),a=document.getElementById("global-start-btn"),l=(n,b)=>{n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let w=n.innerHTML;n.innerHTML="<i class='bx bx-loader-alt spin'></i>",n.disabled=!0;try{await D(`/system/service/${b}`,{method:"POST",body:JSON.stringify({service:"all"})}),setTimeout(()=>Ce(),2e3),setTimeout(()=>Ce(),5e3),setTimeout(()=>{n.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{n.innerHTML=w,n.disabled=!1},1e3)},1e3)}catch(m){console.error(m),n.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{n.innerHTML=w,n.disabled=!1},2e3)}},n.dataset.listenerAttached="true")};l(o,"restart"),l(i,"stop"),l(a,"start");let r=document.querySelector("#hw-os .hw-content"),f=document.querySelector("#hw-cpu .hw-content"),c=document.querySelector("#hw-ram .hw-content"),h=document.querySelector("#hw-gpu .hw-content"),E=document.querySelector("#hw-storage .hw-content"),p=n=>{if(n){if(r&&(r.innerHTML=`
                ...
                </div>`),c){let b=(n.MEMORY_BYTES/1073741824).toFixed(1);c.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <span style="font-size: 0.9em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">Total System Memory</span>
                    <span style="font-size: 1.8em; font-weight: bold; color: #fff;">${b} <span style="font-size: 0.5em; color: #888; font-weight: normal; margin-left: 2px;">GB</span></span>
                </div>`}if(f&&n.CPU&&n.CPU.length>0){let b=n.CPU[0];f.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    <span style="font-size: 1.1em; color: #fff; font-weight: 400; letter-spacing: 0.5px;">${b.LABEL}</span>
                    <div style="display: flex; gap: 30px;">
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #03dac6; line-height: 1;">${b.COUNT}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Cores</span></div>
                        <div style="text-align: right;"><span style="display: block; font-size: 1.3em; font-weight: bold; color: #bb86fc; line-height: 1;">${b.THREADS}</span><span style="font-size: 0.65em; color: #666; text-transform: uppercase; font-weight: 700;">Threads</span></div>
                    </div>
                </div>`}h&&(n.GPU&&n.GPU.length>0?h.innerHTML=n.GPU.map(b=>{let w=(b.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${b.LABEL}</span>
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):h.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),E&&n.STORAGE&&n.STORAGE.length>0?E.innerHTML=n.STORAGE.map(b=>{let w=(b.USED/1073741824).toFixed(1),m=(b.SIZE/(1024*1024*1024)).toFixed(1),$=b.SIZE>0?(b.USED/b.SIZE*100).toFixed(0):0,B=b.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                ...
                                <span style="font-weight: 600; color: #fff; font-size: 0.95em;">${b.DEVICE}</span>
                            </div>
                    ...
                    </div>`}).join(""):E&&(E.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let n=await Kt();n?(p(n),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),f&&!f.hasChildNodes())){let n=await Kt();p(n)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async n=>{let b=n.target.closest(".svc-btn");if(!b)return;let w=b,m=w.dataset.action,$=w.dataset.service;if(!m||!$)return;let B=w.innerHTML;w.innerHTML="<i class='bx bx-loader-alt spin'></i>",w.classList.add("loading"),w.disabled=!0;try{await D(`/system/service/${m}`,{method:"POST",body:JSON.stringify({service:$})}),setTimeout(()=>Ce(),1e3),setTimeout(()=>Ce(),3e3)}catch(S){console.error(S),alert(`Failed to ${m} ${$}`),w.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{w.innerHTML=B,w.classList.remove("loading"),w.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let v=await ns();if(!v||!v.services){t.children.length===0&&(t.innerHTML=J("offline","Failed to load system metrics.","The event service may be offline."));let n=document.querySelector('[data-service-id="upstash-redis-ro"]');if(n){n.className="service-widget service-widget-offline",n.querySelector(".service-widget-status").textContent="ERROR";let b=n.querySelector(".service-widget-body");b&&(b.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Gt=P()&&re.lastDashboard||Date.now(),te(0,Gt||0);let s=v.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let n=document.getElementById("upstash-countdown");if(n){let m=59-new Date().getSeconds();m<=0&&(m+=60),n.textContent=`${m}s`}},1e3)),Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function u(n){if(!n||n==="N/A"||n==="unknown")return"-";let b=n.match(/^(\d+\.\d+\.\d+)/);return b?b[0]:n.split(".").slice(0,3).join(".")||"-"}function C(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function M(n){if(!n||n==="N/A"||n==="unknown")return"-";let b=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!b)return"-";let w=parseInt(b[1])||0,m=parseInt(b[2])||0,$=parseInt(b[3])||0,B=parseFloat(b[4])||0,S=w*86400+m*3600+$*60+B,H=Math.floor(S/86400);if(H>0)return`${H}d`;let W=Math.floor(S/3600);if(W>0)return`${W}h`;let q=Math.floor(S/60);return q>0?`${q}m`:`${Math.floor(S)}s`}function g(n){if(n.id==="upstash-redis-ro"){let G=P()&&(re.lastFrontend||re.lastDashboard)||Date.now();return`
            ...
            </div>`}let b=n.status==="online",w=b?"service-widget-online":"service-widget-offline",m=b?"bx-check-circle":"bx-x-circle",$=b?"OK":"BAD",B=n.version?u(n.version.str):"-",S=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",H=n.memory&&n.memory!=="N/A"?n.memory:"-",W=S!=="-"?"#00ff00":"#666",q=S!=="-"?"#fff":"#666",V=H!=="-"?"#00ff00":"#666",_=H!=="-"?"#fff":"#666",T=M(n.uptime),L="",I=n.type!=="os"&&n.type!=="cli"&&n.type!=="prd",O="";I&&!P()&&(O=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                ...
                <button class="svc-btn svc-stop" title="Stop Service" data-action="stop" data-service="${n.id}" ${b?"":"disabled"}><i class='bx bx-stop'></i></button>
                <button class="svc-btn svc-start" title="Start Service" data-action="start" data-service="${n.id}" ${b?"disabled":""}><i class='bx bx-play'></i></button>
            </div>`),b?L=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${_};">${H}</span>
                    </div>
                </div>${O}`:L=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${O}`;let R=P()?"easter.company":C(n.domain&&n.port?`${n.domain}:${n.port}`:"");return`<div class="service-widget ${w}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${m}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${$}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${R}</span></div>${L}</div></div>`}let d=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),x=new Set(s.map(n=>n.id));for(let[n,b]of d)x.has(n)||b.remove();let y=new Set,A=["upstash-redis-rw","upstash-redis-ro","easter-company","easter-prod","test"];s.forEach(n=>{if(y.has(n.id)||A.includes(n.id))return;y.add(n.id);let b=g(n),w=d.get(n.id);if(w&&w.parentNode)if(n.id==="upstash-redis-ro"){let m=P()&&(re.lastFrontend||re.lastDashboard)||Date.now(),$=new Date(m).toLocaleTimeString(),B=w.querySelector(".sync-time-label");B&&(B.textContent=`Last synced: ${$}`)}else w.outerHTML!==b&&(w.outerHTML=b);else t.insertAdjacentHTML("beforeend",b)})}async function at(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=J("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),te(2,Vt);let o=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function a(c){let h=c.status==="Ready";return`
                ...
                </div>`}function l(c){let h=c.status==="Ready";return`
                ...
                </div>`}function r(c){let h=c.status==="Downloaded",E=h?"service-widget-online":"service-widget-offline",p=h?"OK":"MISSING",v=h&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",s=c.name;return c.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${E}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${h?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let f="";if(i&&(f+=a(i)),e.xtts&&(f+=l(e.xtts)),f+=o.map(r).join(""),!f){t.innerHTML=J("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let o=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),h=document.getElementById("fabricator-reset-btn"),E=document.getElementById("courier-reset-btn"),p=document.getElementById("system-pause-toggle-btn");p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{let A=p.querySelector(".bx-play")?"/agent/resume":"/agent/pause";p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D(A,{method:"POST"}),await ne()}catch{p.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},p.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),h&&!h.dataset.listenerAttached&&(h.onclick=async()=>{h.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{h.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{h.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{h.innerHTML="<i class='bx bx-error'></i>"}},h.dataset.listenerAttached="true"),E&&!E.dataset.listenerAttached&&(E.onclick=async()=>{E.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{E.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{E.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{E.innerHTML="<i class='bx bx-error'></i>"}},E.dataset.listenerAttached="true");let v=document.getElementById("imaginator-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let y=s.agents.guardian||{protocols:{}},A=s.agents.analyzer||{protocols:{}},n=s.agents.imaginator||{protocols:{}},b=s.agents.fabricator||{protocols:{}},w=s.agents.courier||{protocols:{}},m=s.system||{},$=Math.floor(Date.now()/1e3),B={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},S=k=>{k<0&&(k=0);let z=Math.floor(k/3600),N=Math.floor(k%3600/60),U=k%60;return z>0?`${z}h ${N}m`:N>0?`${N}m ${U}s`:`${U}s`},H=(k,z,N,U)=>{if(!k)return;let Y=B[U]||U.toUpperCase(),K=k.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(K&&(K.textContent=Y),!N){k.textContent="Inactive",k.style.color="#666";return}let Ee=N.status;if(Ee==="Working")k.textContent="Working",k.style.color="#bb86fc";else if(Ee==="Ready")k.textContent="Ready",k.style.color="#5eff5e";else{let Te=N.cooldown;if(P()&&(Te=N.next_run-$),Te<=0)k.textContent="Ready",k.style.color="#5eff5e";else{let Ve=Math.floor(Te/60),Ke=Te%60;k.textContent=`${Ve}m ${Ke}s`,k.style.color="#fff"}}z&&N.stats&&(z.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};o&&H(o,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry");let W=document.getElementById("analyzer-synthesis-val"),q=document.getElementById("analyzer-synthesis-stats");W&&H(W,q,A.protocols.synthesis,"synthesis");let V=document.getElementById("imaginator-alert_review-val"),_=document.getElementById("imaginator-alert_review-stats");V&&H(V,_,n.protocols.alert_review,"alert_review");let T=document.getElementById("fabricator-construction-val"),L=document.getElementById("fabricator-construction-stats");T&&H(T,L,b.protocols.construction,"construction");let I=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");I&&H(I,O,w.protocols.researcher,"researcher");let R=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&m.state){let k=m.state,z=S(m.state_time||0);R&&(R.textContent=`State: ${k.toUpperCase()}`),G.textContent=z,k==="idle"?G.style.color=m.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",p&&(k==="paused"?(p.innerHTML="<i class='bx bx-play'></i>",p.title="Resume System",p.style.color="#ff9800"):(p.innerHTML="<i class='bx bx-pause'></i>",p.title="Pause System",p.style.color=""))}a&&(a.textContent=S(m.metrics?.total_idle_time||0)),l&&(l.textContent=S(m.metrics?.total_active_time||0)),r&&(r.textContent=S(m.metrics?.total_waste_time||0))}else[o,i,a,l,r].forEach(A=>{A&&(A.textContent="-",A.style.color="#666")});if(t)return;let u=await Ws(),C=[],M=[],g=[];u&&(Array.isArray(u)?C=u:(C=u.active||[],M=u.queue||[],g=u.history||[],g.sort((y,A)=>(A.end_time||0)-(y.end_time||0)))),e&&(C.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=J("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,C,!1)));let d=document.getElementById("processes-queue-widgets");d&&(M.length===0?!d.querySelector(".tab-placeholder")&&!d.querySelector('div[style*="font-style: italic"]')&&(d.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(d.innerHTML="",it(d,M,!1)));let x=document.getElementById("processes-history-widgets");x&&(g.length===0?x.querySelector(".tab-placeholder")||(x.innerHTML=J("empty","No recent history.")):(x.querySelector(".tab-placeholder")&&(x.innerHTML=""),it(x,g,!0))),Oe(1,C.length)}function it(t,e,o){function i(c){let h="";c.end_time?h=`${c.end_time-c.start_time}s`:h=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let E=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",p=c.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent","voice-mode":"Voice Interaction"};if(v[p]?p=v[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),o)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let a=o?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(a)).map(c=>[c.dataset.channelId,c])),r=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,h]of l)c&&!r.has(c)&&h.remove();let f=new Set;e.forEach(c=>{let h=`${c.channel_id}-${c.start_time}`;if(f.has(h))return;f.add(h);let E=i(c),p=l.get(h);if(p&&p.parentNode){p.outerHTML!==E&&(p.outerHTML=E);let v=t.querySelector(`[data-channel-id="${h}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",E)})}function Ge(){let t=Ye(),e="Notification"in window,o={enabled:e&&Notification.permission==="granted",supported:e};return`
            ...
            </div>`}function ot(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let a=this.dataset.theme;a&&(gt(a),t.setContent(Ge()),ot(t))})});let o=document.getElementById("notifications-toggle");o&&"Notification"in window?o.onclick=async i=>{let a=i.target;if(a.checked)try{await Notification.requestPermission()!=="granted"&&(a.checked=!1)}catch{a.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),a.checked=!0)}:o&&(o.disabled=!0),Vs()}async function Vs(){let t=document.getElementById("service-config-list"),e=document.getElementById("ollama-config-list");if(!t||!e)return;let o=P();try{let a=await(await D("/system/options")).json(),l=a.services||a||{},r=a.ollama||a||{},f="",c=(C,M,g)=>`
        <div class="settings-item">
            <div class="settings-item-info">
            ...
            <label class="toggle-switch">
                <input type="checkbox" class="service-device-toggle" data-service="${M}" ${g==="cuda"?"checked":""} ${o?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>
        </div>`;l.stt&&(f+=c("STT Service","stt",l.stt.device||"cpu")),l.tts&&(f+=c("TTS Service","tts",l.tts.device||"cpu")),f?o&&(f+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Service configuration is read-only in public mode.</div>'):f='<div style="padding: 20px; text-align: center; color: #666;">No configurable services found.</div>',t.innerHTML=f;let h="",E=r.utility_device||"cpu",p=r.utility_speed||"smart",v=o?"disabled":"";if(h+=`
        <div class="settings-item">
            <div class="settings-item-info">
            ...
            <div class="settings-control">
                <select id="ollama-utility-device-select" class="settings-select" ${v}>
                    <option value="cpu" ${E==="cpu"?"selected":""}>CPU (RAM)</option>
                    <option value="gpu" ${E==="gpu"?"selected":""}>GPU (VRAM)</option>
                </select>
            </div>
        </div>
        <div class="settings-item">
            ...
            <div class="settings-control">
                <select id="ollama-utility-speed-select" class="settings-select" ${v}>
                    <option value="fast" ${p==="fast"?"selected":""}>Fast (Efficiency)</option>
                    <option value="smart" ${p==="smart"?"selected":""}>Smart (Fidelity)</option>
                </select>
            </div>
        </div>`,o?h+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Optimization is read-only in public mode.</div>':h+=`<div style="font-size: 0.7em; color: #ffa500; font-style: italic; margin-top: 10px; text-align: center;"><i class='bx bx-info-circle'></i> Changing these settings requires a 'dex restart' to re-apply.</div>`,e.innerHTML=h,o)return;t.querySelectorAll(".service-device-toggle").forEach(C=>{C.addEventListener("change",async M=>{let g=M.target,d=g.dataset.service,x=g.checked?"cuda":"cpu";g.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:d,key:"device",value:x})})}catch{g.checked=!g.checked,alert("Failed to update configuration.")}finally{g.disabled=!1}})});let s=document.getElementById("ollama-utility-device-select");s&&s.addEventListener("change",async C=>{let M=C.target,g=M.value;M.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:"ollama",key:"utility_device",value:g})})}catch{alert("Failed to update utility hardware setting.")}finally{M.disabled=!1}});let u=document.getElementById("ollama-utility-speed-select");u&&u.addEventListener("change",async C=>{let M=C.target,g=M.value;M.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:"ollama",key:"utility_speed",value:g})})}catch{alert("Failed to update utility intelligence setting.")}finally{M.disabled=!1}})}catch{t.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load configuration.</div>',e.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load optimization settings.</div>'}}var as=()=>`
    ...
    `;async function os(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await is(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await is(t)}async function is(t){try{let e=await D("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let o=await e.json();if(!o||o.length===0){t.innerHTML=J("empty","No web history found.");return}let i=`
        ...
        `;o.forEach((f,c)=>{let h=new Date(f.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),E=f.screenshot?`<img src="data:image/png;base64,${f.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ks=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[o,i]of Object.entries(t)){let a=cs.filter(l=>l.category===o);a.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>rs());let h=document.getElementById("terminal-action-btn");h&&(h.onclick=()=>rs())}let o=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");o&&(o.innerHTML="");let a=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let f=document.getElementById("terminal-action-btn");return f&&(f.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",rt=!0,o}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),ie()||(document.body.style.overflow=""),rt=!1}function ls(t,e,o="output"){if(!rt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${o}`,o==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=je(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ys(t){let e=cs.find(r=>r.id===t);if(!e)return;let o=Zs(e);o&&ls(o,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let r of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let f="output";r.includes("[ERROR]")?f="error":r.includes("[SUCCESS]")||r.includes("\u2713")?f="success":r.includes("!")&&(f="warning"),o&&ls(o,r,f)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ks();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code"),i=o?o.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(o=>{let i=o;o.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Ys(a)})})}async function Xs(){if(!P())try{if(!(await D("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Xs(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],o=document.getElementById("windows-container");o&&o.setAttribute("data-count","0");let i=g=>{localStorage.setItem("dex_last_window",g)};function a(){for(;e.length>1;)e.shift().close(!0);let g=document.getElementById("windows-container"),d=document.querySelector("nav"),x=document.querySelector("footer"),y=window.location.pathname==="/"||window.location.pathname==="/index.html",A=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");g&&g.setAttribute("data-count",e.length.toString()),e.forEach(S=>{let H=document.getElementById(S.id);H&&H.classList.remove("hide-close")});let n=document.getElementById("dexter-menu-container"),b=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon"),m=document.getElementById("docs-icon"),$=document.getElementById("close-all-windows"),B=window.innerWidth<880;if(!B){let S=document.getElementById("dexter-dropdown"),H=document.getElementById("dexter-menu-btn");S&&S.classList.remove("active"),H&&H.classList.remove("active"),g&&g.classList.remove("menu-open")}if($e(e.length>0),e.length>0)if(x?.classList.add("hide"),$&&($.style.display=B?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),g&&(g.style.paddingTop="60px"),n&&(n.style.display=B?"flex":"none"),m&&(m.style.display=B?"block":"none"),w&&(w.style.display=B?"block":"none"),!B&&b){let S=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(S)?(b.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(f)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(E)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(h)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(p)})):b.innerHTML=""}else b&&(b.innerHTML="");else d?.classList.remove("window-open"),$&&($.style.display="none"),g&&(g.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),ie()||(document.body.style.overflow=""),y||A?x?.classList.remove("hide"):x?.classList.add("hide"),n&&(n.style.display="flex"),m&&(m.style.display="block"),w&&(w.style.display="block"),b&&(b.innerHTML="");Le()}function l(g){if(g.isOpen()){g.close();return}for(;e.length>0;)e.pop().close(!0);e.push(g),g.open(),a()}function r(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",a);let f=ue({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>pe(),onClose:()=>{let g=e.indexOf(f);g>-1&&e.splice(g,1),a()}}),c=ue({id:"events-window",title:"Events",icon:"bx-calendar-event",content:nt(),onOpen:()=>{c.setContent(nt()),me()},onClose:()=>{let g=e.indexOf(c);g>-1&&e.splice(g,1),a()}}),h=ue({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ut(),onOpen:()=>st(),onClose:()=>{let g=e.indexOf(h);g>-1&&e.splice(g,1),a()}}),E=ue({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(g=>P()?g.title!=="Hardware"&&g.title!=="Logs":!0),onOpen:()=>{Ce(),ne(),at(),os(),Ne()},onClose:()=>{let g=e.indexOf(E);g>-1&&e.splice(g,1),a()}}),p=ue({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Pt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Ot()},{icon:"bx-task",title:"Tasks",content:jt()}],onOpen:()=>tt(),onClose:()=>{let g=e.indexOf(p);g>-1&&e.splice(g,1),a()}}),v=ue({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ge(),onOpen:()=>{v.setContent(Ge()),ot(v)},onClose:()=>{let g=e.indexOf(v);g>-1&&e.splice(g,1),a()}});window.dexter={viewEvent:g=>{c.isOpen()||l(c),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${g}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:g=>{f.isOpen()||l(f),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${g}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let g=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");g&&d&&(g.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let g=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),x=document.getElementById("docs-icon"),y=document.getElementById("close-all-windows"),A=document.getElementById("nav-left-container"),n=document.getElementById("dexter-dropdown"),b=document.getElementById("windows-container");g&&n&&(g.onclick=m=>{m.stopPropagation();let $=window.innerWidth<880;if($)n.style.top="",n.style.left="",n.style.right="",n.style.transform="";else{let S=g.getBoundingClientRect(),W=document.querySelector("nav").getBoundingClientRect(),q=220,V=10,T=S.left+S.width/2-q/2,L=W.right-10;T+q>L&&(T=L-q),T<10&&(T=10),n.style.top=W.bottom+V+"px",n.style.left=T+"px",n.style.right="auto",n.style.transform="none"}n.classList.toggle("active"),g.classList.toggle("active");let B=n.classList.contains("active");if($){let S=document.querySelector("nav");B?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),S?.classList.add("window-open"),b?.classList.add("menu-open"),$e(!0)):(b?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S?.classList.remove("window-open"),ie()||$e(!1)))}}),d&&(d.onclick=m=>{m.stopPropagation(),l(v)}),x&&(x.onclick=m=>{m.stopPropagation(),window.location.href="/docs"}),y&&(y.onclick=m=>{m.stopPropagation(),r()});let w=(m,$,B)=>{let S=document.getElementById(m);S&&(S.onclick=H=>{H.stopPropagation(),s(),B&&i(B),l($)})};w("alerts-menu-item",f,"alerts-window"),w("events-menu-item",c,"events-window"),w("monitor-menu-item",E,"monitor-window"),w("contacts-menu-item",h,"contacts-window"),w("workspace-menu-item",p,"workspace-window"),A&&(A.onclick=m=>{if(m.stopPropagation(),n&&n.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1));return}if(e.length>0)r();else{let $=window.location.pathname;if(!($==="/"||$==="/index.html")){let S=$.endsWith("/")&&$.length>1?$.slice(0,-1):$;if(S.includes("/docs/page/")){window.location.href="/";return}let H=S.split("/");H.pop();let W=H.join("/")||"/";window.location.href=W}}}),document.addEventListener("click",()=>{let m=window.innerWidth<880;n&&n.classList.contains("active")&&(s(),m&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1)))}),n&&(n.onclick=m=>m.stopPropagation())})();let C=window.location.pathname==="/"||window.location.pathname==="/index.html",M=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!C&&!M&&document.querySelector("footer")?.classList.add("hide"),P()||setInterval(()=>{c.isOpen()&&me(),f.isOpen()?pe():et()},1e3),setInterval(()=>{P()&&(f.isOpen()?pe():et()),p.isOpen()?tt():Bt(),P()&&c.isOpen()&&me(),E.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

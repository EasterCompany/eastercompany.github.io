...
`,ze=null,ue=new Set,Ne=[];async function le(t=!1){let e=document.getElementById("alerts-list");if(!e)return;xs(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await B(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];ze=Date.now(),ee(0,ze);let r=Date.now(),f=24*60*60*1e3,c=l.filter(u=>{let g=localStorage.getItem(`alert_read_ts_${u.id}`);if(!g)return!0;let x=parseInt(g);return r-x<f});c.sort((u,g)=>{let x=k=>{let E=k.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},M=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,A=M(x(u)),I=M(x(g));return A!==I?I-A:g.timestamp-u.timestamp}),Ne=c;let p=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},y=[],v=new Set(c.map(u=>p(u))).size>1;if(c.length>0){let u=null;c.forEach(g=>{let x=p(g);v&&x!==u&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),u=x),y.push(g)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),_e();return}let o=u=>{let g=u.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let x=(g.title||"Untitled Alert").trim(),M=(g.body||"No description provided.").trim(),A=g.summary||"",I=g.content||"",k=g.protocol||"",E=(g.priority||"low").toLowerCase(),D=!!g.alert,U=(g.category||"system").trim(),O=g.related_event_ids||[],q=g.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${u.id}`),S=new Date(u.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=J?"event-border-grey":"event-border-blue";!J&&D&&(T="event-border-red"),J&&(E==="high"||E==="critical")?T="event-border-red":J&&E==="medium"&&(T="event-border-orange");let L=J?"alert-read":"alert-unread",_=ue.has(u.id),P=_?"expanded":"",V=_?"display: block;":"display: none;",j="",F="";O.length>0&&(F=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${O.map(re=>`<a href="#" onclick="window.dexter.viewEvent('${re}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${re.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let W="";q&&(W=`
        ...
        `,j=`
                ...
                ${te}
            `;let X=document.createElement("div");X.className=`event-item notification-item ${T} ${L} ${P} cursor-pointer priority-${E}`,X.dataset.alertId=u.id,X.onclick=function(re){let Y=this;if(Y.classList.contains("expanded")){Y.classList.remove("expanded"),ue.delete(u.id);let ke=Y.querySelector(".event-details");ke&&(ke.style.display="none")}else{Y.classList.add("expanded"),ue.add(u.id);let ke=Y.querySelector(".event-details");if(ke&&(ke.style.display="block"),!localStorage.getItem(`alert_read_ts_${u.id}`)){localStorage.setItem(`alert_read_ts_${u.id}`,Date.now().toString()),Y.classList.add("alert-read"),Y.classList.remove("alert-unread"),Y.classList.remove("event-border-blue","event-border-red","event-border-purple");let je="event-border-grey";E==="high"||E==="critical"?je="event-border-red":E==="medium"&&(je="event-border-orange"),Y.classList.add(je),_e()}}};let st=`${k?k.toUpperCase():"GUARDIAN"} ALERT: ${A||x}`,as={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[U]||"bx-bell",os=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";X.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${C}</span>
                    <span class="event-date">${h}</span>
                </div>
                <div class="event-icon"><i class='bx ${as}'></i></div>
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${os}; font-size: 0.8em; margin-left: 5px;">[${E.toUpperCase()}]</span>
                    </div>
            ...
            `;let nt=X.querySelector(".event-details");nt&&nt.addEventListener("click",re=>{re.stopPropagation()});let it=X.querySelector(".close-details-btn");return it&&it.addEventListener("click",re=>{re.stopPropagation(),X.classList.remove("expanded");let Y=X.querySelector(".event-details");Y&&(Y.style.display="none"),ue.delete(u.id)}),X},b=u=>{let g=document.createElement("div");g.className="notification-divider",g.dataset.alertId=u.id;let x="#888888";return u.label==="CRITICAL"?x="#ff4d4d":u.label==="HIGH"?x="#ff8888":u.label==="MEDIUM"&&(x="#ffa500"),g.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,g.innerHTML=`<span style="white-space: nowrap;">${u.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,g},$=Array.from(e.children),w=new Map($.map(u=>[u.dataset.alertId,u])),a=new Set(y.map(u=>u.id));$.forEach(u=>{let g=u.dataset.alertId;(!g||!a.has(g))&&u.remove()});let d=null;y.forEach((u,g)=>{let x=w.get(u.id);if(!x||t){if(x&&x.remove(),u.type==="divider")x=b(u);else{let M=o(u);if(!M)return;x=M}if(!x)return}g===0?e.firstElementChild!==x&&e.prepend(x):d&&d.nextElementSibling!==x&&d.after(x),d=x}),ze=Date.now(),ee(0,ze),_e()}catch(i){console.error("Error fetching alerts:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load alerts.","The event service may be offline."))}}function xs(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),s=document.getElementById("alerts-close-all"),i=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ne.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),le(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Ne.forEach(n=>{ue.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),le(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{ue.clear(),le(!0)},s.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Ne.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,n.toString())}),ue.clear(),le(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}async function Je(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||n++}),gt(n)}catch{}}var ht=()=>`
...
`,ge=new Set,Pe=[],Ee=null;async function fe(t=!1){let e=document.getElementById("roadmap-list");if(e){hs();try{let s=await B("/roadmap");if(!s.ok)throw new Error("Offline");let i=await s.json();Pe=i;let n=p=>{let y=ge.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${o} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=p.id,w.onclick=u=>{if(u.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),ge.delete(p.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),ge.add(p.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;w.innerHTML=`
      ...
      `;let d=w.querySelector(".event-details");return d&&d.addEventListener("click",u=>{u.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>ws(p)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Cs(p)),w.querySelector(".delete-btn")?.addEventListener("click",()=>Es(p.id)),w.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),w.classList.remove("expanded");let g=w.querySelector(".event-details");g&&(g.style.display="none"),ge.delete(p.id)}),w},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),f=new Set(i.map(p=>p.id));if(l.forEach(p=>{let y=p.dataset.itemId;(!y||!f.has(y))&&p.remove()}),!Pe||Pe.length===0){e.innerHTML=N("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((p,y)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function hs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ee=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Ee=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let f=Ee?`/roadmap/${Ee}`:"/roadmap",c=Ee?"PATCH":"POST";try{await B(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let p=document.getElementById("roadmap-editor-container");p&&(p.style.display="none"),fe(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{Pe.forEach(l=>ge.add(l.id)),fe(!0)},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ge.clear(),fe(!0)},n.dataset.listenerAttached="true")}function ws(t){Ee=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function Cs(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),fe(!0)}catch(s){console.error(s)}}async function Es(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),ge.delete(t),fe(!0)}catch(e){console.error(e)}}var Et=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`;async function Ge(){await Promise.all([fe(),ve(),Te()])}var _t=()=>`
    ...
    `;async function Se(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),f=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=f?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),l&&(l.value=""),r&&(r.value=""),Se()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active chores.","Create one to start monitoring.");return}let f=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=f,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this chore?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Se())}})}catch(l){console.error("Failed to fetch chores",l)}}var Ms=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Ht(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Dt(t){if(!document.getElementById("dex-profile-styles")){let s=document.createElement("style");s.id="dex-profile-styles",s.textContent=Ms,document.head.appendChild(s)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",s=>{s.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",De(`/profile/${t.id}`).then(async s=>{if(s.ok){let i=await s.json();Ve(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=Ht(t.id,t.username);Ve(e,t,i)}}).catch(s=>{console.error("Profile fetch error:",s);let i=Ht(t.id,t.username);Ve(e,t,i)})}function Ve(t,e,s){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(s.attributes||s.facts||[]).map(x=>{let M=x.key||x.k,A=x.value||x.v;return`
            <div class="fact-chip">
...
`,As=null;async function Ke(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ke(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let n=s.target.closest(".user-profile-card");if(n&&n.dataset.user)try{let l=JSON.parse(n.dataset.user);Dt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let s=await De("/contacts");if(!s.ok)throw new Error("Service unreachable");let n=(await s.json()).members||[];if(As=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,f)=>{let c=l[r.level]??10,p=l[f.level]??10;return c!==p?c-p:r.username.localeCompare(f.username)}),t.innerHTML=n.map(r=>{let f=r.color&&r.color!==0,c=f?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let m=r.level==="Me",v=m?"rgba(187, 134, 252, 0.08)":f?`${c}11`:"rgba(255,255,255,0.03)",o=m?"2px solid #bb86fc":f?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
...
`,Re=null,ye=new Set,Me=[],K="all",Bs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},_s={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Hs(t){for(let[e,s]of Object.entries(Bs))if(s.includes(t))return e;return"system"}function Ds(t){return _s[t]||"bx-square-rounded"}async function ce(t=!1){let e=document.getElementById("events-timeline");if(!e)return;zs();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await B(i);if(!n.ok)throw new Error("Service unreachable");if(Me=((await n.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),Re=Date.now(),ee(1,Re),Me.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,$=Hs(b),w=Ds(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let u=a?"cursor-pointer":"",g=ye.has(v.id),x=g?"expanded":"",M=g?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Nt(b,o),D=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",U="";if(a){let S="";if(b==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                    ...
                    `}else if(b==="system.analysis.audit"){let C=o.success?"#03dac6":"#ff4d4d",h=o.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,L="";o.error&&(L=`
                        ...
                        `);let _="";if(o.chat_history&&o.chat_history.length>0){let j=o.chat_history.length,F=o.chat_history.map((W,oe)=>{let te=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&te==="USER"&&(te="SYSTEM"),!W.name&&te==="ASSISTANT"&&(te="AGENT");let X=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${oe}" style="display: ${oe===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${X}; letter-spacing: 1px; font-weight: bold;">${te}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${oe+1} of ${j}</span>
                                    </div>
            ...
            `;let q=O.querySelector(".event-details");if(q&&q.addEventListener("click",S=>{S.stopPropagation()}),a){let S=O.querySelector(".close-details-btn");S&&S.addEventListener("click",C=>{C.stopPropagation();let h=C.target.closest(".event-item");if(h){h.classList.remove("expanded"),ye.delete(v.id);let T=h.querySelector(".event-details");T&&(T.style.display="none")}})}let G=O.querySelector(".prev-btn"),J=O.querySelector(".next-btn");if(G&&J){let S=0,C=Array.from(O.querySelectorAll(".history-slide")),h=C.length,T=()=>{C.forEach((L,_)=>{L.style.display=_===S?"block":"none"}),G.disabled=S===0,J.disabled=S===h-1,G.style.opacity=S===0?"0.5":"1",J.style.opacity=S===h-1?"0.5":"1"};G.addEventListener("click",L=>{L.stopPropagation(),S>0&&(S--,T())}),J.addEventListener("click",L=>{L.stopPropagation(),S<h-1&&(S++,T())}),T()}return O},c=Array.from(e.children),p=new Map(c.map(v=>[v.dataset.eventId,v])),y=new Set(Me.map(v=>v.id));c.forEach(v=>{let o=v.dataset.eventId;(!o||!y.has(o))&&v.remove()});let m=null;Me.forEach((v,o)=>{let b=p.get(v.id);(!b||t)&&(b&&b.remove(),b=f(v),!b)||(o===0?e.firstElementChild!==b&&e.prepend(b):m&&m.nextElementSibling!==b&&m.after(b),m=b)}),Re=Date.now(),ee(1,Re)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=N("offline","Failed to load events.","The event service may be offline."))}}function zs(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),s=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me.forEach(n=>ye.add(n.id)),ce(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),ce(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{s.querySelectorAll(".filter-btn").forEach(l=>l.classList.remove("active")),n.classList.add("active"),K=n.dataset.filter||"all",ce(!0)}}),s.dataset.listenerAttached="true"),s&&s.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===K?n.classList.add("active"):n.classList.remove("active")});let i=document.getElementById("events-clear");i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let n=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let l="/events";if(K!=="all"?l+=`?category=${K}`:l+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await B(l,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ye.clear(),ce(!0)}catch(l){console.error("Failed to clear events:",l),alert("Failed to clear events. Check console.")}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var Ns=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`);return`
        ...
        <div id="processes-history-widgets" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 30px; opacity: 0.8;"></div>`,Jt=()=>`
        <div class="system-section-header">
            <i class='bx bxs-server' style="color: #03dac6;"></i>
            <h2>Services</h2>
        </div>
        <div id="services-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Wt=()=>`
        <div class="system-section-header">
            <i class='bx bxs-brain' style="color: #03dac6;"></i>
            <h2>Cognitive Models</h2>
        </div>
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Gt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Rt()}
        </div>`;async function Kt(){Rs(),await Promise.all([de(),Xe(),Qe(),Se()]),await Ae()}var Oe=null;function Rs(){Oe||(Oe=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Oe),Oe=null;return}de(!0)},1e3))}var Ot=null,Ut=null;async function Zt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function jt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function Os(){try{return await(await B("/processes")).json()}catch{return null}}async function Us(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Xe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),f=a=>{if(a){if(s&&(s.innerHTML=`
    ...
    `;async function Qt(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Yt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Yt(t)}async function Yt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=N("empty","No web history found.");return}let i=`
            <div style="display: flex; flex-direction: column; height: 100%;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],js=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,i]of Object.entries(t)){let n=ss.filter(l=>l.category===s);n.length!==0&&(e+=`
    ...
    </div>`,e},tt=!1;function qs(t){let e=document.getElementById("cli-terminal-overlay");if(!e){e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>es());let p=document.getElementById("terminal-action-btn");p&&(p.onclick=()=>es())}let s=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");s&&(s.innerHTML="");let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`

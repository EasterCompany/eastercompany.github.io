(()=>{function Qe(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function et(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var st=1e3;function ae(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++st)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",r),s&&setTimeout(s,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++st,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",a="",g="",$;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((o,d)=>{let f=o.icon?`<i class="bx ${o.icon}"></i>`:`<span class="tab-glyph">${o.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
...
`,Ae=null,re=new Set,Ie=[];async function ie(t=!1){let e=document.getElementById("alerts-list");if(!e)return;rs(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ae=Date.now(),X(0,Ae);let r=Date.now(),v=24*60*60*1e3,l=c.filter(d=>{let f=localStorage.getItem(`alert_read_ts_${d.id}`);if(!f)return!0;let x=parseInt(f);return r-x<v});l.sort((d,f)=>{let x=T=>{let S=T.event;if(typeof S=="string")try{S=JSON.parse(S)}catch{return"low"}return(S.priority||"low").toLowerCase()},L=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,I=L(x(d)),k=L(x(f));return I!==k?k-I:f.timestamp-d.timestamp}),Ie=l;let m=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],b=new Set(l.map(d=>m(d))).size>1;if(l.length>0){let d=null;l.forEach(f=>{let x=m(f);b&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(f)})}if(t&&(e.innerHTML=""),l.length===0){e.innerHTML=R("empty","No alerts yet."),Te();return}let a=d=>{let f=d.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),L=(f.body||"No description provided.").trim(),I=f.summary||"",k=f.content||"",T=f.protocol||"",S=(f.priority||"low").toLowerCase(),_=!!f.alert,j=(f.category||"system").trim(),P=f.related_event_ids||[],A=f.audit_event_id,z=!!localStorage.getItem(`alert_read_ts_${d.id}`),E=new Date(d.timestamp*1e3),C=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=z?"event-border-grey":"event-border-blue";!z&&_&&(M="event-border-red"),z&&(S==="high"||S==="critical")?M="event-border-red":z&&S==="medium"&&(M="event-border-orange");let N=z?"alert-read":"alert-unread",D=re.has(d.id),q=D?"expanded":"",J=D?"display: block;":"display: none;",W="",G="";P.length>0&&(G=`
        ...
        `,W=`
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${N} ${q} cursor-pointer priority-${S}`,Z.dataset.alertId=d.id,Z.onclick=function(se){if(this.classList.contains("expanded")){this.classList.remove("expanded"),re.delete(d.id);let he=this.querySelector(".event-details");he&&(he.style.display="none")}else{this.classList.add("expanded"),re.add(d.id);let he=this.querySelector(".event-details");if(he&&(he.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ne="event-border-grey";S==="high"||S==="critical"?Ne="event-border-red":S==="medium"&&(Ne="event-border-orange"),this.classList.add(Ne),Te()}}};let Ze=`${T?T.toUpperCase():"GUARDIAN"} ALERT: ${I||x}`,Gt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[j]||"bx-bell",Vt=S==="high"||S==="critical"?"#ff4d4d":S==="medium"?"#ffa500":"#888";Z.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${C}</span>
                    <span class="event-date">${w}</span>
                </div>
                <div class="event-icon"><i class='bx ${Gt}'></i></div>
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Vt}; font-size: 0.8em; margin-left: 5px;">[${S.toUpperCase()}]</span>
                    </div>
                    <div class="event-message">${oe(Ze)}</div>
                    <div class="event-details" style="${J}">
                        ${W}
                    </div>
                </div>
            `;let Ye=Z.querySelector(".event-details");Ye&&Ye.addEventListener("click",se=>{se.stopPropagation()});let Xe=Z.querySelector(".close-details-btn");return Xe&&Xe.addEventListener("click",se=>{se.stopPropagation(),Z.classList.remove("expanded");let ze=Z.querySelector(".event-details");ze&&(ze.style.display="none"),re.delete(d.id)}),Z},g=d=>{let f=document.createElement("div");f.className="notification-divider",f.dataset.alertId=d.id;let x="#888888";return d.label==="CRITICAL"?x="#ff4d4d":d.label==="HIGH"?x="#ff8888":d.label==="MEDIUM"&&(x="#ffa500"),f.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,f.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,f},$=Array.from(e.children),h=new Map($.map(d=>[d.dataset.alertId,d])),u=new Set(y.map(d=>d.id));$.forEach(d=>{let f=d.dataset.alertId;(!f||!u.has(f))&&d.remove()});let o=null;y.forEach((d,f)=>{let x=h.get(d.id);(!x||t)&&(x&&x.remove(),d.type==="divider"?x=g(d):x=a(d),!x)||(f===0?e.firstElementChild!==x&&e.prepend(x):o&&o.nextElementSibling!==x&&o.after(x),o=x)}),Ae=Date.now(),X(0,Ae),Te()}catch(s){console.error("Error fetching alerts:",s),e.children.length===0&&(e.innerHTML=R("offline","Failed to load alerts.","The event service may be offline."))}}function rs(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),s=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ie.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ie(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Ie.forEach(n=>{re.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ie(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{re.clear(),ie(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Ie.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,n.toString())}),re.clear(),ie(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}async function Re(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||n++}),lt(n)}catch{}}var ft=()=>`
...
`,le=new Set,Be=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ls();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();Be=s;let n=m=>{let y=le.has(m.id),p=m.state==="published",b=m.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),b&&(a="event-border-purple");let $=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=m.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(m.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(m.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;h.innerHTML=`
      ...
      `;let o=h.querySelector(".event-details");return o&&o.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>cs(m)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ds(m)),h.querySelector(".delete-btn")?.addEventListener("click",()=>ps(m.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(m.id)}),h},c=Array.from(e.children),r=new Map(c.map(m=>[m.dataset.itemId,m])),v=new Set(s.map(m=>m.id));if(c.forEach(m=>{let y=m.dataset.itemId;(!y||!v.has(y))&&m.remove()}),!Be||Be.length===0){e.innerHTML=R("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let l=null;s.forEach((m,y)=>{let p=r.get(m.id);(!p||t)&&(p&&p.remove(),p=n(m),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):l&&l.nextElementSibling!==p&&l.after(p),l=p)})}catch{e.children.length===0&&(e.innerHTML=R("offline","Failed to load roadmap.","The event service may be offline."))}}}function ls(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let r=xe?`/roadmap/${xe}`:"/roadmap",v=xe?"PATCH":"POST";try{await B(r,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(l){console.error(l)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Be.forEach(c=>le.add(c.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function cs(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function ds(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function ps(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var vt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,bt=null,de=new Set,yt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ms();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(yt=c,bt=Date.now(),X(2,bt),c.length===0){e.innerHTML=R("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let r=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(D=>D.trim()),o=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(p.timestamp*1e3),L=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=de.has(p.id),T=k?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,S.dataset.blueprintId=p.id,f&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let j=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";S.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let P="";o.length>0&&(P=`
                ...
                `);let F=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",z=H(),E=f?`
...
`;async function Oe(){await Promise.all([ce(),pe()])}var Ct=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,v=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:v,natural_instruction:c,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(l){console.error(l),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let c=await B("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);let r=await c.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=R("empty","No active chores.","Create one to start monitoring.");return}let v=r.map(l=>{let m=l.last_run===0?"Never":new Date(l.last_run*1e3).toLocaleString(),y=l.memory?l.memory.length:0,p=l.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=v,t.querySelectorAll(".delete-chore-btn").forEach(l=>{l.onclick=async m=>{m.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${l.dataset.id}`,{method:"DELETE"}),ye())}})}catch(c){console.error("Failed to fetch chores",c)}}var us=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function $t(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function St(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=us,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Me(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();Ue(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=$t(t.id,t.username);Ue(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=$t(t.id,t.username);Ue(e,t,s)})}function Ue(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(i.badges||[]).map(L=>`<span class="profile-badge ${L==="Creator"?"master":""}">${L}</span>`).join(""),h=(i.attributes||i.facts||[]).map(L=>{let I=L.key||L.k,k=L.value||L.v;return`
            <div class="fact-chip">
...
`,fs=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);St(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(fs=Date.now(),n.length===0){t.innerHTML=R("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,v)=>{let l=c[r.level]??10,m=c[v.level]??10;return l!==m?l-m:r.username.localeCompare(v.username)}),t.innerHTML=n.map(r=>{let v=r.color?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",l=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",m="#888";r.level==="Me"||r.level==="Master"?m="#bb86fc":r.level==="Admin"?m="#cf6679":r.level==="Moderator"?m="#03dac6":r.level==="Contributor"&&(m="#ffa500");let y=r.level==="Me",p=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
...
`,_e=null,me=new Set,Ce=[],K="all",bs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},vs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function xs(t){for(let[e,i]of Object.entries(bs))if(i.includes(t))return e;return"system"}function ys(t){return vs[t]||"bx-square-rounded"}async function ne(t=!1){let e=document.getElementById("events-timeline");if(!e)return;hs();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Ce=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),_e=Date.now(),X(1,_e),Ce.length===0){e.innerHTML=R("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,$=xs(g),h=ys(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),o="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?o="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?o="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?o="event-border-orange":g==="system.test.completed"?o=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":o="event-border-blue");let d=u?"cursor-pointer":"",f=me.has(b.id),x=f?"expanded":"",L=f?"display: block;":"display: none;",I=new Date(b.timestamp*1e3),k=I.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=I.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=kt(g,a),_=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",j="";if(u){let E="";if(g==="engagement.decision"){let C=w=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${w}</h5>`;E=`
                    ...
                    `}else if(g==="system.attention.expired"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,w=a.timestamp-a.last_active,M=Math.floor(w/60);E=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,w="";a.eval_count&&(w=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,w=a.signals||{};E=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(w.topics||[]).map(N=>`<span class="profile-badge">${N}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(g==="system.analysis.audit"){let C=a.success?"#03dac6":"#ff4d4d",w=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,N="";a.error&&(N=`
                            <div class="event-detail-block">
                                ${M("Error")}
...
`);return`
        ...
        <div id="processes-history-widgets" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 30px; opacity: 0.8;"></div>`,_t=()=>`
        <div class="system-section-header">
        ...
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,zt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Lt()}
        </div>`;async function Ht(){await Promise.all([Se(),We(),Ge(),ye()]),await $e()}var Fe=null,Mt=null;async function Pt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function At(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function $s(){try{return await(await B("/processes")).json()}catch{return null}}async function Ss(){try{return await(await B("/agent/status")).json()}catch{return null}}async function We(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),v=o=>{if(o){if(i&&(i.innerHTML=`
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                <div class="service-widget-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 0;">
                    ...
                    <div style="font-size: 0.6em; color: #444; margin-top: 10px;">Last synced: ${new Date(Fe).toLocaleTimeString()}</div>
                </div>
                ...
                </div>`:F='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let z=H()?"easter.company":b(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${f}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${L}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${z}</span></div>${F}</div></div>`}let $=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),h=new Set(m.map(o=>o.id));for(let[o,d]of $)h.has(o)||d.remove();let u=new Set;m.forEach(o=>{if(u.has(o.id))return;u.add(o.id);let d=g(o),f=$.get(o.id);f&&f.parentNode?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function Ge(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Pt();if(!e){t.children.length===0&&(t.innerHTML=R("offline","Failed to load model status.","The event service may be offline."));return}Mt=Date.now(),X(2,Mt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let m=l.status==="Ready";return`
          ...
          `)};e&&I(e,document.getElementById("guardian-sentry-stats"),h.protocols.sentry,"sentry"),i&&I(i,document.getElementById("guardian-architect-stats"),h.protocols.architect,"architect");let k=document.getElementById("analyzer-synthesis-val"),T=document.getElementById("analyzer-synthesis-stats");k&&I(k,T,u.protocols.synthesis,"synthesis");let S=document.getElementById("imaginator-alert_review-val"),_=document.getElementById("imaginator-alert_review-stats");S&&I(S,_,o.protocols.alert_review,"alert_review");let j=document.getElementById("system-state-label"),P=document.getElementById("system-state-val");if(P&&d.state){let A=d.state,F=L(d.state_time||0);j&&(j.textContent=`State: ${A.toUpperCase()}`),P.textContent=F,A==="idle"?P.style.color=d.state_time>300?"#5eff5e":"#fff":P.style.color="#bb86fc"}n&&(n.textContent=L(d.metrics?.total_idle_time||0)),c&&(c.textContent=L(d.metrics?.total_active_time||0)),r&&(r.textContent=L(d.metrics?.total_waste_time||0))}else[e,i,s,n,c,r].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let y=await $s(),p=[],b=[],a=[];y&&(Array.isArray(y)?p=y:(p=y.active||[],b=y.queue||[],a=y.history||[],a.sort((h,u)=>(u.end_time||0)-(h.end_time||0)))),p.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=R("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Je(t,p,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Je(g,b,!1)));let $=document.getElementById("processes-history-widgets");$&&(a.length===0?$.querySelector(".tab-placeholder")||($.innerHTML=R("empty","No recent history.")):($.querySelector(".tab-placeholder")&&($.innerHTML=""),Je($,a,!0))),we(1,p.length)}function Je(t,e,i){function s(l){let m="";l.end_time?m=`${l.end_time-l.start_time}s`:m=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let y=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[p]?p=b[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
    ...
    `;async function Ut(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Rt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Rt(t)}async function Rt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=R("empty","No web history found.");return}let s=`
            <div style="display: flex; flex-direction: column; height: 100%;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,s]of Object.entries(t)){let n=Ft.filter(c=>c.category===i);n.length!==0&&(e+=`
    ...
    </div>`,e},Ke=!1;function ks(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`

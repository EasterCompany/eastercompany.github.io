"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",i=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function me(t){let e=null,s=t.onClose||null,n=t.onOpen||null;function i(){e&&(e.style.zIndex=(++ut).toString())}function l(){if(e){e.classList.add("open"),i(),window.addEventListener("resize",r),n&&setTimeout(n,10);return}let m=document.getElementById("windows-container");m||(m=document.createElement("div"),m.id="windows-container",m.className="windows-container",document.body.appendChild(m)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",i);let v=t.icon||"bx-window",o="",b="",S,C='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(o=`<div class="tab-bar">${t.tabs.map((c,u)=>{let x=c.icon?`<i class="bx ${c.icon}"></i>`:`<span class="tab-glyph">${c.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,S=`<div class="window-content">${t.tabs.map((c,u)=>`<div class="tab-content ${u===0?"active":""}" data-tab-content="${u}">${c.content}${C}</div>`).join("")}</div>`):(t.title&&(b=`<div class="window-title">${t.title}</div>`),S=`<div class="window-content">${t.content||""}${C}</div>`),e.innerHTML=`
            ...
            ${S}
        `,m.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",a=>{a.stopPropagation(),p()}),window.addEventListener("resize",r),t.tabs&&e.querySelectorAll(".tab").forEach(a=>{a.addEventListener("click",()=>{if(!e)return;let d=a.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(u=>u.classList.remove("active")),a.classList.add("active"),e.querySelectorAll(".tab-content").forEach(u=>u.classList.remove("active"));let c=e.querySelector(`.tab-content[data-tab-content="${d}"]`);c&&c.classList.add("active"),g(a,e)})}),setTimeout(()=>{e&&e.classList.add("open"),n&&n()},10)}function r(){if(!e||!e.classList.contains("open"))return;let m=e.querySelector(".tab.active");m&&g(m,e)}function g(m,v){setTimeout(()=>{let o=v.querySelector(".tab-bar");if(!o)return;let b=Array.from(o.querySelectorAll(".tab")),S=b.indexOf(m),C=o.clientWidth,a=b[Math.max(0,S-2)],d=b[Math.min(b.length-1,S+2)],c=o,u=a.offsetLeft-c.offsetLeft-25,y=d.offsetLeft+d.offsetWidth-c.offsetLeft+25-u,w=y<=C?u-(C-y)/2:m.offsetLeft-c.offsetLeft-C/2+m.offsetWidth/2;o.scrollTo({left:w,behavior:"smooth"})},350)}function p(m=!1){e&&(window.removeEventListener("resize",r),m?(e.remove(),e=null):(e.classList.remove("open"),s&&s(),setTimeout(()=>{e?.remove(),e=null},400)))}function f(m){t.content=m;let v=e?.querySelector(".window-content");v&&(v.innerHTML=m)}function h(){return!!(e&&e.classList.contains("open"))}return{open:l,close:p,setContent:f,isOpen:h,focus:i,id:t.id}}var ft="easter_theme",Q={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Ke(){return localStorage.getItem(ft)||Q.DARK}function gt(t){if(!Object.values(Q).includes(t))throw new Error("Invalid theme");localStorage.setItem(ft,t),bt(t)}function bt(t){let e=document.body;Object.values(Q).forEach(l=>{e.classList.remove(`theme-${l}`)}),e.classList.add(`theme-${t}`);let s=document.querySelector('meta[name="theme-color"]');s||(s=document.createElement("meta"),s.name="theme-color",document.getElementsByTagName("head")[0].appendChild(s));let n={[Q.DARK]:"#050507",[Q.LIGHT]:"#FFFFFF",[Q.LEGACY]:"#000000"},i=n[t]||n[Q.DARK];if(s.setAttribute("content",i),!document.querySelector(".background")){let l=document.createElement("div");l.className="background",document.body.prepend(l)}e.classList.add("is-animated")}function vt(){let t=Ke();bt(t)}function U(t,e,s=null,n="default"){let l={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=s?`<p class="placeholder-action">${s}</p>`:"";return`<div class="tab-placeholder" data-placeholder-id="${n}"><i class='bx ${l} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function P(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function ee(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!s)return;if(!e){s.textContent="Last updated: never";return}let n=Date.now(),i=Math.floor((n-e)/1e3),l;i<60?l=`${i}s ago`:i<3600?l=`${Math.floor(i/60)}m ago`:l=`${Math.floor(i/3600)}h ago`,s.textContent=`Last updated: ${l}`}var us=0;function Ae(t,e){let s=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!s)return;let n=s.querySelector(".notification-badge");n&&(e>0?(n.textContent=e>9?"9+":e.toString(),n.style.display="flex"):n.style.display="none")}var ie=0,ae=0;function xt(t){ie=t,Te()}function ht(t){ae=t,Te()}function Te(){let t=ie+ae;us=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let s=document.getElementById("alerts-menu-item");if(s){let r=s.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",s.appendChild(r)),r.textContent=ie>9?"9+":ie.toString(),r.style.display=ie>0?"flex":"none"}let n=document.getElementById("workspace-menu-item");if(n){let r=n.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",n.appendChild(r)),r.textContent=ae>9?"9+":ae.toString(),r.style.display=ae>0?"flex":"none"}let i=document.getElementById("switch-alerts");if(i){let r=i.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",i.appendChild(r)),r.textContent=ie>9?"9+":ie.toString(),r.style.display=ie>0?"flex":"none"}let l=document.getElementById("switch-workspace");if(l){let r=l.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",l.appendChild(r)),r.textContent=ae>9?"9+":ae.toString(),r.style.display=ae>0?"flex":"none"}}function Ye(){let t=document.getElementById("alerts-list");if(!t)return;ie=t.querySelectorAll(".alert-unread:not(.priority-low)").length,Te()}function wt(){let t=document.getElementById("blueprints-list");if(!t)return;ae=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,Te()}function fs(){return"https://event.easter.company"}function gs(){return"https://discord.easter.company"}var bs="http://127.0.0.1:8100",vs="http://127.0.0.1:8300",ys={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Re(t){let e=P(t);if(!e)return e;e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(i,l)=>{let r=ys[l];return r?`<span class="${r}">`:""});let s=(e.match(/<span/g)||[]).length,n=(e.match(/<\/span/g)||[]).length;return s>n&&(e+="</span>".repeat(s-n)),e}function ue(t){if(!t)return"";let e=P(t);return e&&(e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(s,n)=>{let i=n.split("|").map(l=>l.trim());return i.every(l=>l.match(/^:?-+:?$/))?"":`<div class="md-table-row">${i.map(l=>`<span class="md-table-cell">${l}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e)}var xs="https://sterling-javelin-12539.upstash.io",hs="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function H(){return window.location.hostname.includes("easter.company")}var j=null,oe={lastDashboard:0,lastFrontend:0},Ze=!1,Ct=0,Et="dex_dashboard_snapshot";function ws(){let t=localStorage.getItem(Et);if(t)try{let e=JSON.parse(t);j=e,oe.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{j=null}}async function yt(){if(!(!H()||Ze)){Ze=!0,Ct=Math.floor(Date.now()/1e3);try{let t=await $s("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);j=e,oe.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),oe.lastFrontend=Date.now(),localStorage.setItem(Et,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Ze=!1}}}function Cs(){if(!j||!j.agent_status)return;let t=j.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function Es(){if(!H())return;ws();let t=Math.floor(Date.now()/1e3),e=j?t-j.timestamp:1/0;(!j||e>120)&&await yt(),setInterval(()=>{let s=new Date,n=Math.floor(Date.now()/1e3),i=j?n-j.timestamp:1/0,l=n-Ct,r=s.getSeconds()===59,g=i>300,p=l>60;(r&&p||g&&p)&&yt(),Cs()},1e3)}H()&&Es();async function $s(t,...e){try{let n=await(await fetch(xs,{method:"POST",headers:{Authorization:`Bearer ${hs}`},body:JSON.stringify([t,...e])})).json();if(n.error)throw new Error(n.error);return n.result}catch(s){return console.error("Upstash Error:",s),null}}var Ee=null,$e=null,ze=!1,Ne=!1;async function B(t,e={}){if(H()){if(!j)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(j.monitor),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(j.processes),{status:200});if(t.startsWith("/events")){let i=new URLSearchParams(t.split("?")[1]||""),l=i.get("type")||i.get("event.type"),r=i.get("category"),g=i.get("order")||"desc",p=[];return l==="system.notification.generated"?p=[...j.alerts||[]]:l==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?p=[...j.blueprints||[]]:r==="messaging"?p=[...j.messaging_events||[]]:r==="system"?p=[...j.system_events||[]]:r==="cognitive"?p=[...j.cognitive_events||[]]:r==="moderation"?p=[...j.moderation_events||[]]:p=[...j.events||[]],g==="asc"?p.sort((f,h)=>f.timestamp-h.timestamp):p.sort((f,h)=>h.timestamp-f.timestamp),new Response(JSON.stringify({events:p,count:p.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(j.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let i=t.split("/")[2],l=j.profiles?j.profiles[i]:null;return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(j.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify(j.chores||[]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(Ee)try{let i=await fetch(Ee+t,e);if(i.ok)return i;Ee=null}catch{Ee=null}let s=fs(),n=bs;try{let i=await fetch(s+t,e);if(i.ok)return Ee=s,ze&&(console.log("\u2728 Production event service restored."),ze=!1),i;throw new Error("Primary failed")}catch{ze||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${n}`),ze=!0);try{let l=await fetch(n+t,e);if(l.ok)return Ee=n,l;throw new Error("Fallback failed")}catch(l){throw l}}}async function fe(t,e={}){if(H()){if(!j)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(j.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let i=t.split("/")[2],l=(j.contacts?.members||[]).find(r=>r.id===i);return l?new Response(JSON.stringify(l),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if($e)try{let i=await fetch($e+t,e);if(i.ok)return i;$e=null}catch{$e=null}let s=gs(),n=vs;try{let i=await fetch(s+t,e);if(i.ok)return $e=s,Ne&&(console.log("\u2728 Production discord service restored."),Ne=!1),i;throw new Error("Primary failed")}catch{Ne||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${n}`),Ne=!0);try{let l=await fetch(n+t,e);if(l.ok)return $e=n,l;throw new Error("Fallback failed")}catch(l){throw l}}}var $t=()=>`
...
`,Pe=null,ge=new Set,Oe=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Ts(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await B(s);if(!n.ok)throw new Error("Service is offline or unreachable.");let l=(await n.json()).events||[];Pe=Date.now(),ee(0,Pe);let r=Date.now(),g=24*60*60*1e3,p=l.filter(d=>{let c=localStorage.getItem(`alert_read_ts_${d.id}`);if(!c)return!0;let u=parseInt(c);return r-u<g});p.sort((d,c)=>{let u=E=>{let T=E.event;if(typeof T=="string")try{T=JSON.parse(T)}catch{return"low"}return(T.priority||"low").toLowerCase()},x=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,y=x(u(d)),w=x(u(c));return y!==w?w-y:c.timestamp-d.timestamp}),Oe=p;let f=d=>{let c=d.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return"low"}return(c.priority||"low").toLowerCase()},h=[];if(p.length===0)h.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let c=new Set(p.map(x=>f(x))).size>1,u=null;p.forEach(x=>{let y=f(x);c&&y!==u&&(h.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),u=y),h.push(x)})}t&&(e.innerHTML="");let m=d=>{let c=d.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{return null}let u=(c.title||"Untitled Alert").trim(),x=(c.body||"No description provided.").trim(),y=c.summary||"",w=c.content||"",E=c.protocol||"",T=(c.priority||"low").toLowerCase(),A=!!c.alert,_=(c.category||"system").trim(),N=c.related_event_ids||[],R=c.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),K=new Date(d.timestamp*1e3),L=K.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=K.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=q?"event-border-grey":"event-border-blue";!q&&A&&(k="event-border-red"),q&&(T==="high"||T==="critical")?k="event-border-red":q&&T==="medium"&&(k="event-border-orange");let I=q?"alert-read":"alert-unread",O=ge.has(d.id),z=O?"expanded":"",M=O?"display: block;":"display: none;",F="",D="";N.length>0&&(D=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${N.map(ce=>`<a href="#" onclick="window.dexter.viewEvent('${ce}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ce.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let J="";R&&(J=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ue(w)}</div>
            </div>
        `:se=`
            <div class="event-detail-block" style="text-align: left;">
        ...
        `,F=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${c.related_services&&c.related_services.length>0?c.related_services.join(", "):c.related||"SYSTEM"}</div>
                    </div>
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                ...
                ${se}
            `;let G=document.createElement("div");G.className=`event-item notification-item ${k} ${I} ${z} cursor-pointer priority-${T}`,G.dataset.alertId=d.id,G.onclick=function(ce){let X=this;if(X.classList.contains("expanded")){X.classList.remove("expanded"),ge.delete(d.id);let Le=X.querySelector(".event-details");Le&&(Le.style.display="none")}else{X.classList.add("expanded"),ge.add(d.id);let Le=X.querySelector(".event-details");if(Le&&(Le.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),X.classList.add("alert-read"),X.classList.remove("alert-unread"),X.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ve="event-border-grey";T==="high"||T==="critical"?Ve="event-border-red":T==="medium"&&(Ve="event-border-orange"),X.classList.add(Ve),Ye()}}};let le=`${E?E.toUpperCase():"GUARDIAN"} ALERT: ${y||u}`,Ge={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[_]||"bx-bell",ms=T==="high"||T==="critical"?"#ff4d4d":T==="medium"?"#ffa500":"#888";G.innerHTML=`
            ...
            `;let lt=G.querySelector(".event-details");lt&&lt.addEventListener("click",ce=>{ce.stopPropagation()});let ct=G.querySelector(".close-details-btn");return ct&&ct.addEventListener("click",ce=>{ce.stopPropagation(),G.classList.remove("expanded");let X=G.querySelector(".event-details");X&&(X.style.display="none"),ge.delete(d.id)}),G},v=d=>{let c=document.createElement("div");c.className="notification-divider",c.dataset.alertId=d.id;let u="#888888";return d.label==="CRITICAL"?u="#ff4d4d":d.label==="HIGH"?u="#ff8888":d.label==="MEDIUM"&&(u="#ffa500"),c.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${u}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,c.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${u}44, transparent); flex-grow: 1;"></div>`,c},o=d=>{let c=document.createElement("div");c.innerHTML=U("empty",d.message,null,d.id);let u=c.firstElementChild;return u.dataset.alertId=d.id,u},b=Array.from(e.children),S=new Map(b.map(d=>[d.dataset.alertId,d])),C=new Set(h.map(d=>d.id));b.forEach(d=>{let c=d.dataset.alertId;(!c||!C.has(c))&&d.remove()});let a=null;h.forEach((d,c)=>{let u=S.get(d.id);if(!u||t){if(u&&u.remove(),d.type==="divider")u=v(d);else if(d.type==="placeholder")u=o(d);else{let x=m(d);if(!x)return;u=x}if(!u)return}c===0?e.firstElementChild!==u&&e.prepend(u):a&&a.nextElementSibling!==u&&a.after(u),a=u}),Pe=Date.now(),ee(0,Pe),Ye()}catch(n){console.error("Error fetching alerts:",n),e.children.length===0&&(e.innerHTML=U("offline","Failed to load alerts.","The event service may be offline."))}}function Ts(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),s=document.getElementById("alerts-close-all"),n=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Oe.forEach(i=>{localStorage.getItem(`alert_read_ts_${i.id}`)||localStorage.setItem(`alert_read_ts_${i.id}`,Date.now().toString())}),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Oe.forEach(i=>{ge.add(i.id),localStorage.getItem(`alert_read_ts_${i.id}`)||localStorage.setItem(`alert_read_ts_${i.id}`,Date.now().toString())}),de(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{ge.clear(),de(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let i=Date.now()-48*60*60*1e3;Oe.forEach(l=>{localStorage.setItem(`alert_read_ts_${l.id}`,i.toString())}),ge.clear(),de(!0)}catch(i){console.error("Failed to clear alerts:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}async function Xe(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let n=(await e.json()).events||[],i=0;n.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${l.id}`)||i++}),xt(i)}catch{}}var Tt=()=>`
...
`,be=new Set,je=[],ke=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let s=await B("/roadmap");if(!s.ok)throw new Error("Offline");let n=await s.json();je=n;let i=f=>{let h=be.has(f.id),m=f.state==="published",v=f.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let S=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),C=document.createElement("div");C.className=`event-item notification-item ${o} cursor-pointer ${h?"expanded":""}`,C.dataset.itemId=f.id,C.onclick=c=>{if(c.target.closest("button"))return;if(C.classList.contains("expanded")){C.classList.remove("expanded");let x=C.querySelector(".event-details");x&&(x.style.display="none"),be.delete(f.id)}else{C.classList.add("expanded");let x=C.querySelector(".event-details");x&&(x.style.display="block"),be.add(f.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;C.innerHTML=`
        <div class="event-time">
      ...
      `;let d=C.querySelector(".event-details");return d&&d.addEventListener("click",c=>{c.stopPropagation()}),C.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(f)),C.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ms(f)),C.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(f.id)),C.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),C.classList.remove("expanded");let u=C.querySelector(".event-details");u&&(u.style.display="none"),be.delete(f.id)}),C},l=Array.from(e.children),r=new Map(l.map(f=>[f.dataset.itemId,f])),g=new Set(n.map(f=>f.id));if(l.forEach(f=>{let h=f.dataset.itemId;(!h||!g.has(h))&&f.remove()}),!je||je.length===0){e.innerHTML=U("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let p=null;n.forEach((f,h)=>{let m=r.get(f.id);(!m||t)&&(m&&m.remove(),m=i(f),!m)||(h===0?e.firstElementChild!==m&&e.prepend(m):p&&p.nextElementSibling!==m&&p.after(m),p=m)})}catch{e.children.length===0&&(e.innerHTML=U("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),n=document.getElementById("roadmap-expand-all"),i=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ke=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),ke=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let g=ke?`/roadmap/${ke}`:"/roadmap",p=ke?"PATCH":"POST";try{await B(g,{method:p,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let f=document.getElementById("roadmap-editor-container");f&&(f.style.display="none"),ve(!0)}catch(f){console.error(f)}},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{je.forEach(l=>be.add(l.id)),ve(!0)},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{be.clear(),ve(!0)},i.dataset.listenerAttached="true")}function Ss(t){ke=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function Ms(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(s){console.error(s)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let n=await B(s);if(!n.ok)throw new Error("Service is offline or unreachable.");let l=(await n.json()).events||[];if(At=l,St=Date.now(),ee(2,St),l.length===0){e.innerHTML=U("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Ae(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),S=(v.content||"").trim(),C=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(z=>z.trim()),d=(v.implementation_path||[]).map(z=>z.trim()),c=v.source_event_ids||[],u=v.approved===!0,x=new Date(m.timestamp*1e3),y=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),w=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=ye.has(m.id),T=E?"display: block;":"display: none;",A=document.createElement("div");A.className=`event-item notification-item event-border-purple cursor-pointer ${E?"expanded":""} ${u?"blueprint-approved":""}`,A.dataset.blueprintId=m.id,u&&(A.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",A.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let N=u?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[C]||"bx-paint";A.onclick=function(z){let M=this;if(M.classList.contains("expanded")){M.classList.remove("expanded"),ye.delete(m.id);let D=M.querySelector(".event-details");D&&(D.style.display="none")}else{M.classList.add("expanded"),ye.add(m.id);let D=M.querySelector(".event-details");D&&(D.style.display="block")}};let R="";d.length>0&&(R=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${d.map(z=>`<li style="margin-bottom: 5px;">${P(z)}</li>`).join("")}</ul></div>
                    </div>
                `);let W="";c.length>0&&(W=`
                ...
                `);let q=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",K=H(),L=u?`
            ...
            `;A.innerHTML=`
                ...
                <div class="event-icon" style="${u?"color: #03dac6;":""}"><i class='bx ${N}'></i></div>
                <div class="event-content">
            ...
            `;let $=A.querySelector(".blueprint-approve-btn");$&&($.onclick=async z=>{z.stopPropagation(),$.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(M){console.error("Failed to approve blueprint:",M)}});let k=A.querySelector(".blueprint-delete-btn");k&&(k.onclick=async z=>{z.stopPropagation();let M=!u;k.innerHTML=M?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(F){console.error("Failed to delete blueprint:",F)}});let I=A.querySelector(".event-details");I&&I.addEventListener("click",z=>{z.stopPropagation()});let O=A.querySelector(".close-details-btn");return O&&O.addEventListener("click",z=>{z.stopPropagation(),A.classList.remove("expanded");let M=A.querySelector(".event-details");M&&(M.style.display="none"),ye.delete(m.id)}),A},g=Array.from(e.children),p=new Map(g.map(m=>[m.dataset.blueprintId,m])),f=new Set(l.map(m=>m.id));g.forEach(m=>{let v=m.dataset.blueprintId;(!v||!f.has(v))&&m.remove()});let h=null;l.forEach((m,v)=>{let o=p.get(m.id);if(!o||t){o&&o.remove();let b=r(m);if(!b)return;o=b}v===0?e.firstElementChild!==o&&e.prepend(o):h&&h.nextElementSibling!==o&&h.after(o),h=o}),Ae(2,l.length),wt()}catch(n){console.error("Error fetching blueprints:",n),e.children.length===0&&(e.innerHTML=U("offline","Failed to load blueprints.","The event service may be offline."))}}async function It(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let n=(await e.json()).events||[],i=0;n.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&i++}),ht(i)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(n=>ye.add(n.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(n){console.error("Failed to clear blueprints:",n)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var re="STANDBY",Bt=null,Se=[],Y=null,Ue=null,Ht=()=>`
  ...
  `}async function Me(){let t=document.getElementById("progress-view-root");t&&(re!==Bt&&(t.innerHTML=_t(),Bt=re),re==="ACTIVE"&&_s())}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),s=document.getElementById("active-task-progress-text"),n=document.getElementById("terminal-output");if(t&&Y&&(t.innerText=Y.title),e&&Y&&(e.style.width=`${Y.progress}%`),s&&Y&&(s.innerText=`${Y.progress}% COMPLETE`),n){let i=n.children.length;if(Se.length>i){for(let l=i;l<Se.length;l++){let r=Se[l],g=document.createElement("div");g.className="terminal-line",g.innerHTML=`
            <span class="line-time">${r.time}</span>
            <span class="line-prefix">></span>
            <span class="line-msg">${P(r.msg)}</span>
        `,n.appendChild(g)}n.scrollTop=n.scrollHeight}}}window.addEventListener("dex-mock-start",()=>{re="ACTIVE",Se=[{time:new Date().toLocaleTimeString(),msg:"Initializing cognitive engine..."},{time:new Date().toLocaleTimeString(),msg:"Connecting to event-service..."}],Y={title:"Refactoring System Event Bus",progress:10},Me();let t=0,e=setInterval(()=>{if(re!=="ACTIVE"){clearInterval(e);return}t++,Y.progress=Math.min(100,Y.progress+Math.floor(Math.random()*15));let s=["Analyzing internal/bus.go...","Checking circular dependencies.","Optimizing channel throughput.","Writing unit tests for refactor.","Deploying to staging environment.","Verifying system integrity."];t<s.length&&Se.push({time:new Date().toLocaleTimeString(),msg:s[t]}),Y.progress>=100&&(re="COMPLETED",Ue={duration:"1m 12s",steps:32,result:"Optimized Event Bus"},clearInterval(e)),Me()},1500)});window.addEventListener("dex-mock-stop",()=>{re="STANDBY",Me()});window.addEventListener("dex-mock-reset",()=>{re="STANDBY",Me()});var Dt=()=>`
...
`,zt=()=>`
    ...
    `,Ie=[],Be={"313071000877137920":"Owen",dexter:"Dexter"},te=[],he=null;async function He(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),n=document.getElementById("save-chore-btn"),i=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),g=document.getElementById("task-filter-owner"),p=document.getElementById("new-chore-instruction"),f=document.getElementById("new-chore-url"),h=document.getElementById("new-chore-schedule"),m=document.getElementById("form-title"),v=document.getElementById("form-icon");function o(){if(r){if(te.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=te.map(c=>{let u=Be[c]||c,x=c.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${c==="dexter"?"#bb86fc":x?"#03dac6":"#fff"};">
          <i class='bx ${c==="dexter"?"bx-terminal":x?"bx-hash":"bx-user"}'></i>
          <span>${u}</span>
          <i class='bx bx-x remove-recipient' data-id="${c}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(c=>{c.onclick=()=>{let u=c.dataset.id;te=te.filter(x=>x!==u),o()}})}}function b(c){c&&!te.includes(c)&&(te.push(c),o()),l&&(l.value="")}function S(c=null){s&&(he=c?c.id:null,c?(m&&(m.textContent="Update Research Task"),v&&(v.className="bx bx-edit-alt",v.style.color="#03dac6"),n&&(n.innerHTML="<i class='bx bx-check'></i> Update Task"),p&&(p.value=c.natural_instruction),f&&(f.value=c.execution_plan?.entry_url||""),h&&(h.value=c.schedule),te=c.recipients||(c.owner_id?[c.owner_id]:[]),o()):(m&&(m.textContent="Initialize Research Task"),v&&(v.className="bx bx-plus-circle",v.style.color="#bb86fc"),n&&(n.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),p&&(p.value=""),f&&(f.value=""),h&&(h.value="every_24h"),te=[],o()),s.style.display="block",s.scrollIntoView({behavior:"smooth",block:"start"}))}let C=document.getElementById("contacts-group"),a=document.getElementById("channels-group");if(C&&!C.dataset.populated&&!H())try{fe("/contacts").then(async c=>{c.ok&&(((await c.json()).members||[]).forEach(y=>{if(Be[y.id]=y.nickname||y.username,y.id==="313071000877137920")return;let w=document.createElement("option");w.value=y.id;let E=y.nickname||y.username;w.textContent=`${E} (${y.username})`,C.appendChild(w)}),C.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(a&&!a.dataset.populated&&!H())try{fe("/channels").then(async c=>{c.ok&&((await c.json()).forEach(x=>{let y=`channel:${x.id}`;Be[y]=x.name;let w=document.createElement("option");w.value=y,w.textContent=`#${x.name} (${x.guild})`,a.appendChild(w)}),a.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{b(l.value)},l.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onchange=()=>d(),g.dataset.listenerAttached="true");function d(){if(!t)return;let c=g?.value||"all",u=c==="all"?Ie:Ie.filter(y=>(y.recipients||(y.owner_id?[y.owner_id]:[])).includes(c));if(u.length===0){t.innerHTML=U("empty",c==="all"?"No active tasks.":"No tasks found for this owner.",H()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let x=u.map(y=>{let w=y.last_run===0?"Never":new Date(y.last_run*1e3).toLocaleString(),E=y.memory?y.memory.length:0,T=y.status==="active"?"#03dac6":"#666",_=(y.recipients||(y.owner_id?[y.owner_id]:[])).map(N=>{let R=Be[N]||N.substring(0,8),W=N.startsWith("channel:");return`<span title="${R}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${N==="dexter"?"bx-terminal":W?"bx-hash":"bx-user"}'></i>${R}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${T}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    <div class="service-widget-header" style="display: flex; align-items: flex-start; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 15px; margin-bottom: 15px;">
                        <div style="display: flex; gap: 15px; flex: 1;">
                            <i class='bx bx-search-alt' style="color: ${T}; font-size: 1.5em; margin-top: 2px;"></i>
                            <div style="text-align: left;">
                                <h3 style="font-size: 1.1em; white-space: normal; line-height: 1.4; font-weight: 600; margin: 0;">${y.natural_instruction}</h3>
                                <div style="margin-top: 8px; display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                                    ...
                                    <span style="font-size: 0.7em; color: #666; font-family: 'JetBrains Mono', monospace;"><i class='bx bx-time' style="margin-right: 4px;"></i>${y.schedule}</span>
                                </div>
                            </div>
                        </div>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <button class="icon-btn reset-chore-btn" data-id="${y.id}" title="Reset Progress" style="background: none; border: none; color: #ff9800; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-refresh' style="font-size: 1.2em;"></i></button>
                            <button class="icon-btn edit-chore-btn" data-id="${y.id}" style="background: none; border: none; color: #bb86fc; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-edit-alt' style="font-size: 1.2em;"></i></button>
                            <button class="icon-btn delete-chore-btn" data-id="${y.id}" style="background: none; border: none; color: #cf6679; cursor: pointer; padding: 8px; border-radius: 50%; transition: background 0.2s;"><i class='bx bx-trash' style="font-size: 1.2em;"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="service-widget-body" style="display: flex; flex-wrap: wrap; gap: 30px; align-items: center;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 2px; flex: 1; min-width: 200px;">
                            ...
                            <span style="font-size: 0.8em; color: #888; font-family: 'JetBrains Mono'; word-break: break-all;">${y.execution_plan.entry_url||"Autonomous Detection"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=x,t.querySelectorAll(".edit-chore-btn").forEach(y=>{y.onclick=w=>{w.stopPropagation();let E=y.dataset.id,T=Ie.find(A=>A.id===E);T&&S(T)}}),t.querySelectorAll(".reset-chore-btn").forEach(y=>{y.onclick=async w=>{w.stopPropagation();let E=y.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(y.innerHTML="<i class='bx bx-loader-alt spin'></i>",await B(`/chores/${E}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),He())}}),t.querySelectorAll(".delete-chore-btn").forEach(y=>{y.onclick=async w=>{w.stopPropagation();let E=y.dataset.id;confirm("Delete this task?")&&(await B(`/chores/${E}`,{method:"DELETE"}),He())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display==="none"||he!==null?S(null):s.style.display="none")},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{s&&(s.style.display="none")},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let c=document.getElementById("new-chore-instruction"),u=document.getElementById("new-chore-url"),x=document.getElementById("new-chore-schedule"),y=c?.value,w=x?.value||"every_24h";if(!y)return;if(te.length===0){alert("Please add at least one recipient.");return}let E=n.innerHTML;n.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let T=he?"PATCH":"POST",A=he?`/chores/${he}`:"/chores";await B(A,{method:T,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:te,natural_instruction:y,entry_url:u?.value,schedule:w})}),s&&(s.style.display="none"),c&&(c.value=""),u&&(u.value=""),he=null,te=[],He()}catch(T){console.error(T),alert(he?"Failed to update research task":"Failed to create research task")}finally{n.innerHTML=E}},n.dataset.listenerAttached="true");try{let c=await B("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);if(Ie=await c.json()||[],g){let x=g.value,y=new Set;Ie.forEach(w=>{w.owner_id&&y.add(w.owner_id),w.recipients&&w.recipients.forEach(E=>y.add(E))}),g.innerHTML='<option value="all">All Recipients</option>',y.forEach(w=>{let E=document.createElement("option");E.value=w;let T=w.startsWith("channel:"),A=Be[w]||`Recipient: ${w.substring(0,8)}`;E.textContent=(T&&!A.startsWith("#")?"#":"")+A,g.appendChild(E)}),g.value=x,g.selectedIndex===-1&&(g.value="all")}d()}catch(c){console.error("Failed to fetch chores",c)}}var Nt=()=>`
...
`;async function Qe(){await Promise.all([ve(),xe(),Me(),He()])}var Ds=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function jt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Ut(t){if(!document.getElementById("dex-profile-styles")){let s=document.createElement("style");s.id="dex-profile-styles",s.textContent=Ds,document.head.appendChild(s)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",s=>{s.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",fe(`/profile/${t.id}`).then(async s=>{if(s.ok){let n=await s.json();et(e,t,n)}else{console.log("Profile not found or error, using mock data.");let n=jt(t.id,t.username);et(e,t,n)}}).catch(s=>{console.error("Profile fetch error:",s);let n=jt(t.id,t.username);et(e,t,n)})}function et(t,e,s){let n=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",i=()=>{let S=(s.attributes||s.facts||[]).map(x=>{let y=x.key||x.k,w=x.value||x.v;return`
            <div class="fact-chip">
                <span class="fact-key">${y}:</span>
                <span class="fact-val">${w}</span>
            </div>
        `}).join(""),C=s.cognitive_model||s,a=C.technical_level||C.techLevel||0,d=C.communication_style||C.commStyle||"Unknown",c=C.patience_level||C.patience||"Unknown",u=C.vibe||"Unknown";return`
                    ...
                    <div class="cog-metric">
                     ...
                     <div class="cog-metric">
                        <div class="cog-label">
                            ...
                            <span style="font-family: 'JetBrains Mono'; color: #bb86fc;">${u}</span>
                        </div>
                    </div>
                </div>
        ...
        `},l=()=>{let S=s.dossier||{},C=S.identity||{},a=S.career||{},d=S.personal||{},c=S.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${C.fullName||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Age Range</div>
                        <div class="dossier-value">${C.ageRange||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                        
                        <div class="dossier-label">Location</div>
                        <div class="dossier-value">${C.location||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Gender</div>
                        <div class="dossier-value">${C.gender||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Sexuality</div>
                        <div class="dossier-value" style="color: #ffb74d;">${C.sexuality||"Unknown"}</div>
                        <div style="height: 15px;"></div>

                        <div class="dossier-label">Relationship Status</div>
                        <div class="dossier-value">${C.relationship||"Unknown"}</div>
                    </div>
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(a.skills||[]).map(u=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${u}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(d.hobbies||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(d.habits||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(d.vices||[]).map(u=>`<div class="dossier-list-item" style="color: #cf6679;">${u}</div>`).join("")}
                        </div>
                    </div>
                </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin-top: 30px; text-align: center;">
            ${Object.entries(S).map(([C,a])=>`
                <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                    ...
                    <div style="font-size: 0.8em; text-transform: uppercase; letter-spacing: 1px; color: #aaa;">${C.substring(0,4)}</div>
        ...
        <div style="margin-top: 40px;">
            ...
            <div style="height: 100px; border-bottom: 1px solid #333; display: flex; align-items: flex-end; gap: 5px; padding-bottom: 5px;">
                ${Array.from({length:40}).map(()=>{let C=Math.floor(Math.random()*80)+10;return`<div style="flex: 1; background: ${Math.random()>.7?"#cf6679":Math.random()>.5?"#03dac6":"#444"}; height: ${C}%; border-radius: 2px;"></div>`}).join("")}
            </div>
    ...
    `;t.innerHTML=f;let h=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),v=t.querySelectorAll(".profile-tab-btn"),o=t.querySelectorAll(".tab-content"),b=t.querySelector(".close-profile-btn");b&&b.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),v.forEach(S=>{S.addEventListener("click",()=>{v.forEach(d=>d.classList.remove("active")),o.forEach(d=>d.classList.remove("active")),S.classList.add("active");let C=S.dataset.tab,a=t.querySelector(`#tab-${C}`);a&&a.classList.add("active")})}),m&&m.addEventListener("click",()=>{h.classList.toggle("expanded");let S=h.classList.contains("expanded");m.innerHTML=S?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var qt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function tt(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await tt(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let i=s.target.closest(".user-profile-card");if(i&&i.dataset.user)try{let l=JSON.parse(i.dataset.user);Ut(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let s=await fe("/contacts");if(!s.ok)throw new Error("Service unreachable");let i=(await s.json()).members||[];if(zs=Date.now(),i.length===0){t.innerHTML=U("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};i.sort((r,g)=>{let p=l[r.level]??10,f=l[g.level]??10;return p!==f?p-f:r.username.localeCompare(g.username)}),t.innerHTML=i.map(r=>{let g=r.color&&r.color!==0,p=g?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",f=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",h="#888";r.level==="Me"||r.level==="Master"?h="#bb86fc":r.level==="Admin"?h="#cf6679":r.level==="Moderator"?h="#03dac6":r.level==="Contributor"&&(h="#ffa500");let m=r.level==="Me",v=m?"rgba(187, 134, 252, 0.08)":g?`${p}11`:"rgba(255,255,255,0.03)",o=m?"2px solid #bb86fc":g?`1px solid ${p}66`:"1px solid rgba(255,255,255,0.1)";return`
...
`,qe=null,we=new Set,_e=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,s]of Object.entries(Rs))if(s.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let n=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(n+=`&category=${Z}`);try{let i=await B(n);if(!i.ok)throw new Error("Service unreachable");if(_e=((await i.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),qe=Date.now(),ee(1,qe),_e.length===0){e.innerHTML=U("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,S=Os(b),C=js(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let c=a?"cursor-pointer":"",u=we.has(v.id),x=u?"expanded":"",y=u?"display: block;":"display: none;",w=new Date(v.timestamp*1e3),E=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),A=Ft(b,o),_=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",N="";if(a){let L="";if(b==="engagement.decision"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                    ...
                    `}else if(b==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,k=o.timestamp-o.last_active,I=Math.floor(k/60);L=`
                    ...
                    `}else if(b==="messaging.bot.sent_message"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,k="";o.eval_count&&(k=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let I="";if(o.chat_history&&o.chat_history.length>0){let O=o.chat_history.length,z=o.chat_history.map((M,F)=>{let D=M.name?M.name.toUpperCase():M.role.toUpperCase();!M.name&&D==="ASSISTANT"&&(D="DEXTER");let J=M.role==="user"?"#03dac6":M.role==="system"?"#ffb74d":"#bb86fc",V=F===O-1?"block":"none";return`
                                <div class="history-slide" data-index="${F}" style="display: ${V};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(M.content)}</div>
                                </div>
                            `}).join("");I=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(b==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,k=o.signals||{};L=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(b==="moderation.explicit_content.deleted"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="analysis.link.completed"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="analysis.visual.completed"){let $=I=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${I}</h5>`,k="";o.base64_preview?k=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${o.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:o.url&&(k=`<div class="event-detail-block"><img src="${o.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(b==="analysis.router.decision"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="system.cli.command"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="system.analysis.audit"){let $=o.success?"#03dac6":"#ff4d4d",k=o.success?"SUCCESS":"FAILED",I=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,O="";o.error&&(O=`
                            <div class="event-detail-block">
                                ${I("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(o.error)}</pre>
                            </div>
                        `);let z="";if(o.chat_history&&o.chat_history.length>0){let D=o.chat_history.length,J=o.chat_history.map((V,se)=>{let G=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&G==="USER"&&(G="SYSTEM"),!V.name&&G==="ASSISTANT"&&(G="AGENT");let le=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${se}" style="display: ${se===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${se+1} of ${D}</span>
                                    </div>
                        ...
                        `}let M="";if(o.corrections&&o.corrections.length>0){let D=o.corrections.map((J,V)=>`
                        ...
                        ${M}
                        ${z}
                    `}else if(b==="system.test.completed"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(b==="error_occurred"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(b==="system.cli.status"){let $=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(b==="messaging.user.sent_message"){let $="";o.attachments&&o.attachments.length>0&&($=`
                        ...
                        ${$}
                    `}N=`
                    <div class="event-details" style="${y}">
                        ${L}
                    </div>
                `}let R=document.createElement("div");R.className=`event-item ${d} ${c} ${x}`,R.dataset.eventId=v.id,R.onclick=function(L){if(!a)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),we.delete(v.id);let I=$.querySelector(".event-details");I&&(I.style.display="none")}else{$.classList.add("expanded"),we.add(v.id);let I=$.querySelector(".event-details");I&&(I.style.display="block")}},R.innerHTML=`
                <div class="event-time">
...
`,Jt=qs;function Wt(){return`
    ...
    `}var Fs=null;async function De(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let s=await e.json();if(!s||s.length===0)return t.innerHTML=U("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let n=["local-ollama-0","local-cache-0"],i=s.filter(r=>!n.includes(r.id));i.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),i.reverse();let l=i.map(r=>{let g=r.logs.join(`
`),p=[...r.logs];if(p.length<25){let h=25-p.length;for(let m=0;m<h;m++)p.push("")}else p.length>25&&(p=p.slice(-25));let f=p.map(h=>Re(h)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let g=r,p=unescape(g.dataset.logs||"");navigator.clipboard.writeText(p).then(()=>{let f=g.querySelector("i");f&&(f.classList.remove("bx-copy"),f.classList.add("bx-check"),setTimeout(()=>{f.classList.remove("bx-check"),f.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let p=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${p}?`))try{(await B(`/logs?service_id=${p}`,{method:"DELETE"})).ok&&De()}catch(f){console.error("Error clearing logs:",f)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=U("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Zt=()=>{let t=H()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
        </div>`;async function ss(){Js(),await Promise.all([ne(),it(),at()]),await De()}var Fe=null;function Js(){Fe||(Fe=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Fe),Fe=null;return}ne(!0)},1e3))}var Gt=null,Vt=null;async function ns(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function Kt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await B("/processes")).json()}catch{return null}}async function Gs(){try{return await(await B("/agent/status")).json()}catch{return null}}async function it(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),n=document.querySelector("#hw-cpu .hw-content"),i=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),g=a=>{if(a){if(s&&(s.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}l&&(a.GPU&&a.GPU.length>0?l.innerHTML=a.GPU.map(d=>{let c=(d.VRAM/1073741824).toFixed(1);return`
                    ...
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let a=await Kt();a?(g(a),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),n&&!n.hasChildNodes())){let a=await Kt();g(a)}if(!t)return;let p=await ns();if(!p||!p.services){t.children.length===0&&(t.innerHTML=U("offline","Failed to load system metrics.","The event service may be offline."));let a=document.querySelector('[data-service-id="upstash-redis-ro"]');if(a){a.className="service-widget service-widget-offline",a.querySelector(".service-widget-status").textContent="ERROR";let d=a.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Gt=H()&&oe.lastDashboard||Date.now(),ee(0,Gt||0);let f=p.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let a=document.getElementById("upstash-countdown");if(a){let u=59-new Date().getSeconds();u<=0&&(u+=60),a.textContent=`${u}s`}},1e3)),Array.from(t.children).forEach(a=>{a.classList.contains("service-widget")||a.remove()});function h(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:a.split(".").slice(0,3).join(".")||"-"}function m(a){return!a||a.length<=28?a:a.substring(0,28)+"..."}function v(a){if(!a||a==="N/A"||a==="unknown")return"-";let d=a.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let c=parseInt(d[1])||0,u=parseInt(d[2])||0,x=parseInt(d[3])||0,y=parseFloat(d[4])||0,w=c*86400+u*3600+x*60+y,E=Math.floor(w/86400);if(E>0)return`${E}d`;let T=Math.floor(w/3600);if(T>0)return`${T}h`;let A=Math.floor(w/60);return A>0?`${A}m`:`${Math.floor(w)}s`}function o(a){if(a.id==="upstash-redis-ro"){let K=H()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=a.status==="online",c=d?"service-widget-online":"service-widget-offline",u=d?"bx-check-circle":"bx-x-circle",x=d?"OK":"BAD",y=a.version?h(a.version.str):"-",w=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",E=a.memory&&a.memory!=="N/A"?a.memory:"-",T=w!=="-"?"#00ff00":"#666",A=w!=="-"?"#fff":"#666",_=E!=="-"?"#00ff00":"#666",N=E!=="-"?"#fff":"#666",R=v(a.uptime),W="";d?W=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        <i class="bx bxs-microchip" style="color: ${T};"></i>
                        <span style="color: ${A};">${w}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${_};"></i>
                        <span style="color: ${N};">${E}</span>
                    </div>
                </div>`:W='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let q=H()?"easter.company":m(a.domain&&a.port?`${a.domain}:${a.port}`:"");return`<div class="service-widget ${c}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${u}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${q}</span></div>${W}</div></div>`}let b=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),S=new Set(f.map(a=>a.id));for(let[a,d]of b)S.has(a)||d.remove();let C=new Set;f.forEach(a=>{if(C.has(a.id))return;C.add(a.id);let d=o(a),c=b.get(a.id);if(c&&c.parentNode)if(a.id==="upstash-redis-ro"){let u=H()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),x=new Date(u).toLocaleTimeString(),y=c.querySelector(".sync-time-label");y&&(y.textContent=`Last synced: ${x}`)}else c.outerHTML!==d&&(c.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function at(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=U("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),ee(2,Vt);let s=e.models||[],n=e.whisper;Array.from(t.children).forEach(p=>{p.classList.contains("service-widget")||p.remove()});function i(p){let f=p.status==="Ready";return`
                ...
                </div>`}function l(p){let f=p.status==="Ready";return`
                ...
                </div>`}function r(p){let f=p.status==="Downloaded",h=f?"service-widget-online":"service-widget-offline",m=f?"OK":"MISSING",v=f&&p.size>0?`${(p.size/1e9).toFixed(2)} GB`:"-",o=p.name;return p.type==="custom"&&o.endsWith(":latest")&&(o=o.replace(":latest","")),`<div class="service-widget ${h}" data-model-name="${p.name}"><div class="service-widget-header"><i class="bx ${f?"bx-check-circle":"bx-x-circle"}"></i><h3>${o}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${p.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let g="";if(n&&(g+=i(n)),e.xtts&&(g+=l(e.xtts)),g+=s.map(r).join(""),!g){t.innerHTML=U("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let s=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),i=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),p=document.getElementById("analyzer-reset-btn"),f=document.getElementById("fabricator-reset-btn"),h=document.getElementById("courier-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let u=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B(u,{method:"POST"}),await ne()}catch{m.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},m.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{p.innerHTML="<i class='bx bx-error'></i>"}},p.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),h&&!h.dataset.listenerAttached&&(h.onclick=async()=>{h.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{h.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{h.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{h.innerHTML="<i class='bx bx-error'></i>"}},h.dataset.listenerAttached="true");let v=await Gs();if(v&&v.agents){let c=v.agents.guardian||{protocols:{}},u=v.agents.analyzer||{protocols:{}},x=v.agents.imaginator||{protocols:{}},y=v.agents.fabricator||{protocols:{}},w=v.agents.courier||{protocols:{}},E=v.system||{},T=Math.floor(Date.now()/1e3),A={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},_=M=>{M<0&&(M=0);let F=Math.floor(M/3600),D=Math.floor(M%3600/60),J=M%60;return F>0?`${F}h ${D}m`:D>0?`${D}m ${J}s`:`${J}s`},N=(M,F,D,J)=>{if(!M)return;let V=A[J]||J.toUpperCase(),se=M.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(se&&(se.textContent=V),!D){M.textContent="Inactive",M.style.color="#666";return}let G=D.status;if(G==="Working")M.textContent="Working",M.style.color="#bb86fc";else if(G==="Ready")M.textContent="Ready",M.style.color="#5eff5e";else{let le=D.cooldown;if(H()&&(le=D.next_run-T),le<=0)M.textContent="Ready",M.style.color="#5eff5e";else{let We=Math.floor(le/60),Ge=le%60;M.textContent=`${We}m ${Ge}s`,M.style.color="#fff"}}F&&D.stats&&(F.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};s&&N(s,document.getElementById("guardian-sentry-stats"),c.protocols.sentry,"sentry");let R=document.getElementById("analyzer-synthesis-val"),W=document.getElementById("analyzer-synthesis-stats");R&&N(R,W,u.protocols.synthesis,"synthesis");let q=document.getElementById("imaginator-alert_review-val"),K=document.getElementById("imaginator-alert_review-stats");q&&N(q,K,x.protocols.alert_review,"alert_review");let L=document.getElementById("fabricator-construction-val"),$=document.getElementById("fabricator-construction-stats");L&&N(L,$,y.protocols.construction,"construction");let k=document.getElementById("courier-researcher-val"),I=document.getElementById("courier-researcher-stats");k&&N(k,I,w.protocols.researcher,"researcher");let O=document.getElementById("system-state-label"),z=document.getElementById("system-state-val");if(z&&E.state){let M=E.state,F=_(E.state_time||0);O&&(O.textContent=`State: ${M.toUpperCase()}`),z.textContent=F,M==="idle"?z.style.color=E.state_time>300?"#5eff5e":"#fff":z.style.color="#bb86fc",m&&(M==="paused"?(m.innerHTML="<i class='bx bx-play'></i>",m.title="Resume System",m.style.color="#ff9800"):(m.innerHTML="<i class='bx bx-pause'></i>",m.title="Pause System",m.style.color=""))}i&&(i.textContent=_(E.metrics?.total_idle_time||0)),l&&(l.textContent=_(E.metrics?.total_active_time||0)),r&&(r.textContent=_(E.metrics?.total_waste_time||0))}else[s,n,i,l,r].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});if(t)return;let o=await Ws(),b=[],S=[],C=[];o&&(Array.isArray(o)?b=o:(b=o.active||[],S=o.queue||[],C=o.history||[],C.sort((c,u)=>(u.end_time||0)-(c.end_time||0)))),e&&(b.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=U("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),nt(e,b,!1)));let a=document.getElementById("processes-queue-widgets");a&&(S.length===0?!a.querySelector(".tab-placeholder")&&!a.querySelector('div[style*="font-style: italic"]')&&(a.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(a.innerHTML="",nt(a,S,!1)));let d=document.getElementById("processes-history-widgets");d&&(C.length===0?d.querySelector(".tab-placeholder")||(d.innerHTML=U("empty","No recent history.")):(d.querySelector(".tab-placeholder")&&(d.innerHTML=""),nt(d,C,!0))),Ae(1,b.length)}function nt(t,e,s){function n(p){let f="";p.end_time?f=`${p.end_time-p.start_time}s`:f=`${Math.floor(Date.now()/1e3-p.start_time)}s`;let h=p.retries>0?`<span class="process-retry-badge">Retry ${p.retries}</span>`:"",m=p.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(v[m]?m=v[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),s)return`
        <div class="process-history-item" data-channel-id="${p.channel_id}-${p.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
                ...
                <div class="service-widget process-widget" data-channel-id="${p.channel_id}-${p.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${o}">${p.state}</span>
                        </div>
                        <div class="service-widget-info">
                         ...
                         <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${o}">${p.pid}</span>
                        </div>
                    </div>
                </div>`}let i=s?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(i)).map(p=>[p.dataset.channelId,p])),r=new Set(e.map(p=>`${p.channel_id}-${p.start_time}`));for(let[p,f]of l)p&&!r.has(p)&&f.remove();let g=new Set;e.forEach(p=>{let f=`${p.channel_id}-${p.start_time}`;if(g.has(f))return;g.add(f);let h=n(p),m=l.get(f);if(m&&m.parentNode){m.outerHTML!==h&&(m.outerHTML=h);let v=t.querySelector(`[data-channel-id="${f}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",h)})}function Je(){let t=Ke(),e="Notification"in window,s={enabled:e&&Notification.permission==="granted",supported:e};return`
            <div class="theme-selector">
                ${Object.values(Q).map(n=>`
                    <div class="theme-card ${t===n?"active":""}" data-theme="${n}">
                        ...
                        <div class="theme-info">
                            ...
                            <p>${n===Q.DARK?"Simple, clean, dark.":n===Q.LIGHT?"Heavenly, bright, and glowy.":"The original animated style."}</p>
            ...
            </div>`}function ot(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(n=>{n.addEventListener("click",function(){let i=this.dataset.theme;i&&(gt(i),t.setContent(Je()),ot(t))})});let s=document.getElementById("notifications-toggle");s&&"Notification"in window?s.onclick=async n=>{let i=n.target;if(i.checked)try{await Notification.requestPermission()!=="granted"&&(i.checked=!1)}catch{i.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),i.checked=!0)}:s&&(s.disabled=!0)}var as=()=>`
    ...
    `;async function os(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await is(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await is(t)}async function is(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=U("empty","No web history found.");return}let n=`
        ...
        `;s.forEach((g,p)=>{let f=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),h=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';n+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Vs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,n]of Object.entries(t)){let i=cs.filter(l=>l.category===s);i.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let p=document.getElementById("close-terminal-btn");p&&(p.onclick=()=>rs());let f=document.getElementById("terminal-action-btn");f&&(f.onclick=()=>rs())}let s=document.getElementById("cli-terminal-body"),n=document.getElementById("cli-docs-pane");s&&(s.innerHTML="");let i=t.docs||{overview:t.description,details:[],extended_usage:t.usage};n&&(n.innerHTML=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let g=document.getElementById("terminal-action-btn");return g&&(g.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",rt=!0,s}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",rt=!1}function ls(t,e,s="output"){if(!rt||!t)return;let n=document.createElement("div");if(n.className=`terminal-line terminal-${s}`,s==="prompt")n.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let i=Re(e);n.innerHTML=i||e}t.appendChild(n),t.scrollTop=t.scrollHeight}async function Zs(t){let e=cs.find(r=>r.id===t);if(!e)return;let s=Ks(e);s&&ls(s,`dex ${t}`,"prompt");let n=e.demo_output||["Executing command...","\u2713 Done."];for(let r of n){await new Promise(p=>setTimeout(p,400+Math.random()*600));let g="output";r.includes("[ERROR]")?g="error":r.includes("[SUCCESS]")||r.includes("\u2713")?g="success":r.includes("!")&&(g="warning"),s&&ls(s,r,g)}let i=document.getElementById("terminal-status-badge");i&&(i.className="terminal-status status-success",i.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Vs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let s=e.querySelector("code"),n=s?s.textContent:"";n&&navigator.clipboard.writeText(n).then(()=>{let i=e.querySelector("i");i&&(i.className="bx bx-check",i.style.color="#5eff5e",setTimeout(()=>{i.className="bx bx-copy",i.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(s=>{let n=s;s.addEventListener("mouseenter",()=>{n.style.transform="translateY(-5px)",n.style.borderColor="rgba(255,255,255,0.15)",n.style.backgroundColor="rgba(255,255,255,0.05)"}),s.addEventListener("mouseleave",()=>{n.style.transform="translateY(0)",n.style.borderColor="rgba(255,255,255,0.05)",n.style.backgroundColor="rgba(255,255,255,0.03)"}),s.addEventListener("click",()=>{let i=n.dataset.cmd;i&&Zs(i)})})}async function Ys(){if(!H())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Ys(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],s=document.getElementById("windows-container");s&&s.setAttribute("data-count","0");let n=a=>{localStorage.setItem("dex_last_window",a)};function i(){for(;e.length>1;)e.shift().close(!0);let a=document.getElementById("windows-container"),d=document.querySelector("nav"),c=document.querySelector("footer"),u=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",e.length.toString()),e.forEach(_=>{let N=document.getElementById(_.id);N&&N.classList.remove("hide-close")});let y=document.getElementById("dexter-menu-container"),w=document.getElementById("nav-window-switcher"),E=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),A=window.innerWidth<880;if(!A){let _=document.getElementById("dexter-dropdown"),N=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),N&&N.classList.remove("active"),a&&a.classList.remove("menu-open")}if(Ce(e.length>0),e.length>0)if(c?.classList.add("hide"),T&&(T.style.display=A?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),y&&(y.style.display=A?"flex":"none"),E&&(E.style.display=A?"block":"none"),!A&&w){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(w.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{n("alerts-window"),l(g)}),document.getElementById("switch-events")?.addEventListener("click",()=>{n("events-window"),l(p)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{n("monitor-window"),l(h)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{n("contacts-window"),l(f)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{n("workspace-window"),l(m)})):w.innerHTML=""}else w&&(w.innerHTML="");else d?.classList.remove("window-open"),T&&(T.style.display="none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",u||x?c?.classList.remove("hide"):c?.classList.add("hide"),y&&(y.style.display="flex"),E&&(E.style.display="block"),w&&(w.innerHTML="");Te()}function l(a){if(a.isOpen()){a.close();return}for(;e.length>0;)e.pop().close(!0);e.push(a),a.open(),i()}function r(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",i);let g=me({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:$t(),onOpen:()=>de(),onClose:()=>{let a=e.indexOf(g);a>-1&&e.splice(a,1),i()}}),p=me({id:"events-window",title:"Events",icon:"bx-calendar-event",content:st(),onOpen:()=>{p.setContent(st()),pe()},onClose:()=>{let a=e.indexOf(p);a>-1&&e.splice(a,1),i()}}),f=me({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:qt(),onOpen:()=>tt(),onClose:()=>{let a=e.indexOf(f);a>-1&&e.splice(a,1),i()}}),h=me({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(a=>H()?a.title!=="Hardware"&&a.title!=="Logs":!0),onOpen:()=>{it(),ne(),at(),os(),De()},onClose:()=>{let a=e.indexOf(h);a>-1&&e.splice(a,1),i()}}),m=me({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Nt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Pt()},{icon:"bx-task",title:"Tasks",content:Ot()}],onOpen:()=>Qe(),onClose:()=>{let a=e.indexOf(m);a>-1&&e.splice(a,1),i()}}),v=me({id:"settings-window",title:"Settings",icon:"bx-cog",content:Je(),onOpen:()=>{v.setContent(Je()),ot(v)},onClose:()=>{let a=e.indexOf(v);a>-1&&e.splice(a,1),i()}});window.dexter={viewEvent:a=>{p.isOpen()||l(p),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:a=>{g.isOpen()||l(g),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${a}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let o=()=>{let a=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");a&&d&&(a.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let a=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),c=document.getElementById("close-all-windows"),u=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),y=document.getElementById("windows-container");a&&x&&(a.onclick=E=>{E.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let _=a.getBoundingClientRect(),R=document.querySelector("nav").getBoundingClientRect(),W=220,q=10,L=_.left+_.width/2-W/2,$=R.right-10;L+W>$&&(L=$-W),L<10&&(L=10),x.style.top=R.bottom+q+"px",x.style.left=L+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),a.classList.toggle("active");let A=x.classList.contains("active");if(T){let _=document.querySelector("nav");A?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),y?.classList.add("menu-open"),Ce(!0)):(y?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),Ce(!1)))}}),d&&(d.onclick=E=>{E.stopPropagation(),l(v)}),c&&(c.onclick=E=>{E.stopPropagation(),r()});let w=(E,T,A)=>{let _=document.getElementById(E);_&&(_.onclick=N=>{N.stopPropagation(),o(),A&&n(A),l(T)})};w("alerts-menu-item",g,"alerts-window"),w("events-menu-item",p,"events-window"),w("monitor-menu-item",h,"monitor-window"),w("contacts-menu-item",f,"contacts-window"),w("workspace-menu-item",m,"workspace-window"),u&&(u.onclick=E=>{if(E.stopPropagation(),x&&x.classList.contains("active")){o(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1));return}if(e.length>0)r();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let N=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");N.pop();let R=N.join("/")||"/";window.location.href=R}}}),document.addEventListener("click",()=>{let E=window.innerWidth<880;x&&x.classList.contains("active")&&(o(),E&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),Ce(!1)))}),x&&(x.onclick=E=>E.stopPropagation())})();let S=window.location.pathname==="/"||window.location.pathname==="/index.html",C=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!S&&!C&&document.querySelector("footer")?.classList.add("hide"),H()||setInterval(()=>{p.isOpen()&&pe(),g.isOpen()?de():Xe()},1e3),setInterval(()=>{H()&&(g.isOpen()?de():Xe()),m.isOpen()?Qe():It(),H()&&p.isOpen()&&pe(),h.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

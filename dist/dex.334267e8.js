...
`,ze=null,ue=new Set,Ne=[];async function le(t=!1){let e=document.getElementById("alerts-list");if(!e)return;xs(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await B(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];ze=Date.now(),ee(0,ze);let r=Date.now(),g=24*60*60*1e3,c=l.filter(u=>{let f=localStorage.getItem(`alert_read_ts_${u.id}`);if(!f)return!0;let x=parseInt(f);return r-x<g});c.sort((u,f)=>{let x=k=>{let E=k.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},L=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,A=L(x(u)),I=L(x(f));return A!==I?I-A:f.timestamp-u.timestamp}),Ne=c;let p=u=>{let f=u.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return"low"}return(f.priority||"low").toLowerCase()},y=[],v=new Set(c.map(u=>p(u))).size>1;if(c.length>0){let u=null;c.forEach(f=>{let x=p(f);v&&x!==u&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),u=x),y.push(f)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),_e();return}let o=u=>{let f=u.event;if(typeof f=="string")try{f=JSON.parse(f)}catch{return null}let x=(f.title||"Untitled Alert").trim(),L=(f.body||"No description provided.").trim(),A=f.summary||"",I=f.content||"",k=f.protocol||"",E=(f.priority||"low").toLowerCase(),D=!!f.alert,U=(f.category||"system").trim(),O=f.related_event_ids||[],q=f.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${u.id}`),S=new Date(u.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=J?"event-border-grey":"event-border-blue";!J&&D&&(T="event-border-red"),J&&(E==="high"||E==="critical")?T="event-border-red":J&&E==="medium"&&(T="event-border-orange");let M=J?"alert-read":"alert-unread",_=ue.has(u.id),P=_?"expanded":"",V=_?"display: block;":"display: none;",j="",F="";O.length>0&&(F=`
...
`,fe=new Set,Pe=[],Ee=null;async function ge(t=!1){let e=document.getElementById("roadmap-list");if(e){hs();try{let s=await B("/roadmap");if(!s.ok)throw new Error("Offline");let i=await s.json();Pe=i;let n=p=>{let y=fe.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${o} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=p.id,w.onclick=u=>{if(u.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),fe.delete(p.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),fe.add(p.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;w.innerHTML=`
      ...
      `;let d=w.querySelector(".event-details");return d&&d.addEventListener("click",u=>{u.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>ws(p)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Cs(p)),w.querySelector(".delete-btn")?.addEventListener("click",()=>Es(p.id)),w.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),w.classList.remove("expanded");let f=w.querySelector(".event-details");f&&(f.style.display="none"),fe.delete(p.id)}),w},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),g=new Set(i.map(p=>p.id));if(l.forEach(p=>{let y=p.dataset.itemId;(!y||!g.has(y))&&p.remove()}),!Pe||Pe.length===0){e.innerHTML=N("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((p,y)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function hs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ee=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Ee=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let g=Ee?`/roadmap/${Ee}`:"/roadmap",c=Ee?"PATCH":"POST";try{await B(g,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let p=document.getElementById("roadmap-editor-container");p&&(p.style.display="none"),ge(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{Pe.forEach(l=>fe.add(l.id)),ge(!0)},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{fe.clear(),ge(!0)},n.dataset.listenerAttached="true")}function ws(t){Ee=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function Cs(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ge(!0)}catch(s){console.error(s)}}async function Es(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),ge(!0)}catch(e){console.error(e)}}var Et=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ct=null,be=new Set,Tt=[];async function ve(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;$s();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await B(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(Tt=l,Ct=Date.now(),ee(2,Ct),l.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Me(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),$=(v.content||"").trim(),w=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(_=>_.trim()),d=(v.implementation_path||[]).map(_=>_.trim()),u=v.source_event_ids||[],f=v.approved===!0,x=new Date(m.timestamp*1e3),L=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=be.has(m.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${f?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,f&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(_){let P=this;if(P.classList.contains("expanded")){P.classList.remove("expanded"),be.delete(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="none")}else{P.classList.add("expanded"),be.add(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="block")}};let O="";d.length>0&&(O=`
                ...
                `);let G=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",J=H(),S=f?`
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async _=>{_.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ve(!0)}catch(P){console.error("Failed to approve blueprint:",P)}});let h=E.querySelector(".blueprint-delete-btn");h&&(h.onclick=async _=>{_.stopPropagation();let P=!f;h.innerHTML=P?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&ve(!0)}catch(V){console.error("Failed to delete blueprint:",V)}});let T=E.querySelector(".event-details");T&&T.addEventListener("click",_=>{_.stopPropagation()});let M=E.querySelector(".close-details-btn");return M&&M.addEventListener("click",_=>{_.stopPropagation(),E.classList.remove("expanded");let P=E.querySelector(".event-details");P&&(P.style.display="none"),be.delete(m.id)}),E},g=Array.from(e.children),c=new Map(g.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));g.forEach(m=>{let v=m.dataset.blueprintId;(!v||!p.has(v))&&m.remove()});let y=null;l.forEach((m,v)=>{let o=c.get(m.id);if(!o||t){o&&o.remove();let b=r(m);if(!b)return;o=b}v===0?e.firstElementChild!==o&&e.prepend(o):y&&y.nextElementSibling!==o&&y.after(o),y=o}),Me(2,l.length),bt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function St(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&n++}),gt(n)}catch{}}function $s(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Tt.forEach(i=>be.add(i.id)),ve(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{be.clear(),ve(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),be.clear(),ve(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var ae="STANDBY",kt=null,$e=[],Z=null,Re=null,Mt=()=>`
    <div id="progress-view-root" class="progress-container">
        <!-- Initial render will happen here -->
        ${Lt()}
    </div>
  `;function Lt(){switch(ae){case"ACTIVE":return Ss();case"COMPLETED":return ks();case"STANDBY":default:return Ts()}}function Ts(){return`
...
`;async function Ge(){await Promise.all([ge(),ve(),Te()])}var _t=()=>`
    ...
    `;async function Se(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),g=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=g?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),l&&(l.value=""),r&&(r.value=""),Se()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active chores.","Create one to start monitoring.");return}let g=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=g,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this chore?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Se())}})}catch(l){console.error("Failed to fetch chores",l)}}var Ls=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,Oe=null,ye=new Set,Le=[],K="all",Bs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},_s={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Hs(t){for(let[e,s]of Object.entries(Bs))if(s.includes(t))return e;return"system"}function Ds(t){return _s[t]||"bx-square-rounded"}async function ce(t=!1){let e=document.getElementById("events-timeline");if(!e)return;zs();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await B(i);if(!n.ok)throw new Error("Service unreachable");if(Le=((await n.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),Oe=Date.now(),ee(1,Oe),Le.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,$=Hs(b),w=Ds(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let u=a?"cursor-pointer":"",f=ye.has(v.id),x=f?"expanded":"",L=f?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Nt(b,o),D=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",U="";if(a){let S="";if(b==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                    ...
                    `}else if(b==="system.attention.expired"){let C=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,h=o.timestamp-o.last_active,T=Math.floor(h/60);S=`
                    ...
                    `}else if(b==="messaging.bot.sent_message"){let C=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,h="";o.eval_count&&(h=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let T="";if(o.chat_history&&o.chat_history.length>0){let M=o.chat_history.length,_=o.chat_history.map((P,V)=>{let j=P.name?P.name.toUpperCase():P.role.toUpperCase();!P.name&&j==="ASSISTANT"&&(j="DEXTER");let F=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",W=V===M-1?"block":"none";return`
                                <div class="history-slide" data-index="${V}" style="display: ${W};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${V+1} of ${M}</span>
                                    </div>
                        ...
                        `}
                    `}else if(b==="analysis.user.message_signals"){let C=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,h=o.signals||{};S=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(h.topics||[]).map(M=>`<span class="profile-badge">${M}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(b==="system.analysis.audit"){let C=o.success?"#03dac6":"#ff4d4d",h=o.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,M="";o.error&&(M=`
                            <div class="event-detail-block">
                                ${T("Error")}
                    ...
                    `}else if(b==="messaging.user.sent_message"){let C="";o.attachments&&o.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${o.attachments.map(T=>{let M=T.content_type&&T.content_type.startsWith("image/"),_=(T.size/1024).toFixed(1)+" KB";return`
...
`,Pt=Ns;function Rt(){return`
    ...
    `}var Ps=null;async function Ae(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let s=await e.json();if(!s||s.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],n=s.filter(r=>!i.includes(r.id));n.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),n.reverse();let l=n.map(r=>{let g=r.logs.join(`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=a.status==="online",u=d?"service-widget-online":"service-widget-offline",f=d?"bx-check-circle":"bx-x-circle",x=d?"OK":"BAD",L=a.version?y(a.version.str):"-",A=a.cpu&&a.cpu!=="N/A"?a.cpu:"-",I=a.memory&&a.memory!=="N/A"?a.memory:"-",k=A!=="-"?"#00ff00":"#666",E=A!=="-"?"#fff":"#666",D=I!=="-"?"#00ff00":"#666",U=I!=="-"?"#fff":"#666",O=v(a.uptime),q="";d?q=`
                <div class="service-widget-info">
                ...
                </div>`:q='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let G=H()?"easter.company":m(a.domain&&a.port?`${a.domain}:${a.port}`:"");return`<div class="service-widget ${u}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${f}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${G}</span></div>${q}</div></div>`}let b=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),$=new Set(p.map(a=>a.id));for(let[a,d]of b)$.has(a)||d.remove();let w=new Set;p.forEach(a=>{if(w.has(a.id))return;w.add(a.id);let d=o(a),u=b.get(a.id);if(u&&u.parentNode)if(a.id==="upstash-redis-ro"){let f=H()&&(ie.lastFrontend||ie.lastDashboard)||Date.now(),x=new Date(f).toLocaleTimeString(),L=u.querySelector(".sync-time-label");L&&(L.textContent=`Last synced: ${x}`)}else u.outerHTML!==d&&(u.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Zt();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Ut=Date.now(),ee(2,Ut);let s=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function n(c){let p=c.status==="Ready";return`
                ...
                </div>`}function r(c){let p=c.status==="Downloaded",y=p?"service-widget-online":"service-widget-offline",m=p?"OK":"MISSING",v=p&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",o=c.name;return c.type==="custom"&&o.endsWith(":latest")&&(o=o.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${p?"bx-check-circle":"bx-x-circle"}"></i><h3>${o}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let g="";if(i&&(g+=n(i)),e.xtts&&(g+=l(e.xtts)),g+=s.map(r).join(""),!g){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function de(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let s=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),p=document.getElementById("fabricator-reset-btn"),y=document.getElementById("system-pause-toggle-btn");y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{let u=y.querySelector(".bx-play")?"/agent/resume":"/agent/pause";y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B(u,{method:"POST"}),setTimeout(()=>de(),500)}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),de()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),de()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onclick=async()=>{p.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{p.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{p.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),de()}catch{p.innerHTML="<i class='bx bx-error'></i>"}},p.dataset.listenerAttached="true");let m=await Us();if(m&&m.agents){let d=m.agents.guardian||{protocols:{}},u=m.agents.analyzer||{protocols:{}},f=m.agents.imaginator||{protocols:{}},x=m.agents.fabricator||{protocols:{}},L=m.system||{},A=Math.floor(Date.now()/1e3),I={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction"},k=h=>{h<0&&(h=0);let T=Math.floor(h/3600),M=Math.floor(h%3600/60),_=h%60;return T>0?`${T}h ${M}m`:M>0?`${M}m ${_}s`:`${_}s`},E=(h,T,M,_)=>{if(!h)return;let P=I[_]||_.toUpperCase(),V=h.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(V&&(V.textContent=P),!M){h.textContent="Inactive",h.style.color="#666";return}let j=M.status;if(j==="Working")h.textContent="Working",h.style.color="#bb86fc";else if(j==="Ready")h.textContent="Ready",h.style.color="#5eff5e";else{let F=M.cooldown;if(H()&&(F=M.next_run-A),F<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let W=Math.floor(F/60),oe=F%60;h.textContent=`${W}m ${oe}s`,h.style.color="#fff"}}T&&M.stats&&(T.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
              <span>Runs: ${M.stats.runs||0}</span>
              <span style="color: ${M.stats.failures>0?"#ffa500":"#666"}">Fails: ${M.stats.failures||0}</span>
              <span style="color: ${M.stats.aborted>0?"#ff4d4d":"#666"}">Aborted: ${M.stats.aborted||0}</span>
            </div>
          `)};s&&E(s,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry");let D=document.getElementById("analyzer-synthesis-val"),U=document.getElementById("analyzer-synthesis-stats");D&&E(D,U,u.protocols.synthesis,"synthesis");let O=document.getElementById("imaginator-alert_review-val"),q=document.getElementById("imaginator-alert_review-stats");O&&E(O,q,f.protocols.alert_review,"alert_review");let G=document.getElementById("fabricator-construction-val"),J=document.getElementById("fabricator-construction-stats");G&&E(G,J,x.protocols.construction,"construction");let S=document.getElementById("system-state-label"),C=document.getElementById("system-state-val");if(C&&L.state){let h=L.state,T=k(L.state_time||0);S&&(S.textContent=`State: ${h.toUpperCase()}`),C.textContent=T,h==="idle"?C.style.color=L.state_time>300?"#5eff5e":"#fff":C.style.color="#bb86fc",y&&(y.querySelector(".bx-loader-alt")||y.querySelector(".bx-error")||(h==="paused"?(y.innerHTML="<i class='bx bx-play'></i>",y.title="Resume System",y.style.color="#ff9800"):(y.innerHTML="<i class='bx bx-pause'></i>",y.title="Pause System",y.style.color="")))}n&&(n.textContent=k(L.metrics?.total_idle_time||0)),l&&(l.textContent=k(L.metrics?.total_active_time||0)),r&&(r.textContent=k(L.metrics?.total_waste_time||0))}else[s,i,n,l,r].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});if(t)return;let v=await Os(),o=[],b=[],$=[];v&&(Array.isArray(v)?o=v:(o=v.active||[],b=v.queue||[],$=v.history||[],$.sort((d,u)=>(u.end_time||0)-(d.end_time||0)))),e&&(o.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=N("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),Ye(e,o,!1)));let w=document.getElementById("processes-queue-widgets");w&&(b.length===0?!w.querySelector(".tab-placeholder")&&!w.querySelector('div[style*="font-style: italic"]')&&(w.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(w.innerHTML="",Ye(w,b,!1)));let a=document.getElementById("processes-history-widgets");a&&($.length===0?a.querySelector(".tab-placeholder")||(a.innerHTML=N("empty","No recent history.")):(a.querySelector(".tab-placeholder")&&(a.innerHTML=""),Ye(a,$,!0))),Me(1,o.length)}function Ye(t,e,s){function i(c){let p="";c.end_time?p=`${c.end_time-c.start_time}s`:p=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let y=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(v[m]?m=v[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),s)return`
    ...
    `;async function Qt(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Yt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Yt(t)}async function Yt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=N("empty","No web history found.");return}let i=`
        ...
        `;s.forEach((g,c)=>{let p=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`

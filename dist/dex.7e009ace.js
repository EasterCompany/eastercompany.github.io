(()=>{function et(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function tt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,c=document.createElement("nav");c.innerHTML=n,document.body.prepend(c);let r=document.createElement("div");r.id="dexter-dropdown",r.className="dexter-dropdown",r.innerHTML=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var it=1e3;function le(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++it)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",r),s&&setTimeout(s,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++it,e.addEventListener("mousedown",n);let g=t.icon||"bx-window",a="",v="",S;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((o,d)=>{let m=o.icon?`<i class="bx ${o.icon}"></i>`:`<span class="tab-glyph">${o.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,S=`<div class="window-content">${t.tabs.map((o,d)=>`<div class="tab-content ${d===0?"active":""}" data-tab-content="${d}">${o.content}</div>`).join("")}</div>`):(t.title&&(v=`<div class="window-title">${t.title}</div>`),S=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
            ...
            ${S}
        `,p.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",w=>{w.stopPropagation(),l()}),window.addEventListener("resize",r),t.tabs&&e.querySelectorAll(".tab").forEach(w=>{w.addEventListener("click",()=>{let u=w.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(o=>o.classList.remove("active")),w.classList.add("active"),e.querySelectorAll(".tab-content").forEach(o=>o.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${u}"]`).classList.add("active"),b(w,e)})}),setTimeout(()=>{e.classList.add("open"),s&&s()},10)}function r(){if(!e||!e.classList.contains("open"))return;let p=e.querySelector(".tab.active");p&&b(p,e)}function b(p,g){setTimeout(()=>{let a=g.querySelector(".tab-bar");if(!a)return;let v=Array.from(a.querySelectorAll(".tab")),S=v.indexOf(p),w=a.clientWidth,u=v[Math.max(0,S-2)],o=v[Math.min(v.length-1,S+2)],d=u.offsetLeft-a.offsetLeft-25,h=o.offsetLeft+o.offsetWidth-a.offsetLeft+25-d,k=h<=w?d-(w-h)/2:p.offsetLeft-a.offsetLeft-w/2+p.offsetWidth/2;a.scrollTo({left:k,behavior:"smooth"})},350)}function l(p=!1){e&&(window.removeEventListener("resize",r),p?(e.remove(),e=null):(e.classList.remove("open"),i&&i(),setTimeout(()=>{e?.remove(),e=null},400)))}function f(p){t.content=p;let g=e?.querySelector(".window-content");g&&(g.innerHTML=p)}function y(){return e&&e.classList.contains("open")}return{open:c,close:l,setContent:f,isOpen:y,focus:n,id:t.id}}var nt="easter_theme",Y={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function Re(){return localStorage.getItem(nt)||Y.DARK}function at(t){if(!Object.values(Y).includes(t))throw new Error("Invalid theme");localStorage.setItem(nt,t),ot(t)}function ot(t){let e=document.documentElement,i=document.body;Object.values(Y).forEach(r=>{i.classList.remove(`theme-${r}`)}),i.classList.add(`theme-${t}`);let s=document.querySelector('meta[name="theme-color"]');s||(s=document.createElement("meta"),s.name="theme-color",document.getElementsByTagName("head")[0].appendChild(s));let n={[Y.DARK]:"#050507",[Y.LIGHT]:"#FFFFFF",[Y.LEGACY]:"#000000"},c=n[t]||n[Y.DARK];if(s.setAttribute("content",c),!document.querySelector(".background")){let r=document.createElement("div");r.className="background",document.body.prepend(r)}i.classList.add("is-animated")}function rt(){let t=Re();ot(t)}function N(t,e,i=null){let n={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",c=i?`<p class="placeholder-action">${i}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${n} placeholder-icon'></i><p class="placeholder-message">${e}</p>${c}</div>`}function P(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function X(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!i)return;if(!e){i.textContent="Last updated: never";return}let s=Date.now(),n=Math.floor((s-e)/1e3),c;n<60?c=`${n}s ago`:n<3600?c=`${Math.floor(n/60)}m ago`:c=`${Math.floor(n/3600)}h ago`,i.textContent=`Last updated: ${c}`}var Xt=0;function $e(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!i)return;let s=i.querySelector(".notification-badge");s&&(e>0?(s.textContent=e>9?"9+":e,s.style.display="flex"):s.style.display="none")}var ee=0,te=0;function ct(t){ee=t,ye()}function dt(t){te=t,ye()}function ye(){let t=ee+te;Xt=t;let e=document.getElementById("dexter-nav-badge");e&&(e.style.display=t>0?"flex":"none");let i=document.getElementById("alerts-menu-item");if(i){let r=i.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",i.appendChild(r)),r.textContent=ee>9?"9+":ee,r.style.display=ee>0?"flex":"none"}let s=document.getElementById("workspace-menu-item");if(s){let r=s.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="auto",s.appendChild(r)),r.textContent=te>9?"9+":te,r.style.display=te>0?"flex":"none"}let n=document.getElementById("switch-alerts");if(n){let r=n.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",n.appendChild(r)),r.textContent=ee>9?"9+":ee,r.style.display=ee>0?"flex":"none"}let c=document.getElementById("switch-workspace");if(c){let r=c.querySelector(".notification-badge");r||(r=document.createElement("span"),r.className="notification-badge",r.style.marginLeft="8px",c.appendChild(r)),r.textContent=te>9?"9+":te,r.style.display=te>0?"flex":"none"}}function Le(){let t=document.getElementById("alerts-list");if(!t)return;ee=t.querySelectorAll(".alert-unread:not(.priority-low)").length,ye()}function pt(){let t=document.getElementById("blueprints-list");if(!t)return;te=t.querySelectorAll(".event-item:not(.blueprint-approved)").length,ye()}function Qt(){return"https://event.easter.company"}function es(){return"https://discord.easter.company"}var ts="http://127.0.0.1:8100",ss="http://127.0.0.1:8300",is={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function Me(t){let e=P(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(n,c)=>{let r=is[c];return r?`<span class="${r}">`:""});let i=(e.match(/<span/g)||[]).length,s=(e.match(/<\/span/g)||[]).length;return i>s&&(e+="</span>".repeat(i-s)),e}function ce(t){if(!t)return"";let e=P(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^\|(.+)\|$/gm,(i,s)=>{let n=s.split("|").map(c=>c.trim());return n.every(c=>c.match(/^:?-+:?$/))?"":`<div class="md-table-row">${n.map(c=>`<span class="md-table-cell">${c}</span>`).join("")}</div>`}),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var ns="https://sterling-javelin-12539.upstash.io",as="AjD7AAIgcDLTsB2z5ZUJmdu6PPARA5_w2VGIiEdO34oEKjK3VKsuiw";function z(){return window.location.hostname.includes("easter.company")}var H=null,se={lastDashboard:0,lastFrontend:0},Oe=!1,mt=0,ut="dex_dashboard_snapshot";function os(){let t=localStorage.getItem(ut);if(t)try{let e=JSON.parse(t);H=e,se.lastDashboard=e.timestamp?e.timestamp*1e3:0}catch{H=null}}async function lt(){if(!(!z()||Oe)){Oe=!0,mt=Math.floor(Date.now()/1e3);try{let t=await cs("GET","state:dashboard:full");if(t)try{let e=JSON.parse(t);H=e,se.lastDashboard=e.timestamp?e.timestamp*1e3:Date.now(),se.lastFrontend=Date.now(),localStorage.setItem(ut,JSON.stringify(e))}catch(e){console.error("Failed to parse dashboard snapshot:",e)}}finally{Oe=!1}}}function rs(){if(!H||!H.agent_status)return;let t=H.agent_status;t.system&&typeof t.system.state_time=="number"&&(t.system.state_time+=1)}async function ls(){if(!z())return;os();let t=Math.floor(Date.now()/1e3),e=H?t-H.timestamp:1/0;(!H||e>120)&&await lt(),setInterval(()=>{let i=new Date,s=Math.floor(Date.now()/1e3),n=H?s-H.timestamp:1/0,c=s-mt,r=i.getSeconds()===59,b=n>300,l=c>60;(r&&l||b&&l)&&lt(),rs()},1e3)}z()&&ls();async function cs(t,...e){try{let s=await(await fetch(ns,{method:"POST",headers:{Authorization:`Bearer ${as}`},body:JSON.stringify([t,...e])})).json();if(s.error)throw new Error(s.error);return s.result}catch(i){return console.error("Upstash Error:",i),null}}var ve=null,xe=null,ke=!1,Te=!1;async function I(t,e={}){if(z()){if(!H)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t.startsWith("/system_monitor")||t.startsWith("/system/status"))return new Response(JSON.stringify(H.monitor),{status:200});if(t.startsWith("/processes"))return new Response(JSON.stringify(H.processes),{status:200});if(t.startsWith("/events")){let n=new URLSearchParams(t.split("?")[1]||""),c=n.get("type")||n.get("event.type"),r=n.get("category"),b=n.get("order")||"desc",l=[];return c==="system.notification.generated"?l=[...H.alerts||[]]:c==="system.blueprint.generated"||t.includes("event.type=system.blueprint.generated")?l=[...H.blueprints||[]]:r==="messaging"?l=[...H.messaging_events||[]]:r==="system"?l=[...H.system_events||[]]:r==="cognitive"?l=[...H.cognitive_events||[]]:r==="moderation"?l=[...H.moderation_events||[]]:l=[...H.events||[]],b==="asc"?l.sort((f,y)=>f.timestamp-y.timestamp):l.sort((f,y)=>y.timestamp-f.timestamp),new Response(JSON.stringify({events:l,count:l.length}),{status:200})}if(t.startsWith("/logs"))return new Response(JSON.stringify([]),{status:200});if(t.startsWith("/system/hardware"))return new Response(JSON.stringify({}),{status:200});if(t.startsWith("/agent/status")||t.startsWith("/guardian/status"))return new Response(JSON.stringify(H.agent_status||{}),{status:200});if(t.startsWith("/profile/")){let n=t.split("/")[2],c=H.profiles?H.profiles[n]:null;return c?new Response(JSON.stringify(c),{status:200}):new Response(JSON.stringify({error:"Profile not found"}),{status:404})}return t.startsWith("/web/history")?new Response(JSON.stringify(H.web_history||[]),{status:200}):t.startsWith("/chores")?new Response(JSON.stringify([]),{status:200}):new Response(JSON.stringify({error:"Not available in public demo"}),{status:404})}if(ve)try{let n=await fetch(ve+t,e);if(n.ok)return n;ve=null}catch{ve=null}let i=Qt(),s=ts;try{let n=await fetch(i+t,e);if(n.ok)return ve=i,ke&&(console.log("\u2728 Production event service restored."),ke=!1),n;throw new Error("Primary failed")}catch{ke||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${s}`),ke=!0);try{let c=await fetch(s+t,e);if(c.ok)return ve=s,c;throw new Error("Fallback failed")}catch(c){throw c}}}async function Ae(t,e={}){if(z()){if(!H)return new Response(JSON.stringify({error:"Initializing dashboard cache..."}),{status:503});if(t==="/contacts")return new Response(JSON.stringify(H.contacts||{members:[]}),{status:200});if(t.startsWith("/member/")){let n=t.split("/")[2],c=(H.contacts?.members||[]).find(r=>r.id===n);return c?new Response(JSON.stringify(c),{status:200}):new Response(JSON.stringify({error:"Member not found"}),{status:404})}return new Response(JSON.stringify({error:"Discord API not public"}),{status:404})}if(xe)try{let n=await fetch(xe+t,e);if(n.ok)return n;xe=null}catch{xe=null}let i=es(),s=ss;try{let n=await fetch(i+t,e);if(n.ok)return xe=i,Te&&(console.log("\u2728 Production discord service restored."),Te=!1),n;throw new Error("Primary failed")}catch{Te||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${s}`),Te=!0);try{let c=await fetch(s+t,e);if(c.ok)return xe=s,c;throw new Error("Fallback failed")}catch(c){throw c}}}var ft=()=>`
...
`,Ie=null,de=new Set,Be=[];async function ae(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ds(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await I(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ie=Date.now(),X(0,Ie);let r=Date.now(),b=24*60*60*1e3,l=c.filter(d=>{let m=localStorage.getItem(`alert_read_ts_${d.id}`);if(!m)return!0;let h=parseInt(m);return r-h<b});l.sort((d,m)=>{let h=T=>{let $=T.event;if(typeof $=="string")try{$=JSON.parse($)}catch{return"low"}return($.priority||"low").toLowerCase()},k=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,B=k(h(d)),M=k(h(m));return B!==M?M-B:m.timestamp-d.timestamp}),Be=l;let f=d=>{let m=d.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},y=[],g=new Set(l.map(d=>f(d))).size>1;if(l.length>0){let d=null;l.forEach(m=>{let h=f(m);g&&h!==d&&(y.push({id:`divider-${h}`,type:"divider",label:h.toUpperCase()}),d=h),y.push(m)})}if(t&&(e.innerHTML=""),l.length===0){e.innerHTML=N("empty","No alerts yet."),Le();return}let a=d=>{let m=d.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let h=(m.title||"Untitled Alert").trim(),k=(m.body||"No description provided.").trim(),B=m.summary||"",M=m.content||"",T=m.protocol||"",$=(m.priority||"low").toLowerCase(),_=!!m.alert,O=(m.category||"system").trim(),R=m.related_event_ids||[],W=m.audit_event_id,F=!!localStorage.getItem(`alert_read_ts_${d.id}`),E=new Date(d.timestamp*1e3),C=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),x=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=F?"event-border-grey":"event-border-blue";!F&&_&&(L="event-border-red"),F&&($==="high"||$==="critical")?L="event-border-red":F&&$==="medium"&&(L="event-border-orange");let A=F?"alert-read":"alert-unread",D=de.has(d.id),U=D?"expanded":"",q=D?"display: block;":"display: none;",J="",j="";R.length>0&&(j=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${R.map(ne=>`<a href="#" onclick="window.dexter.viewEvent('${ne}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ne.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let V="";W&&(V=`
            <div style="flex: 1; min-width: 120px; text-align: center;">
                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Audit</div>
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    <a href="#" onclick="window.dexter.viewEvent('${W}'); return false;" style="color: #bb86fc; text-decoration: none; border-bottom: 1px dashed rgba(187, 134, 252, 0.3); white-space: nowrap;">${W.substring(0,8)}...</a>
                </div>
            </div>`);let ie="";T&&(ie=`
            <div style="flex: 1; min-width: 100px; text-align: center;">
                ...
                <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #bb86fc; font-weight: bold;">${T}</div>
            </div>`);let Q="";M?Q=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ce(M)}</div>
            </div>
        `:Q=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ce(k)}</div>
            </div>
        `,J=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${m.related_services&&m.related_services.length>0?m.related_services.join(", "):m.related||"SYSTEM"}</div>
                    </div>
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${L} ${A} ${U} cursor-pointer priority-${$}`,Z.dataset.alertId=d.id,Z.onclick=function(ne){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(d.id);let Ce=this.querySelector(".event-details");Ce&&(Ce.style.display="none")}else{this.classList.add("expanded"),de.add(d.id);let Ce=this.querySelector(".event-details");if(Ce&&(Ce.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Pe="event-border-grey";$==="high"||$==="critical"?Pe="event-border-red":$==="medium"&&(Pe="event-border-orange"),this.classList.add(Pe),Le()}}};let Ye=`${T?T.toUpperCase():"GUARDIAN"} ALERT: ${B||h}`,Zt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[O]||"bx-bell",Yt=$==="high"||$==="critical"?"#ff4d4d":$==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Yt}; font-size: 0.8em; margin-left: 5px;">[${$.toUpperCase()}]</span>
                    </div>
                    <div class="event-message">${ce(Ye)}</div>
                    <div class="event-details" style="${q}">
                        ${J}
                    </div>
                </div>
            `;let Xe=Z.querySelector(".event-details");Xe&&Xe.addEventListener("click",ne=>{ne.stopPropagation()});let Qe=Z.querySelector(".close-details-btn");return Qe&&Qe.addEventListener("click",ne=>{ne.stopPropagation(),Z.classList.remove("expanded");let He=Z.querySelector(".event-details");He&&(He.style.display="none"),de.delete(d.id)}),Z},v=d=>{let m=document.createElement("div");m.className="notification-divider",m.dataset.alertId=d.id;let h="#888888";return d.label==="CRITICAL"?h="#ff4d4d":d.label==="HIGH"?h="#ff8888":d.label==="MEDIUM"&&(h="#ffa500"),m.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${h}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,m.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${h}44, transparent); flex-grow: 1;"></div>`,m},S=Array.from(e.children),w=new Map(S.map(d=>[d.dataset.alertId,d])),u=new Set(y.map(d=>d.id));S.forEach(d=>{let m=d.dataset.alertId;(!m||!u.has(m))&&d.remove()});let o=null;y.forEach((d,m)=>{let h=w.get(d.id);(!h||t)&&(h&&h.remove(),d.type==="divider"?h=v(d):h=a(d),!h)||(m===0?e.firstElementChild!==h&&e.prepend(h):o&&o.nextElementSibling!==h&&o.after(h),o=h)}),Ie=Date.now(),X(0,Ie),Le()}catch(s){console.error("Error fetching alerts:",s),e.children.length===0&&(e.innerHTML=N("offline","Failed to load alerts.","The event service may be offline."))}}function ds(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),s=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Be.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ae(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Be.forEach(n=>{de.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ae(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{de.clear(),ae(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await I("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Be.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,n.toString())}),de.clear(),ae(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}async function Ue(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await I(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||n++}),ct(n)}catch{}}var gt=()=>`
...
`,pe=new Set,_e=[],he=null;async function me(t=!1){let e=document.getElementById("roadmap-list");if(e){ps();try{let i=await I("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();_e=s;let n=f=>{let y=pe.has(f.id),p=f.state==="published",g=f.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),g&&(a="event-border-purple");let S=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=f.id,w.onclick=d=>{if(d.target.closest("button"))return;w.classList.contains("expanded")?(w.classList.remove("expanded"),w.querySelector(".event-details").style.display="none",pe.delete(f.id)):(w.classList.add("expanded"),w.querySelector(".event-details").style.display="block",pe.add(f.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;w.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${P(f.content)}</div>
          <div class="event-details" style="${y?"display: block;":"display: none;"} ">
            ...
            `}
            ${g?`<div style="margin-top: 15px; font-size: 0.8em; color: #888;">Consumed at: ${new Date(f.consumed_at*1e3).toLocaleString()}</div>`:""}
          </div>
        </div>
      `;let o=w.querySelector(".event-details");return o&&o.addEventListener("click",d=>{d.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>ms(f)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>us(f)),w.querySelector(".delete-btn")?.addEventListener("click",()=>fs(f.id)),w.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),w.classList.remove("expanded"),w.querySelector(".event-details").style.display="none",pe.delete(f.id)}),w},c=Array.from(e.children),r=new Map(c.map(f=>[f.dataset.itemId,f])),b=new Set(s.map(f=>f.id));if(c.forEach(f=>{let y=f.dataset.itemId;(!y||!b.has(y))&&f.remove()}),!_e||_e.length===0){e.innerHTML=N("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let l=null;s.forEach((f,y)=>{let p=r.get(f.id);(!p||t)&&(p&&p.remove(),p=n(f),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):l&&l.nextElementSibling!==p&&l.after(p),l=p)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function ps(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{he=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",he=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let r=he?`/roadmap/${he}`:"/roadmap",b=he?"PATCH":"POST";try{await I(r,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",me(!0)}catch(l){console.error(l)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{_e.forEach(c=>pe.add(c.id)),me(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{pe.clear(),me(!0)},n.dataset.listenerAttached="true")}function ms(t){he=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function us(t){let e=t.state==="published"?"draft":"published";try{await I(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),me(!0)}catch(i){console.error(i)}}async function fs(t){if(confirm("Delete this roadmap item?"))try{await I(`/roadmap/${t}`,{method:"DELETE"}),pe.delete(t),me(!0)}catch(e){console.error(e)}}var xt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,vt=null,ue=new Set,ht=[];async function fe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;gs();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await I(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(ht=c,vt=Date.now(),X(2,vt),c.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),$e(2,0);return}t&&(e.innerHTML="");let r=p=>{let g=p.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let a=(g.title||"Untitled Blueprint").trim(),v=(g.summary||g.body||"No summary provided.").trim(),S=(g.content||"").trim(),w=(g.category||"architecture").trim(),u=(g.related_services||g.affected_services||[]).map(D=>D.trim()),o=(g.implementation_path||[]).map(D=>D.trim()),d=g.source_event_ids||[],m=g.approved===!0,h=new Date(p.timestamp*1e3),k=h.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),B=h.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=ue.has(p.id),T=M?"display: block;":"display: none;",$=document.createElement("div");$.className=`event-item notification-item event-border-purple cursor-pointer ${M?"expanded":""} ${m?"blueprint-approved":""}`,$.dataset.blueprintId=p.id,m&&($.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",$.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let O=m?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";$.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),ue.delete(p.id);let q=this.querySelector(".event-details");q&&(q.style.display="none")}else{this.classList.add("expanded"),ue.add(p.id);let q=this.querySelector(".event-details");q&&(q.style.display="block")}};let R="";o.length>0&&(R=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${o.map(D=>`<li style="margin-bottom: 5px;">${P(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let W="";d.length>0&&(W=`
                ...
                `);let G=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",F=z(),E=m?`
            ...
            `;$.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${T}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${P(v)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${P(S)}</div>
                        </div>
                        ${R}
                        ${W}
                        ${E}
            ...
            `;let C=$.querySelector(".blueprint-approve-btn");C&&(C.onclick=async D=>{D.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await I(`/events/${p.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&fe(!0)}catch(U){console.error("Failed to approve blueprint:",U)}});let x=$.querySelector(".blueprint-delete-btn");x&&(x.onclick=async D=>{D.stopPropagation();let U=!m;x.innerHTML=U?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await I(`/events/${p.id}`,{method:"DELETE"})).ok&&fe(!0)}catch(q){console.error("Failed to delete blueprint:",q)}});let L=$.querySelector(".event-details");L&&L.addEventListener("click",D=>{D.stopPropagation()});let A=$.querySelector(".close-details-btn");return A&&A.addEventListener("click",D=>{D.stopPropagation(),$.classList.remove("expanded");let U=$.querySelector(".event-details");U&&(U.style.display="none"),ue.delete(p.id)}),$},b=Array.from(e.children),l=new Map(b.map(p=>[p.dataset.blueprintId,p])),f=new Set(c.map(p=>p.id));b.forEach(p=>{let g=p.dataset.blueprintId;(!g||!f.has(g))&&p.remove()});let y=null;c.forEach((p,g)=>{let a=l.get(p.id);(!a||t)&&(a&&a.remove(),a=r(p),!a)||(g===0?e.firstElementChild!==a&&e.prepend(a):y&&y.nextElementSibling!==a&&y.after(a),y=a)}),$e(2,c.length),pt()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function wt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await I(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&n++}),dt(n)}catch{}}function gs(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ht.forEach(s=>ue.add(s.id)),fe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ue.clear(),fe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await I("/events?type=system.blueprint.generated",{method:"DELETE"}),ue.clear(),fe(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var Ct=()=>`
...
`;async function qe(){await Promise.all([me(),fe()])}var St=()=>`
    ...
    `;async function we(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,b=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:b,natural_instruction:c,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",we()}catch(l){console.error(l),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let c=await I("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);let r=await c.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active chores.","Create one to start monitoring.");return}let b=r.map(l=>{let f=l.last_run===0?"Never":new Date(l.last_run*1e3).toLocaleString(),y=l.memory?l.memory.length:0,p=l.status==="active"?"#03dac6":"#666";return`
                <div class="service-widget" style="border-left: 3px solid ${p}; width: 100%;">
                    <div class="service-widget-header">
                        <i class='bx bx-search-alt' style="color: ${p}"></i>
                        <h3 style="font-size: 0.95em; white-space: normal; line-height: 1.4;">${l.natural_instruction}</h3>
                        <div style="margin-left: auto; display: flex; gap: 10px;">
                            <button class="icon-btn delete-chore-btn" data-id="${l.id}" style="background: none; border: none; color: #cf6679; cursor: pointer; padding: 5px;"><i class='bx bx-trash'></i></button>
                        </div>
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">Schedule:</span>
                            <span class="info-value">${l.schedule}</span>
                        </div>
                        <div class="service-widget-info">
                        ...
                        <div class="service-widget-info" style="grid-column: span 2; margin-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 5px;">
                            ...
                            <span class="info-value" style="font-size: 0.8em; opacity: 0.8; word-break: break-all;">${l.execution_plan.entry_url||"Auto-detected"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=b,t.querySelectorAll(".delete-chore-btn").forEach(l=>{l.onclick=async f=>{f.stopPropagation(),confirm("Delete this chore?")&&(await I(`/chores/${l.dataset.id}`,{method:"DELETE"}),we())}})}catch(c){console.error("Failed to fetch chores",c)}}var bs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Et(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function kt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=bs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ae(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();je(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=Et(t.id,t.username);je(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=Et(t.id,t.username);je(e,t,s)})}function je(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let S=(i.badges||[]).map(k=>`<span class="profile-badge ${k==="Creator"?"master":""}">${k}</span>`).join(""),w=(i.attributes||i.facts||[]).map(k=>{let B=k.key||k.k,M=k.value||k.v;return`
            <div class="fact-chip">
                <span class="fact-key">${B}:</span>
                <span class="fact-val">${M}</span>
            </div>
        `}).join(""),u=i.cognitive_model||i,o=u.technical_level||u.techLevel||0,d=u.communication_style||u.commStyle||"Unknown",m=u.patience_level||u.patience||"Unknown",h=u.vibe||"Unknown";return`
        ...
        `},c=()=>{let S=i.dossier||{},w=S.identity||{},u=S.career||{},o=S.personal||{},d=S.social||[];return`
            <div class="dossier-grid">
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div class="dossier-value">${u.jobTitle||"Unknown"}</div>
                        <div style="font-size: 0.8em; color: #888; margin-top: 2px;">@ ${u.company||"Unknown"}</div>
                        <div style="height: 15px;"></div>
                         <div class="dossier-label">Key Skills</div>
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(u.skills||[]).map(m=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${m}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.hobbies||[]).map(m=>`<div class="dossier-list-item">${m}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.habits||[]).map(m=>`<div class="dossier-list-item">${m}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.vices||[]).map(m=>`<div class="dossier-list-item" style="color: #cf6679;">${m}</div>`).join("")}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="dossier-item" style="background: none; border: none; padding: 0;">
                        ${d.length>0?d.map(m=>`
                            <div class="friend-chip">
                                <div style="width: 35px; height: 35px; background: #333; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #888;">${m.name.substring(0,1)}</div>
                                <div style="flex: 1;">
                                    <div style="color: #fff; font-weight: 500;">${m.name}</div>
                                    <div style="font-size: 0.75em; color: #888;">${m.relation}</div>
                                </div>
                                <div style="font-size: 0.8em; color: #03dac6;">${m.trust}</div>
                            </div>
                        `).join(""):'<div style="color: #666; font-style: italic;">No associates mapped.</div>'}
                    </div>
                </div>
            </div>
        `},r=()=>{let S=i.traits||{};return`
            ...
            ${Object.entries(S).map(([w,u])=>`
                <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                    <div style="width: 10px; height: 150px; background: rgba(255,255,255,0.05); border-radius: 5px; position: relative; overflow: hidden;">
                        <div style="position: absolute; bottom: 0; left: 0; width: 100%; height: ${u}%; background: ${u>50?"#03dac6":"#cf6679"}; transition: height 1s;"></div>
                    </div>
                    <div style="font-size: 0.8em; text-transform: uppercase; letter-spacing: 1px; color: #aaa;">${w.substring(0,4)}</div>
                    <div style="font-family: monospace;">${u}%</div>
                </div>
            `).join("")}
        </div>
        ...
        <div class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="profile-identity">
                    <h2>${P(e.username)}</h2>
                    <div class="profile-badges">${(i.badges||[]).map(S=>`<span class="profile-badge ${S==="Creator"?"master":""}">${S}</span>`).join("")}</div>
                </div>
            </div>
    ...
    `;t.innerHTML=f;let y=t.querySelector(".profile-card"),p=t.querySelector("#profile-expand-toggle"),g=t.querySelectorAll(".profile-tab-btn"),a=t.querySelectorAll(".tab-content");t.querySelector(".close-profile-btn").addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),g.forEach(S=>{S.addEventListener("click",()=>{g.forEach(w=>w.classList.remove("active")),a.forEach(w=>w.classList.remove("active")),S.classList.add("active"),t.querySelector(`#tab-${S.dataset.tab}`).classList.add("active")})}),p.addEventListener("click",()=>{y.classList.toggle("expanded");let S=y.classList.contains("expanded");p.innerHTML=S?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Tt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,vs=null;async function Fe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Fe(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);kt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Ae("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(vs=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,b)=>{let l=c[r.level]??10,f=c[b.level]??10;return l!==f?l-f:r.username.localeCompare(b.username)}),t.innerHTML=n.map(r=>{let b=r.color&&r.color!==0,l=b?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",f=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let p=r.level==="Me",g=p?"rgba(187, 134, 252, 0.08)":b?`${l}11`:"rgba(255,255,255,0.03)",a=p?"2px solid #bb86fc":b?`1px solid ${l}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(r).replace(/"/g,"&quot;")}"
                     style="background: ${g}; padding: 15px; border-radius: 8px; border: ${a}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s, box-shadow 0.2s;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${p?"#bb86fc":b?l:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":b?`2px solid ${l}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
...
`,De=null,ge=new Set,Se=[],K="all",ys={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},hs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ws(t){for(let[e,i]of Object.entries(ys))if(i.includes(t))return e;return"system"}function Cs(t){return hs[t]||"bx-square-rounded"}async function oe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;$s();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await I(s);if(!n.ok)throw new Error("Service unreachable");if(Se=((await n.json()).events||[]).filter(g=>{let a=g.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let v=a.type;return!(v==="system.blueprint.generated"||v==="system.notification.generated")}),De=Date.now(),X(1,De),Se.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=g=>{let a=g.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let v=a.type,S=ws(v),w=Cs(v),u=v==="engagement.decision"||v==="messaging.bot.sent_message"||v==="messaging.user.sent_message"||v==="moderation.explicit_content.deleted"||v==="analysis.link.completed"||v==="analysis.visual.completed"||v==="analysis.router.decision"||v==="analysis.user.message_signals"||v==="system.cli.command"||v==="system.analysis.audit"||v==="system.test.completed"||v==="error_occurred"||v==="system.cli.status"||v==="system.attention.expired"||v.startsWith("system.roadmap")||v.startsWith("system.process"),o="event-border-grey";u&&(v==="moderation.explicit_content.deleted"||v==="error_occurred"?o="event-border-red":v==="analysis.link.completed"||v==="analysis.visual.completed"||v==="analysis.router.decision"||v==="system.analysis.audit"||v==="analysis.user.message_signals"?o="event-border-purple":v==="system.attention.expired"||v==="system.cli.command"||v==="system.cli.status"?o="event-border-orange":v==="system.test.completed"?o=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":o="event-border-blue");let d=u?"cursor-pointer":"",m=ge.has(g.id),h=m?"expanded":"",k=m?"display: block;":"display: none;",B=new Date(g.timestamp*1e3),M=B.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=B.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Lt(v,a),_=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",O="";if(u){let E="";if(v==="engagement.decision"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                    ...
                    `}else if(v==="system.attention.expired"){let C=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,x=a.timestamp-a.last_active,L=Math.floor(x/60);E=`
                        ...
                        <div class="event-detail-block">
                            ...
                            <div class="detail-pre">${ce(a.last_input||"None")}</div>
                        </div>
                        <div class="event-detail-block">
                            ${C("Thought Process (Last Output)")}
                            <div class="detail-pre">${ce(a.last_output||"None")}</div>
                        </div>
                    `}else if(v==="messaging.bot.sent_message"){let C=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,x="";a.eval_count&&(x=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let L="";if(a.chat_history&&a.chat_history.length>0){let A=a.chat_history.length,D=a.chat_history.map((U,q)=>{let J=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&J==="ASSISTANT"&&(J="DEXTER");let j=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc",V=q===A-1?"block":"none";return`
                                <div class="history-slide" data-index="${q}" style="display: ${V};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${j}; letter-spacing: 1px; font-weight: bold;">${J}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${q+1} of ${A}</span>
                                    </div>
                        ...
                        `}
                    `}else if(v==="analysis.user.message_signals"){let C=A=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${A}</h5>`,x=a.signals||{};E=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Sentiment</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${x.sentiment>.3?"#03dac6":x.sentiment<-.3?"#cf6679":"#aaa"}; font-weight: bold;">${x.sentiment?.toFixed(2)||0}</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Intent</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; text-transform: uppercase;">${x.intent||"N/A"}</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Tech Depth</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${x.technical_depth||0}/10</div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Mood</div>
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #bb86fc;">${x.mood||"N/A"}</div>
                            </div>
                        </div>
                        <div class="event-detail-block">
                            ${C("Extracted Topics")}
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${(x.topics||[]).map(A=>`<span class="profile-badge">${A}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${C("Raw Model Output")}
                            <pre class="detail-pre">${P(a.raw_output)||"None"}</pre>
                        </div>
                    `}else if(v==="moderation.explicit_content.deleted"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(v==="analysis.link.completed"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${P(a.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${C("Description")}
                            <pre class="detail-pre">${P(a.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${C("Content Summary")}
                            <pre class="detail-pre">${P(a.summary)||"None"}</pre>
                        </div>
                    `}else if(v==="analysis.visual.completed"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,x="";a.base64_preview?x=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(x=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(v==="analysis.router.decision"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${C("Input Context")}
                            <pre class="detail-pre">${P(a.raw_input)||"None"}</pre>
                        </div>
                    `}else if(v==="system.cli.command"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row">
                        ...
                        <div class="event-detail-block">
                            ${C("Output")}
                            <pre class="detail-pre">${P(a.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(v==="system.analysis.audit"){let C=a.success?"#03dac6":"#ff4d4d",x=a.success?"SUCCESS":"FAILED",L=J=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${J}</h5>`,A="";a.error&&(A=`
                            <div class="event-detail-block">
                                ${L("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(a.error)}</pre>
                            </div>
                        `);let D="";if(a.chat_history&&a.chat_history.length>0){let J=a.chat_history.length,j=a.chat_history.map((V,ie)=>{let Q=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Q==="USER"&&(Q="SYSTEM"),!V.name&&Q==="ASSISTANT"&&(Q="AGENT");let Z=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${ie}" style="display: ${ie===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${ie+1} of ${J}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(V.content)}</div>
                                </div>
                            `}).join("");D=`
                            <div class="event-detail-block">
                        ...
                        `}let U="";if(a.corrections&&a.corrections.length>0){let J=a.corrections.map((j,V)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${j.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${P(j.snippet)}</div>`:""}
                            </div>
                        `).join("");U=`
                            <div class="event-detail-block">
                                ${L(`Corrections (${a.corrections.length})`)}
                                ${J}
                            </div>
                        `}let q="";if(a.parsed_results&&a.parsed_results.length>0){let J=a.parsed_results.map(j=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${P(j.title)}</div>
                                <div style="color: #ddd;">${P(j.summary)}</div>
                            </div>
                        `).join("");q=`
                            <div class="event-detail-block">
                                ${L("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${C}; font-weight: bold;">${x} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${a.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${U}
                        ${D}
                    `}else if(v==="system.test.completed"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row">
                    ...
                    `}else if(v==="error_occurred"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                        ...
                        <div class="event-detail-block">
                            ${C("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${P(a.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${C("Context")}
                            <pre class="detail-pre">${JSON.stringify(a.context||{},null,2)}</pre>
                        </div>
                    `}else if(v==="system.cli.status"){let C=x=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${x}</h5>`;E=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                        ...
                        <div class="event-detail-block">
                            ${C("Message")}
                            <pre class="detail-pre">${P(a.message)}</pre>
                        </div>
                    `}else if(v==="messaging.user.sent_message"){let C="";a.attachments&&a.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${a.attachments.map(L=>{let A=L.content_type&&L.content_type.startsWith("image/"),D=(L.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${A?`<div class="attachment-thumb-container"><img src="${L.url}" alt="${L.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                ...
                `}let R=document.createElement("div");R.className=`event-item ${o} ${d} ${h}`,R.dataset.eventId=g.id,R.onclick=function(E){if(!u)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),ge.delete(g.id);let x=this.querySelector(".event-details");x&&(x.style.display="none")}else{this.classList.add("expanded"),ge.add(g.id);let x=this.querySelector(".event-details");x&&(x.style.display="block")}},R.innerHTML=`
                <div class="event-time">
            ...
            `;let W=R.querySelector(".event-details");if(W&&W.addEventListener("click",E=>{E.stopPropagation()}),u){let E=R.querySelector(".close-details-btn");E&&E.addEventListener("click",C=>{C.stopPropagation();let x=C.target.closest(".event-item");if(x){x.classList.remove("expanded"),ge.delete(g.id);let L=x.querySelector(".event-details");L&&(L.style.display="none")}})}let G=R.querySelector(".prev-btn"),F=R.querySelector(".next-btn");if(G&&F){let E=0,C=R.querySelectorAll(".history-slide"),x=C.length,L=()=>{C.forEach((A,D)=>{A.style.display=D===E?"block":"none"}),G.disabled=E===0,F.disabled=E===x-1,G.style.opacity=E===0?"0.5":"1",F.style.opacity=E===x-1?"0.5":"1"};G.addEventListener("click",A=>{A.stopPropagation(),E>0&&(E--,L())}),F.addEventListener("click",A=>{A.stopPropagation(),E<x-1&&(E++,L())}),L()}return R},l=Array.from(e.children),f=new Map(l.map(g=>[g.dataset.eventId,g])),y=new Set(Se.map(g=>g.id));l.forEach(g=>{let a=g.dataset.eventId;(!a||!y.has(a))&&g.remove()});let p=null;Se.forEach((g,a)=>{let v=f.get(g.id);(!v||t)&&(v&&v.remove(),v=b(g),!v)||(a===0?e.firstElementChild!==v&&e.prepend(v):p&&p.nextElementSibling!==v&&p.after(v),p=v)}),De=Date.now(),X(1,De)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=N("offline","Failed to load events.","The event service may be offline."))}}function $s(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Se.forEach(n=>ge.add(n.id)),oe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ge.clear(),oe(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active")),n.classList.add("active"),K=n.dataset.filter,oe(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===K?n.classList.add("active"):n.classList.remove("active")});let s=document.getElementById("events-clear");s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let n=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let c="/events";if(K!=="all"?c+=`?category=${K}`:c+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await I(c,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ge.clear(),oe(!0)}catch(c){console.error("Failed to clear events:",c),alert("Failed to clear events. Check console.")}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var Ss=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Mt=Ss;function At(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Es=null;async function Ee(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await I("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let s=["local-ollama-0","local-cache-0"],n=i.filter(r=>!s.includes(r.id));n.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),n.reverse();let c=n.map(r=>{let b=r.logs.join(`
`),l=[...r.logs];if(l.length<25){let y=25-l.length;for(let p=0;p<y;p++)l.push("")}else l.length>25&&(l=l.slice(-25));let f=l.map(y=>Me(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let b=unescape(r.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let l=r.querySelector("i");l.classList.remove("bx-copy"),l.classList.add("bx-check"),setTimeout(()=>{l.classList.remove("bx-check"),l.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let b=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await I(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&Ee()}catch(l){console.error("Error clearing logs:",l)}})}),Es=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Dt=()=>{let t=z()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="processes-history-widgets" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 30px; opacity: 0.8;"></div>`,Nt=()=>`
        <div class="system-section-header">
        ...
        <div id="models-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Pt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${At()}
        </div>`;async function Ot(){ks(),await Promise.all([re(),Ge(),Ve(),we()]),await Ee()}var ze=null;function ks(){ze||(ze=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(ze),ze=null;return}re(!0)},1e3))}var It=null,Bt=null;async function Ut(){try{return await(await I("/system_monitor")).json()}catch{return null}}async function _t(){try{return await(await I("/system/hardware")).json()}catch{return null}}async function Ts(){try{return await(await I("/processes")).json()}catch{return null}}async function Ls(){try{return await(await I("/agent/status")).json()}catch{return null}}async function Ge(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),b=o=>{if(o){if(i&&(i.innerHTML=`
                ...
                </div>`),n){let d=(o.MEMORY_BYTES/1073741824).toFixed(1);n.innerHTML=`
                ...
                </div>`}if(s&&o.CPU&&o.CPU.length>0){let d=o.CPU[0];s.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(o.GPU&&o.GPU.length>0?c.innerHTML=o.GPU.map(d=>{let m=(d.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${d.LABEL}</span>
                            <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${m} <span style="font-size: 0.6em; color: #888; font-weight: normal;">GB VRAM</span></span>
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):c.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),r&&o.STORAGE&&o.STORAGE.length>0?r.innerHTML=o.STORAGE.map(d=>{let m=(d.USED/1073741824).toFixed(1),h=(d.SIZE/(1024*1024*1024)).toFixed(1),k=d.SIZE>0?(d.USED/d.SIZE*100).toFixed(0):0,B=d.MOUNT_POINT||"Unmounted";return`
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: flex; justify-content: space-between; font-size: 0.75em; color: #888; margin-bottom: 6px; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px;">
                            <span>${m} GB Used</span>
                            <span>${h} GB Total</span>
                        </div>
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${k>90?"#cf6679":"#03dac6"}; width: ${k}%; height: 100%; box-shadow: 0 0 10px ${k>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await _t();o?(b(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let o=await _t();b(o)}if(!t)return;let l=await Ut();if(!l||!l.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let d=o.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}It=z()&&se.lastDashboard||Date.now(),X(0,It);let f=l.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let h=59-new Date().getSeconds();h<=0&&(h+=60),o.textContent=`${h}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function y(o){return!o||o==="N/A"||o==="unknown"||o.trim()===""?"-":o}function p(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:o.split(".").slice(0,3).join(".")||"-"}function g(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function a(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let m=parseInt(d[1])||0,h=parseInt(d[2])||0,k=parseInt(d[3])||0,B=parseFloat(d[4])||0,M=m*86400+h*3600+k*60+B,T=Math.floor(M/86400);if(T>0)return`${T}d`;let $=Math.floor(M/3600);if($>0)return`${$}h`;let _=Math.floor(M/60);return _>0?`${_}m`:`${Math.floor(M)}s`}function v(o){if(o.id==="upstash-redis-ro"){let E=z()&&(se.lastFrontend||se.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                <div class="service-widget-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 0;">
                    ...
                    <div class="sync-time-label" style="font-size: 0.6em; color: #444; margin-top: 10px;">Last synced: ${new Date(E).toLocaleTimeString()}</div>
                </div>
            </div>`}let d=o.status==="online",m=d?"service-widget-online":"service-widget-offline",h=d?"bx-check-circle":"bx-x-circle",k=d?"OK":"BAD",B=o.version?p(o.version.str):"-",M=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",T=o.memory&&o.memory!=="N/A"?o.memory:"-",$=M!=="-"?"#00ff00":"#666",_=M!=="-"?"#fff":"#666",O=T!=="-"?"#00ff00":"#666",R=T!=="-"?"#fff":"#666",W=a(o.uptime),G="";d?G=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${_};">${M}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${O};"></i>
                        <span style="color: ${R};">${T}</span>
                    </div>
                </div>`:G='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let F=z()?"easter.company":g(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${m}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${h}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${k}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${F}</span></div>${G}</div></div>`}let S=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),w=new Set(f.map(o=>o.id));for(let[o,d]of S)w.has(o)||d.remove();let u=new Set;f.forEach(o=>{if(u.has(o.id))return;u.add(o.id);let d=v(o),m=S.get(o.id);if(m&&m.parentNode)if(o.id==="upstash-redis-ro"){let h=z()&&(se.lastFrontend||se.lastDashboard)||Date.now(),k=new Date(h).toLocaleTimeString(),B=m.querySelector(".sync-time-label");B&&(B.textContent=`Last synced: ${k}`)}else m.outerHTML!==d&&(m.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Ve(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ut();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}Bt=Date.now(),X(2,Bt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let f=l.status==="Ready";return`
                ...
                </div>`}function c(l){let f=l.status==="Ready";return`
                ...
                </div>`}function r(l){let f=l.status==="Downloaded",y=f?"service-widget-online":"service-widget-offline",p=f?"OK":"MISSING",g=f&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",a=l.name;return l.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${f?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${g}</span></div></div></div>`}let b="";if(s&&(b+=n(s)),e.xtts&&(b+=c(e.xtts)),b+=i.map(r).join(""),!b){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==b&&(t.innerHTML=b)}async function re(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),b=document.getElementById("guardian-reset-btn"),l=document.getElementById("analyzer-reset-btn"),f=document.getElementById("fabricator-reset-btn"),y=document.getElementById("system-pause-toggle-btn");y&&!y.dataset.listenerAttached&&(y.onclick=async()=>{let d=y.querySelector(".bx-play")?"/agent/resume":"/agent/pause";y.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I(d,{method:"POST"}),setTimeout(()=>re(),500)}catch{y.innerHTML="<i class='bx bx-error'></i>"}},y.dataset.listenerAttached="true"),b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{b.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{b.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{b.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{b.innerHTML="<i class='bx bx-error'></i>"}},b.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=async()=>{l.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{l.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{l.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{l.innerHTML="<i class='bx bx-error'></i>"}},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await I("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),re()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true");let p=await Ls();if(p&&p.agents){let o=p.agents.guardian||{protocols:{}},d=p.agents.analyzer||{protocols:{}},m=p.agents.imaginator||{protocols:{}},h=p.agents.fabricator||{protocols:{}},k=p.system||{},B=Math.floor(Date.now()/1e3),M={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction"},T=x=>{x<0&&(x=0);let L=Math.floor(x/3600),A=Math.floor(x%3600/60),D=x%60;return L>0?`${L}h ${A}m`:A>0?`${A}m ${D}s`:`${D}s`},$=(x,L,A,D)=>{if(!x)return;let U=M[D]||D.toUpperCase(),q=x.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(q&&(q.textContent=U),!A){x.textContent="Inactive",x.style.color="#666";return}let J=A.status;if(J==="Working")x.textContent="Working",x.style.color="#bb86fc";else if(J==="Ready")x.textContent="Ready",x.style.color="#5eff5e";else{let j=A.cooldown;if(z()&&(j=A.next_run-B),j<=0)x.textContent="Ready",x.style.color="#5eff5e";else{let V=Math.floor(j/60),ie=j%60;x.textContent=`${V}m ${ie}s`,x.style.color="#fff"}}L&&A.stats&&(L.innerHTML=`
          ...
          `)};i&&$(i,document.getElementById("guardian-sentry-stats"),o.protocols.sentry,"sentry");let _=document.getElementById("analyzer-synthesis-val"),O=document.getElementById("analyzer-synthesis-stats");_&&$(_,O,d.protocols.synthesis,"synthesis");let R=document.getElementById("imaginator-alert_review-val"),W=document.getElementById("imaginator-alert_review-stats");R&&$(R,W,m.protocols.alert_review,"alert_review");let G=document.getElementById("fabricator-construction-val"),F=document.getElementById("fabricator-construction-stats");G&&$(G,F,h.protocols.construction,"construction");let E=document.getElementById("system-state-label"),C=document.getElementById("system-state-val");if(C&&k.state){let x=k.state,L=T(k.state_time||0);E&&(E.textContent=`State: ${x.toUpperCase()}`),C.textContent=L,x==="idle"?C.style.color=k.state_time>300?"#5eff5e":"#fff":C.style.color="#bb86fc",y&&(y.querySelector(".bx-loader-alt")||y.querySelector(".bx-error")||(x==="paused"?(y.innerHTML="<i class='bx bx-play'></i>",y.title="Resume System",y.style.color="#ff9800"):(y.innerHTML="<i class='bx bx-pause'></i>",y.title="Pause System",y.style.color="")))}n&&(n.textContent=T(k.metrics?.total_idle_time||0)),c&&(c.textContent=T(k.metrics?.total_active_time||0)),r&&(r.textContent=T(k.metrics?.total_waste_time||0))}else[i,s,n,c,r].forEach(d=>{d&&(d.textContent="-",d.style.color="#666")});if(t)return;let g=await Ts(),a=[],v=[],S=[];g&&(Array.isArray(g)?a=g:(a=g.active||[],v=g.queue||[],S=g.history||[],S.sort((o,d)=>(d.end_time||0)-(o.end_time||0)))),a.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=N("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),We(e,a,!1));let w=document.getElementById("processes-queue-widgets");w&&(v.length===0?!w.querySelector(".tab-placeholder")&&!w.querySelector('div[style*="font-style: italic"]')&&(w.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(w.innerHTML="",We(w,v,!1)));let u=document.getElementById("processes-history-widgets");u&&(S.length===0?u.querySelector(".tab-placeholder")||(u.innerHTML=N("empty","No recent history.")):(u.querySelector(".tab-placeholder")&&(u.innerHTML=""),We(u,S,!0))),$e(1,a.length)}function We(t,e,i){function s(l){let f="";l.end_time?f=`${l.end_time-l.start_time}s`:f=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let y=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,g={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(g[p]?p=g[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
        <div class="process-history-item" data-channel-id="${l.channel_id}-${l.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
                <i class='bx bx-check-circle' style="color: #03dac6; font-size: 1.1em;"></i>
                <span style="color: #fff; font-weight: 600;">${p}</span>
                <span style="color: #666; font-size: 0.9em; background: rgba(0,0,0,0.2); padding: 1px 6px; border-radius: 4px;">${l.state}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                <span>PID: ${l.pid}</span>
                <span style="color: #03dac6; min-width: 40px; text-align: right;">${f}</span>
            </div>
        </div>`;let a="#fff";return`
                <div class="service-widget process-widget" data-channel-id="${l.channel_id}-${l.start_time}" style="">
                    <div class="service-widget-header">
                    ...
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${l.state}</span>
                        </div>
                        <div class="service-widget-info">
                            <span class="info-label">Duration:</span>
                            <span class="info-value" style="color: ${a}">${f}</span>
                        </div>
                         <div class="service-widget-info">
                            <span class="info-label">PID:</span>
                            <span class="info-value" style="color: ${a}">${l.pid}</span>
                        </div>
                    </div>
                </div>`}let n=i?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(n)).map(l=>[l.dataset.channelId,l])),r=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,f]of c)r.has(l)||f.remove();let b=new Set;e.forEach(l=>{let f=`${l.channel_id}-${l.start_time}`;if(b.has(f))return;b.add(f);let y=s(l),p=c.get(f);if(p&&p.parentNode){p.outerHTML!==y&&(p.outerHTML=y);let g=t.querySelector(`[data-channel-id="${f}"]`);g&&t.appendChild(g)}else t.insertAdjacentHTML("beforeend",y)})}function Ne(){let t=Re(),e="Notification"in window,i={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function Ft(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await qt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await qt(t)}async function qt(t){try{let e=await I("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=N("empty","No web history found.");return}let s=`
            <div style="display: flex; flex-direction: column; height: 100%;">
        ...
        `;i.forEach((b,l)=>{let f=new Date(b.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=b.screenshot?`<img src="data:image/png;base64,${b.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
                <div class="web-card" style="flex: 0 0 100%; height: 100%; display: flex; flex-direction: column; scroll-snap-align: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-sizing: border-box; max-width: 100%; overflow: hidden;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; flex-shrink: 0;">
                        <span style="color: #03dac6; font-weight: bold; font-family: monospace;">#${l+1}</span>
                        <span style="color: #888; font-size: 0.8em; font-family: monospace;">${f}</span>
                    </div>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[i,s]of Object.entries(t)){let n=Gt.filter(c=>c.category===i);n.length!==0&&(e+=`
                    ...
                    ${n.map(c=>`
                        <div class="cli-command-card ${c.dummy?"dummy":""}" data-cmd="${c.id}" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 25px; transition: all 0.3s ease; cursor: pointer; position: relative; overflow: hidden;">
                            ...
                            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px; position: relative; z-index: 1;">
                                <i class='bx ${c.icon}' style="font-size: 2em; color: ${s.color};"></i>
    ...
    </div>`,e},Ze=!1;function As(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e),document.getElementById("close-terminal-btn").onclick=()=>Jt(),document.getElementById("terminal-action-btn").onclick=()=>Jt());let i=document.getElementById("cli-terminal-body"),s=document.getElementById("cli-docs-pane");i.innerHTML="";let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};return s.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ze=!0,i}function Jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ze=!1}function Wt(t,e,i="output"){if(!Ze)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Me(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Is(t){let e=Gt.find(n=>n.id===t);if(!e)return;let i=As(e);Wt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(r=>setTimeout(r,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),Wt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Is(s)})})}async function Bs(){if(!z())try{if(!(await I("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Kt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}tt(),st(),Bs(),rt(),et();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Vt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),o=document.querySelector("nav"),d=document.querySelector("footer"),m=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(_=>{let O=document.getElementById(_.id);O&&O.classList.remove("hide-close")});let k=document.getElementById("dexter-menu-container"),B=document.getElementById("nav-window-switcher"),M=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),$=window.innerWidth<880;if(!$){let _=document.getElementById("dexter-dropdown"),O=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),O&&O.classList.remove("active"),u&&u.classList.remove("menu-open")}if(be(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=$?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",o?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),k&&(k.style.display=$?"flex":"none"),M&&(M.style.display=$?"block":"none"),!$&&B){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(B.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(b)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(l)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(f)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):B.innerHTML=""}else B&&(B.innerHTML="");else o?.classList.remove("window-open"),T&&(T.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",m||h?d?.classList.remove("hide"):d?.classList.add("hide"),k&&(k.style.display="flex"),M&&(M.style.display="block"),B&&(B.innerHTML="");ye()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function r(){[...e].forEach(o=>o.close()),e=[]}window.addEventListener("resize",n);let b=le({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:ft(),onOpen:()=>ae(),onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}}),l=le({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Je(),onOpen:()=>{l.setContent(Je()),oe()},onClose:()=>{let u=e.indexOf(l);u>-1&&e.splice(u,1),n()}}),f=le({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Tt(),onOpen:()=>Fe(),onClose:()=>{let u=e.indexOf(f);u>-1&&e.splice(u,1),n()}}),y=le({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Nt()},{icon:"bxs-component",title:"Processes",content:zt()},{icon:"bxs-brain",title:"Models",content:Ht()},{icon:"bx-globe",title:"Web",content:jt()},{icon:"bxs-hdd",title:"Hardware",content:Pt()},{icon:"bxs-terminal",title:"Logs",content:Rt()},{icon:"bxs-zap",title:"Agents",content:Dt()},{icon:"bx-task",title:"Chores",content:St()}].filter(u=>z()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Ge(),re(),Ve(),Ft(),Ee(),we()},onClose:()=>{let u=e.indexOf(y);u>-1&&e.splice(u,1),n()}}),p=le({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-map-alt",title:"Roadmap",content:Ct()},{icon:"bx-paint",title:"Blueprints",content:$t()}],onOpen:()=>qe(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),g=le({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ne(),onOpen:()=>{g.setContent(Ne()),Ke(g)},onClose:()=>{let u=e.indexOf(g);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{l.isOpen()||c(l),setTimeout(()=>{let o=document.querySelector(`.event-item[data-event-id="${u}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{b.isOpen()||c(b),setTimeout(()=>{let o=document.querySelector(`.event-item[data-alert-id="${u}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),o=document.getElementById("dexter-menu-btn");u&&o&&(u.classList.remove("active"),o.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),o=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),m=document.getElementById("nav-left-container"),h=document.getElementById("dexter-dropdown"),k=document.getElementById("windows-container");u&&h&&(u.onclick=M=>{M.stopPropagation();let T=window.innerWidth<880;if(T)h.style.top="",h.style.left="",h.style.right="",h.style.transform="";else{let _=u.getBoundingClientRect(),R=document.querySelector("nav").getBoundingClientRect(),W=220,G=10,E=_.left+_.width/2-W/2,C=R.right-10;E+W>C&&(E=C-W),E<10&&(E=10),h.style.top=R.bottom+G+"px",h.style.left=E+"px",h.style.right="auto",h.style.transform="none"}h.classList.toggle("active"),u.classList.toggle("active");let $=h.classList.contains("active");if(T){let _=document.querySelector("nav");$?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),k?.classList.add("menu-open"),be(!0)):(k?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),be(!1)))}}),o&&(o.onclick=M=>{M.stopPropagation(),c(g)}),d&&(d.onclick=M=>{M.stopPropagation(),r()});let B=(M,T,$)=>{let _=document.getElementById(M);_&&(_.onclick=O=>{O.stopPropagation(),a(),$&&s($),c(T)})};B("alerts-menu-item",b,"alerts-window"),B("events-menu-item",l,"events-window"),B("monitor-menu-item",y,"monitor-window"),B("contacts-menu-item",f,"contacts-window"),B("workspace-menu-item",p,"workspace-window"),m&&(m.onclick=M=>{if(M.stopPropagation(),h&&h.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),be(!1));return}if(e.length>0)r();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let O=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");O.pop();let R=O.join("/")||"/";window.location.href=R}}}),document.addEventListener("click",()=>{let M=window.innerWidth<880;h&&h.classList.contains("active")&&(a(),M&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),be(!1)))}),h&&(h.onclick=M=>M.stopPropagation())})();let S=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!S&&!w&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{l.isOpen()&&oe(),b.isOpen()?ae():Ue()},1e3),setInterval(()=>{z()&&(b.isOpen()?ae():Ue()),p.isOpen()?qe():wt(),z()&&l.isOpen()&&oe(),y.isOpen()&&Ot()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Kt):Kt();})();

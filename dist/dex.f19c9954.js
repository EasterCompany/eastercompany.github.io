"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function me(t){let e=null,n=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++ut).toString())}function l(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",o),i&&setTimeout(i,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",a);let v=t.icon||"bx-window",s="",m="",w,I='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((h,y)=>{let c=h.icon?`<i class="bx ${h.icon}"></i>`:`<span class="tab-glyph">${h.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,w=`<div class="window-content">${t.tabs.map((h,y)=>`<div class="tab-content ${y===0?"active":""}" data-tab-content="${y}">${h.content}${I}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),w=`<div class="window-content">${t.content||""}${I}</div>`),e.innerHTML=`
...
`,je=null,ge=new Set,Ue=[];async function de(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];je=Date.now(),te(0,je);let o=Date.now(),f=24*60*60*1e3,d=l.filter(p=>{let h=localStorage.getItem(`alert_read_ts_${p.id}`);if(!h)return!0;let y=parseInt(h);return o-y<f});d.sort((p,h)=>{let y=E=>{let g=E.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},c=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,r=c(y(p)),C=c(y(h));return r!==C?C-r:h.timestamp-p.timestamp}),Ue=d;let x=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},T=[];if(d.length===0)T.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let h=new Set(d.map(c=>x(c))).size>1,y=null;d.forEach(c=>{let r=x(c);h&&r!==y&&(T.push({id:`divider-${r}`,type:"divider",label:r.toUpperCase()}),y=r),T.push(c)})}t&&(e.innerHTML="");let u=p=>{let h=p.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let y=(h.title||"Untitled Alert").trim(),c=(h.body||"No description provided.").trim(),r=h.summary||"",C=h.content||"",E=h.protocol||"",g=(h.priority||"low").toLowerCase(),k=!!h.alert,B=(h.category||"system").trim(),L=h.related_event_ids||[],_=h.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${p.id}`),G=new Date(p.timestamp*1e3),H=G.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=G.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=q?"event-border-grey":"event-border-blue";!q&&k&&(M="event-border-red"),q&&(g==="high"||g==="critical")?M="event-border-red":q&&g==="medium"&&(M="event-border-orange");let A=q?"alert-read":"alert-unread",O=ge.has(p.id),R=O?"expanded":"",V=O?"display: block;":"display: none;",S="",z="";L.length>0&&(z=`
            <div style="flex: 1; min-width: 150px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    ${L.map(ce=>`<a href="#" onclick="window.dexter.viewEvent('${ce}'); return false;" style="color: #03dac6; text-decoration: none; margin-right: 5px;">${ce.substring(0,8)}...</a>`).join(", ")}
                </div>
            </div>`);let N="";_&&(N=`
            <div style="flex: 1; min-width: 120px; text-align: center;">
                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Audit</div>
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    <a href="#" onclick="window.dexter.viewEvent('${_}'); return false;" style="color: #bb86fc; text-decoration: none; border-bottom: 1px dashed rgba(187, 134, 252, 0.3); white-space: nowrap;">${_.substring(0,8)}...</a>
                </div>
            </div>`);let U="";E&&(U=`
...
`,fe=new Set,St=[],Le=null;async function be(t=!1){let e=document.getElementById("roadmap-list");if(e){ks();try{let n=await D("/roadmap");if(!n.ok)throw new Error("Offline");let i=await n.json();St=i;let a=u=>{let v=fe.has(u.id),s=u.state==="published",m=u.state==="consumed",w="event-border-grey";s&&(w="event-border-blue"),m&&(w="event-border-purple");let b=new Date(u.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),p=document.createElement("div");p.className=`event-item notification-item ${w} cursor-pointer ${v?"expanded":""}`,p.dataset.itemId=u.id,p.onclick=c=>{if(c.target.closest("button"))return;if(p.classList.contains("expanded")){p.classList.remove("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="none"),fe.delete(u.id)}else{p.classList.add("expanded");let C=p.querySelector(".event-details");C&&(C.style.display="block"),fe.add(u.id)}};let h=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${u.state.toUpperCase()}]</span>`;p.innerHTML=`
        <div class="event-time">
      ...
      `;let y=p.querySelector(".event-details");return y&&y.addEventListener("click",c=>{c.stopPropagation()}),p.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(u)),p.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ls(u)),p.querySelector(".delete-btn")?.addEventListener("click",()=>Ms(u.id)),p.querySelector(".close-details-btn")?.addEventListener("click",c=>{c.stopPropagation(),p.classList.remove("expanded");let r=p.querySelector(".event-details");r&&(r.style.display="none"),fe.delete(u.id)}),p},l=u=>{let v=document.createElement("div");v.innerHTML=J("empty",u.message,u.action,u.id);let s=v.firstElementChild;return s.dataset.itemId=u.id,s},o=[];!i||i.length===0?o.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:P()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(u=>o.push({...u,type:"item"}));let f=Array.from(e.children),d=new Map(f.map(u=>[u.dataset.itemId,u])),x=new Set(o.map(u=>u.id));f.forEach(u=>{let v=u.dataset.itemId;(!v||!x.has(v))&&u.remove()}),t&&(e.innerHTML="");let T=null;o.forEach((u,v)=>{let s=d.get(u.id);(!s||t)&&(s&&s.remove(),u.type==="placeholder"?s=l(u):s=a(u),!s)||(v===0?e.firstElementChild!==s&&e.prepend(s):T&&T.nextElementSibling!==s&&T.after(s),T=s)})}catch{e.children.length===0&&(e.innerHTML=J("offline","Failed to load roadmap.","The event service may be offline."))}}}function ks(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),n=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Le=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block")},t.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Le=null},n.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),o=l?l.value:"";if(!o.trim())return;let f=Le?`/roadmap/${Le}`:"/roadmap",d=Le?"PATCH":"POST";try{await D(f,{method:d,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:o})});let x=document.getElementById("roadmap-editor-container");x&&(x.style.display="none"),be(!0)}catch(x){console.error(x)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{St.forEach(l=>fe.add(l.id)),be(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{fe.clear(),be(!0)},a.dataset.listenerAttached="true")}function Ss(t){Le=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let n=document.getElementById("roadmap-editor-container");n&&(n.style.display="block",n.scrollIntoView({behavior:"smooth"}))}async function Ls(t){let e=t.state==="published"?"draft":"published";try{await D(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),be(!0)}catch(n){console.error(n)}}async function Ms(t){if(confirm("Delete this roadmap item?"))try{await D(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),be(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Lt=null,ve=new Set,It=[];async function ye(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];It=l,Lt=Date.now(),te(2,Lt);let o=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let w=(m.title||"Untitled Blueprint").trim(),I=(m.summary||m.body||"No summary provided.").trim(),b=(m.content||"").trim(),p=(m.category||"architecture").trim(),h=(m.related_services||m.affected_services||[]).map(S=>S.trim()),y=(m.implementation_path||[]).map(S=>S.trim()),c=m.source_event_ids||[],r=m.approved===!0,C=new Date(s.timestamp*1e3),E=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=ve.has(s.id),B=k?"display: block;":"display: none;",L=document.createElement("div");L.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${r?"blueprint-approved":""}`,L.dataset.blueprintId=s.id,r&&(L.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",L.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=r?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[p]||"bx-paint";L.onclick=function(S){let z=this;if(z.classList.contains("expanded")){z.classList.remove("expanded"),ve.delete(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="none")}else{z.classList.add("expanded"),ve.add(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="block")}};let q="";y.length>0&&(q=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${y.map(S=>`<li style="margin-bottom: 5px;">${j(S)}</li>`).join("")}</ul></div>
                    </div>
                `);let G="";c.length>0&&(G=`
                    <div class="blueprint-source" style="margin-top: 15px;">
                        ...
                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                            ${c.map(S=>`
                                <a href="#" onclick="window.dexter.viewAlert('${S}'); return false;" style="color: #03dac6; text-decoration: none; font-size: 0.75em; font-family: 'JetBrains Mono', monospace; padding: 4px 8px; background: rgba(3, 218, 198, 0.05); border: 1px solid rgba(3, 218, 198, 0.1); border-radius: 4px;">
                                    <i class='bx bx-link-external'></i> ${S.substring(0,8)}...
                                </a>
                            `).join("")}
                        </div>
                    </div>
                `);let H=h.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${h.join(", ")}</span></div>`:"<div></div>",$=P(),M=r?`
            ...
            `;L.innerHTML=`
                ${r?'<div class="blueprint-sparkle"></div>':""}
                <div class="event-time">
                    <span class="event-time-main">${E}</span>
                    <span class="event-date">${g}</span>
                </div>
                <div class="event-icon" style="${r?"color: #03dac6;":""}"><i class='bx ${W}'></i></div>
                <div class="event-content">
                    ...
                    <div class="event-details" style="${B}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${j(I)}</div>
            ...
            `;let A=L.querySelector(".blueprint-approve-btn");A&&(A.onclick=async S=>{S.stopPropagation(),A.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await D(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ye(!0)}catch(z){console.error("Failed to approve blueprint:",z)}});let O=L.querySelector(".blueprint-delete-btn");O&&(O.onclick=async S=>{S.stopPropagation();let z=!r;O.innerHTML=z?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await D(`/events/${s.id}`,{method:"DELETE"})).ok&&ye(!0)}catch(N){console.error("Failed to delete blueprint:",N)}});let R=L.querySelector(".event-details");R&&R.addEventListener("click",S=>{S.stopPropagation()});let V=L.querySelector(".close-details-btn");return V&&V.addEventListener("click",S=>{S.stopPropagation(),L.classList.remove("expanded");let z=L.querySelector(".event-details");z&&(z.style.display="none"),ve.delete(s.id)}),L},f=s=>{let m=document.createElement("div");m.innerHTML=J("empty",s.message,s.action,s.id);let w=m.firstElementChild;return w.dataset.blueprintId=s.id,w},d=[];l.length===0?d.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):l.forEach(s=>d.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let x=Array.from(e.children),T=new Map(x.map(s=>[s.dataset.blueprintId,s])),u=new Set(d.map(s=>s.id));x.forEach(s=>{let m=s.dataset.blueprintId;(!m||!u.has(m))&&s.remove()});let v=null;d.forEach((s,m)=>{let w=T.get(s.id);(!w||t)&&(w&&w.remove(),s.type==="placeholder"?w=f(s):w=o(s),!w)||(m===0?e.firstElementChild!==w&&e.prepend(w):v&&v.nextElementSibling!==w&&v.after(w),v=w)}),Re(2,l.length),wt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=J("offline","Failed to load blueprints.","The event service may be offline."))}}async function Bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await D(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(l=>{let o=l.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{o={}}o.approved!==!0&&a++}),ht(a)}catch{}}function As(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),n=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{It.forEach(i=>ve.add(i.id)),ye(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ve.clear(),ye(!0)},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){n.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await D("/events?type=system.blueprint.generated",{method:"DELETE"}),ve.clear(),ye(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{n.innerHTML="<i class='bx bx-trash'></i> Clear"}}},n.dataset.listenerAttached="true")}var le="STANDBY",Ht=null,Me=[],X=null,qe=null,_t=()=>`
...
`,Nt=()=>`
    ...
    `,Be=[],He={"313071000877137920":"Owen",dexter:"Dexter"},se=[],xe=null;async function _e(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),n=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),o=document.getElementById("selected-recipients"),f=document.getElementById("task-filter-owner"),d=document.getElementById("new-chore-instruction"),x=document.getElementById("new-chore-url"),T=document.getElementById("new-chore-schedule"),u=document.getElementById("new-chore-time"),v=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(v&&!v.dataset.initialValueAttached){let r=Intl.DateTimeFormat().resolvedOptions().timeZone,C=!1;for(let E=0;E<v.options.length;E++)if(v.options[E].value===r){C=!0;break}if(!C){let E=document.createElement("option");E.value=r,E.textContent=r.replace("_"," "),v.appendChild(E)}v.value=r,v.dataset.initialValueAttached="true"}T&&s&&!T.dataset.timeListenerAttached&&(T.addEventListener("change",()=>{s.style.display=T.value==="daily"?"block":"none"}),T.dataset.timeListenerAttached="true");let m=document.getElementById("form-title"),w=document.getElementById("form-icon");function I(){if(o){if(se.length===0){o.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}o.innerHTML=se.sort((r,C)=>{let E=r.startsWith("channel:"),g=C.startsWith("channel:");return E&&!g?-1:!E&&g?1:0}).map(r=>{let C=He[r]||r,E=r.startsWith("channel:");return`
        <div class="recipient-badge" style="display: flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.05); padding: 4px 10px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); font-size: 0.75em; color: ${r==="dexter"?"#bb86fc":E?"#03dac6":"#fff"};">
          <i class='bx ${r==="dexter"?"bx-terminal":E?"bx-hash":"bx-user"}'></i>
          <span>${C}</span>
          <i class='bx bx-x remove-recipient' data-id="${r}" style="cursor: pointer; margin-left: 4px; opacity: 0.6;"></i>
        </div>
      `}).join(""),o.querySelectorAll(".remove-recipient").forEach(r=>{r.onclick=()=>{let C=r.dataset.id;se=se.filter(E=>E!==C),I()}})}}function b(r){r&&!se.includes(r)&&(se.push(r),I()),l&&(l.value="")}function p(r=null){if(n){if(xe=r?r.id:null,r){if(m&&(m.textContent="Update Research Task"),w&&(w.className="bx bx-edit-alt",w.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),d&&(d.value=r.natural_instruction),x&&(x.value=r.execution_plan?.entry_url||""),T&&(T.value=r.schedule,s&&(s.style.display=r.schedule==="daily"?"block":"none")),u&&r.run_at&&(u.value=r.run_at),v&&r.timezone){let C=!1;for(let E=0;E<v.options.length;E++)if(v.options[E].value===r.timezone){C=!0;break}if(!C){let E=document.createElement("option");E.value=r.timezone,E.textContent=r.timezone.replace("_"," "),v.appendChild(E)}v.value=r.timezone}se=r.recipients||(r.owner_id?[r.owner_id]:[]),I()}else m&&(m.textContent="Initialize Research Task"),w&&(w.className="bx bx-plus-circle",w.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),d&&(d.value=""),x&&(x.value=""),T&&(T.value="every_24h"),se=[],I();n.style.display="block",n.scrollIntoView({behavior:"smooth",block:"start"})}}let h=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(h&&!h.dataset.populated&&!P())try{ue("/contacts").then(async r=>{r.ok&&(((await r.json()).members||[]).forEach(g=>{if(He[g.id]=g.nickname||g.username,g.id==="313071000877137920")return;let k=document.createElement("option");k.value=g.id;let B=g.nickname||g.username;k.textContent=`${B} (${g.username})`,h.appendChild(k)}),h.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!P())try{ue("/channels").then(async r=>{r.ok&&((await r.json()).forEach(E=>{let g=`channel:${E.id}`;He[g]=E.name;let k=document.createElement("option");k.value=g,k.textContent=`#${E.name} (${E.guild})`,y.appendChild(k)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{b(l.value)},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onchange=()=>c(),f.dataset.listenerAttached="true");function c(){if(!t)return;let r=f?.value||"all",C=r==="all"?Be:Be.filter(g=>(g.recipients||(g.owner_id?[g.owner_id]:[])).includes(r));if(C.length===0){t.innerHTML=J("empty",r==="all"?"No active tasks.":"No tasks found for this owner.",P()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let E=C.map(g=>{let k=g.last_run===0?"Never":new Date(g.last_run*1e3).toLocaleString(),B=g.memory?g.memory.length:0,L=g.status==="active"?"#03dac6":"#666",W=(g.recipients||(g.owner_id?[g.owner_id]:[])).sort((q,G)=>{let H=q.startsWith("channel:"),$=G.startsWith("channel:");return H&&!$?-1:!H&&$?1:0}).map(q=>{let G=He[q]||q.substring(0,8),H=q.startsWith("channel:");return`<span title="${G}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${q==="dexter"?"bx-terminal":H?"bx-hash":"bx-user"}'></i>${G}</span>`}).join("<span style='color: #444;'>, </span>");return`
                <div class="service-widget wide-task-card" style="border-left: 4px solid ${L}; width: 100%; display: flex; flex-direction: column; padding: 20px;">
                    <div class="service-widget-header" style="display: flex; align-items: flex-start; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 15px; margin-bottom: 15px;">
                        <div style="display: flex; gap: 15px; flex: 1;">
                            <i class='bx bx-search-alt' style="color: ${L}; font-size: 1.5em; margin-top: 2px;"></i>
            ...
            `}).join("");t.innerHTML=E,t.querySelectorAll(".edit-chore-btn").forEach(g=>{g.onclick=k=>{k.stopPropagation();let B=g.dataset.id,L=Be.find(_=>_.id===B);L&&p(L)}}),t.querySelectorAll(".reset-chore-btn").forEach(g=>{g.onclick=async k=>{k.stopPropagation();let B=g.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(g.innerHTML="<i class='bx bx-loader-alt spin'></i>",await D(`/chores/${B}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),_e())}}),t.querySelectorAll(".delete-chore-btn").forEach(g=>{g.onclick=async k=>{k.stopPropagation();let B=g.dataset.id;confirm("Delete this task?")&&(await D(`/chores/${B}`,{method:"DELETE"}),_e())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{n&&(n.style.display==="none"||xe!==null?p(null):n.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{n&&(n.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let r=document.getElementById("new-chore-instruction"),C=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),g=r?.value,k=E?.value||"every_24h",B=k==="daily"?u?.value:"",L=k==="daily"?v?.value:"";if(!g)return;if(se.length===0){alert("Please add at least one recipient.");return}let _=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=xe?"PATCH":"POST",q=xe?`/chores/${xe}`:"/chores";await D(q,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:g,entry_url:C?.value,schedule:k,run_at:B,timezone:L})}),n&&(n.style.display="none"),r&&(r.value=""),C&&(C.value=""),xe=null,se=[],_e()}catch(W){console.error(W),alert(xe?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=_}},i.dataset.listenerAttached="true");try{let r=await D("/chores");if(!r.ok)throw new Error(`HTTP error! status: ${r.status}`);if(Be=await r.json()||[],f){let E=f.value,g=new Set;Be.forEach(k=>{k.owner_id&&g.add(k.owner_id),k.recipients&&k.recipients.forEach(B=>g.add(B))}),f.innerHTML='<option value="all">All Recipients</option>',g.forEach(k=>{let B=document.createElement("option");B.value=k;let L=k.startsWith("channel:"),_=He[k]||`Recipient: ${k.substring(0,8)}`;B.textContent=(L&&!_.startsWith("#")?"#":"")+_,f.appendChild(B)}),f.value=E,f.selectedIndex===-1&&(f.value="all")}c()}catch(r){console.error("Failed to fetch chores",r)}}var Pt=()=>`
...
`;function Ut(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function qt(t){if(!document.getElementById("dex-profile-styles")){let n=document.createElement("style");n.id="dex-profile-styles",n.textContent=Ds,document.head.appendChild(n)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",n=>{n.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",ue(`/profile/${t.id}`).then(async n=>{if(n.ok){let i=await n.json();tt(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=Ut(t.id,t.username);tt(e,t,i)}}).catch(n=>{console.error("Profile fetch error:",n);let i=Ut(t.id,t.username);tt(e,t,i)})}function tt(t,e,n){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let w=(n.attributes||n.facts||[]).map(c=>{let r=c.key||c.k,C=c.value||c.v;return`
        ...
        `},l=()=>{let w=n.dossier||{},I=w.identity||{},b=w.career||{},p=w.personal||{},h=w.social||[];return`
...
`,Fe=null,he=new Set,De=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Ps))if(n.includes(t))return e;return"system"}function js(t){return Rs[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(De=((await a.json()).events||[]).filter(v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Fe=Date.now(),te(1,Fe),De.length===0){e.innerHTML=J("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,w=Os(m),I=js(m),b=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.bot.voice_response"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),p="event-border-grey";b&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?p="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"||m==="engagement.decision"?p="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?p="event-border-orange":m==="system.test.completed"?p=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":p="event-border-blue");let h=b?"cursor-pointer":"",y=he.has(v.id),c=y?"expanded":"",r=y?"display: block;":"display: none;",C=new Date(v.timestamp*1e3),E=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=Jt(m,s),B=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",L="";if(b){let H="";if(m==="engagement.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.timestamp-s.last_active,A=Math.floor(M/60);H=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,M="";if(s.embed){let R=s.embed,V=R.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(R.title)}</div>`:"",S=R.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${re(R.description)}</div>`:"",z=(R.fields||[]).map(N=>`
              <div style="margin-top: 10px;">
            ...
            `}let A="";s.eval_count&&(A=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,V=s.chat_history.map((S,z)=>{let N=S.name?S.name.toUpperCase():S.role.toUpperCase();!S.name&&N==="ASSISTANT"&&(N="DEXTER");let U=S.role==="user"?"#03dac6":S.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${N}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${z+1} of ${R}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${j(S.content)}</div>
                                </div>
                            `}).join("");O=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(m==="messaging.bot.voice_response"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,M="",A=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let R=O.match(/{.*?}/g);R&&(A=R.map(V=>{let S=V.substring(1,V.length-1),z=S.indexOf(" "),N=S.substring(0,z),U=S.substring(z+1);return U=U.replace(/\[\]$/,"").trim(),{role:N,content:U}}))}if(A.length>0&&s.response_raw&&A.push({role:"assistant",content:s.response_raw}),A.length>0){let R=A.length,V=A.map((S,z)=>{let N=S.role.toUpperCase();N==="ASSISTANT"&&(N="DEXTER");let U=S.role==="user"?"#03dac6":S.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${N}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${z+1} of ${R}</span>
                    ...
                    `}else if(m==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.signals||{};H=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",M=s.success?"SUCCESS":"FAILED",A=z=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${z}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${A("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let z=s.chat_history.length,N=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ce=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let V="";if(s.corrections&&s.corrections.length>0){let z=s.corrections.map((N,U)=>`
                        ...
                        `).join("");V=`
                            <div class="event-detail-block">
                                ${A(`Corrections (${s.corrections.length})`)}
                                ${z}
                            </div>
                        `}let S="";if(s.parsed_results&&s.parsed_results.length>0){let z=s.parsed_results.map(N=>`
                        ...
                        ${V}
                        ${R}
                    `}else if(m==="system.test.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;H=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(A=>{let O=A.content_type&&A.content_type.startsWith("image/"),R=(A.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${A.content_type} \u2022 ${R}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),H=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let _=document.createElement("div");_.className=`event-item ${p} ${h} ${c}`,_.dataset.eventId=v.id,_.onclick=function(H){if(!b)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),he.delete(v.id);let A=$.querySelector(".event-details");A&&(A.style.display="none")}else{$.classList.add("expanded"),he.add(v.id);let A=$.querySelector(".event-details");A&&(A.style.display="block")}},_.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${E}</span>
                    <span class="event-date">${g}</span>
                </div>
...
`,Wt=qs;function Gt(){return`
    ...
    `}var Fs=null;async function ze(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await D("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=J("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(o=>!i.includes(o.id));a.forEach(o=>{o.logs&&Array.isArray(o.logs)?o.logs.reverse():o.logs=[]}),a.reverse();let l=a.map(o=>{let f=o.logs.join(`
...
`);return`
            ...
            `}).join("");return t.innerHTML=l,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let f=o,d=unescape(f.dataset.logs||"");navigator.clipboard.writeText(d).then(()=>{let x=f.querySelector("i");x&&(x.classList.remove("bx-copy"),x.classList.add("bx-check"),setTimeout(()=>{x.classList.remove("bx-check"),x.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let d=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${d}?`))try{(await D(`/logs?service_id=${d}`,{method:"DELETE"})).ok&&ze()}catch(x){console.error("Error clearing logs:",x)}})}),Fs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=J("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Yt=()=>{let t=P()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Gt()}
        </div>`;async function ns(){Js(),await Promise.all([ne(),we(),at()]),await ze()}var Je=null;function Js(){Je||(Je=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Je),Je=null;return}ne(!0)},1e3))}var Vt=null,Kt=null;async function is(){try{return await(await D("/system_monitor")).json()}catch{return null}}async function Zt(){try{return await(await D("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await D("/processes")).json()}catch{return null}}async function Gs(){try{return await(await D("/agent/status")).json()}catch{return null}}async function we(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),n=document.getElementById("global-restart-btn"),i=document.getElementById("global-stop-btn"),a=document.getElementById("global-start-btn"),l=(c,r)=>{c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{let C=c.innerHTML;c.innerHTML="<i class='bx bx-loader-alt spin'></i>",c.disabled=!0;try{await D(`/system/service/${r}`,{method:"POST",body:JSON.stringify({service:"all"})}),setTimeout(()=>we(),2e3),setTimeout(()=>we(),5e3),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML=C,c.disabled=!1},1e3)},1e3)}catch(E){console.error(E),c.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{c.innerHTML=C,c.disabled=!1},2e3)}},c.dataset.listenerAttached="true")};l(n,"restart"),l(i,"stop"),l(a,"start");let o=document.querySelector("#hw-os .hw-content"),f=document.querySelector("#hw-cpu .hw-content"),d=document.querySelector("#hw-ram .hw-content"),x=document.querySelector("#hw-gpu .hw-content"),T=document.querySelector("#hw-storage .hw-content"),u=c=>{if(c){if(o&&(o.innerHTML=`
                ...
                </div>`),d){let r=(c.MEMORY_BYTES/1073741824).toFixed(1);d.innerHTML=`
                ...
                </div>`}if(f&&c.CPU&&c.CPU.length>0){let r=c.CPU[0];f.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}x&&(c.GPU&&c.GPU.length>0?x.innerHTML=c.GPU.map(r=>{let C=(r.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):x.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),T&&c.STORAGE&&c.STORAGE.length>0?T.innerHTML=c.STORAGE.map(r=>{let C=(r.USED/1073741824).toFixed(1),E=(r.SIZE/(1024*1024*1024)).toFixed(1),g=r.SIZE>0?(r.USED/r.SIZE*100).toFixed(0):0,k=r.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):T&&(T.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let c=await Zt();c?(u(c),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),f&&!f.hasChildNodes())){let c=await Zt();u(c)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async c=>{let r=c.target.closest(".svc-btn");if(!r)return;let C=r,E=C.dataset.action,g=C.dataset.service;if(!E||!g)return;let k=C.innerHTML;C.innerHTML="<i class='bx bx-loader-alt spin'></i>",C.classList.add("loading"),C.disabled=!0;try{await D(`/system/service/${E}`,{method:"POST",body:JSON.stringify({service:g})}),setTimeout(()=>we(),1e3),setTimeout(()=>we(),3e3)}catch(B){console.error(B),alert(`Failed to ${E} ${g}`),C.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{C.innerHTML=k,C.classList.remove("loading"),C.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let v=await is();if(!v||!v.services){t.children.length===0&&(t.innerHTML=J("offline","Failed to load system metrics.","The event service may be offline."));let c=document.querySelector('[data-service-id="upstash-redis-ro"]');if(c){c.className="service-widget service-widget-offline",c.querySelector(".service-widget-status").textContent="ERROR";let r=c.querySelector(".service-widget-body");r&&(r.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Vt=P()&&oe.lastDashboard||Date.now(),te(0,Vt||0);let s=v.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let c=document.getElementById("upstash-countdown");if(c){let E=59-new Date().getSeconds();E<=0&&(E+=60),c.textContent=`${E}s`}},1e3)),Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function m(c){if(!c||c==="N/A"||c==="unknown")return"-";let r=c.match(/^(\d+\.\d+\.\d+)/);return r?r[0]:c.split(".").slice(0,3).join(".")||"-"}function w(c){return!c||c.length<=28?c:c.substring(0,28)+"..."}function I(c){if(!c||c==="N/A"||c==="unknown")return"-";let r=c.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!r)return"-";let C=parseInt(r[1])||0,E=parseInt(r[2])||0,g=parseInt(r[3])||0,k=parseFloat(r[4])||0,B=C*86400+E*3600+g*60+k,L=Math.floor(B/86400);if(L>0)return`${L}d`;let _=Math.floor(B/3600);if(_>0)return`${_}h`;let W=Math.floor(B/60);return W>0?`${W}m`:`${Math.floor(B)}s`}function b(c){if(c.id==="upstash-redis-ro"){let R=P()&&(oe.lastFrontend||oe.lastDashboard)||Date.now();return`
            ...
            </div>`}let r=c.status==="online",C=r?"service-widget-online":"service-widget-offline",E=r?"bx-check-circle":"bx-x-circle",g=r?"OK":"BAD",k=c.version?m(c.version.str):"-",B=c.cpu&&c.cpu!=="N/A"?c.cpu:"-",L=c.memory&&c.memory!=="N/A"?c.memory:"-",_=B!=="-"?"#00ff00":"#666",W=B!=="-"?"#fff":"#666",q=L!=="-"?"#00ff00":"#666",G=L!=="-"?"#fff":"#666",H=I(c.uptime),$="",M=c.type!=="os"&&c.type!=="cli"&&c.type!=="prd",A="";M&&!P()&&(A=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                ...
                <button class="svc-btn svc-stop" title="Stop Service" data-action="stop" data-service="${c.id}" ${r?"":"disabled"}><i class='bx bx-stop'></i></button>
                <button class="svc-btn svc-start" title="Start Service" data-action="start" data-service="${c.id}" ${r?"disabled":""}><i class='bx bx-play'></i></button>
            </div>`),r?$=`
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        <i class="bx bxs-microchip" style="color: ${_};"></i>
                ...
                </div>${A}`:$=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${A}`;let O=P()?"easter.company":w(c.domain&&c.port?`${c.domain}:${c.port}`:"");return`<div class="service-widget ${C}" data-service-id="${c.id}"><div class="service-widget-header"><i class="bx ${E}"></i><h3>${c.short_name||c.id}</h3><span class="service-widget-status">${g}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${O}</span></div>${$}</div></div>`}let p=new Map(Array.from(t.querySelectorAll(".service-widget")).map(c=>[c.dataset.serviceId,c])),h=new Set(s.map(c=>c.id));for(let[c,r]of p)h.has(c)||r.remove();let y=new Set;s.forEach(c=>{if(y.has(c.id))return;y.add(c.id);let r=b(c),C=p.get(c.id);if(C&&C.parentNode)if(c.id==="upstash-redis-ro"){let E=P()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),g=new Date(E).toLocaleTimeString(),k=C.querySelector(".sync-time-label");k&&(k.textContent=`Last synced: ${g}`)}else C.outerHTML!==r&&(C.outerHTML=r);else t.insertAdjacentHTML("beforeend",r)})}async function at(){let t=document.getElementById("models-widgets");if(!t)return;let e=await is();if(!e){t.children.length===0&&(t.innerHTML=J("offline","Failed to load model status.","The event service may be offline."));return}Kt=Date.now(),te(2,Kt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function a(d){let x=d.status==="Ready";return`
                ...
                </div>`}function o(d){let x=d.status==="Downloaded",T=x?"service-widget-online":"service-widget-offline",u=x?"OK":"MISSING",v=x&&d.size>0?`${(d.size/1e9).toFixed(2)} GB`:"-",s=d.name;return d.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${T}" data-model-name="${d.name}"><div class="service-widget-header"><i class="bx ${x?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${d.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let f="";if(i&&(f+=a(i)),e.xtts&&(f+=l(e.xtts)),f+=n.map(o).join(""),!f){t.innerHTML=J("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let n=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),d=document.getElementById("analyzer-reset-btn"),x=document.getElementById("fabricator-reset-btn"),T=document.getElementById("courier-reset-btn"),u=document.getElementById("system-pause-toggle-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{let c=u.querySelector(".bx-play")?"/agent/resume":"/agent/pause";u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D(c,{method:"POST"}),await ne()}catch{u.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},u.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),d&&!d.dataset.listenerAttached&&(d.onclick=async()=>{d.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{d.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{d.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{d.innerHTML="<i class='bx bx-error'></i>"}},d.dataset.listenerAttached="true"),x&&!x.dataset.listenerAttached&&(x.onclick=async()=>{x.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{x.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{x.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{x.innerHTML="<i class='bx bx-error'></i>"}},x.dataset.listenerAttached="true"),T&&!T.dataset.listenerAttached&&(T.onclick=async()=>{T.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{T.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{T.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{T.innerHTML="<i class='bx bx-error'></i>"}},T.dataset.listenerAttached="true");let v=document.getElementById("imaginator-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let y=s.agents.guardian||{protocols:{}},c=s.agents.analyzer||{protocols:{}},r=s.agents.imaginator||{protocols:{}},C=s.agents.fabricator||{protocols:{}},E=s.agents.courier||{protocols:{}},g=s.system||{},k=Math.floor(Date.now()/1e3),B={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},L=S=>{S<0&&(S=0);let z=Math.floor(S/3600),N=Math.floor(S%3600/60),U=S%60;return z>0?`${z}h ${N}m`:N>0?`${N}m ${U}s`:`${U}s`},_=(S,z,N,U)=>{if(!S)return;let Y=B[U]||U.toUpperCase(),K=S.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(K&&(K.textContent=Y),!N){S.textContent="Inactive",S.style.color="#666";return}let Ce=N.status;if(Ce==="Working")S.textContent="Working",S.style.color="#bb86fc";else if(Ce==="Ready")S.textContent="Ready",S.style.color="#5eff5e";else{let Ee=N.cooldown;if(P()&&(Ee=N.next_run-k),Ee<=0)S.textContent="Ready",S.style.color="#5eff5e";else{let Ge=Math.floor(Ee/60),Ve=Ee%60;S.textContent=`${Ge}m ${Ve}s`,S.style.color="#fff"}}z&&N.stats&&(z.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};n&&_(n,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry");let W=document.getElementById("analyzer-synthesis-val"),q=document.getElementById("analyzer-synthesis-stats");W&&_(W,q,c.protocols.synthesis,"synthesis");let G=document.getElementById("imaginator-alert_review-val"),H=document.getElementById("imaginator-alert_review-stats");G&&_(G,H,r.protocols.alert_review,"alert_review");let $=document.getElementById("fabricator-construction-val"),M=document.getElementById("fabricator-construction-stats");$&&_($,M,C.protocols.construction,"construction");let A=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");A&&_(A,O,E.protocols.researcher,"researcher");let R=document.getElementById("system-state-label"),V=document.getElementById("system-state-val");if(V&&g.state){let S=g.state,z=L(g.state_time||0);R&&(R.textContent=`State: ${S.toUpperCase()}`),V.textContent=z,S==="idle"?V.style.color=g.state_time>300?"#5eff5e":"#fff":V.style.color="#bb86fc",u&&(S==="paused"?(u.innerHTML="<i class='bx bx-play'></i>",u.title="Resume System",u.style.color="#ff9800"):(u.innerHTML="<i class='bx bx-pause'></i>",u.title="Pause System",u.style.color=""))}a&&(a.textContent=L(g.metrics?.total_idle_time||0)),l&&(l.textContent=L(g.metrics?.total_active_time||0)),o&&(o.textContent=L(g.metrics?.total_waste_time||0))}else[n,i,a,l,o].forEach(c=>{c&&(c.textContent="-",c.style.color="#666")});if(t)return;let m=await Ws(),w=[],I=[],b=[];m&&(Array.isArray(m)?w=m:(w=m.active||[],I=m.queue||[],b=m.history||[],b.sort((y,c)=>(c.end_time||0)-(y.end_time||0)))),e&&(w.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=J("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),it(e,w,!1)));let p=document.getElementById("processes-queue-widgets");p&&(I.length===0?!p.querySelector(".tab-placeholder")&&!p.querySelector('div[style*="font-style: italic"]')&&(p.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(p.innerHTML="",it(p,I,!1)));let h=document.getElementById("processes-history-widgets");h&&(b.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=J("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),it(h,b,!0))),Re(1,w.length)}function it(t,e,n){function i(d){let x="";d.end_time?x=`${d.end_time-d.start_time}s`:x=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let T=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",u=d.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(v[u]?u=v[u]:/^\d+$/.test(u)&&(u=`Channel ${u}`),n)return`
        ...
        <div class="settings-item">
            ...
            <label class="toggle-switch">
                <input type="checkbox" class="service-device-toggle" data-service="${m}" ${w==="cuda"?"checked":""} ${n?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>

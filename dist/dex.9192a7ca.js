...
`,Lt=null,ve=new Set,It=[];async function ye(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];It=l,Lt=Date.now(),te(2,Lt);let o=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let E=(m.title||"Untitled Blueprint").trim(),H=(m.summary||m.body||"No summary provided.").trim(),b=(m.content||"").trim(),p=(m.category||"architecture").trim(),h=(m.related_services||m.affected_services||[]).map(k=>k.trim()),y=(m.implementation_path||[]).map(k=>k.trim()),r=m.source_event_ids||[],c=m.approved===!0,C=new Date(s.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=ve.has(s.id),L=S?"display: block;":"display: none;",B=document.createElement("div");B.className=`event-item notification-item event-border-purple cursor-pointer ${S?"expanded":""} ${c?"blueprint-approved":""}`,B.dataset.blueprintId=s.id,c&&(B.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",B.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=c?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[p]||"bx-paint";B.onclick=function(k){let D=this;if(D.classList.contains("expanded")){D.classList.remove("expanded"),ve.delete(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="none")}else{D.classList.add("expanded"),ve.add(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="block")}};let J="";y.length>0&&(J=`
                ...
                `);let A=h.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${h.join(", ")}</span></div>`:"<div></div>",$=N(),M=c?`
...
`,Fe=null,he=new Set,De=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Ps))if(n.includes(t))return e;return"system"}function js(t){return Rs[t]||"bx-square-rounded"}async function pe(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await _(i);if(!a.ok)throw new Error("Service unreachable");if(De=((await a.json()).events||[]).filter(v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Fe=Date.now(),te(1,Fe),De.length===0){e.innerHTML=F("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,E=Os(m),H=js(m),b=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.bot.voice_response"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),p="event-border-grey";b&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?p="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"||m==="engagement.decision"?p="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?p="event-border-orange":m==="system.test.completed"?p=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":p="event-border-blue");let h=b?"cursor-pointer":"",y=he.has(v.id),r=y?"expanded":"",c=y?"display: block;":"display: none;",C=new Date(v.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),g=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=Jt(m,s),L=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",B="";if(b){let A="";if(m==="engagement.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.timestamp-s.last_active,I=Math.floor(M/60);A=`
            ...
            `}let I="";s.eval_count&&(I=`
                        ...
                        `}
                    `}else if(m==="messaging.bot.voice_response"){let $=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,M="",I=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let R=O.match(/{.*?}/g);R&&(I=R.map(G=>{let k=G.substring(1,G.length-1),D=k.indexOf(" "),z=k.substring(0,D),U=k.substring(D+1);return U=U.replace(/\[\]$/,"").trim(),{role:z,content:U}}))}if(I.length>0&&s.response_raw&&I.push({role:"assistant",content:s.response_raw}),I.length>0){let R=I.length,G=I.map((k,D)=>{let z=k.role.toUpperCase();z==="ASSISTANT"&&(z="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=D===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${D}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${z}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${D+1} of ${R}</span>
                    ...
                    `}else if(m==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,M=s.signals||{};A=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.visual.completed"){let $=I=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${I}</h5>`,M="";s.base64_preview?M=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${s.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:s.url&&(M=`<div class="event-detail-block"><img src="${s.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",M=s.success?"SUCCESS":"FAILED",I=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${I("Error")}
                        ...
                        ${G}
                        ${R}
                    `}else if(m==="system.test.completed"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let $=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`;A=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                ...
                `}let P=document.createElement("div");P.className=`event-item ${p} ${h} ${r}`,P.dataset.eventId=v.id,P.onclick=function(A){if(!b)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),he.delete(v.id);let I=$.querySelector(".event-details");I&&(I.style.display="none")}else{$.classList.add("expanded"),he.add(v.id);let I=$.querySelector(".event-details");I&&(I.style.display="block")}},P.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${g}</span>
                </div>
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            </div>`}let c=r.status==="online",C=c?"service-widget-online":"service-widget-offline",w=c?"bx-check-circle":"bx-x-circle",g=c?"OK":"BAD",S=r.version?m(r.version.str):"-",L=r.cpu&&r.cpu!=="N/A"?r.cpu:"-",B=r.memory&&r.memory!=="N/A"?r.memory:"-",P=L!=="-"?"#00ff00":"#666",W=L!=="-"?"#fff":"#666",J=B!=="-"?"#00ff00":"#666",V=B!=="-"?"#fff":"#666",A=H(r.uptime),$="",M=r.type!=="os"&&r.type!=="cli"&&r.type!=="prd",I="";M&&!N()&&(I=`

...
`,Ue=null,ye=new Set,Fe=[];async function me(t=!1){let e=document.getElementById("alerts-list");if(!e)return;gs(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let n=await H(i);if(!n.ok)throw new Error("Service is offline or unreachable.");let o=(await n.json()).events||[];Ue=Date.now(),pe(0,Ue);let a=Date.now(),p=24*60*60*1e3,c=o.filter(f=>{let v=localStorage.getItem(`alert_read_ts_${f.id}`);if(!v)return!0;let x=parseInt(v);return a-x<p});c.sort((f,v)=>{let x=E=>{let m=E.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},d=E=>E==="critical"?4:E==="high"?3:E==="medium"?2:1,l=d(x(f)),S=d(x(v));return l!==S?S-l:v.timestamp-f.timestamp}),Fe=c;let g=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},$=[];if(c.length===0)$.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let v=new Set(c.map(d=>g(d))).size>1,x=null;c.forEach(d=>{let l=g(d);v&&l!==x&&($.push({id:`divider-${l}`,type:"divider",label:l.toUpperCase()}),x=l),$.push(d)})}t&&(e.innerHTML="");let y=f=>{let v=f.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let x=(v.title||"Untitled Alert").trim(),d=(v.body||"No description provided.").trim(),l=v.summary||"",S=v.content||"",E=v.protocol||"",m=(v.priority||"low").toLowerCase(),w=!!v.alert,A=(v.category||"system").trim(),_=v.related_event_ids||[],B=v.audit_event_id,G=!!localStorage.getItem(`alert_read_ts_${f.id}`),Z=new Date(f.timestamp*1e3),M=Z.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=Z.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=G?"event-border-grey":"event-border-blue";!G&&w&&(k="event-border-red"),G&&(m==="high"||m==="critical")?k="event-border-red":G&&m==="medium"&&(k="event-border-orange");let C=G?"alert-read":"alert-unread",N=ye.has(f.id),D=N?"expanded":"",K=N?"display: block;":"display: none;",q="",j="";_.length>0&&(j=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${ne(S)}</div>
            </div>
        `:ee=`
            <div class="event-detail-block" style="text-align: left;">
...
`,Ae=new Set,Qe=[],$t=t=>{let e=Ae.has(t.number),n=new Date(t.createdAt).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),s=document.createElement("div");return s.className=`event-item notification-item event-border-blue cursor-pointer ${e?"expanded":""}`,s.dataset.issueNumber=t.number,s.id=`roadmap-issue-${t.number}`,s.onclick=o=>{if(o.target.closest("button")||o.target.closest("textarea"))return;if(s.classList.contains("expanded")){s.classList.remove("expanded");let p=s.querySelector(".event-details");p&&(p.style.display="none"),Ae.delete(t.number)}else{s.classList.add("expanded");let p=s.querySelector(".event-details");p&&(p.style.display="block"),Ae.add(t.number)}},s.innerHTML=`
          ...
          <div class="event-content">
            ...
            <div class="event-message" style="font-weight: bold; margin-bottom: 5px;">${P(t.title)}</div>
  ...
  `,s.querySelector(".comment-btn")?.addEventListener("click",async()=>{let o=s.querySelector(".comment-input"),a=o.value.trim();a&&(await vs(t.number,a),o.value="",xe(!0))}),s.querySelector(".close-btn")?.addEventListener("click",async()=>{confirm(`Close issue #${t.number}?`)&&(await ys(t.number),xe(!0))}),s};async function xe(t=!1){let e=document.getElementById("roadmap-list"),i=document.getElementById("roadmap-timeline-container");if(e){bs();try{let n=await H("/roadmap");if(!n.ok)throw new Error("Offline");let s=await n.json();if(s.sort((c,g)=>new Date(c.createdAt).getTime()-new Date(g.createdAt).getTime()),!t&&JSON.stringify(s)===JSON.stringify(Qe))return;Qe=s;let o=Array.from(e.children),a=new Map(o.filter(c=>c.dataset.issueNumber).map(c=>[c.dataset.issueNumber,c]));if(s.length===0){e.innerHTML=W("empty","No open issues found.","Dexter is currently in a perfect state.");return}let p={};if(s.forEach(c=>{let g="General",$=c.title.match(/^\[(.*?)\]/);$&&(g=$[1].toLowerCase()),p[g]||(p[g]=[]),p[g].push(c)}),e.innerHTML="",Object.entries(p).forEach(([c,g])=>{let $=`<div class="service-category-header" style="margin: 20px 0 10px 0; color: #888; font-size: 0.7em; text-transform: uppercase; letter-spacing: 2px;">${c}</div>`;e.insertAdjacentHTML("beforeend",$),g.forEach(y=>{let h=y.number.toString(),r=a.get(h);if(r&&!t){let u=r.querySelector(".event-message");u&&u.textContent!==y.title?e.appendChild($t(y)):e.appendChild(r)}else e.appendChild($t(y))})}),i){let c=s.slice(0,4),g=`
        <div class="roadmap-timeline" style="display: flex; gap: 5px; background: rgba(255,255,255,0.02); padding: 10px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); overflow-x: auto; align-items: center;">
          ...
          ${c.map(($,y)=>`
            <div class="timeline-dot-wrap" style="display: flex; align-items: center; gap: 5px;">
                <div class="timeline-dot" title="#${$.number}: ${$.title}" style="width: 12px; height: 12px; border-radius: 50%; background: #bb86fc; flex-shrink: 0; opacity: 0.8; cursor: pointer;" onclick="dexter.viewRoadmapIssue(${$.number})"></div>
                <span style="font-size: 0.7em; color: #aaa; white-space: nowrap;">#${$.number}</span>
...
`,Ct=()=>`
    ...
    `,_e=[],Be={"313071000877137920":"Owen",dexter:"Dexter"},ie=[],he=null;async function He(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),n=document.getElementById("save-chore-btn"),s=document.getElementById("cancel-chore-btn"),o=document.getElementById("new-chore-owner"),a=document.getElementById("selected-recipients"),p=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),g=document.getElementById("new-chore-url"),$=document.getElementById("new-chore-schedule"),y=document.getElementById("new-chore-time"),h=document.getElementById("new-chore-timezone"),r=document.getElementById("new-chore-time-group");if(h&&!h.dataset.initialValueAttached){let l=Intl.DateTimeFormat().resolvedOptions().timeZone,S=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l){S=!0;break}if(!S){let E=document.createElement("option");E.value=l,E.textContent=l.replace("_"," "),h.appendChild(E)}h.value=l,h.dataset.initialValueAttached="true"}$&&r&&!$.dataset.timeListenerAttached&&($.addEventListener("change",()=>{r.style.display=$.value==="daily"?"block":"none"}),$.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),L=document.getElementById("form-icon");function I(){if(a){if(ie.length===0){a.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}a.innerHTML=ie.sort((l,S)=>{let E=l.startsWith("channel:"),m=S.startsWith("channel:");return E&&!m?-1:!E&&m?1:0}).map(l=>{let S=Be[l]||l,E=l.startsWith("channel:");return`
      ...
      `}).join(""),a.querySelectorAll(".remove-recipient").forEach(l=>{l.onclick=()=>{let S=l.dataset.id;ie=ie.filter(E=>E!==S),I()}})}}function b(l){l&&!ie.includes(l)&&(ie.push(l),I()),o&&(o.value="")}function f(l=null){if(i){if(he=l?l.id:null,l){if(u&&(u.textContent="Update Research Task"),L&&(L.className="bx bx-edit-alt",L.style.color="#03dac6"),n&&(n.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=l.natural_instruction),g&&(g.value=l.execution_plan?.entry_url||""),$&&($.value=l.schedule,r&&(r.style.display=l.schedule==="daily"?"block":"none")),y&&l.run_at&&(y.value=l.run_at),h&&l.timezone){let S=!1;for(let E=0;E<h.options.length;E++)if(h.options[E].value===l.timezone){S=!0;break}if(!S){let E=document.createElement("option");E.value=l.timezone,E.textContent=l.timezone.replace("_"," "),h.appendChild(E)}h.value=l.timezone}ie=l.recipients||(l.owner_id?[l.owner_id]:[]),I()}else u&&(u.textContent="Initialize Research Task"),L&&(L.className="bx bx-plus-circle",L.style.color="#bb86fc"),n&&(n.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),g&&(g.value=""),$&&($.value="every_24h"),ie=[],I();i.style.display="block",i.scrollIntoView({behavior:"smooth",block:"start"})}}let v=document.getElementById("contacts-group"),x=document.getElementById("channels-group");if(v&&!v.dataset.populated&&!z())try{ve("/contacts").then(async l=>{l.ok&&(((await l.json()).members||[]).forEach(m=>{if(Be[m.id]=m.nickname||m.username,m.id==="313071000877137920")return;let w=document.createElement("option");w.value=m.id;let A=m.nickname||m.username;w.textContent=`${A} (${m.username})`,v.appendChild(w)}),v.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(x&&!x.dataset.populated&&!z())try{ve("/channels").then(async l=>{l.ok&&((await l.json()).forEach(E=>{let m=`channel:${E.id}`;Be[m]=E.name;let w=document.createElement("option");w.value=m,w.textContent=`#${E.name} (${E.guild})`,x.appendChild(w)}),x.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}o&&!o.dataset.listenerAttached&&(o.onchange=()=>{b(o.value)},o.dataset.listenerAttached="true"),p&&!p.dataset.listenerAttached&&(p.onchange=()=>d(),p.dataset.listenerAttached="true");function d(){if(!t)return;let l=p?.value||"all",S=l==="all"?_e:_e.filter(m=>(m.recipients||(m.owner_id?[m.owner_id]:[])).includes(l));if(S.length===0){t.innerHTML=W("empty",l==="all"?"No active tasks.":"No tasks found for this owner.",z()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let E=S.map(m=>{let w=m.last_run===0?"Never":new Date(m.last_run*1e3).toLocaleString(),A=m.memory?m.memory.length:0,_=m.status==="active"?"service-widget-online":"service-widget-offline",B=m.status==="active"?"ACTIVE":"PAUSED",G=(m.recipients||(m.owner_id?[m.owner_id]:[])).sort((M,T)=>{let k=M.startsWith("channel:"),C=T.startsWith("channel:");return k&&!C?-1:!k&&C?1:0}).map(M=>{let T=Be[M]||M.substring(0,8),k=M.startsWith("channel:");return`<span title="${T}" style="display: flex; align-items: center; gap: 3px; white-space: nowrap;"><i class='bx ${M==="dexter"?"bx-terminal":k?"bx-hash":"bx-user"}'></i>${T}</span>`}).join("<span style='color: #444; margin: 0 5px;'>|</span>"),Z=`${m.schedule}${m.run_at?" @ "+m.run_at+(m.timezone?" ("+m.timezone.split("/").pop()?.replace("_"," ")+")":""):""}`;return`
                <div class="service-widget task-widget ${_}">
                    <div class="service-widget-header">
                        <i class='bx bx-search-alt'></i>
                        <h3 title="${m.natural_instruction}">${m.natural_instruction}</h3>
                    ...
                    <div class="service-controls">
                        <button class="svc-btn reset-chore-btn" data-id="${m.id}" title="Reset Progress"><i class='bx bx-refresh'></i></button>
                        <button class="svc-btn edit-chore-btn" data-id="${m.id}" title="Edit Task"><i class='bx bx-edit-alt'></i></button>
                        <button class="svc-btn delete-chore-btn" data-id="${m.id}" title="Delete Task"><i class='bx bx-trash'></i></button>
                    </div>
                </div>
...
`,ws=null;async function tt(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await tt(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s&&s.dataset.user)try{let o=JSON.parse(s.dataset.user);At(o)}catch(o){console.error("Failed to parse user data",o)}},t.dataset.listenerAttached="true");try{let i=await ve("/contacts");if(!i.ok)throw new Error("Service unreachable");let s=(await i.json()).members||[];if(ws=Date.now(),s.length===0){t.innerHTML=W("empty","No contacts found.","Check your Discord connection.");return}let o={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};s.sort((a,p)=>{let c=o[a.level]??10,g=o[p.level]??10;return c!==g?c-g:a.username.localeCompare(p.username)}),t.innerHTML=s.map(a=>{let p=a.color&&a.color!==0,c=p?"#"+a.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",g=a.status==="online"?"#5eff5e":a.status==="idle"?"#ffa500":a.status==="dnd"?"#ff4d4d":"#666",$="#888";a.level==="Me"||a.level==="Master"?$="#bb86fc":a.level==="Admin"?$="#cf6679":a.level==="Moderator"?$="#03dac6":a.level==="Contributor"&&($="#ffa500");let y=a.level==="Me",h=y?"rgba(187, 134, 252, 0.08)":p?`${c}11`:"rgba(255,255,255,0.03)",r=y?"2px solid #bb86fc":p?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${y?"#bb86fc":p?c:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${a.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${y?"2px solid #bb86fc":p?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${$}; font-weight: 600; text-transform: uppercase;">${y?"DEXTER (ME)":a.level}</span>
...
`,Je=null,we=new Set,ze=[],Q="all",Ts={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals","system.cognitive.model_inference"],moderation:["moderation.explicit_content.deleted"]},$s={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart","system.cognitive.model_inference":"bx-brain"};function Ss(t){for(let[e,i]of Object.entries(Ts))if(i.includes(t))return e;return"system"}function ks(t){return $s[t]||"bx-square-rounded"}async function ue(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ls();let n=`/events?ml=${Q==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Q!=="all"&&(n+=`&category=${Q}`);try{let s=await H(n);if(!s.ok)throw new Error("Service unreachable");if(ze=((await s.json()).events||[]).filter(h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return!0}let u=r.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Je=Date.now(),pe(1,Je),ze.length===0){e.innerHTML=W("empty","No events found for this filter.");return}t&&(e.innerHTML="");let p=h=>{let r=h.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{return null}let u=r.type,L=Ss(u),I=ks(u),b=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.cognitive.model_inference"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),f="event-border-grey";b&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?f="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?f="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?f="event-border-orange":u==="system.test.completed"?f=r.test?.status==="OK"&&r.lint?.status==="OK"&&r.format?.status==="OK"?"event-border-blue":"event-border-red":f="event-border-blue");let v=b?"cursor-pointer":"",x=we.has(h.id),d=x?"expanded":"",l=x?"display: block;":"display: none;",S=new Date(h.timestamp*1e3),E=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),m=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),w=Bt(u,r),A=r.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${r.user_level})</span>`:"",_="";if(b){let M="";if(u==="engagement.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                    ...
                    `}else if(u==="system.attention.expired"){let T=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,k=r.timestamp-r.last_active,C=Math.floor(k/60);M=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let T=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,k="";if(r.embed){let D=r.embed,K=D.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${P(D.title)}</div>`:"",q=D.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${ne(D.description)}</div>`:"",j=(D.fields||[]).map(U=>`
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${P(U.name)}</div>
                <div style="font-size: 0.85em; color: #eee; white-space: pre-wrap;">${ne(U.value)}</div>
              </div>
            `).join("");k=`
            ...
            `}let C="";r.eval_count&&(C=`
                        ...
                        `);let N="";if(r.chat_history&&r.chat_history.length>0){let D=r.chat_history.length,K=r.chat_history.map((q,j)=>{let U=q.name?q.name.toUpperCase():q.role.toUpperCase();!q.name&&U==="ASSISTANT"&&(U="DEXTER");let V=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",ee=j===D-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${ee};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${V}; letter-spacing: 1px; font-weight: bold;">${U}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${j+1} of ${D}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${P(q.content)}</div>
                                </div>
                            `}).join("");N=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let T=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,k="",C=[],N=r.raw_input||"";if(N.startsWith("[{")&&N.endsWith("}]")){let D=N.match(/{.*?}/g);D&&(C=D.map(K=>{let q=K.substring(1,K.length-1),j=q.indexOf(" "),U=q.substring(0,j),V=q.substring(j+1);return V=V.replace(/\[\]$/,"").trim(),{role:U,content:V}}))}if(C.length>0&&r.response_raw&&C.push({role:"assistant",content:r.response_raw}),C.length>0){let D=C.length,K=C.map((q,j)=>{let U=q.role.toUpperCase();U==="ASSISTANT"&&(U="DEXTER");let V=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",ee=j===D-1?"block":"none";return`
                                <div class="history-slide" data-index="${j}" style="display: ${ee};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${V}; letter-spacing: 1px; font-weight: bold;">${U}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${j+1} of ${D}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let T=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,k=r.signals||{};M=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block" style="margin-top: 15px;">
                            ${T("Raw Model Output")}
                            <pre class="detail-pre">${P(r.raw_output)||"None"}</pre>
                        </div>
                    ...
                    `}else if(u==="analysis.link.completed"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        ...
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            ...
                            <span class="detail-value">${P(r.title)||"N/A"}</span>
                        </div>
                        <div class="event-detail-block">
                            ${T("Description")}
                            <pre class="detail-pre">${P(r.description)||"None"}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${T("Content Summary")}
                            <pre class="detail-pre">${P(r.summary)||"None"}</pre>
                        </div>
              ...
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(u==="analysis.router.decision"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        ...
                        <div class="event-detail-block">
                            ${T("Input Context")}
                            <pre class="detail-pre">${P(r.raw_input)||"None"}</pre>
                        </div>
                    `}else if(u==="system.cli.command"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        ...
                        <div class="event-detail-block">
                            ${T("Output")}
                            <pre class="detail-pre">${P(r.output)||"No output recorded."}</pre>
                        </div>
                    `}else if(u==="system.analysis.audit"){let T=r.success?"#03dac6":"#ff4d4d",k=r.success?"SUCCESS":"FAILED",C=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,N="";r.error&&(N=`
                            <div class="event-detail-block">
                                ${C("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(r.error)}</pre>
                            </div>
                        `);let D="";if(r.chat_history&&r.chat_history.length>0){let j=r.chat_history.length,U=r.chat_history.map((V,ee)=>{let Y=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Y==="USER"&&(Y="SYSTEM"),!V.name&&Y==="ASSISTANT"&&(Y="AGENT");let Ce=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let K="";if(r.corrections&&r.corrections.length>0){let j=r.corrections.map((U,V)=>`
                        ...
                        `}let q="";if(r.parsed_results&&r.parsed_results.length>0){let j=r.parsed_results.map(U=>`
                    ...
                    `}else if(u==="error_occurred"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        ...
                        <div class="event-detail-block">
                            ${T("Error Message")}
                            <pre class="detail-pre" style="color: #ff4d4d;">${P(r.error)}</pre>
                        </div>
                        <div class="event-detail-block">
                            ${T("Context")}
                    ...
                    `}else if(u==="system.cli.status"){let T=k=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${k}</h5>`;M=`
                        ...
                        <div class="event-detail-block">
                            ${T("Message")}
                            <pre class="detail-pre">${P(r.message)}</pre>
                        </div>
                    `}else if(u==="messaging.user.sent_message"){let T="";r.attachments&&r.attachments.length>0&&(T=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${r.attachments.map(C=>{let N=C.content_type&&C.content_type.startsWith("image/"),D=(C.size/1024).toFixed(1)+" KB";return`
...
`),c=[...a.logs];if(c.length<25){let $=25-c.length;for(let y=0;y<$;y++)c.push("")}else c.length>25&&(c=c.slice(-25));let g=c.map($=>qe($)).join(`
`);return`
  ...
  `}async function Nt(){try{let e=await(await H("/processes")).json(),i=e.active||[],n=e.history||[],s=i.find(a=>a.channel_id==="system-fabricator");if(s){Ee="ACTIVE";let p=Math.floor(Date.now()/1e3)-s.start_time,c=10,g="REVIEW";p>30&&(c=30,g="INVESTIGATION"),p>120&&(c=60,g="CONSTRUCTION"),p>300&&(c=90,g="REPORTING"),te={title:s.state||"Fabricating system improvements...",progress:c,phase:g};let $=new Date(s.updated_at*1e3).toLocaleTimeString();(ae.length===0||ae[ae.length-1].msg!==s.state)&&(ae.push({time:$,msg:s.state}),ae.length>50&&ae.shift())}else{let a=n.find(p=>p.channel_id==="system-fabricator");if(a&&Date.now()/1e3-a.end_time<60){Ee="COMPLETED";let p=a.end_time-a.start_time;nt={duration:`${Math.floor(p/60)}m ${p%60}s`,steps:0,result:a.state,timestamp:a.end_time}}else Ee="STANDBY",ae=[]}let o=document.getElementById("progress-view-root");o&&(Ee!==zt&&(o.innerHTML=Rt(),zt=Ee),Ee==="ACTIVE"&&_s())}catch(t){console.error("Failed to update progress:",t)}}function _s(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),i=document.getElementById("active-task-progress-text"),n=document.getElementById("active-task-phase"),s=document.getElementById("terminal-output");if(t&&te&&(t.innerText=te.title),e&&te&&(e.style.width=`${te.progress}%`),i&&te&&(i.innerText=`${te.progress}% COMPLETE`),n&&te&&(n.innerText=`PHASE: ${te.phase}`),s){let o=s.children.length;if(ae.length>o){for(let a=o;a<ae.length;a++){let p=ae[a],c=document.createElement("div");c.className="terminal-line",c.innerHTML=`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ht()}
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}g&&(d.GPU&&d.GPU.length>0?g.innerHTML=d.GPU.map(l=>{let S=(l.VRAM/1073741824).toFixed(1);return`
                        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span style="font-size: 1.1em; color: #fff; font-weight: 400;">${l.LABEL}</span>
                            <span style="font-size: 1.2em; font-weight: bold; color: #fff;">${S} <span style="font-size: 0.6em; color: #888; font-weight: normal;">GB VRAM</span></span>
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):g.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),$&&d.STORAGE&&d.STORAGE.length>0?$.innerHTML=d.STORAGE.map(l=>{let S=(l.USED/1073741824).toFixed(1),E=(l.SIZE/(1024*1024*1024)).toFixed(1),m=l.SIZE>0?(l.USED/l.SIZE*100).toFixed(0):0,w=l.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):$&&($.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let d=await Pt();d?(y(d),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),p&&!p.hasChildNodes())){let d=await Pt();y(d)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async d=>{let l=d.target.closest(".svc-btn");if(!l)return;let S=l,E=S.dataset.action,m=S.dataset.service;if(!E||!m)return;let w=S.innerHTML;S.innerHTML="<i class='bx bx-loader-alt spin'></i>",S.classList.add("loading"),S.disabled=!0;try{await H(`/system/service/${E}`,{method:"POST",body:JSON.stringify({service:m})}),setTimeout(()=>Te(),1e3),setTimeout(()=>Te(),3e3)}catch(A){console.error(A),alert(`Failed to ${E} ${m}`),S.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{S.innerHTML=w,S.classList.remove("loading"),S.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let h=await Hs();if(!h||!h.services){t.children.length===0&&(t.innerHTML=W("offline","Failed to load system metrics.","The event service may be offline."));return}Ot=z()&&be.lastDashboard||Date.now(),pe(3,Ot||0);let r=h.services||[],u={cli:[],fe:[],cs:[],be:[],th:[],co:[],md:[],prd:[],os:[]},L={cli:"CLI",fe:"Front-end",cs:"Core",be:"Backend",th:"Third Party",co:"Cognitive",md:"Models",prd:"Production",os:"Other"};r.forEach(d=>{u[d.type]?u[d.type].push(d):u.os.push(d)});let I="";Object.entries(u).forEach(([d,l])=>{l.length!==0&&(I+=`
            ...
            <div class="service-category-grid system-monitor-widgets" style="padding-bottom: 20px;">
                ${l.map(S=>x(S)).join("")}
            </div>`)}),t.innerHTML!==I&&(t.innerHTML=I);function b(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/^(\d+\.\d+\.\d+)/);return l?l[0]:d.split(".").slice(0,3).join(".")||"-"}function f(d){return!d||d.length<=22?d:d.substring(0,22)+"..."}function v(d){if(!d||d==="N/A"||d==="unknown")return"-";let l=d.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!l)return"-";let S=parseInt(l[1])||0,E=parseInt(l[2])||0,m=parseInt(l[3])||0,w=parseFloat(l[4])||0,A=S*86400+E*3600+m*60+w,_=Math.floor(A/86400);if(_>0)return`${_}d`;let B=Math.floor(A/3600);if(B>0)return`${B}h`;let O=Math.floor(A/60);return O>0?`${O}m`:`${Math.floor(A)}s`}function x(d){if(d.id==="upstash-redis-ro"){let D=z()&&(be.lastFrontend||be.lastDashboard)||Date.now();return`
            ...
            </div>`}let l=d.status==="online",S=l?"service-widget-online":"service-widget-offline",E=l?"bx-check-circle":"bx-x-circle",m=l?"OK":"BAD",w=d.version?b(d.version.str):"-",A=d.cpu&&d.cpu!=="N/A"?d.cpu:"-",_=d.memory&&d.memory!=="N/A"?d.memory:"-",B=A!=="-"?"#00ff00":"#666",O=A!=="-"?"#fff":"#666",G=_!=="-"?"#00ff00":"#666",Z=_!=="-"?"#fff":"#666",M=v(d.uptime),T="",k=d.type!=="os"&&d.type!=="cli"&&d.type!=="prd"&&d.type!=="md",C="";if(k&&!z()&&(C=`
            <div class="service-controls">
            ...
            </div>`),l){let D=d.type==="md"?"Size:":"Version:",K=d.type==="md"?d.memory:w,q=d.type==="md"?"Type:":"RAM:",j=d.type==="md"?d.health_message||"Model":_;T=`
                <div class="service-widget-info">
                ...
                </div>`}else T='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold; margin-top: auto;"><span>OFFLINE</span></div>';let N=z()||d.type==="md"?"easter.company":f(d.domain&&d.port?`${d.domain}:${d.port}`:"");return`<div class="service-widget ${S}" data-service-id="${d.id}"><div class="service-widget-header"><i class="bx ${E}"></i><h3>${d.short_name||d.id}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${N}</span></div>${T}</div>${C}</div>`}}async function re(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),n=document.getElementById("guardian-idle-val"),s=document.getElementById("guardian-total-idle"),o=document.getElementById("guardian-total-active"),a=document.getElementById("guardian-total-waste"),p=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),g=document.getElementById("fabricator-reset-btn"),$=document.getElementById("fabricator-progress-btn"),y=document.getElementById("courier-reset-btn"),h=document.getElementById("system-pause-toggle-btn");$&&!$.dataset.listenerAttached&&($.onclick=()=>{let d=document.querySelector("nav");d&&(d.style.display="none");let l=le({id:"fabricator-progress-window",title:"Mission Control",icon:"bx-loader-circle",className:"fabricator-progress-modal",appendToBody:!0,content:`
    ...
    `;async function Kt(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Gt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Gt(t)}async function Gt(t){try{let e=await H("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=W("empty","No web history found.");return}let n=`
        ...
        `;i.forEach((p,c)=>{let g=new Date(p.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),$=p.screenshot?`<img src="data:image/png;base64,${p.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:`<div style="width: 100%; height: 120px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ns=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `);let o=document.getElementById("terminal-cmd-name");o&&(o.textContent=`dex ${t.id}`);let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-running",a.textContent="Running");let p=document.getElementById("terminal-action-btn");return p&&(p.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",ot=!0,i}function Yt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),oe()||(document.body.style.overflow=""),ot=!1}function Xt(t,e,i="output"){if(!ot||!t)return;let n=document.createElement("div");if(n.className=`terminal-line terminal-${i}`,i==="prompt")n.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let s=qe(e);n.innerHTML=s||e}t.appendChild(n),t.scrollTop=t.scrollHeight}async function Ps(t){let e=Zt.find(a=>a.id===t);if(!e)return;let i=Os(e);i&&Xt(i,`dex ${t}`,"prompt");let n=e.demo_output||["Executing command...","\u2713 Done."];for(let a of n){await new Promise(c=>setTimeout(c,400+Math.random()*600));let p="output";a.includes("[ERROR]")?p="error":a.includes("[SUCCESS]")||a.includes("\u2713")?p="success":a.includes("!")&&(p="warning"),i&&Xt(i,a,p)}let s=document.getElementById("terminal-status-badge");s&&(s.className="terminal-status status-success",s.textContent="Completed (Demo)");let o=document.getElementById("terminal-action-btn");o&&(o.style.display="block")}function Qt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ns();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code"),n=i?i.textContent:"";n&&navigator.clipboard.writeText(n).then(()=>{let s=e.querySelector("i");s&&(s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(i=>{let n=i;i.addEventListener("mouseenter",()=>{n.style.transform="translateY(-5px)",n.style.borderColor="rgba(255,255,255,0.15)",n.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{n.style.transform="translateY(0)",n.style.borderColor="rgba(255,255,255,0.05)",n.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=n.dataset.cmd;s&&Ps(s)})})}async function js(){if(!z())try{if(!(await H("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function es(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}dt(),pt(),js(),bt(),ct();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Qt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let n=b=>{localStorage.setItem("dex_last_window",b)};function s(){for(;e.length>1;)e.shift().close(!0);let b=document.getElementById("windows-container"),f=document.querySelector("nav"),v=document.querySelector("footer"),x=window.location.pathname==="/"||window.location.pathname==="/index.html",d=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");b&&b.setAttribute("data-count",e.length.toString()),e.forEach(_=>{let B=document.getElementById(_.id);B&&B.classList.remove("hide-close")});let l=document.getElementById("dexter-menu-container"),S=document.getElementById("nav-window-switcher"),E=document.getElementById("settings-icon"),m=document.getElementById("docs-icon"),w=document.getElementById("close-all-windows"),A=window.innerWidth<880;if(!A){let _=document.getElementById("dexter-dropdown"),B=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),B&&B.classList.remove("active"),b&&b.classList.remove("menu-open")}if(Se(e.length>0),e.length>0)if(v?.classList.add("hide"),w&&(w.style.display=A?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",f?.classList.add("window-open"),b&&(b.style.paddingTop="60px"),l&&(l.style.display=A?"flex":"none"),m&&(m.style.display=A?"block":"none"),E&&(E.style.display=A?"block":"none"),!A&&S){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(S.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{n("alerts-window"),o(p)}),document.getElementById("switch-events")?.addEventListener("click",()=>{n("events-window"),o(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{n("monitor-window"),o($)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{n("contacts-window"),o(g)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{n("workspace-window"),o(y)})):S.innerHTML=""}else S&&(S.innerHTML="");else f?.classList.remove("window-open"),w&&(w.style.display="none"),b&&(b.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),oe()||(document.body.style.overflow=""),x||d?v?.classList.remove("hide"):v?.classList.add("hide"),l&&(l.style.display="flex"),m&&(m.style.display="block"),E&&(E.style.display="block"),S&&(S.innerHTML="");Ie()}function o(b){if(b.isOpen()){b.close();return}for(;e.length>0;)e.pop().close(!0);e.push(b),b.open(),s()}function a(){[...e].forEach(f=>f.close()),e=[]}window.addEventListener("resize",s);let p=le({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>me(),onClose:()=>{let b=e.indexOf(p);b>-1&&e.splice(b,1),s()}}),c=le({id:"events-window",title:"Events",icon:"bx-calendar-event",content:st(),onOpen:()=>{c.setContent(st()),ue()},onClose:()=>{let b=e.indexOf(c);b>-1&&e.splice(b,1),s()}}),g=le({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:_t(),onOpen:()=>tt(),onClose:()=>{let b=e.indexOf(g);b>-1&&e.splice(b,1),s()}}),$=le({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-component",title:"Processes",content:qt()},{icon:"bxs-zap",title:"Agents",content:jt()},{icon:"bx-globe",title:"Web",content:Vt()},{icon:"bxs-server",title:"Services",content:Ut()},{icon:"bxs-terminal",title:"Logs",content:Jt()},{icon:"bxs-hdd",title:"Hardware",content:Ft()}].filter(b=>z()?b.title!=="Hardware"&&b.title!=="Logs":!0),onOpen:()=>{re(),Te(),Kt(),De()},onClose:()=>{let b=e.indexOf($);b>-1&&e.splice(b,1),s()}}),y=le({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-map-alt",title:"Roadmap",content:Mt()},{icon:"bx-task",title:"Tasks",content:It()}],onOpen:()=>et(),onClose:()=>{let b=e.indexOf(y);b>-1&&e.splice(b,1),s()}}),h=le({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ge(),onOpen:()=>{h.setContent(Ge()),at(h)},onClose:()=>{let b=e.indexOf(h);b>-1&&e.splice(b,1),s()}});window.dexter||(window.dexter={}),window.dexter.viewEvent=b=>{c.isOpen()||o(c),setTimeout(()=>{let f=document.querySelector(`.event-item[data-event-id="${b}"]`);f&&(f.scrollIntoView({behavior:"smooth",block:"center"}),f.classList.add("flash-highlight"),f.classList.contains("expanded")||f.click(),setTimeout(()=>f.classList.remove("flash-highlight"),2e3))},500)},window.dexter.viewAlert=b=>{p.isOpen()||o(p),setTimeout(()=>{let f=document.querySelector(`.event-item[data-alert-id="${b}"]`);f&&(f.scrollIntoView({behavior:"smooth",block:"center"}),f.classList.add("flash-highlight"),f.classList.contains("expanded")||f.click(),setTimeout(()=>f.classList.remove("flash-highlight"),2e3))},500)};let r=()=>{let b=document.getElementById("dexter-dropdown"),f=document.getElementById("dexter-menu-btn");b&&f&&(b.classList.remove("active"),f.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let b=document.getElementById("dexter-menu-btn"),f=document.getElementById("settings-icon"),v=document.getElementById("docs-icon"),x=document.getElementById("close-all-windows"),d=document.getElementById("nav-left-container"),l=document.getElementById("dexter-dropdown"),S=document.getElementById("windows-container");b&&l&&(b.onclick=m=>{m.stopPropagation();let w=window.innerWidth<880;if(w)l.style.top="",l.style.left="",l.style.right="",l.style.transform="";else{let _=b.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),G=220,Z=10,T=_.left+_.width/2-G/2,k=O.right-10;T+G>k&&(T=k-G),T<10&&(T=10),l.style.top=O.bottom+Z+"px",l.style.left=T+"px",l.style.right="auto",l.style.transform="none"}l.classList.toggle("active"),b.classList.toggle("active");let A=l.classList.contains("active");if(w){let _=document.querySelector("nav");A?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),S?.classList.add("menu-open"),Se(!0)):(S?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),oe()||Se(!1)))}}),f&&(f.onclick=m=>{m.stopPropagation(),o(h)}),v&&(v.onclick=m=>{m.stopPropagation(),window.location.href="/docs"}),x&&(x.onclick=m=>{m.stopPropagation(),a()});let E=(m,w,A)=>{let _=document.getElementById(m);_&&(_.onclick=B=>{B.stopPropagation(),r(),A&&n(A),o(w)})};E("alerts-menu-item",p,"alerts-window"),E("events-menu-item",c,"events-window"),E("monitor-menu-item",$,"monitor-window"),E("contacts-menu-item",g,"contacts-window"),E("workspace-menu-item",y,"workspace-window"),d&&(d.onclick=m=>{if(m.stopPropagation(),l&&l.classList.contains("active")){r(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),oe()||Se(!1));return}if(e.length>0)a();else{let w=window.location.pathname;if(!(w==="/"||w==="/index.html")){let _=w.endsWith("/")&&w.length>1?w.slice(0,-1):w;if(_.includes("/docs/page/")){window.location.href="/";return}let B=_.split("/");B.pop();let O=B.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let m=window.innerWidth<880;l&&l.classList.contains("active")&&(r(),m&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),oe()||Se(!1)))}),l&&(l.onclick=m=>m.stopPropagation())})();let L=window.location.pathname==="/"||window.location.pathname==="/index.html",I=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!L&&!I&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{c.isOpen()&&ue(),p.isOpen()?me():Ze()},1e3),setInterval(()=>{z()&&(p.isOpen()?me():Ze()),y.isOpen()&&et(),z()&&c.isOpen()&&ue(),$.isOpen()&&Wt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",es):es();})();

"use strict";(()=>{function dt(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.prepend(t)}}function pt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",a=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var ut=1e3;function ue(t){let e=null,o=t.onClose||null,i=t.onOpen||null;function a(){e&&(e.style.zIndex=(++ut).toString())}function r(){if(e){e.classList.add("open"),a(),window.addEventListener("resize",l),i&&setTimeout(i,10);return}let u=document.getElementById("windows-container");u||(u=document.createElement("div"),u.id="windows-container",u.className="windows-container",document.body.appendChild(u)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++ut).toString(),e.addEventListener("mousedown",a);let x=t.icon||"bx-window",s="",m="",T,_='<div class="window-bottom-spacer"></div>';t.tabs&&t.tabs.length>0?(s=`<div class="tab-bar">${t.tabs.map((h,y)=>{let I=h.icon?`<i class="bx ${h.icon}"></i>`:`<span class="tab-glyph">${h.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,T=`<div class="window-content">${t.tabs.map((h,y)=>`<div class="tab-content ${y===0?"active":""}" data-tab-content="${y}">${h.content}${_}</div>`).join("")}</div>`):(t.title&&(m=`<div class="window-title">${t.title}</div>`),T=`<div class="window-content">${t.content||""}${_}</div>`),e.innerHTML=`
...
`,Fe=null,ge=new Set,Je=[];async function pe(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Ts(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await z(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];Fe=Date.now(),te(0,Fe);let l=Date.now(),f=24*60*60*1e3,c=r.filter(d=>{let h=localStorage.getItem(`alert_read_ts_${d.id}`);if(!h)return!0;let y=parseInt(h);return l-y<f});c.sort((d,h)=>{let y=w=>{let p=w.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return"low"}return(p.priority||"low").toLowerCase()},I=w=>w==="critical"?4:w==="high"?3:w==="medium"?2:1,n=I(y(d)),g=I(y(h));return n!==g?g-n:h.timestamp-d.timestamp}),Je=c;let v=d=>{let h=d.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return"low"}return(h.priority||"low").toLowerCase()},C=[];if(c.length===0)C.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let h=new Set(c.map(I=>v(I))).size>1,y=null;c.forEach(I=>{let n=v(I);h&&n!==y&&(C.push({id:`divider-${n}`,type:"divider",label:n.toUpperCase()}),y=n),C.push(I)})}t&&(e.innerHTML="");let u=d=>{let h=d.event;if(typeof h=="string")try{h=JSON.parse(h)}catch{return null}let y=(h.title||"Untitled Alert").trim(),I=(h.body||"No description provided.").trim(),n=h.summary||"",g=h.content||"",w=h.protocol||"",p=(h.priority||"low").toLowerCase(),$=!!h.alert,A=(h.category||"system").trim(),S=h.related_event_ids||[],H=h.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),V=new Date(d.timestamp*1e3),B=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),E=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=q?"event-border-grey":"event-border-blue";!q&&$&&(L="event-border-red"),q&&(p==="high"||p==="critical")?L="event-border-red":q&&p==="medium"&&(L="event-border-orange");let M=q?"alert-read":"alert-unread",O=ge.has(d.id),P=O?"expanded":"",G=O?"display: block;":"display: none;",k="",D="";S.length>0&&(D=`
...
`,be=new Set,kt=[],Me=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){$s();try{let o=await z("/roadmap");if(!o.ok)throw new Error("Offline");let i=await o.json();kt=i;let a=u=>{let x=be.has(u.id),s=u.state==="published",m=u.state==="consumed",T="event-border-grey";s&&(T="event-border-blue"),m&&(T="event-border-purple");let b=new Date(u.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div");d.className=`event-item notification-item ${T} cursor-pointer ${x?"expanded":""}`,d.dataset.itemId=u.id,d.onclick=I=>{if(I.target.closest("button"))return;if(d.classList.contains("expanded")){d.classList.remove("expanded");let g=d.querySelector(".event-details");g&&(g.style.display="none"),be.delete(u.id)}else{d.classList.add("expanded");let g=d.querySelector(".event-details");g&&(g.style.display="block"),be.add(u.id)}};let h=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${u.state.toUpperCase()}]</span>`;d.innerHTML=`
        <div class="event-time">
      ...
      `;let y=d.querySelector(".event-details");return y&&y.addEventListener("click",I=>{I.stopPropagation()}),d.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(u)),d.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ks(u)),d.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(u.id)),d.querySelector(".close-details-btn")?.addEventListener("click",I=>{I.stopPropagation(),d.classList.remove("expanded");let n=d.querySelector(".event-details");n&&(n.style.display="none"),be.delete(u.id)}),d},r=u=>{let x=document.createElement("div");x.innerHTML=J("empty",u.message,u.action,u.id);let s=x.firstElementChild;return s.dataset.itemId=u.id,s},l=[];!i||i.length===0?l.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:N()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(u=>l.push({...u,type:"item"}));let f=Array.from(e.children),c=new Map(f.map(u=>[u.dataset.itemId,u])),v=new Set(l.map(u=>u.id));f.forEach(u=>{let x=u.dataset.itemId;(!x||!v.has(x))&&u.remove()}),t&&(e.innerHTML="");let C=null;l.forEach((u,x)=>{let s=c.get(u.id);(!s||t)&&(s&&s.remove(),u.type==="placeholder"?s=r(u):s=a(u),!s)||(x===0?e.firstElementChild!==s&&e.prepend(s):C&&C.nextElementSibling!==s&&C.after(s),C=s)})}catch{e.children.length===0&&(e.innerHTML=J("offline","Failed to load roadmap.","The event service may be offline."))}}}function $s(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),o=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me=null;let r=document.getElementById("roadmap-editor-input");r&&(r.value="");let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="block")},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="none"),Me=null},o.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input"),l=r?r.value:"";if(!l.trim())return;let f=Me?`/roadmap/${Me}`:"/roadmap",c=Me?"PATCH":"POST";try{await z(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})});let v=document.getElementById("roadmap-editor-container");v&&(v.style.display="none"),ve(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(r=>be.add(r.id)),ve(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{be.clear(),ve(!0)},a.dataset.listenerAttached="true")}function Ss(t){Me=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block",o.scrollIntoView({behavior:"smooth"}))}async function ks(t){let e=t.state==="published"?"draft":"published";try{await z(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(o){console.error(o)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await z(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Lt=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ms();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await z(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];At=r,Lt=Date.now(),te(2,Lt);let l=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let T=(m.title||"Untitled Blueprint").trim(),_=(m.summary||m.body||"No summary provided.").trim(),b=(m.content||"").trim(),d=(m.category||"architecture").trim(),h=(m.related_services||m.affected_services||[]).map(k=>k.trim()),y=(m.implementation_path||[]).map(k=>k.trim()),I=m.source_event_ids||[],n=m.approved===!0,g=new Date(s.timestamp*1e3),w=g.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=g.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ye.has(s.id),A=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${n?"blueprint-approved":""}`,S.dataset.blueprintId=s.id,n&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=n?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[d]||"bx-paint";S.onclick=function(k){let D=this;if(D.classList.contains("expanded")){D.classList.remove("expanded"),ye.delete(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="none")}else{D.classList.add("expanded"),ye.add(s.id);let U=D.querySelector(".event-details");U&&(U.style.display="block")}};let q="";y.length>0&&(q=`
                ...
                `);let B=h.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${h.join(", ")}</span></div>`:"<div></div>",E=N(),L=n?`
            ...
            `;S.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${A}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${j(_)}</div>
            ...
            `;let M=S.querySelector(".blueprint-approve-btn");M&&(M.onclick=async k=>{k.stopPropagation(),M.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await z(`/events/${s.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&xe(!0)}catch(D){console.error("Failed to approve blueprint:",D)}});let O=S.querySelector(".blueprint-delete-btn");O&&(O.onclick=async k=>{k.stopPropagation();let D=!n;O.innerHTML=D?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await z(`/events/${s.id}`,{method:"DELETE"})).ok&&xe(!0)}catch(R){console.error("Failed to delete blueprint:",R)}});let P=S.querySelector(".event-details");P&&P.addEventListener("click",k=>{k.stopPropagation()});let G=S.querySelector(".close-details-btn");return G&&G.addEventListener("click",k=>{k.stopPropagation(),S.classList.remove("expanded");let D=S.querySelector(".event-details");D&&(D.style.display="none"),ye.delete(s.id)}),S},f=s=>{let m=document.createElement("div");m.innerHTML=J("empty",s.message,s.action,s.id);let T=m.firstElementChild;return T.dataset.blueprintId=s.id,T},c=[];r.length===0?c.push({id:"placeholder-empty",type:"placeholder",message:"No architectural blueprints generated yet.",action:"The Imaginator will synthesize these during system idle periods."}):r.forEach(s=>c.push({...s,type:"blueprint"})),t&&(e.innerHTML="");let v=Array.from(e.children),C=new Map(v.map(s=>[s.dataset.blueprintId,s])),u=new Set(c.map(s=>s.id));v.forEach(s=>{let m=s.dataset.blueprintId;(!m||!u.has(m))&&s.remove()});let x=null;c.forEach((s,m)=>{let T=C.get(s.id);(!T||t)&&(T&&T.remove(),s.type==="placeholder"?T=f(s):T=l(s),!T)||(m===0?e.firstElementChild!==T&&e.prepend(T):x&&x.nextElementSibling!==T&&x.after(T),x=T)}),qe(2,r.length),wt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=J("offline","Failed to load blueprints.","The event service may be offline."))}}async function Bt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await z(t);if(!e.ok)return;let i=(await e.json()).events||[],a=0;i.forEach(r=>{let l=r.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&a++}),ht(a)}catch{}}function Ms(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),o=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{At.forEach(i=>ye.add(i.id)),xe(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ye.clear(),xe(!0)},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){o.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await z("/events?type=system.blueprint.generated",{method:"DELETE"}),ye.clear(),xe(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{o.innerHTML="<i class='bx bx-trash'></i> Clear"}}},o.dataset.listenerAttached="true")}var ce="STANDBY",Ht=null,Ie=[],X=null,We=null,_t=()=>`
...
`,Nt=()=>`
    ...
    `,He=[],_e={"313071000877137920":"Owen",dexter:"Dexter"},se=[],he=null;async function De(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),o=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),r=document.getElementById("new-chore-owner"),l=document.getElementById("selected-recipients"),f=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),v=document.getElementById("new-chore-url"),C=document.getElementById("new-chore-schedule"),u=document.getElementById("new-chore-time"),x=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(x&&!x.dataset.initialValueAttached){let n=Intl.DateTimeFormat().resolvedOptions().timeZone,g=!1;for(let w=0;w<x.options.length;w++)if(x.options[w].value===n){g=!0;break}if(!g){let w=document.createElement("option");w.value=n,w.textContent=n.replace("_"," "),x.appendChild(w)}x.value=n,x.dataset.initialValueAttached="true"}C&&s&&!C.dataset.timeListenerAttached&&(C.addEventListener("change",()=>{s.style.display=C.value==="daily"?"block":"none"}),C.dataset.timeListenerAttached="true");let m=document.getElementById("form-title"),T=document.getElementById("form-icon");function _(){if(l){if(se.length===0){l.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}l.innerHTML=se.sort((n,g)=>{let w=n.startsWith("channel:"),p=g.startsWith("channel:");return w&&!p?-1:!w&&p?1:0}).map(n=>{let g=_e[n]||n,w=n.startsWith("channel:");return`
      ...
      `}).join(""),l.querySelectorAll(".remove-recipient").forEach(n=>{n.onclick=()=>{let g=n.dataset.id;se=se.filter(w=>w!==g),_()}})}}function b(n){n&&!se.includes(n)&&(se.push(n),_()),r&&(r.value="")}function d(n=null){if(o){if(he=n?n.id:null,n){if(m&&(m.textContent="Update Research Task"),T&&(T.className="bx bx-edit-alt",T.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=n.natural_instruction),v&&(v.value=n.execution_plan?.entry_url||""),C&&(C.value=n.schedule,s&&(s.style.display=n.schedule==="daily"?"block":"none")),u&&n.run_at&&(u.value=n.run_at),x&&n.timezone){let g=!1;for(let w=0;w<x.options.length;w++)if(x.options[w].value===n.timezone){g=!0;break}if(!g){let w=document.createElement("option");w.value=n.timezone,w.textContent=n.timezone.replace("_"," "),x.appendChild(w)}x.value=n.timezone}se=n.recipients||(n.owner_id?[n.owner_id]:[]),_()}else m&&(m.textContent="Initialize Research Task"),T&&(T.className="bx bx-plus-circle",T.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),v&&(v.value=""),C&&(C.value="every_24h"),se=[],_();o.style.display="block",o.scrollIntoView({behavior:"smooth",block:"start"})}}let h=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(h&&!h.dataset.populated&&!N())try{fe("/contacts").then(async n=>{n.ok&&(((await n.json()).members||[]).forEach(p=>{if(_e[p.id]=p.nickname||p.username,p.id==="313071000877137920")return;let $=document.createElement("option");$.value=p.id;let A=p.nickname||p.username;$.textContent=`${A} (${p.username})`,h.appendChild($)}),h.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!N())try{fe("/channels").then(async n=>{n.ok&&((await n.json()).forEach(w=>{let p=`channel:${w.id}`;_e[p]=w.name;let $=document.createElement("option");$.value=p,$.textContent=`#${w.name} (${w.guild})`,y.appendChild($)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}r&&!r.dataset.listenerAttached&&(r.onchange=()=>{b(r.value)},r.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onchange=()=>I(),f.dataset.listenerAttached="true");function I(){if(!t)return;let n=f?.value||"all",g=n==="all"?He:He.filter(p=>(p.recipients||(p.owner_id?[p.owner_id]:[])).includes(n));if(g.length===0){t.innerHTML=J("empty",n==="all"?"No active tasks.":"No tasks found for this owner.",N()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let w=g.map(p=>{let $=p.last_run===0?"Never":new Date(p.last_run*1e3).toLocaleString(),A=p.memory?p.memory.length:0,S=p.status==="active"?"#03dac6":"#666",W=(p.recipients||(p.owner_id?[p.owner_id]:[])).sort((q,V)=>{let B=q.startsWith("channel:"),E=V.startsWith("channel:");return B&&!E?-1:!B&&E?1:0}).map(q=>{let V=_e[q]||q.substring(0,8),B=q.startsWith("channel:");return`<span title="${V}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${q==="dexter"?"bx-terminal":B?"bx-hash":"bx-user"}'></i>${V}</span>`}).join("<span style='color: #444;'>, </span>");return`
            ...
            `}).join("");t.innerHTML=w,t.querySelectorAll(".edit-chore-btn").forEach(p=>{p.onclick=$=>{$.stopPropagation();let A=p.dataset.id,S=He.find(H=>H.id===A);S&&d(S)}}),t.querySelectorAll(".reset-chore-btn").forEach(p=>{p.onclick=async $=>{$.stopPropagation();let A=p.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(p.innerHTML="<i class='bx bx-loader-alt spin'></i>",await z(`/chores/${A}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),De())}}),t.querySelectorAll(".delete-chore-btn").forEach(p=>{p.onclick=async $=>{$.stopPropagation();let A=p.dataset.id;confirm("Delete this task?")&&(await z(`/chores/${A}`,{method:"DELETE"}),De())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{o&&(o.style.display==="none"||he!==null?d(null):o.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{o&&(o.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let n=document.getElementById("new-chore-instruction"),g=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),p=n?.value,$=w?.value||"every_24h",A=$==="daily"?u?.value:"",S=$==="daily"?x?.value:"";if(!p)return;if(se.length===0){alert("Please add at least one recipient.");return}let H=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=he?"PATCH":"POST",q=he?`/chores/${he}`:"/chores";await z(q,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:p,entry_url:g?.value,schedule:$,run_at:A,timezone:S})}),o&&(o.style.display="none"),n&&(n.value=""),g&&(g.value=""),he=null,se=[],De()}catch(W){console.error(W),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=H}},i.dataset.listenerAttached="true");try{let n=await z("/chores");if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);if(He=await n.json()||[],f){let w=f.value,p=new Set;He.forEach($=>{$.owner_id&&p.add($.owner_id),$.recipients&&$.recipients.forEach(A=>p.add(A))}),f.innerHTML='<option value="all">All Recipients</option>',p.forEach($=>{let A=document.createElement("option");A.value=$;let S=$.startsWith("channel:"),H=_e[$]||`Recipient: ${$.substring(0,8)}`;A.textContent=(S&&!H.startsWith("#")?"#":"")+H,f.appendChild(A)}),f.value=w,f.selectedIndex===-1&&(f.value="all")}I()}catch(n){console.error("Failed to fetch chores",n)}}var Rt=()=>`
...
`;function qt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=_s,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `;let a=e.querySelector(".close-profile-btn");a&&a.addEventListener("click",()=>{e.click()})};fe(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();Ds(e,t,a)}else console.log("Profile not found (404) or error."),o("Profile Data Unavailable")}).catch(i=>{console.error("Profile fetch error:",i),o("Connection Failed")})}function Ds(t,e,o){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let T=(o.attributes||o.facts||[]).map(I=>{let n=I.key||I.k,g=I.value||I.v;return`
            <div class="fact-chip">
                <span class="fact-key">${n}:</span>
                <span class="fact-val">${g}</span>
            </div>
        `}).join(""),_=o.cognitive_model||o,b=_.technical_level||_.techLevel||0,d=_.communication_style||_.commStyle||"Unknown",h=_.patience_level||_.patience||"Unknown",y=_.vibe||"Unknown";return`
        ...
        `},r=()=>{let T=o.dossier||{},_=T.identity||{},b=T.career||{},d=T.personal||{},h=T.social||[];return`
...
`,Ge=null,we=new Set,ze=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals","system.cognitive.model_inference"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart","system.cognitive.model_inference":"bx-brain"};function Os(t){for(let[e,o]of Object.entries(Rs))if(o.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function me(t=!1){let e=document.getElementById("events-timeline");if(!e)return;qs();let i=`/events?ml=${Z==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await z(i);if(!a.ok)throw new Error("Service unreachable");if(ze=((await a.json()).events||[]).filter(x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ge=Date.now(),te(1,Ge),ze.length===0){e.innerHTML=J("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,T=Os(m),_=js(m),b=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.bot.voice_response"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.cognitive.model_inference"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),d="event-border-grey";b&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?d="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"||m==="engagement.decision"?d="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?d="event-border-orange":m==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let h=b?"cursor-pointer":"",y=we.has(x.id),I=y?"expanded":"",n=y?"display: block;":"display: none;",g=new Date(x.timestamp*1e3),w=g.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=g.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Ft(m,s),A=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",S="";if(b){let B="";if(m==="engagement.decision"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                    ...
                    `}else if(m==="system.cognitive.model_inference"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                    ...
                    `}else if(m==="system.attention.expired"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.timestamp-s.last_active,M=Math.floor(L/60);B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let E=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="";if(s.embed){let P=s.embed,G=P.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(P.title)}</div>`:"",k=P.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${le(P.description)}</div>`:"",D=(P.fields||[]).map(R=>`
              <div style="margin-top: 10px;">
            ...
            `}let M="";s.eval_count&&(M=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let P=s.chat_history.length,G=s.chat_history.map((k,D)=>{let R=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&R==="ASSISTANT"&&(R="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=D===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${D}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        ...
                                        <span style="font-size: 0.7em; color: #666;">Turn ${D+1} of ${P}</span>
                                    </div>
                        ...
                        `}
                    `}else if(m==="messaging.bot.voice_response"){let E=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="",M=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let P=O.match(/{.*?}/g);P&&(M=P.map(G=>{let k=G.substring(1,G.length-1),D=k.indexOf(" "),R=k.substring(0,D),U=k.substring(D+1);return U=U.replace(/\[\]$/,"").trim(),{role:R,content:U}}))}if(M.length>0&&s.response_raw&&M.push({role:"assistant",content:s.response_raw}),M.length>0){let P=M.length,G=M.map((k,D)=>{let R=k.role.toUpperCase();R==="ASSISTANT"&&(R="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=D===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${D}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${R}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${D+1} of ${P}</span>
                    ...
                    `}else if(m==="analysis.user.message_signals"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.signals||{};B=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
              ...
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(m==="analysis.router.decision"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let E=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",M=D=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${D}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let P="";if(s.chat_history&&s.chat_history.length>0){let D=s.chat_history.length,R=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ee=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let D=s.corrections.map((R,U)=>`
                        ...
                        `}let k="";if(s.parsed_results&&s.parsed_results.length>0){let D=s.parsed_results.map(R=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${E}; font-weight: bold;">${L} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                                <div style="font-size: 0.65em; text-transform: uppercase; color: #666; letter-spacing: 1px; margin-bottom: 4px;">Duration</div>
                        ...
                        ${G}
                        ${P}
                    `}else if(m==="system.test.completed"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(m==="messaging.user.sent_message"){let E="";s.attachments&&s.attachments.length>0&&(E=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(M=>{let O=M.content_type&&M.content_type.startsWith("image/"),P=(M.size/1024).toFixed(1)+" KB";return`
                ...
                `}let H=document.createElement("div");H.className=`event-item ${d} ${h} ${I}`,H.dataset.eventId=x.id,H.onclick=function(B){if(!b)return;let E=this;if(E.classList.contains("expanded")){E.classList.remove("expanded"),we.delete(x.id);let M=E.querySelector(".event-details");M&&(M.style.display="none")}else{E.classList.add("expanded"),we.add(x.id);let M=E.querySelector(".event-details");M&&(M.style.display="block")}},H.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${w}</span>
                    <span class="event-date">${p}</span>
                </div>
...
`),c=[...l.logs];if(c.length<25){let C=25-c.length;for(let u=0;u<C;u++)c.push("")}else c.length>25&&(c=c.slice(-25));let v=c.map(C=>Ue(C)).join(`
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
        </div>`;async function ss(){Js(),await Promise.all([ne(),Ce(),ot()]),await Ne()}var Ve=null;function Js(){Ve||(Ve=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Ve),Ve=null;return}ne(!0)},1e3))}var Gt=null,Vt=null;async function ns(){try{return await(await z("/system_monitor")).json()}catch{return null}}async function Kt(){try{return await(await z("/system/hardware")).json()}catch{return null}}async function Ws(){try{return await(await z("/processes")).json()}catch{return null}}async function Gs(){try{return await(await z("/agent/status")).json()}catch{return null}}async function Ce(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),o=document.getElementById("global-restart-btn"),i=document.getElementById("global-stop-btn"),a=document.getElementById("global-start-btn"),r=(n,g)=>{n&&!n.dataset.listenerAttached&&(n.onclick=async()=>{let w=n.innerHTML;n.innerHTML="<i class='bx bx-loader-alt spin'></i>",n.disabled=!0;try{await z(`/system/service/${g}`,{method:"POST",body:JSON.stringify({service:"all"})}),setTimeout(()=>Ce(),2e3),setTimeout(()=>Ce(),5e3),setTimeout(()=>{n.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{n.innerHTML=w,n.disabled=!1},1e3)},1e3)}catch(p){console.error(p),n.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{n.innerHTML=w,n.disabled=!1},2e3)}},n.dataset.listenerAttached="true")};r(o,"restart"),r(i,"stop"),r(a,"start");let l=document.querySelector("#hw-os .hw-content"),f=document.querySelector("#hw-cpu .hw-content"),c=document.querySelector("#hw-ram .hw-content"),v=document.querySelector("#hw-gpu .hw-content"),C=document.querySelector("#hw-storage .hw-content"),u=n=>{if(n){if(l&&(l.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                ...
                </div>`}v&&(n.GPU&&n.GPU.length>0?v.innerHTML=n.GPU.map(g=>{let w=(g.VRAM/1073741824).toFixed(1);return`
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):v.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),C&&n.STORAGE&&n.STORAGE.length>0?C.innerHTML=n.STORAGE.map(g=>{let w=(g.USED/1073741824).toFixed(1),p=(g.SIZE/(1024*1024*1024)).toFixed(1),$=g.SIZE>0?(g.USED/g.SIZE*100).toFixed(0):0,A=g.MOUNT_POINT||"Unmounted";return`
            ...
            </div>`}let g=n.status==="online",w=g?"service-widget-online":"service-widget-offline",p=g?"bx-check-circle":"bx-x-circle",$=g?"OK":"BAD",A=n.version?m(n.version.str):"-",S=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",H=n.memory&&n.memory!=="N/A"?n.memory:"-",W=S!=="-"?"#00ff00":"#666",q=S!=="-"?"#fff":"#666",V=H!=="-"?"#00ff00":"#666",B=H!=="-"?"#fff":"#666",E=_(n.uptime),L="",M=n.type!=="os"&&n.type!=="cli"&&n.type!=="prd",O="";M&&!N()&&(O=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                ...
                </div>${O}`:L=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${O}`;let P=N()?"easter.company":T(n.domain&&n.port?`${n.domain}:${n.port}`:"");return`<div class="service-widget ${w}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${p}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${$}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${P}</span></div>${L}</div></div>`}let d=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),h=new Set(s.map(n=>n.id));for(let[n,g]of d)h.has(n)||g.remove();let y=new Set,I=["upstash-redis-rw","upstash-redis-ro","easter-company","easter-company-production","dex-test-service","easter-company-root"];s.forEach(n=>{if(y.has(n.id)||I.includes(n.id))return;y.add(n.id);let g=b(n),w=d.get(n.id);if(w&&w.parentNode)if(n.id==="upstash-redis-ro"){let p=N()&&(re.lastFrontend||re.lastDashboard)||Date.now(),$=new Date(p).toLocaleTimeString(),A=w.querySelector(".sync-time-label");A&&(A.textContent=`Last synced: ${$}`)}else w.outerHTML!==g&&(w.outerHTML=g);else t.insertAdjacentHTML("beforeend",g)})}async function ot(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=J("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),te(2,Vt);let o=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function a(c){let v=c.status==="Ready";return`
                ...
                </div>`}function l(c){let v=c.status==="Downloaded",C=v?"service-widget-online":"service-widget-offline",u=v?"OK":"MISSING",x=v&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",s=c.name;return c.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${C}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${u}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${x}</span></div></div></div>`}let f="";if(i&&(f+=a(i)),e.xtts&&(f+=r(e.xtts)),f+=o.map(l).join(""),!f){t.innerHTML=J("empty","No models found.");return}t.innerHTML!==f&&(t.innerHTML=f)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let o=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),r=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),f=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),v=document.getElementById("fabricator-reset-btn"),C=document.getElementById("courier-reset-btn"),u=document.getElementById("system-pause-toggle-btn");u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{let I=u.querySelector(".bx-play")?"/agent/resume":"/agent/pause";u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z(I,{method:"POST"}),await ne()}catch{u.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},u.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),C&&!C.dataset.listenerAttached&&(C.onclick=async()=>{C.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{C.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{C.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{C.innerHTML="<i class='bx bx-error'></i>"}},C.dataset.listenerAttached="true");let x=document.getElementById("imaginator-reset-btn");x&&!x.dataset.listenerAttached&&(x.onclick=async()=>{x.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await z("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{x.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{x.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{x.innerHTML="<i class='bx bx-error'></i>"}},x.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let y=s.agents.guardian||{protocols:{}},I=s.agents.analyzer||{protocols:{}},n=s.agents.imaginator||{protocols:{}},g=s.agents.fabricator||{protocols:{}},w=s.agents.courier||{protocols:{}},p=s.system||{},$=Math.floor(Date.now()/1e3),A={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},S=k=>{k<0&&(k=0);let D=Math.floor(k/3600),R=Math.floor(k%3600/60),U=k%60;return D>0?`${D}h ${R}m`:R>0?`${R}m ${U}s`:`${U}s`},H=(k,D,R,U,Y)=>{if(!k)return;let K=A[U]||U.toUpperCase(),Ee=k.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(Ee&&(Ee.textContent=K),Y==="paused"){k.textContent="PAUSED",k.style.color="#ff9800";return}if(!R){k.textContent="Inactive",k.style.color="#666";return}let Re=R.status;if(Re==="Working")k.textContent="Working",k.style.color="#bb86fc";else if(Re==="Ready")k.textContent="Ready",k.style.color="#5eff5e";else{let Te=R.cooldown;if(N()&&(Te=R.next_run-$),Te<=0)k.textContent="Ready",k.style.color="#5eff5e";else{let Ze=Math.floor(Te/60),Pe=Te%60;k.textContent=`${Ze}m ${Pe}s`,k.style.color="#fff"}}D&&R.stats&&(D.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
          ...
          `)};o&&H(o,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry",p.state);let W=document.getElementById("analyzer-synthesis-val"),q=document.getElementById("analyzer-synthesis-stats");W&&H(W,q,I.protocols.synthesis,"synthesis",p.state);let V=document.getElementById("imaginator-alert_review-val"),B=document.getElementById("imaginator-alert_review-stats");V&&H(V,B,n.protocols.alert_review,"alert_review",p.state);let E=document.getElementById("fabricator-construction-val"),L=document.getElementById("fabricator-construction-stats");E&&H(E,L,g.protocols.construction,"construction",p.state);let M=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");M&&H(M,O,w.protocols.researcher,"researcher",p.state);let P=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&p.state){let k=p.state,D=S(p.state_time||0);P&&(P.textContent=`State: ${k.toUpperCase()}`),G.textContent=D,k==="idle"?G.style.color=p.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",u&&(k==="paused"?(u.innerHTML="<i class='bx bx-play'></i>",u.title="Resume System",u.style.color="#ff9800"):(u.innerHTML="<i class='bx bx-pause'></i>",u.title="Pause System",u.style.color=""))}a&&(a.textContent=S(p.metrics?.total_idle_time||0)),r&&(r.textContent=S(p.metrics?.total_active_time||0)),l&&(l.textContent=S(p.metrics?.total_waste_time||0))}else[o,i,a,r,l].forEach(I=>{I&&(I.textContent="-",I.style.color="#666")});if(t)return;let m=await Ws(),T=[],_=[],b=[];m&&(Array.isArray(m)?T=m:(T=m.active||[],_=m.queue||[],b=m.history||[],b.sort((y,I)=>(I.end_time||0)-(y.end_time||0)))),e&&(T.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=J("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),at(e,T,!1)));let d=document.getElementById("processes-queue-widgets");d&&(_.length===0?!d.querySelector(".tab-placeholder")&&!d.querySelector('div[style*="font-style: italic"]')&&(d.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(d.innerHTML="",at(d,_,!1)));let h=document.getElementById("processes-history-widgets");h&&(b.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=J("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),at(h,b,!0))),qe(1,T.length)}function at(t,e,o){function i(c){let v="";c.end_time?v=`${c.end_time-c.start_time}s`:v=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let C=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",u=c.channel_id,x={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent","system-test":"System Validation","voice-mode":"Voice Interaction"};if(x[u]?u=x[u]:/^\d+$/.test(u)&&(u=`Channel ${u}`),o)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ks=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `);let r=document.getElementById("terminal-cmd-name");r&&(r.textContent=`dex ${t.id}`);let l=document.getElementById("terminal-status-badge");l&&(l.className="terminal-status status-running",l.textContent="Running");let f=document.getElementById("terminal-action-btn");return f&&(f.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",lt=!0,o}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),ie()||(document.body.style.overflow=""),lt=!1}function ls(t,e,o="output"){if(!lt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${o}`,o==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Ue(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ys(t){let e=cs.find(l=>l.id===t);if(!e)return;let o=Zs(e);o&&ls(o,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let l of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let f="output";l.includes("[ERROR]")?f="error":l.includes("[SUCCESS]")||l.includes("\u2713")?f="success":l.includes("!")&&(f="warning"),o&&ls(o,l,f)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let r=document.getElementById("terminal-action-btn");r&&(r.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ks();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code"),i=o?o.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(o=>{let i=o;o.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Ys(a)})})}async function Xs(){if(!N())try{if(!(await z("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Xs(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],o=document.getElementById("windows-container");o&&o.setAttribute("data-count","0");let i=b=>{localStorage.setItem("dex_last_window",b)};function a(){for(;e.length>1;)e.shift().close(!0);let b=document.getElementById("windows-container"),d=document.querySelector("nav"),h=document.querySelector("footer"),y=window.location.pathname==="/"||window.location.pathname==="/index.html",I=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");b&&b.setAttribute("data-count",e.length.toString()),e.forEach(S=>{let H=document.getElementById(S.id);H&&H.classList.remove("hide-close")});let n=document.getElementById("dexter-menu-container"),g=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon"),p=document.getElementById("docs-icon"),$=document.getElementById("close-all-windows"),A=window.innerWidth<880;if(!A){let S=document.getElementById("dexter-dropdown"),H=document.getElementById("dexter-menu-btn");S&&S.classList.remove("active"),H&&H.classList.remove("active"),b&&b.classList.remove("menu-open")}if($e(e.length>0),e.length>0)if(h?.classList.add("hide"),$&&($.style.display=A?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),b&&(b.style.paddingTop="60px"),n&&(n.style.display=A?"flex":"none"),p&&(p.style.display=A?"block":"none"),w&&(w.style.display=A?"block":"none"),!A&&g){let S=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(S)?(g.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),r(f)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),r(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),r(C)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),r(v)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),r(u)})):g.innerHTML=""}else g&&(g.innerHTML="");else d?.classList.remove("window-open"),$&&($.style.display="none"),b&&(b.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),ie()||(document.body.style.overflow=""),y||I?h?.classList.remove("hide"):h?.classList.add("hide"),n&&(n.style.display="flex"),p&&(p.style.display="block"),w&&(w.style.display="block"),g&&(g.innerHTML="");Le()}function r(b){if(b.isOpen()){b.close();return}for(;e.length>0;)e.pop().close(!0);e.push(b),b.open(),a()}function l(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",a);let f=ue({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>pe(),onClose:()=>{let b=e.indexOf(f);b>-1&&e.splice(b,1),a()}}),c=ue({id:"events-window",title:"Events",icon:"bx-calendar-event",content:it(),onOpen:()=>{c.setContent(it()),me()},onClose:()=>{let b=e.indexOf(c);b>-1&&e.splice(b,1),a()}}),v=ue({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ut(),onOpen:()=>nt(),onClose:()=>{let b=e.indexOf(v);b>-1&&e.splice(b,1),a()}}),C=ue({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(b=>N()?b.title!=="Hardware"&&b.title!=="Logs":!0),onOpen:()=>{Ce(),ne(),ot(),os(),Ne()},onClose:()=>{let b=e.indexOf(C);b>-1&&e.splice(b,1),a()}}),u=ue({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Rt()},{icon:"bx-map-alt",title:"Roadmap",content:Pt()},{icon:"bx-paint",title:"Blueprints",content:Ot()},{icon:"bx-task",title:"Tasks",content:jt()}],onOpen:()=>st(),onClose:()=>{let b=e.indexOf(u);b>-1&&e.splice(b,1),a()}}),x=ue({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ke(),onOpen:()=>{x.setContent(Ke()),rt(x)},onClose:()=>{let b=e.indexOf(x);b>-1&&e.splice(b,1),a()}});window.dexter={viewEvent:b=>{c.isOpen()||r(c),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${b}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:b=>{f.isOpen()||r(f),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${b}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let b=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");b&&d&&(b.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let b=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),h=document.getElementById("docs-icon"),y=document.getElementById("close-all-windows"),I=document.getElementById("nav-left-container"),n=document.getElementById("dexter-dropdown"),g=document.getElementById("windows-container");b&&n&&(b.onclick=p=>{p.stopPropagation();let $=window.innerWidth<880;if($)n.style.top="",n.style.left="",n.style.right="",n.style.transform="";else{let S=b.getBoundingClientRect(),W=document.querySelector("nav").getBoundingClientRect(),q=220,V=10,E=S.left+S.width/2-q/2,L=W.right-10;E+q>L&&(E=L-q),E<10&&(E=10),n.style.top=W.bottom+V+"px",n.style.left=E+"px",n.style.right="auto",n.style.transform="none"}n.classList.toggle("active"),b.classList.toggle("active");let A=n.classList.contains("active");if($){let S=document.querySelector("nav");A?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),S?.classList.add("window-open"),g?.classList.add("menu-open"),$e(!0)):(g?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S?.classList.remove("window-open"),ie()||$e(!1)))}}),d&&(d.onclick=p=>{p.stopPropagation(),r(x)}),h&&(h.onclick=p=>{p.stopPropagation(),window.location.href="/docs"}),y&&(y.onclick=p=>{p.stopPropagation(),l()});let w=(p,$,A)=>{let S=document.getElementById(p);S&&(S.onclick=H=>{H.stopPropagation(),s(),A&&i(A),r($)})};w("alerts-menu-item",f,"alerts-window"),w("events-menu-item",c,"events-window"),w("monitor-menu-item",C,"monitor-window"),w("contacts-menu-item",v,"contacts-window"),w("workspace-menu-item",u,"workspace-window"),I&&(I.onclick=p=>{if(p.stopPropagation(),n&&n.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1));return}if(e.length>0)l();else{let $=window.location.pathname;if(!($==="/"||$==="/index.html")){let S=$.endsWith("/")&&$.length>1?$.slice(0,-1):$;if(S.includes("/docs/page/")){window.location.href="/";return}let H=S.split("/");H.pop();let W=H.join("/")||"/";window.location.href=W}}}),document.addEventListener("click",()=>{let p=window.innerWidth<880;n&&n.classList.contains("active")&&(s(),p&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1)))}),n&&(n.onclick=p=>p.stopPropagation())})();let T=window.location.pathname==="/"||window.location.pathname==="/index.html",_=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!T&&!_&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{c.isOpen()&&me(),f.isOpen()?pe():tt()},1e3),setInterval(()=>{N()&&(f.isOpen()?pe():tt()),u.isOpen()?st():Bt(),N()&&c.isOpen()&&me(),C.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

"use strict";(()=>{function at(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function ot(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var lt=1e3;function pe(t){let e=null,s=t.onClose||null,i=t.onOpen||null;function n(){e&&(e.style.zIndex=(++lt).toString())}function r(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",l),i&&setTimeout(i,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=(++lt).toString(),e.addEventListener("mousedown",n);let v=t.icon||"bx-window",a="",y="",$;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((d,f)=>{let g=d.icon?`<i class="bx ${d.icon}"></i>`:`<span class="tab-glyph">${d.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,$=`<div class="window-content">${t.tabs.map((d,f)=>`<div class="tab-content ${f===0?"active":""}" data-tab-content="${f}">${d.content}</div>`).join("")}</div>`):(t.title&&(y=`<div class="window-title">${t.title}</div>`),$=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,De=null,ue=new Set,ze=[];async function ce(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ws(),t&&(e.innerHTML="<p>Updating...</p>");let s="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await _(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];De=Date.now(),ee(0,De);let l=Date.now(),u=24*60*60*1e3,c=r.filter(f=>{let g=localStorage.getItem(`alert_read_ts_${f.id}`);if(!g)return!0;let x=parseInt(g);return l-x<u});c.sort((f,g)=>{let x=k=>{let E=k.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},M=k=>k==="critical"?4:k==="high"?3:k==="medium"?2:1,A=M(x(f)),I=M(x(g));return A!==I?I-A:g.timestamp-f.timestamp}),ze=c;let m=f=>{let g=f.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return"low"}return(g.priority||"low").toLowerCase()},b=[],v=new Set(c.map(f=>m(f))).size>1;if(c.length>0){let f=null;c.forEach(g=>{let x=m(g);v&&x!==f&&(b.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),f=x),b.push(g)})}if(t&&(e.innerHTML=""),c.length===0){e.innerHTML=N("empty","No alerts yet."),_e();return}let a=f=>{let g=f.event;if(typeof g=="string")try{g=JSON.parse(g)}catch{return null}let x=(g.title||"Untitled Alert").trim(),M=(g.body||"No description provided.").trim(),A=g.summary||"",I=g.content||"",k=g.protocol||"",E=(g.priority||"low").toLowerCase(),D=!!g.alert,j=(g.category||"system").trim(),O=g.related_event_ids||[],q=g.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${f.id}`),S=new Date(f.timestamp*1e3),C=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),h=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=J?"event-border-grey":"event-border-blue";!J&&D&&(T="event-border-red"),J&&(E==="high"||E==="critical")?T="event-border-red":J&&E==="medium"&&(T="event-border-orange");let L=J?"alert-read":"alert-unread",H=ue.has(f.id),R=H?"expanded":"",V=H?"display: block;":"display: none;",U="",F="";O.length>0&&(F=`
...
`,fe=new Set,Ne=[],$e=null;async function ge(t=!1){let e=document.getElementById("roadmap-list");if(e){Cs();try{let s=await _("/roadmap");if(!s.ok)throw new Error("Offline");let i=await s.json();Ne=i;let n=m=>{let b=fe.has(m.id),p=m.state==="published",v=m.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),v&&(a="event-border-purple");let $=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${a} cursor-pointer ${b?"expanded":""}`,w.dataset.itemId=m.id,w.onclick=f=>{if(f.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),fe.delete(m.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),fe.add(m.id)}};let o=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;w.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${z(m.content)}</div>
      ...
      `;let d=w.querySelector(".event-details");return d&&d.addEventListener("click",f=>{f.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>Es(m)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>$s(m)),w.querySelector(".delete-btn")?.addEventListener("click",()=>Ts(m.id)),w.querySelector(".close-details-btn")?.addEventListener("click",f=>{f.stopPropagation(),w.classList.remove("expanded");let g=w.querySelector(".event-details");g&&(g.style.display="none"),fe.delete(m.id)}),w},r=Array.from(e.children),l=new Map(r.map(m=>[m.dataset.itemId,m])),u=new Set(i.map(m=>m.id));if(r.forEach(m=>{let b=m.dataset.itemId;(!b||!u.has(b))&&m.remove()}),!Ne||Ne.length===0){e.innerHTML=N("empty","Your roadmap is empty.",B()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((m,b)=>{let p=l.get(m.id);(!p||t)&&(p&&p.remove(),p=n(m),!p)||(b===0?e.firstElementChild!==p&&e.prepend(p):c&&c.nextElementSibling!==p&&c.after(p),c=p)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function Cs(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{$e=null;let r=document.getElementById("roadmap-editor-input");r&&(r.value="");let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="none"),$e=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input"),l=r?r.value:"";if(!l.trim())return;let u=$e?`/roadmap/${$e}`:"/roadmap",c=$e?"PATCH":"POST";try{await _(u,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:l})});let m=document.getElementById("roadmap-editor-container");m&&(m.style.display="none"),ge(!0)}catch(m){console.error(m)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{Ne.forEach(r=>fe.add(r.id)),ge(!0)},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{fe.clear(),ge(!0)},n.dataset.listenerAttached="true")}function Es(t){$e=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function $s(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ge(!0)}catch(s){console.error(s)}}async function Ts(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),fe.delete(t),ge(!0)}catch(e){console.error(e)}}var Et=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ct=null,be=new Set,Tt=[];async function ve(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ss();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await _(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];if(Tt=r,Ct=Date.now(),ee(2,Ct),r.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Me(2,0);return}t&&(e.innerHTML="");let l=p=>{let v=p.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let a=(v.title||"Untitled Blueprint").trim(),y=(v.summary||v.body||"No summary provided.").trim(),$=(v.content||"").trim(),w=(v.category||"architecture").trim(),o=(v.related_services||v.affected_services||[]).map(H=>H.trim()),d=(v.implementation_path||[]).map(H=>H.trim()),f=v.source_event_ids||[],g=v.approved===!0,x=new Date(p.timestamp*1e3),M=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=be.has(p.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${g?"blueprint-approved":""}`,E.dataset.blueprintId=p.id,g&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let j=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(H){let R=this;if(R.classList.contains("expanded")){R.classList.remove("expanded"),be.delete(p.id);let U=R.querySelector(".event-details");U&&(U.style.display="none")}else{R.classList.add("expanded"),be.add(p.id);let U=R.querySelector(".event-details");U&&(U.style.display="block")}};let O="";d.length>0&&(O=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${d.map(H=>`<li style="margin-bottom: 5px;">${z(H)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";f.length>0&&(q=`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${k}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${z(y)}</div>
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async H=>{H.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${p.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ve(!0)}catch(R){console.error("Failed to approve blueprint:",R)}});let h=E.querySelector(".blueprint-delete-btn");h&&(h.onclick=async H=>{H.stopPropagation();let R=!g;h.innerHTML=R?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${p.id}`,{method:"DELETE"})).ok&&ve(!0)}catch(V){console.error("Failed to delete blueprint:",V)}});let T=E.querySelector(".event-details");T&&T.addEventListener("click",H=>{H.stopPropagation()});let L=E.querySelector(".close-details-btn");return L&&L.addEventListener("click",H=>{H.stopPropagation(),E.classList.remove("expanded");let R=E.querySelector(".event-details");R&&(R.style.display="none"),be.delete(p.id)}),E},u=Array.from(e.children),c=new Map(u.map(p=>[p.dataset.blueprintId,p])),m=new Set(r.map(p=>p.id));u.forEach(p=>{let v=p.dataset.blueprintId;(!v||!m.has(v))&&p.remove()});let b=null;r.forEach((p,v)=>{let a=c.get(p.id);if(!a||t){a&&a.remove();let y=l(p);if(!y)return;a=y}v===0?e.firstElementChild!==a&&e.prepend(a):b&&b.nextElementSibling!==a&&b.after(a),b=a}),Me(2,r.length),bt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function St(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(r=>{let l=r.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{l={}}l.approved!==!0&&n++}),gt(n)}catch{}}function Ss(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Tt.forEach(i=>be.add(i.id)),ve(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{be.clear(),ve(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),be.clear(),ve(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var ae="STANDBY",kt=null,Te=[],Z=null,Re=null,Mt=()=>`
...
`,It=()=>`
    ...
    `;async function Pe(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn"),r=document.getElementById("new-chore-owner"),l=document.getElementById("task-owner-label");if(r&&!r.dataset.populated&&!B())try{Ee("/contacts").then(async u=>{u.ok&&(((await u.json()).members||[]).forEach(b=>{if(b.id==="313071000877137920")return;let p=document.createElement("option");p.value=b.id;let v=b.nickname||b.username;p.textContent=`${v} (${b.username})`,r.appendChild(p)}),r.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(r&&l&&!r.dataset.listenerAttached&&(r.onchange=()=>{let u=r.options[r.selectedIndex];u&&(l.textContent=`Report result to: ${u.text.split(" (")[0]}`)},r.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let u=document.getElementById("new-chore-instruction"),c=document.getElementById("new-chore-url"),m=document.getElementById("new-chore-owner"),b=u?.value,p=c?.value,v=m?.value||"313071000877137920";if(b){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:v,natural_instruction:b,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),u&&(u.value=""),c&&(c.value=""),Pe()}catch(a){console.error(a),alert("Failed to create research task")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let u=await _("/chores");if(!u.ok)throw new Error(`HTTP error! status: ${u.status}`);let c=await u.json();if(!Array.isArray(c)||c.length===0){t.innerHTML=N("empty","No active tasks.",B()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let m=c.map(b=>{let p=b.last_run===0?"Never":new Date(b.last_run*1e3).toLocaleString(),v=b.memory?b.memory.length:0,a=b.status==="active"?"#03dac6":"#666";return`
                <div class="service-widget" style="border-left: 3px solid ${a}; width: 100%;">
                    <div class="service-widget-header">
                        <i class='bx bx-search-alt' style="color: ${a}"></i>
                        <h3 style="font-size: 0.95em; white-space: normal; line-height: 1.4;">${b.natural_instruction}</h3>
                        <div style="margin-left: auto; display: flex; gap: 10px;">
                            <button class="icon-btn delete-chore-btn" data-id="${b.id}" style="background: none; border: none; color: #cf6679; cursor: pointer; padding: 5px;"><i class='bx bx-trash'></i></button>
                        </div>
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">Schedule:</span>
                            <span class="info-value">${b.schedule}</span>
                        </div>
                        <div class="service-widget-info">
                        ...
                        <div class="service-widget-info" style="grid-column: span 2; margin-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 5px;">
                            ...
                            <span class="info-value" style="font-size: 0.8em; opacity: 0.8; word-break: break-all;">${b.execution_plan.entry_url||"Auto-detected"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=m,t.querySelectorAll(".delete-chore-btn").forEach(b=>{b.onclick=async p=>{p.stopPropagation();let v=b.dataset.id;confirm("Delete this task?")&&(await _(`/chores/${v}`,{method:"DELETE"}),Pe())}})}catch(u){console.error("Failed to fetch chores",u)}}var Bt=()=>`
...
`;async function Ge(){await Promise.all([ge(),ve(),Se(),Pe()])}var Is=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function zt(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Nt(t){if(!document.getElementById("dex-profile-styles")){let s=document.createElement("style");s.id="dex-profile-styles",s.textContent=Is,document.head.appendChild(s)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",s=>{s.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ee(`/profile/${t.id}`).then(async s=>{if(s.ok){let i=await s.json();Ve(e,t,i)}else{console.log("Profile not found or error, using mock data.");let i=zt(t.id,t.username);Ve(e,t,i)}}).catch(s=>{console.error("Profile fetch error:",s);let i=zt(t.id,t.username);Ve(e,t,i)})}function Ve(t,e,s){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(s.attributes||s.facts||[]).map(x=>{let M=x.key||x.k,A=x.value||x.v;return`
        ...
        `}).join(""),w=s.cognitive_model||s,o=w.technical_level||w.techLevel||0,d=w.communication_style||w.commStyle||"Unknown",f=w.patience_level||w.patience||"Unknown",g=w.vibe||"Unknown";return`
        ...
        `},r=()=>{let $=s.dossier||{},w=$.identity||{},o=$.career||{},d=$.personal||{},f=$.social||[];return`
    ...
    `;t.innerHTML=m;let b=t.querySelector(".profile-card"),p=t.querySelector("#profile-expand-toggle"),v=t.querySelectorAll(".profile-tab-btn"),a=t.querySelectorAll(".tab-content"),y=t.querySelector(".close-profile-btn");y&&y.addEventListener("click",()=>{t.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>t.remove(),300)}),v.forEach($=>{$.addEventListener("click",()=>{v.forEach(d=>d.classList.remove("active")),a.forEach(d=>d.classList.remove("active")),$.classList.add("active");let w=$.dataset.tab,o=t.querySelector(`#tab-${w}`);o&&o.classList.add("active")})}),p&&p.addEventListener("click",()=>{b.classList.toggle("expanded");let $=b.classList.contains("expanded");p.innerHTML=$?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Rt=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,Bs=null;async function Ke(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ke(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let n=s.target.closest(".user-profile-card");if(n&&n.dataset.user)try{let r=JSON.parse(n.dataset.user);Nt(r)}catch(r){console.error("Failed to parse user data",r)}},t.dataset.listenerAttached="true");try{let s=await Ee("/contacts");if(!s.ok)throw new Error("Service unreachable");let n=(await s.json()).members||[];if(Bs=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((l,u)=>{let c=r[l.level]??10,m=r[u.level]??10;return c!==m?c-m:l.username.localeCompare(u.username)}),t.innerHTML=n.map(l=>{let u=l.color&&l.color!==0,c=u?"#"+l.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",m=l.status==="online"?"#5eff5e":l.status==="idle"?"#ffa500":l.status==="dnd"?"#ff4d4d":"#666",b="#888";l.level==="Me"||l.level==="Master"?b="#bb86fc":l.level==="Admin"?b="#cf6679":l.level==="Moderator"?b="#03dac6":l.level==="Contributor"&&(b="#ffa500");let p=l.level==="Me",v=p?"rgba(187, 134, 252, 0.08)":u?`${c}11`:"rgba(255,255,255,0.03)",a=p?"2px solid #bb86fc":u?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(l).replace(/"/g,"&quot;")}"
                     style="background: ${v}; padding: 15px; border-radius: 8px; border: ${a}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s, box-shadow 0.2s;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${p?"#bb86fc":u?c:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${l.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":u?`2px solid ${c}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${b}; font-weight: 600; text-transform: uppercase;">${p?"DEXTER (ME)":l.level}</span>
...
`,Oe=null,ye=new Set,Le=[],K="all",Hs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ds={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function zs(t){for(let[e,s]of Object.entries(Hs))if(s.includes(t))return e;return"system"}function Ns(t){return Ds[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Rs();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await _(i);if(!n.ok)throw new Error("Service unreachable");if(Le=((await n.json()).events||[]).filter(v=>{let a=v.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let y=a.type;return!(y==="system.blueprint.generated"||y==="system.notification.generated")}),Oe=Date.now(),ee(1,Oe),Le.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=v=>{let a=v.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let y=a.type,$=zs(y),w=Ns(y),o=y==="engagement.decision"||y==="messaging.bot.sent_message"||y==="messaging.user.sent_message"||y==="moderation.explicit_content.deleted"||y==="analysis.link.completed"||y==="analysis.visual.completed"||y==="analysis.router.decision"||y==="analysis.user.message_signals"||y==="system.cli.command"||y==="system.analysis.audit"||y==="system.test.completed"||y==="error_occurred"||y==="system.cli.status"||y==="system.attention.expired"||y.startsWith("system.roadmap")||y.startsWith("system.process"),d="event-border-grey";o&&(y==="moderation.explicit_content.deleted"||y==="error_occurred"?d="event-border-red":y==="analysis.link.completed"||y==="analysis.visual.completed"||y==="analysis.router.decision"||y==="system.analysis.audit"||y==="analysis.user.message_signals"?d="event-border-purple":y==="system.attention.expired"||y==="system.cli.command"||y==="system.cli.status"?d="event-border-orange":y==="system.test.completed"?d=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let f=o?"cursor-pointer":"",g=ye.has(v.id),x=g?"expanded":"",M=g?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Pt(y,a),D=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",j="";if(o){let S="";if(y==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                    ...
                    `}else if(y==="system.attention.expired"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=a.timestamp-a.last_active,T=Math.floor(h/60);S=`
                    ...
                    `}else if(y==="messaging.bot.sent_message"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h="";a.eval_count&&(h=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `}
                    `}else if(y==="analysis.user.message_signals"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h=a.signals||{};S=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(y==="moderation.explicit_content.deleted"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="analysis.link.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="analysis.visual.completed"){let C=T=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${T}</h5>`,h="";a.base64_preview?h=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&(h=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(y==="analysis.router.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="system.cli.command"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="system.analysis.audit"){let C=a.success?"#03dac6":"#ff4d4d",h=a.success?"SUCCESS":"FAILED",T=U=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${U}</h5>`,L="";a.error&&(L=`
                            <div class="event-detail-block">
                                ${T("Error")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${C}; font-weight: bold;">${h} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${a.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${R}
                        ${H}
                    `}else if(y==="system.test.completed"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row">
                    ...
                    `}else if(y==="error_occurred"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(y==="system.cli.status"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(y==="messaging.user.sent_message"){let C="";a.attachments&&a.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${d} ${f} ${x}`,O.dataset.eventId=v.id,O.onclick=function(S){if(!o)return;let C=this;if(C.classList.contains("expanded")){C.classList.remove("expanded"),ye.delete(v.id);let T=C.querySelector(".event-details");T&&(T.style.display="none")}else{C.classList.add("expanded"),ye.add(v.id);let T=C.querySelector(".event-details");T&&(T.style.display="block")}},O.innerHTML=`
                <div class="event-time">
...
`,Ot=Ps;function jt(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Os=null;async function Ae(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let s=await e.json();if(!s||s.length===0)return t.innerHTML=N("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],n=s.filter(l=>!i.includes(l.id));n.forEach(l=>{l.logs&&Array.isArray(l.logs)?l.logs.reverse():l.logs=[]}),n.reverse();let r=n.map(l=>{let u=l.logs.join(`
`),c=[...l.logs];if(c.length<25){let b=25-c.length;for(let p=0;p<b;p++)c.push("")}else c.length>25&&(c=c.slice(-25));let m=c.map(b=>He(b)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=r,document.querySelectorAll(".copy-logs-btn").forEach(l=>{l.addEventListener("click",()=>{let u=l,c=unescape(u.dataset.logs||"");navigator.clipboard.writeText(c).then(()=>{let m=u.querySelector("i");m&&(m.classList.remove("bx-copy"),m.classList.add("bx-check"),setTimeout(()=>{m.classList.remove("bx-check"),m.classList.add("bx-copy")},2e3))})})}),document.querySelectorAll(".clear-logs-btn").forEach(l=>{l.addEventListener("click",async()=>{let c=l.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${c}?`))try{(await _(`/logs?service_id=${c}`,{method:"DELETE"})).ok&&Ae()}catch(m){console.error("Error clearing logs:",m)}})}),Os=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=N("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Jt=()=>{let t=B()?"display: none;":"";return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${jt()}
                ...
                </div>`}if(i&&o.CPU&&o.CPU.length>0){let d=o.CPU[0];i.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}r&&(o.GPU&&o.GPU.length>0?r.innerHTML=o.GPU.map(d=>{let f=(d.VRAM/1073741824).toFixed(1);return`
                    ...
                    </div>`}).join(""):l&&(l.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await Ft();o?(u(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),i&&!i.hasChildNodes())){let o=await Ft();u(o)}if(!t)return;let c=await Xt();if(!c||!c.services){t.children.length===0&&(t.innerHTML=N("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let d=o.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Ut=B()&&ie.lastDashboard||Date.now(),ee(0,Ut||0);let m=c.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let g=59-new Date().getSeconds();g<=0&&(g+=60),o.textContent=`${g}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function b(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:o.split(".").slice(0,3).join(".")||"-"}function p(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function v(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,g=parseInt(d[2])||0,x=parseInt(d[3])||0,M=parseFloat(d[4])||0,A=f*86400+g*3600+x*60+M,I=Math.floor(A/86400);if(I>0)return`${I}d`;let k=Math.floor(A/3600);if(k>0)return`${k}h`;let E=Math.floor(A/60);return E>0?`${E}m`:`${Math.floor(A)}s`}function a(o){if(o.id==="upstash-redis-ro"){let J=B()&&(ie.lastFrontend||ie.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=o.status==="online",f=d?"service-widget-online":"service-widget-offline",g=d?"bx-check-circle":"bx-x-circle",x=d?"OK":"BAD",M=o.version?b(o.version.str):"-",A=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",I=o.memory&&o.memory!=="N/A"?o.memory:"-",k=A!=="-"?"#00ff00":"#666",E=A!=="-"?"#fff":"#666",D=I!=="-"?"#00ff00":"#666",j=I!=="-"?"#fff":"#666",O=v(o.uptime),q="";d?q=`
                <div class="service-widget-info">
                ...
                </div>`:q='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let G=B()?"easter.company":p(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${f}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${G}</span></div>${q}</div></div>`}let y=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),$=new Set(m.map(o=>o.id));for(let[o,d]of y)$.has(o)||d.remove();let w=new Set;m.forEach(o=>{if(w.has(o.id))return;w.add(o.id);let d=a(o),f=y.get(o.id);if(f&&f.parentNode)if(o.id==="upstash-redis-ro"){let g=B()&&(ie.lastFrontend||ie.lastDashboard)||Date.now(),x=new Date(g).toLocaleTimeString(),M=f.querySelector(".sync-time-label");M&&(M.textContent=`Last synced: ${x}`)}else f.outerHTML!==d&&(f.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Xt();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}qt=Date.now(),ee(2,qt);let s=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function n(c){let m=c.status==="Ready";return`
                ...
                </div>`}function r(c){let m=c.status==="Ready";return`
                ...
                </div>`}function l(c){let m=c.status==="Downloaded",b=m?"service-widget-online":"service-widget-offline",p=m?"OK":"MISSING",v=m&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",a=c.name;return c.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${b}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let u="";if(i&&(u+=n(i)),e.xtts&&(u+=r(e.xtts)),u+=s.map(l).join(""),!u){t.innerHTML=N("empty","No models found.");return}t.innerHTML!==u&&(t.innerHTML=u)}async function oe(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let s=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),r=document.getElementById("guardian-total-active"),l=document.getElementById("guardian-total-waste"),u=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),m=document.getElementById("fabricator-reset-btn"),b=document.getElementById("system-pause-toggle-btn");b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{let f=b.querySelector(".bx-play")?"/agent/resume":"/agent/pause";b.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _(f,{method:"POST"}),await oe()}catch{b.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>oe(),2e3)}},b.dataset.listenerAttached="true"),u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{u.innerHTML="<i class='bx bx-error'></i>"}},u.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/fabricator/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{m.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{m.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),oe()}catch{m.innerHTML="<i class='bx bx-error'></i>"}},m.dataset.listenerAttached="true");let p=await qs();if(p&&p.agents){let d=p.agents.guardian||{protocols:{}},f=p.agents.analyzer||{protocols:{}},g=p.agents.imaginator||{protocols:{}},x=p.agents.fabricator||{protocols:{}},M=p.system||{},A=Math.floor(Date.now()/1e3),I={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction"},k=h=>{h<0&&(h=0);let T=Math.floor(h/3600),L=Math.floor(h%3600/60),H=h%60;return T>0?`${T}h ${L}m`:L>0?`${L}m ${H}s`:`${H}s`},E=(h,T,L,H)=>{if(!h)return;let R=I[H]||H.toUpperCase(),V=h.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(V&&(V.textContent=R),!L){h.textContent="Inactive",h.style.color="#666";return}let U=L.status;if(U==="Working")h.textContent="Working",h.style.color="#bb86fc";else if(U==="Ready")h.textContent="Ready",h.style.color="#5eff5e";else{let F=L.cooldown;if(B()&&(F=L.next_run-A),F<=0)h.textContent="Ready",h.style.color="#5eff5e";else{let W=Math.floor(F/60),re=F%60;h.textContent=`${W}m ${re}s`,h.style.color="#fff"}}T&&L.stats&&(T.innerHTML=`
          ...
          `)};s&&E(s,document.getElementById("guardian-sentry-stats"),d.protocols.sentry,"sentry");let D=document.getElementById("analyzer-synthesis-val"),j=document.getElementById("analyzer-synthesis-stats");D&&E(D,j,f.protocols.synthesis,"synthesis");let O=document.getElementById("imaginator-alert_review-val"),q=document.getElementById("imaginator-alert_review-stats");O&&E(O,q,g.protocols.alert_review,"alert_review");let G=document.getElementById("fabricator-construction-val"),J=document.getElementById("fabricator-construction-stats");G&&E(G,J,x.protocols.construction,"construction");let S=document.getElementById("system-state-label"),C=document.getElementById("system-state-val");if(C&&M.state){let h=M.state,T=k(M.state_time||0);S&&(S.textContent=`State: ${h.toUpperCase()}`),C.textContent=T,h==="idle"?C.style.color=M.state_time>300?"#5eff5e":"#fff":C.style.color="#bb86fc",b&&(h==="paused"?(b.innerHTML="<i class='bx bx-play'></i>",b.title="Resume System",b.style.color="#ff9800"):(b.innerHTML="<i class='bx bx-pause'></i>",b.title="Pause System",b.style.color=""))}n&&(n.textContent=k(M.metrics?.total_idle_time||0)),r&&(r.textContent=k(M.metrics?.total_active_time||0)),l&&(l.textContent=k(M.metrics?.total_waste_time||0))}else[s,i,n,r,l].forEach(f=>{f&&(f.textContent="-",f.style.color="#666")});if(t)return;let v=await Us(),a=[],y=[],$=[];v&&(Array.isArray(v)?a=v:(a=v.active||[],y=v.queue||[],$=v.history||[],$.sort((d,f)=>(f.end_time||0)-(d.end_time||0)))),e&&(a.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=N("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),Ye(e,a,!1)));let w=document.getElementById("processes-queue-widgets");w&&(y.length===0?!w.querySelector(".tab-placeholder")&&!w.querySelector('div[style*="font-style: italic"]')&&(w.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(w.innerHTML="",Ye(w,y,!1)));let o=document.getElementById("processes-history-widgets");o&&($.length===0?o.querySelector(".tab-placeholder")||(o.innerHTML=N("empty","No recent history.")):(o.querySelector(".tab-placeholder")&&(o.innerHTML=""),Ye(o,$,!0))),Me(1,a.length)}function Ye(t,e,s){function i(c){let m="";c.end_time?m=`${c.end_time-c.start_time}s`:m=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let b=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",p=c.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(v[p]?p=v[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),s)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}-${c.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${m}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let n=s?".process-history-item":".process-widget",r=new Map(Array.from(t.querySelectorAll(n)).map(c=>[c.dataset.channelId,c])),l=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,m]of r)c&&!l.has(c)&&m.remove();let u=new Set;e.forEach(c=>{let m=`${c.channel_id}-${c.start_time}`;if(u.has(m))return;u.add(m);let b=i(c),p=r.get(m);if(p&&p.parentNode){p.outerHTML!==b&&(p.outerHTML=b);let v=t.querySelector(`[data-channel-id="${m}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",b)})}function Ue(){let t=Fe(),e="Notification"in window,s={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function ts(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Qt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Qt(t)}async function Qt(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=N("empty","No web history found.");return}let i=`
        ...
        `;s.forEach((u,c)=>{let m=new Date(u.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),b=u.screenshot?`<img src="data:image/png;base64,${u.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';i+=`
                <div class="web-card" style="flex: 0 0 100%; height: 100%; display: flex; flex-direction: column; scroll-snap-align: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-sizing: border-box; max-width: 100%; overflow: hidden;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; flex-shrink: 0;">
                        ...
                        <span style="color: #888; font-size: 0.8em; font-family: monospace;">${m}</span>
                    </div>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Fs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,i]of Object.entries(t)){let n=is.filter(r=>r.category===s);n.length!==0&&(e+=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>ss());let m=document.getElementById("terminal-action-btn");m&&(m.onclick=()=>ss())}let s=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");s&&(s.innerHTML="");let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`
    ...
    `);let r=document.getElementById("terminal-cmd-name");r&&(r.textContent=`dex ${t.id}`);let l=document.getElementById("terminal-status-badge");l&&(l.className="terminal-status status-running",l.textContent="Running");let u=document.getElementById("terminal-action-btn");return u&&(u.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",tt=!0,s}function ss(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",tt=!1}function ns(t,e,s="output"){if(!tt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${s}`,s==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let n=He(e);i.innerHTML=n||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ws(t){let e=is.find(l=>l.id===t);if(!e)return;let s=Js(e);s&&ns(s,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let l of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let u="output";l.includes("[ERROR]")?u="error":l.includes("[SUCCESS]")||l.includes("\u2713")?u="success":l.includes("!")&&(u="warning"),s&&ns(s,l,u)}let n=document.getElementById("terminal-status-badge");n&&(n.className="terminal-status status-success",n.textContent="Completed (Demo)");let r=document.getElementById("terminal-action-btn");r&&(r.style.display="block")}function as(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Fs();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let s=e.querySelector("code"),i=s?s.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let n=e.querySelector("i");n&&(n.className="bx bx-check",n.style.color="#5eff5e",setTimeout(()=>{n.className="bx bx-copy",n.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(s=>{let i=s;s.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),s.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),s.addEventListener("click",()=>{let n=i.dataset.cmd;n&&Ws(n)})})}async function Gs(){if(!B())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function os(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}ot(),rt(),Gs(),mt(),at();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&as();let e=[],s=document.getElementById("windows-container");s&&s.setAttribute("data-count","0");let i=o=>{localStorage.setItem("dex_last_window",o)};function n(){for(;e.length>1;)e.shift().close(!0);let o=document.getElementById("windows-container"),d=document.querySelector("nav"),f=document.querySelector("footer"),g=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");o&&o.setAttribute("data-count",e.length.toString()),e.forEach(D=>{let j=document.getElementById(D.id);j&&j.classList.remove("hide-close")});let M=document.getElementById("dexter-menu-container"),A=document.getElementById("nav-window-switcher"),I=document.getElementById("settings-icon"),k=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let D=document.getElementById("dexter-dropdown"),j=document.getElementById("dexter-menu-btn");D&&D.classList.remove("active"),j&&j.classList.remove("active"),o&&o.classList.remove("menu-open")}if(xe(e.length>0),e.length>0)if(f?.classList.add("hide"),k&&(k.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),o&&(o.style.paddingTop="60px"),M&&(M.style.display=E?"flex":"none"),I&&(I.style.display=E?"block":"none"),!E&&A){let D=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(D)?(A.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),r(u)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),r(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),r(b)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),r(m)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),r(p)})):A.innerHTML=""}else A&&(A.innerHTML="");else d?.classList.remove("window-open"),k&&(k.style.display="none"),o&&(o.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",g||x?f?.classList.remove("hide"):f?.classList.add("hide"),M&&(M.style.display="flex"),I&&(I.style.display="block"),A&&(A.innerHTML="");Ce()}function r(o){if(o.isOpen()){o.close();return}for(;e.length>0;)e.pop().close(!0);e.push(o),o.open(),n()}function l(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",n);let u=pe({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:xt(),onOpen:()=>ce(),onClose:()=>{let o=e.indexOf(u);o>-1&&e.splice(o,1),n()}}),c=pe({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Ze(),onOpen:()=>{c.setContent(Ze()),de()},onClose:()=>{let o=e.indexOf(c);o>-1&&e.splice(o,1),n()}}),m=pe({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Rt(),onOpen:()=>Ke(),onClose:()=>{let o=e.indexOf(m);o>-1&&e.splice(o,1),n()}}),b=pe({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Gt()},{icon:"bxs-component",title:"Processes",content:Wt()},{icon:"bxs-brain",title:"Models",content:Vt()},{icon:"bx-globe",title:"Web",content:es()},{icon:"bxs-hdd",title:"Hardware",content:Kt()},{icon:"bxs-terminal",title:"Logs",content:Zt()},{icon:"bxs-zap",title:"Agents",content:Jt()}].filter(o=>B()?o.title!=="Hardware"&&o.title!=="Logs":!0),onOpen:()=>{Xe(),oe(),Qe(),ts(),Ae()},onClose:()=>{let o=e.indexOf(b);o>-1&&e.splice(o,1),n()}}),p=pe({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Bt()},{icon:"bx-map-alt",title:"Roadmap",content:_t()},{icon:"bx-paint",title:"Blueprints",content:Ht()},{icon:"bx-task",title:"Tasks",content:Dt()}],onOpen:()=>Ge(),onClose:()=>{let o=e.indexOf(p);o>-1&&e.splice(o,1),n()}}),v=pe({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ue(),onOpen:()=>{v.setContent(Ue()),et(v)},onClose:()=>{let o=e.indexOf(v);o>-1&&e.splice(o,1),n()}});window.dexter={viewEvent:o=>{c.isOpen()||r(c),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${o}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:o=>{u.isOpen()||r(u),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${o}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let o=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");o&&d&&(o.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let o=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),f=document.getElementById("close-all-windows"),g=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),M=document.getElementById("windows-container");o&&x&&(o.onclick=I=>{I.stopPropagation();let k=window.innerWidth<880;if(k)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let D=o.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),q=220,G=10,S=D.left+D.width/2-q/2,C=O.right-10;S+q>C&&(S=C-q),S<10&&(S=10),x.style.top=O.bottom+G+"px",x.style.left=S+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),o.classList.toggle("active");let E=x.classList.contains("active");if(k){let D=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),D?.classList.add("window-open"),M?.classList.add("menu-open"),xe(!0)):(M?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),D?.classList.remove("window-open"),xe(!1)))}}),d&&(d.onclick=I=>{I.stopPropagation(),r(v)}),f&&(f.onclick=I=>{I.stopPropagation(),l()});let A=(I,k,E)=>{let D=document.getElementById(I);D&&(D.onclick=j=>{j.stopPropagation(),a(),E&&i(E),r(k)})};A("alerts-menu-item",u,"alerts-window"),A("events-menu-item",c,"events-window"),A("monitor-menu-item",b,"monitor-window"),A("contacts-menu-item",m,"contacts-window"),A("workspace-menu-item",p,"workspace-window"),g&&(g.onclick=I=>{if(I.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),xe(!1));return}if(e.length>0)l();else{let k=window.location.pathname;if(!(k==="/"||k==="/index.html")){let j=(k.endsWith("/")&&k.length>1?k.slice(0,-1):k).split("/");j.pop();let O=j.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let I=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),I&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),xe(!1)))}),x&&(x.onclick=I=>I.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",w=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!w&&document.querySelector("footer")?.classList.add("hide"),B()||setInterval(()=>{c.isOpen()&&de(),u.isOpen()?ce():We()},1e3),setInterval(()=>{B()&&(u.isOpen()?ce():We()),p.isOpen()?Ge():St(),B()&&c.isOpen()&&de(),b.isOpen()&&Yt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",os):os();})();

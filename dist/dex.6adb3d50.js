(()=>{function et(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function tt(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var it=1e3;function oe(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++it)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",r),s&&setTimeout(s,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++it,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",a="",g="",S;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((o,d)=>{let m=o.icon?`<i class="bx ${o.icon}"></i>`:`<span class="tab-glyph">${o.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,S=`<div class="window-content">${t.tabs.map((o,d)=>`<div class="tab-content ${d===0?"active":""}" data-tab-content="${d}">${o.content}</div>`).join("")}</div>`):(t.title&&(g=`<div class="window-title">${t.title}</div>`),S=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,Ie=null,le=new Set,Be=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;cs(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await _(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ie=Date.now(),X(0,Ie);let r=Date.now(),v=24*60*60*1e3,l=c.filter(d=>{let m=localStorage.getItem(`alert_read_ts_${d.id}`);if(!m)return!0;let x=parseInt(m);return r-x<v});l.sort((d,m)=>{let x=T=>{let E=T.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},A=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,L=A(x(d)),k=A(x(m));return L!==k?k-L:m.timestamp-d.timestamp}),Be=l;let f=d=>{let m=d.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return"low"}return(m.priority||"low").toLowerCase()},y=[],b=new Set(l.map(d=>f(d))).size>1;if(l.length>0){let d=null;l.forEach(m=>{let x=f(m);b&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(m)})}if(t&&(e.innerHTML=""),l.length===0){e.innerHTML=H("empty","No alerts yet."),Le();return}let a=d=>{let m=d.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let x=(m.title||"Untitled Alert").trim(),A=(m.body||"No description provided.").trim(),L=m.summary||"",k=m.content||"",T=m.protocol||"",E=(m.priority||"low").toLowerCase(),I=!!m.alert,U=(m.category||"system").trim(),O=m.related_event_ids||[],F=m.audit_event_id,j=!!localStorage.getItem(`alert_read_ts_${d.id}`),C=new Date(d.timestamp*1e3),w=C.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=C.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=j?"event-border-grey":"event-border-blue";!j&&I&&(M="event-border-red"),j&&(E==="high"||E==="critical")?M="event-border-red":j&&E==="medium"&&(M="event-border-orange");let N=j?"alert-read":"alert-unread",D=le.has(d.id),q=D?"expanded":"",J=D?"display: block;":"display: none;",W="",G="";O.length>0&&(G=`
            ...
            <div style="flex: 1; min-width: 120px; text-align: center;">
                ...
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; display: inline-block;">
                    <a href="#" onclick="window.dexter.viewEvent('${F}'); return false;" style="color: #bb86fc; text-decoration: none; border-bottom: 1px dashed rgba(187, 134, 252, 0.3); white-space: nowrap;">${F.substring(0,8)}...</a>
                </div>
            </div>`);let fe="";T&&(fe=`
        ...
        `:Q=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${re(A)}</div>
            </div>
        `,W=`
                <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                    ...
                    <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="metadata-value" style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee;">${U}</div>
                    </div>
                    ${fe}
                    ${V}
                    ${G}
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${N} ${q} cursor-pointer priority-${E}`,Z.dataset.alertId=d.id,Z.onclick=function(ie){if(this.classList.contains("expanded")){this.classList.remove("expanded"),le.delete(d.id);let Ce=this.querySelector(".event-details");Ce&&(Ce.style.display="none")}else{this.classList.add("expanded"),le.add(d.id);let Ce=this.querySelector(".event-details");if(Ce&&(Ce.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Pe="event-border-grey";E==="high"||E==="critical"?Pe="event-border-red":E==="medium"&&(Pe="event-border-orange"),this.classList.add(Pe),Le()}}};let Ye=`${T?T.toUpperCase():"GUARDIAN"} ALERT: ${L||x}`,Kt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[U]||"bx-bell",Zt=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Zt}; font-size: 0.8em; margin-left: 5px;">[${E.toUpperCase()}]</span>
                    </div>
...
`,ce=new Set,_e=[],ye=null;async function de(t=!1){let e=document.getElementById("roadmap-list");if(e){ds();try{let i=await _("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();_e=s;let n=f=>{let y=ce.has(f.id),p=f.state==="published",b=f.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),b&&(a="event-border-purple");let S=new Date(f.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=f.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",ce.delete(f.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",ce.add(f.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${f.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
      ...
      `;let o=h.querySelector(".event-details");return o&&o.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>ps(f)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ms(f)),h.querySelector(".delete-btn")?.addEventListener("click",()=>us(f.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",ce.delete(f.id)}),h},c=Array.from(e.children),r=new Map(c.map(f=>[f.dataset.itemId,f])),v=new Set(s.map(f=>f.id));if(c.forEach(f=>{let y=f.dataset.itemId;(!y||!v.has(y))&&f.remove()}),!_e||_e.length===0){e.innerHTML=H("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let l=null;s.forEach((f,y)=>{let p=r.get(f.id);(!p||t)&&(p&&p.remove(),p=n(f),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):l&&l.nextElementSibling!==p&&l.after(p),l=p)})}catch{e.children.length===0&&(e.innerHTML=H("offline","Failed to load roadmap.","The event service may be offline."))}}}function ds(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ye=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",ye=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let r=ye?`/roadmap/${ye}`:"/roadmap",v=ye?"PATCH":"POST";try{await _(r,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",de(!0)}catch(l){console.error(l)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{_e.forEach(c=>ce.add(c.id)),de(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ce.clear(),de(!0)},n.dataset.listenerAttached="true")}function ps(t){ye=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function ms(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),de(!0)}catch(i){console.error(i)}}async function us(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),ce.delete(t),de(!0)}catch(e){console.error(e)}}var xt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,vt=null,pe=new Set,ht=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;fs();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await _(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(ht=c,vt=Date.now(),X(2,vt),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),$e(2,0);return}t&&(e.innerHTML="");let r=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),S=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(D=>D.trim()),o=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],m=b.approved===!0,x=new Date(p.timestamp*1e3),A=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=pe.has(p.id),T=k?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${m?"blueprint-approved":""}`,E.dataset.blueprintId=p.id,m&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=m?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";E.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),pe.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),pe.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let O="";o.length>0&&(O=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${o.map(D=>`<li style="margin-bottom: 5px;">${R(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let F="";d.length>0&&(F=`
                ...
                `);let B=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",j=z(),C=m?`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${T}">
                        ...
                        <div class="detail-pre" style="margin-bottom: 15px;">${R(g)}</div>

                        <h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">Technical Details</h5>
                        <div class="event-detail-block" style="text-align: left;">
                            <div class="detail-pre">${R(S)}</div>
                        </div>
                        ${O}
                        ${F}
                        ${C}
            ...
            `;let w=E.querySelector(".blueprint-approve-btn");w&&(w.onclick=async D=>{D.stopPropagation(),w.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await _(`/events/${p.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&me(!0)}catch(q){console.error("Failed to approve blueprint:",q)}});let $=E.querySelector(".blueprint-delete-btn");$&&($.onclick=async D=>{D.stopPropagation();let q=!m;$.innerHTML=q?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await _(`/events/${p.id}`,{method:"DELETE"})).ok&&me(!0)}catch(J){console.error("Failed to delete blueprint:",J)}});let M=E.querySelector(".event-details");M&&M.addEventListener("click",D=>{D.stopPropagation()});let N=E.querySelector(".close-details-btn");return N&&N.addEventListener("click",D=>{D.stopPropagation(),E.classList.remove("expanded");let q=E.querySelector(".event-details");q&&(q.style.display="none"),pe.delete(p.id)}),E},v=Array.from(e.children),l=new Map(v.map(p=>[p.dataset.blueprintId,p])),f=new Set(c.map(p=>p.id));v.forEach(p=>{let b=p.dataset.blueprintId;(!b||!f.has(b))&&p.remove()});let y=null;c.forEach((p,b)=>{let a=l.get(p.id);(!a||t)&&(a&&a.remove(),a=r(p),!a)||(b===0?e.firstElementChild!==a&&e.prepend(a):y&&y.nextElementSibling!==a&&y.after(a),y=a)}),$e(2,c.length),pt()}catch(s){console.error("Error fetching blueprints:",s),e.children.length===0&&(e.innerHTML=H("offline","Failed to load blueprints.","The event service may be offline."))}}async function wt(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await _(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&n++}),dt(n)}catch{}}function fs(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),i=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ht.forEach(s=>pe.add(s.id)),me(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{pe.clear(),me(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){i.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await _("/events?type=system.blueprint.generated",{method:"DELETE"}),pe.clear(),me(!0)}catch(s){console.error("Failed to clear blueprints:",s)}finally{i.innerHTML="<i class='bx bx-trash'></i> Clear"}}},i.dataset.listenerAttached="true")}var Ct=()=>`
...
`;async function je(){await Promise.all([de(),me()])}var $t=()=>`
    ...
    `;async function he(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,v=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:v,natural_instruction:c,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",he()}catch(l){console.error(l),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let c=await _("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);let r=await c.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=H("empty","No active chores.","Create one to start monitoring.");return}let v=r.map(l=>{let f=l.last_run===0?"Never":new Date(l.last_run*1e3).toLocaleString(),y=l.memory?l.memory.length:0,p=l.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=v,t.querySelectorAll(".delete-chore-btn").forEach(l=>{l.onclick=async f=>{f.stopPropagation(),confirm("Delete this chore?")&&(await _(`/chores/${l.dataset.id}`,{method:"DELETE"}),he())}})}catch(c){console.error("Failed to fetch chores",c)}}var gs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function St(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function Et(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=gs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ae(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();qe(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=St(t.id,t.username);qe(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=St(t.id,t.username);qe(e,t,s)})}function qe(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let S=(i.badges||[]).map(A=>`<span class="profile-badge ${A==="Creator"?"master":""}">${A}</span>`).join(""),h=(i.attributes||i.facts||[]).map(A=>{let L=A.key||A.k,k=A.value||A.v;return`
            <div class="fact-chip">
                <span class="fact-key">${L}:</span>
                <span class="fact-val">${k}</span>
            </div>
        `}).join(""),u=i.cognitive_model||i,o=u.technical_level||u.techLevel||0,d=u.communication_style||u.commStyle||"Unknown",m=u.patience_level||u.patience||"Unknown",x=u.vibe||"Unknown";return`
        ...
        `},c=()=>{let S=i.dossier||{},h=S.identity||{},u=S.career||{},o=S.personal||{},d=S.social||[];return`
...
`,bs=null;async function Fe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Fe(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);Et(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Ae("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(bs=Date.now(),n.length===0){t.innerHTML=H("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,v)=>{let l=c[r.level]??10,f=c[v.level]??10;return l!==f?l-f:r.username.localeCompare(v.username)}),t.innerHTML=n.map(r=>{let v=r.color&&r.color!==0,l=v?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",f=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let p=r.level==="Me",b=p?"rgba(187, 134, 252, 0.08)":v?`${l}11`:"rgba(255,255,255,0.03)",a=p?"2px solid #bb86fc":v?`1px solid ${l}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(r).replace(/"/g,"&quot;")}"
                     style="background: ${b}; padding: 15px; border-radius: 8px; border: ${a}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s, box-shadow 0.2s;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${p?"#bb86fc":v?l:"transparent"}33, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${p?"2px solid #bb86fc":v?`2px solid ${l}`:"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${v?l:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.username}</h3>
...
`,De=null,ue=new Set,Se=[],K="all",xs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},ys={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function hs(t){for(let[e,i]of Object.entries(xs))if(i.includes(t))return e;return"system"}function ws(t){return ys[t]||"bx-square-rounded"}async function ae(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Cs();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await _(s);if(!n.ok)throw new Error("Service unreachable");if(Se=((await n.json()).events||[]).filter(b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),De=Date.now(),X(1,De),Se.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=b=>{let a=b.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,S=hs(g),h=ws(g),u=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),o="event-border-grey";u&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?o="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?o="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?o="event-border-orange":g==="system.test.completed"?o=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":o="event-border-blue");let d=u?"cursor-pointer":"",m=ue.has(b.id),x=m?"expanded":"",A=m?"display: block;":"display: none;",L=new Date(b.timestamp*1e3),k=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Tt(g,a),I=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",U="";if(u){let C="";if(g==="engagement.decision"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                    ...
                    `}else if(g==="system.attention.expired"){let w=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$=a.timestamp-a.last_active,M=Math.floor($/60);C=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let w=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$="";a.eval_count&&($=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let N=a.chat_history.length,D=a.chat_history.map((q,J)=>{let W=q.name?q.name.toUpperCase():q.role.toUpperCase();!q.name&&W==="ASSISTANT"&&(W="DEXTER");let G=q.role==="user"?"#03dac6":q.role==="system"?"#ffb74d":"#bb86fc",V=J===N-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${V};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${G}; letter-spacing: 1px; font-weight: bold;">${W}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${J+1} of ${N}</span>
                                    </div>
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${R(q.content)}</div>
                                </div>
                            `}).join("");M=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let w=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$=a.signals||{};C=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        <div class="event-detail-block">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                ${($.topics||[]).map(N=>`<span class="profile-badge">${N}</span>`).join("")||'<span style="color: #666;">No topics mapped.</span>'}
                            </div>
                        </div>
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.visual.completed"){let w=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,$="";a.base64_preview?$=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&($=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let w=a.success?"#03dac6":"#ff4d4d",$=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,N="";a.error&&(N=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${R(a.error)}</pre>
                            </div>
                        `);let D="";if(a.chat_history&&a.chat_history.length>0){let W=a.chat_history.length,G=a.chat_history.map((V,fe)=>{let Q=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Q==="USER"&&(Q="SYSTEM"),!V.name&&Q==="ASSISTANT"&&(Q="AGENT");let Z=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                                <div class="history-slide" data-index="${fe}" style="display: ${fe===0?"block":"none"};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; text-transform: uppercase; color: ${Z}; letter-spacing: 1px; font-weight: bold;">${Q}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${fe+1} of ${W}</span>
                                    </div>
                        ...
                        `}let q="";if(a.corrections&&a.corrections.length>0){let W=a.corrections.map((G,V)=>`
                        ...
                        `}let J="";if(a.parsed_results&&a.parsed_results.length>0){let W=a.parsed_results.map(G=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${w}; font-weight: bold;">${$} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${a.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${q}
                        ${D}
                    `}else if(g==="system.test.completed"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let w=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let w="";a.attachments&&a.attachments.length>0&&(w=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${a.attachments.map(M=>{let N=M.content_type&&M.content_type.startsWith("image/"),D=(M.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ${N?`<div class="attachment-thumb-container"><img src="${M.url}" alt="${M.filename}" class="attachment-thumb"></div>`:`<div class="attachment-icon-container"><i class='bx bx-file attachment-icon'></i></div>`}
                                    <div class="attachment-info">
                                        <a href="${M.url}" target="_blank" class="attachment-link">${M.filename}</a>
                                        <span class="attachment-meta">${M.content_type} \u2022 ${D}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),C=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${o} ${d} ${x}`,O.dataset.eventId=b.id,O.onclick=function(C){if(!u)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),ue.delete(b.id);let $=this.querySelector(".event-details");$&&($.style.display="none")}else{this.classList.add("expanded"),ue.add(b.id);let $=this.querySelector(".event-details");$&&($.style.display="block")}},O.innerHTML=`
                <div class="event-time">
            ...
            `;let F=O.querySelector(".event-details");if(F&&F.addEventListener("click",C=>{C.stopPropagation()}),u){let C=O.querySelector(".close-details-btn");C&&C.addEventListener("click",w=>{w.stopPropagation();let $=w.target.closest(".event-item");if($){$.classList.remove("expanded"),ue.delete(b.id);let M=$.querySelector(".event-details");M&&(M.style.display="none")}})}let B=O.querySelector(".prev-btn"),j=O.querySelector(".next-btn");if(B&&j){let C=0,w=O.querySelectorAll(".history-slide"),$=w.length,M=()=>{w.forEach((N,D)=>{N.style.display=D===C?"block":"none"}),B.disabled=C===0,j.disabled=C===$-1,B.style.opacity=C===0?"0.5":"1",j.style.opacity=C===$-1?"0.5":"1"};B.addEventListener("click",N=>{N.stopPropagation(),C>0&&(C--,M())}),j.addEventListener("click",N=>{N.stopPropagation(),C<$-1&&(C++,M())}),M()}return O},l=Array.from(e.children),f=new Map(l.map(b=>[b.dataset.eventId,b])),y=new Set(Se.map(b=>b.id));l.forEach(b=>{let a=b.dataset.eventId;(!a||!y.has(a))&&b.remove()});let p=null;Se.forEach((b,a)=>{let g=f.get(b.id);(!g||t)&&(g&&g.remove(),g=v(b),!g)||(a===0?e.firstElementChild!==g&&e.prepend(g):p&&p.nextElementSibling!==g&&p.after(g),p=g)}),De=Date.now(),X(1,De)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=H("offline","Failed to load events.","The event service may be offline."))}}function Cs(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Se.forEach(n=>ue.add(n.id)),ae(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ue.clear(),ae(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active")),n.classList.add("active"),K=n.dataset.filter,ae(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===K?n.classList.add("active"):n.classList.remove("active")});let s=document.getElementById("events-clear");s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let n=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let c="/events";if(K!=="all"?c+=`?category=${K}`:c+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await _(c,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ue.clear(),ae(!0)}catch(c){console.error("Failed to clear events:",c),alert("Failed to clear events. Check console.")}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var $s=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Lt=$s;function Mt(){return`
    ...
    `}var Ss=null;async function Ee(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await _("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=H("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let s=["local-ollama-0","local-cache-0"],n=i.filter(r=>!s.includes(r.id));n.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),n.reverse();let c=n.map(r=>{let v=r.logs.join(`
...
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let v=unescape(r.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let l=r.querySelector("i");l.classList.remove("bx-copy"),l.classList.add("bx-check"),setTimeout(()=>{l.classList.remove("bx-check"),l.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let v=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await _(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&Ee()}catch(l){console.error("Error clearing logs:",l)}})}),Ss=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=H("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var _t=()=>{let t=z()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Mt()}
        </div>`;async function Rt(){Es(),await Promise.all([we(),Ge(),Ve(),he()]),await Ee()}var ze=null;function Es(){ze||(ze=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(ze),ze=null;return}we(!0)},1e3))}var At=null,It=null;async function Ot(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function Bt(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function ks(){try{return await(await _("/processes")).json()}catch{return null}}async function Ts(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Ge(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),v=o=>{if(o){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="padding: 15px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            ...
                            <span style="font-size: 0.75em; color: #666; font-family: monospace; background: rgba(0,0,0,0.2); padding: 2px 6px; border-radius: 4px;">${L}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; font-size: 0.75em; color: #888; margin-bottom: 6px; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px;">
                            <span>${m} GB Used</span>
                            <span>${x} GB Total</span>
                        </div>
                        <div style="background: rgba(255,255,255,0.05); height: 6px; border-radius: 3px; overflow: hidden;">
                             <div style="background: ${A>90?"#cf6679":"#03dac6"}; width: ${A}%; height: 100%; box-shadow: 0 0 10px ${A>90?"#cf667944":"#03dac644"};"></div>
                        </div>
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await Bt();o?(v(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let o=await Bt();v(o)}if(!t)return;let l=await Ot();if(!l||!l.services){t.children.length===0&&(t.innerHTML=H("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let d=o.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}At=z()&&se.lastDashboard||Date.now(),X(0,At);let f=l.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let x=59-new Date().getSeconds();x<=0&&(x+=60),o.textContent=`${x}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function y(o){return!o||o==="N/A"||o==="unknown"||o.trim()===""?"-":o}function p(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:o.split(".").slice(0,3).join(".")||"-"}function b(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function a(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let m=parseInt(d[1])||0,x=parseInt(d[2])||0,A=parseInt(d[3])||0,L=parseFloat(d[4])||0,k=m*86400+x*3600+A*60+L,T=Math.floor(k/86400);if(T>0)return`${T}d`;let E=Math.floor(k/3600);if(E>0)return`${E}h`;let I=Math.floor(k/60);return I>0?`${I}m`:`${Math.floor(k)}s`}function g(o){if(o.id==="upstash-redis-ro"){let C=z()&&(se.lastFrontend||se.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                <div class="service-widget-body" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 0;">
                    ...
                    <div class="sync-time-label" style="font-size: 0.6em; color: #444; margin-top: 10px;">Last synced: ${new Date(C).toLocaleTimeString()}</div>
                </div>
            </div>`}let d=o.status==="online",m=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",A=d?"OK":"BAD",L=o.version?p(o.version.str):"-",k=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",T=o.memory&&o.memory!=="N/A"?o.memory:"-",E=k!=="-"?"#00ff00":"#666",I=k!=="-"?"#fff":"#666",U=T!=="-"?"#00ff00":"#666",O=T!=="-"?"#fff":"#666",F=a(o.uptime),B="";d?B=`
                <div class="service-widget-info">
                ...
                <div class="service-widget-footer">
                    ...
                    <div class="service-widget-item">
                        ...
                        <span style="color: ${I};">${k}</span>
                    </div>
                    <div class="service-widget-item">
                        <i class="bx bxs-chip" style="color: ${U};"></i>
                        <span style="color: ${O};">${T}</span>
                    </div>
                </div>`:B='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let j=z()?"easter.company":b(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${m}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${A}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${j}</span></div>${B}</div></div>`}let S=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),h=new Set(f.map(o=>o.id));for(let[o,d]of S)h.has(o)||d.remove();let u=new Set;f.forEach(o=>{if(u.has(o.id))return;u.add(o.id);let d=g(o),m=S.get(o.id);if(m&&m.parentNode)if(o.id==="upstash-redis-ro"){let x=z()&&(se.lastFrontend||se.lastDashboard)||Date.now(),A=new Date(x).toLocaleTimeString(),L=m.querySelector(".sync-time-label");L&&(L.textContent=`Last synced: ${A}`)}else m.outerHTML!==d&&(m.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Ve(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ot();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}It=Date.now(),X(2,It);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let f=l.status==="Ready";return`
                ...
                </div>`}function r(l){let f=l.status==="Downloaded",y=f?"service-widget-online":"service-widget-offline",p=f?"OK":"MISSING",b=f&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",a=l.name;return l.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${f?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(s&&(v+=n(s)),e.xtts&&(v+=c(e.xtts)),v+=i.map(r).join(""),!v){t.innerHTML=H("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function we(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),s=document.getElementById("guardian-architect-val"),n=document.getElementById("guardian-idle-val"),c=document.getElementById("guardian-total-idle"),r=document.getElementById("guardian-total-active"),v=document.getElementById("guardian-total-waste"),l=document.getElementById("guardian-reset-btn"),f=document.getElementById("analyzer-reset-btn");l&&!l.dataset.listenerAttached&&(l.onclick=async()=>{l.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{l.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{l.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),we()}catch{l.innerHTML="<i class='bx bx-error'></i>"}},l.dataset.listenerAttached="true"),f&&!f.dataset.listenerAttached&&(f.onclick=async()=>{f.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{f.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{f.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),we()}catch{f.innerHTML="<i class='bx bx-error'></i>"}},f.dataset.listenerAttached="true");let y=await Ts();if(y&&y.agents){let u=y.agents.guardian||{protocols:{}},o=y.agents.analyzer||{protocols:{}},d=y.agents.imaginator||{protocols:{}},m=y.system||{},x=Math.floor(Date.now()/1e3),A={sentry:"Sentry",architect:"Architect",synthesis:"Synthesis",alert_review:"Alert Review"},L=B=>{B<0&&(B=0);let j=Math.floor(B/3600),C=Math.floor(B%3600/60),w=B%60;return j>0?`${j}h ${C}m`:C>0?`${C}m ${w}s`:`${w}s`},k=(B,j,C,w)=>{if(!B)return;let $=A[w]||w.toUpperCase(),M=B.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(M&&(M.textContent=$),!C){B.textContent="Inactive",B.style.color="#666";return}let N=C.status;if(N==="Working")B.textContent="Working",B.style.color="#bb86fc";else if(N==="Ready")B.textContent="Ready",B.style.color="#5eff5e";else{let D=C.cooldown;if(z()&&(D=C.next_run-x),D<=0)B.textContent="Ready",B.style.color="#5eff5e";else{let q=Math.floor(D/60),J=D%60;B.textContent=`${q}m ${J}s`,B.style.color="#fff"}}j&&C.stats&&(j.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
              <span>Runs: ${C.stats.runs||0}</span>
              <span style="color: ${C.stats.failures>0?"#ffa500":"#666"}">Fails: ${C.stats.failures||0}</span>
              <span style="color: ${C.stats.aborted>0?"#ff4d4d":"#666"}">Aborted: ${C.stats.aborted||0}</span>
            </div>
          `)};i&&k(i,document.getElementById("guardian-sentry-stats"),u.protocols.sentry,"sentry"),s&&k(s,document.getElementById("guardian-architect-stats"),u.protocols.architect,"architect");let T=document.getElementById("analyzer-synthesis-val"),E=document.getElementById("analyzer-synthesis-stats");T&&k(T,E,o.protocols.synthesis,"synthesis");let I=document.getElementById("imaginator-alert_review-val"),U=document.getElementById("imaginator-alert_review-stats");I&&k(I,U,d.protocols.alert_review,"alert_review");let O=document.getElementById("system-state-label"),F=document.getElementById("system-state-val");if(F&&m.state){let B=m.state,j=L(m.state_time||0);O&&(O.textContent=`State: ${B.toUpperCase()}`),F.textContent=j,B==="idle"?F.style.color=m.state_time>300?"#5eff5e":"#fff":F.style.color="#bb86fc"}c&&(c.textContent=L(m.metrics?.total_idle_time||0)),r&&(r.textContent=L(m.metrics?.total_active_time||0)),v&&(v.textContent=L(m.metrics?.total_waste_time||0))}else[i,s,n,c,r,v].forEach(o=>{o&&(o.textContent="-",o.style.color="#666")});if(t)return;let p=await ks(),b=[],a=[],g=[];p&&(Array.isArray(p)?b=p:(b=p.active||[],a=p.queue||[],g=p.history||[],g.sort((u,o)=>(o.end_time||0)-(u.end_time||0)))),b.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=H("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),We(e,b,!1));let S=document.getElementById("processes-queue-widgets");S&&(a.length===0?!S.querySelector(".tab-placeholder")&&!S.querySelector('div[style*="font-style: italic"]')&&(S.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(S.innerHTML="",We(S,a,!1)));let h=document.getElementById("processes-history-widgets");h&&(g.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=H("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),We(h,g,!0))),$e(1,b.length)}function We(t,e,i){function s(l){let f="";l.end_time?f=`${l.end_time-l.start_time}s`:f=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let y=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[p]?p=b[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
        <div class="process-history-item" data-channel-id="${l.channel_id}-${l.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let n=i?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(n)).map(l=>[l.dataset.channelId,l])),r=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,f]of c)r.has(l)||f.remove();let v=new Set;e.forEach(l=>{let f=`${l.channel_id}-${l.start_time}`;if(v.has(f))return;v.add(f);let y=s(l),p=c.get(f);if(p&&p.parentNode){p.outerHTML!==y&&(p.outerHTML=y);let b=t.querySelector(`[data-channel-id="${f}"]`);b&&t.appendChild(b)}else t.insertAdjacentHTML("beforeend",y)})}function Ne(){let t=Re(),e="Notification"in window,i={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function qt(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ut(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Ut(t)}async function Ut(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=H("empty","No web history found.");return}let s=`
            <div style="display: flex; flex-direction: column; height: 100%;">
        ...
        `;i.forEach((v,l)=>{let f=new Date(v.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=v.screenshot?`<img src="data:image/png;base64,${v.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ls=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ze=!0,i}function Ft(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ze=!1}function Jt(t,e,i="output"){if(!Ze)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Me(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function As(t){let e=Wt.find(n=>n.id===t);if(!e)return;let i=Ms(e);Jt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(r=>setTimeout(r,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),Jt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Gt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ls();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;As(s)})})}async function Is(){if(!z())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Vt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}tt(),st(),Is(),rt(),et();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Gt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),o=document.querySelector("nav"),d=document.querySelector("footer"),m=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(I=>{let U=document.getElementById(I.id);U&&U.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let I=document.getElementById("dexter-dropdown"),U=document.getElementById("dexter-menu-btn");I&&I.classList.remove("active"),U&&U.classList.remove("active"),u&&u.classList.remove("menu-open")}if(ge(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",o?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),A&&(A.style.display=E?"flex":"none"),k&&(k.style.display=E?"block":"none"),!E&&L){let I=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(I)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(l)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(f)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):L.innerHTML=""}else L&&(L.innerHTML="");else o?.classList.remove("window-open"),T&&(T.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",m||x?d?.classList.remove("hide"):d?.classList.add("hide"),A&&(A.style.display="flex"),k&&(k.style.display="block"),L&&(L.innerHTML="");xe()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function r(){[...e].forEach(o=>o.close()),e=[]}window.addEventListener("resize",n);let v=oe({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:ft(),onOpen:()=>ne(),onClose:()=>{let u=e.indexOf(v);u>-1&&e.splice(u,1),n()}}),l=oe({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Je(),onOpen:()=>{l.setContent(Je()),ae()},onClose:()=>{let u=e.indexOf(l);u>-1&&e.splice(u,1),n()}}),f=oe({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:kt(),onOpen:()=>Fe(),onClose:()=>{let u=e.indexOf(f);u>-1&&e.splice(u,1),n()}}),y=oe({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:zt()},{icon:"bxs-component",title:"Processes",content:Dt()},{icon:"bxs-brain",title:"Models",content:Nt()},{icon:"bx-globe",title:"Web",content:jt()},{icon:"bxs-hdd",title:"Hardware",content:Ht()},{icon:"bxs-terminal",title:"Logs",content:Pt()},{icon:"bxs-zap",title:"Agents",content:_t()},{icon:"bx-task",title:"Chores",content:$t()}].filter(u=>z()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Ge(),we(),Ve(),qt(),Ee(),he()},onClose:()=>{let u=e.indexOf(y);u>-1&&e.splice(u,1),n()}}),p=oe({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:Ct(),onOpen:()=>je(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),b=oe({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ne(),onOpen:()=>{b.setContent(Ne()),Ke(b)},onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{l.isOpen()||c(l),setTimeout(()=>{let o=document.querySelector(`.event-item[data-event-id="${u}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{v.isOpen()||c(v),setTimeout(()=>{let o=document.querySelector(`.event-item[data-alert-id="${u}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),o=document.getElementById("dexter-menu-btn");u&&o&&(u.classList.remove("active"),o.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),o=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),m=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),A=document.getElementById("windows-container");u&&x&&(u.onclick=k=>{k.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let I=u.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),F=220,B=10,C=I.left+I.width/2-F/2,w=O.right-10;C+F>w&&(C=w-F),C<10&&(C=10),x.style.top=O.bottom+B+"px",x.style.left=C+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),u.classList.toggle("active");let E=x.classList.contains("active");if(T){let I=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),I?.classList.add("window-open"),A?.classList.add("menu-open"),ge(!0)):(A?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),I?.classList.remove("window-open"),ge(!1)))}}),o&&(o.onclick=k=>{k.stopPropagation(),c(b)}),d&&(d.onclick=k=>{k.stopPropagation(),r()});let L=(k,T,E)=>{let I=document.getElementById(k);I&&(I.onclick=U=>{U.stopPropagation(),a(),E&&s(E),c(T)})};L("alerts-menu-item",v,"alerts-window"),L("events-menu-item",l,"events-window"),L("monitor-menu-item",y,"monitor-window"),L("contacts-menu-item",f,"contacts-window"),L("workspace-menu-item",p,"workspace-window"),m&&(m.onclick=k=>{if(k.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ge(!1));return}if(e.length>0)r();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let U=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");U.pop();let O=U.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let k=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),k&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ge(!1)))}),x&&(x.onclick=k=>k.stopPropagation())})();let S=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!S&&!h&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{l.isOpen()&&ae(),v.isOpen()?ne():Ue()},1e3),setInterval(()=>{z()&&(v.isOpen()?ne():Ue()),p.isOpen()?je():wt(),z()&&l.isOpen()&&ae(),y.isOpen()&&Rt()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Vt):Vt();})();

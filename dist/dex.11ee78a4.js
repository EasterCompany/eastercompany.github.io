...
`,Pe=null,ge=new Set,Oe=[];async function pe(t=!1){let e=document.getElementById("alerts-list");if(!e)return;$s(),t&&(e.innerHTML="<p>Updating...</p>");let n="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];Pe=Date.now(),te(0,Pe);let r=Date.now(),b=24*60*60*1e3,d=c.filter(l=>{let u=localStorage.getItem(`alert_read_ts_${l.id}`);if(!u)return!0;let g=parseInt(u);return r-g<b});d.sort((l,u)=>{let g=h=>{let v=h.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},E=h=>h==="critical"?4:h==="high"?3:h==="medium"?2:1,f=E(g(l)),T=E(g(u));return f!==T?T-f:u.timestamp-l.timestamp}),Oe=d;let x=l=>{let u=l.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},w=[];if(d.length===0)w.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let u=new Set(d.map(E=>x(E))).size>1,g=null;d.forEach(E=>{let f=x(E);u&&f!==g&&(w.push({id:`divider-${f}`,type:"divider",label:f.toUpperCase()}),g=f),w.push(E)})}t&&(e.innerHTML="");let p=l=>{let u=l.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let g=(u.title||"Untitled Alert").trim(),E=(u.body||"No description provided.").trim(),f=u.summary||"",T=u.content||"",h=u.protocol||"",v=(u.priority||"low").toLowerCase(),k=!!u.alert,M=(u.category||"system").trim(),A=u.related_event_ids||[],N=u.audit_event_id,J=!!localStorage.getItem(`alert_read_ts_${l.id}`),V=new Date(l.timestamp*1e3),L=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),S=J?"event-border-grey":"event-border-blue";!J&&k&&(S="event-border-red"),J&&(v==="high"||v==="critical")?S="event-border-red":J&&v==="medium"&&(S="event-border-orange");let _=J?"alert-read":"alert-unread",O=ge.has(l.id),j=O?"expanded":"",R=O?"display: block;":"display: none;",H="",B="";A.length>0&&(B=`
            ...
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #ddd; font-size: 0.85em; white-space: pre-wrap; line-height: 1.6; text-align: left;">${re(T)}</div>
            </div>
        `:X=`
            <div class="event-detail-block" style="text-align: left;">
                ...
                <div class="md-para" style="color: #fff; white-space: pre-wrap; line-height: 1.6; text-align: left;">${re(E)}</div>
            </div>
        `,H=`
...
`,St=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;As();let n="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(n);if(!i.ok)throw new Error("Service is offline or unreachable.");let c=(await i.json()).events||[];At=c,St=Date.now(),te(2,St);let r=s=>{let m=s.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let C=(m.title||"Untitled Blueprint").trim(),I=(m.summary||m.body||"No summary provided.").trim(),o=(m.content||"").trim(),l=(m.category||"architecture").trim(),u=(m.related_services||m.affected_services||[]).map(H=>H.trim()),g=(m.implementation_path||[]).map(H=>H.trim()),E=m.source_event_ids||[],f=m.approved===!0,T=new Date(s.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=ye.has(s.id),M=k?"display: block;":"display: none;",A=document.createElement("div");A.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,A.dataset.blueprintId=s.id,f&&(A.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",A.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let G=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[l]||"bx-paint";A.onclick=function(H){let B=this;if(B.classList.contains("expanded")){B.classList.remove("expanded"),ye.delete(s.id);let W=B.querySelector(".event-details");W&&(W.style.display="none")}else{B.classList.add("expanded"),ye.add(s.id);let W=B.querySelector(".event-details");W&&(W.style.display="block")}};let J="";g.length>0&&(J=`
                ...
                `);let L=u.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${u.join(", ")}</span></div>`:"<div></div>",$=z(),S=f?`
...
`,Ue=null,we=new Set,_e=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,n]of Object.entries(Rs))if(n.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function me(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Us();let i=`/events?ml=${Z==="all"?100:250}&format=json`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(_e=((await a.json()).events||[]).filter(y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let m=s.type;return!(m==="system.blueprint.generated"||m==="system.notification.generated")}),Ue=Date.now(),te(1,Ue),_e.length===0){e.innerHTML=F("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=y=>{let s=y.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let m=s.type,C=Os(m),I=js(m),o=m==="engagement.decision"||m==="messaging.bot.sent_message"||m==="messaging.bot.voice_response"||m==="messaging.user.sent_message"||m==="moderation.explicit_content.deleted"||m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="analysis.user.message_signals"||m==="system.cli.command"||m==="system.analysis.audit"||m==="system.test.completed"||m==="error_occurred"||m==="system.cli.status"||m==="system.attention.expired"||m.startsWith("system.roadmap")||m.startsWith("system.process"),l="event-border-grey";o&&(m==="moderation.explicit_content.deleted"||m==="error_occurred"?l="event-border-red":m==="analysis.link.completed"||m==="analysis.visual.completed"||m==="analysis.router.decision"||m==="system.analysis.audit"||m==="analysis.user.message_signals"?l="event-border-purple":m==="system.attention.expired"||m==="system.cli.command"||m==="system.cli.status"?l="event-border-orange":m==="system.test.completed"?l=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":l="event-border-blue");let u=o?"cursor-pointer":"",g=we.has(y.id),E=g?"expanded":"",f=g?"display: block;":"display: none;",T=new Date(y.timestamp*1e3),h=T.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),v=T.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=Ft(m,s),M=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",A="";if(o){let L="";if(m==="engagement.decision"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                    ...
                    `}else if(m==="system.attention.expired"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=s.timestamp-s.last_active,_=Math.floor(S/60);L=`
                    ...
                    `}else if(m==="messaging.bot.sent_message"){let $=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,S="";if(s.embed){let j=s.embed,R=j.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${P(j.title)}</div>`:"",H=j.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${re(j.description)}</div>`:"",B=(j.fields||[]).map(U=>`
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${P(U.name)}</div>
                <div style="font-size: 0.85em; color: #eee; white-space: pre-wrap;">${re(U.value)}</div>
              </div>
            `).join("");S=`
              <div class="discord-embed" style="margin-top: 15px; padding: 12px; border-left: 4px solid ${j.color?"#"+j.color.toString(16).padStart(6,"0"):"#bb86fc"}; background: rgba(255,255,255,0.02); border-radius: 0 4px 4px 0;">
                ${R}
                ${H}
                ${B}
              </div>
            `}let _="";s.eval_count&&(_=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let j=s.chat_history.length,R=s.chat_history.map((H,B)=>{let U=H.name?H.name.toUpperCase():H.role.toUpperCase();!H.name&&U==="ASSISTANT"&&(U="DEXTER");let W=H.role==="user"?"#03dac6":H.role==="system"?"#ffb74d":"#bb86fc",X=B===j-1?"block":"none";return`
                                <div class="history-slide" data-index="${B}" style="display: ${X};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${W}; letter-spacing: 1px; font-weight: bold;">${U}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${B+1} of ${j}</span>
                                    </div>
                        ...
                        `}
                    `}else if(m==="messaging.bot.voice_response"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Response Model:</span>
                            <span class="detail-value">${s.response_model||"N/A"}</span>
                        </div>
                    ...
                    `}else if(m==="analysis.user.message_signals"){let $=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,S=s.signals||{};L=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(m==="moderation.explicit_content.deleted"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.link.completed"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="analysis.router.decision"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.cli.command"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="system.analysis.audit"){let $=s.success?"#03dac6":"#ff4d4d",S=s.success?"SUCCESS":"FAILED",_=B=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${B}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${_("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${P(s.error)}</pre>
                            </div>
                        `);let j="";if(s.chat_history&&s.chat_history.length>0){let B=s.chat_history.length,U=s.chat_history.map((W,X)=>{let K=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&K==="USER"&&(K="SYSTEM"),!W.name&&K==="ASSISTANT"&&(K="AGENT");let ce=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let R="";if(s.corrections&&s.corrections.length>0){let B=s.corrections.map((U,W)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                ...
                                ${U.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${P(U.snippet)}</div>`:""}
                            </div>
                        `).join("");R=`
                            <div class="event-detail-block">
                        ...
                        `}let H="";if(s.parsed_results&&s.parsed_results.length>0){let B=s.parsed_results.map(U=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${P(U.title)}</div>
                                <div style="color: #ddd;">${P(U.summary)}</div>
                            </div>
                        `).join("");H=`
                            <div class="event-detail-block">
                                ${_("Parsed Results")}
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${$}; font-weight: bold;">${S} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${s.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${R}
                        ${j}
                    `}else if(m==="system.test.completed"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row">
                    ...
                    `}else if(m==="error_occurred"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(m==="system.cli.status"){let $=S=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${S}</h5>`;L=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(m==="messaging.user.sent_message"){let $="";s.attachments&&s.attachments.length>0&&($=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${s.attachments.map(_=>{let O=_.content_type&&_.content_type.startsWith("image/"),j=(_.size/1024).toFixed(1)+" KB";return`
                ...
                `}let N=document.createElement("div");N.className=`event-item ${l} ${u} ${E}`,N.dataset.eventId=y.id,N.onclick=function(L){if(!o)return;let $=this;if($.classList.contains("expanded")){$.classList.remove("expanded"),we.delete(y.id);let _=$.querySelector(".event-details");_&&(_.style.display="none")}else{$.classList.add("expanded"),we.add(y.id);let _=$.querySelector(".event-details");_&&(_.style.display="block")}},N.innerHTML=`
                <div class="event-time">
                    <span class="event-time-main">${h}</span>
                    <span class="event-date">${v}</span>
                </div>
...
`,Jt=qs;function Wt(){return`
    ...
    `}var Fs=null;async function He(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await D("/logs");if(!e.ok)throw new Error("Logs offline");let n=await e.json();if(!n||n.length===0)return t.innerHTML=F("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let i=["local-ollama-0","local-cache-0"],a=n.filter(r=>!i.includes(r.id));a.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),a.reverse();let c=a.map(r=>{let b=r.logs.join(`
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let l=o.status==="online",u=l?"service-widget-online":"service-widget-offline",g=l?"bx-check-circle":"bx-x-circle",E=l?"OK":"BAD",f=o.version?w(o.version.str):"-",T=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",h=o.memory&&o.memory!=="N/A"?o.memory:"-",v=T!=="-"?"#00ff00":"#666",k=T!=="-"?"#fff":"#666",M=h!=="-"?"#00ff00":"#666",A=h!=="-"?"#fff":"#666",N=y(o.uptime),G="";l?G=`
                ...
                </div>`:G='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let J=z()?"easter.company":p(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${u}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${E}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${J}</span></div>${G}</div></div>`}let m=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),C=new Set(x.map(o=>o.id));for(let[o,l]of m)C.has(o)||l.remove();let I=new Set;x.forEach(o=>{if(I.has(o.id))return;I.add(o.id);let l=s(o),u=m.get(o.id);if(u&&u.parentNode)if(o.id==="upstash-redis-ro"){let g=z()&&(oe.lastFrontend||oe.lastDashboard)||Date.now(),E=new Date(g).toLocaleTimeString(),f=u.querySelector(".sync-time-label");f&&(f.textContent=`Last synced: ${E}`)}else u.outerHTML!==l&&(u.outerHTML=l);else t.insertAdjacentHTML("beforeend",l)})}async function it(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=F("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),te(2,Vt);let n=e.models||[],i=e.whisper;Array.from(t.children).forEach(d=>{d.classList.contains("service-widget")||d.remove()});function a(d){let x=d.status==="Ready";return`
          ...
          `)};n&&A(n,document.getElementById("guardian-sentry-stats"),u.protocols.sentry,"sentry");let N=document.getElementById("analyzer-synthesis-val"),G=document.getElementById("analyzer-synthesis-stats");N&&A(N,G,g.protocols.synthesis,"synthesis");let J=document.getElementById("imaginator-alert_review-val"),V=document.getElementById("imaginator-alert_review-stats");J&&A(J,V,E.protocols.alert_review,"alert_review");let L=document.getElementById("fabricator-construction-val"),$=document.getElementById("fabricator-construction-stats");L&&A(L,$,f.protocols.construction,"construction");let S=document.getElementById("courier-researcher-val"),_=document.getElementById("courier-researcher-stats");S&&A(S,_,T.protocols.researcher,"researcher");let O=document.getElementById("system-state-label"),j=document.getElementById("system-state-val");if(j&&h.state){let R=h.state,H=M(h.state_time||0);O&&(O.textContent=`State: ${R.toUpperCase()}`),j.textContent=H,R==="idle"?j.style.color=h.state_time>300?"#5eff5e":"#fff":j.style.color="#bb86fc",p&&(R==="paused"?(p.innerHTML="<i class='bx bx-play'></i>",p.title="Resume System",p.style.color="#ff9800"):(p.innerHTML="<i class='bx bx-pause'></i>",p.title="Pause System",p.style.color=""))}a&&(a.textContent=M(h.metrics?.total_idle_time||0)),c&&(c.textContent=M(h.metrics?.total_active_time||0)),r&&(r.textContent=M(h.metrics?.total_waste_time||0))}else[n,i,a,c,r].forEach(g=>{g&&(g.textContent="-",g.style.color="#666")});if(t)return;let s=await Ws(),m=[],C=[],I=[];s&&(Array.isArray(s)?m=s:(m=s.active||[],C=s.queue||[],I=s.history||[],I.sort((u,g)=>(g.end_time||0)-(u.end_time||0)))),e&&(m.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=F("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),st(e,m,!1)));let o=document.getElementById("processes-queue-widgets");o&&(C.length===0?!o.querySelector(".tab-placeholder")&&!o.querySelector('div[style*="font-style: italic"]')&&(o.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(o.innerHTML="",st(o,C,!1)));let l=document.getElementById("processes-history-widgets");l&&(I.length===0?l.querySelector(".tab-placeholder")||(l.innerHTML=F("empty","No recent history.")):(l.querySelector(".tab-placeholder")&&(l.innerHTML=""),st(l,I,!0))),Ne(1,m.length)}function st(t,e,n){function i(d){let x="";d.end_time?x=`${d.end_time-d.start_time}s`:x=`${Math.floor(Date.now()/1e3-d.start_time)}s`;let w=d.retries>0?`<span class="process-retry-badge">Retry ${d.retries}</span>`:"",p=d.channel_id,y={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent"};if(y[p]?p=y[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),n)return`

...
`,ge=new Set,Ne=[],Ee=null;async function fe(t=!1){let e=document.getElementById("roadmap-list");if(e){ws();try{let s=await B("/roadmap");if(!s.ok)throw new Error("Offline");let i=await s.json();Ne=i;let n=p=>{let y=ge.has(p.id),m=p.state==="published",v=p.state==="consumed",o="event-border-grey";m&&(o="event-border-blue"),v&&(o="event-border-purple");let $=new Date(p.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=document.createElement("div");w.className=`event-item notification-item ${o} cursor-pointer ${y?"expanded":""}`,w.dataset.itemId=p.id,w.onclick=u=>{if(u.target.closest("button"))return;if(w.classList.contains("expanded")){w.classList.remove("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="none"),ge.delete(p.id)}else{w.classList.add("expanded");let x=w.querySelector(".event-details");x&&(x.style.display="block"),ge.add(p.id)}};let a=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${p.state.toUpperCase()}]</span>`;w.innerHTML=`
      ...
      `;let d=w.querySelector(".event-details");return d&&d.addEventListener("click",u=>{u.stopPropagation()}),w.querySelector(".edit-btn")?.addEventListener("click",()=>Cs(p)),w.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Es(p)),w.querySelector(".delete-btn")?.addEventListener("click",()=>$s(p.id)),w.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),w.classList.remove("expanded");let g=w.querySelector(".event-details");g&&(g.style.display="none"),ge.delete(p.id)}),w},l=Array.from(e.children),r=new Map(l.map(p=>[p.dataset.itemId,p])),f=new Set(i.map(p=>p.id));if(l.forEach(p=>{let y=p.dataset.itemId;(!y||!f.has(y))&&p.remove()}),!Ne||Ne.length===0){e.innerHTML=N("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let c=null;i.forEach((p,y)=>{let m=r.get(p.id);(!m||t)&&(m&&m.remove(),m=n(p),!m)||(y===0?e.firstElementChild!==m&&e.prepend(m):c&&c.nextElementSibling!==m&&c.after(m),c=m)})}catch{e.children.length===0&&(e.innerHTML=N("offline","Failed to load roadmap.","The event service may be offline."))}}}function ws(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),s=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Ee=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Ee=null},s.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let f=Ee?`/roadmap/${Ee}`:"/roadmap",c=Ee?"PATCH":"POST";try{await B(f,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let p=document.getElementById("roadmap-editor-container");p&&(p.style.display="none"),fe(!0)}catch(p){console.error(p)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{Ne.forEach(l=>ge.add(l.id)),fe(!0)},i.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ge.clear(),fe(!0)},n.dataset.listenerAttached="true")}function Cs(t){Ee=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let s=document.getElementById("roadmap-editor-container");s&&(s.style.display="block",s.scrollIntoView({behavior:"smooth"}))}async function Es(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),fe(!0)}catch(s){console.error(s)}}async function $s(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),ge.delete(t),fe(!0)}catch(e){console.error(e)}}var Et=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Ct=null,be=new Set,Tt=[];async function ve(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ts();let s="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await B(s);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];if(Tt=l,Ct=Date.now(),ee(2,Ct),l.length===0){e.innerHTML=N("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),ke(2,0);return}t&&(e.innerHTML="");let r=m=>{let v=m.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let o=(v.title||"Untitled Blueprint").trim(),b=(v.summary||v.body||"No summary provided.").trim(),$=(v.content||"").trim(),w=(v.category||"architecture").trim(),a=(v.related_services||v.affected_services||[]).map(_=>_.trim()),d=(v.implementation_path||[]).map(_=>_.trim()),u=v.source_event_ids||[],g=v.approved===!0,x=new Date(m.timestamp*1e3),M=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),A=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),I=be.has(m.id),k=I?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${I?"expanded":""} ${g?"blueprint-approved":""}`,E.dataset.blueprintId=m.id,g&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=g?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[w]||"bx-paint";E.onclick=function(_){let P=this;if(P.classList.contains("expanded")){P.classList.remove("expanded"),be.delete(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="none")}else{P.classList.add("expanded"),be.add(m.id);let j=P.querySelector(".event-details");j&&(j.style.display="block")}};let O="";d.length>0&&(O=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${d.map(_=>`<li style="margin-bottom: 5px;">${z(_)}</li>`).join("")}</ul></div>
                    </div>
                `);let q="";u.length>0&&(q=`
                    <div class="blueprint-source" style="margin-top: 15px;">
                        ...
                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                            ${u.map(_=>`
                                <a href="#" onclick="window.dexter.viewEvent('${_}'); return false;" style="color: #03dac6; text-decoration: none; font-size: 0.75em; font-family: 'JetBrains Mono', monospace; padding: 4px 8px; background: rgba(3, 218, 198, 0.05); border: 1px solid rgba(3, 218, 198, 0.1); border-radius: 4px;">
                                    <i class='bx bx-link-external'></i> ${_.substring(0,8)}...
                                </a>
                            `).join("")}
                        </div>
                    </div>
                `);let G=a.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${a.join(", ")}</span></div>`:"<div></div>",J=H(),S=g?`
            ...
            `;let C=E.querySelector(".blueprint-approve-btn");C&&(C.onclick=async _=>{_.stopPropagation(),C.innerHTML="<i class='bx bx-loader-alt spin'></i> Approving...";try{(await B(`/events/${m.id}`,{method:"PATCH",body:JSON.stringify({approved:!0})})).ok&&ve(!0)}catch(P){console.error("Failed to approve blueprint:",P)}});let h=E.querySelector(".blueprint-delete-btn");h&&(h.onclick=async _=>{_.stopPropagation();let P=!g;h.innerHTML=P?"<i class='bx bx-loader-alt spin'></i> Declining...":"<i class='bx bx-loader-alt spin'></i> Deleting...";try{(await B(`/events/${m.id}`,{method:"DELETE"})).ok&&ve(!0)}catch(V){console.error("Failed to delete blueprint:",V)}});let T=E.querySelector(".event-details");T&&T.addEventListener("click",_=>{_.stopPropagation()});let L=E.querySelector(".close-details-btn");return L&&L.addEventListener("click",_=>{_.stopPropagation(),E.classList.remove("expanded");let P=E.querySelector(".event-details");P&&(P.style.display="none"),be.delete(m.id)}),E},f=Array.from(e.children),c=new Map(f.map(m=>[m.dataset.blueprintId,m])),p=new Set(l.map(m=>m.id));f.forEach(m=>{let v=m.dataset.blueprintId;(!v||!p.has(v))&&m.remove()});let y=null;l.forEach((m,v)=>{let o=c.get(m.id);if(!o||t){o&&o.remove();let b=r(m);if(!b)return;o=b}v===0?e.firstElementChild!==o&&e.prepend(o):y&&y.nextElementSibling!==o&&y.after(o),y=o}),ke(2,l.length),bt()}catch(i){console.error("Error fetching blueprints:",i),e.children.length===0&&(e.innerHTML=N("offline","Failed to load blueprints.","The event service may be offline."))}}async function St(){let t="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let e=await B(t);if(!e.ok)return;let i=(await e.json()).events||[],n=0;i.forEach(l=>{let r=l.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}r.approved!==!0&&n++}),ft(n)}catch{}}function Ts(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all"),s=document.getElementById("blueprints-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Tt.forEach(i=>be.add(i.id)),ve(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{be.clear(),ve(!0)},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all UNAPPROVED blueprints?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.blueprint.generated",{method:"DELETE"}),be.clear(),ve(!0)}catch(i){console.error("Failed to clear blueprints:",i)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var ae="STANDBY",kt=null,$e=[],Z=null,Pe=null,Mt=()=>`
    <div id="progress-view-root" class="progress-container">
        <!-- Initial render will happen here -->
        ${Lt()}
    </div>
  `;function Lt(){switch(ae){case"ACTIVE":return ks();case"COMPLETED":return Ms();case"STANDBY":default:return Ss()}}function Ss(){return`
  ...
  `}async function Te(){let t=document.getElementById("progress-view-root");t&&(ae!==kt&&(t.innerHTML=Lt(),kt=ae),ae==="ACTIVE"&&Ls())}function Ls(){let t=document.getElementById("active-task-title"),e=document.getElementById("active-task-progress-fill"),s=document.getElementById("active-task-progress-text"),i=document.getElementById("terminal-output");if(t&&Z&&(t.innerText=Z.title),e&&Z&&(e.style.width=`${Z.progress}%`),s&&Z&&(s.innerText=`${Z.progress}% COMPLETE`),i){let n=i.children.length;if($e.length>n){for(let l=n;l<$e.length;l++){let r=$e[l],f=document.createElement("div");f.className="terminal-line",f.innerHTML=`
    ...
    `;async function Re(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),s=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{s&&(s.style.display=s.style.display==="none"?"block":"none")},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{s&&(s.style.display="none")},n.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let l=document.getElementById("new-chore-instruction"),r=document.getElementById("new-chore-url"),f=document.getElementById("new-chore-owner"),c=l?.value,p=r?.value,y=f?.value||"313071000877137920";if(c){i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:y,natural_instruction:c,entry_url:p,schedule:"every_6h"})}),s&&(s.style.display="none"),l&&(l.value=""),r&&(r.value=""),Re()}catch(m){console.error(m),alert("Failed to create chore")}finally{i.innerHTML="Create Task"}}},i.dataset.listenerAttached="true"),!!t)try{let l=await B("/chores");if(!l.ok)throw new Error(`HTTP error! status: ${l.status}`);let r=await l.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=N("empty","No active tasks.",H()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let f=r.map(c=>{let p=c.last_run===0?"Never":new Date(c.last_run*1e3).toLocaleString(),y=c.memory?c.memory.length:0,m=c.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=f,t.querySelectorAll(".delete-chore-btn").forEach(c=>{c.onclick=async p=>{p.stopPropagation();let y=c.dataset.id;confirm("Delete this task?")&&(await B(`/chores/${y}`,{method:"DELETE"}),Re())}})}catch(l){console.error("Failed to fetch chores",l)}}var It=()=>`
...
`;async function Ge(){await Promise.all([fe(),ve(),Te(),Re()])}var As=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`,Is=null;async function Ke(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Ke(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=s=>{let n=s.target.closest(".user-profile-card");if(n&&n.dataset.user)try{let l=JSON.parse(n.dataset.user);zt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let s=await He("/contacts");if(!s.ok)throw new Error("Service unreachable");let n=(await s.json()).members||[];if(Is=Date.now(),n.length===0){t.innerHTML=N("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,f)=>{let c=l[r.level]??10,p=l[f.level]??10;return c!==p?c-p:r.username.localeCompare(f.username)}),t.innerHTML=n.map(r=>{let f=r.color&&r.color!==0,c=f?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",p=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let m=r.level==="Me",v=m?"rgba(187, 134, 252, 0.08)":f?`${c}11`:"rgba(255,255,255,0.03)",o=m?"2px solid #bb86fc":f?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
...
`,Oe=null,ye=new Set,Me=[],K="all",_s={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Hs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Ds(t){for(let[e,s]of Object.entries(_s))if(s.includes(t))return e;return"system"}function zs(t){return Hs[t]||"bx-square-rounded"}async function de(t=!1){let e=document.getElementById("events-timeline");if(!e)return;Ns();let i=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(i+=`&category=${K}`);try{let n=await B(i);if(!n.ok)throw new Error("Service unreachable");if(Me=((await n.json()).events||[]).filter(v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return!0}let b=o.type;return!(b==="system.blueprint.generated"||b==="system.notification.generated")}),Oe=Date.now(),ee(1,Oe),Me.length===0){e.innerHTML=N("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=v=>{let o=v.event;if(typeof o=="string")try{o=JSON.parse(o)}catch{return null}let b=o.type,$=Ds(b),w=zs(b),a=b==="engagement.decision"||b==="messaging.bot.sent_message"||b==="messaging.user.sent_message"||b==="moderation.explicit_content.deleted"||b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="analysis.user.message_signals"||b==="system.cli.command"||b==="system.analysis.audit"||b==="system.test.completed"||b==="error_occurred"||b==="system.cli.status"||b==="system.attention.expired"||b.startsWith("system.roadmap")||b.startsWith("system.process"),d="event-border-grey";a&&(b==="moderation.explicit_content.deleted"||b==="error_occurred"?d="event-border-red":b==="analysis.link.completed"||b==="analysis.visual.completed"||b==="analysis.router.decision"||b==="system.analysis.audit"||b==="analysis.user.message_signals"?d="event-border-purple":b==="system.attention.expired"||b==="system.cli.command"||b==="system.cli.status"?d="event-border-orange":b==="system.test.completed"?d=o.test?.status==="OK"&&o.lint?.status==="OK"&&o.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let u=a?"cursor-pointer":"",g=ye.has(v.id),x=g?"expanded":"",M=g?"display: block;":"display: none;",A=new Date(v.timestamp*1e3),I=A.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),k=A.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Pt(b,o),D=o.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${o.user_level})</span>`:"",U="";if(a){let S="";if(b==="engagement.decision"){let C=h=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${h}</h5>`;S=`
                    ...
                    `}else if(b==="messaging.bot.sent_message"){let C=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`,h="";o.eval_count&&(h=`
                        ...
                        `);let T="";if(o.chat_history&&o.chat_history.length>0){let L=o.chat_history.length,_=o.chat_history.map((P,V)=>{let j=P.name?P.name.toUpperCase():P.role.toUpperCase();!P.name&&j==="ASSISTANT"&&(j="DEXTER");let F=P.role==="user"?"#03dac6":P.role==="system"?"#ffb74d":"#bb86fc",W=V===L-1?"block":"none";return`
                    ...
                    `}else if(b==="system.analysis.audit"){let C=o.success?"#03dac6":"#ff4d4d",h=o.success?"SUCCESS":"FAILED",T=j=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${j}</h5>`,L="";o.error&&(L=`
                        ...
                        `);let _="";if(o.chat_history&&o.chat_history.length>0){let j=o.chat_history.length,F=o.chat_history.map((W,re)=>{let te=W.name?W.name.toUpperCase():W.role.toUpperCase();!W.name&&te==="USER"&&(te="SYSTEM"),!W.name&&te==="ASSISTANT"&&(te="AGENT");let X=W.role==="user"?"#03dac6":W.role==="system"?"#ffb74d":"#bb86fc";return`
                    ...
                    `}else if(b==="messaging.user.sent_message"){let C="";o.attachments&&o.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                                ...
                                <div class="attachments-grid">${o.attachments.map(T=>{let L=T.content_type&&T.content_type.startsWith("image/"),_=(T.size/1024).toFixed(1)+" KB";return`
                                <div class="attachment-item">
                                    ...
                                    <div class="attachment-info">
                                        ...
                                        <span class="attachment-meta">${T.content_type} \u2022 ${_}</span>
                                    </div>
                                </div>`}).join("")}</div>
                            </div>`),S=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
...
`);return`
        ...
        <div id="processes-history-widgets" style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 30px; opacity: 0.8;"></div>`,Wt=()=>`
        <div class="system-section-header">
            <i class='bx bxs-server' style="color: #03dac6;"></i>
            <h2>Services</h2>
        </div>
        <div id="services-widgets" class="system-monitor-widgets" style="margin-bottom: 30px;"></div>`,Gt=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Ot()}
        </div>`;async function Zt(){Os(),await Promise.all([oe(),Xe(),Qe()]),await Le()}var Ue=null;function Os(){Ue||(Ue=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Ue),Ue=null;return}oe(!0)},1e3))}var Ut=null,jt=null;async function Yt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function qt(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function Us(){try{return await(await B("/processes")).json()}catch{return null}}async function js(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Xe(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),s=document.querySelector("#hw-os .hw-content"),i=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),l=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),f=a=>{if(a){if(s&&(s.innerHTML=`
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
                ...
                </div>`:q='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let G=H()?"easter.company":m(a.domain&&a.port?`${a.domain}:${a.port}`:"");return`<div class="service-widget ${u}" data-service-id="${a.id}"><div class="service-widget-header"><i class="bx ${g}"></i><h3>${a.short_name||a.id}</h3><span class="service-widget-status">${x}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${G}</span></div>${q}</div></div>`}let b=new Map(Array.from(t.querySelectorAll(".service-widget")).map(a=>[a.dataset.serviceId,a])),$=new Set(p.map(a=>a.id));for(let[a,d]of b)$.has(a)||d.remove();let w=new Set;p.forEach(a=>{if(w.has(a.id))return;w.add(a.id);let d=o(a),u=b.get(a.id);if(u&&u.parentNode)if(a.id==="upstash-redis-ro"){let g=H()&&(ie.lastFrontend||ie.lastDashboard)||Date.now(),x=new Date(g).toLocaleTimeString(),M=u.querySelector(".sync-time-label");M&&(M.textContent=`Last synced: ${x}`)}else u.outerHTML!==d&&(u.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Qe(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Yt();if(!e){t.children.length===0&&(t.innerHTML=N("offline","Failed to load model status.","The event service may be offline."));return}jt=Date.now(),ee(2,jt);let s=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function n(c){let p=c.status==="Ready";return`
    ...
    `;async function es(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Xt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Xt(t)}async function Xt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let s=await e.json();if(!s||s.length===0){t.innerHTML=N("empty","No web history found.");return}let i=`
            <div style="display: flex; flex-direction: column; height: 100%;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],qs=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `;for(let[s,i]of Object.entries(t)){let n=ns.filter(l=>l.category===s);n.length!==0&&(e+=`
    ...
    </div>`,e},tt=!1;function Fs(t){let e=document.getElementById("cli-terminal-overlay");if(!e){e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
        ...
        `,document.body.appendChild(e);let c=document.getElementById("close-terminal-btn");c&&(c.onclick=()=>ts());let p=document.getElementById("terminal-action-btn");p&&(p.onclick=()=>ts())}let s=document.getElementById("cli-terminal-body"),i=document.getElementById("cli-docs-pane");s&&(s.innerHTML="");let n=t.docs||{overview:t.description,details:[],extended_usage:t.usage};i&&(i.innerHTML=`

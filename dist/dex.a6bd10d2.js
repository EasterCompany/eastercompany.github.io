...
`,Lt=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ms();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let r=(await i.json()).events||[];At=r,Lt=Date.now(),te(2,Lt);let l=s=>{let u=s.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let T=(u.title||"Untitled Blueprint").trim(),z=(u.summary||u.body||"No summary provided.").trim(),v=(u.content||"").trim(),d=(u.category||"architecture").trim(),h=(u.related_services||u.affected_services||[]).map(k=>k.trim()),y=(u.implementation_path||[]).map(k=>k.trim()),I=u.source_event_ids||[],n=u.approved===!0,g=new Date(s.timestamp*1e3),w=g.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=g.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ye.has(s.id),A=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${n?"blueprint-approved":""}`,S.dataset.blueprintId=s.id,n&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=n?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[d]||"bx-paint";S.onclick=function(k){let _=this;if(_.classList.contains("expanded")){_.classList.remove("expanded"),ye.delete(s.id);let U=_.querySelector(".event-details");U&&(U.style.display="none")}else{_.classList.add("expanded"),ye.add(s.id);let U=_.querySelector(".event-details");U&&(U.style.display="block")}};let q="";y.length>0&&(q=`
...
`,Ge=null,we=new Set,ze=[],Z="all",Rs={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals","system.cognitive.model_inference"],moderation:["moderation.explicit_content.deleted"]},Ps={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart","system.cognitive.model_inference":"bx-brain"};function Os(t){for(let[e,o]of Object.entries(Rs))if(o.includes(t))return e;return"system"}function js(t){return Ps[t]||"bx-square-rounded"}async function me(t=!1){let e=document.getElementById("events-timeline");if(!e)return;qs();let i=`/events?ml=${Z==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(ze=((await a.json()).events||[]).filter(x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ge=Date.now(),te(1,Ge),ze.length===0){e.innerHTML=J("empty","No events found for this filter.");return}t&&(e.innerHTML="");let f=x=>{let s=x.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,T=Os(u),z=js(u),v=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.cognitive.model_inference"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),d="event-border-grey";v&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?d="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?d="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?d="event-border-orange":u==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let h=v?"cursor-pointer":"",y=we.has(x.id),I=y?"expanded":"",n=y?"display: block;":"display: none;",g=new Date(x.timestamp*1e3),w=g.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=g.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Ft(u,s),A=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",S="";if(v){let B="";if(u==="engagement.decision"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                    ...
                    `}else if(u==="system.cognitive.model_inference"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                    ...
                    `}else if(u==="system.attention.expired"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.timestamp-s.last_active,M=Math.floor(L/60);B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="messaging.bot.sent_message"){let E=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="";if(s.embed){let P=s.embed,G=P.title?`<div style="font-weight: bold; margin-bottom: 5px; color: #fff;">${j(P.title)}</div>`:"",k=P.description?`<div style="font-size: 0.9em; color: #bbb; white-space: pre-wrap;">${le(P.description)}</div>`:"",_=(P.fields||[]).map(R=>`
              <div style="margin-top: 10px;">
            ...
            `}let M="";s.eval_count&&(M=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let P=s.chat_history.length,G=s.chat_history.map((k,_)=>{let R=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&R==="ASSISTANT"&&(R="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=_===P-1?"block":"none";return`
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let E=P=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${P}</h5>`,L="",M=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let P=O.match(/{.*?}/g);P&&(M=P.map(G=>{let k=G.substring(1,G.length-1),_=k.indexOf(" "),R=k.substring(0,_),U=k.substring(_+1);return U=U.replace(/\[\]$/,"").trim(),{role:R,content:U}}))}if(M.length>0&&s.response_raw&&M.push({role:"assistant",content:s.response_raw}),M.length>0){let P=M.length,G=M.map((k,_)=>{let R=k.role.toUpperCase();R==="ASSISTANT"&&(R="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=_===P-1?"block":"none";return`
                                <div class="history-slide" data-index="${_}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${R}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${_+1} of ${P}</span>
                    ...
                    `}else if(u==="analysis.user.message_signals"){let E=O=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${O}</h5>`,L=s.signals||{};B=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(u==="moderation.explicit_content.deleted"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="analysis.link.completed"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
              ...
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(u==="analysis.router.decision"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.cli.command"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="system.analysis.audit"){let E=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",M=_=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${_}</h5>`,O="";s.error&&(O=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${j(s.error)}</pre>
                            </div>
                        `);let P="";if(s.chat_history&&s.chat_history.length>0){let _=s.chat_history.length,R=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ee=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let _=s.corrections.map((R,U)=>`
                        ...
                        `}let k="";if(s.parsed_results&&s.parsed_results.length>0){let _=s.parsed_results.map(R=>`
                        ...
                        ${G}
                        ${P}
                    `}else if(u==="system.test.completed"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row">
                    ...
                    `}else if(u==="error_occurred"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(u==="system.cli.status"){let E=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;B=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(u==="messaging.user.sent_message"){let E="";s.attachments&&s.attachments.length>0&&(E=`
                            <div class="event-detail-block">
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
                ...
                </div>`}b&&(n.GPU&&n.GPU.length>0?b.innerHTML=n.GPU.map(g=>{let w=(g.VRAM/1073741824).toFixed(1);return`
            ...
            </div>`}let g=n.status==="online",w=g?"service-widget-online":"service-widget-offline",p=g?"bx-check-circle":"bx-x-circle",$=g?"OK":"BAD",A=n.version?u(n.version.str):"-",S=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",H=n.memory&&n.memory!=="N/A"?n.memory:"-",W=S!=="-"?"#00ff00":"#666",q=S!=="-"?"#fff":"#666",V=H!=="-"?"#00ff00":"#666",B=H!=="-"?"#fff":"#666",E=z(n.uptime),L="",M=n.type!=="os"&&n.type!=="cli"&&n.type!=="prd",O="";M&&!N()&&(O=`
                ...
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}-${c.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${s}">${b}</span>
                        </div>
                         <div class="service-widget-info">

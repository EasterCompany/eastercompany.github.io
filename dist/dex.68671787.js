...
`,Ie=null,le=new Set,Be=[];async function ne(t=!1){let e=document.getElementById("alerts-list");if(!e)return;ds(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];Ie=Date.now(),X(0,Ie);let r=Date.now(),b=24*60*60*1e3,l=c.filter(d=>{let u=localStorage.getItem(`alert_read_ts_${d.id}`);if(!u)return!0;let x=parseInt(u);return r-x<b});l.sort((d,u)=>{let x=T=>{let E=T.event;if(typeof E=="string")try{E=JSON.parse(E)}catch{return"low"}return(E.priority||"low").toLowerCase()},A=T=>T==="critical"?4:T==="high"?3:T==="medium"?2:1,L=A(x(d)),k=A(x(u));return L!==k?k-L:u.timestamp-d.timestamp}),Be=l;let m=d=>{let u=d.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},y=[],v=new Set(l.map(d=>m(d))).size>1;if(l.length>0){let d=null;l.forEach(u=>{let x=m(u);v&&x!==d&&(y.push({id:`divider-${x}`,type:"divider",label:x.toUpperCase()}),d=x),y.push(u)})}if(t&&(e.innerHTML=""),l.length===0){e.innerHTML=H("empty","No alerts yet."),Le();return}let a=d=>{let u=d.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let x=(u.title||"Untitled Alert").trim(),A=(u.body||"No description provided.").trim(),L=u.summary||"",k=u.content||"",T=u.protocol||"",E=(u.priority||"low").toLowerCase(),_=!!u.alert,U=(u.category||"system").trim(),O=u.related_event_ids||[],F=u.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),w=new Date(d.timestamp*1e3),C=w.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),$=w.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),M=q?"event-border-grey":"event-border-blue";!q&&_&&(M="event-border-red"),q&&(E==="high"||E==="critical")?M="event-border-red":q&&E==="medium"&&(M="event-border-orange");let N=q?"alert-read":"alert-unread",D=le.has(d.id),j=D?"expanded":"",J=D?"display: block;":"display: none;",W="",G="";O.length>0&&(G=`
        ...
        `,W=`
                ...
                ${Q}
            `;let Z=document.createElement("div");Z.className=`event-item notification-item ${M} ${N} ${j} cursor-pointer priority-${E}`,Z.dataset.alertId=d.id,Z.onclick=function(ie){if(this.classList.contains("expanded")){this.classList.remove("expanded"),le.delete(d.id);let Ce=this.querySelector(".event-details");Ce&&(Ce.style.display="none")}else{this.classList.add("expanded"),le.add(d.id);let Ce=this.querySelector(".event-details");if(Ce&&(Ce.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),this.classList.add("alert-read"),this.classList.remove("alert-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let Pe="event-border-grey";E==="high"||E==="critical"?Pe="event-border-red":E==="medium"&&(Pe="event-border-orange"),this.classList.add(Pe),Le()}}};let Ye=`${T?T.toUpperCase():"GUARDIAN"} ALERT: ${L||x}`,Zt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[U]||"bx-bell",Yt=E==="high"||E==="critical"?"#ff4d4d":E==="medium"?"#ffa500":"#888";Z.innerHTML=`
                ...
                <div class="event-content" style="text-align: left;">
                    <div class="event-service">
                        DEXTER <span class="alert-badge" style="color: ${Yt}; font-size: 0.8em; margin-left: 5px;">[${E.toUpperCase()}]</span>
                    </div>
            ...
            `;let Xe=Z.querySelector(".event-details");Xe&&Xe.addEventListener("click",ie=>{ie.stopPropagation()});let Qe=Z.querySelector(".close-details-btn");return Qe&&Qe.addEventListener("click",ie=>{ie.stopPropagation(),Z.classList.remove("expanded");let He=Z.querySelector(".event-details");He&&(He.style.display="none"),le.delete(d.id)}),Z},g=d=>{let u=document.createElement("div");u.className="notification-divider",u.dataset.alertId=d.id;let x="#888888";return d.label==="CRITICAL"?x="#ff4d4d":d.label==="HIGH"?x="#ff8888":d.label==="MEDIUM"&&(x="#ffa500"),u.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${x}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,u.innerHTML=`<span style="white-space: nowrap;">${d.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${x}44, transparent); flex-grow: 1;"></div>`,u},S=Array.from(e.children),h=new Map(S.map(d=>[d.dataset.alertId,d])),f=new Set(y.map(d=>d.id));S.forEach(d=>{let u=d.dataset.alertId;(!u||!f.has(u))&&d.remove()});let o=null;y.forEach((d,u)=>{let x=h.get(d.id);(!x||t)&&(x&&x.remove(),d.type==="divider"?x=g(d):x=a(d),!x)||(u===0?e.firstElementChild!==x&&e.prepend(x):o&&o.nextElementSibling!==x&&o.after(x),o=x)}),Ie=Date.now(),X(0,Ie),Le()}catch(s){console.error("Error fetching alerts:",s),e.children.length===0&&(e.innerHTML=H("offline","Failed to load alerts.","The event service may be offline."))}}function ds(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),s=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Be.forEach(n=>{localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Be.forEach(n=>{le.add(n.id),localStorage.getItem(`alert_read_ts_${n.id}`)||localStorage.setItem(`alert_read_ts_${n.id}`,Date.now().toString())}),ne(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{le.clear(),ne(!0)},i.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{if(confirm("Are you sure you want to delete all alerts from the backend?")){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{await B("/events?type=system.notification.generated",{method:"DELETE"});let n=Date.now()-48*60*60*1e3;Be.forEach(c=>{localStorage.setItem(`alert_read_ts_${c.id}`,n.toString())}),le.clear(),ne(!0)}catch(n){console.error("Failed to clear alerts:",n)}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}async function Ue(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await B(t);if(!e.ok)return;let s=(await e.json()).events||[],n=0;s.forEach(c=>{let r=c.event;if(typeof r=="string")try{r=JSON.parse(r)}catch{r={}}if((r.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${c.id}`)||n++}),ct(n)}catch{}}var gt=()=>`
...
`,ce=new Set,_e=[],he=null;async function de(t=!1){let e=document.getElementById("roadmap-list");if(e){ps();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();_e=s;let n=m=>{let y=ce.has(m.id),p=m.state==="published",v=m.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),v&&(a="event-border-purple");let S=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=m.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",ce.delete(m.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",ce.add(m.id))};let f=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${R(m.content)}</div>
      ...
      `;let o=h.querySelector(".event-details");return o&&o.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>ms(m)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>us(m)),h.querySelector(".delete-btn")?.addEventListener("click",()=>fs(m.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",ce.delete(m.id)}),h},c=Array.from(e.children),r=new Map(c.map(m=>[m.dataset.itemId,m])),b=new Set(s.map(m=>m.id));if(c.forEach(m=>{let y=m.dataset.itemId;(!y||!b.has(y))&&m.remove()}),!_e||_e.length===0){e.innerHTML=H("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let l=null;s.forEach((m,y)=>{let p=r.get(m.id);(!p||t)&&(p&&p.remove(),p=n(m),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):l&&l.nextElementSibling!==p&&l.after(p),l=p)})}catch{e.children.length===0&&(e.innerHTML=H("offline","Failed to load roadmap.","The event service may be offline."))}}}function ps(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{he=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",he=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let r=he?`/roadmap/${he}`:"/roadmap",b=he?"PATCH":"POST";try{await B(r,{method:b,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",de(!0)}catch(l){console.error(l)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{_e.forEach(c=>ce.add(c.id)),de(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{ce.clear(),de(!0)},n.dataset.listenerAttached="true")}function ms(t){he=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function us(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),de(!0)}catch(i){console.error(i)}}async function fs(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),ce.delete(t),de(!0)}catch(e){console.error(e)}}var xt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,vt=null,pe=new Set,ht=[];async function me(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;gs();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(ht=c,vt=Date.now(),X(2,vt),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),$e(2,0);return}t&&(e.innerHTML="");let r=p=>{let v=p.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let a=(v.title||"Untitled Blueprint").trim(),g=(v.summary||v.body||"No summary provided.").trim(),S=(v.content||"").trim(),h=(v.category||"architecture").trim(),f=(v.related_services||v.affected_services||[]).map(D=>D.trim()),o=(v.implementation_path||[]).map(D=>D.trim()),d=v.source_event_ids||[],u=v.approved===!0,x=new Date(p.timestamp*1e3),A=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=pe.has(p.id),T=k?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${u?"blueprint-approved":""}`,E.dataset.blueprintId=p.id,u&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=u?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";E.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),pe.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),pe.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let O="";o.length>0&&(O=`
                ...
                `);let I=f.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${f.join(", ")}</span></div>`:"<div></div>",q=z(),w=u?`
            ...
            `;E.innerHTML=`
                ...
                <div class="event-icon" style="${u?"color: #03dac6;":""}"><i class='bx ${U}'></i></div>
                <div class="event-content">
...
`;async function qe(){await Promise.all([de(),me()])}var St=()=>`
    ...
    `;async function we(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,b=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:b,natural_instruction:c,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",we()}catch(l){console.error(l),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let c=await B("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);let r=await c.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=H("empty","No active chores.","Create one to start monitoring.");return}let b=r.map(l=>{let m=l.last_run===0?"Never":new Date(l.last_run*1e3).toLocaleString(),y=l.memory?l.memory.length:0,p=l.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=b,t.querySelectorAll(".delete-chore-btn").forEach(l=>{l.onclick=async m=>{m.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${l.dataset.id}`,{method:"DELETE"}),we())}})}catch(c){console.error("Failed to fetch chores",c)}}var bs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Et(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function kt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=bs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ae(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();je(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=Et(t.id,t.username);je(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=Et(t.id,t.username);je(e,t,s)})}function je(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let S=(i.badges||[]).map(A=>`<span class="profile-badge ${A==="Creator"?"master":""}">${A}</span>`).join(""),h=(i.attributes||i.facts||[]).map(A=>{let L=A.key||A.k,k=A.value||A.v;return`
        ...
        `},c=()=>{let S=i.dossier||{},h=S.identity||{},f=S.career||{},o=S.personal||{},d=S.social||[];return`
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(f.skills||[]).map(u=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${u}</span>`).join("")}
                         </div>
                    </div>
                </div>
                ...
                <div class="dossier-column">
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.hobbies||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                    ...
                    <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.habits||[]).map(u=>`<div class="dossier-list-item">${u}</div>`).join("")}
                        </div>
                    </div>
                     ...
                     <div class="dossier-item" style="margin-top: 20px;">
                        ...
                        <div style="margin-top: 10px;">
                            ${(o.vices||[]).map(u=>`<div class="dossier-list-item" style="color: #cf6679;">${u}</div>`).join("")}
                        </div>
                    </div>
                </div>
                    ...
                    <div class="dossier-item" style="background: none; border: none; padding: 0;">
                        ${d.length>0?d.map(u=>`
                            <div class="friend-chip">
                                <div style="width: 35px; height: 35px; background: #333; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #888;">${u.name.substring(0,1)}</div>
                                <div style="flex: 1;">
                                    <div style="color: #fff; font-weight: 500;">${u.name}</div>
                                    <div style="font-size: 0.75em; color: #888;">${u.relation}</div>
                                </div>
                                <div style="font-size: 0.8em; color: #03dac6;">${u.trust}</div>
                            </div>
            ...
            ${Object.entries(S).map(([h,f])=>`
                <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                    <div style="width: 10px; height: 150px; background: rgba(255,255,255,0.05); border-radius: 5px; position: relative; overflow: hidden;">
                        <div style="position: absolute; bottom: 0; left: 0; width: 100%; height: ${f}%; background: ${f>50?"#03dac6":"#cf6679"}; transition: height 1s;"></div>
                    </div>
...
`,vs=null;async function Fe(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Fe(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);kt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Ae("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(vs=Date.now(),n.length===0){t.innerHTML=H("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,b)=>{let l=c[r.level]??10,m=c[b.level]??10;return l!==m?l-m:r.username.localeCompare(b.username)}),t.innerHTML=n.map(r=>{let b=r.color&&r.color!==0,l=b?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",m=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",y="#888";r.level==="Me"||r.level==="Master"?y="#bb86fc":r.level==="Admin"?y="#cf6679":r.level==="Moderator"?y="#03dac6":r.level==="Contributor"&&(y="#ffa500");let p=r.level==="Me",v=p?"rgba(187, 134, 252, 0.08)":b?`${l}11`:"rgba(255,255,255,0.03)",a=p?"2px solid #bb86fc":b?`1px solid ${l}66`:"1px solid rgba(255,255,255,0.1)";return`
...
`,De=null,ue=new Set,Se=[],K="all",ys={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},hs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ws(t){for(let[e,i]of Object.entries(ys))if(i.includes(t))return e;return"system"}function Cs(t){return hs[t]||"bx-square-rounded"}async function ae(t=!1){let e=document.getElementById("events-timeline");if(!e)return;$s();let s=`/events?ml=${K==="all"?100:250}&format=json`;K!=="all"&&(s+=`&category=${K}`);try{let n=await B(s);if(!n.ok)throw new Error("Service unreachable");if(Se=((await n.json()).events||[]).filter(v=>{let a=v.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return!0}let g=a.type;return!(g==="system.blueprint.generated"||g==="system.notification.generated")}),De=Date.now(),X(1,De),Se.length===0){e.innerHTML=H("empty","No events found for this filter.");return}t&&(e.innerHTML="");let b=v=>{let a=v.event;if(typeof a=="string")try{a=JSON.parse(a)}catch{return null}let g=a.type,S=ws(g),h=Cs(g),f=g==="engagement.decision"||g==="messaging.bot.sent_message"||g==="messaging.user.sent_message"||g==="moderation.explicit_content.deleted"||g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="analysis.user.message_signals"||g==="system.cli.command"||g==="system.analysis.audit"||g==="system.test.completed"||g==="error_occurred"||g==="system.cli.status"||g==="system.attention.expired"||g.startsWith("system.roadmap")||g.startsWith("system.process"),o="event-border-grey";f&&(g==="moderation.explicit_content.deleted"||g==="error_occurred"?o="event-border-red":g==="analysis.link.completed"||g==="analysis.visual.completed"||g==="analysis.router.decision"||g==="system.analysis.audit"||g==="analysis.user.message_signals"?o="event-border-purple":g==="system.attention.expired"||g==="system.cli.command"||g==="system.cli.status"?o="event-border-orange":g==="system.test.completed"?o=a.test?.status==="OK"&&a.lint?.status==="OK"&&a.format?.status==="OK"?"event-border-blue":"event-border-red":o="event-border-blue");let d=f?"cursor-pointer":"",u=ue.has(v.id),x=u?"expanded":"",A=u?"display: block;":"display: none;",L=new Date(v.timestamp*1e3),k=L.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=L.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),E=Lt(g,a),_=a.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${a.user_level})</span>`:"",U="";if(f){let w="";if(g==="engagement.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                    ...
                    `}else if(g==="system.attention.expired"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$=a.timestamp-a.last_active,M=Math.floor($/60);w=`
                    ...
                    `}else if(g==="messaging.bot.sent_message"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$="";a.eval_count&&($=`
                            <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                                <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        `);let M="";if(a.chat_history&&a.chat_history.length>0){let N=a.chat_history.length,D=a.chat_history.map((j,J)=>{let W=j.name?j.name.toUpperCase():j.role.toUpperCase();!j.name&&W==="ASSISTANT"&&(W="DEXTER");let G=j.role==="user"?"#03dac6":j.role==="system"?"#ffb74d":"#bb86fc",V=J===N-1?"block":"none";return`
                                <div class="history-slide" data-index="${J}" style="display: ${V};">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: #eee; white-space: pre-wrap; overflow-x: auto; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; max-height: 300px; overflow-y: auto;">${R(j.content)}</div>
                                </div>
                            `}).join("");M=`
                            <div class="event-detail-block">
                        ...
                        `}
                    `}else if(g==="analysis.user.message_signals"){let C=N=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${N}</h5>`,$=a.signals||{};w=`
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                    ...
                    `}else if(g==="moderation.explicit_content.deleted"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.link.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="analysis.visual.completed"){let C=M=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${M}</h5>`,$="";a.base64_preview?$=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${a.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:a.url&&($=`<div class="event-detail-block"><img src="${a.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),w=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="analysis.router.decision"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.cli.command"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="system.analysis.audit"){let C=a.success?"#03dac6":"#ff4d4d",$=a.success?"SUCCESS":"FAILED",M=W=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${W}</h5>`,N="";a.error&&(N=`
                            <div class="event-detail-block">
                                ${M("Error")}
                                <pre class="detail-pre" style="color: #ff4d4d; border-color: rgba(255, 77, 77, 0.2);">${R(a.error)}</pre>
                            </div>
                        `);let D="";if(a.chat_history&&a.chat_history.length>0){let W=a.chat_history.length,G=a.chat_history.map((V,ge)=>{let Q=V.name?V.name.toUpperCase():V.role.toUpperCase();!V.name&&Q==="USER"&&(Q="SYSTEM"),!V.name&&Q==="ASSISTANT"&&(Q="AGENT");let Z=V.role==="user"?"#03dac6":V.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let j="";if(a.corrections&&a.corrections.length>0){let W=a.corrections.map((G,V)=>`
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 15px; margin-bottom: 20px; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 4px; border: 1px solid rgba(255,255,255,0.05); justify-content: space-between; align-items: center;">
                            ...
                            <div style="flex: 1; min-width: 150px; text-align: center;">
                                ...
                                <div style="font-family: 'JetBrains Mono', monospace; font-size: 0.85em; color: ${C}; font-weight: bold;">${$} <span style="font-weight: normal; opacity: 0.6; font-size: 0.9em;">(${a.attempts} att)</span></div>
                            </div>
                            <div style="flex: 1; min-width: 100px; text-align: center;">
                        ...
                        ${j}
                        ${D}
                    `}else if(g==="system.test.completed"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row">
                    ...
                    `}else if(g==="error_occurred"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                    ...
                    `}else if(g==="system.cli.status"){let C=$=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${$}</h5>`;w=`
                        <div class="event-detail-row" style="margin-bottom: 15px;">
                            <span class="detail-label">Status:</span>
                    ...
                    `}else if(g==="messaging.user.sent_message"){let C="";a.attachments&&a.attachments.length>0&&(C=`
                            <div class="event-detail-block">
                ...
                `}let O=document.createElement("div");O.className=`event-item ${o} ${d} ${x}`,O.dataset.eventId=v.id,O.onclick=function(w){if(!f)return;if(this.classList.contains("expanded")){this.classList.remove("expanded"),ue.delete(v.id);let $=this.querySelector(".event-details");$&&($.style.display="none")}else{this.classList.add("expanded"),ue.add(v.id);let $=this.querySelector(".event-details");$&&($.style.display="block")}},O.innerHTML=`
                <div class="event-time">
            ...
            `;let F=O.querySelector(".event-details");if(F&&F.addEventListener("click",w=>{w.stopPropagation()}),f){let w=O.querySelector(".close-details-btn");w&&w.addEventListener("click",C=>{C.stopPropagation();let $=C.target.closest(".event-item");if($){$.classList.remove("expanded"),ue.delete(v.id);let M=$.querySelector(".event-details");M&&(M.style.display="none")}})}let I=O.querySelector(".prev-btn"),q=O.querySelector(".next-btn");if(I&&q){let w=0,C=O.querySelectorAll(".history-slide"),$=C.length,M=()=>{C.forEach((N,D)=>{N.style.display=D===w?"block":"none"}),I.disabled=w===0,q.disabled=w===$-1,I.style.opacity=w===0?"0.5":"1",q.style.opacity=w===$-1?"0.5":"1"};I.addEventListener("click",N=>{N.stopPropagation(),w>0&&(w--,M())}),q.addEventListener("click",N=>{N.stopPropagation(),w<$-1&&(w++,M())}),M()}return O},l=Array.from(e.children),m=new Map(l.map(v=>[v.dataset.eventId,v])),y=new Set(Se.map(v=>v.id));l.forEach(v=>{let a=v.dataset.eventId;(!a||!y.has(a))&&v.remove()});let p=null;Se.forEach((v,a)=>{let g=m.get(v.id);(!g||t)&&(g&&g.remove(),g=b(v),!g)||(a===0?e.firstElementChild!==g&&e.prepend(g):p&&p.nextElementSibling!==g&&p.after(g),p=g)}),De=Date.now(),X(1,De)}catch(n){console.error("Error fetching events:",n),e.children.length===0&&(e.innerHTML=H("offline","Failed to load events.","The event service may be offline."))}}function $s(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Se.forEach(n=>ue.add(n.id)),ae(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ue.clear(),ae(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(n=>{n.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(c=>c.classList.remove("active")),n.classList.add("active"),K=n.dataset.filter,ae(!0)}}),i.dataset.listenerAttached="true"),i&&i.querySelectorAll(".filter-btn").forEach(n=>{n.dataset.filter===K?n.classList.add("active"):n.classList.remove("active")});let s=document.getElementById("events-clear");s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let n=K==="all"?"ALL":K.toUpperCase();if(confirm(`Are you sure you want to delete ${n} events from the backend? This cannot be undone.`)){s.innerHTML="<i class='bx bx-loader-alt spin'></i> Clearing...";try{let c="/events";if(K!=="all"?c+=`?category=${K}`:c+="?exclude_types=system.blueprint.generated,system.notification.generated",!(await B(c,{method:"DELETE"})).ok)throw new Error("Failed to delete events");ue.clear(),ae(!0)}catch(c){console.error("Failed to clear events:",c),alert("Failed to clear events. Check console.")}finally{s.innerHTML="<i class='bx bx-trash'></i> Clear"}}},s.dataset.listenerAttached="true")}var Ss=`<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 348 346" style="color: #bb86fc; fill: currentColor; margin-right: 10px; min-width: 24px; margin-left: 0; max-width: 25px;">
...
`,Mt=Ss;function At(){return`
    ...
    `}var Es=null;async function Ee(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await B("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=H("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let s=["local-ollama-0","local-cache-0"],n=i.filter(r=>!s.includes(r.id));n.forEach(r=>{r.logs&&Array.isArray(r.logs)?r.logs.reverse():r.logs=[]}),n.reverse();let c=n.map(r=>{let b=r.logs.join(`
`),l=[...r.logs];if(l.length<25){let y=25-l.length;for(let p=0;p<y;p++)l.push("")}else l.length>25&&(l=l.slice(-25));let m=l.map(y=>Me(y)).join(`
`);return`
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(r=>{r.addEventListener("click",()=>{let b=unescape(r.dataset.logs);navigator.clipboard.writeText(b).then(()=>{let l=r.querySelector("i");l.classList.remove("bx-copy"),l.classList.add("bx-check"),setTimeout(()=>{l.classList.remove("bx-check"),l.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(r=>{r.addEventListener("click",async()=>{let b=r.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${b}?`))try{(await B(`/logs?service_id=${b}`,{method:"DELETE"})).ok&&Ee()}catch(l){console.error("Error clearing logs:",l)}})}),Es=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=H("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var Dt=()=>{let t=z()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${At()}
        </div>`;async function Ot(){ks(),await Promise.all([fe(),Ge(),Ve(),we()]),await Ee()}var ze=null;function ks(){ze||(ze=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(ze),ze=null;return}fe(!0)},1e3))}var It=null,Bt=null;async function Ut(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function _t(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function Ts(){try{return await(await B("/processes")).json()}catch{return null}}async function Ls(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Ge(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),b=o=>{if(o){if(i&&(i.innerHTML=`
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(o.GPU&&o.GPU.length>0?c.innerHTML=o.GPU.map(d=>{let u=(d.VRAM/1073741824).toFixed(1);return`
                    ...
                    </div>`}).join(""):r&&(r.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let o=await _t();o?(b(o),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let o=await _t();b(o)}if(!t)return;let l=await Ut();if(!l||!l.services){t.children.length===0&&(t.innerHTML=H("offline","Failed to load system metrics.","The event service may be offline."));let o=document.querySelector('[data-service-id="upstash-redis-ro"]');if(o){o.className="service-widget service-widget-offline",o.querySelector(".service-widget-status").textContent="ERROR";let d=o.querySelector(".service-widget-body");d&&(d.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}It=z()&&se.lastDashboard||Date.now(),X(0,It);let m=l.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let o=document.getElementById("upstash-countdown");if(o){let x=59-new Date().getSeconds();x<=0&&(x+=60),o.textContent=`${x}s`}},1e3)),Array.from(t.children).forEach(o=>{o.classList.contains("service-widget")||o.remove()});function y(o){return!o||o==="N/A"||o==="unknown"||o.trim()===""?"-":o}function p(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:o.split(".").slice(0,3).join(".")||"-"}function v(o){return!o||o.length<=28?o:o.substring(0,28)+"..."}function a(o){if(!o||o==="N/A"||o==="unknown")return"-";let d=o.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let u=parseInt(d[1])||0,x=parseInt(d[2])||0,A=parseInt(d[3])||0,L=parseFloat(d[4])||0,k=u*86400+x*3600+A*60+L,T=Math.floor(k/86400);if(T>0)return`${T}d`;let E=Math.floor(k/3600);if(E>0)return`${E}h`;let _=Math.floor(k/60);return _>0?`${_}m`:`${Math.floor(k)}s`}function g(o){if(o.id==="upstash-redis-ro"){let w=z()&&(se.lastFrontend||se.lastDashboard)||Date.now();return`
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=o.status==="online",u=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",A=d?"OK":"BAD",L=o.version?p(o.version.str):"-",k=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",T=o.memory&&o.memory!=="N/A"?o.memory:"-",E=k!=="-"?"#00ff00":"#666",_=k!=="-"?"#fff":"#666",U=T!=="-"?"#00ff00":"#666",O=T!=="-"?"#fff":"#666",F=a(o.uptime),I="";d?I=`
                <div class="service-widget-info">
                ...
                </div>`:I='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let q=z()?"easter.company":v(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${u}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${A}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${q}</span></div>${I}</div></div>`}let S=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),h=new Set(m.map(o=>o.id));for(let[o,d]of S)h.has(o)||d.remove();let f=new Set;m.forEach(o=>{if(f.has(o.id))return;f.add(o.id);let d=g(o),u=S.get(o.id);if(u&&u.parentNode)if(o.id==="upstash-redis-ro"){let x=z()&&(se.lastFrontend||se.lastDashboard)||Date.now(),A=new Date(x).toLocaleTimeString(),L=u.querySelector(".sync-time-label");L&&(L.textContent=`Last synced: ${A}`)}else u.outerHTML!==d&&(u.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Ve(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ut();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}Bt=Date.now(),X(2,Bt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let m=l.status==="Ready";return`
                ...
                </div>`}function c(l){let m=l.status==="Ready";return`
                ...
                </div>`}function r(l){let m=l.status==="Downloaded",y=m?"service-widget-online":"service-widget-offline",p=m?"OK":"MISSING",v=m&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",a=l.name;return l.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let b="";if(s&&(b+=n(s)),e.xtts&&(b+=c(e.xtts)),b+=i.map(r).join(""),!b){t.innerHTML=H("empty","No models found.");return}t.innerHTML!==b&&(t.innerHTML=b)}async function fe(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),b=document.getElementById("guardian-reset-btn"),l=document.getElementById("analyzer-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let o=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B(o,{method:"POST"}),setTimeout(()=>fe(),500)}catch{m.innerHTML="<i class='bx bx-error'></i>"}},m.dataset.listenerAttached="true"),b&&!b.dataset.listenerAttached&&(b.onclick=async()=>{b.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{b.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{b.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),fe()}catch{b.innerHTML="<i class='bx bx-error'></i>"}},b.dataset.listenerAttached="true"),l&&!l.dataset.listenerAttached&&(l.onclick=async()=>{l.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{l.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{l.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),fe()}catch{l.innerHTML="<i class='bx bx-error'></i>"}},l.dataset.listenerAttached="true");let y=await Ls();if(y&&y.agents){let f=y.agents.guardian||{protocols:{}},o=y.agents.analyzer||{protocols:{}},d=y.agents.imaginator||{protocols:{}},u=y.system||{},x=Math.floor(Date.now()/1e3),A={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review"},L=I=>{I<0&&(I=0);let q=Math.floor(I/3600),w=Math.floor(I%3600/60),C=I%60;return q>0?`${q}h ${w}m`:w>0?`${w}m ${C}s`:`${C}s`},k=(I,q,w,C)=>{if(!I)return;let $=A[C]||C.toUpperCase(),M=I.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(M&&(M.textContent=$),!w){I.textContent="Inactive",I.style.color="#666";return}let N=w.status;if(N==="Working")I.textContent="Working",I.style.color="#bb86fc";else if(N==="Ready")I.textContent="Ready",I.style.color="#5eff5e";else{let D=w.cooldown;if(z()&&(D=w.next_run-x),D<=0)I.textContent="Ready",I.style.color="#5eff5e";else{let j=Math.floor(D/60),J=D%60;I.textContent=`${j}m ${J}s`,I.style.color="#fff"}}q&&w.stats&&(q.innerHTML=`
          ...
          `)};i&&k(i,document.getElementById("guardian-sentry-stats"),f.protocols.sentry,"sentry");let T=document.getElementById("analyzer-synthesis-val"),E=document.getElementById("analyzer-synthesis-stats");T&&k(T,E,o.protocols.synthesis,"synthesis");let _=document.getElementById("imaginator-alert_review-val"),U=document.getElementById("imaginator-alert_review-stats");_&&k(_,U,d.protocols.alert_review,"alert_review");let O=document.getElementById("system-state-label"),F=document.getElementById("system-state-val");if(F&&u.state){let I=u.state,q=L(u.state_time||0);O&&(O.textContent=`State: ${I.toUpperCase()}`),F.textContent=q,I==="idle"?F.style.color=u.state_time>300?"#5eff5e":"#fff":F.style.color="#bb86fc",m&&(m.querySelector(".bx-loader-alt")||m.querySelector(".bx-error")||(I==="paused"?(m.innerHTML="<i class='bx bx-play'></i>",m.title="Resume System",m.style.color="#ff9800"):(m.innerHTML="<i class='bx bx-pause'></i>",m.title="Pause System",m.style.color="")))}n&&(n.textContent=L(u.metrics?.total_idle_time||0)),c&&(c.textContent=L(u.metrics?.total_active_time||0)),r&&(r.textContent=L(u.metrics?.total_waste_time||0))}else[i,s,n,c,r].forEach(o=>{o&&(o.textContent="-",o.style.color="#666")});if(t)return;let p=await Ts(),v=[],a=[],g=[];p&&(Array.isArray(p)?v=p:(v=p.active||[],a=p.queue||[],g=p.history||[],g.sort((f,o)=>(o.end_time||0)-(f.end_time||0)))),v.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=H("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),We(e,v,!1));let S=document.getElementById("processes-queue-widgets");S&&(a.length===0?!S.querySelector(".tab-placeholder")&&!S.querySelector('div[style*="font-style: italic"]')&&(S.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(S.innerHTML="",We(S,a,!1)));let h=document.getElementById("processes-history-widgets");h&&(g.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=H("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),We(h,g,!0))),$e(1,v.length)}function We(t,e,i){function s(l){let m="";l.end_time?m=`${l.end_time-l.start_time}s`:m=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let y=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(v[p]?p=v[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ze=!0,i}function Jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ze=!1}function Wt(t,e,i="output"){if(!Ze)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Me(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Is(t){let e=Gt.find(n=>n.id===t);if(!e)return;let i=As(e);Wt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(r=>setTimeout(r,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),Wt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Is(s)})})}async function Bs(){if(!z())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Kt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}tt(),st(),Bs(),rt(),et();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Vt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=f=>{localStorage.setItem("dex_last_window",f)};function n(){for(;e.length>1;)e.shift().close(!0);let f=document.getElementById("windows-container"),o=document.querySelector("nav"),d=document.querySelector("footer"),u=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");f&&f.setAttribute("data-count",e.length),e.forEach(_=>{let U=document.getElementById(_.id);U&&U.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let _=document.getElementById("dexter-dropdown"),U=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),U&&U.classList.remove("active"),f&&f.classList.remove("menu-open")}if(be(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",o?.classList.add("window-open"),f&&(f.style.paddingTop="60px"),A&&(A.style.display=E?"flex":"none"),k&&(k.style.display=E?"block":"none"),!E&&L){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(b)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(l)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(m)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):L.innerHTML=""}else L&&(L.innerHTML="");else o?.classList.remove("window-open"),T&&(T.style.display="none"),f&&(f.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",u||x?d?.classList.remove("hide"):d?.classList.add("hide"),A&&(A.style.display="flex"),k&&(k.style.display="block"),L&&(L.innerHTML="");ye()}function c(f){if(f.isOpen()){f.close();return}for(;e.length>0;)e.pop().close(!0);e.push(f),f.open(),n()}function r(){[...e].forEach(o=>o.close()),e=[]}window.addEventListener("resize",n);let b=oe({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:ft(),onOpen:()=>ne(),onClose:()=>{let f=e.indexOf(b);f>-1&&e.splice(f,1),n()}}),l=oe({id:"events-window",title:"Events",icon:"bx-calendar-event",content:Je(),onOpen:()=>{l.setContent(Je()),ae()},onClose:()=>{let f=e.indexOf(l);f>-1&&e.splice(f,1),n()}}),m=oe({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Tt(),onOpen:()=>Fe(),onClose:()=>{let f=e.indexOf(m);f>-1&&e.splice(f,1),n()}}),y=oe({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Nt()},{icon:"bxs-component",title:"Processes",content:zt()},{icon:"bxs-brain",title:"Models",content:Ht()},{icon:"bx-globe",title:"Web",content:jt()},{icon:"bxs-hdd",title:"Hardware",content:Pt()},{icon:"bxs-terminal",title:"Logs",content:Rt()},{icon:"bxs-zap",title:"Agents",content:Dt()},{icon:"bx-task",title:"Chores",content:St()}].filter(f=>z()?f.title!=="Hardware"&&f.title!=="Logs":!0),onOpen:()=>{Ge(),fe(),Ve(),Ft(),Ee(),we()},onClose:()=>{let f=e.indexOf(y);f>-1&&e.splice(f,1),n()}}),p=oe({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-map-alt",title:"Roadmap",content:Ct()},{icon:"bx-paint",title:"Blueprints",content:$t()}],onOpen:()=>qe(),onClose:()=>{let f=e.indexOf(p);f>-1&&e.splice(f,1),n()}}),v=oe({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ne(),onOpen:()=>{v.setContent(Ne()),Ke(v)},onClose:()=>{let f=e.indexOf(v);f>-1&&e.splice(f,1),n()}});window.dexter={viewEvent:f=>{l.isOpen()||c(l),setTimeout(()=>{let o=document.querySelector(`.event-item[data-event-id="${f}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)},viewAlert:f=>{b.isOpen()||c(b),setTimeout(()=>{let o=document.querySelector(`.event-item[data-alert-id="${f}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let f=document.getElementById("dexter-dropdown"),o=document.getElementById("dexter-menu-btn");f&&o&&(f.classList.remove("active"),o.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let f=document.getElementById("dexter-menu-btn"),o=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),u=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),A=document.getElementById("windows-container");f&&x&&(f.onclick=k=>{k.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let _=f.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),F=220,I=10,w=_.left+_.width/2-F/2,C=O.right-10;w+F>C&&(w=C-F),w<10&&(w=10),x.style.top=O.bottom+I+"px",x.style.left=w+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),f.classList.toggle("active");let E=x.classList.contains("active");if(T){let _=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),A?.classList.add("menu-open"),be(!0)):(A?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),be(!1)))}}),o&&(o.onclick=k=>{k.stopPropagation(),c(v)}),d&&(d.onclick=k=>{k.stopPropagation(),r()});let L=(k,T,E)=>{let _=document.getElementById(k);_&&(_.onclick=U=>{U.stopPropagation(),a(),E&&s(E),c(T)})};L("alerts-menu-item",b,"alerts-window"),L("events-menu-item",l,"events-window"),L("monitor-menu-item",y,"monitor-window"),L("contacts-menu-item",m,"contacts-window"),L("workspace-menu-item",p,"workspace-window"),u&&(u.onclick=k=>{if(k.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),be(!1));return}if(e.length>0)r();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let U=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");U.pop();let O=U.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let k=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),k&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),be(!1)))}),x&&(x.onclick=k=>k.stopPropagation())})();let S=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!S&&!h&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{l.isOpen()&&ae(),b.isOpen()?ne():Ue()},1e3),setInterval(()=>{z()&&(b.isOpen()?ne():Ue()),p.isOpen()?qe():wt(),z()&&l.isOpen()&&ae(),y.isOpen()&&Ot()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Kt):Kt();})();

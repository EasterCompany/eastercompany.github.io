...
`,le=new Set,De=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ps();try{let i=await _("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();De=s;let n=u=>{let y=le.has(u.id),p=u.state==="published",b=u.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),b&&(a="event-border-purple");let S=new Date(u.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=u.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(u.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(u.id))};let f=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${u.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
      ...
      `;let o=h.querySelector(".event-details");return o&&o.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>ms(u)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>us(u)),h.querySelector(".delete-btn")?.addEventListener("click",()=>fs(u.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(u.id)}),h},c=Array.from(e.children),r=new Map(c.map(u=>[u.dataset.itemId,u])),x=new Set(s.map(u=>u.id));if(c.forEach(u=>{let y=u.dataset.itemId;(!y||!x.has(y))&&u.remove()}),!De||De.length===0){e.innerHTML=H("empty","Your roadmap is empty.",z()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let l=null;s.forEach((u,y)=>{let p=r.get(u.id);(!p||t)&&(p&&p.remove(),p=n(u),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):l&&l.nextElementSibling!==p&&l.after(p),l=p)})}catch{e.children.length===0&&(e.innerHTML=H("offline","Failed to load roadmap.","The event service may be offline."))}}}function ps(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let r=xe?`/roadmap/${xe}`:"/roadmap",x=xe?"PATCH":"POST";try{await _(r,{method:x,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(l){console.error(l)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{De.forEach(c=>le.add(c.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function ms(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function us(t){let e=t.state==="published"?"draft":"published";try{await _(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function fs(t){if(confirm("Delete this roadmap item?"))try{await _(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var yt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,xt=null,de=new Set,wt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;gs();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await _(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(wt=c,xt=Date.now(),X(2,xt),c.length===0){e.innerHTML=H("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),Ce(2,0);return}t&&(e.innerHTML="");let r=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),S=(b.content||"").trim(),h=(b.category||"architecture").trim(),f=(b.related_services||b.affected_services||[]).map(D=>D.trim()),o=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],m=b.approved===!0,v=new Date(p.timestamp*1e3),A=v.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=v.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=de.has(p.id),T=k?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${m?"blueprint-approved":""}`,E.dataset.blueprintId=p.id,m&&(E.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",E.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let U=m?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";E.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let O="";o.length>0&&(O=`
                ...
                `);let B=f.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${f.join(", ")}</span></div>`:"<div></div>",j=z(),C=m?`
...
`;async function qe(){await Promise.all([ce(),pe()])}var St=()=>`
    ...
    `;async function ye(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),i=document.getElementById("create-chore-form"),s=document.getElementById("save-chore-btn"),n=document.getElementById("cancel-chore-btn");if(e&&!e.dataset.listenerAttached&&(e.onclick=()=>{i.style.display=i.style.display==="none"?"block":"none"},e.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{i.style.display="none"},n.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=async()=>{let c=document.getElementById("new-chore-instruction").value,r=document.getElementById("new-chore-url").value,x=document.getElementById("new-chore-owner").value||"313071000877137920";if(c){s.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/chores",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({owner_id:x,natural_instruction:c,entry_url:r,schedule:"every_6h"})}),i.style.display="none",document.getElementById("new-chore-instruction").value="",document.getElementById("new-chore-url").value="",ye()}catch(l){console.error(l),alert("Failed to create chore")}finally{s.innerHTML="Create Task"}}},s.dataset.listenerAttached="true"),!!t)try{let c=await _("/chores");if(!c.ok)throw new Error(`HTTP error! status: ${c.status}`);let r=await c.json();if(!Array.isArray(r)||r.length===0){t.innerHTML=H("empty","No active chores.","Create one to start monitoring.");return}let x=r.map(l=>{let u=l.last_run===0?"Never":new Date(l.last_run*1e3).toLocaleString(),y=l.memory?l.memory.length:0,p=l.status==="active"?"#03dac6":"#666";return`
            ...
            `}).join("");t.innerHTML=x,t.querySelectorAll(".delete-chore-btn").forEach(l=>{l.onclick=async u=>{u.stopPropagation(),confirm("Delete this chore?")&&(await _(`/chores/${l.dataset.id}`,{method:"DELETE"}),ye())}})}catch(c){console.error("Failed to fetch chores",c)}}var bs=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Et(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function kt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=bs,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Ie(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();Fe(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=Et(t.id,t.username);Fe(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=Et(t.id,t.username);Fe(e,t,s)})}function Fe(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let S=(i.badges||[]).map(A=>`<span class="profile-badge ${A==="Creator"?"master":""}">${A}</span>`).join(""),h=(i.attributes||i.facts||[]).map(A=>{let L=A.key||A.k,k=A.value||A.v;return`
        ...
        `}).join(""),f=i.cognitive_model||i,o=f.technical_level||f.techLevel||0,d=f.communication_style||f.commStyle||"Unknown",m=f.patience_level||f.patience||"Unknown",v=f.vibe||"Unknown";return`
                    ...
                    <div class="dossier-item">
                        ...
                        <div style="height: 15px;"></div>
                         ...
                         <div style="display: flex; flex-wrap: wrap; gap: 5px; margin-top: 5px;">
                            ${(f.skills||[]).map(m=>`<span style="font-size: 0.75em; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 4px;">${m}</span>`).join("")}
                         </div>
                    </div>
                </div>
...
`,vs=null;async function Je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);kt(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Ie("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(vs=Date.now(),n.length===0){t.innerHTML=H("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((r,x)=>{let l=c[r.level]??10,u=c[x.level]??10;return l!==u?l-u:r.username.localeCompare(x.username)}),t.innerHTML=n.map(r=>{let x=r.color?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",l=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",u="#888";r.level==="Me"||r.level==="Master"?u="#bb86fc":r.level==="Admin"?u="#cf6679":r.level==="Moderator"?u="#03dac6":r.level==="Contributor"&&(u="#ffa500");let y=r.level==="Me",p=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${x}33`;return`
                <div class="user-profile-card" 
                     data-user="${JSON.stringify(r).replace(/"/g,"&quot;")}"
                     style="background: ${p}; padding: 15px; border-radius: 8px; border: ${b}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden; cursor: pointer; transition: transform 0.2s, background 0.2s;">
                    <div class="card-glow" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 0% 0%, ${y?"#bb86fc":x}22, transparent 70%); pointer-events: none;"></div>
                    <div style="position: relative;">
                        <img src="${r.avatar_url}" style="width: 45px; height: 45px; border-radius: 50%; background: #222; border: ${y?"2px solid #bb86fc":"none"};" onerror="this.src='https://cdn.discordapp.com/embed/avatars/0.png'">
                        <div style="position: absolute; bottom: 0; right: 0; width: 12px; height: 12px; background: ${l}; border: 2px solid #1a1a1a; border-radius: 50%;"></div>
                    </div>
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${u}; font-weight: 600; text-transform: uppercase;">${y?"DEXTER (ME)":r.level}</span>
                        </div>
...
`);return`
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${At()}
        </div>`;async function Ot(){ks(),await Promise.all([he(),Ve(),Ke(),ye()]),await Se()}var Ne=null;function ks(){Ne||(Ne=setInterval(()=>{if(!document.getElementById("guardian-sentry-val")){clearInterval(Ne),Ne=null;return}he(!0)},1e3))}var It=null,Bt=null;async function Ut(){try{return await(await _("/system_monitor")).json()}catch{return null}}async function _t(){try{return await(await _("/system/hardware")).json()}catch{return null}}async function Ts(){try{return await(await _("/processes")).json()}catch{return null}}async function Ls(){try{return await(await _("/agent/status")).json()}catch{return null}}async function Ve(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),r=document.querySelector("#hw-storage .hw-content"),x=o=>{if(o){if(i&&(i.innerHTML=`
            ...
            <div class="service-widget service-widget-online" data-service-id="upstash-redis-ro">
            ...
            </div>`}let d=o.status==="online",m=d?"service-widget-online":"service-widget-offline",v=d?"bx-check-circle":"bx-x-circle",A=d?"OK":"BAD",L=o.version?p(o.version.str):"-",k=o.cpu&&o.cpu!=="N/A"?o.cpu:"-",T=o.memory&&o.memory!=="N/A"?o.memory:"-",E=k!=="-"?"#00ff00":"#666",I=k!=="-"?"#fff":"#666",U=T!=="-"?"#00ff00":"#666",O=T!=="-"?"#fff":"#666",F=a(o.uptime),B="";d?B=`
                <div class="service-widget-info">
                ...
                </div>`:B='<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>';let j=z()?"easter.company":b(o.domain&&o.port?`${o.domain}:${o.port}`:"");return`<div class="service-widget ${m}" data-service-id="${o.id}"><div class="service-widget-header"><i class="bx ${v}"></i><h3>${o.short_name||o.id}</h3><span class="service-widget-status">${A}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${j}</span></div>${B}</div></div>`}let S=new Map(Array.from(t.querySelectorAll(".service-widget")).map(o=>[o.dataset.serviceId,o])),h=new Set(u.map(o=>o.id));for(let[o,d]of S)h.has(o)||d.remove();let f=new Set;u.forEach(o=>{if(f.has(o.id))return;f.add(o.id);let d=g(o),m=S.get(o.id);if(m&&m.parentNode)if(o.id==="upstash-redis-ro"){let v=z()&&Ae||Date.now(),A=new Date(v).toLocaleTimeString(),L=m.querySelector(".sync-time-label");L&&(L.textContent=`Last synced: ${A}`)}else m.outerHTML!==d&&(m.outerHTML=d);else t.insertAdjacentHTML("beforeend",d)})}async function Ke(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Ut();if(!e){t.children.length===0&&(t.innerHTML=H("offline","Failed to load model status.","The event service may be offline."));return}Bt=Date.now(),X(2,Bt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function n(l){let u=l.status==="Ready";return`
                ...
                </div>`}function c(l){let u=l.status==="Ready";return`
                ...
                </div>`}function r(l){let u=l.status==="Downloaded",y=u?"service-widget-online":"service-widget-offline",p=u?"OK":"MISSING",b=u&&l.size>0?`${(l.size/1e9).toFixed(2)} GB`:"-",a=l.name;return l.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${l.name}"><div class="service-widget-header"><i class="bx ${u?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${l.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let x="";if(s&&(x+=n(s)),e.xtts&&(x+=c(e.xtts)),x+=i.map(r).join(""),!x){t.innerHTML=H("empty","No models found.");return}t.innerHTML!==x&&(t.innerHTML=x)}async function he(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let i=document.getElementById("guardian-sentry-val"),s=document.getElementById("guardian-architect-val"),n=document.getElementById("guardian-idle-val"),c=document.getElementById("guardian-total-idle"),r=document.getElementById("guardian-total-active"),x=document.getElementById("guardian-total-waste"),l=document.getElementById("guardian-reset-btn"),u=document.getElementById("analyzer-reset-btn");l&&!l.dataset.listenerAttached&&(l.onclick=async()=>{l.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{l.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{l.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),he()}catch{l.innerHTML="<i class='bx bx-error'></i>"}},l.dataset.listenerAttached="true"),u&&!u.dataset.listenerAttached&&(u.onclick=async()=>{u.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await _("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{u.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{u.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),he()}catch{u.innerHTML="<i class='bx bx-error'></i>"}},u.dataset.listenerAttached="true");let y=await Ls();if(y&&y.agents){let f=y.agents.guardian||{protocols:{}},o=y.agents.analyzer||{protocols:{}},d=y.agents.imaginator||{protocols:{}},m=y.system||{},v=Math.floor(Date.now()/1e3),A={sentry:"Sentry",architect:"Architect",synthesis:"Synthesis",alert_review:"Alert Review"},L=B=>{B<0&&(B=0);let j=Math.floor(B/3600),C=Math.floor(B%3600/60),w=B%60;return j>0?`${j}h ${C}m`:C>0?`${C}m ${w}s`:`${w}s`},k=(B,j,C,w)=>{if(!B)return;let $=A[w]||w.toUpperCase(),M=B.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(M&&(M.textContent=$),!C){B.textContent="Inactive",B.style.color="#666";return}let N=C.status;if(N==="Working")B.textContent="Working",B.style.color="#bb86fc";else if(N==="Ready")B.textContent="Ready",B.style.color="#5eff5e";else{let D=C.cooldown;if(z()&&(D=C.next_run-v),D<=0)B.textContent="Ready",B.style.color="#5eff5e";else{let q=Math.floor(D/60),J=D%60;B.textContent=`${q}m ${J}s`,B.style.color="#fff"}}j&&C.stats&&(j.innerHTML=`
          ...
          `)};i&&k(i,document.getElementById("guardian-sentry-stats"),f.protocols.sentry,"sentry"),s&&k(s,document.getElementById("guardian-architect-stats"),f.protocols.architect,"architect");let T=document.getElementById("analyzer-synthesis-val"),E=document.getElementById("analyzer-synthesis-stats");T&&k(T,E,o.protocols.synthesis,"synthesis");let I=document.getElementById("imaginator-alert_review-val"),U=document.getElementById("imaginator-alert_review-stats");I&&k(I,U,d.protocols.alert_review,"alert_review");let O=document.getElementById("system-state-label"),F=document.getElementById("system-state-val");if(F&&m.state){let B=m.state,j=L(m.state_time||0);O&&(O.textContent=`State: ${B.toUpperCase()}`),F.textContent=j,B==="idle"?F.style.color=m.state_time>300?"#5eff5e":"#fff":F.style.color="#bb86fc"}c&&(c.textContent=L(m.metrics?.total_idle_time||0)),r&&(r.textContent=L(m.metrics?.total_active_time||0)),x&&(x.textContent=L(m.metrics?.total_waste_time||0))}else[i,s,n,c,r,x].forEach(o=>{o&&(o.textContent="-",o.style.color="#666")});if(t)return;let p=await Ts(),b=[],a=[],g=[];p&&(Array.isArray(p)?b=p:(b=p.active||[],a=p.queue||[],g=p.history||[],g.sort((f,o)=>(o.end_time||0)-(f.end_time||0)))),b.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=H("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),Ge(e,b,!1));let S=document.getElementById("processes-queue-widgets");S&&(a.length===0?!S.querySelector(".tab-placeholder")&&!S.querySelector('div[style*="font-style: italic"]')&&(S.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(S.innerHTML="",Ge(S,a,!1)));let h=document.getElementById("processes-history-widgets");h&&(g.length===0?h.querySelector(".tab-placeholder")||(h.innerHTML=H("empty","No recent history.")):(h.querySelector(".tab-placeholder")&&(h.innerHTML=""),Ge(h,g,!0))),Ce(1,b.length)}function Ge(t,e,i){function s(l){let u="";l.end_time?u=`${l.end_time-l.start_time}s`:u=`${Math.floor(Date.now()/1e3-l.start_time)}s`;let y=l.retries>0?`<span class="process-retry-badge">Retry ${l.retries}</span>`:"",p=l.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[p]?p=b[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
        <div class="process-history-item" data-channel-id="${l.channel_id}-${l.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${l.channel_id}-${l.start_time}" style="">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${u}</span>
                        </div>
                         <div class="service-widget-info">
                ...
                </div>`}let n=i?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(n)).map(l=>[l.dataset.channelId,l])),r=new Set(e.map(l=>`${l.channel_id}-${l.start_time}`));for(let[l,u]of c)r.has(l)||u.remove();let x=new Set;e.forEach(l=>{let u=`${l.channel_id}-${l.start_time}`;if(x.has(u))return;x.add(u);let y=s(l),p=c.get(u);if(p&&p.parentNode){p.outerHTML!==y&&(p.outerHTML=y);let b=t.querySelector(`[data-channel-id="${u}"]`);b&&t.appendChild(b)}else t.insertAdjacentHTML("beforeend",y)})}function He(){let t=Oe(),e="Notification"in window,i={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function Ft(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await jt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await jt(t)}async function jt(t){try{let e=await _("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=H("empty","No web history found.");return}let s=`
        ...
        `;i.forEach((x,l)=>{let u=new Date(x.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=x.screenshot?`<img src="data:image/png;base64,${x.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ms=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ye=!0,i}function Jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ye=!1}function Wt(t,e,i="output"){if(!Ye)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Is(t){let e=Gt.find(n=>n.id===t);if(!e)return;let i=As(e);Wt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(r=>setTimeout(r,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),Wt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Vt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ms();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Is(s)})})}async function Bs(){if(!z())try{if(!(await _("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Kt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}st(),it(),Bs(),lt(),tt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Vt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=f=>{localStorage.setItem("dex_last_window",f)};function n(){for(;e.length>1;)e.shift().close(!0);let f=document.getElementById("windows-container"),o=document.querySelector("nav"),d=document.querySelector("footer"),m=window.location.pathname==="/"||window.location.pathname==="/index.html",v=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");f&&f.setAttribute("data-count",e.length),e.forEach(I=>{let U=document.getElementById(I.id);U&&U.classList.remove("hide-close")});let A=document.getElementById("dexter-menu-container"),L=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),E=window.innerWidth<880;if(!E){let I=document.getElementById("dexter-dropdown"),U=document.getElementById("dexter-menu-btn");I&&I.classList.remove("active"),U&&U.classList.remove("active"),f&&f.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=E?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",o?.classList.add("window-open"),f&&(f.style.paddingTop="60px"),A&&(A.style.display=E?"flex":"none"),k&&(k.style.display=E?"block":"none"),!E&&L){let I=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(I)?(L.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(x)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(l)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(u)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):L.innerHTML=""}else L&&(L.innerHTML="");else o?.classList.remove("window-open"),T&&(T.style.display="none"),f&&(f.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",m||v?d?.classList.remove("hide"):d?.classList.add("hide"),A&&(A.style.display="flex"),k&&(k.style.display="block"),L&&(L.innerHTML="");ve()}function c(f){if(f.isOpen()){f.close();return}for(;e.length>0;)e.pop().close(!0);e.push(f),f.open(),n()}function r(){[...e].forEach(o=>o.close()),e=[]}window.addEventListener("resize",n);let x=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:gt(),onOpen:()=>ie(),onClose:()=>{let f=e.indexOf(x);f>-1&&e.splice(f,1),n()}}),l=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:We(),onOpen:()=>{l.setContent(We()),ne()},onClose:()=>{let f=e.indexOf(l);f>-1&&e.splice(f,1),n()}}),u=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Tt(),onOpen:()=>Je(),onClose:()=>{let f=e.indexOf(u);f>-1&&e.splice(f,1),n()}}),y=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Nt()},{icon:"bxs-component",title:"Processes",content:zt()},{icon:"bxs-brain",title:"Models",content:Ht()},{icon:"bx-globe",title:"Web",content:qt()},{icon:"bxs-hdd",title:"Hardware",content:Pt()},{icon:"bxs-terminal",title:"Logs",content:Rt()},{icon:"bxs-zap",title:"Agents",content:Dt()},{icon:"bx-task",title:"Chores",content:St()}].filter(f=>z()?f.title!=="Hardware"&&f.title!=="Logs":!0),onOpen:()=>{Ve(),he(),Ke(),Ft(),Se(),ye()},onClose:()=>{let f=e.indexOf(y);f>-1&&e.splice(f,1),n()}}),p=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:$t(),onOpen:()=>qe(),onClose:()=>{let f=e.indexOf(p);f>-1&&e.splice(f,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:He(),onOpen:()=>{b.setContent(He()),Ze(b)},onClose:()=>{let f=e.indexOf(b);f>-1&&e.splice(f,1),n()}});window.dexter={viewEvent:f=>{l.isOpen()||c(l),setTimeout(()=>{let o=document.querySelector(`.event-item[data-event-id="${f}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)},viewAlert:f=>{x.isOpen()||c(x),setTimeout(()=>{let o=document.querySelector(`.event-item[data-alert-id="${f}"]`);o&&(o.scrollIntoView({behavior:"smooth",block:"center"}),o.classList.add("flash-highlight"),o.classList.contains("expanded")||o.click(),setTimeout(()=>o.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let f=document.getElementById("dexter-dropdown"),o=document.getElementById("dexter-menu-btn");f&&o&&(f.classList.remove("active"),o.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let f=document.getElementById("dexter-menu-btn"),o=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),m=document.getElementById("nav-left-container"),v=document.getElementById("dexter-dropdown"),A=document.getElementById("windows-container");f&&v&&(f.onclick=k=>{k.stopPropagation();let T=window.innerWidth<880;if(T)v.style.top="",v.style.left="",v.style.right="",v.style.transform="";else{let I=f.getBoundingClientRect(),O=document.querySelector("nav").getBoundingClientRect(),F=220,B=10,C=I.left+I.width/2-F/2,w=O.right-10;C+F>w&&(C=w-F),C<10&&(C=10),v.style.top=O.bottom+B+"px",v.style.left=C+"px",v.style.right="auto",v.style.transform="none"}v.classList.toggle("active"),f.classList.toggle("active");let E=v.classList.contains("active");if(T){let I=document.querySelector("nav");E?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),I?.classList.add("window-open"),A?.classList.add("menu-open"),fe(!0)):(A?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),I?.classList.remove("window-open"),fe(!1)))}}),o&&(o.onclick=k=>{k.stopPropagation(),c(b)}),d&&(d.onclick=k=>{k.stopPropagation(),r()});let L=(k,T,E)=>{let I=document.getElementById(k);I&&(I.onclick=U=>{U.stopPropagation(),a(),E&&s(E),c(T)})};L("alerts-menu-item",x,"alerts-window"),L("events-menu-item",l,"events-window"),L("monitor-menu-item",y,"monitor-window"),L("contacts-menu-item",u,"contacts-window"),L("workspace-menu-item",p,"workspace-window"),m&&(m.onclick=k=>{if(k.stopPropagation(),v&&v.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)r();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let U=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");U.pop();let O=U.join("/")||"/";window.location.href=O}}}),document.addEventListener("click",()=>{let k=window.innerWidth<880;v&&v.classList.contains("active")&&(a(),k&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),v&&(v.onclick=k=>k.stopPropagation())})();let S=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!S&&!h&&document.querySelector("footer")?.classList.add("hide"),z()||setInterval(()=>{l.isOpen()&&ne(),x.isOpen()?ie():je()},1e3),setInterval(()=>{z()&&(x.isOpen()?ie():je()),p.isOpen()?qe():Ct(),z()&&l.isOpen()&&ne(),y.isOpen()&&Ot()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Kt):Kt();})();

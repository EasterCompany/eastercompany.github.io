...
`,Fe=null,fe=new Set,Je=[];async function pe(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Ts(),t&&(e.innerHTML="<p>Updating...</p>");let o="/events?ml=1000&format=json&event.type=system.notification.generated";try{let i=await D(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];Fe=Date.now(),te(0,Fe);let r=Date.now(),g=24*60*60*1e3,c=l.filter(d=>{let x=localStorage.getItem(`alert_read_ts_${d.id}`);if(!x)return!0;let y=parseInt(x);return r-y<g});c.sort((d,x)=>{let y=w=>{let p=w.event;if(typeof p=="string")try{p=JSON.parse(p)}catch{return"low"}return(p.priority||"low").toLowerCase()},A=w=>w==="critical"?4:w==="high"?3:w==="medium"?2:1,n=A(y(d)),b=A(y(x));return n!==b?b-n:x.timestamp-d.timestamp}),Je=c;let h=d=>{let x=d.event;if(typeof x=="string")try{x=JSON.parse(x)}catch{return"low"}return(x.priority||"low").toLowerCase()},E=[];if(c.length===0)E.push({id:"placeholder-empty",type:"placeholder",message:"No alerts, all systems nominal"});else{let x=new Set(c.map(A=>h(A))).size>1,y=null;c.forEach(A=>{let n=h(A);x&&n!==y&&(E.push({id:`divider-${n}`,type:"divider",label:n.toUpperCase()}),y=n),E.push(A)})}t&&(e.innerHTML="");let m=d=>{let x=d.event;if(typeof x=="string")try{x=JSON.parse(x)}catch{return null}let y=(x.title||"Untitled Alert").trim(),A=(x.body||"No description provided.").trim(),n=x.summary||"",b=x.content||"",w=x.protocol||"",p=(x.priority||"low").toLowerCase(),$=!!x.alert,B=(x.category||"system").trim(),S=x.related_event_ids||[],H=x.audit_event_id,q=!!localStorage.getItem(`alert_read_ts_${d.id}`),V=new Date(d.timestamp*1e3),_=V.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=V.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),L=q?"event-border-grey":"event-border-blue";!q&&$&&(L="event-border-red"),q&&(p==="high"||p==="critical")?L="event-border-red":q&&p==="medium"&&(L="event-border-orange");let I=q?"alert-read":"alert-unread",O=fe.has(d.id),R=O?"expanded":"",G=O?"display: block;":"display: none;",k="",z="";S.length>0&&(z=`
        ...
        `,k=`
                ...
                ${Y}
            `;let K=document.createElement("div");K.className=`event-item notification-item ${L} ${I} ${R} cursor-pointer priority-${p}`,K.dataset.alertId=d.id,K.onclick=function(de){let Q=this;if(Q.classList.contains("expanded")){Q.classList.remove("expanded"),fe.delete(d.id);let Be=Q.querySelector(".event-details");Be&&(Be.style.display="none")}else{Q.classList.add("expanded"),fe.add(d.id);let Be=Q.querySelector(".event-details");if(Be&&(Be.style.display="block"),!localStorage.getItem(`alert_read_ts_${d.id}`)){localStorage.setItem(`alert_read_ts_${d.id}`,Date.now().toString()),Q.classList.add("alert-read"),Q.classList.remove("alert-unread"),Q.classList.remove("event-border-blue","event-border-red","event-border-purple");let Ye="event-border-grey";p==="high"||p==="critical"?Ye="event-border-red":p==="medium"&&(Ye="event-border-orange"),Q.classList.add(Ye),et()}}};let Ee=`${w?w.toUpperCase():"GUARDIAN"} ALERT: ${n||y}`,Te={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[B]||"bx-bell",Ze=p==="high"||p==="critical"?"#ff4d4d":p==="medium"?"#ffa500":"#888";K.innerHTML=`
...
`,be=new Set,kt=[],Me=null;async function ve(t=!1){let e=document.getElementById("roadmap-list");if(e){$s();try{let o=await D("/roadmap");if(!o.ok)throw new Error("Offline");let i=await o.json();kt=i;let a=m=>{let v=be.has(m.id),s=m.state==="published",u=m.state==="consumed",C="event-border-grey";s&&(C="event-border-blue"),u&&(C="event-border-purple");let f=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),d=document.createElement("div");d.className=`event-item notification-item ${C} cursor-pointer ${v?"expanded":""}`,d.dataset.itemId=m.id,d.onclick=A=>{if(A.target.closest("button"))return;if(d.classList.contains("expanded")){d.classList.remove("expanded");let b=d.querySelector(".event-details");b&&(b.style.display="none"),be.delete(m.id)}else{d.classList.add("expanded");let b=d.querySelector(".event-details");b&&(b.style.display="block"),be.add(m.id)}};let x=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;d.innerHTML=`
        <div class="event-time">
      ...
      `;let y=d.querySelector(".event-details");return y&&y.addEventListener("click",A=>{A.stopPropagation()}),d.querySelector(".edit-btn")?.addEventListener("click",()=>Ss(m)),d.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ks(m)),d.querySelector(".delete-btn")?.addEventListener("click",()=>Ls(m.id)),d.querySelector(".close-details-btn")?.addEventListener("click",A=>{A.stopPropagation(),d.classList.remove("expanded");let n=d.querySelector(".event-details");n&&(n.style.display="none"),be.delete(m.id)}),d},l=m=>{let v=document.createElement("div");v.innerHTML=J("empty",m.message,m.action,m.id);let s=v.firstElementChild;return s.dataset.itemId=m.id,s},r=[];!i||i.length===0?r.push({id:"placeholder-empty",type:"placeholder",message:"Your roadmap is empty.",action:N()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`}):i.forEach(m=>r.push({...m,type:"item"}));let g=Array.from(e.children),c=new Map(g.map(m=>[m.dataset.itemId,m])),h=new Set(r.map(m=>m.id));g.forEach(m=>{let v=m.dataset.itemId;(!v||!h.has(v))&&m.remove()}),t&&(e.innerHTML="");let E=null;r.forEach((m,v)=>{let s=c.get(m.id);(!s||t)&&(s&&s.remove(),m.type==="placeholder"?s=l(m):s=a(m),!s)||(v===0?e.firstElementChild!==s&&e.prepend(s):E&&E.nextElementSibling!==s&&E.after(s),E=s)})}catch{e.children.length===0&&(e.innerHTML=J("offline","Failed to load roadmap.","The event service may be offline."))}}}function $s(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),o=document.getElementById("roadmap-cancel"),i=document.getElementById("roadmap-expand-all"),a=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{Me=null;let l=document.getElementById("roadmap-editor-input");l&&(l.value="");let r=document.getElementById("roadmap-editor-container");r&&(r.style.display="block")},t.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let l=document.getElementById("roadmap-editor-container");l&&(l.style.display="none"),Me=null},o.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let l=document.getElementById("roadmap-editor-input"),r=l?l.value:"";if(!r.trim())return;let g=Me?`/roadmap/${Me}`:"/roadmap",c=Me?"PATCH":"POST";try{await D(g,{method:c,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})});let h=document.getElementById("roadmap-editor-container");h&&(h.style.display="none"),ve(!0)}catch(h){console.error(h)}},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{kt.forEach(l=>be.add(l.id)),ve(!0)},i.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{be.clear(),ve(!0)},a.dataset.listenerAttached="true")}function Ss(t){Me=t.id;let e=document.getElementById("roadmap-editor-input");e&&(e.value=t.content);let o=document.getElementById("roadmap-editor-container");o&&(o.style.display="block",o.scrollIntoView({behavior:"smooth"}))}async function ks(t){let e=t.state==="published"?"draft":"published";try{await D(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ve(!0)}catch(o){console.error(o)}}async function Ls(t){if(confirm("Delete this roadmap item?"))try{await D(`/roadmap/${t}`,{method:"DELETE"}),be.delete(t),ve(!0)}catch(e){console.error(e)}}var Mt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Lt=null,ye=new Set,At=[];async function xe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Ms();let o="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let i=await D(o);if(!i.ok)throw new Error("Service is offline or unreachable.");let l=(await i.json()).events||[];At=l,Lt=Date.now(),te(2,Lt);let r=s=>{let u=s.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let C=(u.title||"Untitled Blueprint").trim(),M=(u.summary||u.body||"No summary provided.").trim(),f=(u.content||"").trim(),d=(u.category||"architecture").trim(),x=(u.related_services||u.affected_services||[]).map(k=>k.trim()),y=(u.implementation_path||[]).map(k=>k.trim()),A=u.source_event_ids||[],n=u.approved===!0,b=new Date(s.timestamp*1e3),w=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=ye.has(s.id),B=$?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${$?"expanded":""} ${n?"blueprint-approved":""}`,S.dataset.blueprintId=s.id,n&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let W=n?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[d]||"bx-paint";S.onclick=function(k){let z=this;if(z.classList.contains("expanded")){z.classList.remove("expanded"),ye.delete(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="none")}else{z.classList.add("expanded"),ye.add(s.id);let U=z.querySelector(".event-details");U&&(U.style.display="block")}};let q="";y.length>0&&(q=`
                ...
                `);let _=x.length>0?`<div style="display: flex; align-items: center; gap: 8px; color: #666; font-size: 0.75em;"><span style="font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Related:</span> <span style="background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px;">${x.join(", ")}</span></div>`:"<div></div>",T=N(),L=n?`
...
`,Nt=()=>`
    ...
    `,He=[],_e={"313071000877137920":"Owen",dexter:"Dexter"},se=[],he=null;async function De(){let t=document.getElementById("chores-list"),e=document.getElementById("create-chore-btn"),o=document.getElementById("create-chore-form"),i=document.getElementById("save-chore-btn"),a=document.getElementById("cancel-chore-btn"),l=document.getElementById("new-chore-owner"),r=document.getElementById("selected-recipients"),g=document.getElementById("task-filter-owner"),c=document.getElementById("new-chore-instruction"),h=document.getElementById("new-chore-url"),E=document.getElementById("new-chore-schedule"),m=document.getElementById("new-chore-time"),v=document.getElementById("new-chore-timezone"),s=document.getElementById("new-chore-time-group");if(v&&!v.dataset.initialValueAttached){let n=Intl.DateTimeFormat().resolvedOptions().timeZone,b=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===n){b=!0;break}if(!b){let w=document.createElement("option");w.value=n,w.textContent=n.replace("_"," "),v.appendChild(w)}v.value=n,v.dataset.initialValueAttached="true"}E&&s&&!E.dataset.timeListenerAttached&&(E.addEventListener("change",()=>{s.style.display=E.value==="daily"?"block":"none"}),E.dataset.timeListenerAttached="true");let u=document.getElementById("form-title"),C=document.getElementById("form-icon");function M(){if(r){if(se.length===0){r.innerHTML='<span style="color: #444; font-size: 0.8em; font-style: italic; padding: 4px;">No recipients selected</span>';return}r.innerHTML=se.sort((n,b)=>{let w=n.startsWith("channel:"),p=b.startsWith("channel:");return w&&!p?-1:!w&&p?1:0}).map(n=>{let b=_e[n]||n,w=n.startsWith("channel:");return`
      ...
      `}).join(""),r.querySelectorAll(".remove-recipient").forEach(n=>{n.onclick=()=>{let b=n.dataset.id;se=se.filter(w=>w!==b),M()}})}}function f(n){n&&!se.includes(n)&&(se.push(n),M()),l&&(l.value="")}function d(n=null){if(o){if(he=n?n.id:null,n){if(u&&(u.textContent="Update Research Task"),C&&(C.className="bx bx-edit-alt",C.style.color="#03dac6"),i&&(i.innerHTML="<i class='bx bx-check'></i> Update Task"),c&&(c.value=n.natural_instruction),h&&(h.value=n.execution_plan?.entry_url||""),E&&(E.value=n.schedule,s&&(s.style.display=n.schedule==="daily"?"block":"none")),m&&n.run_at&&(m.value=n.run_at),v&&n.timezone){let b=!1;for(let w=0;w<v.options.length;w++)if(v.options[w].value===n.timezone){b=!0;break}if(!b){let w=document.createElement("option");w.value=n.timezone,w.textContent=n.timezone.replace("_"," "),v.appendChild(w)}v.value=n.timezone}se=n.recipients||(n.owner_id?[n.owner_id]:[]),M()}else u&&(u.textContent="Initialize Research Task"),C&&(C.className="bx bx-plus-circle",C.style.color="#bb86fc"),i&&(i.innerHTML="<i class='bx bx-zap'></i> Deploy Task"),c&&(c.value=""),h&&(h.value=""),E&&(E.value="every_24h"),se=[],M();o.style.display="block",o.scrollIntoView({behavior:"smooth",block:"start"})}}let x=document.getElementById("contacts-group"),y=document.getElementById("channels-group");if(x&&!x.dataset.populated&&!N())try{ge("/contacts").then(async n=>{n.ok&&(((await n.json()).members||[]).forEach(p=>{if(_e[p.id]=p.nickname||p.username,p.id==="313071000877137920")return;let $=document.createElement("option");$.value=p.id;let B=p.nickname||p.username;$.textContent=`${B} (${p.username})`,x.appendChild($)}),x.dataset.populated="true")})}catch{console.warn("Failed to fetch contacts for dropdown")}if(y&&!y.dataset.populated&&!N())try{ge("/channels").then(async n=>{n.ok&&((await n.json()).forEach(w=>{let p=`channel:${w.id}`;_e[p]=w.name;let $=document.createElement("option");$.value=p,$.textContent=`#${w.name} (${w.guild})`,y.appendChild($)}),y.dataset.populated="true")})}catch{console.warn("Failed to fetch channels for dropdown")}l&&!l.dataset.listenerAttached&&(l.onchange=()=>{f(l.value)},l.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onchange=()=>A(),g.dataset.listenerAttached="true");function A(){if(!t)return;let n=g?.value||"all",b=n==="all"?He:He.filter(p=>(p.recipients||(p.owner_id?[p.owner_id]:[])).includes(n));if(b.length===0){t.innerHTML=J("empty",n==="all"?"No active tasks.":"No tasks found for this owner.",N()?"Dexter is not currently performing research.":"Click the plus icon to create a research task.");return}let w=b.map(p=>{let $=p.last_run===0?"Never":new Date(p.last_run*1e3).toLocaleString(),B=p.memory?p.memory.length:0,S=p.status==="active"?"#03dac6":"#666",W=(p.recipients||(p.owner_id?[p.owner_id]:[])).sort((q,V)=>{let _=q.startsWith("channel:"),T=V.startsWith("channel:");return _&&!T?-1:!_&&T?1:0}).map(q=>{let V=_e[q]||q.substring(0,8),_=q.startsWith("channel:");return`<span title="${V}" style="display: flex; align-items: center; gap: 3px;"><i class='bx ${q==="dexter"?"bx-terminal":_?"bx-hash":"bx-user"}'></i>${V}</span>`}).join("<span style='color: #444;'>, </span>");return`
            ...
            `}).join("");t.innerHTML=w,t.querySelectorAll(".edit-chore-btn").forEach(p=>{p.onclick=$=>{$.stopPropagation();let B=p.dataset.id,S=He.find(H=>H.id===B);S&&d(S)}}),t.querySelectorAll(".reset-chore-btn").forEach(p=>{p.onclick=async $=>{$.stopPropagation();let B=p.dataset.id;confirm("Reset this task? It will be re-run immediately on the next cycle.")&&(p.innerHTML="<i class='bx bx-loader-alt spin'></i>",await D(`/chores/${B}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({last_run:0})}),De())}}),t.querySelectorAll(".delete-chore-btn").forEach(p=>{p.onclick=async $=>{$.stopPropagation();let B=p.dataset.id;confirm("Delete this task?")&&(await D(`/chores/${B}`,{method:"DELETE"}),De())}})}e&&!e.dataset.listenerAttached&&(e.onclick=()=>{o&&(o.style.display==="none"||he!==null?d(null):o.style.display="none")},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.onclick=()=>{o&&(o.style.display="none")},a.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=async()=>{let n=document.getElementById("new-chore-instruction"),b=document.getElementById("new-chore-url"),w=document.getElementById("new-chore-schedule"),p=n?.value,$=w?.value||"every_24h",B=$==="daily"?m?.value:"",S=$==="daily"?v?.value:"";if(!p)return;if(se.length===0){alert("Please add at least one recipient.");return}let H=i.innerHTML;i.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{let W=he?"PATCH":"POST",q=he?`/chores/${he}`:"/chores";await D(q,{method:W,headers:{"Content-Type":"application/json"},body:JSON.stringify({recipients:se,natural_instruction:p,entry_url:b?.value,schedule:$,run_at:B,timezone:S})}),o&&(o.style.display="none"),n&&(n.value=""),b&&(b.value=""),he=null,se=[],De()}catch(W){console.error(W),alert(he?"Failed to update research task":"Failed to create research task")}finally{i.innerHTML=H}},i.dataset.listenerAttached="true");try{let n=await D("/chores");if(!n.ok)throw new Error(`HTTP error! status: ${n.status}`);if(He=await n.json()||[],g){let w=g.value,p=new Set;He.forEach($=>{$.owner_id&&p.add($.owner_id),$.recipients&&$.recipients.forEach(B=>p.add(B))}),g.innerHTML='<option value="all">All Recipients</option>',p.forEach($=>{let B=document.createElement("option");B.value=$;let S=$.startsWith("channel:"),H=_e[$]||`Recipient: ${$.substring(0,8)}`;B.textContent=(S&&!H.startsWith("#")?"#":"")+H,g.appendChild(B)}),g.value=w,g.selectedIndex===-1&&(g.value="all")}A()}catch(n){console.error("Failed to fetch chores",n)}}var Pt=()=>`
...
`;function qt(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=_s,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `;let a=e.querySelector(".close-profile-btn");a&&a.addEventListener("click",()=>{e.click()})};ge(`/profile/${t.id}`).then(async i=>{if(i.ok){let a=await i.json();Ds(e,t,a)}else console.log("Profile not found (404) or error."),o("Profile Data Unavailable")}).catch(i=>{console.error("Profile fetch error:",i),o("Connection Failed")})}function Ds(t,e,o){let i=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",a=()=>{let C=(o.attributes||o.facts||[]).map(A=>{let n=A.key||A.k,b=A.value||A.v;return`
        ...
        `}).join(""),M=o.cognitive_model||o,f=M.technical_level||M.techLevel||0,d=M.communication_style||M.commStyle||"Unknown",x=M.patience_level||M.patience||"Unknown",y=M.vibe||"Unknown";return`
        ...
        `},l=()=>{let C=o.dossier||{},M=C.identity||{},f=C.career||{},d=C.personal||{},x=C.social||[];return`
    ...
    `;t.innerHTML=h;let E=t.querySelector(".profile-card"),m=t.querySelector("#profile-expand-toggle"),v=t.querySelectorAll(".profile-tab-btn"),s=t.querySelectorAll(".tab-content"),u=t.querySelector(".close-profile-btn");u&&u.addEventListener("click",()=>{t.classList.remove("active"),ie()||(document.body.style.overflow=""),setTimeout(()=>t.remove(),300)}),v.forEach(C=>{C.addEventListener("click",()=>{v.forEach(d=>d.classList.remove("active")),s.forEach(d=>d.classList.remove("active")),C.classList.add("active");let M=C.dataset.tab,f=t.querySelector(`#tab-${M}`);f&&f.classList.add("active")})}),m&&m.addEventListener("click",()=>{E.classList.toggle("expanded");let C=E.classList.contains("expanded");m.innerHTML=C?"<i class='bx bx-collapse-alt'></i> Collapse View":"<i class='bx bx-expand-alt'></i> Detailed Analysis"})}var Ut=()=>`
    <div class="system-section-header" style="margin-bottom: 20px;">
...
`,zs=null;async function nt(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await nt(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=o=>{let a=o.target.closest(".user-profile-card");if(a&&a.dataset.user)try{let l=JSON.parse(a.dataset.user);qt(l)}catch(l){console.error("Failed to parse user data",l)}},t.dataset.listenerAttached="true");try{let o=await ge("/contacts");if(!o.ok)throw new Error("Service unreachable");let a=(await o.json()).members||[];if(zs=Date.now(),a.length===0){t.innerHTML=J("empty","No contacts found.","Check your Discord connection.");return}let l={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};a.sort((r,g)=>{let c=l[r.level]??10,h=l[g.level]??10;return c!==h?c-h:r.username.localeCompare(g.username)}),t.innerHTML=a.map(r=>{let g=r.color&&r.color!==0,c=g?"#"+r.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",h=r.status==="online"?"#5eff5e":r.status==="idle"?"#ffa500":r.status==="dnd"?"#ff4d4d":"#666",E="#888";r.level==="Me"||r.level==="Master"?E="#bb86fc":r.level==="Admin"?E="#cf6679":r.level==="Moderator"?E="#03dac6":r.level==="Contributor"&&(E="#ffa500");let m=r.level==="Me",v=m?"rgba(187, 134, 252, 0.08)":g?`${c}11`:"rgba(255,255,255,0.03)",s=m?"2px solid #bb86fc":g?`1px solid ${c}66`:"1px solid rgba(255,255,255,0.1)";return`
                <div class="user-profile-card" 
                    ...
                    <div class="user-info" style="text-align: left; flex: 1; min-width: 0;">
                        <h3 style="margin: 0; font-size: 0.95em; color: ${g?c:"#fff"}; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 600;">${r.nickname||r.username}</h3>
                        <div style="display: flex; align-items: center; gap: 5px; margin-top: 2px;">
                            <span style="font-size: 0.75em; color: ${E}; font-weight: 600; text-transform: uppercase;">${m?"DEXTER (ME)":r.level}</span>
...
`,Ge=null,we=new Set,ze=[],Z="all",Ps={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed","analysis.router.decision","analysis.user.message_signals"],moderation:["moderation.explicit_content.deleted"]},Rs={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","analysis.router.decision":"bx-git-branch","analysis.user.message_signals":"bx-pulse","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Os(t){for(let[e,o]of Object.entries(Ps))if(o.includes(t))return e;return"system"}function js(t){return Rs[t]||"bx-square-rounded"}async function me(t=!1){let e=document.getElementById("events-timeline");if(!e)return;qs();let i=`/events?ml=${Z==="all"?100:250}&format=json&exclude_types=system.process.registered,system.process.unregistered`;Z!=="all"&&(i+=`&category=${Z}`);try{let a=await D(i);if(!a.ok)throw new Error("Service unreachable");if(ze=((await a.json()).events||[]).filter(v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return!0}let u=s.type;return!(u==="system.blueprint.generated"||u==="system.notification.generated")}),Ge=Date.now(),te(1,Ge),ze.length===0){e.innerHTML=J("empty","No events found for this filter.");return}t&&(e.innerHTML="");let g=v=>{let s=v.event;if(typeof s=="string")try{s=JSON.parse(s)}catch{return null}let u=s.type,C=Os(u),M=js(u),f=u==="engagement.decision"||u==="messaging.bot.sent_message"||u==="messaging.bot.voice_response"||u==="messaging.user.sent_message"||u==="moderation.explicit_content.deleted"||u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="analysis.user.message_signals"||u==="system.cli.command"||u==="system.analysis.audit"||u==="system.test.completed"||u==="error_occurred"||u==="system.cli.status"||u==="system.attention.expired"||u.startsWith("system.roadmap")||u.startsWith("system.process"),d="event-border-grey";f&&(u==="moderation.explicit_content.deleted"||u==="error_occurred"?d="event-border-red":u==="analysis.link.completed"||u==="analysis.visual.completed"||u==="analysis.router.decision"||u==="system.analysis.audit"||u==="analysis.user.message_signals"||u==="engagement.decision"?d="event-border-purple":u==="system.attention.expired"||u==="system.cli.command"||u==="system.cli.status"?d="event-border-orange":u==="system.test.completed"?d=s.test?.status==="OK"&&s.lint?.status==="OK"&&s.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let x=f?"cursor-pointer":"",y=we.has(v.id),A=y?"expanded":"",n=y?"display: block;":"display: none;",b=new Date(v.timestamp*1e3),w=b.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),p=b.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=Ft(u,s),B=s.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${s.user_level})</span>`:"",S="";if(f){let _="";if(u==="engagement.decision"){let T=L=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${L}</h5>`;_=`
              ...
              <div style="margin-top: 10px;">
                <div style="font-size: 0.7em; text-transform: uppercase; color: #666; letter-spacing: 1px;">${j(P.name)}</div>
                <div style="font-size: 0.85em; color: #eee; white-space: pre-wrap;">${le(P.value)}</div>
              </div>
            `).join("");L=`
            ...
            `}let I="";s.eval_count&&(I=`
                        ...
                        `);let O="";if(s.chat_history&&s.chat_history.length>0){let R=s.chat_history.length,G=s.chat_history.map((k,z)=>{let P=k.name?k.name.toUpperCase():k.role.toUpperCase();!k.name&&P==="ASSISTANT"&&(P="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${P}</span>
                        ...
                        `}
                    `}else if(u==="messaging.bot.voice_response"){let T=R=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${R}</h5>`,L="",I=[],O=s.raw_input||"";if(O.startsWith("[{")&&O.endsWith("}]")){let R=O.match(/{.*?}/g);R&&(I=R.map(G=>{let k=G.substring(1,G.length-1),z=k.indexOf(" "),P=k.substring(0,z),U=k.substring(z+1);return U=U.replace(/\[\]$/,"").trim(),{role:P,content:U}}))}if(I.length>0&&s.response_raw&&I.push({role:"assistant",content:s.response_raw}),I.length>0){let R=I.length,G=I.map((k,z)=>{let P=k.role.toUpperCase();P==="ASSISTANT"&&(P="DEXTER");let U=k.role==="user"?"#03dac6":k.role==="system"?"#ffb74d":"#bb86fc",Y=z===R-1?"block":"none";return`
                                <div class="history-slide" data-index="${z}" style="display: ${Y};">
                                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                        <span style="font-size: 0.7em; color: ${U}; letter-spacing: 1px; font-weight: bold;">${P}</span>
                                        <span style="font-size: 0.7em; color: #666;">Turn ${z+1} of ${R}</span>
              ...
              <div class="event-detail-block" style="margin-top: 10px; height: 100px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); border-radius: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444;">
                ...
                <span style="font-size: 0.65em; font-family: 'JetBrains Mono', monospace; text-transform: uppercase; letter-spacing: 1px; opacity: 0.5;">Visual Data Restricted</span>
                    ...
                    `}else if(u==="system.analysis.audit"){let T=s.success?"#03dac6":"#ff4d4d",L=s.success?"SUCCESS":"FAILED",I=z=>`<h5 style="margin-bottom: 8px; text-align: left; font-family: 'JetBrains Mono', monospace; font-size: 0.75em; text-transform: uppercase; letter-spacing: 1.5px; color: #888;">${z}</h5>`,O="";s.error&&(O=`
                        ...
                        `);let R="";if(s.chat_history&&s.chat_history.length>0){let z=s.chat_history.length,P=s.chat_history.map((U,Y)=>{let K=U.name?U.name.toUpperCase():U.role.toUpperCase();!U.name&&K==="USER"&&(K="SYSTEM"),!U.name&&K==="ASSISTANT"&&(K="AGENT");let Ee=U.role==="user"?"#03dac6":U.role==="system"?"#ffb74d":"#bb86fc";return`
                        ...
                        `}let G="";if(s.corrections&&s.corrections.length>0){let z=s.corrections.map((P,U)=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(255, 77, 77, 0.1); border-left: 2px solid #ff4d4d; font-size: 0.85em;">
                                <div style="color: #ff4d4d; font-weight: bold; margin-bottom: 4px;">[${P.type}] ${P.guidance}</div>
                                ${P.snippet?`<div style="font-family: monospace; color: #aaa; background: rgba(0,0,0,0.3); padding: 4px;">${j(P.snippet)}</div>`:""}
                            </div>
                        `).join("");G=`
                            <div class="event-detail-block">
                        ...
                        `}let k="";if(s.parsed_results&&s.parsed_results.length>0){let z=s.parsed_results.map(P=>`
                            <div style="margin-bottom: 8px; padding: 8px; background: rgba(3, 218, 198, 0.1); border-left: 2px solid #03dac6; font-size: 0.85em;">
                                <div style="color: #03dac6; font-weight: bold; margin-bottom: 4px;">${j(P.title)}</div>
                                <div style="color: #ddd;">${j(P.summary)}</div>
                            </div>
                        `).join("");k=`
                            <div class="event-detail-block">
                                ${I("Parsed Results")}
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${r.id}" title="Clear Logs" style="${N()?"display: none;":""}">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Wt()}
                ...
                </div>`}if(g&&n.CPU&&n.CPU.length>0){let b=n.CPU[0];g.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                    ...
                    <div style="display: flex; gap: 30px;">
                        ...
                        </div>`}).join('<div style="height: 1px; background: rgba(255,255,255,0.05); margin: 15px 0;"></div>'):h.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No GPU detected</p>'),E&&n.STORAGE&&n.STORAGE.length>0?E.innerHTML=n.STORAGE.map(b=>{let w=(b.USED/1073741824).toFixed(1),p=(b.SIZE/(1024*1024*1024)).toFixed(1),$=b.SIZE>0?(b.USED/b.SIZE*100).toFixed(0):0,B=b.MOUNT_POINT||"Unmounted";return`
                    ...
                    </div>`}).join(""):E&&(E.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let n=await Kt();n?(m(n),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),g&&!g.hasChildNodes())){let n=await Kt();m(n)}if(!t)return;t.dataset.controlsAttached||(t.addEventListener("click",async n=>{let b=n.target.closest(".svc-btn");if(!b)return;let w=b,p=w.dataset.action,$=w.dataset.service;if(!p||!$)return;let B=w.innerHTML;w.innerHTML="<i class='bx bx-loader-alt spin'></i>",w.classList.add("loading"),w.disabled=!0;try{await D(`/system/service/${p}`,{method:"POST",body:JSON.stringify({service:$})}),setTimeout(()=>Ce(),1e3),setTimeout(()=>Ce(),3e3)}catch(S){console.error(S),alert(`Failed to ${p} ${$}`),w.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{w.innerHTML=B,w.classList.remove("loading"),w.disabled=!1},2e3)}}),t.dataset.controlsAttached="true");let v=await ns();if(!v||!v.services){t.children.length===0&&(t.innerHTML=J("offline","Failed to load system metrics.","The event service may be offline."));let n=document.querySelector('[data-service-id="upstash-redis-ro"]');if(n){n.className="service-widget service-widget-offline",n.querySelector(".service-widget-status").textContent="ERROR";let b=n.querySelector(".service-widget-body");b&&(b.innerHTML='<div class="service-widget-footer offline"><span>CONNECTION FAILED</span></div>')}return}Gt=N()&&re.lastDashboard||Date.now(),te(0,Gt||0);let s=v.services||[];window.updateCountdownInterval||(window.updateCountdownInterval=setInterval(()=>{let n=document.getElementById("upstash-countdown");if(n){let p=59-new Date().getSeconds();p<=0&&(p+=60),n.textContent=`${p}s`}},1e3)),Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function u(n){if(!n||n==="N/A"||n==="unknown")return"-";let b=n.match(/^(\d+\.\d+\.\d+)/);return b?b[0]:n.split(".").slice(0,3).join(".")||"-"}function C(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function M(n){if(!n||n==="N/A"||n==="unknown")return"-";let b=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!b)return"-";let w=parseInt(b[1])||0,p=parseInt(b[2])||0,$=parseInt(b[3])||0,B=parseFloat(b[4])||0,S=w*86400+p*3600+$*60+B,H=Math.floor(S/86400);if(H>0)return`${H}d`;let W=Math.floor(S/3600);if(W>0)return`${W}h`;let q=Math.floor(S/60);return q>0?`${q}m`:`${Math.floor(S)}s`}function f(n){if(n.id==="upstash-redis-ro"){let G=N()&&(re.lastFrontend||re.lastDashboard)||Date.now();return`
            ...
            </div>`}let b=n.status==="online",w=b?"service-widget-online":"service-widget-offline",p=b?"bx-check-circle":"bx-x-circle",$=b?"OK":"BAD",B=n.version?u(n.version.str):"-",S=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",H=n.memory&&n.memory!=="N/A"?n.memory:"-",W=S!=="-"?"#00ff00":"#666",q=S!=="-"?"#fff":"#666",V=H!=="-"?"#00ff00":"#666",_=H!=="-"?"#fff":"#666",T=M(n.uptime),L="",I=n.type!=="os"&&n.type!=="cli"&&n.type!=="prd",O="";I&&!N()&&(O=`
            <div class="service-controls" style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end;">
                ...
                </div>${O}`:L=`<div class="service-widget-footer offline" style="justify-content: center; opacity: 0.5; letter-spacing: 2px; font-weight: bold;"><span>OFFLINE</span></div>${O}`;let R=N()?"easter.company":C(n.domain&&n.port?`${n.domain}:${n.port}`:"");return`<div class="service-widget ${w}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${p}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${$}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${R}</span></div>${L}</div></div>`}let d=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),x=new Set(s.map(n=>n.id));for(let[n,b]of d)x.has(n)||b.remove();let y=new Set,A=["upstash-redis-rw","upstash-redis-ro","easter-company","easter-company-production","dex-test-service","easter-company-root"];s.forEach(n=>{if(y.has(n.id)||A.includes(n.id))return;y.add(n.id);let b=f(n),w=d.get(n.id);if(w&&w.parentNode)if(n.id==="upstash-redis-ro"){let p=N()&&(re.lastFrontend||re.lastDashboard)||Date.now(),$=new Date(p).toLocaleTimeString(),B=w.querySelector(".sync-time-label");B&&(B.textContent=`Last synced: ${$}`)}else w.outerHTML!==b&&(w.outerHTML=b);else t.insertAdjacentHTML("beforeend",b)})}async function ot(){let t=document.getElementById("models-widgets");if(!t)return;let e=await ns();if(!e){t.children.length===0&&(t.innerHTML=J("offline","Failed to load model status.","The event service may be offline."));return}Vt=Date.now(),te(2,Vt);let o=e.models||[],i=e.whisper;Array.from(t.children).forEach(c=>{c.classList.contains("service-widget")||c.remove()});function a(c){let h=c.status==="Ready";return`
                ...
                </div>`}function r(c){let h=c.status==="Downloaded",E=h?"service-widget-online":"service-widget-offline",m=h?"OK":"MISSING",v=h&&c.size>0?`${(c.size/1e9).toFixed(2)} GB`:"-",s=c.name;return c.type==="custom"&&s.endsWith(":latest")&&(s=s.replace(":latest","")),`<div class="service-widget ${E}" data-model-name="${c.name}"><div class="service-widget-header"><i class="bx ${h?"bx-check-circle":"bx-x-circle"}"></i><h3>${s}</h3><span class="service-widget-status">${m}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${c.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${v}</span></div></div></div>`}let g="";if(i&&(g+=a(i)),e.xtts&&(g+=l(e.xtts)),g+=o.map(r).join(""),!g){t.innerHTML=J("empty","No models found.");return}t.innerHTML!==g&&(t.innerHTML=g)}async function ne(t=!1){let e=document.getElementById("processes-widgets");if(!e&&!t)return;let o=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-idle-val"),a=document.getElementById("guardian-total-idle"),l=document.getElementById("guardian-total-active"),r=document.getElementById("guardian-total-waste"),g=document.getElementById("guardian-reset-btn"),c=document.getElementById("analyzer-reset-btn"),h=document.getElementById("fabricator-reset-btn"),E=document.getElementById("courier-reset-btn"),m=document.getElementById("system-pause-toggle-btn");m&&!m.dataset.listenerAttached&&(m.onclick=async()=>{let A=m.querySelector(".bx-play")?"/agent/resume":"/agent/pause";m.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D(A,{method:"POST"}),await ne()}catch{m.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>ne(),2e3)}},m.dataset.listenerAttached="true"),g&&!g.dataset.listenerAttached&&(g.onclick=async()=>{g.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=all",{method:"POST"}),setTimeout(()=>{g.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{g.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{g.innerHTML="<i class='bx bx-error'></i>"}},g.dataset.listenerAttached="true"),c&&!c.dataset.listenerAttached&&(c.onclick=async()=>{c.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{c.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{c.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{c.innerHTML="<i class='bx bx-error'></i>"}},c.dataset.listenerAttached="true"),h&&!h.dataset.listenerAttached&&(h.onclick=async()=>{h.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=construction",{method:"POST"}),setTimeout(()=>{h.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{h.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{h.innerHTML="<i class='bx bx-error'></i>"}},h.dataset.listenerAttached="true"),E&&!E.dataset.listenerAttached&&(E.onclick=async()=>{E.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=researcher",{method:"POST"}),setTimeout(()=>{E.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{E.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{E.innerHTML="<i class='bx bx-error'></i>"}},E.dataset.listenerAttached="true");let v=document.getElementById("imaginator-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await D("/agent/reset?protocol=alert_review",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),ne()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true");let s=await Gs();if(s&&s.agents){let y=s.agents.guardian||{protocols:{}},A=s.agents.analyzer||{protocols:{}},n=s.agents.imaginator||{protocols:{}},b=s.agents.fabricator||{protocols:{}},w=s.agents.courier||{protocols:{}},p=s.system||{},$=Math.floor(Date.now()/1e3),B={sentry:"Sentry",synthesis:"Synthesis",alert_review:"Alert Review",construction:"Construction",researcher:"Researcher"},S=k=>{k<0&&(k=0);let z=Math.floor(k/3600),P=Math.floor(k%3600/60),U=k%60;return z>0?`${z}h ${P}m`:P>0?`${P}m ${U}s`:`${U}s`},H=(k,z,P,U,Y)=>{if(!k)return;let K=B[U]||U.toUpperCase(),Ee=k.parentElement?.querySelector('span[style*="text-transform: uppercase"]');if(Ee&&(Ee.textContent=K),Y==="paused"){k.textContent="PAUSED",k.style.color="#ff9800";return}if(!P){k.textContent="Inactive",k.style.color="#666";return}let Pe=P.status;if(Pe==="Working")k.textContent="Working",k.style.color="#bb86fc";else if(Pe==="Ready")k.textContent="Ready",k.style.color="#5eff5e";else{let Te=P.cooldown;if(N()&&(Te=P.next_run-$),Te<=0)k.textContent="Ready",k.style.color="#5eff5e";else{let Ze=Math.floor(Te/60),Re=Te%60;k.textContent=`${Ze}m ${Re}s`,k.style.color="#fff"}}z&&P.stats&&(z.innerHTML=`
            <div style="display: flex; flex-direction: column; gap: 2px;">
              <span>Runs: ${P.stats.runs||0}</span>
              <span style="color: ${P.stats.failures>0?"#ffa500":"#666"}">Fails: ${P.stats.failures||0}</span>
              <span style="color: ${P.stats.aborted>0?"#ff4d4d":"#666"}">Aborted: ${P.stats.aborted||0}</span>
            </div>
          `)};o&&H(o,document.getElementById("guardian-sentry-stats"),y.protocols.sentry,"sentry",p.state);let W=document.getElementById("analyzer-synthesis-val"),q=document.getElementById("analyzer-synthesis-stats");W&&H(W,q,A.protocols.synthesis,"synthesis",p.state);let V=document.getElementById("imaginator-alert_review-val"),_=document.getElementById("imaginator-alert_review-stats");V&&H(V,_,n.protocols.alert_review,"alert_review",p.state);let T=document.getElementById("fabricator-construction-val"),L=document.getElementById("fabricator-construction-stats");T&&H(T,L,b.protocols.construction,"construction",p.state);let I=document.getElementById("courier-researcher-val"),O=document.getElementById("courier-researcher-stats");I&&H(I,O,w.protocols.researcher,"researcher",p.state);let R=document.getElementById("system-state-label"),G=document.getElementById("system-state-val");if(G&&p.state){let k=p.state,z=S(p.state_time||0);R&&(R.textContent=`State: ${k.toUpperCase()}`),G.textContent=z,k==="idle"?G.style.color=p.state_time>300?"#5eff5e":"#fff":G.style.color="#bb86fc",m&&(k==="paused"?(m.innerHTML="<i class='bx bx-play'></i>",m.title="Resume System",m.style.color="#ff9800"):(m.innerHTML="<i class='bx bx-pause'></i>",m.title="Pause System",m.style.color=""))}a&&(a.textContent=S(p.metrics?.total_idle_time||0)),l&&(l.textContent=S(p.metrics?.total_active_time||0)),r&&(r.textContent=S(p.metrics?.total_waste_time||0))}else[o,i,a,l,r].forEach(A=>{A&&(A.textContent="-",A.style.color="#666")});if(t)return;let u=await Ws(),C=[],M=[],f=[];u&&(Array.isArray(u)?C=u:(C=u.active||[],M=u.queue||[],f=u.history||[],f.sort((y,A)=>(A.end_time||0)-(y.end_time||0)))),e&&(C.length===0?e.querySelector(".tab-placeholder")||(e.innerHTML=J("empty","No active processes.")):((e.querySelector(".tab-placeholder")||e.querySelector("p"))&&(e.innerHTML=""),at(e,C,!1)));let d=document.getElementById("processes-queue-widgets");d&&(M.length===0?!d.querySelector(".tab-placeholder")&&!d.querySelector('div[style*="font-style: italic"]')&&(d.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(d.innerHTML="",at(d,M,!1)));let x=document.getElementById("processes-history-widgets");x&&(f.length===0?x.querySelector(".tab-placeholder")||(x.innerHTML=J("empty","No recent history.")):(x.querySelector(".tab-placeholder")&&(x.innerHTML=""),at(x,f,!0))),qe(1,C.length)}function at(t,e,o){function i(c){let h="";c.end_time?h=`${c.end_time-c.start_time}s`:h=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let E=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,v={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent","system-courier":"Courier Agent","system-imaginator":"Imaginator Agent","system-fabricator":"Fabricator Agent","voice-mode":"Voice Interaction"};if(v[m]?m=v[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`),o)return`
        <div class="process-history-item" data-channel-id="${c.channel_id}-${c.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            ...
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                </div>`}let a=o?".process-history-item":".process-widget",l=new Map(Array.from(t.querySelectorAll(a)).map(c=>[c.dataset.channelId,c])),r=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,h]of l)c&&!r.has(c)&&h.remove();let g=new Set;e.forEach(c=>{let h=`${c.channel_id}-${c.start_time}`;if(g.has(h))return;g.add(h);let E=i(c),m=l.get(h);if(m&&m.parentNode){m.outerHTML!==E&&(m.outerHTML=E);let v=t.querySelector(`[data-channel-id="${h}"]`);v&&t.appendChild(v)}else t.insertAdjacentHTML("beforeend",E)})}function Ke(){let t=Xe(),e="Notification"in window,o={enabled:e&&Notification.permission==="granted",supported:e};return`
            ...
            </div>`}function rt(t){let e=document.querySelector("#settings-window .window-content");if(!e)return;e.querySelectorAll(".theme-card").forEach(i=>{i.addEventListener("click",function(){let a=this.dataset.theme;a&&(ft(a),t.setContent(Ke()),rt(t))})});let o=document.getElementById("notifications-toggle");o&&"Notification"in window?o.onclick=async i=>{let a=i.target;if(a.checked)try{await Notification.requestPermission()!=="granted"&&(a.checked=!1)}catch{a.checked=!1}else Notification.permission==="granted"&&(alert("To disable notifications, please use your browser settings."),a.checked=!0)}:o&&(o.disabled=!0),Vs()}async function Vs(){let t=document.getElementById("service-config-list"),e=document.getElementById("cognitive-config-list");if(!t||!e)return;let o=N();try{let a=await(await D("/system/options")).json(),l=a.services||a||{},r=a.cognitive||a||{},g="",c=(C,M,f)=>`
        <div class="settings-item">
            <div class="settings-item-info">
            ...
            <label class="toggle-switch">
                <input type="checkbox" class="service-device-toggle" data-service="${M}" ${f==="cuda"?"checked":""} ${o?"disabled":""}>
                <span class="toggle-slider"></span>
            </label>
        </div>`;l.stt&&(g+=c("STT Service","stt",l.stt.device||"cpu")),l.tts&&(g+=c("TTS Service","tts",l.tts.device||"cpu")),g?o&&(g+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Service configuration is read-only in public mode.</div>'):g='<div style="padding: 20px; text-align: center; color: #666;">No configurable services found.</div>',t.innerHTML=g;let h="",E=r.utility_device||"cpu",m=r.utility_speed||"smart",v=o?"disabled":"";if(h+=`
        <div class="settings-item">
            <div class="settings-item-info">
            ...
            <div class="settings-control">
                <select id="cognitive-utility-device-select" class="settings-select" ${v}>
        ...
        <div class="settings-item">
            ...
            <div class="settings-control">
                <select id="cognitive-utility-speed-select" class="settings-select" ${v}>
                    <option value="fast" ${m==="fast"?"selected":""}>Fast (Efficiency)</option>
                    <option value="smart" ${m==="smart"?"selected":""}>Smart (Fidelity)</option>
                </select>
            </div>
        </div>`,o?h+='<div style="font-size: 0.7em; color: #666; font-style: italic; margin-top: 15px; text-align: center;">* Optimization is read-only in public mode.</div>':h+=`<div style="font-size: 0.7em; color: #ffa500; font-style: italic; margin-top: 10px; text-align: center;"><i class='bx bx-info-circle'></i> Changing these settings requires a 'dex restart' to re-apply.</div>`,e.innerHTML=h,o)return;t.querySelectorAll(".service-device-toggle").forEach(C=>{C.addEventListener("change",async M=>{let f=M.target,d=f.dataset.service,x=f.checked?"cuda":"cpu";f.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:d,key:"device",value:x})})}catch{f.checked=!f.checked,alert("Failed to update configuration.")}finally{f.disabled=!1}})});let s=document.getElementById("cognitive-utility-device-select");s&&s.addEventListener("change",async C=>{let M=C.target,f=M.value;M.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:"cognitive",key:"utility_device",value:f})})}catch{alert("Failed to update utility hardware setting.")}finally{M.disabled=!1}});let u=document.getElementById("cognitive-utility-speed-select");u&&u.addEventListener("change",async C=>{let M=C.target,f=M.value;M.disabled=!0;try{await D("/system/options",{method:"POST",body:JSON.stringify({service:"cognitive",key:"utility_speed",value:f})})}catch{alert("Failed to update utility intelligence setting.")}finally{M.disabled=!1}})}catch{t.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load configuration.</div>',e.innerHTML='<div style="padding: 20px; text-align: center; color: #cf6679;">Failed to load optimization settings.</div>'}}var as=()=>`
    ...
    `;async function os(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await is(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await is(t)}async function is(t){try{let e=await D("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let o=await e.json();if(!o||o.length===0){t.innerHTML=J("empty","No web history found.");return}let i=`
        ...
        `;o.forEach((g,c)=>{let h=new Date(g.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),E=g.screenshot?`<img src="data:image/png;base64,${g.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:`<div style="width: 100%; height: 120px; background: rgba(255,255,255,0.02); border: 1px dashed rgba(255,255,255,0.1); display: flex; flex-direction: column; align-items: center; justify-content: center; color: #444; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Ks=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    `);let l=document.getElementById("terminal-cmd-name");l&&(l.textContent=`dex ${t.id}`);let r=document.getElementById("terminal-status-badge");r&&(r.className="terminal-status status-running",r.textContent="Running");let g=document.getElementById("terminal-action-btn");return g&&(g.style.display="none"),e.classList.add("active"),document.body.style.overflow="hidden",lt=!0,o}function rs(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),ie()||(document.body.style.overflow=""),lt=!1}function ls(t,e,o="output"){if(!lt||!t)return;let i=document.createElement("div");if(i.className=`terminal-line terminal-${o}`,o==="prompt")i.innerHTML=`<span class="terminal-prompt">$</span> ${e}`;else{let a=Ue(e);i.innerHTML=a||e}t.appendChild(i),t.scrollTop=t.scrollHeight}async function Ys(t){let e=cs.find(r=>r.id===t);if(!e)return;let o=Zs(e);o&&ls(o,`dex ${t}`,"prompt");let i=e.demo_output||["Executing command...","\u2713 Done."];for(let r of i){await new Promise(c=>setTimeout(c,400+Math.random()*600));let g="output";r.includes("[ERROR]")?g="error":r.includes("[SUCCESS]")||r.includes("\u2713")?g="success":r.includes("!")&&(g="warning"),o&&ls(o,r,g)}let a=document.getElementById("terminal-status-badge");a&&(a.className="terminal-status status-success",a.textContent="Completed (Demo)");let l=document.getElementById("terminal-action-btn");l&&(l.style.display="block")}function ds(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Ks();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let o=e.querySelector("code"),i=o?o.textContent:"";i&&navigator.clipboard.writeText(i).then(()=>{let a=e.querySelector("i");a&&(a.className="bx bx-check",a.style.color="#5eff5e",setTimeout(()=>{a.className="bx bx-copy",a.style.color="#bb86fc"},2e3))})}),t.querySelectorAll(".cli-command-card").forEach(o=>{let i=o;o.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),o.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),o.addEventListener("click",()=>{let a=i.dataset.cmd;a&&Ys(a)})})}async function Xs(){if(!N())try{if(!(await D("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function ps(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}pt(),mt(),Xs(),vt(),dt();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&ds();let e=[],o=document.getElementById("windows-container");o&&o.setAttribute("data-count","0");let i=f=>{localStorage.setItem("dex_last_window",f)};function a(){for(;e.length>1;)e.shift().close(!0);let f=document.getElementById("windows-container"),d=document.querySelector("nav"),x=document.querySelector("footer"),y=window.location.pathname==="/"||window.location.pathname==="/index.html",A=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");f&&f.setAttribute("data-count",e.length.toString()),e.forEach(S=>{let H=document.getElementById(S.id);H&&H.classList.remove("hide-close")});let n=document.getElementById("dexter-menu-container"),b=document.getElementById("nav-window-switcher"),w=document.getElementById("settings-icon"),p=document.getElementById("docs-icon"),$=document.getElementById("close-all-windows"),B=window.innerWidth<880;if(!B){let S=document.getElementById("dexter-dropdown"),H=document.getElementById("dexter-menu-btn");S&&S.classList.remove("active"),H&&H.classList.remove("active"),f&&f.classList.remove("menu-open")}if($e(e.length>0),e.length>0)if(x?.classList.add("hide"),$&&($.style.display=B?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",d?.classList.add("window-open"),f&&(f.style.paddingTop="60px"),n&&(n.style.display=B?"flex":"none"),p&&(p.style.display=B?"block":"none"),w&&(w.style.display=B?"block":"none"),!B&&b){let S=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(S)?(b.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts")?.addEventListener("click",()=>{i("alerts-window"),l(g)}),document.getElementById("switch-events")?.addEventListener("click",()=>{i("events-window"),l(c)}),document.getElementById("switch-monitor")?.addEventListener("click",()=>{i("monitor-window"),l(E)}),document.getElementById("switch-contacts")?.addEventListener("click",()=>{i("contacts-window"),l(h)}),document.getElementById("switch-workspace")?.addEventListener("click",()=>{i("workspace-window"),l(m)})):b.innerHTML=""}else b&&(b.innerHTML="");else d?.classList.remove("window-open"),$&&($.style.display="none"),f&&(f.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),ie()||(document.body.style.overflow=""),y||A?x?.classList.remove("hide"):x?.classList.add("hide"),n&&(n.style.display="flex"),p&&(p.style.display="block"),w&&(w.style.display="block"),b&&(b.innerHTML="");Le()}function l(f){if(f.isOpen()){f.close();return}for(;e.length>0;)e.pop().close(!0);e.push(f),f.open(),a()}function r(){[...e].forEach(d=>d.close()),e=[]}window.addEventListener("resize",a);let g=ue({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Tt(),onOpen:()=>pe(),onClose:()=>{let f=e.indexOf(g);f>-1&&e.splice(f,1),a()}}),c=ue({id:"events-window",title:"Events",icon:"bx-calendar-event",content:it(),onOpen:()=>{c.setContent(it()),me()},onClose:()=>{let f=e.indexOf(c);f>-1&&e.splice(f,1),a()}}),h=ue({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:Ut(),onOpen:()=>nt(),onClose:()=>{let f=e.indexOf(h);f>-1&&e.splice(f,1),a()}}),E=ue({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:Xt()},{icon:"bxs-component",title:"Processes",content:Yt()},{icon:"bxs-brain",title:"Models",content:Qt()},{icon:"bx-globe",title:"Web",content:as()},{icon:"bxs-hdd",title:"Hardware",content:es()},{icon:"bxs-terminal",title:"Logs",content:ts()},{icon:"bxs-zap",title:"Agents",content:Zt()}].filter(f=>N()?f.title!=="Hardware"&&f.title!=="Logs":!0),onOpen:()=>{Ce(),ne(),ot(),os(),Ne()},onClose:()=>{let f=e.indexOf(E);f>-1&&e.splice(f,1),a()}}),m=ue({id:"workspace-window",title:"Workspace",icon:"bx-brain",tabs:[{icon:"bx-loader-circle",title:"Progress",content:Pt()},{icon:"bx-map-alt",title:"Roadmap",content:Rt()},{icon:"bx-paint",title:"Blueprints",content:Ot()},{icon:"bx-task",title:"Tasks",content:jt()}],onOpen:()=>st(),onClose:()=>{let f=e.indexOf(m);f>-1&&e.splice(f,1),a()}}),v=ue({id:"settings-window",title:"Settings",icon:"bx-cog",content:Ke(),onOpen:()=>{v.setContent(Ke()),rt(v)},onClose:()=>{let f=e.indexOf(v);f>-1&&e.splice(f,1),a()}});window.dexter={viewEvent:f=>{c.isOpen()||l(c),setTimeout(()=>{let d=document.querySelector(`.event-item[data-event-id="${f}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)},viewAlert:f=>{g.isOpen()||l(g),setTimeout(()=>{let d=document.querySelector(`.event-item[data-alert-id="${f}"]`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.classList.add("flash-highlight"),d.classList.contains("expanded")||d.click(),setTimeout(()=>d.classList.remove("flash-highlight"),2e3))},500)}};let s=()=>{let f=document.getElementById("dexter-dropdown"),d=document.getElementById("dexter-menu-btn");f&&d&&(f.classList.remove("active"),d.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let f=document.getElementById("dexter-menu-btn"),d=document.getElementById("settings-icon"),x=document.getElementById("docs-icon"),y=document.getElementById("close-all-windows"),A=document.getElementById("nav-left-container"),n=document.getElementById("dexter-dropdown"),b=document.getElementById("windows-container");f&&n&&(f.onclick=p=>{p.stopPropagation();let $=window.innerWidth<880;if($)n.style.top="",n.style.left="",n.style.right="",n.style.transform="";else{let S=f.getBoundingClientRect(),W=document.querySelector("nav").getBoundingClientRect(),q=220,V=10,T=S.left+S.width/2-q/2,L=W.right-10;T+q>L&&(T=L-q),T<10&&(T=10),n.style.top=W.bottom+V+"px",n.style.left=T+"px",n.style.right="auto",n.style.transform="none"}n.classList.toggle("active"),f.classList.toggle("active");let B=n.classList.contains("active");if($){let S=document.querySelector("nav");B?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),S?.classList.add("window-open"),b?.classList.add("menu-open"),$e(!0)):(b?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),S?.classList.remove("window-open"),ie()||$e(!1)))}}),d&&(d.onclick=p=>{p.stopPropagation(),l(v)}),x&&(x.onclick=p=>{p.stopPropagation(),window.location.href="/docs"}),y&&(y.onclick=p=>{p.stopPropagation(),r()});let w=(p,$,B)=>{let S=document.getElementById(p);S&&(S.onclick=H=>{H.stopPropagation(),s(),B&&i(B),l($)})};w("alerts-menu-item",g,"alerts-window"),w("events-menu-item",c,"events-window"),w("monitor-menu-item",E,"monitor-window"),w("contacts-menu-item",h,"contacts-window"),w("workspace-menu-item",m,"workspace-window"),A&&(A.onclick=p=>{if(p.stopPropagation(),n&&n.classList.contains("active")){s(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1));return}if(e.length>0)r();else{let $=window.location.pathname;if(!($==="/"||$==="/index.html")){let S=$.endsWith("/")&&$.length>1?$.slice(0,-1):$;if(S.includes("/docs/page/")){window.location.href="/";return}let H=S.split("/");H.pop();let W=H.join("/")||"/";window.location.href=W}}}),document.addEventListener("click",()=>{let p=window.innerWidth<880;n&&n.classList.contains("active")&&(s(),p&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),ie()||$e(!1)))}),n&&(n.onclick=p=>p.stopPropagation())})();let C=window.location.pathname==="/"||window.location.pathname==="/index.html",M=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!C&&!M&&document.querySelector("footer")?.classList.add("hide"),N()||setInterval(()=>{c.isOpen()&&me(),g.isOpen()?pe():tt()},1e3),setInterval(()=>{N()&&(g.isOpen()?pe():tt()),m.isOpen()?st():Bt(),N()&&c.isOpen()&&me(),E.isOpen()&&ss()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ps):ps();})();

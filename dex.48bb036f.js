(()=>{function Ae(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Ie(t=!1){let e=window.location.pathname,a=e==="/"||e==="/index.html",l=t?`
        <div class="nav-right">
...
`,re=null,z=new Set,ce=[];async function F(t=!1){let e=document.getElementById("notifications-list");if(!e)return;vt(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];re=Date.now(),N(0,re);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let b=localStorage.getItem(`notification_read_ts_${d.id}`);if(!b)return!0;let y=parseInt(b);return c-y<g});u.sort((d,b)=>{let y=C=>{let D=C.event;if(typeof D=="string")try{D=JSON.parse(D)}catch{return"low"}return(D.priority||"low").toLowerCase()},k=C=>C==="critical"?4:C==="high"?3:C==="medium"?2:1,L=k(y(d)),E=k(y(b));return L!==E?E-L:b.timestamp-d.timestamp}),ce=u;let v=d=>{let b=d.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return"low"}return(b.priority||"low").toLowerCase()},x=[],m=new Set(u.map(d=>v(d))).size>1;if(u.length>0){let d=null;u.forEach(b=>{let y=v(b);m&&y!==d&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),d=y),x.push(b)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=$("empty","No notifications yet."),oe();return}let i=d=>{let b=d.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let y=b.title||"Untitled Notification",k=b.body||"No description provided.",L=b.priority||"low",E=!!b.alert,C=b.category||"system",D=b.related_event_ids||[],O=!!localStorage.getItem(`notification_read_ts_${d.id}`),P=new Date(d.timestamp*1e3),M=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),T=O?"event-border-grey":"event-border-blue";!O&&E&&(T="event-border-red"),O&&(L==="high"||L==="critical")?T="event-border-red":O&&L==="medium"&&(T="event-border-orange");let H=O?"notification-read":"notification-unread",B=z.has(d.id),ue=B?"expanded":"",ge=B?"display: block;":"display: none;",Te="",$e="";D.length>0&&($e=`
...
`,Fe=null,J=new Set,je=[];async function de(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;wt();let a="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];if(je=r,Fe=Date.now(),N(2,Fe),r.length===0){e.innerHTML=$("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),R(1,0);return}t&&(e.innerHTML="");let c=p=>{let m=p.event;if(typeof m=="string")try{m=JSON.parse(m)}catch{return null}let i=m.title||"Untitled Blueprint",n=m.summary||m.body||"No summary provided.",o=m.content||"",f=m.category||"architecture",h=m.affected_services||[],w=m.implementation_path||[],d=new Date(p.timestamp*1e3),b=d.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),y=d.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=J.has(p.id),L=k?"display: block;":"display: none;",E=document.createElement("div");E.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""}`,E.dataset.blueprintId=p.id;let D={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[f]||"bx-paint";E.onclick=function(M){if(this.classList.contains("expanded")){this.classList.remove("expanded"),J.delete(p.id);let T=this.querySelector(".event-details");T&&(T.style.display="none")}else{this.classList.add("expanded"),J.add(p.id);let T=this.querySelector(".event-details");T&&(T.style.display="block")}};let Q=h.length>0?`<div class="blueprint-meta-row"><strong>Affected:</strong> ${h.join(", ")}</div>`:"",O="";w.length>0&&(O=`
                ...
                <div class="event-content">
                    ...
                    <div class="event-details" style="${L}">
                        ...
                        <div class="event-detail-block" style="text-align: left; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px; margin: 10px 0;">
                            <p class="detail-pre" style="white-space: pre-wrap; font-family: 'JetBrains Mono', monospace; font-size: 0.85em; text-align: left; color: #ccc;">${A(o)}</p>
                        </div>
                        ${O}
                    </div>
                </div>
            `;let P=E.querySelector(".close-details-btn");return P&&P.addEventListener("click",M=>{M.stopPropagation(),E.classList.remove("expanded");let S=E.querySelector(".event-details");S&&(S.style.display="none"),J.delete(p.id)}),E},g=Array.from(e.children),u=new Map(g.map(p=>[p.dataset.blueprintId,p])),v=new Set(r.map(p=>p.id));g.forEach(p=>{let m=p.dataset.blueprintId;(!m||!v.has(m))&&p.remove()});let x=null;r.forEach((p,m)=>{let i=u.get(p.id);(!i||t)&&(i&&i.remove(),i=c(p),!i)||(m===0?e.firstElementChild!==i&&e.prepend(i):x&&x.nextElementSibling!==i&&x.after(i),x=i)}),R(2,r.length)}catch(l){console.error("Error fetching blueprints:",l),e.children.length===0&&(e.innerHTML=$("offline","Failed to load blueprints.","The event service may be offline or unreachable."))}}function wt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{je.forEach(a=>J.add(a.id)),de(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{J.clear(),de(!0)},e.dataset.listenerAttached="true")}var Ge=()=>`
...
`,me=null,Y=new Set,ie=[],pe="all",St={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},Lt={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function Je(t){for(let[e,a]of Object.entries(St))if(a.includes(t))return e;return"system"}function Tt(t){return Lt[t]||"bx-square-rounded"}async function X(t=!1){let e=document.getElementById("events-timeline");if(!e)return;$t();let l=`/events?ml=${pe==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await I(l);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],g=c;if(pe!=="all"&&(g=c.filter(i=>{let n=i.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return!1}return Je(n.type)===pe})),ie=g.slice(0,50),me=Date.now(),N(1,me),ie.length===0){e.innerHTML=$("empty","No events found for this filter.");return}t&&(e.innerHTML="");let u=i=>{let n=i.event;if(typeof n=="string")try{n=JSON.parse(n)}catch{return null}let o=n.type,f=Je(o),h=Tt(o),w=o==="engagement.decision"||o==="messaging.bot.sent_message"||o==="messaging.user.sent_message"||o==="moderation.explicit_content.deleted"||o==="analysis.link.completed"||o==="analysis.visual.completed"||o==="system.cli.command"||o==="system.analysis.audit"||o==="system.test.completed"||o==="error_occurred"||o==="system.cli.status"||o.startsWith("system.roadmap")||o.startsWith("system.process"),d="event-border-grey";w&&(o==="moderation.explicit_content.deleted"||o==="error_occurred"?d="event-border-red":o==="analysis.link.completed"||o==="analysis.visual.completed"||o==="system.analysis.audit"?d="event-border-purple":o==="system.cli.command"||o==="system.cli.status"?d="event-border-orange":o==="system.test.completed"?d=n.test?.status==="OK"&&n.lint?.status==="OK"&&n.format?.status==="OK"?"event-border-blue":"event-border-red":d="event-border-blue");let b=w?"cursor-pointer":"",y=Y.has(i.id),k=y?"expanded":"",L=y?"display: block;":"display: none;",E=new Date(i.timestamp*1e3),C=E.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),D=E.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),Q=Ke(o,n),O=n.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${n.user_level})</span>`:"",P="";if(w){let S="";if(o==="engagement.decision")S=`
                    ...
                    `;else if(o==="analysis.visual.completed"){let T="";n.base64_preview?T=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${n.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:n.url&&(T=`<div class="event-detail-block"><img src="${n.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),S=`
                        <div class="event-detail-row">
                    ...
                    `;else if(o==="messaging.user.sent_message"){let T="";n.attachments&&n.attachments.length>0&&(T=`
                            <div class="event-detail-block">
            ...
            `,w){let S=M.querySelector(".close-details-btn");S&&S.addEventListener("click",T=>{T.stopPropagation();let H=T.target.closest(".event-item");if(H){H.classList.remove("expanded"),Y.delete(i.id);let B=H.querySelector(".event-details");B&&(B.style.display="none")}})}return M},v=Array.from(e.children),x=new Map(v.map(i=>[i.dataset.eventId,i])),p=new Set(ie.map(i=>i.id));v.forEach(i=>{let n=i.dataset.eventId;(!n||!p.has(n))&&i.remove()});let m=null;ie.forEach((i,n)=>{let o=x.get(i.id);(!o||t)&&(o&&o.remove(),o=u(i),!o)||(n===0?e.firstElementChild!==o&&e.prepend(o):m&&m.nextElementSibling!==o&&m.after(o),m=o)}),me=Date.now(),N(1,me)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=$("offline","Failed to load events.","The event service may be offline or unreachable."))}}function $t(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),a=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ie.forEach(l=>Y.add(l.id)),X(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Y.clear(),X(!0)},e.dataset.listenerAttached="true"),a&&!a.dataset.listenerAttached&&(a.querySelectorAll(".filter-btn").forEach(l=>{l.onclick=()=>{a.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),l.classList.add("active"),pe=l.dataset.filter,X(!0)}}),a.dataset.listenerAttached="true")}function Xe(){return`

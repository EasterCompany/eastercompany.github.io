(()=>{function Me(){console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Be(t=!1){let e=window.location.pathname,i=e==="/"||e==="/index.html",r=`
    ...
    `}
        ...
        `}
    `,c=document.createElement("nav");c.innerHTML=r,document.body.prepend(c)}function De(t){let e=window.location.pathname,i=e==="/"||e==="/index.html",o=document.getElementById("nav-chevron"),s=document.querySelector(".nav-left");!o||!s||(t||!i?(o.style.display="block",s.style.cursor="pointer",s.classList.add("recording"),s.onmouseenter=()=>{o.style.transform="translateX(-3px)"},s.onmouseleave=()=>{o.style.transform="translateX(0)"}):(o.style.display="none",s.style.cursor="default",s.classList.remove("recording"),s.onmouseenter=null,s.onmouseleave=null))}function He(){if(document.querySelector("footer"))return;let t=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var Ne=1e3;function z(t){let e=null,i=t.onClose||null,o=t.onOpen||null;function s(){e&&(e.style.zIndex=++Ne)}function r(){if(e){e.classList.add("open"),s(),window.addEventListener("resize",c),o&&setTimeout(o,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++Ne,e.addEventListener("mousedown",s);let y=t.icon||"bx-window",n="",l="",d;t.tabs&&t.tabs.length>0?(n=`<div class="tab-bar">${t.tabs.map((w,a)=>{let u=w.icon?`<i class="bx ${w.icon}"></i>`:`<span class="tab-glyph">${w.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
            ...
            ${d}
        `,p.appendChild(e),e.querySelector(".window-close")?.addEventListener("click",x=>{x.stopPropagation(),v()}),window.addEventListener("resize",c),t.tabs&&e.querySelectorAll(".tab").forEach(x=>{x.addEventListener("click",()=>{let h=x.getAttribute("data-tab-index");e.querySelectorAll(".tab").forEach(w=>w.classList.remove("active")),x.classList.add("active"),e.querySelectorAll(".tab-content").forEach(w=>w.classList.remove("active")),e.querySelector(`.tab-content[data-tab-content="${h}"]`).classList.add("active"),g(x,e)})}),setTimeout(()=>{e.classList.add("open"),o&&o()},10)}function c(){if(!e||!e.classList.contains("open"))return;let p=e.querySelector(".tab.active");p&&g(p,e)}function g(p,y){setTimeout(()=>{let n=y.querySelector(".tab-bar");if(!n)return;let l=Array.from(n.querySelectorAll(".tab")),d=l.indexOf(p),x=n.clientWidth,h=l[Math.max(0,d-2)],w=l[Math.min(l.length-1,d+2)],a=h.offsetLeft-n.offsetLeft-25,f=w.offsetLeft+w.offsetWidth-n.offsetLeft+25-a,C=f<=x?a-(x-f)/2:p.offsetLeft-n.offsetLeft-x/2+p.offsetWidth/2;n.scrollTo({left:C,behavior:"smooth"})},350)}function v(p=!1){e&&(window.removeEventListener("resize",c),p?(e.remove(),e=null):(e.classList.remove("open"),i&&i(),setTimeout(()=>{e?.remove(),e=null},400)))}function m(p){let y=e?.querySelector(".window-content");y&&(y.innerHTML=p)}function b(){return e&&e.classList.contains("open")}return{open:r,close:v,setContent:m,isOpen:b,focus:s,id:t.id}}function Re(){return!0}function Pe(){return localStorage.getItem("easter_user_email")||"master@easter.company"}function Oe(t){localStorage.setItem("easter_user_email",t.trim())}var Ue="easter_theme",q={DARK:"dark",LIGHT:"light",LEGACY:"legacy"};function se(){return localStorage.getItem(Ue)||q.DARK}function qe(t){if(!Object.values(q).includes(t))throw new Error("Invalid theme");localStorage.setItem(Ue,t),le(t)}function le(t){let e=document.body;if(Object.values(q).forEach(o=>{e.classList.remove(`theme-${o}`)}),e.classList.add(`theme-${t}`),[q.LIGHT,q.LEGACY].includes(t)){if(e.classList.add("is-animated"),!document.querySelector(".background")){let o=document.createElement("div");o.className="background",document.body.prepend(o)}}else e.classList.remove("is-animated"),document.querySelector(".background")?.remove()}function ze(){let t=se();le(t)}function L(t,e,i=null){let s={config:"bx-cog",error:"bx-error-circle",empty:"bx-info-circle",offline:"bx-wifi-off"}[t]||"bx-info-circle",r=i?`<p class="placeholder-action">${i}</p>`:"";return`<div class="tab-placeholder"><i class='bx ${s} placeholder-icon'></i><p class="placeholder-message">${e}</p>${r}</div>`}function A(t){return t&&t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function B(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"] .tab-subtitle`);if(!i)return;if(!e){i.textContent="Last updated: never";return}let o=Date.now(),s=Math.floor((o-e)/1e3),r;s<60?r=`${s}s ago`:s<3600?r=`${Math.floor(s/60)}m ago`:r=`${Math.floor(s/3600)}h ago`,i.textContent=`Last updated: ${r}`}var Fe=0;function je(){return Fe}function j(t,e){let i=document.querySelector(`.tab[data-tab-index="${t}"]`);if(!i)return;let o=i.querySelector(".notification-badge");o&&(e>0?(o.textContent=e>9?"9+":e,o.style.display="flex"):o.style.display="none")}function ne(t){Fe=t;let e=document.getElementById("dexter-nav-badge");e&&(t>0?e.style.display="flex":e.style.display="none");let i=document.getElementById("alerts-menu-item");if(i){let s=i.querySelector(".notification-badge");s||(s=document.createElement("span"),s.className="notification-badge",s.style.marginLeft="auto",i.appendChild(s)),t>0?(s.textContent=t>9?"9+":t,s.style.display="flex"):s.style.display="none"}let o=document.getElementById("switch-alerts");if(o){let s=o.querySelector(".notification-badge");s||(s=document.createElement("span"),s.className="notification-badge",s.style.marginLeft="8px",o.appendChild(s)),t>0?(s.textContent=t>9?"9+":t,s.style.display="flex"):s.style.display="none"}}function de(){let t=document.getElementById("alerts-list");if(!t)return;let e=t.querySelectorAll(".alert-unread:not(.priority-low)").length;ne(e)}function Se(){return"https://event.easter.company"}function kt(){return"https://discord.easter.company"}var It="http://127.0.0.1:8100",At="http://127.0.0.1:8300",_t={31:"ansi-red",91:"ansi-bright-red",32:"ansi-green",33:"ansi-yellow",34:"ansi-blue",35:"ansi-purple",36:"ansi-cyan",37:"ansi-white",90:"ansi-dark-gray"};function me(t){let e=A(t);e=e.replace(/\x1b\[0m/g,"</span>"),e=e.replace(/\x1b\[(\d+)m/g,(s,r)=>{let c=_t[r];return c?`<span class="${c}">`:""});let i=(e.match(/<span/g)||[]).length,o=(e.match(/<\/span/g)||[]).length;return i>o&&(e+="</span>".repeat(i-o)),e}function Ge(t){if(!t)return"";let e=A(t);return e=e.replace(/```([\s\S]*?)```/g,'<pre class="md-code-block">$1</pre>'),e=e.replace(/`([^`]+)`/g,'<code class="md-inline-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/^# (.*$)/gm,'<h3 class="md-header">$1</h3>'),e=e.replace(/^## (.*$)/gm,'<h4 class="md-header">$1</h4>'),e=e.replace(/^### (.*$)/gm,'<h5 class="md-header">$1</h5>'),e=e.replace(/^- (.*$)/gm,'<div class="md-list-item"><span class="md-bullet">\u2022</span> $1</div>'),e=e.replace(/^(\d+)\. (.*$)/gm,'<div class="md-list-item"><span class="md-number">$1.</span> $2</div>'),e}var J=null,Y=null,re=!1,ce=!1;async function k(t,e={}){if(J)try{let s=await fetch(J+t,e);if(s.ok)return s;J=null}catch{J=null}let i=Se(),o=It;try{let s=await fetch(i+t,e);if(s.ok)return J=i,re&&(console.log("\u2728 Production event service restored."),re=!1),s;throw new Error("Primary failed")}catch{re||(console.warn(`\u{1F310} Production service unreachable. Falling back to local: ${o}`),re=!0);try{let r=await fetch(o+t,e);if(r.ok)return J=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}async function We(t,e={}){if(Y)try{let s=await fetch(Y+t,e);if(s.ok)return s;Y=null}catch{Y=null}let i=kt(),o=At;try{let s=await fetch(i+t,e);if(s.ok)return Y=i,ce&&(console.log("\u2728 Production discord service restored."),ce=!1),s;throw new Error("Primary failed")}catch{ce||(console.warn(`\u{1F310} Production discord service unreachable. Falling back to local: ${o}`),ce=!0);try{let r=await fetch(o+t,e);if(r.ok)return Y=o,r;throw new Error("Fallback failed")}catch(r){throw r}}}var Ve=()=>`
...
`,pe=null,G=new Set,ue=[];async function W(t=!1){let e=document.getElementById("alerts-list");if(!e)return;Mt(),t&&(e.innerHTML="<p>Updating...</p>");let i="/events?ml=1000&format=json&event.type=system.notification.generated";try{let o=await k(i);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];pe=Date.now(),B(0,pe);let c=Date.now(),g=24*60*60*1e3,v=r.filter(a=>{let u=localStorage.getItem(`alert_read_ts_${a.id}`);if(!u)return!0;let f=parseInt(u);return c-f<g});v.sort((a,u)=>{let f=_=>{let T=_.event;if(typeof T=="string")try{T=JSON.parse(T)}catch{return"low"}return(T.priority||"low").toLowerCase()},C=_=>_==="critical"?4:_==="high"?3:_==="medium"?2:1,E=C(f(a)),S=C(f(u));return E!==S?S-E:u.timestamp-a.timestamp}),ue=v;let m=a=>{let u=a.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return"low"}return(u.priority||"low").toLowerCase()},b=[],y=new Set(v.map(a=>m(a))).size>1;if(v.length>0){let a=null;v.forEach(u=>{let f=m(u);y&&f!==a&&(b.push({id:`divider-${f}`,type:"divider",label:f.toUpperCase()}),a=f),b.push(u)})}if(t&&(e.innerHTML=""),v.length===0){e.innerHTML=L("empty","No alerts yet."),de();return}let n=a=>{let u=a.event;if(typeof u=="string")try{u=JSON.parse(u)}catch{return null}let f=u.title||"Untitled Alert",C=u.body||"No description provided.",E=u.priority||"low",S=!!u.alert,_=u.category||"system",T=u.related_event_ids||[],I=!!localStorage.getItem(`alert_read_ts_${a.id}`),R=new Date(a.timestamp*1e3),D=R.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),P=R.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=I?"event-border-grey":"event-border-blue";!I&&S&&($="event-border-red"),I&&(E==="high"||E==="critical")?$="event-border-red":I&&E==="medium"&&($="event-border-orange");let M=I?"alert-read":"alert-unread",H=G.has(a.id),N=H?"expanded":"",xe=H?"display: block;":"display: none;",oe="",Ie="";T.length>0&&(Ie=`
                    <div class="event-detail-row">
                        <span class="detail-label">Related Events:</span>
                        <span class="detail-value">${T.map(F=>`<span class="related-event-id" style="font-family: monospace; opacity: 0.7;">${F.substring(0,8)}...</span>`).join(", ")}</span>
                    </div>`),oe=`
                <div class="event-detail-row">
...
`+C)}</div>
            ...
            `;let Ae=U.querySelector(".event-details");Ae&&Ae.addEventListener("click",F=>{F.stopPropagation()});let _e=U.querySelector(".close-details-btn");return _e&&_e.addEventListener("click",F=>{F.stopPropagation(),U.classList.remove("expanded");let we=U.querySelector(".event-details");we&&(we.style.display="none"),G.delete(a.id)}),U},l=a=>{let u=document.createElement("div");u.className="notification-divider",u.dataset.alertId=a.id;let f="#888888";return a.label==="CRITICAL"?f="#ff4d4d":a.label==="HIGH"?f="#ff8888":a.label==="MEDIUM"&&(f="#ffa500"),u.style.cssText=`display: flex; align-items: center; gap: 15px; color: ${f}; font-size: 0.7em; font-weight: 700; margin: 20px 0 10px 0; text-transform: uppercase; letter-spacing: 1.5px; opacity: 0.8;`,u.innerHTML=`<span style="white-space: nowrap;">${a.label} PRIORITY</span><div style="height: 1px; background: linear-gradient(to right, ${f}44, transparent); flex-grow: 1;"></div>`,u},d=Array.from(e.children),x=new Map(d.map(a=>[a.dataset.alertId,a])),h=new Set(b.map(a=>a.id));d.forEach(a=>{let u=a.dataset.alertId;(!u||!h.has(u))&&a.remove()});let w=null;b.forEach((a,u)=>{let f=x.get(a.id);(!f||t)&&(f&&f.remove(),a.type==="divider"?f=l(a):f=n(a),!f)||(u===0?e.firstElementChild!==f&&e.prepend(f):w&&w.nextElementSibling!==f&&w.after(f),w=f)}),pe=Date.now(),B(0,pe),de()}catch(o){console.error("Error fetching alerts:",o),e.children.length===0&&(e.innerHTML=L("offline","Failed to load alerts.","The event service may be offline."))}}function Mt(){let t=document.getElementById("alerts-read-all"),e=document.getElementById("alerts-expand-all"),i=document.getElementById("alerts-close-all"),o=document.getElementById("alerts-clear");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ue.forEach(s=>{localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{ue.forEach(s=>{G.add(s.id),localStorage.getItem(`alert_read_ts_${s.id}`)||localStorage.setItem(`alert_read_ts_${s.id}`,Date.now().toString())}),W(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{G.clear(),W(!0)},i.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{let s=Date.now()-1728e5;ue.forEach(r=>{localStorage.setItem(`alert_read_ts_${r.id}`,s.toString())}),G.clear(),W(!0)},o.dataset.listenerAttached="true")}async function Ke(){let t="/events?ml=1000&format=json&event.type=system.notification.generated";try{let e=await k(t);if(!e.ok)return;let o=(await e.json()).events||[],s=0;o.forEach(r=>{let c=r.event;if(typeof c=="string")try{c=JSON.parse(c)}catch{c={}}if((c.priority||"low").toLowerCase()==="low")return;localStorage.getItem(`alert_read_ts_${r.id}`)||s++}),ne(s)}catch{}}var Je=()=>`
...
`,V=new Set,Xe=[],X=null;async function K(t=!1){let e=document.getElementById("roadmap-list");if(e){Bt();try{let i=await k("/roadmap");if(!i.ok)throw new Error("Offline");let o=await i.json();if(Xe=o,o.length===0){e.innerHTML=L("empty","Your roadmap is empty.",`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let s=m=>{let b=V.has(m.id),p=m.state==="draft",y=m.state==="published",n=m.state==="consumed",l="event-border-grey";y&&(l="event-border-blue"),n&&(l="event-border-purple");let x=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${l} cursor-pointer ${b?"expanded":""}`,h.dataset.itemId=m.id,h.onclick=u=>{if(u.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",V.delete(m.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",V.add(m.id))};let w=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;h.innerHTML=`
        <div class="event-time">
        ...
        <div class="event-content">
          ...
          <div class="event-message" style="white-space: pre-wrap;">${A(m.content)}</div>
          <div class="event-details" style="${b?"display: block;":"display: none;"} ">
            ...
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
              ...
              <button class="notif-action-btn publish-toggle-btn">
                <i class='bx ${y?"bx-pause":"bx-rocket"}'></i> ${y?"Un-publish":"Publish"}
              </button>
      ...
      `;let a=h.querySelector(".event-details");return a&&a.addEventListener("click",u=>{u.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>Dt(m)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>Ht(m)),h.querySelector(".delete-btn")?.addEventListener("click",()=>Nt(m.id)),h.querySelector(".close-details-btn")?.addEventListener("click",u=>{u.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",V.delete(m.id)}),h},r=Array.from(e.children),c=new Map(r.map(m=>[m.dataset.itemId,m])),g=new Set(o.map(m=>m.id));r.forEach(m=>{let b=m.dataset.itemId;(!b||!g.has(b))&&m.remove()});let v=null;o.forEach((m,b)=>{let p=c.get(m.id);(!p||t)&&(p&&p.remove(),p=s(m),!p)||(b===0?e.firstElementChild!==p&&e.prepend(p):v&&v.nextElementSibling!==p&&v.after(p),v=p)})}catch{e.children.length===0&&(e.innerHTML=L("offline","Failed to load roadmap.","The event service may be offline."))}}}function Bt(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),o=document.getElementById("roadmap-expand-all"),s=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{X=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",X=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let r=document.getElementById("roadmap-editor-input").value;if(!r.trim())return;let c=X?`/roadmap/${X}`:"/roadmap",g=X?"PATCH":"POST";try{await k(c,{method:g,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:r})}),document.getElementById("roadmap-editor-container").style.display="none",K(!0)}catch(v){console.error(v)}},e.dataset.listenerAttached="true"),o&&!o.dataset.listenerAttached&&(o.onclick=()=>{Xe.forEach(r=>V.add(r.id)),K(!0)},o.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{V.clear(),K(!0)},s.dataset.listenerAttached="true")}function Dt(t){X=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function Ht(t){let e=t.state==="published"?"draft":"published";try{await k(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),K(!0)}catch(i){console.error(i)}}async function Nt(t){if(confirm("Delete this roadmap item?"))try{await k(`/roadmap/${t}`,{method:"DELETE"}),V.delete(t),K(!0)}catch(e){console.error(e)}}var Ze=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,Qe=null,Q=new Set,tt=[];async function ge(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;Rt();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let o=await k(i);if(!o.ok)throw new Error("Service is offline or unreachable.");let r=(await o.json()).events||[];if(tt=r,Qe=Date.now(),B(2,Qe),r.length===0){e.innerHTML=L("empty","No architectural blueprints generated yet.","The Analyst Worker will generate these when idle."),j(1,0);return}t&&(e.innerHTML="");let c=p=>{let y=p.event;if(typeof y=="string")try{y=JSON.parse(y)}catch{return null}let n=y.title||"Untitled Blueprint",l=y.summary||y.body||"No summary provided.",d=y.content||"",x=y.category||"architecture",h=y.affected_services||[],w=y.implementation_path||[],a=new Date(p.timestamp*1e3),u=a.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),f=a.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),C=Q.has(p.id),E=C?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${C?"expanded":""}`,S.dataset.blueprintId=p.id;let T={architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[x]||"bx-paint";S.onclick=function(P){if(this.classList.contains("expanded")){this.classList.remove("expanded"),Q.delete(p.id);let M=this.querySelector(".event-details");M&&(M.style.display="none")}else{this.classList.add("expanded"),Q.add(p.id);let M=this.querySelector(".event-details");M&&(M.style.display="block")}};let O=h.length>0?`<div class="event-detail-row"><span class="detail-label">Related Services:</span> <span class="detail-value">${h.join(", ")}</span></div>`:"",I="";w.length>0&&(I=`
            ...
            `;let R=S.querySelector(".event-details");R&&R.addEventListener("click",P=>{P.stopPropagation()});let D=S.querySelector(".close-details-btn");return D&&D.addEventListener("click",P=>{P.stopPropagation(),S.classList.remove("expanded");let $=S.querySelector(".event-details");$&&($.style.display="none"),Q.delete(p.id)}),S},g=Array.from(e.children),v=new Map(g.map(p=>[p.dataset.blueprintId,p])),m=new Set(r.map(p=>p.id));g.forEach(p=>{let y=p.dataset.blueprintId;(!y||!m.has(y))&&p.remove()});let b=null;r.forEach((p,y)=>{let n=v.get(p.id);(!n||t)&&(n&&n.remove(),n=c(p),!n)||(y===0?e.firstElementChild!==n&&e.prepend(n):b&&b.nextElementSibling!==n&&b.after(n),b=n)}),j(2,r.length)}catch(o){console.error("Error fetching blueprints:",o),e.children.length===0&&(e.innerHTML=L("offline","Failed to load blueprints.","The event service may be offline."))}}function Rt(){let t=document.getElementById("blueprints-expand-all"),e=document.getElementById("blueprints-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{tt.forEach(i=>Q.add(i.id)),ge(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Q.clear(),ge(!0)},e.dataset.listenerAttached="true")}var st=()=>`
...
`,nt=null;async function ve(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=()=>ve(),e.dataset.listenerAttached="true");try{let i=await We("/contacts");if(!i.ok)throw new Error("Service unreachable");let s=(await i.json()).members||[];if(nt=Date.now(),B(4,nt),s.length===0){t.innerHTML=L("empty","No contacts found.","Check your Discord connection.");return}let r={Me:0,"Master User":1,Admin:2,Moderator:3,Contributor:4,User:5,Anyone:6};s.sort((c,g)=>{let v=r[c.level]??10,m=r[g.level]??10;return v!==m?v-m:c.username.localeCompare(g.username)}),t.innerHTML=s.map(c=>{let g=c.color?"#"+c.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",v=c.status==="online"?"#5eff5e":c.status==="idle"?"#ffa500":c.status==="dnd"?"#ff4d4d":"#666",m="#888";c.level==="Me"||c.level==="Master User"?m="#bb86fc":c.level==="Admin"?m="#cf6679":c.level==="Moderator"?m="#03dac6":c.level==="Contributor"&&(m="#ffa500");let b=c.level==="Me",p=b?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",y=b?"2px solid #bb86fc":`1px solid ${g}33`;return`
                <div class="user-profile-section" style="background: ${p}; padding: 15px; border-radius: 8px; border: ${y}; display: flex; align-items: center; gap: 12px; position: relative; overflow: hidden;">
...
`,fe=null,Z=new Set,ie=[],be="all",Ot={messaging:["message_received","messaging.user.sent_message","messaging.bot.sent_message","messaging.user.transcribed","voice_transcribed","bot_response","messaging.user.joined_voice","messaging.user.left_voice","messaging.bot.joined_voice","messaging.bot.voice_response","messaging.user.speaking.started","messaging.user.speaking.stopped","messaging.webhook.message"],system:["system.cli.command","system.cli.status","system.status.change","metric_recorded","log_entry","error_occurred","webhook.processed","messaging.bot.status_update","messaging.user.joined_server","system.test.completed","system.build.completed","system.roadmap.created","system.roadmap.updated","system.process.registered","system.process.unregistered"],cognitive:["engagement.decision","system.analysis.audit","system.blueprint.generated","analysis.link.completed","analysis.visual.completed"],moderation:["moderation.explicit_content.deleted"]},Ut={message_received:"bx-message-detail","messaging.user.sent_message":"bx-message-rounded-dots","messaging.bot.sent_message":"bx-bot","messaging.user.transcribed":"bx-microphone",voice_transcribed:"bx-microphone","messaging.user.joined_voice":"bx-phone-incoming","messaging.user.left_voice":"bx-phone-outgoing","messaging.webhook.message":"bx-cloud-lightning","system.cli.command":"bx-terminal","system.cli.status":"bx-info-circle","system.test.completed":"bx-check-shield","system.build.completed":"bx-package","system.roadmap.created":"bx-map-pin","system.roadmap.updated":"bx-map-alt","system.process.registered":"bx-play-circle","system.process.unregistered":"bx-stop-circle",error_occurred:"bx-error-alt","engagement.decision":"bx-brain","system.analysis.audit":"bx-search-alt","system.blueprint.generated":"bx-paint","analysis.link.completed":"bx-link","analysis.visual.completed":"bx-image","moderation.explicit_content.deleted":"bx-shield-x","system.status.change":"bx-refresh",metric_recorded:"bx-line-chart"};function ot(t){for(let[e,i]of Object.entries(Ot))if(i.includes(t))return e;return"system"}function qt(t){return Ut[t]||"bx-square-rounded"}async function ee(t=!1){let e=document.getElementById("events-timeline");if(!e)return;zt();let o=`/events?ml=${be==="all"?100:250}&format=json&exclude_types=system.notification.generated`;try{let s=await k(o);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[],g=c;if(be!=="all"&&(g=c.filter(n=>{let l=n.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return!1}return ot(l.type)===be})),ie=g.slice(0,50),fe=Date.now(),B(1,fe),ie.length===0){e.innerHTML=L("empty","No events found for this filter.");return}t&&(e.innerHTML="");let v=n=>{let l=n.event;if(typeof l=="string")try{l=JSON.parse(l)}catch{return null}let d=l.type,x=ot(d),h=qt(d),w=d==="engagement.decision"||d==="messaging.bot.sent_message"||d==="messaging.user.sent_message"||d==="moderation.explicit_content.deleted"||d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.cli.command"||d==="system.analysis.audit"||d==="system.test.completed"||d==="error_occurred"||d==="system.cli.status"||d.startsWith("system.roadmap")||d.startsWith("system.process"),a="event-border-grey";w&&(d==="moderation.explicit_content.deleted"||d==="error_occurred"?a="event-border-red":d==="analysis.link.completed"||d==="analysis.visual.completed"||d==="system.analysis.audit"?a="event-border-purple":d==="system.cli.command"||d==="system.cli.status"?a="event-border-orange":d==="system.test.completed"?a=l.test?.status==="OK"&&l.lint?.status==="OK"&&l.format?.status==="OK"?"event-border-blue":"event-border-red":a="event-border-blue");let u=w?"cursor-pointer":"",f=Z.has(n.id),C=f?"expanded":"",E=f?"display: block;":"display: none;",S=new Date(n.timestamp*1e3),_=S.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),T=S.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),O=at(d,l),I=l.user_level?`<span style="font-size: 0.8em; opacity: 0.6; margin-left: 5px;">(${l.user_level})</span>`:"",R="";if(w){let $="";if(d==="engagement.decision")$=`
                    ...
                    `;else if(d==="analysis.visual.completed"){let M="";l.base64_preview?M=`<div class="event-detail-block"><img src="data:image/jpeg;base64,${l.base64_preview}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`:l.url&&(M=`<div class="event-detail-block"><img src="${l.url}" class="event-image-preview" style="max-width: 100%; border-radius: 4px; margin-top: 10px;"></div>`),$=`
                        <div class="event-detail-row">
                    ...
                    `;else if(d==="messaging.user.sent_message"){let M="";l.attachments&&l.attachments.length>0&&(M=`
                            <div class="event-detail-block">
                                <span class="detail-label">Attachments:</span>
                                <div class="attachments-grid">${l.attachments.map(N=>{let xe=N.content_type&&N.content_type.startsWith("image/"),oe=(N.size/1024).toFixed(1)+" KB";return`
            ...
            `;let P=D.querySelector(".event-details");if(P&&P.addEventListener("click",$=>{$.stopPropagation()}),w){let $=D.querySelector(".close-details-btn");$&&$.addEventListener("click",M=>{M.stopPropagation();let H=M.target.closest(".event-item");if(H){H.classList.remove("expanded"),Z.delete(n.id);let N=H.querySelector(".event-details");N&&(N.style.display="none")}})}return D},m=Array.from(e.children),b=new Map(m.map(n=>[n.dataset.eventId,n])),p=new Set(ie.map(n=>n.id));m.forEach(n=>{let l=n.dataset.eventId;(!l||!p.has(l))&&n.remove()});let y=null;ie.forEach((n,l)=>{let d=b.get(n.id);(!d||t)&&(d&&d.remove(),d=v(n),!d)||(l===0?e.firstElementChild!==d&&e.prepend(d):y&&y.nextElementSibling!==d&&y.after(d),y=d)}),fe=Date.now(),B(1,fe)}catch(s){console.error("Error fetching events:",s),e.children.length===0&&(e.innerHTML=L("offline","Failed to load events.","The event service may be offline."))}}function zt(){let t=document.getElementById("events-expand-all"),e=document.getElementById("events-close-all"),i=document.getElementById("event-filters");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{ie.forEach(o=>Z.add(o.id)),ee(!0)},t.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=()=>{Z.clear(),ee(!0)},e.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.querySelectorAll(".filter-btn").forEach(o=>{o.onclick=()=>{i.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active"),be=o.dataset.filter,ee(!0)}}),i.dataset.listenerAttached="true")}function rt(){return`
        <div id="logs-container" class="logs-container"></div>
    `}var Ft=null;async function ae(){let t=document.getElementById("logs-container");if(!t)return!1;t.classList.remove("placeholder-active");try{let e=await k("/logs");if(!e.ok)throw new Error("Logs offline");let i=await e.json();if(!i||i.length===0)return t.innerHTML=L("empty","No logs found.","Services are quiet."),t.classList.add("placeholder-active"),!1;let o=["local-ollama-0","local-cache-0","cloud-cache-0","cloud-cache-1"],s=i.filter(c=>!o.includes(c.id));s.forEach(c=>{c.logs&&Array.isArray(c.logs)?c.logs.reverse():c.logs=[]}),s.reverse();let r=s.map(c=>{let g=c.logs.join(`
`),v=[...c.logs];if(v.length<25){let b=25-v.length;for(let p=0;p<b;p++)v.push("")}else v.length>25&&(v=v.slice(-25));let m=v.map(b=>me(b)).join(`
`);return`
        ...
        <div id="processes-history-widgets" class="system-monitor-widgets" style="margin-bottom: 30px; opacity: 0.7;"></div>`,ft=()=>`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${rt()}
        </div>`;async function xt(){await Promise.all([ye(),Le(),Ce()]),await ae()}var ct=null,dt=null,mt=null;async function wt(){try{return await(await k("/system_monitor")).json()}catch{return null}}async function pt(){try{return await(await k("/system/hardware")).json()}catch{return null}}async function jt(){try{return await(await k("/processes")).json()}catch{return null}}async function Gt(){try{return await(await k("/analyst/status")).json()}catch{return null}}async function Le(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-info-content"),i=document.getElementById("hardware-refresh-btn"),o=n=>{if(!n){e.children.length===0&&(e.innerHTML=L("offline","Failed to load hardware info.","The event service may be offline."));return}let l="",d=(n.MEMORY_BYTES/(1024*1024*1024)).toFixed(1);if(l+=`
            <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                ...
                </div>`}if(n.GPU&&n.GPU.length>0&&n.GPU.forEach((x,h)=>{let w=(x.VRAM/1073741824).toFixed(1);l+=`
                    <div class="service-widget" style="padding: 10px; min-height: 80px;">
                        <div class="service-widget-header" style="margin-bottom: 5px;">
                    ...
                    </div>`}),n.STORAGE&&n.STORAGE.length>0){let x=0,h=0;n.STORAGE.forEach(f=>{x+=f.USED,h+=f.SIZE});let w=(x/(1024*1024*1024)).toFixed(1),a=(h/(1024*1024*1024)).toFixed(1),u=h>0?(x/h*100).toFixed(0):0;l+=`
                <div class="service-widget" style="padding: 10px; min-height: 80px;">
                    <div class="service-widget-header" style="margin-bottom: 5px;">
                ...
                </div>`}e.innerHTML=l};if(e&&i&&(i.dataset.listenerAttached||(i.onclick=async()=>{i.innerHTML="<i class='bx bx-loader-alt spin'></i> Refreshing...";let n=await pt();n?(o(n),i.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3)):(e.children.length===0&&(e.innerHTML=L("offline","Failed to refresh.","The event service may be offline.")),i.innerHTML="<i class='bx bx-error'></i> Failed",setTimeout(()=>{i.innerHTML="<i class='bx bx-refresh'></i> Refresh"},2e3))},i.dataset.listenerAttached="true"),e.children.length===0)){let n=await pt();o(n)}if(!t)return;let s=await wt();if(!s||!s.services){t.children.length===0&&(t.innerHTML=L("offline","Failed to load system metrics.","The event service may be offline."));return}ct=Date.now(),B(0,ct);let r=s.services||[];Array.from(t.children).forEach(n=>{n.classList.contains("service-widget")||n.remove()});function c(n){return!n||n==="N/A"||n==="unknown"||n.trim()===""?"-":n}function g(n){if(!n||n==="N/A"||n==="unknown")return"-";let l=n.match(/^(\d+\.\d+\.\d+)/);return l?l[0]:n.split(".").slice(0,3).join(".")||"-"}function v(n){return!n||n.length<=28?n:n.substring(0,28)+"..."}function m(n){if(!n||n==="N/A"||n==="unknown")return"-";let l=n.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!l)return"-";let d=parseInt(l[1])||0,x=parseInt(l[2])||0,h=parseInt(l[3])||0,w=parseFloat(l[4])||0,a=d*86400+x*3600+h*60+w,u=Math.floor(a/86400);if(u>0)return`${u}d`;let f=Math.floor(a/3600);if(f>0)return`${f}h`;let C=Math.floor(a/60);return C>0?`${C}m`:`${Math.floor(a)}s`}function b(n){let l=n.status==="online",d=l?"service-widget-online":"service-widget-offline",x=l?"bx-check-circle":"bx-x-circle",h=l?"OK":"BAD",w=n.version?g(n.version.str):"-",a=n.cpu&&n.cpu!=="N/A"?n.cpu:"-",u=n.memory&&n.memory!=="N/A"?n.memory:"-",f=a!=="-"?"#00ff00":"#666",C=a!=="-"?"#fff":"#666",E=u!=="-"?"#00ff00":"#666",S=u!=="-"?"#fff":"#666",_=m(n.uptime),T="";return l?T=`
                ...
                </div>`:T='<div class="service-widget-footer offline"><span>OFFLINE</span></div>',`<div class="service-widget ${d}" data-service-id="${n.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${n.short_name||n.id}</h3><span class="service-widget-status">${h}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${v(n.domain&&n.port?`${n.domain}:${n.port}`:"")}</span></div>${T}</div></div>`}let p=new Map(Array.from(t.querySelectorAll(".service-widget")).map(n=>[n.dataset.serviceId,n])),y=new Set(r.map(n=>n.id));for(let[n,l]of p)y.has(n)||l.remove();r.forEach(n=>{let l=b(n),d=p.get(n.id);d?d.outerHTML!==l&&(d.outerHTML=l):t.insertAdjacentHTML("beforeend",l)})}async function Ce(){let t=document.getElementById("models-widgets");if(!t)return;let e=await wt();if(!e){t.children.length===0&&(t.innerHTML=L("offline","Failed to load model status.","The event service may be offline."));return}dt=Date.now(),B(2,dt);let i=e.models||[],o=e.whisper;Array.from(t.children).forEach(g=>{g.classList.contains("service-widget")||g.remove()});function s(g){let v=g.status==="Ready";return`
                ...
                </div>`}function r(g){let v=g.status==="Downloaded",m=v?"service-widget-online":"service-widget-offline",b=v?"OK":"MISSING",p=v&&g.size>0?`${(g.size/1e9).toFixed(2)} GB`:"-",y=g.name;return g.type==="custom"&&y.endsWith(":latest")&&(y=y.replace(":latest","")),`<div class="service-widget ${m}" data-model-name="${g.name}"><div class="service-widget-header"><i class="bx ${v?"bx-check-circle":"bx-x-circle"}"></i><h3>${y}</h3><span class="service-widget-status">${b}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${g.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${p}</span></div></div></div>`}let c="";if(o&&(c+=s(o)),c+=i.map(r).join(""),!c){t.innerHTML=L("empty","No models found.");return}t.innerHTML!==c&&(t.innerHTML=c)}async function ye(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("analyst-t1-val"),i=document.getElementById("analyst-t2-val"),o=document.getElementById("analyst-t3-val"),s=document.getElementById("analyst-idle-val"),r=document.getElementById("analyst-reset-btn");r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i> Resetting...";try{await k("/analyst/reset?tier=all",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i> Done",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i> Reset Analyst"},2e3)},500),ye()}catch{r.innerHTML="<i class='bx bx-error'></i> Failed"}},r.dataset.listenerAttached="true");let c=await Gt();if(c){let y=Math.floor(Date.now()/1e3),n=c.active_tier,l=(d,x,h)=>{if(n===h||h==="guardian"&&n==="tests"){d.textContent="Working",d.style.color="#bb86fc";return}let w=x-y;if(w<=0)d.textContent="Ready",d.style.color="#5eff5e";else{let a=Math.floor(w/60),u=w%60;d.textContent=`${a}m ${u}s`,d.style.color="#fff"}};if(e&&c.guardian&&l(e,c.guardian.next_run,"guardian"),i&&c.architect&&l(i,c.architect.next_run,"architect"),o&&c.strategist&&l(o,c.strategist.next_run,"strategist"),s&&c.system_idle_time!==void 0){let d=c.system_idle_time,x=Math.floor(d/60),h=d%60;s.textContent=`${x}m ${h}s`,d>300?s.style.color="#5eff5e":d>60?s.style.color="#ffa500":s.style.color="#fff"}}else[e,i,o,s].forEach(n=>{n&&(n.textContent="Offline",n.style.color="#ff4d4d")});let g=await jt(),v=[],m=[];g&&(Array.isArray(g)?v=g:(v=g.active||[],m=g.history||[]));let b=document.getElementById("vitals-processes-val");if(b)if(g){let y=v.length;b.textContent=y>0?`${y} Active`:"Idle",b.style.color=y>0?"#bb86fc":"#fff"}else b.textContent="-",b.style.color="#888";if(g===null){t.children.length===0&&(t.innerHTML=L("offline","Failed to load process status.","The event service may be offline."));return}mt=Date.now(),B(1,mt),v.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=L("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),ut(t,v,!1));let p=document.getElementById("processes-history-widgets");p&&(m.length===0?p.querySelector(".tab-placeholder")||(p.innerHTML=L("empty","No recent history.")):(p.querySelector(".tab-placeholder")&&(p.innerHTML=""),ut(p,m,!0))),j(1,v.length)}function ut(t,e,i){function o(c){let g="";c.end_time?g=`${c.end_time-c.start_time}s (Completed)`:g=`${Math.floor(Date.now()/1e3-c.start_time)}s`;let v=c.retries>0?`<span class="process-retry-badge">Retry ${c.retries}</span>`:"",m=c.channel_id,b={"system-discord":"Discord Engine","system-analyst":"Analyst Worker","system-cli-op":"CLI Operation"};b[m]?m=b[m]:/^\d+$/.test(m)&&(m=`Channel ${m}`);let p=i?"#888":"#fff",y=i?"border-left: 3px solid #666;":"";return`
                <div class="service-widget process-widget" data-channel-id="${c.channel_id}-${c.start_time}" style="${y}">
                    <div class="service-widget-header">
                        <i class="bx bx-cog" style="color: ${p}"></i>
                        <h3 style="color: ${p}">${m}</h3>
                        ${v}
                    </div>
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            <span class="info-label">State:</span>
                            <span class="info-value" style="color: ${p}">${c.state}</span>
                        </div>
                        <div class="service-widget-info">
                            <span class="info-label">Duration:</span>
                            <span class="info-value" style="color: ${p}">${g}</span>
                        </div>
                         <div class="service-widget-info">
                            <span class="info-label">PID:</span>
                            <span class="info-value" style="color: ${p}">${c.pid}</span>
                        </div>
                    </div>
                </div>`}let s=new Map(Array.from(t.querySelectorAll(".process-widget")).map(c=>[c.dataset.channelId,c])),r=new Set(e.map(c=>`${c.channel_id}-${c.start_time}`));for(let[c,g]of s)r.has(c)||g.remove();e.forEach(c=>{let g=`${c.channel_id}-${c.start_time}`,v=o(c),m=s.get(g);m?m.outerHTML!==v&&(m.outerHTML=v):t.insertAdjacentHTML("beforeend",v)})}function he(){let t=se(),e=Pe()||"master@easter.company",i={enabled:Notification.permission==="granted",supported:"Notification"in window},o=localStorage.getItem("easter_analytics_enabled")!=="false";return`
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Vt=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
        <div class="cli-dashboard">
            ...
            <div class="cli-divider">
                ...
                <span>Interactive Demos (v${Wt})</span>
                <i class='bx bx-chevron-down'></i>
            </div>
    `;for(let[i,o]of Object.entries(t)){let s=$t.filter(r=>r.category===i);s.length!==0&&(e+=`
    ...
    </div>`,e},ke=!1;function Kt(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),ke=!0,i}function Et(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),ke=!1}function St(t,e,i="output"){if(!ke)return;let o=document.createElement("div");o.className=`terminal-line terminal-${i}`,i==="prompt"?o.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:o.innerHTML=me(e),t.appendChild(o),t.scrollTop=t.scrollHeight}async function Jt(t){let e=$t.find(s=>s.id===t);if(!e)return;let i=Kt(e);St(i,`dex ${t}`,"prompt");let o=e.demo_output||["Executing command...","\u2713 Done."];for(let s of o){await new Promise(c=>setTimeout(c,400+Math.random()*600));let r="output";s.includes("[ERROR]")?r="error":s.includes("[SUCCESS]")||s.includes("\u2713")?r="success":s.includes("!")&&(r="warning"),St(i,s,r)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Lt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Vt();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let o=e.querySelector("i");o.className="bx bx-check",o.style.color="#5eff5e",setTimeout(()=>{o.className="bx bx-copy",o.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let o=i.dataset.cmd;Jt(o)})})}async function Yt(){if(window.location.hostname==="easter.company")try{if(!(await fetch(`${Se()}/system/status`,{method:"HEAD"})).ok)throw new Error("Service unhealthy")}catch{console.error("Production event service unreachable. Redirecting to 404."),window.location.href="/404.html"}}function Ct(){Yt(),ze(),Me();let t=window.location.pathname;(t.includes("/dexter/cli")||t==="/cli"||t==="/cli/")&&Lt();let e=Re();Be(e),He();let i=document.getElementById("nav-left-container");i&&i.addEventListener("click",()=>{if(r.length>0)p();else{let a=window.location.pathname;if(!(a==="/"||a==="/index.html")){let C=(a.endsWith("/")&&a.length>1?a.slice(0,-1):a).split("/");C.pop();let E=C.join("/")||"/";window.location.href=E}}});let o=window.location.pathname==="/"||window.location.pathname==="/index.html",s=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!o&&!s&&document.querySelector("footer")?.classList.add("hide");let r=[],c=document.getElementById("windows-container");c&&c.setAttribute("data-count","0");let g=a=>{localStorage.setItem("dex_last_window",a)};function v(){return 1}function m(){for(;r.length>1;)r.shift().close(!0);let a=document.getElementById("windows-container"),u=document.querySelector("nav"),f=document.querySelector("footer"),C=window.location.pathname==="/"||window.location.pathname==="/index.html",E=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");a&&a.setAttribute("data-count",r.length);let S=r.length>0;r.forEach(I=>{let R=document.getElementById(I.id);R&&R.classList.remove("hide-close")}),le(se());let _=document.getElementById("dexter-menu-container"),T=document.getElementById("nav-window-switcher"),O=document.getElementById("settings-icon");if(De(r.length>0),r.length>0){if(f?.classList.add("hide"),document.getElementById("close-all-windows")?.style.setProperty("display","block"),document.querySelector("main")?.style.setProperty("opacity","0.3","important"),u?.classList.add("window-open"),a&&(a.style.paddingTop="60px"),_&&(_.style.display="none"),O&&(O.style.display="none"),T){let I=r[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(I)?(T.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{g("alerts-window"),b(y)}),document.getElementById("switch-events").addEventListener("click",()=>{g("events-window"),b(n)}),document.getElementById("switch-monitor").addEventListener("click",()=>{g("monitor-window"),b(d)}),document.getElementById("switch-contacts").addEventListener("click",()=>{g("contacts-window"),b(l)}),document.getElementById("switch-workspace").addEventListener("click",()=>{g("workspace-window"),b(x)})):T.innerHTML=""}}else u?.classList.remove("window-open"),document.getElementById("close-all-windows")?.style.setProperty("display","none"),a&&(a.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),C||E?f?.classList.remove("hide"):f?.classList.add("hide"),_&&(_.style.display="flex"),O&&(O.style.display="block"),T&&(T.innerHTML="");ne(je())}function b(a){if(a.isOpen()){a.close();return}for(;r.length>0;)r.pop().close(!0);r.push(a),a.open(),m()}function p(){[...r].forEach(u=>u.close()),r=[]}window.addEventListener("resize",m);let y=z({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:Ve(),onOpen:()=>W(),onClose:()=>{let a=r.indexOf(y);a>-1&&r.splice(a,1),m()}}),n=z({id:"events-window",title:"Events",icon:"bx-calendar-event",content:lt(),onOpen:()=>ee(),onClose:()=>{let a=r.indexOf(n);a>-1&&r.splice(a,1),m()}}),l=z({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:it(),onOpen:()=>ve(),onClose:()=>{let a=r.indexOf(l);a>-1&&r.splice(a,1),m()}}),d=z({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:ft()},{icon:"bxs-component",title:"Processes",content:vt()},{icon:"bxs-brain",title:"Models",content:bt()},{icon:"bxs-hdd",title:"Hardware",content:yt()},{icon:"bxs-terminal",title:"Logs",content:ht()},{icon:"bxs-zap",title:"Analyst",content:gt()}],onOpen:()=>{Le(),ye(),Ce(),ae()},onClose:()=>{let a=r.indexOf(d);a>-1&&r.splice(a,1),m()}}),x=z({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:st(),onOpen:()=>$e(),onClose:()=>{let a=r.indexOf(x);a>-1&&r.splice(a,1),m()}}),h=z({id:"settings-window",title:"Settings",icon:"bx-cog",content:he(),onOpen:()=>{h.setContent(he()),Te(h)},onClose:()=>{let a=r.indexOf(h);a>-1&&r.splice(a,1),m()}}),w=z({id:"login-window",title:"Welcome",content:'<div class="login-split-container"><div class="login-top-section"><div class="login-form"><p>Enter your email to continue</p><form id="login-form"><input type="email" id="email-input" placeholder="you@easter.company" required autocomplete="email" /><button type="submit">Continue</button><div id="login-error" class="error" style="display: none;"></div></form></div></div><div class="login-bottom-section"><div class="login-disclaimer"><h2>Early Access</h2><p>Contribute on <a href="/dexter/contribute" target="_blank" rel="noopener noreferrer">GitHub</a> to unlock early access.</p></div></div></div>',icon:"bx-log-in"});if(e){let a=document.getElementById("dexter-dropdown");a&&(a.innerHTML=`
            ...
            <div class="dropdown-item" id="workspace-menu-item"><i class='bx bx-brain'></i> Workspace</div>
        `);let u=document.getElementById("dexter-menu-container"),f=document.getElementById("dexter-menu-btn");u&&a&&f&&(u.addEventListener("mouseenter",()=>{a.classList.add("active"),f.classList.add("active")}),u.addEventListener("mouseleave",()=>{a.classList.remove("active"),f.classList.remove("active")}),f.addEventListener("click",C=>{C.stopPropagation();let E=localStorage.getItem("dex_last_window")||"alerts-window",S=null;E==="alerts-window"?S=y:E==="events-window"?S=n:E==="monitor-window"?S=d:E==="contacts-window"?S=l:E==="workspace-window"&&(S=x),S&&b(S)})),document.getElementById("alerts-menu-item")?.addEventListener("click",()=>{g("alerts-window"),b(y)}),document.getElementById("events-menu-item")?.addEventListener("click",()=>{g("events-window"),b(n)}),document.getElementById("monitor-menu-item")?.addEventListener("click",()=>{g("monitor-window"),b(d)}),document.getElementById("contacts-menu-item")?.addEventListener("click",()=>{g("contacts-window"),b(l)}),document.getElementById("workspace-menu-item")?.addEventListener("click",()=>{g("workspace-window"),b(x)}),document.getElementById("settings-icon")?.addEventListener("click",()=>b(h)),document.getElementById("close-all-windows")?.addEventListener("click",()=>p()),setInterval(()=>{y.isOpen()?W():Ke(),n.isOpen()&&ee(),l.isOpen()&&ve(),d.isOpen()&&xt(),x.isOpen()&&$e()},5e3)}else document.getElementById("login-btn")?.addEventListener("click",()=>{w.open(),setTimeout(()=>{document.getElementById("login-form")?.addEventListener("submit",a=>{a.preventDefault();try{Oe(document.getElementById("email-input").value),window.location.reload()}catch(u){let f=document.getElementById("login-error");f&&(f.textContent=u.message,f.style.display="block")}})},100)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ct):Ct();})();

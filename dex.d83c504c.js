...
`,re=null,z=new Set,ce=[];async function F(t=!1){let e=document.getElementById("notifications-list");if(!e)return;ft(),t&&(e.innerHTML="<p>Updating...</p>");let a="/events?ml=1000&format=json&event.type=system.notification.generated";try{let l=await I(a);if(!l.ok)throw new Error("Service is offline or unreachable.");let r=(await l.json()).events||[];re=Date.now(),M(0,re);let c=Date.now(),g=24*60*60*1e3,u=r.filter(d=>{let v=localStorage.getItem(`notification_read_ts_${d.id}`);if(!v)return!0;let y=parseInt(v);return c-y<g});u.sort((d,v)=>{let y=B=>{let A=B.event;if(typeof A=="string")try{A=JSON.parse(A)}catch{return"low"}return(A.priority||"low").toLowerCase()},L=B=>B==="critical"?4:B==="high"?3:B==="medium"?2:1,E=L(y(d)),T=L(y(v));return E!==T?T-E:v.timestamp-d.timestamp}),ce=u;let f=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return"low"}return(v.priority||"low").toLowerCase()},x=[],m=new Set(u.map(d=>f(d))).size>1;if(u.length>0){let d=null;u.forEach(v=>{let y=f(v);m&&y!==d&&(x.push({id:`divider-${y}`,type:"divider",label:y.toUpperCase()}),d=y),x.push(v)})}if(t&&(e.innerHTML=""),u.length===0){e.innerHTML=C("empty","No notifications yet."),oe();return}let i=d=>{let v=d.event;if(typeof v=="string")try{v=JSON.parse(v)}catch{return null}let y=v.title||"Untitled Notification",L=v.body||"No description provided.",E=v.priority||"low",T=!!v.alert,B=v.category||"system",A=v.related_event_ids||[],H=!!localStorage.getItem(`notification_read_ts_${d.id}`),P=new Date(d.timestamp*1e3),_=P.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),S=P.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),$=H?"event-border-grey":"event-border-blue";!H&&T&&($="event-border-red"),H&&(E==="high"||E==="critical")?$="event-border-red":H&&E==="medium"&&($="event-border-orange");let N=H?"notification-read":"notification-unread",D=z.has(d.id),ue=D?"expanded":"",ge=D?"display: block;":"display: none;",$e="",Ce="";A.length>0&&(Ce=`
            ...
            `;let U=document.createElement("div");U.className=`event-item notification-item ${$} ${N} ${ue} cursor-pointer`,U.dataset.notificationId=d.id,U.onclick=function(Z){if(this.classList.contains("expanded")){this.classList.remove("expanded"),z.delete(d.id);let ee=this.querySelector(".event-details");ee&&(ee.style.display="none")}else{this.classList.add("expanded"),z.add(d.id);let ee=this.querySelector(".event-details");if(ee&&(ee.style.display="block"),!localStorage.getItem(`notification_read_ts_${d.id}`)){localStorage.setItem(`notification_read_ts_${d.id}`,Date.now().toString()),this.classList.add("notification-read"),this.classList.remove("notification-unread"),this.classList.remove("event-border-blue","event-border-red","event-border-purple");let ve="event-border-grey";E==="high"||E==="critical"?ve="event-border-red":E==="medium"&&(ve="event-border-orange"),this.classList.add(ve),oe()}}};let Ht=y,dt={system:"bx-cog",messaging:"bx-message-detail",cognitive:"bx-brain",moderation:"bx-shield-x",lifecycle:"bx-git-branch"}[B]||"bx-bell";U.innerHTML=`
                ...
                <div class="event-content">
                    <div class="event-service">${B.toUpperCase()} ${T?'<span class="alert-badge" style="color: #ff4d4d; font-size: 0.8em; margin-left: 5px;">[ALERT]</span>':""}</div>

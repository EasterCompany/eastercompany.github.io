(()=>{function Xe(){if(!document.querySelector(".grid-overlay")){let t=document.createElement("div");t.className="grid-overlay",document.body.appendChild(t)}console.log("Applying base styles dynamically (e.g., for background animation or theme toggles).")}function Qe(){let t=window.location.pathname,e=t==="/"||t==="/index.html",n=`
    ...
    `,c=document.createElement("nav");c.innerHTML=n,document.body.prepend(c);let o=document.createElement("div");o.id="dexter-dropdown",o.className="dexter-dropdown",o.innerHTML=`
    ...
    `,e=document.createElement("footer");e.innerHTML=t,document.body.appendChild(e)}var tt=1e3;function ae(t){let e=null,i=t.onClose||null,s=t.onOpen||null;function n(){e&&(e.style.zIndex=++tt)}function c(){if(e){e.classList.add("open"),n(),window.addEventListener("resize",o),s&&setTimeout(s,10);return}let p=document.getElementById("windows-container");p||(p=document.createElement("div"),p.id="windows-container",p.className="windows-container",document.body.appendChild(p)),e=document.createElement("div"),e.id=t.id,e.className="window",t.tabs&&t.tabs.length>0&&e.classList.add("has-tabs"),e.style.zIndex=++tt,e.addEventListener("mousedown",n);let b=t.icon||"bx-window",a="",g="",$;t.tabs&&t.tabs.length>0?(a=`<div class="tab-bar">${t.tabs.map((l,d)=>{let f=l.icon?`<i class="bx ${l.icon}"></i>`:`<span class="tab-glyph">${l.title?.charAt(0).toUpperCase()||"?"}</span>`;return`
                ...
                `}).join("")}</div>`,$=`<div class="window-content">${t.tabs.map((l,d)=>`<div class="tab-content ${d===0?"active":""}" data-tab-content="${d}">${l.content}</div>`).join("")}</div>`):(t.title&&(g=`<div class="window-title">${t.title}</div>`),$=`<div class="window-content">${t.content||""}</div>`),e.innerHTML=`
...
`,le=new Set,Be=[],xe=null;async function ce(t=!1){let e=document.getElementById("roadmap-list");if(e){ls();try{let i=await B("/roadmap");if(!i.ok)throw new Error("Offline");let s=await i.json();Be=s;let n=m=>{let y=le.has(m.id),p=m.state==="published",b=m.state==="consumed",a="event-border-grey";p&&(a="event-border-blue"),b&&(a="event-border-purple");let $=new Date(m.created_at*1e3).toLocaleDateString(navigator.language,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),h=document.createElement("div");h.className=`event-item notification-item ${a} cursor-pointer ${y?"expanded":""}`,h.dataset.itemId=m.id,h.onclick=d=>{if(d.target.closest("button"))return;h.classList.contains("expanded")?(h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(m.id)):(h.classList.add("expanded"),h.querySelector(".event-details").style.display="block",le.add(m.id))};let u=`<span style="font-size: 0.7em; opacity: 0.6; margin-left: 10px;">[${m.state.toUpperCase()}]</span>`;h.innerHTML=`
      ...
      `;let l=h.querySelector(".event-details");return l&&l.addEventListener("click",d=>{d.stopPropagation()}),h.querySelector(".edit-btn")?.addEventListener("click",()=>cs(m)),h.querySelector(".publish-toggle-btn")?.addEventListener("click",()=>ds(m)),h.querySelector(".delete-btn")?.addEventListener("click",()=>ps(m.id)),h.querySelector(".close-details-btn")?.addEventListener("click",d=>{d.stopPropagation(),h.classList.remove("expanded"),h.querySelector(".event-details").style.display="none",le.delete(m.id)}),h},c=Array.from(e.children),o=new Map(c.map(m=>[m.dataset.itemId,m])),v=new Set(s.map(m=>m.id));if(c.forEach(m=>{let y=m.dataset.itemId;(!y||!v.has(y))&&m.remove()}),!Be||Be.length===0){e.innerHTML=R("empty","Your roadmap is empty.",H()?"Dexter is currently idle.":`Click "New Idea" to start planning Dexter's future.`);return}t&&(e.innerHTML="");let r=null;s.forEach((m,y)=>{let p=o.get(m.id);(!p||t)&&(p&&p.remove(),p=n(m),!p)||(y===0?e.firstElementChild!==p&&e.prepend(p):r&&r.nextElementSibling!==p&&r.after(p),r=p)})}catch{e.children.length===0&&(e.innerHTML=R("offline","Failed to load roadmap.","The event service may be offline."))}}}function ls(){let t=document.getElementById("roadmap-new"),e=document.getElementById("roadmap-save"),i=document.getElementById("roadmap-cancel"),s=document.getElementById("roadmap-expand-all"),n=document.getElementById("roadmap-close-all");t&&!t.dataset.listenerAttached&&(t.onclick=()=>{xe=null,document.getElementById("roadmap-editor-input").value="",document.getElementById("roadmap-editor-container").style.display="block"},t.dataset.listenerAttached="true"),i&&!i.dataset.listenerAttached&&(i.onclick=()=>{document.getElementById("roadmap-editor-container").style.display="none",xe=null},i.dataset.listenerAttached="true"),e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{let c=document.getElementById("roadmap-editor-input").value;if(!c.trim())return;let o=xe?`/roadmap/${xe}`:"/roadmap",v=xe?"PATCH":"POST";try{await B(o,{method:v,headers:{"Content-Type":"application/json"},body:JSON.stringify({content:c})}),document.getElementById("roadmap-editor-container").style.display="none",ce(!0)}catch(r){console.error(r)}},e.dataset.listenerAttached="true"),s&&!s.dataset.listenerAttached&&(s.onclick=()=>{Be.forEach(c=>le.add(c.id)),ce(!0)},s.dataset.listenerAttached="true"),n&&!n.dataset.listenerAttached&&(n.onclick=()=>{le.clear(),ce(!0)},n.dataset.listenerAttached="true")}function cs(t){xe=t.id,document.getElementById("roadmap-editor-input").value=t.content,document.getElementById("roadmap-editor-container").style.display="block",document.getElementById("roadmap-editor-container").scrollIntoView({behavior:"smooth"})}async function ds(t){let e=t.state==="published"?"draft":"published";try{await B(`/roadmap/${t.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({state:e})}),ce(!0)}catch(i){console.error(i)}}async function ps(t){if(confirm("Delete this roadmap item?"))try{await B(`/roadmap/${t}`,{method:"DELETE"}),le.delete(t),ce(!0)}catch(e){console.error(e)}}var bt=()=>`
    <div class="alerts-actions" style="margin: 0; padding: 0; background: none; border: none; box-shadow: none; display: flex; gap: 10px;">
...
`,gt=null,de=new Set,xt=[];async function pe(t=!1){let e=document.getElementById("blueprints-list");if(!e)return;ms();let i="/events?ml=1000&format=json&event.type=system.blueprint.generated";try{let s=await B(i);if(!s.ok)throw new Error("Service is offline or unreachable.");let c=(await s.json()).events||[];if(xt=c,gt=Date.now(),X(2,gt),c.length===0){e.innerHTML=R("empty","No architectural blueprints generated yet.","The Guardian will generate these when idle."),we(2,0);return}t&&(e.innerHTML="");let o=p=>{let b=p.event;if(typeof b=="string")try{b=JSON.parse(b)}catch{return null}let a=(b.title||"Untitled Blueprint").trim(),g=(b.summary||b.body||"No summary provided.").trim(),$=(b.content||"").trim(),h=(b.category||"architecture").trim(),u=(b.related_services||b.affected_services||[]).map(D=>D.trim()),l=(b.implementation_path||[]).map(D=>D.trim()),d=b.source_event_ids||[],f=b.approved===!0,x=new Date(p.timestamp*1e3),L=x.toLocaleTimeString(navigator.language,{hour:"2-digit",minute:"2-digit",second:"2-digit"}),I=x.toLocaleDateString(navigator.language,{month:"short",day:"numeric"}),k=de.has(p.id),T=k?"display: block;":"display: none;",S=document.createElement("div");S.className=`event-item notification-item event-border-purple cursor-pointer ${k?"expanded":""} ${f?"blueprint-approved":""}`,S.dataset.blueprintId=p.id,f&&(S.style.boxShadow="0 0 20px rgba(3, 218, 198, 0.15)",S.style.background="linear-gradient(135deg, rgba(3, 218, 198, 0.05) 0%, rgba(187, 134, 252, 0.05) 100%)");let j=f?"bx-check-shield":{architecture:"bx-vector",optimization:"bx-trending-up",feature:"bx-extension",security:"bx-shield-lock"}[h]||"bx-paint";S.onclick=function(D){if(this.classList.contains("expanded")){this.classList.remove("expanded"),de.delete(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="none")}else{this.classList.add("expanded"),de.add(p.id);let J=this.querySelector(".event-details");J&&(J.style.display="block")}};let P="";l.length>0&&(P=`
                    <div class="blueprint-path" style="margin-top: 15px;">
                        ...
                        <div class="detail-pre"><ul style="margin: 0; padding-left: 20px;">${l.map(D=>`<li style="margin-bottom: 5px;">${U(D)}</li>`).join("")}</ul></div>
                    </div>
                `);let A="";d.length>0&&(A=`
...
`;async function Oe(){await Promise.all([ce(),pe()])}var wt=()=>`
                ...
                <div class="service-widget" style="border-left: 3px solid ${p}; width: 100%;">
                    ...
                    <div class="service-widget-body">
                        ...
                        <div class="service-widget-info" style="grid-column: span 2; margin-top: 5px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 5px;">
                            ...
                            <span class="info-value" style="font-size: 0.8em; opacity: 0.8; word-break: break-all;">${r.execution_plan.entry_url||"Auto-detected"}</span>
                        </div>
                    </div>
                </div>
            `}).join("");t.innerHTML=v,t.querySelectorAll(".delete-chore-btn").forEach(r=>{r.onclick=async m=>{m.stopPropagation(),confirm("Delete this chore?")&&(await B(`/chores/${r.dataset.id}`,{method:"DELETE"}),ye())}})}catch(c){console.error("Failed to fetch chores",c)}}var us=`
    .profile-overlay {
        position: fixed;
        top: 0;
        left: 0;
...
`;function Ct(t,e){return{...t==="313071000877137920"?{techLevel:11,commStyle:"Direct / Efficient",patience:"Infinite",vibe:"Architect",facts:[{k:"Role",v:"Creator"},{k:"Lang",v:"Go"},{k:"OS",v:"Linux"},{k:"Editor",v:"Neovim"}],badges:["Creator","Admin","Architect"],stats:{msgs:14052,tokens:"45.2M",lastSeen:"Now"},topics:[{name:"System Architecture",val:85},{name:"Code Review",val:60},{name:"Music / Vibes",val:30},{name:"Debugging",val:45}],traits:{openness:95,conscientiousness:90,extraversion:40,agreeableness:60,neuroticism:15},dossier:{identity:{fullName:"Owen Easter",ageRange:"25-30",gender:"Male",location:"United Kingdom",sexuality:"Heterosexual",relationship:"Single"},career:{jobTitle:"Founder / Systems Architect",company:"Easter Company",skills:["Go","Distributed Systems","AI Integration"]},personal:{hobbies:["Synthwave Production","Coding","Gaming"],habits:["Late Night Coding","Coffee Consumption","System Optimization"],vices:["Perfectionism"],virtues:["Efficiency","Vision"]},social:[{name:"Dexter",relation:"Creation / AI",trust:"100%"},{name:"Sarah",relation:"Close Friend",trust:"95%"},{name:"Mike",relation:"Developer Peer",trust:"88%"}]}}:{techLevel:[2,4,6,8,3,5][Math.floor(Math.random()*6)],commStyle:["Verbose","Casual","Formal","Chaotic","Inquisitive"][Math.floor(Math.random()*5)],patience:Math.random()>.5?"High":"Medium",vibe:["NPC","Guest","Lurker","Regular","Fan"][Math.floor(Math.random()*5)],facts:[{k:"Role",v:"User"},{k:"Interest",v:Math.random()>.5?"Coding":"Gaming"}],badges:["User"],stats:{msgs:Math.floor(Math.random()*500),tokens:Math.floor(Math.random()*100)+"K",lastSeen:Math.floor(Math.random()*24)+"h ago"},topics:[{name:"General Chat",val:80},{name:"Troubleshooting",val:40},{name:"Off-Topic",val:20}],traits:{openness:Math.floor(Math.random()*100),conscientiousness:Math.floor(Math.random()*100),extraversion:Math.floor(Math.random()*100),agreeableness:Math.floor(Math.random()*100),neuroticism:Math.floor(Math.random()*100)},dossier:{identity:{fullName:"Unknown Subject",ageRange:"Unknown",gender:"Unknown",location:"Global",sexuality:"Unknown",relationship:"Unknown"},career:{jobTitle:"Unknown",company:"Unknown",skills:["General User"]},personal:{hobbies:["Lurking","Chatting"],habits:["Unknown"],vices:["None Observed"],virtues:["None Observed"]},social:[]}},id:t,username:e}}function $t(t){if(!document.getElementById("dex-profile-styles")){let i=document.createElement("style");i.id="dex-profile-styles",i.textContent=us,document.head.appendChild(i)}let e=document.createElement("div");e.className="profile-overlay",e.innerHTML=`
    ...
    `,e.addEventListener("click",i=>{i.target===e&&(e.classList.remove("active"),document.body.style.overflow="",setTimeout(()=>e.remove(),300))}),document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),document.body.style.overflow="hidden",Me(`/profile/${t.id}`).then(async i=>{if(i.ok){let s=await i.json();Ue(e,t,s)}else{console.log("Profile not found or error, using mock data.");let s=Ct(t.id,t.username);Ue(e,t,s)}}).catch(i=>{console.error("Profile fetch error:",i);let s=Ct(t.id,t.username);Ue(e,t,s)})}function Ue(t,e,i){let s=e.status==="online"?"#03dac6":e.status==="idle"?"#ffb74d":"#cf6679",n=()=>{let $=(i.badges||[]).map(L=>`<span class="profile-badge ${L==="Creator"?"master":""}">${L}</span>`).join(""),h=(i.attributes||i.facts||[]).map(L=>{let I=L.key||L.k,k=L.value||L.v;return`
                ...
                <div class="profile-section">
                    ...
                    <div class="cog-metric">
                        ...
                        <div class="cog-bar-bg">
                            <div class="cog-bar-fill" style="width: ${Math.min(l*10,100)}%;"></div>
                        </div>
                    </div>
        ...
        `},c=()=>{let $=i.dossier||{},h=$.identity||{},u=$.career||{},l=$.personal||{},d=$.social||[];return`
    ...
    `,m=`
        <div class="profile-card">
            ...
            <div class="profile-body">
                ...
                <div id="tab-raw" class="tab-content">${r()}</div>
            </div>
...
`,fs=null;async function je(){let t=document.getElementById("contacts-list");if(!t)return;let e=document.getElementById("contacts-refresh");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await je(),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),t.dataset.listenerAttached||(t.onclick=i=>{let s=i.target.closest(".user-profile-card");if(s)try{let n=JSON.parse(s.dataset.user);$t(n)}catch(n){console.error("Failed to parse user data",n)}},t.dataset.listenerAttached="true");try{let i=await Me("/contacts");if(!i.ok)throw new Error("Service unreachable");let n=(await i.json()).members||[];if(fs=Date.now(),n.length===0){t.innerHTML=R("empty","No contacts found.","Check your Discord connection.");return}let c={Me:0,Master:1,Admin:2,Moderator:3,Contributor:4,User:5};n.sort((o,v)=>{let r=c[o.level]??10,m=c[v.level]??10;return r!==m?r-m:o.username.localeCompare(v.username)}),t.innerHTML=n.map(o=>{let v=o.color?"#"+o.color.toString(16).padStart(6,"0"):"rgba(255,255,255,0.1)",r=o.status==="online"?"#5eff5e":o.status==="idle"?"#ffa500":o.status==="dnd"?"#ff4d4d":"#666",m="#888";o.level==="Me"||o.level==="Master"?m="#bb86fc":o.level==="Admin"?m="#cf6679":o.level==="Moderator"?m="#03dac6":o.level==="Contributor"&&(m="#ffa500");let y=o.level==="Me",p=y?"rgba(187, 134, 252, 0.08)":"rgba(255,255,255,0.03)",b=y?"2px solid #bb86fc":`1px solid ${v}33`;return`
                <div class="user-profile-card" 
                    ...
                    <div style="position: relative;">
                        ...
                        <div style="position: absolute; bottom: 0; right: 0; width: 12px; height: 12px; background: ${r}; border: 2px solid #1a1a1a; border-radius: 50%;"></div>
                    </div>
...
`);return`
                <div class="log-report">
                    <div class="log-report-header">
                        ...
                        <div style="display: flex; gap: 5px;">
                            ...
                            <button class="log-action-btn delete clear-logs-btn" data-service-id="${o.id}" title="Clear Logs" style="${H()?"display: none;":""}">
            ...
            `}).join("");return t.innerHTML=c,document.querySelectorAll(".copy-logs-btn").forEach(o=>{o.addEventListener("click",()=>{let v=unescape(o.dataset.logs);navigator.clipboard.writeText(v).then(()=>{let r=o.querySelector("i");r.classList.remove("bx-copy"),r.classList.add("bx-check"),setTimeout(()=>{r.classList.remove("bx-check"),r.classList.add("bx-copy")},2e3)})})}),document.querySelectorAll(".clear-logs-btn").forEach(o=>{o.addEventListener("click",async()=>{let v=o.dataset.serviceId;if(confirm(`Are you sure you want to clear logs for ${v}?`))try{(await B(`/logs?service_id=${v}`,{method:"DELETE"})).ok&&$e()}catch(r){console.error("Error clearing logs:",r)}})}),Cs=Date.now(),!0}catch(e){return console.error("Error fetching logs:",e),t.children.length===0&&(t.innerHTML=R("offline","Failed to load logs.","The event service may be offline."),t.classList.add("placeholder-active")),!1}}var It=()=>{let t=H()?"display: none;":"";return`
        <div class="system-section-header">
        ...
        <div id="logs-container-wrapper" style="margin-bottom: 20px;">
            ${Tt()}
        </div>`;async function Ht(){await Promise.all([Se(),Je(),We(),ye()]),await $e()}var Lt=null,Mt=null;async function Pt(){try{return await(await B("/system_monitor")).json()}catch{return null}}async function At(){try{return await(await B("/system/hardware")).json()}catch{return null}}async function $s(){try{return await(await B("/processes")).json()}catch{return null}}async function Ss(){try{return await(await B("/agent/status")).json()}catch{return null}}async function Je(){let t=document.getElementById("services-widgets"),e=document.getElementById("hardware-refresh-btn"),i=document.querySelector("#hw-os .hw-content"),s=document.querySelector("#hw-cpu .hw-content"),n=document.querySelector("#hw-ram .hw-content"),c=document.querySelector("#hw-gpu .hw-content"),o=document.querySelector("#hw-storage .hw-content"),v=l=>{if(l){if(i&&(i.innerHTML=`
                ...
                </div>`),n){let d=(l.MEMORY_BYTES/1073741824).toFixed(1);n.innerHTML=`
                ...
                </div>`}if(s&&l.CPU&&l.CPU.length>0){let d=l.CPU[0];s.innerHTML=`
                <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px;">
                ...
                </div>`}c&&(l.GPU&&l.GPU.length>0?c.innerHTML=l.GPU.map(d=>{let f=(d.VRAM/1073741824).toFixed(1);return`
                    ...
                    </div>`}).join(""):o&&(o.innerHTML='<p style="text-align: center; color: #666; margin: 0;">No storage devices found</p>')}};if(e&&(e.dataset.listenerAttached||(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>";let l=await At();l?(v(l),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)):(e.innerHTML="<i class='bx bx-error'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3))},e.dataset.listenerAttached="true"),s&&!s.hasChildNodes())){let l=await At();v(l)}if(!t)return;let r=await Pt();if(!r||!r.services){t.children.length===0&&(t.innerHTML=R("offline","Failed to load system metrics.","The event service may be offline."));return}Lt=Date.now(),X(0,Lt);let m=r.services||[];Array.from(t.children).forEach(l=>{l.classList.contains("service-widget")||l.remove()});function y(l){return!l||l==="N/A"||l==="unknown"||l.trim()===""?"-":l}function p(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/^(\d+\.\d+\.\d+)/);return d?d[0]:l.split(".").slice(0,3).join(".")||"-"}function b(l){return!l||l.length<=28?l:l.substring(0,28)+"..."}function a(l){if(!l||l==="N/A"||l==="unknown")return"-";let d=l.match(/(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:([\d.]+)s)?/);if(!d)return"-";let f=parseInt(d[1])||0,x=parseInt(d[2])||0,L=parseInt(d[3])||0,I=parseFloat(d[4])||0,k=f*86400+x*3600+L*60+I,T=Math.floor(k/86400);if(T>0)return`${T}d`;let S=Math.floor(k/3600);if(S>0)return`${S}h`;let _=Math.floor(k/60);return _>0?`${_}m`:`${Math.floor(k)}s`}function g(l){let d=l.status==="online",f=d?"service-widget-online":"service-widget-offline",x=d?"bx-check-circle":"bx-x-circle",L=d?"OK":"BAD",I=l.version?p(l.version.str):"-",k=l.cpu&&l.cpu!=="N/A"?l.cpu:"-",T=l.memory&&l.memory!=="N/A"?l.memory:"-",S=k!=="-"?"#00ff00":"#666",_=k!=="-"?"#fff":"#666",j=T!=="-"?"#00ff00":"#666",P=T!=="-"?"#fff":"#666",A=a(l.uptime),F="";d?F=`
                <div class="service-widget-info">
                ...
                </div>`:F='<div class="service-widget-footer offline"><span>OFFLINE</span></div>';let z=H()?"easter.company":b(l.domain&&l.port?`${l.domain}:${l.port}`:"");return`<div class="service-widget ${f}" data-service-id="${l.id}"><div class="service-widget-header"><i class="bx ${x}"></i><h3>${l.short_name||l.id}</h3><span class="service-widget-status">${L}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Address:</span><span class="info-value">${z}</span></div>${F}</div></div>`}let $=new Map(Array.from(t.querySelectorAll(".service-widget")).map(l=>[l.dataset.serviceId,l])),h=new Set(m.map(l=>l.id));for(let[l,d]of $)h.has(l)||d.remove();let u=new Set;m.forEach(l=>{if(u.has(l.id))return;u.add(l.id);let d=g(l),f=$.get(l.id);f&&f.parentNode?f.outerHTML!==d&&(f.outerHTML=d):t.insertAdjacentHTML("beforeend",d)})}async function We(){let t=document.getElementById("models-widgets");if(!t)return;let e=await Pt();if(!e){t.children.length===0&&(t.innerHTML=R("offline","Failed to load model status.","The event service may be offline."));return}Mt=Date.now(),X(2,Mt);let i=e.models||[],s=e.whisper;Array.from(t.children).forEach(r=>{r.classList.contains("service-widget")||r.remove()});function n(r){let m=r.status==="Ready";return`
                ...
                </div>`}function c(r){let m=r.status==="Ready";return`
                ...
                </div>`}function o(r){let m=r.status==="Downloaded",y=m?"service-widget-online":"service-widget-offline",p=m?"OK":"MISSING",b=m&&r.size>0?`${(r.size/1e9).toFixed(2)} GB`:"-",a=r.name;return r.type==="custom"&&a.endsWith(":latest")&&(a=a.replace(":latest","")),`<div class="service-widget ${y}" data-model-name="${r.name}"><div class="service-widget-header"><i class="bx ${m?"bx-check-circle":"bx-x-circle"}"></i><h3>${a}</h3><span class="service-widget-status">${p}</span></div><div class="service-widget-body"><div class="service-widget-info"><span class="info-label">Type:</span><span class="info-value">${r.type}</span></div><div class="service-widget-info"><span class="info-label">Size:</span><span class="info-value">${b}</span></div></div></div>`}let v="";if(s&&(v+=n(s)),e.xtts&&(v+=c(e.xtts)),v+=i.map(o).join(""),!v){t.innerHTML=R("empty","No models found.");return}t.innerHTML!==v&&(t.innerHTML=v)}async function Se(){let t=document.getElementById("processes-widgets");if(!t)return;let e=document.getElementById("guardian-sentry-val"),i=document.getElementById("guardian-architect-val"),s=document.getElementById("guardian-idle-val"),n=document.getElementById("guardian-total-idle"),c=document.getElementById("guardian-total-active"),o=document.getElementById("guardian-total-waste"),v=document.getElementById("guardian-reset-btn"),r=document.getElementById("analyzer-reset-btn");v&&!v.dataset.listenerAttached&&(v.onclick=async()=>{v.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/guardian/reset?protocol=all",{method:"POST"}),setTimeout(()=>{v.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{v.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{v.innerHTML="<i class='bx bx-error'></i>"}},v.dataset.listenerAttached="true"),r&&!r.dataset.listenerAttached&&(r.onclick=async()=>{r.innerHTML="<i class='bx bx-loader-alt spin'></i>";try{await B("/analyzer/reset?protocol=synthesis",{method:"POST"}),setTimeout(()=>{r.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{r.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},500),Se()}catch{r.innerHTML="<i class='bx bx-error'></i>"}},r.dataset.listenerAttached="true");let m=await Ss();if(m&&m.agents){let h=m.agents.guardian||{protocols:{}},u=m.agents.analyzer||{protocols:{}},l=m.agents.imaginator||{protocols:{}},d=m.system||{},f=Math.floor(Date.now()/1e3),x={sentry:"Sentry",architect:"Architect",synthesis:"Synthesis",alert_review:"Alert Review"},L=A=>{A<0&&(A=0);let F=Math.floor(A/3600),z=Math.floor(A%3600/60),E=A%60;return F>0?`${F}h ${z}m`:z>0?`${z}m ${E}s`:`${E}s`},I=(A,F,z,E)=>{if(!A)return;let C=x[E]||E.toUpperCase(),w=A.parentElement.querySelector('span[style*="text-transform: uppercase"]');if(w&&(w.textContent=C),!z){A.textContent="Inactive",A.style.color="#666";return}let M=z.status;if(M==="Working")A.textContent="Working",A.style.color="#bb86fc";else if(M==="Ready")A.textContent="Ready",A.style.color="#5eff5e";else{let N=z.cooldown;if(H()&&(N=z.next_run-f),N<=0)A.textContent="Ready",A.style.color="#5eff5e";else{let D=Math.floor(N/60),q=N%60;A.textContent=`${D}m ${q}s`,A.style.color="#fff"}}F&&z.stats&&(F.innerHTML=`
          ...
          `)};e&&I(e,document.getElementById("guardian-sentry-stats"),h.protocols.sentry,"sentry"),i&&I(i,document.getElementById("guardian-architect-stats"),h.protocols.architect,"architect");let k=document.getElementById("analyzer-synthesis-val"),T=document.getElementById("analyzer-synthesis-stats");k&&I(k,T,u.protocols.synthesis,"synthesis");let S=document.getElementById("imaginator-alert_review-val"),_=document.getElementById("imaginator-alert_review-stats");S&&I(S,_,l.protocols.alert_review,"alert_review");let j=document.getElementById("system-state-label"),P=document.getElementById("system-state-val");if(P&&d.state){let A=d.state,F=L(d.state_time||0);j&&(j.textContent=`State: ${A.toUpperCase()}`),P.textContent=F,A==="idle"?P.style.color=d.state_time>300?"#5eff5e":"#fff":P.style.color="#bb86fc"}n&&(n.textContent=L(d.metrics?.total_idle_time||0)),c&&(c.textContent=L(d.metrics?.total_active_time||0)),o&&(o.textContent=L(d.metrics?.total_waste_time||0))}else[e,i,s,n,c,o].forEach(u=>{u&&(u.textContent="-",u.style.color="#666")});let y=await $s(),p=[],b=[],a=[];y&&(Array.isArray(y)?p=y:(p=y.active||[],b=y.queue||[],a=y.history||[],a.sort((h,u)=>(u.end_time||0)-(h.end_time||0)))),p.length===0?t.querySelector(".tab-placeholder")||(t.innerHTML=R("empty","No active processes.")):((t.querySelector(".tab-placeholder")||t.querySelector("p"))&&(t.innerHTML=""),Fe(t,p,!1));let g=document.getElementById("processes-queue-widgets");g&&(b.length===0?!g.querySelector(".tab-placeholder")&&!g.querySelector('div[style*="font-style: italic"]')&&(g.innerHTML='<div style="width: 100%; text-align: center; padding: 20px; color: #666; font-style: italic; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.05);">Queue is empty</div>'):(g.innerHTML="",Fe(g,b,!1)));let $=document.getElementById("processes-history-widgets");$&&(a.length===0?$.querySelector(".tab-placeholder")||($.innerHTML=R("empty","No recent history.")):($.querySelector(".tab-placeholder")&&($.innerHTML=""),Fe($,a,!0))),we(1,p.length)}function Fe(t,e,i){function s(r){let m="";r.end_time?m=`${r.end_time-r.start_time}s`:m=`${Math.floor(Date.now()/1e3-r.start_time)}s`;let y=r.retries>0?`<span class="process-retry-badge">Retry ${r.retries}</span>`:"",p=r.channel_id,b={"system-guardian":"Guardian Agent","system-cli-op":"CLI Operation","system-analyzer":"Analyzer Agent"};if(b[p]?p=b[p]:/^\d+$/.test(p)&&(p=`Channel ${p}`),i)return`
        <div class="process-history-item" data-channel-id="${r.channel_id}-${r.start_time}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 15px; background: rgba(255,255,255,0.03); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05); font-family: 'JetBrains Mono', monospace; font-size: 0.85em;">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1;">
                <i class='bx bx-check-circle' style="color: #03dac6; font-size: 1.1em;"></i>
                <span style="color: #fff; font-weight: 600;">${p}</span>
                <span style="color: #666; font-size: 0.9em; background: rgba(0,0,0,0.2); padding: 1px 6px; border-radius: 4px;">${r.state}</span>
            </div>
            <div style="display: flex; align-items: center; gap: 20px; color: #888;">
                ...
                <div class="service-widget process-widget" data-channel-id="${r.channel_id}-${r.start_time}" style="">
                    <div class="service-widget-header">
                    ...
                    <div class="service-widget-body">
                        <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${r.state}</span>
                        </div>
                        <div class="service-widget-info">
                         ...
                         <div class="service-widget-info">
                            ...
                            <span class="info-value" style="color: ${a}">${r.pid}</span>
                        </div>
                    </div>
                </div>`}let n=i?".process-history-item":".process-widget",c=new Map(Array.from(t.querySelectorAll(n)).map(r=>[r.dataset.channelId,r])),o=new Set(e.map(r=>`${r.channel_id}-${r.start_time}`));for(let[r,m]of c)o.has(r)||m.remove();let v=new Set;e.forEach(r=>{let m=`${r.channel_id}-${r.start_time}`;if(v.has(m))return;v.add(m);let y=s(r),p=c.get(m);if(p&&p.parentNode){p.outerHTML!==y&&(p.outerHTML=y);let b=t.querySelector(`[data-channel-id="${m}"]`);b&&t.appendChild(b)}else t.insertAdjacentHTML("beforeend",y)})}function De(){let t=He(),e="Notification"in window,i={enabled:e&&Notification.permission==="granted",supported:e};return`
    ...
    `;async function Ut(){let t=document.getElementById("web-history-content");if(!t)return;let e=document.getElementById("web-refresh-btn");e&&!e.dataset.listenerAttached&&(e.onclick=async()=>{e.innerHTML="<i class='bx bx-loader-alt spin'></i>",await Rt(t),e.innerHTML="<i class='bx bx-check'></i>",setTimeout(()=>{e.innerHTML="<i class='bx bx-refresh'></i>"},2e3)},e.dataset.listenerAttached="true"),await Rt(t)}async function Rt(t){try{let e=await B("/web/history");if(!e.ok)throw new Error("Failed to fetch history");let i=await e.json();if(!i||i.length===0){t.innerHTML=R("empty","No web history found.");return}let s=`
        ...
        `;i.forEach((v,r)=>{let m=new Date(v.timestamp*1e3).toLocaleString([],{dateStyle:"short",timeStyle:"short"}),y=v.screenshot?`<img src="data:image/png;base64,${v.screenshot}" style="width: 100%; max-height: 45%; object-fit: contain; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 15px; flex-shrink: 0; background: #000;">`:'<div style="width: 100%; height: 80px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #666; margin-bottom: 15px; border-radius: 8px; flex-shrink: 0;">No Screenshot</div>';s+=`
                <div class="web-card" style="flex: 0 0 100%; height: 100%; display: flex; flex-direction: column; scroll-snap-align: center; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-sizing: border-box; max-width: 100%; overflow: hidden;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; flex-shrink: 0;">
                        <span style="color: #03dac6; font-weight: bold; font-family: monospace;">#${r+1}</span>
...
dex config reset`},demo_output:["{",'  "id": "event-service",','  "short_name": "event",','  "type": "be",','  "port": 8100,','  "domain": "127.0.0.1",','  "log_path": "~/Dexter/logs/event-service.log"',"}"]}],Es=()=>{let t={cognitive:{title:"Cognitive Core",icon:"bx-brain",color:"#bb86fc"},lifecycle:{title:"Development Lifecycle",icon:"bx-git-branch",color:"#03dac6"},system:{title:"System Management",icon:"bx-cog",color:"#aaa"}},e=`
    ...
    </div>`,e},Ve=!1;function ks(t){let e=document.getElementById("cli-terminal-overlay");e||(e=document.createElement("div"),e.id="cli-terminal-overlay",e.className="cli-execution-overlay",e.innerHTML=`
    ...
    `,document.getElementById("terminal-cmd-name").textContent=`dex ${t.id}`,document.getElementById("terminal-status-badge").className="terminal-status status-running",document.getElementById("terminal-status-badge").textContent="Running",document.getElementById("terminal-action-btn").style.display="none",e.classList.add("active"),document.body.style.overflow="hidden",Ve=!0,i}function jt(){let t=document.getElementById("cli-terminal-overlay");t&&t.classList.remove("active"),document.body.style.overflow="",Ve=!1}function qt(t,e,i="output"){if(!Ve)return;let s=document.createElement("div");s.className=`terminal-line terminal-${i}`,i==="prompt"?s.innerHTML=`<span class="terminal-prompt">$</span> ${e}`:s.innerHTML=Le(e),t.appendChild(s),t.scrollTop=t.scrollHeight}async function Ts(t){let e=Ft.find(n=>n.id===t);if(!e)return;let i=ks(e);qt(i,`dex ${t}`,"prompt");let s=e.demo_output||["Executing command...","\u2713 Done."];for(let n of s){await new Promise(o=>setTimeout(o,400+Math.random()*600));let c="output";n.includes("[ERROR]")?c="error":n.includes("[SUCCESS]")||n.includes("\u2713")?c="success":n.includes("!")&&(c="warning"),qt(i,n,c)}document.getElementById("terminal-status-badge").className="terminal-status status-success",document.getElementById("terminal-status-badge").textContent="Completed (Demo)",document.getElementById("terminal-action-btn").style.display="block"}function Jt(){let t=document.getElementById("cli-interface-container");if(!t)return;t.innerHTML=Es();let e=document.getElementById("install-command-copy");e&&e.addEventListener("click",()=>{let i=e.querySelector("code").textContent;navigator.clipboard.writeText(i).then(()=>{let s=e.querySelector("i");s.className="bx bx-check",s.style.color="#5eff5e",setTimeout(()=>{s.className="bx bx-copy",s.style.color="#bb86fc"},2e3)})}),t.querySelectorAll(".cli-command-card").forEach(i=>{i.addEventListener("mouseenter",()=>{i.style.transform="translateY(-5px)",i.style.borderColor="rgba(255,255,255,0.15)",i.style.backgroundColor="rgba(255,255,255,0.05)"}),i.addEventListener("mouseleave",()=>{i.style.transform="translateY(0)",i.style.borderColor="rgba(255,255,255,0.05)",i.style.backgroundColor="rgba(255,255,255,0.03)"}),i.addEventListener("click",()=>{let s=i.dataset.cmd;Ts(s)})})}async function Ls(){if(!H())try{if(!(await B("/system/status",{method:"GET"})).ok)throw new Error("Service unhealthy")}catch(t){console.error("Service health check failed:",t)}}function Wt(){if(window.location.protocol==="http:"&&window.location.hostname==="easter.company"){window.location.replace("https://"+window.location.hostname+window.location.pathname+window.location.search);return}Qe(),et(),Ls(),at(),Xe();let t=window.location.pathname;(t==="/dexter"||t==="/dexter/")&&Jt();let e=[],i=document.getElementById("windows-container");i&&i.setAttribute("data-count","0");let s=u=>{localStorage.setItem("dex_last_window",u)};function n(){for(;e.length>1;)e.shift().close(!0);let u=document.getElementById("windows-container"),l=document.querySelector("nav"),d=document.querySelector("footer"),f=window.location.pathname==="/"||window.location.pathname==="/index.html",x=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");u&&u.setAttribute("data-count",e.length),e.forEach(_=>{let j=document.getElementById(_.id);j&&j.classList.remove("hide-close")});let L=document.getElementById("dexter-menu-container"),I=document.getElementById("nav-window-switcher"),k=document.getElementById("settings-icon"),T=document.getElementById("close-all-windows"),S=window.innerWidth<880;if(!S){let _=document.getElementById("dexter-dropdown"),j=document.getElementById("dexter-menu-btn");_&&_.classList.remove("active"),j&&j.classList.remove("active"),u&&u.classList.remove("menu-open")}if(fe(e.length>0),e.length>0)if(d?.classList.add("hide"),T&&(T.style.display=S?"none":"block"),document.querySelector("main")?.style.setProperty("opacity","0","important"),document.body.style.overflow="hidden",l?.classList.add("window-open"),u&&(u.style.paddingTop="60px"),L&&(L.style.display=S?"flex":"none"),k&&(k.style.display=S?"block":"none"),!S&&I){let _=e[0].id;["alerts-window","events-window","monitor-window","contacts-window","workspace-window"].includes(_)?(I.innerHTML=`
                  ...
                  `,document.getElementById("switch-alerts").addEventListener("click",()=>{s("alerts-window"),c(v)}),document.getElementById("switch-events").addEventListener("click",()=>{s("events-window"),c(r)}),document.getElementById("switch-monitor").addEventListener("click",()=>{s("monitor-window"),c(y)}),document.getElementById("switch-contacts").addEventListener("click",()=>{s("contacts-window"),c(m)}),document.getElementById("switch-workspace").addEventListener("click",()=>{s("workspace-window"),c(p)})):I.innerHTML=""}else I&&(I.innerHTML="");else l?.classList.remove("window-open"),T&&(T.style.display="none"),u&&(u.style.paddingTop="100px"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.body.style.overflow="",f||x?d?.classList.remove("hide"):d?.classList.add("hide"),L&&(L.style.display="flex"),k&&(k.style.display="block"),I&&(I.innerHTML="");ve()}function c(u){if(u.isOpen()){u.close();return}for(;e.length>0;)e.pop().close(!0);e.push(u),u.open(),n()}function o(){[...e].forEach(l=>l.close()),e=[]}window.addEventListener("resize",n);let v=ae({id:"alerts-window",title:"Alerts",icon:"bx-bell",content:mt(),onOpen:()=>ie(),onClose:()=>{let u=e.indexOf(v);u>-1&&e.splice(u,1),n()}}),r=ae({id:"events-window",title:"Events",icon:"bx-calendar-event",content:qe(),onOpen:()=>{r.setContent(qe()),ne()},onClose:()=>{let u=e.indexOf(r);u>-1&&e.splice(u,1),n()}}),m=ae({id:"contacts-window",title:"Contacts",icon:"bx-book-content",content:St(),onOpen:()=>je(),onClose:()=>{let u=e.indexOf(m);u>-1&&e.splice(u,1),n()}}),y=ae({id:"monitor-window",icon:"bx-pulse",tabs:[{icon:"bxs-server",title:"Services",content:_t()},{icon:"bxs-component",title:"Processes",content:Bt()},{icon:"bxs-brain",title:"Models",content:Dt()},{icon:"bx-globe",title:"Web",content:Ot()},{icon:"bxs-hdd",title:"Hardware",content:zt()},{icon:"bxs-terminal",title:"Logs",content:Nt()},{icon:"bxs-zap",title:"Agents",content:It()},{icon:"bx-task",title:"Chores",content:wt()}].filter(u=>H()?u.title!=="Hardware"&&u.title!=="Logs":!0),onOpen:()=>{Je(),Se(),We(),Ut(),$e(),ye()},onClose:()=>{let u=e.indexOf(y);u>-1&&e.splice(u,1),n()}}),p=ae({id:"workspace-window",title:"Workspace",icon:"bx-brain",content:ht(),onOpen:()=>Oe(),onClose:()=>{let u=e.indexOf(p);u>-1&&e.splice(u,1),n()}}),b=ae({id:"settings-window",title:"Settings",icon:"bx-cog",content:De(),onOpen:()=>{b.setContent(De()),Ge(b)},onClose:()=>{let u=e.indexOf(b);u>-1&&e.splice(u,1),n()}});window.dexter={viewEvent:u=>{r.isOpen()||c(r),setTimeout(()=>{let l=document.querySelector(`.event-item[data-event-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)},viewAlert:u=>{v.isOpen()||c(v),setTimeout(()=>{let l=document.querySelector(`.event-item[data-alert-id="${u}"]`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.classList.add("flash-highlight"),l.classList.contains("expanded")||l.click(),setTimeout(()=>l.classList.remove("flash-highlight"),2e3))},500)}};let a=()=>{let u=document.getElementById("dexter-dropdown"),l=document.getElementById("dexter-menu-btn");u&&l&&(u.classList.remove("active"),l.classList.remove("active"),window.innerWidth<880&&document.getElementById("windows-container")?.classList.remove("menu-open"))};(()=>{let u=document.getElementById("dexter-menu-btn"),l=document.getElementById("settings-icon"),d=document.getElementById("close-all-windows"),f=document.getElementById("nav-left-container"),x=document.getElementById("dexter-dropdown"),L=document.getElementById("windows-container");u&&x&&(u.onclick=k=>{k.stopPropagation();let T=window.innerWidth<880;if(T)x.style.top="",x.style.left="",x.style.right="",x.style.transform="";else{let _=u.getBoundingClientRect(),P=document.querySelector("nav").getBoundingClientRect(),A=220,F=10,E=_.left+_.width/2-A/2,C=P.right-10;E+A>C&&(E=C-A),E<10&&(E=10),x.style.top=P.bottom+F+"px",x.style.left=E+"px",x.style.right="auto",x.style.transform="none"}x.classList.toggle("active"),u.classList.toggle("active");let S=x.classList.contains("active");if(T){let _=document.querySelector("nav");S?(document.querySelector("footer")?.classList.add("hide"),document.querySelector("main")?.style.setProperty("opacity","0","important"),_?.classList.add("window-open"),L?.classList.add("menu-open"),fe(!0)):(L?.classList.remove("menu-open"),e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),_?.classList.remove("window-open"),fe(!1)))}}),l&&(l.onclick=k=>{k.stopPropagation(),c(b)}),d&&(d.onclick=k=>{k.stopPropagation(),o()});let I=(k,T,S)=>{let _=document.getElementById(k);_&&(_.onclick=j=>{j.stopPropagation(),a(),S&&s(S),c(T)})};I("alerts-menu-item",v,"alerts-window"),I("events-menu-item",r,"events-window"),I("monitor-menu-item",y,"monitor-window"),I("contacts-menu-item",m,"contacts-window"),I("workspace-menu-item",p,"workspace-window"),f&&(f.onclick=k=>{if(k.stopPropagation(),x&&x.classList.contains("active")){a(),window.innerWidth<880&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1));return}if(e.length>0)o();else{let T=window.location.pathname;if(!(T==="/"||T==="/index.html")){let j=(T.endsWith("/")&&T.length>1?T.slice(0,-1):T).split("/");j.pop();let P=j.join("/")||"/";window.location.href=P}}}),document.addEventListener("click",()=>{let k=window.innerWidth<880;x&&x.classList.contains("active")&&(a(),k&&e.length===0&&(document.querySelector("footer")?.classList.remove("hide"),document.querySelector("main")?.style.setProperty("opacity","1","important"),document.querySelector("nav")?.classList.remove("window-open"),fe(!1)))}),x&&(x.onclick=k=>k.stopPropagation())})();let $=window.location.pathname==="/"||window.location.pathname==="/index.html",h=window.location.pathname.includes("404")||!!document.getElementById("error-main-view");!$&&!h&&document.querySelector("footer")?.classList.add("hide"),H()||setInterval(()=>{r.isOpen()&&ne(),v.isOpen()?ie():Re()},1e3),setInterval(()=>{H()&&(v.isOpen()?ie():Re()),p.isOpen()?Oe():yt(),H()&&r.isOpen()&&ne(),y.isOpen()&&Ht()},5e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Wt):Wt();})();
